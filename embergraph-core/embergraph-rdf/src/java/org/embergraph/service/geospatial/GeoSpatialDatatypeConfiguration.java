...
public class GeoSpatialDatatypeConfiguration implements Serializable {
  ...
  public GeoSpatialDatatypeConfiguration(
    ...
    if (uriStr == null || uriStr.isEmpty())
      throw new InvalidGeoSpatialDatatypeConfigurationError(
          "URI parameter must not be null or empty");

    try {
      this.uri = new URIImpl(uriStr);
    } catch (Exception e) {
      throw new InvalidGeoSpatialDatatypeConfigurationError(
          "Invalid URI in geospatial datatype config: " + uriStr);
    }
    ...
    if (fields.isEmpty()) {
      throw new InvalidGeoSpatialDatatypeConfigurationError(
          "Geospatial datatype config for datatype "
              + uri
              + " must have at least one field, but has none.");
    }
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      if (ServiceMapping.CUSTOM.equals(curServiceMapping)) {
        ...
        if (customServiceMappings.contains(customServiceMapping)) {
          throw new InvalidGeoSpatialDatatypeConfigurationError(
              "Duplicate custom service mapping used for geospatial datatype config: "
  ...
  public GeoSpatialDatatypeConfiguration(
      ...
      final IGeoSpatialLiteralSerializer literalSerializer,
    ...
    for (int i = 0; i < getFields().size(); i++) {
      ...
      for (final String customField : customFields) {
        ...
        if (ServiceMapping.CUSTOM == field.getServiceMapping()
            && customField != null
            && customField.equals(field.getCustomServiceMapping())) {
