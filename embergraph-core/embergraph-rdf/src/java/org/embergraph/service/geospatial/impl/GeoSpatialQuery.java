 ...
 */
package org.embergraph.service.geospatial.impl;
...
public class GeoSpatialQuery implements IGeoSpatialQuery {
  ...
  private GeoSpatialQuery(
    ...
    this(
        ...
        spatialCircleCenter,
        spatialCircleRadius,
        spatialRectangleSouthWest,
        spatialRectangleNorthEast,
        spatialUnit,
        timeStart,
        timeEnd,
        coordSystem,
   ...
   * @param customFieldsLowerBounds the lower bounds for the custom fields definition (needs to have
   *     same arity)
   * @param customFieldsUpperBounds the upper bounds for the custom fields definition (needs to have
   *     same arity)
   * @return
   */
  public static Map<String, LowerAndUpperValue> toValidatedCustomFieldsConstraints(
    ...
    if (customFields.length != customFieldsLowerBounds.length)
      throw new GeoSpatialSearchException(
          "Nr of custom fields = "
              + customFields.length
              + " differs from number of lower bounds = "
              + customFieldsLowerBounds.length);

    if (customFields.length != customFieldsUpperBounds.length)
      throw new GeoSpatialSearchException(
          "Nr of custom fields = "
              + customFields.length
              + " differs from number of upper bounds = "
              + customFieldsUpperBounds.length);
  ...
  public LowerAndUpperBound getLowerAndUpperBound() {
        ...
        case TIME:
          {
            if (getTimeStart() == null)
              throw new GeoSpatialSearchException(
                  "Start time not specified in query, but required.");
            if (getTimeEnd() == null)
        ...
        case COORD_SYSTEM:
          {
            if (getCoordSystem() == null) {
              throw new GeoSpatialSearchException(
                  "Coordinate system not specified in query, but required.");
            }
            ...
            if (!customFieldsConstraints.containsKey(customServiceMapping)) {
              throw new GeoSpatialSearchException(
                  "Custom field "
                      ...
                      + " not specified in query, but required.");
            }
    ...
    if (latIdx != -1 && lonIdx != -1) {
      ...
      throw new GeoSpatialSearchException(
          "Latitude and longitude must either both be given or not given.");
  ...
  public List<IGeoSpatialQuery> normalize() {
    ...
    if (lowerBoundingBox != null
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "Search rectangle upper left latitude ("
                + lowerBoundingBox.eastWest
                + ") is larger than rectangle lower righ latitude ("
                + upperBoundingBox.eastWest
      ...
      final GeoSpatialQuery query1 =
          new GeoSpatialQuery(
              ...
              incomingBindings,
              new CoordinateDD(
                  lowerBoundingBox.northSouth, Math.nextAfter(-180.0, 0)
                  /** -179.999... */
                  ),
  ...
  void assertConsistency() {
    ...
    if (searchDatatype == null) {
      throw new GeoSpatialSearchException(
          GeoSpatial.SEARCH_DATATYPE + " must be bound but is null.");
    }
    ...
    if (locationVar != null
      ...
      if (!(datatypeConfig.hasLat() && datatypeConfig.hasLon())) {
        throw new GeoSpatialSearchException(
            "Requested extraction of geospatial coordinates (via "
                + GeoSpatial.LOCATION_AND_TIME_VALUE
                + ", "
                + GeoSpatial.LOCATION_AND_TIME_VALUE
                + ", "
                + GeoSpatial.LAT_VALUE
                + ", "
                + GeoSpatial.LON_VALUE
                + " or "
                + GeoSpatial.DISTANCE_VALUE
                + ")"
                + " but the index contains no geospatial coordinates. "
    ...
    if (timeVar != null && !datatypeConfig.hasTime()) {
      throw new GeoSpatialSearchException(
          "Requested extraction of time via "
              + GeoSpatial.TIME_VALUE
    ...
    if (coordSystemVar != null && !datatypeConfig.hasCoordSystem()) {
      throw new GeoSpatialSearchException(
          "Requested extraction of coordinate system via "
              + GeoSpatial.COORD_SYSTEM_VALUE
    ...
    if (customFieldsVar != null && !datatypeConfig.hasCustomFields()) {
      throw new GeoSpatialSearchException(
          "Requested extraction of custom fields via "
              + GeoSpatial.CUSTOM_FIELDS_VALUES
              + " but index does not define any custom fields.");
    }

    // assert that the custom fields defined in the index are identical with the specified custom
    // fields
    ...
    if ((!getCustomFieldsConstraints().keySet().containsAll(datatypeCustomFields))
      ...
      throw new GeoSpatialSearchException(
          "The custom fields defined in the datatype ("
              + Arrays.toString(datatypeCustomFields.toArray())
              ...
              + "the index using predicates "
              + GeoSpatial.CUSTOM_FIELDS
              + ", "
              + GeoSpatial.CUSTOM_FIELDS_LOWER_BOUNDS
              + ", and "
              + GeoSpatial.CUSTOM_FIELDS_UPPER_BOUNDS
              + ".");
    }
      ...
      case IN_CIRCLE:
        {
          ...
          if (spatialCircleCenter == null) {
            throw new GeoSpatialSearchException(
                "Predicate "
                    + GeoSpatial.SPATIAL_CIRCLE_CENTER
                    + " not supported for search function inCircle.");
          }

          if (spatialCircleRadius == null) {
            throw new GeoSpatialSearchException(
                "Predicate "
                    + GeoSpatial.SPATIAL_CIRCLE_RADIUS
                    + " not supported for search function inCircle.");
          }
      ...
      case IN_RECTANGLE:
        {
          ...
          if (spatialRectangleSouthWest == null) {
            throw new GeoSpatialSearchException(
                "Predicate "
                    + GeoSpatial.SPATIAL_RECTANGLE_SOUTH_WEST
                    + " not supported for search function inRectangle.");
          }
