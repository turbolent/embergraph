...
public class ZOrderRangeScanUtil {
   ...
   *
   * <p>If firstBitSet, then load 10000... is executed as defined in the paper (setting the bit
   * identified through position to 1 and all following bits in the respective dimension to 0);
   * otherwise load 01111... is executed (which sets the bit identified through position to 0 and
   * all following bits of the dimension to 1). The method has no return value, but instead as a
   * side effect the passed array arr will be modified.
   */
