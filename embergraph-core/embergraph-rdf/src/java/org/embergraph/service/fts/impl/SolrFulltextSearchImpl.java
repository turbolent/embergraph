 ...
 */
package org.embergraph.service.fts.impl;
...
public class SolrFulltextSearchImpl implements IFulltextSearch<FulltextSearchHit> {
  ...
  public FulltextSearchHiterator<FulltextSearchHit> search(
      org.embergraph.service.fts.IFulltextSearch.FulltextSearchQuery query, HttpClient client) {
  ...
  private FulltextSearchHit[] queryIndex(FulltextSearchQuery query, HttpClient httpClient)
      throws Exception {
      ...
      for (int i = 0; i < params.length; i++) {
        if (params[i] != null) {
          ...
          if (kv.length == 2 && kv[0] != null && !(kv[0].isEmpty())) {
            if (!(kv[0].equals("wt"))) {
              try {
                final String val = kv[1] == null ? "" : URLDecoder.decode(kv[1], "UTF-8");
    ...
    final Integer queryTimeoutUsed =
        queryTimeoutSpecified == null ? FTS.Options.DEFAULT_TIMEOUT : queryTimeoutSpecified;
    ...
    if (statusCode != 200) {
      ...
      throw new FulltextSearchException(
          "Status code != 200 received from " + "external fulltext service: " + statusCode);
    }
  ...
  private FulltextSearchHit[] constructFulltextSearchList(
      JSONObject solrResultsJSON, FulltextSearchQuery query) throws JSONException {
    ...
    for (int i = 0; i < docs.length(); i++) {
      ...
      if (snippetColumn != null && !snippetColumn.isEmpty()) {
        ...
        snippet = result.has(snippetColumn) ? flattenJsonResult(result.get(snippetColumn)) : null;
      }

      String score = null;
      if (scoreColumn != null && !scoreColumn.isEmpty()) {

        score = result.has(scoreColumn) ? flattenJsonResult(result.get(scoreColumn)) : null;
      }
