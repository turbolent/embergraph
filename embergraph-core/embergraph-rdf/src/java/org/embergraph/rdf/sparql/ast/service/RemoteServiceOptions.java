 ...
 *     <p>TODO Add option to indicate that the service supports the ESTCARD feature ( fast range
 *     count for a triple pattern)? The feature should be disclosed through the service description,
 *     but we will need to publish a unique URI for that feature. Maybe we can get openrdf to
 *     implement the feature as well?
 *     <p>TODO Add {@link RemoteServiceOptions} options for additional URL query parameters
 *     (defaultGraph, etc), authentication, etc.
 */
public class RemoteServiceOptions extends ServiceOptionsBase {
   ...
   * state of the SPARQL end point when the end point is a read/write database. However, GET
   * supports HTTP caching and can scale much better when the SPARQL end point is a read-only
   * resource or a read-mostly resource where stale reads are acceptable.
   */
