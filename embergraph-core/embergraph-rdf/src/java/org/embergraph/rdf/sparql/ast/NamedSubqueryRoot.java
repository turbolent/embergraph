...
public class NamedSubqueryRoot extends SubqueryBase implements INamedSolutionSet {
  ...
  public interface Annotations extends INamedSolutionSet.Annotations {
     ...
     *
     * <p>Note: This can be different for each context in the query in which a given named result
     * set is included. When there are different join variables for different INCLUDEs, then we need
     * to build a hash index for each set of join variable context that will be consumed within the
     * query.
     *
     * <p>Note: If no join variables are specified, then the join will consider the N x M cross
     * product, filtering for solutions which join. This is very expensive. Whenever possible you
   ...
   * be computed dynamically by scanning for {@link NamedSubqueryInclude}s within the WHERE clause
   * of the named subquery.
   *
