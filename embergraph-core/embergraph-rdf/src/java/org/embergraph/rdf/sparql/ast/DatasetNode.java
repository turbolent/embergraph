...
public class DatasetNode extends QueryNodeBase {
   ...
   */
  @SuppressWarnings("rawtypes")
  public DatasetNode(final Set<IV> defaultGraphs, final Set<IV> namedGraphs, final boolean update) {

    this(asDataSetSummary(defaultGraphs, update), asDataSetSummary(namedGraphs, update));
  }

  @SuppressWarnings("rawtypes")
  public DatasetNode(
      final Set<IV> defaultGraphs,
      final Set<IV> namedGraphs,
      final IElementFilter<ISPO> defaultGraphFilter,
      final IElementFilter<ISPO> namedGraphFilter,
      final boolean update) {

    this(
        asDataSetSummary(defaultGraphs, update),
        asDataSetSummary(namedGraphs, update),
        defaultGraphFilter,
        namedGraphFilter);
  }

  public DatasetNode(final DataSetSummary defaultGraphs, final DataSetSummary namedGraphs) {
   ...
   *     be used unless a <i>defaultGraphsFilter</i> is applied.
   * @param namedGraphs The list of named graphs (optional). When not specified, all graphs will be
   *     used unless a <i>namedGraphsFilter</i> is applied.
   * @param defaultGraphFilter A filter for default graphs (optional and typically only used when
   *     the <i>defaultGraphs</i> is <code>null</code>, e.g., to apply ACLs).
   * @param namedGraphFilter A filter for named graphs (optional and typically only used when the
   *     <i>namedGraphs</i> is <code>null</code>, e.g., to apply ACLs).
   */
  public DatasetNode(
  ...
  public void setDefaultGraphFilter(final IElementFilter<ISPO> defaultGraphFilter) {
