 ...
 * </pre>
 *
 * FIXME {@link CompareOptimizer}. Replaces Compare with SameTerm whenever possible. (I think that
 * we handle this in the {@link FunctionRegistry}, but that should be verified and documented here.)
 *
 * <p>FIXME {@link ConjunctiveConstraintSplitter}. Takes a FILTER with an AND of constraints and
 * replaces it with one filter per left or right hand side of the AND. This flattens the value
 * expression hierarchy. It also might allow us to reject some solutions earlier in the pipeline
 * since we do not have to wait for all of the variables to become bound and we can potentially
 * evaluate some of the individual constraints earlier than others.
 *
 * <p>FIXME {@link SameTermFilterOptimizer}. Optimizes SameTerm(X,Y) by renaming one of the
 * variables to the other variable. Optimizes SameTerm(X,aURI) by assigning the constant to the
 * variable.
 *
 * <p>FIXME {@link FilterOptimizer}. Pushes filters as far down in the query model as possible.
 * (What purpose does this serve for us? We want to attach filters to the first required join where
 * they can possible by evaluated.)
 *
 * <p>FIXME Sesame has define a bunch of IQueryOptimizers that we are not using (even before this
 * refactor). Many of them apply to their SQL backends. A few might be relevant to us:
 *
 * <ul>
 *   <li>{@link DisjunctiveConstraintOptimizer} - moves SameTerm closer to joins for queries
 *       involving UNIONs.
 *   <li>{@link IterativeEvaluationOptimizer} - ???
 *   <li>{@link QueryModelNormalizer} - various simplifications of their tuple expression query
 *       model. I am not sure whether or not there is anything here we could leverage.
 * </ul>
 *
 * TODO Recognize OR of constraints and rewrite as IN. We can then optimize the IN operator in a
 * variety of ways (in fact, the {@link FunctionRegistry} already handles those optimizations for
 * IN).
 *
 * <p>TODO The combination of DISTINCT and ORDER BY can be optimized using an ORDER BY in which
 * duplicate solutions are discarded after the sort by a filter which compares the current solution
 * with the prior solution.
 *
 * <p>TODO A query with a LIMIT of ZERO (0) should be failed as there will be no solutions.
 *
 * <p>TODO Minor optimization: A bare constant in the ORDER BY value expression list should be
 * dropped. If there are no remaining value expressions, then the entire ORDER BY operation should
 * be dropped.
 *
 * <p>FIXME Write AST optimizer which rejects queries that SELECT variables which do not otherwise
 * appear in the query.
 *
 * <p>TODO If a child group is non-optional and not a union, then flatten it out (lift it into the
 * parent). (This sort of thing is not directly expressible in the SPARQL syntax but it might arise
 * through other AST transforms.)
 *
 * <p>TODO Optimizer to lift optionals outside of an ORDER BY when the OPTIONALs are not used by the
 * ORDER BY. This let's us avoid optional joins until we know what is in the result set.
 *
 * <p>TODO ORDER BY with LIMIT could be optimized as a pipelined + last pass operator which
 * maintained a sorted array of LIMIT+OFFSET items. Any items which order after the LIMIT+OFFSET
 * item in the list are discarded. This will keep down both the memory overhead of the ORDER BY
 * operator and the #of comparisons to be made. (We still need a SLICE to handle the OFFSET unless
 * the ORDER BY operator takes care of that during its last pass by only writing the items from
 * OFFSET through LIMIT-1.
 *
...
public class DefaultOptimizerList extends ASTOptimizerList {
  ...
  public DefaultOptimizerList() {
     ...
     * <b>no consequent optimizer calls are allowed to introduce anonymous aggregates in ORDER
     * BY<b>.
     */
     ...
     * added to the binding set we start out with.
     *
     * <p>IMPORTANT NOTE: setting up the starting binding set, this optimizer is an important
     * prerequisite for others and should be run early in the optimzer pipeline.
     */
     ...
     * <p>Convert:
     *
     * <p>filter(<BDS.SEARCH_IN_SEARCH>(?o,"foo")) .
     *
     * <p>To:
     *
     * <p>filter(?o IN ("foo", "foo bar", "hello foo", ...)) .
     *
     * <p>This is a way of using the full text index to filter instead of using regex.
     */
     ...
     * </pre>
     *
     * Note: This must run before the {@link ASTEmptyGroupOptimizer} in order to eliminate certain
     * UNION/group combinations.
     */
     ...
     * </pre>
     *
     * <p>Note: as a policy decision in embergraph 1.1, we do not WANT to combine non-empty join
     * groups. The group structure is left AS IS and provides a means for people to control the
     * pruning of variables. Also, even if the group structure were automatically flattened as much
     * as possible for non-empty groups, the optimizer(s) responsible for pruning intermediate
     * variables would cause a group structure to be re-introduced.
     */
     ...
     *
     * <p>Note: This needs to be run before anything else which looks at the {@link ProjectionNode}.
     */
     ...
     *
     * <p>Note: Search is most efficiently expressed within named subqueries. This let's you perform
     * additional joins against the solutions from the search service before the results are
     * materialized on a hash index to be joined into the main query.
     */
     ...
     * named subquery (score, snippet, etc).
     */
     ...
     * to correctly impose the GRAPH constraints on the named subquery.
     *
     * <p>FIXME Semantics for GRAPH ?g {} (and unit test).
     *
     * <p>FIXME Semantics for GRAPH <uri> {} (and unit test).
     */
     ...
     *
     * <p>FIXME This is not implemented yet.
     */
     ...
     * from the group (which could be an optional group, a join group, or a union).
     *
     * <p>If a required join is known to fail, then the parent will also fail. Continue recursively
     * up the parent hierarchy until we hit a UNION or an OPTIONAL. If we reach the root of the
     * where clause for a subquery, then continue up the groups in which the subquery appears.
     *
     * <p>If the parent is a UNION, then remove the child from the UNION.
     *
     * <p>If a UNION has one child, then replace the UNION with the child.
     *
     * <p>If a UNION is empty, then fail the group in which it fails (unions are not optional).
     *
     * <p>These rules should be triggered if a join is known to fail, which includes the case of an
     * unknown IV in a statement pattern as well <code>GRAPH uri {}</code> where uri is not a named
     * graph.
     *
     * <pre>
     *
     ...
     * </pre>
     *
     * and also if we encounter a value not in the lexicon, we can still continue with the query if
     * the value is in either an optional tail or an optional join group (i.e. if it appears on the
     * right side of a LeftJoin). We can also continue if the value is in a UNION. Otherwise we can
     * stop evaluating right now.
     *
     * <pre>
     ...
     * </pre>
     *
     * ASTUnknownTermOptimizer: If an unknown terms appears in a StatementPatternNode then we get to
     * either fail the query or prune that part of the query. If it appears in an optional, then
     * prune the optional. if it appears in union, the prune that part of the union. if it appears
     * at the top-level then there are no solutions for that query. This is part of what
     * EmbergraphEvaluationStrategyImpl3#toPredicate(final StatementPattern stmtPattern) is doing.
     * Note that toVE() as called from that method will throw an UnknownValueException if the term
     * is not known to the database.
     *
     * <p>FIXME Mike started on this, but it is not yet finished.
     */
     ...
     * group to the statement pattern node and then cause them to be attached to the JOIN when we
     * generate the JOIN.
     */
     ...
     * named subqueries.
     *
     * <p>Note: This rewrite step is necessary to preserve the "run-once" contract for a SERVICE
     * call. If a {@link ServiceNode} appears in any position other than the head of a named
     * subquery, it will be invoked once for each solution which flows through that part of the
     * query plan. This is wasteful since the SERVICE call does not run "as-bound" (source solutions
     * are not propagated to the SERVICE when it is invoked).
     */
     ...
     */
    if (!QueryHints.DEFAULT_OLD_JOIN_ORDER_OPTIMIZER) add(new ASTJoinGroupOrderOptimizer());
    else add(new ASTJoinOrderByTypeOptimizer());
     ...
     * ?z is pushed inside a SELECT DISTINCT ?z { triple-pattern } subquery, which may be amenable
     * to optimization through the {@link ASTDistinctTermScanOptimizer}, which is applied in the
     * subsequent step.
     */
     ...
     *
     * <p>Note: Either this must run after the {@link ASTRangeCountOptimizer} in order to modify the
     * estimated cardinality associated with using the {@link DistinctTermAdvancer} (which does less
     * work than a scan) or the {@link ASTRangeCountOptimizer} must not overwrite the cardinality
     * estimates attached by this optimizer and this optimizer could run first.
     *
     * @see <a href="http://trac.blazegraph.com/ticket/1035" > DISTINCT PREDICATEs query is slow
     *     </a>
     */
     ...
     *
     * <p>TODO Make the static join optimizer more robust by modifying it to process a "flattened"
     * join group, in which SPs are lifted up from child groups and subqueries. (This is possible
     * unless the subquery uses some aggregation mechanisms, SLICE, etc., in which case its
     * cardinality is governed by the #of groups output by the aggregation and/or by the
     * OFFSET/LIMIT).
     *
     * <p>FIXME The static optimizer does not take responsibility for all kinds of IJoinNode. It
     * needs to handle UnionNode, JoinGroupNode, SubqueryRoot, NamedSubqueryInclude, and
     * ServiceNode, not just StatementPatternNode. In many cases, it can do this by running the
     * optimization as if the sub-groups were flattened or as-if the where clause of a SubqueryRoot
     * were flattened into the query. It must treat the variables bound by a NamedSubqueryInclude or
     * a ServiceNode, but it should not attempt to reorder those join nodes when ordering the joins
     * in an parent join group.
     *
     * <p>FIXME The static optimizer needs to be aware of the effective range count which results
     * from a datatype and/or range constraints and treat those variables as "somewhat" bound. The
     * importance of the range constraint depends on the ordinal index of the key component. For
     * example, a datatype or value range constraint on the OS(C)P index is much more selective than
     * a fully unbound SP, but is significantly less selective than a 1-bound SP. The actual range
     * count can be determined when the SP appears first in the join order, but it must be summed
     * over the union of the ground datatypes and value range constraints for the SP. (This may
     * require reasoning about the datatypes and value range constraints before the static join
     * optimizer runs followed by compilation of the appropriate datatype and range constraint after
     * the join order has been fixed.)
     */
     ...
     *
     * <p>(1) Any required joins before the first such complex optional are lifted out into a single
     * named subquery.
     *
     * <p>(2) Each complex optional is then turned into a named subquery. Any intermediate variables
     * used solely within the complex optional group are NOT projected out of the named subquery
     * (variable pruning).
     *
     * <p>Each such named subquery will INCLUDE either:
     *
     * <p>(a) the named subquery lifted out in step (1) (if the complex optionals are independent)
     *
     * <p>-or-
     *
     * <p>(b) the result of the previous named subquery (if they complex optionals must be fed into
     * one another).
     *
     * <p>Note: Sub-groups, including complex optionals, are handled using a hash join pattern even
     * without this optimizer, but they are not lifted into named subqueries unless this optimizer
     * runs and intermediate variables used solely during the complex optional group are not
     ...
     *     <p>FIXME The blocking issue is now that the static join order optimizer does not order
     *     the sub-groups and sub-selects within the parent group, but only the SPs within each
     *     group. The ASTHashJoinOptimizer needs to run before the static join order optimizer
     *     since, it were to run after, it could attempt to push down SPs into subgroups which could
     *     create a very bad ordering in the parent.
     */
     ...
     * the sub-select as a full IRunningQuery using the same hash join pattern. (We can also achieve
     * essentially the same result by feeding the result of one named subquery into another.)
     */
     ...
     *
     * <p>TODO This should probably recognize the pattern of INCLUDEs which licenses a MERGE JOIN.
     * Or we can just do that when we generate the query plan (might be simpler since we do not have
     * to mess with the INCLUDE AST).
     */
