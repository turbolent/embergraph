 ...
 * deciding how their own order.
 *
 * <p>We want to optimize all {@link JoinGroupNode}s recursively, from the top down. This is because
 * the join group needs to take into account its ancestral join ordering when deciding its own join
 * ordering. A statement pattern with a shared variable with the ancestral groups should be
 * preferred over one with no shared variables.
 */
public class ASTStaticJoinOptimizer implements IASTOptimizer {
  ...
  public interface Annotations extends AST2BOpBase.Annotations {
     ...
     * two sides (range count for a statement pattern, previous join cardinality for a predecessor
     * join). The default value is <code>1.0</code>, which is the historical behavior.
     *
     * <p>For a value of <code>0.67</code> the optimizer takes a mostly optimistic view by default -
     * the join cardinality will be <code>0.67
     * * the MIN + 0.33 * the MAX</code>. This settting will eliminate some of the worst possible
     * outcomes (ones where we guess wrong and get a very bad join order as a result).
     *
     * <p>BSBM BI Q1 is a good example of a query that benefits from the pessimistic approach, and
     * LUBM Q2 is a good example of a query that benefits from the optimistic approach.
     */
  ...
  /** Return <code>true</code> if the static join optimizer is enabled for the given join group. */
  static boolean isStaticOptimizer(
      final IEvaluationContext context, final JoinGroupNode joinGroup) {
    ...
    ////        } else {
    //
    ////            optimizer = context == null || context.queryHints == null ?
    // QueryOptimizerEnum.Static
  ...
  private abstract class GroupNodeOptimizer<T extends GraphPatternGroup<?>> {
    final T op;
    final AST2BOpContext ctx;
    private final IBindingSet exogenousBindings;
    final QueryRoot queryRoot;

    public GroupNodeOptimizer(
        AST2BOpContext ctx,
    ...
    private void optimizeRecursively() {
      ...
      for (int i = 0; i < op.arity(); i++) {
        ...
        if (child instanceof GraphPatternGroup<?>) {
           ...
           * translated into named subquery includes, there is no ancestry at all (named subquery
           * includes will be evaluated independently, bottom up). This is, for instance, the case
           * for subqueries with aggregation or slices.
           */
  ...
  private class JoinGroupNodeOptimizer extends GroupNodeOptimizer<JoinGroupNode> {
    ...
    private void addToAncestry(
        List<? extends IBindingProducerNode> moreAncestors, String dbgMessage) {
  ...
  private GroupNodeOptimizer<?> createGroupNodeOptimizer(
      final AST2BOpContext ctx,
    ...
    if (op instanceof JoinGroupNode) {
      return new JoinGroupNodeOptimizer(
          ctx, exogenousBindings, queryRoot, ancestry, (JoinGroupNode) op);
  ...
  private void optimizeJoinGroup(
      final AST2BOpContext ctx,
      final QueryRoot queryRoot,
      List<IBindingProducerNode> ancestry,
  ...
  //    		if (iv == null) {
  //
  //    			throw new AssertionError("this optimizer cannot run with unknown IVs in statement
  // patterns");
  //
  //    		}
  //
