 ...
 *
 * <p>Note: When the filter is lifted, it must be attached to the statement pattern node such that
 * toPredicate() puts them onto the predicate since they must run *with* the join for that
 * predicate. (The problem is that ?x != Bar is filtering the optional join, not ?x).
 *
 * <pre>
 * where {
 ...
 * </pre
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">
 *
 * Bryan Thompson</a>
 *
 * @version $Id: ASTSimpleOptionalOptimizer.java 5197 2011-09-15 19:10:44Z thompsonbry $
 */
public class ASTSimpleOptionalOptimizer implements IASTOptimizer {
...

    {
      if (queryRoot.getNamedSubqueries() != null) {
        ...
        for (NamedSubqueryRoot namedSubquery : queryRoot.getNamedSubqueries()) {
          ...
          collectOptionalGroups(namedSubquery.getWhereClause(), optionalGroups);
        }
      }
   ...
   *
   * <p>Note: Do not bother to collect an "optional" unless it has a parent join group node (they
   * all should).
   */
  @SuppressWarnings("unchecked")
  private void collectOptionalGroups(
    ...
    for (IGroupMemberNode child : group) {
      ...
      if (child instanceof ServiceNode) {
        ...
        final GraphPatternGroup<IGroupMemberNode> graphPattern = serviceNode.getGraphPattern();
      ...
      collectOptionalGroups((GraphPatternGroup<IGroupMemberNode>) child, optionalGroups);
    }
  }
  ...
  private void liftOptionalGroup(final StaticAnalysis sa, final JoinGroupNode group) {
    ...
    final List<FilterNode> mockFilters = new LinkedList<FilterNode>();
    {
      ...
      for (IGroupMemberNode child : group) {
        ...
        if (child instanceof StatementPatternNode) {
          ...
          final INeedsMaterialization req = filter.getMaterializationRequirement();
          ...
          if (req instanceof ComputedMaterializationRequirement) {
            ...
            final IValueExpressionNode ven =
                BOpUtility.deepCopy((ValueExpressionNode) filter.getValueExpressionNode());

            ven.setValueExpression(TrueBOp.INSTANCE);

            final ComputedMaterializationRequirement mockReq =
                new ComputedMaterializationRequirement(
                    Requirement.ALWAYS,
                    ((ComputedMaterializationRequirement) req).getVarsToMaterialize());

            final MockFilterNode mockFilter = new MockFilterNode(ven, mockReq);
          ...
          throw new AssertionError(
              "Unexpected child for simple optional: group=" + group + ", child=" + child);
        }
      }
     ...
     */
    if (!filters.isEmpty()) sp.setAttachedJoinFilters(filters);
   ...
   * @param group Some candidate {@link JoinGroupNode}.
   */
  private static boolean isSimpleOptional(
      final StaticAnalysis sa, final JoinGroupNode p, final JoinGroupNode group) {
    ...
    for (IQueryNode node : group) {
      ...
      if (node instanceof StatementPatternNode) {
        ...
        if (false && req instanceof ComputedMaterializationRequirement) {
          ...
          final Set<IVariable<?>> requiredVars =
   ...
   * group.
   */
  static final class MockFilterNode extends FilterNode {
    ...
    /** @param ve */
    public MockFilterNode(
        final IValueExpressionNode ve, final ComputedMaterializationRequirement req) {
