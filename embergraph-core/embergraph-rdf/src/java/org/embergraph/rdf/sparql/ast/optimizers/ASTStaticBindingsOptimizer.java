...
import org.embergraph.rdf.sparql.ast.GroupMemberValueExpressionNodeBase;
 ...
 *
 * <p>The optimizer may apply to the following construct:
 *
 * <p>(A) BIND + VALUES clauses with constant values; (B) FILTER IN clauses with URIs; (C) FILTERs
 * with sameTerm(?x,<http://someUri>) and ?x=<http://someUri> clauses
 *
 * <p>It proceeds as follows:
 *
 * <p>1. Identify the constructs mentioned above in the query and record identified static bindings.
 * We distinguish between produced static bindings, namely constructs that introduce bindings (case
 * A above) and enforced static bindings (cases B and C) above. Produced bindings are recorded in
 * class {@link StaticBindingInfo}, and for them we also remove the construct producing the
 * bindings, as they will later be added to as static bindings to the query top level (see step 3a
 * below). See step 3b below for the treatment of enforced bindings
 *
 * <p>2. Along the way, we record usages of all variables, see class {@link VariableUsageInfo}.
 * These usages (i) may be used for inlining in a later step, whenever we detected static bindings
 * and (b) are used to decide whether static bindings can be considered global. There are some
 * pitfalls here, in particular with FILTER expressions, see the code for in-depth comments.
 *
 * <p>3. Having extracted the static bindings and the associated variable usages, we proceed as
 * follows:
 *
 * <p>3a. Produced bindings are moved to the top-level. For the main query this means inserting them
 * into the set of exogeneous bindings, for subqueries we construct a VALUES clause.
 *
 * <p>3b. Both the produced and enforced bindings are "inlined" to identified usages of the
 * variable. This essentially means replacing occurrences of the variable through a hybrid of
 * Variable+Constant. This info can then later be used in further optimization steps and at runtime.
 *
 * <p>4. As a side effect, the optimizer re-initializes the {@link StaticAnalysis} class with the
 * statically known bindings for the top-level query, which againg may be valuable input for further
 * optimization.
 *
 * <p>Note: This optimizer generalizes the {@link ASTSimpleBindingsOptimizer}, the {@link
 * ASTValuesOptimizer}, and the {@link ASTBindingAssigner}, which have been disabled and marked
 * deprecated
 *
 * <p>The following extensions are not considered crucial for now, but might be subject to future
 * work on this optimizer: (i) we may want to decompose FILTERs prior to running this optimizer,
 * which may be useful to identify; (ii) in some cases it is also valid to propagate static bindings
 * to optional patterns; implementing this might increase the benefit of the optimizer; (iii) we
 * could implement some special handling for the variables reported by {@link
 * IEvaluationContext#getGloballyScopedVariables()}: currently, they're just treated as "normal"
 * variables and hence only inlined at top-level, but it would be possible to inline them into
 * subqueries and in any nested scope as well, giving us a (in general) a better evaluation plan.
 *
...
public class ASTStaticBindingsOptimizer implements IASTOptimizer {
  ...
  public QueryNodeWithBindingSet optimize(
    ...
    /**
     * We collect statically enforced bindings in this binding set, which will later be post
     * processed and injected (by joining all bindings).
     */
    final StaticBindingInfo staticBindingInfo = new StaticBindingInfo(bindingSets);

    /**
     * In case the binding sets variable is null, nothing needs to be done (query will return empty
     * result anyways). Also, this optimizer is only applicable to top-level queries.
     */
    ...
    /**
     * Apply the optimization (eliminating static binding producing constructs from the query and
     * collecting the staticBindings set)
     */
  ...
  private IBindingSet[] optimize(
    ...
    /**
     * First, collect static bindings from the outer VALUES clause. In case of the root query, those
     * will be added to the exogeneous binding set later on; in case of subqueries, we'll merge them
     * with other produced static bindings into a new VALUES clause in the end.
     */
    ...
    /**
     * The optimization phase performs rewriting & extraction of static binding infos and inline
     * tasks.
     */
    optimize(sa, queryRoot.getWhereClause(), staticBindingInfo, ancestorVarUsageInfo, inlineTasks);
    ...
    /**
     * Return the binding set, leaving the caller the decision on what to do with it (i.e., adding
     * it to the exogeneous bindings or constructing a VALUES clause). The binding set is
     * constructed by joining all produced bindings. Note that this is the case for the outer query;
     * bindings for inner queries (where we add them to their VALUES clause are treated in the
     * optimize method for the subquery root).
     */
  ...
  private void optimize(
    ...
    if (group instanceof JoinGroupNode
        && (((JoinGroupNode) group).isOptional() || ((JoinGroupNode) group).isMinus())) {
      return;
    }
    ...
    /**
     * Collect information about which variables are used where, for later inlining. Further, this
     * information is required to make sure that we do not make static bindings exogeneous for some
     * variable ?x for which a filter exists in the same scope or in the parent scope, which would
     * lead to the case that this variable is effectively considered bound in the filter scope where
     * it shouldn't (such cases will be captured by rewritings in the ASTBottomUpOptimizer later
     * on).
     */
    ...
    for (IGroupMemberNode child : group) {
      ...
      if (child instanceof AssignmentNode) {
         ...
         * dictionary and later joins might fail. We could add this dictionary resolving step for
         * the constructed static bindings (i.e., the exogeneous bindings), but this is something
         * we'd need to do before applying the inlining. For now, this is out of scope. Maybe a
         * simpler to implement strategy would be to resolve these nodes in a prior phase and look
         * them up, then leaving this code here unchanged.
         */
        ...
        for (IVariable<?> var : constantVars.keySet()) {
          ...
          if (usageInfo.varUsed(var)) {
            ...
            inlineTasks.add(
                new InlineTasks(var, (IConstant) constantVal, usageInfo.getVarUsages(var)));
          }
        }
        ...
        if (functionURI.equals(FunctionRegistry.SAME_TERM)
          ...
          final IValueExpressionNode left = (IValueExpressionNode) functionNode.get(0);

          final IValueExpressionNode right = (IValueExpressionNode) functionNode.get(1);
          ...
          if (left instanceof VarNode && right instanceof ConstantNode) {
            ...
            if (functionURI.equals(FunctionRegistry.EQ) && constant.isLiteral()) return;
          ...
          if (!bs.isEmpty() && bs.size() == 1) {
            ...
            // add inline tasks for variable
            if (usageInfo.varUsed(var)) {
              inlineTasks.add(new InlineTasks(var, val, usageInfo.getVarUsages(var)));
            }
      ...
      if (child instanceof IBindingProducerNode && StaticAnalysis.isMinusOrOptional(child)) {

        sa.getMaybeProducedBindings((IBindingProducerNode) child, optOrMinusVars, true);
      }
    }

    /**
     * Remove the children for which static bindings were extracted (they were recorded in the prior
    ...
    for (IGroupMemberNode child : group) {
      ...
      if (child instanceof GroupNodeBase) {
        ...
        optimize(
            sa, (GroupNodeBase<?>) child, staticBindingInfo, ancOrSelfVarUsageInfo, inlineTasks);
  ...
  /**
   * Helper class used to record usage of a given variable, i.e. linking variables to constructs in
   * which they occur.
   */
  public static class VariableUsageInfo {
    ...
    /**
     * Gets the map recoding all usages of all variables (internal data structure maintained by the
     * class).
     *
     * @return
     */
    ...
    /**
     * Extracts variable usage information (variables and the nodes they occur in), investigating
     * FILTERs, assignment nodes, and statement pattern nodes, but /not/ recursing into children.
     *
    ...
    private void extractVarUsageInfoForPropertyPathNode(final PropertyPathNode ppNode) {
     ...
     */
    private void extractVarUsageInfo(final IQueryNode node, final IValueExpressionNode vexpNode) {

      /**
       * If the vexpNode is a VarNode, we're done and add it to the map (if it has not yet been
       * recorded before).
       */
    ...
    /**
     * Extracts usage information for the variable from statement patterns being direct children of
     * the current node (ignoring the node itself).
     *
    ...
    /**
     * Extracts usage information for the variable inside statement patterns from the current node
     * itself and its children (recursively).
     *
     * <p>Note that we do not report variable usage in filters, as those might be not in scope and
     * inlining is not safe (interference with {@link
     * ASTBottomUpOptimizer#handleFiltersWithVariablesNotInScope})!). This maybe somewhat too strict
     * (leading to situations where we do not inline static variable bindings), but is safe.
     *
     * @param group
     */
    @SuppressWarnings("rawtypes")
     ...
     */
    private void registerVarToChildMappingInUsageMap(
        final IVariable<?> var, final IQueryNode child) {
    ...
    /**
     * Merges two {@link VariableUsageInfo}, creating a new one containing the merged information.
     * The original objects are not modified.
     */
    public static VariableUsageInfo merge(final VariableUsageInfo x, final VariableUsageInfo y) {

      final VariableUsageInfo merged = new VariableUsageInfo();
      final Map<IVariable<?>, List<IQueryNode>> usageMap = merged.getUsageMap();
   ...
   * The class maintains a list of "produced" static bindings, i.e. such derived from assignment
   * nodes etc.
   */
  public static class StaticBindingInfo {
    ...
    /**
     * Joins the staticBindings with the queryInput binding set, returning the resulting binding
     * set.
     */
        ...
        for (IBindingSet left : leftBindingSets) {
          ...
          while (rightItr.hasNext()) {
            ...
            final IBindingSet join =
                BOpContext.bind(left, right, null /* constraints */, null /* varsToKeep */);
  ...
  /**
   * Task specifying the inlining opportunities for a given variable in the form of a list of query
   * nodes in which a statically derived value for the given variable can be inlined.
   *
   * <p>Can be applied by calling the {@link InlineTasks#apply()} method.
   */
  public static class InlineTasks {
     ...
     */
    @SuppressWarnings("rawtypes")
    public InlineTasks(final IVariable var, final IConstant<IV> val, final List<IQueryNode> nodes) {
    ...
    /**
     * Applies the {@link InlineTasks} for the variable through the given constant to the patterns
    ...
    /**
     * Applies the {@link InlineTasks} for the class variable with the parameter val to the given
     * query node.
     *
     * @param val
     * @param node
     ...
     */
    @SuppressWarnings("rawtypes")
    private void applyToStatementPattern(final IV val, final StatementPatternNode spn) {
     ...
     */
    @SuppressWarnings("rawtypes")
    private void applyToPropertyPathNode(final IV val, final PropertyPathNode ppn) {
    ...
    /**
     * Applies the {@link InlineTasks} for the class variable with the parameter val to the value
     * expression node statement pattern node.
     *
     * @param val
     * @param spn
     */
    ...
    private void applyToValueExpressionNode(final IV val, final IValueExpressionNode vexpr) {
      ...
      for (int i = 0; i < functionNode.arity(); i++) {
        ...
        final IValueExpressionNode cur = (IValueExpressionNode) functionNode.get(i);
