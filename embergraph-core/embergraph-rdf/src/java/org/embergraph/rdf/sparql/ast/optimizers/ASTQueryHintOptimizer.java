 ...
 *
 * <p>Once recognized, query hints are removed from the AST. All query hints are declared internally
 * using interfaces. It is an error if the specified query hint has not been registered with the
 * {@link QueryHintRegistry}, if the {@link IQueryHint} can not validate the value, if the {@link
 * QueryHintScope} is not legal for the {@link IQueryHint}, etc.
 *
 * <p>For example:
 *
 * <pre>
...
public class ASTQueryHintOptimizer implements IASTOptimizer {
    ...
    // First, process any pre-existing named subqueries.
    {
      ...
      if (namedSubqueries != null) {
        ...
        for (NamedSubqueryRoot namedSubquery : namedSubqueries) {
          ...
          processGroup(context, queryRoot, namedSubquery, namedSubquery.getWhereClause());
        }
      }
    }
  ...
  private boolean isNodeAcceptingQueryHints(final BOp op) {
     ...
     * basically the value expressions.
     *
   ...
   * @param queryHints
   */
  private void applyGlobalQueryHints(
      final AST2BOpContext context, final QueryRoot queryRoot, final Properties queryHints) {

    if (queryHints == null || queryHints.isEmpty()) return;
    ...
    while (itr.hasNext()) {
      ...
      if (!isNodeAcceptingQueryHints(op)) continue;
    ...
    for (ASTBase t : list) {
      ...
      applyQueryHints(context, queryRoot, QueryHintScope.Query, t, queryHints);
    }
  }
    ...
    int nfound = 0;
    {
      ...
      for (int i = 0; i < arity; i++) {
        ...
        if (child instanceof StatementPatternNode) {
          ...
          // Apply query hint.
          applyQueryHint(context, queryRoot, queryBase, group, prior, sp);
        ...
        if (child instanceof GraphPatternGroup<?>) {
          ...
          processGroup(
              context, queryRoot, (SubqueryRoot) child, ((SubqueryRoot) child).getWhereClause());
          ...
          final GraphPatternGroup<IGroupMemberNode> filterGraphPattern =
              ((SubqueryFunctionNodeBase) ((FilterNode) child).getValueExpressionNode())
                  .getGraphPattern();

          processGroup(context, queryRoot, queryBase, filterGraphPattern);
        }
   ...
   *
   * <p>Within that namespace we have public query hints whose localName is declared in the {@link
   * QueryHints} interface, e.g., {@link QueryHints#CHUNK_SIZE} which appears as follows in a query:
   *
   * <pre>
   * hint:chunkSize
   * </pre>
   ...
   * </pre>
   *
   * In both cases, an {@link IQueryHint} is registered with the {@link QueryHintRegistry}. The
   * "non-public" query hints are simply not declared by the {@link QueryHints} interface and are
   * intended more for internal development and performance testing rather than as generally useful
   * query hints.
   *
   * @param sp The {@link StatementPatternNode}.
  ...
  private void applyQueryHint(
    ...
    if (hint == null) throw new IllegalArgumentException();
    ...
    if (log.isInfoEnabled()) log.info("name=" + name + ", scope=" + scope + ", value=" + value);
  ...
  private void applyToQuery(
    ...
    if (false && context.queryHints != null) {
       ...
       * is basically backwards. The query hints in the global scope provide a default. Explicitly
       * given query hints in the query are delegated to IQueryHint implementations and then applied
       * to the appropriate AST nodes.
       *
       * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/791" > Clean up query hints
       *     </a>
       */
    ...
    for (IGroupMemberNode child : group) {
      ...
      if (child instanceof GraphPatternGroup<?>) {
        ...
        applyToGroupAndSubGroups(
            context, queryRoot, scope, (GraphPatternGroup<IGroupMemberNode>) child, name, value);
      }
    }
