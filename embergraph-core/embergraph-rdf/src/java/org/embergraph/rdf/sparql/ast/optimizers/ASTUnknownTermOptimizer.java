 ...
 *
 * <p>If a statement pattern contains an unknown term, a with this statement pattern will certainly
 * fail. Thus the group in which the statement pattern appears (the parent) will also fail. Continue
 * recursively up the parent hierarchy until we hit a UNION or an OPTIONAL parent. If we reach the
 * root of the where clause for a subquery, then continue up the groups in which the subquery
 * appears.
 *
 ...
 * </pre>
 *
 * and also if we encounter a value not in the lexicon, we can still continue with the query if the
 * value is in either an optional tail or an optional join group (i.e. if it appears on the right
 * side of a LeftJoin). We can also continue if the value is in a UNION. Otherwise we can stop
 ...
 * </pre>
 *
 * ASTPruneUnknownTerms : If an unknown terms appears in a StatementPatternNode then we get to
 * either fail the query or prune that part of the query. If it appears in an optional, then prune
 * the optional. if it appears in union, the prune that part of the union. if it appears at the
 * top-level then there are no solutions for that query. This is part of what
 * EmbergraphEvaluationStrategyImpl3#toPredicate(final StatementPattern stmtPattern) is doing. Note
 * that toVE() as called from that method will throw an UnknownValueException if the term is not
 * known to the database.
 *
 * <p>FIXME Isolate pruning logic since we need to use it in more than one place.
 */
public class ASTUnknownTermOptimizer implements IASTOptimizer {
  ...
  private static void eliminateGroupsWithUnknownTerms(
      final QueryRoot queryRoot, final GroupNodeBase<IGroupMemberNode> op) {
  ...
  private static void pruneGroup(
      final QueryRoot queryRoot, final GroupNodeBase<IGroupMemberNode> op) {
  ...
  private static void removeEmptyChildGroups(final GraphPatternGroup<?> op) {
