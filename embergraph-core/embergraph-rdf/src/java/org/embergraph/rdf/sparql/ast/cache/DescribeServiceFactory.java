...
public class DescribeServiceFactory extends CustomServiceFactoryBase {
  ...
  public void startConnection(final EmbergraphSailConnection conn) {
     ...
     * are multiple entry points, including AST evaluation, the DescribeServlet, and the test suite.
     * AST2BOpContext does this, but it is not always created before we need the cache connection.
     */
    final ICacheConnection cacheConn =
        CacheConnectionFactory.getCacheConnection(conn.getEmbergraphSail().getQueryEngine());
    ...
    final IDescribeCache describeCache =
        cacheConn.getDescribeCache(tripleStore.getNamespace(), tripleStore.getTimestamp());
   ...
   *
   * <dl>
   *   <dt>Invalidation only.
   *   <dd>Changes for resources in the subject or object position cause the resource to be
   *       invalidated in the cache.
   *   <dt>Maintenance
   *   <dd>The cache is actually a fully populated and maintained index. All updates are propagated
   *       into the cache so it remains consistent with the statement indices.
   * </dl>
   *
   * In addition to these strategies, we could treat the cache as a partial representation of the
   * linked data available on the open web for the resources and track metadata about the age of the
   * resource description for each linked data authority.
   *
   * <p>Another twist for invalidation would be to bound the cache capacity. That would require us
   * to also maintain metadata to support an eviction policy. The easiest way to do that is to
   * manage the raw journal entries on the store and a LIRS/LRU eviction policy on the {@link IV}s,
   * together with the address of the raw record (Blob/Stream).
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private static class DescribeCacheChangeLogListener implements IChangeLog {
    ...
    public void changeEvent(final IChangeRecord record) {
      ...
      if (log.isTraceEnabled()) log.trace("Invalidation notice: spo=" + spo);

      if (ivs.add(spo.s())) size++;

      if (ivs.add(spo.o())) size++;
