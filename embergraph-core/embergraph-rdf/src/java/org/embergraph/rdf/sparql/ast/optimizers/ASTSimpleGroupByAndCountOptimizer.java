 ...
 * solutions.
 *
 * <p>The optimizer aims at establishing an execution plan that applies a combination of distinct
 * term scan pattern (to efficiently compute the distinct values for the group variable) and fast
 * range count pattern to efficiently calculate the COUNT, without materialization of the variables
 * on which the COUNT operation is performed.
 *
 * <p>The basic idea is to
 *
 * <p>(i) replace the GROUP BY pattern through a SELECT DISTINCT subquery to calculate the distinct
 * bindings for variable ?z first, and (ii) use a fast range count operator to efficiently calculate
 * the COUNT.
 *
 * <p>Note that the sub query in step (i) may (where possible) be optimized by the {@link
 * ASTDistinctTermScanOptimizer}, i.e. if possible the subquery producing the ?z bindings will be
 * replaced by a distinct term scan in a later optimization step.
 *
...
public class ASTSimpleGroupByAndCountOptimizer implements IASTOptimizer {
  ...
  private static final transient Logger log =
      Logger.getLogger(ASTSimpleGroupByAndCountOptimizer.class);
  ...
  public QueryNodeWithBindingSet optimize(
    ...
    if (context.getAbstractTripleStore().getSPORelation().indicesHaveDeleteMarkers()) {
       ...
       * key-range scan, which is not desired.
       *
       * <p>While AccessPath.rangeCountExact(true) method will do the right thing even if the index
       * has delete markers (it will convert to a scan for either delete markers or if there is a
       * FILTER attached to the index), converting to a scan defeats the purpose of the
       * ASTFastRangeCountOptimizer. In this case, the cost would have been the same if we had not
       * rewritten the AST. Hence we do not rewrite the query.
       */
     ...
     * exhibits an additional having clause (which might apply any [possibly other] kind of
     * aggregation on the query body).
     */
   ...
   * @param context
   * @param sa
   * @param queryRoot The top-level of the query.
   * @param queryBase Either a top-level query or a sub-query.
   */
  private void doSelectQuery(
     ...
     *
     * <p>(C2) Single triple pattern in body
     *
     * <p>(C3) Single GROUP BY variable that is part of the triple pattern
     *
     * <p>(C4) Projection for the GROUP BY variable plus a COUNT operation, which operates over star
     * or any other variable in the triple pattern, plus the COUNT must not be DISTINCT
     */
...

    {
      ...
      final AssignmentNode assignmentNodeInGroupBy = (AssignmentNode) groupByNode.get(0);
      ...
      final VarNode groupingVarRenamed = (VarNode) assignmentNodeInGroupBy.get(0);
      ...
      for (int i = 0; i < 2; i++) {
        ...
        final IValueExpressionNode valNode = curNode.getValueExpressionNode();

        if (valNode instanceof FunctionNode && indexOfCountNode == null) {
          final FunctionNode fNode = (FunctionNode) curNode.getValueExpressionNode();

          if (!fNode.getFunctionURI().equals(FunctionRegistry.COUNT)) return; // NOT COUNT
          ...
          boolean countOnPotentiallyUnboundVariable =
              varNodesInStmtPattern.contains(innerVarNode) && !innerVarNode.equals(graphVarNode);
     ...
     *
     * <p>(O1) Wrap the existing statement pattern into a SELECT DISTINCT subquery, which could
     * (potentially) be optimized to a distinct term scan by the #{ASTDistinctTermScanOptimzer}
     *
     * <p>(O2) Duplicate the statement pattern and append a fast range count annotation, binding the
     * count var of the SELECT clause
     *
     * <p>(O3) Transform the SELECT clause: the COUNT expression is replaced by a projection for the
     * variable introduced in O2
     *
     * <p>(O4) Eliminate the group by clause
     */
    {
      // apply optimization step (O1)
      final SubqueryRoot selectDistinct = new SubqueryRoot(QueryType.SELECT);
      final ProjectionNode projection = new ProjectionNode();
      final AssignmentNode assignemntNode = new AssignmentNode(groupingVar, groupingVar);
      ...
      final StatementPatternNode stmtPatternClone = new StatementPatternNode(stmtPattern);
