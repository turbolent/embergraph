...
public interface IVariableBindingRequirements {
   ...
   * is to be sent.
   *
   * <p>By *must* be bound we mean that, if it is possible to bring the variables in a join group in
   * order such that they are bound when executing the construct (e.g., think of a FILTER), then we
   * *must* consider that. However, when the {@link IVariableBindingRequirements#getRequiredBound()}
   * interface cannot be satisified, the query may still be valid and return results (for the SPARQL
   * 1.1 service example mentioned above, it may not, for the FILTER it may).
   *
   * <p>The set of these variables must be disjoint from {@link
   ...
   * reported through method {@link IVariableBindingRequirements#getRequiredBound(StaticAnalysis)}.
   *
