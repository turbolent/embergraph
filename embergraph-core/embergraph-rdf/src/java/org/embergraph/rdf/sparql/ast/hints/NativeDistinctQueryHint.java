 ...
 * (sub-)select basis (by interpreting the AST annotation when we are generating the query plan.
 * When supporting this more selective application, write a unit test which verifies that the native
 * distinct annotation is correctly interpreted when it appears on a {@link QueryBase}.
 *
 * <p>TODO The same is true of {@link QueryHints#NATIVE_DISTINCT_SPO} and several other similar
 * "analytic" hints. However, this is not possible (or is more difficult) when the native versus JVM
 * semantics of the operation span non-local operators (i.e., named subqueries with their non-local
 * includes).
 */
final class NativeDistinctQueryHint extends AbstractBooleanQueryHint {

  protected NativeDistinctQueryHint() {
    super(QueryHints.NATIVE_DISTINCT_SOLUTIONS, QueryHints.DEFAULT_NATIVE_DISTINCT_SOLUTIONS);
  }
