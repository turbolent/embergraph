...
import org.embergraph.striterator.SingleValueChunkedIterator;
 ...
 * @version $Id$
 *     <p>TODO Add support for slicing the join using tools like CUTOFF (to limit the #of solutions)
 *     and specifying a {@link RangeBOp} (to impose the desired key range). CUTOFF and a
 *     single-threaded pipeline join can guarantee that we report out the top-N subjects in terms of
 *     the hit relevance. {@link RangeBOp} would work on a cluster. None of this is perfectly
 *     satisfactory.
 */
public class SearchServiceFactory extends AbstractServiceFactoryBase {
  ...
  public EmbergraphServiceCall create(final ServiceCallCreateParams params) {
    ...
    if (map == null) throw new RuntimeException("Not a search request.");
  ...
  private void validateSearch(
    ...
    if (!uris.contains(BDS.SEARCH)) {
      throw new RuntimeException(
          "Required search predicate not found: " + BDS.SEARCH + " for searchVar=" + searchVar);
    }
  }
  ...
  private static class SearchCall implements EmbergraphServiceCall {
    ...
    public SearchCall(
      ...
      if (searchQuery instanceof ConstantNode) {
        ...
        this.search = (IVariable<IV>) c.getProperty(Constant.Annotations.VAR);
      } else {
        this.search = null;
      }
      ...
      for (StatementPatternNode meta : statementPatterns.values()) {
        ...
        final Literal oVal = meta.o().isConstant() ? (Literal) meta.o().getValue() : null;
    ...
    private Hiterator<IHit<?>> getHiterator() {
      ...
      //            final IValueCentricTextIndexer<IHit> textIndex = (IValueCentricTextIndexer)
      // store
      //                    .getLexiconRelation().getSearchEngine();

      final ITextIndexer<IHit> textIndex =
          (ITextIndexer)
              (this.subjectSearch
                  ? store.getLexiconRelation().getSubjectCentricSearchEngine()
                  : store.getLexiconRelation().getSearchEngine());

      if (textIndex == null) throw new UnsupportedOperationException("No free text index?");
          ...
          textIndex.search(
              new FullTextQuery(
                  ...
                  minRelevance == null
                      ...
                      : minRelevance.doubleValue() /* minCosine */,
                  maxRelevance == null
                      ? BDS.DEFAULT_MAX_RELEVANCE
                      : maxRelevance.doubleValue() /* maxCosine */,
                  minRank == null ? BDS.DEFAULT_MIN_RANK /*1*/ : minRank.intValue() /* minRank */,
                  maxRank == null
                      ? BDS.DEFAULT_MAX_RANK /*Integer.MAX_VALUE*/
                      : maxRank.intValue() /* maxRank */,
                  searchTimeout == null
                      ? BDS.DEFAULT_TIMEOUT /*0L*/
                      : searchTimeout.longValue() /* timeout */,
     ...
     * relevance variables then we would to notice that here. We would also have to produce one
     * solution for each binding set input to the service.
     */
    @Override
    @SuppressWarnings({"rawtypes", "unchecked"})
    public ICloseableIterator<IBindingSet> call(final IBindingSet[] bindingsClause) {
