...
import org.embergraph.bop.join.HTreeHashJoinAnnotations;
...
import org.embergraph.relation.rule.EmptyAccessPathExpander;
...
public class AST2BOpJoins extends AST2BOpFilters {
  ...
  /*
   * Add a join for a statement pattern. This handles triples-mode, named-graph and default graph
   * join patterns whether on a single machine or on a cluster.
   *
   * @param left
   * @param pred The predicate describing the statement pattern.
   * @param doneSet The set of variables already known to be materialized.
   * @param constraints Constraints on that join (optional).
   * @param cutoffLimit When non-null, this is the limit for a cutoff join (RTO).
   * @param queryHints Query hints associated with that {@link StatementPatternNode}.
  ...
  public static PipelineOp join(
    ...
    anns.add(
        new NV(
            ...
            getJoinConstraints2(
                constraints, needsMaterialization, cutoffLimit == null /* conditionalRouting */)));
    ...
    final boolean quads = pred.getProperty(Annotations.QUADS, Annotations.DEFAULT_QUADS);

    // when non-null use distinct-term-scan. see #1035.
    final VarNode distinctTermScanVar =
        (VarNode) pred.getProperty(StatementPatternNode.Annotations.DISTINCT_TERM_SCAN_VAR);

    // when non-null use fast-range-count. see #1037.
    final VarNode fastRangeCountVar =
        (VarNode) pred.getProperty(StatementPatternNode.Annotations.FAST_RANGE_COUNT_VAR);

    // pull off the Sesame dataset before we strip the annotations.
    final DatasetNode dataset = (DatasetNode) pred.getProperty(Annotations.DATASET);

    // strip off annotations that we do not want to propagate.
    pred =
        pred.clearAnnotations(
            new String[] {
              Annotations.SCOPE,
              Annotations.QUADS,
              Annotations.DATASET,
    ...
    if (fastRangeCountVar != null) {
      ...
      left =
          fastRangeCountJoin(
              left, anns, pred, dataset, cutoffLimit, fastRangeCountVar, queryHints, ctx);
    ...
    if (distinctTermScanVar != null) {
      ...
      left =
          distinctTermScanJoin(
              left, anns, pred, dataset, cutoffLimit, distinctTermScanVar, queryHints, ctx);
    ...
    if (quads) {
      ...
      switch (scope) {
        case NAMED_CONTEXTS:
          left = namedGraphJoin(left, anns, pred, dataset, cutoffLimit, queryHints, ctx);
          break;
        case DEFAULT_CONTEXTS:
          left = defaultGraphJoin(left, anns, pred, dataset, cutoffLimit, queryHints, ctx);
          break;
        default:
      ...
      left = triplesModeJoin(left, anns, pred, cutoffLimit, queryHints, ctx);
    }
    ...
    if (cutoffLimit != null) {
      ...
      left =
          addNonConditionalMaterializationSteps(
              left, doneSet, needsMaterialization, cutoffLimit, queryHints, ctx);
  ...
  /*
   * FIXME We need to handle cutoff joins here or the distinct-term-scan will not work with the RTO
   * (alternatively, make sure the RTO is only using pipeline joins when sampling the join graph).
   *
  ...
  private static PipelineOp distinctTermScanJoin(
    ...
    final SPOKeyOrder keyOrder = SPOKeyOrder.getKeyOrder(mockPred, ctx.isQuads() ? 4 : 3);
  ...
  private static PipelineOp fastRangeCountJoin(
    ...
    if (ctx.gpuEvaluation != null
        && pred.getProperty(
            GpuAnnotations.EVALUATE_ON_GPU, GpuAnnotations.DEFAULT_EVALUATE_ON_GPU)) {
      return ctx.gpuEvaluation.fastRangeCountJoin(
          left, anns, pred, dataset, cutoffLimitIsIgnored, fastRangeCountVar, queryHints, ctx);
    }

    anns.add(
        new NV(FastRangeCountOp.Annotations.COUNT_VAR, fastRangeCountVar.getValueExpression()));
   ...
   * @param dataset
   * @return
   * @todo If the context position is shared by some other variable which we know to be bound based
   *     on the selected join order, then we need to treat the context variable as bound during this
   *     analysis.
   * @todo Since we do not know the specific asBound values, but only that they will be bound, we
   *     should defer the SCAN versus SUBQUERY decision until we actually evaluate that access path.
   *     This is basically a special case of runtime query optimization.
   */
  private static PipelineOp namedGraphJoin(
    ...
    if (summary.nknown == 0) {
      ...
      pred =
          (Predicate<?>)
              pred.setUnboundProperty(
                  IPredicate.Annotations.ACCESS_PATH_EXPANDER, EmptyAccessPathExpander.INSTANCE);

      anns.add(new NV(PipelineJoin.Annotations.PREDICATE, pred));

      return newJoin(
          left, anns, false /* defaultGraphFilter */, summary, cutoffLimit, queryHints, ctx);
    }
    ...
    if (summary.nknown == 1) {
      ...
      pred =
          pred.asBound(
              (IVariable<?>) pred.get(3),
              new Constant<IV<?, ?>>((IVariable) pred.get(3), summary.firstContext));
    ...
    /*
     */
    final int accessPathSampleLimit =
        pred.getProperty(QueryHints.ACCESS_PATH_SAMPLE_LIMIT, ctx.accessPathSampleLimit);
    ...
    if (estimateCosts) {
      ...
      scanCostReport =
          ((AccessPath)
                  ctx.context.getAccessPath(
                      r,
                      (Predicate<?>)
                          pred.setProperty(IPredicate.Annotations.REMOTE_ACCESS_PATH, true)))
              .estimateCost();
       ...
       */
      subqueryCostReport =
          summary.estimateSubqueryCost(
              ctx.context,
              accessPathSampleLimit,
              (Predicate<?>) pred.setProperty(IPredicate.Annotations.REMOTE_ACCESS_PATH, true));
      ...
      scanAndFilter =
          pred.getProperty(QueryHints.ACCESS_PATH_SCAN_AND_FILTER, ctx.accessPathScanAndFilter);
    }
  ...
  private static PipelineOp defaultGraphJoin(
    ...
    final DataSetSummary summary = dataset == null ? null : dataset.getDefaultGraphs();
    ...
    if (dataset != null && summary == null) {
      ...
      return newJoin(
          left, anns, ctx.defaultGraphDistinctFilter, summary, cutoffLimit, queryHints, ctx);
    }
    ...
    //                final boolean shardTripleConstraint =
        ...
        pred =
            (Predicate<?>)
                pred.setUnboundProperty(
                    ...
                    new DGExpander(maxParallel, summary.getGraphs(), estimatedRangeCount));
      }
   ...
   *
   * <p>Note: The native memory based DISTINCT filter MUST NOT be used for pipelined joins.
   * Pipelined joins run "as-bound" and the per-as-bound cardinality is typically very small (1s to
   * 1000s). However, a hash join can hit a very large cardinality for the default graph distinct
   * filter since it sees all SPOs at once.
   *
   * @param pred The predicate.
   * @param summary The {@link DataSetSummary} (when available).
   * @param hashJoin <code>true</code> iff a hash join was chosen for this predicate.
   * @return
   */
  private static BOpFilterBase newDistinctFilter(
      final AST2BOpContext ctx,
      final Predicate<?> pred,
    ...
    if (nativeDistinct) {
       ...
       */
      final Long rangeCount = (Long) pred.getProperty(Annotations.ESTIMATED_CARDINALITY);
      if (rangeCount != null) {
    ...
    if (nativeDistinct) {
      ...
      final SPOKeyOrder indexKeyOrder =
          SPOKeyOrder.getKeyOrder((IPredicate) pred, 4 /* keyArity */);
  ...
  /*
   * Create and return an appropriate type of join. The default is the pipeline join. A hash join
   * can be selected using the appropriate query hint. The query hints which control this decision
   * must be annotated on the {@link IPredicate} by the caller.
   *
   * @param left
   * @param anns
   * @param defaultGraphFilter <code>true</code> iff a DISTINCT filter must be imposed on the SPOs.
   *     This is never done for a named graph query. It is normally done for default graph queries,
   *     but there are some edge cases where the SPOs are provably distinct and we do not need to
   *     bother.
   * @param summary The {@link DataSetSummary} (when available).
   * @param queryHints The query hints from the dominating operator context.
   * @param ctx The evaluation context.
   * @return
  ...
  private static PipelineOp newJoin(
      PipelineOp left,
      final List<NV> anns,
    ...
    final boolean hashJoin =
        cutoffLimit == null && pred.getProperty(QueryHints.HASH_JOIN, QueryHints.DEFAULT_HASH_JOIN);
    ...
    if (cutoffLimit != null) {
       ...
       * this join.
       */
      final boolean simpleJoin =
          ((Boolean) map.get(Annotations.SIMPLE_JOIN)).booleanValue()
      ...
      map.put(PipelineJoin.Annotations.MAX_PARALLEL_CHUNKS, Integer.valueOf(0));
      ...
      if (simpleJoin) {
        ...
        map.put(PipelineJoin.Annotations.EVALUATION_CONTEXT, BOpEvaluationContext.CONTROLLER);
    ...
    if (defaultGraphFilter) {
      ...
      pred = pred.addAccessPathFilter(newDistinctFilter(ctx, pred, summary, hashJoin));
    ...
    if (hashJoin) {
      ...
      final IVariable<?>[] joinVars =
          (IVariable<?>[]) pred.getRequiredProperty(HashJoinAnnotations.JOIN_VARS);
       ...
       */
      map.put(
          HashJoinOp.Annotations.NAMED_SET_REF,
          NamedSolutionSetRefUtility.newInstance(null, "--namedSet-" + ctx.nextId(), joinVars));
       ...
       */
      BOpEvaluationContext evaluationContext =
          (BOpEvaluationContext) map.get(BOp.Annotations.EVALUATION_CONTEXT);
