...
public final class StaticOptimizer {
  ...
  private StaticOptimizer(
      final StaticAnalysis sa,
      final IBindingProducerNode[] ancestry,
      final List<IReorderableNode> nodes,
      final double optimistic) {

    if (ancestry == null) throw new IllegalArgumentException();
      ...
      for (int i = 0; i < arity; i++) {
        ...
        log.debug(
            node.getClass()
                ...
                + node.isReorderable()
                + ", estcard: "
                + node.getEstimatedCardinality(this)
                + ", vars: "
                + getVars(i));
  ...
  /** Computes and sets the evaluation order, and returns an estimated cardinality. */
  private long calc() {
    ...
    if (ASTStaticJoinOptimizer.log.isDebugEnabled()) {
      ASTStaticJoinOptimizer.log.debug(
          "bindings from ancestry: " + Arrays.toString(ancestryVars.toArray()));
    }
     ...
     */
    if (preferredFirstTail == -1) preferredFirstTail = getNextTailThatSharesVarsWithAncestry();
    ...
    if (preferredFirstTail == -1) join = getFirstJoin();
    else join = getFirstJoin(preferredFirstTail);
  ...
  private StaticOptimizer.Join getFirstJoin() {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      for (int j = 0; j < arity; j++) {
        ...
        if (ASTStaticJoinOptimizer.log.isDebugEnabled())
          ASTStaticJoinOptimizer.log.debug(
              "evaluating " + i + " X " + j + ": cardinality= " + joinCardinality);
  ...
  private Tail getTail(int tailIndex) {
    if (tail[tailIndex] == null) {
      tail[tailIndex] = new Tail(tailIndex, rangeCount(tailIndex), getVars(tailIndex));
    }
   ...
   * of each join dimension. If there are shared variables but no unshared variables, then the
   ...
   *
   * <p>TODO: Any join involving an optional will have infinite cardinality, so that optionals get
   * placed at the end.
   *
   * @param d1 the first join dimension
  ...
  protected long computeJoinCardinality(IJoinDimension d1, IJoinDimension d2) {
    ...
    if (sharedVars == false) {
      ...
      if (unsharedVars == false) {
        ...
        joinCardinality =
            (long)
                ((long) (optimistic * Math.min(d1.getCardinality(), d2.getCardinality()))
  ...
  protected int getNextTailThatSharesVarsWithAncestry() {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (ASTStaticJoinOptimizer.log.isDebugEnabled()) {
        ASTStaticJoinOptimizer.log.debug("considering tail: " + nodes.get(i));
      }

      if (ASTStaticJoinOptimizer.log.isDebugEnabled()) {
        ASTStaticJoinOptimizer.log.debug("vars: " + Arrays.toString(getVars(i).toArray()));
      }
