...
public class ASTJoinGroupOrderOptimizer extends AbstractJoinGroupOptimizer
  ...
  protected void optimizeJoinGroup(
      AST2BOpContext ctx, StaticAnalysis sa, IBindingSet[] bSets, JoinGroupNode joinGroup) {
     ...
     * https://docs.google.com/document/d/1Fu0QLj1ML6CdaysREDFsJt8fT048zBWp0ssMDITAYt8 for a formal
     * explanation and justification of our approach.
     */
     ...
     *
     * <p>Note: we may want to pass in (and use) information about existing filters, which might be
     * valuable information for the optimizer.
     */
    if (reorderNodes) {
  ...
  void optimizeAcrossPartitions(
    ...
    for (int i = 1; i < partitionList.size(); i++) {
       ...
       * Calculate the conflicting vars as the intersection of the maybe vars of the bordering
          ...
          final GroupNodeVarBindingInfo candidateBindingInfo = bindingInfoMap.get(candidate);
          conflictingVars.retainAll(candidateBindingInfo.getMaybeProduced());

          conflictingVars.removeAll(definitelyProducedUpToPartition.get(j + 1));

          if (conflictingVars.isEmpty()
              && definitelyProducedUpToPartition
                  .get(j)
                  .containsAll(candidateBindingInfo.getRequiredBound())) {
            ...
            final ExplainHint explainHint =
                new JoinOrderExplainHint(
                    JoinOrderExplainHint.ACROSS_PARTITION_REORDERING_PROBLEM, candidate);
       ...
       * Given that the node has been moved to partitionForCandidate, the
           ...
           *
           * <p>Note that k<=i<partitions.size() guarantees that we don't run into an index out of
           * bound exception.
           */
          for (int k = partitionForCandidate + 1; k <= i; k++) {
            definitelyProducedUpToPartition
                .get(k)
                .addAll(bindingInfoMap.get(candidate).getDefinitelyProduced());
          }
  ...
  void optimizeWithinPartition(
        ...
        new ASTTypeBasedNodeClassifierConstraint() {
          ...
          boolean appliesTo(final IGroupMemberNode node) {
            ...
            if (node instanceof ServiceNode) {
              ...
              if (!sn.getResponsibleServiceFactory()
                  .equals(ServiceRegistry.getInstance().getDefaultServiceFactory())) {
                return true;
              }
     ...
     * can be satisified through this partition.
     */
    classifier.addConstraintForType(
     ...
     *
     * <p>Note: this is the place where we want to integrate the StaticOptimizer.
     */
     ...
     * the remainingToBePlaced constructs (e.g., a subsequent BIND node, as in the ticket example)
     * s.t. it is desirable to place the VALUES clause early on: this clears the way for placing
     * subsequent nodes earlier in the query execution plan.
     *
     * <p>We therefore check the variables in the remainingToBePlaced array for dependencies towards
     * variables introduced in our bindings clauses. If there are such dependencies, we place the
     * VALUES clauses at the first possible position, otherwise we choose the first contributing
     * position.
     */
    ...
    final List<IGroupMemberNode> orderedNodes =
        orderNodesByDependencies(
            nonValueNodesToBePlaced, partition.bindingInfoMap, knownBoundSomewhere);
   ...
   * patterns such as BIND(?x AS ?y) . BIND(?y AS ?x)). We may want to check the query for such
   * patterns statically and throw a pre-runtime exception, as the SPARQL 1.1 standard suggests.
   *
   * <p>Failing means we return an order that is not guaranteed to be "safe", which might give us
   * unexpected results in some exceptional cases. However, whenever the pattern is valid according
   * to the SPARQL 1.1 semantics, this method should return nodes in a valid order.
   *
   * @param bindNodes the list of BIND nodes to reorder
   * @param bindingInfoMap hash map for binding info lookup
   * @param knownBoundSomewhere variables that are known to be bound somewhere in the node list in
   *     which we want to place the BIND nodes
  ...
  List<IGroupMemberNode> orderNodesByDependencies(
    ...
    while (!toBePlaced.isEmpty()) {
       ...
       * The first condition is that the node can be safely placed. The second condition is a
         ...
         * condition).
         */
        if (nodeBindingInfo.leftToBeBound(knownBound).isEmpty() || i + 1 == toBePlaced.size()) {
