...
public class CacheConnectionImpl implements ICacheConnection {
   ...
   * #close()}.
   *
   * <p>Note: It is substantially easier to get the visiblity criteria correct when using the main
  ...
  public CacheConnectionImpl(final QueryEngine queryEngine) {
    ...
    properties.setProperty(
        org.embergraph.journal.Options.INITIAL_EXTENT, "" + (1 * Bytes.megabyte));
  ...
  public void destroyCaches(final String namespace, final long timestamp) {
    ...
    final IDescribeCache describeCache = getDescribeCache(namespace, timestamp);
   ...
   */
  @Override
  public IDescribeCache getDescribeCache(final String namespace, final long timestamp) {
    ...
    synchronized (this) {
      ...
      if (htree == null) {
        ...
        final HTreeIndexMetadata metadata = new HTreeIndexMetadata(name, UUID.randomUUID());
  ...
  /** The {@link InnerCacheJournal} provides the backing store for transient named solution sets. */
  private class InnerCacheJournal extends AbstractJournal {
    ...
    public DefaultResourceLocator getResourceLocator() {
      ...
      return (DefaultResourceLocator) getLocalIndexManager().getResourceLocator();
    }
    ...
    public ScheduledFuture<?> addScheduledTask(
      ...
      return getLocalIndexManager().addScheduledTask(task, initialDelay, delay, unit);
    }
