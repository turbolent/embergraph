...
public interface IEvaluationContext {
  ...
  /** Return <code>true</code> iff the target {@link AbstractTripleStore} is in quads mode. */
  boolean isQuads();

  /** Return <code>true</code> iff the target {@link AbstractTripleStore} is in SIDS mode. */
  boolean isSIDs();

  /** Return <code>true</code> iff the target {@link AbstractTripleStore} is in triples mode. */
  boolean isTriples();
   ...
   * transaction, in which case we need to use the last commit point in order to see any writes
   * which it may have performed (lexicon writes are always unisolated).
   */
