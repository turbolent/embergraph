 ...
 *
 * <p>This class corresponds to "VerbPath".
 *
 * <p>A VerbPath (PathNode) has one Path. VerbPath ::= Path
 *
 * <p>A Path has one PathAlternative. Path ::= PathAlt
 *
 * <p>A PathAlternative has one or more PathSequences. PathAlternative ::= PathSequence ( '|'
 * PathSequence )*
 *
 * <p>A PathSequence has one or more PathEltOrInverses. PathSequence ::= PathEltOrInverse ( '/'
 * PathEltOrInverse )*
 *
 * <p>A PathEltOrInverse has one PathElt and a boolean flag for inverse ('^'). PathEltOrInverse ::=
 * PathElt | '^' PathElt
 *
 * <p>A PathElt has a PathPrimary and an optional PathMod. PathElt ::= PathPrimary PathMod?
 *
 * <p>A PathPrimary has either an iri, a PathNegatedPropertySet, or a nested Path. PathPrimary ::=
 * iri | '!' PathNegatedPropertySet | '(' Path ')'
 *
 * <p>A PathMod is one from the enumeration '?', '*', or '+'. '?' means zero or one (simple
 * optional), '+' means one or more (fixed point), and '*' means zero or more (optional fixed
 * point). PathMod ::= '?' | '*' | '+'
 *
 * <p>A PathNegatedPropertySet is zero or more PathOneInPropertySets. PathNegatedPropertySet ::=
 * PathOneInPropertySet | '(' (PathOneInPropertySet ( '|' PathOneInPropertySet )* )? ')'
 *
 * <p>A PathOneInPropertySet is an iri and a boolean flag for inverse ('^'). PathOneInPropertySet
 * ::= iri | '^' iri
 *
 * <p>This model is actually flattened a bit by Sesame, so I followed Sesame's model instead of the
 * grammar. In Sesame's model, the top level is PathAlternative, which contains one or more
 * PathSequences. Each PathSequence contains one or more PathElt. Each PathElt has two modifiers -
 * the PathMod (for arbitrary length and zero length paths) and the inverse modifier. It also has
 * the actual element - one of either a TermNode, a nested path (a PathAlternative), a
 * NegatedPropertySet, or a zero length path.
 *
...
public class PathNode extends ASTBase {
  ...
  public static class PathElt extends ASTBase {
     ...
     *       <li>{@link PathAlternative}
     *       <li>{@link PathNegatedPropertySet}
     *       <li>{@link ZeroLengthPathNode}
     *           <ul>
     */
    public PathElt(final BOp arg, final boolean inverse, final PathMod mod) {
      this(
          new BOp[] {arg},
          NV.asMap(new NV(Annotations.INVERSE, inverse), new NV(Annotations.MOD, mod)));
