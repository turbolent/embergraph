...
import org.embergraph.rdf.sparql.ast.IGraphPatternContainer.Annotations;
 ...
 *
 * <p>Like the FILTER in which the (NOT EXISTS) appears (and like MINUS) the order in which graph
 * pattern must be evaluated is determined by the same general principles which govern the
 * attachment of FILTERs to joins. That is, when a variable on which the filter depends is bound by
 * a required join they must not run before that required join. And, when a variable on which the
 * filter depends is only bound by an optional join, then they must not run until after that
 * optional join.
 *
 * <p>For (NOT) EXISTS evaluation we need to bind the outcome of the graph pattern (that is, whether
 * or not the graph pattern is satisified) onto an anonymous variable (assigned by the SPARQL
 * parser). It is the truth state of that anonymous variable which is tested by the filter. This is
 * handled by special handshaking with the join in which we declare the anonymous variable to the
 * join (this is the "ASK_VAR") and project only that anonymous variable out of the join.
 *
 * <p>All variables which are in scope when the (NOT) EXISTS graph pattern is evaluated must be
 * projected into the subquery (since they must be visible to it), but the bindings produced by the
 * subquery (other than the anonymous variable indicating whether or not the graph pattern "exists")
 * must be discarded. Again, this is handled by projecting out only the "anonymous" variable and the
 * join variables (which are bound on entry to the join and are used within the (NOT) EXISTS graph
 * pattern). Other bindings are discarded.
 *
 * <p>The interpretation of the truth state of the variable by the FILTER provides the "exists" or
 * "not exists" semantics. The hash index containing the ASK VAR bindings must remain visible until
 * the corresponding FILTER has been evaluated.
 *
 * <p>Note: This rewrite must run relatively early to ensure that other optimizers are able to run
 * against the graph pattern once it has been lifted out of the (NOT) EXISTS onto a subquery.
 *
 * @see ExistsNode
 * @see NotExistsNode
...
public class ASTExistsOptimizer implements IASTOptimizer {
  ...
  public QueryNodeWithBindingSet optimize(
    ...
    if (queryRoot.getNamedSubqueries() != null) {
      ...
      for (NamedSubqueryRoot subqueryRoot : queryRoot.getNamedSubqueries()) {
        ...
        final GraphPatternGroup<IGroupMemberNode> whereClause = subqueryRoot.getWhereClause();
  ...
  private void rewrite(
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (child instanceof FilterNode) {
        ...
        if (vexp != null) {
          final Object gpGroup = child.get(0).getProperty(Annotations.GRAPH_PATTERN, null);
          if (gpGroup instanceof GraphPatternGroup) {
            rewrite(sa, exogenousVars, query, (GraphPatternGroup<IGroupMemberNode>) gpGroup);
          }
        }
      ...
      if (child instanceof GraphPatternGroup<?>) {
        ...
        // Recursion.
        rewrite(sa, exogenousVars, query, (GraphPatternGroup<IGroupMemberNode>) child);
      }
   ...
   *
   * @param p The group in which the filter was found (aka the parent).
   * @param filterOrAssignment The FILTER or BIND node in which an {@link ExistsNode} or {@link
   *     NotExistsNode} might appears.
   * @param ve Part of the value expression for that filter.
   */
  private void rewrite(
      final StaticAnalysis sa,
      final Set<IVariable<?>> exogenousVars,
      ...
      if (graphPattern != null) {
        ...
        if ((subqueryFunction instanceof ExistsNode)
          ...
          if (pipelinedHashJoinHint != null) {
            subquery.setQueryHint(QueryHints.PIPELINED_HASH_JOIN, pipelinedHashJoinHint);
          }
           ...
           */
          final LinkedHashSet<IVariable<?>> vars;
          {
            vars = new LinkedHashSet<>(exogenousVars);
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (child instanceof IValueExpressionNode) {
        ...
        // Recursion.
        rewrite(sa, exogenousVars, query, p, filterOrAssignment, (IValueExpressionNode) child);
      }
    }
  }
}
