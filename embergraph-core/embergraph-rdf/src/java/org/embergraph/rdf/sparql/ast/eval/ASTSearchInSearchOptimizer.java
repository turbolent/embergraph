...
public class ASTSearchInSearchOptimizer extends AbstractJoinGroupOptimizer {
  ...
  protected FunctionNode convert(final AST2BOpContext ctx, final FunctionNode searchInSearch) {
  ...
  protected IV[] getHits(
      final AST2BOpContext ctx, final Literal search, final String match, final String regex) {

    final ITextIndexer<IHit> textIndex =
        (ITextIndexer<IHit>) ctx.getAbstractTripleStore().getLexiconRelation().getSearchEngine();
