...
public class DataSetSummary {
   ...
   * access path will be queried for that graph. However, a non- {@link IRawTripleStore#NULL} term
   * identifier may also identify a graph which does not exist, in which case an access path will be
   ...
   *     in the quad store.
   * @param update When <code>true</code>, unknown {@link IV}s WILL NOT be pruned from the {@link
   *     DataSetSummary}. This is because those graphs might be implicitly created during an UPDATE
   *     operation.
   */
   ...
   *
   * @param context
   * @param limit The maximum #of samples to take.
   * @param pred The predicate.
   * @return The estimated cost report. This is adjusted based on the sample size and the #of graphs
   *     against which the query was issued and represents the total expected cost of the subqueries
   *     against all of the graphs in the {@link Dataset}.
   * @todo Subquery will be less efficient than a scan when the access path is remote since there
   *     will be remote requests. This model does not capture that additional overhead. We need to
   *     measure the overhead using appropriate data sets and queries and then build it into the
   *     model. The overhead itself could be changed dramatically by optimizations in the {@link
   *     FederatedQueryEngine} and the {@link ResourceService}.
  ...
  public SubqueryCostReport estimateSubqueryCost(
    ...
    for (IV graph : graphs) {
      ...
      final Predicate tmp = pred.asBound((IVariable) pred.get(3), new Constant(graph));
      ...
      rangeCount +=
          context.getAccessPath(context.getRelation(tmp), tmp).rangeCount(false /* exact */);
    ...
    return new SubqueryCostReport(nknown, limit, nsamples, rangeCount, subqueryCost);
  }

  @Override
  public String toString() {

    return "DataSetSummary{ngraphs="
        + graphs.size()
        + ", nknown="
        + nknown
        + ", nunknown="
  ...
  public boolean equals(final Object o) {
    ...
    if (!(o instanceof DataSetSummary)) return false;
