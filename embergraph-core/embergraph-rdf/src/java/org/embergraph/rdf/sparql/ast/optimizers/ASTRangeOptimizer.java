...
public class ASTRangeOptimizer extends AbstractJoinGroupOptimizer implements IASTOptimizer {
  ...
  protected void optimizeJoinGroup(
    ...
    for (IQueryNode node : group) {
      ...
      if (log.isDebugEnabled()) log.debug(filter);

      if (!(filter.getValueExpressionNode() instanceof FunctionNode)) continue;

      final FunctionNode function = (FunctionNode) filter.getValueExpressionNode();

      if (log.isDebugEnabled()) log.debug(function);
    ...
    for (IQueryNode node : group) {
      ...
      if (!sp.getQueryHintAsBoolean(QueryHints.RANGE_SAFE, false)) continue;

      if (!sp.o().isVariable()) continue;

      final VarNode v = (VarNode) sp.o();

      if (!ranges.containsKey(v)) continue;
  ...
  public static RangeBOp toRangeBOp(
      final BOpContextBase context, final RangeNode range, final GlobalAnnotations globals) {
    ...
    if (from != null) {
      ...
      final IValueExpression<? extends IV> ve = AST2BOpUtility.toVE(context, globals, from);
    ...
    if (to != null) {
      ...
      final IValueExpression<? extends IV> ve = AST2BOpUtility.toVE(context, globals, to);
  ...
  private void processFunction(final FunctionNode function, final Map<VarNode, RangeNode> ranges) {
  ...
  private void addLowerBoundIfConstant(
    ...
    if (from != null) range.setFrom(from);
  }
}
