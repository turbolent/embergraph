 ...
 * solution.
 *
 * <p>Note: This supports construct of quads, but the SPARQL grammar does not.
 *
...
public class ASTConstructIterator
   ...
   * <code>null</code> for a DESCRIBE query since we want to have consistent blank node ID
   * assigments for all statements in the response graph.
   *
   * <p>Note: DO NOT access this directly. Instead, use the {@link #getBNodeMap()} to set a
   * <strong>lexically local</strong> map reference and then clear that map reference to <code>null
   * </code> when you are done with a given solution. If the {@link #bnodes} reference is <code>null
   * </code>, then a <strong>new</strong> blank node map will be returned by every invocation of
   * {@link #getBNodeMap()}. If it is non- <code>null</code> then the <strong>same</strong> map is
   * returned by every invocation. This makes it possible to determine the scope of the blank nodes
   * simply by whether or not this map is supplied to the constructor.
   */
   ...
   *
   * <p>Note: It is possible to disable this filter (in the code) by setting it to <code>null</code>
   * .
   *
   * @see DistinctFilter
   ...
   * @param bnodes A mapping from bnode IDs to {@link EmbergraphBNode}s that will be imposed
   *     (optional). This MUST be <code>null</code> when the top-level operation is a CONSTRUCT
   *     since the semantics of CONSTRUCT <strong>require</strong> that blank node ID assignments
   *     are scoped to the solution. However, when a DESCRIBE query is rewritten into a CONSTRUCT,
   *     the blank node identifier mappings must be scoped to the response graph. For a DESCRIBE
   *     that uses a recursive expansion (CBD, etc.), the blank node IDs must be scoped to the
   *     entire response, not just the individual DESCRIBE queries that are issued in support of the
   *     top-level DESCRIBE request.
   * @param src The solutions that will be used to construct the triples.
   */
  public ASTConstructIterator(
    ...
    for (StatementPatternNode pat : construct) {
      ...
      if (pat.isGround()) {
        ...
        final EmbergraphStatement stmt = makeStatement(pat, EmptyBindingSet.getInstance(), bnodes);
        ...
        if (DEBUG) log.debug("Ground statement:\npattern=" + pat + "\nstmt=" + stmt);
  ...
  private IFilterTest createDistinctFilter(
      final AbstractTripleStore tripleStore,
      final ConstructNode construct,
      final GraphPatternGroup<?> whereClause) {
    /*
    ...
    final boolean distinctQuads =
        construct.isDistinctQuads() && tripleStore.isQuads() && hasMixedQuadData(templates);
    ...
    final boolean isObviouslyDistinct =
        isObviouslyDistinct(tripleStore.isQuads(), templates, whereClause);
   ...
   * distinct.
   *
   * @see <a href="https://jira.blazegraph.com/browse/BLZG-260">native distinct in quad mode
   *     (insert/delete) </a>
   */
   ...
   * that use case, the triples are (of necessity) already DISTINCT. Therefore, when there is a
   * single triple pattern in the WHERE clause and a single template in the CONSTRUCT, we DO NOT
   * impose a DISTINCT filter. This saves resources when the CONSTRUCTed graph is large.
   *
   * <p>Note: The specifics of the triple pattern in the where clause and the template do not matter
   * as long as the triple pattern in the WHERE clause can not introduce a cardinality which is
   * higher than the cardinality of the triples constructed by the template. This condition is
   * satisfied if all variables used in the triple pattern in the WHERE clause are also used in the
   * triple pattern in the template.
   *
   * <p>Note: The ground triples in the construct template are ignored for these purposes. We are
   * only concerned with disabling the DISTINCT filter when the #of duplicate triples must be zero
   * or near zero, not with providing a 100% guarantee that there are no duplicate triples in the
   * CONSTRUCT output.
   *
   * @param quads <code>true</code> iff the database is in quads mode.
   * @param templates The non-ground triple templates.
   * @param whereClause The WHERE clause for the query.
   * @return <code>true</code> iff the construct will obviously produce a graph containing distinct
   *     triples.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/579">CONSTRUCT should apply
   *     DISTINCT (s,p,o) filter </a>
   */
  ...
  static boolean isObviouslyDistinct(
    ...
    final StatementPatternNode sp2 = (StatementPatternNode) whereClause.get(0);
   ...
   *     <p>FIXME NATIVE DISTINCT: This method needs to be vectored for native distinct.
   * @see <a href="https://jira.blazegraph.com/browse/BLZG-260">native distinct in quad mode
   *     (insert/delete) </a>
   */
  private void addStatementToBuffer(final EmbergraphStatement stmt) {

    if (DEBUG) log.debug(stmt.toString());
   ...
   * this solution.
   *
   * @param pat A statement pattern from the construct template.
   * @param solution A solution from the query.
   * @param bnodes A map used to scope blank nodes to the solution.
   * @return A statement if a valid statement could be constructed for that statement pattern and
   *     this solution.
   */
  private EmbergraphStatement makeStatement(
      final StatementPatternNode pat,
    ...
    final EmbergraphValue c = pat.c() == null ? null : getValue(pat.c(), solution, bnodes);

    // filter out unbound values.
    if (s == null || p == null || o == null) return null;

    // filter out bindings which do not produce legal statements.
    if (!(s instanceof Resource)) return null;
    if (!(p instanceof URI)) return null;
    if (!(o instanceof Value)) return null;
  ...
  private EmbergraphValue getValue(
      final TermNode term, final BindingSet solution, final Map<String, EmbergraphBNode> bnodes) {
    ...
    if (term instanceof ConstantNode) {
      ...
      if (value == null) {
         ...
         *
         * <p>This problem was first observed when developing CBD support. The CBD class is
         * explicitly setting the IVs when it builds the DESCRIBE queries for the CBD expansions.
         * However, it was failing to resolve the EmbergraphValue for those IVs and set the
         * valueCache on the IV. This assert was added to detect that problem and prevent
         * regressions.
         *
         * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/578">Concise Bounded
         *     Description </a>
         */
        throw new AssertionError(
            "EmbergraphValue not available: "
                + term
                + ", term.iv="
                + term.getValueExpression().get());
      }
   ...
   */
  private EmbergraphBNode getBNode(final String id, final Map<String, EmbergraphBNode> bnodes) {
    ...
    final EmbergraphBNode bnode = f.createBNode("b" + Integer.valueOf(bnodeIdFactory++).toString());
