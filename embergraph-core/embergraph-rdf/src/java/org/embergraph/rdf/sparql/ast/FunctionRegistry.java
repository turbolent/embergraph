...
import org.embergraph.rdf.internal.constraints.ConcatBOp;
import org.embergraph.rdf.internal.constraints.DatatypeBOp;
import org.embergraph.rdf.internal.constraints.DateBOp;
import org.embergraph.rdf.internal.constraints.DateBOp.DateOp;
import org.embergraph.rdf.internal.constraints.DigestBOp;
import org.embergraph.rdf.internal.constraints.DigestBOp.DigestOp;
import org.embergraph.rdf.internal.constraints.EncodeForURIBOp;
import org.embergraph.rdf.internal.constraints.FalseBOp;
import org.embergraph.rdf.internal.constraints.FuncBOp;
import org.embergraph.rdf.internal.constraints.IfBOp;
import org.embergraph.rdf.internal.constraints.InHashBOp;
import org.embergraph.rdf.internal.constraints.IriBOp;
import org.embergraph.rdf.internal.constraints.IsBNodeBOp;
import org.embergraph.rdf.internal.constraints.IsBoundBOp;
import org.embergraph.rdf.internal.constraints.IsLiteralBOp;
import org.embergraph.rdf.internal.constraints.IsNumericBOp;
import org.embergraph.rdf.internal.constraints.IsURIBOp;
import org.embergraph.rdf.internal.constraints.LangBOp;
import org.embergraph.rdf.internal.constraints.LangMatchesBOp;
import org.embergraph.rdf.internal.constraints.LcaseBOp;
...
import org.embergraph.rdf.internal.constraints.NowBOp;
import org.embergraph.rdf.internal.constraints.NumericBOp;
import org.embergraph.rdf.internal.constraints.NumericBOp.NumericOp;
import org.embergraph.rdf.internal.constraints.OrBOp;
import org.embergraph.rdf.internal.constraints.RandBOp;
import org.embergraph.rdf.internal.constraints.RegexBOp;
import org.embergraph.rdf.internal.constraints.ReplaceBOp;
...
import org.embergraph.rdf.internal.constraints.StrAfterBOp;
import org.embergraph.rdf.internal.constraints.StrBOp;
import org.embergraph.rdf.internal.constraints.StrBeforeBOp;
import org.embergraph.rdf.internal.constraints.StrcontainsBOp;
import org.embergraph.rdf.internal.constraints.StrdtBOp;
import org.embergraph.rdf.internal.constraints.StrendsBOp;
import org.embergraph.rdf.internal.constraints.StrlangBOp;
import org.embergraph.rdf.internal.constraints.StrlenBOp;
import org.embergraph.rdf.internal.constraints.StrstartsBOp;
import org.embergraph.rdf.internal.constraints.SubstrBOp;
 ...
 *     functions</a>
 *     <p>TODO We should restrict registration of functions in the {@link BD#NAMESPACE} to static
 *     initialization. The code already does not permit replacement of registered functions.
 */
...
public class FunctionRegistry {
  ...
  public static final String SPARQL10_UNDEFINED_FUNCTIONS =
      "http://www.embergraph.org/sparql-1.0-undefined-functions";
  ...
  public static final String SPARQL11_UNDEFINED_FUNCTIONS =
      "http://www.embergraph.org/sparql-1.1-undefined-functions";
  ...
  public static final URI ENCODE_FOR_URI =
      FN.ENCODE_FOR_URI; // new URIImpl(SPARQL_FUNCTIONS+"encodeForUri");
  public static final URI STR_LEN =
      FN.STRING_LENGTH; // new URIImpl(XPATH_FUNCTIONS+"string-length");
   ...
   * href="http://www.w3.org/TR/sparql11-query/#SparqlOps">SPARQL 1.1 Query Language for RDF</a>.
   */
  public static final URI NOT_EXISTS = new URIImpl(SPARQL11_UNDEFINED_FUNCTIONS + "not-exists");
  ...
  public static final URI ROUND =
      FN.NUMERIC_ROUND; // new URIImpl(SPARQL_FUNCTIONS+"numeric-round");
  public static final URI CEIL = FN.NUMERIC_CEIL; // new URIImpl(SPARQL_FUNCTIONS+"numeric-ceil");
  public static final URI FLOOR =
      FN.NUMERIC_FLOOR; // new URIImpl(SPARQL_FUNCTIONS+"numeric-floor");
   ...
   * @param types The type constraints. If there are more arguments given than constraints, then the
   ...
   * @param scalarValues Scalar values for the function (optional). This is used for things like the
   *     <code>separator</code> in GROUP_CONCAT.
  ...
  public static final IValueExpression<? extends IV> toVE(
    ...
    if (functionURI == null) throw new IllegalArgumentException("functionURI is null");
   ...
   * @return The factory -or- <code>null</code> if there was no function registered for that {@link
   *     URI}.
   */
  ...
  public interface Factory {
     ...
     *     This is used if we need to statically evaluate an {@link IValueExpression} during query
     *     optimization. During query execution the {@link ContextBindingSet} will convey this
     *     information.
     * @param globals The global annotations, including the lexicon namespace.
     * @param scalarValues The scalar arguments (used by some {@link IAggregate}s).
     * @param args The function arguments.
     * @return The {@link IValueExpression}.
     * @see BLZG-1372 create() was refactored to pass in the {@link BOpContextBase} to allow correct
  ...
  public static class CompareFactory implements Factory {
    ...
    public IValueExpression<? extends IV> create(
      ...
      final IValueExpression<? extends IV> left = AST2BOpUtility.toVE(context, globals, args[0]);

      final IValueExpression<? extends IV> right = AST2BOpUtility.toVE(context, globals, args[1]);
      ...
      if (!(op == CompareOp.EQ || op == CompareOp.NE)) {
        ...
        if (left instanceof Constant && ((Constant<? extends IV>) left).get().isURI()) {

          return SparqlTypeErrorBOp.INSTANCE;
        }

        if (right instanceof Constant && ((Constant<? extends IV>) right).get().isURI()) {
  ...
  public static class CastFactory implements Factory {
    ...
    public IValueExpression<? extends IV> create(
      ...
      final IValueExpression<? extends IV>[] bops = new IValueExpression[args.length];
  ...
  public static class GroupConcatFactory implements AggregateFactory {
    ...
    public IValueExpression<? extends IV> create(
      ...
      return new org.embergraph.bop.rdf.aggregate.GROUP_CONCAT(new BOp[] {ve}, tmp);
    }
  }
   ...
   *
   * <p>This optimizes IN/0 (foo IN()) as FALSE.
   *
   * <p>This optimizes IN/1 (foo IN(valueExpr) as SameTerm.
   *
   * <p>This optimizes expr IN Constants using a hash set over the constants.
   */
  public static class InFactory implements Factory {
    ...
    /** @param not <code>true</code> iff this is NOT IN. */
    ...
    public IValueExpression<? extends IV> create(
       ...
       * MP: Changed this to check for allowLiterals.  When we are allowing
        ...
        if (allowLiterals) {
          ...
          final IValueExpression ret =
              SameTermFactory.INSTANCE.create(context, globals, scalarValues, args);

          if (not) return new NotBOp(ret);
        ...
        final IValueExpression ret =
            new CompareFactory(CompareOp.EQ).create(context, globals, scalarValues, args);
        ...
        try {
          ...
          for (int i = 1; i < args.length; i++) {
            ...
            if (ret == null) {
              ...
              ret = compare.create(context, globals, scalarValues, args[0], args[i]);

            } else {

              ret =
                  new OrBOp(ret, compare.create(context, globals, scalarValues, args[0], args[i]));
            }
          }
   ...
   * variable for <code>false</code>. The name of that anonymous variable is the sole {@link
   * ValueExpressionNode} passed into this factory.
   *
   * <p>The "guts" of the EXISTS logic is translated by an {@link IASTOptimizer} into an ASK
   * subquery (an instance of {@link SubqueryRoot}). This will be executed as a special BOp that
   * bind the success or failure of that ASK subquery on the anonymous variable. The "ASK" subquery
   * does not fail, it just binds the truth state of the ASK subquery on the anonymous variable. The
   * {@link IValueExpression} returned by this factory just tests the truth state of that anonymous
   * variable.
   */
