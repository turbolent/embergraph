 ...
 *
 * <ul>
 *   <li>Lift out SPARQL 1.1 subqueries which use both LIMIT and ORDER BY. Due to the interaction of
 *       the LIMIT and ORDER BY clause, these subqueries MUST be run first since they can produce
 *       different results if they are run "as-bound".
 *   <li>Lift out SPARQL 1.1 subqueries involving aggregates. This typically provides more efficient
 *       evaluation than repeated as-bound evaluation of the sub-select. It also prevents
 *       inappropriate sharing of the internal state of the {@link IAggregate} functions.
 *   <li>Lift out SPARQL 1.1 subqueries if there are no incoming bound variables which are also
 *       projected by the subquery. Such subqueries must be lifted or we will simply be doing the
 *       same work over and over since no bindings will be projected into the subquery.
 *   <li>Lift out SPARQL 1.1 subqueries if {@link SubqueryRoot#isRunOnce()} is <code>true</code>.
 * </ul>
 *
 * FIXME The code to lift out sub-selects if there are no join variables has been disabled since we
 * are not correctly computing the join variables at this point. Deciding the join variables
 * requires that we either apply heuristics or the RTO to decide on a join ordering. Once the join
 * order is known we can then recognize which variables will be definitely bound at the point in the
 * join group where the subquery would be evaluated.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: ASTSparql11SubqueryOptimizer.java 5193 2011-09-15 14:18:56Z thompsonbry $
 */
public class ASTSparql11SubqueryOptimizer implements IASTOptimizer {
  ...
  public static boolean needsLifting(final SubqueryRoot subqueryRoot) {
    if (subqueryRoot == null) return false;
