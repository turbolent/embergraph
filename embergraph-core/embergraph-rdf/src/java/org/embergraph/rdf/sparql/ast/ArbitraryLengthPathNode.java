 ...
 */
public class ArbitraryLengthPathNode extends GroupMemberNodeBase<ArbitraryLengthPathNode>
  ...
  public ArbitraryLengthPathNode(
      final TermNode left,
      final TermNode right,
      final VarNode tVarLeft,
      final VarNode tVarRight,
      final PathMod mod) {
    this(
        new BOp[] {new JoinGroupNode()},
        NV.asMap(
  ...
  /** Set the edge var and middle term. Only used by the ALP service when projecting edges. */
  ...
  private void addUsedVar(final TermNode t, final Set<IVariable<?>> vars) {

    addVar(t, vars, true);
  }

  private void addProducedBinding(final TermNode t, final Set<IVariable<?>> producedBindings) {
   ...
   */
  private void addVar(
      final TermNode t, final Set<IVariable<?>> producedBindings, final boolean addAnonymous) {
  ...
  public long getEstimatedCardinality(final StaticOptimizer opt) {
     ...
     */
    final long groupCard =
        group.getProperty(AST2BOpBase.Annotations.ESTIMATED_CARDINALITY, Long.MAX_VALUE);
    ...
    if (group.arity() == 1 && upperBound() >= 1 && upperBound() < Long.MAX_VALUE) {
      ...
      if (pathExpr != null
          && pathExpr.getProperty(AST2BOpBase.Annotations.ESTIMATED_CARDINALITY) != null) {
     ...
     */
    final Iterator<BOp> it = BOpUtility.preOrderIteratorWithAnnotations(group);
    ...
    while (it.hasNext()) {
      ...
      final long estCard =
          sp.getProperty(AST2BOpBase.Annotations.ESTIMATED_CARDINALITY, Long.MAX_VALUE);
