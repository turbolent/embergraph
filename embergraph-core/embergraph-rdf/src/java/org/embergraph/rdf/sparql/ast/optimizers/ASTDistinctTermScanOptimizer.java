 ...
 * scan. If the access path would be evaluated as-bound with at least one variable bound, then the
 * distinct term scan might not have any advantage over the pipeline join (and the semantics of
 * DISTINCT would be violated with multiple as-bound evaluations of the distinct-term-scan without a
 * hash index to impose the DISTINCT constraint).
 *
 * <p>TODO We are doing something very similar for <code>GRAPH ?g {}</code>. It would be worth while
 * to look at that code in the light of this optimizer.
 *
...
public class ASTDistinctTermScanOptimizer implements IASTOptimizer {
  ...
  private void doRecursiveRewrite(
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (child instanceof GraphPatternGroup<?>) {
        ...
        doRecursiveRewrite(context, sa, queryRoot, ((GraphPatternGroup<IGroupMemberNode>) child));

      } else if (child instanceof SubqueryRoot) {

        // Recursion into subqueries.
        final SubqueryRoot subqueryRoot = (SubqueryRoot) child;
        doRecursiveRewrite(context, sa, queryRoot, subqueryRoot.getWhereClause());
  ...
  private void doSelectQuery(
    ...
    IKeyOrder<ISPO> keyOrder = getApplicableKeyOrderIfExists(sp, projectedVar, context);
    if (keyOrder == null) {
      return;
    }
     ...
     */
    final Long oldCard = (Long) sp.getProperty(AST2BOpBase.Annotations.ESTIMATED_CARDINALITY);

    if (oldCard == null) {
      throw new AssertionError("Expecting estimated-cardinality to be bound: sp=" + sp);
    }
  ...
  private IKeyOrder<ISPO> getApplicableKeyOrderIfExists(
    ...
    if (isQuads) {
      args[3] =
          sp.c() == null ? Var.var("--anon-" + context.nextId()) : sp.c().getValueExpression();
    }

    Set<SPOKeyOrder> candidateKeyOrder = getCandidateKeyOrders(sp, termScanVar, context, isQuads);
  ...
  private Set<SPOKeyOrder> getCandidateKeyOrders(
    ...
    if (isQuads) {
      if (sp.c() == null || sp.c().getValueExpression() == null) {
        ...
        int pcC = getPositionConstraint(sp.c().getValueExpression(), termScanVar);
     ...
     *
     * <p>See https://jira.blazegraph.com/browse/BLZG-1346.
     */
  ...
  private void getPermutations(String prefix, String str, Set<String> collector) {
    ...
    if (n == 0) collector.add(prefix);
      ...
      for (int i = 0; i < n; i++)
        getPermutations(
            prefix + str.charAt(i), str.substring(0, i) + str.substring(i + 1, n), collector);
    }
  }
}
