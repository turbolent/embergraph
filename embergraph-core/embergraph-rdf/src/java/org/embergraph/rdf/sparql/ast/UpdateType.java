...
public enum UpdateType {
   ...
   * </pre>
   *
   * Note: Embergraph does not support empty graphs, so {@link #Clear} and {@link #Drop} have the
   * same semantics.
   *
   ...
   * graph. Data from the input graph is not affected, but data from the destination graph, if any,
   ...
   * graph. The input graph is removed after insertion and data from the destination graph, if any,
   * is removed before insertion.
   *
   * <pre>
  ...
  /*
   * The DROP ENTAILMENTS operation drop the entailments. This is only required if you have removed
   * some statements from the database.
   */
  DropEntailments(false /* graphUpdate */),

  /*
   * The CREATE ENTAILMENTS operation compute the entailments using an efficient "database-at-once"
   * closure operation.
   */
  CreateEntailments(false /* graphUpdate */),

  /** The DISABLE ENTAILMENTS operation can be used to disable incremental truth maintenance. */
