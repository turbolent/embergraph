 ...
 * @version $Id$
 *     <p>TODO Implement {@link IValueExpressionMetadata} so we can report the aggregated variables
 *     across the filters which need to be materialized?
 */
public class HavingNode extends ValueExpressionListBaseNode<IValueExpressionNode> {
  ...
  public IConstraint[] getConstraints() {
    ...
    for (IValueExpressionNode node : this) {
      ...
      exprs[i++] =
          new SPARQLConstraint<XSDBooleanIV<EmbergraphLiteral>>(node.getRequiredValueExpression());
    }
