...
import org.embergraph.bop.rdf.update.ChunkedResolutionOp;
import org.embergraph.bop.rdf.update.CommitOp;
import org.embergraph.bop.rdf.update.InsertStatementsOp;
import org.embergraph.bop.rdf.update.ParseOp;
import org.embergraph.bop.rdf.update.RemoveStatementsOp;
import org.embergraph.rdf.error.SparqlDynamicErrorException.GraphEmptyException;
import org.embergraph.rdf.error.SparqlDynamicErrorException.GraphExistsException;
import org.embergraph.rdf.error.SparqlDynamicErrorException.SolutionSetDoesNotExistException;
import org.embergraph.rdf.error.SparqlDynamicErrorException.SolutionSetExistsException;
...
import org.embergraph.rdf.sparql.ast.CopyGraph;
...
import org.embergraph.rdf.sparql.ast.MoveGraph;
...
public class AST2BOpUpdate extends AST2BOpUtility {
   ...
   * EmbergraphSail} API.
   *
   * <p>TODO By coming in through the SAIL, we automatically pick up truth maintenance and related
   * logics. All of that needs to be integrated into the generated physical operator plan before we
   * can run updates on the query engine. However, there will be advantages to running updates on
   * the query engine, including declarative control of parallelism, more similarity for code paths
   * between a single machine and cluster deployments, and a unified operator model for query and
   * update evaluation.
   */
  ...
  /** The s,p,o, and c variable names used for binding sets which model {@link Statement}s. */
   ...
   * @throws Exception
   */
  protected static PipelineOp convertUpdate(final AST2BOpUpdateContext context) throws Exception {

    if (context.db.isReadOnly()) throw new UnsupportedOperationException("Not a mutable view.");

    if (context.conn.isReadOnly()) throw new UnsupportedOperationException("Not a mutable view.");
    ...
    for (Update op : updateRoot) {
      ...
      if (updateIndex > 0) {
        ...
        ASTDeferredIVResolution.resolveUpdate(
            context.db, op, context.getQueryBindingSet(), context.getDataset());
      ...
      final DeleteInsertWhereStats deleteInsertWhereStats =
          new DeleteInsertWhereStats(); // @see BLZG-1446.
      ...
      try {
        ...
        context
            ...
            .getSailConnection()
            .fireEvent(
                new SPARQLUpdateEvent(
                    op,
                    elapsed,
                    connectionFlushNanos,
                    batchResolveNanos,
                    cause,
   ...
   *
   * <p>Note: Not required unless the end of the update sequence or we desire a checkpoint on the
   * sequences of operations.
   *
   * <p>Note: The commit really must not happen until the update plan(s) are known to execute
   * successfully. We can do that with an AT_ONCE annotation on the {@link CommitOp} or we can just
   * invoke commit() at appropriate checkpoints in the UPDATE operation.
   */
  private static PipelineOp convertCommit(PipelineOp left, final AST2BOpUpdateContext context)
      throws Exception {
    ...
    if (AUTO_COMMIT) {
      ...
      if (runOnQueryEngine) {
        ...
        left =
            new CommitOp(
                ...
                NV.asMap(
                    ...
                    new NV(CommitOp.Annotations.TIMESTAMP, context.getTimestamp()),
        ...
        if (log.isDebugEnabled()) log.debug("COMMIT: commitTime=" + commitTime);
      }
    }
   ...
   * @throws Exception
   */
  private static PipelineOp convertUpdateSwitch(
      PipelineOp left,
      final Update op,
    ...
    switch (updateType) {
      ...
      case DeleteData:
        {
          left = convertInsertOrDeleteData(left, (AbstractGraphDataUpdate) op, context);
          break;
        }
      case Load:
        {
   ...
   */
  private static PipelineOp convertDeleteInsert(
      PipelineOp left,
     ...
     */
    {
       ...
       */
      final JoinGroupNode whereClause = new JoinGroupNode(op.getWhereClause());
      ...
      if (dataset != null) queryRoot.setDataset(dataset);
       ...
       */
      astContainer.setProperty(
          ASTContainer.Annotations.RESOLVED,
          context.astContainer.getProperty(ASTContainer.Annotations.RESOLVED));
      ...
      if (insertClause == null && op.getDeleteClause() == null) {
        ...
        final Iterator<StatementPatternNode> itr =
            BOpUtility.visitAll(whereClause, StatementPatternNode.class);

        while (itr.hasNext()) {

          final StatementPatternNode t = (StatementPatternNode) itr.next().clone();

          deleteTemplate.addChild(t);
        }

        final QuadsDataOrNamedSolutionSet deleteClause =
            new QuadsDataOrNamedSolutionSet(deleteTemplate);
      ...
      if (isDeleteInsert) {
        ...
        final LexiconRelation lexicon = context.getAbstractTripleStore().getLexiconRelation();
        ...
        final MutableTupleQueryResult result =
            new MutableTupleQueryResult(
        ...
        try {
          ...
          if (deleteClause != null) {
            ...
            if (isSolutionSet) {
              ...
              context.solutionSetManager.putSolutions(
                  tempSolutionSet, asEmbergraphIterator(lexicon, chunkSize, result));
              ...
              try {
                ...
                final JoinGroupNode minusOp =
                    new JoinGroupNode(new NamedSubqueryInclude(tempSolutionSet));
              ...
              final ConstructNode template =
                  op.getDeleteClause().getQuadData().flatten(new ConstructNode(context));
              ...
              final ASTConstructIterator itr =
          ...
          if (insertClause != null) {
            ...
            if (isSolutionSet) {
              ...
              final ICloseableIterator<IBindingSet[]> titr =
                  asEmbergraphIterator(lexicon, chunkSize, result);
              ...
              final ConstructNode template =
                  op.getInsertClause().getQuadData().flatten(new ConstructNode(context));
        ...
        final QuadsDataOrNamedSolutionSet clause = isInsert ? insertClause : deleteClause;
        ...
        if (isSolutionSet) {
           ...
           * isolated by a transaction.
           */
          final ICloseableIterator<IBindingSet[]> result =
              ASTEvalHelper.evaluateTupleQuery2(
                  context.conn.getTripleStore(),
                  astContainer,
                  context.getQueryBindingSet() /* bindingSets */,
          ...
          final QuadData quadData =
              (insertClause == null ? deleteClause : insertClause).getQuadData();

          // Flatten the original WHERE clause into a CONSTRUCT
          // template.
          final ConstructNode template = quadData.flatten(new ConstructNode(context));
          ...
          try {
            ...
            while (result.hasNext()) {
              ...
              final EmbergraphStatement stmt = (EmbergraphStatement) result.next();

              addOrRemoveStatement(context.conn.getSailConnection(), stmt, isInsertOnly);
            }
   ...
   *
   * @param r The {@link LexiconRelation}.
   * @param chunkSize When converting the openrdf binding set iteration pattern into a chunked
   *     iterator pattern, this will be the target chunk size.
   * @param result The openrdf solutions.
   * @return An iterator visiting chunked embergraph solutions.
   *     <p>TODO We should not have to do this. We should stay within native embergraph
   *     IBindingSet[]s and the native embergraph iterators
   */
   ...
   * </pre>
   *
   * TODO If there is a BIND() to a constant in the SELECT expression, then this transform will not
   * capture that binding. We would also need to process the PROJECTION and pull down any constants
   * into BIND()s in the WHERE clause.
   */
  private static void convertQueryForDeleteSolutions(
      final QueryRoot queryRoot, final String solutionSet) {

    final JoinGroupNode oldWhereClause = (JoinGroupNode) queryRoot.getWhereClause();
   ...
   * @throws RepositoryException
   */
  private static PipelineOp convertAddGraph(
      PipelineOp left, final AddGraph op, final AST2BOpUpdateContext context)
      throws RepositoryException {

    if (runOnQueryEngine) throw new UnsupportedOperationException();

    final EmbergraphURI sourceGraph =
        (EmbergraphURI) (op.getSourceGraph() == null ? null : op.getSourceGraph().getValue());

    final EmbergraphURI targetGraph =
        (EmbergraphURI) (op.getTargetGraph() == null ? null : op.getTargetGraph().getValue());
   ...
   * SHOULD/MAY report an error if the source graph does not exist (the spec is not consistent here
   * across those operations). Further, there is no explicit create/drop of graphs in embergraph so
   * it WOULD be Ok if we just ignored the SILENT keyword.
   */
  private static void copyStatements(
      final AST2BOpUpdateContext context,
    ...
    if (log.isDebugEnabled())
      log.debug("sourceGraph=" + sourceGraph + ", targetGraph=" + targetGraph);
    ...
    final RepositoryResult<Statement> result =
        context.conn.getStatements(
            null /* s */, null /* p */, null /* o */, context.isIncludeInferred(), sourceGraph);
   ...
   * @throws SailException
   */
  private static PipelineOp convertMoveGraph(
      PipelineOp left, final MoveGraph op, final AST2BOpUpdateContext context)
    ...
    final EmbergraphURI sourceGraph =
        (EmbergraphURI)
            (op.getSourceGraph() == null
                ? context.f.asValue(BD.NULL_GRAPH)
                : op.getSourceGraph().getValue());

    final EmbergraphURI targetGraph =
        (EmbergraphURI)
            (op.getTargetGraph() == null
                ? context.f.asValue(BD.NULL_GRAPH)
   ...
   * @throws SailException
   */
  private static PipelineOp convertCopyGraph(
      PipelineOp left, final CopyGraph op, final AST2BOpUpdateContext context)
      throws RepositoryException, SailException {
   ...
   * @throws Exception
   */
  private static PipelineOp convertLoadGraph(
      PipelineOp left, final LoadGraph op, final AST2BOpUpdateContext context) throws Exception {
    ...
    if (!runOnQueryEngine) {
      ...
      try {
        ...
        final EmbergraphURI defaultContext =
            (EmbergraphURI) (op.getTargetGraph() == null ? null : op.getTargetGraph().getValue());

        if (log.isDebugEnabled())
          log.debug("sourceURI=" + urlStr + ", defaultContext=" + defaultContext);

        // Take overrides from LOAD request, defaults from triple store and fall back to static
        // defaults.
        ...
        final boolean verifyData =
            Boolean.parseBoolean(
                op.getProperty(
                    LoadGraph.Annotations.VERIFY_DATA,
                    p.getProperty(
                        RDFParserOptions.Options.VERIFY_DATA,
                        RDFParserOptions.Options.DEFAULT_VERIFY_DATA)));

        final boolean preserveBlankNodeIDs =
            Boolean.parseBoolean(
                op.getProperty(
                    LoadGraph.Annotations.PRESERVE_BLANK_NODE_IDS,
                    p.getProperty(
        ...
        final boolean stopAtFirstError =
            Boolean.parseBoolean(
                op.getProperty(
                    LoadGraph.Annotations.STOP_AT_FIRST_ERROR,
                    p.getProperty(
        ...
        final DatatypeHandling dataTypeHandling =
            DatatypeHandling.valueOf(
                op.getProperty(
                    LoadGraph.Annotations.DATA_TYPE_HANDLING,
                    p.getProperty(
        ...
        final RDFParserOptions parserOptions =
            new RDFParserOptions(
                verifyData, preserveBlankNodeIDs, stopAtFirstError, dataTypeHandling);
        ...
        final String msg = "Could not load: url=" + urlStr + ", cause=" + t;
     ...
     */
    {
      ...
      if (op.isSilent()) anns.put(ParseOp.Annotations.SILENT, true);
   ...
   *     <p>TODO See {@link ParseOp} for a significantly richer pipeline operator which will parse a
   *     document. However, this method is integrated into all of the truth maintenance mechanisms
   *     in the Sail and is therefore easier to place into service.
   */
  private static void doLoad(
    ...
    final Resource defactoContext =
        defaultContext == null ? new URIImpl(sourceURL.toExternalForm()) : defaultContext;
    ...
    try {
       ...
       * parse MIMEType when doing conneg. </a>
       */
      ...
      if (format == null) throw new UnknownContentTypeException(contentType);

      final RDFParserFactory rdfParserFactory = RDFParserRegistry.getInstance().get(format);

      if (rdfParserFactory == null) throw new UnknownContentTypeException(contentType);
      ...
      rdfParser.setRDFHandler(new AddStatementHandler(conn, nmodified, defactoContext, op));
  ...
  private static class AddStatementHandler extends RDFHandlerBase {
    ...
    public void handleStatement(final Statement stmt) throws RDFHandlerException {
      ...
      if ((nparsed % 10000) == 0L) {
        ...
        // notify listener(s)
        conn.fireEvent(new SPARQLUpdateEvent.LoadProgress(op, elapsed, nparsed, false /* done */));
      }
    }
    ...
    public void endRDF() throws RDFHandlerException {
      ...
      // notify listener(s)
      conn.fireEvent(new SPARQLUpdateEvent.LoadProgress(op, elapsed, nparsed, true /* done */));
    }
  }
   ...
   * @throws SailException
   */
  private static PipelineOp convertClearOrDropGraph(
      PipelineOp left, final DropGraph op, final AST2BOpUpdateContext context)
      throws RepositoryException, SailException {
    ...
    final EmbergraphURI targetGraph =
        targetGraphNode == null ? null : (EmbergraphURI) targetGraphNode.getValue();

    clearGraph(
        op.isSilent(),
        op.getTargetSolutionSet(),
        targetGraph,
        op.getScope(),
        op.isAllGraphs(),
        op.isAllSolutionSets(),
        context);
   ...
   * @param context The {@link AST2BOpUpdateContext} used to perform the operation.
   * @throws RepositoryException
   * @throws SailException
   */
  private static final void clearOneGraph(final URI targetGraph, final AST2BOpUpdateContext context)
    ...
    clearGraph(
        true /* silent */,
        null /* targetSolutionSet */,
        targetGraph,
        null /* scope */,
        false /* allGraphs */,
        false /* allSolutionSets */,
        context);
  }
   ...
   *     caller.
   * @param targetSolutionSet The target solution set to be cleared -or- <code>null</code> if no
   *     target solution set was named.
   * @param targetGraph The graph to be cleared -or- <code>null</code> if no target graph was named.
   * @param scope The scope iff just the graphs in either the {@link Scope#DEFAULT_CONTEXTS} or
   *     {@link Scope#NAMED_CONTEXTS} should be cleared and otherwise <code>null</code>.
   * @param allGraphs iff all graphs should be cleared.
   * @param allSolutionSets iff all solution sets should be cleared.
   * @param context The {@link AST2BOpUpdateContext} used to perform the operation.
   * @throws RepositoryException
   * @throws SailException
   */
  // CLEAR/DROP ( SILENT )? (GRAPH IRIref | DEFAULT | NAMED | ALL | GRAPHS | SOLUTIONS | SOLUTIONS
  // %VARNAME)
  private static void clearGraph(
    ...
    if (log.isDebugEnabled()) log.debug("targetGraph=" + targetGraph + ", scope=" + scope);
    ...
    if (targetGraph != null) {
      ...
      sailConn.removeStatements(null /* s */, null /* p */, null /* o */, targetGraph);
    }
      ...
      if (scope == Scope.DEFAULT_CONTEXTS) {
        ...
        try {
          ...
          while (result.hasNext()) {
            ...
            sailConn.removeStatements(null /* s */, null /* p */, null /* o */, c);
          }
  ...
  private static PipelineOp convertDropEntailments(
      final PipelineOp left, final AST2BOpUpdateContext context) throws SailException {
  ...
  private static PipelineOp convertDisableEntailments(
      PipelineOp left, AST2BOpUpdateContext context) {
  ...
  private static PipelineOp convertCreateEntailments(PipelineOp left, AST2BOpUpdateContext context)
      throws SailException {
    ...
    if (log.isDebugEnabled()) {
      long inferredCount =
          context.conn.getSailConnection().getTripleStore().getStatementCount(true) - stmtCount;
   ...
   *
   * <p>SOLUTIONS : If the named solution set already exists (is registered, but may be empty), then
   * this is an error (unless SILENT). Otherwise, the named solution set is provisioned according to
   ...
   * @return
   */
  private static PipelineOp convertCreateGraph(
      final PipelineOp left, final CreateGraph op, final AST2BOpUpdateContext context) {

    if (op.isTargetSolutionSet()) {

      final String solutionSet = op.getTargetSolutionSet();

      final boolean exists = context.solutionSetManager.existsSolutions(solutionSet);
   ...
   * @throws Exception
   */
  private static PipelineOp convertInsertOrDeleteData(
      PipelineOp left, final AbstractGraphDataUpdate op, final AST2BOpUpdateContext context)
      throws Exception {
    ...
    if (!runOnQueryEngine) {
      ...
      if (log.isDebugEnabled())
        log.debug((insert ? "INSERT" : "DELETE") + " DATA: #stmts=" + stmts.length);
    ...
    final IBindingSet[] bindingSets;
    {
      ...
      bindingSets = getData(op.getData(), targetGraphIV, context.isQuads());
    }
   ...
   * @param insert <code>true</code> iff the statement is to be inserted and <code>false</code> iff
   *     the statement is to be removed.
   * @throws SailException
   */
  private static void addOrRemoveStatement(
   ...
   * @throws SailException
   */
  private static void addOrRemoveStatementData(
      final EmbergraphSailConnection conn, final EmbergraphStatement stmt, final boolean insert)
      throws SailException {
   ...
   * @return
   */
  private static PipelineOp addInsertOrDeleteDataPipeline(
      PipelineOp left, final boolean insert, final AST2BOpUpdateContext context) {
    ...
    left =
        new ChunkedResolutionOp(
            ...
            NV.asMap(
                new NV(BOp.Annotations.BOP_ID, context.nextId()),
                new NV(ChunkedResolutionOp.Annotations.TIMESTAMP, context.getTimestamp()),
                new NV(
   ...
   * @param targetGraph The target graph (optional, but required if quads).
   * @param quads <code>true</code> iff the target {@link AbstractTripleStore} is in quads mode.
   * @return The {@link IBindingSet}[].
   *     <p>TODO Either we need to evaluate this NOW (rather than deferring it to pipelined
   *     evaluation later) or this needs to be pumped into a hash index associated with the query
   *     plan in order to be available when there is more than one INSERT DATA or REMOVE DATA
   *     operation (or simply more than one UPDATE operation).
   *     <p>That hash index could be joined into the solutions immediate before we undertake the
   *     chunked resolution operation which then flows into the add/remove statements operation.
   *     <p>Variables in the query can not be projected into this operation without causing us to
   *     insert/delete the cross product of those variables, which has no interesting effect.
   *     <p>The advantage of running one plan per {@link Update} is that the data can be flowed
   *     naturally into the {@link IRunningQuery}.
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  private static IBindingSet[] getData(
      final ISPO[] data, final IV<?, ?> targetGraph, final boolean quads) {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      if (spo.c() != null) g = new Constant(spo.c());
  ...
  private static void executeUpdate(
      final PipelineOp left, IBindingSet[] bindingSets, final AST2BOpUpdateContext context)
      throws Exception {
   ...
   */
  private static void assertGraphNotEmpty(
      final AST2BOpUpdateContext context, final EmbergraphURI sourceGraph) {
    ...
    if (context
        .conn
        .getTripleStore()
        .getAccessPath(null /* s */, null /* p */, null /* o */, sourceGraph)
        .isEmpty()) {
   ...
   * SailConnection in case the view is isolated by a transaction.
   *
   * <p>Note: If the IV could not be resolved, then that is proof that there is no named graph for
   * that RDF Resource.
   *
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/569 (LOAD-CREATE-LOAD using virgin
   *     journal fails with "Graph exists" exception)
   */
  private static void assertGraphExists(final AST2BOpUpdateContext context, final EmbergraphURI c) {

    if (c.getIV() != null
        && context
                .conn
                .getTripleStore()
                .getAccessPath(null /* s */, null /* p */, null /* o */, c.getIV())
