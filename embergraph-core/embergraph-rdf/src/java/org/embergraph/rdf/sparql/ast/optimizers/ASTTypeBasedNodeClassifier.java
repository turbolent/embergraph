 ...
 * in a dedicated list.
 *
 * <p>There is an additional method to inject custom constraints for the individual classes. In case
 * a constraint is set for a class, the node is only added to the type cluster if the {@link
 * ASTTypeBasedNodeClassifierConstraint#applies()} returns true for the node of the given type.
 *
    ...
    for (IGroupMemberNode node : nodeList) {
      ...
      for (int i = 0; i < clazzez.length && !classified; i++) {
        ...
        if (clazz.isInstance(node)) {
          ...
          final ASTTypeBasedNodeClassifierConstraint constraint = clazzConstraints.get(clazz);
