 ...
 * </pre>
 *
 * Note: An empty <code>{}</code> matches a single empty solution. Since we always push in an empty
 * solution and the join of anything with an empty solution is that source solution, this is the
 * same as not running the group, so we just eliminate the empty group.
 *
...
public class ASTEmptyGroupOptimizer implements IASTOptimizer {
  ...
  private static void eliminateEmptyGroups(
      final QueryBase queryBase, final GraphPatternGroup<?> op) {
    ...
    if (arity == 0
      ...
      final IGroupNode<IGroupMemberNode> parent = op.getParent();
      ...
      swap(queryBase, parent, child);
        ...
        &&
        //                !(((IGroupNode<?>) op.getParent()) instanceof JoinGroupNode &&
        // !(((JoinGroupNode) op
        //                        .getParent()).isMinus())) &&
  ...
  private static void copyQueryHints(final BOp node, final Properties queryHints) {
    ...
    if (!(node instanceof QueryNodeBase)) return;
