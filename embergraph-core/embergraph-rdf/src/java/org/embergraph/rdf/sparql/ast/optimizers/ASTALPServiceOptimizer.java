...
import org.embergraph.rdf.sparql.ast.hints.BasicBooleanQueryHint;
import org.embergraph.rdf.sparql.ast.hints.BasicIntQueryHint;
import org.embergraph.rdf.sparql.ast.hints.BasicStringQueryHint;
  ...
  protected void optimizeJoinGroup(
    ...
    for (ServiceNode node : group.getChildren(ServiceNode.class)) {
      ...
      for (IGroupMemberNode child : subgroup.getChildren()) {
        if (!(child instanceof StatementPatternNode || child instanceof FilterNode)) {
      ...
      final boolean bidirectional = subgroup.getQueryHintAsBoolean(BIDIRECTIONAL, false);
          ...
          if (bidirectional) {
            final StatementPatternNode forward =
                new StatementPatternNode(
                    tVarLeft, child.p(), tVarRight, child.c(), child.getScope());
            ...
            final StatementPatternNode reverse =
                new StatementPatternNode(
                    tVarRight, child.p(), tVarLeft, child.c(), child.getScope());
      ...
      for (FilterNode f : subgroup.getChildren(FilterNode.class)) {
        ...
        final Iterator<BOp> it = BOpUtility.preOrderIteratorWithAnnotations(f);
      ...
      GroupMemberNodeBase child : subgroup.getChildren(GroupMemberNodeBase.class)) {
        if (!child.getQueryHintAsBoolean(PATH_EXPR, false)) {
           ...
           */
          final Iterator<BOp> it = BOpUtility.preOrderIteratorWithAnnotations(child);
          while (it.hasNext()) {
            final BOp bop = it.next();
            if (bop instanceof VarNode) {
              final VarNode v = (VarNode) bop;
