...
public class ASTHashJoinOptimizer implements IASTOptimizer {
   ...
   * variable between the sub-group and the parent (this condition is satisified if one of the joins
   * shares a variable with the parent group).
   */
  private void optimizeJoinGroups(
     ...
     */
    final JoinSetUtil joinSets = new JoinSetUtil(sa, null /* knownBound */, group);
    ...
    if (directJoinSetCount > 1) {
      ...
      final VertexJoinSet[] a =
          joinSets.directJoinSets.toArray(new VertexJoinSet[directJoinSetCount]);
      ...
      for (int i = 0; i < a.length; i++) {
        ...
        for (int j = i + 1; j < a.length; j++) {
          ...
          for (FilterNode f : joinSets.joinFilters) {
            ...
            if (sa.isFullyBound(f, set1) || sa.isFullyBound(f, set2)) {
       ...
       * This join filter does not run with either of
               ...
               */
              log.error(
                  "indirect join: joinSet1=" + a[i] + ",joinSet2=" + a[j] + " on filter=" + f);
            }
          }
        }
      }
  ...
  private static class VertexJoinSetComparator implements Comparator<VertexJoinSet> {
