...
public enum QueryOptimizerEnum {
   ...
   * query optimizer takes into account the actual cardinality and correlation in the query and the
   * data selected by that query. The runtime query optimizer can have slightly more overhead than
   * the static query optimizer, but it never produces a bad join ordering and often identifies the
   * <em>best</em> join ordering. For cases where the <code>static</code> query optimizer produces a
   * bad join ordering, the runtime query optimizer can find join orderings which are orders of
