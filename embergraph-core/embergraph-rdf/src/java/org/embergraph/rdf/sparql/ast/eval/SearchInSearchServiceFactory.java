 ...
 *
 * <p>This service will then use the full text index to filter out incoming bindings for ?s that do
 * not link to a Literal that is found via the full text index with the supplied search string. If
 * there are no incoming bindings (or none that have ?s bound), this service will produce no output.
 */
public class SearchInSearchServiceFactory extends AbstractServiceFactoryBase {
  ...
  private void validateSearch(
    ...
    if (!uris.contains(BDS.SEARCH_IN_SEARCH)) {
      throw new RuntimeException(
          "Required search predicate not found: "
              + BDS.SUBJECT_SEARCH
              + " for searchVar="
              + searchVar);
    }
  ...
  private static class SearchCall implements EmbergraphServiceCall {
    ...
    private Hiterator<IHit<?>> getHiterator() {
      ...
      final ITextIndexer<IHit> textIndex =
          (ITextIndexer) store.getLexiconRelation().getSearchEngine();
     ...
     * prune it. Then iterate the full text search results. For each result (O binding), test the
     * incoming binding sets to see if there is a link between the binding for the searchVar and the
     * O. If there is, add the binding set to the output and remove it from the set to be tested
     * against subsequent O bindings.
     */
    @Override
