 ...
 * <pre>SELECT COUNT(*) { GRAPH :g {:s ?p ?o} }</pre>
 *
 * Combinations using a constrained range-count with a QUADS mode access path where the triple
 * pattern is 1-unbound and the COUNT() references the unbound variable.
 *
 ...
 *
 * Combinations using DISTINCT/REDUCED and a constrained range-count, explicitly naming the
 * variables, and having variables that are not in the COUNT() aggregate and not projected in are
 * NOT handled here. These are covered by the {@link ASTDistinctTermScanOptimizer} instead:
 *
 * <pre>SELECT COUNT(?p) {:s ?p ?o}</pre>
 *
 ...
 *
 * Sub-select that would be handled as a distinct term scan with something projected in.
 *
...
public class ASTFastRangeCountOptimizer implements IASTOptimizer {
  ...
  private void doRecursiveRewrite(
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (child instanceof GraphPatternGroup<?>) {
        ...
        doRecursiveRewrite(context, sa, ((GraphPatternGroup<IGroupMemberNode>) child));
  ...
  private void doSelectQuery(
      final AST2BOpContext context, final StaticAnalysis sa, final QueryBase queryBase) {
    ...
    if (projection.arity() > 1) return;

    final AssignmentNode assignmentNode = projection.getExpr(0);

    if (!(assignmentNode.getValueExpressionNode() instanceof FunctionNode)) return;

    final FunctionNode functionNode = (FunctionNode) assignmentNode.getValueExpressionNode();
     ...
     * the variables in the triple pattern.
     *
     * <p>Note: A simple BPG does not declare the graph variable. The graph variable is ONLY
     * declared by GRAPH ?g {BPG}. For quads mode APs, the graph variable needs to be declared
     * (using GRAPH ?g {BPG}) and used in the COUNT( expression-list ) in order for the rewrite to
     * have the correct semantics.
     *
     * <p>FIXME We also need to handle named graph vs default graph.
     *
     * <p>Thus any of the following can be converted:
     *
     * <p>- COUNT(*) {GRAPH ?g {?s ?p ?o}}
     *
     * <p>- COUNT(*) {GRAPH ?g {:s ?p ?o}}
     *
     * <p>- COUNT(*) {GRAPH :g {?s ?p ?o}}
     *
     * <p>- COUNT(?s ?p ?p ?g) {GRAPH ?g {?s ?p ?o}}
     *
     * <p>However, in quads mode the following MAY NOT be converted (unless there is a single named
     * graph) because the hidden graph variable is not part of the expression-list for COUNT.
     *
     * <p>- COUNT(*) {?s ?p ?o}
     *
     * <p>- COUNT(?s ?p ?o) {?s ?p ?o}
     *
     * <p>In particular, we would get the WRONG answer if we converted the following in quads mode
     * since the COUNT(DISTINCT ?s ?p ?o) is just distinct TRIPLES but the AP fast range count would
     * report distinct QUADS.
     *
     * <p>- COUNT(DISTINCT ?s ?p ?o) {?s ?p ?o} where defaultGraph=ALL
     *
     * <p>TODO Another possibility for this last case is to explicitly compute the sum of the range
     * counts over the triple pattern for the set of named or default graphs.
     */
    ...
    if (!isCountStar && functionNode.arity() == context.getAbstractTripleStore().getSPOKeyArity()) {
      ...
      for (int i = 0; i < functionNode.arity(); i++) {
        final ValueExpressionNode arg = (ValueExpressionNode) functionNode.get(i);
   ...
   */
  protected boolean markForFastRangeCount(
      final AST2BOpContext context,
