...
public class JoinGroupNode extends GraphPatternGroup<IGroupMemberNode> {
   ...
   * the deep copy semantics. The <code>JoinGroupNode(IGroupMemberNode child)
   * </code<
   ...
   * </strong>
   * This means that the following code DOES NOT create a nested structure of a JoinGroupNode in a
   * JoinGroupNode.
   *
   * <pre>
   ...
   * and otherwise read up the parent chain.
   */
  @Override
  ...
  public final boolean isSubgroupOfALPNode() {
    ...
    return getProperty(
        Annotations.IS_SUBGROUP_OF_ALP_NODE, Annotations.DEFAULT_IS_SUBGROUP_OF_ALP_NODE);
  }

  public final void setSubgroupOfALPNode(final boolean isSubgroup) {
   ...
   *
   * <p>Note: {@link AssignmentNode}s MUST NOT be reordered. They MUST be evaluated left-to-right in
   * the order given in the original query.
   */
   ...
   * which we have attached an inline access path. That transformation needs to happen in a rewrite
   * rule, which means that we will wind up removing the IN filter and replacing it with an AST node
   * for that inline AP (something conceptually similar to a statement pattern but for a column
   * projection of the variable for the IN expression). That way we do not have to magically
   * "subtract" the known "IN" filters out of the join- and post- filters.
   *
