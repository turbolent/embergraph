 ...
 *
 * <p>Note: <code>toVE()</code> is a NOP for {@link VarNode}s and {@link ConstantNode}s. In fact, in
 * only acts on the value expression of an {@link AssignmentNode} and {@link FunctionNode}s.
 *
 * <p>Note: This has to be done carefully to avoid a side-effect during traversal.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: ASTSetValueExpressionsOptimizer.java 5193 2011-09-15 14:18:56Z thompsonbry $
 */
...
//     * Original version caches to avoid side-effects and visits all VENs both
   ...
   * IValueExpressionNodeContainer} interface.
   *
   * @param lex
   * @param query
   */
  private void convert2(
      final BOpContextBase context, final GlobalAnnotations globals, final QueryRoot query) {

    /*
     * Visit nodes that require modification.
     */
    final IStriterator it =
        new Striterator(BOpUtility.preOrderIteratorWithAnnotations(query))
            .addFilter(
                new Filter() {
    ...
    while (it.hasNext()) {
      ...
      if (op instanceof IValueExpressionNodeContainer) {
        ...
        AST2BOpUtility.toVE(
            context, globals, ((IValueExpressionNodeContainer) op).getValueExpressionNode());
        ...
        if (range != null) {
          ...
          if (range.from() != null) AST2BOpUtility.toVE(context, globals, range.from());

          if (range.to() != null) AST2BOpUtility.toVE(context, globals, range.to());
        }
      }
