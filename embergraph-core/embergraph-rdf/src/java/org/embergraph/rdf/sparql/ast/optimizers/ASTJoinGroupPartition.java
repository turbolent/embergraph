 ...
 * (i.e., the OPTIONAL/MINUS will not contribute to this list).
 *
...
public class ASTJoinGroupPartition {
  ...
  public void replaceNonOptionalNonMinusNodesWith(
      final List<IGroupMemberNode> ordered, final boolean recomputeDefinitelyProduced) {
   ...
   *
   * <p>NOTE: requires the node to be contained in the partitions binding info map.
   */
  ...
  void placeAtFirstPossiblePosition(
    ...
    placeAtPosition(node, getFirstPossiblePosition(node, additionalKnownBound, requiresAllBound));
    definitelyProduced.addAll(bindingInfoMap.get(node).getDefinitelyProduced());
  }
   ...
   */
  void placeAtPosition(final IGroupMemberNode node, final Integer positionToPlace) {
   ...
   * variables may be bound anymore. The flag requiresAllBound can be used to turn off the last
   * condition, requiring that all variables need to be bound (and returning null in case this
   * condition is never satisfied). This flag is useful when distributing FILTERs across partitions.
   *
   * @param the node to place
   * @param additional variables that are known to be bound
   * @param requiresAllBound requires that all variables are bound, i.e. when set to true it is not
   *     sufficient that unbound variables can't be bound anymore
  ...
  Integer getFirstPossiblePosition(
     ...
     * are bound or no more of it required variables can be bound. If no such position exists, the
     * method returns null (in that case, only the end of the partition may be a safe place for the
     * node).
     */
    for (int i = 0; i < nonOptionalNonMinusNodes.size(); i++) {
      ...
      if (!requiresAllBound) {
        final Set<IVariable<?>> maybeProducedByCur = bindingInfoMap.get(cur).getMaybeProduced();
    ...
    if (canBePlacedAtPosition(
        requiresAllBound, bindingInfo, knownBound, remainingMaybeBound, lastPosition)) {
   ...
   */
  private boolean canBePlacedAtPosition(
      final boolean requiresAllBound,
    ...
    final Set<IVariable<?>> leftToBeBound = bindingInfo.leftToBeBound(knownBound);
  ...
  private void addMaybeProducedToMultiset(
      final Map<IVariable<?>, Integer> multiset, IGroupMemberNode node) {
