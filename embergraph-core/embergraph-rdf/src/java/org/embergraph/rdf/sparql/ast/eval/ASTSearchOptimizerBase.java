...
public abstract class ASTSearchOptimizerBase implements IASTOptimizer {
  ...
  private void extractSearches(
    ...
    if (tmp != null) {
      ...
      for (Map.Entry<IVariable<?>, Map<URI, StatementPatternNode>> e : tmp.entrySet()) {
        ...
        final ServiceNode serviceNode =
            createServiceNode(ctx, queryBase, group, searchVar, statementPatterns);
        ...
        if (log.isInfoEnabled()) log.info("Rewrote group: " + group);
      }
    }
  }
   ...
   * basically imposes a constraint that the search results will only be reported for the statement
   * appearing in graphs which are visible to the query. For a <code>GRAPH ?g {...}</code> group
   * without an explicit subject join, it also serves to bind the graph variable, which could
   * otherwise not be bound as nothing was actually joined against a statement index.
   *
   * @param queryRoot Used to gain access to the {@link DatasetNode}.
   * @param searchVar The search variable (the literal whose text is the free text query).
   * @param group The group in which the search magic predicates appear.
   */
  private void enforceGraphConstraint(
    ...
    if (group.getContext() != null) {
      ...
      if (log.isInfoEnabled())
        log.info("Added subject join to imposed named graph constraint: " + group);
      ...
      if (log.isInfoEnabled())
        log.info("Added subject join to imposed default graph constraint: " + group);
    }
  }
  ...
  private ServiceNode createServiceNode(
    ...
    @SuppressWarnings("unchecked")
    final TermId<EmbergraphURI> iv = (TermId<EmbergraphURI>) TermId.mockIV(VTE.URI);
   ...
   * @param statementPatterns The statement pattern nodes.
   */
  private void removeSearchPredicates(
    ...
    for (StatementPatternNode sp : statementPatterns.values()) {
      ...
      if (!group.removeArg(sp)) throw new AssertionError();
    }
  }
}
