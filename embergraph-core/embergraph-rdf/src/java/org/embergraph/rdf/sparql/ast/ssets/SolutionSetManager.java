 ...
 *     SOLUTION SETS </a>
 * @see <a href="http://aksw.org/Projects/QueryCache">Adaptive SPARQL Query Cache </a>
 * @see <a href="http://www.informatik.uni-leipzig.de/~auer/publication/caching.pdf > Improving the
 *     Performance of Semantic Web Applications with SPARQL Query Caching </a>
 * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/524">SPARQL Query Cache </a>
...
//     * MVCC VIEWS: Convert over to Name2Addr and layered resolution with
  ...
  public SolutionSetManager(
      final IBTreeManager store, final String namespace, final long timestamp) {
   ...
   * AbstractTripleStore is destroyed. The hook is currently in {@link AbstractResource#destroy()}.
   */
  @Override
  public void close() {
   ...
   *     <p>TODO Support different index orders. We can do a prefix scan on Name2Addr to find the
   *     existing index orders (but this is not currently efficient due to a bug in Name2Addr).
   *     <p>When reading out the {@link ISolutionSetStats} or the solutions themselves, all we need
   *     is the first such index that we find for a given namespace and localName.
   *     <p>When writing, we need to write on ALL such indices.
   *     <p>Either way, the scan should give us what we need.
   *     <p>For CREATE, the desired index order(s) should be declared in the create metadata.
   */
  private String getFQN(final String localName) {

    return NamedSolutionSetRefUtility.getFQN(namespace, localName, IVariable.EMPTY /* joinVars */);
  }
  ...
  public void clearAllSolutions() {
    ...
    final String prefix = NamedSolutionSetRefUtility.getPrefix(namespace).toString();

    if (log.isInfoEnabled()) log.info("Scanning: prefix=" + prefix);

    final Iterator<String> itr = getStore().indexNameScan(prefix, timestamp);
    ...
    while (itr.hasNext()) {
      ...
      if (log.isInfoEnabled()) log.info("Dropping: " + name);
    }
   ...
   * @return The named solution set -or- <code>null</code> if it was not found.
   */
  private SolutionSetStream getSolutionSet(final String fqn) {

    if (timestamp == ITx.READ_COMMITTED) {

      return (SolutionSetStream) getStore().getIndexLocal(fqn, getStore().getLastCommitTime());
       ...
       * Note: This code path is used by the TemporaryStore and possibly ResourceManager (for a
         ...
         *
         * <p>TODO Use the readsOnCommitTime. Test coverage for TemporaryStore and DataService (but
         * there is an open question about how to handle hash partitioned solution sets on a
         * federation).
         *
         * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/266">Refactor native long
      ...
      return (SolutionSetStream) getStore().getIndexLocal(fqn, timestamp);
    }
  ...
  public boolean existsSolutions(final String solutionSet) {
    ...
    if (solutionSet == null) throw new IllegalArgumentException();
  ...
  public boolean clearSolutions(final String solutionSet) {
    ...
    if (log.isInfoEnabled()) log.info("solutionSet: " + solutionSet);
  ...
  @Override
  public void putSolutions(final String solutionSet, final ICloseableIterator<IBindingSet[]> src) {
   ...
   *     <p>TODO ISPO[] params is ignored (you can not configure for a BTree or HTree index for the
   *     solutions with a specified set of join variables for the index).
   */
  private SolutionSetStream _create(final String fqn, final ISPO[] params) {

    SolutionSetStream sset = getSolutionSet(fqn);

    if (sset != null) throw new RuntimeException("Exists: " + fqn);

    if (log.isInfoEnabled()) log.info("Create: fqn=" + fqn + ", params=" + Arrays.toString(params));
  ...
  private void writeSolutions(
      final SolutionSetStream sset, final ICloseableIterator<IBindingSet[]> src) {
  ...
  public ISolutionSetStats getSolutionSetStats(final String solutionSet) {
    ...
    if (sset != null) {
      ...
      if (stats == null) throw new RuntimeException("No statistics? solutionSet=" + solutionSet);
  ...
  @Override
  public ICloseableIterator<IBindingSet[]> getSolutions(final String solutionSet) {

    final String fqn = getFQN(solutionSet);

    final SolutionSetStream sset = getSolutionSet(fqn);

    if (sset == null) throw new IllegalStateException("Not found: " + solutionSet);
   ...
   *     <p>TODO This is ignored and needs to be reconciled with {@link IndexMetadata}. However, we
   *     do want to provide this metadata in a CREATE schema as triples.
   */
