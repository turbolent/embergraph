...
public class AST2BOpBase {
  ...
  public interface Annotations {
     ...
     *
     * <p>Note: This annotation MUST be stripped from the query plan to prevent an attempt to
     * serialized it for RMI in scale-out (the {@link Dataset} is not {@link Serializable}, can be
     * quite large, and is captured by other constructions in the generated query plan).
     */
     ...
     * of variables that are not already known to be materialized).
     */
     ...
     * scale-out, the binding sets are send to the node having the shard on which the asBound
     * predicate would read.
     *
     * <p>TODO It would make sense to lift this annotation into a different AST optimizer so it is
     * always present. An optimization for index locality for as-bound evaluation depends on the
     ...
     * the end since they can not increase the selectivity of the query).
     */
    String ESTIMATED_CARDINALITY = AST2BOpBase.class.getName() + ".estimatedCardinality";
     ...
     */
    String COST_SUBQUERY = AST2BOpBase.class.getName() + ".cost.subquery";

    /** The #of known graphs in the {@link Dataset} for a default graph or named graph query. */
    String NKNOWN = AST2BOpBase.class.getName() + ".nknown";

    /** Boolean indicating whether the AST2BOpUpdate should autocommit. Default value is TRUE. */
...
//     * Apply any query hints to the operator as annotations of that operator.
   ...
   * ASTBase#getQueryHints()} onto a generated {@link PipelineOp}.
   *
   * @param op The pipeline operator generated from some AST node.
   * @param node The AST node from which the pipeline operator was generated (required). The query
   *     hints (from {@link ASTBase#getQueryHints()}) will be applied to that pipeline operator.
   * @param ctx The evaluation context (ignored). Global query hints declared here will be applied
   *     to the generated pipeline operator. Global hints are applied <strong>first</strong> so they
   *     can be override by AST node specific hints.
   * @return A copy of that operator to which the query hints (if any) have been applied. If there
   *     are no query hints then the original operator is returned.
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/791" > Clean up query hints </a>
   */
  protected static PipelineOp applyQueryHints(
      PipelineOp op, final ASTBase node, final AST2BOpContext ctx) {
   ...
   * ASTBase#getQueryHints()} onto a generated {@link PipelineOp}.
   *
   * <p>Note: This alternative form is for use within caller contexts in which (due to historical
   * reasons) we have the query hints object for the AST node, but not the AST node itself. This
   * pattern shows up in the join() methods since the {@link StatementPatternNode} is not passed
   * through.
   *
   * @param op The pipeline operator generated from some AST node (required).
   * @param nodeQueryHints The query hints for the AST node from which the pipeline operator was
   *     generated or its dominating operator context since not all operators have query hints
   *     applied (required).
   * @param ctx The evaluation context (ignored). Global query hints declared here will be applied
   *     to the generated pipeline operator. Global hints are applied <strong>first</strong> so they
   *     can be override by AST node specific hints.
   ...
   */
  protected static PipelineOp applyQueryHints(
      PipelineOp op, final Properties nodeQueryHints, final AST2BOpContext ctx) {
   ...
   *     are no query hints then the original operator is returned.
   */
  private static PipelineOp _applyQueryHints(PipelineOp op, final Properties queryHints) {

    if (queryHints == null) return op;
    ...
    while (pnames.hasMoreElements()) {
      ...
      if (log.isDebugEnabled())
        log.debug(
            "Query hint: op=" + (op.getClass().getSimpleName()) + " [" + name + "=" + value + "]");
