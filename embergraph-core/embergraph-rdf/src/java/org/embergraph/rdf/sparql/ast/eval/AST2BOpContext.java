...
public class AST2BOpContext implements IdFactory, IEvaluationContext {
  ...
  /** The {@link ISolutionSetManager} -or- <code>null</code> iff that cache is not enabled. */
   ...
   * nodes. In addition, the {@link ASTQueryHintOptimizer} uses registered {@link IQueryHint}
   * implementations to annotate the original AST as one of the steps when transforming it into the
   * optimized AST. This is done both for the global {@link #queryHints}s and for magic predicates
   * using the {@link QueryHints#NAMESPACE} that appear in the query. Once a query hint is set on an
   * AST node, it will eventually be copied across to {@link PipelineOp}s generated from that AST
   * node.
   *
   * @see QueryHints
  ...
  /** Factory for resolving relations and access paths used primarily by {@link AST2BOpJoins}s. */
   ...
   * 1.2.1 was ONE (1). While this join can be evaluated concurrently for multiple input chunks,
   * assessment done with r6411 on BSBM (explore) and govtrack failed to demonstrate any performance
   * advantage when using maxParallel = 5 (the default maximum parallelism as specified by {@link
   * PipelineOp.Annotations#DEFAULT_MAX_PARALLEL}. This parameter makes it easier to re-test the
   * effect of parallelism in these joins in the future.
   *
   * @see AST2BOpUtility
   ...
   *
   * <p>Note: It is only safe and efficient to materialize the projection from within the query plan
   * when the query does not specify an OFFSET / LIMIT. Materialization done from within the query
   * plan needs to occur before the SLICE operator since the SLICE will interrupt the query
   * evaluation when it is satisfied, which means that downstream operators will be canceled.
   * Therefore a materialization operator can not appear after a SLICE. However, doing
   * materialization within the query plan is not efficient when the query uses a SLICE since we
   * will materialize too many solutions. This is true whether the OFFSET and/or the LIMIT was
   * specified.
   *
   * <p>Note: It is advantegous to handle the materialization step inside of the query plan since
   * that provides transparency into the materialization costs. When we handle materialization
   * outside of the query plan, those materialization costs are not reflected in the query plan
   * statistics.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/489">Optimize RDF Value
   *     materialization performance on cluster </a>
   */
   ...
   * for each of the sampled contexts are combined to estimate the total cost of visiting all of the
   ...
   * join will always use the SCAN + FILTER approach.
   */
  ...
  /** Globally scoped variables, as injected through Sesame's Operation.setBindings() method. */
  private Set<IVariable<?>> globallyScopedVariables;

  @Override
  public ISolutionSetStats getSolutionSetStats() {

    if (sss == null) return EmptySolutionSetStats.INSTANCE;
  ...
  public void setStaticAnalysisStats(final StaticAnalysisStats saStats) {
    ...
    if (saStats == null) throw new IllegalArgumentException();
   ...
   * AST2BOpUtility}.
   *
   * <p>Note: This is not initialized earlier since it holds a reference to the optimized AST.
   */
   ...
   *
   * @return The map -or- <code>null</code> if no query attributes have been declared.
   */
   ...
   *     <p>TODO We should be passing in the {@link IIndexManager} rather than the {@link
   *     AbstractTripleStore} in order to support cross kb queries. The AST can be annotated with
   *     the namespace of the default KB instance, which can then be resolved from the {@link
   *     IIndexManager}. This would also allow us to clear up the [lex] namespace parameter to the
   *     {@link FunctionNode} and {@link FunctionRegistry}.
   */
  public AST2BOpContext(final ASTContainer astContainer, final AbstractTripleStore db) {
  ...
  AST2BOpContext(
      final ASTContainer astContainer, final AbstractTripleStore db, final IdFactory idFactory) {

    if (astContainer == null) throw new IllegalArgumentException();

    if (db == null) throw new IllegalArgumentException();

    if (idFactory == null) throw new IllegalArgumentException();
    ...
    this.queryEngine = QueryEngineFactory.getInstance().getQueryController(db.getIndexManager());
    ...
    final String queryIdStr =
        queryHints == null ? null : queryHints.getProperty(QueryHints.QUERYID);

    final UUID queryId = queryIdStr == null ? UUID.randomUUID() : UUID.fromString(queryIdStr);

    this.queryId = queryId;

    // NAMED SOLUTION SETS
    if (queryEngine.getIndexManager() instanceof IBTreeManager) {
      this.solutionSetManager =
          new SolutionSetManager(
              (IBTreeManager) queryEngine.getIndexManager(), db.getNamespace(), db.getTimestamp());
    } else {
      /*
  ...
  private static ASTOptimizerList newOptimizerList(final AbstractTripleStore db) {
    ...
    try {
      final Class<?> cls = Class.forName(QueryHints.DEFAULT_AST_OPTIMIZER_CLASS);
  ...
  private static IExternalAST2BOp initGPUEvaluation() {
    ...
    // "org.embergraph.rdf.gpu.sparql.ast.eval.GPUEvaluation" );
    //
  ...
  public DescribeModeEnum getDescribeMode(final ProjectionNode projection) {
     ...
     */
    final String describeDefaultStr =
        db.getProperties().getProperty(EmbergraphSail.Options.DESCRIBE_MODE);
  ...
  public int getDescribeIterationLimit(final ProjectionNode projection) {
    ...
    final String limitStr =
        db.getProperties().getProperty(EmbergraphSail.Options.DESCRIBE_ITERATION_LIMIT);
  ...
  public int getDescribeStatementLimit(final ProjectionNode projection) {
     ...
     */
    final String limitStr =
        db.getProperties().getProperty(EmbergraphSail.Options.DESCRIBE_STATEMENT_LIMIT);
