 ...
 *
 * <p>For example, the optimizer changes this:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Note: The {@link ASTConstructOptimizer} will add a {@link ProjectionNode} based on the
 * generated CONSTRUCT template.
 *
...
public class ASTDescribeOptimizer implements IASTOptimizer {
  ...
  public QueryNodeWithBindingSet optimize(
    ...
    //        final DescribeModeEnum describeMode = projection.getDescribeMode() == null ?
    ...
    /*
     * We need to keep the projection so we can correlate the original variables for the resources
     * that are being described with the bindings on those variables in order to figure out what
     * resources were described when we are maintaining a DESCRIBE cache.
     *
     * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/584">DESCRIBE CACHE </a>
     */
    /*
     * We need to keep the projection since the DescribeMode annotation is attached to the
     * projection.
     *
    ...
    if (describeMode.isForward() && describeMode.isReifiedStatements()) {
      ...
      db.getLexiconRelation()
          .addTerms(new EmbergraphValue[] {tmp}, 1 /* numTerms */, true /* readOnly */);
    ...
    for (TermNode term : terms) {
       ...
       * Note: Each statement has to be in a different part of the
        ...
        final StatementPatternNode sp =
            new StatementPatternNode(
                term, new VarNode("p" + termNum + "a"), new VarNode("o" + termNum));
      ...
      if (describeMode.isReverse()) { // ?sN ?pN-b <term>

        final StatementPatternNode sp =
            new StatementPatternNode(
                new VarNode("s" + termNum), new VarNode("p" + termNum + "b"), term);
       ...
       * Pick up properties associated with reified statement models
        ...
        final StatementPatternNode sp =
            new StatementPatternNode(
                new VarNode("stmt" + termNum), new ConstantNode(rdfSubject.getIV()), term);
