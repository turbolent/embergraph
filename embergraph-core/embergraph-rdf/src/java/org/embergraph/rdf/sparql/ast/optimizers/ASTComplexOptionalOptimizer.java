 ...
 * original idea, the actual implementation slightly differs in (i) that it passes around
 * intermediate solutions (rather than computing all joins over the original outer mapping set) and
 * (ii) does not compute a final join over all intermediate solutions but simply returns the last
 * computed result (which is now possible due to (i)).
 *
 * <p>Also note that the {@link AST2BOpUtility} has been refactored (as part of ticket #1071 and
 * #1118) and now implements intelligent pushing of projection variables into OPTIONAL blocks, so
 * there's probably no more need for this optimizer.
 *
 * <p>Rewrite a join group using two or more complex OPTIONAL groups using a hash join pattern.
 *
 * <p>Note: this optimization is not required if there is only one complex optional in the join
 * group. It is only when there are multiple complex optional groups that we need to lift those
 * groups out as named subqueries (since they need to feed each other). If there is only one complex
 * optional group, then we can run it as a sub-group instead.
 *
 * <p>NOte: This optimization presumes that simple optional groups were already translated into
 * optional {@link StatementPatternNode}s.
 *
 * <p>Queries with multiple complex optional groups can be rewritten into the hash join of solution
 * sets as follows.
 *
 * <ol>
 *   <li>First, create a hash index from the required joins and any simple optional joins. Given the
 *       modeling abilities of the AST, this is most easily achieved by converting the required
 *       joins into a named subquery. The join variable(s) for that named subquery will the subset
 *       of variable(s) which are shared by each of the complex OPTIONAL groups. The {@link
 *       ASTNamedSubqueryOptimizer} already handles the assignment of join variables, so we do not
 *       need to consider it further here.
 *   <li>For each complex optional group, use the solution step generated in (1) and run the
 *       optional group as a named subquery producing a new solution set. The WHERE clause of the
 *       named subquery should look like:
 *       <pre>
 * {INCLUDE %set . OPTIONAL {...}}
 * </pre>
 *   <li>Join all of the named solution sets in (2) back together. For example, if there were two
 *       complex optional groups and the required joins resulted in known bound variables for var1
 *       and var2, then those result sets might be combined as follows in the main WHERE clause of
 *       the rewritten query.
 *       <pre>
 * INCLUDE %set1 .
 * INCLUDE %set2 JOIN ON (?var1, ?var2) .
 * </pre>
 *       Note: The join variables for those INCLUDEs MUST be identified through static analysis.
 *       Failure to use available join variables will result in an extremely inefficient query plan
 *       as the full cross product of the solutions will be compared to identify solutions which
 *       join.
 * </ol>
 *
 * TODO The rewrite into named subquery includes means that we wind up building more hash indices
 * than we strictly require as a hash index will also be built at the start of each optional group.
 * However, since the hash index at the start of the optional group has exactly the same data as the
 * named subquery include's hash index, we should elide the step which builds the extra hash index.
 *
 * <p>TODO This optimzer should not be strictly necessary at all. The same behavior should arise
 * from running the complex optionals as sub-groups. Based on a few govtrack CI queries, it looks
 * like we do better when the complex optional groups are lifted out as named subqueries. We need to
 * go back and investigate whether or not this is true and why.
 *
...
public class ASTComplexOptionalOptimizer implements IASTOptimizer {
   ...
   *
   * <p>Note: We have confirm that this change does not have a negative impact on either govtrack or
   * BSBM. The change also fixes a known problem with some customer queries.
   *
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/668" > Join Group Optimization
   *     </a>
   */
  ...
  public QueryNodeWithBindingSet optimize(
    ...
    final Set<IVariable<?>> exogenousVars = context.getSolutionSetStats().getUsedVars();
    ...
    // First, process any pre-existing named subqueries.
    {
      ...
      if (namedSubqueries != null) {
        ...
        for (NamedSubqueryRoot namedSubquery : roots) {
          ...
          convertComplexOptionalGroups(
              context, sa, namedSubquery, namedSubquery.getWhereClause(), exogenousVars);
        }
      }
    }

    // Now process the main where clause.
    convertComplexOptionalGroups(context, sa, queryRoot, queryRoot.getWhereClause(), exogenousVars);

    return new QueryNodeWithBindingSet(queryNode, bindingSets);
  }

  /*
   * Using a depth-first recursion, examine each join group. If a join group contains two or more
   * complex optional groups (that is, an optional group which was not translated into a optional
   * statement pattern node) then convert the join group having those complex optional child groups.
   */
  private void convertComplexOptionalGroups(
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (child instanceof GraphPatternGroup<?>) {
        ...
        if (recurse) {
          ...
          convertComplexOptionalGroups(context, sa, query, childGroup, exogenousVars);
        }
        ...
        if (recurse) {
          ...
          convertComplexOptionalGroups(
              context, sa, query, subqueryRoot.getWhereClause(), exogenousVars);
        }
      }
    }
    ...
    if (complexOptionalGroupCount >= 2 && (group instanceof JoinGroupNode)) {
      ...
      convertJoinGroup(context, sa, query, (JoinGroupNode) group, exogenousVars);
    }
  }

  /*
   * 1. Move the required joins (INCLUDEs, statement pattern nodes, and required subqueries) plus
   * any simple OPTIONALs (which are statement pattern nodes) into a new named subquery (unless the
   * required joins are already a single INCLUDE).
   *
   * <p>2. Lift each complex optional group into a new named subquery which INCLUDEs the result from
   * (1), replacing it with an INCLUDE of the named subquery in this group.
   *
   * <p>Note: The named solution set from (1) is NOT directly INCLUDEd back into this join group.
   * Instead, it is INCLUDEd into each the named subquery for each complex optional lifted out of
   * this join group.
   *
   * @param query The (sub-)query in which the join group appears. We need to know what is being
   *     projected out of the (sub-)query in order to compute the projections when converting
   *     sub-groups into subqueries.
   * @param group The join group to be rewritten.
   * @param exogenousVars The exogenous variables for the query.
   */
  private void convertJoinGroup(
      final AST2BOpContext context,
      final StaticAnalysis sa,
      final QueryBase query,
     ...
     */
    {
      final IGroupMemberNode[] members = group.toArray(new IGroupMemberNode[] {});
      ...
      for (int i = 0; i < members.length && !optimizationApplicable; i++) {
        ...
        optimizationApplicable |=
            t instanceof IBindingProducerNode
                && !(t instanceof AssignmentNode)
                && // assignments run last
                !(t instanceof JoinGroupNode && ((JoinGroupNode) t).isOptional());
      }

      if (!optimizationApplicable) return; // not safe, can't do anything here
    }
    ...
    {
      final NamedSubqueryRoot nsr = new NamedSubqueryRoot(QueryType.SELECT, mainSolutionSetName);
       ...
       */
      {
        ...
        if (query.getProjection() != null) {
          ...
          //                    tmp.getProjectionVars(afterVars);// FIXME This needs to be the
          // variables USED in the SELECT expressions, NOT the variables projected out of the query.
        }

        final Set<IVariable<?>> projectedVars = new LinkedHashSet<>(groupVars);
    ...
    /*
     * Extract maybe produced variables from the complex join groups, making the accessible in an
    ...
    for (int i = 0; i < complexGroups.size(); i++) {
      ...
      final NamedSubqueryRoot nsr = new NamedSubqueryRoot(QueryType.SELECT, solutionSetName);
      ...
      final NamedSubqueryInclude mainInclude = new NamedSubqueryInclude(precedingSolutionName);

      whereClause.addChild(mainInclude);
      whereClause.addChild(childGroup);

      final NamedSubqueryInclude anInclude = new NamedSubqueryInclude(solutionSetName);
       ...
       */
      if (group.replaceWith(childGroup, anInclude) != 1) throw new AssertionError();
       ...
       */
      {
        ...
        if (i != complexGroups.size() - 1) {
          if (!group.removeArg(anInclude)) throw new AssertionError();
        }
      }
