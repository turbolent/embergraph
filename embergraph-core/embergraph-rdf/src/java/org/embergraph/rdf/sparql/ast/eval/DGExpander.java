...
public class DGExpander implements IAccessPathExpander<ISPO> {
   ...
   *     graphs.
   * @todo A parallelism limitation is placed on the ClientIndexView through the {@link
   *     IEmbergraphClient.Options#CLIENT_MAX_PARALLEL_TASKS_PER_REQUEST} . We should be able to
   *     override that through annotations on a query plan.
   */
  @SuppressWarnings("rawtypes")
  public DGExpander(
      final int maxParallel, final Collection<IV> graphs, final long estimatedRangeCount) {
  ...
  private final class DefaultGraphParallelEvaluationAccessPath implements IAccessPath<ISPO> {
    ...
    public String toString() {
      ...
      return super.toString() + "{baseAccessPath=" + sourceAccessPath.toString() + "}";
    }
    ...
    public DefaultGraphParallelEvaluationAccessPath(final SPOAccessPath accessPath) {
      ...
      this.executor =
          new LatchedExecutor(accessPath.getIndexManager().getExecutorService(), maxParallel);
    }
    ...
    public boolean isEmpty() {
      ...
      final IChunkedOrderedIterator<ISPO> itr =
          iterator(0L /* offset */, 1 /* limit */, 1 /* capacity */);
     ...
     * SPARQL which lacks SELECT ... INSERT or SELECT ... DELETE constructions, at least at this
     * time.
     */
    @Override
     ...
     *     paths and a merge sort to combine chunks drawn from each access path, and then an
     *     iterator which skips over duplicates by considering the last returned (s,p,o). We need
     *     to: (a) allocate a buffer each time we draw from the current chunks based on the total
     *     size of the current chunks; and (b) we can only draw keys from the current chunks up to
     *     the min(nextKey) for each chunk. The min(nextKey) constraint is necessary to ensure that
     *     a merge sort will get rid of duplicates. Without that constraint it is possible that a
     *     latter chunk from some access path will report an (s,p,o) that has already be visited.
     *     (The constraint allows us to use a closed world assumption to filter duplicates after the
     *     merge sort.)
     */
    @Override
    public IChunkedOrderedIterator<ISPO> iterator(
      ...
      final ICloseableIterator<ISPO> src = new InnerIterator1(offset, limit, capacity);
     ...
     *
     * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
     * @version $Id: DefaultGraphSolutionExpander.java 3678 2010-09-29 15:48:34Z thompsonbry $
     */
    private class InnerIterator1 implements ICloseableIterator<ISPO> {
       ...
       *     but I think that wrapping the DISTINCT filter around the DGExpander fixed that
       *     problem.)
       */
       ...
       * @param capacity
       */
      public InnerIterator1(final long offset, final long limit, final int capacity) {
       ...
       * The outer access path will impose the "DISTINCT SPO"
          ...
          final IFilter filter = sourceAccessPath.getPredicate().getAccessPathFilter();
          if (filter != null) {
            src =
                new ChunkedWrappedIterator<ISPO>(
                    new Striterator(buffer.iterator()).addFilter(filter));
          } else {
            src = buffer.iterator();
          }
       ...
       * caller's buffer.
       *
       * @param buffer The elements drained from the iterators will be added to this buffer.
       * @return The task whose future is set on the buffer.
       */
      private Callable<Void> newRunIteratorsTask(final BlockingBuffer<ISPO> buffer) {
      ...
      private final class RunIteratorsTask implements Callable<Void> {
        ...
        public Void call() throws Exception {
          ...
          try {
            ...
            for (Future<Void> f : tasks) f.cancel(true /* mayInterruptIfRunning */);
       ...
       * blocking buffer.
       *
       * <p>Note: don't pass the top-level offset, limit, capacity into the per-graph AP iterator or
       * it will skip over offset results per graph! The limit needs to be imposed on the data
       * pulled from the blocking buffer, not here.
       */
      private final class DrainIteratorTask implements Callable<Void> {
        ...
        public DrainIteratorTask(final IV<?, ?> termId) {
          ...
          if (termId == null) throw new IllegalArgumentException();
       ...
       * Clear various annotations from source predicate.
          ...
          final Predicate<ISPO> asBound =
              sourcePred.asBound(
                  (IVariable<?>) sourcePred.get(3 /* cvar */), new Constant<IV<?, ?>>(termId));

          // Obtain the access path for the asBound predicate.
          final IAccessPath<ISPO> asBoundAP = sourceAccessPath.getRelation().getAccessPath(asBound);

          //                    System.err.println(asBoundAP.toString());

          final IChunkedOrderedIterator<ISPO> itr = asBoundAP.iterator();
          ...
          try {
            ...
            if (log.isDebugEnabled()) log.debug("Ran iterator: c=" + termId + ", nvisited=" + n);
