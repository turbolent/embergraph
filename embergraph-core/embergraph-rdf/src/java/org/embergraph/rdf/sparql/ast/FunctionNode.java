...
public class FunctionNode extends ValueExpressionNode {
  ...
  interface Annotations extends ValueExpressionNode.Annotations {
    ...
    /**
     * The scalar values needed to construct the {@link IValueExpression}. This is a {@link
     * Serializable} {@link Map} with {@link String} keys and {@link Object} values.
     */
   ...
   *
   * @param functionURI the function URI. see {@link FunctionRegistry}
   * @param scalarValues One or more scalar values that are passed to the function
   * @param args the arguments to the function.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/672">Occasional error on BSBM
   *     Explore query</a>
   */
  public FunctionNode( //
  ...
  /**
   * Returns an unmodifiable map because if the scalar values are modified, we need to clear the
   * value expression cache. This is handled correctly by {@link #setScalarValues(Map)}.
   */
  public Map<String, Object> getScalarValues() {

    @SuppressWarnings("unchecked")
    final Map<String, Object> scalarValues =
        (Map<String, Object>) getProperty(Annotations.SCALAR_VALS);
  ...
  /**
   * Return <code>true</code> iff the {@link FunctionNode} makes use of a {@link
   * FunctionRegistry#BOUND} operator.
   *
   * @return <code>true</code>iff it uses <code>BOUND()</code> TODO Unit test.
   */
  public boolean isBound() {

    if (FunctionRegistry.BOUND.equals(getFunctionURI())) return true;
