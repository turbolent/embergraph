...
public class JoinSetUtil {
   ...
   * </pre>
   *
   * Would form a direct join set each join will share a variable directly. Even though there are
   * some vertices which do not share variables directly, they can be joined directly once other
   * joins have been processed. For example <code>p(z,t)</code> can be joined once we have <code>
   * p(y,z)</code>.
   *
   * <p>The number of such direct join sets varies with the query. If all vertices fit into one
   * direct join set, then the query can be pipelined efficiently. If there is more than one such
   * set then we need to look further to see if we can identify joins on indirectly shared variables
   * (shared through a FILTER) which can be used to piece these join sets together. If not, there is
   * an unconstrained cross product in the query.
   */
  ...
  public static class VertexJoinSet {
    ...
    public String toString() {
      return getClass().getSimpleName() + "{vertices=" + vertices + ",joinvars=" + joinvars + "}";
    }
   ...
   * the vertices with which it has joined (initially this contains just that row). For each column
   * that it joins with, add in all variables for that column and add the column to the set of
   * joined vertices. Once all columns have been processed for the initial row, we have the first
   * join set of vertices.
   *
   * <p>Repeat for the first row not found in that join set until all joins have been incorporated
   * into a join set. This gives us the #of join sets which do not overlap based on directly shared
   * variables. It would be ideal to have one such join set. Where there is more than one there is
   * clearly a question concerning which join set we should handle first.
   */
  private Set<VertexJoinSet> calcDirectJoinSets() {
    ...
    for (int i = 0; i < requiredJoinCount; i++) {
      ...
      if (log.isInfoEnabled()) log.info("joinSet: " + joinSet.vertices + " on " + joinSet.joinvars);
    }
    // All vertices must have been used.
    assert used.size() == requiredJoinCount
        : "used=" + used + ", but requiredJoinCount=" + requiredJoinCount;
    return joinSets;
  }

  private void expandJoinSet(
      final int i,
      final Set<Integer> joinSet,
