...
public class OwlSameAsPropertiesExpandingIterator implements IChunkedOrderedIterator<ISPO> {
  ...
  public OwlSameAsPropertiesExpandingIterator(
      IV s, IV p, IV o, AbstractTripleStore db, final IV sameAs, final IKeyOrder<ISPO> keyOrder) {
    this.db = db;
    this.s = s;
    this.p = p;
    this.o = o;
    ...
    */
    ...
    final IRule rule =
        new Rule(
            ...
            head,
            new IPredicate[] {
              new SPOPredicate(SPO, _sameS, sameAs, s, sameAsSelfExpander),
              new SPOPredicate(SPO, _sameO, sameAs, o, sameAsSelfExpander),
              new SPOPredicate(SPO, _sameS, _p, _sameO)
            },
            QueryOptions.DISTINCT, // distinct
            // constraints on the rule.
            new IConstraint[] {
              Constraint.wrap(new RejectSameAsSelf(head.s(), head.p(), head.o()))
            });
    runQuery(rule);
  }
    ...
    */
    ...
    final IRule rule =
        new Rule(
            ...
            head,
            new IPredicate[] {
              new SPOPredicate(SPO, _sameS, sameAs, s, sameAsSelfExpander), // ?s sameAs y
              new SPOPredicate(SPO, _sameS, _p, _o), // ?s b ?o  -> y b w
              new SPOPredicate(SPO, _o, sameAs, _sameO, true, sameAsSelfExpander)
            },
            QueryOptions.DISTINCT, // distinct
            // constraints on the rule.
            new IConstraint[] {
    ...
    final IRule rule =
        new Rule(
            ...
            new IPredicate[] {
              ...
              new SPOPredicate(SPO, _s, sameAs, _sameS, true, sameAsSelfExpander)
            },
            QueryOptions.DISTINCT, // distinct
            // constraints on the rule.
            new IConstraint[] {
    ...
    */
    ...
    final IRule rule =
        new Rule(
            ...
            head,
            new IPredicate[] {
              new SPOPredicate(SPO, _sameS, sameAs, _s, true, sameAsSelfExpander),
              new SPOPredicate(SPO, _sameO, sameAs, _o, true, sameAsSelfExpander),
  ...
  public ISPO next() {
    ...
    if (chunk == null || i == chunk.length) {
      chunk = nextChunk();
      i = 0;
      if (log.isInfoEnabled()) log.info("got a chunk, length = " + chunk.length);
    }
    return chunk[i++];
  }
  ...
  private class RejectSameAsSelf extends BOpBase implements BooleanValueExpression {
    ...
    public RejectSameAsSelf(
        final IVariableOrConstant<IV> _s,
