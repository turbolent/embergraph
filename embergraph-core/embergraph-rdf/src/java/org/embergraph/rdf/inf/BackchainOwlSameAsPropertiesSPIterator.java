...
/*
 * Provides backward chaining for property collection on owl:sameAs for the SP? and S?? access
 * paths.
 *
 * <p>Note: This is a simple properties query: we know s and we want to know all {p,o} tuples that
 * constitute the properties for s.
 *
 * @see InferenceEngine
 * @see InferenceEngine.Options
 * @author <a href="mailto:mpersonick@users.sourceforge.net">Mike Personick</a>
 */
public class BackchainOwlSameAsPropertiesSPIterator extends BackchainOwlSameAsIterator {
   ...
   *     </code>).
   * @param s The subject of the triple pattern. Cannot be null.
   * @param p The predicate of the triple pattern. Can be null.
  ...
  public BackchainOwlSameAsPropertiesSPIterator(
      IChunkedOrderedIterator<ISPO> src, IV s, IV p, AbstractTripleStore db, final IV sameAs) {
     ...
     */
    {
      ...
      while (samesIt.hasNext()) {
        ...
        while (propsIt.hasNext()) {
          ...
          if (IVUtility.equals(prop.p(), sameAs) && IVUtility.equals(s, prop.o())) {
            continue;
          }
  ...
  /*
   * First iterate the source iterator and then iterate the sameAs{2} iterator, which was fully
   * populated in the ctor. Along the way, collect up the sameAs{3} inferences, which will then be
  ...
  /*
   * Find all the alternate s values for this SPO, which we need to do since o is unbound in this
   * access path.
   *
   * @param spo the spo being visited by the source iterator or the sameAs{2} iterator
   */
  ...
  public void close() {
    ...
    if (sameAs2 != null) sameAs2.close();
    if (sameAs3 != null) sameAs3.close();
  }
