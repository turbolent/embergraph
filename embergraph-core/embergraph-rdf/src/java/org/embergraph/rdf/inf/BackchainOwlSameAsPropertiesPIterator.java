 ...
 */
public class BackchainOwlSameAsPropertiesPIterator extends BackchainOwlSameAsIterator {
  ...
  public BackchainOwlSameAsPropertiesPIterator(
      IChunkedOrderedIterator<ISPO> src, IV p, AbstractTripleStore db, final IV sameAs) {
   ...
   * connect up the {s,o} pairs. When p is unbound we are essentially visiting all predicates in the
   * db and then doing the same thing.
   *
   * @param spo the spo being visited by the source iterator
   */
  ...
  public void close() {
    ...
    if (sameAs2and3 != null) sameAs2and3.close();
  }
