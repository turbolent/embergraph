 ...
 * {@link #assertAll(TempTripleStore)} or {@link #retractAll(TempTripleStore)} as appropriate to
 * update the closure of the database, at which point the {@link TempTripleStore} is discarded
 * (closed and deleted). An instance of this class may be reused, but you need to obtain a new
 * {@link TempTripleStore} using {@link #newTempTripleStore()} each time you want to buffer more
 * {@link SPO}s after updating the closure.
 *
 * <p>Note: Neither this class nor updating closure is thread-safe. In particular, clients MUST NOT
 * write on either the database or the {@link TempTripleStore} when the closure is being updated.
 * The truth maintenance algorithm assumes that the database and tempStore are unchanging outside of
 * the actions taken by the algorithm itself. Concurrent writers would violate this assumption and
 ...
 *     <p>FIXME integrate with <a href="http://iris-reasoner.org/">IRIS</a> (magic sets) to avoid
 *     the necessity for truth maintenance and recover the space and time lost to the justification
 *     chains as well. See http://sourceforge.net/projects/iris-reasoner/ as well.
...
public class TruthMaintenance {
  ...
  /**
   * Return a new {@link TempTripleStore} backed by a shared {@link TemporaryStore} that may be used
   * to buffer {@link SPO}s to be either asserted or retracted from the database. It is recommended
   * to use this factory method to provision the tempStore as it disables the lexicon (it will not
   * be used) but leaves other features enabled (such as all access paths) which support truth
   * maintenance.
   *
   * <p>You can wrap this with an {@link IStatementBuffer} using:
   *
   * <pre>
   * new StatementBuffer(getTempTripleStore(), database, bufferCapacity);
   * </pre>
   *
   * and then write on the {@link IStatementBuffer}, which will periodically perform batch writes on
   * the tempStore.
   *
   * <p>Likewise, you can use {@link IRawTripleStore#addStatements(IChunkedOrderedIterator,
   * IElementFilter)} and it will automatically perform batch writes.
   *
   * <p>Regardless, when you have written all data on the tempStore, use {@link
   * #assertAll(TempTripleStore)} or {@link #retractAll(TempTripleStore)} to update the closure of
   * the database.
   *
   * <p>Note: DO NOT use {@link TempTripleStore#__tearDownUnitTest()} since the backing store is
   * shared!
   */
  public TempTripleStore newTempTripleStore() {
    ...
    properties.setProperty(org.embergraph.rdf.store.AbstractTripleStore.Options.LEXICON, "false");
    ...
    properties.setProperty(
        org.embergraph.rdf.store.AbstractTripleStore.Options.BLOOM_FILTER, "false");

    final TempTripleStore tempStore =
        new TempTripleStore(database.getIndexManager().getTempStore(), properties, database);
  ...
  /** @param inferenceEngine The inference engine for the database. */
  public TruthMaintenance(InferenceEngine inferenceEngine) {

    if (inferenceEngine == null) throw new IllegalArgumentException();
  ...
  /**
   * Any statements in the <i>fousStore</i> that are already in the database are converted to
   * explicit statements (iff they are not already explicit) and <strong>removed</strong> from the
   * <i>focusStore</i> as a side-effect. This prevents the application of the rules to data that is
   * already known to the database.
   *
   * @param focusStore The store whose closure is being computed.
   * @param database The database.
   * @param filter An optional filter. Statements matching the filter are NOT written on the
   *     database, but they are still removed from the focusStore.
   * @return The #of statements that were removed from the focusStore.
   * @todo this uses some techniques that are not scaleable if the focusStore is extremely large.
   */
  public static int applyExistingStatements(
      final AbstractTripleStore focusStore,
      final AbstractTripleStore database,
      final IElementFilter<ISPO> filter) {

    return applyExistingStatements(focusStore, database, filter, null /* changeLog */);
  }
  ...
  public static int applyExistingStatements(
    ...
    if (INFO) log.info("Filtering statements already known to the database");
    ...
    final IChunkedOrderedIterator<ISPO> itr =
        focusStore.getAccessPath(SPOKeyOrder.SPO, ExplicitSPOFilter.INSTANCE).iterator();
    ...
    try {
      ...
      final SPOAssertionBuffer assertionBuffer =
          new SPOAssertionBuffer(
              database, database, filter, capacity, false /* justified */, changeLog);
      ...
      final SPORetractionBuffer retractionBuffer =
          new SPORetractionBuffer(
   ...
   *
   * <p>This method computes the closure of the temporary store against the database, writing
   * entailments into the temporary store. Once all entailments have been computed, it then copies
   * the all statements in the temporary store into the database and deletes the temporary store.
   *
   * @param tempStore A temporary store containing statements to be asserted. The tempStore will be
   *     closed as a post-condition.
   */
   ...
   * @param changeLog optional change log for change notification
   */
  public ClosureStats assertAll(final TempTripleStore tempStore, final IChangeLog changeLog) {
    ...
    if (INFO)
      log.info("Computing closure of the temporary store with " + nbeforeClosure + " statements");
    ...
    applyExistingStatements(tempStore, database, inferenceEngine.doNotAddFilter, changeLog);
    ...
    if (INFO)
      log.info("There are " + nafterClosure + " statements in the temporary store after closure");

    /*
     * copy statements from the temporary store to the database.
     */

    if (INFO) log.info("Copying statements from the temporary store to the database");

    //        tempStore.dumpStore(database,true,true,false,true);

    final long ncopied =
        tempStore.copyStatements(
            database, null /* filter */, true /* copyJustifications */, changeLog);

    //        database.dumpStore(database,true,true,false,true);

    // note: this is the number that are _new_ to the database.
    if (INFO) log.info("Copied " + ncopied + " statements that were new to the database.");

    final long elapsed = System.currentTimeMillis() - begin;

    stats.elapsed.add(elapsed);

    if (INFO) log.info("Computed closure in " + elapsed + "ms");
   ...
   *
   * <p>When the closure is computed, each statement to be retracted is examined to determine
   * whether or not it is still entailed by the database without the support of the statements that
   * were explicitly retracted. Statements that were explicit in the database that are still
   * provable are converted to inferences. Statements which can no longer be proven (i.e., that are
   * not supported by a grounded {@link Justification} chain) are retracted from the database and
   * added into another temporary store and their justifications are deleted from the database. This
   * process repeats with the new temporary store until no fixed point (no more ungrounded
   * statements are identified).
   *
   * @param tempStore A temporary store containing explicit statements to be retracted from the
   *     database. The tempStore will be closed and as a post-condition.
   ...
   * @return statistics about the closure operation.
   */
  public ClosureStats retractAll(final TempTripleStore tempStore, final IChangeLog changeLog) {
    ...
    if (INFO) log.info("Computing closure of the temporary store with " + ngiven + " statements");
    ...
    if (database.getStatementIdentifiers()) {
      ...
      if (INFO)
        log.info(
            "Computing closure of the temporary store with "
                + ngiven
    ...
    if (INFO)
      log.info(
          "Retracted "
              + ngiven
              + " given and updated closure on the database in "
              + elapsed
              + " ms");
   ...
   *
   * <p>Note: When this is first called, the tempStore SHOULD contain only those statements that
   * were known to be explicit statements in the database. However, when this is invoked recursively
   * the tempStore will contain only inferred statements -- these are statements that are part of
   * the closure of the original statements that were retracted and whose grounded justifications we
   * are now seeking. Regardless of whether this is invoked with the explicitly given set of
   * statements to retracted or (recursively) with inferences that may need to be retracted, the
   * statements in the tempStore MUST still in the database (they should have been copied to the
   * tempStore, not removed from the database).
   *
   * <p>The steps are:
   *
   * <ol>
   *   <li>Create a focusStore into which we will write all statements that will not be grounded
   *       once the statements in the tempStore have been deleted from the database. In the first
   *       pass this will contain the original explicit statements which are no longer provable. In
   *       subsequent (recursive) passes it will contain any inferences drawn from those statements
   *       which are themselves no longer provable.
   *   <li>For each statement in the tempStore, determine whether or not it is has a grounded
   *       justification chain. If yes, then change the statement type to inferred in the database
   *       and we are done with that statement.
   *   <li>If there is no grounded justification chain for a statement then it gets put into a
   *       {@link SPOAssertionBuffer} writing on the focusStore and into an {@link
   *       SPORetractionBuffer} writing on the database (so that the statement and its {@link
   *       Justification}s get deleted from the database).
   *   <li>Once all statements in the tempStore have been processed we flush the various buffers and
   *       {@link TempTripleStore#close()} the tempStore.
   *   <li>We then compute the closure of the focusStore against the database in order to discover
   *       additional statements that may no longer be supported and hence will have to be
   *       retracted. This closure operation is similar to the incremental load closure but with two
   *       twists: (1) we do NOT generate {@link Justification} chains; and (2) we do NOT copy the
   *       result onto the database - instead we leave it in the focusStore.
   *   <li>Once we have the new set of statements to consider for retraction we simply invoke
   *       retractAll() again on that focusStore.
   * </ol>
   *
   * @param stats
   * @param tempStore
   * @param depth Recursive depth - this is ZERO(0) the first time the method is called. At depth
   *     ZERO(0) the tempStore MUST contain only the explicit statements to be retracted.
   * @param changeLog optional change log for change notification
   */
  private void retractAll(
    ...
    if (INFO)
      log.info("Doing truth maintenance with " + tempStoreCount + " statements : depth=" + depth);
    ...
    //      final TempTripleStore focusStore = new TempTripleStore(database.getProperties(),
    // database);
    ...
    try {
       ...
       * no longer grounded.
       */
      final SPOAssertionBuffer ungroundedBuffer =
          new SPOAssertionBuffer(
       ...
       */
      final SPOAssertionBuffer downgradeBuffer =
          new SPOAssertionBuffer(
              database, // focusStore,
      ...
      final SPORetractionBuffer retractionBuffer =
          new SPORetractionBuffer(
              database, capacity, false /* computeClosureForStatementIdentifiers */, changeLog);
      ...
      while (itr.hasNext()) {
        ...
        for (int i = 0; i < chunk.length; i++) {
          ...
          if (spo.getStatementType() == StatementEnum.Axiom) {
            ...
            final SPO tmp = new SPO(spo.s(), spo.p(), spo.o(), StatementEnum.Axiom);
            ...
            final SPO tmp = new SPO(spo.s, spo.p, spo.o, StatementEnum.Inferred);
    ...
    if (DEBUG && database.getStatementCount() < 200) {
      ...
      log.debug(
          "dumping database after retraction: depth="
              + depth
              + "\n"
              + database.dumpStore(
                  database,
     ...
     */
    {
      ...
      final SPOArrayIterator tmp =
          new SPOArrayIterator(
              focusStore,
              focusStore.getAccessPath(SPOKeyOrder.SPO),
              0 /* limit */,
              null /* filter */);
      ...
      if (DEBUG && database.getStatementCount() < 200) {
        ...
        log.debug(
            "focusStore before closure: depth="
                + depth
                + "\n"
                + focusStore.dumpStore(
                    database,
                    true /*explicit*/,
                    true /*inferred*/,
                    false /*axioms*/,
      ...
      if (DEBUG && database.getStatementCount() < 200) {
        ...
        log.debug(
            "focusStore after closure: depth="
                + depth
                + "\n"
                + focusStore.dumpStore(
                    database,
      ...
      final long nremoved =
          focusStore.removeStatements(tmp, false /* computeClosureForStatementIdentifiers */);

      if (DEBUG && database.getStatementCount() < 200) {

        log.debug(
            "focusStore after subtracting out tmp: depth="
                + depth
                + "\n"
                + focusStore.dumpStore(
                    database,
      ...
      if (INFO) log.info("removed " + nremoved + " from focusStore");
    }
