 ...
 * </pre>
 *
 * <p>Then the triple pattern for the tail is:
 *
 * <p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>where 0 reprents a {@link IRawTripleStore#NULL} term identifier.
 *
 * <p>So a justification chain for <code>rdf1</code> would be:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>In fact, the total bindings for the rule are represented as a long[] with the head occupying
 * the 1st N positions in that array and the bindings for the tail appearing thereafter in the
 * declared order of the predicates in the tail.
 *
 * <p>When a {@link StatementEnum#Explicit} statement is to be retracted from the database we need
 * to determined whether or not there exists a <strong>grounded</strong> justification for that
 * statement (same head). For each justification for that statement we consider the tail. If there
 * exists either an explicit statement that satisifies the triple pattern for the tail or if there
 * exists an inference that satisifies the triple pattern for the tail and the inference can be
 * proven to be grounded by recursive examination of its justifications, then the head is still
 * valid and is converted from an explicit statement into an inference.
 *
 * <p>This looks more or less like: <strong>Find all statements matching the pattern. If any are
 * explicit, then that part of the tail is grounded. If none are explicit, then chase the
 * justification recursively. Only retract a justification when it can no longer be
 * grounded.</strong>
 *
 * <p>The concept of grounded vs ungrounded justifications is described in <a
 * href="http://www.openrdf.org/doc/papers/inferencing.pdf">Inferencing and Truth Maintenance in RDF
 * Schema : Exploring a naive practical approach</a> by Jeen Broekstra and Arjohn Kampman.
 *
 * @todo the tails could be represented more efficiently if we only stored the variable bindings and
 *     not all values in each tail. however, we might then need the rule on hand in order to decode
 *     the tail(s) and substitute in the missing constants.
...
public class Justification implements Comparable<Justification> {
   ...
   *
   * <p>Divide the length by the #of terms in a statement #N and subtract one to find the #of
   * bindings for the tail. The first N entries are the head. The rest are the tail.
   *
   * <p>Note: A term identifier MAY be {@link IRawTripleStore#NULL} to indicate a wildcard.
   */
  final IV[] ivs;
   ...
   *
   * <p>Note: The {@link StatementEnum} associated with the head is actually unknown, but it is
   * marked as {@link StatementEnum#Inferred} in the returned object. In order to discover the
   * {@link StatementEnum} for the head you MUST either already know it (this is not uncommon) or
   * you MUST read one of the statement indices.
   *
   * @return
   */
  public SPO getHead() {
   ...
   *
   * <p>Note: The {@link StatementEnum} associated triple patterns in the tail is actually unknown,
   * but it is marked as {@link StatementEnum#Inferred} in the returned object. In fact, since the
   * tail consists of triple patterns and not necessarily fully bound triples, the concept of a
  ...
  public SPO[] getTail() {
    ...
    for (int i = 0; i < m; i++, j += N) {
      ...
      tail[i] = new SPO(ivs[j], ivs[j + 1], ivs[j + 2], StatementEnum.Inferred);
    }
   ...
   *     <p>FIXME If [rule == null] then this ctor will thrown an {@link AssertionError}. In the
   *     original rule execution layer the bindings were are long[] and the were encoded by
   *     position. At the moment the bindingSet is indexed by the {@link IVariable} (essentially by
   *     the variable name). This means that we MUST have the {@link IRule} on hand in order extract
   *     the bindings that we need.
   *     <p>Modify the new rule execution layer to assign variables an integer index in [0:nvars]
   *     for each rule and then we can do positional decoding of the binding set and loose the
   *     requirement for the rule when generating justifications.
   *     <p>This might not be that important for scale-out since the solutions are, I believe,
   *     processed solely in a local buffer for Insert and Delete and only serialized for Query.
   */
  ...
  /**
   * Used by the {@link JustificationTupleSerializer} to materialize justifications.
   *
   * @param ids The bindings on the head and tail(s).
   */
  ...
  /**
   * Serialize a justification as an index key. The key length is a function of the #of bindings in
   * the justification.
   *
   * @param keyBuilder A key builder.
   * @return The key.
   */
  public static byte[] getKey(IKeyBuilder keyBuilder, Justification jst) {
    ...
    if (jst == null) throw new IllegalArgumentException();
  ...
  /**
   * Places the justifications into an ordering that clusters them based on the entailment is being
   * justified.
   */
   ...
   *
   * @param focusStore The focusStore contains the set of statements that are being retracted from
   *     the database. When looking for grounded justifications we do NOT consider any statement
   *     that is found in this store. This prevents statements that are being retracted from
   *     providing either their own justification or the justiciation of any other statement that is
   *     being retracted at the same time.
   * @param db The database from which the statements are to be retracted and in which we will
   *     search for grounded justifications.
   * @param head A triple pattern. When invoked on a statement during truth maintenance this will be
   *     fully bound. However, during recursive processing triple patterns may be encountered in the
   *     tail of {@link Justification}s that are not fully bound. In such cases we test for any
   *     statement matching the triple pattern that can be proven to be grounded.
   * @param testHead When <code>true</code> the <i>head</i> will be tested against the database on
   *     entry before seeking a grounded justification chain. When <code>false</code> head will not
   *     be tested directly but we will still seek a grounded justification chain.
   * @param testFocusStore
   * @param visited A set of head (whether fully bound or query patterns) that have already been
   *     considered. This parameter MUST be newly allocated on each top-level call. It is used in
   *     order to avoid infinite loops by rejecting for further consideration any head which has
   *     already been visited.
   * @return True iff the statement is entailed by a grounded justification chain in the database.
  ...
  public static boolean isGrounded(
    ...
    if (testHead) {
      ...
      final IChunkedOrderedIterator<ISPO> itr =
          db.getAccessPath(head.s(), head.p(), head.o()).iterator();
    ...
    if (head.isFullyBound()) {
      ...
      final IJustificationIterator itr = new FullyBufferedJustificationIterator(db, head);
      ...
      while (itr.hasNext()) {
        ...
        if (DEBUG) log.debug("Considering:\n" + jst.toString(db));
        ...
        for (SPO t : tail) {
          ...
          if (!isGrounded(inf, focusStore, db, t, true /* testHead */, testFocusStore, visited)) {
            ...
            if (DEBUG) log.debug("Not grounded: tail=" + t.toString(db));
  ...
  /**
   * A collection of {@link SPO} objects (either fully bound or query patterns) that have already
   * been visited.
   *
   * <p>Note: This is a very specialized {@link SPO} set implementation. How it is created and
   * destroyed is tightly integrated with how {@link TruthMaintenance} works.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @todo this class is public only because of TestJustifications. it should be private.
   */
  public static class VisitedSPOSet {
    ...
    /**
     * Create an {@link SPO} set backed by a {@link BTree} on the temporary store associated with
     * the [focusStore] on which truth maintenance is being performed. The data written on this set
     * will not last longer than {@link Justification#isGrounded(InferenceEngine, TempTripleStore,
     * AbstractTripleStore, SPO, boolean, boolean)} . When that method exists it {@link #close()}s
     * this {@link VisitedSPOSet} which causes the {@link BTree} to be discarded but DOES NOT close
     * the backing store since it is still in use by {@link TruthMaintenance}.
     *
     * @param tempStore The backing store on which the set will be maintained. This is the
     *     [focusStore] for {@link TruthMaintenance}.
     */
    ...
    /**
     * @param spo
     * @return <code>true</code> iff the set did not already contain the element (i.e., if the
     *     element was added to the set).
     */
    public boolean add(final ISPO spo) {

      if (DEBUG) log.debug(spo.toString());
    ...
    /**
     * Discards anything written on the btree. If nothing has been written on the backing store yet
