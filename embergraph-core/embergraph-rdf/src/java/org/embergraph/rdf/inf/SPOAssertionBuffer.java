 ...
 * @version $Id$
 * @deprecated by {@link InsertBuffer} and {@link InsertSolutionBuffer} and the changes to how truth
 *     maintenance is handled (by rule rewrites).
 */
public class SPOAssertionBuffer extends AbstractSPOBuffer implements ISPOAssertionBuffer {
   ...
   *     (required). This is either the database or a temporary focusStore used during incremental
   *     TM.
   * @param db The database in which the terms are defined (required).
   * @param filter Option filter. When present statements matched by the filter are NOT retained by
   *     the {@link SPOAssertionBuffer} and will NOT be added to the <i>focusStore</i>.
   * @param capacity The maximum {@link SPO}s that the buffer can hold before it is {@link
   *     #flush()}ed.
   * @param justified true iff the Truth Maintenance strategy requires that we focusStore {@link
   *     Justification}s for entailments.
   */
  public SPOAssertionBuffer(
  ...
  public int flush() {
    ...
    if (numJustifications == 0) {
      ...
      if (changeLog == null) {
        ...
        n =
            org.embergraph.rdf.changesets.StatementWriter.addStatements(
                db,
                focusStore,
                true /* copyOnly */,
                null /* filter */,
