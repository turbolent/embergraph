 ...
 * instance of this class via the {@link #changeEvent(IChangeRecord)} method. These events will
 * occur on an ongoing basis as statements are added to or removed from the indices. It is the
 * change log's responsibility to collect change records. When the transaction is actually committed
 * (or aborted), the change log will receive notification via {@link #transactionCommited()} or
...
public interface IChangeLog {
   ...
   *
   * <p>Note: The listener will have observed all updates by the time this message is generated.
   * Thus, this message can be used to validate post-conditions for the transaction.
   */
