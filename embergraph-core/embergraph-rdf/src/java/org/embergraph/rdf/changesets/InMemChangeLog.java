 ...
 * buffer can be drained by another thread that doesn't block the actual read/write operations,
 * although then we need to be careful not to issue the committed() notification before the buffer
 * is drained).
 *
 * @author mike
 */
public class InMemChangeLog implements IChangeLog {
  ...
  public synchronized void transactionCommited(final long commitTime) {
    ...
    if (log.isInfoEnabled()) log.info("transaction committed: " + commitTime);
   ...
   * addition to just the internal identifiers (IVs) for those terms.
   *
   * @param db the database containing the lexicon needed to materialize the EmbergraphStatement
   *     objects
   * @param unresolved the ISPO change records that came from IChangeLog notification events
   * @return the fully resolves EmbergraphStatement change records
   */
