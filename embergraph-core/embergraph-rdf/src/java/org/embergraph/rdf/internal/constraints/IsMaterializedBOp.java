...
public class IsMaterializedBOp extends XSDBooleanIVValueExpression {
  ...
  public IsMaterializedBOp(final BOp[] args, final Map<String, Object> anns) {
    ...
    if (getProperty(Annotations.MATERIALIZED) == null) throw new IllegalArgumentException();
  }
  ...
  public boolean accept(final IBindingSet bs) {
    ...
    final boolean materialized = (Boolean) getRequiredProperty(Annotations.MATERIALIZED);

    final IV<?, ?> iv = get(0).get(bs);

    if (log.isDebugEnabled()) {
      log.debug(iv);
      if (iv != null) log.debug("materialized?: " + iv.hasValue());
    }
