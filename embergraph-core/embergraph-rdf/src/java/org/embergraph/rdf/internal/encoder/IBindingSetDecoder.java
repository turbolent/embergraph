...
public interface IBindingSetDecoder {
   ...
   * the resolved {@link IVCache} associations. For example, we do not need the {@link IVCache}
   * association in order to decide if two {@link IBindingSet}s can join. However, once we have a
   * solution from a join, we may need to resolve the {@link IVCache} metadata for the joined
   * solution.
   *
   * @param val The encoded IV[].
   * @param off The starting offset.
   * @param len The #of bytes of data to be decoded.
   * @param resolveCachedValues When <code>true</code>, any decoded {@link IV}s will have their
   *     {@link IVCache} association resolved before the {@link IBindingSet} is returned to the
   *     caller. When <code>false</code>, the resolution step is not performed.
   ...
   * when joining a decoded solution with another solution, the resolution step may be deferred
   * until we know whether or not the join was successful.
   *
   * @param bset A solution having {@link IV}s which need to be reunited with their cached {@link
   *     EmbergraphValue}s.
   */
