 ...
 * TERMS index for the tuple for a given RDF {@link Value}. While flexible, {@link Value}s models as
 * {@link BlobIV}s require indirection through the TERMS index in order to materialize the {@link
 * Value}. We are often able to inline {@link Value}s compactly and efficiently into the statement
 * indices, in which case those values simply do not appear in the TERMS index. The {@link BlobIV}
 * remains useful when the RDF {@link Value} is large and not otherwise amenable to inlining.
 *
 * <p>Note: The {@link BlobIV} uses a hash code based on the hash code of the RDF {@link Value}.
 * This gives us a fixed length key for the TERMS index, even when the RDF {@link Value} is very
 * large. This makes the {@link BlobIV} suitable for use with very large RDF {@link Value}s.
 * However, the hash code results in widely dispersed updates which cause a lot of IO scatter. This
 * makes the TERMS index slower on write than the TERM2ID and ID2TERM indices. Therefore shorter RDF
 * {@link Value}s should be inserted by preference into the TERM2ID / ID2TERM indices while larger
 * values should be inserted as {@link BlobIV}s into the TERMS index.
 */
public class BlobIV<V extends EmbergraphValue> extends AbstractNonInlineExtensionIV<V, Void> {
   ...
   * </code>
   *
   * @see IVUtility#fromString(String), which parses the string value returned by this method.
   */
  ...
  public static BlobIV<?> fromString(final String s) {
    ...
    if (pos == -1) throw new RuntimeException("Not a BlobIV: [" + s + "]");
   ...
   * this simply copies the private unsigned byte[] into the {@link IKeyBuilder} since it is already
   * the exact key for the {@link BlobIV}.
   */
  @Override
  public final IKeyBuilder encode(final IKeyBuilder keyBuilder) {
  ...
  public String getID() {
    ...
    final long id =
        ((long) flags()) << 56 /* hash:int + counter:short */
            | ((long) hash) << 16 /* short */
            | counter;
