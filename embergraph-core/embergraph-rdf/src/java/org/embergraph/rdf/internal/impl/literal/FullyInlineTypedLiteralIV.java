 ...
 *
 * <p>Note: Optimized support for <code>xsd:string</code> inlining is provided by the {@link
 * XSDStringExtension}.
 *
 * <p>TODO Validate methods on this class against {@link Literal} and {@link EmbergraphLiteralImpl}
 * (API compliance).
 */
public class FullyInlineTypedLiteralIV<V extends EmbergraphLiteral>
    extends AbstractLiteralIV<V, String> implements IInlineUnicode, Literal {
  ...
  /** The Literal's language code (optional but exclusive with the {@link #datatype}). */
   ...
   * the triple store. <code>Temporary</code> {@link IV}s have to be recreated while executing
   * update, as new terms may need adding to Term2ID and ID2Term indexes.
   *
  ...
  public FullyInlineTypedLiteralIV(
      final String label, final String languageCode, final URI datatypeURI) {

    this(label, languageCode, datatypeURI, 0 /* byteLength */, false /* temp */);
  }

  public FullyInlineTypedLiteralIV(
      final String label, final String languageCode, final URI datatypeURI, final boolean temp) {

    this(label, languageCode, datatypeURI, 0 /* byteLength */, temp);
  }

  public FullyInlineTypedLiteralIV(
      final String label, final String languageCode, final URI datatypeURI, final int byteLength) {
  ...
  public FullyInlineTypedLiteralIV(
    ...
    if (languageCode != null && datatypeURI != null) throw new IllegalArgumentException();
  ...
  public boolean equals(final Object o) {
    ...
    if (!(o instanceof FullyInlineTypedLiteralIV<?>)) return false;

    final FullyInlineTypedLiteralIV<?> t = (FullyInlineTypedLiteralIV<?>) o;

    if (!label.equals(t.getLabel())) return false;
          ...
          1 // flags
              ...
              + ((termCode == ITermIndexCodes.TERM_CODE_LCL)
                  ...
                  : (termCode == ITermIndexCodes.TERM_CODE_DTL)
                      ? IVUnicode.byteLengthUnicode(datatype.stringValue())
                      : 0)
   ...
   * The Unicode compression scheme (BOCU) preserves the code point order, so we can compare the
   * {@link String} values of the datatype URI and/or language code and the labels to determine the
   * ordering. The ordering provided is disjoint across distinct datatypes and language codes. It is
   * the same order which is present for the keys in the index. The [termCode] is used to partition
   * the space into plain literals, language code literals, and datatype literals. Orderings are
   * defined within those partitions.
   */
  @Override
  public int _compareTo(final IV o) {
    ...
    switch (termCode) {
      ...
      case ITermIndexCodes.TERM_CODE_DTL:
        ret =
            IVUnicode.IVUnicodeComparator.INSTANCE.compare(
                datatype.stringValue(), t.datatype.stringValue());
