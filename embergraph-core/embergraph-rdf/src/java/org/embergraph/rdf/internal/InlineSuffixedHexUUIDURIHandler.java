...
public class InlineSuffixedHexUUIDURIHandler extends InlineHexUUIDURIHandler {
  ...
  protected AbstractLiteralIV createInlineIV(final String localName) {
    if (localName.endsWith(suffix)) {
      return super.createInlineIV(
          localName.substring(0, localName.length() - this.suffix.length()));
    }
  ...
  public String getLocalNameFromDelegate(AbstractLiteralIV<EmbergraphLiteral, ?> delegate) {
    ...
    final String localName = super.getLocalNameFromDelegate(delegate) + suffix;
    return localName;
  }
}
