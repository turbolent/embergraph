 ...
 *
 * <p>As a contract, the class provides guarantees that the representation of a given binding set is
 * always the same. This makes it possible to use an HTree based DistinctFilter over encoded binding
 * sets.
 *
 * <p>Note: This implementation does NOT maintain the {@link IVCache} associations.
 *
...
public class IVBindingSetEncoder implements IBindingSetEncoder, IBindingSetDecoder {
   ...
   * solutions flow through the filter.
   */
   ...
   *     original solutions flow through the filter.
   */
  ...
  @Override
  public byte[] encodeSolution(final IBindingSet bset, final boolean updateCache) {
     ...
     * last variable in the schema. This is required because we need *exactly the same syntactical
     * representation* when encoding the same mapping at a later point (which is required because we
     * need to do equality lookups on the values when storing encoded binding sets in the HTree). As
     * an example, assume we encounter (and encode) the following sequence of mapping sets:
     *
     * <p>1.) { ?a -> 1 } 2.) { ?a -> 2, ?b -> 1 } 3.) { ?b -> 1 } 4.) { ?a -> 1 } 5.) { ?c -> 1 }
     * 6.) { ?a-> 2, ?c -> 1 }
     *
     * <p>Here's how it works:
     *
     * <p>1.) When processing the first binding set, we do not yet know the full extend of the
     * schema, all that is known at this point is that ?a belongs to the schema. So our encoding
     * will be enc(1) and the first schema variable is recorded to be ?a: => schema:=[?a] / encoded
     * value:=enc(1)
     *
     * <p>2.) We extend the schema to schema:=[?a,?b] and encode value:=enc(2).enc(1). Note that
     * both positions are present.
     *
     * <p>3.) The schema remains schema:=[?a,?b]. In order to account for the fact that we now
     * encode ?b, we must pad an "empty" encoded binding, i.e. we encode enc(null).enc(1).
     *
     * <p>4.) The interesting part comes now (and this is what was failing in the bug), when we see
     * our initial mapping again. The schema remains unchanged, but we must not encode two values in
     * that case, i.e. the result *must* be the same as in step 1.). We can do that by encoding *up
     * to the latest position for which there is a variable in the binding set. By following this
     * rule, we will always obtain the same result that we computed at the time when we encoded the
     * mapping for the first time, just because that's what we did last time. In our case, the only
     * variable in the binding set is ?a, so we only iterate up to the first position and get
     * encoded value:=enc(1), just like in step 1.).
     *
     * <p>5.) ?c is a new variable, so we set schema:=[?a,?b,?c] and encode the value as
     * enc(null).enc(null).enc(1).
     *
     * <p>6.) Following our rules, we keep the schema and get enc(2).enc(null).enc(1).
     *
     * <p>The key observation here is that this strategy gives us a bidirectional and unique mapping
     * scheme.
     */
    ...
    while (vitr.hasNext() && numBoundInBSet > 0) {
      ...
      if (c == null) {
        ...
        if (iv.isNullIV()) {
           ...
           * properly decoded as a mocked IV later on.
           */
  ...
  @Override
  public IBindingSet decodeSolution(
    ...
    for (IVariable<?> v : schema) {
      ...
      if (iv instanceof MockedValueIV) {
        ...
        if (innerIv instanceof FullyInlineURIIV) {
          ...
          final FullyInlineTypedLiteralIV innerIvAsLiteral = (FullyInlineTypedLiteralIV) innerIv;
          ...
          final FullyInlineUnicodeBNodeIV innerIvAsBNode = (FullyInlineUnicodeBNodeIV) innerIv;
    ...
    if (resolveCachedValues) resolveCachedValues(bset);
