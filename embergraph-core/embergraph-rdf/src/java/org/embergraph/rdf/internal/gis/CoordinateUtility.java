...
public class CoordinateUtility {
   ...
   * of the Earth).
   *
   * @param degreesNorth The latitude (north/south).
   * @return The real width of a degree of longitude at sea level at the given latitude.
  ...
  public static double realMetersPerDegreeOfLongitudeAtSeaLevel(double degreesNorth) {
    ...
    final double nom = (equatorialRadius4 * cos * cos) + (polarRadius4 * sin * sin);
    final double denom = Math.pow(equatorialRadius * cos, 2d) + Math.pow(polarRadius * sin, 2d);
    /*
     ...
     */
    final double lengthOfArc = _pi_div_180 * Math.cos(radians) * radiusAtLatitude;
  ...
  public static CoordinateDD boundingBoxSouthWest(
    ...
    final double deltaNorthSouth = distanceAsMeters / metersPerDegreeOfLatitudeAtSeaLevel;

    // compute numbers of degrees to travel to the left
    final Double currentLat = start.northSouth;

    final Double deltaEastWest =
        (1 / (111320 * Math.cos(currentLat / 360 * 2 * Math.PI))) * distanceAsMeters;
    ...
    final CoordinateDD ret = new CoordinateDD(start.northSouth - deltaNorthSouth, lon, true);
  ...
  public static void assertDegreeLatitude(double d) {
    if (d <= -90d || d > 90d) throw new IllegalArgumentException("" + d + " is not in [90:-90)");
  }
...
// * The real width of a longitudinal degree on a given latitude.
   ...
   * but does not account for the flattening of the Earth.
   *
   * @param degreesNorth The latitude (north/south).
  ...
  public static double approxMetersPerDegreeOfLongitudeAtSeaLevel(double degreesNorth) {
     ...
     */
    final double lengthOfArc = _pi_div_180 * Math.cos(radians) * averageRadius;
    return lengthOfArc;
  }
...
// * Return the angle converted to the transverse graticule that is used by
    ...
    double distRad =
        Math.acos(
            Math.sin(toRadians(latP1)) * Math.sin(toRadians(latP2))
                + Math.cos(toRadians(latP1))
                    * Math.cos(toRadians(latP2))
                    * Math.cos(toRadians(lonP1 - lonP2)));
   ...
   * @param minutes Minutes (w/ fraction).
   * @param seconds Seconds (w/ fractial seconds).
