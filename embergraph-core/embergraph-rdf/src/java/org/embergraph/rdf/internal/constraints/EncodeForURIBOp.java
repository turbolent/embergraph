...
public class EncodeForURIBOp extends IVValueExpression<IV> implements INeedsMaterialization {
  ...
  public EncodeForURIBOp(final IValueExpression<? extends IV> x, final GlobalAnnotations globals) {
    super(x, globals);
  }
  ...
  public IV get(final IBindingSet bs) throws SparqlTypeErrorException {
    ...
    try {
      final EmbergraphLiteral str =
          getValueFactory()
              .createLiteral(URLEncoder.encode(lit.getLabel(), "UTF-8").replace("+", "%20"));
