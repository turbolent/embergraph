 ...
 * handler to apply if there are multiple namespaces that use different prefixed or suffixed inline
 * handlers.
 *
 * <p>For example: <code>
 ...
 * </code> Each of these can be supported as a different {@link
 * InlinePrefixedSuffixedIntegerURIHandler} at the namespace
 * http://rdf.ncbi.nlm.nih.gov/pubchem/descriptor/.
 *
 * <p>It current supports up to 32 different inline values for a single namespace URI. It is used in
 * conjunction with the {@link InlineIntegerURIHandlerMap}.
 *
 * <p>For an example, see {@see org.embergraph.vocab.pubchem.PubchemInlineURIFactory}.
 *
 * <p>The max inline integer value that can supported in this scheme is 2^57 - 1, which is 2^5 less
...
public abstract class InlineLocalNameIntegerURIHandler extends InlineSignedIntegerURIHandler {
  ...
  /** The maximum int value supported with packing. 2^57 -1 */
