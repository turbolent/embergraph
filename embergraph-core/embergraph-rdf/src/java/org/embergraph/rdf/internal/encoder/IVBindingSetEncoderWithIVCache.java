 ...
 *
 * <p>Note: Two different {@link BTree} instances are used. One for {@link TermId}s and another for
 * {@link BlobIV}s. These indices use exactly the same schema as the ID2TERM and BLOBS indices.
 *
...
public class IVBindingSetEncoderWithIVCache extends IVBindingSetEncoder {
   ...
   * @param op The operator whose annotations are used to parameterize the creation of the backing
   *     indices for the {@link IV} to {@link EmbergraphValue} cache.
   */
  public IVBindingSetEncoderWithIVCache(final IRawStore store, final boolean filter, final BOp op) {
    ...
    if (!filter) {
      ...
      namespace = ((String[]) op.getRequiredProperty(Predicate.Annotations.RELATION_NAME))[0];
  ...
  private void checkpointBTree(final AtomicReference<BTree> ref) {
    ...
    if (tmp != null) {
      ...
      final BTree readOnly =
          BTree.load(tmp.getStore(), checkpoint.getCheckpointAddr(), true /* readOnly */);
  ...
  public void flush() {
    ...
    final Id2TermTupleSerializer tupSer =
        (Id2TermTupleSerializer) ivCache.getIndexMetadata().getTupleSerializer();
   ...
   * collect up a bunch of solutions which needed resolution, then collect up the IVs which do not
   * have cached values for variables which might have values in the ivCache. We would then sort the
   * IVs and do a vectored resolution against the ivCache. Finally, the solutions could be output in
   * a chunk with their resolved Values.
   *
   * <p>If the operator is not vectored, then we should just fold it into {@link
  ...
  public void resolveCachedValues(final IBindingSet bset) {
    ...
    final Tuple ivCacheTuple = new Tuple(ivCache, IRangeQuery.KEYS | IRangeQuery.VALS);
  ...
  void cacheSchemaAndValue(final IVariable<?> v, final IV<?, ?> iv, final boolean updateCache) {
    ...
    if (iv.needsMaterialization() && iv.hasValue() && !filter) {
      ivCacheSchema.add(v);
      if (updateCache && cache != null) cache.put(iv, iv.getValue());
    }
  }
}
