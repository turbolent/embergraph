...
public class IVSolutionSetDecoder implements IBindingSetDecoder {
   ...
   * for which bindings have been observed) is assembled incrementally from the decoded solutions
   * and the encoding is sensitive to the order in which the variables are first observed. Also, the
   * presence of the version field in the first solution makes it impossible to re-process a stream
   * of solutions with the same decoder.
   */
  @Override
  public IBindingSet decodeSolution(
      final byte[] data, int off, int len, final boolean resolveCachedValues) {
   ...
   * @param resolveCachedValues <code>true</code> if {@link IVCache} associations should be resolved
   *     as the solutions are decoded.
   * @return The next decoded solution.
   */
  public IBindingSet decodeSolution(final DataInputBuffer in, final boolean resolveCachedValues) {
  ...
  private IBindingSet _decodeSolution(
    ...
    try {
      ...
      final int nbytesVarBits = numBindings == 0 ? 0 : BytesUtil.bitFlagByteLength(schemaSize);
      ...
      final int nbytesCacheValueBits =
          newCached == 0 ? 0 : BytesUtil.bitFlagByteLength(numBindings);
      ...
      if (numBindings > 0) {
        ...
        while (i < schemaSize) {
          ...
          if (isSet) {
            ...
            final IV<?, ?> iv = IVUtility.decodeFromOffset(data, ivoff, false /* nullIsNullRef */);
      ...
      if (newCached > 0) {
        ...
        while (i < numBindings) {
          ...
          if (isSet) {
            ...
            final EmbergraphValue value = valueSer.deserialize(in, tmp);
            if (!iv.isNullIV()) {
              cache.put(iv, value);
            } else {
              /*
      ...
      if (numBindings > 0) resolveCachedValues(bset);
