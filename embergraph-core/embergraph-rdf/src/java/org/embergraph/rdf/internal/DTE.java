 ...
 * actually inlined is a configuration option for the lexicon.
 *
 * <p>If a data type is not inlined, then the representation of the value must be materialized.
 * Non-inline values are {@link BlobIV}s and are materialized by looking {@link IV} in the TERMS
 * index. More recently, we are also permitting indirection from the TERMS index. For example, very
 * large objects may be stored in the file system, in S3, etc. In those cases, you must still
 * resolve the {@link IV} against the TERMS index. Now the resulting object may be a {@link
 * EmbergraphValue}. However, the object materialized from the TERMS index may also provide
 * indirection into the file system, S3, etc.
 *
 * <p>The {@link VTE} has 4 distinctions (URI, Literal, BlankNode, and SID) and is coded in the high
 * 2 bits of a byte while the {@link DTE} has 16 possible distinctions, one of which is reserved
 * against future use and one of which is reserved against extensibility in the set of intrinsic
 * types.
 *
 * <p>Note: Unicode values CAN NOT be inlined because (a) Unicode sort keys are not decodable; and
 * (b) the collation rules for Unicode depend on the lexicon configuration, which specifies
 * parameters such as Locale, Strength, etc.
 *
 * <p>Blanks nodes (their IDs are UUIDs) and data types with larger values (UUIDs) or varying length
 * values (xsd:integer, xsd:decimanl) can be inlined. Whether it makes sense to do so is a question
 * which trades off redundancy in the statement indices for faster materialization of the data type
 * values and a smaller lexicon. UUIDs for purposes other than blank nodes can also be inlined,
 * however they will have a different prefix to indicate that they are Literals rather than blank
 * nodes.
 *
 * <p>Note: While multidimensional data types (such as points or rectangles) could be inlined (in
 * the sense that their values could be converted to unsigned byte[] keys and the keys could be
 * decoded), they can not be placed into a total order which has meaningful semantics by a single
 * index. For example, geo:point describes a 2-dimensional location and lacks any meaningful
 * locality when inlined into the index. The only reason to inline such data types is to avoid
 * indirection through the lexicon to materialize their values. Efficiently resolving points in a
...
public enum DTE {
   ...
   */
  XSDBoolean((byte) 0, Bytes.SIZEOF_BYTE, Boolean.class, XSD.BOOLEAN, DTEFlags.NOFLAGS),

  /** The "inline" value is a signed byte (xsd:byte). */
  XSDByte((byte) 1, Bytes.SIZEOF_BYTE, Byte.class, XSD.BYTE, DTEFlags.NUMERIC),

  /** The "inline" value is a signed short (xsd:short). */
  XSDShort((byte) 2, Bytes.SIZEOF_SHORT, Short.class, XSD.SHORT, DTEFlags.NUMERIC),

  /** The "inline" value is a signed 4 byte integer (xsd:int). */
  XSDInt((byte) 3, Bytes.SIZEOF_INT, Integer.class, XSD.INT, DTEFlags.NUMERIC),

  /** The "inline" value is a signed 8 byte integer (xsd:long). */
  XSDLong((byte) 4, Bytes.SIZEOF_LONG, Long.class, XSD.LONG, DTEFlags.NUMERIC),
  ...
  XSDUnsignedByte(
      (byte) 5, Bytes.SIZEOF_BYTE, Byte.class, XSD.UNSIGNED_BYTE, DTEFlags.UNSIGNED_NUMERIC),

  /** The "inline" value is a unsigned short (xsd:unsignedShort). */
  XSDUnsignedShort(
      (byte) 6, Bytes.SIZEOF_SHORT, Short.class, XSD.UNSIGNED_SHORT, DTEFlags.UNSIGNED_NUMERIC),

  /** The "inline" value is an unsigned 4 byte integer (xsd:unsignedInt). */
  XSDUnsignedInt(
      (byte) 7, Bytes.SIZEOF_INT, Integer.class, XSD.UNSIGNED_INT, DTEFlags.UNSIGNED_NUMERIC),

  /** The "inline" value is an unsigned 8 byte integer (xsd:unsignedLong). */
  XSDUnsignedLong(
      (byte) 8, Bytes.SIZEOF_LONG, Long.class, XSD.UNSIGNED_LONG, DTEFlags.UNSIGNED_NUMERIC),
  ...
  XSDFloat((byte) 9, Bytes.SIZEOF_FLOAT, Float.class, XSD.FLOAT, DTEFlags.NUMERIC),

  /** The "inline" value is a double precision floating point number (xsd:double). */
  XSDDouble((byte) 10, Bytes.SIZEOF_DOUBLE, Double.class, XSD.DOUBLE, DTEFlags.NUMERIC),
   ...
   * representations of ZERO with different precision, etc. If we had to represent the precision, we
   * could not use xsd:decimal in an index!)
   */
  XSDDecimal((byte) 12, 0 /* variable length */, BigDecimal.class, XSD.DECIMAL, DTEFlags.NUMERIC),
   ...
   * the growth in the B+Tree leaf size for a statement index versus the overhead required when
   * indirecting through the {@link LexiconRelation} to materialize the RDF {@link Value}. Further,
   * there is a practical upper bound on the size of a key in the B+Tree. Therefore, inlining of
   * Unicode values having between 32 and 64 characters is suggested as a recommended practice.
   * Beyond that, inlining can contribute significantly to the growth in the B+Tree leaf size and
   * have a negative impact on join performance for the statement indices.
   *
   * <p>This {@link DTE} may be used in combination with {@link VTE} as follows:
   *
   * <dl>
   *   <dt>{@link VTE#BNODE}
   *   <dd>Represent an inline blank node identifier.
   *   <dt>{@link VTE#LITERAL}
   *   <dd>Represent a plain literal.
   *   <dt>{@link VTE#LITERAL} plus the extension bit
   *   <dd>Represent a data type literal where the extension IV is the data type of the literal.
   *   <dt>{@link VTE#URI} plus the extension bit
   *   <dd>Represent a URI where the extension IV is the namespace of the URI and the inline Unicode
   *       component is the local name of the URI. This depends on the openrdf definition of a
   *       {@link URI}'s namespace and local name (basically, everything after the last '/' in the
   *       URI path or after the '#' if there is a URI anchor).
   * </dl>
   */
   ...
   * {@link DTEExtension}.
   *
   * <p>Note: This is NOT the same as the {@link AbstractIV#isExtension()} bit. The latter
   * <strong>always</strong> indicates that an {@link IV} follows the <code>flags</code> byte and
   * indicates the actual datatype URI. In contrast, {@link DTE#Extension} gives you another byte
   * which you can use to handle additional "intrinsic" types.
   *
   * @see DTEExtension
   ...
   *     (xsd:integer, xsd:decimal).
   * @param cls The class of the Java object used to represent instances of the coded data type.
   ...
   * @return The {@link DTE} for that datatype -or- {@link #Extension} if the datatype is <code>null
   *     </code> (there is no specific datatype for an extension since extensions by their nature
  ...
  public static final DTE valueOf(final URI datatype) {
    ...
    //            return Extension;
    //        }
    if (datatype.equals(XSD.BOOLEAN)) return XSDBoolean;
    if (datatype.equals(XSD.BYTE)) return XSDByte;
    if (datatype.equals(XSD.SHORT)) return XSDShort;
    if (datatype.equals(XSD.INT)) return XSDInt;
    if (datatype.equals(XSD.LONG)) return XSDLong;
    if (datatype.equals(XSD.UNSIGNED_BYTE)) return XSDUnsignedByte;
    if (datatype.equals(XSD.UNSIGNED_SHORT)) return XSDUnsignedShort;
    if (datatype.equals(XSD.UNSIGNED_INT)) return XSDUnsignedInt;
    if (datatype.equals(XSD.UNSIGNED_LONG)) return XSDUnsignedLong;
    if (datatype.equals(XSD.FLOAT)) return XSDFloat;
    if (datatype.equals(XSD.DOUBLE)) return XSDDouble;
    if (datatype.equals(XSD.INTEGER)) return XSDInteger;
    if (datatype.equals(XSD.DECIMAL)) return XSDDecimal;
    if (datatype.equals(XSD.UUID)) return UUID;
    if (datatype.equals(XSD.STRING)) return XSDString;
    /*
  ...
  /** An <code>byte</code> value whose whose lower 6 bits code the {@link DTE}. */
  public final byte v() {
    return v;
  }
