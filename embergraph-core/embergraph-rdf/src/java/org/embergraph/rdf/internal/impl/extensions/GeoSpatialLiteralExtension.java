 ...
 *
 * <p>The code to create a literal is, e.g.: <code>
 * "2#4"^^<http://www.embergraph.org/rdf/geospatial#geoSpatialLiteral></code> The two components are
 * first broken down long integers, namely - 2 -> 00000000 00000000 00000010 - 4 -> 00000000
 * 00000000 00000100
 *
 * <p>The z-order encoding of these two strings is then 00000000 00000000 00000000 00000000 00000000
 * 00000000 00011000
 *
 * <p>Interpreted as BigInteger, this is the value 24, which is stored as integer literal in the
 * database. The asValue method reverts this (lossless) encoding.
 *
...
public class GeoSpatialLiteralExtension<V extends EmbergraphValue> implements IExtension<V> {
   ...
   *
   * <p>A.) We get as value a literal such as "2.54#3.21"^^geo:geoSpatialLiteral
   *
   * <p>B.) The literal is split into its components.
   *
   * <p>C.) The components are mapped to long values (either trivially, if they represent long
   * values according to the schema, or based on a precision); for instance, assuming precision=2 in
   * the schema for the two components, the literal above would be converted into [254,321]
   *
   * <p>D.) Next, from these long components, we compute the z-order string: D1.) If specified in
   * the schema, we apply a range shift based on the minimum value known to shop up in the data.
   * Assuming, e.g., our minimum value is zero, the components are shifted as [Long.MIN+254,
   * Long.MIN+321]. D2.) We compute the z-order string by mixing up the components bit
   * representation. For instance (not matching the values from the example above), if component one
   * has bit representation 0011 and component two has bit representation 0110, we mix the bits as
   * 00101101, where position 0,2,4,6 represent the second string, position 1,3,5,7 represent the
   * first string.
   *
   * <p>E.) We pad a 0 byte to the z-order string, to make sure that the BigInteger constructor
   * (which expects a two's complement), does not destroy order.
   *
   * <p>F.) The 0 byte padded string is converted into a BigInteger
   *
   * <p>G.) The BigInteger is converted to an XSDIntegerIV
   * ************************************************************************
   */
  ...
  /** Create a two-components byte[] from a component array. Implements transformation B->E */
   ...
   *
   * <p>Longs (or other objects being parseable as Long) are copied to the target array without
   * modification. Floats (or objects being parseable as Float) are converted into Long according to
   * the precision specified in the passed {@link SchemaDescription}.
   *
   * <p>Implements step B->C.
   */
  public final long[] componentsAsLongArr(
    ...
    if (numDimensions != components.length) {
      throw new InvalidGeoSpatialLiteralError(
          "Literal value has wrong format. Expected "
              + numDimensions
              + " components for datatype, but literal has "
              + components.length
              + " components.");
    }
    ...
    try {
      ...
      for (int i = 0; i < components.length; i++) {
        ...
        switch (fieldConfig.getValueType()) {
          case DOUBLE:
            {
              ...
              final BigDecimal componentAsBigDecimal =
                  component instanceof BigDecimal
                      ? (BigDecimal) component
                      : new BigDecimal(component.toString());
          ...
          case LONG:
            {
              final BigInteger precisionAdjustment =
                  BigInteger.valueOf(fieldConfig.getMultiplier());

              final BigInteger componentAsBigInteger =
                  component instanceof BigInteger
                      ? (BigInteger) component
                      : new BigInteger(component.toString());
          ...
          default:
            throw new IllegalArgumentException(
                "Invalid field configuration: value type not supported.");
        }
      }
  ...
  public byte[] toZOrderByteArray(
    ...
    for (int i = 0; i < componentsAsLongArr.length; i++) {
      ...
      final long componentAsLongRangeShifted =
          minValue == null ? componentAsLong : encodeRangeShift(componentAsLong, minValue);
   ...
   * differ. More concretely, having padded the zero, we may safely call the {@link BigInteger}
   * constructor (which expects a two's complement input).
   *
   * <p>Implements step D->E.
   */
   ...
   * xsd:integer.
   *
   * <p>Implements transformation G->C
   *
   * @param iv
   * @return
   */
   ...
   *
   * <p>Implements transformation D2 -> C.
   */
  ...
  /** Invert {@link #encodeRangeShift(Long, Long)} operation. Implements steps D1->C. */
   ...
   *
   * <p>Implements step C->B.
   */
  public final Object[] longArrAsComponentArr(final long[] arr) {

    final int numDimensions = datatypeConfig.getNumDimensions();
    if (arr.length != numDimensions) {
      throw new IllegalArgumentException(
          "Encoding has wrong format. Expected " + numDimensions + " components for datatype.");
    }
