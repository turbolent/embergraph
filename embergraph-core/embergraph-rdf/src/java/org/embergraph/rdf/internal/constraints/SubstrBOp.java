...
public class SubstrBOp extends IVValueExpression<IV> implements INeedsMaterialization {
  ...
  public IV get(final IBindingSet bs) throws SparqlTypeErrorException {
    ...
    // see also https://jira.blazegraph.com/browse/BLZG-4249 (SUBSTR with starting location less
    // than 1)
    ...
    if (arity() > 2 && get(2) != null) {
      ...
      label =
          label.substring(
              Math.min(label.length(), (int) (start + offset)),
              Math.min(label.length(), Math.max(0, (int) (start + length) - 1)));
