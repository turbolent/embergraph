...
import org.embergraph.rdf.internal.constraints.DateTimeUtility;
 ...
 */
public class LexiconConfiguration<V extends EmbergraphValue> implements ILexiconConfiguration<V> {
   ...
   *
   * <p>Note: {@link Integer#MAX_VALUE} disables the BLOBS index.
   *
  ...
  private final BigInteger MAX_UNSIGNED_LONG =
  ...
  public LexiconConfiguration(
    ...
    if (blobsThreshold < 0) throw new IllegalArgumentException();

    if (maxInlineTextLength < 0) throw new IllegalArgumentException();

    if (vocab == null) throw new IllegalArgumentException();
    ...
    while (itr.hasNext()) {
      ...
      for (EmbergraphURI datatype : extension.getDatatypes()) {
        ...
        if (iv2ext.containsKey(datatype.getIV())) {
          if (log.isInfoEnabled()) log.warn("multiple IExtension implementations for: " + datatype);
        }
  ...
  public V asValue(final LiteralExtensionIV<?> iv
    ...
    if (ext == null) throw new RuntimeException("Unknown extension: " + datatypeIV);
   ...
   * @return The inline {@link IV} -or- <code>null</code> if the {@link URI} can not be inlined into
   ...
   */
  @Override
  public String getInlineURILocalNameFromDelegate(
   ...
   * LexiconConfiguration}, then return its inline {@link IV}.
   *
   * @param value The {@link Literal}.
   * @return The inline {@link IV} -or- <code>null</code> if the {@link Literal} can not be inlined
  ...
  private IV<EmbergraphLiteral, ?> createInlineLiteralIV(final Literal value) {
    ...
    if (datatype != null && datatype2ext.containsKey(datatype.stringValue())) {
      ...
      if ((iv = createExtensionIV(value, datatype)) != null) return iv;
    }
     ...
     */
    if ((iv = createInlineDatatypeIV(value, datatype)) != null) return iv;
    ...
    if (inlineTextLiterals && maxInlineTextLength > 0) {
      ...
      if ((iv = createInlineUnicodeLiteral(value)) != null) return iv;
    }
   ...
   *
   * @return The {@link LiteralExtensionIV} -or- <code>null</code> if no {@link IExtension} was
   *     registered for that datatype.
   *     <p>TODO Should we explicitly disallow extensions which override the basic inlining behavior
   *     for xsd datatypes?
   */
  private AbstractInlineIV<EmbergraphLiteral, ?> createExtensionIV(
      final Literal value, final URI datatype) {

    final IExtension<? extends EmbergraphValue> xFactory = datatype2ext.get(datatype.stringValue());

    try {

      @SuppressWarnings("unchecked")
      final AbstractInlineIV<EmbergraphLiteral, ?> iv = xFactory.createIV(value);
   ...
   * @return The fully inline IV -or- <code>null</code> if the {@link Literal} could not be inlined
   *     within the configured constraints.
   */
  private AbstractInlineIV<EmbergraphLiteral, ?> createInlineUnicodeLiteral(final Literal value) {
   ...
   * @param datatype The XSD datatype {@link URI}.
   * @return The {@link IV} -or- <code>null</code> if the value could not be inlined.
  ...
  private AbstractInlineIV<EmbergraphLiteral, ?> createInlineDatatypeIV(
    ...
    if (!isInline(VTE.LITERAL, dte)) return null;
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.warn("Value does not validate against datatype: " + value);
   ...
   * LexiconConfiguration}, then return its inline {@link IV}.
   *
   * @param value The {@link BNode}.
   * @return The inline {@link IV} -or- <code>null</code> if the {@link BNode} can not be inlined
   *     into the statement indices.
   */
  private IV<EmbergraphBNode, ?> createInlineBNodeIV(final BNode value) {
     ...
     */
    if (c == 'u' && id.length() == 37 && isInline(VTE.BNODE, DTE.UUID)) {
