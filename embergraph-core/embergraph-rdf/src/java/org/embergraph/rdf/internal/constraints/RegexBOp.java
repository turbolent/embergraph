...
public class RegexBOp extends XSDBooleanIVValueExpression implements INeedsMaterialization {
  ...
  private static Map<String, Object> anns(
      final IValueExpression<? extends IV> pattern, final IValueExpression<? extends IV> flags) {
    ...
    try {
      ...
      if (pattern instanceof IConstant && (flags == null || flags instanceof IConstant)) {
        ...
        if (parg.hasValue() && (farg == null || farg.hasValue())) {
          ...
          return NV.asMap(new NV(Annotations.PATTERN, getPattern(pargVal, fargVal)));
        }
      }
  ...
  public RegexBOp(
      final IValueExpression<? extends IV> var, final IValueExpression<? extends IV> pattern) {
  ...
  public boolean accept(final IBindingSet bs) {
    ...
    return accept(var, pattern.getValue(), flags != null ? flags.getValue() : null);
  }
  ...
  private boolean accept(final Value arg, final Value parg, final Value farg) {
    ...
    if (debug) {
      ...
      log.debug(
          QueryHints.REGEX_MATCH_NON_STRING
              + ": "
              + this.getProperty(
                  QueryHints.REGEX_MATCH_NON_STRING, QueryHints.DEFAULT_REGEX_MATCH_NON_STRING));
