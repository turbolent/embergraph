 ...
 * inlining of the RDF Value.
 *
 * <p>Literals which are projected onto primitive data types with a natural order (int, float,
 * double, long, etc.) are in total order within that segment of the statement index and such
 * segments are partitioned into non-overlapping key ranges. Thus key range scans may be used for
 * where the primitive data type allows it while filtering on the value must be used where the data
 * type does not have a natural ordering. Unicode values do not have a "natural" ordering within the
 * statement indices as they are modeled by a reversible compression rather than a collation sort
 * key. Therefore, when a high level query is constrained such that a variable is known to be of a
 * given data type you can use {@link IV} aware operations. Otherwise, the {@link IV}s must be
 * materialized and operations performed on {@link EmbergraphValue}s instead.
 *
 * <h3>Binary record format</h3>
 *
 * We currently have 14 built-in (or intrinsic) data types (see {@link DTE}). Each of those types
 * has a natural order which we can encode and decode from the B+Tree key. In general, there is a
 * relatively limited set of interesting intrinsic codings, which is important since we will
 * dedicate just 4 bits for to code the natural order of the value space, which is just only 16
 * distinctions. Given that we have 14 intrinsic data types, that leaves room for just two more. One
 * of those bits provides for Unicode data (see {@link DTE#XSDString} without a collation order).
 * The other bit provides extensibility in the framework itself as described below (see {@link
 * DTE#Extension}).
 *
 * <p>The header byte contains various bit flags which are laid out as follows:
 *
 * <pre>
 * [valueType]    : 2 bits
 * [inline]       : 1 bit
 ...
 * <dl>
 *   <dt>valueType
 *   <dd>These 2 bits distinguish between URIs, Literals, Blank Nodes, and statement identifiers
 *       (SIDs). These bits are up front and therefore partition the key space first by the RDF
 *       Value type. See {@link VTE} which governs these bits.
 *   <dt>inline
 *   <dd>This bit indicates whether the value is inline or represented by a term identifier in the
 *       key. This bit is set based on how a given triple store or quad store instance is
 *       configured. However, because the bit is present in the flags, we know how to decode the key
 *       without reference to this configuration metadata.
 *   <dt>extension
 *   <dd>This bit is ignored (and should be zero) unless the RDF Value is a Literal with a data type
 *       URI which is being inlined. For data type literals, this bit is set if the actual data type
 *       is not one of those which we handle intrinsically but is one of those which has been
 *       registered (by the application) as an "extended" data type projected onto one of the
 *       intrinsic data types. Thus, this bit partitions the key space into the intrinsic data types
 *       and the extended data types.<br>
 *       When <code>true</code>, this bit signals that information about the actual RDF Value data
 *       type will follow (see below). When <code>false</code>, the datatype URI is directly
 *       recoverable (for a data type Literal) from the <code>dataTypeCode</code>.
 *   <dt>dataTypeCode
 *   <dd>These 4 bits indicate the intrinsic data type for the inline value and are ignored (and
 *       should be zero) unless a data type Literal is being inlined. These bits partition the key
 *       space. However, since <code>extension</code> bit comes first this will not interleave
 *       inline values for intrinsic and extended data types having the same <code>dataTypeCode
 *       </code>. <br>
 *       Note: The <code>dataTypeCode</code> <code>0xf</code> ({@link DTE#Extension)} is reserved
 *       for extending the set of intrinsic data types. When the code is <code>0xf</code> the next
 *       byte must be considered as well to determine the actual intrinsic data type code.
 * </dl>
 *
 * <pre>
 ...
 * </pre>
 *
 * If the DTE value was <code>DTE.Extension</code> was, then the next byte(s) encode the
 * DTEExtension (extended intrinsic datatype aka primitive datatype).
 *
 * <pre>
 * ---------- byte boundary ----------
 * </pre>
 *
 * If <code>extension</code> was true, then the next byte(s) encode information about the source
 * data type URI (its {@link IV}) and the key space will be partitioned based on the extended data
 * type URI.
 *
 * <pre>
 * ---------- byte boundary ----------
 * </pre>
 *
 * The unsigned byte[] representation of the value in the value space for one of the intrinsic
 * types. The length of this byte[] may be directly determined from the [dataTypeCode] for most data
 * types. However, for xsd:integer and xsd:decimal, the length is part of the representation.
 *
 * <pre>
 ...
 * <dl>
 *   <dt>projections
 *   <dd>A projection takes an application specific data type and maps it onto one of the intrinsic
 *       data types (int, float, double, etc). Projections provide an extensible mechanism which
 *       allows an application to benefit from inline representation of RDF Values and allows the
 *       query optimizer to chose key-range scans for application defined data types if they can be
 *       projected onto intrinsic data types. For example, if you define an application specific
 *       data type <code>foo:milliseconds</code> representing milliseconds since the epoch, then the
 *       value space of that data type can be projected onto an <code>xsd:long</code>.
 *   <dt>enumerations
 *   <dd>An enumeration is an application specific data type having a specific set of values. Those
 *       values are then projected onto an intrinsic data type such as <code>byte</code> (256
 *       distinctions) or <code>short</code> (64k distinctions). Enumerations make it possible to
 *       inline application specific data types while benefiting from XSD validation of those RDF
 *       Values. When an enumeration is registered, the order in which the members of the
 *       enumeration are given may optionally specify the natural order of that enumeration. The
 *       natural order is imposed by projecting the first member of the enumeration one ZERO, the
 *       second member onto ONE, etc. An enumeration with a natural order will be sorted based on
 *       that defined order and query optimizations may perform key-range scans informed by that
 *       natural order.<br>
 *       Enumerations may be used in cases where you might otherwise use short character codes. For
 *       example, an enumeration could be defined for the two character abbreviations for the 50 US
 *       States. That enumeration could be mapped onto a single byte.
 *   <dt>custom indices
 *   <dd>The best example here is spatial data, which requires literals which represent points,
 *       rectangles, circles, arcs, clouds, etc to be inserted into special spatial indices. Queries
 *       must be aware of spatial data and must be rewritten to run against the appropriate spatial
 *       indices.<br>
 *       Another use case would be carrying specialized indices for bioinformatics or genomics data.
 * </dl>
 *
 * Note: Both projected and enumerated extensible data types MAY map many RDF Values onto the same
 * internal value but each internal value MUST map onto a single RDF Value (materialization must be
 * deterministic). This can be seen as normalization imposed by the database.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @param <V> The generic type for the RDF {@link Value} implementation.
 * @param <T> The generic type for the inline value.
 */
public abstract class AbstractIV<V extends EmbergraphValue, T> implements IV<V, T> {
   ...
   * has 4 distinctions and is thus TWO (2) bits wide. The <i>dte</i> allows for up to 16
   * distinctions and is SIX (6) bits wide. The bits allocated to these sets of distinctions are
   * combined within a single byte as follows:
   *
   * <pre>
   * [vviedddd]
   * </pre>
   *
   * where <code>v</code> is a {@link VTE} bit, <code>i</code> is the <i>inline</i> bit, <code>e
   * </code> is the extension bit, and <code>d</code> is a {@link DTE} bit.
   *
   * @param vte The RDF Value type (URI, BNode, Literal, or Statement).
   * @param inline <code>true</code> iff the RDF value will be represented inline in the key. When
   *     <code>false</code>, the term identifier of the RDF Value will be represented inline instead
   *     of its actual value.
   * @param extension When <code>true</code>, the actual RDF data type URI differs from the
   *     intrinsic data type (the {@link DTE}) but has been projected onto the natural order of the
   *     intrinsic data type.
   * @param dte The internal datatype for the RDF value (termId, xsd:int, xsd:long, xsd:float,
   *     xsd:double, etc).
   * @see VTE
   * @see DTE
   */
  protected AbstractIV(
      final VTE vte, final boolean inline, final boolean extension, final DTE dte) {
   ...
   * internal value}.
   *
   * @param vte The value type
   * @param inline <code>true</code> iff the value is inline (no index access is required to
   *     materialize the value).
   * @param extension <code>true</code> iff the value is an extension data type (a user specific
   *     data type defined whose underlying data type is specified by the {@link DTE}).
   * @param dte The basic data type.
   ...
   */
  public static boolean isInline(final byte flags) {
   ...
   * @param flags The flags byte.
   */
  public static boolean isExtension(final byte flags) {
   ...
   * byte. However, the VTE bits are actually stored in the TWO (2) MSB bits of the <i>flags</i>
   * byte. This method is responsible for shifting the VTE bits down before invoking {@link
   * VTE#valueOf(byte)}.
   *
   ...
   * @return The {@link DTE}
   */
  public static DTE getDTE(final byte flags) {
   ...
   * many derived classes which have compile time knowledge of whether the RDF value is inline or
   * not.
   */
  @Override
   ...
   * is only used for specific kinds of {@link IV}s, this method can be overridden in many derived
   * classes which have compile time knowledge of whether the value is an RDF {@link Literal} or
   * not.
   */
  @Override
   ...
   * with {@link EmbergraphValue#hashCode()}. Therefore you MUST NOT mix {@link IV}s and {@link
   * EmbergraphValue}s in the keys of a map or the values of a set.
   */
  @Override
   ...
   *
   * <p>Note: The {@link IV} implementations typically DO NOT compare equals() with {@link
   * EmbergraphValue}s. Therefore you MUST NOT mix {@link IV}s and {@link EmbergraphValue}s in the
   * keys of a map or the values of a set.
   */
  @Override
  ...
  public final int compareTo(final IV o) {
  ...
  public IKeyBuilder encode(final IKeyBuilder keyBuilder) {
    ...
    if (!isInline()) {
      ...
      if (isExtension()) {
         ...
         */
        final AbstractNonInlineExtensionIVWithDelegateIV<?, ?> t =
            (AbstractNonInlineExtensionIVWithDelegateIV<?, ?>) this;
   ...
   */
  private transient volatile V cache = null;
  ...
  public final V getValue() {
    ...
    if (v == null) throw new NotMaterializedException(toString());
  ...
  /** Return the cached {@link EmbergraphValue} or -<code>null</code> if it is not cached. */
