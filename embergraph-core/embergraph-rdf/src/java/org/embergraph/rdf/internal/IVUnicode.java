 ...
 * bytes in the {@link IV} representation, this means that we wind up with a length prefix followed
 * by some representation of the character data. This can not be consistent with the code point
 * ordering imposed by {@link String#compareTo(String)}. Therefore, the {@link IVUnicodeComparator}
 * is used to make the ordering over the {@link String} data consistent with the encoded
 * representation of that data.
 *
 * <p>Note: This is not the only way to solve the problem. We could also have generated the encoded
 * representation from any {@link IV} having inline Unicode data each time we need to compare two
 * {@link IV}s, but that could turn into a lot of overhead.
 *
 * <p>Note: This does not attempt to make the Unicode representation "tight" and is not intended to
 * handle very large Unicode strings. Large Unicode data in the statement indices causes them to
 * bloat and has a negative impact on the overall system performance. The use case for inline
 * Unicode data is when the data are small enough that they are worth inserting into the statement
 * indices rather than indirecting through the TERM2ID/ID2TERM indices. Large RDF Values should
 * always be inserted into the BLOBS index which is designed for that purpose.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 *     <p>TODO This is directly persisting char[] data. Is that portable?
 */
public class IVUnicode {
   ...
   */
  public static byte[] encode1(final String s) {
    //        return un.encode1(s);
    //        /*
    ...
    if (slen > Short.MAX_VALUE) throw new UnsupportedOperationException();
   ...
   * @throws IOException
   */
  static int decode(final InputStream in, final StringBuilder out) throws IOException {
    /*
     * Dead simple decode.
     *
