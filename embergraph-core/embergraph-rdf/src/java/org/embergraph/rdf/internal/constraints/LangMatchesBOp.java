...
public class LangMatchesBOp extends XSDBooleanIVValueExpression implements INeedsMaterialization {
  ...
  public LangMatchesBOp(
      final IValueExpression<? extends IV> tag, final IValueExpression<? extends IV> range) {
  ...
  protected boolean accept(final IBindingSet bs) {
    ...
    // not yet bound
    if (tag == null) throw new SparqlTypeErrorException();

    final IV<?, ?> range = get(1).get(bs);

    // not yet bound
    if (range == null) throw new SparqlTypeErrorException();
    ...
    if (tagVal == null) throw new NotMaterializedException();

    if (!QueryEvaluationUtil.isSimpleLiteral(tagVal)) throw new SparqlTypeErrorException();

    final EmbergraphValue rangeVal = range.getValue();

    // not yet materialized
    if (rangeVal == null) throw new NotMaterializedException();

    if (!QueryEvaluationUtil.isSimpleLiteral(rangeVal)) throw new SparqlTypeErrorException();
