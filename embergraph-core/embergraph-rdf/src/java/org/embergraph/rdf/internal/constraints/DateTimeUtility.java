...
public class DateTimeUtility implements IMathOpHandler {
  ...
  public boolean canInvokeMathOp(final Literal... args) {
    for (Literal lit : args) {
      ...
      if (!(XMLDatatypeUtil.isCalendarDatatype(dt) || dt.equals(XMLSchema.DURATION))) return false;
    }
    return true;
  }
  ...
  public IV doMathOp(
    ...
    if (!canInvokeMathOp(l1, l2)) throw new SparqlTypeErrorException();
    ...
    Duration d1 =
        dt1.equals(XMLSchema.DURATION) ? datatypeFactory.newDuration(l1.getLabel()) : null;
    Duration d2 =
        dt2.equals(XMLSchema.DURATION) ? datatypeFactory.newDuration(l2.getLabel()) : null;
    if (op == MathOp.PLUS) {
      if (c1 != null && c2 != null) {
      ...
      if (c1 != null && c2 != null) {
        long milliseconds =
            c1.toGregorianCalendar().getTimeInMillis() - c2.toGregorianCalendar().getTimeInMillis();
