...
public class CompareBOp extends XSDBooleanIVValueExpression implements INeedsMaterialization {
  ...
  public static boolean compare(
      final IV<EmbergraphValue, ?> left, final IV<EmbergraphValue, ?> right, final CompareOp op) {
  ...
  private static boolean compareLiterals(
    ...
    if (left instanceof LiteralExtensionIV && right instanceof LiteralExtensionIV) {
      ...
      if (leftDatatype.equals(rightDatatype)) {
        ...
        final boolean accept = _accept(left.compareTo(right), op);
    ...
    try {
      ...
      // use the Sesame implementation directly
      final boolean accept = QueryEvaluationUtil.compareLiterals(l1, l2, op);
  ...
  private static boolean valuesEqual(final IV<?, ?> left, final IV<?, ?> right) {
  ...
  public boolean accept(final IBindingSet s) {
    ...
    // not yet bound
    if (left == null || right == null) throw new SparqlTypeErrorException();
  ...
  protected static boolean _accept(final int compare, final CompareOp op) {
