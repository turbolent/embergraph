 ...
 *     <p>TODO add a variable length nibble encoding of a byte[]?
 */
public enum DTEExtension {
   ...
   * technique we're using is order preserving only for the valid range.
   */
   ...
   * indicates the actual datatype URI. In contrast, {@link #RESERVED} is a placeholder that could
   * give us another byte to handle additional "intrinsic" types.
   *
   * @see BLZG-1507 (Implement support for DTE extension types for URIs)
   *     <p>TODO If we wind up extending things again, it would perhaps be nicer to have an
   *     extension mechanism for non-intrinsic data types (ones that can be registered without
   *     having to explicitly create them in the code).
   */
  ...
  public static final DTEExtension valueOf(final URI datatype) {
    ...
    if (datatype.equals(IPV4.datatypeURI)) return IPV4;
    if (datatype.equals(PACKED_LONG.datatypeURI)) return PACKED_LONG;
