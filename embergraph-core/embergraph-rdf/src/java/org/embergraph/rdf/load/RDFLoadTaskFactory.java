...
public class RDFLoadTaskFactory<S extends Statement, T extends Runnable>
   ...
   * @param fallback An attempt will be made to determine the interchange syntax using {@link
   *     RDFFormat}. If no determination can be made then the loader will presume that the files are
   *     in the format specified by this parameter (if any). Files whose format can not be
   *     determined will be logged as errors.
   * @todo drop the writeBuffer arg.
   */
  public RDFLoadTaskFactory(
   ...
   *     in the format specified by this parameter (if any). Files whose format can not be
   *     determined will be logged as errors.
   * @param factory Used to buffer and load statements.
   */
  public RDFLoadTaskFactory(
      final AbstractTripleStore db,
      final RDFParserOptions parserOptions,
  ...
  public CounterSet getCounters() {
    ...
    if (bufferFactory instanceof IAsynchronousWriteStatementBufferFactory) {
      ...
      counterSet.attach(((IAsynchronousWriteStatementBufferFactory) bufferFactory).getCounters());
    }
   ...
   * written by this client. Therefore if there are concurrent writers then the apparent TPS here
   * will be higher than was reported by the counters for just this client -- all writers on the
