...
public class AbstractRDFTaskFactory<S extends Statement, T extends Runnable>
   ...
   * by {@link #elapsed()}.
   *
   * @todo Once we are done loading data the client should be told to flush its counters to the load
   *     balancer so that we have the final state snapshot once it is ready.
   */
   ...
   * @return The {@link RDFFormat} -or- <code>null</code> iff {@link #fallback} is <code>null</code>
   *     and the no format was recognized for the <i>filename</i>
   */
  ...
  protected AbstractRDFTaskFactory(
  ...
  public T newTask(final String resource) throws Exception {
    ...
    if (rdfFormat == null) {
      ...
      throw new RuntimeException(
          "Could not determine interchange syntax - skipping : file=" + resource);
    }
    ...
    return (T)
        new SingleResourceReaderTask(
            resource, baseURL, rdfFormat, parserOptions, deleteAfter, bufferFactory, toldTriples);
  }
}
