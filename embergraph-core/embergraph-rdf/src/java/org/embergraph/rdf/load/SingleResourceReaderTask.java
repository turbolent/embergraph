...
public class SingleResourceReaderTask implements Runnable {
  ...
  /*
   * Note: Updates to <i>toldTriples</i> MUST NOT occur unless the task succeeds, otherwise tasks
   * which error and then retry will cause double-counting.
   *
   * @param resource
   * @param baseURL
   * @param rdfFormat
   ...
   * @param toldTriples
   */
  public SingleResourceReaderTask(
      String resource,
      String baseURL,
      RDFFormat rdfFormat,
      final RDFParserOptions parserOptions,
    ...
    if (bufferFactory == null) throw new IllegalArgumentException();

    if (toldTriples == null) throw new IllegalArgumentException();
  ...
  /*
   * Reads an RDF resource and either loads it into the database or verifies that the triples in the
   * resource are found in the database.
   */
