 ...
 * parallelism across index partitions for a read-historical iterator since the data service
 * locators are immutable for historical reads.
 *
 * <p>Rule-level parallelism (for fix point closure of a rule set) and join subquery-level
 * parallelism could be distributed to available workers in a cluster. In a similar way, high-level
 * queries could be distributed to workers in a cluster to evaluation. Such distribution would
 * increase the practical parallelism beyond what a single machine could support as long as the
 * total parallelism does not overload the cluster.
 *
 * <p>There is a pragmatic limit on the #of concurrent threads for a single host. When those threads
 * target a blocking queue, then thread contention becomes very high and throughput drops
 * dramatically. We can reduce this problem by allocating a distinct {@link
 * UnsynchronizedArrayBuffer} to each task. The task collects a 'chunk' in the {@link
 * UnsynchronizedArrayBuffer}. When full, the buffer propagates onto a thread-safe buffer of chunks
 * which flushes either on an {@link IMutableRelation} (mutation) or feeding an {@link
 * IAsynchronousIterator} (high-level query). It is chunks themselves that accumulate in this
 * thread-safe buffer, so each add() on that buffer may cause the thread to yield, but the return
 * for yielding is an entire chunk in the buffer, not just a single element.
 *
 * <p>There is one high-level buffer factory corresponding to each of the kinds of {@link
 * ActionEnum}: {@link #newQueryBuffer()}; {@link #newInsertBuffer(IMutableRelation)}; and {@link
 * #newDeleteBuffer(IMutableRelation)}. In addition there is one for {@link
 * UnsynchronizedArrayBuffer}s -- this is a buffer that is NOT thread-safe and that is designed to
 * store a single chunk of elements, e.g., in an array E[N]).
 *
...
public class RDFJoinNexus extends AbstractJoinNexus implements IJoinNexus {
  ...
  /** when <code>true</code> the backchainer will be enabled for access path reads. */
  private final boolean backchain;

  private final IRuleStatisticsFactory ruleStatisticsFactory =
      new IRuleStatisticsFactory() {
...
//         * Factory will resolve term identifiers in {@link IPredicate}s in the
  ...
  private static class RDFRuleStats extends RuleStats {
     ...
     */
    public RDFRuleStats(
        final IIndexManager indexManager, final long timestamp, final IRuleState ruleState) {
    ...
    protected String toString(final IPredicate pred) {
      ...
      final SPORelation spoRelation =
          (SPORelation)
              indexManager.getResourceLocator().locate(pred.getRelationName(0), timestamp);
      ...
      try {

        {
          ...
          if (t.isVar()) s = t.getName();
          else s = db.toString(t.get());
        }

        {
          final IVariableOrConstant<IV> t = pred.get(1);

          if (t.isVar()) p = t.getName();
          else p = db.toString(t.get());
        }

        {
          final IVariableOrConstant<IV> t = pred.get(2);

          if (t.isVar()) o = t.getName();
          else o = db.toString(t.get());
        }
      } catch (Throwable t) {
   ...
   */
  public RDFJoinNexus(
      final RDFJoinNexusFactory joinNexusFactory, final IIndexManager indexManager) {
   ...
   * defined (not <code>-1</code>) then the returned access path will be for the specified shard
   * using the data service local index manager ( {@link #indexManager} MUST be the data service
   * local index manager for this case) and expanders WILL NOT be applied (they require a view of
   * the total relation, not just a shard).
   *
   * @see InferenceEngine
   * @see BackchainAccessPath
   * @todo consider encapsulating the {@link IRangeCountFactory} in the returned access path for
   *     non-exact range count requests. this will make it slightly harder to write the unit tests
   *     for the {@link IEvaluationPlanFactory}
   */
  @Override
  @SuppressWarnings("unchecked")
  public IAccessPath getTailAccessPath(final IRelation relation, final IPredicate predicate) {
    ...
    IAccessPath accessPath =
        relation.getAccessPath(indexManager /* localIndexManager */, keyOrder, predicate);
    ...
    if (backchain && relation instanceof SPORelation) {
      ...
      if (expander == null || expander.backchain()) {
        ...
        accessPath =
            new BackchainAccessPath(
                spoRelation.getContainer(),
                accessPath,
                joinNexusFactory.isOwlSameAsUsed ? Boolean.TRUE : Boolean.FALSE);
      }
    }
   ...
   *
   * <p>FIXME unit tests for SORT with and without DISTINCT and with the various combinations used
   * in the unit tests for DISTINCT. Note that SORT, unlike DISTINCT, requires that all solutions
   * are materialized before any solutions can be returned to the caller. A lot of optimization can
   * be done for SORT implementations, including merge sort of large blocks (ala map/reduce), using
   * compressed sort keys or word sort keys with 2nd stage disambiguation, etc.
   *
   * <p>FIXME Add property for sort {ascending,descending,none} to {@link IRule}. The sort order can
   * also be specified in terms of a sequence of variables. The choice of the variable order should
   * be applied here.
   *
   * <p>FIXME The properties that govern the Unicode collator for the generated sort keys should be
   * configured by the {@link RDFJoinNexusFactory}. In particular, Unicode should be handled however
   * it is handled for the {@link LexiconRelation}.
   */
  ...
  public static class InsertSPOAndJustificationBuffer<E> extends AbstractSolutionBuffer<E> {
     ...
     * @param relation
     */
    public InsertSPOAndJustificationBuffer(final int capacity, final IMutableRelation<E> relation) {
    ...
    private long writeChunk(final ISolution<E>[] chunk) {
      ...
      if (log.isDebugEnabled()) log.debug("chunkSize=" + n);
      ...
      if (log.isInfoEnabled())
        log.info("Wrote " + mutationCount + " statements and justifications in " + elapsed + "ms");
  ...
  public IBuffer<ISolution[]> newInsertBuffer(final IMutableRelation relation) {
    ...
    if (getAction() != ActionEnum.Insert) throw new IllegalStateException();
    ...
    if (justify) {
      ...
      return new InsertSPOAndJustificationBuffer(chunkOfChunksCapacity, relation);
    }
    ...
    return new AbstractSolutionBuffer.InsertSolutionBuffer(chunkOfChunksCapacity, relation);
  }
}
