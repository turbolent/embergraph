 ...
 * @version $Id$
 * @todo Improve write efficiency for the proofs - they are slowing things way down. Perhaps turn
 *     off the range count metadata inside of the B+Tree for that index? Note that using magic sets
 *     or a backward chainer will let us avoid writing proofs altogether since we can prove whether
 *     or not a statement is still entailed without recourse to reading proofs chains.
 * @todo explore an option for "owl:sameAs" semantics using destructive merging (the terms are
 *     assigned the same term identifier, one of them is treated as a canonical, and there is no way
 *     to retract the sameAs assertion). If you take this approach then you must also re-write all
 *     existing assertions using the term whose term identifier is changed to be that of another
 *     term.
 */
public class InferenceEngine {
  ...
  public interface Options {
     ...
     * neither computed NOR stored and a backward chainer or magic sets technique must be used to
     * generate the entailments at query time.
     *
     * <p>Note: Eagerly materializing those entailments takes a lot of time and space but it reduces
     * time during query IF you are asking for these entailments (many realistic queries do not).
     * Therefore it is generally a win to turn this option off.
     *
     * @see BackchainAccessPath
     * @see BackchainTypeResourceIterator
     */
    String FORWARD_CHAIN_RDF_TYPE_RDFS_RESOURCE =
        InferenceEngine.class.getName() + ".forwardChainRdfTypeRdfsResource";
     ...
     * org.embergraph.rdf.store.AbstractTripleStore.Options#AXIOMS_CLASS} is used to completely
     * disable those entailments, e.g., by specifying either {@link NoAxioms} or {@link RdfsAxioms}.
     * When <code>false</code> those entailments are not computed and <code>owl:sameAs</code>
     * processing is disabled.
     */
    String FORWARD_CHAIN_OWL_SAMEAS_CLOSURE =
        InferenceEngine.class.getName() + ".forwardChainOwlSameAsClosure";
     ...
     * false</code>, rules that produce those entailments are turned off such that they are neither
     * computed NOR stored and the entailments may be accessed at query time using the {@link
     * BackchainAccessPath}.
     *
     * <p>Note: The default is <code>false</code> since those entailments can take up a LOT of space
     * in the store and are expensive to compute during data load. It is a lot easier to compute
     * them dynamically when presented with a specific triple pattern. While more computation is
     * performed if a fill triple scan is frequently requested, that is an unusual case and
     * significantly less data will be stored regardless.
     *
     * <p>Note: I've changed the default to true until we fix the query time computation, which is
     * currently broken. --MP
     *
     * @see BackchainAccessPath
     */
    String FORWARD_CHAIN_OWL_SAMEAS_PROPERTIES =
        InferenceEngine.class.getName() + ".forwardChainOwlSameAsProperties";
     ...
     * turned off such that they are neither computed NOR stored and a backward chainer or magic
     * sets technique must be used to generate the entailments at query time.
     *
     * @todo implement backward chaining for owl:equivalentProperty and compare performance?
     */
    String FORWARD_CHAIN_OWL_EQUIVALENT_PROPERTY =
        InferenceEngine.class.getName() + ".forwardChainOwlEquivalentProperty";
     ...
     * such that they are neither computed NOR stored and a backward chainer or magic sets technique
     * must be used to generate the entailments at query time.
     *
     * @todo implement backward chaining for owl:equivalentClass and compare performance?
     */
    String FORWARD_CHAIN_OWL_EQUIVALENT_CLASS =
        InferenceEngine.class.getName() + ".forwardChainOwlEquivalentClass";
     ...
     * generate the entailments at query time.
     */
    String FORWARD_CHAIN_OWL_INVERSE_OF =
        InferenceEngine.class.getName() + ".forwardChainOwlInverseOf";
     ...
     * sets technique must be used to generate the entailments at query time.
     */
    String FORWARD_CHAIN_OWL_TRANSITIVE_PROPERTY =
        InferenceEngine.class.getName() + ".forwardChainOwlTransitiveProperty";
     ...
     * that they are neither computed NOR stored and a backward chainer or magic sets technique must
     * be used to generate the entailments at query time.
     */
    String FORWARD_CHAIN_OWL_HAS_VALUE =
        InferenceEngine.class.getName() + ".forwardChainOwlHasValue";
     ...
     * sets technique must be used to generate the entailments at query time.
     */
    String FORWARD_CHAIN_OWL_SYMMETRIC_PROPERTY =
        InferenceEngine.class.getName() + ".forwardChainOwlSymmetricProperty";
     ...
     * closure will fail if violations are found in the data.
     */
    String ENABLE_OWL_FUNCTIONAL_AND_INVERSE_FUNCTIONAL_PROPERTY =
        InferenceEngine.class.getName() + ".enableOwlFunctionalAndInverseFunctionalProperty";
  ...
  public InferenceEngine(AbstractTripleStore database) {
    ...
    this.forwardChainRdfTypeRdfsResource =
        Boolean.parseBoolean(
            properties.getProperty(
                Options.FORWARD_CHAIN_RDF_TYPE_RDFS_RESOURCE,
                Options.DEFAULT_FORWARD_RDF_TYPE_RDFS_RESOURCE));

    if (INFO)
      log.info(
          Options.FORWARD_CHAIN_RDF_TYPE_RDFS_RESOURCE + "=" + forwardChainRdfTypeRdfsResource);
    ...
    if (rdfsOnly) {
      ...
      this.forwardChainOwlSameAsClosure =
          Boolean.parseBoolean(
              properties.getProperty(
                  Options.FORWARD_CHAIN_OWL_SAMEAS_CLOSURE,
                  Options.DEFAULT_FORWARD_CHAIN_OWL_SAMEAS_CLOSURE));

      if (INFO)
        log.info(Options.FORWARD_CHAIN_OWL_SAMEAS_CLOSURE + "=" + forwardChainOwlSameAsClosure);

      if (forwardChainOwlSameAsClosure) {

        this.forwardChainOwlSameAsProperties =
            Boolean.parseBoolean(
                properties.getProperty(
                    Options.FORWARD_CHAIN_OWL_SAMEAS_PROPERTIES,
      ...
      this.forwardChainOwlEquivalentProperty =
          Boolean.parseBoolean(
              properties.getProperty(
      ...
      this.forwardChainOwlEquivalentClass =
          Boolean.parseBoolean(
              properties.getProperty(
      ...
      if (INFO)
        log.info(Options.FORWARD_CHAIN_OWL_EQUIVALENT_CLASS + "=" + forwardChainOwlEquivalentClass);

      this.forwardChainOwlInverseOf =
          Boolean.parseBoolean(
              properties.getProperty(
                  Options.FORWARD_CHAIN_OWL_INVERSE_OF,
                  Options.DEFAULT_FORWARD_CHAIN_OWL_INVERSE_OF));

      if (INFO) log.info(Options.FORWARD_CHAIN_OWL_INVERSE_OF + "=" + forwardChainOwlInverseOf);

      this.forwardChainOwlTransitiveProperty =
          Boolean.parseBoolean(
              properties.getProperty(
      ...
      this.forwardChainOwlHasValue =
          Boolean.parseBoolean(
              properties.getProperty(
                  Options.FORWARD_CHAIN_OWL_HAS_VALUE,
                  Options.DEFAULT_FORWARD_CHAIN_OWL_HAS_VALUE));

      if (INFO) log.info(Options.FORWARD_CHAIN_OWL_HAS_VALUE + "=" + forwardChainOwlHasValue);

      this.forwardChainOwlSymmetricProperty =
          Boolean.parseBoolean(
              properties.getProperty(
      ...
      if (INFO)
        log.info(
            Options.FORWARD_CHAIN_OWL_SYMMETRIC_PROPERTY + "=" + forwardChainOwlSymmetricProperty);

      this.enableOwlFunctionalAndInverseFunctionalProperty =
          Boolean.parseBoolean(
              properties.getProperty(
   ...
   * database.
   *
   * <p>Note: This is lazily instantiated in order to avoid a cyclic initialization dependency
   * between the {@link InferenceEngine} and the {@link BaseClosure} constructors.
   */
   ...
   * query time by {@link #backchainIterator(long, long, long)}.
   */
   ...
   *
   * <p>Note: When <code>false</code>, NO owl:sameAs processing will be performed since there is no
   * privision for backward chaining the owl:sameAs closure.
   */
   ...
   * will compute those entailments at query time in {@link #backchainIterator(long, long, long)}.
   */
   ...
   *
   * <p>Note: before calling this method with a non-<code>null</code> <i>focusStore</i>, the caller
   * SHOULD examine the statements in the <i>focusStore</i> and then database. For each statement in
   * the focusStore, if this statement exists explicitly in the database then remove it from the
   * focusStore. If this statement exists implicitly in the database. Regardless of whether the
   * statement was explicit or inferred in the database, remove it from the focusStore. This step
   * prevents the needless (and expensive) reapplication of the rules to data already known to the
   * database!
   *
   * <p>Note: If the <i>focusStore</i> is given, then the entailments will be asserted against the
   * focusStore. Either this method or the caller MUST copy the <i>focusStore</i> onto the database
   * using {@link AbstractTripleStore#copyStatements(AbstractTripleStore, IElementFilter, boolean)}.
   * If you are loading data from some kind of resource, then see {@link DataLoader} which already
   * knows how to do this.
   *
   * <p>See {@link TruthMaintenance#assertAll(TempTripleStore)}, which first handles statements
   * already in the database, then calls this method, and finally copies the remaining explicit
   * statements in the focusStore and the entailments into the database.
   *
   * @param focusStore The data set that will be closed against the database (optional). When <code>
   *     null</code> the store will be closed against itself.
   ...
   * other purpose or you may risk failing to generate justifications.
   *
   * <p>Note: While this is synchronized, there is a stronger constraint on truth maintenance --
   * only one process can safely update the closure of the database at a time. Concurrent closure
   * updates will result in an incoherent knowledge base (the closure will not be at fixed point but
   * the underlying indices will be coherent). Closure operations MUST be serialized to avoid
   * inconsistency in the knowledge base.
   *
   * @param focusStore The data set that will be closed against the database.
   * @param justify {@link Justification}s will be generated iff this flag is <code>true</code>.
   * @return Statistics about the operation.
  ...
  public synchronized ClosureStats computeClosure(AbstractTripleStore focusStore, boolean justify) {
    ...
    final MappedProgram program =
        baseClosure.getProgram(
            database.getSPORelation().getNamespace(),
            (focusStore == null ? null : focusStore.getSPORelation().getNamespace()));

    if (INFO) {

      log.info("\n\nforwardClosure=" + baseClosure.getClass().getName() + ", program=" + program);
    }
    ...
    try {
      ...
      final int solutionFlags =
          IJoinNexus.ELEMENT | (justify ? IJoinNexus.RULE | IJoinNexus.BINDINGS : 0)
      ...
      final IJoinNexus joinNexus = joinNexusFactory.newInstance(database.getIndexManager());
