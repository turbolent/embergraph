 ...
 * @version $Id$
 * @todo for a general purpose solution, this could use extSer for a compact serialization and that
 *     will work well with joins binding variables with many different data types. the set of
 *     serializers actually used would have to be sent as part of the serialized state as any
 *     assigned classIds.
 * @todo unit tests and performance comparisons of serialization options.
 * @todo factor out the variable declarations. binding sets coming from joins tend to have natural
 *     order based on the sequence in which the variables are becoming bound. Leverage that order to
 *     produce a much more compact serialization since duplicate bindings can be elided.
 * @todo given all the options for {@link IBindingSet} (and {@link ISolution}) serialization, these
 *     classes really need to version their own serialization formats.
 * @todo for RDF, this can of course be optimized quite a bit since we know that all the values are
 *     long integers.
 * @todo do a version for {@link ISolution}s as well that accepts the same constraints (variable
 *     bindings are long integers), but also factor out the rule (which is optional) and the
 *     materialized element (which will be an {@link SPO} and which is also optional). Note that all
 *     {@link ISolution}s that are (de-)serialized together will share the same flags for whether
 *     the rule, bindingset, or element are present. The serialization of the binding sets
 *     themselves can be done with the {@link SPOBindingSetSerializer}.
 */
public class SPOBindingSetSerializer implements IStreamSerializer<IBindingSet[]> {
  ...
  public void serialize(final ObjectOutput out, final IBindingSet[] obj) {
    ...
    //         * compression.
    //         */
    //        final LinkedHashMap<IVariable, Integer> vars = new LinkedHashMap<IVariable,
    // Integer>();
    //        int nvars = 0;
    //        {
    //
