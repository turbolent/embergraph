...
public class FastClosure extends BaseClosure {
  ...
  public MappedProgram getProgram(String db, String focusStore) {
    ...
    final MappedProgram program =
        new MappedProgram(
            "fastForwardClosure", focusStore, false /* parallel */, false /* closure */);
...

    {
      ...
      final MappedProgram subProgram =
          new MappedProgram(
              "fastClosure{5 6}", focusStore, true /* parallel */, false /* closure */);
    ...
    if (forwardChainRdfTypeRdfsResource) {
      ...
      // 14-15. RuleRdf04
      MappedProgram subProgram =
          new MappedProgram("rdfs04", focusStore, true /* parallel */, false /* closure */);
    ...
    if (!rdfsOnly) {
      ...
      if (forwardChainOwlSameAsClosure) {
        ...
        if (forwardChainOwlSameAsProperties) {
          ...
          // constraints.
          final IBindingSet noBindings =
              EmptyBindingSet.INSTANCE; // new ArrayBindingSet(0/* capacity */);
          {
            final Rule tmp = new RuleOwlSameAs2(db, vocab);

            // the variable in the predicate position of the head of
            // the rule.
            final IVariable<IV> p = (IVariable<IV>) tmp.getHead().get(1 /* p */);

            program.addStep(
                tmp.specialize(
                    noBindings, new IConstraint[] {Constraint.wrap(new NEConstant(p, owlSameAs))}));
          }
