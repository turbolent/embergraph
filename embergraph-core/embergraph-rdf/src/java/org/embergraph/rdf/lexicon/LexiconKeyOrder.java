...
public class LexiconKeyOrder extends AbstractKeyOrder<EmbergraphValue> {
   ...
   * records.
   *
   * <p>The index whose keys are formed from the hash code of the RDF {@link Value} plus a counter
   * (to break ties on the hash code). The {@link IV} for an entry in the TERMS index is formed by
   * wrapping this key. The values are the RDF {@link Value}s and are often represented by raw
   * records on the backing {@link Journal}. This index is only used for "large" {@link Value}s.
   * Most RDF {@link Value}s wind up inlined into the statement indices.
   */
  ...
  /** The positional index corresponding to the {@link IV} in a {@link LexPredicate}. */
   ...
   */
  public static LexiconKeyOrder valueOf(final int index) {
   ...
   * the {@link VTE}, the hashCode (of the {@link EmbergraphValue}), and a collision counter. The
   * collision counter is not known unless you actually scan the collision bucket in the TERMS
   * index. So there is no way to provide a comparator for the TERMS index unless all of the {@link
   * EmbergraphValue}s have been resolved to their {@link BlobIV}s.
   *
   * @throws UnsupportedOperationException
   */
  public final Comparator<EmbergraphValue> getComparator() {
  ...
  protected void appendKeyComponent(
      final IKeyBuilder keyBuilder, final int i, final Object keyComponent) {
