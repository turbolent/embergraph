...
public interface ISubjectCentricTextIndexer<A extends IHit> extends ITextIndexer<A> {
   ...
   * are tokenized using the default {@link Locale}. In the subject-centric text index, these
   * tokenized literals are rolled up by subject rather than using the IV for the literals as the
   * docId in the text index.
   *
   * @param subject The subject to which these values belong.
