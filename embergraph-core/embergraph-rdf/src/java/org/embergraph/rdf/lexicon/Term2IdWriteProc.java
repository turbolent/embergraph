 ...
 * using the assigned term identifiers until the corresponding {@link Id2TermWriteProc} operation
 * has succeeded.
 *
 * <p>In order for the lexicon to remain consistent if the client fails for any reason after the
 * forward mapping has been made restart-safe and before the reverse mapping has been made
 * restart-safe clients MUST always use a successful {@link Term2IdWriteProc} followed by a
 * successful {@link Id2TermWriteProc} before inserting statements using term identifiers into the
 * statement indices. In particular, a client MUST NOT treat lookup against the terms index as
 * satisfactory evidence that the term also exists in the reverse mapping.
 *
 ...
 * (a) unisolated operations are single-threaded; (b) term identifiers are assigned in an unisolated
 * atomic operation by {@link Term2IdWriteProc}; and (c) the reverse mapping is made consistent with
 * the assignments made/discovered by the forward mapping.
 *
 * <p>Note: The {@link Term2IdWriteProc} and {@link Id2TermWriteProc} operations may be analyzed as
 * a batch variant of the following pseudo code.
 *
 * <pre>
 *
 *  for each term:
 ...
 * </pre>
 *
 * In addition, the actual operations against scale-out indices are performed on index partitions
 * rather than on the whole index.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 */
public class Term2IdWriteProc extends AbstractKeyArrayIndexProcedure<Result>
   ...
   * flag is ALSO set.
   *
   * <p><strong>WARNING: This IS NOT scalable! </strong>
   *
   * <p><strong>WARNING: This option IS NOT safe when using more than one triple store either
   * concurrently or in sequence! For example, you can use it to examine a single unit test for
   * inconsistencies, but not a sequence of unit tests since the data will be kept within the same
   * global map and hence confound the test!</strong>
   */
  ...
  public static class Term2IdWriteProcConstructor
    ...
    public Term2IdWriteProcConstructor(
    ...
    @Override
    public Term2IdWriteProc newInstance(
      ...
      if (log.isInfoEnabled()) log.info("TERM2ID Proc Ctor: ntuples=" + (toIndex - fromIndex));

      return new Term2IdWriteProc(
          keySer, fromIndex, toIndex, keys, readOnly, storeBlankNodes, scaleOutTermIdBitsToReverse);
    }
  }
   ...
   * be communicated back to the client, and insert the {term,termId} entry into the <em>terms</em>
   * index.
   *
   * @param ndx The terms index.
   * @return The {@link Result}, which contains the discovered / assigned term identifiers.
   *     <p>TODO no point sending bnodes when readOnly.
   */
  @Override
  public Result applyOnce(final IIndex ndx, final IRaba keys, final IRaba vals) {
    ...
    final TermIdEncoder encoder =
        readOnly
            ? null
            : scaleOutTermIdBitsToReverse == 0
                ? null
                : new TermIdEncoder(scaleOutTermIdBitsToReverse);
    ...
    for (int i = 0; i < numTerms; i++) {
      ...
      if (!storeBlankNodes && code == ITermIndexCodes.TERM_CODE_BND) {
        ...
        if (readOnly) {
          ...
          final long termId = encoder == null ? ctr : encoder.encode(ctr);
        ...
        if (tmp == null) {
          ...
          if (readOnly) {
            ...
            final byte[] bytes = iv.encode(KeyBuilder.newInstance()).getKey();
  ...
  private void groundTruthTest(
      final byte[] key, final long termId, final IIndex ndx, final ICounter counter) {
    ...
    if (oldId != null && oldId.longValue() != termId) {
      ...
      throw new AssertionError(
          "different termId assigned"
              ...
              + ", pmd="
              + ndx.getIndexMetadata().getPartitionMetadata());
    }
