 ...
 */
public class EmbergraphSubjectCentricFullTextIndex extends FullTextIndex
    implements ISubjectCentricTextIndexer<Hit> {

  private static final transient Logger log =
      Logger.getLogger(EmbergraphSubjectCentricFullTextIndex.class);

  /** The basename of the search index. */
  public static final transient String NAME_SUBJ_SEARCH = "subjectSearch";

  public static EmbergraphSubjectCentricFullTextIndex getInstance(
    ...
    return new EmbergraphSubjectCentricFullTextIndex(
        indexManager, namespace, timestamp, properties);
  }
   ...
   * @param properties
   */
  public EmbergraphSubjectCentricFullTextIndex(
      final IIndexManager indexManager,
      final String namespace,
      final Long timestamp,
      final Properties properties) {
  ...
  public void create() {
    ...
    if (log.isInfoEnabled()) log.info("Registered new subject-centric text index: name=" + name);
  ...
  public void index(final IV<?, ?> subject, final Iterator<EmbergraphValue> valuesIterator) {
    ...
    while (valuesIterator.hasNext()) {
      ...
      index(buffer, subject, 0 /* fieldId */, languageCode, new StringReader(text));
    ...
    if (log.isInfoEnabled()) log.info("indexed " + n + " new values for s: " + subject);
  }
}
