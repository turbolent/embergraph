 ...
 */
public class Id2TermWriteProc extends AbstractKeyArrayIndexProcedure<Void>
    implements IParallelizableIndexProcedure<Void>, IMutableRelationIndexWriteProcedure<Void> {
   ...
   * identifiers. When such errors are reported, they generally indicate a problem with the TERM2ID
   * index where it is failing to maintain a consistent mapping.
   *
   * <p>Validation may be disabled for releases, however it is not really that much overhead since
   * the operation is on the in-memory representation.
   *
   * @deprecated Validation can not be reasonably applied it the Unicode collation is less than
   *     Identical. It also has problems for datatype literals if different lexical forms are all
   *     mapped onto the same key,e.g.,
   *     <pre>
   * 12&circ;&circ;&lt;xsd:float&gt;
   * 12.0&circ;&circ;&lt;xsd:float&gt;
   * 12.00&circ;&circ;&lt;xsd:float&gt;
   * </pre>
   *     will all be mapped to the same key and hence would give the appearance of a conflict if we
   *     were to reject any of these forms when another of the forms was already present under the
   *     key.
   *     <p>Note: Now it's not only deprecated, but the code that relies on it has been commented
   *     out. This is because it makes assumptions about how {@link TermId} objects are encoded and
   *     decoded. Under the legacy model they were simple longs. After the lexicon refactor we use
   *     the byte flags also. So if we ever decide to do validation again here, we need to figure
   *     out how to give this class access to an {@link IIVEncoder}.
   */
  ...
  public static class Id2TermWriteProcConstructor
      extends AbstractKeyArrayIndexProcedureConstructor<Id2TermWriteProc> {
    ...
    @Override
    public Id2TermWriteProc newInstance(
        final IRabaCoder keysCoder,
        final IRabaCoder valsCoder,
        final int fromIndex,
        final int toIndex,
      ...
      return new Id2TermWriteProc(keysCoder, valsCoder, fromIndex, toIndex, keys, vals);
    }
  }
   ...
   * ids index to be re-inserted, thereby reducing writes of dirty index nodes.
   *
   * @param ndx The index.
  ...
  public Void applyOnce(final IIndex ndx, final IRaba keys, final IRaba vals) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      //                         * datatype literals.
      //                         */
      //                        final EmbergraphValueSerializer valSer = new
