 ...
 * value side of the tuple in order to keep the key format simpler.
 *
 * <p>Note: The RDF database does not make use of the "field" concept in the keys of the full text
 * index. The fieldId will always be reported as -1.
 *
...
public class RDFFullTextIndexTupleSerializer
  ...
  private static final transient Logger log =
      Logger.getLogger(RDFFullTextIndexTupleSerializer.class);
   ...
   * @param leafValsCoder The coder used for the leaf values (custom coding may provide tighter
   *     representations of the {@link ITermDocVal} s in the index entries).
   * @param fieldsEnabled When <code>true</code> the <code>fieldId</code> will be included as a
   *     component in the generated key. When <code>false</code> it will not be present in the
   *     generated key.
   */
  public RDFFullTextIndexTupleSerializer(
  ...
  public byte[] serializeKey(final Object obj) {
    ...
    if (log.isDebugEnabled()) {
      ...
      log.debug("{" + termText + "," + docId + "}, key=" + BytesUtil.toString(key));
    }
  ...
  public byte[] serializeVal(final ITermDocVal obj) {
    ...
    final int byteLen = ((IV) ((ITermDocRecord) obj).getDocId()).byteLength();
  ...
  protected ITermDocKey deserialize(final ITuple tuple, final boolean keyOnly) {
    ...
    try {
      ...
      //                    .getValueStream());
      byteLength = ShortPacker.unpackShort((DataInput) tuple.getValueStream());
    ...
    return new ReadOnlyTermDocRecord(null /* token */, docId, NO_FIELD, /* termFreq, */ termWeight);
  }
