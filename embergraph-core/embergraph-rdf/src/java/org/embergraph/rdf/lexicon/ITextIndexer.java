...
public interface ITextIndexer<A extends IHit> {
  ...
  class FullTextQuery implements Serializable {
    ...
    public FullTextQuery(final String query, final String languageCode, final boolean prefixMatch) {
      this(
          query,
          languageCode,
          prefixMatch,
     ...
     * @param languageCode The language code that should be used when tokenizing the query -or-
     *     <code>null</code> to use the default {@link Locale} ).
     * @param prefixMatch When <code>true</code>, the matches will be on tokens which include the
     *     query tokens as a prefix. This includes exact matches as a special case when the prefix
     *     is the entire token, but it also allows longer matches. For example, <code>free</code>
     *     will be an exact match on <code>free</code> but a partial match on <code>freedom</code>.
     *     When <code>false</code>, only exact matches will be made.
     * @param matchRegex A regex filter to apply to the search.
     * @param matchAllTerms if true, return only hits that match all search terms
     * @param matchExact if true, return only hits that have an exact match of the search string
     * @param minCosine The minimum cosine that will be returned (in [0:maxCosine]). If you specify
     *     a minimum cosine of ZERO (0.0) you can drag in a lot of basically useless search results.
     * @param maxCosine The maximum cosine that will be returned (in [minCosine:1.0]). Useful for
    ...
    public boolean equals(Object obj) {
      ...
      if (languageCode == null) {
        if (other.languageCode != null) return false;
      } else if (!languageCode.equals(other.languageCode)) return false;
