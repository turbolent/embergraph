...
public class LexiconKeyBuilder implements ITermIndexCodes {
   ...
   *     among the rdf {@link Value}s. In general, this should support Unicode and should use {@link
   *     StrengthEnum#Identical} so that all distinctions in the {@link Value} space are recognized
   *     by the lexicon.
   * @see IKeyBuilder
   ...
   * language code, so we require that the languageCode is non-empty here. The language code
   * specifications require that the language code comparison is case-insensitive, so we force the
   * code to upper case for the purposes of comparisons.
   *
   * @see Literal#getLanguage()
   */
  public byte[] languageCodeLiteral2key(final String languageCode, final String text) {
