 ...
 *
 * <p>The first way to compute an analytic over a dynamic graph is to specify the timestamp of the
 * view as {@link ITx#READ_COMMITTED}. The view of the graph <em>in each round</em> will be
 * automatically advanced to the most recently committed view of that graph. Thus, if there are
 * concurrent commits, each time the {@link IGASProgram} is executed within a given round of
 * evaluation, it will see the most recently committed state of the data graph.
 *
 * <p>The second way to compute an analytic over a dynamic graph is to explicitly change the view
 * before each round. This can be achieved by tunneling the {@link EmbergraphGraphAccessor}
 * interface from {@link IGASProgram#nextRound(IGASContext)}. If you take this approach, then you
 * could explicitly walk through an iterator over the commit record index and update the timestamp
 * of the view. This approach allows you to replay historical committed states of the graph at a
 * known one-to-one rate (one graph state per round of the GAS computation).
 *
 * <p>TODO Algorithms that need to visit all vertices in each round (CC, BC, PR) can be more
 * optimially executed by a different implementation strategy. The vertex state should be arranged
 * in a dense map (maybe an array) and presized. For example, this could be done on the first pass
 * when we identify a vertex index for each distinct V in visitation order.
 *
 * <p>TODO Vectored expansion with conditional materialization of attribute values could be achieved
 * using CONSTRUCT. This would force URI materialization as well. If we drop down one level, then we
 * can push in the frontier and avoid the materialization. Or we can just write an operator that
 * accepts a frontier and returns the new frontier and which maintains an internal map containing
 * both the visited vertices, the vertex state, and the edge state.
 *
 * <p>TODO Some computations could be maintained and accelerated. A great example is Shortest Path
 * (as per RDF3X). Reachability queries for a hierarchy can also be maintained and accelerated
 * (again, RDF3X using a ferrari index).
 *
 * <p>TODO Option to materialize Literals (or to declare the set of literals of interest) [Note: We
 * can also require that people inline all URIs and Literals if they need to have them materialized,
 * but a materialization filter for Gather and Scatter would be nice if it can be selective for just
 * those attributes or vertex identifiers that matter).
 *
 * <p>TODO DYNAMIC GRAPHS: Another possibility would be to replay a history log, explicitly making
 * changes to the graph. In order to provide high concurrency for readers, this would require a
 * shadowing of the graph (effectively, shadowing the indices). That might be achieved by replaying
 * the changes into a version fork of the graph and then using a read-only view of the fork. This is
 * basically a retroactive variant of replaying the commit points from the commit record index. I am
 * not sure if it has much to recommend it.
 *
 * <p>The thing that is interesting about the history index, is that it captures just the delta.
 * Actually computing the delta between two commit points is none-trivial without the history index.
 * However, I am not sure how we can leverage that delta in an interesting fashion for dynamic
 * graphs.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 */
public class EmbergraphGASEngine extends GASEngine {
   ...
   *
   * @param sail The sail (must be a {@link EmbergraphSail}).
   * @param nthreads The number of threads to use for the SCATTER and GATHER phases.
   */
  public EmbergraphGASEngine(final Sail sail, final int nthreads) {

    this(((EmbergraphSail) sail).getIndexManager(), nthreads);
  }

  /**
   * @param indexManager The index manager.
   * @param nthreads The number of threads to use for the SCATTER and GATHER phases.
   *     <p>TODO Scale-out: The {@link IIndexmanager} MAY be an {@link IEmbergraphFederation}. The
   *     {@link EmbergraphGASEngine} would automatically use remote indices. However, for proper
   *     scale-out we want to partition the work and the VS/ES so that would imply a different
   *     {@link IGASEngine} design.
   */
  public EmbergraphGASEngine(final IIndexManager indexManager, final int nthreads) {
  ...
  public <VS, ES, ST> IGASState<VS, ES, ST> newGASState(
      final IGraphAccessor graphAccessor, final IGASProgram<VS, ES, ST> gasProgram) {
  ...
  /** Returns <code>true</code> since the IOs will be vectored if the frontier is sorted. */
  @Override
  public boolean getSortFrontier() {
    return true;
  }

  public static class EmbergraphGraphAccessor implements IGraphAccessor {
    ...
    /**
     * @param indexManager The index manager.
     * @param namespace The namespace of the graph.
     * @param timestamp The timestamp of the view. If you specify {@link ITx#READ_COMMITTED}, then
     *     {@link IIndexManager#getLastCommitTime()} will be used to locate the most recently
     *     committed view of the graph at the start of each iteration. This provides one mechanism
     *     for dynamic graphs.
     */
    public EmbergraphGraphAccessor(
    ...
    public EmbergraphGraphAccessor(final IIndexManager indexManager) {

      this(
          indexManager, EmbergraphSail.Options.DEFAULT_NAMESPACE, indexManager.getLastCommitTime());
    }
     ...
     *
     * @return The graph.
     * @throws RuntimeException if the graph could not be resolved.
     */
    ...
    /** Access path abstraction for a vertex for either its in-edges or its out-edges. */
    private class AP {
      ...
      public AP(
         ...
         */
        posOptimization = linkTypeIV != null && linkAttrTypeIV == null && inEdges;
       ...
       *
       * <p>FIXME This is not optimized for the cases where the BTree range count would be exact.
       * Part of the problem is that we need to tunnel the BTree API and discover whether or not the
       * indicies support delete markers. However, the larger problem is that we can not apply the
       * optional {@link IGASProgram#constrainFilter(IGASContext, IStriterator)}. Due to the API of
       * that method (which optionally adds a filter to the striterator or wraps it), we do not know
       * whether or not a constrain filter was specified and are unable to decide here whether or
       * not we need to enumerate the edges in order to find only those which also satisify that
       * filter.
       */
    ...
    private IV<?, ?> getIV(final Value u) {
      ...
      if (u instanceof IV) return (IV<?, ?>) u;

      if (u instanceof EmbergraphValue) {

        final IV<?, ?> iv = ((EmbergraphValue) u).getIV();

        if (iv == null) throw new NotMaterializedException(u.toString());
    ...
    @Override
    @SuppressWarnings({"rawtypes"})
    public long getEdgeCount(final IGASContext<?, ?, ?> ctx, final Value u, final EdgesEnum edges) {
        ...
        case AllEdges:
          {
            ...
            final long b = getEdgeCount(kb, false /* outEdges */, ctx, getIV(u));
            final long n = a + b;
            return n;
          }
        default:
    ...
    public VertexDistribution getDistribution(final Random r) {
      ...
      @SuppressWarnings("unchecked")
      final ITupleIterator<ISPO> titr = getKB().getSPORelation().getPrimaryIndex().rangeIterator();
     ...
     *
     * <p>Note: This sampling procedure has a bias in favor of the vertices with the most edges and
     * properties (vertices are choosen randomly in proportion to the #of edges and properties for
     * the vertex).
     *
     * @param desiredSampleSize The desired sample size.
     * @return The distinct samples that were found.
     *     <p>TODO This is a (MUCH) more efficient way to create a biased sample. However, having a
     *     representative sample is probably more important than being efficient. Keep this code or
     *     discard? Maybe we could refactor the code to produce a {@link VertexDistribution} that
     *     was a sample of the total population, from which we then took a sample using the {@link
     *     VertexDistribution} API? That could be useful for large graphs (better than scanning the
     *     SPO(C) index!)
     */
    @SuppressWarnings("rawtypes")
    private IV[] getRandomSample(
        final Random r, final AbstractTripleStore kb, final int desiredSampleSize) {
     ...
     *
     * <p>The vertex is sampled in proportion to the number of statements having that vertex as a
     * subject, hence the sample is proportional to the #of property values and out-links for a
     * given vertex (in-links are not considered by this sampling procedure).
     */
    @SuppressWarnings("rawtypes")
    private IV getRandomVertex(final Random r, final AbstractTripleStore kb) {
      ...
      IV startingVertex = null;
      {
        ...
        while (size > 0L && startingVertex == null) {
          ...
          final ISPO edge = (ISPO) ndx.getIndexMetadata().getTupleSerializer().deserialize(tuple);

          // Use the subject for that edge as the starting vertex.
          startingVertex = edge.s();

          if (log.isInfoEnabled()) log.info("Starting vertex: " + startingVertex);
        }
      }
