 ...
 * Finally, the frontier segments are combined using a {@link MergeSortIterator} - this is a
 * sequential step with a linear cost in the size of the frontier.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 *     <p>TODO Discard if dominated by {@link TLScheduler2}.
 */
public class TLScheduler implements IGASSchedulerImpl {
  ...
  private static class MySTScheduler extends STScheduler {
     ...
     *
     * <p>Note: The schedule (for each thread) is using a set - see the {@link STScheduler} base
     * class. This means that the schedule (for each thread) is compact, but not ordered. We need to
     * use (and re-use) an array to order that compact per-thread schedule. The compact per-thread
     * schedules are then combined into a single compact frontier for the new round.
     */
  ...
  private IGASScheduler threadLocalScheduler() {
    ...
    if (s == null) {
      ...
      final IGASScheduler old = map.putIfAbsent(id, s = new MySTScheduler(gasEngine));
  ...
  public void compactFrontier(final IStaticFrontier frontier) {
    ...
    if (nsources > nthreads) {
      ...
      throw new AssertionError("nsources=" + nsources + ", nthreads=" + nthreads);
    }
  ...
  /**
   * Now merge sort the ordered frontier segments and populate the new frontier.
   *
   * @param nsources The #of frontier segments.
   * @param nvertices The total #of vertice across those segments (may double-count across
   *     segments).
   * @param frontiers The ordered, compact frontier segments
   * @param frontier The new frontier to be populated.
   */
  private void mergeSortSourcesAndSetFrontier(
      final int nsources,
      final int nvertices,
      final IArraySlice<Value>[] frontiers,
     ...
     */
    frontier.resetFrontier(nvertices /* minCapacity */, false /* sortFrontier */, itr);
  }
}
