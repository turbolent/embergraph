 ...
 *
 * @todo Consider a {@link WeakValueCache} on this factory to avoid duplicate values.
...
public class EmbergraphValueFactoryImpl implements EmbergraphValueFactory {
   ...
   * of the {@link EmbergraphValueFactory}, which is used for query/update parsing. It SHOULD NOT be
   * used in code working with triple-store.
   *
  ...
  private EmbergraphValueFactoryImpl(final String namespace) {
    ...
    // @see <a href="http://trac.blazegraph.com/ticket/983"> Concurrent insert data with boolean
   ...
   *
   * <p>Note: The backing LRU should be small (and can be zero) since instances SHOULD be finalized
   * quickly once they are no longer strongly reachable (which would imply that there was no {@link
   * LexiconRelation} for that instance and that all {@link EmbergraphValueImpl}s for that instance
   * had become weakly reachable or been swept).
   */
  private static CanonicalFactory<
          String /* namespace */, EmbergraphValueFactoryImpl, String /*State*/>
      cache =
          new CanonicalFactory<String, EmbergraphValueFactoryImpl, String>(1 /* capacity */) {
            @Override
  ...
  //    private static WeakValueCache<String/* namespace */, EmbergraphValueFactoryImpl> cache = new
   ...
   * namespace of the {@link LexiconRelation}. This makes the instances canonical within a JVM
   * instance, which is all that we care about. The actual assignments of term identifiers to {@link
   * EmbergraphValue} s is performed by the {@link LexiconRelation} itself and is globally
   * consistent for a given lexicon.
   *
   * @param namespace The namespace of the {@link LexiconRelation}.
   *     <p>TODO This method introduces the possibility that two journals in the same JVM would
   *     share the same {@link EmbergraphValueFactory} for a kb with the same namespace. This is
   *     doubtless not desired. A workaround is to use the {@link UUID} of the Journal as part of
   *     the namespace of the KB, which would serve to make sure that all KB instances have distinct
   *     namespaces.
   */
   ...
   * #remove(String)}ed if the {@link LexiconRelation} is destroyed in case a distinct lexicon is
   * subsequently creating with the same namespace. There is no need to discard an entry during
   * abort processing.
   */
  //    * @param namespace
  //    *            The namespace of the {@link LexiconRelation}.
   ...
   * index partition(s). More efficient ordered writes may be realized using {@link
   * #newBNodeContext()} to obtain a derived {@link EmbergraphValueFactory} instance that is
   * specific to a document that is being loaded into the RDF DB.
   *
   * @see #newBNodeContext()
   */
  @Override
  ...
  public EmbergraphBNodeImpl createBNode(final EmbergraphStatement stmt) {
    ...
    if (originalS != s || originalP != p || originalO != o || originalC != c) {
      ...
      effectiveStmt =
          new EmbergraphStatementImpl(s, p, o, c, stmt.getStatementType(), stmt.getUserFlag());
  ...
  private final EmbergraphURIImpl xsd_string = new EmbergraphURIImpl(this, xsd + "string");

  private final EmbergraphURIImpl xsd_dateTime = new EmbergraphURIImpl(this, xsd + "dateTime");

  private final EmbergraphURIImpl xsd_date = new EmbergraphURIImpl(this, xsd + "date");

  private final EmbergraphURIImpl xsd_long = new EmbergraphURIImpl(this, xsd + "long");

  private final EmbergraphURIImpl xsd_int = new EmbergraphURIImpl(this, xsd + "int");

  private final EmbergraphURIImpl xsd_byte = new EmbergraphURIImpl(this, xsd + "byte");

  private final EmbergraphURIImpl xsd_short = new EmbergraphURIImpl(this, xsd + "short");

  private final EmbergraphURIImpl xsd_ulong = new EmbergraphURIImpl(this, xsd + "unsignedLong");

  private final EmbergraphURIImpl xsd_uint = new EmbergraphURIImpl(this, xsd + "unsignedInt");

  private final EmbergraphURIImpl xsd_ubyte = new EmbergraphURIImpl(this, xsd + "unsignedByte");

  private final EmbergraphURIImpl xsd_ushort = new EmbergraphURIImpl(this, xsd + "unsignedShort");

  private final EmbergraphURIImpl xsd_double = new EmbergraphURIImpl(this, xsd + "double");

  private final EmbergraphURIImpl xsd_float = new EmbergraphURIImpl(this, xsd + "float");

  private final EmbergraphURIImpl xsd_boolean = new EmbergraphURIImpl(this, xsd + "boolean");
   ...
   *     causes IllegalArgumentException </a>
   * @see <a href="http://trac.blazegraph.com/ticket/980">Object position of query hint is not a
   *     Literal </a>
   */
  @Override
  ...
  public EmbergraphLiteralImpl createLiteral(byte arg0, final boolean unsigned) {
    ...
    return new EmbergraphLiteralImpl(
        this,
        "" + (unsigned ? XSDUnsignedByteIV.promote(arg0) : arg0),
        null,
        unsigned ? xsd_ubyte : xsd_byte);
  }
  ...
  public EmbergraphLiteralImpl createLiteral(short arg0, final boolean unsigned) {
    ...
    return new EmbergraphLiteralImpl(
        this,
        "" + (unsigned ? XSDUnsignedShortIV.promote(arg0) : arg0),
        null,
        unsigned ? xsd_ushort : xsd_short);
  }
  ...
  public EmbergraphLiteralImpl createLiteral(int arg0, final boolean unsigned) {
    ...
    return new EmbergraphLiteralImpl(
        this,
        "" + (unsigned ? XSDUnsignedIntIV.promote(arg0) : arg0),
        null,
        unsigned ? xsd_uint : xsd_int);
  }
  ...
  public EmbergraphLiteralImpl createLiteral(long arg0, final boolean unsigned) {
    ...
    return new EmbergraphLiteralImpl(
        this,
        "" + (unsigned ? XSDUnsignedLongIV.promote(arg0) : arg0),
        null,
        unsigned ? xsd_ulong : xsd_long);
  }
  ...
  public EmbergraphLiteralImpl createLiteral(final XMLGregorianCalendar arg0) {
    ...
    return new EmbergraphLiteralImpl(
        ...
        arg0.toString(),
        null /* languageCode */,
        createURI(XMLDatatypeUtil.qnameToURI(arg0.getXMLSchemaType()).stringValue()));
  }
  ...
  public EmbergraphLiteralImpl createLiteral(String label, URI datatype, String language) {
    ...
    return new EmbergraphLiteralImpl(this, label, language, (EmbergraphURIImpl) datatype);
  }
  ...
  @Override
  public EmbergraphStatementImpl createStatement(Resource s, URI p, Value o, Resource c) {

    return createStatement(s, p, o, c, null /* type */);
  }

  @Override
  public EmbergraphStatementImpl createStatement(
  ...
  @Override
  public final EmbergraphValue asValue(final Value v) {

    if (v == null) return null;

    if (v instanceof EmbergraphValueImpl && ((EmbergraphValueImpl) v).getValueFactory() == this) {
