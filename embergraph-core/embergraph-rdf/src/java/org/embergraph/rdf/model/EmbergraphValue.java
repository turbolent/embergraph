...
public interface EmbergraphValue extends Value, IElement {//, Comparable<EmbergraphValue> {
	 ...
	 * singleton (there will only be one {@link EmbergraphValueFactory} instance
	 * for on a given JVM for all {@link EmbergraphValue}s associated with a given
     ...
     * <p>
     * Note: Both {@link IV} and {@link EmbergraphValue} can cache one another. The
     * pattern for caching is that you <em>always</em> cache the {@link IV} on
     * the {@link EmbergraphValue} using {@link EmbergraphValue#setIV(IV)}. However,
     * the {@link EmbergraphValue} is normally NOT cached on the {@link IV}. The
     * exception is when the {@link EmbergraphValue} has been materialized from the
     * {@link IV} by joining against the lexicon. The query plan is responsible
     * for deciding when to materialize the {@link EmbergraphValue} from the
     * {@link IV}.
     * 
     * @param iv
     *            The internal value.
