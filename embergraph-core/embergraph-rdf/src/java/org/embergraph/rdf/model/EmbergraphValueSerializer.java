 ...
 * @version $Id$
 */
public class EmbergraphValueSerializer<V extends Value> {
   ...
   * (though it might be possible to optimize the code paths for {@link #VERSION1}).
   *
   * <p>Note: {@link #VERSION0} can not be used with the BLOBS index since it can not handle very
   * large {@link Value}s. When changing this to {@link #VERSION0}, that change needs to be
   * exclusive of the version used for the BLOBS index (ie, it is good for ID2TERM and related
   * indices such as the ivCache, but not for the BLOBS index and related indices such as the
   * blobsCache).
   *
   ...
   */
  public EmbergraphValueSerializer(final ValueFactory valueFactory) {
  ...
  private byte getTermCode(final Value val) {
    ...
    if (val instanceof URI) {
      ...
      if (lit.getLanguage() != null) return ITermIndexCodes.TERM_CODE_LCL;

      if (lit.getDatatype() != null) return ITermIndexCodes.TERM_CODE_DTL;
   ...
   *
   * @param val The value.
   * @param out The buffer - the caller is responsible for resetting the buffer before each
   *     invocation.
   * @param tmp A buffer used to compress the component Unicode strings. This will be reset as
   *     necessary by this method. It will be lazily allocated if <code>null</code>.
   * @return The byte[] containing the serialized data record. This array is newly allocated so that
   *     a series of invocations of this method return distinct byte[]s.
   */
  public byte[] serialize(final V val, final DataOutputBuffer out, final ByteArrayBuffer tmp) {
   ...
   *     necessary by this method. It will be lazily allocated if <code>null</code>.
   */
  public void serialize2(final V val, final DataOutputBuffer out, ByteArrayBuffer tmp) {
   ...
   * EmbergraphValue}s from the de-serialized state so the factory reference is always set on the
   * returned {@link EmbergraphValueImpl}.
   *
   * @param b The byte[] containing the serialized data record.
   ...
   * returned {@link EmbergraphValueImpl}.
   *
   * @param b An input stream from which the serialized data may be read.
   * @param tmp A buffer used to decode the component Unicode strings. The length of the buffer will
   *     be reset as necessary by this method.
   * @return The {@link EmbergraphValue}.
   * @throws RuntimeException if there is an IO problem.
   * @see {@link #serialize()}
   */
  public V deserialize(final DataInputBuffer in, final StringBuilder tmp) {
    ...
    try {
      ...
      switch (version) {
        ...
        default:
          throw new UnsupportedOperationException(ERR_VERSION + " : " + version);
      }
   ...
   * @param version The serialization version number (which has already been written on <i>out</i>
   *     by the caller).
   * @param out The data are written here.
   * @throws IOException
   */
  private void serializeVersion0(final V val, final short version, final DataOutput out)
      throws IOException {
   ...
   * returned {@link EmbergraphValueImpl}.
   *
   * @param version The serialization version number (which has already been read by the caller).
   * @param in The data are read from here.
   * @throws IOException
   */
  @SuppressWarnings("unchecked")
  private V deserializeVersion0(final short version, final DataInput in) throws IOException {
      ...
      case ITermIndexCodes.TERM_CODE_DTL:
        {
          ...
          return (V) valueFactory.createLiteral(label, valueFactory.createURI(datatype));
        }
   ...
   *     by the caller).
   * @param out The data are written here.
   * @param tmp A buffer used to compress the component Unicode strings.
   * @throws IOException
   */
  private void serializeVersion1(
      final V val, final short version, final OutputStream out, final ByteArrayBuffer tmp)
      throws IOException {
   ...
   * @param version The serialization version number (which has already been read by the caller).
   * @param in The data are read from here.
   * @param tmp Buffer used to extract bytes to be decompressed.
   * @param sb Buffer used to decompress bytes.
   * @throws IOException
   */
  @SuppressWarnings("unchecked")
  private V deserializeVersion1(
      final short version, final DataInputBuffer in, final StringBuilder tmp) throws IOException {
   ...
   */
  public static long getStringLength(final Value v) {
    ...
    if (v instanceof URI) {
      ...
      final int datatypeLength =
          value.getDatatype() == null ? 0 : value.getDatatype().stringValue().length();

      final int languageLength = value.getLanguage() == null ? 0 : value.getLanguage().length();

      final long totalLength = label.length() + datatypeLength + languageLength;
