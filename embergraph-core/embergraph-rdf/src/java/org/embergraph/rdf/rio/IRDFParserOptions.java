...
public interface IRDFParserOptions {
  ...
  /**
   * Return <code>true</code> if the parser should stop at the first error and <code>false</code> if
  ...
  /** Sets the datatype handling mode (default is {@link DatatypeHandling#VERIFY}). */
  ...
  /**
   * Sets whether the parser should stop immediately if it finds an error in the data (default value
   * is <code>true</code>).
   */
  void setStopAtFirstError(final boolean stopAtFirstError);

  /**
   * Sets whether the parser should verify the data it parses (default value is <code>true</code>).
   */
