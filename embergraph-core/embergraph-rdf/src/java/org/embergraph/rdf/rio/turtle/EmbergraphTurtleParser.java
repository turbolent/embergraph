 ...
 *
 * <ul>
 *   <li>Normalization of integer, floating point and boolean values is dependent on the specified
 *       datatype handling. According to the specification, integers and booleans should be
 *       normalized, but floats don't.
 *   <li>Comments can be used anywhere in the document, and extend to the end of the line. The
 *       Turtle grammar doesn't allow comments to be used inside triple constructs that extend over
 *       multiple lines, but the author's own parser deviates from this too.
 * </ul>
 *
 * @author Arjohn Kampman
 * @openrdf
 */
public class EmbergraphTurtleParser extends TurtleParser {

  /** Standalone variant of the parser, should be used only when connection is not available. */
  public EmbergraphTurtleParser() {
  ...
  protected Value parseURIOrSid() throws IOException, RDFParseException {
    ...
    if (n == '<') {
      ...
      if (this.valueFactory == null) {
        reportFatalError("must use a EmbergraphValueFactory to use the RDR syntax");
      }
      return parseSid();
    } else {
      unread(c);
  ...
  protected Value parseSid() throws IOException, RDFParseException {
    ...
    if (i == '>' && read() == '>') {
      if (valueFactory == null || valueFactory instanceof EmbergraphValueFactory == false) {
         ...
         */
        throw new RDFParseException("You must use a EmbergraphValueFactory to use the RDR syntax");
      }
      ...
      final EmbergraphValueFactory valueFactory = (EmbergraphValueFactory) super.valueFactory;
