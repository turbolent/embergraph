...
public class RDFParserOptions implements Serializable, IRDFParserOptions {
   ...
   */
  @Override
  public synchronized DatatypeHandling getDatatypeHandling() {
  ...
  public RDFParserOptions(final Properties properties) {
    ...
    setVerifyData(
        Boolean.parseBoolean(
            properties.getProperty(Options.VERIFY_DATA, Options.DEFAULT_VERIFY_DATA)));

    if (log.isInfoEnabled()) log.info(Options.VERIFY_DATA + "=" + getVerifyData());

    // stopAtFirstError
    setStopAtFirstError(
        Boolean.parseBoolean(
            properties.getProperty(
                Options.STOP_AT_FIRST_ERROR, Options.DEFAULT_STOP_AT_FIRST_ERROR)));

    if (log.isInfoEnabled()) log.info(Options.STOP_AT_FIRST_ERROR + "=" + getStopAtFirstError());

    // datatypeHandling
    setDatatypeHandling(
        DatatypeHandling.valueOf(
            properties.getProperty(Options.DATATYPE_HANDLING, Options.DEFAULT_DATATYPE_HANDLING)));

    if (log.isInfoEnabled()) log.info(Options.DATATYPE_HANDLING + "=" + getDatatypeHandling());

    setPreserveBNodeIDs(
        Boolean.parseBoolean(
            properties.getProperty(
                Options.PRESERVE_BNODE_IDS, Options.DEFAULT_PRESERVE_BNODE_IDS)));
  }
  ...
  public RDFParserOptions(
    ...
    if (datatypeHandling == null) throw new IllegalArgumentException();
   ...
   */
  @Override
  public synchronized void setDatatypeHandling(final DatatypeHandling datatypeHandling) {
   ...
   */
  @Override
  public synchronized void setPreserveBNodeIDs(final boolean preserveBNodeIDs) {
   ...
   */
  @Override
  public synchronized void setStopAtFirstError(final boolean stopAtFirstError) {
    this.stopAtFirstError = stopAtFirstError;
  }
   ...
   */
  @Override
  public synchronized void setVerifyData(final boolean verifyData) {
    this.verifyData = verifyData;
  }
   ...
   * @param p The parser.
   */
  public synchronized void apply(final RDFParser p) {
  ...
  public boolean equals(final Object o) {
    ...
    if (!(o instanceof IRDFParserOptions)) return false;

    final IRDFParserOptions t = (IRDFParserOptions) o;

    if (verifyData != t.getVerifyData()) return false;

    if (preserveBNodeIDs != t.getPreserveBNodeIDs()) return false;
