 ...
 * reward for reducing {@link Value}s to only the distinct {@link Value}s during processing. On the
 * other hand, there is little {@link Statement} duplication. Hence we pay an unnecessary overhead
 * if we try to make the statements distinct in the buffer.
 *
 * <p>Note: This also provides an explanation for why neither this class nor writes of SPOs do
 * better when "distinct" statements is turned on - the "Value" objects in that case are only
 * represented by long integers and duplication in their values does not impose a burden on either
 * the heap or the index writers. In contrast, the duplication of {@link Value}s in the {@link
 * StatementBuffer} imposes a burden on both the heap and the index writers.
 *
...
public class StatementBuffer<S extends Statement>
   ...
   *
   * @see https://jira.blazegraph.com/browse/BLZG-1708 (DataLoader fails with
   ...
   * from the current source and therefore that the blank node is NOT a statement identifier. This
   * map is used IFF statement identifiers are enabled. When statement identifiers are NOT enabled
   * blank nodes are always blank nodes and we do not need to defer statements, only maintain the
   * canonicalizing {@link #bnodes} mapping.
   */
   ...
   * #processDeferredStatements()} in order to reuse the {@link StatementBuffer} for batch writes of
   * the deferred statement as well.
   *
   ...
   *
   * <p>Note: This is lazily initialized since {@link #reset()} does not have the semantics of
   * "close()" and the {@link StatementBuffer} MIGHT be reused.
   */
  ...
  public String toString() {
    ...
    return "numURIs="
        ...
        + numSIDs
        + ", values.length="
        + (values != null ? String.valueOf(values.length) : "null")
        + ", stmts.length="
        + (stmts != null ? String.valueOf(stmts.length) : "null")
        + ", bnodes.size()="
        + (bnodes != null ? String.valueOf(bnodes.size()) : "null")
        + ", distinctTermMap.size()="
        + (distinctTermMap != null ? String.valueOf(distinctTermMap.size()) : "null")
        + ", reifiedStmts.size()="
        + (reifiedStmts != null ? String.valueOf(reifiedStmts.size()) : "null")
        + ", deferredStmts.size()="
        + (deferredStmts != null ? String.valueOf(deferredStmts.size()) : "null")
   ...
   *
   * @todo Use an {@link IBuffer} pattern can be used to make the statement buffer chunk-at-a-time.
   *     The buffer has a readOnly argument and will visit SPOs for the source statements. When
   *     readOnly, new terms will not be added to the database.
   * @todo Once we have the {@link SPO}s we can just feed them into whatever consumer we like and do
   *     bulk completion, bulk filtering, write the SPOs onto the database,etc.
   * @todo must also support the focusStore patterns, which should not be too difficult.
   */
   ...
   *
   * @param changeLog The change log listener.
   */
  ...
  public interface IWrittenSPOArray {
     ...
     *
     * @param stmts An array of the statements written onto the backing store.
     * @param numStmts The number of entries in that array that were written.
     */
   ...
   * have {@link SPO}s then use {@link IRawTripleStore#addStatements(IChunkedOrderedIterator,
   * IElementFilter)} and friends.
   *
   * @param database The database into which the termS and statements will be inserted.
   * @param capacity The #of statements that the buffer can hold.
   */
  ...
  public StatementBuffer(
      final AbstractTripleStore database, final int capacity, final int queueCapacity) {
   ...
   *
   * @param statementStore The store into which the statements will be inserted (optional). When
   *     <code>null</code>, both statements and terms will be inserted into the <i>database</i>.
   *     This optional argument provides the ability to load statements into a temporary store while
   *     the terms are resolved against the main database. This facility is used during incremental
   *     load+close operations.
   * @param database The database. When <i>statementStore</i> is <code>null</code>, both terms and
   *     statements will be inserted into the <i>database</i>.
   * @param capacity The #of statements that the buffer can hold.
   * @param queueCapacity The capacity of blocking queue used by the {@link StatementBuffer} -or-
   *     ZERO (0) to disable the blocking queue and perform synchronous writes (default is {@value
   *     #DEFAULT_QUEUE_CAPACITY} statements). The blocking queue holds parsed data pending writes
   *     onto the backing store and makes it possible for the parser to race ahead while writer is
   *     blocked writing onto the database indices.
   * @see BLZG-1552 (added blocking queue)
   */
  public StatementBuffer(
      final TempTripleStore statementStore,
   ...
   * Calling this method will flush the buffer, cause any deferred statements to be written, and
   * cause the canonicalizing mapping for blank nodes to be discarded.
   *
  ...
  //                                + " out of "
  //                                + nbefore
  //                                + " deferred statements used only blank nodes (vs statement
  // identifiers).");
  //
  //                /*
  ...
  private static class MergeUtility<S extends Statement> {
     ...
     * @throws IllegalArgumentException if the argument is an empty list.
     * @throws IllegalArgumentException if the argument does not contain at least two batches.
     */
    public Batch<S> merge(final List<Batch<S>> avail) {

      if (avail == null) throw new IllegalArgumentException();
      if (avail.isEmpty()) throw new IllegalArgumentException();
      if (avail.size() < 2) throw new IllegalArgumentException();
     ...
     *
     * @param term A term.
     * @return Either the term or the pre-existing term in the {@link Batch} with the same data.
     * @throws IllegalArgumentException if the argument is null (so do not pass a null context in
     *     here!)
     */
  ...
  private static class Batch<S extends Statement> {
    ...
    /** Singleton instance used to indicate that no more elements will be added to the queue. */
     ...
     *     it is possible (there is enough capacity for the blank nodes, which has to be written to
     *     DB); in this case the caller MUST invoke {@link #writeNow()} synchronously. (This is used
     *     to make it easier to compare the two approaches without introducing any new overhead).
     * @see https://jira.blazegraph.com/browse/BLZG-1889 (ArrayIndexOutOfBound Exception)
     */
    Batch(final StatementBuffer<S> sb, final boolean avoidCloningIfPossible) {
      ...
      if (sb.bnodes != null) {
        ...
        for (EmbergraphBNode bnode : sb.bnodes.values()) {
          ...
          // sid, skip
          if (bnode.isStatementIdentifier()) continue;
      ...
      if (avoidCloningIfPossible && bnodes.isEmpty()) {
        ...
        // Direct use of StatementBuffer values array is possible, as there are no blank nodes to be
        // written
    ...
    private BatchResult writeNow() {
      ...
      if (log.isInfoEnabled()) log.info("numValues=" + numValues + ", numStmts=" + numStmts);
          ...
          for (int i = 0; i < numValues; i++) {
            log.debug(
                "adding term: "
                    ...
                    + values[i].getIV()
                    + ")"
                    + ((values[i] instanceof BNode)
                        ? "sid=" + ((EmbergraphBNode) values[i]).isStatementIdentifier()
                        : ""));
          }
        }
      ...
      if (numStmts > 0) {
        ...
        nwritten =
            addStatements(database, statementStore, stmts, numStmts, changeLog, didWriteCallback);
        if (DEBUG) {
          for (int i = 0; i < numStmts; i++) {
      ...
      if (log.isInfoEnabled()) {
        ...
        log.info(
            "numValues=" + numValues + ", numStmts=" + numStmts + ", elapsed=" + elapsed + "ms");
      }
    ...
    private static void addTerms(
      ...
      if (log.isInfoEnabled()) log.info("writing " + numTerms);
      ...
      final long l = database.getLexiconRelation().addTerms(terms, numTerms, readOnly);
     ...
     * defined.
     *
     * <p>Note: If statement identifiers are enabled and the context position is non-<code>null
     * </code> then it will be unified with the statement identifier assigned to that statement. It
     * is an error if the context position is a URI (since it can not be unified with the assigned
     * statement identifier). It is an error if the context position is a blank node which is
     * already bound to a term identifier whose value is different from the statement identifier
     * assigned/reported by the {@link #database}.
     *
     ...
     * @param statementStore When non-<code>null</code> the statements will be written on this
     *     store. When <code>null</code> the statements are written onto the <i>database</i>. (This
     *     is used to support incremental truth maintenance.)
     * @param stmts An array of statements in any order.
    ...
    private static final <S> long addStatements(
      ...
      for (int i = 0; i < tmp.length; i++) {
        ...
        if (DEBUG) log.debug("adding: " + stmt.toString() + " (" + spo + ")");
      ...
      final long nwritten =
          writeSPOs(database, statementStore, tmp.clone(), numStmts, didWriteCallback);
      ...
      ////                                "URI not permitted in context position when statement
      ...
      //                                                + c.getIV()
      //                                                + ")"
      //                                                + " in context position with statement
      // identifier="
      //                                                + sid + ": " + stmt + " (" + spo
      //                                                + ")");
      //
     ...
     *     store. When <code>null</code> the statements are written onto the <i>database</i>. (This
     *     is used to support incremental truth maintenance.)
     * @param stmts An array of the statements to be written onto the backing store.
     * @param numStmts The number of entries in that array to be written.
     * @return The #of statements written on the database.
     * @see AbstractTripleStore#addStatements(AbstractTripleStore, boolean, IChunkedOrderedIterator,
     *     IElementFilter)
     */
    private static <S> long writeSPOs(
        final AbstractTripleStore database,
        final AbstractTripleStore statementStore,
        final SPO[] stmts,
        final int numStmts,
      ...
      final long nwritten =
          database.addStatements(sink, false /* copyOnly */, itr, null /* filter */);
   ...
   *
   * @return True if the bufferQueue might overflow if another statement were added.
   */
  public boolean nearCapacity() {

    if (numStmts + 1 > bufferCapacity) return true;
   ...
   * @param _p The predicate.
   * @param _o The object.
   * @param _c The context (may be null).
   * @param type The statement type.
   * @throws IndexOutOfBoundsException if the buffer capacity is exceeded.
   * @see #nearCapacity()
   */
  protected void handleStatement(
      Resource _s, URI _p, Value _o, Resource _c, final StatementEnum type) {
    ...
    final EmbergraphResource s =
        (EmbergraphResource) getDistinctTerm(valueFactory.asValue(_s), true);
    final EmbergraphURI p = (EmbergraphURI) getDistinctTerm(valueFactory.asValue(_p), true);
    final EmbergraphValue o = getDistinctTerm(valueFactory.asValue(_o), true);
    final EmbergraphResource c =
        (EmbergraphResource) getDistinctTerm(valueFactory.asValue(_c), true);
    ...
    if (statementIdentifiers && s instanceof BNode) {
      ...
      if (equals(p, RDF_SUBJECT, RDF_PREDICATE, RDF_OBJECT)) {
        ...
        if (DEBUG) log.debug("reified piece: " + stmt);
