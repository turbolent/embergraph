 ...
 *
 * @todo In fact, RIO also keeps a blank node map so that it can reuse the same blank node object if
 *     it sees the same ID more than once.
 * @todo the {@link StatementBuffer} does not appear to correctly canonicalize terms when statement
 *     identifiers are enabled. Per below, this just needs to be rewritten. The code could be
 *     simplified dramatically. If the value is a BNode, then it goes into a map for canonicalizing
 *     blank nodes with a life cycle of the document being loaded. If a statement uses blank nodes
 *     then it must be deferred (this is true whether or not statement identifiers are in use) so do
 *     NOT make the {s,p,o} canonical since the statement and its terms will be processed later.
 *     Otherwise it goes into a canonicalizing Set (add iff not found and return, otherwise return
 *     the existing Value). The canonicalized value is used by the statement. An incremental write
 *     will cause all terms in the Value[] to be assigned term identifiers, so they should be
 *     EmbergraphValue objects. The statements now have term identifiers and they are written onto
 *     the DB. When the end of the document is reached, there will be deferred statements iff there
 *     were blank nodes. Those are then processed per the existing code. (If statement identifiers
 *     exist, then unify blank nodes with statment identifiers otherwise just assign term
 *     identifiers to blank nodes.) Note that the Value[] should be empty after each incremental
 *     write. If there are deferred statements, then they already have EmbergraphValue objects
 *     binding their term identifiers. When we process the deferred statements we should only be
 *     assigning term identifiers for blank nodes -- everything else should already have its term
 *     identifier assigned for the deferred statements.
 *     <p>TODO This class is nearly unused and mostly not tested. It is a write of the {@link
 *     StatementBuffer} class with some refactoring. It is only used by the {@link
 *     TripleStoreUtility} in one helper method.
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: AbstractStatementBuffer.java 6022 2012-02-13 17:55:15Z thompsonbry $
 * @param <F> The generic type of the source {@link Statement} added to the buffer by the callers.
 * @param <G> The generic type of the {@link EmbergraphStatement}s stored in the buffer.
 */
public abstract class AbstractStatementBuffer<F extends Statement, G extends EmbergraphStatement>
  ...
  /*
   * The database against which the {@link Value}s will be resolved (or added). If this database
   * supports statement identifiers, then statement identifiers for the converted statements will be
   * resolved (or added) to the lexicon.
   */
  private final AbstractTripleStore db;

  /*
   * When <code>true</code>, {@link Value}s will be resolved against the {@link LexiconRelation} and
   * {@link Statement}s will be resolved against the {@link SPORelation}, but unknown {@link Value}s
   * and unknown {@link Statement}s WILL NOT be inserted into the corresponding relations.
   */
  protected final boolean readOnly;

  /*
   * The maximum #of {@link EmbergraphStatement}s that can be buffered before the buffer {@link
   * #overflow()}s (from the ctor).
   */
  private final int capacity;

  /*
   * Buffer for canonical {@link EmbergraphValue}s. This buffer is cleared each time it overflows.
   */
  ...
  /*
   * Buffer for accepted {@link EmbergraphStatement}s. This buffer is cleared each time it would
   * overflow.
   */
   ...
   * from the current source. Blank nodes that can not be unified with a statement identifier once
   * {@link #flush()} is invoked are interpreted as normal blank nodes rather than statement
   * identifiers.
   *
   * <p>This map is used IFF statement identifers are enabled. When statement identifiers are NOT
   * enabled blank nodes are always blank nodes and we do not need to defer statements, only
   * maintain the canonicalizing {@link #bnodes} mapping.
   */
   ...
   */
  private final Map<Value, EmbergraphValue> distinctValues;
   ...
   * if you are loading a large document with a lot of blank nodes the map will also become large.
   *
   * <p>Note: This is allocated lazily so that we can implement {@link
   * IStatementBuffer#setBNodeMap(Map)}
   */
  ...
  /*
   * Note: Returns the same value as {@link #getDatabase()} since the distinction is not captured by
   * this class. This MUST be overriden in derived classes which make this distinction.
   */
  ...
  /** The {@link ValueFactory} for {@link Statement}s and {@link Value}s created by this class. */
  ...
  /*
   * @param db The database against which the {@link Value}s will be resolved (or added). If this
   *     database supports statement identifiers, then statement identifiers for the converted
   *     statements will be resolved (or added) to the lexicon.
   * @param readOnly When <code>true</code>, {@link Value}s (and statement identifiers iff enabled)
   *     will be resolved against the {@link LexiconRelation}, but entries WILL NOT be inserted into
   *     the {@link LexiconRelation} for unknown {@link Value}s (or for statement identifiers for
   *     unknown {@link Statement}s when statement identifiers are enabled).
   * @param capacity The capacity of the backing buffer.
   */
  @SuppressWarnings("unchecked")
  public AbstractStatementBuffer(
      final AbstractTripleStore db, final boolean readOnly, final int capacity) {
  ...
  public void setBNodeMap(final Map<String, EmbergraphBNode> bnodes) {
    ...
    if (this.bnodes != null) throw new IllegalStateException();

    this.bnodes = bnodes;
  }

  /*
   * Return a canonical {@link EmbergraphValue} instance representing the given <i>value</i>. The
   * scope of the canonical instance is until the next internal buffer overflow ({@link URI}s and
   * {@link Literal}s) or until {@link #flush()} ({@link BNode}s, since blank nodes are global for a
   * given source). The purpose of the canonicalizing mapping is to reduce the buffered {@link
   * EmbergraphValue}s to the minimum variety required to represent the buffered {@link
   * EmbergraphStatement}s, which improves throughput significantly (40%) when resolving terms to
   * the corresponding term identifiers using the {@link LexiconRelation}.
   *
   * <p>Note: This is not a true canonicalizing map when statement identifiers are used since values
   * used in deferred statements will be held over until the buffer is {@link #flush()}ed. This
   * relaxation of the canonicalizing mapping is not a problem since the purpose of the mapping is
   * to provide better throughput and nothign relies on a pure canonicalization of the {@link
   * Value}s.
   *
   * @param value A value.
   * @return The corresponding canonical {@link EmbergraphValue} for the target {@link
   *     EmbergraphValueFactory}. This will be <code>null</code> iff the <i>value</i> is <code>null
   *     </code> (allows for the context to be undefined).
   */
  ...
  public int size() {
    ...
    return nstmts + (deferredStatementBuffer == null ? 0 : deferredStatementBuffer.size());
  }
  ...
  /*
   * Imposes a canonical mapping on the subject, predicate, and objects of the given {@link
   * Statement}s and stores a new {@link EmbergraphStatement} instance in the internal buffer. If
   * the given statement is a {@link EmbergraphStatement} then its {@link StatementEnum} will be
   * used. Otherwise the new statement will be {@link StatementEnum#Explicit}.
   *
   * <p>Note: Unlike the {@link Value}s, a canonicalizing mapping is NOT imposed for the statements.
   * This is because, unlike the {@link Value}s, there tends to be little duplication in {@link
   * Statement}s when processing RDF.
   */
  @SuppressWarnings("unchecked")
        ...
        (G)
            getValueFactory()
                .createStatement(
                    ...
                    (e instanceof EmbergraphStatement
                        ? ((EmbergraphStatement) e).getStatementType()
  ...
  /*
   * Efficiently resolves/adds term identifiers for the buffered {@link EmbergraphValue}s.
   *
   * <p>If {@link #readOnly}), then the term identifier for unknown values will remain {@link
   * IRawTripleStore#NULL}.
   */
  protected void processBufferedValues() {

    if (INFO) log.info("nvalues=" + nvalues);

    db.getLexiconRelation().addTerms(valueBuffer, nvalues, readOnly);
  }

  /*
   * Processes any {@link EmbergraphStatement}s in the {@link #deferredStatementBuffer}, adding them
   * to the {@link #statementBuffer}, which may cause the latter to {@link #overflow()}.
   */
  protected void processDeferredStatements() {

    if (deferredStatementBuffer == null || deferredStatementBuffer.isEmpty()) return;
  ...
  /*
   * Invoked each time the {@link #statementBuffer} buffer would overflow. This method is
   * responsible for bulk resolving / adding the buffered {@link EmbergraphValue}s against the
   * {@link #db} and adding the fully resolved {@link EmbergraphStatement}s to the queue on which
   * the iterator is reading.
   */
  @SuppressWarnings("unchecked")
  protected final void overflow() {

    if (INFO) log.info("nvalues=" + nvalues + ", nstmts=" + nstmts);
   ...
   *
   * @param a An array of processed {@link EmbergraphStatement}s.
   * @return The delta that will be added to the {@link #counter} reported by {@link #flush()}.
   */
  protected abstract int handleProcessedStatements(G[] a);

  /*
   * Converts any buffered statements and any deferred statements and then invokes {@link
   * #overflow()} to flush anything remaining in the buffer.
   *
   * @return The total #of converted statements processed so far. (The counter is reset to zero as a
   *     side-effect.)
   */
  public long flush() {
  ...
  /*
   * Discards all state (term map, bnodes, deferred statements, the buffered statements, and the
   * counter whose value is reported by {@link #flush()}).
   */
  public void reset() {
    ...
    if (deferredStatementBuffer != null) deferredStatementBuffer.clear();
  ...
  /*
   * Clears the state associated with the {@link EmbergraphStatement}s in the internal buffer but
   * does not discard the blank nodes or deferred statements.
   */
  ...
  //     *            the buffer.
  //     */
  //    public static class StatementResolvingBuffer<F extends Statement, G extends
  // EmbergraphStatement>
  ...
  public static class StatementBuffer2<F extends Statement, G extends EmbergraphStatement>
    ...
    /*
     * @param lexiconStore The database against which the {@link Value}s will be resolved (or
     *     added). If this database supports statement identifiers, then statement identifiers for
     *     the converted statements will be resolved (or added) to the lexicon.
     * @param readOnly When <code>true</code>, {@link Value}s (and statement identifiers iff
     *     enabled) will be resolved against the {@link LexiconRelation}, but entries WILL NOT be
     *     inserted into the {@link LexiconRelation} for unknown {@link Value}s (or for statement
     *     identifiers for unknown {@link Statement}s when statement identifiers are enabled).
     * @param capacity The capacity of the backing buffer.
     */
    public StatementBuffer2(
        final AbstractTripleStore lexiconStore, final boolean readOnly, final int capacity) {
     ...
     *     added). If this database supports statement identifiers, then statement identifiers for
     *     the converted statements will be resolved (or added) to the lexicon.
     * @param statementStore The database against which the {@link Statement}s will be resolved (or
     *     added).
     * @param readOnly When <code>true</code>, {@link Value}s (and statement identifiers iff
     *     enabled) will be resolved against the {@link LexiconRelation}, but entries WILL NOT be
     *     inserted into the {@link LexiconRelation} for unknown {@link Value}s (or for statement
     *     identifiers for unknown {@link Statement}s when statement identifiers are enabled).
     *     Likewise, when <code>true</code>, {@link Statement}s will not be written onto the {@link
     *     SPORelation}.
     * @param capacity The capacity of the backing buffer.
     */
    public StatementBuffer2(
        final AbstractTripleStore lexiconStore,
    ...
    /*
     * @return The total #of statements written on the {@link #getStatementStore()}. This will be
     *     ZERO (0L) if <code>readOnly == true</code>.
     */
    public long flush() {
