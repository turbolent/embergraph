 ...
 *
 * <p>This parser has been modified to support the inline notation for statements about statements.
 *
 * <p>This parser has been modified to reuse the same {@link StringBuilder} in order to minimize
 * heap churn.
 *
 * <p>This parser has been modified to permit "-" and "_" in blank node IDs (they are not allowed in
 * that position for NTRIPLES). This was done to support a demonstration use case. That change could
 * (and should) be backed out. It is documented by FIXMEs in the code. One of the test files would
 ...
 * @openrdf
 */
public class EmbergraphNTriplesParser extends RDFParserBase {
  ...
  private Map<Statement, EmbergraphBNode> sids =
      new LinkedHashMap<Statement, EmbergraphBNode>() {
        /** */
        private static final long serialVersionUID = 1L;

        @Override
        protected boolean removeEldestEntry(final Map.Entry<Statement, EmbergraphBNode> eldest) {
...
//	 * Return a buffer of zero length and non-zero capacity. The same buffer is
   ...
   */
  public EmbergraphNTriplesParser(EmbergraphValueFactory valueFactory) {
    super(valueFactory);
  }
  ...
  protected EmbergraphValueFactory getValueFactory() {
    return (EmbergraphValueFactory) valueFactory;
  }
   ...
   * interface.
   *
   * @param in The InputStream from which to read the data, must not be <tt>null</tt>. The
   *     InputStream is supposed to contain 7-bit US-ASCII characters, as per the N-Triples
   *     specification.
  ...
  private int parseTriple(int c, final boolean embedded)
    ...
    if (c == -1) {
      ...
      // Embedded.
      if (c != '>') reportFatalError("Expected '>', found: " + (char) c);
      c = reader.read();
      if (c != '>') reportFatalError("Expected '>', found: " + (char) c);
    ...
    if (embedded) {
      // Create statement.
      EmbergraphStatement st =
          (EmbergraphStatement) createStatement(state.subject, state.predicate, state.object);

      // add the RDR statement inside the << >>.
      rdfHandler.handleStatement(st);

      state.lastSID = ((EmbergraphValueFactory) valueFactory).createBNode(st);

      //			// Resolve against LRU map to blank node for statement.
      //			EmbergraphBNode sid = sids.get(st);
      //			if (sid != null) {
      //				state.lastSID = sid;
      //			} else {
      //				/*
      ...
      //				state.lastSID = sid = (EmbergraphBNode) createBNode();
      ...
      //				((EmbergraphBNodeImpl) sid).setStatement(st);
      ...
      final Statement st = createStatement(state.subject, state.predicate, state.object);
      rdfHandler.handleStatement(st);
    }
   ...
   * another statement looks like this:
   *
   * <pre>
  ...
  private int parseLiteral(
      int c, final StringBuilder value, final StringBuilder lang, final StringBuilder datatype)
   ...
   * buffer to a {@link String} before the buffer is reused.
   *
   * @param capacityIsIgnored
   * @return
   */
