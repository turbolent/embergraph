 ...
 *
 * <ul>
 *   <li>Changed the visibility of the method parseValue() from private to protected so we could
 *       override it.
 *   <li>Pulled some code out of parseQueryResultInternal into its own method so that we can
 *       override it.
 * </ul>
 *
 * @author Peter Ansell
...
public abstract class SPARQLJSONParserBase extends QueryResultParserBase {
  ...
  protected boolean parseQueryResultInternal(
    ...
    while (jp.nextToken() != JsonToken.END_OBJECT) {
      ...
      if (baseStr.equals(HEAD)) {
        if (jp.nextToken() != JsonToken.START_OBJECT) {
          throw new QueryResultParseException(
              "Did not find object under " + baseStr + " field",
              jp.getCurrentLocation().getLineNr(),
              jp.getCurrentLocation().getColumnNr());
        }
        ...
        while (jp.nextToken() != JsonToken.END_OBJECT) {
          ...
          if (headStr.equals(VARS)) {
            ...
            if (jp.nextToken() != JsonToken.START_ARRAY) {
              throw new QueryResultParseException(
                  "Expected variable labels to be an array",
                  jp.getCurrentLocation().getLineNr(),
                  jp.getCurrentLocation().getColumnNr());
            }
          ...
          if (jp.getCurrentName().equals(BINDINGS)) {
            ...
            while (jp.nextToken() != JsonToken.END_ARRAY) {
              ...
              while (jp.nextToken() != JsonToken.END_OBJECT) {
                ...
                //									}
                //									else {
                //										throw new QueryResultParseException("Unexpected field name: " +
                // fieldName,
                //												jp.getCurrentLocation().getLineNr(),
                //												jp.getCurrentLocation().getColumnNr());
                //
