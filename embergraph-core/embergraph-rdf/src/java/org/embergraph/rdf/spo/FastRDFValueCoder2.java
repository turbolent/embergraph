...
import org.embergraph.btree.raba.codec.AbstractCodedRaba;
 ...
 * [null]. This is just the low nibble of the {@link StatementEnum#code()}. This "nibble" encoding
 * makes it fast and easy to extract the value from the coded record. The first value is stored in
 * the low nibble, the next in the high nibble, then it is on to the low nibble of the next byte.
 *
 * <p>Note: the 'override' flag is NOT stored in the statement indices, but it is passed by the
 * procedure that writes on the statement indices so that we can decide whether or not to override
 * the type when the statement is pre-existing in the index.
 *
 * <p>Note: this procedure can not be used if {@link
 * AbstractTripleStore.Options#STATEMENT_IDENTIFIERS} are enabled.
 *
 * @see StatementEnum
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @todo A mutable coded value raba could be implemented for the statement indices. With a fixed bit
 *     length per value, we can represent the data in m/2 bytes. This is also true for things like
 *     TERM2ID where the values could be represented as a long[].
 */
public class FastRDFValueCoder2 implements Externalizable, IRabaCoder {
  ...
  private static final transient int O_values = (1 /* version */ + Bytes.SIZEOF_INT /* size */);
  ...
  public ICodedRaba encodeLive(final IRaba raba, final DataOutputBuffer buf) {
    ...
    if (raba == null) throw new UnsupportedOperationException();
    ...
    final int initialCapacity = 1 + Bytes.SIZEOF_INT + BytesUtil.bitFlagByteLength(4 * n);
    ...
    for (int i = 0; i < size; i += 2) {
      ...
      final byte highNibble = (i + 1 == size ? 0 : ((raba.isNull(i + 1) ? 7 : raba.get(i + 1)[0])));
  ...
  private static class CodedRabaImpl extends AbstractCodedRaba {
     ...
     * @param size The size of the coded {@link IRaba}.
     */
    public CodedRabaImpl(final AbstractFixedByteArrayBuffer data, final int size) {
     ...
     * @throws IndexOutOfBoundsException unless the index is in [0:size-1].
     */
    protected final byte getBits(final int index) {
    ...
    public final int length(final int index) {
      ...
      if (isNull(index)) throw new NullPointerException();
