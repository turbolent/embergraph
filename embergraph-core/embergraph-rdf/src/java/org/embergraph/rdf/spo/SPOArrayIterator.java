 ...
 * @version $Id$
 * @deprecated by {@link ChunkedArrayIterator} but this still has some very purpose specific uses
 *     mainly dealing with {@link TruthMaintenance} and with fully buffering precisely because the
 *     DB impl is not using concurrency controls.
 */
public class SPOArrayIterator implements IChunkedOrderedIterator<ISPO> {
  ...
  /** The index of the next entry in {@link #stmts} that will be returned by {@link #next()}. */
  private int i = 0;
   ...
   * known.
   *
   * <p>Note: This constructor variant results in an iterator that does NOT support {@link
   * #remove()} and whose {@link #getKeyOrder()} method returns <code>null</code>.
   *
   ...
   *
   * <p>Note: This constructor is much lighter weight than the {@link SPOIterator} when you are
   * trying to do an existance test (limit of 1) or read only a few 100 {@link SPO}s.
   *
   * @param db The database (MAY be null, but then {@link #remove()} is not supported).
   * @param accessPath The access path (including the triple pattern).
   * @param limit When non-zero, this is the maximum #of {@link SPO}s that will be read. When zero,
   *     all {@link SPO}s for that access path will be read and buffered.
   * @param filter An optional filter. When non-<code>null</code>, only matching statements will be
   *     visited.
   */
  public SPOArrayIterator(
    ...
    final int n = (int) (limit > 0 ? Math.min(rangeCount, limit) : rangeCount);
    ...
    final IChunkedOrderedIterator<ISPO> itr = accessPath.iterator(0L, (long) n, 0 /* chunkSize */);
   ...
   * up a set of SPOs to be removed and then batching the remove requests on all indices in
   * parallel.
   *
   * @throws UnsupportedOperationException if a ctor variant was used that did not convey the
   *     database reference.
   */
  public void remove() {
