 ...
 * identifiers (based on their bit pattern).
 *
...
//	 * Denotes which bit to find the sidable flag within the {@link #flags}.
  ...
  @Override
  @SuppressWarnings("rawtypes")
  public final IV get(final int index) {
    switch (index) {
      case 0:
        return s;
      case 1:
   ...
   * appropriate {@link Justification}.
   *
   * @param s
   * @param p
   * @param o
   ...
   * @param type
   */
  public SPO(final IV s, final IV p, final IV o, final IV c, final StatementEnum type) {
   ...
   */
  public SPO(
      final IConstant<IV> s,
      final IConstant<IV> p,
      final IConstant<IV> o,
   ...
   *
   * <p>Note: This is NOT the same test as {@link EmbergraphStatementImpl#equals(Object)} since the
   * latter is implemented per the {@link Statement} interface.
   */
  public boolean equals(final ISPO stmt2) {

    if (stmt2 == this) return true;

    return IVUtility.equals(this.s, stmt2.s())
   ...
   *
   * @param store The store (optional). When non-<code>null</code> the store will be used to resolve
   *     term identifiers to terms.
  ...
  public String toString(final IRawTripleStore store) {
    ...
    if (store != null) {
      ...
      return t + (isModified() ? "(*)" : "") + " : " + store.toString(s, p, o, c);
  ...
  private void type(final StatementEnum type) {
    ...
    if (type == null) {
      b =
          Bits.set(
              Bits.set(Bits.set(b, TYPE_BIT, false), (TYPE_BIT + 1), false), (TYPE_BIT + 2), false);
    } else {
      switch (type) {
        case Explicit:
          b =
              Bits.set(
                  Bits.set(Bits.set(b, TYPE_BIT, true), (TYPE_BIT + 1), false),
                  (TYPE_BIT + 2),
                  false);
          break;
        case Axiom:
          b =
              Bits.set(
                  Bits.set(Bits.set(b, TYPE_BIT, false), (TYPE_BIT + 1), true),
                  (TYPE_BIT + 2),
                  false);
          break;
        case Inferred:
          b =
              Bits.set(
                  Bits.set(Bits.set(b, TYPE_BIT, true), (TYPE_BIT + 1), true),
                  (TYPE_BIT + 2),
                  false);
          break;
        case History:
          b =
              Bits.set(
                  Bits.set(Bits.set(b, TYPE_BIT, false), (TYPE_BIT + 1), false),
                  (TYPE_BIT + 2),
                  true);
          break;
        default:
  ...
  private ModifiedEnum modified() {
    ...
    switch (b) {
      case 0:
        return ModifiedEnum.NONE; // 00000
      case 8:
        return ModifiedEnum.INSERTED; // 01000
      case 16:
        return ModifiedEnum.REMOVED; // 10000
      case 24:
        return ModifiedEnum.UPDATED; // 11000
    }
...
//	 * Sidable is hiding in the 7 bit of the flags.
   ...
   * a Value if it is inline or if it has been materialized. If the IV is not-inline and its value
   * has not been materialized from the appropriate index or explicitly set through {@link
   * IVCache#setValue(EmbergraphValue)}, then an attempt to access the state of the {@link Value}
   * through its {@link Value} interface will throw a {@link NotMaterializedException}.
   */
  @Override
