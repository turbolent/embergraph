...
/*
 * Bulk completes the {@link StatementEnum} and/or statement identifier (SID) for {@link ISPO}s
 * using the given statement {@link IIndex}.
 *
  ...
  public ISPO[] convert(final ISPO[] chunk) {
    ...
    final ResultBufferHandler resultHandler =
        new ResultBufferHandler(
            keys.length, ndx.getIndexMetadata().getTupleSerializer().getLeafValuesCoder());
