...
public class SPOIndexWriter implements Callable<Long> {
   ...
   * @param a The {@link ISPO}[].
   * @param numStmts The #of elements in <i>a</i> to be written.
   * @param clone When true the statements are cloned.
   *     <p>Note:One of the {@link SPOIndexWriter}s gets to use the caller's array. The others MUST
   *     be instructed to clone the caller's array so that they can impose their distinct sort
   *     orders.
   * @param keyOrder Identifies the statement index on which to write.
   * @param filter An optional filter.
   * @param sortTime Incremented as a side-effect to report the elapsed time sorting the statements.
   * @param insertTime Incremented as a side-effect to report the elapsed time writing the
   *     statements on the <i>statementStore</i>.
   * @param numWritten Incremented as a side-effect to report the #of statements actually written on
   *     the SPO index (the counter is only incremented when writing on the SPO index to avoid
   *     double counting).
   * @param reportMutations When <code>true</code>, an indication will be reported for each
   *     statement whose state in the index was changed as a result of this operation.
   */
  public SPOIndexWriter(
      ...
      final AtomicLong insertTime,
      final AtomicLong numWritten,
      final boolean reportMutations) {

    if (spoRelation == null) throw new IllegalArgumentException();
   ...
   * or the value associated with the statement has been changed. This helps to keep down the IO
   * costs associated with index writes when the data are already in the index.
   *
  ...
  public Long call() throws Exception {
    ...
    for (int i = 0; i < numStmts; i++) {
      ...
      if (spo == null) throw new IllegalArgumentException("null @ index=" + i);
      ...
      if (reportMutation) denseStmts[numToAdd] = spo;
    ...
    if (reportMutation) {
      ...
      final ResultBitBufferHandler aggregator = new ResultBitBufferHandler(numToAdd, 2);
      ...
      ndx.submit(
          ...
          keys,
          vals,
          IndexWriteProcConstructor.REPORT_MUTATION,
          aggregator);
      ...
      ndx.submit(
          ...
          keys,
          vals,
          IndexWriteProcConstructor.INSTANCE,
          aggregator);
