 ...
 * index uses a permutation of those term identifiers, e.g., {s,p,o}, {o,s,p}, or {p,o,s}. The
 * {@link SPOKeyOrder} identifies the specific permutation for a given index. The keys are fully
 * decodable and are NOT stored redundantly in the tuple's value.
 *
 * <p>The tuple value encodes the {@link StatementEnum}, indicating whether the statement is
 * {explicit, inferred, or an axiom}, and optionally the unique statement identifier.
 *
 * <p>Note: While the static methods used to decode an existing key are safe for concurrent readers,
 * concurrent readers also form keys using {@link #statement2Key(IV, IV, IV)} and therefore require
 * a thread-local {@link IKeyBuilder}.
 *
...
public class SPOTupleSerializer extends DefaultTupleSerializer<SPO, SPO> {
  ...
  public byte[] serializeKey(final Object obj) {
    ...
    if (obj instanceof SPO) return serializeKey((SPO) obj);
   ...
   *
   * @param override <code>true</code> iff you want the {@link StatementEnum#MASK_OVERRIDE} bit set
   *     (this is only set when serializing values for a remote procedure that will write on the
   *     index, it is never set in the index itself).
   * @param userFlag <code>true</code> iff you want the {@link StatementEnum#MASK_USER_FLAG} bit
   *     set.
   * @param type The {@link StatementEnum}.
   * @return The value that would be written into a statement index for this {@link SPO}.
   */
  //    * @param buf
  ...
  public byte[] serializeVal( // final ByteArrayBuffer buf,
      final boolean override, final boolean userFlag, final StatementEnum type) {
  ...
  /** The current version. */
  private static final transient byte VERSION = VERSION1;
