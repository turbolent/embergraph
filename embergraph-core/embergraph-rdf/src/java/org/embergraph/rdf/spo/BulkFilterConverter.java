...
public class BulkFilterConverter implements IChunkConverter<ISPO, ISPO> {
  ...
  /*
   * @param ndx The SPO index.
   * @param present true if true, filter for statements that exist in the db, otherwise filter for
  ...
  public BulkFilterConverter(final IIndex ndx, final boolean present) {
    ...
    tupleSer = (SPOTupleSerializer) ndx.getIndexMetadata().getTupleSerializer();
  }
  ...
  public ISPO[] convert(final IChunkedOrderedIterator<ISPO> src) {
    ...
    if (chunkSize == 0) return new ISPO[] {};

    // size the return array correctly
    final ISPO[] filtered; // = new SPO[chunkSize];

    // dynamic type.
    filtered = (ISPO[]) java.lang.reflect.Array.newInstance(chunk[0].getClass(), chunkSize);
