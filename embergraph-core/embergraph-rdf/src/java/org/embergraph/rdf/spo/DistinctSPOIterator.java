 ...
 * @deprecated By a simple distinct filter and a filter which strips off the context position from
 *     an SPOC. Stack them together and it does the same thing. (The fall back to the B+Tree might
 *     still be interesting if we do not have a persistent hash map to fall back on instead and we
 *     want streaming results. Otherwise use an external merge sort.)
 */
public class DistinctSPOIterator implements ICloseableIterator<ISPO> {
  ...
  /** The next element to be visited or <code>null</code> if we need to scan ahead. */
  ...
  /** The #of distinct {@link ISPO}s read from the {@link #src} iterator so far. */
   ...
   *     smaller threshold, but the hash set is much faster (10x or better). Large memory JVMs might
   *     want to use an even larger threshold. [I've set this to MAX_VALUE since the BTree is a huge
   *     performance hit.]
   */
  static final int MAX_HASH_SET_CAPACITY = Integer.MAX_VALUE; // 100000;

  /** @param src The source iterator. */
  public DistinctSPOIterator(final SPORelation spoRelation, final ICloseableIterator<ISPO> src) {
  ...
  public boolean hasNext() {
    ...
    if (hashSet == null) {
      ...
      if (EmbergraphStatics.debug) System.err.println("Distinct SPO iterator overflow");
