 ...
 * @version $Id$
 * @todo Remove reliance on the {@link SPOPredicate}. It only adds the s(), p(), o(), and c()
 *     methods.
 */
public class SPOPredicate extends Predicate<ISPO> {
   ...
   * @param p
   * @param o
   * @param expander MAY be <code>null</code>.
   */
  @SuppressWarnings("rawtypes")
  public SPOPredicate(
      final String relationName,
...
//     * Constrain the predicate by setting the context position. If the context
  ...
  public SPOPredicate asBound(final IBindingSet bindingSet) {
    ...
    return (SPOPredicate) new SPOPredicate(argsCopy(), annotationsRef())._asBound(bindingSet);
