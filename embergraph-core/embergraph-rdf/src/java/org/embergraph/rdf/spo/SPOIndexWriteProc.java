 ...
 *
 * <p>The value for each statement is a byte that encodes the {@link StatementEnum} and also encodes
 * whether or not the "override" flag is set using - see {@link StatementEnum#MASK_OVERRIDE} -
 * followed by 8 bytes representing the statement identifier IFF statement identifiers are enabled
 * AND the {@link StatementEnum} is {@link StatementEnum#Explicit}. The value requires
 * interpretation to determine the byte[] that will be written as the value on the index - see the
 * code for more details.
 *
 * <p>Note: This needs to be a custom batch operation using a conditional insert so that we do not
 * write on the index when the data would not be changed and to handle the overflow flag and the
 * optional statement identifier correctly.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 */
public class SPOIndexWriteProc extends AbstractKeyArrayIndexProcedure<Object>
    implements IParallelizableIndexProcedure<Object>, IMutableRelationIndexWriteProcedure<Object> {
  ...
  public static class IndexWriteProcConstructor
     ...
     */
    public static IndexWriteProcConstructor INSTANCE =
        new IndexWriteProcConstructor(false /* reportMutation */);
    ...
    @Override
    public SPOIndexWriteProc newInstance(
        final IRabaCoder keySer,
        final IRabaCoder valSer,
        final int fromIndex,
        final int toIndex,
      ...
      return new SPOIndexWriteProc(keySer, valSer, fromIndex, toIndex, keys, vals, reportMutation);
    }
  }
  ...
  public Object applyOnce(final IIndex ndx, final IRaba keys, final IRaba vals) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (oldval == null) {
        ...
        if (reportMutation) modified[i] = ModifiedEnum.INSERTED;
        ...
        if (oldType == StatementEnum.History || newType == StatementEnum.History) {
          ...
          if (oldType != newType) {
            ...
            ndx.insert(key, tupleSer.serializeVal(false /* override */, userFlag, newType));

            if (isPrimaryIndex && DEBUG) {
              log.debug(
                  "Changing statement type: key="
            ...
            if (reportMutation)
              modified[i] =
                  newType == StatementEnum.History ? ModifiedEnum.REMOVED : ModifiedEnum.INSERTED;
          }
          ...
          if (oldType != newType) {
            ...
            if (reportMutation) modified[i] = ModifiedEnum.UPDATED;
          }

        } else {

          final StatementEnum maxType = StatementEnum.max(oldType, newType);
    ...
    if (isPrimaryIndex && INFO)
      log.info("Wrote " + writeCount + " SPOs on ndx=" + ndx.getIndexMetadata().getName());
