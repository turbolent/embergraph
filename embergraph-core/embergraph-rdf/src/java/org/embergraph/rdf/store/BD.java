...
public interface BD {
   ...
   * per-statement identifier. The value of the <code>embergraph:sid</code> attribute must conform
   * to the syntactic constraints of a blank node. By re-using the value of the <code>embergraph:sid
   * </code> attribute within other statements in the same RDF/XML document, the client can express
   * statements about the statements.
   *
   * <p>This RDF/XML extension allows us to inline provenance (statements about statements) with
   * RDF/XML to clients in a manner which has minor impact on unaware clients (they will perceive
   * additional statements whose predicate is <code>embergraph:sid</code>). In addition, clients
   * aware of this extension can submit RDF/XML with inline provenance.
   *
   * <p>For example:
   *
   * <pre>
   ...
   *
   * <p>The value will be one of
   *
   * <dl>
   *   <dt>Axiom
   *   <dd>The statement is an axiom that has not been explicitly asserted.
   *   <dt>Inferred
   *   <dd>The statement is an inference that has not been explicitly asserted.
   *   <dt>Explicit
   *   <dd>The statement has been explicitly asserted.
   * </dl>
   *
   * Note: If the knowledge base supports truth maintenance and an explicit statement is deleted
   * from the knowledge base but it remains provable as an inference or an axiom then then knowledge
   * base will continue to report it as either an axiom or an inference as appropriate.
   */
   ...
   * statements from the "null" graph as well as from any other context. {@link
   * org.embergraph.rdf.sail.EmbergraphSailConnection#getStatements(Resource, URI, Value, boolean,
   ...
   * <dl>
   *   <dt>foo(s,p,o)
   *   <dd>All named graphs are addressed. The <i>contexts</i> parameter will be a Resource[0]
   *       reference.
   *   <dt>foo(s,p,o,(Resource[]) null)
   *   <dd><em>Note: openrdf does not allow this invocation pattern - the Resource[] MUST NOT be a
   *       <code>null</code> reference.</em>
   *   <dt>foo(s,p,o,(Resource) null)
   *   <dd>The openrdf "nullGraph" is addressed. The <i>contexts</i> parameter will be
   *       Resource[]{null}. Java will autobox the Resource reference as a Resource[]{null} array.
   *   <dt>foo(s,p,o,x,y,z)
   *   <dd>The openrdf named graphs (x,y,z) are addressed. The <i>contexts</i> parameter will be
   *       Resource[]{x,y,z}.
   *   <dt>foo(s,p,o,x,null,z)
   *   <dd>The openrdf named graphs (x,nullGraph,z) are addressed. The <i>contexts</i> parameter
   *       will be Resource[]{x,null,z}
   * </dl>
   *
   ...
   * @see org.embergraph.rdf.sail.EmbergraphSailConnection#addStatement(Resource, URI, Value,
   *     Resource...)
   * @see org.embergraph.rdf.sail.EmbergraphSailConnection#getStatements(Resource, URI, Value,
   *     boolean, Resource...)
   * @see SESAME#NIL
   */
   ...
   * </pre>
   *
   * <p>Virtual graphs are addressed through a SPARQL syntax extension:
   *
   * <pre>
   * FROM VIRTUAL GRAPH
   ...
   * </pre>
   *
   * <p>If <code>:vg</code> as declared above is addressed using <code>FROM VIRTUAL GRAPH</code>
   * then its member graphs (:g1,:g2) are added to the default graph for the query.
   *
   * <p>If <code>:vg</code> as declared above is addressed using <code>FROM NAMED VIRTUAL GRAPH
   * </code> then its member graphs (:g1,:g2) are added to the named graphs for the query.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/493">Virtual Graphs (ticket)</a>
   * @see <a href="https://sourceforge.net/apps/mediawiki/bigdata/index.php?title=VirtualGraphs">
