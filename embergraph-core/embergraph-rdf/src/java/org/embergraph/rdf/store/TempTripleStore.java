 ...
 * a significant performance boost when you do not need transactions or the ability to purge
 * historical data versions from the store as they age.
 *
 * <p>Note: Users of the {@link TempTripleStore} may find it worthwhile to turn off a variety of
 * options in order to minimize the time and space burden of the temporary store depending on the
 * use which will be made of it, including {@link Options#LEXICON} and {@link
 * Options#ONE_ACCESS_PATH}.
 *
 * <p>Note: Multiple KBs MAY be created in the backing {@link TemporaryStore} but all MUST be
 * assigned the {@link TemporaryStore#getUUID()} as their prefix in order to avoid possible
 * conflicts within a global namespace. This is especially important when the relations in the
 * {@link TemporaryStore} are resolvable by an {@link IEmbergraphFederation} or {@link Journal}.
 *
 * <p>Note: This class is often used to support inference. When so used, the statement indices are
 * populated with the term identifiers from the main database and the {@link SPORelation} in the
 * {@link TempTripleStore} is disabled using {@link Options#LEXICON}.
 *
 * <p>Note: If you want an in-memory {@link ITripleStore} that supports commit and abort then use a
 * {@link LocalTripleStore} and specify {@link org.embergraph.journal.Options#BUFFER_MODE} as {@link
 * BufferMode#Temporary} or as {@link BufferMode#Transient} if you want the triple store to begin in
 * memory and overflow to disk if necessary. Both of these configurations provide full concurrency
 * control and group commit.
 *
...
public class TempTripleStore extends AbstractLocalTripleStore {
   ...
   * TemporaryStore} it is NOT safe to perform a {@link TemporaryStore#checkpoint()}. There is no
   * coordination of write access to the indices so the checkpoint can not be atomic. Therefore this
   ...
   * @deprecated by {@link TempTripleStore#TempTripleStore(TemporaryStore, Properties,
   *     AbstractTripleStore)} which permits you to reuse the same backing {@link TemporaryStore}
   *     instance until it becomes full.
   */
   ...
   *
   * <p>Note: This variant is especially useful when the {@link TempTripleStore} will have its own
   * lexicon and you need it to be compatible with the lexicon for the <i>db</i>.
   *
   * <p>Note: When <i>db</i> is non-<code>null</code>, the relations on the {@link TempTripleStore}
   * will be locatable by the specified <i>db</i>.
   *
   * @param properties Overrides for the database's properties.
   * @param db The optional database (a) will establish the defaults for the {@link
   *     TempTripleStore}; and (b) will be able to locate relations declared on the backing {@link
   *     TemporaryStore}.
   * @deprecated Use {@link #TempTripleStore(TemporaryStore, Properties, AbstractTripleStore)}
   *     instead and provide the {@link TemporaryStore} reference returned by {@link
   *     IIndexStore#getTempStore()}. This has the advantage of reusing a single shared {@link
   *     TemporaryStore} instance until it becomes "large" and then allocating a new instance (note
   *     that each instance will consume a direct {@link ByteBuffer} from the {@link
   *     DirectBufferPool}). This is especially important for operations like {@link
   *     TruthMaintenance} which have to create a lot of temporary stores.
   */
   ...
   *
   * @param store The {@link TemporaryStore}.
   * @param properties Overrides for the database's properties.
   * @param db The optional database (a) will establish the defaults for the {@link
   *     TempTripleStore}; and (b) will be able to locate relations declared on the backing {@link
   *     TemporaryStore}.
  ...
  public TempTripleStore(
      final TemporaryStore store, final Properties properties, final AbstractTripleStore db) {
   ...
   */
  private TempTripleStore(final TemporaryStore store, final Properties properties) {
   ...
   *
   * <p>Note: This also ensures that the {@link TempTripleStore} has a unique namespace name based
   * on the namespace of the main triple store instance plus the {@link UUID} of the {@link
   * TemporaryStore}. This addresses a problem where the {@link TempTripleStore} could otherwise be
   * located by the DefaultResourceLocator after an abort() had cleared the DefaultResourceLocator
   * cache of the unisolated view of the main triple store. See BLZG-2023, BLZG-2041.
   *
   * @param properties The properties for the {@link TempTripleStore}.
   * @param db The database from which we will obtain default properties.
   * @return The stacked properties.
   */
  private static Properties stackProperties(
      final Properties properties, final TemporaryStore tempStore, final AbstractTripleStore db) {
