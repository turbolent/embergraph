 ...
 * application MAY directly use this API to read and write explicit statements on the knowledge
 * base. However, if the knowledge base is to directly store any entailments, then the application
 * MUST NOT invoke operations on this API that add statements to, or remove statements from, the
 * knowledge base as the entailments will not be updated properly.
 *
 * <p>When entailments are stored in the knowledge base, a TM strategy MUST be used to made those
 * entailments based on the explicit triples asserted or retracted by the application. When an
 * application requests that statements are added to a knowledge base that maintains entailments,
 * the TM strategy MAY need to add additional entailments to the knowledge base. When an application
 * requests that statement(s) are removed from the knowledge base, the TM strategy needs to consider
 * the state of the knowledge base. In general, a statement should be removed IFF it was {@link
 * StatementEnum#Explicit} AND the statement is no longer entailed by the model theory and the
 * remaining statements in the knowledge base.
 *
 * @see IRawTripleStore
...
public interface ITripleStore {
   ...
   *     indices are being used, then this will be an upper bound.
   ...
   *     key-range scan of one of the statement indices.
   * @return The #of statements in the database. When <i>exact</i> is <code>false</code> and either
   *     transactions or key-range partitioned indices are being used, then this will be an upper
   *     bound.
   * @deprecated by {@link #getStatementCount(Resource, boolean)}
   */
   ...
   * @param exact When <code>true</code> the result will be an exact count, which may require a full
   *     key-range scan of one of the statement indices.
   * @return The #of statements in the named graph or in the database if <i>c</i> is <code>null
   *     </code>. When <i>exact</i> is <code>false</code> and either transactions or key-range
   *     partitioned indices are being used, then this will be an upper bound.
   */
   ...
   *
   * <p>Many RDF {@link Value}s are inlined into the statement indices. Inlined values are not be
   ...
   * <p>This may be an estimate when using partitioned indices.
   *
   * <p>This will always return ZERO (0) if {@link AbstractTripleStore.Options#STORE_BLANK_NODES} is
   * <code>false</code> since there will not be any blank nodes in the lexicon.
   *
   * <p>Many RDF {@link Value}s are inlined into the statement indices. Inlined values are not be
   * reported by this method.
   */
   ...
   *
   * <p>Note: The non-batch API is horridly inefficient and can not be used to co-reference blank
   * nodes. The batch load API for Sesame {@link Value} objects is:
   *
   * <pre>
   *
   ...
   * </pre>
   *
   * @param s The subject (required).
   * @param p The predicate (required).
   * @param o The object (required).
   * @param c The context (required IFF a quad store and otherwise ignored).
   */
   ...
   * @param c The context (optional and ignored unless a quad store).
   */
   ...
   * then you MUST either {@link EmbergraphValue#clearInternalValue()} or create a new {@link Value}
   * object which either is not aware of the term identifier or does not have its term identifier
   * set in order to avoid lookup using the term identifier rather than indirecting through the
   * lexicon.
   *
   * @param s The subject (required).
   * @param p The predicate (required).
   * @param o The object (required).
   * @param c The context (required iff a quad store and otherwise ignored).
   * @return The statement -or- <code>null</code> iff the triple is not defined in the database.
   * @see #asValue(Value)
   */
   ...
   * @return The iterator.
   */
  EmbergraphStatementIterator getStatements(Resource s, URI p, Value o, Resource c);
   ...
   *     </code>.
   * @return A corresponding Sesame {@link Value} object -or- <code>null</code> iff <i>value</i> is
   *     <code>null</code>.
   */
   ...
   * @param o The object (optional).
   * @param c The context (optional).
   * @return The object that may be used to read efficiently on the indices for that triple pattern.
   *     In the special case where any of the given {@link Value}s is <strong>not known</strong> to
   *     the database this method will return an {@link EmptyAccessPath}.
   * @see IAccessPath
   ...
   *
   * @param src An {@link IChunkedOrderedIterator} visiting {@link SPO}s
   ...
   * configure the database.
   *
   * <p>Note: The first time this object is requested it will attempt to write the axioms on the
   * database.
   *
   * @see Options
   */
   ...
   * the write set), temporary (ignored since the store is not restart safe), or a federation
   * (ignored since unisolated writes on the federation are atomic and auto-committed).
   */
  void abort();
   ...
   * commit), temporary (ignored), or a federation (ignored since unisolated writes on the
