...
public class EmbergraphStatementIteratorImpl
    extends AbstractChunkedResolverator<ISPO, EmbergraphStatement, AbstractTripleStore>
   ...
   */
  public EmbergraphStatementIteratorImpl(
      final AbstractTripleStore db, final IChunkedOrderedIterator<ISPO> src) {
   ...
   *     to blank node objects across a "connection" context.
   * @param src The source iterator (will be closed when this iterator is closed).
   */
  public EmbergraphStatementIteratorImpl(
  ...
  protected EmbergraphStatement[] resolveChunk(final ISPO[] chunk) {
    ...
    for (ISPO spo : chunk) {
...

      {
        ...
        if (c != null) handleIV(c, ivs);
      }
    }

    if (log.isDebugEnabled()) log.debug("Resolving " + ivs.size() + " term identifiers");
     ...
     * this chunk.
     */
    final Map<IV<?, ?>, EmbergraphValue> terms = state.getLexiconRelation().getTerms(ivs);
    ...
    for (ISPO spo : chunk) {
      ...
      final EmbergraphStatement stmt =
          valueFactory.createStatement(s, p, o, c, spo.getStatementType(), spo.getUserFlag());
   ...
   */
  private void handleIV(final IV<?, ?> iv, final Collection<IV<?, ?>> ids) {
   ...
   * it is defined.
   *
   * @param terms The terms mapping obtained from the lexicon.
   * @param iv The term identifier.
   * @return The {@link EmbergraphValue}.
   */
  private EmbergraphValue resolve(final Map<IV<?, ?>, EmbergraphValue> terms, final IV<?, ?> iv) {
