 ...
 *     triple patterns </a>
 */
public class EmbergraphTriplePatternMaterializer
    extends AbstractChunkedResolverator<EmbergraphTriplePattern, ISPO, AbstractTripleStore>
    implements ICloseableIterator<ISPO> {
  //        implements IChunkedOrderedIterator<ISPO> {

  private final int nthreads;

  public EmbergraphTriplePatternMaterializer(
      final AbstractTripleStore db, final IChunkedOrderedIterator<EmbergraphTriplePattern> src) {
    this(db, src, 4 /* nthreads */);
  }

  public EmbergraphTriplePatternMaterializer(
      final AbstractTripleStore db,
      final IChunkedOrderedIterator<EmbergraphTriplePattern> src,
      final int nthreads) {
  ...
  @Override
  public EmbergraphTriplePatternMaterializer start(final ExecutorService service) {
  ...
  protected ISPO[] resolveChunk(final EmbergraphTriplePattern[] chunk) {
    ...
    try {
      ...
      for (FutureTask<Long> ft : tasks) {
        nfound += ft.get();
        if (nfound > Integer.MAX_VALUE) throw new UnsupportedOperationException();
      }
      ...
      while ((src = out.poll()) != null) {
        System.arraycopy(src /* src */, 0 /* srcPos */, dest, destPos, src.length);
        destPos += src.length;
      }
      ...
      for (FutureTask<Long> ft : tasks) ft.cancel(true /* mayInterruptIfRunning */);
    }
  }
  ...
  private class ResolveTriplePatternTask implements Callable<Long> {
    ...
    private final EmbergraphTriplePattern stmt;
    private final Queue<ISPO[]> out;

    public ResolveTriplePatternTask(final EmbergraphTriplePattern stmt, final Queue<ISPO[]> out) {
      this.stmt = stmt;
      this.out = out;
    }
