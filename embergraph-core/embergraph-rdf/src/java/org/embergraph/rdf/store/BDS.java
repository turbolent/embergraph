 ...
 * @see <a href="http://sourceforge.net/apps/mediawiki/bigdata/index.php?title=FullTextSearch">Free
 *     Text Index </a>
...
public interface BDS {
   ...
   * </pre>
   *
   * The value MUST be bound and MUST be a literal. The <code>languageCode</code> attribute is
   * permitted. When specified, the <code>languageCode</code> attribute will be used to determine
   * how the literal is tokenized - it does not filter for matches marked with that <code>
   * languageCode</code> attribute. The <code>datatype</code> attribute is not allowed.
   *
   * <p>The subject MUST NOT be bound.
   *
   * <p>This expression will evaluate to a set of bindings for the subject position corresponding to
   * the indexed literals matching any of the terms obtained when the literal was tokenized.
   *
   ...
   * </pre>
   *
   * Relevance is the cosine of the angle between the query vector (search terms) and the document
   * vector (terms in the indexed literals). The minimum relevance is ZERO (0.0). The maximum
   * relevance is ONE (1.0).
   *
   * @see #MIN_RELEVANCE
   * @see #MAX_RELEVANCE
   */
   ...
   * #MIN_RANK} and {@link #MAX_RANK} may be used to "slice" the full text index search results. Use
   * this query hint conjunction with {@link #SEARCH} as follows:
   *
   * <p>
   *
   * <pre>
   ...
   * </pre>
   *
   * You can use {@link #MIN_RANK} and {@link #MAX_RANK} together to page through the search
   * results. This is often key to achieving low latency graph search. By limiting the number of
   * results that are fed into the remained of the SPARQL query, you can ensure that the SPARQL
   * query runs quickly. If you do not get enough results from the SPARQL query, you can feed the
   ...
   * </pre>
   *
   * The relevance scores are in [0.0:1.0], where 0.0 is the minimum possible relevance and 1.0 is
   * the maximum possible relevance. You should NOT specify a minimum relevance of ZERO (0.0) as
   ...
   *
   * <p>Magic predicate used to query for free text search metadata to turn on prefix matching.
   * Prefix matching will match all full text index tokens that begin with the specified token(s)
   * (default {@value #DEFAULT_PREFIX_MATCH}). Use in conjunction with {@link #SEARCH} as follows:
   *
   * <p>
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>This will turn on prefix matching.
   *
   ...
   * </pre>
   *
   * <p>The subject-centric search index must be enabled via {@link
