...
public class TripleStoreUtility {
   ...
   *
   * <p>Note: Comparison is performed in terms of the externalized RDF {@link Statement}s rather
   * than {@link SPO}s since different graphs use different lexicons.
   *
   * <p>Note: If the graphs differ in which entailments they are storing in their data and which
   * entailments are backchained then you MUST make them consistent in this regard. You can do this
   * by exporting one or both using {@link #bulkExport(AbstractTripleStore)}, which will cause all
   * entailments to be materialized in the returned {@link TempTripleStore}.
   *
   * @param expected One graph.
   * @param actual Another graph <strong>with a consistent policy for forward and backchained
   *     entailments</strong>.
  ...
  public static ICloseableIterator<EmbergraphStatement> notFoundInTarget(
     ...
     * statements).
     */
    final EmbergraphStatementIterator itr2 =
        expected.asStatementIterator(expectedAccessPath.iterator());
    ...
    final StatementBuffer2<Statement, EmbergraphStatement> sb =
       ...
       * Statements not found in [actual] are written on the BlockingBuffer.
          ...
          protected int handleProcessedStatements(final EmbergraphStatement[] a) {
            ...
            if (log.isInfoEnabled()) log.info("Given " + a.length + " statements");

            // bulk filter for statements not present in [actual].
            final IChunkedOrderedIterator<ISPO> notFoundItr =
                actual.bulkFilterStatements(a, a.length, false /* present */);
            ...
            try {
              ...
              while (notFoundItr.hasNext()) {
                ...
                if (log.isInfoEnabled()) log.info("Not found: " + notFoundStmt);
            ...
            if (log.isInfoEnabled())
              log.info(
                  "Given " + a.length + " statements, " + nnotFound + " of them were not found");
   ...
   *
   * <p>Note: This is not a general purpose bulk export as it uses only a single access path, does
   * not store justifications, and does retain the {@link Axioms} model of the source graph. This
   * method is specifically designed to export "just the triples", e.g., for purposes of comparison.
   *
   * @param db The source database.
   * @return The {@link TempTripleStore}.
   */
  public static TempTripleStore bulkExport(final AbstractTripleStore db) {
    ...
    properties.setProperty(Options.STATEMENT_IDENTIFIERS, "" + db.isStatementIdentifiers());
    ...
    try {
      ...
      final IChunkedOrderedIterator<ISPO> itr1 =
          new BackchainAccessPath(db, db.getAccessPath(NULL, NULL, NULL)).iterator();
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (!file1.exists()) throw new FileNotFoundException(file1.toString());

    if (!file2.exists()) throw new FileNotFoundException(file2.toString());
    ...
    try {
      ...
      final AbstractTripleStore ts1;
      {
        ...
        properties.setProperty(org.embergraph.journal.Options.FILE, file1.toString());

        j1 = new Journal(properties);

        ts1 =
            (AbstractTripleStore)
                j1.getResourceLocator()
                    .locate(namespace, TimestampUtility.asHistoricalRead(j1.getLastCommitTime()));
      }
      ...
      final AbstractTripleStore ts2;
      {
        ...
        properties.setProperty(org.embergraph.journal.Options.FILE, file2.toString());

        j2 = new Journal(properties);

        ts2 =
            (AbstractTripleStore)
                j2.getResourceLocator()
                    .locate(namespace, TimestampUtility.asHistoricalRead(j2.getLastCommitTime()));
      }
