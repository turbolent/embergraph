 ...
 * {@link BTree} class (writers are single threaded, reads may be concurrent, but not concurrent
 * with a writer) and by using different views (unisolated vs read-historical) of the indices when
 * computing entailments or performing high-level query.
 *
...
public class LocalTripleStore extends AbstractLocalTripleStore {
  ...
  public synchronized long commit() {
    ...
    if (log.isInfoEnabled()) log.info("commit: commit latency=" + elapsed + "ms");
    ...
    try {
      ...
      // Check for pre-existing instance.
      {
        final LocalTripleStore lts =
            (LocalTripleStore) journal.getResourceLocator().locate(namespace, ITx.UNISOLATED);
