 ...
 * @version $Id$
 * @todo consider refactor making this the primary interface so that we can realize non-Sesame
 *     integrations by layering an appropriate interface over this one.
 */
public interface IRawTripleStore extends ITripleStore {
  ...
  /*
   * Add a term into the term:id index and the id:term index, returning the assigned term identifier
   * (non-batch API).
   *
   * <p>Note: This method delegates to the batch API, but it is extremely inefficient for scale-out
   * as it does one RMI per request!
   *
   ...
   *
   * @param value Any {@link Value} reference (MAY be <code>null</code>).
   ...
   *
   * @param s The internal value for the subject -or- null.
   * @param p The internal value for the predicate -or- null.
   ...
   * pattern. This is generally used only when you want to perform a {@link
   * IAccessPath#distinctTermScan()}.
   *
   ...
   *
   * <p>Note: This may be used to examine the {@link StatementEnum}.
   *
   * @param s The internal value ({@link IV}) for the subject.
   * @param p The internal value ({@link IV}) for the predicate.
   * @param o The internal value ({@link IV}) for the object.
   * @param c The internal value ({@link IV}) for the context (required for quads and ignored for
   *     triples).
   * @return The {@link SPO} for that statement, including its {@link StatementEnum} -or- <code>null
   *     </code> iff the statement is not in the database.
   * @exception IllegalArgumentException if the s, p, or o is null.
   ...
   *
   * @param stmts The statements (sorted into {@link IKeyOrder#SPO} order as a side-effect).
   * @param numStmts The #of entries in <i>stmts</i> that are valid.
   * @return The #of statements that were written on the indices (a statement that was previously an
   ...
   * @param numStmts The #of entries in <i>stmts</i> that are valid.
   * @param filter Optional statement filter. Statements matching the filter are NOT added to the
   *     database.
   * @return The #of statements that were written on the indices (a statement that was previously an
   *     axiom or inferred and that is converted to an explicit statement by this method will be
   *     reported in this count as well as any statement that was not pre-existing in the database).
   ...
   * Writes the statements onto the statement indices (batch, parallel, NO truth maintenance).
   *
   * @param itr An iterator visiting the statements to be added.
   * @param filter Optional statement filter. Statements matching the filter are NOT added to the
   *     database. The iterator is closed by this operation.
   * @return The #of statements that were written on the indices (a statement that was previously an
   *     axiom or inferred and that is converted to an explicit statement by this method will be
   *     reported in this count as well as any statement that was not pre-existing in the database).
   ...
   *
   * <p>Note: The {@link StatementEnum} on the {@link SPO}s is ignored by this method. It will
   * delete all statements having the same bindings regardless of whether they are inferred,
   * explicit, or axioms.
   *
   ...
   * in the database, depending on the value of the supplied boolean variable (batch API).
   *
   * @param stmts the statements to test
   * @param numStmts the number of statements to test
   * @param present if true, filter for statements that exist in the db, otherwise filter for
   ...
   * "not present" in the database, depending on the value of the supplied boolean variable (batch
   * api).
   *
   * @param itr an iterator over the set of statements to test
   * @param present if true, filter for statements that exist in the db, otherwise filter for
   *     statements that do not exist
   * @return an iteration over the filtered set of statements
   */
  ...
  /*
   * This method fills out the statement metadata (type and sid) for {@link ISPO}s that are present
   * in the database. {@link ISPO}s not present in the database are left as-is.
   *
