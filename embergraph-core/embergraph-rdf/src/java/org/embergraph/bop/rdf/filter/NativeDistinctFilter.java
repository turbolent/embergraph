...
import org.embergraph.bop.BTreeAnnotations;
 ...
 * the distinct filter is being run. For this reason, it is allocating a private {@link MemStore}
 * and using a finalizer pattern to ensure the eventual release of that {@link MemStore} and the
 * backing direct buffers.
 *
 * <p>Note: This can not be used with pipelined joins because it would allocate one instance per
 * as-bound evaluation of the pipeline join.
 *
 * <p>Note: You can change the code over the HTree/BTree by modifying only a few lines. See the
 * comments in the file.
 *
 * <p>TODO Reads against the index will eventually degrade since we can not use ordered reads
 * because the iterator filter pattern itself is not vectored. We might be able to fix this with a
 * chunked filter pattern. Otherwise fixing this will require a more significant refactor.
 *
 * <p>TODO It would be nicer if we left the MRU 10k in the map and evicted the LRU 10k each time the
 * map reached 20k. This can not be done with the {@link LinkedHashMap} as its API is not sufficient
 * for this purpose. However, similar batch LRU update classes have been defined in the <code>
 * org.embergraph.cache</code> package and could be adapted here for that purpose.
 *
...
public class NativeDistinctFilter extends BOpFilterBase {
  ...
  public interface Annotations
      extends org.embergraph.bop.ap.filter.BOpFilter.Annotations,
          BTreeAnnotations,
          HashMapAnnotations {
   ...
   *     This is used to decide on the filter key order which will have the best locality given the
   *     order of arrival.
   */
  ...
  public NativeDistinctFilter(final BOp[] args, final Map<String, Object> annotations) {
   ...
   * of the order in which the SPOs are arriving.
   *
   * <p>The return valuer is an <code>int[3]</code>. The index is the ordinal position of the
   * triples mode key component for the filter keys. The value at that index is the position in the
   * {@link SPOKeyOrder} of the quads mode index whose natural order determines the order of arrival
   * of the {@link ISPO} objects at this filter.
   *
   * <p>Thus, given indexKeyOrder = {@link SPOKeyOrder#CSPO}, the array:
   *
   * <pre>
   * int[] = {1,2,3}
   * </pre>
   *
   * would correspond to the filter key order SPO, which is the best possible filter key order for
   * the natural order order of the {@link SPOKeyOrder#CSPO} index.
   *
   * <p>Note, however, that key orders can be expressed in this manner which are not defined by
   * {@link SPOKeyOrder}. For example, given {@link SPOKeyOrder#PCSO} the best filter key order is
   * <code>PSO</code>. While there is no <code>PSO</code> key order declared by the {@link
   * SPOKeyOrder} class, we can use
   *
   * <pre>
   * int[] = {0,2,3}
   * </pre>
   *
   * which models the <code>PSO</code> key order for the purposes of this class.
   *
   * <p>Note: This method now accepts triples in support of the {@link ASTConstructIterator}
   *
  ...
  public static int[] getFilterKeyOrder(final SPOKeyOrder indexKeyOrder) {
    ...
    if (indexKeyOrder == null) throw new IllegalArgumentException();
  ...
  public static class DistinctFilterImpl extends Filter implements ICloseable {
     ...
     *
     * <p>Note: Maybe either a {@link BTree} or an {@link HTree}. The code has paths for both.
     */
     ...
     * HTree}.
     *
     * <p>Note: Historical testing indicated that the {@link BTree} was faster for this application.
     *
     * <p>TODO Edit HTree/BTree here.
     */
     ...
     * pro-actively whenever possible.
     *
     * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/582">IStriterator does not
     *     support close() protocol for Ifilter </a>
     */
    @Override
    public void close() {
    ...
    private static <T> T getRequiredProperty(final IPropertySet pset, final String name) {
      ...
      if (val == null)
        if (val == null)
          throw new IllegalStateException(
              "Required property: " + name + " : " + NativeDistinctFilter.class);
    ...
    public DistinctFilterImpl(final IPropertySet properties) {
      ...
      final int initialCapacity =
          getProperty(
              properties, Annotations.INITIAL_CAPACITY, Annotations.DEFAULT_INITIAL_CAPACITY);
       ...
       */
      {
        ...
        metadata.setRawRecords(
            getProperty(properties, Annotations.RAW_RECORDS, Annotations.DEFAULT_RAW_RECORDS));
    ...
    private void evictAll() {
      ...
      final byte[][] a = new byte[n][];
      {
        ...
        final Iterator<Map.Entry<ISPO, byte[]>> itr = lru.entrySet().iterator();
        int i = 0;
        while (itr.hasNext()) {
     ...
     * scales to very large data sets. The {@link #lru} is checked first. If the object is not
     * found, then the persistence capable index is checked. If the object is not found there
     * either, then object will be added the {@link #lru}. Objects inserted into the {@link #lru}
     * are eventually batched through to the backing index.
     *
     * @param spo The {@link SPO}.
