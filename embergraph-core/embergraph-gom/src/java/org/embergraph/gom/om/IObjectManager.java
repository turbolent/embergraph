...
public interface IObjectManager extends INativeTransaction {
   ...
   * of the iterator will not cause them to be retained any longer than they otherwise would have
   * been retained.
   *
   * <p>TODO We could do one-step look ahead and buffer a hard reference to the next {@link IGPO} to
   * be visited. That would allow us to eliminate the {@link WeakReference} from this method
   * signature.
   */
   ...
   *
   * @exception IllegalStateException if the store is already closed.
   */
  void close();

  /** Return <code>true</code> iff the object manager is backed by some persistence layer. */
