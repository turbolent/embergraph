...
package org.embergraph.gom.skin;
 ...
 *
 * <p>Note: Registered {@link IGenericSkin} implementations that are to be instantiated for a
 * backing {@link IGPO} MUST provide a public constructor accepting a single {@link IGenericSkin}
 * argument.
 *
 * <p>Note: Registered {@link ILinkSetSkin} implementations that are to be instantiated for a
 * backing {@link ILinkSet} MUST provide a public constructor accepting a single {@link
 * ILinkSetSkin} argument.
 */
public class GenericSkinRegistry {
   ...
   * {@link ILinkSet} by naming the interface (or implementation class) that you want to expose for
   * the persistent data.
   *
   * <p>
   *
   * @param theClass The implementation class.
   * @todo Consider that more than one implementation class for the same interface may be
   *     registered. This should at LEAST generate a warning since only the first such
   *     implementation class for an interface will be used.
   * @exception IllegalArgumentException If <i>theClass </i> does not implement either {@link
   *     IGenericSkin} or {@link ILinkSetSkin}.
   */
  public static void registerClass(Class theClass) {
    ...
    if (theClass.isInterface()) {
      ...
      throw new IllegalArgumentException(
          "This is an interface, not an implementation class: " + theClass.getName());
    }
    ...
    //	    ! ILinkSetSkin.class.isAssignableFrom( theClass )
      ...
      throw new IllegalArgumentException(
          "Class MUST implement: " + IGenericSkin.class.getName()
          //		  +" -or- "+
   ...
   * @exception UnsupportedOperationException if there is no registered skin identified by the
   *     specified class or interface.
   * @exception IllegalArgumentException if <i>theClassOrInterface</i> is <code>null</code>.
   */
    ...
    if (theClassOrInterface.isInterface()) {
      ...
      if (theImplementationClass == null) {
        ...
        throw new UnsupportedOperationException(
            "No implementation class has been registered for that interface: "
                + theClassOrInterface.getName());
      }
      ...
      throw new UnsupportedOperationException(
          "No registered class matches or is assignable from the parameter: "
              + theClassOrInterface.getName());
    }
   ...
   * <p>
   *
   * @return A {@link IGenericSkin} for <i>g</i> that is an instance of the desired class or that
   *     implements the desired interface.
   * @exception UnsupportedOperationException if a suitable {@link IGenericSkin} did not exist and
   *     could not be minted. Typically this means that no suitable implementation class has been
   *     registered, or that the class does not implement a suitable constructor.
   */
  public static IGenericSkin asClass(IGPO g, Class theClassOrInterface)
    ...
    Class theImplementationClass = getImplementationClass(theClassOrInterface);
