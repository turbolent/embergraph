 ...
 * properties. In addition, various methods that generate {@link Iterator}s and the like are
 * typically re-written in terms of link sets.
 *
 * <p>This interface SHOULD be implemented so that the backing {@link IGPO} implementation object
 * may be recovered. Only a single method, {@link #asGeneric()} is required by this interface in
 * order to minimize the possibity of interface collisions when using a generic object model to
 * implement other interfaces, e.g., the W3C DOM or some such.
 *
 * <p>Note: This interface is extended by {@link IGPO}. This makes it easy to write common code for
 * {@link IGPO}s and {@link IGenericSkin}s.
 *
 * <p>
 *
...
public interface IGenericSkin {
   ...
   * {@link IGPO} behavior (vs delegating the interface to an implementation object), then that
   * object is returned. The return value is never <code>null</code>.
   *
   * <p>
   */
  IGPO asGeneric();
