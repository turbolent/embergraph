 ...
 *     <p>FIXME We should materialize the link set unless the cardinality is excessive. This applies
 *     in both the forward and reverse direction. We will need a means to obtain a "DESCRIBE" of a
 *     resource (complete set of attributes, forward, and reverse links) and a "SKETCH" of a
 *     resource (cardinality counts only when a forward or reverse link set is large and otherwise
 *     they are inlined as well into the sketch).
 */
public class LinkSet implements ILinkSet {
  ...
  public LinkSet(final IGPO owner, final URI linkProperty, final boolean linksIn) {
    ...
    if (linkProperty == null) throw new IllegalArgumentException();
    m_owner = owner;
  ...
  public boolean contains(Object arg) {
    ...
    while (values.hasNext()) {
      if (values.next() == gpo) return true;
    }
    return false;
  }
    ...
    if (false) {
      ...
      final String query =
          "SELECT ?x\n"
              + "WHERE {\n"
              + "  ?x <"
              + encode(m_linkProperty)
              + "> <"
              + encode(m_owner.getId())
              + ">\n"
              + "}";
      ...
      return new Iterator<IGPO>() {
        ...
        @Override
        public IGPO next() {
          if (nextGPO == null) throw new NoSuchElementException();
