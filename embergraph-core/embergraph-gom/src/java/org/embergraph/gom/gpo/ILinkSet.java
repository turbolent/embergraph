 ...
 * resource rather than many links from one resource pointing to many other resources).
 *
 * <p>The LinksOut as the low-cardinality part of a many-many association reflect common asymmetry
 * of such associations. Experience suggests that if the association is more symmetric the
 * cardinalities tend not to be large.
 */
public interface ILinkSet extends Set<IGPO> {
   ...
   *
   * @exception UnsupportedOperationException if <i>theClassOrInterface</i> is not a registered
   *     {@link IGenericSkin}.
   */
