 ...
 * a {@link ILinkSet link set}. Associations and link sets provide an uniform, efficient and
 * scalable API for object navigation. Link sets may be {@link ILinkSetIndex indexed }for efficient
 * search.
 *
 * <p>Extensible behavior is realized by wrapping up a generic object within some application
 * specific interface. Generic object implementations should be considered as <code>final</code>.
 * Specific backends may support subclassing but use of subclassing will lock an application into a
 * specific GOM platform. Instead applications layer additional behavior onto generic objects using
 * "skins" - see {@link #asClass( Class )}. A skin delegates the management of persistent state to a
 * backing generic object. The use of skins provides transparent migration of application code
 * across different GOM implementations.
 *
 * <p>The generic object model provides transparent and scalable persistence when using an
 * appropriate backing store. The backing store is selected and configured using the {@link
...
public interface IGPO extends IGenericSkin // @todo , INativeTransaction?
{
   ...
   * the scope of the {@link IObjectManager}).
   */
  ...
  /** Return the {@link IObjectManager} used to materialize or create this generic object. */
   ...
   *
   * <p>While this clears all link sets held by the generic object, the members of those link sets
   * are NOT explicitly removed since they may participate in other associations.
   *
   * <p>The {@link IGPO} interface may no longer be used for this object if this operation is
   * successful.
   *
   * <p>
   */
   ...
   * interface, then it is simply returned. If a suitable {@link IGenericSkin} has already been
   * minted for this generic object, then it is simply returned. Otherwise an implementation class
   * for the interface MUST have been registered with the runtime and MUST implement a constructor
   * accepting a single {@link IGenericSkin} argument.
   *
   * <p>The life cycle of a skin object returned by this method MUST be no less than the in-memory
   * life cycle of the generic object for which the skin was minted. This constraint makes it
   * possible to have transient yet expensive initialization, such as caches, on skin objects.
   *
   * <p>
   *
   * @exception UnsupportedOperationException If the runtime is unable to identify an implementation
   *     class for the specified interface.
   * @return An object that wraps the same persistent data but implements the desired interface.
   * @see IGenericSkin
