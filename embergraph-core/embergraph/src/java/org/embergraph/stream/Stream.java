 ...
 *     <p>FIXME GIST : The base Stream should handle byte[]s, much like the basic BTree or HTree.
 *     That way it can be a concrete class and used for a variety of things. It might also allow an
 *     append() method (similar to insert) for non-iterator based incremental write protocols. This
 *     would be really just a thin wrapper over the {@link IPSOutputStream}, but integrated with the
...
public abstract class Stream implements ICheckpointProtocol {
   ...
   * @param readOnly When <code>true</code> the view will be immutable.
  ...
  public Stream(
    ...
    if (checkpoint == null) throw new IllegalArgumentException();
  ...
  public static class StreamIndexMetadata extends IndexMetadata {
    ...
    public interface Options extends IndexMetadata.Options {
      ...
      /** The name of a class derived from {@link Stream} that will be used to re-load the index. */
      String STREAM_CLASS_NAME = Stream.class.getName() + ".className";

      /** The {@link CompressionEnum} for the stream. */
      String STREAM_COMPRESSION_TYPE = Stream.class.getName() + ".compressionType";
    }
     ...
     * various setter methods, but some values can not be safely overridden after the index is in
     * use.
     *
     ...
     * @param indexUUID The indexUUID. The same index UUID MUST be used for all component indices in
     *     a scale-out index.
     * @throws IllegalArgumentException if the indexUUID is <code>null</code>.
     */
    ...
    public StreamIndexMetadata(
       ...
       */
      this.streamClassName =
          getProperty(
                  indexManager,
                  properties,
                  namespace,
                  Options.STREAM_CLASS_NAME,
                  Stream.class.getName())
              .intern();

      this.streamCompressionType =
          CompressionEnum.valueOf(
              getProperty(
                  indexManager,
                  properties,
                  namespace,
    ...
    public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
      ...
      streamCompressionType = CompressionEnum.values()[LongPacker.unpackInt(in)];
    }
   ...
   */
  public static SolutionSetStream create(
      final IRawStore store, final StreamIndexMetadata metadata) {
  ...
  public static SolutionSetStream load(
    ...
    try {
      ...
      final SolutionSetStream solutions =
          (SolutionSetStream)
              ctor.newInstance(new Object[] {store, checkpoint, metadata, readOnly});
  ...
  protected OutputStream wrapOutputStream(final OutputStream out) {
    ...
    final CompressionEnum compressionType = metadata.getStreamCompressionType();
    ...
    switch (compressionType) {
      ...
      default:
        throw new UnsupportedOperationException("CompressionEnum=" + compressionType);
    }
  }
   ...
   *
   * @return <code>true</code> true iff changes would be lost unless the {@link Stream} was flushed
   *     to the backing store using {@link #writeCheckpoint()}.
   */
  ...
  private final Checkpoint _writeCheckpoint2() {
    ...
    if (EmbergraphStatics.debug || log.isInfoEnabled()) {
      final String msg = "name=" + metadata.getName() + "} : " + checkpoint;
  ...
  public final void setLastCommitTime(final long lastCommitTime) {
    ...
    if (log.isInfoEnabled()) log.info("old=" + this.lastCommitTime + ", new=" + lastCommitTime);
