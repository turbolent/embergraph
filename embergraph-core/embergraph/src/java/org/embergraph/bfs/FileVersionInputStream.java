...
public class FileVersionInputStream extends InputStream {
  ...
  private boolean nextBlock() {
    ...
    if (!src.hasNext()) {
      ...
      EmbergraphFileSystem.log.info("No more blocks: id=" + id + ", version=" + version);
    ...
    if (addr == 0L) {
      ...
      EmbergraphFileSystem.log.info(
          "Read zero bytes: id=" + id + ", version=" + version + ", block=" + block);
      ...
      try {
        ...
        if (nread != len) {
          ...
          throw new RuntimeException("Expecting " + len + " bytes but read " + nread);
        }
      ...
      EmbergraphFileSystem.log.info(
          "Read " + b.length + " bytes: id=" + id + ", version=" + version + ", block=" + block);
    }
