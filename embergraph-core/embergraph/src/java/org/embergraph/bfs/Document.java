...
public interface Document extends DocumentHeader {
  ...
  /**
   * A reader on the byte[] stored in the {@link IContentRepository} where bytes are decoded to
   * characters using the encoding identified by {@link DocumentHeader#getContentEncoding()}.
   *
   * @throws IllegalStateException if the {@link DocumentHeader#getContentEncoding()} is not set.
   */
