...
public class AtomicBlockAppendProc implements ISimpleIndexProcedure<Object>, Externalizable {
   ...
   */
  public AtomicBlockAppendProc(
      EmbergraphFileSystem repo, String id, int version, byte[] b, int off, int len) {
   ...
   * and block whose value is the address of the block's data on the {@link Journal}.
   *
   * <p>Note: The caller MUST have correctly identified the data service on which the tail of the
   * file exists (or on which the head of the file will be written).
   *
   * <p>The block identifier is computed by reading and decoding the key for the last block written
   * for this file version (if any). Special cases exist when the file version spans more than one
   * index partition, when the block would be the first block (in key order) for the index
   * partition, and when the block would be the last block (in key order) for the index partition.
   *
   ...
   *
   * <p>Note: This implementation uses an {@link IRangeQuery#REVERSE} iterator to locate the last
   * block in the file and is capable of scale-out.
   *
   * @todo This implies that the leftSeparator for the index partition MUST NOT split the blocks for
   *     a file unless there is at least one block in the index partition. In practice this
   *     guarentee is easy to maintain. By default we choose to split an index partition on a file
   *     boundary. If that would result in an uneven split (or an empty split in the case of very
   *     large files) then we choose a split point that lies within the file's data - leaving at
   *     least one block for the file (probably many) in both partitions created by the split.
   * @param ndx
   * @param keyBuilder
   * @return
   */
  protected long getNextBlockIdentifierInFileVersion2(IIndex ndx, IKeyBuilder keyBuilder) {
    ...
    final byte[] toKey =
        keyBuilder
            ...
            .appendText(id, true /* unicode */, false /* successor */)
            .append(version)
            .append(Long.MAX_VALUE /*max block*/)
            .getKey();
    ...
    ITupleIterator itr =
        ndx.rangeIterator(
            ...
            toKey,
            1 /* capacity */,
            IRangeQuery.KEYS | IRangeQuery.REVERSE,
            null /*filter*/);
   ...
   * after any existing block for that file and version.
   *
   * <p>Note: This implementation uses the {@link ILinearList} API to locate the last block in the
   * file and is NOT capable of scale-out since that API is NOT available for an index partition
   * view (a {@link FusedView}).
   *
   * @todo This implies that the leftSeparator for the index partition MUST NOT split the blocks for
   *     a file unless there is at least one block in the index partition. In practice this
   *     guarentee is easy to maintain. By default we choose to split an index partition on a file
  ...
  protected long getNextBlockIdentifierInFileVersion(IIndex ndx, IKeyBuilder keyBuilder) {
    ...
    assert toIndex < 0
        : "Expecting insertion point: id=" + id + ", version=" + version + ", toIndex=" + toIndex;

    if (log.isDebugEnabled()) log.debug("insertionPoint=" + toIndex);
    ...
    if (log.isDebugEnabled()) log.debug("toIndex=" + toIndex + ", entryCount=" + entryCount);
    ...
    if (toIndex == 0) {
      ...
      if (log.isDebugEnabled())
        log.debug(
            "Insertion point is before all entries in the index partition: id="
                + id
                + ", version="
                + version);
      ...
      if (toIndex == entryCount) {
        ...
        if (log.isDebugEnabled())
          log.debug(
              "Insertion point is after all entries in the index partition: id="
                  + id
                  + ", version="
                  + version);
        ...
        if (log.isDebugEnabled())
          log.debug("Insertion point is at the toKey: id=" + id + ", version=" + version);
      }
      ...
      assert key != null
          : "Expecting entry: id=" + id + ", version=" + version + ", toIndex=" + toIndex;
   ...
   * as the block identifer to be used.
   *
   * @param keyBuilder The key builder.
   * @param key The key - either from the index partition or in some cases from the leftSeparator of
   *     the index partition metadata.
   *     <p>Note that the leftSeparator MAY be an empty byte[] (e.g., for the 1st index partition in
   *     the key order) and MIGHT NOT include the block identifier (the block identifier is only
   *     included when it is necessary to split a file across index partitions). When the block
   *     identifier is omitted from the key and the key encodes the same file and version we
   *     therefore use zero (0L) as the next block identifier since we will be appending the first
   *     block to the file version.
   * @return The block identifier that will be used by the atomic append operation.
   */
  protected long getNextBlockFromPriorKey(IKeyBuilder keyBuilder, byte[] key) {
    ...
    if (key.length >= prefix.length) {
      ...
      final int cmp =
          BytesUtil.compareBytesWithLenAndOffset(0, prefix.length, prefix, 0, prefix.length, key);
      ...
      if (cmp == 0) {
        ...
        if (prefix.length + Bytes.SIZEOF_LONG == key.length) {
          ...
          final long block = KeyBuilder.decodeLong(key, key.length - Bytes.SIZEOF_LONG) + 1;

          if (block > EmbergraphFileSystem.MAX_BLOCK) {

            throw new RuntimeException(
                "File version has maximum #of blocks: id=" + id + ", version=" + version);
          }

          if (INFO)
            log.info(
                "Appending to existing file version: id="
                    + id
                    + ", version="
                    + version
                    + ", block="
          ...
          if (INFO) log.info("Key is for same file version but does not contain block identifier.");
        }
        ...
        if (DEBUG) log.debug("Key does not compare as equal for length of prefix.");
      }
    ...
    if (INFO)
      log.info(
          "Appending to new file version: id=" + id + ", version=" + version + ", block=" + 0L);
