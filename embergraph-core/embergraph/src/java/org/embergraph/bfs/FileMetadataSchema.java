 ...
 * namespace!
 *
 * <p>Note: File version creation time and update times are available using the {@link
 * SparseRowStore}, which stores and reports the timestamp for each property value. Convenience
 * methods are available on {@link RepositoryDocumentImpl} to report those timestamps. Timestamps
 * for file blocks can NOT be obtained.
 *
 * <p>Note: A content length property was deliberately NOT defined. The design is geared towards
 * very large file and asynchronous read/write of file blocks. The length of short files may be
 * readily computed by the expediency of sucking their contents into a buffer. Large files should
 * always be processed using a stream-oriented technique or distributed to concurrent clients in
 * block sized pieces.
 *
 * @todo other obvious metadata would include the user identifier associated with each update
 *     request.
 */
public class FileMetadataSchema extends Schema {
   ...
   * </code> header and at times the leading bytes of the response body itself.
   */
  ...
  public static final transient String INDEX_TEXT =
      FileMetadataSchema.class.getPackage().getName() + ".IndexText";
