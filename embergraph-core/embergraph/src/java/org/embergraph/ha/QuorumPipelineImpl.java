 ...
 *
 * <ul>
 *   <li>{@link QuorumMember#pipelineAdd()}
 *   <li>{@link QuorumMember#pipelineRemove()}
 *   <li>{@link QuorumMember#pipelineChange(UUID, UUID)}
 * </ul>
 *
 * When a quorum is met, the <i>leader</i> is always first in the write pipeline since it is the
 * node which receives writes from clients. When a service joins the write pipeline, it always does
 * so at the end of the chain. Services may enter the write pipeline before joining a quorum in
 * order to synchronize with the quorum. If a service in the middle of the chain leaves the
 * pipeline, then the upstream node will reconfigure and retransmit the current cache block to its
 * new downstream node. This prevent nodes which are "bouncing" during synchronization from causing
 * write sets to be discarded. However, if the leader leaves the write pipeline, then the quorum is
 * broken and the write set will be discarded.
 *
 * <p>Since the write pipeline is used to synchronize services trying to join the quorum as well as
 * the replicate writes for services joined with the quorum, {@link HAReceiveService} may be live
 * for a met quorum even though the {@link QuorumMember} on whose behalf this class is acting is not
 * joined with the met quorum.
 *
 * <h3>Pipeline maintenance</h3>
 *
 * There are three broad categories which have to be handled: (1) leader leaves; (2) pipeline leader
 * election; and (3) follower leaves. A leader leave causes the quorum to break, which will cause
 * service leaves and pipeline leaves for all joined services. However, services must add themselves
 * to the pipeline before they join the quorum and the pipeline will be reorganized if necessary
 * when the quorum leader is elected. This will result in a {@link #pipelineElectedLeader()} event.
 * A follower leave only causes the follower to leave the pipeline and results in a {@link
 * #pipelineChange(UUID, UUID)} event.
 *
 * <p>There are two cases for a follower leave: (A) when the follower did not did not have a
 * downstream node; and (B) when there is downstream node. For (B), the upstream node from the left
 * follower should reconfigure for the new downstream node and retransmit the current cache block
 * and the event should be otherwise unnoticed.
 *
 * <p>Handling a follower join requires us to synchronize the follower first which requires some
 * more infrastructure and should be done as part of the HA synchronization test suite.
 *
 * <p>What follows is an example of how events will arrive for a quorum of three services: A, B, and
 * C.
 *
 * <pre>
 ...
 * </pre>
 *
 * At this point the pipeline order is <code>[A,B,C]</code>. Notice that the {@link HASendService}
 * for A is not established until the <code>A.pipelineChange(null,B)</code> sets B as the new
 * downstream service for A. Likewise, B will not relay to C until it handles the <code>
 * B.pipelineChange(null,C)</code> event.
 *
 * <p>Given the pipeline order <code>[A,B,C]</code>, if B were to leave, then the events would be:
 *
 * <pre>
 * B.getActor().pipelineRemove() => B.pipelineRemove(); A.pipelineChange(B,C);
 * </pre>
 *
 * and when this class handles the <code>A.pipelineChange(B,C)</code> event, it must update the
 * {@link HAReceiveService} such that it now relays data to C.
 *
 * <p>On the other hand, given the pipeline order <code>[A,B,C]</code>, if C were to leave the
 * events would be:
 *
 * <pre>
 ...
 * </pre>
 *
 * and when this class handles the <code>B.pipelineChange(C,null)</code> event, it must update the
 * C's {@link HAReceiveService} such that it continues to receive data, but no longer relays data to
 * a downstream service.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @param <S>
 * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/681" > HAJournalServer deadlock:
 *     pipelineRemove() and getLeaderId() </a>
 */
public abstract class QuorumPipelineImpl<S extends HAPipelineGlue> /*extends
        QuorumStateChangeListenerBase */
    implements QuorumPipeline<S>, QuorumStateChangeListener {

  private static final transient Logger log = Logger.getLogger(QuorumPipelineImpl.class);

  /*
   * The timeout for a sleep before the next retry. These timeouts are designed to allow some
   * asynchronous processes to reconnect the {@link HASendService} and the {@link HAReceiveService}s
   * in write pipeline such that a retransmit can succeed after a service has left the pipeline.
   * Depending on the nature of the error (i.e., a transient network problem versus a pipeline
   * reorganization), this can involve a number of zookeeper events.
   *
   * <p>The retries will continue until {@link #getRetrySendTimeoutNanos()} has elapsed.
   *
   * <p>FIXME If this timeout is LTE 50ms, then ChecksumErrors can appear in the sudden kill tests.
   * The root cause is the failure to consistently tear down and setup the pipeline when there are
   * multiple requests queued for the pipeline. This issue has also shown up when there is a
   * replicated write and a concurrent HALog replication. Specifically, an interrupt in sendHALog()
   * does not cause the socket channel on which the payload is transmitted to be closed. See <a
   * href="https://sourceforge.net/apps/trac/bigdata/ticket/724">HA wire pulling and sudden kill
   * testing</a>.
   */
   ...
   *
   * <p>Note: This gets overridden in the ZK aware version and set to a constant greater than the
   * then-current negotiated timeout for the client.
   */
  ...
  /*
   * Condition signalled when the write replication pipeline has been changed (either the upstream
   * and/or downstream service was changed).
   */
  ...
  /** The buffer used to relay the data. This is only allocated for a follower. */
  ...
  /*
   * Core implementation of the handler for the various events. Always run while holding the {@link
   * #lock}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/681" > HAJournalServer deadlock:
  ...
  private final class InnerEventHandler extends QuorumStateChangeListenerBase {
    ...
    /*
     * A queue of events that can only be handled when a write replication operation owns the {@link
     * QuorumPipelineImpl#lock}.
     *
    ...
    private void queue(final QuorumStateChangeEvent e) {
      ...
      if (log.isInfoEnabled()) log.info("Adding StateChange: " + e);
     ...
     *
     * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/681" > HAJournalServer
     *     deadlock: pipelineRemove() and getLeaderId() </a>
     */
    private static final boolean s_eventElission = true;

    /*
     * Event elission endeavours to ensure that events processed represent current state change.
     *
     * <p>This is best explained with an example from its original usage in processing graphic
     * events. Whilst a "button click" is a singular event and all button clicks should be
     * processed, a "mouse move" event could be elided with the next "mouse move" event. Thus the
     * move events (L1 -> L2) and (L2 -> L3) would elide to a single (L1 -> L3).
     *
     * <p>In HA RMI calls can trigger event processing, whilst other threads monitor state changes -
     * such as open sockets. Without elission, monitoring threads will observe unnecessary
     * transitional state changes. HOWEVER, there remains a problem with this pattern of
     * synchronization.
     */
    ...
    private void dispatchEvents() {
      ...
      while ((e = queue.poll()) != null) {
        ...
        if (log.isInfoEnabled()) log.info("Dispatching: " + e);
     ...
     *
     * @param e The event.
     * @throws IllegalMonitorStateException if the caller does not own the {@link #lock}.
     */
    private void dispatchEvent(final QuorumStateChangeEvent e) throws IllegalMonitorStateException {
...
//      * Extended to setup this service as a leader ({@link #setUpLeader()}),
     ...
     * {@inheritDoc}
     *
     * <p>This implementation sets up the {@link HASendService} or the {@link HAReceiveService} as
     * appropriate depending on whether or not this service is the first in the pipeline order.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>This implementation tears down the {@link HASendService} or {@link HAReceiveService}
     ...
     * {@inheritDoc}
     *
     * <p>This implementation changes the target of the {@link HASendService} for the leader (or the
     * {@link HAReceiveService} for a follower) to send (or relay) write cache blocks to the
     * specified service.
     */
    @Override
    public void pipelineChange(final UUID oldDownStreamId, final UUID newDownStreamId) {
       ...
       * Nothing to do. The pipeline is already configured
        ...
        final InetSocketAddress addrNext = nextState == null ? null : nextState.addr;
       ...
       * Terminate the existing connection (we were the first
          ...
          if (addrNext != null) {
            if (log.isDebugEnabled()) log.debug("sendService.start(): addrNext=" + addrNext);
       ...
       * Reconfigure the receive service to change how it is
           ...
           */
          if (log.isDebugEnabled())
            log.debug("receiveService.changeDownStream(): addrNext=" + addrNext);
        ...
        if (log.isDebugEnabled()) log.debug("pipelineChange - done.");
      } finally {
        lock.unlock();
      }
    }
    ...
    /*
     * Request the {@link InetSocketAddress} of the write pipeline for a service (RMI).
     *
     * @param downStreamId The service.
     * @return It's {@link InetSocketAddress}
     */
    private PipelineState<S> getAddrNext(final UUID downStreamId) {

      if (downStreamId == null) return null;
    ...
    /*
     * Tear down any state associated with the {@link QuorumPipelineImpl}. This implementation tears
     * down the send/receive service and releases the receive buffer.
     */
...
//         * Populate or clear the {@link #pipelineState} cache.
    ...
    /*
     * Setup the service to receive pipeline writes and to relay them (if there is a downstream
     * service).
     */
    private void setUpReceiveService() {
      ...
      try {
        ...
        final InetSocketAddress addrSelf = member.getService().getWritePipelineAddr();
        ...
        receiveService =
            new HAReceiveService<HAMessageWrapper>(
                ...
                new IHAReceiveCallback<HAMessageWrapper>() {
                  @Override
                  public void callback(final HAMessageWrapper msg, final ByteBuffer data)
                      throws Exception {
                    // delegate handling of write cache blocks.
                    handleReplicatedWrite(
                        msg.getHASyncRequest(), (IHAWriteMessage) msg.getHAWriteMessage(), data);
                  }

                  @Override
                  public void incReceive(
                      final HAMessageWrapper msg, final int nreads, final int rdlen, final int rem)
                      throws Exception {
  ...
  /*
   * Extended to invoke {@link #tearDown()} in order to guarantee the eventual release of the {@link
   * #receiveBuffer} and the shutdown of the {@link #sendService} or {@link #receiveService}.
   */
  @Override
  ...
  /*
   * Return the index at which the given serviceId appears in the array of serviceIds.
   *
   * @param serviceId The {@link UUID} of some quorum member.
   * @param a An array of service {@link UUID}s.
   * @return The index of the service in the array -or- <code>-1</code> if the service does not
   *     appear in the array.
   */
  private static int getIndex(final UUID serviceId, final UUID[] a) {
  ...
  /*
   * Return the NIO buffer used to receive payloads written on the HA write pipeline.
   *
   * @return The buffer -or- <code>null</code> if the pipeline has been torn down or if this is the
   *     leader.
   */
  ...
  /*
   * Return the {@link HAReceiveService} used to receive payloads written on the HA write pipeline.
   *
   * @return The buffer -or- <code>null</code> if the pipeline has been torn down or if this is the
   *     leader.
   */
  ...
  /*
   * Return the {@link HASendService} used to write payloads on the HA write pipeline.
   *
   * @return The {@link HASendService} -or- <code>null</code> if the pipeline has been torn down.
   */
  ...
  public void pipelineAdd() {
    ...
    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.PIPELINE_ADD));
  }

  @Override
  public void pipelineElectedLeader() {

    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.PIPELINE_ELECTED_LEADER));
  }

  @Override
  public void pipelineRemove() {

    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.PIPELINE_REMOVE));
  }

  @Override
  public void pipelineChange(final UUID oldDownStreamId, final UUID newDownStreamId) {

    innerEventHandler.queue(
        new QCE(
            QuorumStateChangeEventEnum.PIPELINE_CHANGE,
            new UUID[] {oldDownStreamId, newDownStreamId},
            null /* lastCommitTimeConsensus */,
            null /* token */,
            null /* leaderId */));
  }
  ...
  public void pipelineUpstreamChange() {
    ...
    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.PIPELINE_UPSTREAM_CHANGE));
  }
  ...
  public void memberRemove() {
    ...
    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.MEMBER_REMOVE));
  }
  ...
  public void consensus(final long lastCommitTime) {
    ...
    innerEventHandler.queue(
        new QCE(
            ...
            null /* downstreamIds */,
            lastCommitTime /* lastCommitTimeConsensus */,
            null /* token */,
            null /* leaderId */));
  }
  ...
  public void lostConsensus() {
    ...
    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.LOST_CONSENSUS));
  }

  @Override
  public void serviceJoin() {

    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.SERVICE_JOIN));
  }

  @Override
  public void serviceLeave() {

    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.SERVICE_LEAVE));
  }
  ...
  public void quorumBreak() {
    ...
    innerEventHandler.queue(new QCE(QuorumStateChangeEventEnum.QUORUM_BREAK));
  }
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private class ResetPipelineTaskImpl implements Callable<IHAPipelineResetResponse> {
    ...
    /*
     * If there is an identified problem service and that service is either our upstream or
     * downstream service, then we need to wait until we observe a pipeline change event such that
     * it is no longer our upstream or downstream service. Otherwise we can go ahead and reset our
     * pipeline.
     *
     * <p>TODO What if this service is the problem service?
     */
  ...
  /** Task robustly replicates an {@link IHAWriteMessage} and the associated payload. */
  private class RobustReplicateTask implements Callable<Void> {

    /*
     * An historical message is indicated when the {@link IHASyncRequest} is non-<code>null</code>.
     */
    ...
    /*
     * The token for the leader. The service that initiates the replication of a message MUST be the
     * leader for this token.
     *
     * <p>The token is either taken from the {@link IHAWriteMessage} (if this is a live write) or
     * from the current {@link Quorum#token()}.
     *
     * <p>Either way, we verify that this service is (and remains) the leader for that token
     * throughout the {@link RobustReplicateTask}.
     */
    private final long quorumToken;

    public RobustReplicateTask(
      ...
      if (snd == null) throw new IllegalArgumentException();
      ...
      if (b.remaining() == 0) {
        ...
        throw new IllegalStateException(
            "Empty buffer: req=" + req + ", msg=" + msg + ", buffer=" + b);
      }
    ...
    /*
     * This collects the assertion(s) that we make for the service that is attempting to robustly
     * replicate a write into a single method. This was done in order to concentrate any conditional
     * logic and design rationale into a single method.
     *
     * <p>Note: IFF we allow non-leaders to replicate HALog messages then this assert MUST be
     * changed to verify that the quorum token remains valid and that this service remains joined
     * with the met quorum, i.e.,
     *
     * <pre>
     ...
     *
     * <p>Note: {@link Future} for {@link #call()} is [WCS.remoteWriteFuture].
     *
     * <p>Note: In order for replication from the leader to be robust while still permitting the WCS
     * to shutdown, we need to distinguish two different ways in which replication can be
     * interrupted/cancelled:
     *
     * <p>1. WCS.close()/reset() => throw;
     *
     * <p>2. pipelineChange() => retrySend()
     *
     * <p>If the WCS is being shutdown, then we MUST NOT attempt to cure the exception thrown out of
     * innerReplicate(). This will show up as an {@link InterruptedException} if the interrupt is
     * encountered in this thread (call()).
     *
     * <p>If there is a {@link QuorumPipelineImpl#pipelineChange(UUID, UUID)}, then that method will
     * cause {@link SendBufferTask} and/or {@link ReceiveAndReplicateTask} to be cancelled. {@link
     * Future#get()} for those tasks thus can report a {@link CancellationException}.
     *
     * <p>If we fail to distinguish these cases, then a pipelineChange() can cause the {@link
     * RobustReplicateTask} to fail, which is precisely what it should not do. Instead the pipeline
     * should be re-established and the writes replicated by the leader along the new pipeline
     * order.
     *
     * <p>The following stack traces are illustrative of a problem when the pipeline is [A,C] and
     * [B] joins while a write is being replicated from [A] to [C]. The join of [B] triggers
     * pipelineChange() which causes the {@link SendBufferTask} to be cancelled. The untrapped
     * {@link CancellationException} propagates from this method to WCS.WriteTask.writeCacheBlock()
     * to fail, failing the transaction. The correct behavior in this case is to enter retrySend()
    ...
    /*
     * Replicate from the leader to the first follower. Each non-final follower will
     * receiveAndReplicate the write cache buffer. The last follower will receive the buffer.
     *
     * @param retryCount The #of attempts and ZERO (0) if this is the first attempt.
     * @throws Exception
     */
    ...
    /*
     * Robust retransmit of the current cache block. This method is designed to handle several kinds
     * of recoverable errors, including:
     *
     * <ul>
     *   <li>downstream service leaves the pipeline
     *   <li>intermittent failure sending the RMI message
     *   <li>intermittent failure sending the payload
     * </ul>
     *
     * The basic pattern is that it will retry the operation a few times to see if there is a
     * repeatable error. Each time it attempts the operation it will discover the current downstream
     * serviceId and verify that the quorum is still valid. Each error (including the first) is
     * logged. If the operation fails, the original error is rethrown. If the operation succeeds,
     * then the cache block was successfully transmitted to the current downstream service and this
    ...
    private boolean retrySend() throws InterruptedException {
      ...
      while (true) {
        ...
        if (remaining <= retrySleepNanos) break;
        ...
        try {
          ...
          log.error(
              "retry="
                  + tryCount
                  + ", elapsed="
                  + TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - beginNanos)
                  + "ms : "
                  + t,
              t);
...
//         *
  ...
  private static class SendBufferTask<S extends HAPipelineGlue> implements Callable<Void> {
    ...
    public SendBufferTask(
        final QuorumMember<S> member,
        final long token,
        final IHASyncRequest req,
        final IHASendState snd,
        final IHAWriteMessage msg,
    ...
    private void doRunWithLock() throws InterruptedException, ExecutionException, IOException {
      ...
      try {
        try {
          ...
          try {
            futRmt = downstream.service.receiveAndReplicate(req, snd, msg);
   ...
   *
   * @param isLeader <code>true</code> iff this service is the quorum leader.
   * @param token The quorum token.
   * @param member The {@link QuorumMember} for this service.
   * @param outerClass The outer class - required for {@link #resetPipeline()}.
   * @param t The throwable.
   */
  private static <S extends HAPipelineGlue> void launderPipelineException(
     ...
     */
    final PipelineImmediateDownstreamReplicationException remoteCause =
        (PipelineImmediateDownstreamReplicationException)
            InnerCause.getInnerCause(t, PipelineImmediateDownstreamReplicationException.class);
     ...
     */
    final ImmediateDownstreamReplicationException directCause =
        remoteCause == null
            ? (ImmediateDownstreamReplicationException)
                InnerCause.getInnerCause(t, ImmediateDownstreamReplicationException.class)
            : null;
    ...
    if (isLeader) {
       ...
       * If we can identify the problem service, then force it out of
        ...
        // Log and continue.
        log.error("Problem on force remove: problemServiceId=" + problemServiceId, e);
      ...
      /*
       * Reset the pipeline on each service (including the leader). If replication fails, the socket
       * connections both upstream and downstream of the point of failure can be left in an
       * indeterminate state with partially buffered data. In order to bring the pipeline back into
       * a known state (without forcing a quorum break) we message each service in the pipeline to
       * reset its HAReceiveService (including the inner HASendService. The next message and payload
       * relayed from the leader will cause new socket connections to be established.
       *
  ...
  /*
   * Issue concurrent requests to each service in the pipeline to reset the pipeline on that
   * service. The request to the leader is executed in the caller's thread so it will own whatever
   * locks the caller already owns - this is done to avoid deadlock.
   *
   * @param token The quorum token on the leader.
   * @param problemServiceId The problem service in the write pipeline (if known).
   */
  private void resetPipeline(final long token, final UUID problemServiceId) {

    log.error(
        "Leader will reset pipeline: token=" + token + ", problemServiceId=" + problemServiceId);
    ...
    final IHAPipelineResetRequest msg =
        new HAPipelineResetRequest(token, problemServiceId, TimeUnit.MILLISECONDS.toNanos(5000));
  ...
  private static class PipelineResetMessageTask<S extends HAPipelineGlue>
    ...
    public PipelineResetMessageTask(
        final ServiceLookup<S> serviceLookup,
    ...
    @Override
    protected Future<IHAPipelineResetResponse> doRMI(final S service) throws IOException {
  ...
  /*
   * Lock used to ensure that at most one message is being sent along the write pipeline at a time.
   */
  ...
  public Future<Void> receiveAndReplicate(
     ...
     */
    final long token = req == null ? msg.getQuorumToken() : member.getQuorum().token();
    ...
    try {
      ...
      if (log.isTraceEnabled())
        log.trace(
            "Will receive " + ((downstream != null) ? " and replicate" : "") + ": msg=" + msg);
  ...
  private static class ReceiveTask<S extends HAPipelineGlue> implements Callable<Void> {
    ...
    public Void call() throws Exception {
      ...
      // wrap the messages together.
      final HAMessageWrapper wrappedMsg = new HAMessageWrapper(req, snd, msg);

      // Get Future for receive() outcome on local service.
      final Future<Void> futRec = receiveService.receiveData(wrappedMsg, b);
  ...
  /** A service in the middle of the write pipeline (not the first and not the last). */
  private static class ReceiveAndReplicateTask<S extends HAPipelineGlue> implements Callable<Void> {
    ...
    public ReceiveAndReplicateTask(
        final QuorumMember<S> member,
        final long token,
        final IHASyncRequest req,
        final IHASendState snd,
    ...
    public Void call() throws Exception {
      ...
      // Get Future for receive() outcome on local service.
      final Future<Void> futLoc = receiveService.receiveData(wrappedMsg, b);
       ...
       * Note: We want to check the remote Future for the
        ...
        launderPipelineException(false /* isLeader */, token, member, outerClass, t);
      }
      // done
      return null;
    }
   ...
   * this method is NOT responsible for replication.
   *
   * @param req The synchronization request (optional). When non- <code>null</code> the msg and
   *     payload are historical data. When <code>null</code> they are live data.
   * @param msg Metadata about a buffer containing data replicated to this node.
   * @param data The buffer containing the data.
   * @throws Exception
   */
  protected abstract void handleReplicatedWrite(
      final IHASyncRequest req, final IHAWriteMessage msg, final ByteBuffer data) throws Exception;
   ...
   * @throws Exception
   */
  protected abstract void incReceive(
      final IHASyncRequest req,
      final IHAWriteMessage msg,
      final int nreads,
      final int rdlen,
  ...
  /*
   * A utility class that bundles together the Internet address and port at which the downstream
   * service will accept and relay cache blocks for the write pipeline and the remote interface
   * which is used to communicate with that service using RMI.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private static class PipelineState<S extends HAPipelineGlue> implements Externalizable {

    /** */
    private static final long serialVersionUID = 1L;

    /*
     * The Internet address and port at which the downstream service will accept and relay cache
     * blocks for the write pipeline.
     */
    public InetSocketAddress addr;

    /*
     * The remote interface for the downstream service which will accept and relay cache blocks from
     * this service.
     *
     * <p>Note: In order for an instance of this class to be serializable, an exported proxy for the
     * {@link HAGlue} object must be used here rather than the local object reference.
     */
    public S service;
