...
public class HAWriteMessage extends HAWriteMessageBase implements IHAWriteMessage {
   ...
   *     for its local store).
   * @param commitCounter The commit counter for the current root block for the write set which is
   *     being replicated by this message.
   * @param commitTime The commit time for the current root block for the write set which is being
   *     replicated by this message.
   * @param sequence The write cache block sequence number. This is reset to ZERO (0) for the first
   *     replicated write cache block in each write set.
   * @param sze The #of bytes in the payload.
   * @param chk The checksum of the payload.
   * @param storeType The type of backing store (RW or WORM).
   * @param quorumToken The quorum token for which this message is valid.
   * @param fileExtent The length of the backing file on the disk.
   * @param firstOffset The file offset at which the data will be written (WORM only).
   ...
   * @param storeType The type of backing store (RW or WORM).
   * @param quorumToken The quorum token for which this message is valid.
   * @param replicationFactor The replication factor in effect for the leader.
   * @param fileExtent The length of the backing file on the disk.
   * @param firstOffset The file offset at which the data will be written (WORM only).
   * @param compressorKey The key under which an {@link IRecordCompressor} has been registered
   *     against the {@link CompressorRegistry} -or- <code>null</code> for no compression.
   */
  public HAWriteMessage(
      final UUID uuid,
      final long commitCounter,
    ...
    if (storeType == null) throw new IllegalArgumentException();
   ...
   * files an HAJournal files for the branch in which we were doing development on the
   * HAJournalServer. In all released versions, the {@link #uuid} field should always be non-<code>
   * null</code>.
   */
  ...
  public boolean equals(final Object obj) {
    ...
    if (!(obj instanceof IHAWriteMessage)) return false;
  ...
  public void writeExternal(final ObjectOutput out) throws IOException {
    ...
    if (currentVersion >= VERSION1 && uuid != null) {
      ...
      if (currentVersion >= VERSION2) {
        ...
        if (compressorKey != null) out.writeUTF(compressorKey);
      }
    ...
    if (currentVersion >= VERSION3) out.writeInt(replicationFactor);
