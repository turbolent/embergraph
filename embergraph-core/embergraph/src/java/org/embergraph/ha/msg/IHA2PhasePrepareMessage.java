...
public interface IHA2PhasePrepareMessage extends IHA2PhaseCommitProtocolMessage {
   ...
   *
   * <p>Note: This is used to support atomic decisions about whether or not a service was joined
   * with the met quorum at the time that the leader decided to update the consensus release time.
   * Services that are in the pipeline and resynchronizing were either joined or not for the
   * purposes of a given 2-phase GATHER operation based on this flag.
   */
   ...
   *
   * <p>Note: This is used to support atomic decisions about whether or not a service was joined
   * with the met quorum at the time that the leader decided to commit. Services that are in the
   * pipeline and resynchronizing will either be joined or not for the purposes of a given 2-phase
   * commit based on this flag.
   *
   * <p>Note: A better name for this flag would be <code>isPrepareService()</code>.
   */
