 ...
 * @param <S>
 */
public class QuorumReadImpl<S extends HAReadGlue> extends QuorumStateChangeListenerBase
   ...
   *
   * @param joinedServiceIds The {@link UUID}s of the services currently joined with the quorum.
   * @return The {@link UUID} of the service to which the failover read will be directed.
   */
   ...
   *     where it is holding a lock on the local low-level store!
   */
  public byte[] readFromQuorum(final UUID storeId, final long addr)
      throws InterruptedException, IOException {

    if (log.isInfoEnabled()) log.info("storeId=" + storeId + ", addr=" + addr);
    ...
    final IHAReadRequest msg = new HAReadRequest(token, storeId, addr);
    /*
