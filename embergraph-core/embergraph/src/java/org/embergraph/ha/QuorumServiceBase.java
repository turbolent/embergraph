...
public abstract class QuorumServiceBase<S extends HAGlue, L extends AbstractJournal>
   ...
   *     service).
   * @param service The interface for the local service that is exposed to remote clients (typically
   *     as a smart proxy).
   * @param localService The local service implementation.
   */
  protected QuorumServiceBase(
      final String logicalServiceId, final UUID serviceId, final S service, final L localService) {

    super(logicalServiceId, serviceId);

    if (localService == null) throw new IllegalArgumentException();
  ...
  @Override
  public CommitResponse commit2Phase(final CommitRequest req)
   ...
   * cancel the futures.
   *
   * <p>NOte: This is not being done in parallel. However, due to a DGC thread leak issue, we now
   * use {@link ThickFuture}s. Thus, the tasks that are being cancelled are all local tasks running
   * on the {@link #executorService}. If that local task is doing an RMI, then cancelling it will
   * cause an interrupt in the NIO request.
   */
