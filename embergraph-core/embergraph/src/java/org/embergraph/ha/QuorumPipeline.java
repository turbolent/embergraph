...
public interface QuorumPipeline<S extends HAPipelineGlue> {
   ...
   *
   * <p>Note: The implementation of this method should be robust to changes in the write pipeline.
   * Specifically, if a follower leaves the write pipeline, it should attempt to retransmit the
   * message and the payload while allowing time for the write pipeline to be reconfigured in
   * response to the related {@link QuorumMember} events.
   *
   * @param req A synchronization request (optional). This is only non-null when historical {@link
   *     WriteCache} blocks are being replayed down the write pipeline in order to synchronize a
   *     service.
   * @param msg The RMI metadata about the payload.
   * @param b The payload. The bytes from the position to the limit will be transmitted (note that
   *     the #of bytes remaining in the buffer MUST agree with {@link IHAWriteMessage#getSize()}).
   */
   ...
   * @param data The {@link WriteCache} block.
   */
   ...
   * HALogWriter}.
   *
   * <p>Note: This method is ONLY invoked as part of the 2-phase commit protocol. Therefore, this
   * method ONLY applies to the live HALog file. A service is atomically either joined with the met
   * quorum at a 2-phase commit point or not joined. The PREPARE and COMMIT messages are ONLY
   * generated for services that were joined with the met quorum as of that atomic decision point in
   * the commit protocol. Therefore, this method is never called for a service that was not joined
   * as of that atomic decision point.
   *
   * @param rootBlock The root block for the commit point that was just achieved.
   */
   ...
   *
   * <p>Note: The open HALog file is NOT removed by this method.
   *
   * @param token The quorum token. The quorum must remain fully met for this token across this
   *     operation. If that invariant is violated then the operation will terminate once this
