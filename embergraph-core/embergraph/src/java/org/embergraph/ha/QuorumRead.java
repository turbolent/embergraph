...
public interface QuorumRead<S extends HAReadGlue> { // extends QuorumService<S> {
   ...
   * service.
   *
   * <p>Note: This is NOT the normal path for reading on a record from a service. This is used to
   * handle bad reads (when a checksum or IO error is reported by the local disk) by reading the
   * record from another member of the quorum.
   *
   * @param storeId The {@link UUID} of the {@link IRawStore} from which the record should be read.
   * @param addr The address of a record on that store.
   * @return The record.
   * @throws IllegalStateException if the quorum is not highly available.
   * @throws RuntimeException if the quorum is highly available but the record could not be read.
