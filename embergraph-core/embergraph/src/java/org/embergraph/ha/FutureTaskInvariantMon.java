 ...
 * check is made to verify that the invariant holds on entry. This pattern ensures there is no
 * possibility of a missed event.
 *
 * <p>This {@link FutureTask} wrapper will return the value of the {@link Callable} (or the
 * specified result for the {@link Runnable}) iff the task completes successfully without the
...
public abstract class FutureTaskInvariantMon<T> extends FutureTaskMon<T> implements QuorumListener {
  ...
  public FutureTaskInvariantMon(
    ...
    if (quorum == null) throw new IllegalArgumentException();
  ...
  public FutureTaskInvariantMon(
      final Runnable runnable, final T result, Quorum<HAGlue, QuorumService<HAGlue>> quorum) {
  ...
  public void assertMember(final UUID serviceId) {
    m_triggers.add(new QuorumEventInvariant(QuorumEventEnum.MEMBER_REMOVE, serviceId));
  ...
  public void assertJoined(final UUID serviceId) {
    m_triggers.add(new QuorumEventInvariant(QuorumEventEnum.SERVICE_LEAVE, serviceId));
  ...
  public void assertInPipeline(final UUID serviceId) {
    m_triggers.add(new QuorumEventInvariant(QuorumEventEnum.PIPELINE_REMOVE, serviceId));
  ...
  public void assertQuorumMet() {
    m_triggers.add(new QuorumEventInvariant(QuorumEventEnum.QUORUM_BROKE, null /* serviceId */));

    // now check that quorum is met and break if not
    if (!m_quorum.isQuorumMet()) broken();
    if (m_quorum.token() != token) broken();
  }

  /** Establish an invariant that the quorum is fully met. */
  public void assertQuorumFullyMet() {
    // no-one must leave!
    m_triggers.add(new QuorumEventInvariant(QuorumEventEnum.SERVICE_LEAVE, null /* serviceId */));

    // now check that quorum is fully met on the current token and break if not
    if (!m_quorum.isQuorumFullyMet(m_quorum.token())) broken();
  }

  private void assertMembership(final UUID[] members, final UUID serviceId) {
    if (isMember(members, serviceId)) return;
  ...
  private boolean isMember(final UUID[] members, final UUID serviceId) {
    for (UUID member : members) {
      if (member.equals(serviceId)) return true;
    }
  ...
  public void notify(final QuorumEvent e) {
    ...
    if (interrupt) {
      ...
      if (log.isDebugEnabled()) log.debug("Ignoring event: " + e);
    }
  }
  ...
  private class QuorumEventInvariant implements QuorumEvent, Serializable {
    ...
    public QuorumEventInvariant(final QuorumEventEnum qe, final UUID sid) {
      if (qe == null) throw new IllegalArgumentException();
      m_qe = qe;
      m_sid = sid;
    }
    ...
    public boolean matches(final QuorumEvent qe) {
      return qe.getEventType() == m_qe && (m_sid == null || m_sid.equals(qe.getServiceId()));
    }
  }
}
