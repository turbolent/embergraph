 ...
 * @version $Id$
 * @todo Scale-out needs to add {@link AbstractJournal#closeForWrites(long)} to this API and
 *     reconcile it with the methods to send index segments (and historical journal files) across
 *     the wire. Perhaps we need both an HAJournalGlue and an HADataServiceGlue interface?
 * @todo The naming convention for these interfaces should be changed to follow the standard jini
 *     smart proxy naming pattern.
 */
public interface HAGlue
    extends HAGlueBase, HAPipelineGlue, HAReadGlue, HACommitGlue, HATXSGlue, IService {
   ...
   * replication pipeline and enter the commit. The callback to the leader ensures that the service
   * join is visible to the leader before the leader makes an atomic decision about the set of
   * services that are joined with the met quorum for a 2-phase commit.
   *
   * @param req The request.
   * @return The most recent consensus release time for the quorum leader. This information is used
   *     to ensure that a service which joins after a gather and before a PREPARE will join with the
   *     correct release time for its local journal and thus will not permit transactions to start
   *     against commit points which have been recycled by the quorum leader.
   * @throws InterruptedException
   * @throws TimeoutException if the timeout is exceeded before the service join becomes visible to
   *     this service.
   ...
   * a quorum by replicating the root blocks of the quorum leader.
   *
   ...
   * PURPOSES.</strong>
   *
   * <p>The digest is useless if there are concurrent writes since it can not be meaningfully
   ...
   *
   * @param req The request (optional). When <code>null</code>, the {@link Future} for any existing
   *     snapshot operation will be returned but the request WILL NOT schedule a snapshot if none is
   *     running.
   ...
   *
   * <ul>
   *   <li>The quorum must be met and there must be an {@link HAStatusEnum#Ready} leader.
   *   <li>This service must be {@link HAStatusEnum#NotReady}.
   *   <li>This service MUST NOT be at the same commit point as the leader (if it is, then the
   *       service could meet in a data race with the met quorum and we do not permit RESTORE if the
   *       service is joined with the met quorum).
   *   <li>The {@link HAJournalServer} must not be running a RESTORE (we don't want it to
   *       accidentally interrupt a RESTORE that is in progress).
   * </ul>
   *
   ...
   * concurrency control should submit an {@link IApiTask} in order to have their task queue for the
   * necessary locks and run on the appropriate executor service.
   *
   * @param callable The task to run on the service.
   * @param asyncFuture <code>true</code> if the task will execute asynchronously and return a
   *     {@link Future} for the computation that may be used to inspect and/or cancel the
