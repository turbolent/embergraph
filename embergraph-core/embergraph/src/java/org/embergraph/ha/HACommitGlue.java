...
public interface HACommitGlue extends Remote {
   ...
   * return "no".
   *
   * @param prepareMessage The message used to prepare for the commit.
   ...
   *
   * <p>Note: A service automatically does a local abort() if it leaves the pool of services joined
   * with the quorum or if the leader fails over. Therefore a service should reject this message if
   * the token has been invalidated since the service either has dine or will shortly do a low-level
   * abort() on its own initiative.
   *
