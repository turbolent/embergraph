...
abstract class AbstractMessageTask<S extends Remote, T, M extends IHAMessage>
  ...
  public AbstractMessageTask(
      final ServiceLookup<S> serviceLookup, final UUID serviceId, final M msg) {
   ...
   * @throws IOException if there is a problem with the RMI.
   */
  protected abstract Future<T> doRMI(final S service) throws IOException;
}
