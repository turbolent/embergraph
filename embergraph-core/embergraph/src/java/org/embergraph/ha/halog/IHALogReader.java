...
public interface IHALogReader {
   ...
   * file. Specifically, if two readers are open for the same file and one is closed, then other
   * will remain open. Likewise, if a reader is open for the live HALog file, closing the writer
   * will not close the reader and closing the reader will not close the writer.
   */
   ...
   *
   * <p>FIXME The code disagress and will report [false] if the live log has the same root blocks
   ...
   * the next message is made available on the live HALog by the writer.
   *
   * <p>Note: This method is non-blocking if this is not the live HALog since the decision can be
   * made deterministically by inspecting the #of messages available (in the closing root block) and
   * the #of messages consumed by the reader.
   *
   * @return Return <code>false</code> if (a) the file is closed on entry to this method; (b) the
   *     live HALog is closed while waiting for more data to become available; or (c) the end of a
   *     historical HALog file has been reached. Return <code>true</code> iff more data can be read
   *     from the file.
   * @throws IOException if there is an error reading from the backing file.
   */
