 ...
 *
 * <p>The writer encapsulates not only the writing of individual messages but also the closing and
...
public class HALogWriter implements IHALogWriter {
  ...
  public static final int MAGIC = 0x83d9b735;
   ...
   *
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/738#comment:13" >Longevity and
   *     stress test protocol for HA QA </a>
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/679">HAJournalServer can not
   *     restart due to logically empty log file </a>
   */
  ...
  /** @throws IllegalStateException if the HALog is not open. */
  private void assertOpen() {

    if (m_state == null) throw new IllegalStateException();
  }
   ...
   * @param commitCounter The closing commit counter (the HALog file is named for the commit counter
   *     that will be associated with the closing root block).
  ...
  public String toString() {
    ...
    return getClass().getName() + "{" + m_state == null
        ? "closed"
   ...
   *     <code>false</code>, the HALog is flushed only once, after the closing root block is
   *     written.
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/738#comment:13" >Longevity and
   *     stress test protocol for HA QA </a>
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/679">HAJournalServer can not
  ...
  public void createLog(final IRootBlockView rootBlock) throws FileNotFoundException, IOException {
    ...
    if (haLog.isInfoEnabled()) haLog.info("rootBlock=" + rootBlock, new StackInfoReport());
  ...
  public void closeHALog(final IRootBlockView rootBlock) throws IOException {
    ...
    try {
      ...
      if (haLog.isInfoEnabled()) haLog.info("rootBlock=" + rootBlock);
      ...
      if (expectedCommitCounter != rootBlock.getCommitCounter()) {
        ...
        throw new IllegalStateException(
            "CommitCounter: expected="
                + expectedCommitCounter
                + ", actual="
      ...
      if (!this.m_rootBlock.getUUID().equals(rootBlock.getUUID())) {
        ...
        throw new IllegalStateException(
            "Store UUID: expected=" + (m_rootBlock.getUUID()) + ", actual=" + rootBlock.getUUID());
      }
       ...
       * Flush the HALog records to the disk.
         ...
         * flushed to the disk. However, better durability guarantees are provided by battery backup
         * on the disk controller and similar such nicities. If such techniques are used, you can
         * disable the doubleSync option and still have a guarantee that writes on the HALog are
         * durable and respect the applications ordering of write requests (in terms of restart safe
         * visibility).
         *
         * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/738#comment:13" >Longevity
         *     and stress test protocol for HA QA </a>
         */
        flush();
      }
  ...
  public void writeOnHALog(final IHAWriteMessage msg, final ByteBuffer data)
    ...
    try {
      ...
      if (m_rootBlock.getCommitCounter() != msg.getCommitCounter())
        throw new IllegalStateException(
            "commitCounter=" + m_rootBlock.getCommitCounter() + ", but msg=" + msg);

      if (m_rootBlock.getLastCommitTime() != msg.getLastCommitTime())
        throw new IllegalStateException(
            "lastCommitTime=" + m_rootBlock.getLastCommitTime() + ", but msg=" + msg);

      if (m_nextSequence != msg.getSequence())
        throw new IllegalStateException("nextSequence=" + m_nextSequence + ", but msg=" + msg);

      if (haLog.isDebugEnabled()) haLog.debug("msg=" + msg + ", position=" + m_position);

      if (m_position < headerSize0)
        throw new AssertionError("position=" + m_position + ", but headerSize=" + headerSize0);
   ...
   * been closed and which exists, then a reader will be returned for that HALog file. Otherwise, an
   * exception is thrown.
   *
   * @param commitCounter The commit counter associated with the commit point at the close of the
   *     write set (the commit counter that is in the file name).
  ...
  public IHALogReader getReader(final long commitCounter)
    ...
    try {
      ...
      if (haLog.isDebugEnabled()) haLog.debug("Opening historical HALog: file=" + logFile);
  ...
  /** The FileState class encapsulates the file objects shared by the Writer and Readers. */
  private static class FileState {
    ...
    private FileState(final File file, final StoreTypeEnum storeType) throws FileNotFoundException {
      m_haLogFile = file;
     ...
     * on a read.
     *
     * <p>TODO In fact, we probably should have the set of {@link OpenHALogReader}s so we can
     * explicitly close them. See {@link OpenHALogReader#close()}.
     */
    public void forceCloseAll() throws IOException {
      synchronized (this) {
        if (m_accessors > 0) m_accessors = 1;
        close();
      }
    }
  ...
  static class OpenHALogReader implements IHALogReader {
    ...
    public IRootBlockView getOpeningRootBlock() throws IOException {
      ...
      final IRootBlockView openRootBlock =
          tmp.rootBlock0 == closeRootBlock ? tmp.rootBlock1 : tmp.rootBlock0;
    ...
    public boolean hasMoreBuffers() {
       ...
       * Note: synchronized(FileState) makes these decisions atomic.
        ...
        if (m_state.isCommitted() && m_state.recordCount() <= m_record) return false;

        if (m_state.recordCount() > m_record) return true;
    ...
    public IHAWriteMessage processNextBuffer(final ByteBuffer clientBuffer) throws IOException {
      ...
      synchronized (m_state) {
        ...
        msg =
            HALogReader.processNextBuffer(
                m_state.m_raf, m_state.reopener, m_state.m_storeType, clientBuffer);
    ...
    public void close() throws IOException {
       ...
       * TODO This is a bit of a hack. The problem is that disableHALog() can force the close
             ...
             * invoking OpenHALogReader.close() on each of them in forceCloseAll().
             *
             * @see forceCloseAll()
             */
            return;
