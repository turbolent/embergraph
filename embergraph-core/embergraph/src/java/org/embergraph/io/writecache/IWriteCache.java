...
public interface IWriteCache {
   ...
   * @param data The record. The bytes from the current {@link ByteBuffer#position()} to the {@link
   ...
   *     buffer without changing the state of the cache (i.e., the data are copied into the cache).
   * @param chk The checksum of the <i>data</i> (optional). When checksums are not enabled this
   *     should be ZERO (0). When checksums are enabled, {@link #read(long)} will validate the
   *     checksum before returning <i>data</i>.
   * @return <code>true</code> iff the caller's record was transferred to the cache. When <code>
   *     false</code>, there is not enough room left in the write cache for this record.
   ...
   * @return The data read -or- <code>null</code> iff the record does not lie within the {@link
   *     IWriteCache}. When non-null, this will be a newly allocated exact fit mutable {@link
   *     ByteBuffer} backed by a Java <code>byte[]</code>. The buffer will be flipped to prepare for
   *     reading (the position will be zero and the limit will be the #of bytes read). The data DOES
   *     NOT include the bytes used to code checksum even when checksums are enabled.
   * @throws InterruptedException
   * @throws IllegalStateException if the buffer is closed.
   ...
   * abort protocol at the {@link IAtomicStore} level is responsible for ensuring that processes do
   * not see old data after an abort. This is generally handled by re-loading the appropriate root
   * block and reinitializing various things from that root block.
   *
   * @throws InterruptedException
   */
