 ...
 * @todo unit tests (this is used by RWStore and so is in general tested as part of that class, but
 *     it does not have its own test suite and it should not be all that difficult to write one,
 *     especially if we factor out an API for reporting the slotSize and then use a mock object in
 *     place of the RWStore).
...
public class BufferedWrite {
   ...
   * caller on the buffer will assign the {@link #m_startAddr starting offset} of the data in the
   * buffer.
   *
   * <p>Guarded by synchronized(this) (paranoia)
   */
  ...
  public synchronized int write(
      final long offset, final ByteBuffer data, final IReopenChannel<FileChannel> opener)
      throws IOException {
   ...
   * @throws IOException
   */
  public synchronized int flush(final IReopenChannel<FileChannel> opener) throws IOException {
  ...
  public String getStats(final StringBuffer buf, final boolean reset) {
    ...
    final String ret =
        "BufferedWrites, data: "
            + m_storeCounters.bufferDataWrites
            + ", file: "
            + m_storeCounters.bufferFileWrites
            + ", bytes: "
