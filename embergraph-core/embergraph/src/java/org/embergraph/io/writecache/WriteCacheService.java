 ...
 * cached writes must be written through to the backing channel). This implementation supports high
 * availability using a write replication pipeline.
 *
 * <p>A pool of {@link WriteCache} instances is used. Readers test all of the {@link WriteCache}
 * using a shared {@link ConcurrentMap} and will return immediately the desired record or <code>null
 * </code> if the record is not in any of the {@link WriteCache} instances. Write caches remain
 * available to readers until they need to be recycled as the current write cache (the one servicing
 * new writes).
 *
 * <p>The {@link WriteCacheService} maintains a dirty list of {@link WriteCache} instances. A single
 * thread handle writes onto the disk and onto the write replication pipeline (for HA). When the
 * caller calls flush() on the write cache service it flush() the current write cache is transferred
 * to the dirty list and then wait until the write cache instances now on the dirty list have been
 * serviced. In order to simplify the design and the provide boundary conditions for HA decision
 * making, writers block during {@link #flush(boolean, long, TimeUnit)}.
 *
 * <p>Instances of this class are used by both the {@link RWStrategy} and the {@link WORMStrategy}.
 * These classes differ in how they allocate space on the backing file and in the concurrency which
 * they permit for writers.
 *
 * <dl>
 *   <dt>{@link WORMStrategy}
 *   <dd>The {@link WORMStrategy} serializes all calls to {@link #writeChk(long, ByteBuffer, int)}
 *       since it must guarantee the precise offset at which each record is written onto the backing
 *       file. As a consequence of its design, each {@link WriteCache} is a single contiguous chunk
 *       of data and is transferred directly to a known offset on the disk. This append only
 *       strategy makes for excellent transfer rates to the disk.
 *   <dt>{@link RWStrategy}
 *   <dd>The {@link RWStrategy} only needs to serialize the decision making about the offset at
 *       which the records are allocated. Since the records may be allocated at any location in the
 *       backing file, each {@link WriteCache} results in a scattered write on the disk.
 * </dl>
 *
 * Both the {@link WORMStrategy} and the {@link RWStrategy} implementations need to also establish a
 * read-write lock to prevent changes in the file extent from causing corrupt data for concurrent
 * read or write operations on the file. See {@link #writeChk(long, ByteBuffer, int)} for more
 * information on this issue (it is a workaround for a JVM bug).
 *
 * <h2>Checksums</h2>
 *
 * The WORM and RW buffer strategy implementations, the WriteCacheService, and the WriteCache all
 * know whether or not checksums are in use. When they are, the buffer strategy computes the
 * checksum and passes it down (otherwise it passes down a 0, which will be ignored since checksums
 * are not enabled). The WriteCache adjusts its capacity by -4 when checksums are enabled and adds
 * the checksum when transferring the caller's data into the WriteCache. On read, the WriteCache
 * will verify the checksum if it exists and returns a new allocation backed by a byte[] showing
 * only the caller's record.
 *
 * <p>{@link IAddressManager#getByteCount(long)} must be the actual on the disk record length, not
 * the size of the record when it reaches the application layer. This on the disk length is the
 * adjusted size after optional compression and with the optional checksum. Applications which
 * assume that lengthOf(addr) == byte[].length will break, but that's life.
 *
 * <h2>ReadCache</h2>
 *
 * Without a hotList the readCache is managed naively by clearing any new readCache. This
 * potentially results in frequently accessed records being lost to the cache.
 *
 * <h2>HotCache</h2>
 *
 * With the HotCache evicted readCaches hot records get transferred to hotList and 'old' hotCaches
 * get added to end of readCache. Pattern is needed to pluck reserve hotCache from readList so that
 * it is always possible to transfer hot records from the readList.
 *
 * <p>Start with hotCache AND hotReserve.
 *
 * <p>If new reserve needed, because existing one is now used, try and compress new readCache into
 * current hotCache - if won't fit, then call resetWith and lose those writes, cycle again, moving
 * front hotCache to readList and compress that one.
 *
 * <p>LIMIT: If we begin with full caches with above threshold hitCounts then the whole list will
 * cycle around until we hit original cache which will contain records with zero hitCounts - for
 * practical purposes ignoring any concurrent reads.
 *
 * @see WriteCache
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @todo There needs to be a unit test which verifies overwrite of a record in the {@link
 *     WriteCache} (a write at the same offset in the backing file, but at a different position in
 *     the {@link WriteCache} buffer). It is possible for this to occur with the {@link RWStore} if
 *     a record is written, deleted, and the immediately reallocated. Whether or not this is a
 *     likely event depends on how aggressively the {@link RWStore} reallocates addresses which were
 *     allocated and then deleted within the same native transaction.
 * @todo When compression is enabled, it is applied above the level of the {@link WriteCache} and
 *     {@link WriteCacheService} (which after all require the caller to pass in the checksum of the
 *     compressed record). It is an open question as to whether the caller or the store handles
 *     record compression. Note that the B+Tree leaf and node records may require an uncompressed
 *     header to allow fixup of the priorAddr and nextAddr fields.
 */
public abstract class WriteCacheService implements IWriteCache {
   ...
   *
   * <p>Note: {@link #read(long)} is non-blocking. It does NOT use this lock!!!
   */
   ...
   * not want to yield that lock when awaiting the {@link #dirtyListEmpty} condition.
   *
   * <p>Note: If you wake up from this condition you MUST also test {@link #halt}.
   *
   * @see #dirtyListLock.
   ...
   * well.
   *
   * <p>Note: Error handling MUST cause the write cache service buffers to be {@link #reset()} and
   * make sure the HA write pipeline is correctly configured. This is handled by a high-level
   * abort() on the journal. It is NOT Ok to simply re-try writes of partly filled buffers since
   * they may already have been partly written to the disk. A high-level abort() is necessary to
   * ensure that we discard any bad writes. The abort() will need to propagate to all members of the
   * {@link Quorum} so they are all reset to the last commit point and have reconfigured write cache
   * services and write pipelines.
   */
   ...
   * @param minCleanListSize The maximum #of {@link WriteCache} buffers on the {@link #dirtyList}
   *     before we start to evict {@link WriteCache} buffers to the disk -or- ZERO (0) to use a
   *     default value. <br>
   *     Note: As a rule of thumb, you should set <code>maxDirtyListSize LTE nbuffers-4</code> such
   *     that we have at least: (1) for [current], (1) for [compactingCache], (1) for reserve and
   *     (1) buffer left available on the {@link #cleanList}.
   * @param prefixWrites When <code>true</code>, the {@link WriteCacheService} is supporting an RWS
   *     mode store and each {@link WriteCache} buffer will directly encode the fileOffset of each
   *     record written onto the {@link WriteCache}. When <code>false</code>, the {@link
   *     WriteCacheService} is supporting a WORM mode store and the {@link WriteCache} buffers
   *     contain the exact data to be written onto the backing store.
   * @param compactionThreshold The minimum percentage of space that could be reclaimed before we
   *     will attempt to coalesce the records in a {@link WriteCache} buffer. When <code>100</code>,
   *     compaction is explicitly disabled.
   *     <p>Note: This is ignored for WORM mode backing stores since we can not compact the buffer
   *     in that mode.
   * @param useChecksum <code>true</code> iff record level checksums are enabled.
   * @param fileExtent The current extent of the backing file.
   * @param opener The object which knows how to (re-)open the channel to which cached writes are
   *     flushed.
   * @param quorumManager The object which manages {@link Quorum} state changes on the behalf of
   *     this service.
   * @throws InterruptedException
   */
  public WriteCacheService(
      final int nwriteBuffers,
    ...
    if (nwriteBuffers <= 0) throw new IllegalArgumentException();
    ...
    if (minCleanListSize > nwriteBuffers) minCleanListSize = nwriteBuffers;

    if (minCleanListSize < 0) throw new IllegalArgumentException();

    if (compactionThreshold <= 0) throw new IllegalArgumentException();

    if (compactionThreshold > 100) throw new IllegalArgumentException();

    if (fileExtent < 0L) throw new IllegalArgumentException();

    if (opener == null) throw new IllegalArgumentException();
     ...
     * appear to correlate with it being enabled. It may be that with compaction enabled other
     * errors are more likely that are not directly associated with the compaction; for example as a
     * result of denser data content.
     *
     * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/674" > WCS write cache
     *     compaction causes errors in RWS postHACommit() </a>
     */
    this.compactionEnabled = canCompact() && compactionThreshold < 100;

    if (log.isInfoEnabled())
      log.info("Compaction Enabled: " + compactionEnabled + " @ threshold=" + compactionThreshold);
    ...
    current.set(
        writeBuffers[0] =
            newWriteCache(
                null /* buf */, useChecksum, false /* bufferHasData */, opener, fileExtent));
    //        if (nbuffers > 1) {
    ...
    for (int i = 1; i < nwriteBuffers; i++) {
      ...
      final WriteCache tmp =
          newWriteCache(null /* buf */, useChecksum, false /* bufferHasData */, opener, fileExtent);
    ...
    localWriteService =
        Executors.newSingleThreadExecutor(new DaemonThreadFactory(getClass().getName()));
   ...
   *
   * <p>Note: This method is package private for access by {@link WriteCacheService}.
   */
   ...
   *
   * <p>Note: For the WORM there is no advantage to any buffering, but the RWStore may recycle
   * storage, so: 1) Writes can be avoided if delayed 2) Buffers could potentially be compacted,
   * further delaying writes.
   *
   * <p>Note: This MUST BE GTE ONE (1) since WriteTask.call() will otherwise drop through without
   * actually taking anything off of the dirtyList.
   */
   ...
   * frequently read buffers are transferred.
   *
   * <p>Data is added to the readCache:
   * <li>after an evicted WriteCache is written to disk/HA
   * <li>on a cache miss, disk reads are added to the cache
   *
   *     <p>Data is added to the hotList when a readCache is evicted from the readList. resetWith
   *     uses the hitCount associated with live data records to determine which data is transferred
   *     to the hotList.
   *
   *     <p>When a readCache is evicted from the hotList, the entire cache is moved to the readList.
   */
   ...
   *
   * <p>The HIRS cache is used in conjunction with the readCache which a naive copying strategy
   * would be a kind of LIRS cache. Instead, cache hits from "older" read cache records are copied
   * to the HIRS cache which should be recycled more slowly.
   *
   * <p>Once the HIRS cache is full (maximum number of buffers in use) then then the per record hit
   * count is used to determine which records are transferred to be maintained.
   */
  ...
  class WriteTask implements Callable<Void> {
     ...
     * close() and will return null rather than bad data. The reprovisioning of the write cache
     * service (e.g., by reset()) must hold the writeLock so as to occur when there are no
     * outstanding reads executing against the write cache service.
     *
     * @todo If resynchronization rolls back the lastCommitTime for a store, then we need to
    ...
    public Void call() {
      try {
        if (quorum != null) {
          ...
          checksumBuffer = ByteBuffer.allocate(writeBuffers[0].peek().capacity());
        } else {
          checksumBuffer = null;
        }
        doRun();
    ...
    private void doRun() throws Exception {
      ...
      while (true) {
        ...
        if (!wasEmpty) {
          ...
          if (compactionEnabled && !directWrite && percentEmpty >= compactionThreshold) {

            if (log.isDebugEnabled()) log.debug("percentEmpty=" + percentEmpty + "%");
        ...
        if (dirtyList.take() != cache) throw new AssertionError();
        counters.get().ndirty--;
        ...
        if (!wasEmpty && log.isInfoEnabled()) {
          ...
          final int hitRate = (int) (100 * ((ntests == 0L ? 0d : (double) nhit / ntests)));
          final WriteCacheServiceCounters c = counters.get();
          log.info(
              "WriteCacheService: bufferCapacity="
                  + writeBuffers[0].capacity()
                  + ",nbuffers="
                  + tmp.nbuffers
                  + ",nclean="
                  + tmp.nclean
     ...
     * "current" compacting buffer avoiding contention with writing threads 3) If required replace
     * current compacting buffer with reserved, adding compacting buffer to dirty list 4) Release
     * compacted
     *
    ...
    private boolean compactCache(final WriteCache cache) throws Exception {
      ...
      if (compactingReserveRef.get() == null) {
        ...
        if (tmp == null) return false; // cannot guarantee compaction
      ...
      try {
        ...
        if (curCompactingCache != null) {
          if (log.isTraceEnabled()) log.trace("Transferring to curCompactingCache");

          done =
              WriteCache.transferTo(
                  cache /* src */, curCompactingCache /* dst */, serviceMap, 0 /*threshold*/);
          if (done) {
            ...
            if (log.isDebugEnabled())
              log.debug(
                  "RETURNING RESERVE: curCompactingCache.bytesWritten="
        ...
        if (log.isDebugEnabled())
          log.debug(
              "USING RESERVE: curCompactingCache.bytesWritten="
                  + curCompactingCache.bytesWritten());
        sendAddressMetadata(cache);
     ...
     * created. This method will synchronously send those address notices and and also makes sure
     * that the followers see the recycled addresses records so they can keep both their allocators
     * and the actual allocations synchronized with the leader.
     *
     * @param cache A {@link WriteCache} whose data has been transfered into another {@link
     *     WriteCache} through a "compact" operation.
     ...
     *
     * <p>Note: This DOES NOT remove the {@link WriteCache} from the {@link #dirtyList}. It uses a
     * peek(). The {@link WriteCache} will remain on the {@link #dirtyList} until it has been
     * handled by {@link #doRun()}.
     *
      ...
      try {
        ...
        while (true) {
          if (!flush) {
            ...
            if (dirtyList.size() < m_dirtyListThreshold && !halt) {
        ...
        if (c.maxdirty < c.ndirty) c.maxdirty = c.ndirty;

        // Guaranteed available.
        final WriteCache cache = dirtyList.peek();
        if (cache == null) throw new AssertionError();
    ...
    private void writeCacheBlock(final WriteCache cache)
       ...
       * writing the HALog files.
       *
       * @see <a href="http://trac.blazegraph.com/ticket/721">HA1 </a>
       */
      final boolean isHALeader = quorum != null && quorum.getClient().isLeader(quorumToken);
      ...
      if (isHALeader) { // quorum != null && quorum.isHighlyAvailable()) {
        ...
        final QuorumPipeline<HAPipelineGlue> quorumMember =
            (QuorumPipeline<HAPipelineGlue>) quorum.getMember();

        assert quorumMember != null : "Not quorum member?";

        final WriteCache.HAPackage pkg =
            cache.newHAPackage(
       ...
       */
      {
        if (log.isDebugEnabled()) log.debug("Writing to file: " + cache.toString());
   ...
   * @param useChecksum <code>true</code> iff record level checksums are enabled.
   * @param bufferHasData <code>true</code> iff the buffer has data to be written onto the local
   *     persistence store (from a replicated write).
   * @param opener The object which knows how to re-open the backing channel (required).
   * @param fileExtent The then current extent of the backing file.
   * @return A {@link WriteCache} wrapping that buffer and able to write on that channel.
   * @throws InterruptedException
   */
  public abstract WriteCache newWriteCache(
      IBufferAccess buf,
      boolean useChecksum,
   ...
   * the clean list are NOT reset since they may contain valid cached reads (data which is known to
   * be on the disk). We do not want to discard the read cache on reset().
   *
   * <p>Note: This approach deliberately does not cause any buffers belonging to the caller of
   * {@link #writeChk(long, ByteBuffer, int)} to become part of the {@link #cleanList}.
   *
   * <p>Note: <strong>You MUST set the {@link #setExtent(long) file extent} </strong> after {@link
   * #reset() resetting} the {@link WriteCacheService}. This is necessary in order to ensure that
   * the correct file extent is communicated along the write replication pipeline when high
   * availability is enabled.
   *
   * <p>Note: {@link #reset()} MUST NOT interrupt readers. It should only reset those aspects of the
   * write cache state that are associated with writes. On the other hand, {@link #close()} must
   * close all buffers and must not permit readers to read from closed buffers.
   */
  public void reset() throws InterruptedException {
    ...
    try {
      ...
      try {
        ...
        dirtyListLock.lockInterruptibly();
        try {
          if (!dirtyList.isEmpty()) throw new AssertionError();
        } finally {
          dirtyListLock.unlock();
        }
        if (compactingCacheRef.get() != null) throw new AssertionError();

        // ensure cleanList is not empty after WriteTask terminates, handling single buffer case
        cleanListLock.lockInterruptibly();
        try {
          if (writeBuffers.length > 1 && cleanList.isEmpty()) throw new AssertionError();
        } finally {
          cleanListLock.unlock();
        }
      }
  ...
  public void close() { // throws InterruptedException {
     ...
     */
    if (firstCause.compareAndSet(
        null /* expect */, new AsynchronousCloseException() /* update */)) {
      halt = true;
    }
   ...
   *
   * <p>Note: {@link #read(long)} DOES NOT throw an exception if the service is closed,
   * asynchronously closed, or even just plain dead. It just returns <code>null</code> to indicate
   * that the desired record is not available from the cache.
   *
   * @throws IllegalStateException if the service is closed.
   * @throws RuntimeException if the {@link WriteTask} has failed.
   */
  private void assertOpenForWriter() {

    if (!open.get()) throw new IllegalStateException(firstCause.get());
   ...
   * @return The buffer.
   * @throws InterruptedException
   * @throws IllegalStateException if the {@link WriteCacheService} is closed.
   * @throws RuntimeException if the service has been {@link #halt halted}
   */
   ...
   * #current} buffer is moved to the {@link #dirtyList} while holding the {@link WriteLock} and
   * flush() then waits until the dirtyList becomes empty, at which point all dirty records have
   * been written through to the backing file.
   *
   * <p>Note: Any exception thrown from this method MUST trigger error handling resulting in a
   * high-level abort() and {@link #reset()} of the {@link WriteCacheService}.
   *
   * <p>TODO flush() is currently designed to block concurrent writes() in order to give us clean
   * decision boundaries for the HA write pipeline and also to simplify the internal locking design.
   * Once we get HA worked out cleanly we should explore whether or not we can relax this constraint
   * such that writes can run concurrently with flush(). That would have somewhat higher throughput
   * since mutable B+Tree evictions would no longer cause concurrent tasks to block during the
   * commit protocol or the file extent protocol. [Perhaps by associating each write set with a
   * distinct sequence counter (that is incremented by both commit and abort)?]
   *
   * <p>TODO Flush should order ALL {@link WriteCache}'s on the dirtyList by their fileOffset and
   * then evict them in that order. This reordering will maximize the opportunity for locality
   * during the IOs. With a large write cache (multiple GBs) this reordering could substantially
   * reduce the IOWait associated with flush() for a large update. Note: The reordering should only
   * be performed by the leader in HA mode - the followers will receive the {@link WriteCache}
   * blocks in the desired order and can just drop them onto the dirtyList.
   *
   * @see WriteTask
   * @see #dirtyList
   * @see #dirtyListEmpty
   */
  public boolean flush(final boolean force, final long timeout, final TimeUnit units)
    ...
    final WriteLock writeLock = lock.writeLock();
    if (!writeLock.tryLock(remaining, TimeUnit.NANOSECONDS)) throw new TimeoutException();
    try {
      ...
      // remaining := (total - elapsed).
      remaining = nanos - (System.nanoTime() - begin);
      if (!cleanListLock.tryLock(remaining, TimeUnit.NANOSECONDS)) throw new TimeoutException();
      try {
        ...
        current.set(nxt);
        if (haLog.isInfoEnabled()) haLog.info("Flushed the write cache: seq=" + cacheSequence);
        return true;
      } finally {
        cleanListLock.unlock();
      }
   ...
   * use the value read from the {@link WriteCache} message to adjust the extent of its backing
   * file.
   *
   * <p>Note: Changes in the file extent for persistence store implementations MUST (a) be mutually
   * exclusive with reads and writes on the backing file (due to a JVM bug); and (b) force the file
   * data and the file metadata to the disk. Thus any change in the {@link #fileExtent} MUST be
   * followed by a {@link #flush(boolean, long, TimeUnit)}.
   *
   * <p>Note: You MUST set the file extent each time you invoke {@link #reset()} so the {@link
   * WriteCacheService} is always aware of the correct file extent.
   *
   * @throws InterruptedException
   * @throws IllegalStateException
   */
  public void setExtent(final long fileExtent) throws IllegalStateException {
    ...
    //
    //        try {
    if (log.isDebugEnabled()) log.debug("Set fileExtent: " + fileExtent);
   ...
   * are full. This method does not impose synchronization on writes which fit the capacity of a
   * cache buffer.
   *
   * <p>When integrating with the {@link RWStrategy} or the {@link WORMStrategy} there needs to be a
   * read/write lock such that file extension is mutually exclusive with file read/write operations
   * (due to a Sun bug). The caller can override {@link #newWriteCache(ByteBuffer, IReopenChannel)}
   * to acquire the necessary lock (the read lock of a {@link ReadWriteLock}). This is even true
   * when the record is too large for the cache since we delegate the write to a temporary {@link
   * WriteCache} wrapping the caller's buffer.
   *
   * <p>Note: Any exception thrown from this method MUST trigger error handling resulting in a
   * high-level abort() and {@link #reset()} of the {@link WriteCacheService}.
   *
   * @param latchedAddr The latched address (RWStore only).
   * @return <code>true</code> since the record is always accepted by the {@link WriteCacheService}
   *     (unless an exception is thrown).
   * @see http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6371642
   * @todo The WORM serializes invocations on this method because it must put each record at a
   *     specific offset into the user extent of the file. However, the RW store does not do this.
   *     Therefore, for the RW store only, we could use a queue with lost cost access and scan for
   *     best fit packing into the write cache buffer. When a new buffer is set as [current], we
   *     could pack the larger records in the queue onto that buffer first. This might provide
   *     better throughput for the RW store but would require an override of this method specific to
   *     that implementation.
  ...
  private boolean write_timed(
    ...
    if (remaining == 0) throw new IllegalArgumentException(AbstractBufferStrategy.ERR_BUFFER_EMPTY);
     ...
     */
    {
      ...
      try {
        ...
        if (cache.write(offset, data, chk, useChecksum, latchedAddr)) {
          ...
          if (old == cache) {
            throw new AssertionError(
                "Record already in cache: offset=" + offset + " " + addrDebugInfo(offset));
          }
     ...
     */
    {
      ...
      try {
        ...
        try {
           ...
           */
          if (!current.compareAndSet(cache /* expect */, null /* update */)) {
            throw new AssertionError();
          }
          dirtyListLock.lockInterruptibly();
          try {
   ...
   * been written.
   *
   * <p>This implementation will write the record onto a sequence of {@link WriteCache} objects and
   * wait until all of those objects have been written through to the backing file and the optional
   * HA write pipeline. A checksum will be appended after the last chunk of the record. This
   * strategy works for the WORM since the bytes will be laid out in a contiguous region on the
   * disk.
   *
   * <p>Note: For the WORM, this code MUST NOT allow the writes to proceed out of order or the data
   * will not be laid out correctly on the disk !!!
   *
   * <p>Note: The RW store MUST NOT permit individual allocations whose size on the disk is greater
   * than the capacity of an individual {@link WriteCache} buffer (@todo Or is this Ok? Perhaps it
   * is if the RW store holds a lock across the write for a large record? Maybe if we also add a
   * low-level method for inserting an entry into the record map?)
   *
   * <p>Note: This method DOES NOT register the record with the shared {@link #serviceMap}. Since
   * the record spans multiple {@link WriteCache} objects it can not be directly recovered without
   * reading it from the backing file.
   *
   * <h2>Dialog on large records</h2>
   *
   * It seems to me that the RW store is designed to break up large records into multiple
   * allocations. If we constrain the size of the largest allocation slot on the RW store to be the
   * capacity of a WriteCache buffer (including the bytes for the checksum and other record level
   * metadata) then we do not have a problem with breaking up large records for it in the
   * WriteCacheService and it will automatically benefit from HA using the write replication logic.
   *
   * <p>The WORM does not have these limits on the allocation size, so it seems likely that breaking
   * it up across multiple WriteCache buffer instances would have to be done inside of the
   * WriteCacheService in order to prevent checksums from being interleaved with each WriteCache
   * worth of data it emits for a large record. We can't raise this out of the WriteCacheService
   * because the large record would not be replicated for HA.
   */
  protected boolean writeLargeRecord(
      final long offset, final ByteBuffer data, final int chk, final boolean useChecksum)
      throws InterruptedException, IllegalStateException {
    ...
    if (remaining < capacity) throw new AssertionError();
    ...
    try {
       ...
       */
      if (log.isTraceEnabled()) log.trace("FLUSHING LARGE RECORD");
   ...
   *
   * <p>Note: If there is buffer available on the {@link #cleanList} then this method can return
   * immediately. Otherwise, this method will block until a clean buffer becomes available.
   *
   * @return A clean buffer.
   * @throws InterruptedException
   * @throws IllegalMonitorStateException unless the current thread is holding the {@link WriteLock}
   *     for {@link #lock}.
   */
  private WriteCache moveBufferToDirtyList() throws InterruptedException {

    if (!lock.isWriteLockedByCurrentThread()) throw new IllegalMonitorStateException();
   ...
   * cleanList. Potentially removing latency on acquiring a new cache from the clean list.
   *
   * <p>If a readCache is in operation then we will transfer to the read cache
   */
  ...
  public boolean installReads(final WriteCache cache) throws InterruptedException {
    if (readListSize == 0) return false;
    ...
    synchronized (readCache) {
      ...
      if (!WriteCache.transferTo(cache, rcache, serviceMap, 0)) {
        ...
        if (!WriteCache.transferTo(cache, ncache, serviceMap, 0)) {
          throw new AssertionError(
              "Unable to complete transfer to new cache with remaining: " + ncache.remaining());
        }
        ...
        while (tmp != null) {
          if (log.isDebugEnabled() && !tmp.isEmpty()) {
            ...
            while (values.hasNext()) {
              ...
              if (md.getHitCount() > 0) {
                hitRecords++;
                if (md.getHitCount() > hotCacheThreshold) hotRecords++;
              }
            }
            log.debug(
                "Recycled ReadCache, hot(>"
                    + hotCacheThreshold
                    + "): "
                    + hotRecords
                    + ", hit: "
          ...
          if (log.isDebugEnabled()) log.debug("Cycle HOTCACHE: " + ++cycles);
          ...
          if (!hotReserve.isEmpty()) throw new AssertionError();
   ...
   * already closed NOR if there is an asynchronous close of the service. Instead it just returns
   * <code>null</code> to indicate a cache miss.
   */
  public ByteBuffer read(final long offset, final int nbytes)
    ...
    if (tmp != null) {
      ...
      if (tmp.remaining() == 0) throw new AssertionError();
  ...
  private static class LoadRecordRequest {
    ...
    public LoadRecordRequest(final WriteCacheService service, final long offset, final int nbytes) {
    ...
    public boolean equals(final Object o) {
      ...
      if (!(o instanceof LoadRecordRequest)) return false;
  ...
  private static class ReadMemoizer
      extends Memoizer<LoadRecordRequest /* request */, ByteBuffer /* child */> {
  ...
  private ByteBuffer _getRecord(final long offset, final int nbytes)
    ...
    try {
      ...
      if (willInstall && theCache.decrementReferenceCount() == 0) {
        ...
        if (theCache == readCache.get()) throw new AssertionError();
      }
    }
  }
  ...
  private final ByteBuffer _readFromLocalDiskIntoNewHeapByteBuffer(
    ...
    final ByteBuffer ret = reader.readRaw(offset, ByteBuffer.allocate(nbytes));

    final int chk =
        ChecksumUtility.getCHK()
            .checksum(ret.array(), 0 /* offset */, nbytes - 4 /* len */); // read checksum

    final int tstchk = ret.getInt(nbytes - 4);

    if (chk != tstchk)
      throw new ChecksumError(
          "offset=" + offset + ",nbytes=" + nbytes + ",expected=" + tstchk + ",actual=" + chk);

    ret.limit(nbytes - 4);

    if (ret.remaining() == 0) throw new AssertionError();
   ...
   *
   * <p>If dirty {@link WriteCache}s are flushed in order then it does not matter, however, if we
   * want to be able to combine {@link WriteCache}s then it makes sense that there are no duplicate
   * writes.
   *
   * <p>On reflection this is more likely needed since for the {@link RWStore}, depending on session
   * parameters, the same cached area could be overwritten. We could still maintain multiple writes
   * but we need a guarantee of order when retrieving data from the write cache (newest first).
   *
   * <p>So the question is, whether it is better to keep cache consistent or to constrain with read
   * order?
   *
   * @param offset the address to check
   */
      ...
      while (true) {
        ...
        try {
          ...
          if (oldValue == null) {
             ...
             *
             * @see <a href= "https://sourceforge.net/apps/trac/bigdata/ticket/654" Rare
             *     AssertionError in WriteCache.clearAddrMap() </a>
             */
            continue;
          }
          if (oldValue != cache) {
   ...
   *
   * <p>Write errors often show up as a checksum error, so the length of data written to the address
   * cab be crucial information in determining the root of any problem.
   *
   ...
   * cache is checked.
   *
   * <p>Note: Unless the caller is holding an appropriate lock across this operation, the result is
   * NOT guaranteed to be correct at any time other than the moment when the cache was tested.
   */
  ...
  /** Note: Atomic reference is used so the counters may be imposed from outside. */
