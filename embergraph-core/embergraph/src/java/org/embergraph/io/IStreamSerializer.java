...
public interface IStreamSerializer<T> extends Serializable {
   ...
   * may include serializer state as well, such as dictionary that will be used on the other end to
   * decode the object. In such cases the serializer needs to know how to de-serialize both the
   * dictionary and the data. Stateful serializers such as <em>extSer</em> must encapsulate all
   * requisite state on the output stream.
   *
   * @param out The stream onto which the object's state will be written.
