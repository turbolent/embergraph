 ...
 * with the InputStream to determine which data to write to the OutputStream, alternately
 * transferring bytes from the InputStream and the ISnapshotData.
 *
...
public class MergeStreamWithSnapshotData {
  ...
  public static void process(
      final InputStream in, final ISnapshotData snapshotData, final OutputStream out)
      throws IOException {
    ...
    while (bbs.hasNext()) {
      ...
      if (log.isInfoEnabled()) log.info("Write ByteBuffer at " + pos + ", length: " + blen);
  ...
  private static void transfer(
      final InputStream in, final long len, final byte[] buf, final OutputStream out)
      throws IOException {
    long rem = len;
    while (rem > buf.length) {
      int rdlen = in.read(buf);
      if (rdlen == -1) return; // eof
