...
public class FileChannelUtility {
   ...
   *
   * <p>This handles partial reads by reading until nbytes bytes have been transferred.
   *
   * @param channel The source {@link FileChannel}.
   * @param src A {@link ByteBuffer} into which the data will be read.
   * @param pos The offset from which the data will be read.
   * @return The #of disk read operations that were required.
   * @throws IOException
   * @throws IllegalArgumentException if any parameter is <code>null</code>.
   * @throws IllegalArgumentException if <i>pos</i> is negative.
   * @throws IllegalArgumentException if <i>dst</i> does not have any bytes remaining.
   * @todo This might need to use a static direct buffer pool to avoid leaking temporary direct
   *     buffers since Java may attempt to allocate a "temporary" direct buffer if [dst] is not
   *     already a direct buffer.
   * @deprecated by {@link #readAll(IReopenChannel, ByteBuffer, long)} which handles transparent
   *     re-opening of the store in order to complete the read operation.
   */
  public static int readAll(final FileChannel channel, final ByteBuffer src, final long pos)
      throws IOException {
   ...
   * <p>This handles partial reads by reading until nbytes bytes have been transferred.
   *
   * @param opener An object which knows how to re-open the {@link FileChannel} and knows when the
  ...
  public static int readAll(
      final IReopenChannel<FileChannel> opener, final ByteBuffer src, final long pos)
      throws IOException {
    ...
    while (count < nbytes) {
      ...
      if (nread == -1)
        throw new IOException(
            "EOF reading on channel: remaining="
                + (nbytes - count)
                + ", nread="
                + nreads
                + ", pos="
  ...
  public static class AsyncTransfer {
     ...
     * and failed, then the future is cleared and the operation is rescheduled. This is done in
     * order to allow us to complete a high level read on the channel when the backing channel may
     * have been closed by an interrupt in another thread due to Java IO channel semantics (e.g.,
     * driven by query termination during reads).
     *
    ...
    private void read(final AsynchronousFileChannel channel)
        throws IllegalArgumentException, NonReadableChannelException, CancellationException,
      ...
      if (!isDone()) {
        ...
        m_fut =
            channel.read(
                m_buffer, m_addr); // throws IllegalArgumentException, NonReadableChannelException
      }
    }
    ...
    public void cancel() {
      if (m_fut != null) m_fut.cancel(true /* mayInterruptIfRunning */);
    }
   ...
   * @throws IOException
   */
  public static long readAllAsync(final IAsyncOpener opener, final List<AsyncTransfer> transfers)
      throws IOException {
    int ntries = 0;
    ...
    try {
      while (true) {
        ...
        if (ntries == 100) {
          ...
          log.warn(
              "reading on channel: remaining="
                  + (totalBytesExpected - totalBytesRead)
                  + ", ntransfers="
                  + transfers.size()
                  + ", ntries="
                  + ntries
        ...
        try {
          ...
          for (AsyncTransfer transfer : transfers) {
            totalBytesRead +=
                transfer.complete(); // throws InterruptedException, ExecutionException.
          }
   ...
   *
   * <p>Note: I have seen count != remaining() for a single invocation of FileChannel#write(). This
   * occurred 5 hours into a run with the write cache disabled (so lots of small record writes). All
   * of a sudden, several writes wound up reporting too few bytes written - this persisted until the
   * end of the run (Fedora core 6 with Sun JDK 1.6.0_03). I have since modified this code to use a
   * loop to ensure that all bytes get written.
   *
   * @return The #of disk write operations that were required.
   * @todo This might need to use a static direct buffer pool to avoid leaking temporary direct
   *     buffers since Java probably will attempt to allocate a "temporary" direct buffer if [data]
   *     is not already a direct buffer. There is logic in {@link TemporaryRawStore} for handling
   *     the overflow from a heap buffer onto the disk which already does this.
   * @deprecated by {@link #writeAll(IReopenChannel, ByteBuffer, long)}
   */
  public static int writeAll(final FileChannel channel, final ByteBuffer data, final long pos)
      throws IOException {
   ...
   * thread closed then it will be re-opened and the write will continue.
   *
   * <p>Note: I have seen count != remaining() for a single invocation of FileChannel#write(). This
   * occurred 5 hours into a run with the write cache disabled (so lots of small record writes). All
   * of a sudden, several writes wound up reporting too few bytes written - this persisted until the
  ...
  public static int writeAll(
      final IReopenChannel<FileChannel> opener, final ByteBuffer data, final long pos)
      throws IOException {
    ...
    if (count != nbytes) {
      ...
      throw new RuntimeException(
          "Expecting to write " + nbytes + " bytes, but wrote " + count + " bytes in " + nwrites);
    }
   ...
   * will be the positioned after the last byte transferred (this is done by {@link
   * FileChannel#transferFrom(java.nio.channels.ReadableByteChannel, long, long)} so we have no
   * choice about that). The position on the <i>out</i> file is updated by this method and will be
   * located after the last byte transferred (this is done since we may have to extend the output
   * file in which case its position will have been changed anyway so this makes the API more
   * uniform).
   *
   * @param sourceChannel The source {@link FileChannel}.
   * @param fromPosition The first byte to transfer from the <i>sourceChannel</i>.
   * @param count The #of bytes to transfer.
   * @param out The output file.
   * @param toPosition The data will be transferred onto the output channel starting at this
   *     position.
   ...
   *     > FileChannel.transferTo() doesn't work if address space runs out </a>
   * @todo as a workaround, detect the Windows platform and do the IO ourselves when the #of bytes
   *     to transfer exceeds 500M.
   * @todo does not handle asynchronous closes due to interrupts in other threads during a
   *     concurrent NIO operation on the same source/dest file channel (I have never seen this
   *     problem and perhaps the NIO API excludes it).
   * @todo I have seen this exception when the JVM was allocated to much of the total RAM on the
   *     machine (in this case 2600m out of 3G available with 4G installed). The workaround is to
   *     reduce the -Xmx parameter value. This could also be corrected by failing over to a copy
   *     based on normal file IO.
   *     <pre>
  ...
  public static int transferAll(
    ...
    if (INFO)
      log.info(
          "Transferred "
              + count
              + " bytes from disk channel at offset "
              + fromPosition
              + " to disk channel at offset="
              + toPosition
              + " in "
              + nwrites
              + " writes and "
  ...
  public static class ReopenerInputStream extends InputStream {
    ...
    public int read(final byte[] dst, final int off, final int len) throws IOException {
      ...
      if (ret > 0) {
        ...
        if (log.isTraceEnabled()) log.trace("Request for " + len + " bytes");
