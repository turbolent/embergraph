 ...
 * IDataRecord#len() length} is automatically extended iff the backing buffer is extended.
 *
 * <p>Note: This class implements {@link OutputStream} so that it may be wrapped by an {@link
 * OutputBitStream}. Likewise it implements {@link RepositionableStream} so that you can rewind the
 * {@link OutputBitStream}.
 *
 * <p>Note: The concept of {@link #limit()} and {@link #capacity()} are slightly different than for
 * {@link ByteBuffer} since the limit and the capacity may be transparently extended on write.
 * However, neither the limit nor the capacity will be extended on read (this prevents you from
 * reading bad data). The relative <i>put</i> operations always set the limit to the position as a
 * post-condition of the operation. The absolute get/put operations ignore the limit entirely.
 *
 * <p>This class is NOT thread-safe for mutation. Not only is the underlying {@link
 * FixedByteArrayBuffer} not thread-safe for mutation, but the operation which replaces the {@link
 * #slice} when the capacity of the backing buffer must be extended is not atomic.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 */
public class ByteArrayBuffer extends OutputStream
    implements IByteArrayBuffer, RepositionableStream, LongPacker.IByteBuffer {
   ...
   * @param v The value.
   * @return The value.
   * @exception IllegalArgumentException unless the value is non-negative.
   */
   ...
   */
  public final void ensureFree(final int pos, final int len) {
   ...
   *     current position, which would be somewhat faster.
   */
   ...
   * @todo this does not need to be final. also, caller's could set the policy including a policy
   *     that refuses to extend the capacity.
   */
  protected int extend(final int required) {

    final int capacity = Math.max(required, capacity() * 2);

    if (log.isInfoEnabled()) log.info("Extending buffer to capacity=" + capacity + " bytes.");
   ...
   * fit byte[].
   *
   * <p>Note: A {@link #slice(int, int)} with a view of <i>this</i> {@link ByteArrayBuffer} will
   * continue to have a view onto the backing buffer associated with <i>this</i> instance. This
   * works because {@link SliceImpl#array()} delegates to the outer {@link ByteArrayBuffer#array()}
   * method. This makes it possible to {@link #trim()} a {@link ByteArrayBuffer} on which you have
   * {@link #slice(int, int)}s, while maintaining the validity of those slices.
   *
   * @return The old byte[].
   */
  ...
  public final void put(final int pos, final byte[] b, final int off, final int len) {
  ...
  public final void putByte(int pos, final byte v) {
  ...
  public final void putInt(int pos, final int v) {
    ...
    if (pos + 4 > buf.length) ensureCapacity(pos + 4);
  ...
  public final void putFloat(int pos, final float f) {
  ...
  public final void putLong(int pos, final long v) {
    ...
    if (pos + 8 > buf.length) ensureCapacity(pos + 8);
  ...
  public final void putDouble(int pos, final double d) {
   ...
   * @see #asByteBuffer()
   * @todo this returns the data in [0:pos], which is essentially the data written on the buffer
   *     using relative put operations but without requiring a {@link #flip()} to set the pos to
   *     zero while leaving the read limit alone. that is at odds with the rest of the relative api
   *     but there is a lot of use of this method already. perhaps it can be moved to the {@link
   *     DataOutputBuffer} where it would share semantics with {@link
   ...
   * buffer.
   *
   * @todo review absolute writes on the buffer. they are the underlying write operation in all
   *     cases, right?
   */
  int limit;
   ...
   * as required.
   *
   * <p>Note: The buffer reference is used directly rather than making a copy of the data.
   *
   * @param pos The initial {@link #pos() position}.
   * @param limit The initial {@link #limit()}.
   * @param buf The backing byte[].
   */
  public ByteArrayBuffer(final int pos, final int limit, final byte[] buf) {
    ...
    if (limit > buf.length) throw new IllegalArgumentException("limit>buf.length");
   ...
   * @throws IllegalArgumentException if <i>pos</i> is less than ZERO (0).
   * @throws IllegalArgumentException if <i>pos</i> is greater than or equal to the current capacity
   *     of the buffer.
   */
  public final int pos(final int pos) {

    if (pos < 0 || pos >= buf.length)
      throw new IllegalArgumentException("pos=" + pos + ", capacity=" + buf.length);
  ...
  public final void ensureFree(final int len) {
   ...
   */
  public final int copy(final ByteBuffer src) {
   ...
   */
  public final int copyRest(final ByteArrayBuffer src) {
   ...
   */
  public final int copyAll(final ByteArrayBuffer src) {
   ...
   *     data will be overwritten if {@link #reset()} is invoked followed by any operations that
   *     write on the buffer.
   */
   ...
   *
   * @param len The #of bytes to advance (non-negative).
   */
  public final void advancePosAndLimit(final int len) {
   ...
   */
  public final void put(final byte[] b, final int off, final int len) {
   ...
   */
  public final void putByte(final byte v) {
   ...
   *
   * @return The value.
   * @exception IndexOutOfBoundsException if the position is greater than or equal to the limit.
   * @todo rename as get(byte) to conform with {@link ByteBuffer}?
   */
  public final byte getByte() {

    if (pos >= limit) throw new IndexOutOfBoundsException();
  ...
  public final short getShort() {
    ...
    if (pos + 2 >= limit) throw new IndexOutOfBoundsException();
  ...
  public final int getInt() {
    ...
    if (pos + 4 >= limit) throw new IndexOutOfBoundsException();
  ...
  public final long getLong() {
    ...
    if (pos + 8 >= limit) throw new IndexOutOfBoundsException();
   ...
   */
  public final int packLong(final long v) {
   ...
   */
  public final int packShort(final short v) {
  ...
  public final boolean getBit(final long bitIndex) {
    ...
    if (byteIndexForBit > buf.length) ensureCapacity(byteIndexForBit);
  ...
  public int skip(final int nbytes) {
    ...
    if (pos + nbytes < 0) throw new IllegalArgumentException();

    if (pos + nbytes > capacity()) ensureCapacity(pos + nbytes);
   ...
   *
   * @param off The starting offset into the backing buffer of the slice.
   * @param len The length of that slice.
   * @return The slice.
   */
   ...
   * @todo Add OBS(byte[],off,len) ctor for faster operations. However, you MUST pre-extend the
   *     buffer to have sufficient capacity since an {@link OutputBitStream} wrapping a byte[] will
   *     not auto-extend.
   */
