...
public class QCE implements QuorumStateChangeEvent {
  ...
  public QCE(
      final QuorumStateChangeEventEnum eventType,
    ...
    switch (eventType) {
      case PIPELINE_CHANGE:
        if (downstreamOldAndNew == null) throw new IllegalArgumentException();
        if (downstreamOldAndNew.length != 2) throw new IllegalArgumentException();
        break;
      case CONSENSUS:
        if (lastCommitTimeConsensus == null) throw new IllegalArgumentException();
        break;
      case QUORUM_MEET:
  ...
  public String toString() {
    return "QuorumStateChangeEvent"
        + "{type="
        + eventType
        + (eventType == QuorumStateChangeEventEnum.PIPELINE_CHANGE
            ? ",oldDownstreamId="
                + downstreamOldAndNew[0]
                + ",newDownstreamId="
                + downstreamOldAndNew[1]
            : "")
        + (eventType == QuorumStateChangeEventEnum.CONSENSUS
            ? ",lastCommitTime=" + lastCommitTimeConsensus
            : "")
        + (eventType == QuorumStateChangeEventEnum.QUORUM_MEET
            ? ",token=" + token + ",leaderId=" + leaderId
            : "")
        + "}";
  }
}
