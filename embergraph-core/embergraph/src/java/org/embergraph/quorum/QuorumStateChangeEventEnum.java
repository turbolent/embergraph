...
public enum QuorumStateChangeEventEnum {
  ...
  PIPELINE_REMOVE,
  /*
   * Event generated when a service already in the pipeline becomes the first service in the write
   * pipeline because all previous services in the pipeline order have been removed from the
   * pipeline.
   *
  ...
  /*
   * Event generated when the downstream service in the write pipeline has changed. Services always
   * enter at the end of the write pipeline, but may be removed at any position in the write
   * pipeline.
   *
  ...
  PIPELINE_CHANGE,
  /*
   * Event generated when the upstream service in the write pipeline has been removed. This hook
   * provides an opportunity for the service to close out its connection with the old upstream
  ...
  /*
   * Event generated when a consensus has been achieved among <code>(k+1)/2</code> services
   * concerning a shared lastCommitTime (really, this is not a consensus but a simple majority).
   * This message is sent to each member service regardless of whether or not they participated in
   * the consensus.
   *
  ...
  CONSENSUS,
  /*
   * Event generated when the consensus is lost. Services do not withdraw their cast votes until a
   * quorum breaks and a new consensus needs to be established.
   *
