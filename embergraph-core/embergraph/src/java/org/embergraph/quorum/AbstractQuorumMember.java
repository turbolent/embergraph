 ...
 * @version $Id$
 */
public abstract class AbstractQuorumMember<S extends Remote> extends AbstractQuorumClient<S>
    implements QuorumMember<S> {

  private final UUID serviceId;

  protected AbstractQuorumMember(final String logicalServiceId, final UUID serviceId) {
  ...
  public boolean isMember() {
    ...
    for (UUID t : a) {
      if (serviceId.equals(t)) return true;
    }
    return false;
  }
   ...
   *     test is valid as of the same met quorum.
   *     <p>Right now this is only used by unit tests. Maybe the method should go away?
   */
  @Override
