 ...
 * the {@link QuorumMember} will issue various commands to its {@link QuorumActor}.
 *
 * <p>This interface defines a collection of messages which are used to inform the {@link
 * QuorumMember} of significant changes in the distributed quorum state as it pertains to
 * <i>this</i> quorum member. See {@link QuorumStateChangeListener}.
 *
...
public interface QuorumMember<S extends Remote> extends QuorumClient<S>, QuorumStateChangeListener {
   ...
   * non-remote interface. These interfaces typically define similar methods (for example, compare
   * {@link org.embergraph.ha.HACommitGlue}, which is {@link Remote}, with {@link QuorumCommit},
   * which is non-remote). The {@link Remote} interface in turn will have a local implementation
   * object inside of the JVM and an exported proxy for that {@link Remote} interface. This method
   * returns the <em>local</em> implementation object for the {@link Remote} interface and is
   * intended to facilitate operations which the {@link QuorumMember} executes against its own
   * {@link Remote} interface. While the {@link Remote} interface will be exposed to other services
   * using a proxy, how that happens is outside of the scope of this method.
   */
  S getService();
   ...
   * will notice when it enters and leaves the pipeline and when the downstream service for this
   * service changes in the quorum.
   */
   ...
   * return <code>true</code> unless the quorum is met.
   *
   * @param token The quorum token for which the request was made.
   */
   ...
   * @throws QuorumException if the caller's token is not the current quorum token.
   * @throws QuorumException if this is not the quorum leader.
   */
   ...
   * pipeline.
   *
   * @return The UUID of the downstream service in the write pipeline. This will return <code>null
   *     </code> for services which are not part of the write pipeline, for the leader, and for the
   *     last service in the write pipeline.
