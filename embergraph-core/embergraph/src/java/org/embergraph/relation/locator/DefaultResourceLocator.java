 ...
 * </pre>
 *
 * <p>A relation is located using the {@link IIndexStore#getGlobalRowStore(long)} and materialized
 * by supplying an {@link IIndexManager} that will be able to resolve the indices for the relation's
 * view. Several different contexts are handled:
 *
 * <dl>
 *   <dt>{@link IEmbergraphFederation}
 *   <dd>The {@link IRelation} will be resolved using the {@link
 *       IEmbergraphFederation#getGlobalRowStore(long)} and the {@link IEmbergraphFederation} as its
 *       {@link IIndexManager}. The makes access to a remote and potentially distributed {@link
 *       IIndex} transparent to the {@link IRelation}. However, it is NOT possible to resolve local
 *       resources on other JVMs - only scale-out indices registered against the {@link
 *       IEmbergraphFederation}.
 *   <dt>{@link Journal}
 *   <dd>The {@link IRelation} will be resolved using the {@link Journal#getGlobalRowStore(long)}
 *       and will use the local index objects directly.
 *   <dt>{@link AbstractTask}
 *   <dd>If the index is local and monolithic then you can declare the index to the {@link
 *       AbstractTask} and <em>override</em> the locator to use {@link AbstractTask#getJournal()} as
 *       its index manager. This will give you access to the local index object from within the
 *       concurrency control mechanism
 *   <dt>{@link TemporaryStore}
 *   <dd>When used by itself, this is just like a {@link Journal}. However, {@link TemporaryStore}s
 *       are also used to provide local resources for more efficient data storage for a variety of
 *       purposes. When used in this manner, you must explicitly notify the locator of the existence
 *       of the {@link TemporaryStore} in order to be able to resolve {@link IRelation}s on the
 *       {@link TemporaryStore}. It is prudent to use a prefix for such local resources that
 *       guarentees uniqueness, e.g., an {@link UUID}
 * </dl>
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @param <T> The generic type of the [R]elation.
 */
public class DefaultResourceLocator<T extends ILocatableResource<T>>
   ...
   * GlobalRowStoreHelper}, items in this cache can not cause references to indices or other
   * persistence capable / high cost resources to remain strongly reachable.
   */
  ...
  public DefaultResourceLocator(
      final IIndexManager indexManager, final IResourceLocator<T> delegate) {
   ...
   * @param delegate Optional {@link IResourceLocator} to which unanswered requests are then
   *     delegated.
   * @param cacheCapacity The capacity of the internal weak value cache.
   * @param cacheTimeout The timeout in milliseconds for stale entries in that cache.
   */
  public DefaultResourceLocator(
    ...
    if (cacheCapacity <= 0) throw new IllegalArgumentException();
     ...
     */
    {
      final int propertyCacheCapacity = Math.max(cacheCapacity, cacheCapacity * 10);
  ...
  public T locate(final String namespace, final long timestamp) {
    ...
    if (resource != null) {
      ...
      if (log.isDebugEnabled()) log.debug("cache hit: " + resource);
    ...
    try {
      ...
      if (resource != null) {
        ...
        if (INFO) log.info("cache miss: namespace=" + namespace + ", timestamp=" + timestamp);
      ...
      if (resource == null) {
        ...
        if (INFO) log.info("Not found: " + nt);

      } else {

        if (INFO) log.info("Caching: " + nt + " as " + resource);
   ...
   * @return The resolved resource -or- <code>null</code> if the resource could not be resolved for
   *     that namespace and timestamp.
   */
   ...
   * @param timestamp
   * @param foundOn Used to pass back the {@link IIndexManager} on which the resource was found as a
   *     side-effect.
   * @return The properties for that resource.
   */
  protected Properties locateResource(
      final String namespace, final long timestamp, final AtomicReference<IIndexManager> foundOn) {
    ...
    synchronized (seeAlso) { // FIXME Probably a read/write lock since [seeAlso] normally empty.
       ...
       * Note: Asynchronous close is common for temporary
            ...
            if (INFO) log.info("Closed? " + indexManager);
    ...
    if (properties != null) {
      ...
      if (INFO) {
        ...
        log.info("Found: namespace=" + namespace + " on " + indexManager);
      }
   ...
   *
   * <p>Note: A <code>null</code> return is an indication that the resource was NOT FOUND on this
   * {@link IIndexManager} and the caller SHOULD try another {@link IIndexManager}.
   *
   * @param indexManager
   * @param namespace The resource identifier - this is the primary key.
   * @param timestamp The timestamp of the resource view.
   * @return The {@link Properties} iff there is a logical row for the given namespace.
   */
  protected Properties locateResourceOn(
      final IIndexManager indexManager, final String namespace, final long timestamp) {

    if (INFO) {

      log.info(
          "indexManager=" + indexManager + ", namespace=" + namespace + ", timestamp=" + timestamp);
    }
    ...
    if (TimestampUtility.isReadOnly(timestamp) && !TimestampUtility.isReadCommitted(timestamp)) {
       ...
       * Note: Using the local transaction manager to resolve a
        ...
        final ITx tx = journal.getLocalTransactionManager().getTx(timestamp);
        ...
        if (tx != null) {
          ...
          final ICommitRecord commitRecord =
              journal.getCommitRecord(TimestampUtility.asHistoricalRead(timestamp));
       ...
       * Property cache miss.
        ...
        final SparseRowStore rowStore = indexManager.getGlobalRowStore(readTime);
        ...
        try {
          ...
          throw new RuntimeException(
              ex.getMessage()
                  ...
                  + ", readTime="
                  + TimestampUtility.toString(readTime),
              ex);
        }
       ...
       * TODO This should use the readsOnCommitTime on the cluster
        ...
        throw new AssertionError("timestamp=" + TimestampUtility.toString(timestamp));
      }
   ...
   *     for the relation.
   * @param nt The namespace and timestamp for the view of the relation.
   * @param properties Configuration properties for the relation.
  ...
  protected T newInstance(
    ...
    if (nt == null) throw new IllegalArgumentException();
   ...
   *
   * <p>Note: This is done automatically by {@link AbstractResource}.
   *
  ...
  public T putInstance(final T instance) {
    ...
    try {
      ...
      if (INFO) {
        ...
        log.info(
            "Instance added to cache: nt="
                + nt
                + ", instance="
                + instance
                + ", indexManager="
                + (instance instanceof AbstractResource
                    ? ((AbstractResource<?>) instance).getIndexManager()
                    : "n/a"));
      }
  ...
  @Override
  public void discard(final ILocatableResource<T> instance, final boolean destroyed) {
  ...
  public void clearUnisolatedCache() {
    ...
    {
      final Iterator<Map.Entry<NT, WeakReference<Map<String, Object>>>> itr =
          propertyCache.entryIterator();
      while (itr.hasNext()) {
   ...
   * asynchronously then a warning will be logged until its reference is cleared.
   *
   * <p>Note: The default {@link IIndexManager} specified to the ctor normally ensures that the
   * global namespace is consistent (it is not possible to have two indices or {@link
   * ILocatableResource}s with the same name). When you add additional {@link IIndexManager}s, the
   * opportunity for an <em>inconsistent</em> unified namespace is introduced. You can protect
   * yourself by ensuring that resources located on {@link TemporaryStore}s and the like are always
   * created with a unique prefix, e.g., the {@link UUID} of the {@link TemporaryStore} itself or a
   * {@link UUID} for each container that is allocated on the {@link TemporaryStore}.
   *
   * @param indexManager Typically a {@link TemporaryStore} or {@link Journal} containing local
   *     resources.
  ...
  public void add(final IIndexManager indexManager) {
    ...
    synchronized (seeAlso) {
      ...
      if (INFO) {
        ...
        log.info("size=" + seeAlso.size() + ", added indexManager=" + indexManager);
      }
    }
  }
