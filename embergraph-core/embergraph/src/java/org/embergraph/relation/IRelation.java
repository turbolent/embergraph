 ...
 * @version $Id$
 * @param E The generic type for the elements in the relation.
 */
public interface IRelation<E> extends ILocatableResource<IRelation<E>> {
   ...
   * into the relation.
   *
   * @param args An ordered list of variables and/or constants.
   * @param bindingSet A set of bindings.
   * @return The new element.
   * @throws IllegalArgumentException if any parameter is <code>null</code>.
   * @throws IllegalArgumentException if any of <i>a</i> is neither an {@link IVariable} nor an
   *     {@link IPredicate}.
   * @throws IllegalStateException if there exists a variable which is not bound given those
   *     bindings.
   */
   ...
   * @deprecated by {@link #getKeyOrders()}. However, note that some odd ball cases exist such as
   *     the free text index of the {@link LexiconRelation} and the justifications index of the
   *     {@link SPORelation}. These are currently enumerated by {@link #getIndexNames()} but they
   *     are not enumerated by {@link #getKeyOrders()} since they are not a simple transform of the
   *     original relation.
   */
   ...
   * @return The {@link IKeyOrder} for the perfect access path -or- <code>null</code> if there is no
   *     index which provides a perfect access path for that predicate.
   * @todo What about "best" versus "perfect"? Perfect is more a concept from RDF with covering
   *     indices. For other schemas we will often just have "best". If you only have one index then
   *     it is always "best". However, in order to use the "best" access path we must impose the
   *     constraint on the {@link IAccessPath} that it filter out any elements visited during the
   *     scan which do not satisfy the places where bound variables or constants appear in the
   *     predicate.
   *     <p>Range count estimates will report all rows to be scanned. Exact range counts will
   *     perform the scan, applying any filters.
   */
   ...
   * variables.
   *
   * <p>If there is an {@link IIndex} that directly corresponds to the natural order implied by the
   * variable pattern on the predicate then the access path should use that index. Otherwise you
   * should choose the best index given the constraints and make sure that the {@link IAccessPath}
   * incorporates additional filters that will allow you to filter out the irrelevant {@link
   * ITuple}s during the scan - this is very important when the index is remote!
   *
   * <p>If there are any {@link IElementFilter}s then the access path MUST incorporate those
   * constraints such that only elements that satisfy the constraints may be visited.
   *
   * <p>Whether the constraints arise because of the lack of a perfect index for the access path or
   * because they were explicitly specified for the {@link IPredicate}, those constraints should be
   * translated into constraints imposed on the underlying {@link ITupleIterator} and sent with it
   * to be evaluated local to the data.
   *
   ...
   * then. This can be accomplished by adding the filter as a constraint on the predicate when
   * specifying the access path.
   *
   * @param predicate The constraint on the elements to be visited.
   ...
   * @param predicate The predicate (required).
   ...
   *
   * Together, this method and {@link AccessPath} is responsible for interpreting the following
   * {@link IPredicate} {@link IPredicate#Annotations}:
   *
   * <ul>
   *   <li>{@link IPredicate.Annotations#ACCESS_PATH_EXPANDER}
   *   <li>
   * </ul>
   *
   * <h2>Access paths and a Standalone Database</h2>
   *
   * There is only one {@link IIndexManager} when running against a standalone database. That {@link
   * IIndexManager} is available from {@link IRelation#getIndexManager()}.
   *
   * <h2>Access Paths and Scale-Out Operators</h2>
   *
   * <p>Scale-out {@link BOp operators} which are {@link BOpEvaluationContext#SHARDED} or {@link
   * BOpEvaluationContext#HASHED} require access to the local index manager on the data service and
   * signal this using:
   *
   * <ul>
   *   <li>The {@link BOpEvaluationContext#SHARDED} or {@link BOpEvaluationContext#HASHED} (on a
   *       join); and
   *   <li>The {@link IPredicate.Annotations#PARTITION_ID} (for a shard-wise access path).
   * </ul>
   *
   * In order to service a request for a local access path in scale-out, the caller MUST specify the
   * <i>localIndexManager</i>.
   *
   * <p>Scale-out {@link BOp operators} MAY use remote access paths and signal this using:
   *
   * <ul>
   *   <li>The {@link BOpEvaluationContext#ANY} (on a join); and
   *   <li>Setting {@link IPredicate.Annotations#REMOTE_ACCESS_PATH} to <code>true</code>.
   * </ul>
   *
   * When a remote access path is required, the <i>localIndexManager</i> is ignored and {@link
   * IRelation#getIndexManager()} is used to obtain the {@link IEmbergraphFederation} from which the
   * {@link IRelation} definition was materialized.
   *
   * @param localIndexManager The local index manager (optional).
   * @param keyOrder Identifies which index to use (required).
   * @param predicate The predicate (required).
   * @return The {@link IAccessPath}.
   */
   ...
   *
   * @param keyOrder The natural index order.
   * @return The index -or- <code>null</code> iff the index does not exist as of the timestamp for
