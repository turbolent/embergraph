 ...
 * instantiated when an {@link IRelation} instance is created to operation on some relation's data.
 * The schema is freely extensible and may be used to store additional properties that are made
 * available to the {@link IRelation} when a new instance of that {@link Class} is instantiated. The
 * relation namespace keys are Unicode.
 *
...
public class RelationSchema extends Schema {
   ...
   *     <p>TODO This is a workaround for and should be removed when we replace the native long tx
   *     identifier with a thin interface.
   */
  public static final String COMMIT_TIME =
      (RelationSchema.class.getPackage().getName() + ".commitTime").intern();
