...
public class QueryTask extends AbstractStepTask {
  ...
  /** The {@link IBlockingBuffer} on which the {@link ISolution}s will be written. */
  private final IBlockingBuffer<ISolution[]> buffer;

  /** @param buffer The {@link IBlockingBuffer} on which the {@link ISolution}s will be written. */
  public QueryTask(
      IStep step,
  ...
  public RuleStats call() throws Exception {
    ...
    final List<Callable<RuleStats>> tasks = newQueryTasks(step, joinNexus, buffer);
   ...
   */
  protected RuleStats runTasks(final IJoinNexus joinNexus, final List<Callable<RuleStats>> tasks)
   ...
   * @param buffer The thread-safe buffer onto which the individual tasks emit chunks.
   * @return The list of tasks to run.
   */
  protected List<Callable<RuleStats>> newQueryTasks(
      IStep step, IJoinNexus joinNexus, IBlockingBuffer<ISolution[]> buffer) {

    if (log.isDebugEnabled()) log.debug("step=" + step.getName());
