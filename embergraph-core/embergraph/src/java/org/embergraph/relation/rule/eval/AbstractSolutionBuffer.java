 ...
 * @version $Id$
 * @param <R> The generic type of the [R]elation elements.
...
public abstract class AbstractSolutionBuffer<R> extends AbstractArrayBuffer<ISolution<R>[]> {
  ...
  /** @param capacity */
  protected AbstractSolutionBuffer(final int capacity, final IMutableRelation<R> relation) {
   ...
   *
   * <p>Note: Combining chunks together is a huge performance win if the source chunks tend to be
   * small (100). For example, this is true for scale-out joins such as LUBM U1 EDS. There the
   * performance gain is better than 20:1.
   */
  @Override
  protected final long flush(final int n, final ISolution<R>[][] a) {
   ...
   *
   * <p>Note: The {@link ISolution}s generally appear in an arbitrary order. They can either be
   * resolved to {@link IRelation} elements or be sorted into ordered {@link ISolution} chunks using
   * a {@link SolutionComparator}.
   *
   * @param itr The iterator (the order of the elements is unknown).
  ...
  public static class InsertSolutionBuffer<E> extends AbstractSolutionBuffer<E> {
    ...
    protected long flush(IChunkedOrderedIterator<ISolution<E>> itr) {
      ...
      final IChunkedOrderedIterator<E> itr2 =
          new ChunkedResolvingIterator<E, ISolution<E>>(itr) {
  ...
  public static class DeleteSolutionBuffer<E> extends AbstractSolutionBuffer<E> {
    ...
    protected long flush(IChunkedOrderedIterator<ISolution<E>> itr) {
      ...
      //                    itr)
      //                    .addFilter(new
      // org.embergraph.striterator.Resolver<Iterator<ISolution<E>>, ISolution<E>, E>() {
      //
      //                        protected E resolve(ISolution<E> e) {
      //
      //                            return e.get();
  ...
  //        protected long flush(IChunkedOrderedIterator<ISolution<E>> itr) {
  //
  //            final IChunkedOrderedIterator<E> itr2 = new ChunkedResolvingIterator<E,
  // ISolution<E>>(
  //                    itr) {
  //
  //                @Override
