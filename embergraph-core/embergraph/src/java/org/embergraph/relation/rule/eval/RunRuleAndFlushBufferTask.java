...
public class RunRuleAndFlushBufferTask implements IStepTask, Serializable {
   ...
   *     task.
   */
  public RunRuleAndFlushBufferTask(final IStepTask stepTask, final IBuffer<ISolution[]> buffer) {

    if (stepTask == null) throw new IllegalArgumentException();
  ...
  public RuleStats call() throws Exception {
    ...
    if (DEBUG) {
      ...
      log.debug(
          "Flushing buffer: size=" + buffer.size() + ", class=" + buffer.getClass().getName());
    }
