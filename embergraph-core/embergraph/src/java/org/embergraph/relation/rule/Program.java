...
public class Program implements IProgram {
   ...
   *     not imply that they will be executed in parallel, only that they do not have dependencies
   *     among the steps and hence are in principle parallelizable).
   */
   ...
   *     not imply that they will be executed in parallel, only that they do not have dependencies
   *     among the steps and hence are in principle parallelizable).
   * @param queryOptions Options that will be imposed if the iprogram is evaluated as a query.
   */
   ...
   *     not imply that they will be executed in parallel, only that they do not have dependencies
   *     among the steps and hence are in principle parallelizable).
   * @param closure <code>true</code> iff the steps in the program must be run until a fixed point
   *     is achieved.
   */
   ...
   * @throws IllegalArgumentException if <i>rules</i> or any element of <i>rules</i> is <code>null
   *     </code>.
   * @throws IllegalStateException if <i>this</i> program is parallel.
   */
  public void addClosureOf(IRule[] rules) {
    ...
    if (rules == null) throw new IllegalArgumentException();

    if (rules.length == 0) throw new IllegalArgumentException();
   ...
   *
   * @param rule The rule.
   * @throws IllegalArgumentException if <i>rule</i> is <code>null</code>.
   * @throws IllegalStateException if <i>this</i> program is parallel.
   */
