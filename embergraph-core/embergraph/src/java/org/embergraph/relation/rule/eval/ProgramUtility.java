...
public class ProgramUtility {
  ...
  //
  //            throw new IllegalStateException("Program uses local resources but federation is
  // distributed.");
  //
  //        }
  //
  ...
  /*
   * <code>true</code> iff the program contains an embedded closure operation. <code>false</code> if
   * the program is a rule or is itself a closure operation.
   *
   * @param step The program.
   * @return
   */
  public boolean isClosureProgram(IStep step) {
    ...
    if (program.isClosure()) return false;
