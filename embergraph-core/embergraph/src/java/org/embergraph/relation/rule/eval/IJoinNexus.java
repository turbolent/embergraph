 ...
 * execution.
 *
 * <p>Note: This interface is NOT {@link Serializable}. Use an {@link IJoinNexusFactory} to create
...
public interface IJoinNexus {
   ...
   * UnsynchronizedArrayBuffer}s is already quite large (10k or better elements, defining a single
   ...
   *
   * @param rule The rule.
   * @param index The index of the {@link IPredicate} in the body of the {@link Rule}.
   ...
   * @return <code>true</code> unless the new bindings would violate any of the {@link IConstraint}s
   *     declared for the {@link Rule}).
   ...
   * @param constraints An array of constraints which must be satisfied (optional).
   * @param e An element materialized by the {@link IAccessPath} for that {@link IPredicate}.
   * @param bindingSet the bindings to which new bindings from the element will be applied.
   ...
   * will be used to evaluate the predicate when it appears in the tail of an {@link IRule} for a
   * given {@link IEvaluationPlan}.
   *
   * @param predicate The predicate.
   * @param var A variable appearing in that predicate.
   ...
   *
   * <p>Note: For many purposes, it is only the computed {@link #ELEMENT}s that are of interest. For
   * high-level query, you will generally specify only the {@link #BINDINGS}. The {@link #BINDINGS}
   * are also useful for some truth maintenance applications. The {@link #RULE} is generally only of
   * interest for inspecting the behavior of some rule set.
   *
   * @param rule The rule.
   * @param bindingSet The bindings (the implementation MUST clone the bindings if they will be
   *     saved with the {@link ISolution}).
   ...
   *
   * @param rule The rule that will determine the order imposed amoung the bound variables (which
   *     variable is 1st, 2nd, 3rd, etc.).
   ...
   * IRule} before returning the {@link IBindingSet}.
   *
   * @param rule The rule whose bindings will be stored in the binding set.
   ...
   * the state of the {@link IMutableRelation}(s) are updated before the next {@link IRule} is
   * executed.
   *
   * @param parallel <code>true</code> unless the rule is a step is a sequential {@link IProgram}.
   *     Note that a sequential step MUST flush its buffer since steps are run in sequence precisely
   *     because they have a dependency!
   * @param rule A rule that is a step in some program. If the program is just a rule then the value
   *     of <i>parallel</i> does not matter. The buffer will is cleared when it flushed so a
   *     re-flushed is always a NOP.
   * @return The {@link IStepTask} to execute for that rule.
   * @see RunRuleAndFlushBufferTaskFactory
   * @see RunRuleAndFlushBufferTask
   * @deprecated by annotations on individual {@link BOp}s which specify which join operator is to
   *     be used on a join by join basis.
   */
   ...
   *
   * @param pred The {@link IPredicate}, which MUST be the head of some {@link IRule}.
   * @return The {@link IRelation}, which will never be a fused view and which will accept writes
   *     iff the rules are being executed as a mutation operation.
   */
   ...
   * was specified for the {@link IPredicate}. It SHOULD be used whenever the {@link IRelation} is
   * selected based on a predicate in the tail of an {@link IRule} and could therefore be a fused
   * view of more than one relation instance. (The head of the {@link IRule} must be a simple {@link
   * IRelation} and not a view.)
   *
   * <p>Note: The implementation should choose the read timestamp for each relation in the view
   * using {@link #getReadTimestamp(String)}.
   *
   * @param pred The {@link IPredicate}, which MUST be a tail from some {@link IRule}.
   * @return The {@link IRelation}, which might be a {@link RelationFusedView}.
   */
   ...
   * rule).
   *
   * <p>Note that passing in the {@link IRelation} is important since it otherwise must be
   * discovered using the {@link IResourceLocator}. By requiring the caller to resolve it before
   * hand and pass it into this method the contention and demand on the {@link IResourceLocator}
   * cache is reduced.
   *
   * @param relation The relation.
   * @param pred The predicate. When {@link IPredicate#getPartitionId()} is set, the returned {@link
   *     IAccessPath} MUST read on the identified local index partition (directly, not via RMI).
   * @return The access path.
   */
   ...
   * that predicate.
   *
   * <p>Note: You can use an {@link IDataServiceCallable} to obtain the reference of the {@link
   * IDataService} and pass that into your {@link AbstractTask} in order to have the federation
   * reference available when running under the {@link ConcurrencyManager}.
   *
   * @param predicate The predicate, with whatever bindings already applied.
   * @param fed The federation, which is required in order to access the {@link IMetadataIndex} for
   *     a scale-out index.
   ...
   * @throws IllegalStateException unless this is an {@link ActionEnum#Query}.
   * @throws IllegalArgumentException if either argument is <code>null</code>.
   * @todo ISolution is only used when constructing entailments for which we also need to know the
   *     rule which licensed the entailment in order to generate the justification chain. This
   *     should all be done by a CONSTRUCT operator. The {@link #solutionFlags()} should also go
   *     away, including {@link #BINDINGS}, {@link #ELEMENT}, {@link #RULE}, and {@link #ALL}.
   */
   ...
   *
   * @param step The {@link IRule} or {@link IProgram}.
   * @return The mutation count (#of distinct elements modified in the relation(s)).
   * @throws IllegalArgumentException unless {@link ActionEnum#isMutation()} is <code>true</code>.
   * @throws IllegalArgumentException if either argument is <code>null</code>.
   */
   ...
   * <i>targetBuffer</i>.
   *
   * <p>The returned buffer MUST apply the optional filter value returned by {@link
   * #getSolutionFilter()} in order to keep individual {@link ISolution}s out of the buffer.
   * Filtering is done at this level since the <i>targetBuffer</i> contains chunks of solutions.
   *
   * @param targetBuffer A thread-safe buffer for chunks of {@link ISolution}s that was allocated
   *     with {@link #newQueryBuffer()}, {@link #newInsertBuffer(IMutableRelation)}, or {@link
   *     #newDeleteBuffer(IMutableRelation)}.
   * @param chunkCapacity The capacity of the new buffer. This should be maximum chunk size that
   *     will be produced or {@link #getChunkCapacity()} if you do not have better information.
   ...
   * @todo Normally the {@link IElementFilter} is specified in terms of the elements of some {@link
   *     IRelation} and then wrapped as a {@link SolutionFilter}.
   *     <p>The return type here should be strongly typed. The generic argument is not a sufficient
   *     guarantee and the compiler can be fooled by passing an {@link IElementFilter} that is
   *     already defined in terms of an {@link ISolution} rather than element of an {@link
   *     IRelation}.
   */
