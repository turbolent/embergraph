 ...
 * execution is distributed across an {@link IEmbergraphFederation} using RMI. Implementations are
 * presumed to carry some state relating to the desired execution context, including the
 * <i>solutionFlags</i>, any {@link IElementFilter} to be applied to the created buffers, etc.
 *
...
public interface IJoinNexusFactory extends Serializable {
   ...
   * IJoinNexus} instances for a given combination of {@link IJoinNexusFactory} and {@link
   * IIndexManager} objects. A distinct {@link IJoinNexus} instance is of course generated when the
   * {@link IJoinNexusFactory} is de-serialized, e.g., during RMI, and when a different {@link
   * IIndexManager} is used.
   *
   * @param indexManager Used to locate relations and parallelize operations during rule execution.
   */
