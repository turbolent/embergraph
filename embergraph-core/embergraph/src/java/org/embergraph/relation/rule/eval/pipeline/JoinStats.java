...
public class JoinStats implements Serializable {
   ...
   * level) or to the start of some specific {@link JoinTask} (detail level).
   */
   ...
   * the same join dimension could, in principle, have a different fan in based on the actual
   * binding sets propagated this is not necessarily the "actual" fan in for the join dimension.
   */
  public int fanIn;
   ...
   * re-opened for the same query, join dimension and index partition, and since each join task for
   * the same join dimension could, in principle, have a different fan out based on the actual
   * binding sets propagated this is not necessarily the "actual" fan out for the join dimension.
   */
  public int fanOut;
  ...
  /** Map used to track the #of distinct partition identifiers for this join dimension. */
   ...
   * generates chunk of distinct {@link AccessPathTask}s from a chunk of {@link IBindingSet}s read
   * from its source(s) {@link JoinTask}s.
   *
   * <p>Note: While the {@link IPredicate}s for those tasks may have the same bindings, the source
   * {@link IBindingSet}s typically (always?) have variety not represented in the bound {@link
   * IPredicate} and therefore are combined under a single {@link AccessPathTask}. This reduces
   * redundant reads on an {@link IAccessPath} while producing exactly the same output {@link
   * IBindingSet}s that would have been produced if we did not identify the duplicate {@link
   * IAccessPath}s.
   */
   ...
   *
   * <p>Note: An {@link IBindingSet} can be written onto more than one index partition for the next
   * join dimension, so one generated {@link IBindingSet} MAY result in N GTE ONE "binding sets
   * out". This occurs when the {@link IAccessPath} required to read on the next {@link IPredicate}
   * in the evaluation order spans more than one index partition.
   */
   ...
   *
   * <p>Note: The mutationCount MUST be obtained from {@link IBuffer#flush()} for the buffer on
   * which the {@link JoinTask}(s) for the last join dimension write their solutions. For mutation,
   * this buffer is obligated to report the #of elements whose state was changed in the target
   * relation. Failure to correctly obey this contract can result in non-termination of fix point
   * closure operations.
   *
   ...
   *
   * @param orderIndex The index in the evaluation order.
   */
  ...
  synchronized void add(final JoinStats o) {
    ...
    if (this.orderIndex != o.orderIndex) throw new IllegalArgumentException();
   ...
   * @param a The {@link JoinStats}.
   * @return The table view.
   */
  public static StringBuilder toString(
      final IRule rule, final IRuleState ruleState, final JoinStats[] a) {
    ...
    sb.append(
        "startTime, rule, orderIndex, keyOrder, nvars, rangeCount, fanIn, fanOut, partitionCount, bindingSetChunksIn, bindingSetsIn, accessPathCount, accessPathDups, chunkCount, elementCount, bindingSetsOut, bindingSetChunksOut, mutationCount, tailIndex, tailPredicate");
