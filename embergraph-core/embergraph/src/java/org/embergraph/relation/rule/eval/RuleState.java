...
public class RuleState implements IRuleState {
   ...
   * for the next join dimension. This increases performance by making sure that ordered reads are
   * used by the subqueries for any given chunk on some join dimension.
   */
   ...
   * binding set that is passed downstream to the next tail. So right before a binding set is cloned
   * and sent downstream, we will check the RuleState to see what variables can be dropped based on
   * what is required at the next tail downstream.
   */
   ...
   *
   * @param order The evaluation order.
   * @param nvars The #of unbound variables for each tail predicate is assigned by side-effect.
   * @return An array of the {@link IKeyOrder}s for each tail predicate. The array is correlated
   *     with the predicates index in the tail of the rule NOT its evaluation order.
   */
  protected IKeyOrder[] computeKeyOrderForEachTail(
      final IRule rule, final IJoinNexus joinNexus, final int[] order, final int[] nvars) {

    if (order == null) throw new IllegalArgumentException();

    if (order.length != rule.getTailCount()) throw new IllegalArgumentException();
    ...
    for (int orderIndex = 0; orderIndex < tailCount; orderIndex++) {
      ...
      final IKeyOrder<?> keyOrder = joinNexus.getTailAccessPath(rel, asBound).getKeyOrder();
   ...
   * will need its own variables, plus any variables that appear later in the join.
   *
   * @param rule The rule being executed.
   * @param order The evaluation order.
   * @return The array of required variables for each tail index.
   */
  public static IVariable[][] computeRequiredVarsForEachTail(final IRule rule, final int[] order) {
    ...
    if (log.isDebugEnabled()) {
      ...
      log.debug("requiredVars[]=" + Arrays.toString(s) + ", rule=" + rule);
    }
