...
public class LocalJoinTask extends JoinTask {
  ...
  /** The asynchronous iterator that is the source for this {@link LocalJoinTask}. */
  ...
  public LocalJoinTask(
    ...
    if (lastJoin && solutionBuffer == null) throw new IllegalArgumentException();
  ...
  protected IBuffer<ISolution[]> getSolutionBuffer() {
    ...
    if (!lastJoin) throw new IllegalStateException();
   ...
   * solution buffer. Otherwise it will write on {@link #syncBuffer}, which is drained by the sink
   * {@link LocalJoinTask}.
   */
   ...
   * from {@link #call()}.
   *
   * @param f The future for the downstream join dimension.
  ...
  protected final void setSinkFuture(final Future<?> f) {
    ...
    if (sinkFuture != null) throw new IllegalStateException();
  ...
  protected IBindingSet[] nextChunk() throws InterruptedException {
    ...
    if (DEBUG) log.debug("orderIndex=" + orderIndex);
    ...
    while (!source.isExhausted()) {
      ...
      if (source.hasNext(10, TimeUnit.MILLISECONDS)) {
        ...
        if (DEBUG)
          log.debug(
              "Read chunk from source: chunkSize=" + chunk.length + ", orderIndex=" + orderIndex);
    ...
    if (DEBUG) log.debug("Source exhausted: orderIndex=" + orderIndex);
