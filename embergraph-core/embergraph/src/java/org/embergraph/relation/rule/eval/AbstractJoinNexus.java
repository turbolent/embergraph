...
import org.embergraph.bop.joinGraph.DefaultRangeCountFactory;
import org.embergraph.bop.joinGraph.IEvaluationPlanFactory;
import org.embergraph.bop.joinGraph.IRangeCountFactory;
import org.embergraph.btree.keys.DelegateSortKeyBuilder;
...
public abstract class AbstractJoinNexus implements IJoinNexus {
  ...
  public final String getProperty(final String name, final String defaultValue) {
    ...
    return Configuration.getProperty(
        indexManager, joinNexusFactory.getProperties(), null /* namespace */, name, defaultValue);
  }
  ...
  public final <T> T getProperty(
    ...
    return Configuration.getProperty(
        indexManager,
        joinNexusFactory.getProperties(),
        null /* namespace */,
        name,
        defaultValue,
        validator);
  ...
  /** @todo caching for the same relation and database state shared across join nexus instances. */
  ...
  public final boolean forceSerialExecution() {
    ...
    if (log.isInfoEnabled()) log.info("forceSerialExecution=" + forceSerialExecution);
...
//     * @deprecated by {@link #getTailAccessPath(IRelation, IPredicate)}
  ...
  public final ISolution newSolution(final IRule rule, final IBindingSet bindingSet) {
  ...
  /*
   * FIXME Custom serialization for solution sets, especially since there tends to be a lot of
   * redundancy in the data arising from how bindings are propagated during JOINs.
   *
   * @todo We can sort the {@link ISolution}s much like we already do for DISTINCT or intend to do
   *     for SORT and use the equivalent of leading key compression to reduce IO costs (or when they
   *     are SORTed we could leverage that to produce a more compact serialization).
  ...
  /*
   * FIXME Custom serialization for binding sets, especially since there tends to be a lot of
   * redundancy in the data arising from how bindings are propagated during JOINs.
   *
   * @todo We can sort the {@link ISolution}s much like we already do for DISTINCT or intend to do
   *     for SORT and use the equivalent of leading key compression to reduce IO costs (or when they
  ...
  public final IRuleTaskFactory getRuleTaskFactory(final boolean parallel, final IRule rule) {
  ...
  public final IBuffer<ISolution> newUnsynchronizedBuffer(
  ...
  /*
   * Note: {@link ISolution} (not relation elements) will be written on the buffer concurrently by
   * different rules so there is no natural order for the elements in the buffer.
   */
   ...
   * {@inheritDoc}
   *
   * <p>Note: {@link #getSolutionFilter()} is applied by {@link #newUnsynchronizedBuffer(IBuffer,
   * int)} and NOT by the buffer returned by this method.
   */
  @SuppressWarnings("unchecked")
  ...
  public IBuffer<ISolution[]> newDeleteBuffer(final IMutableRelation relation) {
    ...
    if (getAction() != ActionEnum.Delete) throw new IllegalStateException();
    ...
    return new AbstractSolutionBuffer.DeleteSolutionBuffer(chunkOfChunksCapacity, relation);
  }

  /*
   * Return the {@link ISortKeyBuilder} used to impose DISTINCT on the solutions generated by a
   * query.
   *
   * @param head The head of the rule.
   * @return The {@link ISortKeyBuilder}.
   * @todo This should be based on bop annotations and a hash table for distinct unless it is very
   *     high volume and you can wait for the first result, in which case a SORT should be selected.
   *     For high volume with low latency to the first result, use a persistent hash table on a
   *     temporary store.
   */
  protected abstract ISortKeyBuilder<?> newSortKeyBuilder(final IPredicate<?> head);

  @SuppressWarnings("unchecked")
  public IChunkedOrderedIterator<ISolution> runQuery(final IStep step) throws Exception {

    if (step == null) throw new IllegalArgumentException();

    if (log.isInfoEnabled()) log.info("program=" + step.getName());
    ...
    if (isEmptyProgram(step)) {
      ...
      return (IChunkedOrderedIterator<ISolution>)
          new EmptyProgramTask(ActionEnum.Query, step).call();
    }
    ...
    if (step.isRule() && step.getQueryOptions().isDistinct()) {
       ...
       * Head exists and elements are requested, so impose DISTINCT
        ...
        sortKeyBuilder =
            new DelegateSortKeyBuilder(newSortKeyBuilder(((IRule) step).getHead())) {
       ...
       * Bindings were requested so impose DISTINCT based on those
          ...
          sortKeyBuilder =
              new DelegateSortKeyBuilder<ISolution, IBindingSet>(
          ...
          throw new UnsupportedOperationException(
              "You must specify BINDINGS since the rule does not have a head: " + step);
        }
      }
  ...
  public final long runMutation(final IStep step) throws Exception {

    if (step == null) throw new IllegalArgumentException();

    if (!action.isMutation()) throw new IllegalStateException();
    ...
    if (log.isInfoEnabled()) log.info("action=" + action + ", program=" + step.getName());
  ...
  /*
   * Core impl. This handles the logic required to execute the program either on a target {@link
   * DataService} (highly efficient) or within the client using the {@link IClientIndex} to submit
   * operations to the appropriate {@link DataService}(s) (not very efficient, even w/o RMI).
   *
   * @return Either an {@link IChunkedOrderedIterator} (query) or {@link Long} (mutation count).
   */
  protected final Object runProgram(final ActionEnum action, final IStep step) throws Exception {
    ...
    if (indexManager instanceof IEmbergraphFederation<?>) {
      ...
      return runDistributedProgram((IEmbergraphFederation<?>) indexManager, action, step);
  ...
  protected final Object runDistributedProgram(
      final IEmbergraphFederation<?> fed, final ActionEnum action, final IStep step)
      throws Exception {
