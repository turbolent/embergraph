...
public class DistributedJoinTask extends JoinTask {
   ...
   *
   * <p>Note: This is a thread-safe collection since new sources may be added asynchronously during
   * processing.
   */
   ...
   * no willing to accept new {@link #sources}.
   *
  ...
  public DistributedJoinTask(
    ...
    if (joinNexus instanceof IEmbergraphFederation<?>) throw new IllegalArgumentException();
    ...
    if (lastJoin) {
      ...
      if (action.isMutation()) {
        ...
        final IMutableRelation<?> relation =
            (IMutableRelation<?>) tmp.getHeadRelationView(rule.getHead());
   ...
   * @throws IllegalArgumentException if the <i>source</i> is <code>null</code>.
   */
  public boolean addSource(final IAsynchronousIterator<IBindingSet[]> source) {
    ...
    try {
      ...
      if (sourcesExhausted) {
        ...
        if (INFO)
          log.info("source rejected: orderIndex=" + orderIndex + ", partitionId=" + partitionId);
  ...
  protected void closeSources() {
    ...
    try {
      ...
      final IAsynchronousIterator<?>[] a = sources.toArray(new IAsynchronousIterator[] {});
  ...
  private void removeFromSession() {
    ...
    try {
      ...
      final String namespace =
          JoinTaskFactoryTask.getJoinTaskNamespace(masterUUID, orderIndex, partitionId);
   ...
   * atomic so that the termination condition of {@link #nextChunk()} is consistent -- it should
   * terminate when there are no sources remaining. The first source is added when the {@link
   * DistributedJoinTask} is created. Additional sources are added (and can result in a fan-in
   * greater than one) when a {@link JoinTaskFactoryTask} identifies that there is an existing
   * {@link DistributedJoinTask} and is able to atomically assign a new source to that {@link
   * DistributedJoinTask}. If the atomic assignment of the new source fails (because all sources are
   * exhausted before the assignment occurs) then a new {@link DistributedJoinTask} will be created
   * for the same {@link DistributedJoinMasterTask}, orderIndex, and index partition identifier and
   * the source will be assigned to that {@link DistributedJoinTask} instead.
   *
   * @todo javadoc update
   */
   ...
   * JoinTask}s.
   *
   * @return A chunk assembled from one or more chunks from one or more of the source {@link
   *     JoinTask}s.
   */
  protected IBindingSet[] nextChunk() throws InterruptedException {
    ...
    if (DEBUG)
      log.debug(
          "Reading chunk of bindings from source(s): orderIndex="
              + orderIndex
              + ", partitionId="
              + partitionId);
    ...
    while (!sourcesExhausted) {
      ...
      while (!halt && !sources.isEmpty() && bindingSetCount < chunkCapacity) {
        ...
        if (trace) System.err.print("\norderIndex=" + orderIndex);

        if (trace) System.err.print(": reading");
        ...
        final IAsynchronousIterator<IBindingSet[]>[] sources =
            (IAsynchronousIterator<IBindingSet[]>[])
                this.sources.toArray(new IAsynchronousIterator[] {});

        // #of sources that are exhausted.
        int nexhausted = 0;

        for (int i = 0; i < sources.length && bindingSetCount < chunkCapacity; i++) {

          if (trace) System.err.print(" <<(" + i + ":" + sources.length + ")");
          ...
          if (src.hasNext(1L, TimeUnit.MILLISECONDS)) {
            ...
            if (trace) System.err.print("[" + chunk.length + "]");

            if (DEBUG)
              log.debug(
                  "Read chunk from source: sources["
                      + i
                      + "], chunkSize="
                      + chunk.length
                      + ", orderIndex="
            ...
            if (trace) System.err.print("X{" + nexhausted + "}");

            if (DEBUG) log.debug("Source is exhausted: nexhausted=" + nexhausted);
        ...
        if (nexhausted == sources.length) {
          ...
          try {
            ...
            if (this.sources.isEmpty()) {
              ...
              if (INFO)
                log.info(
                    "Sources are exhausted: orderIndex="
                        + orderIndex
                        + ", partitionId="
                        + partitionId);
    ...
    if (DEBUG)
      log.debug("Sources are exhausted: orderIndex=" + orderIndex + ", partitionId=" + partitionId);

    if (trace) System.err.print(" exhausted");
   ...
   * @param bindingSetCount The #of bindingSets in those chunks.
   * @return
   */
  protected IBindingSet[] combineChunks(
      final List<IBindingSet[]> chunks, final int bindingSetCount) {
    ...
    if (trace) System.err.print(" chunk[" + chunk.length + "]");
   ...
   *
   * <p>Note: Closing the {@link BlockingBuffer} from which a sink {@link JoinTask} is reading will
   * cause the source iterator for that sink task to eventually return <code>false</code> indicating
   * that it is exhausted (assuming that the sink keeps reading on the iterator).
   *
   * @throws InterruptedException if interrupted while awaiting the future for a sink.
   */
  @Override
  protected void flushAndCloseBuffersAndAwaitSinks()
    ...
    if (lastJoin) {
...

      {
        ...
        final List<Future<Void>> futures = fed.getExecutorService().invokeAll(tasks);
      ...
      // Await sinks.
      {
        ...
        while (itr.hasNext()) {
          ...
          if (DEBUG) log.debug("Waiting for Future: sink=" + sink);
   ...
   * scale-out index for the next {@link IPredicate} in the evaluation order.
   *
   * @param locator The locator for the index partition.
   * @return The sink.
   * @throws RuntimeException If the {@link JoinTaskFactoryTask} fails.
   * @throws InterruptedException If the {@link JoinTaskFactoryTask} is interrupted.
   */
  ...
  private static class SinkRequest {
    ...
    public boolean equals(final Object o) {
      ...
      if (!(o instanceof SinkRequest)) return false;
  ...
  private static class SinkMemoizer
      extends Memoizer<SinkRequest /* request */, JoinTaskSink /* sink */> {
     ...
     * realized yet. When we are breaking a join, we probably want to cancel all of the requests to
     * obtain sinks in addition to canceling any running sinks. A similar problem may exist if we
     * implement native SLICE since we could break the join while there are requests out to create
     * sinks.
     *
     * <p>One way to handle this is to pull the cancelSinks() method into this memoizer.
     *
     * <p>However, if we broad cast the rule to the nodes and move away from this sinks model to
     * using NIO buffers then we will just broadcast the close of each tail in turn or broadcast the
     * break of the join.
     */
    @SuppressWarnings("unchecked")
    Iterator<JoinTaskSink> getSinks() {
      return new Striterator(cache.values().iterator())
          .addFilter(
              new Filter() {
   ...
   * @return The sink which will write on the downstream {@link JoinTask} running on the node for
   *     that shard.
  ...
  private JoinTaskSink _getSink(final PartitionLocator locator)
    ...
    if (DEBUG)
      log.debug(
          "Creating join task: nextOrderIndex="
              + nextOrderIndex
              + ", indexName="
              + nextScaleOutIndexName
              + ", partitionId="
    ...
    if (fed.isDistributed()) {
      ...
      sourceItrProxy =
          ((AbstractDistributedFederation<?>) fed)
              .getProxy(
   ...
   * evaluated, and even correlate the {@link JoinTask#masterUUID} in order to figure out what
   * really happened.
   */
  @Override
