 ...
 * @version $Id$
 * @param E The generic type of the [E]lements materialized by the head of the rule. This should be
 *     the same as the generic type of the {@link IRelation} that can be materialized from the
 *     {@link String} associated with the {@link IPredicate} that is the head of the rule.
 */
public interface IRule<E> extends IStep {
   ...
   * that are executed as queries DO NOT need to specify a head. However, rules that will be
   * executed as mutation operations (insert or delete) MUST specify the head as it determines the
   ...
   * an appended single quote to indicate that it is a derived variant.
   *
   * @param bindingSet Bindings for zero or more free variables in this rule. The rule will be
   *     rewritten such that the variable is replaced by the binding throughout the rule. An attempt
   *     to bind a variable not declared by the rule will be ignored.
   * @param constraints An array of additional constraints to be imposed on the rule (optional).
   ...
   * #specialize(String, IBindingSet, IConstraint[]) specialized}.
   *
   * <p>Note: {@link IJoinNexus#newBindingSet(IRule)} MUST apply the constants before returning the
   ...
   *
   * @param index1 The index of a predicate in the {@link #tail}.
   * @param index2 The index of a different predicate in the {@link #tail}.
   ...
   *
   * @param index The index of a predicate declared the {@link #getTail() tail} of the {@link
   *     IRule}.
   * @param bindingSet The variable bindings.
   * @return True iff it is fully bound (a mixture of constants and/or bound variables).
   ...
   * @param index The index of a predicate declared the {@link #getTail() tail} of the {@link
   *     IRule}.
   * @param bindingSet The bindings under which the variable count will be obtained (any variables
  ...
  /*
   * An optional {@link IRuleTaskFactory} that will be used in place of the default {@link
   * IRuleTaskFactory} to evaluate this rule (optional)
   *
