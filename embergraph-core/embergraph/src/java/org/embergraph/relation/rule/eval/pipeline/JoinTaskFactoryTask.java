 ...
 * DistributedJoinTask#addSource(IAsynchronousIterator)} and specify the {@link #sourceItrProxy} as
 * another source for that join task.
 *
 * <p>The use of a factory pattern allows us to concentrate all {@link DistributedJoinTask}s which
 * target the same tail predicate and index partition for the same rule execution instance onto the
 * same {@link DistributedJoinTask}. The concentrator effect achieved by the factory only matters
 * when the fan-out is GT ONE (1).
 *
 * @todo The factory semantics requires something like a "session" concept on the {@link
 *     DataService}. However, it could also be realized by a canonicalizing mapping of {masterProxy,
 *     orderIndex, partitionId} onto an object that is placed within a weak value cache.
 * @todo Whenever a {@link DistributedJoinTask} is interrupted or errors it must make sure that the
 *     entry is removed from the session (it could also interrupt/cancel the remaining {@link
 *     DistributedJoinTask}s for the same {masterInstance}, but we are already doing that in a
 *     different way.)
 * @todo We need to specify the failover behavior when running query or mutation rules. The simplest
 *     answer is that the query or closure operation fails and can be retried.
 *     <p>When retried a different data service instance could take over for the failed instance.
 *     This presumes some concept of "affinity" for a data service instance when locating a join
 *     task. If there are replicated instances of a data service, then affinity would be the
 *     tendency to choose the same instance for all join tasks with the same master, orderIndex, and
 *     partitionId. That might be more efficient since it allows aggregation of binding sets that
 *     require the same access path read. However, it might be more efficient to distribute the
 *     reads across the failover instances - it really depends on the workload.
 *     <p>Ideally, a data service failure would be handled by restarting only those parts of the
 *     operation that had failed. This means that there is some notion of idempotent for the
 *     operation. For at least the RDF database, this may be achievable. Failure during query
 *     leading to resend of some binding set chunks to a new join task could result in
 *     overgeneration of results, but those results would all be duplicates. If that is acceptable,
 *     then this approach could be considered "safe". Failure during mutation (aka closure) is even
 *     easier for RDF as redundant writes on an index still lead to the same fixed point.
...
public class JoinTaskFactoryTask extends DataServiceCallable<Future> {
  ...
  /** Set by {@link #call()} to the federation instance available on the {@link DataService}. */
   ...
   * @param partitionId
   * @param masterProxy
   * @param masterUUID (Avoids RMI to obtain this later).
   * @param sourceItrProxy
   * @param nextScaleOutIndexName
   */
  public JoinTaskFactoryTask(
      final String scaleOutIndexName,
      final IRule rule,
      final IJoinNexusFactory joinNexusFactory,
      final int[] order,
    ...
    if (partitionId < 0) throw new IllegalArgumentException();
    if (masterProxy == null) throw new IllegalArgumentException();
    if (masterUUID == null) throw new IllegalArgumentException();
    if (sourceItrProxy == null) throw new IllegalArgumentException();
    if (keyOrders == null || keyOrders.length != order.length) throw new IllegalArgumentException();
  ...
  public Future call() {
    ...
    if (sourceItrProxy instanceof ClientAsynchronousIterator) {
      ...
      ((ClientAsynchronousIterator) sourceItrProxy).start(fed.getExecutorService());
    }

    final String namespace = getJoinTaskNamespace(masterUUID, orderIndex, partitionId);
  ...
  protected DistributedJoinTask newJoinTask() {
    ...
    final DistributedJoinTask task;
    {
       ...
       */
      final IIndexManager indexManager = new DelegateIndexManager(getDataService());

      task =
          new DistributedJoinTask(
              /*scaleOutIndexName,*/ rule,
  ...
  protected Future<Void> submit(final DistributedJoinTask task) {
    ...
    if (log.isDebugEnabled())
      log.debug(
          "Submitting new JoinTask: orderIndex="
              + orderIndex
              + ", partitionId="
              + partitionId
              + ", indexName="
    ...
    Future<Void> joinTaskFuture = getFederation().getExecutorService().submit(task);

    if (fed.isDistributed()) {

      // create a proxy for the future.
      joinTaskFuture = ((AbstractDistributedFederation) fed).getProxy(joinTaskFuture);
    }
   ...
   * @return
   */
  public static String getJoinTaskNamespace(
      final UUID masterUUID, final int orderIndex, final int partitionId) {
   ...
   * does not implement {@link IIndexManager} or even {@link IIndexStore}. Therefore this class was
   * introduced. It passes most of the methods on to the {@link IEmbergraphFederation} but {@link
   * #getIndex(String, long)} is delegated to {@link IndexManager#getIndex(String, long)} which is
   * the method that knows how to create the index partition view.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   ...
   *     one method to the {@link IndexManager}, most methods to the {@link IEmbergraphFederation}
   *     and disallows {@link #dropIndex(String)} and {@link #registerIndex(IndexMetadata)} in an
   *     attempt to stay out of trouble. That may be enough reason to keep it private.
   */
