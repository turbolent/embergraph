...
public class JoinTaskSink {
  ...
  protected void setFuture(final Future f) {
    ...
    if (DEBUG)
      log.debug(
          "sinkOrderIndex=" + sinkOrderIndex + ", sinkPartitionId=" + locator.getPartitionId());
  }
  ...
  public String toString() {
    ...
    return "JoinSinkTask{ sinkOrderIndex="
        + sinkOrderIndex
        + ", sinkPartitionId="
   ...
   * assign its future to the returned object using {@link #setFuture(Future)}.
   *
   * @param fed The federation.
   * @param locator The locator for the index partition.
   * @param sourceJoinTask The current join dimension.
   */
  public JoinTaskSink(
    ...
    if (sourceJoinTask == null) throw new IllegalArgumentException();
