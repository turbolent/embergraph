 ...
 * exclusive access to the unisolated index for writers while allowing readers concurrent access.
 *
...
public class LocalJoinMasterTask extends JoinMasterTask {
  ...
  /*
   * Applies an initial {@link IBindingSet} to the first join dimension. Intermediate {@link
   * IBindingSet}s will propagate to each join dimension. The final {@link IBindingSet}s will be
   * generated by the last join dimension and written on the {@link #getSolutionBuffer()}.
   *
   * @return The {@link Future} for the {@link LocalJoinTask} for each join dimension.
   */
  @Override
  protected List<Future<Void>> start() throws Exception {
    ...
    for (int orderIndex = 0; orderIndex < tailCount; orderIndex++) {
      ...
      assert src != null
          : "No source: orderIndex=" + orderIndex + ", tailCount=" + tailCount + ", rule=" + rule;

      // create the local join task.
      final LocalJoinTask joinTask =
          new LocalJoinTask(
              /*indexName, */ rule,
              joinNexus,
              order,
