...
public class Rule<E> implements IRule<E> {
   ...
   *
   * @see Var#var(String)
   * @todo it is a good idea to use this factory rather than {@link Var#var(String)} as the latter
   *     MAY be replaced by per-rule-instance variables rather than globally canonical variables
   *     <p>the only problem with this is that we need to access the variables in a bindingset by
   *     name after high-level query since the rule may not be available to the caller, e.g., the
   *     match rule uses dynamically generated rules that are not visible to the caller who only
   *     sees the binding sets.
   */
...
//     * <code>true</code> iff a DISTINCT constraint will be imposed when the
  ...
  public final IConstraint getConstraint(int index) {
    ...
    if (constraints == null) throw new IndexOutOfBoundsException();
   ...
   * @param head The subset of bindings that are selected by the rule.
   * @param tail The tail (aka body) of the rule.
   * @param constraints An array of constraints on the legal states of the bindings materialized for
   *     the rule.
   * @throws IllegalArgumentException if the <i>name</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>head</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>tail</i> is <code>null</code>.
   * @throws IllegalArgumentException if any element of the <i>tail</i> is <code>null</code>.
   * @throws IllegalArgumentException if any element of the optional <i>constraints</i> is <code>
   *     null</code>.
   * @throws IllegalArgumentException if the <i>tail</i> is empty.
   * @throws IllegalArgumentException if the <i>head</i> declares any variables that are not
   ...
   * @param head The subset of bindings that are selected by the rule.
   * @param tail The tail (aka body) of the rule.
   * @param queryOptions Options that effect evaluation of the rule as a query.
   * @param constraints An array of constraints on the legal states of the bindings materialized for
   *     the rule.
   * @throws IllegalArgumentException if the <i>name</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>head</i> is <code>null</code>.
  ...
  public Rule(
      ...
      IPredicate head,
      IPredicate[] tail,
      final IQueryOptions queryOptions,
   ...
   * @param head The head of the rule. This is optional for rules that will be evaluated using
   *     {@link ActionEnum#Query} but required for rules that will be evaluated using a mutation
   *     {@link ActionEnum}.
   * @param tail The predicates in the tail of the rule.
   * @param queryOptions Additional constraints on the evaluate of a rule as a query (required, but
   *     see {@link QueryOptions#NONE}).
   * @param constraints The constraints on the rule (optional).
   * @param constants Bindings for variables that are bound as constants for the rule (optional).
   * @param taskFactory Optional override for rule evaluation (MAY be <code>null</code>).
   * @param requiredVars Optional set of required variables. If <code>null</code>, all variables are
  ...
  public Rule(
    ...
    if (queryOptions == null) throw new IllegalArgumentException();
    ...
    if (head != null) {
      ...
      if (head.getRelationCount() != 1) {
        ...
        throw new IllegalArgumentException(
            "Expecting a single relation identifier for the head: head=" + head);
      }
  ...
  public IRule<E> specialize(String name, IBindingSet bindingSet, IConstraint[] constraints) {
    ...
    final IPredicate newHead = (head == null ? null : head.asBound(bindingSet));
   ...
   * @param p2 A different predicate.
   * @return The variables in common -or- <code>null</code> iff there are no variables in common.
   * @throws IllegalArgumentException if the two predicates are the same reference.
   */
  ...
  public boolean isFullyBound(IBindingSet bindingSet) {
    ...
    for (int i = 0; i < tail.length; i++) {
      ...
      if (!isFullyBound(i, bindingSet)) return false;
    }
