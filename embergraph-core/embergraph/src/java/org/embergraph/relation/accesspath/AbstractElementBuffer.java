...
public abstract class AbstractElementBuffer<R> extends AbstractArrayBuffer<R> {
   ...
   *     null</code> if you do not have a <em>strong</em> guarantee for that order.
   * @param filter An optional filter for keeping elements out of the buffer.
   */
  protected AbstractElementBuffer(
      final int capacity,
   ...
   *
   * <p>Note: The elements generally appear in an arbitrary order and need to be sorted into ordered
   * chunks using {@link IKeyOrder#getComparator()} for the desired natural order(s).
   *
   * @param itr
  ...
  //         * @param transform
  //         */
  //        protected UpdateBuffer(int capacity, IMutableRelation<R> relation, ITransform<R>
  // transform) {
  //
