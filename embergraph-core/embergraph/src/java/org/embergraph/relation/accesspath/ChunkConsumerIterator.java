 ...
 * @version $Id: ChunkConsumerIterator.java 6130 2012-03-15 10:31:25Z thompsonbry $
 * @param <E>
 * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/483">Eliminate unnecessary
 *     dechunking and rechunking </a>
 */
...
//     * Total elapsed time for the iterator instance.
  ...
  /** #of chunks materialized so far via {@link #nextChunk()} or {@link #nextChunk(IKeyOrder)}. */
   ...
   *     be visited -or- <code>null</code> if no known.
   */
  public ChunkConsumerIterator(final ICloseableIterator<E[]> src, final IKeyOrder<E> keyOrder) {
  ...
  public E next() {
    ...
    if ((lastIndex == -1) || ((lastIndex + 1) == chunk.length)) {
      ...
      if (INFO)
        log.info("read chunk from source iterator: nchunks=" + nchunks + ", size=" + chunk.length);
    }
  ...
  public E[] nextChunk() {
    ...
    if ((lastIndex == -1) || ((lastIndex + 1) == chunk.length)) {
      ...
      if (INFO)
        log.info("read chunk from source iterator: nchunks=" + nchunks + ", size=" + a.length);
    ...
    final E[] a =
        (E[])
            java.lang.reflect.Array.newInstance(
                ...
                chunk.getClass().getComponentType(), remaining);
    ...
    if (INFO) log.info("remainder chunk: nchunks=" + nchunks + ", size=" + remaining);
  ...
  public void close() {
    ...
    if (INFO) log.info("#chunks=" + nchunks + ", #elements=" + nelements);
