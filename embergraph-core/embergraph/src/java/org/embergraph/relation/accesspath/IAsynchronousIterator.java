...
public interface IAsynchronousIterator<E> extends ICloseableIterator<E> {
   ...
   *
   * <p>Note that a <code>false</code> return DOES NOT signify that the iterator is exhausted.
   * However, if you specify an infinite timeout using {@link Long#MAX_VALUE} {@link
   * TimeUnit#SECONDS} then you MAY safely interpret a <code>false</code> return as an indication
   * that the iterator is exhausted.
   *
   * @param timeout The length of time that the method may block awaiting an element to appear.
   * @param unit The units in which the <i>timeout</i> is expressed.
   ...
   * that element returned to the caller.
   *
   * @param timeout The timeout (overrides the <i>chunkTimeout</i> specified to the {@link
   *     BlockingBuffer} ctor).
   * @param unit The unit in which the timeout is expressed.
   * @return The element -or- <code>null</code> iff the timeout was exceeded before {@link
   *     #hasNext(long, TimeUnit)} returned <code>true</code>.
   *     <p>In order to have a definitive indication that the iterator is exhausted you need to
   *     invoke {@link #hasNext()} without a timeout (blocking) or {@link #isExhausted()}
   *     (non-blocking).
   ...
   * up and deadlocking. For this reason, {@link #close()} has consequences NOT entailed by {@link
   * ICloseableIterator}.
   *
   * <p>Note: Depending on the semantics of the producer, it MAY choose to treat an interrupt() as
   * normal (but eager) termination. For example, rule execution treats an interrupt() as normal
   * (but eager) termination with the consequence that queries may be safely interrupted once some
