 ...
 * them. This can be accomplished by adding the filter as a constraint on the predicate when
 * specifying the access path.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @param R The generic type of the elements of the {@link IRelation}.
 * @todo Add support for non-perfect access paths. This class should layer on an index local {@link
 *     IFilter} which rejects tuples which do not satisfy the {@link IPredicate}'s bindings. This
 *     will give the effect of a SCAN with an implied filter. The javadoc on {@link
 *     IRelation#getKeyOrder(IPredicate)} should also be updated to reflect the allowance for
 *     non-perfect access paths.
 */
public class AccessPath<R> implements IAccessPath<R>, IBindingSetAccessPath<R> {
   ...
   * factor or chunk capacity. It has been temporarily raised to a very large value in order to
   * support truth maintenance where the code assumes access to the fully buffered result. That
   * change needs to be examined for an impact on query performance. It is effectively forcing all
   * access path reads to be fully buffered rather than using an asynchronous iterator pattern.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/606">Array limits in truth
   *     maintenance code. </a>
   */
   ...
   * basis since range counts and range iterators are both constrained to a specific key range of
   * interest for an {@link IAccessPath} while they would span the entire {@link IIndex} otherwise.
   *
   * @todo cache the {@link IAccessPath}s themselves so that we benefit from reuse of the cached
   *     data.
   ...
   * the {@link IAccessPath} to be empty.
   */
  ...
  /** The filter derived from optional {@link IPredicate.Annotations#ACCESS_PATH_FILTER}. */
   ...
   * @param localIndexManager Access to the indices, resource locators, executor service, etc.
   * @param predicate The constraints on the access path.
   * @param keyOrder The order in which the elements would be visited for this access path.
   */
  public AccessPath(
    ...
    if (!(relation.getIndexManager() instanceof IEmbergraphFederation<?>)) {
      ...
      if (localIndexManager == null) {
        throw new RuntimeException(
            "Local index manager not given but"
                + " access path specifies local index: pred="
                + predicate);
      }
     ...
     */
    {
      ...
      timestamp =
          (timestamp == ITx.UNISOLATED && (flags & IRangeQuery.READONLY) != 0
              ? ITx.READ_COMMITTED
              : timestamp);
    ...
    if (partitionId != -1) {
      ...
      final String name =
          DataService.getIndexPartitionName(namespace + "." + keyOrder.getIndexName(), partitionId);
      ...
      if (pmd == null) throw new RuntimeException("Not an index partition");

      if (pmd.getPartitionId() != partitionId) {

        throw new RuntimeException(
            "Expecting partitionId=" + partitionId + ", but have " + pmd.getPartitionId());
      }
    ...
    final int chunkCapacity =
        predicate.getProperty(
            BufferAnnotations.CHUNK_CAPACITY, BufferAnnotations.DEFAULT_CHUNK_CAPACITY);
...

    {
       ...
       */
      final SameVariableConstraint<R> sameVarConstraint =
          SameVariableConstraint.newInstance(predicate);
    ...
    final IKeyBuilder keyBuilder = ndx.getIndexMetadata().getTupleSerializer().getKeyBuilder();
  ...
  /** @throws IllegalStateException unless {@link #init()} has been invoked. */
  ...
  public boolean isEmpty() {
    ...
    final IChunkedIterator<R> itr = iterator(0L /* offset */, 1L /* limit */, 1 /* capacity */);
  ...
  public ICloseableIterator<IBindingSet[]> solutions(
      final BOpContext context, final long limit, final BaseJoinStats stats) {

    //        final IVariable<?>[] vars = BOpUtility
    //                .getDistinctArgumentVariables(predicate);

    return context.solutions(iterator(0L /* offset */, limit, 0 /* capacity */), predicate, stats);
  }
   ...
   *     force the use of the {@link #synchronousIterator(long, long, Iterator)} when the offset or
   *     limit are non-zero, but that is only permitted up to a limit of {@link
   *     #MAX_FULLY_BUFFERED_READ_LIMIT}.
   *     <p>FIXME in order to support large limits we need to verify that the asynchronous iterator
   *     can correctly handle REMOVEALL and that incremental materialization up to the [limit] will
   *     not effect the semantics for REMOVEALL or the other iterator flags (per above). (In fact,
   *     the asynchronous iterator does not support either [offset] or [limit] at this time).
   *     <p>FIXME write unit tests for slice handling by this method and modify the SAIL integration
   *     to use it for SLICE on an {@link IAccessPath} scan. Note that there are several {@link
   *     IAccessPath} implementations and they all need to be tested with SLICE.
   *     <p>Those tests should be located in {@link org.embergraph.rdf.spo.TestSPOAccessPath}.
  ...
  public final IChunkedOrderedIterator<R> iterator(final long offset, long limit, int capacity) {
    ...
    if (isFullyBoundForKey) {
      ...
      if (DEBUG) log.debug("Predicate is fully bound for the key.");
      ...
      final long rangeCountRemaining = rangeCount(false /* exact */) - offset;
    ...
    final ITupleIterator<R> tupleItr = rangeIterator(capacity, flags, indexLocalFilter);

    // Wrap raw tuple iterator with resolver that materializes the elements
    // from the visited tuples.
    final Iterator<R> src = new Striterator(tupleItr).addFilter(new TupleObjectResolver());
    ...
    if (fullyBufferedRead) {
      ...
      final IChunkedOrderedIterator<R> tmp = synchronousIterator(offset, limit, src);
   ...
   * @param offset The first element that will be materialized (non-negative).
   * @param limit The maximum #of elements that will be materialized (must be positive, so use a
   *     range count before calling this method if there was no limit specified by the caller).
   *     <p>FIXME pass the offset and limit into the source iterator and remove them from this
   *     method's signature. This will require a change to the {@link IRangeQuery} API and {@link
   *     ITupleIterator} impls.
   */
  @SuppressWarnings("unchecked")
  protected final IChunkedOrderedIterator<R> synchronousIterator(
    ...
    assert limit < MAX_FULLY_BUFFERED_READ_LIMIT
        : "limit=" + limit + ", max=" + MAX_FULLY_BUFFERED_READ_LIMIT;
    ...
    while (nused < limit && src.hasNext()) {
      ...
      if (buffer == null) {
        ...
        buffer = (R[]) java.lang.reflect.Array.newInstance(e.getClass(), (int) limit);
      }
   ...
   * @return
   * @throws RejectedExecutionException if the {@link ExecutorService} is shutdown or has a maximum
   *     capacity and is saturated.
   */
  protected final IChunkedOrderedIterator<R> asynchronousIterator(final Iterator<R> src) {
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private static class ChunkConsumerTask<R> implements Callable<Void> {
    ...
    public ChunkConsumerTask(
        final AccessPath<R> accessPath, final Iterator<R> src, final BlockingBuffer<R[]> buffer) {
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        while (src.hasNext()) {
          ...
          if (DEBUG)
            log.debug(
                "#chunks=" + nchunks + ", chunkSize=" + chunk.length + ", nelements=" + nelements);
  ...
  @Override
  public final long rangeCount(final boolean exact) {
    ...
    if (DEBUG) {
      ...
      log.debug("exact=" + exact + ", filter=" + hasFilter + ", n=" + n + " : " + toString());
    }
  ...
  @SuppressWarnings({"unchecked"})
  protected ITupleIterator<R> rangeIterator(
   ...
   * backing index.
   *
   * <p>Note: If you are maintaining multiple indices then you MUST override this method to remove
   * the data from each of those indices.
   */
  @Override
  public long removeAll() {
    ...
    final ITupleIterator<?> itr =
        rangeIterator(0 /* capacity */, IRangeQuery.REMOVEALL, indexLocalFilter);
  ...
  public ScanCostReport estimateCost() {
    ...
    if (ndx instanceof UnisolatedReadWriteIndex) {
      ...
      return ((UnisolatedReadWriteIndex) ndx)
          .estimateCost(diskCostModel, rangeCount(false /* exact */));
    }
    ...
    if (ndx instanceof ILocalBTreeView) {
      ...
      return estimateCost((ILocalBTreeView) ndx, rangeCount, fromKey, toKey);
    }
   ...
   */
  private ScanCostReport estimateCost(
    ...
    final double cost =
        new BTreeCostModel(diskCostModel)
            .rangeScan(
                rangeCount,
                stats.getBranchingFactor(),
                stats.getHeight(),
   ...
   */
  private static ScanCostReport estimateCost(
      final ILocalBTreeView view, final long rangeCount, final byte[] fromKey, final byte[] toKey) {
    ...
    for (AbstractBTree source : sources) {
      ...
      if (source instanceof IndexSegment) {
        ...
        final int bytesPerLeaf = (int) Math.ceil(((double) extentLeaves) / leafCount);

        cost +=
            new IndexSegmentCostModel(diskCostModel)
                .rangeScan(
   ...
   * @return
   * @todo Remote scans can be parallelized. If flags includes PARALLEL then the cost can be as
   *     little as the cost of scanning one shard. However, the {@link IClientIndex} has a
   *     configuration value which specifies the maximum parallelism of any given operation (this is
   *     self-reported if we cast to the implementation class). Further, even if we assume that the
   *     shards are evenly distributed over the nodes, when the #of shards is significantly larger
   *     than the #of nodes then the scan can interfere with itself. Finally, this should include an
   *     estimate of the RMI overhead.
   */
  private ScanCostReport estimateCost(final IScaleOutClientIndex ndx) {
    ...
    final IMetadataIndex mdi = ndx.getFederation().getMetadataIndex(name, timestamp);
    ...
    final long rangeCountOnJournal = rangeCount / (partitionCount * (njournals + nsegments));

    final double costPerJournal =
        new BTreeCostModel(diskCostModel)
            .rangeScan(
                rangeCountOnJournal,
    ...
    final double costPerSegment = diskCostModel.seekTime + Bytes.megabyte * 100;
  ...
  private static final class EstimateShardScanCost
      implements ISimpleIndexProcedure<ScanCostReport> {
    ...
    public EstimateShardScanCost(final long rangeCount, final byte[] fromKey, final byte[] toKey) {
    ...
    public ScanCostReport apply(final IIndex ndx) {
      ...
      final ScanCostReport scanCostReport =
          AccessPath.estimateCost(((ILocalBTreeView) ndx), rangeCount, fromKey, toKey);
   ...
   * cost model.
   *
   * @todo This is not parameterized. A simple cost model is always assumed. The correct cost model
   *     is necessary in order to get the tradeoff point right for SCAN+FILTER versus SUBQUERY on
   *     SSD or RAID arrays with lots of spindles versus normal disk.
   * @todo In a shared disk deployment, we might introduce one cost model for local SSD used to
   *     cache journals, one for local non-SSD disks used to cache index segments, and one for
   *     remote storage used to materialize historical journals and index segments for query.
   * @todo In a federation, this should be reported out as metadata for the federation. Perhaps as a
   *     Jini attribute. Or we could self-publish this using a System property whose value was
   *     either the name of the desired cost model enum or a representation of the cost model which
   *     we could then parse.
   */
