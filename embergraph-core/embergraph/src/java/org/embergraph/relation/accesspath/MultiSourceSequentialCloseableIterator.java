...
public class MultiSourceSequentialCloseableIterator<E> implements IMultiSourceCloseableIterator<E> {
   ...
   *
   * <p>Note: This can be asynchronously closed if the application invokes {@link #close()}. Methods
   * which test on this can not assume that it will be non-<code>null</code> the next time they
   * check unless they are holding the {@link #lock}. Methods which do not obtain the lock can offer
   * a weaker atomicity by copying the reference to a local variable and then testing that variable.
   */
  ...
  public void close() {
    ...
    try {
      ...
      this.current = null;
      if (current != null) {
        if (INFO) log.info("Closing source: " + current);
        current.close();
      }
      // Close any sources still in the queue.
      for (ICloseableIterator<E> t : sources) {
        if (INFO) log.info("Closing source: " + t);
        t.close();
      }
      // Clear the queue.
      sources.clear();
   ...
   *
   * @return The next source -or- <code>null</code> if there are no sources available.
   */
  private ICloseableIterator<E> nextSource() {
    final ICloseableIterator<E> tmp = current;
    if (tmp == null) return null;
    if (tmp.hasNext()) return current; // Note: MAY be asynchronously cleared!
    ...
    try {
       ...
       *     MultiSourceSequentialCloseableIterator.nextSource() can throw NPE </a>
       */
   ...
   *     for {@link #hasNext()} to report true and for {@link #next()} to throw {@link
   *     NoSuchElementException} if the iterator has been concurrently closed.
   */
  @Override
  public E next() {
    while (true) {
      ...
      if (tmp.hasNext()) return tmp.next();
    }
  }
