 ...
 * (they can not set the {@link Future} on the objects returned by this factory or obtain its {@link
 * IBlockingBuffer#iterator()}, but those operations can be performed against the shared {@link
 * IBlockingBuffer}).
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: MultiplexBlockingBuffer.java 3830 2010-10-20 18:30:25Z thompsonbry $
 */
public class MultiplexBlockingBuffer<E> {
  ...
  public IBlockingBuffer<E> newInstance() {
    ...
    try {
      ...
      if (!set.add(n)) throw new AssertionError();
      counter++;
      return n;
    } finally {
      lock.unlock();
   ...
   * semantics are likely to cause problems when interpreted in the light of a skin over a shared
   * buffer. The only way these methods could be given clear semantics is if the skin were actually
   * a full {@link BlockingBuffer} which was coupled to the shared buffer. However, that involves
   * double buffering and double copying and I do not think that this is worth it.
   */
  private class InnerBlockingBuffer implements IBlockingBuffer<E> {
    ...
    public void abort(final Throwable cause) {
      lock.lock();
      try {
        if (!innerBufferOpen) throw new BufferClosedException();
        b.abort(cause);
      } finally {
        lock.unlock();
      }
