...
public abstract class AbstractUnsynchronizedArrayBuffer<E> implements IBuffer<E> {
   ...
   */
  public AbstractUnsynchronizedArrayBuffer(final int capacity, final Class<? extends E> cls) {
   ...
   */
  public AbstractUnsynchronizedArrayBuffer(
      final int capacity, final Class<? extends E> cls, final IElementFilter<E> filter) {
   ...
   *
   * @param e An element.
   * @return <code>true</code> iff adding the element caused the buffer to {@link #overflow()}
   */
  ...
  public boolean add2(final E e) {
    ...
    if (!accept(e)) {
      ...
      if (DEBUG) log.debug("reject: " + e.toString());
  ...
  protected void overflow() {
    ...
    if (size > 0) {
      ...
      if (INFO) log.info("size=" + size + ", dense=" + dense);
