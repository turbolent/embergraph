...
public class UnsynchronizedUnboundedChunkBuffer<E> extends AbstractUnsynchronizedArrayBuffer<E> {
   ...
   */
  public UnsynchronizedUnboundedChunkBuffer(final int capacity, final Class<? extends E> cls) {
   ...
   * @param keyOrder The order of the elements in the chunks written onto the buffer or <code>null
   *     </code> iff not known.
   */
  public UnsynchronizedUnboundedChunkBuffer(
      final int capacity,
    ...
    synchronized (queue) {
      ...
      final Iterator<E[]> src =
          Arrays.asList(
                  (E[][])
                      queue.toArray(
                          (E[][]) java.lang.reflect.Array.newInstance(chunkClass, 0 /* size */)))
              .iterator();
  ...
  private static class ListOfChunksIterator<E> implements IChunkedOrderedIterator<E> {
     ...
     */
    public ListOfChunksIterator(final Iterator<E[]> src, final IKeyOrder<E> keyOrder) {
    ...
    /** The current chunk or null if another chunk should be read from the queue. */
    private E[] chunk;
    ...
    public E[] nextChunk() {
      ...
      if (i == 0) {
        ...
        ret =
            (E[])
                java.lang.reflect.Array.newInstance(chunk.getClass().getComponentType(), remaining);
