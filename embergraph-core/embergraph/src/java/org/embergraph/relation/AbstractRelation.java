 ...
 */
public abstract class AbstractRelation<E> extends AbstractResource<IRelation<E>>
   ...
   */
  public static <E> String getFQN(
      final IRelation<E> relation, final IKeyOrder<? extends E> keyOrder) {
   ...
   *
   * @param relation The relation.
   * @param localName The local name of the index.
   * @return The fully qualified index name.
   */
  public static <E> String getFQN(final IRelation<E> relation, final String localName) {
   ...
   * @see #getIndex(String)
   * @todo For efficiency the concrete implementations need to override this saving a hard reference
   *     to the index and then use a switch like construct to return the correct hard reference.
   *     This behavior should be encapsulated.
   */
  @Override
   ...
   * @return The named index -or- <code>null</code> iff the named index does not exist as of that
   *     timestamp.
   * @throws IllegalArgumentException if <i>name</i> is <code>null</code>.
   * @see BOpContextBase#getIndex(IIndexManager, String, long)
   * @todo hard references to the indices must be dropped when an abort is processed. this is a bit
   *     awkward. the abort() could be raised into the relation container (it is for the
   *     AbstractTripleStore) and into the relation itself to facilitate this. alternatively the
   *     index objects themselves could be notified of an abort and make themselves invalid (this
   *     really only applies to the unisolated index and to indices isolated by a transaction).
   */
   ...
   * access to a named index, neither the {@link Journal} nor the {@link TemporaryRawStore} does
   * this. Therefore this method encapsulates the unisolated index for the latter classes in order
   * to impose the correct concurrency constraints. It does this using an {@link
   * UnisolatedReadWriteIndex}. This allows the caller to use the returned index view without regard
   * to concurrency controls (it will appear to be a thread-safe object).
   *
   ...
   * @return The named index -or- <code>null</code> iff the named index does not exist as of that
   *     timestamp.
   * @throws IllegalArgumentException if <i>indexManager</i> is <code>null</code>.
   * @throws IllegalArgumentException if <i>name</i> is <code>null</code>.
   */
  public static IIndex getIndex(
      final IIndexManager indexManager, final String fqn, final long timestamp) {

    if (indexManager == null) throw new IllegalArgumentException();

    if (fqn == null) throw new IllegalArgumentException();
   ...
   *     a shard local access path in scale-out).
   * @param predicate The predicate used to request the access path.
   * @param keyOrder The index which the access path will use.
  ...
  @Override
  public final IAccessPath<E> getAccessPath(final IPredicate<E> predicate) {

    return getAccessPath(getKeyOrder(predicate), predicate);
  }

  @Override
  public final IAccessPath<E> getAccessPath(
      final IKeyOrder<E> keyOrder, final IPredicate<E> predicate) {
  ...
  public final IAccessPath<E> getAccessPath(
    ...
    if (expander != null) {
       ...
       */
      final IPredicate<E> predicateWithoutExpander =
          (IPredicate<E>)
              ((Predicate<E>) predicate).clearProperty(IPredicate.Annotations.ACCESS_PATH_EXPANDER);

      // Obtain access path for the predicate w/o the expander annotation.
      accessPath = newAccessPath(localIndexManager, predicateWithoutExpander, keyOrder);
