 ...
 * within a given buddy bucket.
 *
 * <p>Note: Overflow pages must be provided for when the #of duplicates in a bucket exceeds the
 * capacity of the bucket (this can be done by conversion of the bucket to a blob record or by
 * chaining the bucket).
 *
 ...
 *     <p>TODO Consider mutable implementation based on a compacting record ala GOM. This is
 *     especially attractive for the hash tree. The implementation would have to be wholly different
 *     from the {@link MutableLeafData} class. Instead of managing the {@link IRaba} for the keys
 *     and values separately, each {@link ITuple} would be appended into a byte[] (or {@link
 *     IDataRecord}). There would be a budget for that backing buffer which is the maximum in-memory
 *     size of the bucket. An index would provide random access into the buffer for only those
 *     entries which are "live" and a counter is maintain of the #of entries in the buffer which are
 *     no longer in use. When the buffer capacity is reached, the buffer is compacted by copying all
 *     of the entries accessible from the index into a new buffer and the old buffer is released.
 *     <p>Records which are too large for the buffer should be moved out of line.
 *     <p>This can be used in combination with a dynamically maintained trie for fast hash lookup,
 *     or we could just scan the entries.
 *     <p>Lexicon key search can scan the entries using the index. Scanning can have a side-effect
 *     in which the position of the entry offsets in the index is swapped if the keys are out of
 *     order. This would give us MonetDB style "cracking". The keys would have to be put into full
 *     order no later than when the record is made persistent.
 *     <p>Even though mutation is not thread safe, compacting the data record must not cause the
 *     assignment of indices to tuples to change when the caller is iterating through the entries by
 *     index.
 *     <p>TODO Consider disallowing delete markers and version timestamps for the {@link HTree} if
 *     it is to be used only with in unisolated contexts in which case we can drop support for that
...
public class MutableBucketData implements ILeafData {
  ...
  protected final boolean rangeCheckTupleIndex(final int index) {
    ...
    if (index < 0 || index > keys.capacity()) throw new IndexOutOfBoundsException();
...
//     * For a leaf the #of entries is always the #of keys.
   ...
   */
  public void insert(
      final int insIndex, final byte[] key, final byte[] val, final boolean rawRecord) {
    ...
    if (hasDeleteMarkers()) {
      System.arraycopy(
          deleteMarkers, insIndex, deleteMarkers, insIndex + 1, vals.nvalues - insIndex - 1);
  ...
  public void remove(final int index) {
    if (hasDeleteMarkers()) {
      ...
      if (hasVersionTimestamps()) {
        System.arraycopy(
            versionTimestamps, index + 1, versionTimestamps, index, vals.nvalues - index);
