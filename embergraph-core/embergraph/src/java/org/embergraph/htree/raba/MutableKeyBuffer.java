 ...
 *
 * <p>While the total #of non-null keys is reported by {@link #size()}, this is the value for the
 * bucket page as a whole. The {@link HTree} must explicitly examine a buddy hash bucket and count
 * the non-<code>null</code> keys in order to know the "size" of a given buddy hash bucket.
 *
 * <p>Note: Because the slots are divided logically among the buddy buckets any slot may have a non-
 * <code>null</code> key and the {@link IRaba} methods as implemented by this class DO NOT range
 * check the index against {@link #size()}.
 *
...
public class MutableKeyBuffer implements IRaba {
  ...
  public MutableKeyBuffer(final int nkeys, final byte[][] keys) {
    ...
    if (nkeys < 0 || nkeys > keys.length) throw new IllegalArgumentException();
   ...
   * buffer.
   *
   * @param src An existing instance.
   */
   ...
   * Unless there is only one buddy bucket on the page, you MUST explicitly scan a buddy bucket to
   * determine the #of keys in a buddy bucket on the page.
   */
  @Override
  ...
  @Override
  public final void set(final int index, final byte[] key) {
  ...
  public final void insert(final int index, final byte[] key) {
   ...
   * one.
   *
   * @param index The index in [0:{@link #capacity()}-1].
   * @param key The key.
   ...
   * zero.
   *
   * <p>Note that for duplicate keys the returned search index reflects the first location in the
   * array.
   */
  @Override
   ...
   *
   * <p>Note that it does not return the first found position but the first index position to match
   *
   * @param key
  ...
  private int search(final byte[] key, final byte[][] keys, final int start, final int length) {
    if (length == 1) {
      ...
      if (tstkey == null) return -(start + 1); // indicate insertion at start
