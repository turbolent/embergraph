 ...
 * and quickly achieves a maximum size for any given {@link HTree}.
 *
...
public class NodeSerializer {
   ...
   *
   * <p>Note: It is important that this field NOT be used for a read-only {@link HTree} since only
   * mutable {@link HTree}s are single threaded - concurrent readers are allowed for read-only
   * {@link HTree}s.
   *
   ...
   *     ILeafData leaves}.
   * @param addressBits The #of address bits for target {@link HTree}.
   ...
   *     being serialized for the {@link HTree}. When zero (0), a default is used. A non-zero value
   *     is worth specifying only when the actual buffer size is consistently less than the default
   *     for some {@link HTree}. See {@link #DEFAULT_BUFFER_CAPACITY_PER_ENTRY}
   ...
   */
  public AbstractPage wrap(
      final AbstractHTree btree, final long addr, final IAbstractNodeData data) {
  ...
  public <T extends IAbstractNodeData> T encodeLive(final T node) {
    ...
    if (node.isLeaf()) {
      ...
      codedNode = (T) nodeCoder.encodeLive((IDirectoryData) node, _writeBuffer);
    }
