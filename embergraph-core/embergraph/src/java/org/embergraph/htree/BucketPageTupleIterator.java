 ...
 */
class BucketPageTupleIterator<E> implements ITupleIterator<E> {
  ...
  BucketPageTupleIterator(
      final AbstractHTree htree, final int flags, final Iterator<BucketPage> src) {
  ...
  private boolean findNextSlot() {
    if (currentBucketPage == null) throw new IllegalStateException();
