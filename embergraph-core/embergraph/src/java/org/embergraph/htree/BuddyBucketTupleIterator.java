 ...
 */
class BuddyBucketTupleIterator<E> implements ITupleIterator<E> {
  ...
  public BuddyBucketTupleIterator(
      final byte[] key, final BucketPage bucket) { // , final int buddyOffset) {
   ...
   * LeafTupleIterator#remove()}. I also did a listener based iterator for GOM which supports
   * concurrent mutation (as long as you obey the thread safety for the API).
   */
  public void remove() {
