 ...
 * @author thompsonbry
 */
abstract class AbstractPage extends PO
   ...
   * custom method to read the node with the HTree reference on hand so that we can set this field.
   */
   ...
   * {@link HTree}) or a dirty child (held by the {@link DirectoryPage}). The parent reference is
   * set when a node is attached as the child of another node.
   *
   ...
   * dynamically when the child will be materialized by counting the #of pointers to the the child
   * in the appropriate buddy hash table in the parent. This local depth value is passed into the
   * constructor when the child is materialized to set the global depth of the child.
   */
   ...
   *
   * <p>When the global depth is increased, the hash table requires twice as many slots on the page.
   * This forces the split of the directory page onto two pages in order to accommodate the
   * additional space requirements. The maximum global depth is <code>addressBits</code>, at which
   * point the hash table fills the entire directory page. The minimum global depth is ZERO (0), at
   * which point the buddy hash table has a single slot.
   *
   * <p>The global depth of a child page is just the local depth of the directory page in its
   * parent. The global depth of the child page is often called its <em>local depth</em>.
   *
   * <p>The global depth of the root is always <i>addressBits</i>.
   */
   ...
   *
   * <p>If the key does not have enough bits (to retrieve globalDepth bits at prefixLength) then
   * assume the equivalent of a zero-filled extension.
   *
   * @param key The key.
   * @param prefixLength The #of MSB bits in the key which are to be ignored at this level of the
   *     hash tree. This is computed dynamically during recursive traversal of the hash tree. This
   *     is ZERO (0) for the root directory. It is incremented by <i>globalDepth</i> (the #of
   *     address bits used by a given node) at each level of recursion for insert, lookup, etc.
  ...
  public int getLocalHashCode(final byte[] key, final int prefixLength) {
    ...
    if (prefixLength >= maxbits) return 0;
  ...
  protected AbstractPage(final AbstractPage src) {
    ...
    assert src == htree.root || (src.parent != null && src.parent.get() != null);
   ...
   * @param materialize When <code>true</code>, children will be materialized as necessary to dump
   *     the tree.
   * @return <code>true</code> unless an inconsistency was detected.
   */
  protected abstract boolean dump(
      Level level, PrintStream out, int height, boolean recursive, boolean materialize);
   ...
   */
  public final Iterator<AbstractPage> postOrderNodeIterator(final boolean dirtyNodesOnly) {
   ...
   * then the new root will be set on the {@link HTree}. This method must MUST be invoked any time
  ...
  protected AbstractPage copyOnWrite(final long triggeredByChildId) {
    ...
    if (htree.root == this) {
      ...
      if (INFO) log.info("Copy-on-write : replaced root node on htree.");
   ...
   * HTree}. They can cause {@link BucketPage}s to become full and split even as we are trying to
   * handle the full {@link BucketPage} which started off this cascade of mutation. Eventually,
   * things settle down.
   *
   * <p>FIXME Replace with an interface declaring push(AbstractPage) and pop() methods. Write two
   * implementations of the interface. One is a NOP. The other does not use a stack, but just
   * increments and decrements the reference count on the {@link AbstractPage}, checking triggering
   * eviction on decrement if zero. Use the NOP version if the HTree is read-only ( loaded from a
   * checkpoint and not mutable) and the other version otherwise. Remember, the mutable {@link
   * HTree} is single threaded.
   */
  ...
  /** Return <code>true</code> iff self and all materialized children are clean. */
  abstract boolean isClean();

  public abstract int removeAll(final byte[] key);

  public abstract byte[] removeFirst(final byte[] key);

  public abstract void dumpPages(
