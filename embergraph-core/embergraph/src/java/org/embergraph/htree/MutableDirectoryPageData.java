 ...
 *     <p>FIXME Unit tests for this class, including constructors.
 */
public class MutableDirectoryPageData implements IDirectoryData {
   ...
   * persisting child nodes requires that we use a pre-order traversal (the general case is a
   * directed graph) so that we can update the addresses record in the parent before the parent
   * itself is persisted. Unlike the {@link BTree}, a {@link IRawStore#NULL} entry is permitted in
   * the array. This is because the {@link HTree} is not balanced and empty paths may be folded up.
   */
...
//	 * The #of entries spanned by each direct child of this node.
   ...
   * @param hasVersionTimestamps <code>true</code> iff the HTree is maintaining per tuple version
   *     timestamps.
   */
  public MutableDirectoryPageData(
      final byte[] overflowKey, final int addressBits, final boolean hasVersionTimestamps) {
   ...
   *     various arrays to the correct capacity.
