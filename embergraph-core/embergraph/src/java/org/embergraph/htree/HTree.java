 ...
 *     Multilevel Extendible Hashing </a> by Sven Helmer, Thomas Neumann, Guido Moerkotte. ISCIS
 *     2003: 220-227.
 *     <p>TODO It should be possible to define an native int32 hash table in parallel to the
 *     unsigned byte[] hash table simply by having an alternative descent passing an int32 key all
 *     the way down and using the variant of getBits() method which operates on the int32 values. We
 *     could also optimize the storage and retrieval of the int32 keys, perhaps with a custom
 *     mutable bucket page and mutable bucket data implementation for int32 keys. This optimization
 *     is likely to be quite worth while as the majority of use cases for the hash tree use int32
 *     keys.
 *     <p>TODO It is quite possible to define a range query interface for the hash tree. You have to
 *     use an order preserving hash function, which is external to the HTree implementation.
 *     Internally, the HTree must either double-link the pages or crawl the directory structure.
 *     <p>TODO The keys should be declared as a computed key based on the data fields in the record.
 *     The {@link HTree} supports arbitrary bit length keys, but can be optimized for int32 keys
 *     easily enough.
 *     <p>TODO Instrument performance counters for structural modifications, insert, remove, etc.
 *     per {@link BTreeCounters}.
 */
...
//  IRangeQuery
{
   ...
   */
  public final void setIndexMetadata(final HTreeIndexMetadata indexMetadata) {
...
//     * Handle request for a commit by {@link #writeCheckpoint()}ing dirty nodes
   ...
   * @param checkpoint A {@link Checkpoint} record for that {@link HTree}.
   * @param metadata The metadata record for that {@link HTree}.
   * @param readOnly When <code>true</code> the {@link HTree} will be immutable.
  ...
  private final void newRoot() {
    ...
    nnodes++;
    assert r.getSlotsPerBuddy() == (1 << addressBits) : "slotsPerBuddy=" + r.getSlotsPerBuddy();
  ...
  public final boolean flush() {
    ...
    if (root != null && root.isDirty()) {
      ...
      if (INFO) log.info("flushed root: addr=" + root.getIdentity());
  ...
  public HTree asReadOnly() {
    ...
    return HTree.load(store, checkpoint.getCheckpointAddr(), true /* readOnly */);
  }
  ...
  private final Checkpoint _writeCheckpoint2() {
    ...
    if (metadata.getMetadataAddr() == 0L) {
      ...
      if (checkpoint != null) {
        ...
        if (addr != IRawStore.NULL) store.delete(addr);
      }
   ...
   *
   * @param key The key.
   * @return <code>true</code> iff the hash tree contains at least one tuple with that <i>key</i>.
   * @throws IllegalArgumentException if the <i>key</i> is <code>null</code>.
   *     <p>TODO Parallel to insert(), consider a contains() signature which permits testing for a
   *     specific value as well. Maybe in a wrapper class?
   */
  public boolean contains(final byte[] key) {
    ...
    while (true) {
      ...
      buddyOffset =
          HTreeUtil.getBuddyOffset(
              hashBits, current.globalDepth, child.globalDepth /* localDepthOfChild */);
   ...
   *
   * @param key The key.
   * @return The first value for the key -or- <code>null</code> if there are no tuples in the index
   *     having that key. Note that the return value is not diagnostic if the application allows
   ...
   *     <p>TODO If the application wants to restrict the hash tree to such that it does not contain
   *     duplicate tuples then it must first search in the tree for an exact match (key and value).
   *     It is easier to do that from within the insert logic so expand the method signature to pass
   *     an insert enum {ALLDUPS,DUPKEYS,NODUPS}.
   */
  public byte[] insert(final byte[] key, final byte[] value) {

    if (DEBUG) log.debug("key=" + BytesUtil.toString(key) + ", value=" + Arrays.toString(value));
   ...
   * keys and values are allowed and will result in multiple tuples.
   *
   * @param src The source {@link BucketPage} src
   * @param slot The slot in the source {@link BucketPage} whose tuple will be inserted (really
   *     copied).
   */
   ...
   * @throws IllegalStateException If you attempt to create two {@link HTree} objects from the same
   ...
   */
  public static HTree create(final IRawStore store, final HTreeIndexMetadata metadata) {
   ...
   * IRawStore}).
   *
   * <p>Fully transient {@link HTree}s provide the functionality of a {@link HTree} without a
   * backing persistence store. Internally, reachable nodes and leaves of the transient {@link
   * HTree} use hard references to ensure that remain strongly reachable. Deleted nodes and leaves
   * simply clear their references and will be swept by the garbage collector shortly thereafter.
   *
   ...
   * HTree} are unsigned byte[]s. This means that application keys and values are always converted
   * into unsigned byte[]s before being stored in the {@link HTree}. Hence if an object that is
   * inserted into the {@link HTree} and then looked up using the {@link HTree} API, you WILL NOT
   ...
   * @return The {@link HTree} or derived class loaded from that {@link Checkpoint} record.
   * @throws IllegalArgumentException if store is <code>null</code>.
   */
  @SuppressWarnings("unchecked")
  public static HTree load(
