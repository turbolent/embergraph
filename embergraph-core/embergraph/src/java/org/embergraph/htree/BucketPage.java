 ...
 *
 * <p>The entire bucket page is logically a fixed size array of tuples. The individual buddy hash
 * buckets are simply offsets into that logical tuple array. While inserts of distinct keys drive
 * splits, inserts of identical keys do not. Therefore, this simple logical picture is upset by the
 * need for a bucket to hold an arbitrary number of tuples having the same key.
 *
 * <p>Each tuple is represented by a key, a value, and various metadata bits using the {@link
 * ILeafData} API. The tuple keys are always inline within the page and are often 32-bit integers.
 * The tuple values may be either "raw records" on the backing {@link IRawStore} or inline within
 * the page.
 *
 * <p>The {@link ILeafData#getPriorAddr()} and {@link ILeafData#getNextAddr()} fields of the {@link
 * ILeafData} record are reserved by the hash tree to encode the search order for range queries when
 * used in combination with an order preserving hash function.
 *
 * <p>TODO One way to tradeoff the simplicity of a local tuple array with the requirement to hold an
 * arbitrary number of duplicate keys within a bucket is to split the bucket if it becomes full
 * regardless of whether or not there are duplicate keys.
 *
 * <p>Splitting a bucket doubles its size which causes a new bucket page to be allocated to store
 * 1/2 of the data. If the keys can be differentiated by increasing the local depth, then this is
 * the normal case and the tuples are just redistributed among buddy buckets on the original bucket
 * page and the new bucket page. If the keys are identical but we force a split anyway, then we will
 * still have fewer buckets on the original page and they will be twice as large. The duplicate keys
 * will all wind up in the same buddy bucket after the split, but at least the buddy bucket is 2x
 * larger. This process can continue until there is only a single buddy bucket on the page (the
 * global depth of the parent is the same as the global depth of the buddy bucket). At that point, a
 * "full" page can simply "grow" by permitting more and more tuples into the page (as long as those
 * tuples have the same key). We could also chain overflow pages at that point - it all amounts to
 * the same thing. An attempt to insert a tuple having a different key into a page in which all keys
 * are known to be identical will immediately trigger a split. In this case we could avoid some
 * effort if we modified the directory structure to impose a split since we already know that we
 * have only two distinct keys (the one found in all tuples of the bucket and the new key). When a
 * bucket page reaches this condition of containing only duplicate keys we could of course compress
 * the keys enormously since they are all the same.
 *
 * <p>This works well when we only store the address of the objects in the bucket (rather than the
 * objects themselves, e.g., raw records mode) and choose the address bits based on the expected
 * size of a tuple record. However, we can also accommodate tuples with varying size (think binding
 * sets) with in page locality if split the buddy bucket with the most bytes when an insert into the
 * page would exceed the target page size. This looks pretty much like the case above, except that
 * we split buddy buckets based on not only whether their alloted #of slots are filled by tuples but
 * also based on the data on the page.
 *
 * <p>TODO Delete markers will also require some thought. Unless we can purge them out at the tx
 * commit point, we can wind up with a full bucket page consisting of a single buddy bucket filled
 * with deleted tuples all having the same key.
 *
 * <p>TODO Explore cracking. Concerning cracking, we need to be careful about the thread-safety
 * guarantee. If we did cracking for a mutation operation, that would be Ok since we are single
 * threaded. However, we could not do cracking for a read operation even against a mutable HTree
 * since we allow concurrent read operations as long as there is no writer.
 */
class BucketPage extends AbstractPage implements ILeafData, IRawRecordAccess {
   ...
   *
   * <p>Note: This is package private in order to expose it to {@link HTree}.
   */
  ILeafData data;
   ...
   *
   * @param key The key.
   * @param buddyOffset The offset within the {@link BucketPage} of the buddy hash bucket to be
   *     searched.
   * @return <code>true</code> if a tuple is found in the buddy hash bucket for the specified key.
   */
   ...
   * @param buddyOffset The offset within the {@link BucketPage} of the buddy hash bucket to be
   *     searched.
   * @return The value associated with the first tuple found in the buddy hash bucket for the
   *     specified key and <code>null</code> if no such tuple was found. Note that the return value
   *     is not diagnostic if the application allows <code>null</code> values into the index.
   */
  final byte[] lookupFirst(final byte[] key, final int buddyOffset) {
    ...
    if (hasRawRecords()) {
      ...
      if (addr != IRawStore.NULL) return getBytes(readRawRecord(addr));
    }
   ...
   *
   * @param key The key.
   * @return An iterator which will visit each tuple in the buddy hash table for the specified key
   *     and never <code>null</code>.
   *     <p>TODO Specify the contract for concurrent modification both here and on the {@link
   *     HTree#lookupAll(byte[])} methods.
   */
  //	 * @param buddyOffset
   ...
   * @throws IllegalArgumentException if <i>parent</i> is <code>null</code>.
   * @throws IndexOutOfBoundsException if <i>buddyOffset</i> is out of the allowed range.
   */
  boolean insert(final byte[] key, final byte[] val) {
    ...
    //		try {
    /** In any event: create new bucket page and insert key/value */
    ...
    if (pd.isOverflowDirectory()) { // already handles blobs
      ...
      if (pd.getLevel() * htree.addressBits > key.length * 8) throw new AssertionError();
    ...
    // assert (1 << (htree.addressBits - this.globalDepth)) ==
    // getParentDirectory().countChildRefs(this);
    // assert (1 << (htree.addressBits - newPage.globalDepth)) ==
    // getParentDirectory().countChildRefs(newPage);
   ...
   * are not materialized during the copy); (b) it correctly handles version counters and delete
   * markers; and (c) the #of tuples in the index is unchanged.
   *
   * @param srcPage The source {@link BucketPage}.
   * @param srcSlot The slot in that {@link BucketPage} having the tuple to be copied.
   * @param key The key (already materialized).
   * @param parent The parent {@link DirectoryPage} and never <code>null</code> (this is required
   *     for the copy-on-write pattern).
   ...
   * @throws IndexOutOfBoundsException if <i>buddyOffset</i> is out of the allowed range.
   */
  boolean insertRawTuple(
      final BucketPage srcPage, final int srcSlot, final byte[] key) { // , final int buddyOffset) {
  ...
  private class InnerBucketPageTupleIterator<E> implements ITupleIterator<E> {
    ...
    private boolean findNextSlot() {
      ...
      for (; nextNonEmptySlot < size; nextNonEmptySlot++) {
        if (keys.isNull(nextNonEmptySlot)) continue;
        return true;
      }
   ...
   * @return The pretty print representation of the value associated with the tuple at that slot.
   */
  private String PPVAL(final int index, final boolean showBinary) {

    if (index >= getKeys().size()) return "-";
    ...
    final String keyStr =
        showBinary
            ? BytesUtil.toString(key) + "(" + BytesUtil.toBitString(key) + ")"
   ...
   *
   * <p>The additional complexity of determining whether the page can really be split is left to the
   * parent. A new directory, covering the required prefixBits would initially be created with depth
   * 1. But if the specified bit is discriminated within buddy buckets AND other bits do not further
   * separate the buckets then the depth of the directory will need to be increased before the
   * bucket page can be split.
   *
   * @return bit depth increase from current offset required -or- <code>-1</code> if it is not
   *     possible to split the page no matter how many bits we have.
   */
  ...
  void addLevel() {
    /** Ensure parent directory is mutable - at present this requires making a Leaf mutable */
    final BucketPage copy = (BucketPage) copyOnWrite();
    if (copy != this) {
      copy.addLevel();
      return;
