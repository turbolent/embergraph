...
import org.embergraph.btree.IReadWriteLockManager;
import org.embergraph.btree.ISimpleTreeIndexAccess;
...
public abstract class AbstractHTree
    implements ICounterSetAccess, ICheckpointProtocol, ISimpleTreeIndexAccess {
   ...
   * pretty large page. <code>2^20</code> is 1,048,576. It is pretty difficult to imagine use cases
   * where the fan out for the {@link HTree} should be that large.
   */
...
//	 * The #of entries in a directory bucket, which is 2^{@link #addressBits}
   ...
   * on {@link DirectoryPage#childRefs}. This is package private since it must be visible to {@link
   * DirectoryPage#_getChild(int)}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  static class ChildMemoizer
      extends Memoizer<LoadChildRequest /* request */, AbstractPage /* child */> {

    /** @param c */
    public ChildMemoizer(final Computable<LoadChildRequest, AbstractPage> c) {
...
//         * Called from {@link AbstractBTree#close()}.
   ...
   * reference queue in combination with {@link #touch(AbstractPage)} and with hard references held
   ...
   *
   * <p>Note: The code in {@link AbstractPage#postOrderNodeIterator(boolean, boolean)} and {@link
   ...
   * the {@link WeakReference} to the child {@link DirectoryPage} or {@link BucketPage} using
   * top-down navigation as long as the {@link DirectoryPage} or {@link BucketPage} remains strongly
   * reachable (that is, as long as it is on the {@link #writeRetentionQueue} or otherwise strongly
   * held). This means that lookup in a map is not required for top-down navigation.
   */
   ...
   *
   * <p>Note: {@link AbstractHTree} is NOT thread-safe and {@link #close()} MUST be invoked in a
   ...
   *     HTree and there are mutations that have not been written through to the store)
   */
  @Override
    ...
    if (INFO || EmbergraphStatics.debug) {
      ...
      final String msg =
          "HTree close: name="
              ...
              + getEntryCount()
              + ", impl="
              + (this instanceof HTree
                  ? ((HTree) this).getCheckpoint().toString()
   ...
   * {@link HTree} is first created and will remain ZERO (0L) until the {@link HTree} is committed.
...
//     * The timestamp associated with unisolated writes on this index. This
   ...
   * then a 10 bit address space implies a 4k page size.
   */
   ...
   *     read-only.
   * @param metadata The {@link IndexMetadata} object for this {@link AbstractHTree}.
   * @param recordCompressorFactory Object that knows how to (de-)compress the serialized data
   *     records.
   * @throws IllegalArgumentException if addressBits is LT ONE (1).
   * @throws IllegalArgumentException if addressBits is GT (16).
   */
  protected AbstractHTree(
    ...
    if (addressBits < MIN_ADDRESS_BITS)
      throw new IllegalArgumentException(ERROR_TOO_SMALL + "addressBits=" + addressBits);

    if (addressBits > MAX_ADDRESS_BITS)
      throw new IllegalArgumentException(ERROR_TOO_LARGE + "addressBits=" + addressBits);
   ...
   * encounter a copy-on-write problem where the parent directory becomes immutable during a
   * mutation on the child.
   */
  IHardReferenceQueue<PO> newWriteRetentionQueue(final boolean readOnly) {

    final int writeRetentionQueueCapacity = metadata.getWriteRetentionQueueCapacity();

    final int writeRetentionQueueScan = metadata.getWriteRetentionQueueScan();
  ...
  public boolean dump(final Level level, final PrintStream out, final boolean materialize) {
   ...
   * queue and its {@link AbstractPage#referenceCount} is incremented. If a node is being appended
   ...
   *     index)
   */
  protected final void writeNodeRecursive(final AbstractPage node) {
  ...
  protected final void writeNodeRecursiveCallersThread(final AbstractPage node) {
     ...
     */
    final Iterator<AbstractPage> itr =
    ...
    while (itr.hasNext()) {
      ...
      if (t instanceof BucketPage) nleaves++;
    }
  ...
  protected final void writeNodeRecursiveConcurrent(final AbstractPage node) {
    ...
    //        assert dirtyLevelCount <= getHeight() + 1 : "dirtyLevelCount=" + dirtyLevelCount + ",
    // height=" + getHeight()
   ...
   *
   * <p>Note: The caller MUST set the {@link AbstractPage#globalDepth} on the returned value.
   *
   ...
   * @see WeakReference
   */
  final <T extends AbstractPage> Reference<AbstractPage> newRef(final AbstractPage child) {
   ...
   * {@link Reference} objects for {@link AbstractPage#self}, {@link AbstractPage#parent}, {@link
   * DirectoryPage#childRefs}, etc. and on the other hand by the impossibility of defining your own
   * direct subclass of {@link Reference} (a runtime security manager exception will result).
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @param <T>
   */
   ...
   * order. Tuples within a buddy hash bucket will be delivered in a random order.
   *
   * <p>Note: The {@link HTree} does not currently maintain metadata about the #of spanned tuples in
   * a {@link DirectoryPage}. Without that we can not provide fast range counts, linear list
   * indexing, etc.
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  public ITupleIterator rangeIterator() {

    return new BucketPageTupleIterator(
        this,
        IRangeQuery.DEFAULT /* flags */,
        new Striterator(getRoot().postOrderIterator())
            .addFilter(
                new Filter() {
   ...
   *
   * <p>Note: This allocates a new byte[] for each visited value. It is more efficient to reuse a
   * buffer for each visited {@link Tuple}. This can be done using {@link #rangeIterator()}.
   */
  @SuppressWarnings("unchecked")
  public Iterator<byte[]> values() {

    return new Striterator(rangeIterator())
        .addFilter(
            new Resolver() {
              private static final long serialVersionUID = 1L;
