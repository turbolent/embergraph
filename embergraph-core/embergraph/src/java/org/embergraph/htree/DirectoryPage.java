...
class DirectoryPage extends AbstractPage implements IDirectoryData {
   ...
   * index before each descent into a child.
   *
   * @param hashBits The relevant bits from the key.
   * @param buddyOffset The offset into the child of the first slot for the buddy hash table or
   *     buddy hash bucket.
   ...
   * slots (by testing the first key value) and leaving the other half as null.
   *
   * <p>This will result in a lazy creation of the second BucketPage if required. Such that we could
   * end up introducing several new layers without the needless creation of empty BucketPages.
   */
   ...
   *
   * <p>If all slots are empty, then they will all be filled with references to the new page.
   *
   * <p>The regions tested follow the pattern below: [A A A A A A A A] [A A A A B B B B] [A A B B C
   * C D D] [A B C D E F G H]
   *
   * <p>Examples with 8 slots, test slot 'T' [- - - - T - - -] // all empty [S S S S S S S S] // set
   * ALL 8
   *
   * <p>[F F - - T - - -] // other half not empty [F F - - S S S S] // set 4 slots
   *
   * <p>[F F - T - - - -] // same half not empty [F F S S - - - -] // set 2 slots
   *
  ...
  private int fillEmptySlots(
      final int slot, final BucketPage bucketPage, final int from, final int to) {
  ...
  void setChildAddr(final AbstractPage child) {
    ...
    if (childRefs == null) throw new IllegalStateException("childRefs must not be NULL");
    ...
    for (int i = 0; i < slotsPerPage; i++) {
      ...
      if (childRefs[i] == child.self) {
        ...
        ((MutableDirectoryPageData) data).childAddr[i] = child.getIdentity();
   ...
   *     materialized, the buddyOffset will be computed based on the globalDepth and the #of
   *     pointers to that child will be computed so its depth may be set.
   ...
   * constraint. For the read-only B+Tree, {@link AbstractHTree#loadChild(DirectoryPage, int)}
   * provides this guarantee using a {@link Memoizer} pattern. This method explicitly handshakes
   * with the {@link ChildMemoizer} to clear the {@link FutureTask} from the memoizer's internal
   * cache as soon as the reference to the child has been set on the appropriate element of {@link
   * #childRefs}.
   *
  ...
  AbstractPage _getChild(final int index, final LoadChildRequest req) {
     ...
     */
    final int localDepth = HTreeUtil.getLocalDepth(htree.addressBits, globalDepth, npointers);
    ...
    synchronized (childRefs) {
      ...
      for (int i = tableOffset; i < lastIndex; i++) {
        ...
        if (data.getChildAddr(i) == addr) {
           ...
           */
          assert childRefs[i] == null || childRefs[i].get() == null
              : "Child is already set: this=" + this + ", index=" + i;
   ...
   * looked up the offset of the buddy hash table on the page and the offset of the slot within the
   * buddy hash table for the desired key. You must also know the localDepth of the child when it is
   * materialized so that information can be set on the child, where is becomes the globalDepth of
   * the child.
   *
   ...
   * automatically converted into a mutable instance.
   */
   ...
   *     directory page. The global depth of a node is defined recursively as the local depth of
   *     that node within its parent. The global/local depth are not stored explicitly. Instead, the
   *     local depth is computed dynamically when the child will be materialized by counting the #of
   *     pointers to the the child in the appropriate buddy hash table in the parent. This local
   *     depth value is passed into the constructor when the child is materialized and set as the
   *     global depth of the child.
   */
  @SuppressWarnings("unchecked")
  public DirectoryPage(final HTree htree, final byte[] overflowKey, final int globalDepth) {
   ...
   */
  protected DirectoryPage(final DirectoryPage src, final long triggeredByChildId) {
    ...
    this.data =
        src.isReadOnly() ? new MutableDirectoryPageData(htree.addressBits, src.data) : src.data;
              ...
              protected Iterator expand(final Object childObj) {
                ...
                if (!child.isLeaf()) {
                  ...
                  final Striterator itr =
                      new Striterator(((DirectoryPage) child).postOrderIterator2());
  ...
  private class ChildIterator implements Iterator<AbstractPage> {
     ...
     *
     * <p>Note: For the special case of an overflow directory we allow a <code>null</code> pointer
     * for a child at a given index.
     *
     * @return <code>true</code> iff there is another distinct child reference.
     */
    private boolean nextChild() {
      ...
      for (; slot < slotsPerPage; slot++) {
        ...
        if (tmp == null && data.getChildAddr(slot) == NULL) {
  ...
  public String toString() {
    ...
    // indicate if each child is loaded or unloaded.
    {
      ...
      for (int i = 0; i < nchildren; i++) {
        ...
        final AbstractPage child = childRefs[i] == null ? null : childRefs[i].get();
            ...
            new Expander() {
              ...
              protected Iterator expand(final Object childObj) {
                ...
                if (child instanceof DirectoryPage) {
                  ...
                  final Striterator itr =
                      new Striterator(
                          ((DirectoryPage) child).postOrderIterator1(dirtyNodesOnly, nodesOnly));
   ...
   *
   * <p>TODO The directory page validation should include checks on the bucket references and
   * addresses. For a given buddy hash table, the reference and address should pairs should be
   * consistent if either the reference or the address appears in another slot of that table. Also,
   * there can not be "gaps" between observations of a reference to a given bucket - once you see
   * another bucket reference a previously observed reference can not then appear.
   *
    ...
    for (int i = 0; i < (1 << htree.addressBits); i++) {
      ...
      if (child != null) {
        ...
        if (child.parent == null || child.parent.get() == null) {
           ...
           * reachable.
           */
          out.println(indent(height) + "  ERROR child[" + i + "] does not have parent reference.");
          ok = false;
        }
  ...
  public void dumpPages(
      final boolean recursive, final boolean visitLeaves, final HTreePageStats stats) {
   ...
   */
  public static StringBuilder toString(final IDirectoryData data, final StringBuilder sb) {
  ...
  void replaceChildRef(final long oldChildAddr, final AbstractPage newChild) {
    ...
    for (int i = 0; i < slotsOnPage; i++) {
      ...
      if (data.childAddr[i] == oldChildAddr) {
        ...
        if (npointers == 0) htree.deleteNodeOrLeaf(oldChildAddr);
  ...
  void _addLevel(final BucketPage bucketPage) {
     ...
     * directory's depth and insert the requisite references
     */

    // Create new directory to insert
    final DirectoryPage ndir =
        new DirectoryPage(
            (HTree) htree,
            null, // overflowKey
    ...
    final Reference<AbstractPage> a =
        (Reference<AbstractPage>) (fillLowerSlots ? newPage.self : null);
    final Reference<AbstractPage> b =
        (Reference<AbstractPage>) (fillLowerSlots ? null : newPage.self);
  ...
  void _addChild(final AbstractPage child) {
    ...
    for (int i = 0; i < pdata.childAddr.length; i++) {
      ...
      final AbstractPage aChild = childRefs[i] == null ? null : childRefs[i].get();
    ...
    if (pd.isOverflowDirectory()) { // already handles blobs
      ...
      final DirectoryPage blob =
          new DirectoryPage((HTree) htree, getOverflowKey(), getOverflowPageDepth());
      blob._addChild(this);
      blob._addChild(child);

      pd.replaceChildRef(this.self, blob);

      if (INFO) log.info("New Overflow Level: " + getLevel());
    }
  }

  /** Return the last non-<code>null</code> child of an overflow directory page. */
  BucketPage lastChild() {
    ...
    for (int i = data.getChildCount() - 1; i >= 0; i--) {
      ...
      if (data.getChildAddr(i) != NULL) return (BucketPage) getChild(i);
    }
  ...
  public void split(final int buddyOffset, final DirectoryPage oldChild) {
    ...
    if (buddyOffset < 0) throw new IllegalArgumentException();
  ...
  private void updatePointers(
      ...
      final int oldDepth,
      final AbstractPage oldChild,
      final AbstractPage newChild) {
    ...
    if (slotsPerBuddy <= 1) throw new AssertionError("slotsPerBuddy=" + slotsPerBuddy);
    ...
    for (int i = firstSlot; i < lastSlot; i++) {
      if (childRefs[i] == oldChild.self) {
        if (firstPointer == -1) firstPointer = i;
        nfound++;
    ...
    if (firstPointer == -1) throw new RuntimeException("No pointers to child");
    if (nfound != npointers)
      throw new RuntimeException(
          "Expected " + npointers + " pointers to child, but found=" + nfound);
    if (discontiguous)
      throw new RuntimeException(
    ...
    for (int i = firstPointer + (npointers >> 1); i < npointers; i++) {
      ...
      if (childRefs[i] != oldChild.self) throw new RuntimeException("Does not point to old child.");
   ...
   * are found. Any reference in a source buddy hash table will wind up in the "same" buddy hash
   * table afterwards, but the page and offset on the page of the buddy hash table may have been
   * changed and the size of the buddy hash table will have doubled.
   *
   * <p>When a {@link DirectoryPage} is split, the size of each buddy hash table is doubled. The
   * additional slots in each buddy hash table are filled in by (a) spacing out the old slot entries
   * in each buddy hash table; and (b) filling in the uncovered slot with a copy of the previous
   * slot.
   *
   * <p>We proceed backwards, moving the upper half of the buddy hash tables to the new directory
   * page first and then spreading out the lower half of the source page among the new buddy hash
   * table boundaries on the source page.
   *
   * @param oldDepth The depth of the old {@link DirectoryPage} before the split.
   * @param newDepth The depth of the old and new {@link DirectoryPage} after the split (this is
   *     just oldDepth+1).
   * @param oldDir The old {@link DirectoryPage}.
   * @param newDir The new {@link DirectoryPage}.
   * @deprecated with {@link #splitDirectoryPage(DirectoryPage, int, DirectoryPage)}
   */
  private void redistributeBuddyTables(
      final int oldDepth,
      final int newDepth,
      final DirectoryPage oldDir,
     ...
     */
    {
      ...
      for (int srcBuddyIndex = lastSrcBuddyIndex - 1, dstBuddyIndex = lastDstBuddyIndex - 1;
        ...
        for (int srcSlot = firstSrcSlot, dstSlot = firstDstSlot;
            srcSlot < lastSrcSlot;
            srcSlot++, dstSlot += 2) {
     ...
     */
    {
      ...
      for (int srcBuddyIndex = lastSrcBuddyIndex - 1, dstBuddyIndex = lastDstBuddyIndex - 1;
        ...
        for (int srcSlot = lastSrcSlot - 1, dstSlot = lastDstSlot - 1;
   ...
   *
   * <p>In this case, this directory has been created to extend the bit resolution so that the new
   * key/value is resolved to a different slot and a new BucketPage
   *
   * @param overflowKey
   * @param child
   */
   ...
   * unique contents. Examples where X indicates specified slot:
   *
   * <p>2bits X211 112X
   *
   * <p>3bits X3221111 3X221111 22X31111 1111X322
   *
   ...
   */
  private void _fillChildSlots(
      final int slot, final int offset, final int length, final int depth) {
    assert !isReadOnly();
   ...
   * @param bp
   */
  void _ensureUniqueBucketPage(final byte[] key, Reference<? extends AbstractPage> bp) {
  ...
  public final int removeAll(final byte[] key) {
    if (!isOverflowDirectory())
    ...
    for (int i = 0; i < childRefs.length; i++) {
      ...
      if (addr != IRawStore.NULL) htree.deleteNodeOrLeaf(addr);
    }

    return ret;
  }

  public final byte[] removeFirst(final byte[] key) {
    if (!isOverflowDirectory())
      throw new UnsupportedOperationException("Only valid if page is an overflow directory");
     ...
     * key.
     *
     * <p>If so, then we need to ensure the BucketPage is not empty after removal and, if so, to
     * remove the reference and mark as deleted.
     *
     * <p>This is further complicated if the previous sibling is a DirectoryPage. In this case, the
     * original parent directory should be replaced
     */
  ...
  public void removeAll() {
    ...
    while (chs.hasNext()) {
      ...
      if (ch instanceof BucketPage) {
        if (TRACE) log.trace("Removing bucket page: " + ch.PPID());

        if (ch.isPersistent()) htree.store.delete(ch.getIdentity());
      } else {
        if (TRACE) log.trace("Removing child directory page: " + ch.PPID());

        ((DirectoryPage) ch).removeAll();
      }
    }
    if (identity != IRawStore.NULL) htree.store.delete(identity);
  }
}
