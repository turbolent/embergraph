 ...
 * concurrent.
 *
 * <p>There are several thread pools that facilitate concurrency. They are:
 *
 * <dl>
 *   <dt>{@link #readService}
 *   <dd>Concurrent historical and read-committed tasks are run against a
 *       <strong>historical</strong> view of a named index using this service. No locking is
 *       imposed. Concurrency is limited by the size of the thread pool.
 *   <dt>{@link #writeService}
 *   <dd>Concurrent unisolated writers running against the <strong>current</strong> view of (or more
 *       more) named index(s) (the "live" or "mutable" index(s)). The underlying {@link BTree} is
 *       NOT thread-safe for writers. Therefore writers MUST predeclare their locks, which allows us
 *       to avoid deadlocks altogether. This is also used to schedule the commit phrase of
 *       transactions (transaction commits are in fact unisolated tasks).
 *   <dt>{@link #txWriteService}
 *   <dd>
 *       <p>This is used for the "active" phrase of transaction. Transactions read from historical
 *       states of named indices during their active phase and buffer the results on isolated
 *       indices backed by a per-transaction {@link TemporaryStore}. Since transactions never write
 *       on the unisolated indices during their "active" phase, distinct transactions may be run
 *       with arbitrary concurrency. However, concurrent tasks for the same transaction must obtain
 *       an exclusive lock on the isolated index(s) that are used to buffer their writes.
 *       <p>A transaction that requests a commit using the {@link ITransactionManagerService}
 *       results in a unisolated task being submitted to the {@link #writeService}. Transactions are
 *       selected to commit once they have acquired a lock on the corresponding unisolated indices,
 *       thereby enforcing serialization of their write sets both among other transactions and among
 *       unisolated writers. The commit itself consists of the standard validation and merge
 *       phrases.
 *
...
//     * True iff the {@link #log} level is DEBUG or less.
  ...
  public interface Options extends IServiceShutdown.Options {
     ...
     */
    String TX_SERVICE_CORE_POOL_SIZE =
        ConcurrencyManager.class.getName() + ".txService.corePoolSize";
     ...
     * configured {@link #WRITE_SERVICE_MAXIMUM_POOL_SIZE}.
     *
     * <p>The main factor that influences the throughput of group commit is the <em>potential</em>
     * concurrency of the submitted {@link ITx#UNISOLATED} tasks (both how many can be submitted in
     * parallel by the application and how many can run concurrency - tasks writing on the same
     * index have a shared dependency and can not run concurrently).
     *
     * <p>Each {@link ITx#UNISOLATED} task that completes processing will block until the next
     * commit, thereby absorbing a worker thread. For this reason, it can improve performance if you
     * set the core pool size and the maximum pool size for the write service <em>higher</em> that
     * the potential concurrency with which the submitted tasks could be processed.
     *
     * <p>There is also a strong effect as the JVM performs optimizations on the running code, so
     * randomize your tests. See {@link StressTestGroupCommit} for performance tuning.
     *
     * @see #DEFAULT_WRITE_SERVICE_CORE_POOL_SIZE
     */
    String WRITE_SERVICE_CORE_POOL_SIZE =
        ConcurrencyManager.class.getName() + ".writeService.corePoolSize";
     ...
     *
     * <p>Note: This property is <strong>ignored</strong> if the {@link
     * #WRITE_SERVICE_QUEUE_CAPACITY} is ZERO (0) or {@link Integer#MAX_VALUE}!
     *
     * @see #DEFAULT_WRITE_SERVICE_MAXIMUM_POOL_SIZE
     */
    String WRITE_SERVICE_MAXIMUM_POOL_SIZE =
        ConcurrencyManager.class.getName() + ".writeService.maximumPoolSize";
     ...
     */
    String WRITE_SERVICE_KEEP_ALIVE_TIME =
        ConcurrencyManager.class.getName() + ".writeService.keepAliveTime";
     ...
     */
    String WRITE_SERVICE_PRESTART_ALL_CORE_THREADS =
        ConcurrencyManager.class.getName() + ".writeService.prestartAllCoreThreads";
     ...
     *
     * <p>Note: When the {@link #WRITE_SERVICE_QUEUE_CAPACITY} is ZERO (0), a {@link
     * SynchronousQueue} is used, the maximumPoolSize is ignored, and new {@link Thread}s will be
     * created on demand. This allow the #of threads to change in response to demand while ensuring
     * that tasks are never rejected.
     *
     * <p>Note: A {@link LinkedBlockingQueue} will be used if the queue capacity is {@link
     * Integer#MAX_VALUE}. The corePoolSize will never increase for an unbounded queue so the value
     * specified for maximumPoolSize will be ignored and tasks will never be rejected. See {@link
     * ThreadPoolExecutor}'s discussion on queues for more information on this issue.
     *
     * <p>Note: When a bounded queue capacity is specified, tasks will be rejected if the the
     * corePoolThreads are busy and the work queue is full.
     *
     * @see ThreadPoolExecutor
     * @see #DEFAULT_WRITE_SERVICE_QUEUE_CAPACITY
     */
    String WRITE_SERVICE_QUEUE_CAPACITY =
        ConcurrencyManager.class.getName() + ".writeService.queueCapacity";
     ...
     * wait for other tasks and the commit group will therefore always consist of a single task.
     *
     * @see #DEFAULT_WRITE_SERVICE_GROUP_COMMIT_TIMEOUT
     */
    String WRITE_SERVICE_GROUP_COMMIT_TIMEOUT =
        ConcurrencyManager.class.getName() + ".writeService.groupCommitTimeout";
     ...
     * process SHOULD be performed (asynchronous overflow processing is enabled, asynchronous
     * overflow processing is not ongoing, and the live journal extent exceeds the threshold
     * extent).
     *
     * <p>The lock timeout needs to be of significant duration or a lock request for a write service
     * under heavy write load will timeout, in which case an error will be logged. If overflow
     * processing is not performed the live journal extent will grow without bound and the service
     * will be unable to release older resources on the disk.
     */
    String WRITE_SERVICE_OVERFLOW_LOCK_REQUEST_TIMEOUT =
        ConcurrencyManager.class.getName() + ".writeService.overflowLockRequestTimeout";

    String DEFAULT_WRITE_SERVICE_OVERFLOW_LOCK_REQUEST_TIMEOUT = "" + (2 * 60 * 1000);
  }
   ...
   * store. The size of the thread pool for this service governs the maximum practical concurrency
   * for transactions.
   *
   * <p>Transactions always read from historical data and buffer their writes until they commit.
   * Transactions that commit MUST acquire unisolated writable indices for each index on which the
   * transaction has written. Once the transaction has acquired those writable indices it then runs
   * its commit phrase as an unisolated operation on the {@link #writeService}.
   */
   ...
   * of the thread pool for this service governs the maximum practical concurrency for unisolated
   * readers.
   *
   * <p>Note that unisolated read operations on the <strong>current</strong> state of an index DO
   * conflict with unisolated writes and such tasks must be run as unisolated writers.
   *
   * <p>Note: unisolated readers of historical data do require the rention of historical commit
   * records (which may span more than one logical journal) until the reader terminates.
   */
   ...
   * named index (or index partition) conflict and must be serialized. The size of this thread pool
   * and the #of distinct named indices (or namespaces) together govern the maximum practical
   * concurrency for unisolated writers.
   *
   * <p>Serialization of access to unisolated named indices is accomplished by gaining an exclusive
   * lock on the named resource(s) corresponding to the index(es) or namespace.
   */
...
//     * An object wrapping the properties specified to the ctor.
  ...
  public void shutdownNow() {
    ...
    if (sampleService != null) sampleService.shutdownNow();
   ...
   * @param transactionManager The object managing the local transactions.
   * @param resourceManager The object managing the resources on which the indices are stored.
   */
  public ConcurrencyManager(
    ...
    // readServicePoolSize
    {
      ...
      if (log.isInfoEnabled())
        log.info(
            ConcurrencyManager.Options.READ_SERVICE_CORE_POOL_SIZE + "=" + readServicePoolSize);
    }
    ...
    // shutdownTimeout
    {
      val =
          properties.getProperty(
              ConcurrencyManager.Options.SHUTDOWN_TIMEOUT,
      ...
      if (shutdownTimeout < 0) {
        ...
        throw new RuntimeException(
            "The '" + ConcurrencyManager.Options.SHUTDOWN_TIMEOUT + "' must be non-negative.");
      }
      ...
      txWriteService =
          (ThreadPoolExecutor)
              Executors.newCachedThreadPool(
                  new DaemonThreadFactory(getClass().getName() + ".txWriteService"));
    } else {
      // fixed thread pool.
      txWriteService =
          (ThreadPoolExecutor)
    ...
    // setup thread pool for unisolated write operations.
    {
      ...
      if (log.isInfoEnabled())
        log.info(ConcurrencyManager.Options.WRITE_SERVICE_KEEP_ALIVE_TIME + "=" + keepAliveTime);
      ...
      writeService =
          new WriteExecutorService(
              ...
              keepAliveTime,
              TimeUnit.MILLISECONDS, // keepAliveTime
              queue,
...

    {
      ...
      if (log.isInfoEnabled())
        log.info(IEmbergraphClient.Options.COLLECT_QUEUE_STATISTICS + "=" + collectQueueStatistics);
    }
    ...
    if (collectQueueStatistics) {
      ...
      writeServiceQueueStatisticsTask =
          new ThreadPoolExecutorStatisticsTask("writeService", getWriteService(), countersUN, w);

      txWriteServiceQueueStatisticsTask =
          new ThreadPoolExecutorStatisticsTask("txWriteService", txWriteService, countersTX, w);

      readServiceQueueStatisticsTask =
          new ThreadPoolExecutorStatisticsTask("readService", readService, countersHR, w);
      ...
      // the write service.
      sampleService.scheduleWithFixedDelay(
          writeServiceQueueStatisticsTask, initialDelay, delay, unit);

      // the lock service for the write service.
      sampleService.scheduleWithFixedDelay(
          getWriteService().getLockManager().statisticsTask, initialDelay, delay, unit);

      // the tx write service.
      sampleService.scheduleWithFixedDelay(
          txWriteServiceQueueStatisticsTask, initialDelay, delay, unit);

      // the read service.
      sampleService.scheduleWithFixedDelay(
          readServiceQueueStatisticsTask, initialDelay, delay, unit);
  ...
  public CounterSet getCounters() {
    ...
    countersRoot.addCounter("elapsed", new ServiceElapsedTimeInstrument(serviceStartTime));
    ...
    if (collectQueueStatistics) {
      ...
      // readService
      {
        countersRoot
            ...
            .attach(readServiceQueueStatisticsTask.getCounters());
      }
      ...
      // txWriteService
      {
        countersRoot
            ...
            .attach(txWriteServiceQueueStatisticsTask.getCounters());
      }
   ...
   * @exception NullPointerException if task null
   */
  @Override
  public <T> FutureTask<T> submit(final AbstractTask<T> task) {
    ...
    if (task.readOnly) {
       ...
       * A task that reads from historical data and writes on isolated
        ...
        if (log.isInfoEnabled())
          log.info(
              "Submitted to the transaction service: "
                  + task.getClass().getName()
                  + ", timestamp="
                  + task.timestamp);
       ...
       * A task that reads from and writes on "live" indices. The live
        ...
        if (log.isInfoEnabled())
          log.info(
              "Submitted to the write service: "
                  + task.getClass().getName()
                  + ", timestamp="
                  + task.timestamp);
  ...
  private <T> FutureTask<T> submitWithDynamicLatency(
      final AbstractTask<T> task, final ExecutorService service, final TaskCounters taskCounters) {

    /*
     * Track the total inter-arrival time.
     */
    synchronized (taskCounters.lastArrivalNanoTime) {
      final long lastArrivalNanoTime = taskCounters.lastArrivalNanoTime.get();
    ...
    if (backoff && service instanceof ThreadPoolExecutor) {
      ...
      final BlockingQueue<Runnable> queue = ((ThreadPoolExecutor) service).getQueue();
       ...
       * Note: Any delay here what so ever causes the #of
            ...
            if (EmbergraphStatics.debug) System.err.print("z");
    ...
    if (service instanceof WriteExecutorService) {
      ...
      final NonBlockingLockManagerWithNewDesign<String> lockManager =
          ((WriteExecutorService) service).getLockManager();
...
//    * Actively running for the {@link WriteExecutorService}.
   ...
   *     <p>FIXME Should also shutdown/restart the {@link #txWriteService}
   */
  void abortAllTx() {
   ...
   *
   * @todo revisit the question of imposed latency here based on performance analysis (of queue
   *     length vs response time) for the federation under a variety of workloads (tasks such as rdf
   *     data load, rdf data query, embergraph repository workloads, etc.).
   *     <p>Note that {@link Executors#newCachedThreadPool()} uses a {@link SynchronousQueue} and
   *     that queue has ZERO capacity.
   */
   ...
   */
  @Override
  public <T> List<Future<T>> invokeAll(final Collection<? extends AbstractTask<T>> tasks)
