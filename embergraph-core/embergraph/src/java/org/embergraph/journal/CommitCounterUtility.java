 ...
 * The root directory is at depth ZERO (0). Each directory contains up to <code>1000</code>
 * children. The children in the non-leaf directories are subdirectories labeled <code>0..999</code>
 * . The leaf directories are at depth SIX (6). Leaf directories contain files. Each file in a leaf
 * directory is labeled with a <code>21</code> digit base name and some purpose specific file
 * extension. Each such file has data for the specific commit point encoded by the basename of the
 * file.
 *
...
public class CommitCounterUtility {
   ...
   *
   * <p>Note: 21 := (leafDirDepth+1) * digitsPerDir
   */
   ...
   * @param commitCounter The commit counter for the current root block on the journal.
   * @param ext The filename extension.
   * @return The name of the corresponding snapshot file.
   */
  public static File getCommitCounterFile(
      final File dir, final long commitCounter, final String ext) {
   ...
   * system.
   *
   * @param commitCounter The commit counter.
   * @return The basename of the file consisting of the foramtted commit counter with the
   *     appropriate leading zeros.
   */
   ...
   * @throws NumberFormatException if the file name can not be interpreted as a commit counter.
   */
  public static long parseCommitCounterFile(final String name, final String ext)
   ...
   *
   * <p>Note: A dedicated version of this method exists here to thrown an {@link IOException} if we
   * can not delete a file. This is deliberate. It is thrown to prevent a REBUILD from proceeding
   * unless we can clear out the old snapshot and HALog files.
   *
   * @param errorIfDeleteFails When <code>true</code> and {@link IOException} is thrown if a file
   *     matching the filter or an empty directory matching the filter can not be removed. When
   *     <code>false</code>, that event is logged @ WARN instead.
   * @param f A file or directory.
   * @param fileFilter A filter matching the files and directories to be visited and removed. If
   *     directories are matched, then they will be removed iff they are empty. A depth first
   *     visitation is used, so the files and sub-directories will be cleared before we attempt to
   *     remove the parent directory.
   * @throws IOException if any file or non-empty directory can not be deleted (iff
   *     <i>errorIfDeleteFails</i> is <code>true</code>).
   */
  public static void recursiveDelete(
      final boolean errorIfDeleteFails, final File f, final FileFilter fileFilter)
      throws IOException {
   ...
   *
   * @param f The root of the directory structure for the snapshot or HALog files.
   * @param fileFilter Either the {@link SnapshotManager#SNAPSHOT_FILTER} or the {@link
  ...
  public static File findGreatestCommitCounter(final File f, final FileFilter fileFilter)
    ...
    if (fileFilter == null) throw new IllegalArgumentException();
