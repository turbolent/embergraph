 ...
 * support, group commit, and low-latency startup (since the file is not created until the store
 * attempts to write through to the disk). However, {@link TemporaryStore} is lighter weight
 * precisely because it does not provide concurrency control.
 *
...
public class TemporaryStore extends TemporaryRawStore implements IBTreeManager {
   ...
   *
   * <p>Note: This is a mutable {@link BTree} so it is NOT thread-safe. We always synchronize on
   * this object before accessing it.
   */
   ...
   * @param offsetBits This determines the capacity of the store file and the maximum length of a
   *     record. The value is passed through to {@link WormAddressManager#WormAddressManager(int)}.
   * @param file The backing file (may exist, but must be empty if it exists).
   */
  ...
  @Override
  public final BTree registerIndex(final String name, final BTree btree) {
   ...
   * ITx#UNISOLATED} index view.
   *
   * <p>Note: If {@link ITx#READ_COMMITTED} is requested, then the returned {@link BTree} will
   * reflect the state of the named index as of the last {@link #checkpoint()}. This view will be
   * read-only and is NOT updated by {@link #checkpoint()}. You must actually {@link #checkpoint()}
   * before an {@link ITx#READ_COMMITTED} view will be available. When there is not a checkpoint
   * available, the {@link ITx#UNISOLATED} index view will be returned.
   */
  ...
  public ICheckpointProtocol getIndexLocal(final String name, final long commitTime) {
    ...
    if (commitTime == ITx.READ_COMMITTED) {
      ...
      final ICheckpointProtocol ndx =
          Checkpoint.loadFromCheckpoint(this, checkpointAddr, true /* readOnly */);
     ...
     * reading on the temporary store and going with the unisolated index anyway.
     *
     * <p>As a work around, I have modified the TemporaryStore in the JOURNAL_HA_BRANCH to use the
     * unisolated view of the index. This allows the RWStore to be used in combination with
     * incremental truth maintenance.
     *
     * <p>In the BOP model, the timestamp is associated with the predicate. This means that we can
     * correctly annotate the predicate reading on the RWStore with the read-only tx identifier and
     * the predicate reading on the TemporaryStore with the UNISOLATED timestamp (0L). Therefore,
     * once the RWStore is brought into the QUADS_QUERY_BRANCH we can modify the rules to specify
     * UNISOLATED for the reads on the TemporaryStore and the tx for the reads on the RWStore and
     * then roll back the change to TemporaryStore? to force the use of the unisolated index when a
     * read-only tx was specified.
     *
     * @see <a hreg="https://sourceforge.net/apps/trac/bigdata/ticket/215" > RWStore /
     *     TemporaryStore interaction when performing incremental truth maintenance </a>
     */
