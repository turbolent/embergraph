 ...
 * space on disk required by the (new) backing store and improve locality in the (new) backing
 * store.
 *
 * <p>Note: The new {@link Journal} WILL NOT include any historical commit points other than the one
 * selected by the caller specified <i>commitTime</i>.
 *
 * <p>Note: If any indices use references to raw records then they MUST define an {@link
 * IOverflowHandler} in order for the raw records to be copied to the new store and those references
 * updated in the index to point to the records in the new store. For example, the {@link
 * EmbergraphFileSystem} uses such references and defines an {@link IOverflowHandler} so that the
 * raw file blocks will not be lost on overflow.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @see Journal#compact(File)
 * @todo it would be easy enough to change the branching factor during this task.
 * @todo add listener api for start/end notice (and perhaps error).
 * @todo add export task that builds index segments from the indices on a journal and work this into
 *     {@link CompactJournalUtility} or a similar utility class.
 */
public class CompactTask implements Callable<Journal> {
  ...
  /** The {@link ICommitRecord} corresponding to the caller specified commit time. */
  ...
  /*
   * The task reads the state of each named index as of the given commitTime and writes the index
   * data in order on the output journal.
   *
   * <p>Note: Unlike the {@link IndexSegmentBuilder}, this does not produce a perfect read-optimized
   * index. However, in many cases this task does significantly improve the locality of reference
   * for the {@link BTree}s and will discard any deleted data or data which has been overrwritten.
   *
   * @param src The source journal.
   * @param outFile The output file.
   * @param commitTime The commit time whose state will be compacted onto the output file (the first
   *     commit point whose commit time is LTE to the given commit time will be used).
   */
  public CompactTask(final Journal src, final File outFile, final long commitTime) {
  ...
  /*
   * Compact the {@link #oldJournal} journal onto the {@link #outFile} file.
   *
   * @return The already open {@link Journal} iff this task succeeds. If the task fails, then the
  ...
  protected Journal createJournal() {
    ...
    if (p.getProperty(Options.BUFFER_MODE) != null) {
      ...
      BufferMode bufferMode = BufferMode.valueOf(p.getProperty(Options.BUFFER_MODE));
  ...
  /*
   * Copy all named indices from the {@link #oldJournal} to the new journal.
   *
   * <p>Note: This code is similar to that found in the {@link OverflowManager}.
   *
   * @param newJournal The new journal.
   */
  protected void copyIndices(final Journal newJournal) {
    ...
    final int nindices = (int) oldJournal.getName2Addr(commitTime).rangeCount(null, null);

    final Iterator<String> nitr = oldJournal.indexNameScan(null /* prefix */, commitTime);
     ...
     */
    //        final ParallelismLimitedExecutorService service = new
    // ParallelismLimitedExecutorService(
    ...
    if (INFO) log.info("Copied " + nindices + " in " + elapsed + "ms");
  } // copyIndices
  ...
  protected class CopyIndexTask implements Callable<Void> {
    ...
    public CopyIndexTask(final Journal newJournal, final String name) {
      ...
      if (newJournal == null) throw new IllegalArgumentException();
    ...
    public Void call() {
      ...
      try {
        ...
        if (INFO) log.info("Start: name=" + name);
         ...
         * (GIST)
         */
        final BTree oldBTree = (BTree) oldJournal.getIndexWithCommitRecord(name, commitRecord);
        ...
        if (DEBUG)
          log.debug("Copying data to new journal: name=" + name + ", entryCount=" + entryCount);
        ...
        if (DEBUG) log.debug("Done with index: name=" + name);
