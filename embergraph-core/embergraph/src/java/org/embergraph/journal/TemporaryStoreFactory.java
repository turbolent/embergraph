...
public class TemporaryStoreFactory {
  ...
  public interface Options {
     ...
     *
     * <p>{@link TemporaryStore}s are reused in order to keep down the #of file handles and the
     * latency to create a new {@link TemporaryStore} for operations which make heavy use of
     * temporary data. Old {@link TemporaryStore} instances will be reclaimed (and deleted on the
     * disk) once they are no longer strongly referenced.
     *
     * <p>This option DOES NOT place an absolute limit on the maximum extent of a {@link
     * TemporaryStore} instance. A {@link TemporaryStore} will continue to grow in side as long as a
     * process continues to write on the {@link TemporaryStore}.
     */
     ...
     *
     * <p>Note: Each file system has its own limits on the maximum size of a file. FAT16 limits the
     * maximum file size to only 2G. FAT32 supports 4G files. NTFS and most un*x file systems
     * support 16G+ files. A safe point for allocating a new temporary store for new requests is
     * therefore LT the smallest maximum file size supported by any of the common file systems.
     *
     * <p>A temporary store that reaches the maximum size allowed for the file system will fail when
     * a request is made to extend that file. How that effects processing depends of course on the
     * purpose to which the temporary store was being applied. E.g., to buffer a transaction, to
     * perform truth maintenance, etc.
     *
     * @todo If we had more visibility into the file system for a given logical disk then we could
     *     apply that information to dynamically set the cutover point for the temporary store based
     *     on the backing file system. Unfortunately, {@link File} does not provide us with that
     *     information.
     */
  ...
  public TemporaryStoreFactory() {
    ...
    this(
        new File(
            System.getProperty(Options.TMP_DIR, System.getProperty(Options.JAVA_TMP_DIR_PROPERTY))),
        Integer.valueOf(System.getProperty(Options.OFFSET_BITS, Options.DEFAULT_OFFSET_BITS)),
    ...
    this(
        new File(
            properties.getProperty(
                Options.TMP_DIR, System.getProperty(Options.JAVA_TMP_DIR_PROPERTY))),
        Integer.valueOf(properties.getProperty(Options.OFFSET_BITS, Options.DEFAULT_OFFSET_BITS)),
   ...
   * @param offsetBits This value governs how many records can exist within the {@link
   *     TemporaryStore} and the maximum size of those records. A good default value is {@link
   *     WormAddressManager#SCALE_UP_OFFSET_BITS}.
   * @param maxExtent The maximum extent of the current {@link TemporaryStore} before {@link
   *     #getTempStore()} will return a new {@link TemporaryStore}.
   * @throws IllegalArgumentException if <i>maxExtent</i> is negative (zero is allowed and will
   *     cause each request to return a distinct {@link TemporaryStore}).
   */
  public TemporaryStoreFactory(final File tmpDir, final int offsetBits, final long maxExtent) {
    ...
    if (maxExtent < 0L) throw new IllegalArgumentException();
   ...
   * returned. Otherwise the existing instance is returned.
   */
   ...
   * finalized if the store is shutdown and that {@link File#deleteOnExit()} will not be called, in
    ...
    while (itr.hasNext()) {
      ...
      if (store.isOpen()) {
        ...
        try {
          ...
          if (InnerCause.isInnerCause(t, InterruptedException.class)) interrupted = true;
        }
      }
    }
