...
import org.embergraph.ha.CommitRequest;
import org.embergraph.ha.CommitResponse;
...
import org.embergraph.ha.IIndexManagerCallable;
import org.embergraph.ha.IJoinedAndNonJoinedServices;
import org.embergraph.ha.JoinedAndNonJoinedServices;
import org.embergraph.ha.PrepareRequest;
import org.embergraph.ha.PrepareResponse;
import org.embergraph.ha.QuorumService;
import org.embergraph.ha.RunState;
import org.embergraph.ha.msg.HANotifyReleaseTimeResponse;
import org.embergraph.ha.msg.HAReadResponse;
import org.embergraph.ha.msg.HARootBlockRequest;
import org.embergraph.ha.msg.HARootBlockResponse;
import org.embergraph.ha.msg.HAWriteSetStateResponse;
...
import org.embergraph.ha.msg.Mock2PhaseCommitProtocolException;
...
import org.embergraph.quorum.QuorumTokenTransitions;
 ...
 *
 * <p>This class is an abstract implementation of the {@link IJournal} interface that does not
 * implement the {@link IConcurrencyManager}, {@link IResourceManager}, or {@link
 * ITransactionService} interfaces. The {@link Journal} provides a concrete implementation that may
 * be used for a standalone database complete with concurrency control and transaction management.
 *
 * <h2>Limitations</h2>
 *
 * <p>The {@link IIndexStore} implementation on this class is NOT thread-safe. The basic limitation
 * is that the mutable {@link BTree} is NOT thread-safe. The {@link #getIndex(String)} method
 * exposes this mutable {@link BTree}. If you use this method to access the mutable {@link BTree}
 * then YOU are responsible for avoiding concurrent writes on the returned object.
 *
 * <p>See {@link IConcurrencyManager#submit(AbstractTask)} for a thread-safe API that provides
 * suitable concurrency control for both isolated and unisolated operations on named indices. Note
 * that the use of the thread-safe API does NOT protect against applications that directly access
 * the mutable {@link BTree} using {@link #getIndex(String)}.
 *
 * <p>The {@link IRawStore} interface on this class is thread-safe. However, this is a low-level API
 * that is not used by directly by most applications. The {@link BTree} class uses this low-level
 * API to read and write its nodes and leaves on the store. Applications generally use named indices
 * rather than the {@link IRawStore} interface.
 *
 * <p>Note: transaction processing MAY occur be concurrent since the write set of a each transaction
 * is written on a distinct {@link TemporaryStore}. However, without additional concurrency
 * controls, each transaction is NOT thread-safe and MUST NOT be executed by more than one
 * concurrent thread. Again, see {@link IConcurrencyManager#submit(AbstractTask)} for a
 * high-concurrency API for both isolated operations (transactions) and unisolated operations. Note
 * that the {@link TemporaryStore} backing a transaction will spill automatically from memory onto
 * disk if the write set of the transaction grows too large.
 *
 * <h2>Commit processing</h2>
 *
 * <p>The journal maintains two root blocks. Commit updates the root blocks using the Challis
 * algorithm. (The root blocks are updated using an alternating pattern and "timestamps" are
 * recorded at the head and tail of each root block to detect partial writes. See {@link
 * IRootBlockView} and {@link RootBlockView}.) When the journal is backed by a disk file, the data
 * are {@link Options#FORCE_ON_COMMIT optionally flushed to disk on commit}. If desired, the writes
 * may be flushed before the root blocks are updated to ensure that the writes are not reordered -
 * see {@link Options#DOUBLE_SYNC}.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @todo There are lots of annoying ways in which asynchronously closing the journal, e.g., using
 *     {@link #close()} or {@link #shutdown()} can cause exceptions to be thrown out of concurrent
 *     threads. It would be nice if we could throw a single exception that indicated that the
 *     journal had been asynchronously closed.
 */
public abstract class AbstractJournal
    implements IJournal /* , ITimestampService */, IAllocationManager, IAllocationManagerStore {
   ...
   * necessary since that operation changes the {@link UUID} of the backing store, which is one of
   * the things reported by the {@link JournalMetadata} class.
   *
  ...
  /** The current root block. This is updated each time a new root block is written. */
   ...
   * thread which causes the value of either of these fields to be changed. The "writers" are {@link
   * #abort()}, {@link #closeForWrites(long)}, etc. These methods are invoked relatively
   * infrequently in comparison with "read" access to these fields.
   */
   ...
   * {@link #commitNow(long)}.
   *
   * <p>Note: This is set in the constructor and modified by {@link #_abort()} but (once set by the
   * constructor) it is never <code>null</code> until the store is closed.
   */
   ...
   *
   * <p>Note: The {@link CommitRecordIndex} object is NOT systematically protected by <code>
   * synchronized</code> within this class. Therefore it is NOT safe for use by outside classes and
   * CAN NOT be made safe simply by synchronizing their access on the {@link CommitRecordIndex}
   * object itself. This is mainly for historical reasons and it may be possible to systematically
   * protect access to this index within a synchronized block and then expose it to other classes.
   *
   * <p>Note: This is set in the constructor and modified by {@link #_abort()} but (once set by the
   * constructor) it is never <code>null</code> until the store is closed.
   */
   ...
   * objects by prevent them from being swept by the garbage collector.
   *
   * <p>Note: the "live" version of an object MUST NOT be placed into this cache since its state
   * will continue to evolve with additional writes while the cache is intended to provide a
   * canonicalizing mapping to the historical committed states of the object. This means that
   * objects such as indices and the {@link Name2Addr} index MUST NOT be inserted into the cache if
   * the are being read from the store for "live" use. For this reason {@link Name2Addr} uses its
   * own caching mechanisms.
   *
   * <p>Note: {@link #abort()} discards the contents of this cache in order to ensure that partial
   * writes are discarded.
   *
   * @see Options#HISTORICAL_INDEX_CACHE_CAPACITY
   * @see Options#HISTORICAL_INDEX_CACHE_TIMEOUT
   *     <p>TODO This should have {@link ICheckpointProtocol} values. We have to update the {@link
   *     IRWStrategy#registerExternalCache(ConcurrentWeakValueCache, int)} method in order to make
   *     that change.
   */
   ...
   *
   * <p>Note: This cache is in front of the {@link #historicalIndexCache} as the latter is only
   * tested once we have the {@link ICommitRecord} and have resolved the entry in {@link Name2Addr}.
   * This cache allows us to avoid both of those steps.
   *
   * <p>Note: The {@link #historicalIndexCache} imposes a canonicalizing mapping. It remains
   * necessary, even with the introduction of the {@link #indexCache}.
   *
   ...
   * of the named btree.
   *
   * <p>The "live" name2addr index is required for unisolated writers regardless whether they are
   * adding an index, dropping an index, or just recovering the "live" version of an existing named
   * index.
   *
   * <p>Operations that read on historical {@link Name2Addr} objects can of course be concurrent.
   * Those objects are loaded from an {@link ICommitRecord}. See {@link #getIndex(String,
   * ICommitRecord)}.
   *
   * <p>Note: access to the "live" {@link Name2Addr} index MUST be bracketed with <code>
   * synchronized({@link #_name2Addr})</code>.
   *
   * @see #getName2Addr()
   */
   ...
   *
   * <p>It is checked in the commit() method ensure updates are protected.
   *
   ...
   * ICommitRecord)}.
   *
   * <p>Note: the "live" {@link Name2Addr} index is a mutable {@link BTree}. All access to the
   * object MUST be synchronized on that object.
   */
   ...
   * address of the named {@link BTree} on the {@link Journal}.
   */
  public IIndex getName2Addr() {
    ...
    try {
      ...
      if (_name2Addr == btree) throw new AssertionError();
   ...
   * @return The read-only view -or- <code>null</code> if there is no commit record for that
   *     commitTime.
   * @see #getName2Addr()
   */
  public IIndex getName2Addr(final long commitTime) {
    ...
    final Name2Addr n2a = (Name2Addr) getIndexWithCheckpointAddr(checkpointAddr);
  ...
  protected static String getProperty(
      final Properties properties, final String name, final String defaultValue) {

    return Configuration.getProperty(
        null /* indexManager */, properties, "" /*
																			 * no
																			 * namespace
																			 */, name, defaultValue);
  }
  ...
  protected <E> E getProperty(
      final String name, final String defaultValue, IValidator<E> validator) {
   ...
   * @throws RuntimeException If there is a problem when creating, opening, or reading from the
   *     journal file.
   * @see Options
   */
   ...
   *
   * @param properties The properties as defined by {@link Options}.
   * @param quorum The quorum with which the journal will join (HA mode only).
   * @throws RuntimeException If there is a problem when creating, opening, or reading from the
   *     journal file.
   * @see Options
   */
  protected AbstractJournal(
      Properties properties, final Quorum<HAGlue, QuorumService<HAGlue>> quorum) {
    ...
    minimumExtension =
        getProperty(
            ...
            Options.DEFAULT_MINIMUM_EXTENSION,
    ...
    forceOnCommit =
        ForceEnum.parse(getProperty(Options.FORCE_ON_COMMIT, Options.DEFAULT_FORCE_ON_COMMIT));

    doubleSync =
        Boolean.parseBoolean(getProperty(Options.DOUBLE_SYNC, Options.DEFAULT_DOUBLE_SYNC));

    deleteOnClose =
        Boolean.parseBoolean(getProperty(Options.DELETE_ON_CLOSE, Options.DEFAULT_DELETE_ON_CLOSE));
    ...
    try {
      ...
      if (BufferMode.valueOf(getProperty(Options.BUFFER_MODE, Options.DEFAULT_BUFFER_MODE))
        ...
        final long createTime =
            Long.parseLong(getProperty(Options.CREATE_TIME, "" + System.currentTimeMillis()));
        ...
        final BufferMode bufferMode =
            BufferMode.valueOf(getProperty(Options.BUFFER_MODE, Options.DEFAULT_BUFFER_MODE));
          ...
          case Transient:
            {
              final boolean useDirectBuffers =
                  Boolean.parseBoolean(
                      getProperty(Options.USE_DIRECT_BUFFERS, Options.DEFAULT_USE_DIRECT_BUFFERS));

              _bufferStrategy =
                  new TransientBufferStrategy(offsetBits, initialExtent, 0L /*
      ...
      if (this._icuVersionRecord != null
          && !ICUVersionRecord.newInstance().equals(this._icuVersionRecord)) {

        final boolean update =
            Boolean.valueOf(properties.getProperty(Options.UPDATE_ICU_VERSION, "false"));

        if (!update) {

          throw new RuntimeException(
              "ICUVersionChange: store="
      ...
      // report event.
      ResourceManager.openJournal(
          getFile() == null ? null : getFile().toString(),
          size(),
   ...
   *     Properties object, or a flattened Properties object serialized as key-value pairs, in which
   *     case the data could just be loaded into a btree and the btree api could be used to change
   *     the persistent properties as necessary.
   */
  @Override
   ...
   * if we want to invoke {@link IBufferStrategy#deleteResources()} and we can only invoke that
  ...
  protected void finalize() throws Throwable {
    ...
    if (_bufferStrategy.isOpen()) {
      ...
      if (log.isInfoEnabled()) log.info("Closing journal: " + getFile());
   ...
   * object exists.
   *
   * <p>Note: one-shot counter are NOT used so that the LBS can aggregate the different values which
   * this counter takes on across different live journal instances for the same data service. For
   * example, the createTime for each live journal or the name of the file backing the current live
   * journal.
   */
  private static class CountersFactory {

    public static CounterSet getCounters(final AbstractJournal jnl) {
          ...
          new Instrument<String>() {
            ...
            public void sample() {
              ...
              if (jnl != null) {
                ...
                if (file != null) setValue(file.toString());
              }
            }
          });
   ...
   * recently opened.
   *
   * <p>Note: This method prefers the live view of the index since this gets us the most recent
   * metadata for the index depth, #of nodes, #of leaves, etc. However, when the live view is not
   * currently open, we prefer the most recent view of the index.
   *
   * <p>Note: This scans the live {@link Name2Addr}s internal index cache for the live indices and
   * then scans the {@link #historicalIndexCache} for the read-only index views. Only a single
   * {@link CounterSet} is reported for any given named index.
   *
   * @return A new {@link CounterSet} reflecting the named indices that were open as of the time
   *     that this method was invoked.
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/626">Expose performance counters
   *     for read-only indices </a>
   */
  protected final CounterSet getIndexCounters() {
        ...
        new HashMap<String /* name */, ICheckpointProtocol>();
    {
      ...
      while (itr.hasNext()) {
        ...
        final Map.Entry<NT, WeakReference<ICheckpointProtocol>> e = itr.next();
    ...
    for (Map.Entry<String /* name */, ICheckpointProtocol> e : map.entrySet()) {
  ...
  protected void assertBefore(
      final UUID serviceId1, final UUID serviceId2, final long t1, final long t2)
      throws ClocksNotSynchronizedException {
   ...
   * maximum clock skew that will be corrected without operator intervention.
   *
   * <p>Note: This is overridden by the HAJournal.
   *
  ...
  protected void _close() {
    ...
    if (quorum != null) quorum.terminate();
   ...
   *
   * @exception IllegalStateException if the journal is open.
   */
  @Override
   ...
   * ConcurrencyManager} is used, that means that the caller MUST have an exclusive lock on the
   * {@link WriteExecutorService}.
   *
   * <p>Note: The {@link BufferMode#DiskRW} does NOT support this operation. This is because it
   * stores meta-allocation information at the end of the file, which makes it impossible to shrink
   * the file. Therefore this method will return without causing the file on disk to be shrunk for
   * the RWStore.
   */
   ...
   *
   * <p>Note: You need an exclusive write lock on the journal to extend it.
   *
   * @param minFree The minimum #of bytes free for the user extent.
  ...
  public long ensureMinFree(final long minFree) {
    ...
    if (minFree < 0L) throw new IllegalArgumentException();
   ...
   * <i>closeTime</i>.
   *
   * <p>This implementation sets the "closeTime" on the root block such that the journal will no
   * longer accept writes, flushes all buffered writes, and releases any write cache buffers since
   * they will no longer be used. This method is normally used when one journal is being closed out
   * for writes during synchronous overflow processing and new writes will be buffered on a new
   * journal. This has advantages over closing the journal directly including that it does not
   ...
   * {@link WriteExecutorService}.
   *
   * <p>Note: This does NOT perform a commit - any uncommitted writes will be discarded.
   *
   * @throws IllegalStateException If there are no commits on the journal.
   * @todo There should also be an option to convert a journal from {@link BufferMode#Direct} to
   *     {@link BufferMode#Disk}. We would want to do that not when the journal is sealed but as
   *     soon as asynchronous overflow processing is done. Ideally this will not require us to close
  ...
  public void closeForWrites(final long closeTime) {
    ...
    try {
      ...
      if (log.isInfoEnabled())
        log.info(
            "Closing journal for further writes: closeTime="
                + closeTime
                + ", lastCommitTime="
                + lastCommitTime);

      if (log.isDebugEnabled()) log.debug("before: " + _rootBlock);
      ...
      if (log.isDebugEnabled()) log.debug("after: " + _rootBlock);
   ...
   * running during {@link #shutdown()} and one of the various task services times out while
   * awaiting termination. Such exceptions are normal since the store was closed asynchronously
   * while task(s) were still running.
   *
   * @exception IllegalStateException if the store is closed.
   */
   ...
   * quorum has met and {@link #init()} has been invoked for the {@link Quorum}.
   */
  @Override
   ...
   *
   * <p>Note: The root block reference can be <code>null</code> until the journal has been
   * initialized. Once it has been set, the root block will always be non-<code>null</code>. Since
   * this method does not obtain the inner lock, it is possible for another thread to change the
   * root block reference through a concurrent {@link #abort()} or {@link #commitNow(long)}. The
   ...
   * (the committers are themselves transient objects).
   *
   * @param rootSlot The slot in the root block where the address of the {@link ICommitter} will be
   *     recorded.
   * @param committer The commiter.
   */
  @Override
  public final void setCommitter(final int rootSlot, final ICommitter committer) {
    ...
    try {
      ...
      if (quorum != null) {
        ...
        try {
          ...
          haLog.error("2-Phase abort failure.  Will do local abort. cause=" + t, t);
   ...
   * demand.
   *
   * <p>Note: The {@link WriteExecutorService} handles commit group and uses an index {@link
   * Checkpoint} strategy so that it is able to abort individual tasks simply by discarding their
   * changes and without interrupting concurrent writers. An {@link #abort()} is therefore an action
   * of last resort and is generally triggered by things such as running out of disk space or memory
   * within the JVM.
   *
   * <p>If a {@link Thread}s is interrupted in the midst of an IO operation on a {@link Channel}
   * then the channel will be asynchronously closed by the JDK. Since some {@link IBufferStrategy}s
   * use a {@link FileChannel} to access the backing store, this means that we need to re-open the
   * backing store transparently so that we can continue operations after the commit group was
   * aborted. This is done automatically when we re-load the current {@link ICommitRecord} from the
   * root blocks of the store.
   */
  ...
  private void _abort() {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("ABORT", new StackInfoReport("ABORT"));
   ...
   *
   * <p>Note: To restore the last root block we copy the alternative root block over the current
   * root block. That gives us two identical root blocks and restores us to the root block that was
   * in effect before the last commit.
   *
   * @deprecated Do not use this method. HA provides point in time restore. Use that. Or you can
   *     open a journal using the alternate root block by specifying {@link
  ...
  public void rollback() {
    ...
    try {
       ...
       */
      final IRootBlockView newRootBlock =
          new RootBlockView(_rootBlock.isRootBlock0(), buf, checker);
      ...
      /** Ensure cache is clear to prevent access to invalid BTree from last commit point */
      historicalIndexCache.clear();
      indexCache.clear();
   ...
   * for more than N milliseconds. This will allow us to ensure that time moves forward when the
   * leader fails over to another node with modest clock skew. If there is a large clock skew, the
   * operator intervention will be required.
   *
   * <p>Note: This also makes sense for a non-HA deployment since we still want time to move forward
   * at each commit point.
   *
   * <p>TODO This also makes sense when the Journal is opened since we often issue queries against
   * historical commit points on the journal based on the clock. [Unit test for this in standalone
   * and HA modes?]
   */
  ...
  private static class CommitCounters implements ICounterSetAccess {
    ...
    /** Elapsed nanoseconds for GATHER (consensus release time protocol : HA only). */
   ...
   *
   * <p>The non-final fields in this class are laid directly below the method which set those
   * fields. The methods in the class are laid out in the top-to-bottom order in which they are
   * executed by commitNow().
   */
  private static class CommitState {
    ...
    /** The current root block on the journal as of the start of the commit protocol. */
     ...
     *     ITimestampService} for both purposes.
     */
     ...
     *
     * @return <code>true</code> if the store is dirty and the commit should proceed and <code>false
     *     </code> otherwise.
     */
    private boolean notifyCommitters() {
       ...
       */
      rootAddrs[PREV_ROOTBLOCK] = store.m_rootBlockCommitter.handleCommit(commitTime);

      store.commitCounters.elapsedNotifyCommittersNanos.add(System.nanoTime() - beginNanos);
     ...
     *
     * <p>Note: This is also responsible for recycling the deferred frees for {@link
     * IHistoryManager} backends.
     */
    private void writeCommitRecord() {
      ...
      if (_bufferStrategy instanceof IHistoryManager) {
        ...
        ((IHistoryManager) _bufferStrategy).checkDeferredFrees(store);
      }

      final ICommitRecord commitRecord = new CommitRecord(commitTime, newCommitCounter, rootAddrs);

      this.commitRecordAddr =
          store.write(ByteBuffer.wrap(CommitRecordSerializer.INSTANCE.serialize(commitRecord)));
       ...
       * the store.
       */
      commitRecordIndexAddr = store._commitRecordIndex.writeCheckpoint();
     ...
     * backing store.
     *
     * <p>Note: The address of the root of the {@link CommitRecordIndex} needs to go right into the
     * {@link IRootBlockView}. We are unable to place it into the {@link ICommitRecord} since we
     * need to serialize the {@link ICommitRecord}, get its address, and add the entry to the {@link
     * CommitRecordIndex} before we can flush the {@link CommitRecordIndex} to the store.
     *
     * @see #writeCommitRecord()
     */
     ...
     *
     * <p>Note: required for {@link RWStore} since the metaBits allocations are not made until
     * commit, leading to invalid addresses for recent store allocations.
     *
     * <p>Note: After this, we do not write anything on the backing store other than the root block.
     * The rest of this code is dedicated to creating a properly formed root block. For a non-HA
     * deployment, we just lay down the root block. For an HA deployment, we do a 2-phase commit.
     *
     * <p>Note: In HA, the followers lay down the replicated writes synchronously. Thus, they are
     * guaranteed to be on local storage by the time the leader finishes WriteCacheService.flush().
     * This does not create much latency because the WriteCacheService drains the dirtyList in a
     * seperate thread.
     */
    ...
    private void newRootBlock() {
      ...
      if (_bufferStrategy instanceof IHABufferStrategy) {
        ...
        blockSequence = ((IHABufferStrategy) _bufferStrategy).getBlockSequence();
      ...
      final long firstCommitTime =
          (old.getFirstCommitTime() == 0L ? commitTime : old.getFirstCommitTime());
    ...
    private void gatherPhase() {
      ...
      if (!(_bufferStrategy instanceof IHABufferStrategy)) return;
       ...
       * critical section will block (on the leader or the folllower) unless they are guaranteed to
       * be allowable, e.g., based on the current minReleaseAge, the new tx would read from the most
       * recent commit point, the new tx would ready from a commit point that is already pinned by
       * an active transaction on that node, etc.
       *
       * <p>Note: Lock makes this section MUTEX with awaitServiceJoin().
       *
       * @see <a href=
       *     "https://docs.google.com/document/d/14FO2yJFv_7uc5N0tvYboU-H6XbLEFpvu-G8RhAzvxrk/edit?pli=1#"
       *     > HA TXS Design Document </a>
       * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/623" > HA TXS / TXS
       *     Bottleneck </a>
       */
      ...
      try {
        ...
        gatherJoinedAndNonJoinedServices = new JoinedAndNonJoinedServices(quorum);

        // Run the GATHER protocol.
        consensusReleaseTime =
            ((AbstractHATransactionService)
                    store.getLocalTransactionManager().getTransactionService())
                .updateReleaseTimeConsensus(
                    newCommitCounter,
                    commitTime,
                    gatherJoinedAndNonJoinedServices.getJoinedServiceIds(),
                    store.getHAReleaseTimeConsensusTimeout(),
        ...
        store.commitCounters.elapsedGatherNanos.add(System.nanoTime() - beginNanos);
      }
    }
    ...
    private void commitSimple() {
      ...
      if (txLog.isInfoEnabled()) txLog.info("COMMIT: commitTime=" + commitTime);

      store.commitCounters.elapsedSimpleCommitNanos.add(System.nanoTime() - beginNanos);
    }
     ...
     * the met quorum or not. This information will be propagated through the HA 2-phase prepare
     * message so services will know how they must intepret the 2-phase prepare(), commit(), and
     * abort() requests. The atomic decision is necessary in order to enforce a consistent role on a
     * services that is resynchronizing and which might vote to join the quorum and enter the quorum
     * asynchronously with respect to this decision point.
     *
     * <p>TODO If necessary, we could also explicitly provide the zk version metadata for the znode
     * that is the parent of the joined services. However, we would need an expanded interface to
     * get that metadata from zookeeper out of the Quorum.
     *
     * @throws IOException
    ...
    private void prepare2Phase() throws InterruptedException, TimeoutException, IOException {
      ...
      try {
        ...
        prepareJoinedAndNonJoinedServices = new JoinedAndNonJoinedServices(quorum);
        ...
        if (haLog.isInfoEnabled()) haLog.info(prepareResponse.toString());
        ...
        store.commitCounters.elapsedPrepare2PhaseNanos.add(System.nanoTime() - beginNanos);
      }
    }
    ...
    private void commit2Phase() throws Exception {
      ...
      try {
        ...
        commitRequest = new CommitRequest(prepareRequest, prepareResponse);
        ...
        store.commitCounters.elapsedCommit2PhaseNanos.add(System.nanoTime() - beginNanos);
      }
    }
    // Fields set by the method above.
   ...
   * address if its state has not changed). That address is saved in the {@link ICommitRecord} under
   * the index for which that committer was {@link #registerCommitter(int, ICommitter) registered}.
   * We then force the data to stable store, update the root block, and force the root block and the
   * file metadata to stable store.
   *
   * <p>Note: Each invocation of this method MUST use a distinct <i>commitTime</i> and the
   * commitTimes MUST be monotonically increasing. These guarantees support both the database
   * version history mechanisms and the High Availability mechanisms.
   *
   * @param commitTime The commit time either of a transaction or of an unisolated commit. Note that
   *     when mixing isolated and unisolated commits you MUST use the same {@link ITimestampService}
   *     for both purposes.
   * @return The timestamp assigned to the commit record -or- 0L if there were no data to commit.
   */
  ...
  protected long commitNow(final long commitTime) {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("commitTime=" + commitTime);

      // Critical Section Check. @see #1021 (Add critical section protection to
      // AbstractJournal.abort() and EmbergraphSailConnection.rollback())
      if (abortRequired.get()) throw new AbortRequiredException();
      ...
      if (EmbergraphStatics.debug || log.isInfoEnabled()) {
        ...
        else if (log.isInfoEnabled()) log.info(msg);
      ...
      commitCounters.elapsedTotalCommitNanos.add(System.nanoTime() - beginNanos);
    }
  }
   ...
   *
   * @param commitTime The proposed commit time.
   * @throws IllegalArgumentException if the <i>commitTime</i> is LTE the value reported by {@link
  ...
  @Override
  public final long getRootAddr(final int index) {
    ...
    try {
      ...
      if (commitRecord == null) throw new AssertionError();
   ...
   * <i>releaseTime</i>.
   *
   * <p>Note: This method is used for HA. The caller provides a releaseTime based on the
   * readsOnCommitTime of the earliestActiveTx and the minReleaseAge rather than {@link
   * ITransactionService#getReleaseTime()} since the latter is only updated by the release time
   * consensus protocol during a 2-phase commit.
   */
  protected ICommitRecord getEarliestVisibleCommitRecordForHA(final long releaseTime) {
    ...
    try {
      ...
      if (commitCounter == 0L) {
        ...
        if (log.isTraceEnabled()) log.trace("No commit points");
      ...
      if (releaseTime >= lastCommitTime) {
        ...
        throw new IllegalArgumentException(
            "releaseTime(" + releaseTime + ") >= lastCommitTime(" + lastCommitTime + ")");
      }

      final CommitRecordIndex commitRecordIndex = _commitRecordIndex;

      if (commitRecordIndex == null) throw new AssertionError();
      ...
      final ICommitRecord commitRecord = commitRecordIndex.findNext(effectiveTimestamp);

      if (commitRecord == null)
        throw new AssertionError(
            "commitCounter="
                + commitCounter
                + " but no commitRecord for releaseTime="
                + releaseTime
                + ", effectiveTimestamp="
                + effectiveTimestamp
                + " :: "
      ...
      if (log.isTraceEnabled())
        log.trace("releaseTime=" + releaseTime + ",commitRecord=" + commitRecord);
  ...
  private ICommitRecord _getCommitRecord() {
    ...
    if (log.isInfoEnabled()) log.info("Reading commit record from: " + commitRecordAddr);
  ...
  protected void invalidateCommitters() {
    ...
    if (log.isDebugEnabled())
      log.debug(
          "invalidating commiters for: " + this + ", lastCommitTime: " + this.getLastCommitTime());

    assert _fieldReadWriteLock.writeLock().isHeldByCurrentThread();

    final Throwable t =
        new StackInfoReport(
            "ABORT journal " + this + ", lastCommitTime: " + this.getLastCommitTime());

    for (ICommitter committer : _committers) {
      if (committer != null) committer.invalidate(t);
    }
   ...
   *
   * <p>The default implementation discards the btree mapping names to named btrees.
   *
   * <p>Subclasses MAY extend this method to discard their own committers but MUST NOT override it
   * completely.
   */
   ...
   *
   * <p>The basic implementation sets up the btree that is responsible for resolving named btrees.
   *
   * <p>Subclasses may extend this method to setup their own committers.
   */
  ...
  private class ICUVersionCommitter implements ICommitter {
    ...
    private ICUVersionCommitter() {
      ...
      // the "update" option.
      update = Boolean.valueOf(properties.getProperty(Options.UPDATE_ICU_VERSION, "false"));
    ...
    public long handleCommit(final long commitTime) {
      ...
      if (lastAddr == NULL || !(r.equals(_icuVersionRecord) && update)) {
        ...
        if (_icuVersionRecord != null && update)
          log.warn("Updating ICUVersion: old=" + _icuVersionRecord + ", new=" + r);
   ...
   * {@link #abort()} .
   *
   * @param addr The root address of the btree -or- 0L iff the btree has not been defined yet.
   * @see Options#LIVE_INDEX_CACHE_CAPACITY
   */
  Name2Addr setupName2AddrBTree(final long addr) {
    ...
    if (addr == 0L) {
      ...
      if (log.isInfoEnabled()) log.info("New " + Name2Addr.class.getName());
      ...
      if (log.isInfoEnabled()) log.info("Loading " + Name2Addr.class.getName() + " from " + addr);
  ...
  public CommitRecordIndex getReadOnlyCommitRecordIndex() {
    ...
    try {
      ...
      final CommitRecordIndex commitRecordIndex =
          getCommitRecordIndex(_rootBlock.getCommitRecordIndexAddr(), true /* readOnly */);
  ...
  private CommitRecordIndex _getCommitRecordIndex() {
    ...
    try {
      ...
      if (log.isDebugEnabled()) log.debug("Loading from addr=" + addr);
   ...
   *
   * <p><strong>CAUTION: DO NOT EXPOSE THE LIVE COMMIT RECORD INDEX OUTSIDE OF THIS CLASS. IT IS NOT
   * POSSIBLE TO HAVE CORRECT SYNCHRONIZATION ON THAT INDEX IN ANOTHER CLASS.</code>
   *
   * @param addr The root address of the index -or- 0L if the index has not been created yet. When
   *     addr is non-{@link #NULL}, each invocation will return a distinct {@link CommitRecordIndex}
   *     object.
   * @param readOnly When <code>false</code> the returned is NOT cached.
   * @return The {@link CommitRecordIndex} for that address or a new index if <code>0L</code> was
   *     specified as the address.
   * @see #_commitRecordIndex
   */
  protected CommitRecordIndex getCommitRecordIndex(final long addr, final boolean readOnly) {

    if (log.isInfoEnabled()) log.info("addr=" + toString(addr));
   ...
   * when the commit record is still present in the commit record index.
   *
  ...
  private boolean isHistoryGone(final long commitTime) {
    ...
    if (this._bufferStrategy instanceof IHistoryManager) {
      ...
      final long lastReleaseTime = ((IHistoryManager) _bufferStrategy).getLastReleaseTime();

      if (commitTime <= lastReleaseTime) {

        if (log.isDebugEnabled()) log.info("History gone: commitTime=" + commitTime);
   ...
   *     transactions need to use this code path in order to locate named indices but the {@link
   *     WriteExecutorService} can also write on this index. I have tried some different approaches
   *     to handling this.
   */
  @Override
  public ICommitRecord getCommitRecord(final long commitTime) {

    if (isHistoryGone(commitTime)) return null;
   ...
   * @return The commit record -or- <code>null</code> if there is no commit record whose timestamp
   *     is strictly greater than <i>commitTime</i>.
   */
   ...
   * transaction will hit the {@link #indexCache}. If the transaction passes the startTime instead,
   ...
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/546" > Add cache for access to
   *     historical index views on the Journal by name and commitTime. </a>
   *     <p>FIXME GIST Reconcile API tension with {@link IIndex} and {@link ICheckpointProtocol},
   *     however this method is overridden by {@link Journal} and is also implemented by {@link
   *     IEmbergraphFederation}. The central remaining tensions are {@link FusedView} and the
   *     local/remote aspect. {@link FusedView} could probably be "fixed" by extending {@link
   *     AbstractBTree} rather than having an inner delegate for the mutable view. The local/remote
   ...
   */
  @Override
  public final ICheckpointProtocol getIndexLocal(final String name, final long commitTime) {
    ...
    if (commitTime == ITx.UNISOLATED
      ...
      throw new UnsupportedOperationException(
          "name=" + name + ",commitTime=" + TimestampUtility.toString(commitTime));
    }
    ...
    try {
      ...
      if (commitRecord == null) {
        ...
        //              if (log.isInfoEnabled())
        log.warn("No commit record: name=" + name + ", timestamp=" + commitTime);
   ...
   * Entry#commitTime} for the historical {@link Name2Addr} instance for that {@link ICommitRecord}.
   *
   * <p>Note: This method should be preferred to {@link #getIndexWithCheckpointAddr(long)} for
   * read-historical indices since it will explicitly mark the index as read-only and specifies the
   * <i>lastCommitTime</i> on the returned index based on {@link Name2Addr.Entry#commitTime}, which
   * is the actual commit time for the last update to the index.
   *
   * @return The named index -or- <code>null</code> iff the named index did not exist as of that
   *     commit record.
   */
  public final ICheckpointProtocol getIndexWithCommitRecord(
   ...
   *
   * <p>Note: This method imposes a canonicalizing mapping and ensures that there will be at most
   * one object providing a view of the historical data structure as of the specified timestamp.
   * This guarentee is used to facilitate buffer management.
   *
   * <p>Note: The canonicalizing mapping for unisolated views of persistence capable data structures
   * is maintained by the {@link ITx#UNISOLATED} {@link Name2Addr} instance.
   *
   * @param checkpointAddr The address of the {@link Checkpoint} record.
   * @return The read-only persistence capable data structure associated with that {@link
   *     Checkpoint}.
   * @see Options#HISTORICAL_INDEX_CACHE_CAPACITY
   */
  public final ICheckpointProtocol getIndexWithCheckpointAddr(final long checkpointAddr) {
    ...
    if (ndx == null) {
       ...
       */
      ndx = Checkpoint.loadFromCheckpoint(this, checkpointAddr, true /* readOnly */);
   ...
   *
   * <p>Note: You MUST {@link #commit()} before the registered index will be either restart-safe or
   * visible to new transactions.
   */
  @Override
  public final void registerIndex(final IndexMetadata metadata) {
   ...
   */
  @Override
  public final BTree registerIndex(final String name, final IndexMetadata metadata) {
  ...
  @Override
  public final BTree registerIndex(final String name, final BTree ndx) {
  ...
  public final void registerIndex(final String name, final HTree ndx) {
   ...
   * be visible to new transactions. Storage will be reclaimed IFF the backing store support that
   * functionality.
   */
  @Override
  public void dropIndex(final String name) {
    ...
    if (ndx == null) throw new NoSuchIndexException(name);
  ...
  public Iterator<String> indexNameScan(final String prefix, final long timestamp) {
    ...
    if (timestamp == ITx.READ_COMMITTED) {
      ...
      if (tx == null) throw new TransactionNotFoundException(timestamp);
   ...
   */
  @Override
  public final BTree getIndex(final String name) {
   ...
   */
  @Override
  public final ICheckpointProtocol getUnisolatedIndex(final String name) {
   ...
   * Quorum}.
   *
   * <p>The state of the field changes when a new quorum is negotiated or when an existing quorum is
   * broken. However, state changes in this field MUST be coordinated with the journal in order to
   * cover transitions into the quorum (blocked read/write requests must be released) and
   * transitions when the quorum breaks (the current write set must be discarded by the master). In
   * addition, when there is no quorum, the resynchronization protocol may affect both the
   * persistent state of the journal and any state which the journal keeps buffered in memory
   * (record cache, address translation cache, etc).
   *
   * <p>Access to this field is protected by the {@link #_fieldReadWriteLock} but MUST also be
   ...
   * associated quorum events. However, those events can not be delivered until the service is
   * connected to zookeeper and one of the common causes for entering the error state for the
   * HAJournalServer is because the zk client connection has been closed, hence, no zk events.
   *
  ...
  protected void setQuorumToken(final long newValue) {
    ...
    final boolean isServiceJoined = localService != null && localService.isJoinedMember(newValue);
   ...
   * @param isServiceJoined <code>true</code> iff this service is known to be a service that is
   *     joined with the met quorum.
   */
  private void setQuorumToken2(final long newValue, final boolean isServiceJoined) {

    if (haLog.isInfoEnabled())
      log.info("current: " + quorumToken + ", new: " + newValue + ", joined=" + isServiceJoined);
    ...
    final QuorumTokenTransitions transitionState =
        new QuorumTokenTransitions(quorumToken, newValue, isServiceJoined, haReadyToken);

    if (haLog.isInfoEnabled()) haLog.info(transitionState.toString());
    ...
    try {
       ...
       * </pre>
       *
       * In addition, there is a case where a service is joined as perceived by the ZKQuorum but not
       * yet HAReady. If a 2-phase commit is initiated, then the service will enter an error state
       * (because it is not yet HAReady). This net-zero change case is explicitly handled below.
       */
      if (transitionState.didLeaveMetQuorum) {
         ...
         * UpdateTask and the execution of the REST Mutation API methods in a well-known ThreadGuard
         * and then do interruptAll() to force the cancelation of any running task? We could also
         * wrap any IIndexManagerCallable in HAGlue.submit() with a FutureTask implementation that
         * uses the appropriate ThreadGuard to ensure that any unisolated tasks are cancelled (that
         * is actually overkill since it would not differentiate TX based operations from unisolated
         * operations - we could also use that ThreadGuard in AbstractTask). Add unit tests for both
         * UPDATE and other REST mutation methods.
         *
         * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/753" (HA doLocalAbort()
        ...
        this.haStatus =
            isLeader
                ? HAStatusEnum.Leader
                : isFollower ? HAStatusEnum.Follower : HAStatusEnum.NotReady;
        ...
        if (!installedRBs) {
           ...
           *
           * <p>If didJoinMetQuorum==true, then we MUST be leaving the Resync run state in the
           * HAJournalServer, so should NOT need to complete a localAbort.
           *
           * <p>TODO We should still review this point. If we do not delete a committed HALog, then
           * why is doLocalAbort() a problem here? Ah. It is because doLocalAbort() is hooked by the
           * HAJournalServer and will trigger a serviceLeave() and a transition to the error state.
           *
           * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/695">HAJournalServer
           *     reports "follower" but is in SeekConsensus and is not participating in commits</a>
           */
          if (haLog.isInfoEnabled())
            haLog.info(
                "Calling localAbort if NOT didJoinMetQuorum: " + transitionState.didJoinMetQuorum);
    ...
    if (haLog.isInfoEnabled())
      haLog.info(
          "done: token=" + quorumToken + ", HAReady=" + haReadyToken + ", HAStatus=" + haStatus);
  }
  ...
  public final long awaitHAReady(final long timeout, final TimeUnit units)
    ...
    try {
      ...
      while (((t = haReadyToken) == Quorum.NO_QUORUM)
          && getQuorum().getClient() != null
          && remaining > 0) {
        if (!haReadyCondition.await(remaining, TimeUnit.NANOSECONDS)) throw new TimeoutException();
  ...
  /** Returns the current value of the <code>haReadyToken</code> (non-blocking). */
   ...
   * API.
   *
   * @return The {@link HAStatusEnum} or <code>null</code> if the store is not associated with a
   *     {@link Quorum}.
   ...
   */
  public final void assertHAReady(final long token) throws QuorumException {
   ...
   *
   * <ol>
   *   <li>When the quorum meets for the first time, we need to take the root block from the leader
   *       and use it to replace both of our root blocks (the initial root blocks are identical).
   *       That will make the root blocks the same on all quorum members.
   *   <li>REBUILD: When a service goes through an automated disaster recovery, we need to install
   *       new root blocks in order to make the local journal logically empty. This prevents the
   *       service from attempting to interpret the data on the backing file if there is a restart
   *       part way through the rebuild operation.
   * </ol>
   *
   ...
   */
  protected void installRootBlocks(
    ...
    if (!rootBlock0.isRootBlock0()) throw new IllegalArgumentException();
    if (rootBlock1.isRootBlock0()) throw new IllegalArgumentException();
    ...
    if (!rootBlock0.getStoreType().equals(rootBlock1.getStoreType()))
      throw new IllegalArgumentException();
    if (!rootBlock0.getUUID().equals(rootBlock1.getUUID())) throw new IllegalArgumentException();
    ...
    try {
      ...
      haLog.warn(
          "Installed new root blocks: rootBlock0=" + rootBlock0 + ", rootBlock1=" + rootBlock1);

      // now reset the store with the root block
      if (_bufferStrategy instanceof IHABufferStrategy)
        ((IHABufferStrategy) _bufferStrategy).resetFromHARootBlock(_rootBlock);
   ...
   * offline.
   *
   * <p>TODO This method should be protected. If we move the HARestore class into this package, then
   * it can be changed from public to protected or package private.
   */
  public final void doLocalCommit(final IRootBlockView rootBlock) {
   ...
   *     whether the service is the leader. When the service is not the leader, we need to do some
   *     additional work to maintain the {@link IRWStrategy} allocators in synch at each commit
   *     point.
   * @param rootBlock The new root block.
   */
  protected void doLocalCommit(
      final QuorumService<HAGlue> localService, final IRootBlockView rootBlock) {
    ...
    try {
      ...
      final boolean shouldFlush =
          localService == null || (haStatus == null || haStatus == HAStatusEnum.NotReady);
      ...
      if (leader) {
        ...
        if (haLog.isInfoEnabled() && localService != null)
          haLog.info("PostHACommit: serviceUUID=" + localService.getServiceId());
      ...
      if (txLog.isInfoEnabled())
        txLog.info(
            "COMMIT: commitCounter=" + rootBlock.getCommitCounter() + ", commitTime=" + commitTime);
  ...
  protected IRootBlockView[] getRootBlocks() {
    ...
    try {
      ...
      final IRootBlockView rb0 =
          new RootBlockView(
              true /* rootBlock0 */,
              getBufferStrategy().readRootBlock(true /* rootBlock0 */),
              checker);

      final IRootBlockView rb1 =
          new RootBlockView(
              false /* rootBlock0 */,
              getBufferStrategy().readRootBlock(false /* rootBlock0 */),
              checker);
   ...
   *
   * <p>If this is not done then it is possible for the allocation data - both metabits and fixed
   * allocator commit bits - to be overwritten and inconsistent with the saved root blocks.
   *
   * @throws IOException
   */
  public ISnapshotData snapshotAllocationData(final AtomicReference<IRootBlockView> rbv)
      throws IOException {
    final Lock lock = _fieldReadWriteLock.readLock();
    ...
    try {
      ...
      // Disabling this test allows demonstration of the need to atomically snapshot the metabits
      // and allocators
      //	for the RWStore in conjunction with
      // TestHA1SnapshotPolicy.test_snapshot_stressMultipleTx_restore_validate
  ...
  protected class BasicHA implements HAGlue {
    ...
    protected BasicHA(final UUID serviceId, final InetSocketAddress writePipelineAddr) {

      if (serviceId == null) throw new IllegalArgumentException();
      if (writePipelineAddr == null) throw new IllegalArgumentException();
    ...
    public long awaitHAReady(final long timeout, final TimeUnit units)
        throws AsynchronousQuorumCloseException, InterruptedException, TimeoutException {
     ...
     * a new message type or return the right message. The joining service should be able to verify
     * that the release time is applicable for its commit counter (in {@link
     * AbstractHATransactionService#runWithBarrierLock(Runnable)}.
     */
    @Override
    public IHANotifyReleaseTimeResponse awaitServiceJoin(final IHAAwaitServiceJoinRequest req)
        throws AsynchronousQuorumCloseException, InterruptedException, TimeoutException {
        ...
        while ((remaining = nanos - (System.nanoTime() - begin)) > 0) {
          ...
          for (UUID t : joined) {
            ...
            if (serviceUUID.equals(t)) {
              ...
              if (log.isInfoEnabled()) log.info("Found Joined Service: " + serviceUUID);

              final JournalTransactionService ts =
                  (JournalTransactionService) getLocalTransactionManager().getTransactionService();

              return new HANotifyReleaseTimeResponse(ts.getReleaseTime(), -1);
            }
          }
          ...
          if (remaining > 0) {
            ...
            final long sleepMillis =
                Math.min(TimeUnit.NANOSECONDS.toMillis(remaining), 10 /* ms */);
     ...
     */
    protected final <E> Future<E> getProxy(final Future<E> future) {
     ...
     *
     * @param future The future.
     * @param asyncFuture When <code>true</code>, the service should not wait for the {@link Future}
     *     to complete but should return a proxy object that may be used by the client to monitor or
     *     cancel the {@link Future}. When <code>false</code>, the method should wait for the {@link
     *     Future} to complete and then return a "thick" {@link Future} which wraps the completion
     *     state but does not permit asynchronous monitoring or cancellation of the operation
     *     wrapped by the {@link Future}.
     * @return The proxy for that future.
     */
    protected <E> Future<E> getProxy(final Future<E> future, final boolean asyncFuture) {

      return future;
    }

    @Override
    public Future<Boolean> prepare2Phase(final IHA2PhasePrepareMessage prepareMessage) {

      if (prepareMessage == null) throw new IllegalArgumentException();
      ...
      if (haLog.isInfoEnabled())
        haLog.info(
            "isJoinedService="
                + prepareMessage.isJoinedService()
                + ", isRootBlock0="
                + isRootBlock0
                + ", rootBlock="
                + rootBlock
     ...
     * participate. This provides an atomic decision point with respect to when a service that is
     * rebuilding or resynchronizing will participate in a 2-phase commit. By voting NO here, the
     * commit2Phase() operation will be a NOP for THIS service.
     *
     * <p>Note: The vote of a service that was not joined with the met quorum at the time that we
     * begin the 2-phase commit protocol is ignored.
     */
     ...
     *
     * <p>Note: This code path is only when [isJoined := true].
     */
    private class Prepare2PhaseTask implements Callable<Boolean> {
      ...
      public Prepare2PhaseTask(
          final boolean isLeader, final IHA2PhasePrepareMessage prepareMessage) {
      ...
      public Boolean call() throws Exception {
        ...
        try {
          ...
          if (haLog.isInfoEnabled()) haLog.info("VOTE=" + vote.get());
          ...
          if (!vote.get()) {
             ...
             * actions to be taken, which is why then have been commented out above.
             *
             * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/695">HAJournalServer
             *     reports "follower" but is in SeekConsensus and is not participating in
             *     commits</a>
             */
            if (localService != null) {
      ...
      private Boolean innerCall() throws Exception {
         ...
         */
        final Future<IHANotifyReleaseTimeResponse> oldFuture =
            gatherFuture.getAndSet(null /* newValue */);

        if (haLog.isInfoEnabled()) haLog.info("gatherFuture=" + oldFuture);
        ...
        if (rootBlock == null) throw new IllegalStateException();
        ...
        final IHANotifyReleaseTimeResponse consensusReleaseTime =
            prepareMessage.getConsensusReleaseTime();

        {
          ...
          final long localReleaseTime =
              getLocalTransactionManager().getTransactionService().getReleaseTime();

          // Note: Per the GatherTask (in Journal.java).
          final long expectedReleaseTime = Math.max(0L, consensusReleaseTime.getCommitTime() - 1);
        ...
        /*isJoined &&*/ !isLeader) {
           ...
           * follower.
           *
           * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/673" > Native thread
           *     leak in HAJournalServer process </a>
           */
          ...
          try {
            ...
            if ((tmp.getCommitCounter() != consensusReleaseTime.getCommitCounter())
                || (tmp.getCommitTime() != consensusReleaseTime.getCommitTime())) {
            ...
            haLog.error("Gather failed on follower: serviceId=" + getServiceId() + " : " + e, e);
            return vote.get();
          }
        }
     ...
     *
     * <ul>
     *   <li>The UUID of the store must be the same.
     *   <li>The commitTime must be strictly increasing.
     *   <li>The commitCounter must increase by ONE (1).
     *   <li>
     * </ul>
     *
     * Note: This code path is only when [isJoined := true] (that is, the service was joined with
     * the met quorum at the atomic decision point for the joined set for the 2-phase commit).
     *
     * @param isLeader iff this service is the leader for this commit.
     * @param oldRB the old (aka current) root block.
     * @param newRB the new (aka proposed) root block.
     */
    //        * @param isJoined
    ...
    protected void validateNewRootBlock( // final boolean isJoined,
        final boolean isLeader, final IRootBlockView oldRB, final IRootBlockView newRB) {

      if (oldRB == null) throw new IllegalStateException();

      if (newRB == null) throw new IllegalStateException();
      ...
      if (!newRB.getUUID().equals(oldRB.getUUID())) {
        ...
        throw new IllegalStateException(
            "Store UUID: old=" + oldRB.getUUID() + " != new=" + newRB.getUUID());
      }
      ...
      // Validate the new commit counter.
      {
        ...
        if (newcc != (oldcc + 1)) {
          ...
          throw new IllegalStateException(
              "commitCounter: ( old=" + oldcc + " + 1 ) != new=" + newcc);
        }
      }
      ...
      if (isLeader) {
        ...
        if (!HAStatusEnum.Leader.equals(st)) {
          ...
          throw new IllegalStateException(
              "HAStatusEnum: expected=" + HAStatusEnum.Leader + ", actual=" + st);
        }
        ...
        if (!HAStatusEnum.Follower.equals(st)) {
          ...
          throw new IllegalStateException(
              "HAStatusEnum: expected=" + HAStatusEnum.Follower + ", actual=" + st);
        }
      }
      ...
      if (prepareToken != tmp) {
        ...
        throw new IllegalStateException(
            "HAReadyToken: expected=" + prepareToken + ", actual=" + tmp);
      }
    }
    ...
    private class Commit2PhaseTask implements Runnable {
      ...
      public Commit2PhaseTask(final IHA2PhaseCommitMessage commitMessage) {
        ...
        if (commitMessage == null) throw new IllegalArgumentException();
      ...
      public void run() {
        ...
        try {
          ...
          try {
            ...
            haLog.error(
                "ERROR IN 2-PHASE COMMIT: "
                    + t
                    + ", rootBlock="
                    + prepareRequest.get().getRootBlock(),
                t);
      ...
      private void doInnerRun(final QuorumService<HAGlue> localService) throws Exception {
        ...
        final IRootBlockView rootBlock =
            prepareMessage == null ? null : prepareMessage.getRootBlock();
        ...
        if (haLog.isInfoEnabled())
          haLog.info(
              "commitTime="
                  + commitTime
                  + ", commitCounter="
                  + prepareMessage.getRootBlock().getCommitCounter()
                  + ", vote="
                  + vote);
    ...
    private class Abort2PhaseTask implements Runnable {
      ...
      public Abort2PhaseTask(final IHA2PhaseAbortMessage abortMessage) {
        ...
        if (abortMessage == null) throw new IllegalArgumentException();
     ...
     *     there is a cache miss. Unfortunately the cache holds objects, some of which declare how
     *     to access the underlying {@link IDataRecord} using the {@link IDataRecordAccess}
     *     interface.
     * @todo Since these are rare events it may not be worthwhile to setup a separate low-level
     *     socket service to send/receive the data.
     */
    @Override
    public Future<IHAReadResponse> readFromDisk(final IHAReadRequest msg) {
    ...
    public Future<Void> receiveAndReplicate(
      ...
      if (haLog.isDebugEnabled()) haLog.debug("req=" + req + ", msg=" + msg);

      final Future<Void> ft = quorum.getClient().receiveAndReplicate(req, snd, msg);
    ...
    @Override
    public IHAWriteSetStateResponse getHAWriteSetState(final IHAWriteSetStateRequest req) {
    ...
    public IHARootBlockResponse getRootBlock(final IHARootBlockRequest msg) {
      ...
      if (haLog.isInfoEnabled()) haLog.info("storeId=" + storeId);
    ...
    public Future<IHAPipelineResetResponse> resetPipeline(final IHAPipelineResetRequest req)
        throws IOException {
      final Future<IHAPipelineResetResponse> f = quorum.getClient().resetPipeline(req);
      return getProxy(f);
    }
    ...
    public void gatherMinimumVisibleCommitTime(final IHAGatherReleaseTimeRequest req)
      ...
      if (leader == null) throw new RuntimeException("Could not discover the quorum leader.");

      // Get our serviceId.
      final UUID serviceId = getServiceId();

      if (serviceId == null) throw new AssertionError();

      final Callable<IHANotifyReleaseTimeResponse> task =
          ((AbstractHATransactionService)
                  AbstractJournal.this.getLocalTransactionManager().getTransactionService())
    ...
    public IHANotifyReleaseTimeResponse notifyEarliestCommitTime(
      ...
      return ((HATXSGlue) AbstractJournal.this.getLocalTransactionManager().getTransactionService())
     ...
     *
     * <p>Note: This method is NOT exposed to RMI. However, it can still be overridden by the unit
     * tests.
     *
    ...
    public <T> Future<T> submit(final IIndexManagerCallable<T> callable, final boolean asyncFuture)
      ...
      final Future<T> ft = getIndexManager().getExecutorService().submit(callable);
   ...
   * CommitRecordIndex where the CommitRecords reference the deleteBlocks that have been deferred.
   *
   * <p>Once processed the records for the effected range muct be removed as they reference invalid
   * states.
   *
   * @param fromKey
   * @param toKey
   ...
   */
  public int removeCommitRecordEntries(final byte[] fromKey, final byte[] toKey) {
