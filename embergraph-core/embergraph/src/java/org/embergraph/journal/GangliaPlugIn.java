 ...
 *
 * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/441">Ganglia Integration</a>
...
public class GangliaPlugIn implements IPlugIn<Journal, GangliaService> {
  ...
  public interface Options {
    ...
    String GANGLIA_LISTEN_GROUP = Journal.class.getName() + ".ganglia.listenGroup";
    ...
    String GANGLIA_LISTEN_PORT = Journal.class.getName() + ".ganglia.listenPort";
     ...
     */
    String GANGLIA_LISTEN = Journal.class.getName() + ".ganglia.listen";
     ...
     * then the embedded {@link GangliaService} will not be started.
     */
    String GANGLIA_REPORT = Journal.class.getName() + ".ganglia.report";
     ...
     * a multicast group.
     */
    String GANGLIA_SERVERS = Journal.class.getName() + ".ganglia.servers";
  ...
  public void startService(final Journal journal) {
    ...
    final boolean listen =
        Boolean.valueOf(
            properties.getProperty(Options.GANGLIA_LISTEN, Options.DEFAULT_GANGLIA_LISTEN));

    final boolean report =
        Boolean.valueOf(
            properties.getProperty(Options.GANGLIA_REPORT, Options.DEFAULT_GANGLIA_REPORT));

    if (!listen && !report) return;
    ...
    try {
      ...
      final InetAddress listenGroup =
          InetAddress.getByName(
              properties.getProperty(
                  Options.GANGLIA_LISTEN_GROUP, Options.DEFAULT_GANGLIA_LISTEN_GROUP));

      final int listenPort =
          Integer.valueOf(
              properties.getProperty(
                  Options.GANGLIA_LISTEN_PORT, Options.DEFAULT_GANGLIA_LISTEN_PORT));
      ...
      final InetSocketAddress[] metricsServers =
          GangliaUtil.parse(
              // server(s)
              properties.getProperty(Options.GANGLIA_SERVERS, Options.DEFAULT_GANGLIA_SERVERS),
      ...
      final int monitoringInterval =
          (int)
              TimeUnit.MILLISECONDS.toSeconds(
                  Long.parseLong(
                      properties.getProperty(Options.REPORT_DELAY, Options.DEFAULT_REPORT_DELAY)));
      ...
      gangliaService.addMetricCollector(
          new QueryEngineMetricsCollector(journal, statisticsCollector));
   ...
   * the GangliaService in order for the thread pool to shutdown.
   */
  @Override
