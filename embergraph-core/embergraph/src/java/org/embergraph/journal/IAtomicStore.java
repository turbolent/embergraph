...
public interface IAtomicStore extends IRawStore {
   ...
   * #abort()} to abandon the current write set.
   *
   * @return The timestamp assigned to the {@link ICommitRecord} -or- 0L if there were no data to
   *     commit.
   ...
   *
   * @param index The slot in the root block where the address of the {@link ICommitter} will be
   *     recorded.
