...
//     * Instance used to encode the timestamp into the key.
  ...
  public byte[] getKey(final long commitTime) {
    ...
    return getIndexMetadata().getKeyBuilder().reset().append(commitTime).getKey();
  }
   ...
   * time. In this context the LTE search identifies the most recent commit state that not later
   * than the start time of the transaction.
   *
   * @param timestamp The given timestamp.
   * @return The description of the relevant journal resource -or- <code>null</code> iff there are
   *     no journals in the index that satisify the probe.
   * @throws IllegalArgumentException if <i>timestamp</i> is less than ZERO (0L).
   */
   ...
   * @return The root block of that entry -or- <code>null</code> if there is no entry whose
   *     timestamp is strictly greater than <i>timestamp</i>.
   * @throws IllegalArgumentException if <i>timestamp</i> is less than ZERO (0L).
   */
   ...
   *
   * @param commitTime The timestamp.
   * @return The index of the entry associated with the largest commitTime that is less than or
   *     equal to the given timestamp -or- <code>-1</code> iff the index is empty.
   * @throws IllegalArgumentException if <i>timestamp</i> is less than ZERO (0L).
   */
   ...
   * @exception IllegalArgumentException if <i>rootBLock</i> is <code>null</code>.
   ...
   * @throws IllegalArgumentException if <code>commitCounter LT ZERO (0)</code> TODO It is possible
   *     to improve the performance for this for large indices using a binary search. Each time we
   *     probe the index we discover a specific commit counter value. If the value is LT the target,
   *     we need to search above that probe. If GT the target, we need to search below that problem.
   */
   ...
   *
   * <p>Note: The effective index is given by <code>(entryCount-1)-index</code>. If the effective
   * index is LT ZERO (0) then there is no such entry and this method will return <code>null</code>.
   *
   * @param index The index.
   * @return The entry -or- <code>null</code> if there is no such entry.
   * @throws IllegalArgumentException if <code>index LT ZERO (0)</code>
   */
  public T getEntryByReverseIndex(final int index) {
    ...
    try {
      ...
      if (entryCount > Integer.MAX_VALUE) throw new AssertionError();
      ...
      final ITuple<T> t = valueAt(effectiveIndex, getLookupTuple());
