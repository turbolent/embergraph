...
public interface IDistributedTransactionService extends ITransactionService {
   ...
   * tx to impose a partial order on commits. That partial order guarantees that commits do not
   * deadlock in contention for the same resources.
   *
   * @param tx The transaction identifier.
   * @param dataService The {@link UUID} an {@link IDataService} on which the transaction will
   *     write.
   * @param resource An array of the named resources which the transaction will use on that {@link
   *     IDataService} (this may be different for each operation submitted by that transaction to
   ...
   * each {@link IDataService}.
   *
   * <p>Note: If this method throws ANY exception then the task MUST cancel the commit, discard the
   * local write set of the transaction, and note that the transaction is aborted in its local
   * state.
   *
   * @param tx The transaction identifier.
   * @param dataService The {@link UUID} of the {@link IDataService} which sent the message.
   ...
   * and the task MUST rollback the live journal to the previous commit point. If the return is
   * <code>true</code> then the distributed commit was successful and the task should halt
   * permitting the {@link IDataService} to return from the {@link ITxCommitProtocol#prepare(long,
   * long)} method.
   *
   * @param tx The transaction identifier.
   * @param dataService The {@link UUID} of the {@link IDataService} which sent the message.
   * @return <code>true</code> if the distributed commit was successfull and <code>false</code> if
   *     there was a problem.
   * @throws IOException
   */
