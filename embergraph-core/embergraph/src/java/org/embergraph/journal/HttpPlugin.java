...
public class HttpPlugin implements IPlugIn<Journal, CounterSetHTTPD> {
  ...
  public interface Options {
     ...
     * Journal#getHttpdURL()}. The httpd service may be disabled by specifying <code>-1</code> as
     * the port.
     */
   ...
   * Journal#getHttpdPort()}, on a randomly assigned port if the port is <code>0</code>, or NOT
   * started if the port is <code>-1</code>. If the service is started, then the URL for the service
   * is reported to the load balancer and also written into the file system. When started, the httpd
  ...
  public void startService(final Journal indexManager) {
    ...
    final int httpdPort =
        Integer.valueOf(indexManager.getProperty(Options.HTTPD_PORT, Options.DEFAULT_HTTPD_PORT));
    ...
    synchronized (this) {
      ...
      if (log.isInfoEnabled()) log.info("Performance counters: " + httpdURL);
    }
  }
