...
public class Name2Addr extends BTree {
  ...
  /*
   * Cache of added/retrieved btrees by _name_. This cache is ONLY used by the "live" {@link
   * Name2Addr} instance.
   *
   * <p>Map from the name of an index to a weak reference for the corresponding "live" version of
   * the named index. Entries will be cleared from this map after they have become only weakly
   * reachable. In order to prevent dirty indices from being cleared, we register an {@link
   * IDirtyListener}. When it is informed that an index is dirty it places a hard reference to that
   * index into the {@link #commitList}.
   *
   * <p>Note: The capacity of the backing hard reference LRU effects how many _clean_ indices can be
   * held in the cache. Dirty indices remain strongly reachable owing to their existence in the
   * {@link #commitList}.
   */
  private ConcurrentWeakValueCache<String, ICheckpointProtocol> indexCache = null;

  /*
   * Holds hard references for the dirty indices along with the index name. This collection prevents
   * dirty indices from being cleared from the {@link #indexCache}, which would result in lost
   * updates.
   *
   * <p>Note: Operations on unisolated indices always occur on the "current" state of that index.
   * The "current" state is either unchanged (following a successful commit) or rolled back to the
   * last saved state (by an abort following an unsuccessful commit). Therefore all unisolated index
   * write operations MUST complete before a commit and new unisolated operations MUST NOT begin
   * until the commit has either succeeded or been rolled back. Failure to observe this constraint
   * can result in new unisolated operations writing on indices that should have been rolled back if
   ...
   * administer to listen for events indicating that the index is dirty. When we get that event we
   * stick the {@link DirtyListener} on the {@link #commitList}. This makes the commit protocol
   * simpler since the {@link DirtyListener} has both the name of the index and the reference to the
   * index and we need both on hand to do the commit.
   *
  ...
  private class DirtyListener implements IDirtyListener, Comparable<DirtyListener> {
    ...
    public String toString() {
      ...
      return "DirtyListener{name="
          + name
          + ","
          + (needsCheckpoint ? "needsCheckpoint" : "checkpointAddr=" + checkpointAddr)
          + "}";
    }

    private DirtyListener(
        final String name, final ICheckpointProtocol btree, final boolean needsCheckpoint) {
    ...
    /** Return the {@link Name2Addr} instance to which this listener is reporting. */
    ...
    /*
     * Add <i>this</i> to the {@link Name2Addr#commitList} and set {@link #needsCheckpoint} to
     * <code>true</code>.
     *
     * @param btree
     */
   ...
   */
  public static Name2Addr create(final IRawStore store) {
  ...
  /*
   * Many methods on this class will throw an {@link IllegalStateException} unless they are invoked
   * on the {@link ITx#UNISOLATED} {@link Name2Addr} instance. This method is used to test that
   * assertion for those methods.
   *
   * @throws IllegalStateException unless this is the {@link ITx#UNISOLATED} {@link Name2Addr}
   *     instance.
   */
  ...
  /** Return <code>true</code> iff this is the {@link ITx#UNISOLATED} {@link Name2Addr} instance. */
   ...
   *
   * <p>Note: This cache is <code>null</code> unless initialized and is ONLY used by the "live"
   * version of the {@link Name2Addr} index. The only method that creates or loads the "live" {@link
   * Name2Addr} index is {@link AbstractJournal#setupName2AddrBTree()}.
   *
   * @param cacheCapacity The capacity of the inner {@link LRUCache} for the {@link WeakValueCache}.
   * @param cacheTimeout The timeout in milliseconds for stale entries in the cache.
  ...
  /*
   * An iterator that visits the entries in the internal {@link #indexCache}. You must test the weak
   * reference for each entry in order to determine whether its value has been cleared as of the
   * moment that you request that value.
   *
   * @throws IllegalStateException unless this is the {@link ITx#UNISOLATED} instance.
   */
  private Iterator<java.util.Map.Entry<String, WeakReference<ICheckpointProtocol>>>
   ...
   *
   * <p>Note: This is synchronized even through the commitList is thread-safe in order to make the
   * test atomic with respect to {@link #handleCommit(long)}.
   *
   * @param name The index name.
   * @throws IllegalStateException unless this is the {@link ITx#UNISOLATED} instance.
   */
  public synchronized boolean willCommit(final String name) {
  ...
  /*
   * Flush a dirty index to the disk. This is used to flush each dirty index in parallel, providing
   * increased IO throughput and reduced latency during the commit.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
  ...
  private static class CommitIndexTask implements Callable<CommitIndexTask> {
    ...
    /*
     * @param l The {@link DirtyListener}.
     * @param commitTime The commitTime associated with the commitPoint.
     */
    ...
    public CommitIndexTask call() {
      ...
      if (l.needsCheckpoint) {
        ...
        if (checkpointAddr == 0L) {
          ...
          throw new RuntimeException("Checkpoint address not written: name=" + l.name);
        }
      }
   ...
   *
   * <p>This method applies the {@link #commitList} and then flushes the backing {@link
   * ICheckpointProtocol} object to the store. The {@link #commitList} consists of {@link
   * DirtyListener}s. If the listener has its {@link DirtyListener#needsCheckpoint} flag set, then
   * the {@link ICheckpointProtocol} implementation to which that listener is attached will have its
   * {@link BTree#writeCheckpoint() checkpoint written} . Otherwise the current {@link Checkpoint}
   * address is recovered. Either way, the {@link Entry} in {@link Name2Addr}s backing {@link BTree}
   * is updated to reflect the <i>commitTime</i> and {@link Checkpoint} address for the index.
   *
   * <p>Finally {@link Name2Addr} {@link Checkpoint}s itself using {@link
   * ICommitter#handleCommit(long)} and returns the address from which {@link Name2Addr} may be
   * reloaded.
   *
   * <p>Note: The {@link #commitList} MUST be protected against concurrent modification during the
   * commit otherwise concurrent tasks could be reporting dirty objects while we are doing a commit
   * and those notices would be lost. Persistence capable objects ({@link ICheckpointProtocol}
   * implementations) get onto the {@link #commitList} via the {@link DirtyListener}, so it is also
   * synchronized.
   *
   * <p>Note: {@link Name2Addr} DOES NOT obtain a resource lock on the {@link ICheckpointProtocol}
   * implementation. Therefore it MUST NOT checkpoint an index on which an {@link AbstractTask} has
   * obtained a resource lock. Otherwise we have concurrent writers on the {@link BTree} and the
   * {@link BTree} is not thread-safe for concurrent writers. Instead, the {@link AbstractTask}
   * checkpoints the {@link BTree} itself while it is holding the resource lock and then sets {@link
   * DirtyListener#needsCheckpoint} to <code>false</code> using {@link #putOnCommitList(String,
   * ICheckpointProtocol, boolean)} as an indication to {@link Name2Addr} that it MUST persist the
   * current checkpointAddr for the {@link BTree} on its next commit (and MUST NOT write on the
   * index when it does that commit).
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/675" >Flush indices in parallel
   *     during checkpoint to reduce IO latency</a>
   */
  @Override
  public synchronized long handleCommit(final long commitTime) {
    ...
    try {
      ...
      final ExecutorService executorService = ((IIndexManager) getStore()).getExecutorService();
    ...
    for (Future<CommitIndexTask> f : futures) {
      ...
      try {
        ...
        if (oldEntry == null
            || oldEntry.checkpointAddr != checkpointAddr
  ...
  /*
   * Return the {@link ITx#UNISOLATED} view of the named persistence capable data structure - this
   * method tests a cache of the named persistence capable data structures and will return the
   * existing instance if the index is found in the cache and will otherwise load the {@link
   * ITx#UNISOLATED} view of the data structure from the backing store.
   *
   * @param name The index name.
   * @return The named index or <code>null</code> iff there is no index with that name.
   * @throws IllegalArgumentException if <i>name</i> is <code>null</code>.
   * @throws IllegalStateException if this is not the {@link ITx#UNISOLATED} {@link Name2Addr}
   *     instance.
   */
  public ICheckpointProtocol getIndex(final String name) {
    ...
    final Entry entry =
        EntrySerializer.INSTANCE.deserialize(new DataInputStream(new ByteArrayInputStream(val)));

    // Load from the backing store.
    ndx = Checkpoint.loadFromCheckpoint(store, entry.checkpointAddr, false /* readOnly */);
   ...
   *
   * <p>Note: This is a lower-level access mechanism that is used by {@link Journal#getIndex(String,
   * ICommitRecord)} when accessing historical named indices from an {@link ICommitRecord}.
   *
   * @param name The index name.
   * @return The {@link Entry} for the named index -or- <code>null</code> if there is no entry for
   *     that <i>name</i>.
   */
   ...
   *
   * @param name The index name.
   * @param btree The index.
   * @exception IllegalArgumentException if <i>name</i> is <code>null</code>.
   * @exception IllegalArgumentException if <i>btree</i> is <code>null</code>.
   * @exception IndexExistsException if there is already an index registered under that name.
   */
  public synchronized void registerIndex(final String name, final ICheckpointProtocol btree) {
  ...
  /*
   * Adds the named index to the commit list and sets a {@link DirtyListener} on the {@link
   * ICheckpointProtocol} so that this {@link Name2Addr} object will be informed if the associated
   * persistent data structure becomes dirty.
   *
   * @param name The index name.
   * @param btree The persistence capable data structure.
   * @param needsCheckpoint Specify <code>true</code> if {@link Name2Addr} should invoke {@link
   *     ICheckpointProtocol#writeCheckpoint()} rather than just updating the {@link Entry} for the
   *     persistenc capable data structure using {@link ICheckpointProtocol#getCheckpoint()}
   */
  protected synchronized void putOnCommitList(
      final String name, final ICheckpointProtocol btree, final boolean needsCheckpoint) {
   ...
   *
   * @param name The index name.
   * @param btree The {@link ITx#UNISOLATED} view of the persistence capable data structure.
   * @param replace If an existing entry for that name may be replaced.
   */
  protected synchronized void putIndexCache(
      final String name, final ICheckpointProtocol btree, final boolean replace) {
  ...
  /*
   * Return the current entry, if any, for the named {@link ITx#UNISOLATED} index in the {@link
   * #indexCache}.
   *
   * <p>Note: This method is more direct than {@link #getIndex(String)}. {@link AbstractTask} uses
   * this method together with {@link #putIndexCache(String, ICheckpointProtocol, boolean)} to allow
   * different tasks access to the same pool of {@link ITx#UNISOLATED} indices.
   *
   * @param name The index name.
   * @return The index iff it was found in the cache.
   */
  protected synchronized ICheckpointProtocol getIndexCache(final String name) {
  ...
  /*
   * Removes the entry for the named index. The named index will no longer participate in commits.
   *
   * @param name The index name.
   * @exception IllegalArgumentException if <i>name</i> is <code>null</code>.
   * @exception NoSuchIndexException if the index does not exist.
   */
  ...
  public void invalidate(final Throwable t) {
    ...
    final Iterator<java.util.Map.Entry<String, WeakReference<ICheckpointProtocol>>> itr =
        indexCacheEntryIterator();
  ...
  /*
   * Return a {@link CounterSet} reflecting the named indices that are currently open (more
   * accurately, those open named indices whose references are in {@link Name2Addr}s internal {@link
   * #indexCache}). When index partitions are in use their {@link CounterSet}s are reported under a
   * path formed from name of the scale-out index and partition identifier. Otherwise the {@link
   * CounterSet}s are reported directly under the index name.
   *
   * @param counterSet When non-<code>null</code> the performance counters are entered into the
   *     caller's collection. Otherwise they are entered into a new collection.
   * @param found When non-<code>null</code>, the names of the indices whose performance counters
   *     are being returned is reported as a side-effect on this {@link Set}.
   ...
   */
  protected CounterSet getIndexCounters(
      final CounterSet counterSet, final Set<String /* Name */> found) {

    assertUnisolatedInstance();

    final CounterSet tmp = counterSet == null ? new CounterSet() : counterSet;
   ...
   *
   * <p>The {@link Entry} reports the {@link #name} of the index, its {@link #checkpointAddr}, and
   * the {@link #commitTime} when it was last updated. If you want to know more about the index,
   * then you need to open it.
   *
   * <p>Do NOT open the index directly from the {@link #checkpointAddr}. That will circumvent the
   * canonical mapping imposed by the {@link IIndexManager} on the open indices for a given name and
   * commit time. Instead, just ask the {@link IIndexManager} to open the index having the specified
   * {@link #name} as of the desired commit time.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  public static class Entry {
    ...
    /** The address of the last known {@link Checkpoint} record for the index with that name. */
    public final long checkpointAddr;

    /** The commit time associated with the last commit point for the named index. */
    public final long commitTime;

    public Entry(final String name, final long checkpointAddr, final long commitTime) {
  ...
  public static class Name2AddrTupleSerializer extends DefaultTupleSerializer<String, Entry> {
     ...
     */
    public Name2AddrTupleSerializer(final IKeyBuilderFactory keyBuilderFactory) {
     ...
     *
     * @param obj The name of an index.
     */
    @Override
    public byte[] serializeKey(final Object obj) {
      ...
      //            log.error("name=" + obj + ", key=" + BytesUtil.toString(a)+",
  ...
  /*
   * Prefix scan of a {@link Name2Addr} index. This scan assumes that the caller has provided for
   * any possible thread-safety issues.
   *
  ...
  public static Iterator<String> indexNameScan(final String prefix, final IIndex n2a) {
    ...
    if (hasPrefix) // && restrictScan)
    {
      ...
      final IKeyBuilder keyBuilder = n2a.getIndexMetadata().getPrimaryKeyBuilder();
  ...
  //     *
  //     * Note that this has only been analyzed for the ICU collator, the standard Java collator
  // does include
  //     * 00 bytes in the key.  However, it too appears to delimit the value key with a 01 byte so
  // the
