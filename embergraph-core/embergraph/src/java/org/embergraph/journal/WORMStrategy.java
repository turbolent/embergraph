...
import org.embergraph.io.writecache.WriteCacheCounters;
 ...
 * only large sequential writes are performed on the store. Reads read through the write cache for
 * consistency.
 *
 * <p>Note: This is used to realize both the {@link BufferMode#Disk} and the {@link
 * BufferMode#Temporary} {@link BufferMode}s. When configured for the {@link BufferMode#Temporary}
 * mode: the root blocks will not be written onto the disk, writes will not be forced, and the
 * backing file will be created the first time the {@link DiskOnlyStrategy} attempts to write
 * through to the disk. For many scenarios, the backing file will never be created unless the write
 * cache overflows. This provides very low latency on start-up, the same MRMW capability, and allows
 * very large temporary stores.
 *
 * <p>FIXME Examine behavior when write caching is enabled/disabled for the OS. This has a profound
 * impact. Asynchronous writes of multiple buffers, and the use of smaller buffers, may be
 * absolutely when the write cache is disabled. It may be that swapping sets in because the Windows
 * write cache is being overworked, in which case doing incremental and async IO would help. Compare
 * with behavior on server platforms. See http://support.microsoft.com/kb/259716,
 ...
 * http://support.microsoft.com/kb/811392, http://mail-archives.apache.org/mod_mbox
 ...
 * </pre>
 *
 * @todo report whether or not the on-disk write cache is enabled for each platform in {@link
 *     AbstractStatisticsCollector}. offer guidance on how to disable that write cache.
 * @todo test verifying that the write cache can be disabled.
 * @todo test verifying that the buffer position and limit are updated correctly by {@link
 *     #write(ByteBuffer)} regardless of the code path.
 * @todo Retro fit the concept of a write cache into the {@link DirectBufferStrategy} so that we
 *     defer writes onto the disk until (a) a threshold of data has been buffered; or (b) {@link
 *     #force(boolean)} is invoked. Note that the implementation will be a bit different since the
 *     Direct mode is already fully buffered so we do not need to allocate a separate writeCache.
 *     However, we will still need to track the {@link #writeCacheOffset} and maintain a {@link
 *     #writeCacheIndex}.
 ...
 */
public class WORMStrategy extends AbstractBufferStrategy
    implements IDiskBasedStrategy, IHABufferStrategy, IBackingReader {
   ...
   * For {@link BufferMode#Disk}, this field is initially <code>true</code>.
   *
   * <p>The value of this field determines the behavior of {@link #reopenChannel()}.
   */
   ...
   *
   * <p>This MAY be <code>null</code> for a read-only store or if the write cache is disabled. It is
   * required for HA.
   *
   * <p>TODO This should not really be volatile. For HA, we wind up needing to set a new value on
   * this field in {@link #abort()}. The field used to be final. Perhaps an {@link AtomicReference}
    ...
    public CounterSet getCounters() {
       ...
       * write
        ...
        disk.addCounter(
            ...
            new Instrument<Double>() {
              public void sample() {
                final double bytesPerDiskWrite =
                    (ndiskWrite == 0 ? 0d : (bytesWrittenOnDisk / (double) ndiskWrite));
        ...
        disk.addCounter(
            ...
            new Instrument<Double>() {
              public void sample() {
                final double diskWriteSecs = (elapsedDiskWriteNanos / 1000000000.);
                final double bytesWrittenPerSec =
                    (diskWriteSecs == 0L ? 0d : bytesWrittenOnDisk / diskWriteSecs);
                setValue(bytesWrittenPerSec);
              }
            });
        ...
        disk.addCounter(
            ...
            new Instrument<Double>() {
              public void sample() {
                final double diskWriteSecs = (elapsedDiskWriteNanos / 1000000000.);
                final double writeLatency = (diskWriteSecs == 0 ? 0d : diskWriteSecs / ndiskWrite);
  ...
  WORMStrategy(
    ...
    super(
        ...
        Long.valueOf(
            fileMetadata.getProperty(Options.INITIAL_EXTENT, Options.DEFAULT_INITIAL_EXTENT)),
        //                fileMetadata.extent,
        maximumExtent,
        fileMetadata.offsetBits,
        fileMetadata.nextOffset,
    ...
    final boolean useWriteCacheService =
        fileMetadata.writeCacheEnabled && !fileMetadata.readOnly && fileMetadata.closeTime == 0L
            || isQuorumUsed;
  ...
  private WORMWriteCacheService newWriteCacheService() {
    ...
    try {
      ...
      return new WORMWriteCacheService(writeCacheBufferCount, useChecksums, extent, opener, quorum);
  ...
  private class WORMWriteCacheService extends WriteCacheService {
    ...
    WORMWriteCacheService(
      ...
      super(
          ...
          0 /* minCleanListSize */,
          readCacheBufferCount,
          false /* prefixWrites */,
          100 /* compactionThreshold */,
          hotCacheSize,
          hotCacheThreshold,
          useChecksums,
          extent,
          opener,
          quorum,
          WORMStrategy.this /*reader*/);
    }

    @Override
    public WriteCacheImpl newWriteCache(
  ...
  private class WriteCacheImpl extends WriteCache.FileChannelWriteCache {
    ...
    public WriteCacheImpl(
      ...
      super(baseOffset, buf, useChecksum, isQuorumUsed, bufferHasData, opener, fileExtent);
    }
   ...
   * are made, but only after they have been flushed.
   *
   * <p>TODO This pattern arose because {@link WriteCacheService#resetSequence()} is called from
   * {@link #commit()}. It might be more pleasant to call that from {@link
   * AbstractJournal#commitNow(long)} so we could consolidate this logic. That requires us to expose
   * the {@link WriteCacheService} to the {@link AbstractJournal} through the {@link
   * IHABufferStrategy}.
   */
   ...
   * other threads will be discarded from the cache!
   */
  @Override
  public void abort() {
   ...
   * <p>{@inheritDoc}
   *
   * @todo hook for monitoring (nagios, etc). bad reads indicate a problem with the disk which
   *     should be tracked over time.
   * @todo If we see a read error from a checksum and want to update the record on the backing file
   *     then we would have to go around the write cache to do a direct disk write since (at least
   *     for the WORM) the assumption is pure append for the write cache.
   *     <p>An attempt to overwrite a bad record on the disk could itself be a bad idea. If it was
   *     just a high write, then it might be Ok. But many other kinds of errors are likely to have
   *     long pauses while the OS attempts to get a good read/write from the file system.
   * @todo We might want to maintain a set of known bad records and fail the node when the size of
   *     that set grows too large. That would also help us to avoid "hanging" on a bad read when we
   *     know that we have to get the data from another node based on past experience for that
   *     record.
   */
  @Override
      ...
      if (quorum != null && quorum.isHighlyAvailable()) {
        if (quorum.isQuorumMet()) {
          try {
            ...
            final byte[] a =
                ((QuorumRead<?>) quorum.getMember()).readFromQuorum(storeUUIDRef.get(), addr);
   ...
   * local disk on a cache miss. This is automatically invoked by {@link #read(long)}.
   */
  public ByteBuffer readFromLocalStore(final long addr) throws InterruptedException {

    final long begin = System.nanoTime();

    if (addr == 0L) throw new IllegalArgumentException(ERR_ADDRESS_IS_NULL);
    ...
    {
      final StoreCounters<?> storeCounters = (StoreCounters<?>) this.storeCounters.get().acquire();
      try {
     ...
     */
    {
      ...
      if (log.isTraceEnabled()) log.trace("diskRead: addr=" + toString(addr));
   ...
   *
   * @param offset The offset of the first byte (now absolute, not relative to the start of the data
   *     region).
  ...
  public ByteBuffer readRaw(final long offset, final ByteBuffer dst) {
    ...
    try {
      ...
      try {
        ...
        // read on the disk.
        final int ndiskRead = FileChannelUtility.readAll(opener, dst, pos);
   ...
   * open the store at the same time.
   *
   * <p>Note: This method is invoked by both readers and writers. It WILL NOT be invoked by {@link
   * FileChannelUtility} if the channel was closed by an interrupt in the current thread (a
  ...
  private FileChannel reopenChannel() throws IOException {
    ...
    synchronized (opener) {
       ...
       * Note: This is true of NFS volumes. This is Ok and should be
        ...
        if (log.isInfoEnabled()) log.info("FileLock not supported: file=" + file, ex);
      }
  ...
  public long write(final ByteBuffer data) {
    ...
    if (data == null) throw new IllegalArgumentException(ERR_BUFFER_NULL);

    if (isReadOnly()) throw new IllegalStateException(ERR_READ_ONLY);
    ...
    if (remaining == 0) throw new IllegalArgumentException(ERR_BUFFER_EMPTY);
    ...
    final int chk = useChecksums ? ChecksumUtility.threadChk.get().checksum(data) : 0;
    ...
    try {
       ...
       * Allocate address for a new record with [nbytes] of data.
        ...
        if (writeCacheService != null) {
          if (!writeCacheService.write(paddr, data, chk)) throw new AssertionError();
          wroteOnCache = true;
        }
        if (!wroteOnCache) {
   ...
   * it returns to its caller. This way {@link #writeOnDisk(ByteBuffer, long)} does not have to
   * check the extent on the disk.
   *
   * @param offset The offset into the file (NOT adjusted for the root blocks).
   * @param nbytes The #of bytes to be written at that offset.
   */
  ...
  private final void createBackingFile() {
    ...
    if (!fileOpened && temporaryStore) {
      ...
      try {
        ...
        if (log.isInfoEnabled()) log.info("Opened backing file for temporary store: " + file);

      } catch (IOException e) {

        throw new RuntimeException("Could not open temp file: file=" + file, e);
      }
    }
  }
   ...
   * #extensionLock}. Normal writes should hold the read lock. File size changes MUST hold the write
   * lock.
   *
   * @param data The data. The bytes from the current {@link ByteBuffer#position()} to the {@link
   *     ByteBuffer#limit()} will be written and the {@link ByteBuffer#position()} will be advanced
   *     to the {@link ByteBuffer#limit()} . The caller may subsequently modify the contents of the
   *     buffer without side effects (i.e., the data are copied onto the disk).
   * @param offset The offset in the user extent at which the data will be written (it is
   *     automatically adjusted for the size of the root blocks).
  ...
  private int writeOnDisk(final ByteBuffer data, final long offset) {
    ...
    assert extensionLock.getReadHoldCount() > 0 || extensionLock.getWriteHoldCount() > 0;
       ...
       * Note: There are only two places where the journal writes on the
        ...
        log.trace(
            "wrote on disk: bytes="
                ...
                + "ms; totals: write="
                + TimeUnit.NANOSECONDS.toMillis(storeCounters.get().elapsedDiskWriteNanos)
                + "ms, read="
                + TimeUnit.NANOSECONDS.toMillis(storeCounters.get().elapsedDiskReadNanos)
                + "ms");
      }
  ...
  public void truncate(final long newExtent) {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("newLength=" + cf.format(newExtent) + ", file=" + file);

      if (log.isInfoEnabled()) log.info(getCounters().toString());
   ...
   */
  @Override
  public synchronized long transferTo(final RandomAccessFile out) throws IOException {
   ...
   * difficulties for concurrent readers.
   */
  @Override
   ...
   * WriteCacheService}, passing through the operation to the disk.
   *
  ...
  public void writeRawBuffer(final IHAWriteMessage msg, final IBufferAccess b)
    ...
    if (true || log.isTraceEnabled()) {
      log.warn("Buffer, position: " + xb.position() + ", limit: " + xb.limit());
    }
     ...
     */
    final WriteCacheImpl writeCache =
        writeCacheService.newWriteCache(
            ba, useChecksums, true /* bufferHasData */, opener, msg.getFileExtent());
  ...
  public void writeOnStream(
    ...
    final FileChannelUtility.ReopenerInputStream instr =
        new FileChannelUtility.ReopenerInputStream(opener);
    try {
  ...
  @Override
  public void writeRawBuffer(HARebuildRequest req, IHAWriteMessage msg, ByteBuffer transfer)
      throws IOException {
    // expand buffer before writing on the store.
    ...
    //          throw new IllegalStateException("Invalid sequence number for rebuild, expected: " +
    // m_rebuildSequence + ", actual: " + msg.getSequence());
