 ...
 * will never observe distinct concurrent instances of the same {@link ICommitRecord}. This in turn
 * facilitates canonicalizing caches for objects loaded from that {@link ICommitRecord}.
 */
public class CommitRecordIndex extends BTree {
   ...
   * already done the lookup in the index).
   *
   * <p>The main purpose of this cache is not to speed up access to historical {@link
   * ICommitRecord}s but rather to establish a canonicalizing lookup service for {@link
   * ICommitRecord}s so that we can in turn establish a canonicalizing mapping for read-only objects
   ...
   */
  public static CommitRecordIndex create(final IRawStore store) {
    ...
    metadata.setTupleSerializer(
        new CommitRecordIndexTupleSerializer(new ASCIIKeyBuilderFactory(Bytes.SIZEOF_LONG)));
   ...
   * @param metadataId The metadata record for the index.
   */
  public CommitRecordIndex(
   ...
   * @return true iff such an {@link ICommitRecord} exists in the index with that commit timestamp
   *     (exact match(.
   */
   ...
   *
   * @param commitTime The commit time.
   * @return The {@link ICommitRecord} index or <code>null</code> iff there is no {@link
   *     ICommitTimestamp} for that commit time.
   */
  public synchronized ICommitRecord get(final long commitTime) {
    ...
    if (commitRecord != null) return commitRecord;
   ...
   * LTE search identifies the most recent commit state that not later than the start time of the
   * transaction.
   *
   * @param timestamp The given timestamp (may be negative for historical reads or positive for
   *     transaction identifiers, but MAY NOT be {@link ITx#UNISOLATED} NOR {@link
   *     ITx#READ_COMMITTED}).
   * @return The commit record -or- <code>null</code> iff there are no {@link ICommitRecord}s in the
   *     index that satisify the probe.
   * @see #get(long)
   */
  public synchronized ICommitRecord find(final long timestamp) {
   ...
   *     is strictly greater than <i>timestamp</i>.
   */
  public synchronized ICommitRecord findNext(final long timestamp) {
   ...
   *
   * @return The index of the {@link ICommitRecord} having the largest timestamp that is less than
   *     or equal to the given timestamp -or- <code>-1</code> iff there are no {@link
   *     ICommitRecord}s defined.
   */
   ...
   * @param addr The address of the {@link ICommitRecord}.
   ...
   * @exception IllegalArgumentException if <i>commitRecord</i> is <code>null</code>.
   * @exception IllegalArgumentException if there is already a {@link ICommitRecord} registered
   *     under for the {@link ICommitRecord#getTimestamp()}.
   * @exception IllegalArgumentException if <i>addr</i> is invalid.
   */
  public synchronized void add(final long commitRecordAddr, final ICommitRecord commitRecord) {

    if (commitRecord == null) throw new IllegalArgumentException();

    if (commitRecordAddr == 0L) throw new IllegalArgumentException();
    ...
    if (!super.contains(key)) {
      ...
      super.insert(key, ser.serializeEntry(new Entry(commitTime, commitRecordAddr)));
  ...
  public static class Entry {
    ...
    /** The address of the {@link ICommitRecord} whose commit timestamp is {@link #commitTime}. */
  ...
  public static class CommitRecordIndexTupleSerializer extends DefaultTupleSerializer<Long, Entry> {
     ...
     */
    public CommitRecordIndexTupleSerializer(final IKeyBuilderFactory keyBuilderFactory) {
