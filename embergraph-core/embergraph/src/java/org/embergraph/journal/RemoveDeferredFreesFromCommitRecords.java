 ...
 * opened.
 *
 * <p>This utility attempts to update the retained CommitRecords with a zero address for the
 * associated deferredFrees.
 *
 * <p>The assumption is that it is a DiskRW store.
 *
  ...
  public static void main(final String[] args) {
    ...
    try {
      { // Simplest approach is to remove the commitRecords
        ...
        final byte[] fromKey = metadata.getTupleSerializer().serializeKey(0L);
        final byte[] toKey = metadata.getTupleSerializer().serializeKey(System.currentTimeMillis());
