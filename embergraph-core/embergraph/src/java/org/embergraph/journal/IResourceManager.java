 ...
 * {@link IndexSegment}.
 *
 * <p>Note: For historical reasons there are two implementations of this interface. The {@link
 * Journal} uses an implementation that does not support {@link #overflow()} since the managed
 * "journal" is always the actual {@link Journal} reference. The {@link ResourceManager} supports
 * {@link #overflow()} and decouples the management of the journal and index segment resources and
 * the concurrency control from the journal itself.
 *
...
public interface IResourceManager extends IServiceShutdown {
   ...
   *
   * <p>Note: Index sources loaded from a historical timestamp (vs the live unisolated index view)
   ...
   *     view, {@link ITx#READ_COMMITTED}, or <code>timestamp</code> for a historical view no later
   *     than the specified timestamp.
   * @return The sources for the index view or <code>null</code> if the index was not defined as of
   *     the timestamp.
   * @see FusedView
   */
   ...
   * index partition, then it will also be the sole element of the array). Otherwise return an array
   * consisting of a single element, which is the {@link BTree}.
   *
   * @param btree A {@link BTree}.
   ...
   *     <i>timestamp</i>, including if the timestamp is a transaction identifier and the
   *     transaction is unknown or not active.
   * @exception IllegalArgumentException if <i>name</i> is <code>null</code>
   * @exception StaleLocatorException if the named index does not exist at the time the operation is
   *     executed and the {@link IResourceManager} has information which indicates that the index
   *     partition has been split, joined or moved.
   * @see IIndexStore#getIndex(String, long)
   *     <p>FIXME GIST - this only supports {@link ILocalBTreeView}. We need to also support {@link
   *     HTree}, etc. See {@link IBTreeManager#getIndexLocal(String, long)} which is the
   ...
   * new journal and the last committed state of the index on the old journal.
   *
   * <p>Note: When this method returns <code>true</code> journal references MUST NOT be presumed to
   * survive this method. In particular, the old journal MAY be closed out by this method and marked
   * as read-only henceforth.
   *
   * <p>Note: The caller MUST ensure that they have an exclusive lock on the {@link
   * WriteExecutorService} such that no task is running with write access to the {@link
   * #getLiveJournal() live journal}.
   *
   * <p>Note: The implementation MUST NOT write on the old journal - those writes will not be made
   * restart safe by the {@link WriteExecutorService} - but it MAY write on the new journal.
   *
