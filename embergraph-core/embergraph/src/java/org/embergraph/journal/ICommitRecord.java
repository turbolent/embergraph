 ...
 * Journal#getCommitRecord()} index over the commits records so that {@link Tx transactions} can
 * rapidly recover the commit record corresponding to their historical, read-only ground state.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @todo consider modifying to allow named root addresses for a small #of root names. E.g., an set
 *     of {name,addr} pairs that is either scanned as an array list or lookup using a hash table.
 */
public interface ICommitRecord {
   ...
   *
   * @todo The {@link IResourceManager} introduced overflow of journals. The commitCounter always
   *     starts out at zero on a new journal, so the tx should probably examine the {@link
