 ...
 * The address returned by {@link #handleCommit()} will be saved in the {@link ICommitRecord} under
 * the index identified by the {@link ICommitter} when it was registered.
 *
...
public interface ICommitter {
  ...
  /**
   * Flush dirty state to the store in preparation for an atomic commit and return the address from
   * which the persistence capable data structure may be reloaded.
   *
   * @param commitTime The timestamp assigned to the commit.
   * @return The address of the record from which the persistence capable data structure may be
