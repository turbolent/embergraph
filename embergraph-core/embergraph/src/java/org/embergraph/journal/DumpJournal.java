 ...
 *
 * <p>TODO GIST : Support all types of indices.
 *
...
public class DumpJournal {
   ...
   * <dl>
   *   <dt>-namespace
   *   <dd>Dump only those indices having the specified namespace prefix.
   *   <dt>-history
   *   <dd>Dump metadata for indices in all commit records (default only dumps the metadata for the
   *       indices as of the most current committed state).
   *   <dt>-indices
   *   <dd>Dump the indices (does not show the tuples by default).
   *   <dt>-pages
   *   <dd>Dump the pages of the indices and reports some information on the page size.
   *   <dt>-tuples
   *   <dd>Dump the records in the indices.
   * </dl>
   *
  ...
  public static void main(final String[] args) {
    ...
    if (args.length == 0) {
      ...
      System.err.println(
          "usage: (-namespace <namespace>|-history|-indices|-pages|-tuples|-addr <address>) <filename>+");
    ...
    for (; i < args.length; i++) {
       ...
       * Stat the file and report on its size, etc.
...

        {
          ...
          if (alternateRootBlock) properties.setProperty(Options.ALTERNATE_ROOT_BLOCK, "" + true);
        ...
        try {
          ...
          try {
            ...
            dumpJournal.dumpJournal(
                out, namespaces, dumpHistory, dumpPages, dumpIndices, showTuples);
   ...
   * @param dumpIndices Dump the indices (does not show the tuples by default).
   * @param showTuples Dump the records in the indices.
   */
  public void dumpJournal(
      final boolean dumpHistory,
      final boolean dumpPages,
      final boolean dumpIndices,
    ...
    try {
      ...
      dumpJournal(w, null /* namespaces */, dumpHistory, dumpPages, dumpIndices, showTuples);
...
//         * Start a transaction. This will bracket all index access and protect
    ...
    if (fmd != null) {
      ...
      out.println(
          "extent="
              + fmd.extent
              + "("
              + fmd.extent / Bytes.megabyte
              + "M)"
              + ", userExtent="
              + fmd.userExtent
              + "("
              + fmd.userExtent / Bytes.megabyte
              + "M)"
              + ", bytesAvailable="
              + bytesAvailable
              + "("
              + bytesAvailable / Bytes.megabyte
              + "M)"
              + ", nextOffset="
              + fmd.nextOffset);
    }

    {
       ...
       */
      {
        final ByteBuffer rootBlock0 =
            journal.getBufferStrategy().readRootBlock(true /* rootBlock0 */);

        if (rootBlock0 != null) {

          out.println(
              new RootBlockView(true /* rootBlock0 */, rootBlock0, new ChecksumUtility())
                  .toString());
        }
      }

      {
        final ByteBuffer rootBlock1 =
            journal.getBufferStrategy().readRootBlock(false /* rootBlock0 */);

        if (rootBlock1 != null) {

          out.println(
              new RootBlockView(false /* rootBlock0 */, rootBlock1, new ChecksumUtility())
                  .toString());
        }
      }
      ...
      out.println(
          "The current root block is #" + (journal.getRootBlockView().isRootBlock0() ? 0 : 1));
    }
    ...
    if (strategy instanceof RWStrategy) {
      ...
      if (validateDeleteBlocks) {
        ...
        final Set<Integer> duplicateAddrs = stats.getDuplicateAddresses();
        ...
        if (!duplicateAddrs.isEmpty()) {
          ...
          for (int latchedAddr : duplicateAddrs) {
            ...
            try {
              ...
              log.error("Could not read: latchedAddr=" + latchedAddr, ex);
              continue;
            }
            ...
            if (obj == null) {
              System.err.println("Could not decode: latchedAddr=" + latchedAddr);
              final StringBuilder sb = new StringBuilder();
              BytesUtil.printHexString(sb, BytesUtil.toHexString(b, b.length));
              System.err.println("Undecoded record:" + sb.toString());
            } else {
              System.err.println(
                  "Decoded record: latchedAddr="
                      + latchedAddr
                      + " :: class="
                      + obj.getClass()
                      + ", object="
     ...
     */
    final CommitRecordIndex commitRecordIndex = journal.getReadOnlyCommitRecordIndex();
    ...
    if (dumpHistory) {
      ...
      //                final long fromTime =
      // KeyBuilder.decodeLong(commitRecordIndex.keyAt(indexOf-3), 0/*off*/);
      //                final long toTime =
      // KeyBuilder.decodeLong(commitRecordIndex.keyAt(indexOf+3), 0/*off*/);
      //
      //                final ITupleIterator<CommitRecordIndex.Entry> itr =
      // commitRecordIndex.rangeIterator(
      ...
      while (itr.hasNext()) {
        ...
        out.print(
            "Commit Record: " + entry.commitTime + ", addr=" + journal.toString(entry.addr) + ", ");

        final ICommitRecord commitRecord = journal.getCommitRecord(entry.commitTime);

        out.println(commitRecord.toString());

        dumpNamedIndicesMetadata(out, namespaces, commitRecord, dumpPages, dumpIndices, showTuples);
      }
  ...
  private void dumpGlobalRowStore(final PrintWriter out) {
    ...
    final SparseRowStore grs = journal.getGlobalRowStore(journal.getLastCommitTime());

    {
      final Iterator<? extends ITPS> itr = grs.rangeIterator(GlobalRowStoreSchema.INSTANCE);
    ...
    {
      final Iterator<? extends ITPS> itr = grs.rangeIterator(RelationSchema.INSTANCE);
   ...
   */
  private void dumpNamedIndicesMetadata(
      final PrintWriter out,
      final List<String> namespaces,
    ...
    while (nitr.hasNext()) {
       ...
       * This is typically a tuple serializer that has a
          ...
          log.warn(
              "Could not load index: " + InnerCause.getInnerCause(t, ClassNotFoundException.class));
       ...
       */
      {
        final BaseIndexStats stats =
            ndx.dumpPages(dumpPages /* recursive */, dumpPages /* visitLeaves */);
  ...
  private String dumpRawRecord(final long addr) {
    ...
    if (journal.getBufferStrategy() instanceof IRWStrategy) {
       ...
       */
      final IStore store = ((IRWStrategy) journal.getBufferStrategy()).getStore();
      try {
    ...
    if (buf == null) throw new IllegalArgumentException("Nothing at that address");
   ...
   *
   * @param b The data.
   * @return The decoded object -or- <code>null</code> if the object could not be decoded.
   */
  private Object decodeData(final ByteBuffer buf) {
     ...
     */
    {
      ...
      final Iterator<String> nitr = journal.indexNameScan(null /* prefix */, commitTime);
      ...
      while (nitr.hasNext()) {
        ...
        final ICheckpointProtocol ndx = journal.getIndexLocal(name, commitTime);

        final IndexTypeEnum indexType = ndx.getCheckpoint().getIndexType();
          ...
          case BTree:
            {
              ...
              final org.embergraph.btree.NodeSerializer nodeSer = btree.getNodeSerializer();

              try {

                final org.embergraph.btree.data.IAbstractNodeData nodeOrLeaf =
                    nodeSer.decode(buf.duplicate());
