...
public interface ISnapshotFactory {
   ...
   * If the snapshot is to be compressed, then the best practice is to have the file name end with
   * ".gz". See the {@link CommitCounterUtility} for helper classes for dealing with commit counters
   * as fixed width strings.
   *
   * @param rbv The current root block. The snapshot will be consistent as of the commit point
   *     associated with this root block.
   * @return The file on which the snapshot will be written. If the file exists, then it must be
   *     empty. If the file is in a directory, the directory must exist and must be writable.
   * @throws IOException
