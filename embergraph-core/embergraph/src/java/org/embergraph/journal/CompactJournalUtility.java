...
public class CompactJournalUtility {
  ...
  /*
   * Compacts the source journal, writing a new journal containing only the most current committed
   * view onto the specified output file.
   *
   * @param args <i>inFile</i> <i>outFile</i>
   */
    ...
    final Journal sourceJournal;
    {
      ...
      System.out.println(
          "source: nbytes="
              + sourceJournal.size()
              + ", nindices="
              + sourceJournal.getName2Addr(sourceJournal.getLastCommitTime()).rangeCount());
    }
