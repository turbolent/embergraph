...
import org.embergraph.resources.NoSuchStoreException;
 ...
 *
 * <p>A read-committed task runs against the most recently committed view of the named index. A
 * historical read task runs against a historical view of the named index, but without guarantees of
 * transactional isolation. Concurrent readers are permitted without locking on the same index.
 *
 * <p>An unisolated task reads and writes on the "live" index. Note that only a single thread may
 * write on a {@link BTree} at a time. Therefore unisolated tasks (often referred to as writers)
 * obtain an exclusive lock on the named index(s). When more than one named index is used, the locks
 * are used to infer a partial ordering of the writers allowing as much concurrency as possible.
 * Pre-declaration of locks allows us to avoid deadlocks in the lock system.
 *
 * <p>Isolated tasks are part of a larger transaction. Transactions are started and committed using
 * an {@link ITransactionManagerService}. Transactional tasks run with full concurrency using an
 * MVCC (Multi-Version Concurrency Control) strategy. When a transaction is committed (by the {@link
 * ITransactionManagerService}) it must wait for lock(s) on the unisolated named indices on which it
 * has written before it may validate and commit.
 *
 * <p>Note: You MUST submit a distinct instance of this task each time you {@link
 * ConcurrencyManager#submit(AbstractTask)} it.
 *
...
public abstract class AbstractTask<T> implements Callable<T>, ITask<T> {
   ...
   *     the task to the concurrency manager and hence it was not processed by the lock manager.
   *     E.g., you invoked call() directly on some subclass of AbstractTask. That is a no-no, unless
   *     you are using the Journal as a personal (no concurrency) store. In that case it is Ok and
   *     some of the unit tests are written that way which is why this is turned off by default. In
   *     particular, it tends to interfere with some of the tx test suites since they were written
   *     before the concurrency API was added.
   */
   ...
   *
   * <p>Note: When the operation is an unisolated writer an exclusive lock MUST be obtain on the
   * named resources(s) before the operation may execute.
   *
   * <p>Note: this is private so that people can not mess with the resource names in {@link
   * #doTask()}.
   */
   ...
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @todo are these bits (combined with the replacement of the {@link Entry} in {@link #n2a})
   *     sufficiently flexible to allow a task to do a drop/add sequence, or a more general
   *     {add,drop}+ sequence.
   */
  private static class Entry extends Name2Addr.Entry {
   ...
   *
   * <p>There are several special cases designed to handle add and drop of named indices in
   * combination with transient flags on the {@link Entry}:
   *
   * <ol>
   *   <li>If an resource was declared by the task and there was no such index in existence when the
   *       task began to execute, then there will NOT be an {@link Entry} for the resource in this
   *       map.
   *   <li>If an existing index was dropped by the task, then then the resource will be associated
   *       with a {@link Entry} whose [droppedIndex] flag is set but the index will NOT be on the
   *       {@link #commitList}. When the task checkpoints the indices, that [dropppedIndex] flag
   *       will be used to drop the named index from the {@link ITx#UNISOLATED} {@link Name2Addr}
   *       object (and it is an error if the named index does not exist in {@link Name2Addr} at that
   *       time).
   *   <li>If an index is registered by a task, then either there MUST NOT be an entry in {@link
   *       #n2a} as of the time that the task registers the index -or- the entry MUST have the
   *       [droppedIndex] flag set.
   * </ol>
   */
   ...
   * task fails, then {@link #n2a} is simply discarded (since we have not written on the unisolated
   * {@link Name2Addr} we do not need to rollback any changes).
   */
  private void setupIndices() {
    ...
    if (name2Addr == null) {
      ...
      throw new AssertionError("Name2Addr not loaded? : " + resourceManager.getLiveJournal());
    }
    ...
    synchronized (name2Addr) {
      for (String s : resource) {
        ...
        if (tmp != null) {
           ...
           *
           * <p>Note: We do NOT fetch the indices here, just copy their last checkpoint metadata
           * from Name2Addr.
           *
           * @see <a href="http://trac.blazegraph.com/ticket/566" > Concurrent unisolated operations
           *     against multiple KBs </a>
           */
          final Iterator<String> itr = Name2Addr.indexNameScan(s, name2Addr);
   ...
   * protocol simpler since the {@link DirtyListener} has both the name of the index and the
  ...
  private class DirtyListener implements IDirtyListener {
    ...
    public void dirtyEvent(final ICheckpointProtocol btree) {
      ...
      if (commitList.put(name, this) != null) {
        ...
        if (log.isInfoEnabled()) log.info("Added index to commit list: name=" + name);
      }
    }
  }
   ...
   * all cases access to the index is delegated to this method. First, the task can use this method
   * directly. Second, the task can use {@link #getJournal()} and then use {@link
    ...
    if (timestamp == ITx.UNISOLATED) {
      ...
      synchronized (name2Addr) {
        ...
        if ((resourceManager.getLiveJournal().getBufferStrategy() instanceof IRWStrategy)) {
          ...
          final ICheckpointProtocol tmp_ndx = name2Addr.getIndexCache(name);
        ...
        if (ndx == null) {
          ...
          if (ndx instanceof AbstractBTree) {
            ((AbstractBTree) ndx).setBTreeCounters(resourceManager.getIndexCounters(name));
          } else if (ndx instanceof AbstractHTree) {
            ((AbstractHTree) ndx).setBTreeCounters(resourceManager.getIndexCounters(name));
          }
        }
      }
   ...
   *
   * <p>Note: This method is used both when registering a new index ( {@link #registerIndex(String,
   * BTree)}) and when reading an index view from the source ({@link #getIndex(String)}).
   *
  ...
  private ILocalBTreeView getUnisolatedIndexView(final String name, final BTree btree) {
    ...
    final AbstractBTree[] sources = resourceManager.getIndexSources(name, ITx.UNISOLATED, btree);
   ...
   *
   * @param name The index name.
   * @param btree The {@link BTree} that will absorb writes for the index.
   * @return The index on which writes may be made. Note that if the {@link BTree} describes an
   *     index partition with multiple sources then the returned object is a {@link FusedView} for
   *     that index partition as would be returned by {@link IResourceManager#getIndex(String,
   *     long)}.
   * @throws UnsupportedOperationException unless the task is {@link ITx#UNISOLATED}
   * @throws IndexExistsException if the index was already registered as of the time that this task
   *     began to execute.
   * @todo should allow add/drop of indices within fully isolated read-write transactions as well.
   * @see IBTreeManager#registerIndex(String, BTree)
   */
  public synchronized IIndex registerIndex(final String name, final BTree btree) {
   ...
   * @throws UnsupportedOperationException unless the task is {@link ITx#UNISOLATED}
   * @throws NoSuchIndexException if the named index is not registered as of the time that this task
   *     began to execute.
   * @see IIndexManager#dropIndex(String)
   */
  public synchronized void dropIndex(final String name) {
   ...
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @see <a href="http://trac.blazegraph.com/ticket/675" > Flush indices in parallel during
   *     checkpoint to reduce IO latency</a>
   */
  private class CheckpointIndexTask implements Callable<Void> {
    ...
    public Void call() {
      ...
      try {
        ...
        throw new RuntimeException("Could not commit index: name=" + l.name, t);
      }
   ...
   *
   * <p>This method is invoked after an {@link ITx#UNISOLATED} task has successfully completed its
   * work, but while the task still has its locks.
   *
  ...
  private long checkpointTask() {
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info("There are " + ndirty + " dirty indices " + commitList.keySet() + " : " + this);
    }
    ...
    for (final DirtyListener l : commitList.values()) {
      ...
      assert indexCache.containsKey(l.name) : "Index not in cache? name=" + l.name;
    ...
    try {
      ...
      final ExecutorService executorService = resourceManager.getLiveJournal().getExecutorService();
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info(
          "Flushed " + ndirty + " indices in " + TimeUnit.NANOSECONDS.toMillis(elapsed) + "ms");
    }
    ...
    synchronized (name2Addr) {
      for (final Entry entry : n2a.values()) {
        ...
        if (entry.droppedIndex) {
          ...
          if (log.isInfoEnabled()) log.info("Dropping index on Name2Addr: " + entry.name);
          ...
          if (l != null) {
            ...
            name2Addr.putOnCommitList(l.name, l.ndx, false /*needsCheckpoint*/);
          }
        }
      }
    }
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info("End task checkpoint after " + TimeUnit.NANOSECONDS.toMillis(elapsed) + "ms");
    }
   ...
   * @param timestamp The transaction identifier -or- {@link ITx#UNISOLATED} IFF the operation is
   *     NOT isolated by a transaction -or- <code> - timestamp </code> to read from the most recent
   *     commit point not later than the absolute value of <i>timestamp</i> (a historical read).
   * @param resource The resource on which the task will operate. E.g., the names of the index. When
   *     the task is an unisolated write task an exclusive lock will be requested on the named
   *     resource and the task will NOT run until it has obtained that lock.
   *     <p>The name may identify either a namespace or a concrete index object. If a concrete index
   *     object is discovered, only that index is isolated. Otherwise all indices having the same
   *     prefix as the namespace are isolated.
   */
  protected AbstractTask(
      final IConcurrencyManager concurrencyManager, final long timestamp, final String resource) {
   ...
   *     timestamp</code> to read from the most recent commit point not later than that timestamp.
   * @param resource The resource(s) on which the task will operate. E.g., the names of the
   *     index(s). When the task is an unisolated write task an exclusive lock will be requested on
   *     each named resource and the task will NOT run until it has obtained those lock(s).
   *     <p>The name may identify either a namespace or a concrete index object. If a concrete index
   *     object is discovered, only that index is isolated. Otherwise all indices having the same
   *     prefix as the namespace are isolated.
   */
  protected AbstractTask(
    ...
    this.concurrencyManager =
        (ConcurrencyManager)
            (concurrencyManager instanceof Journal
                ? ((Journal) concurrencyManager).getConcurrencyManager()
                : concurrencyManager);

    this.transactionManager =
        (AbstractLocalTransactionManager) concurrencyManager.getTransactionManager();
    ...
    if (isReadWriteTx) {
      ...
      if (resourceManager instanceof ResourceManager) {
        ...
        try {
          ...
          final IDistributedTransactionService txs =
              (IDistributedTransactionService) transactionManager.getTransactionService();
        ...
        if (transactionManager.getTx(timestamp) == null) {
           ...
           */
          new Tx(transactionManager, resourceManager, timestamp, timestamp /* readsOnCommitTime */);
        }
      }
   ...
   * @exception IllegalStateException if more than one resource was declared.
   */
  @Override
  public String getOnlyResource() {

    if (resource.length > 1) throw new IllegalStateException("More than one resource was declared");
  ...
  public boolean isResource(final String theRequestedResource) {
    ...
    if (theRequestedResource == null) throw new IllegalArgumentException();
    ...
    for (String theDeclaredResource : resource) {
       ...
       * since getIndex(name) will fail if the Name2Addr entry has not been buffered within the
       * [n2a] cache.
       *
       * @see <a href="http://trac.blazegraph.com/ticket/753" > HA doLocalAbort() should interrupt
       *     NSS requests and AbstractTasks </a>
       * @see <a href="http://trac.blazegraph.com/ticket/566" > Concurrent unisolated operations
       *     against multiple KBs </a>
       */
   ...
   * @exception IllegalStateException if the <i>resource</i> was not declared to the constructor.
   */
   ...
   * MUST throw an exception, such as {@link InterruptedException}, if they are interrupted. This
   * behavior allows tasks to be canceled in a timely manner.
   *
   * <p>If you ignore or fail to test {@link Thread#interrupted()} then your task CAN NOT be
   * aborted. If it is {@link Future#cancel(boolean)} with <code>false</code> then the task will run
   * to completion even though it has been cancelled (but the {@link Future} will appear to have
   * been cancelled).
   *
   * <p>If you simply <code>return</code> rather than throwing an exception then the {@link
   * WriteExecutorService} will assume that your task completed and your (partial) results will be
   * made restart-safe at the next commit!
   *
   * @return The object that will be returned by {@link #call()} iff the operation succeeds.
   * @throws Exception The exception that will be thrown by {@link #call()} iff the operation fails.
   * @exception InterruptedException This exception SHOULD be thrown if {@link Thread#interrupted()}
   *     becomes true during execution.
   */
   ...
   * <dl>
   *   <dt>taskname
   *   <dd>The name of the task as reported by {@link #getTaskName()}.
   *   <dt>timestamp
   *   <dd>The {@link #timestamp} specified to the ctor.
   *   <dt>resources
   *   <dd>The named resource(s) specified to the ctor IFF logging @ INFO or above.
   * </dl>
   */
  protected void setupLoggingContext() {
    ...
    if (log.isInfoEnabled()) MDC.put("resources", Arrays.toString(resource));
  }

  /** Clear fields set by {@link #setupLoggingContext()} from the {@link MDC} logging context. */
   ...
   * otherwise invokes abort() so that partial task executions are properly discarded. When
   * possible, the original exception is re-thrown so that we do not encapsulate the cause unless it
   * would violate our throws clause.
   *
   * <p>Commit and abort are NOT invoked for an isolated operation regardless of whether the
   * operation succeeds or fails. It is the responsibility of the "client" to commit or abort a
   * transaction as it sees fit.
   *
   * <p>Note: Exceptions that are thrown from here will be wrapped as {@link ExecutionException}s by
   * the {@link ExecutorService}. Use {@link InnerCause} to test for these exceptions.
   *
   * @throws StaleLocatorException if the task requests an index partition which has been split,
   *     joined, or moved to another data service.
   * @throws NoSuchIndexException if the task requests an index that is not registered on the data
   *     service.
   * @throws InterruptedException can be thrown if the task is interrupted, for example while
   *     awaiting a lock, if the commit group is being discarded, or if the journal is being
   *     shutdown (which will cause the executor service running the task to be shutdown and thereby
  ...
  public final T call() throws Exception {
    ...
    try {
      ...
      if (timestamp != ITx.UNISOLATED) {
        ...
        taskCounters.queuingNanoTime.addAndGet(nanoTime_finishedWork - nanoTime_submitTask);
      }
    }
  }
  ...
  private T call2() throws Exception {
    ...
    try {
      ...
      if (isReadWriteTx) {
        ...
        if (log.isInfoEnabled()) log.info("Running read-write tx: timestamp=" + timestamp);
  ...
  private T doUnisolatedReadWriteTask() throws Exception {
    ...
    // concurrencyManager.getWriteService().getLockManager();

    final Thread t = Thread.currentThread();

    if (log.isInfoEnabled()) log.info("Unisolated write task: " + this + ", thread=" + t);
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("Task Ok: class=" + this);
      ...
      if (!ran) {
        ...
        if (log.isInfoEnabled()) log.info("Task failed: class=" + this + " : " + t2);
      ...
      if (t2 instanceof Exception) throw (Exception) t2;
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  protected abstract static class DelegateTask<T> implements ITask<T> {
   ...
   * transactions.
   */
  protected static class InnerReadWriteTxServiceCallable<T> extends DelegateTask<T> {
   ...
   *
   * <p>Note: This inner class delegates the execution of the task to {@link AbstractTask#doTask()}
   * on the outer class.
   *
   * <p>Note: If there is only a single writer thread then the lock system essentially does nothing.
   * When there are multiple writer threads the lock system imposes a partial ordering on the
   * writers that ensures that writes on a given named index are single-threaded and that deadlocks
   * do not prevent tasks from progressing. If there is strong lock contention then writers will be
   * more or less serialized.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  protected static class InnerWriteServiceCallable<T> extends DelegateTask<T> {
    ...
    public T call() {
      ...
      try {
        ...
        try {
          ...
          if (verifyLocks) log.error(delegate, t); // log as an error.
          // fall through
        }
      }
    }
   ...
   * registered or dropped immediately. Those intentions are propagated to {@link Name2Addr} on a
   * task-by-task basis only when (a) this task is part of the current commit group; and (b) the
   * {@link WriteExecutorService} performs a group commit. If there is a conflict between the state
   * of {@link Name2Addr} at the time of the commit and the intentions of the tasks in the commit
   * group (for example, if two tasks try to drop the same index), then the 2nd task will be
   * interrupted. The enumeration of the intentions of tasks always begins with the task executed by
   * the thread performing the commit so that no other task's intention could cause the commit
  ...
  class IsolatedActionJournal implements IJournal {
    ...
    public IsolatedActionJournal(final AbstractJournal source) {
      ...
      resourceLocator =
          new DefaultResourceLocator(
              ...
              ((source instanceof ManagedJournal)
                  ? source.getResourceLocator()
                  : null /* DO-NOT-DELEGATE */));
      ...
      if (bufferStrategy instanceof IRWStrategy) {
        ...
        if (context == null) {
          throw new AssertionError(
              "Null context returned by " + bufferStrategy.getClass().getName());
        }
      } else {
        m_rawTx = null;
        context = null;
     ...
     */
    @Override
    public IIndex registerIndex(final String name, final IndexMetadata indexMetadata) {
     ...
     */
    @Override
    public ICheckpointProtocol getIndexLocal(final String name, final long commitTime) {
       ...
       * ClassCastException.
       *
       * @see <a href="http://trac.blazegraph.com/ticket/585" > GIST </a>
       */
    ...
    public SparseRowStore getGlobalRowStore(final long timestamp) {
      ...
      final IIndex ndx = delegate.getIndex(GlobalRowStoreHelper.GLOBAL_ROW_STORE_INDEX, timestamp);
     ...
     * TemporaryStore}, you can create an {@link ILocatableResource} on a {@link TemporaryStore} and
     * then locate it from within the {@link AbstractTask}. This has the advantage that the
     * isolation and read/write constraints of the {@link AbstractTask} will be imposed on access to
     * the {@link ILocatableResource}s.
     *
     * <p>FIXME GIST Reconsider the inner journal classes on AbstractTask. This is a heavy weight
     * mechanism for enforcing isolation for temporary stores. It would be better to have isolation
     * in the locator mechanism itself. This will especially effect scale-out query using temporary
     * stores and will break semantics when the task is isolated by a transaction rather than
     * unisolated.
     */
    @Override
     ...
     * execute until control returns from its {@link AbstractTask#doTask()} method.
     */
    @Override
    public void abort() {
      aborted = true;
    ...
    @Override
    public long commit() {
      if (aborted) throw new IllegalStateException("aborted");
      return 0;
    }
  ...
  private class ReadOnlyJournal implements IJournal {
    ...
    public IIndex getIndex(final String name, final long timestamp) {
      ...
      if (timestamp == ITx.UNISOLATED) throw new UnsupportedOperationException();
    ...
    /** Note: Not supported since this method returns the {@link ITx#UNISOLATED} index. */
    @Override
    ...
    public SparseRowStore getGlobalRowStore() {
      ...
      // last commit time.
      final long lastCommitTime = delegate.getRootBlockView().getLastCommitTime();
    ...
    public EmbergraphFileSystem getGlobalFileSystem() {
      ...
      final IIndexManager tmp =
          new DelegateIndexManager(this) {

            @Override
            public IIndex getIndex(final String name, final long timestampIsIgnored) {

              // last commit time.
              final long commitTime = delegate.getRootBlockView().getLastCommitTime();

              return delegate.getIndex(name, TimestampUtility.asHistoricalRead(commitTime));
            }
          };
     ...
     * the {@link ILocatableResource}s.
     *
     * <p>FIXME Reconsider the inner journal classes on AbstractTask. This is a heavy weight
     * mechanism for enforcing isolation for temporary stores. It would be better to have isolation
     * in the locator mechanism itself. This will especially effect scale-out query using temporary
     * stores and will break semantics when the task is isolated by a transaction rather than
     * unisolated.
