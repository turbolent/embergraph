...
public abstract class BasicBufferStrategy extends AbstractBufferStrategy {
   ...
   *
   * <p>Note: The {@link #buffer} reference is updated by {@link #truncate(long)}. Since both
   * readers and writers MUST use the current value for this variable it is marked as <code>volatile
   * </code>.
   *
   * <p>Note: Access to the buffer's internal state (its offset and limit) MUST be serialized in
   * order to prevent a concurrent modification of the buffer's internal state resulting in an
   * inconsistent view. Synchronization is on <i>this</i> (the {@link BasicBufferStrategy}
   * reference).
   */
  ...
  public synchronized void truncate(final long newExtent) {
    ...
    if (log.isInfoEnabled()) log.info("Buffer: newCapacity=" + cf.format(newCapacity));
  }
