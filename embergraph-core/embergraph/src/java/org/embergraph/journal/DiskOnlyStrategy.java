...
public class DiskOnlyStrategy extends AbstractBufferStrategy implements IDiskBasedStrategy {
  ...
  /**
   * The next offset at which data in the {@link #writeCache} will be written on the disk. The
   * offset is relative to the start of the user data space. Offset zero(0) addresses the first byte
   * after the root blocks.
   *
  ...
  private class WriteCache {
    ...
    /**
     * The buffer used to absorb writes that are destined for the disk. Writes are simply appended
     * into this buffer until it would overflow. On overflow, {@link #flushWriteCache()} is invoked
     * to flush the data to the disk (without synchronizing the disk). If a record is too large to
     * fit into this buffer, then the write cache is flushed and the record is written directly on
     * the disk.
     *
     * <p>Note: We must clone the data since the {@link IRawStore#write(ByteBuffer)} contract says
     * that the caller can reuse the buffer once we return. In order minimize heap churn we simply
     * copy the data into {@link #buf}, a {@link ByteBuffer} that buffers recently written records.
     * Writes are deferred until the buffer is would overflow and then all buffered are written at
     * once onto the disk.
     *
     * <p>In order to ensure consistency we read through the {@link #buf} in {@link #read(long)}.
     * Otherwise a {@link #write(ByteBuffer)} could return and a subsequent read on the record while
     * it is in the {@link #buf} would "miss" causing us to read through to the disk (which would
     * not have the correct data).
     *
     * <p>Note: The write cache design assumes an "append only" store. In particular, it assumes
     * that the application data records are written in are purely sequential manner on the end of
     * the file (the root blocks are outside of the application data). Either the write cache must
     * be disabled or a different design must be used if you are using a store where records may be
     * deleted and recycled.
     *
     * <p>The write cache offers a 27% performance gain when compared to the same condition without
     * the write cache as measured by {@link AbstractMRMWTestCase}.
     */
    private IBufferAccess buf;

    /**
     * An index into the write cache used for read through on the cache. The keys are the addresses
     * that would be used to read the corresponding record. The values are the position in {@link
     * #buf} where that record is buffered. A cache miss means that you need to read the record from
     * the disk.
     */
     ...
     *
     * <p>Note: {@link FileChannel} IO is fact perform using a direct {@link ByteBuffer}. When the
     * caller supplies a {@link ByteBuffer} that is allocated on the Java heap as opposed to in
     * native memory a temporary direct {@link ByteBuffer} will be allocated for the IO operation.
     * The JVM can fail to release this temporary direct {@link ByteBuffer}, resulting in a memory
     * leak. For this reason, the write cache should be a direct {@link ByteBuffer} and the same
     * direct {@link ByteBuffer} instance should be shared when overflow causes the live journal
     * overflow, being replaced by a new live journal. This bug forces us to pass in the write cache
     * {@link ByteBuffer} directly via the {@link ManagedJournal} constructor.
     *
     * @see
     *     http://bugs.sun.com/bugdatabase/view_bug.do;jsessionid=8fab76d1d4479fffffffffa5abfb09c719a30?bug_id=6210541
     * @param writeCache A {@link ByteBuffer} to be used as the write cache (optional).
     * @param capacity
     */
    public WriteCache(final IBufferAccess writeCache) {

      if (writeCache == null) throw new IllegalArgumentException();
     ...
     *
     * @param addr The address assigned to that record in the journal.
     ...
     *
     * @param addr The address assigned to that record in the journal.
     * @param nbytes The length of the record (decoded from the address by the caller).
     * @return A read-write view onto the record in the write cache buffer -or- <code>null</code>
     *     iff the record does not lie within this {@link WriteCache}.
     *     <p>Note: The caller MUST copy the data from the view since concurrent operations may
     *     result in the write cache being flushed and the view overwritten with new data.
     *     <p>Note: A read-write view is returned in order to support {@link
     *     DiskOnlyStrategy#update(long, int, ByteBuffer)} for those cases when the record to be
     *     updated in still in the {@link WriteCache}.
     */
   ...
   *
   * <p>For the DiskOnlyStrategy flushing the writeCache also ensures the backing file is created if
   * the file is temporary.
   *
   * <p>Note that the internal call to flush the writeCache must be synchronized or concurrent
   * writers to the cache will cause problems.
   */
  @Override
  public void commit() {
    if (writeCache != null) {
  ...
  /**
   * Writes the {@link #writeCache} through to the disk and its position is reset to zero.
   *
   * <p>The caller MUST be synchronized on <i>this</i>.
   */
  ...
  //                            new Instrument<Double>() {
  //                                public void sample() {
  //                                    final double diskReadSecs = (elapsedDiskReadNanos /
  // 1000000000.);
  ...
  DiskOnlyStrategy(final long maximumExtent, final FileMetadata fileMetadata) {
    ...
    if (fileMetadata.writeCacheEnabled && !fileMetadata.readOnly && fileMetadata.closeTime == 0L) {
      ...
      if (log.isInfoEnabled()) log.info("Enabling writeCache: capacity=" + tmp.buffer().capacity());
  ...
  /**
   * Note: {@link ClosedChannelException} and {@link AsynchronousCloseException} can get thrown out
   * of this method (wrapped as {@link RuntimeException}s) if a reader task is interrupted.
   */
  public ByteBuffer read(final long addr) {
    ...
    synchronized (this) {
      ...
      try {
        ...
        storeCounters.ndiskRead += FileChannelUtility.readAll(opener, dst, pos);
   ...
   *
   * <p>Note: This method is synchronized so that concurrent readers do not try to all open the
   * store at the same time.
   *
   * @todo This method is ONLY invoked by readers. It should be used for writers as well. Note that
   *     this method WILL NOT be invoked by {@link FileChannelUtility} if the channel was closed by
   *     an interrupt in the current thread (a different exception is thrown).
   */
  ...
  private long allocate(final int nbytes) {
    ...
    if (nbytes <= 0) throw new IllegalArgumentException(ERR_BAD_RECORD_SIZE);
  ...
  //                        storeCounters.elapsedCacheWriteNanos += (System.nanoTime() -
  // beginCache);
  //
  //                    }
  //
  ...
  public long write(final ByteBuffer data) {
    ...
    if (nbytes == 0) throw new IllegalArgumentException(ERR_BUFFER_EMPTY);
   ...
   * <i>offset</i> bytes into the file.
   *
   * <p>Note: The caller MUST be synchronized on <i>this</i>.
   *
   ...
   * <p>Note: The caller MUST be synchronized on <i>this</i>.
   *
   * <p>Note: This updates {@link #writeCacheOffset} as well (but only if the write is an append).
   *
   * <p>Note: It is possible for {@link #update(long, int, ByteBuffer)} to force a non-append write
   * that is beyond the {@link #writeCacheOffset}. This will occur if the record that is being
   * updated is too large for the {@link #writeCache} while there are also records buffered by this
   * write cache.
   *
   ...
   * @param offset The offset in the file at which the data will be written.
   * @param append <code>true</code> iff the write is an append (most record writes are appends).
   * @todo When integrating the new WriteCache, this method will still have to make sure that the
   *     backing file exists and handle overflow of the file (file extension).
   */
    ...
    if (false && EmbergraphStatics.debug) {
      ...
      System.err.println(
          "wrote on disk: bytes="
              ...
              + "ms; totals: write="
              + TimeUnit.NANOSECONDS.toMillis(storeCounters.elapsedDiskWriteNanos)
              + "ms, read="
              + TimeUnit.NANOSECONDS.toMillis(storeCounters.elapsedDiskReadNanos)
              + "ms");
    }
  }
