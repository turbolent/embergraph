...
public interface IHABufferStrategy extends IBufferStrategy {
   ...
   * @param msg The {@link IHAWriteMessage}.
   * @param b The raw buffer. Bytes from position to limit will be sent. remaining() must equal
   *     {@link IHAWriteMessage#getSize()}.
   ...
   *     service.
   * @param sequence The sequence of this {@link IHAWriteMessage} (origin ZERO (0)).
   * @param quorumToken The quorum token of the leader, which must remain valid across the rebuild
   *     protocol.
   * @param fileExtent The file extent as of the moment that the leader begins to replicate the
   *     existing backing file.
   * @param offset The starting offset (relative to the root blocks).
   * @param nbytes The #of bytes to be sent.
   * @param b The raw buffer. The buffer will be cleared and filled with the specified data, then
   *     sent down the write pipeline.
   * @return The {@link Future} for that request.
   * @throws IOException
   * @throws InterruptedException
   */
  Future<Void> sendRawBuffer(
      IHARebuildRequest req,
      // long commitCounter,
      // long commitTime,
      long sequence,
      long quorumToken,
   ...
   * for HA.
   *
   * <p>Note: This method is used when the root blocks of the leader are installed onto a follower.
   * This can change the {@link UUID} for the backing store file. The {@link IHABufferStrategy}
   * implementation MUST update any cached value for that {@link UUID}.
   *
   * <p>Use {@link #postHACommit(IRootBlockView)} rather than this method in the 2-phase commit on
   * the follower.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/662" > Latency on followers
   *     during commit on leader </a>
   */
   ...
   * the follower with the delta in the allocators from the write set associated with that commit.
   *
   * @param rootBlock The newly installed root block.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/662" > Latency on followers
   *     during commit on leader </a>
   */
   ...
   *
   * <p>Note: This DOES NOT reflect the current value of the block sequence counter for ongoing
   * writes. That counter is owned by the {@link WriteCacheService}.
   *
   * @see WriteCacheService#resetSequence()
   * @see #getCurrentBlockSequence()
   *     <p>FIXME I would prefer to expose the {@link WriteCacheService} to the {@link
   *     AbstractJournal} and let it directly invoke {@link WriteCacheService#resetSequence()}. The
   *     current pattern requires the {@link IHABufferStrategy} implementations to track the
   ...
   *     snapshot is a NOP for the {@link IBufferStrategy} (e.g., for the WORM).
   */
   ...
   * etc).
   *
   * <p>Note: The digest is not reliable unless you either use a snapshot or suspend writes (on the
   * quorum) while it is computed.
   *
   * @param snapshot The allocator snapshot (optional). When given, the digest is computed only for
   *     the snapshot. When <code>null</code> it is computed for the entire file.
   ...
   * themselves.
   *
   * <p>Note: The caller is able to obtain both root blocks atomically, while the strategy may not
   * be aware of the root blocks or may not be able to coordinate their atomic capture.
   *
   * <p>Note: The caller must ensure that the resulting snapshot will be consistent either by
   * ensuring that no writes occur or by taking a read-lock that will prevent overwrites of
   * committed state during this operation.
   *
   * @param os Where to write the data.
   * @param quorum The {@link Quorum}.
   * @param token The token that must remain valid during this operation.
   * @throws IOException
   * @throws QuorumException if the service is not joined with the met quorum for that token at any
   *     point during the operation.
   */
  void writeOnStream(
      OutputStream os,
      ISnapshotData coreData,
