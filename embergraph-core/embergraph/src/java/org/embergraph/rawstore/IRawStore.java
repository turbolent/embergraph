 ...
 *
 * <p>The {@link AbstractJournal} is the principal implementation of this interface and provides
 * both transient and durable options and the facilities for atomic commit. {@link BTree} provides a
 * higher level interface for operations on an {@link IRawStore} and uses a copy-on-write policy
 * designed to support transactional semantics by making it possible to read from a consistent
 * historical state. The {@link AbstractJournal} provides the necessary mechanisms to support
 * transactions based on the copy-on-write semantics of the {@link BTree}.
 *
 * <p>The {@link IRawStore} supports write and read back on immutable addresses and does NOT
 * directly support update of data for an immutable address once that data has been written.
 * Applications seeking Create, Read, Update, Delete (CRUD) semantics should use a {@link BTree} to
 * store their application data. Unlike this interface, the {@link BTree} can correctly support
 * update and delete of application data using a persistent identifier (the key).
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @see AbstractJournal
 * @see BTree
 *     <p>FIXME This should be restated in terms of {@link IByteArrayBuffer}. The use of the {@link
 *     ByteBuffer} in this API has become limiting. It would be nicer to specify a byte[] with
 *     optional offset and length parameters. This would make it easier to wrap the byte[] in
 *     different ways for reading and writing. The {@link ByteBuffer} has an advantage when reading
 *     since it allows us to return a read-only view of the record, but in practice that is only
 *     effective for an in-memory store (with or without a backing file). The byte[] is more
 *     flexible. The caller is free to reuse the byte[] and the store always copies the data. In
 *     practice, we tend to work around this by copying to an array backed {@link ByteBuffer} or to
 *     a byte[] wrapped as an {@link IByteArrayBuffer} .
 */
public interface IRawStore extends IAddressManager, ICounterSetAccess, IStreamStore {
   ...
   *     buffer without changing the state of the store (i.e., the data are copied into the store).
   * @return A long integer formed that encodes both the offset from which the data may be read and
   *     the #of bytes to be read. See {@link IAddressManager}.
   * @throws IllegalArgumentException if <i>data</i> is <code>null</code>.
   * @throws IllegalArgumentException if <i>data</i> has zero bytes {@link ByteBuffer#remaining()}.
   * @throws IllegalStateException if the store is not open.
   * @throws IllegalStateException if the store does not allow writes.
   * @todo define exception if the maximum extent would be exceeded.
   * @todo the addresses need to reflect the ascending offset at which the data are written, at
   *     least for a class of append only store. some stores, such as the Journal, also have an
...
//     *
  ...
  //     * @param oldAddr as returned from a previous write of the same object, or zero if a new
  // write
  //     *
   ...
   * on the ability to read at that address.
   *
   * @param addr A long integer formed using {@link Addr} that encodes both the offset at which the
   *     data was written and the #of bytes that were written.
   * @exception IllegalArgumentException If the address is known to be invalid (never written or
   *     deleted). Note that the address 0L is always invalid.
   *     <p>It is only applicable in the context of a garbage collection strategy. With an append
   *     only store and with eviction of btrees into index segments there is no reason to delete
   *     anything on the store - and nothing to keep track of the delete.
   ...
   *     the #of bytes to be read. See {@link IAddressManager#toAddr(int, long)}.
   * @return The data read. The buffer will be flipped to prepare for reading (the position will be
   *     zero and the limit will be the #of bytes read).
   ...
   *
   * <p>Note: This does not guarantee that the OS will not swap the buffer onto disk.
   *
   * @exception IllegalStateException if the store is not open.
   */
