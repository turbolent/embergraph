 ...
 * @version $Id$
 * @param <E> The generic type of the elements in the relation.
 */
public interface IKeyOrder<E> {
   ...
   * @param element An element for the associated relation.
   * @return The key for the index associated with this {@link IKeyOrder}.
   * @todo DDL: This does not really let you handle something which does not implement {@link
   *     IElement} without overriding {@link #getKey(IKeyBuilder, Object)}.
   *     <p>We should have named based access to the "elements" of a relation based on a schema
   *     (DDL) but also support schema flexible access for GOM. (RDF is a fixed schema in this sense
   *     with {s,p,o,c} properties).
   *     <p>We will also need to support name-based {@link IPredicate}s in order to work with GOM.
   */
   ...
   *
   * @param keyBuilder The object which will be used to construct the key.
   * @param predicate A predicate describing bound and unbound fields for the key.
   * @return The key corresponding to the inclusive lower bound for a query against that {@link
   *     IPredicate}.
   */
   ...
   * @param keyBuilder The object which will be used to construct the key.
   * @param predicate A predicate describing bound and unbound fields for the key.
   * @return The key corresponding to the exclusive upper bound for a query against that {@link
   *     IPredicate}.
   */
