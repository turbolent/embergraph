 ...
 *
 * @version $Id$
 * @param <E> The generic type of the source elements.
 * @param <F> The generic type of the converted elements.
 * @see IChunkConverter
...
public class ChunkedConvertingIterator<E, F> implements IChunkedOrderedIterator<F> {
   ...
   * @param converter The chunk-at-a-time converter.
   */
  public ChunkedConvertingIterator(
      final IChunkedOrderedIterator<E> src, final IChunkConverter<E, F> converter) {
   ...
   * @param src The source iterator.
   * @param converter The chunk-at-a-time converter.
   * @param keyOrder The {@link IKeyOrder} for the converted chunks -or- <code>null</code> iff not
   *     known.
   */
  public ChunkedConvertingIterator(
   ...
   *
   * @param src A chunk of elements from the source iterator.
  ...
  protected F[] convert(final IChunkedOrderedIterator<E> src) {
    ...
    if (tmp == null) throw new AssertionError("Converter returns null: " + converter.getClass());
  ...
  public F next() {
    ...
    if (log.isInfoEnabled()) log.info("returning converted[" + pos + "]");
      ...
      final F[] chunk =
          (F[])
              java.lang.reflect.Array.newInstance(
                  ...
                  converted.getClass().getComponentType(), remaining);
