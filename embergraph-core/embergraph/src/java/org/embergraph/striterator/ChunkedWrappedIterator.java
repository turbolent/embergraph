...
public class ChunkedWrappedIterator<E> implements IChunkedOrderedIterator<E> {
  ...
  public ChunkedWrappedIterator(
      final Iterator<E> src, final int chunkSize, Class<? extends E> elementClass) {
   ...
   * @param elementClass The class for the array component type (optional, but the default will use
   *     the runtime type of the first element of the array which can be too restrictive).
   * @param keyOrder The order in which the elements will be visited by the source iterator if known
   *     and <code>null</code> otherwise.
   * @param filter Optional filter. When non-<code>null</code> only elements accepted by the filter
  ...
  public void close() {
    ...
    if (open) {
      ...
      if (log.isInfoEnabled()) log.info("#chunks=" + nchunks + ", #elements=" + nelements);
    }
  }
  ...
  /** The next chunk of elements in whatever order the were visited by {@link #next()}. */
  @Override
  @SuppressWarnings("unchecked")
  public E[] nextChunk() {
        ...
        chunk =
            (E[])
                java.lang.reflect.Array.newInstance(
                    (elementClass != null ? elementClass : t.getClass()), chunkSize);
      }
    ...
    if (n != chunkSize) {
      ...
      final E[] tmp =
          (E[])
              java.lang.reflect.Array.newInstance(
                  (elementClass != null ? elementClass : chunk[0].getClass()), n);
    ...
    if (log.isDebugEnabled())
      log.debug("#chunks=" + nchunks + ", chunkSize=" + chunk.length + ", #elements=" + nelements);
