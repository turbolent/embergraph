...
public class Striterator<I extends Iterator<E>, E> implements IStriterator<I, E> {
   ...
   * advance the aggregate iterator efficiently to the next chunk without re-scanning tuples.
   *
   ...
   * the iterator.</strong>
   *
   * @param srcEnum The source enumeration.
   */
   ...
   * order to ensure that the current iterator element is removed from the iteration source. In the
   * general case, it may not be possible to implement {@link #remove()} for striterator patterns.
   *
   * <p>When the striterator is also an {@link IChunkedIterator} then {@link #remove()} semantics
   * face an additional difficulty. If the source iterator is being consumed a {@link
   * IChunkedIterator#nextChunk() chunk} at a time, then {@link #remove()} only has the semantics of
   * removing the last element in the chunk.
   *
   * @throws UnsupportedOperationException For the above reasons, {@link #remove()} is NOT supported
   *     for chunked striterator patterns and this method will always throw an {@link
