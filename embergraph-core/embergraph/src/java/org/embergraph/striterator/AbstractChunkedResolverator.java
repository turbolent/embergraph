 ...
 * @param <F> The generic type of the elements visited by this iterator (the resolved elements).
 * @param <S> The generic type of the application state object.
 */
public abstract class AbstractChunkedResolverator<E, F, S> implements ICloseableIterator<F> {
  ...
  /*
   * @param state Application specified state (optional).
   * @param src The source iterator (will be closed or exhausted when this iterator is closed).
   * @param buffer The {@link BlockingBuffer} on which the resolved chunks will be written by the
   *     {@link ChunkConsumerTask} and from which this iterator will read those chunks.
   */
  public AbstractChunkedResolverator(
      final S state, final IChunkedOrderedIterator<E> src, final BlockingBuffer<F[]> buffer) {
  ...
  /*
   * Creates and runs a task that will consume chunks from the source iterator, resolve the elements
   * in each chunk, and place the resolved chunks onto the {@link BlockingBuffer}.
   *
   * <p><strong>You MUST invoke this before consuming results from the iterator.</strong>
   *
   * <p>Implementations SHOULD strengthen the return type.
   *
   * @param service The service on which the task will be executed.
   */
  // Note: Synchronized for atomic decision making and publication of [resolvedItr].
  public synchronized AbstractChunkedResolverator<E, F, S> start(final ExecutorService service) {

    if (resolvedItr != null) throw new IllegalStateException();
  ...
  /*
   * Iterator draining chunks of resolved elements from the {@link #buffer}. This is <code>null
   * </code> until {@link #start(ExecutorService)} is invoked.
   */
  private IAsynchronousIterator<F[]> resolvedItr;

  /*
   * Consumes chunks from the source iterator, placing the converted chunks on a queue.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
    ...
    public Long call() throws Exception {
      ...
      try {
        ...
        while (buffer.isOpen() && src.hasNext()) {
           ...
           *     collection of triple patterns </a>
           */
          ...
          if (log.isDebugEnabled()) log.debug("nchunks=" + nchunks + ", chunkSize=" + chunk.length);
        }
   ...
   *
   * <p>Note: This method is invoked by the {@link ChunkConsumerTask} which runs asynchronously.
   *
   * @param chunk The next chunk from the source iterator.
   * @return The resolved chunk.
   */
  protected abstract F[] resolveChunk(final E[] chunk);

  /** @throws IllegalStateException unless {@link #start(ExecutorService)} has been invoked. */
  @Override
  ...
  public F next() {
    ...
    if (lastIndex == -1 || chunk != null && lastIndex + 1 == chunk.length) {
      ...
      if (log.isDebugEnabled())
        log.debug("nextChunk ready: size=" + chunk.length + ", chunk=" + Arrays.toString(chunk));
    }
    ...
    if (log.isDebugEnabled())
      log.debug("lastIndex=" + lastIndex + ", chunk.length=" + chunk.length + ", visting=" + f);
