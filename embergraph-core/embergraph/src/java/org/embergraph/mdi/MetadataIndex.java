 ...
 *
 * <p>Note: At this time the recommended scale-out approach for the metadata index is to place the
 * metadata indices on a {@link MetadataService} (the same {@link MetadataService} may be used for
 * an arbitrary #of scale-out indices) and to <em>replicate</em> the state for the {@link
 * MetadataService} onto failover {@link MetadataService}s. Since the {@link MetadataIndex} may grow
 * without bound, you simply need to have enough disk on hand for it (the size requirements are
 * quite modest). Further, the {@link MetadataService} MUST NOT be used to hold the data for the
 * scale-out indices themselves since the {@link MetadataIndex} can not undergo {@link
 * IResourceManager#overflow()}.
 *
 * <p>One advantage of this approach is that the {@link MetadataIndex} is guaranteed to hold all
 * historical states of the partition definitions for each index - effectively it is an immortal
 * store for the partition metadata. On the other hand it is not possible to compact the metadata
 * index without taking the database offline.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @todo The {@link MetadataIndex} does NOT support either overflow (it may NOT be a {@link
 *     FusedView}) NOR key-range splits. There are several issues involved:
 *     <p>(a) How to track the next partition identifier to be assigned to an index partition for
 *     the managed index. Currently this value is written in the {@link MetadataIndexCheckpoint}
 *     record and is propagated to the new backing store on overflow. However, if the metadata index
 *     is split into partitions then additional care MUST be taken to use only the value of that
 *     field on the 'meta-meta' index.
 *     <p>(b) how to locate the partitions of the metadata index itself.
 * @todo one way to locate the metadata-index partitions is to hash partition the metadata index and
 *     range queries can be flooded to all partitions. the #of metadata service nodes can be changed
 *     by a suitable broadcast event in which clients have to change to the new hash basis. this
 *     feature can be generalized to provide hash partitioned indices as well as key-range
 *     partitioned indices.
 * @todo A metadata index can be recovered by a distributed process running over the data services.
 *     Each data service reports all index partitions. The reports are collected and the index is
 *     rebuilt from the reports. Much like a map/reduce job.
 */
public class MetadataIndex extends BTree implements IMetadataIndex {
   ...
   * committed.
   *
   * <p>Note: The metadata index uses a 32-bit partition identifier rather than the {@link
   * #getCounter()}. The reason is that the {@link Counter} uses the partition identifier in the
   * high word and a partition local counter in the low word. Therefore we have to centralize the
   * assignment of the partition identifier, even when the metadata index is itself split into
   * partitions. Requests for partition identifiers need to be directed to the root partition (L0)
   * for the {@link MetadataIndex}.
   */
   ...
   * @param indexUUID The unique identifier for the metadata index.
   * @param managedIndexMetadata The metadata template for the managed scale-out index.
   */
  public static MetadataIndex create(
      IRawStore store, UUID indexUUID, IndexMetadata managedIndexMetadata) {

    final MetadataIndexMetadata metadata =
        new MetadataIndexMetadata(managedIndexMetadata.getName(), indexUUID, managedIndexMetadata);
  ...
  public static class MetadataIndexMetadata extends IndexMetadata implements Externalizable {
     ...
     * @param indexUUID The UUID of the metadata index.
     * @param managedIndexMetadata The metadata template for the managed index.
     */
    public MetadataIndexMetadata(
        String managedIndexName, UUID indexUUID, IndexMetadata managedIndexMetadata) {
  ...
  public static class PartitionLocatorTupleSerializer
      extends DefaultTupleSerializer<byte[] /*key*/, PartitionLocator /*val*/> {
