...
public interface IResourceMetadata extends Serializable, Cloneable {
   ...
   * directory).
   *
   * <p>Note: This property is primarily used for debugging. It is NOT used by the {@link
   ...
   * indicates that no specific commit time is indicated. For historical journals, this implies a
   * read from the lastCommitTime on the journal in order to constitute the view. For the current
   * journal, this implies a read at whatever timestamp the caller desires.
   *
   * @see IndexManager
   */
  long getCommitTime();
