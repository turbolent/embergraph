...
public class DumpIndexSegment {
  ...
  public static void usage() {
    ...
    System.err.println(
        "usage: " + DumpIndexSegment.class.getSimpleName() + " [options] " + " file(s)");
   ...
   * turn up the logging level in order to see the output. However, when true they will apply a
   * variety of validation tests to the nodes and leaves regardless of whether their state is
   * written onto the console.
   *
   * @param args usage <code>[file|-d level|-nodeState|-leafState]+</code>, where
   *     <dl>
   *       <dt>file
   *           <dl>
   ...
   *       <dt>level
   *           <dl>
   *             is the name of the {@link Level} to be used for the {@link AbstractBTree#dumpLog}
   *       </dl>
   *       <dt>-nodeState
   *           <dl>
  ...
  static void dumpIndexSegment(
      final File file, final boolean dumpNodeState, final boolean dumpLeafState)
      throws IOException {
  ...
  static void dumpHeaders(final IndexSegmentStore store) throws IOException {
    ...
    System.out.println(
        "bloomFilter : "
            + (store.getCheckpoint().addrBloom != IRawStore.NULL
                ? store.readBloomFilter().toString()
                : "N/A"));
  }
  ...
  static void dumpNodes(
      final IndexSegmentStore store, final Node node, final boolean dumpNodeState) {

    if (dumpNodeState) node.dump(System.out);
   ...
   */
  static AbstractNode<?> decode(final AbstractBTree btree, final long addr, final ByteBuffer buf) {
   ...
   * @param store
   */
  static void dumpLeavesReverseScan(final IndexSegmentStore store, final boolean dumpLeafState) {
    ...
    {
      final long addr2 = getLastLeafAddr(store, store.getCheckpoint().addrRoot);

      if (addr != addr2) {

        log.error(
            "Last leaf address is inconsistent? checkpoint reports: "
                + addr
                + " ("
                + store.toString(addr)
                + ")"
    ...
    while (true) {
      ...
      if (priorAddr == -1L) {
        ...
        log.error(
            "Expecting the prior address to be known - aborting scan: current addr="
                + addr
                + " ("
                + store.toString(addr)
                + ")");
  ...
  static void dumpLeavesMultiBlockForwardScan(final IndexSegmentStore store) {
    ...
    System.out.println(
        "Visited " + nscanned + " tuples using multi-block forward scan in " + elapsed + " ms");
  }
