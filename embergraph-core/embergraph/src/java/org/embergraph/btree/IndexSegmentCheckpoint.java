 ...
 * root block). Since the file format is immutable it is ok to have what is essentially only a
 * single root block. If the timestamps do not agree then the build was not successfully completed.
 *
 * <p>Similar to the {@link BTree}'s {@link Checkpoint} record, this record contains only data that
 * pertains specifically to the {@link IndexSegment} checkpoint or data otherwise required to
 * bootstrap the load of the {@link IndexSegment} from the file. General purpose metadata is stored
 * in the {@link IndexMetadata} record.
 *
...
public class IndexSegmentCheckpoint implements ICheckpoint {
   ...
   * its original value of 256.
   */
  static final int SIZEOF_UNUSED_VERSION0 = 256;

  static final int SIZEOF_UNUSED_VERSION1 =
      SIZEOF_UNUSED_VERSION0
          - (Bytes.SIZEOF_BYTE // useChecksums
          );
  static final int SIZEOF_UNUSED_VERSION2 =
      SIZEOF_UNUSED_VERSION1
          - (Bytes.SIZEOF_LONG // int64 value for [entries]
          );

  /** The #of bytes required by the current {@link IndexSegmentCheckpoint} record format. */
  static final int SIZE =
      SIZEOF_MAGIC
          ...
          + // segment UUID.
          SIZEOF_OFFSET_BITS
          + // #of bits used to represent a byte offset.
          SIZEOF_COUNTS * 4
          + // height, #leaves, #nodes, (nentries:int32 now unused)
          SIZEOF_NBYTES
          + // max record length
          Bytes.SIZEOF_LONG * 6
          + // {offset,extent} tuples for the {leaves, nodes, blobs} regions.
          SIZEOF_ADDR * 5
          + // address of the {root node/leaf, indexMetadata, bloomFilter, {first,last}Leaf}.
          Bytes.SIZEOF_LONG
          + // file size in bytes.
          Bytes.SIZEOF_BYTE
          + // compactingMerge flag (0 | 1).
          Bytes.SIZEOF_BYTE
          + // useChecksums flag (0 | 1) [VERSION1]
          Bytes.SIZEOF_LONG
          + // nentries:int64 as of VERSION2
          SIZEOF_UNUSED_VERSION2
          + // available bytes for future versions.
          SIZEOF_CHECKSUM
          + // the checksum for the proceeding bytes in the checkpoint record.
          Bytes.SIZEOF_LONG // timestamp1
      ;

  /** Magic value written at the start of the {@link IndexSegmentCheckpoint} record. */
  public static final transient int MAGIC = 0x87ab34f5;
   ...
   *
   * <dl>
   *   <dt>{@link #useChecksums}
   *   <dd><code>true</code> iff record level checksums are enabled. The default for earlier
   *       versions is <code>false</code>, which provides backward compatibility for existing {@link
   *       IndexSegment} files.
   * </dl>
   */
   ...
   * value.
   */
  public static final transient int VERSION2 = 0x2;
   ...
   *
   * <p>Note: {@link IndexSegmentBuilder} is restricted to MAX_INT leaves in its build plan and
   * there are always more leaves than nodes in a BTree so this is also an int32 value.
   */
   ...
   * after the {@link IndexSegmentCheckpoint} record.
   */
   ...
   * when attempting a bottom up reconstruction of a scale-out index from its components on various
   * journals and {@link IndexSegmentStore}s.
   */
   ...
   * the many commit points that are typically available on a {@link Journal}.
   *
   * <p>Note: This field is written at the head and tail of the {@link IndexSegmentCheckpoint}
   * record. If the timestamps on that record do not agree then the build operation probably failed
   * while writing the checkpoint record.
   */
   ...
   * @throws RootBlockException if the {@link IndexSegmentCheckpoint} record is invalid (it doubles
   *     as a root block), including if the total file length is not large enough to contain an
   *     valid {@link IndexSegmentCheckpoint} record.
   */
  public IndexSegmentCheckpoint(final RandomAccessFile raf) throws IOException {
    ...
    final int checksum =
        new ChecksumUtility().checksum(buf, 0, SIZE - (SIZEOF_CHECKSUM + SIZEOF_TIMESTAMP));

    if (checksum != this.checksum) {

      throw new RootBlockException(
          "Bad checksum: expected=" + this.checksum + ", but actual=" + checksum);
    }
    ...
    if (length != raf.length()) {
      ...
      throw new RootBlockException(
          "Length differs: actual=" + raf.length() + ", expected=" + length);
    }
  ...
  public void validate() {
    ...
    //        assert height >= 0;
    if (height < 0) throw new RootBlockException("height=" + height);

    // must be non-negative.
    if (nentries < 0) throw new RootBlockException("nentries=" + nentries);
    ...
    //        } else {
    {
      if (nleaves <= 0) throw new RootBlockException("nleaves=" + nleaves);

      // zero nodes is Ok - the B+Tree may be just a root leaf.
      if (nnodes < 0) throw new RootBlockException("nnodes=" + nnodes);
      ...
      if (maxNodeOrLeafLength <= 0)
        throw new RootBlockException("maxNodeOrLeafLength=" + maxNodeOrLeafLength);
      ...
      // validate addrLeaves
      {
        ...
        if (offsetLeaves != SIZE) {
          ...
          throw new RootBlockException("offsetLeaves=" + offsetLeaves + ", but expecting " + SIZE);
        }
        ...
        if (addrFirstLeaf == 0L) throw new RootBlockException("No address for the first leaf?");

        if (addrLastLeaf == 0L) throw new RootBlockException("No address for the first leaf?");
      }
       ...
       * The root is a node.
        ...
        if (offsetNodes == 0L || extentNodes == 0L) {
          ...
          throw new RootBlockException(
              "nodes={extent=" + extentNodes + ", offset=" + offsetNodes + "}");
        }
  ...
  private ByteBuffer createView() {
    ...
    if (currentVersion < VERSION2) {
      if (nentries > Integer.MAX_VALUE) throw new RuntimeException();
    ...
    checksum = new ChecksumUtility().checksum(buf, 0, SIZE - (SIZEOF_CHECKSUM + SIZEOF_TIMESTAMP));
    ...
    assert buf.position() == SIZE
        : "position=" + buf.position() + " but checkpoint record should be " + SIZE + " bytes";
   ...
   * address from which it can be re-read.
   *
   * @return <code>0L</code>
   */
  @Override
