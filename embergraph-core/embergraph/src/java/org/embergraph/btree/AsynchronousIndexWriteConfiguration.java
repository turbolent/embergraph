 ...
 * @see AbstractSubtask
 * @todo rename since not specific to asynchronous index writes, even if normally used in that
 *     context.
 */
public class AsynchronousIndexWriteConfiguration implements Externalizable {
   ...
   * the the sink will eventually decide that it is idle, will flush any buffered writes, and will
   * close itself.
   *
   * <p>If the idle timeout is LT the {@link #getSinkChunkTimeoutNanos()} then a sink will remain
   * open as long as new chunks appear and are combined within idle timeout, otherwise the sink will
   * decide that it is idle and will flush its last chunk and close itself. If the idle timeout is
   * {@link Long#MAX_VALUE} then the sink will identify itself as idle and will only be closed if
   * the master is closed or the sink has received a {@link StaleLocatorException} for the index
   ...
   *     index partition.
   * @param masterChunkSize The desired size of the chunks that the master will process.
   * @param masterChunkTimeoutNanos The time in nanoseconds that the master will combine smaller
   ...
   *     Any buffered writes are flushed when the sink is closed. The idle timeout is reset (a) if a
   *     chunk is available to be drained by the sink; or (b) if a chunk is drained from the sink.
   *     If no chunks become available the the sink will eventually decide that it is idle, will
   *     flush any buffered writes, and will close itself.
   *     <p>If the idle timeout is LT the {@link #getSinkChunkTimeoutNanos()} then a sink will
   *     remain open as long as new chunks appear and are combined within idle timeout, otherwise
   *     the sink will decide that it is idle and will flush its last chunk and close itself. If
   *     this is {@link Long#MAX_VALUE} then the sink will identify itself as idle and will only be
   *     closed if the master is closed or the sink has received a {@link StaleLocatorException} for
   *     the index partition on which the sink is writing.
  ...
  public AsynchronousIndexWriteConfiguration(
    ...
    if (masterQueueCapacity <= 0) throw new IllegalArgumentException();

    if (masterChunkSize <= 0) throw new IllegalArgumentException();

    if (masterChunkTimeoutNanos <= 0) throw new IllegalArgumentException();
