...
public abstract class AbstractTuple<E> implements ITuple<E> {
   ...
   * {@link #kbuf} and the application can either examine the data in the {@link #kbuf} or copy it
  ...
  public final DataInputBuffer getKeyStream() {
    ...
    if (keyStream == null) {
      ...
      keyStream = new DataInputBuffer(kbuf.array(), 0 /* offset */, kbuf.limit() /* len */);
  ...
  public final DataInputBuffer getValueStream() {
    ...
    if (versionDeleted) throw new UnsupportedOperationException();
  ...
  public final byte[] getValue() {
    ...
    if (versionDeleted || isNull) return null;
   ...
   *     context of a scale-out index, the state must include enough information to locate the data
   *     service for the index view and then the source index within that view.
   *     <p>Consider the notion of leases for the read back from a data service. If you do not read
   *     on the block soon enough the lease is gone and you will get an error if you try to read on
   *     the block. The lease can be used to limit the time that an index partition is locked to a
   *     data service.
   */
   ...
   * @param leaf The leaf. If a raw record must be materialized, it will be read from the backing
   *     store for the {@link Leaf}.
   * @todo The various copy methods should also set the [sourceIndex] property and {@link
   *     ITuple#getSourceIndex()} should be implemented by this class (or maybe add a
   *     setSourceIndex() to be more flexible).
   */
  public void copy(final int index, final ILeafData leaf) {
     ...
     */
    versionTimestamp = leaf.hasVersionTimestamps() ? leaf.getVersionTimestamp(index) : 0L;
        ...
        if (!isNull) {
          ...
          if (leaf.hasRawRecords()) {
            ...
            if (addr == IRawStore.NULL) {
              ...
              final ByteBuffer tmp = ((IRawRecordAccess) leaf).readRawRecord(addr);
  ...
  public void copyTuple(
      final byte[] key, final byte[] val, final boolean deleted, final long timestamp) {
  ...
  /** Clears the buffered data copied into the {@link AbstractTuple} from the source btree. */
  protected void clear() {
    ...
    if (kbuf != null) this.kbuf.reset();

    if (vbuf != null) this.vbuf.reset();
   ...
   * value, which will, of course, fail. The fix for this problem is to specify the {@link
   * NOPTupleSerializer} for the index. This problem may still show up in some of the test suites
   * when the trace level is turned up since many of the tests were written before the {@link
