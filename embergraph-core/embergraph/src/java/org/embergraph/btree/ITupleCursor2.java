 ...
 * request. Likewise {@link #tuple()} will return <code>null</code> if the cursor position does not
 * correspond to a tuple in the index or if the tuple at that cursor position has since been
 * deleted.
 *
...
public interface ITupleCursor2<E> extends ITupleCursor<E> {
   ...
   *
   * <p>Note: When delete markers are enabled in the index and a tuple is deleted, the tuple is NOT
   * removed from the index. Instead a "delete" marker is set and the value associated with the key
   * is cleared to <code>null</code>.
   *
   ...
   * position and {@link #tuple()} to obtain the visitable tuple in the index corresponding to that
   ...
   * index for that key).
   *
   * @return The key corresponding to the current current position -or- <code>null</code> iff the
   ...
   *
   * @return The current tuple -or- <code>null</code> iff there is no visitable tuple corresponding
   ...
   *     there is no visitable tuple corresponding to the current cursor position or if the current
   *     cursor position is undefined.
   */
  ITuple<E> tuple();
   ...
   *
   * @return The tuple -or- <code>null</code> iff there is no such visitable tuple.
   */
  ITuple<E> nextTuple();
