 ...
 * {@link ConcurrencyManager} will automatically provide appropriate concurrency controls. When
 * wrapping a local index, you can use an {@link UnisolatedReadWriteIndex} to impose {@link Map}
 * operation level concurrency control if your application does not otherwise handle this
 * constraint. Also, note that point tests for the scale-out indices are VERY expensive when
 * compared to ordered reads and writes.
 *
 * <p>
 *
 * <p>Note: The total order of the {@link EmbergraphMap} is completely determined by {@link
 * ITupleSerializer#serializeKey(Object)}. There is NO concept of a {@link Comparator}. The {@link
 * ITupleSerializer} is responsible for coercing application keys into variable length
 * <strong>unsigned</strong> byte[]s which are the keys for the underlying B+Tree. The order for the
 * B+Tree is the natural order for the <strong>unsigned byte[]</strong>s. {@link KeyBuilder}
 * supports the generation of unsigned byte[]s from various kinds of Java primitives and Unicode
 * {@link String}s and is typically used to write the {@link ITupleSerializer#serializeKey(Object)}
 * method.
 *
 * <p>Note: The coercion of the application keys into unsigned byte[]s is not type safe unless you
 * either consistently use a strongly typed instance of this class or specify an {@link
 * ITupleSerializer} for the backing B+Tree that only allows application keys that are instances of
 * acceptable classes. This issue is more critical for keys than for values since the keys define
 * the total index order and the default coercion rules for keys are provided by {@link
 * KeyBuilder#append(Object)} which does not attempt to partition the key space by the application
 * key type (keys are not safely polymorphic by default).
 *
 * <p>Note: When storing Java objects in the tuple value, the value MUST be treated as if it is
 * immutable since it will be serialized on insert or update and deserialized on read. In
 * particular, the pattern of updating the value stored in the map without {@link #put(Object,
 * Object)}ting the value back into the map WILL NOT cause the value associated with the tuple to be
 * updated.
 *
 * <p>Note: Both {@link Map#equals(Object)} and {@link Map#hashCode()} are VERY expensive, but that
 * is how they are defined.
 *
 * @param K The generic type for the keys stored in the map.
 * @param V The generic type for the values stored in the map.
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 */
public class EmbergraphMap<K, V> extends AbstractMap<K, V> implements SortedMap<K, V> {
   ...
   * @param ndx The index.
   */
  public EmbergraphMap(IIndex ndx) {
   ...
   */
  EmbergraphMap(IIndex ndx, byte[] fromKey, byte[] toKey) {
   ...
   * @throws RuntimeException if the <i>key</i> does not lie within the legal key range.
   * @deprecated for greater efficiency, the key should be encoded first, then invoke {@link
   *     #rangeCheck(byte[], boolean)} instead of this method and finally pass along the encoded
   *     key/val pair to the backing {@link IIndex} implementation.
   */
  ...
  public void clear() {
    ...
    if (fromKey == null && toKey == null && ndx instanceof AbstractBTree) {
      ...
      ndx.rangeIterator(fromKey, toKey, 0 /* capacity */, IRangeQuery.REMOVEALL, null /*filter*/);
    }
  }
  ...
  public boolean isEmpty() {
    ...
    if (ndx instanceof AbstractBTree && ((AbstractBTree) ndx).getEntryCount() == 0) {
    ...
    final ITupleIterator itr =
        ndx.rangeIterator(
            fromKey, toKey, 0 /* capacity */, IRangeQuery.NONE /* flags */, null /* filter */);
  ...
  public long rangeCount(boolean exactCount) {
    ...
    if (exactCount && ndx.getIndexMetadata().getDeleteMarkers()) {
      ...
      final Iterator itr =
          ndx.rangeIterator(fromKey, toKey, 0 /* capacity */, 0 /* flags */, null /* filter */);
  ...
  public K firstKey() {
    ...
    final ITupleIterator itr =
        ndx.rangeIterator(fromKey, toKey, 1 /* capacity */, IRangeQuery.DEFAULT, null /* filter */);
   ...
   *     (basically those that should be using ordered reads or ordered writes).
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  protected class EntrySet extends AbstractSet<Map.Entry<K, V>> {
    ...
    public int size() {
      ...
      return EmbergraphMap.this.size();
    }
  }
   ...
   * ITuple rather than doing it lazily but that maps the {@link Map} interface either blow up (the
   * key can not be recovered), do more work for cases where the key is going to be stored in the
   * value, and work efficiently only for those cases where the key can be directly decoded, e.g.,
  ...
  protected class Entry implements Map.Entry<K, V> {
    ...
    public V setValue(V value) {
      ...
      return EmbergraphMap.this.put(getKey(), value);
    }
  }
}
