...
/**
 * Class supporting random access to tuples and sequential tuple-based cursor movement for an {@link
 * AbstractBTree}.
 *
 * <p>The tuple position is defined in terms of the current key on which the tuple "rests". If there
 * is no tuple associated with that key in the index then you will not be able to read the value or
 * optional metadata (delete markers or version timestamps) for the key. If the key is associated
 * with a deleted tuple then you can not read the value associated with the key, but you can read
 * the (optional) delete marker and version metadata if the cursor was provisioned to visit deleted
 * tuples.
 *
...
public abstract class AbstractBTreeTupleCursor<I extends AbstractBTree, L extends Leaf, E>
  ...
  private static final transient String LOG_CURSOR_POSITION_NOT_VISITABLE =
      "Cursor position is not visitable";
  ...
  /**
   * The cursor position is undefined until {@link #first(boolean)}, {@link #last(boolean)}, or
   * {@link #seek(byte[])} is used to position the cursor.
   *
   * @throws IllegalStateException if the cursor position is not defined.
   */
  protected final void assertCursorPositionDefined() {

    if (!isCursorPositionDefined()) throw new IllegalStateException();
  }

  /**
   * Create a new cursor.
   *
   * @param btree The B+Tree whose tuples are visited by this cursor.
   * @param tuple The tuple into which the data will be copied. {@link IRangeQuery#KEYS} MUST be
   *     specified for that {@link Tuple}. The keys of the index will always be copied into the
   *     {@link Tuple}, but the <i>flags</i> on the {@link Tuple} will determine whether the values
   *     paired to the key will be copied and whether or not deleted tuples will be visited.
   * @param fromKey The optional inclusive lower bound.
   * @param toKey The optional exclusive upper bound.
   * @throws IllegalArgumentException if the <i>btree</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>tuple</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>tuple</i> is not associated with that <i>btree</i>.
   * @throws IllegalArgumentException if the <i>fromKey</i> is GTE the <i>toKey</i>.
   */
  public AbstractBTreeTupleCursor(
      final I btree, final Tuple<E> tuple, final byte[] fromKey, final byte[] toKey) {
  ...
  public String toString() {
    ...
    return "Cursor{fromKey="
        + BytesUtil.toString(fromKey)
        + ", toKey="
        + BytesUtil.toString(toKey)
        + ", currentKey="
        + BytesUtil.toString(currentKey())
        + ", visitDeleted="
        + visitDeleted
        + "}";
  }
   ...
   */
  protected final boolean rangeCheck(final byte[] key) {
  ...
  /**
   * The optional inclusive lower bound. If the <i>fromKey</i> constraint was specified when the
   * {@link ITupleCursor} was created, then that value is returned. Otherwise, if the index is an
   * index partition then the {@link LocalPartitionMetadata#getLeftSeparatorKey()} is returned.
   * Finally, <code>null</code> is returned if there is no inclusive lower bound.
   */
  ...
  /**
   * The optional exclusive upper bound. If the <i>toKey</i> constraint was specified when the
   * {@link ITupleCursor} was created, then that value is returned. Otherwise, if the index is an
   * index partition then the {@link LocalPartitionMetadata#getRightSeparatorKey()} is returned.
   * Finally, <code>null</code> is returned if there is no exclusive upper bound.
   */
  ...
  /**
   * Return a new {@link ICursorPosition} from the <i>leafCursor</i>, tuple <i>index</i>, and
   * <i>key</i>
   *
   * @param leafCursor The {@link ILeafCursor} (already positioned on the desired leaf).
   * @param index The index of the tuple corresponding to the <i>key</i> within the current leaf of
   *     the <i>leafCursor</i> -or- a negative integer representing the insertion point for the
   *     <i>key</i> if the <i>key</i> is spanned by that leaf but there is no tuple for that
   *     <i>key</i> in the <i>leaf</i>.
   * @param key The key.
   * @return The new {@link ICursorPosition}.
   * @throws IllegalArgumentException if <i>leafCursor</i> is <code>null</code>.
   * @throws IllegalArgumentException if <i>key</i> is <code>null</code>.
   */
  protected abstract AbstractCursorPosition<L, E> newPosition(
      ILeafCursor<L> leafCursor, int index, byte[] key);

  /**
   * Return a clone of the given {@link ICursorPosition} designed for use by {@link #hasNext()} and
   * {@link #hasPrior()} (temporary test without side-effects).
   *
   * @param p The cursor position.
   * @return A clone of that cursor position.
   * @deprecated This is never used.
   */
  protected abstract AbstractCursorPosition<L, E> newTemporaryPosition(ICursorPosition<L, E> p);

  /**
   * Return a new {@link ICursorPosition} that is initially positioned on the inclusive lower bound
   * and on <code>new byte[]{}</code> if there is no inclusive lower bound.
   */
  ...
  /**
   * Return a new {@link ICursorPosition} that is initially positioned on the given <i>key</i>.
   *
   * @param leaf A leaf (required).
   * @param key A key that is spanned by that leaf (required, but there is no requirement that a
   *     tuple corresponding to that key is present in the leaf).
   * @return The new {@link ICursorPosition}.
   * @throws IllegalArgumentException if the key is <code>null</code>.
   * @throws KeyOutOfRangeException if the key lies outside of the optional constrain on the {@link
   *     ITupleCursor}.
   */
  @SuppressWarnings("unchecked")
  protected final AbstractCursorPosition<L, E> newPosition(final byte[] key) {
  ...
  /**
   * Return a new {@link ICursorPosition} that is initially positioned on the last tuple in the
   * key-range (does not skip over deleted tuples).
   *
   * <p>Note: If there is no exclusive upper bound and there are no tuples in the index then the
   * position is set to <code>new byte[]{}</code> (the same behavior as first for an empty index).
   *
  ...
  public byte[] currentKey() {
    ...
    if (!isCursorPositionDefined()) {
      ...
      if (DEBUG) log.debug(LOG_NO_CURSOR_POSITION);
  ...
  public ITuple<E> last() {
    ...
    if (!currentPosition.reverseScan(false /*skipCurrent*/, false /*testOnly*/)) {
      ...
      // discard the current position since we have scanned beyond the start of the index / key
      // range.
      currentPosition = null;
  ...
  public final ITuple<E> seek(Object key) {
    ...
    return seek(getIndex().getIndexMetadata().getTupleSerializer().serializeKey(key));
  }
  ...
  public ITuple<E> seek(final byte[] key) {
    ...
    if (DEBUG) log.debug("key=" + BytesUtil.toString(key));
   ...
   * {@inheritDoc}
   *
   * <p>Note: This is lighter weight than {@link #hasNext()} and {@link #next()} since it does not
   * need to scan to verify that the next position exists before visiting that tuple.
   */
  @Override
  public ITuple<E> nextTuple() {
    ...
    if (!currentPosition.forwardScan(true /*skipCurrent*/, false /*testOnly*/)) {
      ...
      if (DEBUG) log.debug(LOG_NO_SUCCESSOR);
  ...
  public boolean hasNext() {
    ...
    if (DEBUG) log.debug(pos.toString());
   ...
   *
   * <p>Note: This is lighter weight than {@link #hasNext()} and {@link #next()} since it does not
   * need to scan to verify that the prior position exists before visiting that tuple.
   */
  @Override
  public ITuple<E> priorTuple() {
    ...
    if (!currentPosition.reverseScan(true /*skipCurrent*/, false /*testOnly*/)) {
      ...
      if (DEBUG) log.debug(LOG_NO_PREDECESSOR);
  ...
  /**
   * FIXME This needs to be overridden for the {@link IsolatedFusedView} in order to correctly
   * propagate the version timestamp onto the tuple. See the insert() and remove() methods on that
   * class. This will probably be done inside of an implementation that extends the {@link
   * FusedView} cursor implementation, at which point this notice can be removed.
   */
  @Override
  public void remove() {

    if (btree.isReadOnly()) throw new UnsupportedOperationException();
  ...
  interface ICursorPosition<L extends Leaf, E> {
     ...
     *
     * <p>Note: If the {@link ICursorPosition} is not on a visitable tuple then this method will
     * return <code>null</code> rather than the caller's tuple. This situation can arise if the
     * index is empty (e.g., a root leaf with nothing in it) or if current cursor position does not
     * correspond to a visitable tuple in the index.
     *
     * @param tuple The caller's buffer.
     ...
     * cursor position for which there is no corresponding tuple in the index is not a visitable
     * tuple. A cursor position for which the corresponding tuple in the index is deleted is
     * visitable iff the {@link ITupleCursor} was provisioned to visit deleted tuples.
     *
     ...
     *
     * @param skipCurrent If the current tuple should be skipped over. when <code>false</code>
     *     position will not be advanced if the current tuple is "visitable".
     * @param testOnly When <code>true</code> the method will report whether or not a visitable
  ...
  abstract static class AbstractCursorPosition<L extends Leaf, E> implements ICursorPosition<L, E> {
    ...
    /**
     * The key corresponding to the current cursor position. This is update each time we settle on a
     * new tuple or when the cursor position is directed to an explicit key using seek(). Having
     * this state allows us to unambiguously position the cursor in response to a seek() and also
     * allows us to unambiguously re-position the cursor if the current leaf has been invalidated.
     */
    private final DataOutputBuffer kbuf;

    /**
     * When true the methods on this class will re-locate the leaf using the key associated with the
     * current tuple before doing anything else (this flag is cleared as a side-effect once the leaf
     * has been re-located).
     *
     * <p>Note: This is used to support {@link ILeafListener}s by some derived classes
     */
    ...
    /**
     * <code>true</code> iff the class registers as an {@link ILeafListener} and can therefore
     * handle events that invalidate the position by re-locating the appropriate leaf within the
     * B+Tree.
     */
     ...
     *
     * @param cursor The {@link ITupleCursor}.
     * @param leaf The leaf cursor (already positioned on the desired leaf).
     * @param index The index of the tuple in the <i>leaf</i> -or- the insertion point for the
     *     <i>key</i> if there is no tuple for that key in the leaf.
     * @param key The key (required).
     */
    protected AbstractCursorPosition(
        final ITupleCursor2<E> cursor,
        final ILeafCursor<L> leafCursor,
        final int index,
        final byte[] key) {
      ...
      if (leafCursor == null) throw new IllegalArgumentException();
    ...
    public void seek(final AbstractCursorPosition<L, E> src) {
      ...
      if (cursor != src.cursor) throw new IllegalArgumentException();
    ...
    /**
     * <code>true</code> if the cursor position corresponds to a tuple in the index.
     *
     * <p>Note: A cursor position can be initially constructed with [index==nkeys], in which case
     * the cursor is not "on" a tuple. Normally the caller will scan forward / backward until they
     * reached a visitable tuple so as to avoid that fence post.
     *
     * <p>Note: The other cause the cursor position not being "on" a tuple is an empty root leaf
     */
    ...
    public Tuple<E> get(final Tuple<E> tuple) {
      ...
      if (!isVisitableTuple()) {
        ...
        if (DEBUG) log.debug(LOG_CURSOR_POSITION_NOT_VISITABLE);
    ...
    /**
     * The contract for this method is to re-locate the {@link #leaf} and then re-locate the {@link
     * #index} of current tuple within that leaf using the key reported by {@link #getKey()}. This
     * method is triggered when an {@link ILeafListener} is registered against a mutable {@link
     * BTree}).
     *
     * <p>The default implementation does nothing if {@link #leafValid} is <code>true</code> (and it
     * will always be true if the B+Tree is read-only).
     *
     * @return true unless if the leaf was re-located but the current tuple is no longer present in
     *     the leaf (e.g., it was deleted from the index). Note that the return is always <code>true
     *     </code> if delete markers are in effect since a remove() is actually an update in that
     *     case.
     * @throws UnsupportedOperationException if {@link #leafValid} is <code>false</code>
     * @todo the return value is never used so this could be declared to return <code>void</code>
     */
     ...
     *
     * <p>Note: This method is NOT required to validate that the next leaf lies within the optional
     * key-range constraint on the owning {@link ITupleCursor}.
     *
     * @return <code>true</code> if there is successor of the current leaf.
     */
     ...
     *
     * <p>Note: This method is NOT required to validate that the prior leaf lies within the optional
     * key-range constraint on the owning {@link ITupleCursor}.
     *
     * @return <code>true</code> if there is a predecessor of the current leaf.
     */
    ...
    /**
     * Return <code>true</code> if the key at the current {@link #index} in the current {@link
     * #leaf} lies inside of the optional half-open range constraint.
     *
     * @param leaf The current leaf.
     * @param index The index of the current tuple in that leaf.
     * @param forward <code>true</code> iff the cursor is moving in the forward direction
     *     (increasing key order) and <code>false</code> iff the cursor is moving in the reverse
     *     direction (decreasing key order). We only check the <code>fromKey</code> when moving in
     *     reverse order and the <code>toKey</code> when moving in forward order.
     * @return <code>true</code> unless the current key is LT [fromKey] or GTE [toKey].
     * @todo could only rangeCheck in the direction of the traversal, but this causes cursor test
     *     failures for some reason.
     * @todo for a read-only view, we could compute the leaf addr and the tuple index of the fromKey
     *     and/or to key and then just compare those with the given leaf and index to decide if we
     *     were within the necessary bounds.
     */
    private boolean rangeCheck(final L leaf, final int index) { // , boolean forward) {
      ...
      if (true) {
        ...
        if (fromKey != null) {
          ...
          if (BytesUtil.compareBytesWithLenAndOffset(0, tlen, a, 0, fromKey.length, fromKey) < 0) {

            if (DEBUG) {

              log.debug(
                  "key="
                      + BytesUtil.toString(a, 0, tlen)
                      + " LT fromKey"
                      + BytesUtil.toString(fromKey));
            }
        ...
        if (toKey != null) {
          ...
          if (BytesUtil.compareBytesWithLenAndOffset(0, tlen, a, 0, toKey.length, toKey) >= 0) {
    ...
    /**
     * Used to update the position once a visitable tuple has been found. Both {@link
     * #forwardScan(boolean, boolean)} and {@link #reverseScan(boolean, boolean)} are written to
     * have NO SIDE EFFECT on the position unless this method is invoked.
     */
    private void updatePosition(final ILeafCursor<L> leafCursor, final int index) {
    ...
    public final boolean forwardScan(final boolean skipCurrent, final boolean testOnly) {
      ...
      if (INFO) log.info(LOG_NO_SUCCESSOR);

      // no visitable tuple.
      return false;
    }

    public final boolean reverseScan(final boolean skipCurrent, final boolean testOnly) {
      ...
      if (INFO) log.info(LOG_NO_PREDECESSOR);
  ...
  /**
   * {@link ICursorPosition} for a read-only {@link BTree} (does not establish listeners).
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @param <E>
   */
  private static class ReadOnlyCursorPosition<E> extends AbstractCursorPosition<Leaf, E> {
     ...
     * @param key
     */
    public ReadOnlyCursorPosition(
        ITupleCursor2<E> cursor, ILeafCursor<Leaf> leafCursor, int index, byte[] key) {
  ...
  /**
   * {@link ICursorPosition} for a mutable {@link BTree} (establishes listeners to determine when
   * the current leaf and/or tuple index has been invalidated).
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @param <E>
   */
  private static class MutableCursorPosition<E> extends ReadOnlyCursorPosition<E>
      implements ILeafListener {
     ...
     * @param index
     */
    protected MutableCursorPosition(
        ITupleCursor2<E> cursor, ILeafCursor<Leaf> leafCursor, int index, byte[] key) {
    ...
    /**
     * Note: rather than immediately re-locate the leaf this just notes that the leaf is invalid and
     * it will be re-located the next time we need to do anything with the leaf.
     */
    ...
    protected boolean relocateLeaf() {
      ...
      if (INFO) log.info("Relocating leaf: key=" + BytesUtil.toString(key));
  ...
  /**
   * An {@link ITuple} that directly supports forward and reverse cursor operations on a local
   * {@link BTree}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
  ...
  public static class ReadOnlyBTreeTupleCursor<E> extends AbstractBTreeTupleCursor<BTree, Leaf, E> {
    ...
    public ReadOnlyBTreeTupleCursor(
        final BTree btree, final Tuple<E> tuple, final byte[] fromKey, final byte[] toKey) {
   ...
   * mutable {@link BTree}.
   *
   * <p>This implementation supports concurrent modification of the {@link BTree} but is NOT
   * thread-safe. This means that you can interleave cursor-based tuple operations with insert,
   * update, or delete of tuples using the {@link BTree}. However, the {@link BTree} itself is NOT
   * thread-safe for mutation and this class does not relax that limitation.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @param <E>
  ...
  public static class MutableBTreeTupleCursor<E> extends ReadOnlyBTreeTupleCursor<E> {
    ...
    /**
     * Note: This is only used by {@link #hasNext()} and {@link #hasPrior()} for a temporary test
     * without side-effects on the state of the {@link ITupleCursor} and therefore we do NOTNOT
     * register an {@link ILeafListener} since that is just more overhead and it will not be used.
     */
    @Override
