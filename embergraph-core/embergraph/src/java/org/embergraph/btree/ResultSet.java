 ...
 * throw an {@link UnsupportedOperationException}. You can test this using {@link
 * #hasDeleteMarkers()}, {@link #hasVersionTimestamps()} and related methods.
 *
 * <p>FIXME Do not decode the delete flags or version timestamps. Code per {@link ReadOnlyLeafData}
...
public class ResultSet implements ILeafData, Externalizable {
  ...
  /** The version timestamps -or- <code>null</code> if not requested -or- not maintained. */
   ...
   *
   * @todo represent using a {@link BitVector}. {@link LongArrayBitVector} when allocating. Directly
   *     write the long[] backing bits (getBits()) onto the output stream. Reconstruct from backing
   *     long[] when reading. Could also consider run length encoding.
   */
   ...
   * format that is actually stored in the index and the timestamped property sets that are visited
   * by the logical row scan.
   *
   * <p>Note: This means that the ITupleSerializer will be <code>null</code> if the iterator does
   * not visit any elements!
   */
  ...
  public final IRaba getKeys() {
    ...
    if (keys == null) throw new UnsupportedOperationException();
  ...
  public final IRaba getValues() {
    ...
    if (vals == null) throw new UnsupportedOperationException();

    return vals;
  }

  /** The values returned by {@link ITuple#getSourceIndex()} for each visited index entry. */
  public final int getSourceIndex(final int index) {
   ...
   * populated.
   *
   * <p>The values returned by {@link ITuple#getSourceIndex()} may be used to identify the resource
   * from which a given tuple was read. That information is used to direct {@link
   * ITuple#readBlock(long)} requests to the correct resource on the {@link IDataService}.
   */
   ...
   *     The caller should either use the suggested capacity, compute either an actual limit or the
   *     upper bound based on additional information, such as a range count, or treat the suggested
   *     capacity as a soft limit if it is not possible to determine the upper bound.
   */
  protected void init(int limit) {
    ...
    this.keys = (sendKeys ? new MutableKeysRaba(0, 0, limit, new byte[limit][]) : null);

    this.vals = (sendVals ? new MutableValuesRaba(0, 0, limit, new byte[limit][]) : null);
   ...
   *
   * <p>Note: the point of this method is to communicate the restart point for a continuation query
   * (or that no continuation query is necessary). If you are scanning ahead to decide whether or
   * not the next coherent chunk of tuples (e.g., a logical row) would fit in the buffer, then the
   * restart point is the point from which the continuation query should start and the key to report
   * is the last key before the start of the first logical row that was rejected because it would
   * overflow the internal buffers.
   *
   * <p>Note: <code>!exhausted</code> the caller will issue a continuation query against this index
   * partition whose <i>fromKey</i> is the successor of <i>lastKey</i> (the successor is formed
   * using {@link BytesUtil#successor(byte[])}).
   *
   * <p>Note: If an index partition is <i>exhausted</i> there may still be data for the key range on
   * subsequent index partitions the caller will discover the locator for the next index partition
   * in key order whose left separator key is LE the <i>toKey</i> and query it for more results. If
   * there is no such index partition then the aggregate iterator is finished.
   *
   * @param exhausted <code>true</code> iff the source iterator will not visit any more tuples
   *     {@link Iterator#hasNext()} is <code>false</code>.
   * @param lastKey The key from the last tuple scanned by the source iterator regardless of whether
   *     it was included in the result set -or- <code>null</code> iff no tuples were scanned (this
   *     implies that there is no data for the query in the key range on this index partition and
   *     hence that it is <i>exhausted</i>).
   */
  ...
  protected void copyTuple(final ITuple<?> tuple) {
    ...
    if (deleteMarkers != null) {
      ...
      deleteMarkers[ntuples] = (byte) (tuple.isDeletedVersion() ? 1 : 0);
    }
    ...
    if (versionTimestamps != null) {
      ...
      if (t < minimumVersionTimestamp) minimumVersionTimestamp = t;

      if (t > maximumVersionTimestamp) maximumVersionTimestamp = t;
    }
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    if (haveKeys) {
      ...
      if (ntuples == 0) {
        ...
        keys = tupleSerializer.getLeafKeysCoder().decode(FixedByteArrayBuffer.wrap(a));
      }
    ...
    if (haveVals) {
      ...
      if (ntuples == 0) {
        ...
        // wrap the coded record.
        vals = tupleSerializer.getLeafValuesCoder().decode(FixedByteArrayBuffer.wrap(a));
      }
  ...
  public void writeExternal(final ObjectOutput out) throws IOException {
    ...
    final DataOutputBuffer buf = keys != null || vals != null ? new DataOutputBuffer() : null;
    ...
    if (keys != null && ntuples > 0) {
      ...
      final AbstractFixedByteArrayBuffer slice =
          tupleSerializer.getLeafKeysCoder().encode(keys, buf);
    ...
    if (vals != null && ntuples > 0) {
      ...
      // code the values.
      final AbstractFixedByteArrayBuffer slice =
          tupleSerializer.getLeafValuesCoder().encode(vals, buf);
   ...
   *
   * <ol>
   *   <li>Create a new {@link ResultSet}
   *   <li>Invoke {@link #init(int)} to setup the internal buffers.
   *   <li>Apply the source {@link ITupleIterator}, using {@link #copyTuple(ITuple)} to copy data
   *       into those buffers.
   *   <li>Signal completion using {@link #done(boolean, byte[])}
   * </ol>
   *
   * @param ndx The index.
   * @param flags The flags specified for the iterator. See {@link IRangeQuery}.
   */
  protected ResultSet(final IIndex ndx, final int flags) {
    ...
    commitTime = ((ILocalBTreeView) ndx).getMutableBTree().getLastCommitTime();
   ...
   * so that we can report the lastKey visited for continuation queries.
   *
   * @param ndx The index.
   * @param capacity The requested capacity for the operation.
   * @param flags The flags specified for the iterator.
   * @param itr The source iterator.
   */
  public ResultSet(
      final IIndex ndx, final int capacity, final int flags, final ITupleIterator itr) {
  ...
  private boolean rangeCheckIndex(final int index) {
    ...
    if (index < 0 || index >= ntuples) throw new IndexOutOfBoundsException();
  ...
  public final AbstractFixedByteArrayBuffer data() {
    ...
    if (!isCoded()) throw new UnsupportedOperationException();
   ...
   * been materialized and therefore can not be reported by {@link #getRawRecord(int)} using this
   * class.
   */
