 ...
 *
 * <p>Note: No mechanism is exposed for fetching a node or leaf of the tree other than the root by
 * its key. This is because the parent reference on the node (or leaf) can only be set when it is
 * read from the store in context by its parent node.
 *
 * <p>Note: the leaves can not be stitched together with prior and next references without forming
 * cycles that make it impossible to write out the leaves of the btree. This restriction arises
 * because each time we write out a node or leaf it is assigned a persistent identifier as an
 * unavoidable artifact of providing isolation for the object index.
 *
 * <p>Note: This implementation is thread-safe for concurrent readers BUT NOT for concurrent
 * writers. If a writer has access to a {@link BTree} then there MUST NOT be any other reader -or-
 * writer operating on the {@link BTree} at the same time.
 *
 * @todo consider also tracking the #of deleted entries in a key range (parallel to how we track the
 *     #of entries in a key range) so that we can report the exact #of non-deleted entries when the
 *     btree supports isolation (right now you have to do a key range scan and count the #of
 *     non-deleted entries).
 * @todo Choose the split point in the branch nodes within a parameterized region such that we
 *     choose the shortest separator keys to promote to the parent. This is a bit more tricky for
 *     the {@link IndexSegmentBuilder}.
 * @todo reuse a buffer when copying keys and values out of the index, e.g., for the {@link
 *     ITupleIterator}, in order to minimize heap churn.
 * @todo The B+Tree implementation does not support limits on the serialized size of a node or leaf.
 *     The design strategy is to allow flexible sizes for serialized node and leaf records since
 *     larger branching factors and continuous IO are cheaper and nothing in particular requires the
 *     use of fixed size records when accessing disk. However, it would still be useful to be able
 *     to place an upper bound on the serialized size of a node or leaf. Nodes and leaves are only
 *     serialized on eviction, so this would require the ability to split nodes and/or leaves
 *     immediately before eviction if they would be likely to overflow the upper bound on the record
 *     size. Probably the node or leaf needs to be made immutable, the serialized size checked, and
 *     then a split operation invoked if the record would exceed the upper bound. If this happens a
 *     lot, then the branching factor is too high for the data and would have to be lowered to
 *     regain performance.
 *     <p>Another use case for limits on the size of a node/leaf is maintaining the data in a
 *     serialized form. This can have several benefits, including reducing heap churn by copying
 *     keys and values (not their references) into the record structure, and reducing (eliminating)
 *     deserialization costs (which are substantially more expensive than serialization). This can
 *     be approached using new implementations of the key buffer and a value buffer implementation
 *     (one that can be used for nodes or for leaves). If an object implements both then it is a
 *     small additional step towards using it for the entire serialized record (minus some
 *     header/tailer).
 *     <p>Adaptive packed memory arrays can be used in this context to minimize the amount of
 *     copying that needs to be performed on mutation of a node or leaf.
 *     <p>Leading key compression is intrinsically part of how the key buffer maintains its
 *     representation so, for example, a micro-index would have to be maintained as part of the key
 *     buffer.
 * @todo we could defer splits by redistributing keys to left/right siblings that are under capacity
 *     - this makes the tree a b*-tree. however, this is not critical since the journal is designed
 *     to be fully buffered and the index segments are read-only but it would reduce memory by
 *     reducing the #of nodes -- and we can expect that the siblings will be either resident or in
 *     the direct buffer for the journal
 * @todo evict subranges by touching the node on the way up so that a node that is evicted from the
 *     hard reference cache will span a subrange that can be evicted together. this will help to
 *     preserve locality on disk. with this approach leaves might be evicted independently, but also
 *     as part of a node subrange eviction.
 * @todo since forward scans are much more common, change the post-order iterator for commit
 *     processing to use a reverse traversal so that we can write the next leaf field whenever we
 *     evict a sequence of leaves as part of a sub-range commit (probably not much of an issue since
 *     the journal is normally fully buffered and the perfect index segments will not have this
 *     problem).
 * @todo pre-fetch leaves for range scans? if we define a parallel iterator flag?
 * @todo Note that efficient support for large branching factors requires a more sophisticated
 *     approach to maintaining the key order within a node or leaf. E.g., using a red-black tree or
 *     adaptive packed memory array. However, we can use smaller branching factors for btrees in the
 *     journal and use a separate implementation for bulk generating and reading "perfect" read-only
 *     key range segments.
 * @todo derive a string index that uses patricia trees in the leaves per several published papers.
...
public class BTree extends AbstractBTree
   ...
   *
   * <p>Note: When the {@link BTree} is part of a scale-out index then the counter will assign
   * values within a namespace defined by the partition identifier.
   */
  @Override
   ...
   * by {@link #load(IRawStore, long, boolean)}.
   */
   ...
   * needs to write the actual value store in this counter into its serialized record (without the
   ...
   * @param checkpoint A {@link Checkpoint} record for that {@link BTree}.
   * @param metadata The metadata record for that {@link BTree}.
   * @param readOnly When <code>true</code> the {@link BTree} will be immutable.
  ...
  public BTree(
    ...
    assert readOnly
        || writeRetentionQueue.capacity()
            >= IndexMetadata.Options.MIN_WRITE_RETENTION_QUEUE_CAPACITY;
   ...
   *
   * <p>Note: A bloom filter can be relatively large. The bit length of a bloom filter is
   * approximately one byte per index entry, so a filter for an index with 10M index entries will be
   * on the order of 10mb. Therefore this method will typically have high latency.
   *
   * <p>Note: the {@link Checkpoint} record initially stores <code>0L</code> for the bloom filter
   * address. {@link #newRootLeaf()} is responsible for allocating the bloom filter (if one is to be
   * used) when the root leaf is (re-)created. The address then gets stored in the {@link
   * Checkpoint} record by {@link #writeCheckpoint()} (if invoked and once the bloom filter is
   * dirty).
   *
   * @throws IllegalStateException if the bloom filter does not exist (the caller should check this
   *     first to avoid obtaining a lock).
   */
  ...
  public final long getRevisionTimestamp() {
    ...
    if (readOnly) throw new UnsupportedOperationException(ERROR_READ_ONLY);

    return lastCommitTime + 1;
  }

  @Override
  public final void setLastCommitTime(final long lastCommitTime) {

    if (lastCommitTime == 0L) throw new IllegalArgumentException();
    ...
    if (INFO) log.info("old=" + this.lastCommitTime + ", new=" + lastCommitTime);
   ...
   * visibility of this field, through I do seem to remember that visibility might be guaranteed by
   * how the BTree class is discovered and returned to the class. Still, it does no harm to make
   * this a volatile read.
   */
  private volatile long lastCommitTime = 0L; // Until the first commit.
   ...
   */
  @Override
  public final void setDirtyListener(final IDirtyListener listener) {
   ...
   *
   * <p>Note: This does NOT flush all persistent state. See {@link #writeCheckpoint()} which also
   * handles the optional bloom filter, the {@link IndexMetadata}, and the {@link Checkpoint} record
   * itself.
   *
  ...
  private final boolean flush() {
    ...
    if (root != null && root.dirty) {
      ...
      if (INFO) log.info("flushed root: addr=" + root.identity);
   ...
   *
   * @throws IllegalStateException If the {@link BTree} is dirty.
   * @todo The {@link Checkpoint} could hold a {@link WeakReference} singleton to the read-only view
  ...
  public BTree asReadOnly() {
    ...
    if (needsCheckpoint()) throw new IllegalStateException();
   ...
   * (if dirty), the {@link IndexMetadata} (if dirty), and then writes a new {@link Checkpoint}
   * record on the backing store, saves a reference to the current {@link Checkpoint} and returns
   * the address of that {@link Checkpoint} record.
   *
   * @see #writeCheckpoint2(), which returns the {@link Checkpoint} record itself.
  ...
  private final Checkpoint _writeCheckpoint2() {
     ...
     * recycler)
     */
    if (checkpoint != null && getRoot() != null && checkpoint.getRootAddr() != getRoot().identity) {
   ...
   *
   * <p>The class identified by {@link IndexMetadata#getCheckpointClassName()} MUST declare a public
  ...
  public final Checkpoint getCheckpoint() {
    ...
    if (checkpoint == null) throw new AssertionError();
   ...
   *
   * <p>Note: In order to avoid needless checkpoints this method will return <code>false</code> if:
   *
   * <ul>
   *   <li>the metadata record is persistent -AND-
   *       <ul>
   *         <li>EITHER the root is <code>null</code>, indicating that the index is closed (in which
   *             case there are no buffered writes);
   *         <li>OR the root of the btree is NOT dirty, the persistent address of the root of the
   *             btree agrees with {@link Checkpoint#getRootAddr()}, and the {@link #counter} value
   *             agrees {@link Checkpoint#getCounter()}.
   *       </ul>
   * </ul>
   *
   * @return <code>true</code> true iff changes would be lost unless the B+Tree was flushed to the
   ...
   *
   * <p>Note: this method should be used with caution.
   *
   * @param indexMetadata The new metadata description for the {@link BTree}.
   * @throws IllegalArgumentException if the new value is <code>null</code>
   * @throws IllegalArgumentException if the new {@link IndexMetadata} record has already been
   *     written on the store - see {@link IndexMetadata#getMetadataAddr()}
  ...
  public final void setIndexMetadata(final IndexMetadata indexMetadata) {
    ...
    if (indexMetadata.getMetadataAddr() != 0) throw new IllegalArgumentException();
   ...
   *
   * <p>Note: In order to avoid needless writes the existing metadata record is always returned if
   * {@link #needsCheckpoint()} is <code>false</code>.
   *
   * <p>Note: The address of the existing {@link Checkpoint} record is always returned if {@link
   * #getAutoCommit() autoCommit} is disabled.
   *
   * @return The address of a {@link Checkpoint} record from which the btree may be reloaded.
   */
  @Override
   ...
   * resets the counters (height, #of nodes, #of leaves, etc). to be consistent with that new root.
   * If the btree is then made restart-safe by the commit protocol of the backing store then the
   * effect is as if all entries had been deleted. Old nodes and leaves will be swept from the store
   * eventually when the journal overflows.
   *
   * <p>When delete markers are enabled all un-deleted entries in the index are overwritten with a
   * delete marker.
   *
   * <p>Note: The {@link IIndexManager} defines methods for registering (adding) and dropping
   * indices vs removing the entries in an individual {@link BTree}.
   */
  @Override
  public final void removeAll() {
    ...
    if (!getIndexMetadata().getDeleteMarkers() && getStore() instanceof IRWStrategy) {
      ...
      final Iterator<AbstractNode> itr =
          getRoot().postOrderNodeIterator(false /* dirtyNodesOnly */, true /* nodesOnly */);
      ...
      final ITupleIterator itr =
          rangeIterator(null, null, 0 /* capacity */, REMOVEALL /* flags */, null /* filter */);
   ...
   * with an empty leaf as part of this operation. The {@link LocalPartitionMetadata} is updated to
   * reflect the new view.
   *
   * <p>Note: This method is used by the scale-out architecture for which it performs a very
   * specific function. It should not be used for any other purpose and should not be invoked by
   * user code. This encapsulates all of the trickery for creating the necessary checkpoint without
   * exposing any methods which could be used to replace the root node with an empty root leaf.
   *
   * @return The timestamp associated with the checkpoint record from which this {@link BTree} was
   *     loaded. This is the timestamp that was placed on the 2nd element of the view. Any read-only
   *     build or merge task will read from this timestamp.
   * @throws IllegalStateException if the {@link BTree} is read only.
   * @throws IllegalStateException if the {@link BTree} is dirty.
   * @throws IllegalStateException if the {@link BTree} has never been committed.
   */
   ...
   * @see #load(IRawStore, long, boolean)
   * @exception IllegalStateException If you attempt to create two btree objects from the same
   *     metadata record since the metadata address will have already been noted on the {@link
   *     IndexMetadata} object. You can use {@link IndexMetadata#clone()} to obtain a new copy of
   *     the metadata object with the metadata address set to <code>0L</code>.
   * @exception IllegalStateException if the {@link IndexTypeEnum} in the supplied {@link
   *     IndexMetadata} object is not {@link IndexTypeEnum#BTree}.
   */
  public static BTree create(final IRawStore store, final IndexMetadata metadata) {
    ...
    if (metadata.getIndexType() != IndexTypeEnum.BTree) {
      ...
      throw new IllegalStateException("Wrong index type: " + metadata.getIndexType());
    }
   ...
   * IRawStore}).
   *
   * <p>Fully transient {@link BTree}s provide the functionality of a B+Tree without a backing
   * persistence store. Internally, reachable nodes and leaves of the transient {@link BTree} use
   * hard references to ensure that remain strongly reachable. Deleted nodes and leaves simply clear
   * their references and will be swept by the garbage collector shortly thereafter.
   *
   * <p>Operations which attempt to write on the backing store will fail.
   *
   * <p>While nodes and leaves are never persisted, the keys and values of the transient {@link
   * BTree} are unsigned byte[]s. This means that application keys and values are always converted
   * into unsigned byte[]s before being stored in the {@link BTree}. Hence if an object that is
   * inserted into the {@link BTree} and then looked up using the {@link BTree} API, you WILL NOT
   * get back the same object reference.
   *
  ...
  //     *
  //     * <i>className</i>(IRawStore store, Checkpoint checkpoint, BTreeMetadata metadata, boolean
  // readOnly)
  //     *
  //     * </code>
  //     *
   ...
   *
   * @param store The store.
   * @param addrCheckpoint The address of a {@link Checkpoint} record for the index.
   * @param readOnly When <code>true</code> the {@link BTree} will be marked as read-only. Marking
   *     has some advantages relating to the locking scheme used by {@link Node#getChild(int)} since
   *     the root node is known to be read-only at the time that it is allocated as per-child
   *     locking is therefore in place for all nodes in the read-only {@link BTree}. It also results
   *     in much higher concurrency for {@link AbstractBTree#touch(AbstractNode)}.
   * @return The {@link BTree} or derived class loaded from that {@link Checkpoint} record.
   * @throws IllegalArgumentException if store is <code>null</code>.
   */
  @SuppressWarnings("unchecked")
  public static BTree load(
      final IRawStore store, final long addrCheckpoint, final boolean readOnly) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Could not load Checkpoint: store="
              + store
              + ", addrCheckpoint="
              + store.toString(addrCheckpoint),
          t);
    }
    ...
    try {
      ...
      throw new RuntimeException(
          "Could not read IndexMetadata: store=" + store + ", checkpoint=" + checkpoint, t);
    }
  ...
  public static class PartitionedCounter implements ICounter {
     ...
     *
     * @param tmp The source counter value.
     * @return The partition-specific counter value.
     * @throws RuntimeException if the source counter has overflowed.
     * @see TermIdEncoder#combine(pid,ctr), which performs the same operation and MUST be consistent
     *     with this method.
     */
  ...
  protected static class Stack {
     ...
     * very large number of tuples even with a small branching factor.
     */
    public Stack() {
   ...
   * notice if mutation causes the current leaf to become invalid. In general, you need to have a
   * specific <em>key</em> in mind in order to re-locate the appropriate leaf after such mutation
   * events.
   *
   * <p>Note: The {@link MutableBTreeTupleCursor} does register such listeners.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  public class LeafCursor implements ILeafCursor<Leaf> {
     ...
     *
     * <p>The purpose of this stack is to ensure that the ancestors of the current leaf remain
     * strongly reachable while the cursor is positioned on that leaf. This is necessary in order
     * for the methods that change the cursor position {@link #next()} {@link #prior()} to succeed
     * since they depend on the parent references which could otherwise be cleared (parent
     * references of nodes and leaves are weak references). This means that hard references MUST be
     * held to all {@link Node}s that are ancestors of the current {@link Leaf}. This criteria is
     * satisfied by normal top-down navigation since the hard references are on the stack frame.
     * However, non-recursive constructions violate this requirement since the hard references to
     * the parents are not on the stack frame. This {@link Stack} of {@link Node}s is therefore
     * maintained by all of the methods on this class such that we always have hard references for
     * the ancestors of the leaf that is being visited.
     *
     * <p>Another benefit of this stack is that is ensures that not only the current leaf but also
     * all nodes above the current leaf remain strongly reachable and hence require NO IOs. This
     * should have the most pronounced effect on a read-only {@link BTree} with a large number of
     * concurrent cursors as the hard reference queue for the index might otherwise let the relevant
     * nodes become weakly reachable. It will have less of a performance effect for a mutable {@link
     * BTree} since that class is single-threaded for writers.
     *
     * <p>Note: The {@link IndexSegment} handles this differently since it has the address of the
     * prior and next leaf on hand and does not need the parent references for navigation.
     */
    ...
    public Leaf next() {
      ...
      if (leaf.isDeleted()) throw new IllegalStateException("deleted");
      ...
      AbstractNode<?> sibling = null;
      {
        ...
        while (true) {
          ...
          if (p != stack.pop()) throw new AssertionError();
     ...
     *
     * <ol>
     *   <li>Recursive ascent via the parent until there is a left-sibling of the child node (the
     *       index of the child in the parent must be GT zero). If we reach the root and there is no
     *       left-sibling then we are already on the left-most leaf and we are done.
     *   <li>t = leftSibling
     *   <li>while(!t.isLeaf()) right-descend
     * </ol>
     *
     * As we go up we pop each parent off of the hard reference stack.
     *
     * <p>As we go down we add each non-leaf node to the hard reference stack.
     *
     * <p>In order to make this operation atomic, the nodes to be popped are added to a temporary
     * stack until we
     *
     * @return The prior leaf -or- <code>null</code> if there is no predecessor of this leaf.
     */
    @Override
    public Leaf prior() {
