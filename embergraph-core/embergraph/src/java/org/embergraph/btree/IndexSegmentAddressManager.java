 ...
 *
 * <p>Together with {@link IndexSegmentRegion}, this class class provides a workaround for node
 * offsets (which are relative to the start of the nodes block) in contrast to leaf offsets (which
 * are relative to a known offset from the start of the index segment file). This condition arises
 * as a side effect of serializing nodes at the same time that the {@link IndexSegmentBuilder} is
 * serializing leaves such that we can not group the nodes and leaves into distinct regions and know
 * the absolute offset to each node or leaf as it is serialized.
 *
 * <p>The offsets for blobs are likewise relative to the start of a {@link IndexSegmentRegion#BLOB}
 * region. The requirement for a blob region arises in a similar manner: blobs are serialized during
 * the {@link IndexSegmentBuilder} operation onto a buffer and then bulk copied onto the output
 * file. This means that only the relative offset into the blob region is available at the time that
 * the blob's address is written in an index entry's value.
 *
...
public class IndexSegmentAddressManager extends WormAddressManager {
   ...
   * However, choosing {@link IndexSegmentRegion#NODE} or {@link IndexSegmentRegion#BLOB} regions
   * does restrict the address to referencing a node (or blob) respectively.
   *
   * @param addr The address.
   * @return The region relative to which this address was encoded.
   */
  public final IndexSegmentRegion getRegion(final long addr) {
  ...
  public String toString(long addr) {
    ...
    return "{region="
        ...
        + ",off="
        + IndexSegmentRegion.decodeOffset(encodedOffset)
        + ",len="
        + nbytes
        + "}";
  ...
  public boolean isNodeAddr(long addr) {
    ...
    final boolean isNodeAddr =
        offset >= offsetNodes && (offset + length) <= (offsetNodes + extentNodes);
  ...
  public boolean isLeafAddr(long addr) {
    ...
    final boolean isNodeAddr =
        offset >= offsetLeaves && (offset + length) <= (offsetLeaves + extentLeaves);
