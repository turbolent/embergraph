 ...
 * @version $Id$
 * @param <E> The type of the elements visited by the iterator (tuples of some sort).
 */
public abstract class TupleFilter<E> extends FilterBase implements ITupleFilter<E> {
  ...
  public static class TupleFilterator<E> implements ITupleIterator<E> {
     ...
     * base {@link ITupleIterator} implementations reuse the same {@link Tuple} instance for each
     * tuple visited by the iterator. A copy of the data must be made in order to avoid side-effects
     * from the one-step lookahead used by the filter.
     */
    private final AbstractTuple<E> returnValue;

    public TupleFilterator(
      ...
      if (this.nextValue != null) {
        ...
        this.returnValue =
            new AbstractTuple<E>(nextValue.flags()) {
     ...
     *
     * @param tuple The tuple that will be visited.
     */
     ...
     *
     * <p>Note: The filter imposes a one-step lookahead means that invoking {@link #remove()} on the
     * source iterator would cause the wrong element to be removed from the source iterator.
     * Therefore this operation is disabled unless the {@link ITupleCursor}. {@link ITupleCursor} is
     * safe for traversal with concurrent modification, so we can just remove the key from the
     * source index.
     *
     * <p>Note: An {@link ITupleCursor} can be requested either at the top-level or by specifying
     * {@link IRangeQuery#CURSOR} to {@link TupleFilter#TupleFilter(int)}.
     *
     * @throws UnsupportedOperationException unless the source iterator is an {@link ITupleCursor}.
     */
    public void remove() {
       ...
       * Otherwise the one step lookahead imposed by the filter means
        ...
        throw new UnsupportedOperationException(
            "Source iterator does not implement " + ITupleCursor.class.getName());
      }
    }
