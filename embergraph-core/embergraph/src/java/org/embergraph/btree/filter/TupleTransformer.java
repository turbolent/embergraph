 ...
 * @version $Id$
 * @param E The generic type for the objects materialized from the source tuples.
 * @param F The generic type for the objects that can be materialized from the output tuples.
 * @todo better encapsulation for patterns with a 1:1 transform (one tuple in, one out) and patterns
 *     with a M:1 (many in, one out) and 1:M (1 in, many out)?
 */
public abstract class TupleTransformer<E, F> extends FilterBase implements ITupleFilter<F> {
  ...
  public ITupleIterator<F> filterOnce(Iterator src, final Object context) {
    ...
    src = new LookaheadTupleFilter().filterOnce((ITupleIterator<E>) src, context);
