 ...
 * byte[] and materialize the corresponding application key.
 *
...
public class DefaultTupleSerializer<K, V>
   ...
   *
   * @param leafKeysCoder The new value.
   */
  public final void setLeafKeysCoder(final IRabaCoder leafKeysCoder) {

    if (leafKeysCoder == null) throw new IllegalArgumentException();
   ...
   * been stored in an {@link IIndex} using another value as existing data MAY become unreadable.
   *
   * @param valuesCoder The new value.
   */
  public final void setLeafValuesCoder(final IRabaCoder valuesCoder) {
  ...
  /** The {@link #delegateKeyBuilderFactory} wrapped up in thread-local factory. */
  ...
  public DefaultTupleSerializer(final IKeyBuilderFactory keyBuilderFactory) {
    ...
    this(keyBuilderFactory, getDefaultLeafKeysCoder(), getDefaultValuesCoder());
  }
  ...
  public DefaultTupleSerializer(
    ...
    if (keyBuilderFactory == null) throw new IllegalArgumentException();

    if (leafKeysCoder == null) throw new IllegalArgumentException();

    if (leafValsCoder == null) throw new IllegalArgumentException();
   ...
   * host/JVM where the {@link DefaultTupleSerializer} instance was first created. That backing
   * {@link IKeyBuilderFactory} object is serialized along with the {@link DefaultTupleSerializer}
   * so that the specific configuration values are persisted, even when the {@link
   ...
   * @return The serialized representation of the object as a byte[] -or- <code>null</code> if the
   ...
   * inlining of datatype literals for the RDF database.
   */
