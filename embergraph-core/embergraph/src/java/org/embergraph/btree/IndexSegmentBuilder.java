 ...
 *
 * <ol>
 *   <li>Evicting a key range of an index into an optimized on-disk index. In this case, the input
 *       is a {@link BTree} that is ideally backed by a fully buffered {@link IRawStore} so that no
 *       random reads are required.
 *   <li>Merging index segments. In this case, the input is typically records emerging from a
 *       merge-sort. There are two distinct cases here. In one, we simply have raw records that are
 *       being merged into an index. This might occur when merging two key ranges or when external
 *       data are being loaded. In the other case we are processing two time-stamped versions of an
 *       overlapping key range. In this case, the more recent version may have "delete" markers
 *       indicating that a key present in an older version has been deleted in the newer version.
 *       Also, key-value entries in the newer version replaced (rather than are merged with)
 *       key-value entries in the older version. If an entry history policy is defined, then it must
 *       be applied here to cause key-value whose retention is no longer required by that policy to
 *       be dropped.
 * </ol>
 *
 * <h3>One pass vs. Two Pass Design Alternatives</h3>
 *
 * There are at least three design alternatives for index segment builds: (A) do an exact range
 * count instead and generate a perfect plan; (B) fully buffer the source iterator into byte[][]
 * keys, byte[][] vals, boolean[] deleteMarkers, and long[] versionTimestamps and generate an exact
 * plan, consuming the buffered byte[]s directly from RAM; and (C) use the fast range count to
 * generate a plan based on an overestimate of the tuple count and then apply a variety of hacks
 * when the source iterator is exhausted to make the output B+Tree usable, but not well formed.
 *
 * <p>The disadvantage of (A) is that it requires two passes over the source view, which
 * substantially increases the run time of the algorithm. In addition, the passes can drive
 * evictions in the global LRU and could defeat caching for a view approaching the nominal size for
 * a split. However, with (A) we can do builds for very large source B+Trees. Therefore, (A) is
 * implemented for such use cases.
 *
 * <p>The disadvantage of (B) is that it requires more memory. However, it is much faster than (A).
 * To compensate for the increased memory demand, we can single thread builds, merges, and splits
 * and fall back to (A) if memory is very tight or the source view is very large.
 *
 * <p>The disadvantage of (C) is that the "hacks" break encapsulation and leak into the API where
 * operations such as retrieving the right sibling of a node could return an empty leaf (since we
 * ran out of tuples for the plan). Since these "hacks" would break encapsulation, it would be
 * difficult to have confidence that the B+Tree API was fully insulated against the effects of
 * ill-formed {@link IndexSegment}s. Therefore, I have discarded this approach and backed out
 * changes designed to support it from the code base.
 *
 * <h3>Design alternatives for totally ordered nodes and leaves</h3>
 *
 * In order for the nodes to be written in a contiguous block we either have to buffer them in
 * memory or have to write them onto a temporary file and then copy them into place after the last
 * leaf has been processed. This concern was not present in West's algorithm because it did not
 * attempt to place the nodes and/or leaves contiguously onto the generated B+Tree file.
 *
 * <p>For the two pass design described above as option (A), the code buffers the nodes and leaves
 * onto {@link TemporaryRawStore}s. This approach is scalable, which is the concern of (A), but
 * requires at least twice the IO when compared to directly writing the nodes and leaves onto the
 * output file.
 *
 * <p>When sufficient memory is available, as cases where (B) would apply, we can write the leaves
 * directly on the backing file (using double-buffering to update the prior/next addrs). Since there
 * are far fewer nodes than leaves, we can buffer the nodes in memory, writing them once the leaves
 * are finished.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: IndexSegmentBuilder.java 2265 2009-10-26 12:51:06Z thompsonbry $
 * @see "Post-order B-Tree Construction" by Lawerence West, ACM 1992. Note that West's algorithm is
 *     for a b-tree (values are stored on internal stack as well as leaves), not a b+-tree (values
 *     are stored only on the leaves). Our implementation is therefore an adaptation.
 * @see "Batch-Construction of B+-Trees" by Kim and Won, ACM 2001. The approach outlined by Kim and
 *     Won is designed for B+-Trees, but it appears to be less efficient on first glance.
 * @see IndexSegment
 * @see IndexSegmentFile
 * @see IndexSegmentCheckpoint
 * @todo Put profiler on (B) for build stress tests. The source view should be pre-generated such
 *     that we only measure the build behavior in the profiler. See {@link
 *     TestIndexSegmentBuilderWithLargeTrees}. [We need to first remove the synchronization for disk
 *     reads on the journal so we have the maximum possible IO rate, and possibly materialize the
 *     leaves of the source view in parallel using pre-fetch.]
 *     <p>Make sure that {@link #elapsed} reports the total build time, including the range count or
 *     pre-materialization costs as those are a significant part of the total cost.
 *     <p>Much of the cost of the build is the range iterator, including decoding the nodes and
 *     leaves and materializing tuples. Most of the remaining cost is the coding of the new nodes
 *     and leaves and their IO. Some of that cost can be trimmed by faster coders, but we can also
 *     trim the IO using parallel materialization of leaves (for {@link IndexSegment}) and pre-fetch
 *     of nodes and leaves (for {@link BTree}s).
 *     <p>The B+Tree iterator on the fused view of the journal and index segments is effectively an
 *     incremental merge of the source iterators. However, the iterator has a [capacity] parameter
 *     which is a hint for materialization. We could do this as a parallel merge sort of the leaves
 *     spanned of the key range with a restriction for the capacity. That could also be done on a
 *     GPU. The same iterator is used as the source for a compacting merge. If we tunnel the
 *     representation to something like the byte[][] keys, byte[][] vals, boolean[] deleteMarkers,
 *     long[] revisionTimestamps, then we could do a merge sort of the leaves from the (ordered)
 *     view and then do a parallel build segment from the backing representation. We could do that
 *     build step in java or on a GPU.
 * @todo GPU parallel builds. Once we have the data in a bunch of byte[][]s we could conceivable get
 *     this stuff organized to the point where each tuple and child in the output B+Tree could be
 *     assigned to a thread and each leaf and node to a core on a GPU. At that point, we run the
 *     "build" in parallel. For this approach, the uncoded leaves will all wind up in memory using
 *     the same byte[]s for keys and values as the source view.
 *     <p>This approach could be carried further to code the nodes and leaves in parallel, perhaps
 *     as a 2nd GPU program. If the source view was fully buffered, then it should be released once
 *     the nodes and leaves have been coded as no more use will be made of those data.
 *     <p>While it may not improve the IO substantially, it is possible to use gathered writes if
 *     the leaves and nodes are fully buffered in memory. For the leaves, we write them in the index
 *     order. For the nodes, we write them in their pre-order traversal. The order in which we would
 *     write out the nodes and leaves should be part of the state during a GPU build regardless of
 *     whether the IO is done sequentially or as a gathered write.
 * @todo Make sure it is possible to grab the {@link IndexMetadata} and the bloom filter from the
 *     generated file in a single IO. This could be useful when the index segment files are stored
 *     on a parallel file system. [It is possible to do this since these data are contiguous and in
 *     the same region of the generated file (they both use addresses relative to the BASE of the
 *     file).]
 */
public class IndexSegmentBuilder implements Callable<IndexSegmentCheckpoint> {
  ...
  /*
   * Error message when the #of tuples in the {@link IndexSegment} would exceed {@link
   * Integer#MAX_VALUE}.
   *
   * <p>Note: This is not an inherent limit in the {@link IndexSegment} but rather a limit in the
   * {@link IndexSegmentPlan} (and perhaps the {@link IndexSegmentBuilder}) which presumes that the
   * entry count is an <code>int</code> rather than a <code>long</code>.
   */
  ...
  /** The file mode used to open the file on which the {@link IndexSegment} is written. */
  final String mode = "rw"; // also rws or rwd

  /** The file specified by the caller on which the {@link IndexSegment} is written. */
  ...
  /*
   * The iterator specified to the ctor. This is the source for the keys and values that will be
   * written onto the generated {@link IndexSegment}.
   */
  private final ITupleIterator<?> entryIterator;

  /*
   * The commit time associated with the view from which the {@link IndexSegment} is being generated
   * (from the ctor). This value is written into {@link IndexSegmentCheckpoint#commitTime}.
   */
  public final long commitTime;

  /*
   * <code>true</code> iff the generated {@link IndexSegment} will incorporate all state for the
   * source index (partition) as of the specified <i>commitTime</i>.
   *
   * <p>Note: This flag is written into the {@link IndexSegmentCheckpoint} but it has no other
   * effect on the build process.
   */
  public final boolean compactingMerge;

  /** The name of the index or index partition for which the build is being performed. */
  final String name;

  /*
   * A copy of the metadata object provided to the ctor. This object is further modified before
   * being written on the {@link IndexSegmentStore}.
   */
  public final IndexMetadata metadata;

  /*
   * <code>true</code> iff the source index is isolatable (supports both deletion markers and
   * version timestamps).
   */
   ...
   *
   * <p>Note: delete markers are ONLY copied for an incremental build (when {@link #compactingMerge}
   * is <code>false</code>).
   */
  final boolean deleteMarkers;

  /** <code>true</code> iff the source index has tuple revision timestamps enabled. */
  final boolean versionTimestamps;

  /*
   * <code>true</code> iff the source index has raw records enabled.
   *
   * <p>Note: raw records will be copied into the BLOBS region of the index segment and the address
   * of the raw record in the output tuple will be updated to reflect the relative address of the
   * record within the index segment.
   */
  ...
  /*
   * The cache for the generated {@link IndexSegmentStore}. When non- <code>null</code> the
   * generated {@link INodeData} objects will be placed into the cache, which is backed by a shared
   * LRU. This helps to reduce latency when an index partition built or merge operation finishes and
   * the index partition view is updated since the data will already be present in the cache.
   * Generating the index segment will drive evictions from the shared LRU, but those will be the
   * least recently used records and the new {@link IndexSegmentStore} is often hot as soon as it is
   * generated.
   *
   * <p>Note: If the build fails, then the cache will be cleared.
   *
   * @todo The {@link IndexMetadata} and the {@link BloomFilter} should be in the {@link
   *     #storeCache} as well. Make sure that we do this for read and write for both the {@link
   *     BTree} and the {@link IndexSegment}.
   */
  /*
  ...
  /*
   * Note: The offset bits on the {@link IndexSegmentFileStore} does NOT have to agree with the
   * offset bits on the source store. However, it must be large enough to handle the large branching
   * factors typically associated with an {@link IndexSegment} vs a {@link BTree}. Further, if blobs
   * are to be copied into the index segment then it generally must be large enough for those blobs
   * (up to 64M per record).
   *
   * <p>Note: The same #of offset bits MUST be used by the temporary stores that we use to buffer
   * nodes, leaves, and blobs as are used by the generated index segment!
   */
  final int offsetBits = WormAddressManager.SCALE_OUT_OFFSET_BITS;

  /*
   * The {@link IAddressManager} used to form addresses for the generated file. Addresses are formed
   * from a byteCount and an <em>encoded</em> offset comprised of a relative offset into a known
   * region and the region identifier.
   *
  ...
  /*
   * When <code>true</code> record level checksums will be used in the generated file.
   *
   * <p>FIXME This can not be enabled until we factor out the direct use of the {@link WriteCache}
   * since special handling is otherwise required to ensure that the checksum makes it into the
   * output record when we write directly on the disk.
   *
   * <p>FIXME When enabling this, make sure that the bloom filter, {@link IndexMetadata}, and the
   * blobs are all checksummed and make sure that the {@link IndexSegmentStore} verifies the
   * checksums when it reads through to the disk and only returns the raw record w/o the trailing
   * checksum.
   *
   * <p>FIXME The right time to reconcile these things may be when this branch (HAJournal) is merged
   * with the dynamic shard refactor branch.
   */
  private final boolean useChecksums = false;

  /** Used to compute record level checksums when {@link #useChecksums} is <code>true</code>. */
  private final ChecksumUtility checker = new ChecksumUtility();

  /*
   * The file on which the {@link IndexSegment} is written. The file is closed regardless of the
   * outcome of the operation.
   */
  protected RandomAccessFile out = null;

  /** The {@link IndexSegmentCheckpoint} record written on the {@link IndexSegmentStore}. */
...
//     * The buffer used to hold leaves so that they can be evicted en mass onto a
  ...
  /*
   * This is used to buffer the leaves written onto the output file for greater efficiency.
   *
   * <p>FIXME Use a WriteCacheService which will hide this complexity and give better throughput.
   */
  private WriteCache.FileChannelWriteCache leafWriteCache;

  /*
   * Class combines the address at which a node is written onto the output file (relative to the
   * start of the nodes region) with the coded data record for the node.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  ...
  /*
   * The buffer used to hold nodes so that they can be evicted en mass onto a region of the {@link
   * #outFile}. This is conditionally enabled depending on whether #bufferNodes is true.
   */
  private TemporaryRawStore nodeBuffer;

  /*
   * When the nodes are to be fully buffered they are added into this list in the order in which
   * they are generated.
   */
  private List<NodeMetadata> nodeList;

  /*
   * When <code>true</code> the generated nodes will be fully buffered in RAM. Otherwise they will
   * be buffered on the {@link #nodeBuffer} and then transferred to the output file en mass.
   */
  protected final boolean bufferNodes;

  /*
   * The optional buffer used to hold records referenced by index entries. This is opened if the
   * index uses raw records -or- if the index specifies and {@link IOverflowHandler}.
   */
  ...
  /*
   * The encoded address of the first leaf written on the {@link IndexSegmentStore} (there is always
   * at least one, even if it is the root leaf).
   *
   * <p>Note: A copy of this value is preserved by {@link IndexSegmentCheckpoint#addrFirstLeaf}.
   */
  private long addrFirstLeaf = 0L;

  /*
   * The encoded address of the last leaf written on the {@link IndexSegmentStore} (there is always
   * at least one, even if it is the root leaf).
   *
   * <p>Note: A copy of this value is preserved by {@link IndexSegmentCheckpoint#addrLastLeaf}.
   */
...
//     * The size in bytes of the last leaf written onto the output file (the size
  ...
  /*
   * Tracks the maximum length of any serialized node or leaf. This is used to fill in one of the
   * {@link IndexSegmentCheckpoint} fields.
   */
  ...
  /*
   * The #of nodes written for the output tree. This will be zero if all entries fit into a root
   * leaf.
   */
  int nnodesWritten = 0;
  ...
  /*
   * The #of nodes or leaves that have been written out in each level of the tree.
   *
   ...
   * always nodes while the last element is always a leaf ({@link #leaf} is the same reference as
   * the last element in this array). The nodes and the leaf in this array are reused rather than
   * being reallocated.
   */
  final AbstractSimpleNodeData[] stack;

  /*
   * The current leaf that is being populated from the source btree. This leaf is reused for each
   * output leaf rather than being reallocated. In the degenerate case when the output btree is a
   * single root leaf then this will be that leaf. This reference is always the same as the last
  ...
  /*
   * The timestamp in milliseconds when {@link #call()} was invoked -or- ZERO (0L) if {@link
   * #call()} has not been invoked.
   */
  ...
  /*
   * The time to setup the index build, including the generation of the index plan and the
   * initialization of some helper objects.
   */
  public final long elapsed_setup;

  /*
   * The time to write the nodes and leaves into their respective buffers, not including the time to
   * transfer those buffered onto the output file.
   */
  public long elapsed_build;

  /*
   * The time to write the nodes and leaves from their respective buffers onto the output file and
   * synch and close that output file.
   */
  ...
  /*
   * Builder factory will build an {@link IndexSegment} from an index (partition). Delete markers
   * are propagated to the {@link IndexSegment} unless <i>compactingMerge</i> is <code>true</code>.
   *
   * @param src A view of the index partition as of the <i>createTime</i>. When
   *     <i>compactingMerge</i> is <code>false</code> then this MUST be a single {@link BTree} since
   *     incremental builds are only support for a {@link BTree} source while compacting merges are
   *     defined for any {@link IIndex}.
   * @param outFile The file on which the {@link IndexSegment} will be written. The file MAY exist,
   *     but if it exists then it MUST be empty.
   ...
   *     will therefore be propagated to the new {@link IndexSegment} (aka an incremental build).
   * @param createTime The commit time associated with the view from which the {@link IndexSegment}
   ...
   *     is no lower bound.
   * @param toKey The first key that will be included (exclusive). When <code>null</code> there is
   *     no upper bound.
  ...
  public static IndexSegmentBuilder newInstance(
    ...
    if (tmpDir == null) throw new IllegalArgumentException();
     ...
     * cache.
     */
    if (false && stats.sumSegBytes < MAX_SIZE_ON_DISK && fastRangeCount < MAX_TUPLES_IN_VIEW) {
  ...
  /*
   * A two pass build algorithm. The first pass is used to obtain an exact entry count for the view.
   * Based on that exact range count we can compute a plan for a balanced B+Tree. A second pass over
   * the view is required to populate the output B+Tree. This flavor also buffers the leaves and
   * nodes on temporary stores, which means that it does more IO. However, this version is capable
   * of processing very large source views.
   */
  protected static IndexSegmentBuilder newInstanceTwoPass(
    ...
    final ITupleIterator<?> itr =
        src.rangeIterator(fromKey, toKey, 0 /* capacity */, flags, null /* filter */);
  ...
  /*
   * A one pass algorithm which materializes the tuples in RAM, computing the exact tuple count as
   * it goes. This is faster than the two-pass algorithm and is a better choice when the source view
   * and the output index segment are within the normal ranges for an index partition, e.g., an
   * output index segment file of ~200M on the disk.
   *
   * <p>FIXME The unit tests need to run against both builds based on the materialized tuples and
   * builds based on two passes in order to obtain the exact range count. They already do for {@link
   * TestIndexSegmentBuilderWithLargeTrees} but not yet for the other test suite variants.
   */
  protected static IndexSegmentBuilder newInstanceFullyBuffered(
    ...
    final boolean hasVersionTimestamps = indexMetadata.getVersionTimestamps();
    ...
    final MutableLeafData tleaf =
        new MutableLeafData(
            (int) fastRangeCount, hasVersionTimestamps, hasDeleteMarkers, hasRawRecords);
    ...
    final ITupleIterator<?> titr =
        src.rangeIterator(
            fromKey, toKey, (int) fastRangeCount /* capacity */, flags, null /* filter */);
  ...
  /*
   * Variant using an array of objects in the desired order. A single root leaf is generated from
   * those objects. The root leaf is then fed into the algorithm to efficient construct the
   * corresponding read-only {@link IndexSegment}.
   *
   * @param a The array of objects to be written onto the index. The index must know how to generate
   *     tuples from these objects. The objects must already be in the natural order of the keys
   *     that will be generated for those tuples.
   * @param alen The #of elements in that array.
   * @param indexMetadata The {@link IndexMetadata} that will serve as the template for the
   *     generated {@link IndexSegment}.
   * @param outFile The file on which the {@link IndexSegment} will be written. The file MAY exist,
   *     but if it exists then it MUST be empty.
   * @param tmpDir The temporary directory in data are buffered during the build (optional - the
   *     default temporary directory is used if this is <code>null</code>).
   * @param m The branching factor for the generated {@link IndexSegment}.
   * @param compactingMerge When <code>true</code> the caller asserts that <i>src</i> is a {@link
   *     FusedView} and deleted index entries WILL NOT be included in the generated {@link
   *     IndexSegment}. Otherwise, it is assumed that the only select component(s) of the index
   *     partition view are being exported onto an {@link IndexSegment} and deleted index entries
   ...
   * @param createTime The commit time associated with the view from which the {@link IndexSegment}
   *     is being generated. This value is written into {@link IndexSegmentCheckpoint#commitTime}.
   * @param bufferNodes When <code>true</code> the generated nodes will be fully buffered in RAM
   *     (faster, but imposes a memory constraint). Otherwise they will be written onto a temporary
   *     file and then transferred to the output file en mass.
   * @return
   * @throws IOException
   *     <p>TODO We could pass a flag indicating whether the leaf needs to be sorted after it is
   *     generated, but the caller would still be responsible for ensuring that there are no
  ...
  public static IndexSegmentBuilder newInstance(
    ...
    if (alen > a.length) throw new IllegalArgumentException();
    ...
    // A temporary leaf used to buffer the data in RAM.
    final MutableLeafData tleaf =
        new MutableLeafData(alen, hasVersionTimestamps, hasDeleteMarkers, hasRawRecords);
  ...
  /*
   * A more flexible factory for an {@link IndexSegment} build which permits override of the index
   * segment branching factor, replacement of the {@link IndexMetadata}, and the use of the caller's
   * iterator.
   *
   * <p>Note: The caller must determine whether or not deleted index entries are present in the
   * view. The <i>entryCount</i> MUST be the exact #of index entries that are visited by the given
   * iterator. In general, this is not difficult. However, if a compacting merge is desired (that
   * is, if you are trying to generate a view containing only the non-deleted entries) then you MUST
   * explicitly count the #of entries that will be visited by the iterator, e.g., it will require
   * two passes over the iterator to setup the index build operation.
   *
   * <p>Note: With a branching factor of 4096 a tree of height 2 (three levels) could address
   * 68,719,476,736 entries - well beyond what we want in a given index segment! Well before that
   * the index segment should be split into multiple files. The split point should be determined by
   * the size of the serialized leaves and nodes, e.g., the amount of data on disk required by the
   * index segment and the amount of memory required to fully buffer the index nodes. While the size
   * of a serialized node can be estimated easily, the size of a serialized leaf depends on the
   * kinds of values stored in that index. The actual sizes are recorded in the {@link
   * IndexSegmentCheckpoint} record in the header of the {@link IndexSegment}.
   *
   * @param outFile The file on which the index segment is written. The file MAY exist but MUST have
   *     zero length if it does exist (this permits you to use the temporary file facility to create
   *     the output file).
   * @param tmpDir The temporary directory in data are buffered during the build (optional - the
   *     default temporary directory is used if this is <code>null</code>).
   * @param entryCount The #of entries that will be visited by the iterator. This MUST be an exact
   *     range count.
   * @param entryIterator Visits the index entries in key order that will be written onto the {@link
   *     IndexSegment}.
   * @param m The branching factor for the generated tree. This can be chosen with an eye to
   *     minimizing the height of the generated tree. (Small branching factors are permitted for
   *     testing, but generally you want something relatively large.)
   * @param metadata The metadata record for the source index. A copy will be made of this object.
   *     The branching factor in the generated tree will be overridden to <i>m</i>.
   * @param commitTime The commit time associated with the view from which the {@link IndexSegment}
   *     is being generated. This value is written into {@link IndexSegmentCheckpoint#commitTime}.
   * @param compactingMerge <code>true</code> iff the generated {@link IndexSegment} will
   *     incorporate all state for the source index (partition) as of the specified
   *     <i>commitTime</i>. This flag is written into the {@link IndexSegmentCheckpoint} but does
   *     not otherwise effect the build process.
   * @param bufferNodes When <code>true</code> the generated nodes will be fully buffered in RAM
   *     (faster, but imposes a memory constraint). Otherwise they will be written onto a temporary
   *     file and then transferred to the output file en mass.
   * @throws IOException
  ...
  public static IndexSegmentBuilder newInstance(
    ...
    return new IndexSegmentBuilder(
        outFile,
        tmpDir,
        entryCount,
        entryIterator,
  ...
  /*
   * Designated constructor sets up a build of an {@link IndexSegment} for some caller defined
   * read-only view.
   *
   * <p>Note: The caller must determine whether or not deleted index entries are present in the
   * view. The <i>entryCount</i> MUST be the exact #of index entries that are visited by the given
  ...
  protected IndexSegmentBuilder(
    ...
    if (entryCount == 0 && log.isInfoEnabled()) log.info(ERR_NO_TUPLES);

    if (entryIterator == null) throw new IllegalArgumentException();

    if (commitTime <= 0L) throw new IllegalArgumentException();
    ...
    name =
        (metadata.getPartitionMetadata() == null)
            ...
            : metadata.getName() + "#" + metadata.getPartitionMetadata().getPartitionId();
     ...
     * B+Tree.
     */
    this.recordAddrBuf = rawRecords ? new ByteArrayBuffer(Bytes.SIZEOF_LONG) : null;
     ...
     */
    {
      ...
      for (int h = 0; h < plan.height; h++) {
        ...
        final SimpleNodeData node = new SimpleNodeData(h, plan.m, versionTimestamps);
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info(
          "name=" + name + ", nentries=" + entryCount + ", compactingMerge=" + compactingMerge);
    }
  }

  /** Build the {@link IndexSegment} given the parameters specified to the constructor. */
  public IndexSegmentCheckpoint call() throws Exception {
    ...
    if (outFile.exists() && outFile.length() != 0L) {
      ...
      throw new IllegalArgumentException(
          "File exists and is not empty: " + outFile.getAbsoluteFile());
    }
    ...
    try {
       ...
       */
      blobBuffer =
          (rawRecords || overflowHandler != null) ? new TemporaryRawStore(offsetBits) : null;
      ...
      // data rate in MB/sec.
      mbPerSec = (elapsed == 0 ? 0 : checkpoint.length / Bytes.megabyte32 / (elapsed / 1000f));
  ...
  /*
   * Scan the source tuple iterator in key order writing output leaves onto the index segment file
   * with the new branching factor. We also track a stack of nodes that are being written out
   * concurrently on a temporary channel.
   *
   * <p>The plan tells us the #of values to insert into each leaf and the #of children to insert
   * into each node. Each time a leaf becomes full (according to the plan), we "close" the leaf,
   * writing it out onto the store and obtaining its "address". The "close" logic also takes care of
   * setting the address on the leaf's parent node (if any). If the parent node becomes filled
   * (according to the plan) then it is also "closed".
   *
   * <p>Each time (except the first) that we start a new leaf we record its first key as a
   * separatorKey in the appropriate parent node.
   *
   * <p>Note: The root may be a leaf as a degenerate case.
   *
   * @todo Verify correct rejection if the source iterator visits too many or too few tuples.
   */
   ...
   *
   * @param j The index in the leaf to which the tuple will be copied.
  ...
  private void copyTuple(final int j, final ITuple<?> tuple) {
    ...
    if (deleteMarkers) leaf.deleteMarkers[j] = tuple.isDeletedVersion();
    ...
    if (versionTimestamps) {
      ...
      if (t < leaf.minimumVersionTimestamp) leaf.minimumVersionTimestamp = t;

      if (t > leaf.maximumVersionTimestamp) leaf.maximumVersionTimestamp = t;
    }
    ...
    if (rawRecords) {
      ...
      if (val != null && val.length > maxRecLen) {
        ...
        final long addr =
            addressManager.toAddr(nbytes, IndexSegmentRegion.BLOB.encodeOffset(offset));
        // save its address in the values raba.
        leaf.vals.values[j] = AbstractBTree.encodeRecordAddr(recordAddrBuf, addr);
  ...
  /*
   * This is invoked for the first tuple visited to make sure that the iterator is reporting the
   * data we need.
   */
  private void assertIteratorOk(final ITuple<?> tuple) {

    if (!tuple.getKeysRequested()) throw new RuntimeException("keys not reported by itr.");

    if (!tuple.getValuesRequested()) throw new RuntimeException("vals not reported by itr.");

    if (!compactingMerge && deleteMarkers && ((tuple.flags() & IRangeQuery.DELETED) == 0)) {
     ...
     */
    assert !isolatable || (isolatable && ((tuple.flags() & IRangeQuery.DELETED) == 0))
   ...
   *
   * <p>Note: When a node or leaf is flushed we write it out to obtain its address and set that
   * address on its direct parent using {@link #addChild(SimpleNodeData, long,
   * AbstractSimpleNodeData, boolean)}. This also updates the per-child counters of the #of entries
   * spanned by a node.
   *
  ...
  /*
   * Record the persistent address of a child on its parent and the #of entries spanned by that
   * child. If all children on the parent become assigned then the parent is closed.
   *
   * @param parent The parent.
   * @param childAddr The address of the child (node or leaf).
   * @param child The child reference.
   */
  protected void addChild(
      final SimpleNodeData parent, final long childAddr, final AbstractSimpleNodeData child) {

    // #of entries spanned by this node.
    final long nentries =
        (child.isLeaf() ? child.getKeyCount() : ((INodeData) child).getSpannedTupleCount());
    ...
    if (versionTimestamps) {
      ...
      parent.minimumVersionTimestamp =
          Math.min(parent.minimumVersionTimestamp, child.minimumVersionTimestamp);

      parent.maximumVersionTimestamp =
          Math.max(parent.maximumVersionTimestamp, child.maximumVersionTimestamp);
    }
  ...
  /*
   * The {@link #stack} contains nodes which are reused for each node or leaf at a given level in
   * the generated B+Tree. This method prepares a node in the stack for reuse.
   */
  ...
  /*
   * Copies the first key of a new leaf as a separatorKey for the appropriate parent (if any) of
   * that leaf. This must be invoked when the first key is set on that leaf. However, it must not be
   * invoked on the first leaf.
   *
   * @param leaf The current leaf. The first key on that leaf must be defined.
   */
  ...
  /*
   * Copies the separatorKey into the appropriate parent (if any). This method is self-recursive.
   *
   * @param parent A node which is a parent of the current leaf or an ancestor of the node which is
   *     the parent of the current leaf (non-null).
   * @param separatorKey The separator key to be assigned to the parent (non-null).
   */
  private void addSeparatorKey(final SimpleNodeData parent, final byte[] separatorKey) {

    if (parent == null) throw new AssertionError();

    if (separatorKey == null) throw new AssertionError();
   ...
   * @return The parent or <code>null</code> iff <i>node</i> is the root node or leaf.
   */
   ...
   *
   * @return The address that may be used to read the node or leaf from the file. Note that the
   *     address of a node is relative to the start of the node channel and therefore must be
   *     adjusted before reading the node from the final index segment file.
   */
  protected long writeNodeOrLeaf(final AbstractSimpleNodeData node) {

    return node.isLeaf() ? writeLeaf((SimpleLeafData) node) : writeNode((SimpleNodeData) node);
  }

  /*
   * Code the leaf, obtaining its address, update the prior/next addr of the previous leaf, and
   * write that previous leaf onto the output file.
   *
   * <p>Note: For leaf addresses we know the absolute offset into the {@link IndexSegmentStore}
   * where the leaf will wind up so we encode the address of the leaf using the {@link
   * IndexSegmentRegion#BASE} region.
   *
   * <p>Note: In order to write out the leaves using a double-linked list with prior-/next-leaf
   * addresses we have to use a "write behind" strategy. Instead of writing out the leaf as soon as
   * it is serialized, we save the uncoded address and a copy of the coded data record on private
   * member fields. When we code the next leaf (or if we learn that we have no more leaves to code
   * because {@link IndexSegmentPlan#nleaves} EQ {@link #nleavesWritten}) then we patch the coded
   * representation of the prior leaf and write it on the store at the previously obtained address,
   * thereby linking the leaves together in both directions. It is definitely confusing.
   *
   * @return The address that may be used to read the leaf from the file backing the {@link
   *     IndexSegmentStore}.
   */
  protected long writeLeaf(final SimpleLeafData leaf) {
    ...
    final long addr;
    {
      ...
      if (log.isDebugEnabled())
        log.debug(
            "allocated storage for leaf data record" + ": addr=" + addressManager.toString(addr));
       ...
       * Update the previous leaf, but only for the 2nd+ leaf.
        ...
        if (log.isDebugEnabled())
          log.debug(
              "updating previous leaf"
                  + ": addr="
                  + addressManager.toString(encodeLeafAddr(bufLastLeafAddr))
                  + ", priorAddr="
                  + addressManager.toString(addrPriorLeaf)
                  + ", nextAddr="
                  + addressManager.toString(addr)
              //                            + ", exhausted=" + exhausted
              );
    ...
    if (plan.nleaves == nleavesWritten) { // ||*/ exhausted) {
      ...
      else if (log.isInfoEnabled()) System.err.print("."); // wrote a leaf.
  ...
  /*
   * The address factory for the leaves. Note that addresses are relative to the start of the leaf
   * region, not the start of the output file.
   */
  ...
  private void writeLeafForReal(final long addr, final ByteBuffer data) {
    ...
    try {
       ...
       * The leaf is larger than the write cache, so we will write
          ...
          FileChannelUtility.writeAll(leafWriteCache.opener, data, offset);
        }
      }
   ...
   *
   * @throws UnsupportedOperationException if we are not buffering nodes in RAM.
   */
  private long allocateNodeAddr(final int nbytes) {

    if (!bufferNodes) throw new UnsupportedOperationException();
  ...
  /*
   * The address factory for the nodes used when we will buffer the nodes in RAM. Note that
   * addresses are relative to the start of the node region, not the start of the output file.
   */
   ...
   *
   * <p>Note: This updates {@link #maxNodeOrLeafLength} as a side-effect.
   *
   * @param addr1 The address of a leaf as allocated by {@link #allocateLeafAddr(int)}
   * @return The encoded address of the leaf relative to the {@link IndexSegmentRegion#BASE} region
   *     where it will appear once the leaves have been copied onto the output file.
   */
  private long encodeLeafAddr(final long addr1) {
     ...
     */
    final long offset = addressManager.getOffset(addr1) + IndexSegmentCheckpoint.SIZE;

    // Encode the address of the leaf.
    final long addr = addressManager.toAddr(nbytes, IndexSegmentRegion.BASE.encodeOffset(offset));
  ...
  /** The address of the previous leaf, but encoded for the generated {@link IndexSegmentStore}. */
  private long addrPriorLeaf = 0L;

  /*
   * The address of the last leaf allocated (but not yet written out).
   *
   * <p>Note: This address is NOT encoded for the {@link IndexSegmentStore}. Instead, it is encoded
   * for the output file using the {@link #addressManager} and is relative to the start of leaves
   * region in the output file.
   *
...
//     * Buffer holds a copy of the serialized representation of the last leaf.
  ...
  /*
   * Buffer holds a copy of the coded representation of the last leaf. This buffer is written by
   * {@link #writeLeaf(SimpleLeafData)}. The contents of this buffer are used to write out the
   * serialized representation of the previous leaf in key order after it has been patched to
   * reflect the prior and next leaf addresses. The coded {@link ILeafData} record is modified
   * before the previous leaf is written out to reflect the address assigned to the next leaf in key
   * order.
   */
   ...
   *
   * @return An <em>relative</em> address that must be correctly decoded before you can read the
   *     compressed node from the file. This value is also set on {@link SimpleNodeData#addr}.
   * @see SimpleNodeData
   * @see IndexSegmentRegion
   * @see IndexSegmentAddressManager
   */
  protected long writeNode(final SimpleNodeData node) {
    ...
    if (log.isInfoEnabled()) System.err.print("x"); // wrote a node.
     ...
     */
    final long addr = addressManager.toAddr(nbytes, IndexSegmentRegion.NODE.encodeOffset(offset));
   ...
   *
   * <ol>
   *   <li>fixed length {@link IndexSegmentCheckpoint} record (required)
   *   <li>leaves (required)
   *   <li>nodes (may be empty)
   *   <li>the bloom filter (optional)
   *   <li>the {@link IndexMetadata} record (required, but extensible)
   * </ol>
   *
   * <p>The index segment metadata is divided into a base {@link IndexSegmentCheckpoint} record with
   * a fixed format containing only essential data and additional metadata records written at the
   * end of the file including the optional bloom filter and the required {@link IndexMetadata}
   * record. The latter is where we write variable length metadata including the _name_ of the
   * index, or additional metadata defined by a specific class of index.
   *
   * <p>Once all nodes and leaves have been buffered we are ready to start writing the data. We skip
   * over a fixed size metadata record since otherwise we are unable to pre-compute the offset to
   * the leaves and hence the addresses of the leaves. The node addresses are written in an encoding
   * that requires active translation by the receiver who must be aware of the offset to the start
   * of the node region. We can not write the metadata record until we know the size and length of
   * each of these regions (leaves, nodes, and the bloom filter, or other metadata records) since
   * that information is required in order to be able to form their addresses for insertion in the
   * metadata record.
   *
   * @param outChannel
   * @param commitTime
   ...
   *     <p>FIXME There is no sense of an atomic commit when building a new index segment. We should
   *     write ZEROs into the checkpoint record initially and then seek back to the head of the file
   *     once we are done and write out the correct checkpoint record.
   *     <p>Note: There are similar issues involved when we replicate index segment or journal files
   *     to verify that they are good.
   */
  protected IndexSegmentCheckpoint writeIndexSegment(
    ...
    if (plan.nleaves == 0) {
       ...
       * Seek to the start of the nodes region (the write cache does
        ...
        assert outChannel.position() == offsetNodes
            : "position=" + outChannel.position() + ", but offsetNodes=" + offsetNodes;
       ...
       * Write the nodes onto the output file.
        ...
        // Setup a write cache.
        final WriteCache.FileChannelWriteCache writeCache =
            new WriteCache.FileChannelWriteCache(
                offsetNodes,
                null /* buf */,
                useChecksums,
        ...
        try {
          ...
          for (NodeMetadata md : nodeList) {
            ...
            final AbstractFixedByteArrayBuffer slice = md.data.data();
    ...
    if (log.isInfoEnabled())
      log.info("addrRoot: " + addrRoot + ", " + addressManager.toString(addrRoot));
    ...
    if (bloomFilter == null) {
      ...
      addrBloom =
          addressManager.toAddr(bloomBytes.length, IndexSegmentRegion.BASE.encodeOffset(offset));
    ...
    final long addrMetadata;
    {
      ...
      addrMetadata =
          addressManager.toAddr(metadataBytes.length, IndexSegmentRegion.BASE.encodeOffset(offset));
    ...
    //             */
    //            final byte[] icuVersionBytes =
    // SerializerUtil.serialize(ICUVersionRecord.newInstance());
    //
    //            // #of bytes written so far on the output file.
    //            final long offset = out.length();
    //
     ...
     */
    {
       ...
       */
      if (nnodesWritten > Integer.MAX_VALUE) throw new AssertionError();
      if (nleavesWritten > Integer.MAX_VALUE) throw new AssertionError();

      final IndexSegmentCheckpoint md =
      ...
      if (log.isInfoEnabled()) log.info(md.toString());
   ...
   *
   * @throws IllegalStateException if requested before the build operation is complete.
   */
  ...
  /*
   * Abstract base class for classes used to construct and serialize nodes and leaves written onto
   * the index segment.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  protected abstract static class AbstractSimpleNodeData implements IAbstractNodeData {

    /*
     * The level in the output tree for this node or leaf (origin zero). The root is always at level
     * zero (0).
     */
    final int level;
    ...
    /*
     * The max/max version timestamp for the node/leaf. These data are only used when the B+Tree is
     * maintaining per tuple revision timestamps.
     */
    ...
    /*
     * We precompute the #of children to be assigned to each node and the #of values to be assigned
     * to each leaf and store that value in this field. While the field name is "max", this is the
     * exact that must be assigned to the node.
     */
    int max = -1;
    ...
    /*
     * @param max The #of children to be assigned to this node -or- the #of tuples to be assigned to
     *     a leaf.
     */
    ...
    /*
     * Yes (however, note that the {@link IndexSegmentBuilder} directly accesses and modified the
  ...
  /*
   * A class that can be used to (de-)serialize the data for a leaf without any of the logic for
   * operations on the leaf.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  protected static class SimpleLeafData extends AbstractSimpleNodeData implements ILeafData {

    /** The values stored in the leaf (directly accessed by the {@link IndexSegmentBuilder}). */
    ...
    public SimpleLeafData(final int level, final int m, final IndexMetadata metadata) {
    ...
    /*
     * Yes - the caller maintains the necessary information and then updates the coded {@link
     * ReadOnlyLeafData} record once we have the address of the next record.
     */
    ...
    /*
     * @throws UnsupportedOperationException since the data are maintained externally and patched on
  ...
  /*
   * A class that can be used to (de-)serialize the data for a node without any of the logic for
   * operations on the node.
   *
   * <p>Note: All node addresses that are internal to a node and reference a child node (vs a leaf)
   * are correct relative to the start of the {@link IndexSegmentRegion#NODE} region. This is an
   * unavoidable consequence of serializing the nodes before we have the total offset to the start
   * of the {@link IndexSegmentRegion#NODE} region.
   *
   * @see IndexSegmentRegion
   * @see IndexSegmentAddressManager
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  protected static class SimpleNodeData extends AbstractSimpleNodeData implements INodeData {

    // mutable.

    /*
     * The relative address at which this node was written on the temporary channel. This is a
     * negative integer. If you flip the sign then it encodes a relative offset to the start of the
     * index node block and the correct size for the compressed node.
     */
    long addr = 0L;

    /*
     * The address at which the child nodes were written. This is a negative integer iff the child
     * is a node and a positive integer iff the child is a leaf. When it is a negative integer, you
     * must flip the sign to obtain a relative offset to the start of the index node block and the
     * correct size for the compressed node. The actual offset of the index node block must be added
     * to the relative offset before you can use this to address into the output file.
     */
    final long[] childAddr;

    /*
     * This tracks the #of defined values in {@link #childAddr} separately from the #of defined
     * keys. The reason that we do this is that the logic for handling a leaf eviction and recording
     * the address of the child and the separator key for the _next_ child requires an awareness of
     * the intermediate state - when we have filled in the childAddr for the last leaf but not yet
     * filled in the separatorKey for the next leaf.
     */
    int nchildren = 0;
    ...
    /** <code>true</code> iff the node is tracking the min/max tuple revision timestamps. */
    ...
    public final long getChildAddr(final int index) {
      ...
      if (index < 0 || index > keys.size() + 1) throw new IllegalArgumentException();
    ...
    public SimpleNodeData(final int level, final int m, final boolean hasVersionTimestamps) {
     ...
     *
     * @param max The new limit on the #of children to fill on this node.
     */
    protected void reset(final int max) {
  ...
  protected static class NOPNodeFactory implements INodeFactory {
    ...
    public Leaf allocLeaf(final AbstractBTree btree, final long addr, final ILeafData data) {

      throw new UnsupportedOperationException();
    }

    public Node allocNode(final AbstractBTree btree, final long addr, final INodeData data) {
  ...
  /*
   * A tuple iterator backed by a mutable leaf. This implementation is used when we materialize the
   * view in RAM in a single leaf and then do the build over that. The implementation always returns
   * the pre-allocated byte[] for the key or value in order to avoid redundant allocations. This is
   * safe since the data in that {@link MutableLeafData} instance were allocated when the view was
   * materialized and their references can be safely reused when we build the output B+Tree.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @param <E> The generic type of the objects which would be materialized from the tuples.
   * @todo in fact, we could clear the references from the {@link MutableLeafData} as we go.
   */
  private static class MyTupleIterator<E> implements ITupleIterator<E> {
    ...
    private class MyTuple implements ITuple<E> {
      ...
      public MyTuple(final int flags) {
        ...
        if (!needsKeys) throw new UnsupportedOperationException();
        if (!needsVals) throw new UnsupportedOperationException();
      }
      ...
      public boolean isDeletedVersion() {
        if (!hasDeleteMarkers) return false;
      ...
      public long getVersionTimestamp() {
        if (!hasVersionTimestamp) return 0L;
    ...
    /*
     * Examines the entry at {@link #i}. If it passes the criteria for an entry to visit then return
     * true. Otherwise increment the {@link #i} until either all entries in this leaf have been
     * exhausted -or- the an entry is identified that passes the various criteria.
     */
  ...
  enum BuildEnum {
    ...
    /*
     * Do two passes over the source iterator. The first pass will compute the exact range count.
     * The second pass will build the {@link IndexSegment}. The two pass algorithm uses less memory
     * and can be highly efficient when using {@link IndexSegmentMultiBlockIterator} since it will
     * read the data from any source {@link IndexSegment}(s) at the disk transfer rate.
     */
    TwoPass,
    /*
  ...
  protected static void usage(final String[] args, final String msg, final int exitCode) {
    ...
    System.err.println(
        "       -merge (true|false)\tWhen true, performs a compacting merge (default is merge).");
    System.err.println("       -O outDir\tThe output directory.");
    System.err.println(
        "       -bufferNodes (true|false)\tWhen true, the nodes are fully buffered in memory (default true).");
   ...
   *
   * @param args <code>[opts] journal [name]*</code>, where <i>journal</i> is the name of the
   *     journal file, where <i>name</i> is the name of a B+Tree registered on that journal, and
   *     where <i>opts</i> are any of:
   *     <dl>
   *       <dt>-m #
   *       <dd>Override the default branching factor for the index segment.
   *       <dt>-alg <i>algorithm</i>
   *       <dd>Specify which build algorithm to use. See {@link BuildEnum}.
   *       <dt>-merge or -build
   *       <dd>Specifies whether to do a compacting merge (deleted tuples are purged from the
   *           generated index segment) or an incremental build (deleted tuples are preserved). The
   *           default is <i>merge</i>.
   *       <dt>-O outDir
   *       <dd>Specify the name of the directory on which the generated index segment file(s) will
   *           be written. This defaults to the current working directory. Each index segment file
   *           will be named based on the name of the source index with the <code>.seg</code>
   *           extension). .
   *     </dl>
   *     . If no <i>name</i>s are specified, then an index segment will be generated for each named
   *     B+Tree registered on the source journal.
   * @throws Exception
   */
  //    *            <dt>-verify</dt>
  ...
  public static void main(final String[] args) throws Exception {
     ...
     */
    {
      ...
      if (!outDir.exists() && !outDir.mkdirs()) {
        ...
        throw new IOException(
            "Output directory does not exist and could not be created: " + outDir);
      }
    } // parse command line.
    ...
    try {
      ...
      if (names.isEmpty()) {
        ...
        final ITupleIterator<Name2Addr.Entry> itr = journal.getName2Addr().rangeIterator();
        ...
        for (String name : names) {
          ...
          final File outFile = new File(outDir, name + Journal.Options.SEG);
      ...
      for (String name : names) {
        ...
        final int m =
            branchingFactorOverride == null
                ? btree.getIndexMetadata().getIndexSegmentBranchingFactor()
        ...
        switch (buildEnum) {
          case TwoPass:
            builder =
                IndexSegmentBuilder.newInstanceTwoPass(
                    btree,
                    outFile,
                    tmpDir,
                    m,
       ...
       * Verify the generated index segment against the source
          ...
          try {
            ...
            try {
              ...
              assertSameEntryIterator(name, btree.rangeIterator(), seg.rangeIterator());
  ...
  private static void assertSameEntryIterator(
      final String name, final ITupleIterator<?> expectedItr, final ITupleIterator<?> actualItr) {
    ...
    if (actualItr.hasNext()) throw new RuntimeException(name + ":: Not expecting more tuples");
  }
}
