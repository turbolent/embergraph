...
public abstract class PageStats extends BaseIndexStats {
  ...
  /** The #of allocations that are larger than the maximum slot size in {@link #SLOT_SIZES}. */
  public long blobs;
   ...
   *
   * @return
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/592">Optimize RWStore allocator
   *     sizes </a>
   *     <p>TODO This says "branching factor", but {@link #m} is overloaded and is the branching
   *     factor for the BTree and the addressBits for the HTree. This method should be renamed (or
   *     moved into the concrete instances of this class).
   */
   ...
   * #ntuples}, {@link #nnodes}, {@link #nleaves}, and {@link #m} fields.
   *
   * @param ndx The index.
   * @param node A node or leaf in that index.
   */
  public void visit(final ISimpleTreeIndexAccess ndx, final IAbstractNodeData node) {

    final PageStats stats = this;

    if (stats.nvisited == 0) {

      stats.name = ((ICheckpointProtocol) ndx).getIndexMetadata().getName();

      stats.indexType = ((ICheckpointProtocol) ndx).getCheckpoint().getIndexType();
    }
    ...
    if (po.isPersistent()) {
      ...
      if (isLeaf) {
        ...
        if (stats.minLeafBytes > nbytes || stats.minLeafBytes == 0) stats.minLeafBytes = nbytes;
        if (stats.maxLeafBytes < nbytes) stats.maxLeafBytes = nbytes;
          ...
          if (data.hasRawRecords()) {
            for (int i = 0; i < data.getKeys().size(); i++) {
              ...
              if (rawAddr != IRawStore.NULL) {
                stats.nrawRecs++;
                stats.rawRecBytes += store.getByteCount(rawAddr);
              }
            }
          }
        }
        ...
        if (stats.minNodeBytes > nbytes || stats.minNodeBytes == 0) stats.minNodeBytes = nbytes;
        if (stats.maxNodeBytes < nbytes) stats.maxNodeBytes = nbytes;
      }
    } // if(isPersistent())
