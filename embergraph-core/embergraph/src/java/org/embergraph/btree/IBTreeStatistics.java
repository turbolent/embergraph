...
public interface IBTreeStatistics {
   ...
   * is means for the btree to be "balanced"). Also note that the height only changes when we split
   * or join the root node (a btree maintains balance by growing and shrinking in levels from the
   * top rather than the leaves).
   */
  int getHeight();

  /** The #of non-leaf nodes in the {@link AbstractBTree}. This is zero (0) for a new btree. */
  long getNodeCount();
