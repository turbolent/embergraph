 ...
 * load the {@link BTree} from the {@link Checkpoint} record and you can start using the index.
 *
...
public class Checkpoint implements ICheckpoint, Externalizable {
  ...
  /** A human readable representation of the state of the {@link Checkpoint} record. */
  @Override
  public final String toString() {
    ...
    return "Checkpoint"
        ...
        + (indexType == IndexTypeEnum.BTree
            ? ",height=" + height
            : (indexType == IndexTypeEnum.HTree ? ",globalDepth=" + height : ""))
        + ",nnodes="
        + nnodes
        + ",nleaves="
        + nleaves
   ...
   *
   * @param metadata The index metadata record.
   */
   ...
   *
   * @param metadata The index metadata record.
   * @param oldCheckpoint The last {@link Checkpoint} for the index on the old backing store. The
   *     {@link Checkpoint#counter} is propagated to the new {@link Checkpoint} record.
   */
   ...
   *
   * <ul>
   *   <li>The root is clean.
   *   <li>The metadata record is clean.
   *   <li>The optional bloom filter is clean if it is defined.
   * </ul>
   *
   * Note: if the root is <code>null</code> then the root is assumed to be clean and the root
   * address from the last {@link Checkpoint} record is used. Otherwise the address of the root is
   * used (in which case it MUST be defined).
   *
   * <p>Note: <strong>This method is invoked by reflection.</strong>
   *
  ...
  public Checkpoint(final BTree btree) {
    ...
    this(
         ...
         */
        (btree.bloomFilter == null
            ? btree.getCheckpoint().getBloomFilterAddr()
            : btree.bloomFilter.isEnabled() ? btree.bloomFilter.getAddr() : 0L),
        btree.height,
        btree.nnodes,
        btree.nleaves,
        btree.nentries,
   ...
   * to support the possibility that we might do an in place update of a {@link Checkpoint} record
   * as part of a data migration strategy. For the same reason, the {@link Checkpoint} record
   * includes some unused fields. Those fields are available for future version changes without
   * requiring us to change the length of the {@link Checkpoint} record.
   */
   ...
   * <dl>
   *   <dt>nodeCount
   *   <dd>Changed from int32 to int64.
   *   <dt>leafCount
   *   <dd>Changed from int32 to int64.
   *   <dt>entryCount
   *   <dd>Changed from int32 to int64.
   *   <dt>recordVersion
   *   <dd>Added a new field which record the <em>next</em> record version identifier to be used
   *       when the next node or leaf data record is written onto the backing store. The field
   *       provides sequential numbering of those data record which can facilitate certain kinds of
   *       forensics. For example, all updated records falling between two checkpoints may be
   *       identified by a file scan filtering for the index UUID and a record version number GT the
   *       last record version written for the first checkpoint and LTE the last record version
   *       number written for the second checkpoint. The initial value is ZERO (0).
   * </dl>
   *
   * In addition, the <strong>size</strong> of the checkpoint record has been increased in order to
   * provide room for future expansions.
   */
   ...
   *
   * @param store
   * @throws IllegalStateException if the {@link Checkpoint} record has already been written.
   */
  public final void write(final IRawStore store) {
   ...
   * @param addrCheckpoint The address from which to read the {@link Checkpoint} record. This
   *     address is set on the {@link Checkpoint} record as a side-effect.
   */
  ...
  public void writeExternal(final ObjectOutput out) throws IOException {
    ...
    if (currentVersion <= VERSION1) {
      ...
      if (nnodes > Integer.MAX_VALUE) throw new RuntimeException();
      if (nleaves > Integer.MAX_VALUE) throw new RuntimeException();
   ...
   *
   * <p><strong>Note: The caller is responsible for marking the returned object as read-only or not
   * depending on the context. This method should be used from trusted code such as {@link
   * Name2Addr} and {@link AbstractJournal} which can make this decision.</strong>
   *
   * @param store The backing store.
   * @param checkpointAddr The address of the checkpoint record.
   * @param readOnly <code>true</code> if the object will be read-only.
   * @return The persistence capable data structure loaded from that checkpoint.
   */
  public static ICheckpointProtocol loadFromCheckpoint(
      final IRawStore store, final long checkpointAddr, final boolean readOnly) {
