 ...
 *
 * <p>Note: Counters for mutation are plain fields. Counters for read-only operations are {@link
 * AtomicLong} or {@link AtomicInteger} objects since read operations may involve high concurrently
 * and could otherwise lead to lost counter updates.
 *
 * @todo {@link #add(BTreeCounters)}, {@link #subtract(BTreeCounters)} and the copy ctor are not
 *     atomic. I have not seen any problems resulting from this.
 * @todo instrument the key search time
 * @todo instrument bloom filter time when the bloom filter is used (this should be done separately
 *     on the bloom filter object).
 * @todo (I should recheck these costs now that that refactor has been done). At present,
 *     deserialization is much slower than serialization, primarily because of object creation.
 *     Changing to a raw record format for nodes and leaves would likely reduce the deserialization
 *     costs significantly. The time could also be reduced by lazy decompression of the values and
 *     lazy deserialization of the values such that tuple scans for keys only do not require value
 *     deserialization.
...
public final class BTreeCounters implements Cloneable, ICounterSetAccess {
   ...
   * write set and are counted here.
   */
   ...
   * persistence store reclaims the storage associated with deleted nodes and leaves.
   *
   ...
   *
   * <p>The score is the serialization / deserialization time plus the read / write time. Time was
   * chosen since it is a common unit and since it reflects the combination of CPU time, memory time
   * (for allocations and garbage collection - the latter can be quite significant), and the disk
   * wait time. The other main component of time is key search, but that is not instrumented right
   * now.
   *
   * <p>Serialization and deserialization are basically a CPU activity and drive memory to the
   * extent that allocations are made, especially during deserialization.
   *
   * <p>The read/write time is strongly dominated by actual DISK IO and by garbage collection time
   * (garbage collection can cause threads to be suspended at any time). For deep B+Trees, DISK READ
   * time dominates DISK WRITE time since increasing numbers of random reads are required to
   * materialize any given leaf.
   *
   * <p>The total read-write cost (in seconds) for a BTree is one of the factors that is considered
   * when choosing which index partition to move. Comparing the total read/write cost for BTrees
   * across a database can help to reveal which index partitions have the heaviest load. If only a
   * few index partitions for a given scale-out index have a heavy load, then those index partitions
   * are hotspots for that index.
   *
   ...
   * @param totalRawScore The raw score computed from the totals.
   * @return The normalized score.
   */
  public static double normalize(final double rawScore, final double totalRawScore) {
  ...
  public CounterSet getCounters() {
     ...
     */
    {
      ...
      tmp.addCounter(
          ...
          new Instrument<Long>() {
            @Override
            protected void sample() {
              setValue(bytesOnStore_nodesAndLeaves.get() + bytesOnStore_rawRecords.get());
            }
          });
      ...
      tmp.addCounter(
          ...
          new Instrument<Double>() {
            ...
            public void sample() {
              ...
              final double bytesReadPerSec = (readSecs == 0L ? 0d : (bytesRead.get() / readSecs));
      ...
      tmp.addCounter(
          "serializeLatencyNanos",
          new Instrument<Double>() {
            @Override
            public void sample() {
              final long nwritten = nodesWritten.get() + leavesWritten.get();
              final double serializeLatencyNanos =
                  (nwritten == 0L ? 0d : (serializeNanos.get() / nwritten));
      ...
      tmp.addCounter(
          ...
          new Instrument<Double>() {
            ...
            public void sample() {
              final long nwritten = nodesWritten.get() + leavesWritten.get();
              final double serializeSecs = (serializeNanos.get() / 1000000000.);
              final double serializePerSec =
                  (serializeSecs == 0L ? 0d : (nwritten / serializeSecs));
      ...
      tmp.addCounter(
          "deserializeLatencyNanos",
          new Instrument<Double>() {
            @Override
            public void sample() {
              final long nread = nodesRead.get() + leavesRead.get();
              final double deserializeLatencyNanos =
                  (nread == 0L ? 0d : (deserializeNanos.get() / nread));
      ...
      tmp.addCounter(
          ...
          new Instrument<Double>() {
            ...
            public void sample() {
              final long nread = nodesRead.get() + leavesRead.get();
              final double deserializeSecs = (deserializeNanos.get() / 1000000000.);
              final double deserializePerSec =
                  (deserializeSecs == 0L ? 0d : (nread / deserializeSecs));
    ...
    //                                final double latencyNanos = (n == 0L ? 0d :
    // (syncTouchNanos.get() / n));
    ...
    //                            public void sample() {
    //                                final long n = touchCount.get();
    //                                final double latencyNanos = (n == 0L ? 0d : (touchNanos.get()
    // / n));
    //                                setValue(latencyNanos);
    //                            }
    //                        });
    ...
    // counterSet.makePath(IBTreeCounters.WriteRetentionQueue);
    //
