 ...
 *
 * <ol>
 *   <li>The writeSet of the transaction on the index is isolated on a {@link BTree} visible only to
 *       that transaction.
 *   <li>Version timestamps are maintained for index entries in both the isolated write set and
 *       groundState from which the transaction is reading.
 *   <li>The groundState is defined as the view of the index (partition) as of the abs(startTime) of
 *       the transaction.
 *   <li>Reads are performed against an ordered view defined by the writeSet followed by the ordered
 *       set of indices defining the groundState of the index.
 *   <li>Writes first read through the ordered view to locate the most recent version for an index
 *       entry. If the index entry is located in the isolated writeSet then it is overwritten and
 *       its timestamp is unchanged. If the index entry is located in the groundState then the
 *       timestamp is copied from that index entry and written on the new version in the isolated
 *       writeSet.
 *   <li>During validation, version timestamps in the isolated writeSet are compared against the
 *       then current view of the corresponding unisolated index. If the timestamp in the unisolated
 *       view differs from that in the writeSet then there is a write-write conflict. Write-write
 *       conflicts MAY be validated if the index has a registered {@link IConflictResolver}.
 *   <li>If the writeSet is validated then it is mergedDown (copied onto) the then current
 *       unisolated index view. During the mergeDown phase the revision timestamp of the transaction
 *       is applied to all index entries copied from the write set. Transactions that later try to
 *       commit will recognize write-write conflicts based on those updated timestamps. Note that
 *       revision timestamps ARE NOT commit timestamps. Revision timestamps are assigned at the
 *       start of the validation phase. All tuples modified by a transaction are annotated with the
 *       same revision timestamp during the validation phase of the transaction. Write-write
 *       conflicts are detected on the basis of the per-tuple revision timestamps. Commit timestamps
 *       are assigned once the write set has been validated and checkpointed and all shards
 *       participating in the commit protocol signal that they are prepared and ready to commit.
 * </ol>
 *
 * <p>Note: The timestamp from which the post-commit state of the transaction may be read IS NOT
 * defined for an {@link IEmbergraphFederation}. It is not possible to define this timestamp without
 * requiring concurrent commit processing to be paused on all data services on which the transaction
 * has written, which is viewed as too high a cost. Instead, the commit timestamp is the state from
 * which you can read the data written by the transaction. Reads on tuples NOT updated by the
 * transaction MAY have been changed by concurrent transactions. Note: The process of validating,
 * merging down changes, and committing those changes MUST be atomic. Therefore no other operations
 * may be permitted access to the unisolated indices corresponding to the isolated indices on which
 * the transaction during this process. This constraint is generally achieved by holding a write
 * lock on the unisolated indices corresponding to the indices isolated by the transaction, e.g., by
 * declaring those indices to an {@link ITx#UNISOLATED} {@link AbstractTask} which handles this
 * process.
 *
...
public class IsolatedFusedView extends FusedView {
   ...
   * <i>timestamp</i>.
   *
   * <p>Reads will read through the <i>writeSet</i> and then the resource(s) in the
   * <i>groundState</i> in the order in which they are given. A read is satisfied by the first
   * resource containing an index entry for the search key.
   *
   * <p>Writes will first read through looking for a @todo javadoc
   *
   * @param timestamp The timestamp associated with the <i>groundState</i>.
   * @param sources An ordered array of sources comprised of the {@link BTree} that will absorb
   *     writes and the historical ground state.
   */
  public IsolatedFusedView(final long timestamp, final AbstractBTree[] sources) {

    super(sources);

    if (!TimestampUtility.isCommitTime(timestamp)) throw new IllegalStateException();

    this.startTime = timestamp;

    if (sources.length < 2) throw new IllegalArgumentException();
   ...
   * to an undetectable write conflict.
   *
   * @todo counters could probably be enabled within transactions if we used the counter from the
   *     then current mutable btree. This would have to be passed into the constructor. In addition,
   *     the counter logic would have to be carefully checked to make sure that counter assignments
   *     remain consistent. The counter itself is an {@link AtomicInteger}. However additional care
   *     needs to be taken to ensure that the counter value is persisted if it is changed (by
   *     updating the {@link BTree} {@link Checkpoint} record). The cases where the tx bumps the
   *     counter need to be carefully examined since it could force the write of the unisolated
   *     btree when we actually do not want to commit the btree - some kind of locking may be
  ...
  public byte[] insert(final byte[] key, final byte[] val) {
    ...
    if (tuple == null) {
      ...
      //            srcs[0]
      getMutableBTree()
          .insert(key, val, false /*delete*/, false /*putIfAbsent*/, startTime, null /*tuple*/);
  ...
  public byte[] putIfAbsent(final byte[] key, final byte[] val) {
    ...
    if (tuple == null) {
      ...
      //            srcs[0]
      getMutableBTree()
          .insert(key, val, false /*delete*/, true /*putIfAbsent*/, timestamp, null /*tuple*/);
  ...
  public byte[] remove(final byte[] key) {
    ...
    if (tuple == null) {
      ...
      //            srcs[0]
      getMutableBTree()
          .insert(key, null, true /* delete */, false /*putIfAbsent*/, startTime, null /*tuple*/);
       ...
       * Write a delete marker whose timestamp is copied from the
        ...
        getMutableBTree()
            .insert(
                key, null, true /* delete */, false /*putIfAbsent*/, timestamp, null /* tuple */);
      }
   ...
   * version (an MVCC style strategy). Write-write conflicts are detected by backward validation
   * against the last committed state of the journal. A write-write conflict exists IFF the version
   * counter on the transaction index entry differs from the version counter in the global index
   * scope. Once detected, the resolution of a write-write conflict is delegated to a {@link
   * IConflictResolver conflict resolver}. If a write-write conflict can not be validated, then
   * validation will fail and the transaction must abort.
   *
   * <p>Validation occurs as part of the prepare/commit protocol. Concurrent transactions MAY
   * continue to run without limitation. A concurrent commit (if permitted) would force
   * re-validation since the transaction MUST now be validated against the new baseline. (It is
   * possible that this validation could be optimized.)
   *
   * <p>The version counters used to detect write-write conflicts are incremented during the commit
   * as part of the {@link #mergeDown()} of the {@link IsolatedFusedView} onto the corresponding
   * unisolated indices in the global scope.
   *
   * @param groundStateSources The ordered view of the unisolated index. This MUST be the current
   *     view of the ground state as of when the transaction is validated (NOT when it was created).
   *     This view WILL NOT the same as the groundState specified to the constructor if intervening
   *     transactions have committed on the index.
  ...
  public boolean validate(final AbstractBTree[] groundStateSources) {
     ...
     */
    final IConflictResolver conflictResolver = writeSet.getIndexMetadata().getConflictResolver();
     ...
     */
    final IIndex groundState =
        (groundStateSources.length == 1
            ? groundStateSources[0]
    ...
    final ITupleIterator<?> itr =
        writeSet.rangeIterator(null, null, 0 /* capacity */, ALL /* flags */, null);

    // tuple for reading from the groundState index.
    final Tuple<?> groundStateTuple =
        new Tuple(
    ...
    while (itr.hasNext()) {
      ...
      if (groundState instanceof AbstractBTree) {
        ...
        baseEntry = ((AbstractBTree) groundState).lookup(key, groundStateTuple);

      } else {

        baseEntry = ((FusedView) groundState).lookup(key, groundStateTuple);
      }
       ...
       * If the version counters do not agree then we need to perform
        ...
        if (baseEntry.getVersionTimestamp() != txEntry.getVersionTimestamp()) {
       ...
       * Apply the conflict resolver in an attempt to resolve the
          ...
          try {
            ...
            if (!conflictResolver.resolveConflict(tmp, txEntry, baseEntry)) {
   ...
   * set of an {@link IsolatedFusedView} into the global scope. This operation does NOT check for
   * conflicts. The pre-condition is that the transaction has already been validated (hence, there
   * will be no conflicts).
   *
   * <p>Note: This method is also responsible for updating the version timestamps that are used to
   * detect write-write conflicts during validation - they are set to the <i>revisionTime</i>.
   *
   * @param revisionTime The revision timestamp assigned to the commit point of the transaction.
   * @param groundStateSources The ordered view of the unisolated index. This MUST be the current
   *     view of the ground state as of when the transaction is validated (NOT when it was created).
   *     This view WILL NOT the same as the groundState specified to the constructor if intervening
   *     transactions have committed on the index.
   */
  public void mergeDown(final long revisionTime, final AbstractBTree[] groundStateSources) {
