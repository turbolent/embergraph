 ...
 * requirements.
 *
 * <p>Note: iterators returned by this class do not support removal (the nodes and leaves will all
 * refuse mutation operations).
 *
...
public class IndexSegment extends AbstractBTree { // implements ILocalBTreeView {
   ...
   * {@inheritDoc}
   *
   * <p>Overridden to be thread-safe without requiring any locks.
   */
   ...
   *
   * @param fileStore The store containing the {@link IndexSegment}.
   ...
   * {@inheritDoc}
   *
   * <p>Overridden to a more constrained type.
   */
  @Override
  ...
  /**
   * Extended to explicitly close the {@link IndexSegment} and the backing {@link
   * IndexSegmentStore}. A finalizer is necessary for this class because we maintain {@link
   * IndexSegment}s in a weak value cache and do not explicitly close then before their reference is
   * cleared. This leads to the {@link IndexSegmentStore} being left open. The finalizer fixes that.
   */
  @Override
  protected void finalize() throws Throwable {
    ...
    if (isOpen()) {
      ...
      if (INFO) log.info("Closing IndexSegment: " + fileStore.getFile());
  ...
  /**
   * Type-safe method reads the leaf from the backing file given the address of that leaf. This
   * method is suitable for ad-hoc examination of the leaf or for a low-level iterator based on the
   * prior/next leaf addresses.
   *
   * <p>Note: The parent is NOT set on the leaf but it MAY be defined if the leaf was read from
   * cache.
   *
   * @param addr The address of a leaf.
   * @return That leaf.
   */
   ...
   * @throws RuntimeException if the key does not lie within the optional key-range constraints for
   *     an index partition.
   */
  ...
  protected static class ImmutableNodeFactory implements INodeFactory {
    ...
    /**
     * Immutable node throws {@link UnsupportedOperationException} for the public mutator API but
     * does not try to override all low-level mutation behaviors.
     *
     * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
     * @version $Id$
     */
    public static class ImmutableNode extends Node {
       ...
       * @param data
       */
      protected ImmutableNode(final AbstractBTree btree, final long addr, final INodeData data) {
    ...
    /**
     * A double-linked, read-only, empty leaf. This is used for the root leaf of an empty {@link
     * IndexSegment}.
     *
     * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
    ...
    private static class EmptyReadOnlyLeafData implements ILeafData {
      ...
      public EmptyReadOnlyLeafData(
          final boolean deleteMarkers, final boolean versionTimestamps, final boolean rawRecords) {
    ...
    /**
     * Immutable leaf throws {@link UnsupportedOperationException} for the public mutator API but
     * does not try to override all low-level mutation behaviors.
     *
     * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
     * @version $Id$
    ...
    public static class ImmutableLeaf extends Leaf {
      ...
      /**
       * Ctor used when the {@link IndexSegment} is empty (no tuples) to create an empty (and
       * immutable) root leaf.
       *
       * @param btree
       */
       ...
       * @param values
       */
      protected ImmutableLeaf(final AbstractBTree btree, final long addr, final ILeafData data
      ...
      public ImmutableLeaf nextLeaf() {
        ...
        if (nextAddr == 0L) {
          ...
          if (DEBUG) log.debug("No more leaves");
   ...
   */
  private static class CursorPosition<E> extends AbstractCursorPosition<ImmutableLeaf, E> {
    ...
    /**
     * Create position on the specified key, or on the successor of the specified key if that key is
     * not found in the index.
     *
     * @param cursor The {@link ITupleCursor}.
     * @param leafCursor The leaf cursor.
     * @param index The index of the tuple in the <i>leaf</i>.
     * @param key The key.
     */
    public CursorPosition(
        IndexSegmentTupleCursor<E> cursor,
        ILeafCursor<ImmutableLeaf> leafCursor,
        int index,
        byte[] key) {
  ...
  /**
   * Implementation for an immutable {@link IndexSegment}. This implementation uses the prior/next
   * leaf references for fast forward and reference scans of the {@link IndexSegment}.
   *
   * <p>Note: Since the {@link IndexSegment} is immutable it does not maintain listeners for
   * concurrent modifications.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @param <E> The generic type for the objects de-serialized from the index.
   */
  public static class IndexSegmentTupleCursor<E>
      extends AbstractBTreeTupleCursor<IndexSegment, ImmutableLeaf, E> {

    public IndexSegmentTupleCursor(
        final IndexSegment btree, final Tuple<E> tuple, final byte[] fromKey, final byte[] toKey) {
