 ...
 * @todo it could be useful to byte align the codedValueOffset[] IF we also coded the offsets using
 *     nbits, where nbits was a multiple of 8.
 *     <p>FIXME Generalize to allow alphabets of size GT 256 so we can reuse this to directly encode
 *     the long[] keys of the leaves in the RDF DB. This will still be an {@link IRaba} (a random
 *     access byte[]). The difference is that each symbol will correspond to a sequence of 8 bytes.
 *     So what we need is a means to decouple the generation of the frequency[], the assumption that
 *     alphabets of 256 are "complete", and the application of the codec and the decoder, which must
...
public class CanonicalHuffmanRabaCoder implements IRabaCoder, Externalizable {
   ...
   *
   * @param symbol2byte The symbol table.
   * @param obs The symbol table is written on this bit stream.
   */
  protected void writeSymbolTable(final Symbol2Byte symbol2byte, final OutputBitStream obs)
      throws IOException {
   ...
   * </pre>
   *
   * where <code>min</code> is the bit length of the shortest code word; <code>max</code> is the bit
   * length of the longest code word; <code>count</code> is the #of code words of a given length;
   * and <code>symbol(s)</code> are the symbols associated with each code word. All values are
   * nibble coded and will generally fit in 1-2 bytes each.
   *
   * @param decoderInputs This contains both the bit lengths of the canonical huffman code and the
   *     symbols assigned to each code word.
   * @param obs The output bit stream.
   * @param sb Debugging information is added to this buffer (optional).
   * @throws IOException
   * @see {@link DecoderInputs}
   */
  protected static void writeDecoderInputs(
      final DecoderInputs decoderInputs, final OutputBitStream obs, final StringBuilder sb)
      throws IOException {
    ...
    if (length.length != symbol.length) throw new IllegalArgumentException();
   ...
   *
   * @param nsymbols The #of symbols.
   * @param ibs The input bit stream.
   * @param sb Debugging information is added to this buffer (optional).
   * @return The decoded bit lengths and the corresponding symbol indices for the canonical huffman
   *     code.
   * @throws IOException
   */
  protected static DecoderInputs readDecoderInputs(
      final int nsymbols, final InputBitStream ibs, final StringBuilder sb) throws IOException {
    ...
    final BitVector shortestCodeWord =
        LongArrayBitVector.getInstance().length(shortestCodeWordLength);
   ...
   * @param raba The logical byte[][].
   * @param byte2symbol The mapping from byte values to symbol indices.
   * @return The total bit length of the coded values.
   * @deprecated Leave this field and the #of bits per codedValueOffset[] element blank until we
   *     have written out the coded values and then rewind the OBS and fill in those fields.
   *     Otherwise we are encoding the same byte[][] data twice, which is wasted effort.
   */
  protected long getSumCodedValueBitLengths(
      final BitVector[] codeWords, final IRaba raba, final Byte2Symbol byte2symbol) {
   ...
   * @param raba The logical byte[][].
   * @param byte2symbol The mapping from byte values to symbol indices.
   * @param codedValueOffset An optional array dimensioned to <code>nvalues+1</code>, where
   *     <i>nvalues</i> is #of values in the logical byte[][]. When given, the array will be
   *     populated with the relative bit offset of the start of each coded value. The offsets are
   *     relative to the start of the first coded value.
   * @return The #of bits written (sum total of the bit lengths of the coded values).
   */
  protected long writeCodedValues(
      final PrefixCoder coder,
      final IRaba raba,
      final Byte2Symbol byte2symbol,
      final long[] codedValueOffset,
    ...
    if (codedValueOffset != null) {
      ...
      if (codedValueOffset.length != nvalues + 1) throw new IllegalArgumentException();
    }
    ...
    for (int i = 0; i < nvalues; i++) {
      ...
      if (codedValueOffset != null) codedValueOffset[i] = bitsWritten;
      ...
      if (a != null) {
        ...
        for (byte b : a) {
          ...
          if (symbol == -1) throw new UnsupportedOperationException("Can not code value: " + b);
    ...
    if (codedValueOffset != null) codedValueOffset[nvalues] = bitsWritten;

    if (log.isDebugEnabled()) log.debug("codedValueOffset[]=" + Arrays.toString(codedValueOffset));
  ...
  interface Byte2Symbol {
     ...
     * @return The symbol used to code that byte value -or- <code>-1</code> if the byte value was
     *     not assigned to any symbol.
     */
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  protected abstract static class AbstractCodingSetup implements Byte2Symbol, Symbol2Byte {
     ...
     * @param symbol2byte The mapping from symbol indices to byte value.
     * @return A representation of the code book.
     */
    protected static String printCodeBook(
        final BitVector[] codeWords, final Symbol2Byte symbol2byte) {
     ...
     *
     * @param raba The data.
     * @return An 256 element array giving the frequency of each byte value. Values not observed
     *     will have a zero frequency count.
     */
    ...
    protected int getSymbolCount(final int[] frequency) {
      ...
      if (frequency == null) throw new IllegalArgumentException();

      if (frequency.length != 256) throw new IllegalArgumentException();
     ...
     *     frequency of occurrence of the corresponding byte value.
     * @param packedFrequency The non-zero symbol frequency counts. This array is correlated with
     *     the packed symbol table. The array must be pre-allocated by the caller with
     *     <i>nsymbol</i> elements.
     * @param symbol2byte The forward lookup symbol table. The array must be pre-allocated by the
     *     caller with <i>nsymbol</i> elements.
     * @return The reverse symbol table.
     */
    protected Byte2IntOpenHashMap buildSymbolTable(
        final int[] frequency, final int[] packedFrequency, final byte[] symbol2byte) {
      ...
      final Byte2IntOpenHashMap byte2symbol = new Byte2IntOpenHashMap(nsymbols);
   ...
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   *     <p>FIXME If most byte values are used, then simply code them all. This will be more
   *     efficient (smaller data record, no indirection through the symbol table).
   */
  protected static class RabaCodingSetup extends AbstractCodingSetup {
    ...
    @Override
    public final int byte2symbol(final byte b) {
  ...
  public ICodedRaba encodeLive(final IRaba raba, final DataOutputBuffer buf) {
    ...
    final long sumCodedValueBitLengths =
        nsymbols == 0 ? 0 : getSumCodedValueBitLengths(setup.codec().codeWords(), raba, setup);
     ...
     */
    final int codedValueOffsetBits =
        nsymbols == 0 ? 0 : Fast.mostSignificantBit(sumCodedValueBitLengths) + 1;
    ...
    final int initialCapacity =
        (int) (512 + sumCodedValueBitLengths + (size + 1) * codedValueOffsetBits);
    ...
    try {
      ...
      assert obs.writtenBits() == CodedRabaImpl.O_symbols + (isSymbolTable ? nsymbols * 8 : 0);
      ...
      if (nsymbols == 0) {
        ...
        final long sumCodedValueBitLengths2 =
            writeCodedValues(setup.codec().coder(), raba, setup, codedValueOffset, obs);
      ...
      return new CodedRabaImpl(slice, setup.codec().decoder(), decoderInputsBitLength);
  ...
  public static class CodedRabaImpl extends AbstractCodedRaba {
     ...
     * ByteArrayBuffer#trim()}. In order to protect against such changes, we only store the offset
     * into the data record (which is immutable across trim()).
     *
     * @todo if we implement a compacting store for the coded data records then the offset could
     *     change and we would also have to protect the API level operations of all {@link
     *     ICodedRaba}s against relocation of the backing data record due to asynchronous compacting
     *     operations.
     */
     ...
     * consult these bit flags in order to distinguish a <code>null</code> from a zero length
     * byte[].
     */
     ...
     *
     * @param data The record containing the coded data.
     * @param decoder The decoder (optional). When not given the decoder is reconstructed from the
     *     record.
     * @param decoderInputsBitLength The bit length of the {@link DecoderInputs} in the coded data
     *     record. This information is used to skip beyond the {@link DecoderInputs} without having
     *     to read them from the {@link InputBitStream}.
     */
    public CodedRabaImpl(
      ...
      if (decoder != null && decoderInputsBitLength == 0) throw new IllegalArgumentException();
      ...
      try {
        ...
        if (debug) log.debug(sb.toString());
    ...
    public int length(final int index) {
      ...
      try {
        ...
        ibs.position(O_codedValueOffsets + ((long) index) * codedValueOffsetBits);

        final long O_from = ibs.readLong(codedValueOffsetBits);

        // get the bit offset of the start of the next coded value.
        ibs.position(O_codedValueOffsets + (index + 1L) * codedValueOffsetBits);
     ...
     * @param nsymbols The #of symbols to decode.
     * @return The exact fit byte[] containing the decoded symbols.
     * @throws IOException
     */
    private byte[] getFrom(final InputBitStream ibs, final int nsymbols) throws IOException {
     ...
     * @param codeLength The bit length of the sequence of code words to be decoded.
     * @param os Where to put the data.
     * @return The #of decoded symbols that were copied.
     * @throws IOException
     */
    private int copyFrom(final InputBitStream ibs, final long codeLength, final OutputStream os)
        throws IOException {
    ...
    public Iterator<byte[]> iterator() {
       ...
       * buffer. The data in the buffer is then cloned to obtain an exact fit byte[].
       *
       * @todo if we knew the max byte[] length for the instance then we could use that here and
       *     avoid over/under allocating.
       */
    ...
    public int search(final byte[] probe) {
      ...
      if (probe == null) throw new IllegalArgumentException();
     ...
     * in the code space.
     *
     * @param index The index of the coded value.
     * @param key The search probe key.
     * @param array The reference to the coded data record.
     * @return a negative integer, zero, or a positive integer if the coded value identified by the
     *     <i>index</i> is less than, equal to, or greater than the <i>key</i> when they are
     *     compared in the code space.
     * @throws IOException
     */
    private int compare(
        final InputBitStream ibs, final int index, final byte[] key, final byte[] array)
        throws IOException {
