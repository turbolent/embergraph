...
public class FrontCodedRabaCoder implements IRabaCoder, Externalizable {
  ...
  public static class DefaultFrontCodedRabaCoder extends FrontCodedRabaCoder {
    ...
    public static final transient DefaultFrontCodedRabaCoder INSTANCE =
        new DefaultFrontCodedRabaCoder();
   ...
   *     front-coded values. Linear search is then performed within the bucket of front-coded values
   *     in which the key would be found if it is present. Therefore the ratio is also the maximum
   *     of steps in the linear scan.
   *     <p>Let <code>m := n / ratio</code>, where <i>n</i> is the #of entries in the <code>byte[][]
   *     </code> (the size of the total search problem), <i>m</i> is the size of the binary search
   *     problem and ratio is the size of the linear search problem. Solving for ratio, we have:
   *     <code>ratio := n / m</code>. Some examples:
   *     <pre>
   * m = n(64)/ratio(16) = 4
   *
   * m = n(64)/ratio(8) = 8
  ...
  /** The byte offset of the field coding the #of entries in the logical byte[][]. */
  ...
  public ICodedRaba encodeLive(final IRaba raba, final DataOutputBuffer buf) {
    ...
    if (!raba.isKeys()) throw new UnsupportedOperationException("Must be keys.");
    ...
    if (log.isInfoEnabled())
      log.info("n=" + raba.size() + ", capacity=" + raba.capacity() + ", ratio=" + ratio);
    ...
    final CustomByteArrayFrontCodedList decoder =
        new CustomByteArrayFrontCodedList(raba.iterator(), ratio, isDuplicateKeys());
  ...
  private static class CodedRabaImpl extends AbstractCodedRaba {
     ...
     */
    public CodedRabaImpl(final AbstractFixedByteArrayBuffer data, final boolean hasDups) {
      ...
      this.decoder =
          new CustomByteArrayFrontCodedList(
              size, ratio, data.array(), data.off() + O_DATA, data.len(), hasDups);
    ...
    public CodedRabaImpl(
        final AbstractFixedByteArrayBuffer data, final CustomByteArrayFrontCodedList decoder) {
    ...
    /** Always returns <code>true</code> since the front-coded representation is dense. */
    @Override
    ...
    /** Always returns <code>false</code> (<code>null</code>s are not allowed). */
    @Override
