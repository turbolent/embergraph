...
public class ConditionalRabaCoder implements IRabaCoder, Externalizable {
   ...
   */
  public ConditionalRabaCoder(
      final IRabaCoder smallCoder, final IRabaCoder bigCoder, final int bigSize) {

    final boolean isKeyCoder = smallCoder.isKeyCoder() && bigCoder.isKeyCoder();

    final boolean isValueCoder = smallCoder.isValueCoder() && bigCoder.isValueCoder();

    if (!isKeyCoder && !isValueCoder) throw new IllegalArgumentException();
  ...
  public ICodedRaba encodeLive(final IRaba raba, final DataOutputBuffer buf) {
    ...
    final AbstractFixedByteArrayBuffer delegateSlice = delegateCodedRaba.data();
  ...
  private static class CodedRabaSlice implements ICodedRaba {
    ...
    CodedRabaSlice(final AbstractFixedByteArrayBuffer data, final IRaba delegate) {
