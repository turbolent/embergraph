 ...
 * keys and supports {@link #search(byte[])} -or- stores B+Tree values and allows <code>null</code>s
 * but does not support search. for storing <code>null</code> s and search are mutually exclusive.
 * Some {@link IRaba} implementations may be used for either purpose, but the {@link IRaba} instance
 * is always either B+Tree keys or B+Tree values. In both cases, the {@link IRaba} stores variable
 * length byte[]s. However, when an {@link IRaba} stores B+Tree keys, the byte[]s interpreted as
 * <code>unsigned byte[]</code>s for the purpose of {@link #search(byte[])}.
 *
 * <h3>B+Tree keys</h3>
 *
 * When used for B+Tree keys, the interface provides operations on an ordered set of variable length
 * <code>unsigned byte[]</code> keys and <code>null</code>s ARE NOT permitted.
 *
 * <h3>B+Tree values</h3>
 *
 * When used for B+Tree values, the interface provides operations on an unordered collection of
 * variable length <code>byte[]</code>s, does NOT support {@link #search(byte[])} and <code>null
 * </code>s are allowed.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @todo consider discarding all of the {@link #add(byte[])} methods. It is typically easier and
 *     more convenient to directly manage the existing {@link MutableKeyBuffer} and {@link
 *     MutableValueBuffer}. However, there are also {@link MutableKeysRaba} and {@link
 *     MutableValueRaba} implementations which are, I believe, only used by the {@link ResultSet}.
 */
public interface IRaba extends Iterable<byte[]> {
   ...
   * not stored.
   *
   * <p>When <code>false</code>, the {@link IRaba} allows <code>null</code>s and the elements are
   * interpreted as <code>signed byte[]</code> (B+Tree values).
   *
   * @return <code>true</code> if the {@link IRaba} represents B+Tree keys and <code>false</code> if
   *     it represents B+Tree values.
   */
  boolean isKeys();
   ...
   *     a sense of capacity GTE size. Some of the unit tests assume that all rabas are dimensions
   *     based on the branching factor, which is no longer true. By moving this method onto the
   ...
   *
   * @param index The index in [0:{@link #size()}-1].
   * @return The byte[] value at that index and <code>null</code> if a <code>null</code> value was
   ...
   *
   * @param index The index in [0:{@link #size()}-1].
   * @param out The output stream onto which the key will be copied.
   * @return The #of bytes copied.
   * @throws IndexOutOfBoundsException unless index is in [0:{@link #size()}-1].
   * @throws NullPointerException if the byte[] value at that index is <code>null</code>.
   * @throws RuntimeException if the {@link OutputStream} throws an {@link IOException} (generally
   ...
   * @param len The #of bytes to be read.
   * @return The #of values in the logical byte[][] (postcondition).
   ...
   * support binary search of the coded byte[] values. Others may require a mixture of binary and
   * linear search, etc.
   *
   * <p>
   *
   ...
   *     found.
   * @exception IllegalArgumentException if the searchKey is null.
