...
public class ReadOnlyKeysRaba extends AbstractRaba {
   ...
   * @param a The backing byte[][].
   */
  public ReadOnlyKeysRaba(final int fromIndex, final int toIndex, final byte[][] a) {
  ...
  /*
   * Asserts that the <code>unsigned byte[]</code>s in the logical byte[][] are in a total order.
   *
   * @param fromIndex The first index in the view (inclusive lower bound).
   * @param toIndex The first index outside the view (exclusive upper bound).
   * @param a The backing byte[][].
   * @return true if the total ordering constraint is respected by the data.
   */
  protected boolean assertTotalOrder(final int fromIndex, final int toIndex, final byte[][] a) {
    ...
    for (int i = fromIndex; i < toIndex; i++) {
      ...
      if (i > fromIndex) {
        ...
        assert ret < 0
            : "unsigned byte[]s are out of order at index="
                + i
                + ", ret="
                + ret
                + "\na["
                + (i - 1)
                + "]="
                + BytesUtil.toString(a[i - 1])
                + "\na["
                + (i)
                + "]="
                + BytesUtil.toString(a[i]);
      }
    }
