...
public class MutableKeyBuffer extends AbstractKeyBuffer {
   ...
   *
   * @param index The index in [0:nkeys] (you are allowed to append using this method).
   * @param key The key.
   * @return The #of keys in the buffer.
   * @todo if index==0 || index==nkeys-1 then update prefixLength, lazily compute prefix.
   */
  public final int insert(final int index, final byte[] key) {
   ...
   * @param key The key.
   * @return The #of keys in the buffer.
   * @todo if index==0 || index==nkeys-1 then update prefixLength, lazily compute prefix (requires
   *     that the application never directly modifies keys).
   */
  ...
  protected final int _prefixMatchLength(final int prefixLength, final byte[] searchKey) {
     ...
     */
    final int compareLen = (searchKeyLen <= prefixLength) ? searchKeyLen : prefixLength;

    int ret =
        BytesUtil.compareBytesWithLenAndOffset(0, compareLen, searchKey, 0, compareLen, keys[0]);
    ...
    if (ret < 0) {
       ...
       */
      if (searchKeyLen < prefixLength) return -1;
  ...
  protected final int _linearSearch(final int searchKeyOffset, final byte[] searchKey) {
    ...
    for (int i = 0; i < nkeys; i++) {
      ...
      final int ret =
          BytesUtil.compareBytesWithLenAndOffset(
              searchKeyOffset, keyLen, key, searchKeyOffset, searchKeyLen, searchKey);
  ...
  protected final int _binarySearch(final int searchKeyOffset, final byte[] searchKey) {
  ...
  public final void assertKeysMonotonic() {
    ...
    for (int i = 1; i < nkeys; i++) {
      ...
      if (BytesUtil.compareBytes(keys[i], keys[i - 1]) <= 0) {
        ...
        throw new AssertionError("Keys out of order at index=" + i + ", keys=" + this.toString());
      }
    }
