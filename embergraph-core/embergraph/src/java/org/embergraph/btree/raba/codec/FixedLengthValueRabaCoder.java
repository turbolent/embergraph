 ...
 * class only supports B+Tree values.
 *
 * <h2>Binary Format</h2>
 *
 * <pre>
 ...
 * <dl>
 *   <dt>size
 *   <dd>The #of elements in the {@link IRaba}.
 *   <dt>nulls
 *   <dd>A vector of bit flags identifying <code>null</code> values. The length of the vector is
 *       rounded up to the nearest whole byte based on <i>size</i>.
 *   <dt>len
 *   <dd>The fixed length of each non-<code>null</code> value as specified to the constructor.
 *   <dt>values
 *   <dd>The uncompressed representation of each fixed length value. In order to avoid the overhead
 *       of an offset map, <code>null</code> values occupy the same space in the array as non-<code>
 *       null</code> values.
 * </dl>
 *
...
public class FixedLengthValueRabaCoder implements IRabaCoder, Externalizable {
   ...
   *     having values with a different length will result in a runtime exception when they are
   *     encoded.
   */
   ...
   * values whose length is not the length specified to the constructor.
   *
   * @throws UnsupportedOperationException if the {@link IRaba} has a non-<code>null</code> value
   *     with a length other than the length specified to the constructor.
   */
  @Override
  public ICodedRaba encodeLive(final IRaba raba, final DataOutputBuffer buf) {
    ...
    if (raba.isKeys()) throw new UnsupportedOperationException();
    ...
    final int capacity =
        SIZEOF_VERSION + SIZEOF_SIZE + BytesUtil.bitFlagByteLength(size) + size * len;
  ...
  private static class CodedRabaImpl extends AbstractCodedRaba {
    ...
    /** The byte length of each non-null entry (from the constructor on the outer class). */
    ...
    /** @param data */
    public CodedRabaImpl(final int len, final AbstractFixedByteArrayBuffer data) {
    ...
    public CodedRabaImpl(final int len, final AbstractFixedByteArrayBuffer data, final int size) {
