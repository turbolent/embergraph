...
public class SubRangeRaba implements IRaba {
   ...
   * @throws IllegalArgumentException if the fromIndex is LT ZERO (0).
   * @throws IllegalArgumentException if the toIndex is GT the delegate {@link IRaba#size()}.
   * @throws IllegalArgumentException if the fromIndex is GTE to toIndex.
   */
  public SubRangeRaba(final IRaba delegate, final int fromIndex, final int toIndex) {
    ...
    if (toIndex > delegate.size()) throw new IllegalArgumentException();
   ...
   *     <p>TODO I am not sure about the semantics of search() for a sub-range so it is just
   *     disallowed for now. The issue is the meaning of a key probe that is LT the key at fromIndex
   *     in the delegate or GTE the key at toIndex in the delegate. Search in fact can generally
   *     report positions before the start of an ordered array of keys or after their end. But in
   *     this case we can't see any keys before fromIndex or GTE toIndex. This leads to some
   *     ambiguity about how to respond to key probes that really below in a different {@link
   *     SubRangeRaba}.
   */
  @Override
