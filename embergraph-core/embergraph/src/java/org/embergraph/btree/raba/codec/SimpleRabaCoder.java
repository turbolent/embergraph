...
public class SimpleRabaCoder implements IRabaCoder, Externalizable {
   ...
   * @since #VERSION1
   */
  private static final int SIZEOF_CAPACITY = Bytes.SIZEOF_INT;
  /** The size of the field coding the #of elements in the offset[]. */
  private static final int SIZEOF_OFFSET = Bytes.SIZEOF_INT;

  /** The byte offset to the version identifier. */
  private static final int O_VERSION = 0;
  /** The byte offset of the bit flags. */
  private static final int O_FLAGS = O_VERSION + SIZEOF_VERSION;
  ...
  public ICodedRaba encodeLive(final IRaba raba, final DataOutputBuffer buf) {
    ...
    // logical capacity.
    if (CURRENT_VERSION >= VERSION1) buf.putInt(capacity);
  ...
  private static class CodedRabaImpl extends AbstractCodedRaba {
    ...
    public CodedRabaImpl(final AbstractFixedByteArrayBuffer data) {
      ...
      if (version >= VERSION1) this.capacity = data.getInt(O_SIZE + SIZEOF_SIZE);
      ...
      final int bitFlagByteCount = isKeys ? 0 : BytesUtil.bitFlagByteLength(size);
    ...
    public int length(final int index) {
      ...
      final int offset2 = data.getInt(O_offsets + (index + 1) * SIZEOF_OFFSET);
    ...
    public int search(final byte[] key) {
      ...
      while (low <= high) {
        ...
        final int aoff = data.getInt(O_offsets + offset * SIZEOF_OFFSET);

        // length of that byte[].
        final int alen = data.getInt(O_offsets + (offset + 1) * SIZEOF_OFFSET) - aoff;

        // compare actual data vs probe key.
        final int tmp =
            BytesUtil.compareBytesWithLenAndOffset(
                data.off() + aoff, alen, data.array(), 0, key.length, key);
