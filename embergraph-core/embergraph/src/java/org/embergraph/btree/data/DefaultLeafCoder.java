...
public class DefaultLeafCoder implements IAbstractNodeDataCoder<ILeafData>, Externalizable {
  ...
  public String toString() {
    ...
    return super.toString() + "{keysCoder=" + keysCoder + ", valsCoder=" + valsCoder + "}";
  }
   ...
   */
  public DefaultLeafCoder(final IRabaCoder keysCoder, final IRabaCoder valsCoder) {

    if (keysCoder == null) throw new IllegalArgumentException();

    if (valsCoder == null) throw new IllegalArgumentException();
  ...
  public ILeafData encodeLive(final ILeafData leaf, final DataOutputBuffer buf) {
    ...
    //        final boolean hasHashKeys = leaf instanceof IBucketData; // @todo add hasHashKeys()
    // method?
    if (hasDeleteMarkers) {
    ...
    final ICodedRaba encodedKeys = keysCoder.encodeLive(leaf.getKeys(), buf);

    // encode the values into the buffer.
    final ICodedRaba encodedValues = valsCoder.encodeLive(leaf.getValues(), buf);
    ...
    buf.putInt(O_keysSize + AbstractReadOnlyNodeData.SIZEOF_KEYS_SIZE, encodedValues.data().len());
    ...
    if (hasVersionTimestamps) {
       ...
       * Note: We only write the deltas if there is more than one
        ...
        final int byteLength =
            BytesUtil.bitFlagByteLength(nkeys * versionTimestampBits /* nbits */);
  ...
  @Override
  public AbstractFixedByteArrayBuffer encode(final ILeafData leaf, final DataOutputBuffer buf) {
   ...
   * The keys and values are coded using a caller specified {@link IRabaCoder}. The specific coding
   * scheme is specified by the {@link IndexMetadata} for the B+Tree instance and is not stored
   * within the leaf data record. The use of prefix coding for keys is a good general choices, but
   * should not be used in combination with a hash tree unless an order preserving hashing function
   * is being used.
   *
  ...
  //	 * this manner).
  private static class ReadOnlyLeafData extends AbstractReadOnlyNodeData<ILeafData>
     ...
     * of {@link #nkeys} coded timestamp values (one per tuple). Those timestamps are coded in
     * {@link #versionTimestampBits} each.
     */
     ...
     */
    protected ReadOnlyLeafData(
        final AbstractFixedByteArrayBuffer buf, final ICodedRaba keys, final ICodedRaba values) {
     ...
     */
    protected ReadOnlyLeafData(
        final AbstractFixedByteArrayBuffer buf,
      ...
      if (valuesCoder == null) throw new IllegalArgumentException();
      ...
      throw new RuntimeException("nkeys=" + nkeys + ", vals.size=" + vals.size());
...
//         * For a leaf the #of tuples is always the #of keys.
    ...
    public final long getVersionTimestamp(final int index) {
      ...
      final long bitpos =
          ((O_versionTimestamps + (2L * SIZEOF_TIMESTAMP)) << 3)
              + ((long) index * versionTimestampBits);

      final long bitIndex = (b.off() << 3) + bitpos;

      final long deltat = BytesUtil.getBits64(b.array(), (int) bitIndex, versionTimestampBits);
    ...
    public final boolean getDeleteMarker(final int index) {
      ...
      if (!hasDeleteMarkers()) throw new UnsupportedOperationException();
    ...
    public final long getRawRecord(final int index) {
      ...
      if (!hasRawRecords()) throw new UnsupportedOperationException();

      final boolean flag = b.getBit((O_rawRecords << 3) + index);

      if (!flag) return IRawStore.NULL;
...
//		 * Visits the index of each bucket entry having a matching hash code.
    ...
    /** Return <code>true</code> if the leaf encodes the address or the prior and next leaves. */
    @Override
...
//         * Update the data record to set the prior and next leaf address.
    ...
    public final long getPriorAddr() {
      ...
      if (!isDoubleLinked()) throw new UnsupportedOperationException();
   ...
   */
  public static StringBuilder toString(final ILeafData leaf, final StringBuilder sb) {
