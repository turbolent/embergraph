...
public class IndexSegmentPlan {
   ...
   *
   * @see #numInLeaf numInLeaf, which is a reference to the last element of this array.
   */
   ...
   *
   * @param m The branching factor of the output tree (#of keys/values for a leaf or the #of
   *     children for a node).
   * @param nentries The #of entries in the tree.
   * @throws IllegalArgumentException if the branching factor is less than {@value
   *     Options#MIN_BRANCHING_FACTOR}.
   * @throws IllegalArgumentException if the #of index entries is negative (zero is allowed as a
   *     special case).
   */
  public IndexSegmentPlan(final int m, final long nentries) {

    if (m < Options.MIN_BRANCHING_FACTOR) throw new IllegalArgumentException();

    if (nentries < 0) throw new IllegalArgumentException();
    ...
    if (nentries == 0) {
      ...
      if (log.isInfoEnabled()) log.info("Empty tree.");
    ...
    for (int h = height - 1; h >= 0; h--) {
       ...
       */
      final long numThisLevel = (long) Math.ceil((double) nchildren / (double) m);
   ...
   * @param nleaves The #of leaves that must be addressable by the tree.
   * @throws UnsupportedOperationException if it is not possible to build a B+Tree with that
   *     branching factor and that many leaves without exceeding <i>maxHeight</i> (statically
   *     configured to <code>10</code>).
   */
  public static int getMinimumHeight(final int m, final long nleaves) {
    ...
    throw new UnsupportedOperationException(
        "Can not build tree with height less than "
            + maxHeight
            + " given branchingFactor="
            + m
            + ",  nleaves="
            + nleaves);
  }
   ...
   * capacity. To that end, we calculate the #of entries that would remain if we filled up n-1
   * leaves completely. If the #of remaining entries is less than or equal to the minimum capacity
   * of a leaf, then we have to adjust the allocation of entries such that the last leaf is at its
   * minimum capacity. This is done by computing the shortage and then distributing that shortage
   * among the leaves. Once we have deferred enough entries we are guaranteed that the final leaf
   * will not be under capacity.
   *
   * @param m The branching factor in the output tree.
   * @param m2 The minimum capacity for a leaf in the output tree, which is computed as (m+1)/2.
   * @param nleaves The #of leaves in the output tree.
   * @param nentries The #of entries to be inserted into the output tree.
   * @return An array indicating how many entries should be inserted into each leaf of the output
   *     tree. The array index is the leaf order (origin zero). The value is the capacity to which
   *     that leaf should be filled.
   * @throws IllegalArgumentException if there is a problem with the arguments.
  ...
  public static int[] distributeKeys(
      final int m, final int m2, final long nleaves, final long nentries) {

    if (m < Options.MIN_BRANCHING_FACTOR) throw new IllegalArgumentException();
    if (m > Options.MAX_INDEX_SEGMENT_BRANCHING_FACTOR) throw new IllegalArgumentException();
    if (m2 < (m + 1) / 2) throw new IllegalArgumentException();
    if (m2 > m) throw new IllegalArgumentException();
    if (nleaves <= 0) throw new IllegalArgumentException();
    ...
    if (nleaves == 1) {
      ...
      if (nentries > m) throw new RuntimeException();
   ...
   * difference when distributing children among nodes is that the result returned to the caller
   * must be interpreted as the #of children to assigned to each node NOT the #of keys (for leaves
   * the #of values and the #of keys is always the same).
   *
   * @param m The branching factor in the output tree.
   * @param m2 The minimum capacity, which should be computed as (m+1)/2.
   * @param nnodes The #of nodes in the output tree for some given level of the output tree.
   * @param nchildren The #of children to be distributed among those nodes.
   * @return An array indicating how many children should be inserted into each node of the output
   *     tree at the given level. The array index is the node order (origin zero). The value is the
   *     #of children which must be assigned to that leaf.
   * @see TestIndexSegmentPlan
   * @see TestIndexSegmentBuilderWithSmallTree#test_problem3_buildOrder3()
   */
