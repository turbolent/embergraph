 ...
 * primitive data types and Unicode strings. The total ordering imposed by the index is that of a
 * bit-wise comparison of the variable length unsigned byte[] keys. Encoding Unicode keys is support
 * by an integration with ICU4J and applications may choose the locale, strength, and other
 * properties that govern the sort order of sort keys generated from Unicode strings. Sort keys
 * produces by different collator objects are NOT compatible and applications that use Unicode data
 * in their keys MUST make sure that they use a collator that imposes the same sort order each time
 * they provision a {@link KeyBuilder}.
 *
 * <p>The use of variable length unsigned byte[] keys makes it possible for the B+-Tree to perform
 * very fast comparison of a search key with keys in the nodes and leaves of the tree. To support
 * fast search, the leading prefix is factored out each time a node or leaf is made immutable, e.g.,
 * directly proceeding serialization. Further, the separator keys are chosen to be the shortest
 * separator key in order to further shorten the keys in the nodes of the tree.
 *
 * <p>The B+Tree can optionally maintain version metadata (a version timestamp and deletion marker).
 * Version metadata MUST be enabled (a) if the index will be used with transactional isolation; or
 * (b) if the index is part of a scale-out index. In both cases the version timestamps and deletion
 * markers play a critical role when reading from a fused view of an ordered set of indices
 * describing an index or an index partition. Note that the existence of deletion markers means that
 * {@link #rangeCount(byte[], byte[])} and {@link #getEntryCount()} are upper bounds as deleted
 * entries will be reported until they are purged from the index by a compacting merge of the view.
 *
...
public abstract class AbstractBTree
   ...
   * changes in the reference to be visible to threads.
   */
   ...
   * backing store uses the {@link BTreeCounters} associated with that index since the {@link
   * DataService} was last (re-)started.
   *
   * @param btreeCounters The counters to be used.
   * @throws IllegalArgumentException if the argument is <code>null</code>.
   */
  public final void setBTreeCounters(final BTreeCounters btreeCounters) {

    if (btreeCounters == null) throw new IllegalArgumentException();
  ...
  static class LoadChildRequest {
    ...
    public boolean equals(final Object o) {
      ...
      if (!(o instanceof LoadChildRequest)) return false;
   ...
   * Node#childRefs}. This is package private since it must be visible to {@link
   * Node#_getChild(int)}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  static class ChildMemoizer
      extends Memoizer<LoadChildRequest /* request */, AbstractNode<?> /* child */> {

    /** @param c */
    public ChildMemoizer(final Computable<LoadChildRequest, AbstractNode<?>> c) {
   ...
   *
   * @param parent The node whose child will be materialized.
   * @param index The index of that child.
   * @return The child and never <code>null</code>.
   * @see Node#getChild(int)
   */
  AbstractNode<?> loadChild(final Node parent, final int index) {

    if (false && store instanceof Journal && ((Journal) store).getReadExecutor() != null) {
      ...
      final FutureTask<AbstractNode<?>> ft =
          new FutureTask<>(
              () -> memo.compute(new LoadChildRequest(parent, index)));
   ...
   * updates.
   *
   * <p>This hard reference is cleared to <code>null</code> if an index is {@link #close() closed}.
   * {@link #getRoot()} automatically uses {@link #reopen()} to reload the root so that closed
   * indices may be transparently made ready for further use (indices are closed to reduce their
   * resource burden, not to make their references invalid). The {@link AbstractNode} and derived
   * classes <em>assume</em> that the root is non-null. This assumption is valid if {@link #close()}
   * is invoked by the application in a manner consistent with the single-threaded contract for the
   * {@link AbstractBTree}.
   *
   * <p>Note: This field MUST be marked as [volatile] in order to guarantee correct semantics for
   * double-checked locking in {@link BTree#reopen()} and {@link IndexSegment#reopen()}.
   *
   ...
   * inconsistenly serialized) evicted pages. Once this becomes non- <code>null</code> the index
   * MUST be reloaded from the most recent checkpoint before it can be used (that is, you need to
   * obtain a new view of the unisolated index since this field is sticky once set).
   *
   * @see <a href="http://trac.blazegraph.com/ticket/1005">Invalidate BTree objects if error occurs
   *     during eviction </a>
   */
   ...
   * report by the bloom filter MUST be confirmed by testing against the index to verify that the
   * key was in fact present in the index. Since bloom filters do not support removal of keys, any
   * key that is deleted from the index will remain "present" in the bloom filter but a read against
   * the index will serve to disprove the existence of the key. The bloom filter is enabled by
   * specifying a non-zero value for the {@link IndexMetadata#getErrorRate()}.
   *
   * <p>Note: The bloom filter is read with the root node when the index is {@link #reopen()}ed and
   * is discarded when the index is {@link #close()}ed. While we do not use double-checked locking
   * on the {@link #bloomFilter}, it is assigned from within the same code that assigns the {@link
   * #root}. For historical reasons this is handled somewhat differently by the {@link BTree} and
   ...
   * necessary.
   *
   * @return The bloom filter -or- <code>null</code> if there is no bloom filter (including the case
   *     where there is a bloom filter but it has been disabled since the {@link BTree} has grown
   *     too large and the expected error rate of the bloom filter would be too high).
   */
   ...
   * WeakReference}s. Likewise, nodes refer to their children with a {@link WeakReference}. The hard
   * reference queue in combination with {@link #touch(AbstractNode)} and with hard references held
   * on the stack ensures that the parent and/or children remain reachable during operations. Once
   * the node is no longer strongly reachable weak references to that node may be cleared by the VM
   * - in this manner the node will become unreachable by navigation from its ancestors in the
   * btree. The special role of the hard reference queue is to further ensure that dirty nodes
   * remain dirty by defering persistence until the reference count for the node is zero during an
   * eviction from the queue.
   *
   * <p>Note that nodes are evicted as new nodes are added to the hard reference queue. This occurs
   * in two situations: (1) when a new node is created during a split of an existing node; and (2)
   * when a node is read in from the store. Inserts on this hard reference queue always drive
   * evictions. Incremental writes basically make it impossible for the commit set to get "too
   * large" - the maximum #of nodes to be written is bounded by the size of the hard reference
   * queue. This helps to ensure fast commit operations on the store.
   *
   * <p>The minimum capacity for the hard reference queue is two (2) so that a split may occur
   * without forcing eviction of either node participating in the split.
   *
   * <p>Note: The code in {@link Node#postOrderNodeIterator(boolean, boolean)} and {@link
   * DirtyChildIterator} MUST NOT touch the hard reference queue since those iterators are used when
   * persisting a node using a post-order traversal. If a hard reference queue eviction drives the
   * serialization of a node and we touch the hard reference queue during the post-order traversal
   * then we break down the semantics of {@link HardReferenceQueue#add(Object)} as the eviction does
   * not necessarily cause the queue to reduce in length. Another way to handle this is to have
   * {@link HardReferenceQueue#add(Object)} begin to evict objects before is is actually at
   * capacity, but that is also a bit fragile.
   *
   * <p>Note: The {@link #writeRetentionQueue} uses a {@link HardReferenceQueue}. This is based on a
   * {@link RingBuffer} and is very fast. It does not use a {@link HashMap} because we can resolve
   * the {@link WeakReference} to the child {@link Node} or {@link Leaf} using top-down navigation
   * as long as the {@link Node} or {@link Leaf} remains strongly reachable (that is, as long as it
   * is on the {@link #writeRetentionQueue} or otherwise strongly held). This means that lookup in a
   * map is not required for top-down navigation.
   *
   * <p>
   *
   * @todo consider a policy that dynamically adjusts the queue capacities based on the height of
   *     the btree in order to maintain a cache that can contain a fixed percentage, e.g., 5% or
   *     10%, of the nodes in the btree. The minimum and maximum size of the cache should be
   *     bounded. Bounding the minimum size gives better performance for small trees. Bounding the
   *     maximum size is necessary when the trees grow very large. (Partitioned indices may be used
   *     for very large indices and they can be distributed across a cluster of machines.)
   *     <p>There is a discussion of some issues regarding such a policy in the code inside of
   ...
   * @see BLZG-1665 (Reduce commit latency by parallel checkpoint by level of dirty pages in an
   *     index)
   */
   ...
   * applied when there is a significantly opportunity for concurrent eviction, such as during a
   * checkpoint against an index with a large dirty write set.
   *
   * @see #writeNodeRecursiveConcurrent(AbstractNode)
   * @see BLZG-1665 (Reduce commit latency by parallel checkpoint by level of dirty pages in an
   *     index)
   ...
   * the {@link BTreeCounters}s.
   *
   * <p>Note: counters reporting directly on the {@link AbstractBTree} use a snapshot mechanism
   * which prevents a hard reference to the {@link AbstractBTree} from being attached to the return
   * {@link CounterSet} object. One consequence is that these counters will not update until the
   * next time you invoke {@link #getCounters()}.
   *
   * <p>Note: In order to snapshot the counters use {@link OneShotInstrument} to prevent the
   * inclusion of an inner class with a reference to the outer {@link AbstractBTree} instance.
   *
   * <p>Note: This method reports information that is NOT available from {@link BTreeCounters}s.
   * Some of this information is either static or relatively static (e.g., the {@link UUID} for this
   * index or its implementation class). Other information is dynamic (such as the #of nodes and
   * leaves or the current size of the writeRetentionQueue). The dynamic information is valid for
   * the specific index view as of the moment that it is sampled.
   *
   ...
   * @todo estimate heap requirements for nodes and leaves based on their state (keys, values, and
   *     other arrays). report estimated heap consumption here.
   */
  public final CounterSet getCounters() {
     ...
     */
    {
      ...
      final long bytes =
          btreeCounters.bytesOnStore_nodesAndLeaves.get()
   ...
   *     read-only.
   * @param metadata The {@link IndexMetadata} object for this B+Tree.
   * @param recordCompressorFactory Object that knows how to (de-)compress the serialized data
   *     records.
   */
  protected AbstractBTree(
    ...
    if (branchingFactor < Options.MIN_BRANCHING_FACTOR) throw new IllegalArgumentException();
    ...
    maxParallelEvictThreads =
        Integer.parseInt(
            System.getProperty(
                IndexMetadata.Options.MAX_PARALLEL_EVICT_THREADS,
    ...
    minDirtyListSizeForParallelEvict =
        Integer.parseInt(
            System.getProperty(
                IndexMetadata.Options.MIN_DIRTY_LIST_SIZE_FOR_PARALLEL_EVICT,
   ...
   *
   * <p>This implementation clears the hard reference queue (releasing all node references),
   * releases the hard reference to the root node, and releases the buffers on the {@link
   * NodeSerializer} (they will be naturally reallocated on reuse).
   *
   * <p>Note: {@link AbstractBTree} is NOT thread-safe and {@link #close()} MUST be invoked in a
   * context in which there will not be concurrent threads -- the natural choice being the
   * single-threaded commit service on the journal.
   *
   * @exception IllegalStateException if the root is <code>null</code>, indicating that the index is
   *     already closed.
   * @exception IllegalStateException if the root is dirty (this implies that this is a mutable
   *     btree and there are mutations that have not been written through to the store)
   */
    ...
    if (INFO || EmbergraphStatics.debug) {
      ...
      final String msg =
          "BTree close: name="
              ...
              + getEntryCount()
              + ", impl="
              + (this instanceof BTree
                  ? ((BTree) this).getCheckpoint().toString()
   ...
   * {@link #root} is <code>null</code> (indicating that the index has been closed). This method is
   ...
   * guaranteed to hold a lock on <i>this</i>. This is done to ensure that at most one thread gets
   * to re-open the index from the backing store.
   */
  ...
  /** Return <code>true</code> iff this is a transient data structure (no backing store). */
  ...
  protected final void assertNotReadOnly() {
    ...
    if (error != null)
      throw new IndexInconsistentError(
          ERROR_ERROR_STATE + ": " + this.getIndexMetadata().getName() + ", store: " + store,
          error);
  }
   ...
   * {@link BTree} is first created and will remain ZERO (0L) until the {@link BTree} is committed.
   * If the backing store does not support atomic commits, then this value will always be ZERO (0L).
   *
   * <p>Note: This is fixed for an {@link IndexSegment}.
   *
   ...
   *
   * <p>The revision timestamp assigned by this method is <code>lastCommitTime+1</code>. The
   * reasoning is as follows. Revision timestamps are assigned by the transaction manager when the
   * transaction is validated as part of its commit protocol. Therefore, revision timestamps are
   * assigned after the transaction write set is complete. Further, the assigned revisionTimestamp
   * will be strictly LT the commitTime for that transaction. By using <code>lastCommitTime+1</code>
   * we are guaranteed that the revisionTimestamp for new writes (which will be part of some future
   * commit point) will always be strictly GT the revisionTimestamp of historical writes (which were
   * part of some prior commit point).
   *
   * <p>Note: Unisolated operations using this timestamp ARE NOT validated. The timestamp is simply
   * applied to the tuple when it is inserted or updated and will become part of the restart safe
   * state of the B+Tree once the unisolated operation participates in a commit.
   *
   * <p>Note: If an unisolated operation were to execute concurrent with a transaction commit for
   * the same index then that could produce inconsistent results in the index and could trigger
   * concurrent modification errors. In order to avoid such concurrent modification errors,
   * unisolated operations which are to be mixed with full transactions MUST ensure that they have
   * exclusive access to the unisolated index before proceeding. There are two ways to do this: (1)
   * take the application off line for transactions; (2) submit your unisolated operations to the
   * {@link IConcurrencyManager} which will automatically impose the necessary constraints on
   * concurrent access to the unisolated indices.
   *
  ...
  public final IResourceMetadata[] getResourceMetadata() {
    ...
    if (store == null) {
       ...
       * case.
       */
      return new IResourceMetadata[] {new TransientResourceMetadata(metadata.getIndexUUID())};
    }
   ...
   * modification. However, only a single cloned copy of the metadata record will be shared between
   * all callers for a given instance of this class.
   *
   * @todo the clone once policy is a compromise between driving the read only semantics into the
   *     {@link IndexMetadata} object, cloning each time, and leaving it to the caller to clone as
   *     required. Either the former or the latter might be a better choice.
   *     <p>FIXME if the metadata record is updated (and it can be) then we really need to
   *     invalidate the cloned metadata record also.
  ...
  public String toString() {
    ...
    sb.append(", m=").append(getBranchingFactor());
   ...
   * index partition.
   *
   * <p>Note: An index partition is identified by {@link IndexMetadata#getPartitionMetadata()}
   * returning non- <code>null</code>.
   *
   * <p>Note: This method is used liberally in <code>assert</code>s to detect errors that can arise
   * is client code when an index partition is split, joined, or moved.
   *
   * @param key The key.
   ...
   *     <p>The method is disabled because it forces the caller to ensure that the query lies within
   *     the specified range. While that is all well and good, this places an undue burden on {@link
   *     FusedView} which must substitute the actual key range of a shard view for the caller's key
   *     range. This makes the test effectively into a self-check of whether {@link FusedView} has
   *     correctly done this substitution rather than a check of whether a request was mapped onto
   *     the correct shard, which was the original purpose of this test. The logic for performing
   *     such range checks was moved into {@link RangeCheckUtil}.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/461">KeyAfterPartitionException
   *     </a>
   */
  protected final boolean rangeCheck(final byte[] key, final boolean allowUpperBound) {
  ...
  /** The object responsible for (de-)serializing the nodes and leaves of the {@link IIndex}. */
   ...
   * method automatically {@link #reopen()}s the index if it is closed, making it available for use.
   */
   ...
   *
   * @param key The key.
   * @return Either the root node of the tree or a recently touched leaf that is known to span the
   *     key.
   * @todo This is a place holder for finger(s) for hot spots in the B+Tree, but the finger(s) are
   *     currently disabled.
   *     <p>One way to implement fingers is to track the N most frequent leaf accesses using a
   *     streaming algorithm.
   */
   ...
   * {@link #insert(Object, Object)} and {@link #remove(Object)}.
   *
   * <p>While the {@link Tuple} is not safe for use by concurrent threads, the mutation API is not
   * safe for concurrent threads either.
   */
   ...
   * {@link AbstractBTree}.
   *
   * <p>While the returned {@link Tuple} is not safe for use by concurrent threads, each instance
   * returned by this method is safe for use by the thread in which it was obtained.
   *
   ...
   *
   * <p>The tuple does not copy either the keys or the values. Contains is implemented as a lookup
   * operation that either return this tuple or <code>null</code>. When isolation is supported, the
   * version metadata is examined to determine if the matching entry is flagged as deleted in which
   * case contains() will report "false".
   *
   * <p>While the returned {@link Tuple} is not safe for use by concurrent threads, each instance
   * returned by this method is safe for use by the thread in which it was obtained.
   *
  ...
  //    };
  //
  //    private final ThreadLocal<WeakReference<Tuple>> containsTupleRef = new
  // ThreadLocal<WeakReference<Tuple>>() {
  //
  //        @Override
  //        protected WeakReference<Tuple> initialValue() {
   ...
   *
   * <p>Note: This field is NOT static. This limits the scope of the {@link ThreadLocal} {@link
   ...
   * to determine if the matching entry is flagged as deleted in which case contains() will report
   * "false".
   *
   * <p>Note: This field is NOT static. This limits the scope of the {@link ThreadLocal} {@link
   * Tuple} to the containing {@link AbstractBTree} instance.
  ...
  @Override
  public final byte[] insert(final byte[] key, final byte[] value) {
  ...
  @Override
  public final byte[] putIfAbsent(final byte[] key, final byte[] value) {
   ...
   * @param value The variable length byte[] value (MAY be <code>null</code>).
   * @param delete <code>true</code> iff the index entry should be marked as deleted (this behavior
   *     is supported iff the btree supports delete markers).
   * @param putIfAbsent When <code>true</code>, a pre-existing entry for the key will NOT be
   *     replaced (unless it is a deleted tuple, which is the same as if there was no entry under
   *     the key). This should ONLY be true when the top-level method is <code>putIfAbsent</code>.
   *     Historical code paths should specify false for an unconditional mutation. See BLZG-1539.
   * @param timestamp The timestamp to be associated with the new or updated index entry (required
   *     iff the btree supports transactional isolation and otherwise 0L).
   * @param tuple Data and metadata for the old value under the key will be copied onto this object
   *     (optional).
   * @return <i>tuple</i> -or- <code>null</code> if there was no entry under that key. See {@link
   *     ITuple#isDeletedVersion()} to determine whether or not the entry is marked as deleted.
   * @throws UnsupportedOperationException if the index is read-only.
   */
  public final Tuple insert(
      final byte[] key,
      final byte[] value,
    ...
    final Tuple oldTuple =
        getRootOrFinger(key).insert(key, value, delete, putIfAbsent, timestamp, tuple);
    ...
    if (oldTuple == null) {
      ...
      if (filter != null) {
        ...
        if (getEntryCount() > filter.getMaxN()) {
          ...
          if (INFO)
            log.info(
                "Bloom filter disabled - maximum error rate would be exceeded"
                    + ": entryCount="
                    + getEntryCount()
                    + ", factory="
  ...
  @Override
  public final byte[] remove(final byte[] key) {
   ...
   * instead of this method to mark the index entry as deleted when delete markers are being
   * maintained.
   *
   * <p>Note: removing a key has no effect on the optional bloom filter. If a key is removed from
   * the index by this method then the bloom filter will report a <em>false positive</em> for that
   * key, which will be detected when we test the index itself. This works out fine in the scale-out
   * design since the bloom filter is per {@link AbstractBTree} instance and split/join/move
   * operations all result in new mutable {@link BTree}s with new bloom filters to absorb new
   * writes. For a non-scale-out deployment, this can cause the performance of the bloom filter to
   * degrade if you are removing a lot of keys. However, in the special case of a {@link BTree} that
   * does NOT use delete markers, {@link BTree#removeAll()} will create a new root leaf and a new
   * (empty) bloom filter as well.
   *
   * @param key The search key.
   * @param tuple An object that will be used to report data and metadata for the pre-existing
   *     version (optional).
   * @return <i>tuple</i> or <code>null</code> if there was no version under that key.
   * @throws UnsupportedOperationException if delete markers are being maintained.
   * @throws UnsupportedOperationException if the index is read-only.
   */
  public final Tuple remove(final byte[] key, final Tuple tuple) {
   ...
   * indices vs removing the entries in an individual {@link AbstractBTree}.
   */
  @Override
   ...
   * bloom filter exists and reports <code>true</code>, then looks up the value for the key in the
   * index (note that the key might not exist in the index since a bloom filter allows false
   * positives, further the key might exist for a deleted entry).
   *
   * @param key The search key.
   * @param tuple An object that will be used to report data and metadata for the pre-existing
   *     version (required).
   * @return <i>tuple</i> or <code>null</code> if there is no entry in the index under the key.
   */
   ...
   *
   * @return <code>true</code> iff the key does not exist. Or, if the btree supports isolation, if
  ...
  public boolean contains(final byte[] key) {
    ...
    final ITuple tuple = getRootOrFinger(key).lookup(key, getContainsTuple());
  ...
  public byte[] keyAt(final long index) {
    ...
    if (index < 0) throw new IndexOutOfBoundsException(ERROR_LESS_THAN_ZERO);

    if (index >= getEntryCount()) throw new IndexOutOfBoundsException(ERROR_TOO_LARGE);
  ...
  public final Tuple valueAt(final long index, final Tuple tuple) {

    if (index < 0) throw new IndexOutOfBoundsException(ERROR_LESS_THAN_ZERO);

    if (index >= getEntryCount()) throw new IndexOutOfBoundsException(ERROR_TOO_LARGE);

    if (tuple == null || !tuple.getValuesRequested()) throw new IllegalArgumentException();
  ...
  @Override
  public final long rangeCountExact(final byte[] fromKey, final byte[] toKey) {
    ...
    final Iterator itr =
        rangeIterator(fromKey, toKey, 0 /* capacity */, 0 /* flags */, null /* filter */);
  ...
  public final long rangeCount(Object fromKey, Object toKey) {
    ...
    fromKey = fromKey == null ? null : metadata.getTupleSerializer().serializeKey(fromKey);

    toKey = toKey == null ? null : metadata.getTupleSerializer().serializeKey(toKey);
   ...
   * AbstractNode#indexOf(Object)}. Since it does not scan the tuples it can not differentiate
   * between deleted and undeleted tuples for an index that supports delete markers, but the result
   * will be exact if delete markers are not being used. The cost is equal to the cost of lookup of
   * the both keys. If both keys are <code>null</code>, then the cost is zero (no IOs).
   */
  @Override
  public final long rangeCount(final byte[] fromKey, final byte[] toKey) {
   ...
   * must report the exact range count when considering all sources at once. It uses a range
   * iterator scan visiting both deleted and undeleted tuples for that.
   */
  @Override
   ...
   */
  public final ITupleIterator rangeIterator(
      Object fromKey, Object toKey, final int capacity, final int flags, final IFilter filter) {
   ...
   *
   * <p>Note: If {@link IRangeQuery#CURSOR} is specified the returned iterator supports traversal
   * with concurrent modification by a single-threaded process (the {@link BTree} is NOT thread-safe
   * for writers). Write are permitted iff {@link AbstractBTree} allows writes.
   *
   * <p>Note: {@link IRangeQuery#REVERSE} is handled here by wrapping the underlying {@link
   * ITupleCursor}.
   *
   * <p>Note: {@link IRangeQuery#REMOVEALL} is handled here by wrapping the iterator.
   *
   * <p>Note: {@link FusedView#rangeIterator(byte[], byte[], int, int, IFilter)} is also responsible
   * for constructing an {@link ITupleIterator} in a manner similar to this method. If you are
   * updating the logic here, then check the logic in that method as well!
   *
   * @todo add support to the iterator construct for filtering by a tuple revision timestamp range.
   */
  @Override
  public ITupleIterator rangeIterator(
    ...
    if ((this instanceof BTree)
      ...
      if (this instanceof IndexSegment) {
        ...
        if (true
            && ((flags & REVERSE) == 0)
            && ((flags & CURSOR) == 0)
            && (seg.getStore().getCheckpoint().maxNodeOrLeafLength <= pool.getBufferCapacity())
            && ((rangeCount(fromKey, toKey) / branchingFactor) > 2)) {

          src = new IndexSegmentMultiBlockIterator(seg, pool, fromKey, toKey, flags);
        ...
        if (isReadOnly()) {
          ...
          src = new ReadOnlyBTreeTupleCursor(((BTree) this), tuple, fromKey, toKey);

        } else {

          // Note: this iterator supports traversal with concurrent
          // modification.
          src = new MutableBTreeTupleCursor(((BTree) this), tuple, fromKey, toKey);
        }

      } else {

        throw new UnsupportedOperationException(
            "Unknown B+Tree implementation: " + this.getClass().getName());
      }
    ...
    if (filter != null) {
      ...
      src = new WrappedTupleIterator(filter.filter(src, null /* context */));
    }
   ...
   * @param fromKey The first key that will be copied (inclusive). When <code>null</code> there is
   *     no lower bound.
   * @param toKey The first key that will NOT be copied (exclusive). When <code>null</code> there is
   *     no upper bound.
   * @param overflow When <code>true</code> the {@link IOverflowHandler} defined for the source
   *     index (if any) will be applied to copy raw records onto the backing store for this index.
   *     This value SHOULD be <code>false</code> if the two indices have the same backing store and
   *     <code>true</code> otherwise.
   * @throws UnsupportedOperationException if the <i>src</i> and <i>this</i> do not have the same
   ...
   * @see CompactTask
   * @see OverflowManager
   * @todo this would be more efficient if we could reuse the same buffer for keys and values in and
   *     out. As it stands it does a lot of byte[] allocation.
   * @todo write tests for all variations (delete markers, timestamps, overflow handler, etc).
   */
  public long rangeCopy(
      final IIndex src, final byte[] fromKey, final byte[] toKey, final boolean overflow) {

    assertNotReadOnly();

    final ITupleIterator itr =
        src.rangeIterator(fromKey, toKey, 0 /* capacity */, IRangeQuery.ALL, null /* filter */);
    ...
    final IOverflowHandler overflowHandler =
        (overflow ? sourceMetadata.getOverflowHandler() : null);
   ...
   * @return The right-most child in the B+Tree -or- <code>null</code> if the root of the B+Tree is
   *     a leaf and <code>nodesOnly == true</code>.
   */
    ...
    if (info) {
      ...
      out.println(
          "branchingFactor="
              ...
              + nentries
              + ", nodeUtil="
              + utils.getNodeUtilization()
              + "%, leafUtil="
              + utils.getLeafUtilization()
              + "%, utilization="
   ...
   * @return ZERO (0) iff <i>t == root</i> and otherwise the number of levels that <i>t</i> is below
   *     the root.
   * @throws IllegalArgumentException if <i>t</i> is null.
   * @throws IllegalArgumentException if <i>t</i> does not belong to this B+Tree.
   */
   ...
   *
   * @param t A node or leaf that belongs to this B+Tree.
   * @param node A node or leaf that is either <i>t</i> or (recursively) some parent of <i>t</i>.
   * @return ZERO (0) iff <i>t == root</i> and otherwise the number of levels that <i>t</i> is below
   *     the root.
   * @throws IllegalArgumentException if <i>t</i> is null.
   * @throws IllegalArgumentException if <i>t</i> does not belong to this B+Tree.
   * @throws IllegalArgumentException if <i>node</i> is null.
   * @throws IllegalArgumentException if <i>node</i> does not belong to this B+Tree.
   * @throws NotChildException if <i>t</i> is neither <i>node</node> nor some descendant of
   *     <i>node</i>.
   */
  @SuppressWarnings("rawtypes")
  public int getLevel(final AbstractNode t, final AbstractNode node) {
    ...
    if (t.btree != this) throw new IllegalArgumentException();
    ...
    if (level == -1) {
      ...
      throw new NotChildException(
          "Not a child of the given node: t=" + t.toShortString() + ", node=" + node);
    }
   ...
   * [node].
   *
   * @param t Either node or a descendant of node.
   * @param node Some node.
   * @param level Zero on the first invocation.
   * @return The discovered level of [t] beneath [node], ZERO (0) iff t==node, and -1 if [t] is not
   *     dominated by [node].
   */
  @SuppressWarnings("rawtypes")
   ...
   * children). The concurrency requirements and the implementation behavior and guarentees differ
   * depending on whether the B+Tree is read-only or mutable for two reasons: For writers, the
   * B+Tree is single-threaded so there is no contention. For readers, every touch on the B+Tree
   * goes through this point, so it is vital to make this method non-blocking.
   *
   * <h3>Writers</h3>
   *
   * <p>This method guarantees that the specified node will NOT be synchronously persisted as a side
   * effect and thereby made immutable. (Of course, the node may be already immutable.)
   *
   * <p>In conjunction with {@link DefaultEvictionListener}, this method guarantees that the
   * reference counter for the node will reflect the #of times that the node is actually present on
   * the {@link #writeRetentionQueue}.
   *
   * <p>If the node is not found on a scan of the head of the queue, then it is appended to the
   * queue and its {@link AbstractNode#referenceCount} is incremented. If a node is being appended
   * to the queue and the queue is at capacity, then this will cause a reference to be evicted from
   * the queue. If the reference counter for the evicted node or leaf is zero and the evicted node
   * or leaf is dirty, then a data record will be coded for the evicted node or leaf and written
   * onto the backing store. A subsequent attempt to modify the node or leaf will force
   * copy-on-write for that node or leaf.
   *
   * <p>For the mutable B+Tree we also track the #of references to the node/leaf on the ring buffer.
   * When that reference count reaches zero we do an eviction and the node/leaf is written onto the
   * backing store if it is dirty. Those reference counting games DO NOT matter for read-only views
   * so we can take a code path which does not update the per-node/leaf reference count and we do
   * not need to use either synchronization or atomic counters to track the reference counts.
   *
   * <h3>Readers</h3>
   *
   * <p>In order to reduce contention for the lock required to update the backing queue, the {@link
   * #writeRetentionQueue} is configured to collect references for touched nodes or leaves in a
   * thread-local queue and then batch those references through to the backing hard reference queue
   * while holding the lock.
   *
   * @param node The node or leaf.
   * @todo The per-node/leaf reference counts and {@link #ndistinctOnWriteRetentionQueue} fields are
   *     not guaranteed to be consistent for of concurrent readers since no locks are held when
   *     those fields are updated. Since the reference counts only effect when a node is made
   *     persistent (assuming its dirty) and since we already require single threaded access for
   *     writes on the btree, this does not cause a problem but can lead to unexpected values for
   *     the reference counters and {@link #ndistinctOnWriteRetentionQueue}.
   * @todo This might be abstract here and final in BTree and in IndexSegment. For {@link
   *     IndexSegment}, we know it is read-only so we do not need to test anything. For {@link
   *     BTree}, we can break encapsulation and check whether or not the {@link BTree} is read-only
   *     more readily than we can in this class.
   */
  //    synchronized
  //    final
  protected void touch(final AbstractNode<?> node) {
     ...
     * test relies on the UnisolatedReadWriteIndex class to provide concurrency control for such
     * interleaved read-only and mutation operations on an unisolated (aka mutable) index.
     *
     * <p>There are three broad ways in which concurrency controls for the index classes are
     * realized:
     *
     * <p>(1) Explicit synchronization. For example, the AbstractJournal uses explicit
     * synchronization to protect operations on the unisolated Name2Addr.
     *
     * <p>(2) Explicit pre-declaration of ordered locks. The ConcurrencyManager and AbstractTask
     * support this protection mechanism. The task runs once it has acquired the locks for the
     * declared unisolated indices.
     *
     * <p>(3) UnisolatedReadWriteIndex. This is used to provide transparent concurrency control for
     * unisolated indices for the triple and quad store classes.
     *
     * <p>The index is mutable (unisolated view). If the thread owns a read-only lock then the
     * operation is read-only and we MUST NOT drive evictions from this thread.
     *
     * <p>Note: The order in which we obtain the real read lock and increment (and decrement) the
     * per-thread read lock counter on the AbstractBTree is not critical because
     * AbstractBTree.touch() relies on the thread both owning the read lock and having the
   ...
   * be coordinated via a lock to prevent concurrent modification when touches drive the eviction of
   * a dirty node or leaf.
   *
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/71 (Concurrency problem with unisolated
   *     btree and memoizer)
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/201 (Hot spot in AbstractBTree#touch())
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/284 (IndexOfOfBounds? in Node#getChild())
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/288 (Node already coded)
   *     <p>and possibly
   ...
   * commit protocol where it is invoked with the root of the tree, but it may also be used to
   * incrementally flush dirty non-root {@link Node}s.
   *
   * <p>Note: This will throw an exception if the backing store is read-only.
   *
   * @param node The root of the hierarchy of nodes to be written. The node MUST be dirty. The node
   *     this does NOT have to be the root of the tree and it does NOT have to be a {@link Node}.
   * @see BLZG-1665 (Reduce commit latency by parallel checkpoint by level of dirty pages in an
   *     index)
   */
  protected final void writeNodeRecursive(final AbstractNode<?> node) {
   ...
   * @param node
   */
  protected final void writeNodeRecursiveCallersThread(final AbstractNode<?> node) {
     ...
     */
    final Iterator<AbstractNode> itr =
        node.postOrderNodeIterator(true /* dirtyNodesOnly */, false /* nodesOnly */);
    ...
    //
    //            final String s = "wrote: "+(metadata.getName()!=null?"name="+metadata.getName()+",
    // ":"") + ndirty + " records (#nodes=" + nnodes
   ...
   * {@link #writeCheckpoint()} or {@link #touch(AbstractNode)}, we have the same contract with
   * respect to eviction as the single-threaded eviction logic - we are just doing it in parallel
   * level sets.
   *
   * @param node
   * @see BLZG-1665 (Reduce commit latency by parallel checkpoint by level of dirty pages in an
   ...
   */
  @SuppressWarnings("rawtypes")
  protected final void writeNodeRecursiveConcurrent(final AbstractNode<?> node) {
    ...
    while (itr.hasNext()) {
      ...
      // decendentOrSelfOf="+node.toShortString());
    ...
    assert dirtyLevelCount <= getHeight() + 1
        : "dirtyLevelCount="
            + dirtyLevelCount
            + ", height="
            + getHeight()
    ...
    for (int i = dirtyLevelCount - 1; i >= 0; i--) {
      ...
      if (dirtyList == null)
        throw new AssertionError(
            "Dirty list not found: level=" + i + ", #levels=" + dirtyLevelCount);
      ...
      if (dirtyListSize < minDirtyListSizeForParallelEvict) {
        ...
        if (log.isInfoEnabled())
          log.info(
              "Evicting " + dirtyListSize + " dirty nodes/leaves using " + nparallel + " threads.");
        ...
        final LatchedExecutor executor =
            new LatchedExecutor(((IIndexManager) getStore()).getExecutorService(), nparallel);
   ...
   * will NOT be visited. By coding the nodes and leaves as they are evicted from the {@link
   * #writeRetentionQueue}, the B+Tree continuously converts nodes and leaves to their more compact
   * coded record forms which results in a smaller in memory footprint.
   *
   * <p>Note: For a transient B+Tree, this merely codes the node but does not write the node on the
   * store (there is none).
   *
   * @throws UnsupportedOperationException if the B+Tree (or the backing store) is read-only.
  ...
  private long writeNodeOrLeaf(final AbstractNode<?> node, final NodeSerializer nodeSer) {
    ...
    if (error != null) throw new IllegalStateException(ERROR_ERROR_STATE, error);
     ...
     * that one task could be interrupted during an eviction from the write retention queue and that
     * another task could obtain the UnisolatedReadWriteIndex lock and then hit the error since the
     * BTree, the write retention queue, and the nodes that were being evicted would an inconsistent
     * state state. The test does not fail if it is run to completion (no timeout).
     *
     * @see <a href="http://trac.blazegraph.com/ticket/343" >Stochastic assert in
     *     AbstractBTree#writeNodeOrLeaf() in CI </a>
     *     <p>TestMROWTransactions might also demonstrate an issue occasionally. If so, then check
    ...
    if (store == null) {
      ...
      //            	log.warn("Concurrent modification of thread guard", new RuntimeException("WTF2:
      // " + node.hashCode()));
      //
    ...
    if (storeCache != null) {
      ...
      if (null != storeCache.putIfAbsent(addr, node.getDelegate())) {
        ...
        throw new AssertionError("addr already in cache: " + addr + " for " + store.getFile());
      }
    }
   ...
   * desired node or leaf in from the store and attach the reference to the newly read node or leaf
   * as appropriate into the existing data structures (e.g., as the root reference or as a child of
   * a node or leaf already live in memory).
   *
   * @param addr The address in the store.
   * @return The node or leaf.
   * @throws IllegalArgumentException if the address is {@link IRawStore#NULL}.
   */
  protected AbstractNode<?> readNodeOrLeaf(final long addr) {
    ...
    // Note: This is not necessary.  The most likely place to be interrupted is in the IO on the raw
    // store.  It is not worth testing for an interrupt here since we are more liklely to notice one
    // in the raw store and this method is low latency except for the potential IO read.
   ...
   * @see WeakReference
   */
  final <T extends AbstractNode<T>> Reference<AbstractNode<T>> newRef(final AbstractNode<T> child) {
   ...
   * {@link Reference} objects for {@link AbstractNode#self}, {@link AbstractNode#parent}, {@link
   * Node#childRefs}, etc. and on the other hand by the impossibility of defining your own direct
   * subclass of {@link Reference} (a runtime security manager exception will result).
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @param <T>
   */
   ...
   *
   * @param recordAddrBuf The buffer that will be used to format the record address.
   * @param addr The raw record address.
   * @return A newly allocated byte[] which encodes that address.
   */
  public static byte[] encodeRecordAddr(final ByteArrayBuffer recordAddrBuf, final long addr) {
   ...
   */
  public static long decodeRecordAddr(final byte[] buf) {
   ...
   *
   * @return The maximum size of an inline <code>byte[]</code> value before it is promoted to a raw
   *     record.
   */
  int getMaxRecLen() {
   ...
   *     <p>FIXME Add raw record compression.
   */
  ...
  long writeRawRecord(final byte[] b) {
    ...
    if (isReadOnly()) throw new IllegalStateException(ERROR_READ_ONLY);
   ...
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/434#comment:2">Simplify tracking
   *     of address release for B+Tree and HTree </a>
   */
   ...
   * {@link BTreeCounters}.
   *
   * @param addr The address to be recycled.
   * @return The #of bytes which were recycled and ZERO (0) if the address is {@link
   *     IRawStore#NULL}.
   */
  protected int recycle(final long addr) {

    if (addr == IRawStore.NULL) return 0;
