...
public interface IReadWriteLockManager {
   ...
   * processes which need to obtain the write lock to coordinate external operations.
   *
   * <p>Note: If the persistence capable data structure is read-only then the returned {@link Lock}
   * is a singleton that ignores all lock requests. This is because our read-only persistence
   * capable data structures are already thread-safe for concurrent readers.
   *
   * @return The lock.
   */
  Lock readLock();
   ...
   * write lock to coordinate external operations.
   *
   * @return The lock.
   * @throws UnsupportedOperationException unless the view supports mutation.
   */
  Lock writeLock();
   ...
   *     read-only (read locks are not tracked for a read-only index view).
   */
