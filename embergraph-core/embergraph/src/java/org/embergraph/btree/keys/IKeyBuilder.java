 ...
 *
 * <p>A sort key is an unsigned byte[] that preserves the total order of the original data. Sort
 * keys may potentially be formed from multiple fields but field markers do not appear within the
 * resulting sort key. While the original values can be extracted from sort keys (this is true of
 * all the fixed length fields, such as int, long, float, or double) they can not be extracted from
 * Unicode variable length fields (the collation ordering for a Unicode string depends on the {@link
 * Locale}, the collation strength, and the decomposition mode and is a non-reversable operation).
 *
 * <h2>Unicode</h2>
 *
 * <p>Factory methods are defined by {@link KeyBuilder} for obtaining instances of this interface
 * that optionally support Unicode. Instances may be created for a given {@link Locale}, collation
 * strength, decomposition mode, etc.
 *
 * <p>The ICU library supports generation of compressed Unicode sort keys and is used by default
 * when available. The JDK {@link java.text} package also supports the generation of Unicode sort
 * keys, but it does NOT produce compressed sort keys. The resulting sort keys are therefore (a)
 * incompatible with those produced by the ICU library and (b) much larger than those produced by
 * the ICU library.
 *
 * <p>Support for Unicode MAY be disabled using {@link Options#COLLATOR}, by using {@link
 * KeyBuilder#newInstance()} or another factory method that does not enable Unicode support, or by
 * using one of the {@link KeyBuilder} constructors that does not support Unicode.
 *
 * <h2>Multi-field keys with variable length fields</h2>
 *
 * <p>Multi-field keys in which variable length fields are embedded within the key present a special
 * problem. Any run of fixed length fields can be compared as unsigned byte[]s. Likewise, any any
 * key with a fixed length prefix (including zero) but a variable length field in its tail can also
 * be compared directly as unsigned byte[]s. However, the introduction of a variable length field
 * into any non-terminal position in a multi-field key must be handled specially since simple
 * concatenation of the field keys will NOT produce the correct total ordering. (This is why SQL
 * requires that text fields compare as if they were padded out with ASCII blanks (0x20) to some
 * maximum length for the field.) A utility method exists specifically for this purpose - see {@link
...
public interface IKeyBuilder extends ISortKeyBuilder<Object>, IManagedByteArray {
   ...
   *
   * <p>Note that keys are <em>donated</em> to the btree so it is important to allocate new keys
   * when running in the same process space. When using a network api, the api provides the
   ...
   *
   * <p>Note: The {@link SuccessorUtil#successor(String)} of a string is formed by appending a
   * trailing <code>nul</code> character. However, since <code>IDENTICAL</code> appears to be
   * required to differentiate between a string and its successor (with the trailing <code>nul
   * </code> character), you MUST form the sort key first and then its successor (by appending a
   * trailing <code>nul</code>). Failure to follow this pattern will lead to the successor of the
   * key comparing as EQUAL to the key. For example,
   *
   * <pre>
   *
   ...
   *
   * @param s A string.
   * @throws UnsupportedOperationException if Unicode is not supported.
   * @return <i>this</i>
   ...
   *     <p>FIXME update the javadoc further to speak to handling of multi-field keys.
   * @todo provide a more flexible interface for handling Unicode, including the means to encode
   ...
   *
   * <p>Note: Trailing pad characters are normalized to a representation as a single pad character
   * (1 byte) followed by the #of actual or implied trailing pad characters represented as an
   * unsigned short integer (2 bytes). This technique serves to keep multi-field keys with embedded
   * variable length text fields aligned such that the field following a variable length text field
   * does not bleed into the lexiographic ordering of the variable length text field.
   *
   * <p>Note: While the ASCII encoding happens to use one byte for each character that is NOT true
   * of the Unicode encoding. The space requirements for the Unicode encoding depend on the text,
   * the Locale, the collator strength, and the collator decomposition mode.
   *
   * <p>Note: The <i>successor</i> option is designed to encapsulate some trickiness around forming
   * the successor of a variable length text field embedded in a multi-field key. In particular,
   * simply appending a <code>nul</code> byte will NOT work (it works fine when the text field is
   * the last field in the key or when it is the only component in the key). This approach breaks
   * encapsulation of the field boundaries such that the resulting "successor" is actually ordered
   * before the original key. This happens because you introduce a 0x0 byte right on the boundary of
   * the next field, effectively causing the next field to have a smaller value. Consider the
   * following example (in hex) where "|" represents the end of the "text" field:
   *
   * <pre>
   *     ab cd | 12
   * </pre>
   ...
   *
   * @param text The text.
   * @param unicode When true the text is interpreted as Unicode according to the {@link
   *     Options#COLLATOR} option. Otherwise it is interpreted as ASCII.
   * @param successor When true, the successor of the text will be encoded. Otherwise the text will
   *     be encoded.
...
//     * Encodes a character as a Unicode sort key by first converting it to a
   ...
   * </code>).
   *
   * <p>Note: This restriction only applies to multi-field keys where the text field appears in a
   * non-terminal position within the key - that is as encoded by . When a text field appears in
   ...
   * byte.
   *
   * <p>Note: This method is potentially much faster than the Unicode aware {@link #append(String)}.
   * However, this method is NOT unicode aware and non-ASCII characters will not be encoded
   * correctly. This method MUST NOT be mixed with keys whose corresponding component is encoded by
   * the unicode aware methods, e.g., {@link #append(String)}.
   *
   ...
   * @param len The #of bytes to append.
   ...
   *
   * <p>Note: this converts -0d and +0d to the same key.
   *
   ...
   *
   * <p>Note: this converts -0f and +0f to the same key.
   *
...
//     * Encodes a character as a 16-bit unsigned integer.
   ...
   * negative followed by the <code>byte[]</code> as returned by {@link BigInteger#toByteArray()}.
   *
   ...
   *
   * <p>1. Split the key into n components, namely: 00100101 10100100 01010100 2. Merge the
   * component bit by bit: 010 001 110 001 000 111 000 100 3. The result is this merged array
   *
   * @param baseSize
   * @param numDimensions
   */
