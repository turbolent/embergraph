...
public enum CollatorEnum {
  ...
  /*
   * The JDK bundles support for generating Unicode sort keys, but that support does NOT include
   * compressed sort keys and embeds <code>nul</code> bytes into its Unicode sort keys.
   */
  JDK,

  /*
   * ICU is the basis for the Unicode support in the JDK and also supports compressed sort keys,
   * which can be a big savings in an index.
   */
  ICU,
...
//     * A JNI plugin for ICU (native code for faster generation of sort keys).
  ...
  /*
   * A configuration option to force the interpretation of Unicode text as ASCII (only the low byte
