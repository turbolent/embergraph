...
public class DefaultKeyBuilderFactory implements IKeyBuilderFactory, Serializable {
  ...
  /** The selected collator strength (MAY be <code>null</code>, which means no override). */
  private final Object strength;

  /** The selected decomposition mode (MAY be <code>null</code>, which means no override). */
   ...
   * @param def The default (MAY be <code>null</code>).
   * @return The value -or- <i>def</i> if no value was found.
   */
  private static String getProperty(
      final Properties properties, final String key, final String def) {
   ...
   * Options#USER_COUNTRY}, and {@link Options#USER_VARIANT} MAY be overridden. The factory will
  ...
  public DefaultKeyBuilderFactory(final Properties properties) {
...

    {
      ...
      collator =
          CollatorEnum.valueOf(
              getProperty(properties, Options.COLLATOR, CollatorEnum.ICU.toString()));

      // true iff the collator was _explicitly_ specified.
      final boolean explicitCollatorChoice = getProperty(properties, Options.COLLATOR) != null;
      ...
      if (!explicitCollatorChoice) {
        ...
        if (log.isInfoEnabled()) {
          ...
          log.info("Defaulting: " + Options.COLLATOR + "=" + collator);
        }
      }
    }
...

    {
      ...
      if (log.isInfoEnabled()) log.info(Options.STRENGTH + "=" + tmpStrength);
...

    {
      ...
      if (getProperty(properties, Options.DECOMPOSITION) != null) {
        ...
        if (log.isInfoEnabled()) log.info(Options.DECOMPOSITION + "=" + mode);
      }
  ...
  public IKeyBuilder getKeyBuilder() {
    ...
    return KeyBuilder.newInstance(initialCapacity, collator, locale, strength, decompositionMode);
  }
  ...
  public IKeyBuilder getPrimaryKeyBuilder() {
    ...
    return KeyBuilder.newInstance(
        initialCapacity, collator, locale, StrengthEnum.Primary, decompositionMode);
  }
