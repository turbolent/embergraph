 ...
 * required.
 *
 * <p>Note: Avoid any dependencies within this class on the ICU libraries so that the code may run
 * without those libraries when they are not required.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @see SuccessorUtil, Compute the successor of a value before encoding it as a component of a key.
 * @see BytesUtil#successor(byte[]), Compute the successor of an encoded key.
 * @todo introduce a mark and restore feature for generating multiple keys that share some leading
 *     prefix. in general, this is as easy as resetting the len field to the mark. keys with
 *     multiple components could benefit from allowing multiple marks (the sparse row store is the
 *     main use case).
 */
public class KeyBuilder implements IKeyBuilder, LongPacker.IByteBuffer {
   ...
   * Unicode methods will all throw an {@link UnsupportedOperationException}.
   */
  private final UnicodeSortKeyGenerator sortKeyGenerator;

  /** Creates a key builder with an initial buffer capacity of <code>1024</code> bytes. */
  public KeyBuilder() {
   ...
   *     zero (0) the {@link #DEFAULT_INITIAL_CAPACITY} will be used.
   */
   ...
   *     null</code> Unicode collation support is disabled).
   * @param len The #of bytes of data in the provided buffer.
   * @param buf The buffer, with <i>len</i> pre-existing bytes of valid data. The buffer reference
   *     is used directly rather than making a copy of the data.
   */
  protected KeyBuilder(
      final UnicodeSortKeyGenerator sortKeyGenerator, final int len, final byte[] buf) {
  ...
  public final void position(final int pos) {
    ...
    if (len < 0 || len > buf.length) {
      ...
      throw new IndexOutOfBoundsException("pos=" + pos + ", but capacity=" + buf.length);
    }

    len = pos;
  }

  public final KeyBuilder append(final byte b) {

    return appendUnsigned(b);
  }

  public final KeyBuilder append(final byte[] a) {

    return append(a, 0, a.length);
  }

  public final KeyBuilder append(final byte[] a, final int off, final int len) {
   ...
   * this {@link IKeyBuilder} instance.
   */
  ...
  public final KeyBuilder append(final String s) {
   ...
   *
   * @param key The key.
   * @param off The offset of the start of the string.
   * @param len The #of bytes to decode (one byte per character).
   ...
   * representation. In practice this means you can choose 0x20 (a space) or 0x00 (a nul). This
   * limit arises in {@link #appendText(String, boolean, boolean)} which assumes that it can write a
   * pad character (or its successor) in one byte. 0xff will NOT work since its successor is not
   * defined within an bit string of length 8.
   *
   * @todo make this a configuration option? if so then verify that the choice (and its successor)
   *     fit in 8 bits.
   */
  ...
  public KeyBuilder appendText(String text, final boolean unicode, final boolean successor) {
  ...
  public final KeyBuilder append(final double d) {
  ...
  public static double decodeDouble(final byte[] key, final int off) {
  ...
  public final KeyBuilder append(final float f) {
  ...
  public static float decodeFloat(final byte[] key, final int off) {
  ...
  public final KeyBuilder append(final UUID uuid) {
   ...
   *
   * <p>Note: The order is NOT fully preserved. Any long which is encoded into less than 8 bytes has
   * its order preserved. However, a long which is encoded into 8 bytes will wind up ordered before
   * any longs which pack into fewer bytes.
   *
   * @param v The unsigned long value.
   * @return The #of bytes onto which the unsigned long value was packed.
   */
  public final KeyBuilder pack(final long v) {
  ...
  public void put(final byte[] b, final int off, final int len) {
    ...
    System.arraycopy(
        b /* src */, 0 /* srcPos */, buf /* dest */, this.len /* destPos */, len /* length */);
  ...
  public final KeyBuilder appendUnsigned(final byte v) {
  ...
  public final KeyBuilder appendSigned(final byte v) {
   ...
   */
  public static int byteLength(final BigInteger value) {
   ...
   * are encoded by the same representation and both will decode to <code>0</code>.
   *
   * <h2>Implementation details</h2>
   *
   * The encoding to a BigDecimal requires the expression of scale and length {@link
   * BigDecimal#scale()} indicates the precision of the number, where '3' is three decimal places
   * and '-3' rounded to '000' {@link BigDecimal#precision()} is the number of unscaled digits
   * therefore <code>precision - scale</code> is an expression of the exponent of the normalized
   * number. This means that the exponent could be zero or negative so the sign of the number cannot
   * be indicated by adding to the exponent. Instead an explicit sign byte,'0' or '1' is used. The
   * actual {@link BigDecimal} serialization uses the {@link String} conversions supported by {@link
   * BigDecimal}, less the '-' sign character if applicable. The length of this data is terminated
   * by a trailing byte. The value of that byte depends on the sign of the original {@link
   * BigDecimal} and is used to impose the correct sort order on negative {@link BigDecimal} values
   * which differ only in the digits in the decimal portion.
   *
   * <p>The variable encoding of BigNumbers requires this String representation and negative
   * representations are further encoded using {@link #flipDigits(String)} for the equivalent of 2s
   * compliment negative representation.
   *
   * <p>There are two cases where scale and trailing zeros interact. The case of "0.000" is
   * represented as precision of 1 and scale of 3, indicating the "0" is shifted down 3 decimal
   * places. While "5.000" is represented as precision of 4 and scale of 3. The special case of zero
   * is allowed because shifting zero to the right leaves a new zero on the left, so a zero value
   * must be checked for explicitly, while if we want to compare "5", "5.00" and "5.0000" as equal
   * we must remove and compensate for trailing zeros.
   *
   ...
   * @return The byte length of its unsigned byte[] representation.
   */
  public static int byteLength(final BigDecimal value) {
    ...
    if (value.signum() == 0) {
      ...
      byteLength = +1 /* sign */ + 4 /* exponent */ + dataLen /* data */ + 1 /* termination byte */;
    }
   ...
   */
  public static byte encodeByte(final int v) {
   ...
   */
  public static byte decodeByte(final int v) {
   ...
   * floating-point "double format" bit layout using {@link Double#doubleToLongBits(double)} and
   * then converting the resulting long into a two's complement number.
   *
   * <p>See <a href="http://www.cygnus-software.com/papers/comparingfloats/comparingfloats.htm">
   * Comparing floating point numbers </a> by Bruce Dawson.
   *
   * @param d The double precision floating point value.
   * @return The corresponding long integer value that maintains the same total ordering.
   */
   ...
   * format" bit layout using {@link Float#floatToIntBits(float)} and then converting the resulting
   * int into a two's complement number.
   *
   * <p>See <a href="http://www.cygnus-software.com/papers/comparingfloats/comparingfloats.htm">
   * Comparing floating point numbers </a> by Bruce Dawson.
   *
   * @param f The floating point value.
   * @return The corresponding integer value that maintains the same total ordering.
   */
   ...
   * @param off The offset at which to decode the key.
   * @return The signed long value.
   */
  public static long decodeLong(final byte[] buf, int off) {
   ...
   */
  public static UUID decodeUUID(final byte[] buf, int off) {
   ...
   */
  public static int decodeInt(final byte[] buf, int off) {
   ...
   */
  public static short decodeShort(final byte[] buf, int off) {
   ...
   */
  public static BigInteger decodeBigInteger(final int offset, final byte[] key) {
   ...
   */
  public static byte[] decodeBigInteger2(final int offset, final byte[] key) {
    ...
    System.arraycopy(
        key /* src */, offset + 2 /* srcpos */, b /* dst */, 0 /* destPos */, runLength);
   ...
   */
  public static BigDecimal decodeBigDecimal(final int offset, final byte[] key) {
    int curs = offset;
   ...
   */
  private static String flipDigits(final String str) {
   ...
   * ITupleSerializer}, this means that Unicode keys for an index will be interpreted in the same
   * manner on any machine where {@link ITuple}s for that index are (de-)materialized. </strong>
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  public interface Options {
     ...
     * disable Unicode support entirely, treating the characters as if they were ASCII. If your data
     * is not actually Unicode then this offers a substantial performance benefit.
     *
     * @see CollatorEnum
     */
     ...
     * <dl>
     *   <dt>0
     *   <dd>{@link Collator#PRIMARY}
     *   <dt>1
     *   <dd>{@link Collator#SECONDARY}
     *   <dt>2
     *   <dd>{@link Collator#TERTIARY}
     * </dl>
     *
   ...
   *
   * @param properties The properties to be used (optional). When <code>null</code> the {@link
   *     System#getProperties() System properties} are used.
   * @see Options
   * @throws UnsupportedOperationException
   *     <p>The ICU library was required but was not located. Make sure that the ICU JAR is on the
   *     classpath. See {@link Options#COLLATOR}.
   *     <p>Note: If you are trying to use ICU4JNI then that has to be locatable as a native
   *     library. How you do this is different for Windows and Un*x.
   */
   ...
   * Options#USER_COUNTRY}, and {@link Options#USER_VARIANT} MAY be overriden. The factory will
   * support Unicode unless {@link CollatorEnum#ASCII} is explicitly specified for the {@link
   * Options#COLLATOR} property.
   *
   * @param properties The properties to be used (optional). When <code>null</code> the {@link
   *     System#getProperties() System properties} are used.
   * @see Options
   ...
   *     #DEFAULT_INITIAL_CAPACITY} will be used.
   * @param collatorChoice Identifies the collator that will be used to generate sort keys from
   *     Unicode values.
   * @param locale When <code>null</code> the {@link Locale#getDefault() default locale} will be
   *     used.
   * @param strength Either an {@link Integer} or a {@link StrengthEnum} specifying the strength to
   *     be set on the collator object (optional). When <code>null</code> the default strength of
   *     the collator will not be overridden.
   * @param mode The decomposition mode to be set on the collator object (optional). When <code>null
   *     </code> the default decomposition mode of the collator will not be overridden.
   * @return The new instance.
   * @throws UnsupportedOperationException
   *     <p>The ICU library was required but was not located. Make sure that the ICU JAR is on the
   *     classpath.
   *     <p>Note: If you are trying to use ICUJNI then that has to be locatable as a native library.
   *     How you do this is different for Windows and Un*x.
   */
  public static IKeyBuilder newInstance(
      int capacity,
