 ...
 * @todo There is no successor method for UUID. This should be treated as a special case of a fixed
 *     length field. Handle as two longs. add 1 to the low order bits. on overflow add one to the
 *     high order bits. on overflow of the high order bits there is no successor.
 * @see BytesUtil#successor(byte[]), which computes the successor of a variable length unsigned
 *     byte[].
 */
public class SuccessorUtil {
   ...
   * tests that directly distinguish positive and negative zero using >, <, or ==.
   */
   ...
   * tests that directly distinguish positive and negative zero using >, <, or ==.
   */
  public static final float FNEG_ZERO = Float.intBitsToFloat(0x80000000);

  /** Smallest non-zero positive value. */
  public static final float FPOS_MIN = Float.intBitsToFloat(0x00000001);

  /** Smallest non-zero negative value. */
  public static final float FNEG_MIN = Float.intBitsToFloat(0x80000001);

  /** Positive one (1f). */
  public static final float FPOS_ONE = 1f;

  /** Negative one (-1f). */
  public static final float FNEG_ONE = -1f;

  /** Largest positive float. */
  public static final float FPOS_MAX = Float.MAX_VALUE;

  /** Largest negative float. */
  public static final float FNEG_MAX = Float.intBitsToFloat(0xFF7FFFFF);
   ...
   * tests that directly distinguish positive and negative zero using >, <, or ==.
   */
  public static final double DNEG_ZERO = Double.longBitsToDouble(0x8000000000000000L);

  /** Smallest non-zero positive value. */
  public static final double DPOS_MIN = Double.longBitsToDouble(0x0000000000000001L);

  /** Smallest non-zero negative value. */
  public static final double DNEG_MIN = Double.longBitsToDouble(0x8000000000000001L);

  /** Positive one (1d). */
  public static final double DPOS_ONE = 1d;

  /** Negative one (-1d). */
  public static final double DNEG_ONE = -1d;

  /** Largest positive double. */
  public static final double DPOS_MAX = Double.MAX_VALUE;

  /** Largest negative double. */
  public static final double DNEG_MAX = Double.longBitsToDouble(0xFFEFFFFFFFFFFFFFL);
   ...
   * @exception NoSuccessorException if there is no successor for that value.
   */
   ...
   * floating point value using a bit manipulation trick. See <a
   * href="http://www.cygnus-software.com/papers/comparingfloats/comparingfloats.htm">Comparing
   * floating point numbers </a> by Bruce Dawson. The Java {@link Float} and {@link Double} classes
   * provide the static methods required to convert a float or double into its IEEE 754 floating
   * point bit layout, which can be treated as an int (for floats) or a long (for doubles). By
   * testing for the sign, you can just add (or subtract) one (1) to get the bit pattern of the
   * successor (see the above referenced article). Special exceptions must be made for NaNs,
   * negative infinity and positive infinity.
   *
   * @param f The float value.
   * @return The next value in the value space for <code>float</code>.
   * @exception NoSuccessorException if there is no next value in the value space.
   */
  public static float successor(final float f) throws NoSuccessorException {
   ...
   * @throws NoSuccessorException If all bytes overflow.
   * @throws NoSuccessorException If the byte[] has zero length.
   */
  public static byte[] successor(final byte[] b) {
   ...
   *
   * @param b The byte[].
   * @param off The offset of the start of the bitstring in the byte[].
   * @param len The #of bytes in the bit string within the byte[].
   * @return b, which has been modified as a side-effect.
   * @throws NoSuccessorException If all bytes overflow.
   * @throws NoSuccessorException If the byte[] has zero length.
   * @todo unit tests when offset is non-zero.
   */
  public static byte[] successor(final byte[] b, final int off, final int len) {
    ...
    for (int i = off + len - 1; i >= off; i--) {
      ...
      //
      //            /*
      //             * Negative integers will be sign extended so we onlyStrip off the sign bit when
      // checking overflow so that the sign
