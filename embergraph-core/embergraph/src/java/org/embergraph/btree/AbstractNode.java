...
public abstract class AbstractNode<
   ...
   * <dl>
   *   <dt>info
   *   <dd>A high level trace of insert, split, joint, and remove operations. You MUST test on
   *       {@link Logger#isInfoEnabled()} before generating log messages at this level to avoid
   *       string concatenation operations would otherwise kill performance.
   *   <dt>
   *   <dd>A low level trace including a lot of dumps of leaf and node state. * You MUST test on
   *       {@link Logger#isDebugEnabled()} before generating log messages at this level to avoid
   *       string concatenation operations would otherwise kill performance.
   * </dl>
   *
   * @see BTree#log
   ...
   * custom method to read the node with the btree reference on hand so that we can set this field.
   */
   ...
   * {@link BTree}) or a dirty child (held by the {@link Node}). The parent reference is set when a
   * node is attached as the child of another node.
   *
   * <p>Note: When a node is cloned by {@link #copyOnWrite()} the parent references for its
   * <em>clean</em> children are set to the new copy of the node. This is referred to in several
   * places as "stealing" the children since they are no longer linked back to their old parents via
   ...
   * Reference} object for any given {@link Node}.
   */
   ...
   * node is written on the store. This mechanism is critical because it prevents a node entering
   * the queue from forcing IO for the same node in the edge case where the node is also on the tail
   * on the queue. Since the counter is incremented before it is added to the queue, it is
   * guaranteed to be non-zero when the node forces its own eviction from the tail of the queue.
   * Preventing this edge case is important since the node can otherwise become immutable at the
   * very moment that it is touched to indicate that we are going to update its state, e.g., during
   * an insert, split, or remove operation. This mechanism also helps to defer IOs since IO can not
   * occur until the last reference to the node is evicted from the queue.
   *
   * <p>Note that only mutable {@link BTree}s may have dirty nodes and the {@link BTree} is NOT
   * thread-safe for writers so we do not need to use synchronization or an AtomicInteger for the
   * {@link #referenceCount} field.
   */
   ...
   *
   * @return The parent or null if (a) this is the root node or (b) the {@link WeakReference} to the
   ...
   * fields rather than permitting that logic to be replicated throughout the code with the
   * corresponding difficulty in ensuring that the logic is correct throughout.
   *
   * @param btree The btree to which the node belongs.
   * @param branchingFactor The branching factor for the node. By passing the branching factor
   *     rather than using the branching factor declared on the btree we are able to support
   *     different branching factors at different levels of the tree.
   ...
   * same state but a distinct (and not yet assigned) address on the backing store. If the source
   * node has immutable data for some aspect of its state, then a mutable copy of that data is made.
   *
   * <p>Note: The <strong>caller</strong> MUST {@link #delete()} the source node after invoking this
   * copy constructor. If the backing store supports the operation, the source node will be
   * reclaimed as free space at the next commit.
   *
   * <p>The source node must be deleted since it is no longer accessible and various aspects of its
   * state have been stolen by the copy constructor. If the btree is committed then both the delete
   * of the source node and the new tree structure will be made restart-safe atomically and all is
   * well. If the operation is aborted then both changes will be undone and all is well. In no case
   * can we access the source node after this operation unless all changes have been aborted, in
   * which case it will simply be re-read from the backing store.
   *
   ...
   * then the new root will be set on the {@link BTree}. This method must MUST be invoked any time
   * an mutative operation is requested for the leaf.
   *
   ...
   * this method, MUST delegate the mutation operation to the copy iff a copy was made, and MUST
   * result in an awareness in the caller that the copy exists and needs to be used in place of the
   * immutable version of the node.
   *
   ...
   * copied, then the new root will be set on the {@link BTree}. This method must MUST be invoked
  ...
  protected AbstractNode<T> copyOnWrite(final long triggeredByChildId) {
    ...
    if (btree.root == this) {
      ...
      if (DEBUG) log.debug("Copy-on-write : replaced root node on btree.");
   ...
   *
   * @param dirtyNodesOnly When true, only dirty nodes and leaves will be visited
   * @return Iterator visiting {@link AbstractNode}s.
   */
  public final Iterator<AbstractNode> postOrderNodeIterator(final boolean dirtyNodesOnly) {
   ...
   *
   * @param dirtyNodesOnly When true, only dirty nodes and leaves will be visited
   * @param nodesOnly When <code>true</code>, the leaves will not be visited.
   * @return Iterator visiting {@link AbstractNode}s.
   */
  public abstract Iterator<AbstractNode> postOrderNodeIterator(
   ...
   *     no lower bound.
   * @param toKey The first key that will NOT be visited (exclusive). When <code>null</code> there
   *     is no upper bound.
   * @param flags indicating whether the keys and/or values will be materialized.
   */
  public ITupleIterator rangeIterator(final byte[] fromKey, final byte[] toKey, final int flags) {

    return new PostOrderEntryIterator(
  ...
  private static class PostOrderEntryIterator implements ITupleIterator {
    ...
    public PostOrderEntryIterator(
        final AbstractBTree btree,
        final Iterator postOrderNodeIterator,
        final byte[] fromKey,
        final byte[] toKey,
        int flags) {
   ...
   *
   * <ul>
   *   <li>A node with nkeys + 1 children.
   *   <li>A node must have between [m/2:m] children (alternatively, between [m/2-1:m-1] keys since
   *       nkeys + 1 == nchildren for a node).
   *   <li>A leaf has no children and has between [m/2:m] key-value pairs (the same as the #of
   *       children on a node).
   *   <li>The root leaf may be deficient (may have less than m/2 key-value pairs).
   * </ul>
   *
   * where <code>m</code> is the branching factor and a node is understood to be a non-leaf node in
   * the tree.
   *
   * <p>In addition, all leaves are at the same level (not tested by this assertion).
   */
   ...
   *
   * <p>Note: Whenever possible the key reference is copied rather than copying the data. This
   * optimization is valid since we never modify the contents of a key.
   *
   * @param dstpos The index position to which the key will be copied on this node.
   * @param srckeys The source keys.
   * @param srcpos The index position from which the key will be copied.
   */
  protected final void copyKey(final int dstpos, final IRaba srckeys, final int srcpos) {
   ...
   * cause the separator key in the parent to be adjusted.
   *
   * <p>Join is invoked when a leaf has become deficient (too few keys/values). This method is never
   * invoked for the root leaf therefore the parent of this leaf must be defined. Further, since the
   * minimum #of children is two (2) for the smallest branching factor three (3), there is always a
   * sibling to consider.
   *
   * <p>Join first considers the immediate siblings. if either is materialized and has more than the
   * minimum #of values, then it redistributes one key and value (child) from the sibling into this
   * leaf (node). If either sibling is materialized and has only the minimum #of values, then it
   * merges this leaf (node) with that sibling.
   *
   * <p>If no materialized immediate sibling meets these criteria, then first materialize and test
   * the right sibling. if the right sibling does not meet these criteria, then materialize and test
   * the left sibling.
   *
   * <p>Note that (a) we prefer to merge a materialized sibling with this leaf to materializing a
   * sibling; and (b) merging siblings is the only way that a separator key is removed from a
   * parent. If the parent becomes deficient through merging then join is invoked on the parent as
   * well. Note that join is never invoked on the root node (or leaf) since it by definition has no
   * siblings.
   *
   * <p>Note that we must invoked copy-on-write before modifying a sibling. However, the parent of
   * the leaf MUST already be mutable (aka dirty) since that is a precondition for removing a key
   * from the leaf. This means that copy-on-write will not force the parent to be cloned.
   */
  protected void join() {
   ...
   * @param isRightSibling True iff the sibling is the rightSibling of this node.
   * @todo redistribution should proceed until the node and the sibling have an equal #of keys (or
   *     perhaps more exactly until the node would have more keys than the sibling if another key
   *     was redistributed into the node from the sibling). this takes advantage of the fact that
   *     the node and the sibling are known to be in memory to bring them to the point where they
   *     are equally full. along the same lines when both siblings are resident we could actually
   *     redistribute keys from both siblings into the node until the keys were equally distributed
   *     among the node and its siblings.
   * @todo a b*-tree variant simply uses redistribution of keys among siblings during insert to
   *     defer a split until the node and its siblings are all full.
   */
   ...
   * @param delete <code>true</code> iff the entry is to marked as deleted (delete markers must be
   *     supported for if this is true).
   ...
   * @param timestamp The timestamp associated with the version (the value is ignored unless version
   *     metadata is being maintained).
   * @param tuple A tuple that may be used to obtain the data and metadata for the pre-existing
   *     index entry overwritten by the insert operation (optional).
   * @return The <i>tuple</i> iff there was a pre-existing entry under that key and <code>null
   *     </code> otherwise.
   */
   ...
   * @param tuple A tuple that may be used to obtain the data and metadata for the pre-existing
   *     index entry that was either removed by the remove operation (optional).
   * @return The <i>tuple</i> iff there was a pre-existing entry under that key and <code>null
   *     </code> otherwise.
   */
   ...
   *
   * @param searchKey The search key.
   * @return the index of the search key, if found; otherwise, <code>(-(insertion point) - 1)</code>
   *     . The insertion point is defined as the point at which the key would be found it it were
   *     inserted into the btree without intervening mutations. Note that this guarantees that the
   ...
   * key for that entry.
   *
   * @param index The index position of the entry (origin zero and relative to this node or leaf).
   * @return The key at that index position.
   * @exception IndexOutOfBoundsException if index is less than zero.
   * @exception IndexOutOfBoundsException if index is greater than the #of entries.
   */
   ...
   * @param level The logging level.
   * @param out Where to write the dump.
   * @param height The height of this node in the tree or -1 iff you need to invoke this method on a
   *     node or leaf whose height in the tree is not known.
   * @param recursive When true, the node will be dumped recursively using a pre-order traversal.
   * @return True unless an inconsistency was detected.
   */
  public abstract boolean dump(Level level, PrintStream out, int height, boolean recursive);
}
