 ...
 *     Stream. This will be another step towards GIST support. There are protected methods which are
 *     used on those classes which should be lifted into the abstract base class. Also, try to
 *     reconcile this interface with {@link ILocalBTreeView} implementations that do not implement
 *     {@link ICheckpointProtocol} ({@link FusedView}, {@link ReadCommittedView}).
...
public interface ICheckpointProtocol
   ...
   * Name2Addr} based on the actual commitTime of the commit during which an {@link Entry} for that
   * index was last committed. It is set for both historical index reads and unisolated index reads
   * using {@link Entry#commitTime}. The lastCommitTime for an unisolated index will advance as
   * commits are performed with that index.
   *
   * @param lastCommitTime The timestamp of the last committed state of this index.
   * @throws IllegalArgumentException if lastCommitTime is ZERO (0).
   ...
   *
   * <p>Note: A checkpoint by itself is NOT an atomic commit. The commit protocol is at the store
   * level and uses {@link Checkpoint}s to ensure that the state of the persistence capable data
   * structure is current on the backing store.
   *
   * @return The address at which the {@link Checkpoint} record for the persistence capable was
   ...
   * structure is current on the backing store.
   *
   * @return The {@link Checkpoint} record for the persistent data structure which was written onto
   ...
   * {@link IndexMetadata} for remote clients in scale-out.
   *
...
//     * Remove all entries in the index.
   ...
   * store). Such an index reference remains valid after a {@link #close()}. A closed index is
   * transparently {@link #reopen() reopened} by any access to the index data (scanning the index,
   * probing the index, etc).
   *
   * <p>Note: A {@link #close()} on a dirty index MUST discard writes rather than flushing them to
   * the store and MUST NOT update its {@link Checkpoint} record - ({@link #close()} is used to
   * discard indices with partial writes when an {@link AbstractTask} fails). If you are seeking to
   ...
   *     leaves) using a low-level approach.
   * @param visitLeaves When <code>true</code> and <code>recursive:=true</code> then the leaves of
   *     the index are also visited.
   * @return Some interesting statistics about the index (and optionally the pages in that index)
   *     which the caller can print out.
