...
public abstract class AbstractIndexProcedure<T> implements IIndexProcedure<T> {
   ...
   * the clients. Note that the failure to specify a specific serialVersionUID means that the actual
   * value was being computed at runtime and that round-trip serialization depended on the JVMs
   * computing the same value for this field, so there is not any well known historical value that
