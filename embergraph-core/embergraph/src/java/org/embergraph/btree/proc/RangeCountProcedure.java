...
public class RangeCountProcedure extends AbstractKeyRangeIndexProcedure<Long>
   ...
   * @param deleted iff deleted tuples must be included in the count.
   ...
   * @throws IllegalArgumentException if <i>exact</i> is <code>false</code> and <i>deleted</i> is
   *     <code>true</code> (there is no approximate reporting for range counts of deleted and
   *     undeleted tuples).
   */
  public RangeCountProcedure(boolean exact, boolean deleted, byte[] fromKey, byte[] toKey) {
    ...
    if (!exact && deleted) throw new IllegalArgumentException();
  ...
  /** Return <code>true</code> iff deleted tuples must be included in the result. */
   ...
   * bound estimate of the #of key-value pairs in the key range of the index. The estimate is an
   * upper bound because duplicate or deleted entries in that have not been eradicated through a
   * suitable compacting merge will be reported. An exact count may be obtained using a range
   * iterator by NOT requesting either the keys or the values.
   *
   * @return The upper bound estimate of the #of key-value pairs in the key range of the named
   *     index.
   */
  @Override
