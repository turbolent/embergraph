...
public abstract class AbstractKeyArrayIndexProcedureConstructor<T extends IKeyArrayIndexProcedure> {
   ...
   *     be sent to a remote {@link IDataService}.
   ...
   * @param keys The keys.
   * @param vals The values (may be optional depending on the semantics of the operation).
   * @return An instance of the procedure.
   * @todo we will need a different method signature to support hash-partitioned (vs range
   *     partitioned) indices.
   */
   ...
   * @param vals
   * @return
   * @todo Why does this variant exist? Is it just to make life easier when the {@link
