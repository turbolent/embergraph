...
public abstract class AbstractChunkedTupleIterator<E> implements ITupleIterator<E> {
  ...
  protected static final transient Logger log =
      Logger.getLogger(AbstractChunkedTupleIterator.class);
   ...
   * or not it exhausted the entries that could be matched for that index partition. If not, then we
   * will issue "continuation" query against the same index position. If we are scanning forward,
   * then the continuation query will start (toKey) from the successor of the last key scanned (if
   * we are scanning backwards, then toKey will be the leftSeparator of the index partition and
   * fromKey will be the last key scanned).
   *
   * <p>Note: A result set will be empty if there are no entries (after filtering) that lie within
   * the key range in a given index partition. It is possible for any of the result sets to be
   * empty. Consider a case of static partitioning of an index into N partitions. When the index is
   * empty, a range query of the entire index will still query each of the N partitions. However,
   * since the index is empty none of the partitions will have any matching entries and all result
   * sets will be empty.
   *
   * @todo it would be useful if the {@link ResultSet} reported the maximum length for the keys and
   *     for the values. This could be used to right size the buffers which otherwise we have to let
   *     grow until they are of sufficient capacity.
   * @see #rangeQuery()
   ...
   * When <code>false</code> {@link #continuationQuery()}s will use whatever value is returned by
   * {@link #getTimestamp()}. Read-consistent semantics for a partitioned index are achieved using
   * the timestamp returned by {@link IIndexStore#getLastCommitTime()} rather than {@link
   * ITx#READ_COMMITTED}.
   */
   ...
   * ResultSet}. Otherwise it is the value returned by {@link #getTimestamp()}.
   *
   * @throws IllegalStateException if {@link #getReadConsistent()} is <code>true</code> and the
   *     initial {@link ResultSet} has not been read since the commitTime for that {@link ResultSet}
  ...
  public AbstractChunkedTupleIterator(
      final byte[] fromKey,
      final byte[] toKey,
      int capacity,
      final int flags,
  ...
  protected abstract ResultSet getResultSet(
      long timestamp, byte[] fromKey, byte[] toKey, int capacity, int flags, IFilter filter);
  ...
  protected void continuationQuery() {
     ...
     */
    lastVisitedKeyInPriorResultSet = tuple.getKeysRequested() ? tuple.getKey() : null;
    ...
    if ((flags & IRangeQuery.REVERSE) == 0) {
      ...
      final byte[] _fromKey =
          fixedLengthSuccessor
              ? SuccessorUtil.successor(rset.getLastKey().clone())
              : BytesUtil.successor(rset.getLastKey());
      ...
      // continuation query.
      rset = getResultSet(getReadTime(), fromKey, _toKey, capacity, flags, filter);
    }
   ...
   * been invoked. It is cleared by {@link #next()} so that it does not hang around.
   */
   ...
   * exhausted}, then we issue a {@link #continuationQuery()} against the same index partition. If
   * the {@link ResultSet} is {@link ResultSet#isExhausted() exhausted}, then we test to see whether
   * or not we have visited all index partitions. If so, then the iterator is exhausted. Otherwise
   * we issue a range query against the {@link #nextPartition()}.
   *
  ...
  public class ResultSetTuple implements ITuple<E> {
    ...
    public byte[] getValue() {
      ...
      if (!getValuesRequested()) throw new UnsupportedOperationException();
   ...
   *
   * @param sourceIndex The value from {@link ITuple#getSourceIndex()}.
   * @param addr The value supplied to {@link ITuple#readBlock(long)}.
   */
