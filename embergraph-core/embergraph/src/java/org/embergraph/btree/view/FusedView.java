 ...
 * key for a source from reading through to an older source. If a deletion marker is encountered the
 * index entry will be understood as "not found" in the fused view rather than reading through to an
 * older source where it might still have a binding.
 *
 * @todo consider implementing {@link IAutoboxBTree} here and collapsing {@link ILocalBTreeView} and
 *     {@link IAutoboxBTree}.
 * @todo Can I implement {@link ILinearList} here? That would make it possible to use keyAt() and
 *     indexOf() and might pave the way for a {@link MetadataService} that supports overflow since
 *     the index segments could be transparent at that point.
...
public class FusedView implements IIndex, ILocalBTreeView { // , IValueAge {
  ...
  private static class HardRefSources implements ISources {
     ...
     * reference and hard references to the {@link IndexSegmentStore} using {@link
     * IndexSegmentStore#loadIndexSegment()}. We could actually use hard references for the index
     * segments inside of a {@link WeakReference} to an array of those references.
     */
   ...
   * WeakReference} to (re-)open the {@link IndexSegment} on demand.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
   ...
   * @exception IllegalArgumentException if a source is used more than once.
   * @exception IllegalArgumentException unless all sources have the same indexUUID
   * @exception IllegalArgumentException unless all sources support delete markers.
   */
  ...
  static void checkSources(final AbstractBTree[] srcs) {
    ...
    if (srcs == null) throw new IllegalArgumentException("sources is null");
    ...
    for (int i = 0; i < srcs.length; i++) {
      ...
      if (srcs[i] == null) throw new IllegalArgumentException("Source null @ index=" + i);
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (srcs[i] == srcs[j]) throw new IllegalArgumentException("Source used more than once");

        if (!srcs[i]
            .getIndexMetadata()
            .getIndexUUID()
            .equals(srcs[j].getIndexMetadata().getIndexUUID())) {
   ...
   * we need to obtain the Tuple for the key from the view. If the tuple is null or deleted, then we
   * can do an unconditional insert. Otherwise there is an entry under the key and we return the
   * value of the entry from the Tuple. Note that the value COULD be a null.
   */
  @Override
   ...
   */
  @Override
  public final byte[] lookup(final byte[] key) {
   ...
   * @param tuple A tuple to be populated with data and metadata about the index entry (required).
   * @return <i>tuple</i> iff an index entry was found under that <i>key</i>.
   */
  public final Tuple lookup(final byte[] key, final Tuple tuple) {
   ...
   *
   * @param startIndex The index of the first source to be read. This permits the lookup operation
   *     to start at an index into the {@link #srcs} other than zero. This is used by {@link
   *     IsolatedFusedView} to read from just the groundState (everything except the writeSet, which
   *     is the source at index zero(0)).
   * @param key The search key.
   * @param tuple A tuple to be populated with data and metadata about the index entry (required).
   * @return <i>tuple</i> iff an index entry was found under that <i>key</i>.
   */
  protected final Tuple lookup(final int startIndex, final byte[] key, final Tuple tuple) {
   ...
   * {@link AbstractBTree} with an index entry under the key is non-deleted.
   */
  @Override
   ...
   * entries that could lie within that key range. However, the actual number could be less if there
   * are entries for the same key in more than one source index.
   */
  @Override
  ...
  public final long rangeCount(byte[] fromKey, byte[] toKey) {
    ...
    if (fromKey == null || toKey == null) {
      ...
      final LocalPartitionMetadata pmd = getIndexMetadata().getPartitionMetadata();
  ...
  public final long rangeCountExact(byte[] fromKey, byte[] toKey) {
    ...
    while (itr.hasNext()) {
      ...
      if (n == Long.MAX_VALUE) throw new RuntimeException(ERR_RANGE_COUNT_EXCEEDS_MAX_LONG);
  ...
  public long rangeCountExactWithDeleted(byte[] fromKey, byte[] toKey) {
    ...
    final Iterator itr =
        rangeIterator(
            fromKey, toKey, 0 /* capacity */, IRangeQuery.DELETED /* flags */, null /* filter */);
   ...
   * index, the entry is chosen based on the order in which the indices were declared to the
   * constructor.
   */
  @Override
  public final ITupleIterator rangeIterator(final byte[] fromKey, final byte[] toKey) {
   ...
   * of the source {@link AbstractBTree}s. The <i>filter</i> is NOT passed through to these source
   * iterators. Instead, an {@link FusedTupleIterator} is obtained and the filter is applied to that
   * iterator. This means that filters always see a fused representation of the source iterators.
   *
   * <p>Note: This implementation supports {@link IRangeQuery#REVERSE}. This may be used to locate
   * the {@link ITuple} before a specified key, which is a requirement for several aspects of the
   * overall architecture including atomic append of file blocks, locating an index partition in the
   * metadata index, and finding the last member of a set or map.
   *
   * <p>Note: When the {@link IRangeQuery#CURSOR} flag is specified, it is passed through and an
   * {@link ITupleCursor} is obtained for each source {@link AbstractBTree}. A {@link
   * FusedTupleCursor} is then obtained which implements the {@link ITupleCursor} extensions.
   */
  @Override
  @SuppressWarnings("unchecked")
  public ITupleIterator rangeIterator(
      byte[] fromKey, byte[] toKey, final int capacity, final int flags, final IFilter filter) {
    ...
    final int sourceFlags =
        ((flags | KEYS | DELETED) | (reverseScan || removeAll ? CURSOR : 0))
            & (~REMOVEALL) // turn off
            & (~REVERSE) // turn off
        ;
    ...
    if (cursorMode || removeAll || reverseScan) {
      ...
      for (AbstractBTree t : sources) {
        ...
        itrs[i++] =
            (ITupleCursor)
                t.rangeIterator(fromKey, toKey, capacity, sourceFlags, null /* filter */);
      }
      ...
      for (AbstractBTree t : sources) {
        ...
        itrs[i++] = t.rangeIterator(fromKey, toKey, capacity, sourceFlags, null /* filter */);
      }
  ...
  @Override
  public final <T> T submit(final byte[] key, final ISimpleIndexProcedure<T> proc) {
  ...
  public final void submit(
    ...
    if (fromKey == null) {
       ...
       */
      fromKey = getIndexMetadata().getPartitionMetadata().getLeftSeparatorKey();
    }
    ...
    if (toKey == null) {
       ...
       * tuples lying outside of the index partition boundaries!
       */
      toKey = getIndexMetadata().getPartitionMetadata().getRightSeparatorKey();
    }
  ...
  protected class FusedBloomFilter implements IBloomFilter {
     ...
     * has been disabled) for a given component index then the code treats the filter as always
     * reporting <code>true</code> (that is, forcing us to check the index). So a source component
     * index without a bloom filter or an index with a disabled bloom filter acts as its filter has
     * a high false positive rate, however the test is a NOP so it is cheap.
     */
     ...
     * the bloom filter and needs a means to report false positives. It should be the only one that
     * calls this method on this implementation class.
     */
