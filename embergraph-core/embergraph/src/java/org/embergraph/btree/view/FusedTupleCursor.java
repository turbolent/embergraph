...
public class FusedTupleCursor<E> extends FusedTupleIterator<ITupleCursor<E>, E>
  ...
  /*
   * The source view. This is required by {@link #getIndex()} and also to implement {@link
   * #remove()}.
   */
  private final IIndex ndx;

  /*
   * <code>true</code> iff the current direction of iterator progress is "forward", meaning that
   * {@link #hasNext()} was called and that the {@link #next()} {@link ITuple}s are currently
   * buffered by {@link #sourceTuple}.
   *
   * <p>This is field always <code>true</code> for this class, but the direction can be changed in
   * the {@link FusedTupleCursor} subclass by calling {@link ITupleCursor#hasPrior()}. If the
   * direction is changed then {@link #sourceTuple}s MUST be cleared before testing whether or not
   * the next/prior tuple exists. Likewise, {@link ITupleCursor#seek(byte[])} must clear {@link
   * #sourceTuple}s but can leave this flag alone.
   */
   ...
   * @param flags
   * @param deleted
   * @param srcs The source iterators.
   * @param ndx The {@link FusedView} from which the source iterators were drawn (required). Note
   *     that the semantics of {@link #remove()} on a fused view require that the tuple is
   *     overwritten by a delete marker in the 1st index of the view.
   */
  public FusedTupleCursor(
      final int flags, final boolean deleted, final ITupleCursor<E>[] srcs, final IIndex ndx) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (lastKeyVisited != null) {
          ...
          while (tuple != null) {
            ...
            final int ret = BytesUtil.compareBytes(tuple.getKey(), lastKeyVisited);
  ...
  /*
   * Delegates the operation to the source view (correct deletion requires that a delete marker for
   * the tuple is written onto first source index rather than deleting the tuple from the source
   * from which it was materialized).
   *
   * <p>Note: You must specify {@link IRangeQuery#CURSOR} in order for {@link #remove()} to be
   * supported.
   */
