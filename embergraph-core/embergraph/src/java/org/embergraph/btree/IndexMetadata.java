 ...
 *
 * <p>An instance of this class is required in order to create a {@link BTree} or {@link
 * IndexSegment}. Further, when registering a scale-out index you will first create an instance of
 * this class that will serve as the metadata <i>template</i> for all index resources which are part
 * of that scale-out index.
 *
 * <h2>Delete markers, version timestamps, and isolatable indices</h2>
 *
 * <p>By default a {@link BTree} does not maintain delete markers and a request to delete an entry
 * under a key will cause that entry to be removed from the live version of the index. However, such
 * indices do not support "overflow" (they can not be evicted onto read-only {@link IndexSegment}s)
 * and as such they do not support scale-out).
 *
 * <p>The {@link SparseRowStore} handles a "delete" of a property value by writing a <code>null
 * </code> value under a key and does NOT require the use of index entry delete markers, even in
 * scale-out deployments. A compacting merge of a {@link SparseRowStore} applies a history policy
 * based on a consideration of the timestamped property values, including values bound to a <code>
 * null</code>.
 *
 * <p>Delete markers combined with an ordered set of index resources is sufficient to support all
 * features of range-partitioned indices, including compacting merge. Given three index resources
 * {A,B,C} for a single index partition, the order over the resources gives us the guarantee that
 * any index entry in A will be more recent than any index enty in B or C. So when reading a fused
 * view we always stop once we have an index entry for a key, even if that entry has the deleted
 * flag set.
 *
 * <p>Delete markers occupy very little space in the leaf data structure (one bit each), however
 * when they are used a deleted index entry is NOT removed from the index. Instead, the key remains
 * in the leaf paired to a delete bit and a <code>null</code> value (or simply elided). These
 * "deleted" entries can only be removed from the index by a compacting merge. When transactional
 * isolation is used, the criteria for removing deleted entries are stronger - they must no longer
 * be visible to any active or allowable transaction as determined by the transaction manager, see
 * below for more on this.
 *
 * <p>Transaction isolation requires delete markers <i>plus</i> version timestamps. The version
 * timestamps in the unisolated index (the live index) give the timestamp of the commit during which
 * the index entry was last updated. The timestamp in the write set of transaction is copy from the
 * index view corresponding to the ground state of the transaction the first time that index entry
 * is overwritten within that transaction (there is a special case when the index entry was not
 * pre-existing - we assign the start time of the transaction in that case so when we validate we
 * are only concerned that the entry is either not found (never written) or that the entry exists
 * with the same timestamp - other conditions are write-write conflicts). On commit we write the
 * commit time on the updated index entries in the unisolated index.
 *
 * <h2>History policies and timestamps</h2>
 *
 * <p>There are in fact two kinds of timestamps in use - isolatable indices place a timestamp on the
 * index entry itself while the {@link SparseRowStore} places a timestamp in the <i>key</i>. One
 * consequence of this is that it is very efficient to read historical data from the {@link
 * SparseRowStore} since the data are right there in key order. On the other hand, reading
 * historical data from an isolatable index requires reading from each historical commit state of
 * that index which is not interest (this is NOT efficient). This is why the {@link SparseRowStore}
 * design places timestamps in the key - so that the application can efficiently read both the
 * current and historical property values within a logical row.
 *
 * <p>Regardless of whether the timestamp is on the index entry (as it always is for isolatable
 * indices) or in the key ({@link SparseRowStore}), the existence of timestamps makes it possible
 * for an application to specify a history policy governing when property values will be deleted.
 *
 * <p>When an index participates in transactions the transaction manager manages the life cycle of
 * overwritten and deleted index entries (and the resources on which the indices exist). This is
 * done by preserving such data until no transaction exists that can read from those resources.
 * Unless an immortal store is desired, the "purge time" is set at a time no more recent than the
 * earliest fully isolated transaction (either a read-only tx as of the start time of the tx or a
 * read-write tx as of its start time). The role of a "history policy" with transactions is
 * therefore how much history to buffer between the earliest running tx and the chosen "purge time".
 * When the transaction manager updates the "purge time" it notifies the journal/data services.
 * Resources having no data later than the purge time may be deleted and SHOULD NOT be carried
 * forward when building new index segments.
 *
 * <p>History policies for non-transactional indices are somewhat different. An scale-out index
 * without timestamps will buffer historical data only until the next compacting merge of a given
 * index partition. The compacting merge uses the fused view of the resources comprising the index
 * partition and only writes out the undeleted index entries.
 *
 * <p>If an application instead chooses to use timestamps in a non-transactional index then (a)
 * timestamps must be assigned by either the client or the data service; and (b) applications can
 * specify a history policy where data older than a threshold time (but not #of versions) will be
 * eradicated. This approach is possible, but not well-supported in the higher level APIs.
 *
 * <p>The {@link SparseRowStore} design is more flexible since it allows (a) fast access to
 * historical property values for the same "row"; and (b) history policies that may be specified in
 * terms of the #of versions, the age of a datum, and that can keep at least N versions of a datum.
 * This last point is quite important as it allows you to retain the entirety of the most current
 * revision of the logical row, even when some datums are much older than others.
 *
 * <h2>Serialization</h2>
 *
 * <p>Note: Derived classes SHOULD extend the {@link Externalizable} interface and explicitly manage
 * serialization versions so that their metadata may evolve in a backward compatible manner.
 *
 * @todo Make sure that metadata for index partition "zones" propagates with the partition metadata
 *     so that appropriate policies are enforcable locally (access control, latency requirements,
 *     replication, purging of historical deleted versions, etc).
 * @todo add optional property containing IndexMetadata to be used as of the next overflow so that
 *     people can swap out key and value serializers and the like during overflow operations. the
 *     conops is that you map the new metadata over the index partitions and either lazily or
 *     eagerly overflow is triggered and the resulting {@link BTree} and {@link IndexSegment}
 *     objects will begin to use the new key / value serializers, etc. while the existing objects
 *     will still have their old key/val serializers and therefore can still be read.
...
public class IndexMetadata implements Serializable, Externalizable, Cloneable, IKeyBuilderFactory {
   ...
   * Options that apply to all {@link AbstractBTree}s are specified within that namespace while
   * those that are specific to {@link BTree} or {@link IndexSegment} are located within their
   * respective class namespaces. Some properties, such as the branchingFactor, are defined for both
   * the {@link BTree} and the {@link IndexSegment} because their defaults tend to be different when
   * an {@link IndexSegment} is generated from an {@link BTree}.
   *
   * @todo It should be possible to specify the key, value, and node/leaf coders via this interface.
   *     This is easy enough if there is a standard factory interface, since we can specify the
   *     class name, and more difficult if we need to create an instance.
   *     <p>Note: The basic pattern here is using the class name, having a default instance of the
   *     class (or a factory for that instance), and then being able to override properties for that
   *     instance. Beans stuff really, just simpler.
   * @todo it should be possible to specify the overflow handler and its properties via options (as
   *     you can with beans or jini configurations).
   * @todo it should be possible to specify a different split handler and its properties via options
   *     (as you can with beans or jini configurations).
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  public interface Options {
     ...
     * values are of benefit if you are doing sustained writes on the index and have a large java
     * heap (and even then, GC will probably prevent values larger than 10000 from being useful).
     */
     ...
     * rate would be too large given the #of index entries. For scale-out, as the index grows we
     * keep splitting it into more and more index partitions, and those index partitions are
     * comprised of both views of one or more {@link AbstractBTree}s. While the mutable {@link
     * BTree}s might occasionally grow to large to support a bloom filter, data is periodically
     * migrated onto immutable {@link IndexSegment}s which have perfect fit bloom filters. This
     ...
     */
    String BLOOM_FILTER =
        (org.embergraph.btree.BTree.class.getPackage().getName() + ".bloomFilter").intern();
     ...
     */
    String MAX_REC_LEN =
        (org.embergraph.btree.BTree.class.getPackage().getName() + ".maxRecLen").intern();
     ...
     * up to the administrator to not assign the same name to different data service instances and
     * an arbitrary instance having the desired name will be used if more than one instance is
     * assigned the same name). The default behavior is to select a data service using the load
     * balancer, which is done automatically by {@link
     * IEmbergraphFederation#registerIndex(IndexMetadata, UUID)} if {@link
     * IndexMetadata#getInitialDataServiceUUID()} returns <code>null</code>.
     */
     ...
     *
     * <p>The purpose of this queue is to retain recently touched nodes and leaves and to defer
     * eviction of dirty nodes and leaves in case they will be modified again soon. Once a node
     * falls off the write retention queue it is checked to see if it is dirty. If it is dirty, then
     * it is serialized and persisted on the backing store. If the write retention queue capacity is
     * set to a large value (say, GTE 1000), then that will will increase the commit latency and
     * have a negative effect on the overall performance. Too small a value will mean that nodes
     * that are undergoing mutation will be serialized and persisted prematurely leading to
     * excessive writes on the backing store. For append-only stores, this directly contributes to
     * what are effectively redundant and thereafter unreachable copies of the intermediate state of
     * nodes as only nodes that can be reached by navigation from a {@link Checkpoint} will ever be
     * read again. The value <code>500</code> appears to be a good default. While it is possible
     * that some workloads could benefit from a larger value, this leads to higher commit latency
     * and can therefore have a broad impact on performance.
     *
     * <p>Note: The write retention queue is used for both {@link BTree} and {@link IndexSegment}.
     * Any touched node or leaf is placed onto this queue. As nodes and leaves are evicted from this
     * queue, they are then placed onto the optional read-retention queue.
     *
     * <p>The default value is a function of the JVM heap size. For small heaps, it is {@value
     * #DEFAULT_WRITE_RETENTION_QUEUE_CAPACITY}. For larger heaps the value may be 8000 (1G), or
     * 20000 (10G). These larger defaults are heuristics. Values larger than 8000 benefit the size
     * of disk of the journal, while values up to 8000 can also improve throughput dramatically.
     * Larger values are ONLY useful if the application is performing sustained writes on the index
     * (hundreds of thousands to millions of records).
     */
    String WRITE_RETENTION_QUEUE_CAPACITY =
        (org.embergraph.btree.AbstractBTree.class.getPackage().getName()
     ...
     * eviction drives IOs required to write the leaves on the store, but incremental writes occur
     * iff the {@link AbstractNode#referenceCount} is zero and the node or leaf is dirty.
     */
    String WRITE_RETENTION_QUEUE_SCAN =
        (org.embergraph.btree.AbstractBTree.class.getPackage().getName()
     ...
     * object).
     *
     * <p>FIXME {@link KeyBuilder} configuration support is not finished.
     */
    String KEY_BUILDER_FACTORY =
...
//         * Option determines whether or not per-child locks are used by
     ...
     * base class.
     *
     * @todo in order to provide a similar specialization mechanism for scale-out indices you would
     *     need to specify the class name for the {@link IndexSegment} and the {@link FusedView}.
     *     You might also need to override the {@link Checkpoint} class - for example the {@link
     *     MetadataIndex} does this.
     */
     ...
     * capacity to 64 (was 32) and 8000 (was 500) respectively. This change in the B+Tree branching
     * factor reduces the height of B+Trees on the Journal, increases the size of the individual
     * records on the disk, and aids performance substantially. The larger write retention queue
     * capacity helps to prevent B+Tree nodes and leaves from being coded and flushed to disk too
     * soon, which decreases disk IO and keeps things in their mutable form in memory longer, which
     * improves search performance and keeps down the costs of mutation operations. [Dropped back to
     * 32/500 on 9/15/09 since this does not do so well at scale on machines with less RAM.]
     */
...
//         * The #of entries on the hard reference queue that will be scanned for
     ...
     *     <p>FIXME Record level compression support is not finished.
     */
    String BTREE_RECORD_COMPRESSOR_FACTORY =
        (BTree.class.getName() + ".recordCompressorFactory").intern();
     ...
     *
     * <p>Note: This is currently a System property (set with -D).
     *
     * @see https://jira.blazegraph.com/browse/BLZG-1665 (Reduce commit latency by parallel
     *     checkpoint by level of dirty pages in an index)
     */
     ...
     *     checkpoint by level of dirty pages in an index)
     */
    String MIN_DIRTY_LIST_SIZE_FOR_PARALLEL_EVICT =
        BTree.class.getName() + ".minDirtyListSizeForParallelEvict";
     ...
     * IndexSegment}.
     */
    String INDEX_SEGMENT_BRANCHING_FACTOR =
        (IndexSegment.class.getName() + ".branchingFactor").intern();
     ...
     * IndexSegmentBuilder}. That region may be fully buffered when the {@link IndexSegment} is
     * opened, in which case queries against the {@link IndexSegment} will incur NO disk hits for
     * the nodes and only one disk hit per visited leaf.
     *
     * <p>Note: The nodes are read into a buffer allocated from the {@link DirectBufferPool}. If the
     * size of the nodes region in the {@link IndexSegmentStore} file exceeds the capacity of the
     * buffers managed by the {@link DirectBufferPool}, then the nodes WILL NOT be buffered. The
     * {@link DirectBufferPool} is used both for efficiency and because a bug dealing with temporary
     * direct buffers would otherwise cause the C heap to be exhausted!
     *
     * @see #DEFAULT_INDEX_SEGMENT_BUFFER_NODES
     * @todo should be on by default? (but verify that the unit tests do not run out of memory when
     *     it is enabled by default).
     */
    String INDEX_SEGMENT_BUFFER_NODES = (IndexSegment.class.getName() + ".bufferNodes").intern();
...
//         *
     ...
     *     <p>FIXME Record level compression support is not finished.
     */
    String INDEX_SEGMENT_RECORD_COMPRESSOR_FACTORY =
        (IndexSegment.class.getName() + ".recordCompressorFactory").intern();
...
//         * The #of samples to take per estimated split (non-negative, and
     ...
     */
    String MASTER_QUEUE_CAPACITY =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".masterQueueCapacity").intern();

    String DEFAULT_MASTER_QUEUE_CAPACITY = "5000";

    /** The desired size of the chunks that the master will draw from its queue. */
    String MASTER_CHUNK_SIZE =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".masterChunkSize").intern();
     ...
     */
    String MASTER_CHUNK_TIMEOUT_NANOS =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".masterChunkTimeoutNanos").intern();
     ...
     */
    String SINK_POLL_TIMEOUT_NANOS =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".sinkPollTimeoutNanos").intern();
    ...
    String SINK_QUEUE_CAPACITY =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".sinkQueueCapacity").intern();
     ...
     * sink}.
     */
    String SINK_CHUNK_SIZE =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".sinkChunkSize").intern();
     ...
     * sink will simply wait until {@link #SINK_CHUNK_SIZE} elements have accumulated before writing
     * on the index partition. This makes it much easier to adjust the performance since you simply
     * adjust the {@link #SINK_CHUNK_SIZE}.
     */
    String SINK_CHUNK_TIMEOUT_NANOS =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".sinkChunkTimeoutNanos").intern();
     ...
     * chunk is drained from the sink. If no chunks become available the the sink will eventually
     * decide that it is idle, will flush any buffered writes, and will close itself.
     *
     * <p>If the idle timeout is LT the {@link #SINK_CHUNK_TIMEOUT_NANOS} then a sink will remain
     * open as long as new chunks appear and are combined within idle timeout, otherwise the sink
     * will decide that it is idle and will flush its last chunk and close itself. If this is {@link
     * Long#MAX_VALUE} then the sink will identify itself as idle and will only be closed if the
     * master is closed or the sink has received a {@link StaleLocatorException} for the index
     * partition on which the sink is writing.
     */
    // GTE chunkTimeout
    String SINK_IDLE_TIMEOUT_NANOS =
        (AsynchronousIndexWriteConfiguration.class.getName() + ".sinkIdleTimeoutNanos").intern();
     ...
     *
     * <p>Sometimes a scatter split is not the "right" thing for an index. An example would be an
     * index where you have to do a LOT of synchronous RPC rather than using asynchronous index
     * writes. In this case, the synchronous RPC can be a bottleneck unless the "chunk" size of the
     * writes is large. This is especially true when writes on other indices must wait for the
     * outcome of the synchronous RPC. E.g., foreign keys.
     *
     * @see OverflowManager.Options#SCATTER_SPLIT_ENABLED
     */
    String SCATTER_SPLIT_ENABLED =
        (ScatterSplitConfiguration.class.getName() + ".enabled").intern();
     ...
     * into multiple partitions and distribute those index partitions across the federation in order
     * to allow more resources to be brought to bear on the scale-out index. The value must LT the
     * nominal index partition split point or normal index splits will take precedence and a scatter
     * split will never be performed. The allowable range is therefore constrained to <code>
     * (0.1 : 1.0)</code>.
     */
    String SCATTER_SPLIT_PERCENT_OF_SPLIT_THRESHOLD =
        (ScatterSplitConfiguration.class.getName() + ".percentOfSplitThreshold").intern();
     ...
     */
    String SCATTER_SPLIT_DATA_SERVICE_COUNT =
        (ScatterSplitConfiguration.class.getName() + ".dataServiceCount").intern();
     ...
     * </code> index partitions, where NDATA_SERVICES is either {@link
     * #SCATTER_SPLIT_DATA_SERVICE_COUNT} or the #of discovered data services when that option is
     * ZERO (0).
     *
     * <p>The "ideal" number of index partitions is generally between (NCORES x NDATA_SERVICES /
     * NINDICES) and (NCORES x NDATA_SERVICES). When there are NCORES x NDATA_SERVICES index
     * partitions, each core is capable of servicing a distinct index partition assuming that the
     * application and the "schema" are capable of driving the data service writes with that
     * concurrency. However, if you have NINDICES, and the application drives writes on all index
     * partitions of all indices at the same rate, then a 1:1 allocation of index partitions to
     * cores would be "ideal".
     *
     * <p>The "right" answer also depends on the data scale. If you have far less data than can fill
     * that many index partitions to 200M each, then you should adjust the scatter split to use
     * fewer index partitions or fewer data services.
     *
     * <p>Finally, the higher the scatter the more you will need to use asynchronous index writes in
     * order to obtain high throughput with sustained index writes.
     */
    String SCATTER_SPLIT_INDEX_PARTITION_COUNT =
        (ScatterSplitConfiguration.class.getName() + ".indexPartitionCount").intern();
   ...
   * is read from, or written on, the store. It is zero when you {@link #clone()} a metadata record
   * until it's been written onto the store.
   */
   ...
   * set by the ctor(s) that are used to create a new {@link IndexMetadata} instance, so no
   * additional lookups are performed during de-serialization.
   *
   ...
   * structure.
   *
   * <p>Note: When using a scale-out index the same <i>indexUUID</i> MUST be assigned to each
   * mutable and immutable B+Tree having data for any partition of that scale-out index. This makes
   * it possible to work backwards from the B+Tree data structures and identify the index to which
   * they belong.
   */
   ...
   *
   * <p>Note: When the index is a scale-out index, this is the name of the scale-out index NOT the
   * name under which an index partition is registered.
   *
   * <p>Note: When the index is a metadata index, then this is the name of the metadata index itself
   * NOT the name of the managed scale-out index.
   */
   ...
   * returns at which the amount of copying performed to move the data around in the nodes and
   * leaves exceeds the performance gain from having fewer levels. The branching factor for the
   * read-only {@link IndexSegment}s is generally much larger in order to reduce the number of disk
   * seeks.
   */
   ...
   * journal. There are two reasons why it makes sense to use a larger branching factor for an index
   * segment. First, the WORM Journal is used to buffer writes in scale-out and IO on an index on
   * the WORM Journal is driven by node and leaf revisions so the index often uses a smaller
   * branching factor on the WORM. Second, the index segment is laid out in total key order in the
   * file and each node and leaf is a contiguous sequences of bytes on the disk (like the WORM, but
   * unlike the RWStore). Since most of the latency of the disk is the seek, reading larger leaves
   * from an index segment is efficient.
   *
   * <p>Note: the value of this property will determine the branching factor of the {@link
   * IndexSegment}. When the {@link IndexSegment} is built, it will be given a {@link #clone()} of
   * this {@link IndexMetadata} and the actual branching factor for the {@link IndexSegment} be set
   * on the {@link #getBranchingFactor()} at that time.
   *
   * <p>Note: a branching factor of 256 for an index segment and split limits of (1M,5M) imply an
   * average B+Tree height of 1.5 to 1.8. With a 10ms seek time and NO CACHE that is between 15 and
   * 18ms average seek time.
   *
   * <p>Note: a branching factor of 512 for an index segment and split limits of (1M,5M) imply an
   * average B+Tree height of 1.2 to 1.5. With a 10ms seek time and NO CACHE that is between 12 and
   * 15ms average seek time.
   *
   * <p>Note: the actual size of the index segment of course depends heavily on (a) whether or now
   * block references are being stored since the referenced blocks are also stored in the index
   * segment; (b) the size of the keys and values stored in the index; and (c) the key, value, and
   * record compression options in use.
   */
...
//     * @see Options#BTREE_READ_RETENTION_QUEUE_SCAN
   ...
   * minimum length separator keys dynamically when a leaf is split or joined the keys in the node
   * typically DO NOT conform to application expectations and MAY be assigned a different {@link
   * IRabaCoder} for that reason.
   *
   ...
   * validation. If a write-write conflict is detected and a conflict resolver is defined, then the
   * conflict resolver is expected to make a best attempt using data type specific rules to
   * reconcile the state for two versions of the same persistent identifier. If the conflict can not
   * be resolved, then validation will fail. State-based conflict resolution when combined with
   * validation (aka optimistic locking) is capable of validating the greatest number of
   * interleavings of transactions (aka serialization orders).
   *
   * @return The conflict resolver to be applied during validation or <code>null</code> iff no
   ...
   *
   * <p>Delete markers MUST be enabled to use scale-out indices. Index partition views depend on an
   * ordered array of {@link AbstractBTree}s. The presence of a delete marker provides an indication
   * of a deleted index entry and is used to prevent reading of index entries for the same key which
   * might exist in an earlier {@link AbstractBTree} which is part of the same index partition view.
   *
   * <p>Delete markers MUST be enabled for transaction support where they play a similar role
   * recording within the write set of the transaction the fact that an index entry has been
   * deleted.
   */
   ...
   * revision timestamp range. This filtering is efficient because it skips any node (and all
   * spanned nodes or leaves) which does not have data for the desired revision timestamp range. In
   * order to find all updates after a given timestamp revision, you specify
   * (fromRevision,Long.MAX_VALUE). In order to visit the delta between two revisions, you specify
   * (fromRevision, toRevision+1).
   *
   * <p>Tuple revision filtering can be very efficient for some purposes. For example, it can be
   * used to synchronize disconnected clients or compute the write set of a committed transaction.
   * However, it requires more space in the {@link INodeData} records since we must store the
   * minimum and maximum timestamp revision for each child of a given node.
   *
   * <p>Per-tuple timestamp revisions MAY be used without support for per-tuple revision filtering.
   *
   ...
   *
   * @param versionTimestampFilters <code>true</code> iff version timestamp filtering should be
   *     supported.
   */
  public final void setVersionTimestampFilters(final boolean versionTimestampFilters) {
   ...
   *
   * @param isolatable <code>true</code> if delete markers and version timestamps will be enabled
   *     -or- <code>false</code> if they will be disabled.
   */
   ...
   * However, the leaves will occupy slightly more space (~ 1 bit per tuple) if this option is
   * enabled and none of the values stored in the index exceed the configured maximum value length.
   * {@link IRabaCoder}s which rely on a uniform value length generally already use small values and
   * should typically turn this feature off in order to make the leaf as compact as possible.
   *
   * @param rawRecords <code>true</code> if the feature is to be enabled.
   ...
   * record references. Note that this method returns the configured value regardless of the value
   * of {@link #getRawRecords()} - the caller must check {@link #getRawRecords()} in order to
   * correctly interpret the value returned by this method.
   *
   ...
   *     value of ZERO (0) may be used to force all values into raw records.
   * @throws IllegalArgumentException if the argument is negative or greater than {@link
   *     Short#MAX_VALUE}
   * @see #setRawRecords(boolean)
   */
  public final void setMaxRecLen(final int maxRecLen) {

    if (maxRecLen < 0 || maxRecLen > Short.MAX_VALUE) throw new IllegalArgumentException();
  ...
  public void setNodeKeySerializer(final IRabaCoder nodeKeysCoder) {
    ...
    if (nodeKeysCoder == null) throw new IllegalArgumentException();
   ...
   * configurable probability of a false positive. Since the bloom filter does not give positive
   * results with 100% certainity, the index is tested iff the bloom filter states that the key
   * exists.
   *
   * <p>Note: Bloom filters are NOT enabled by default since point tests are not a bottleneck (or
   * even used) for some indices. Also, when multiple indices represent different access paths for
   * the same information, you only need a bloom filter on one of those indices.
   *
   * @return Return the object that will be used to configure an optional bloom filter for a {@link
   *     BTree} or {@link IndexSegment}. When <code>null</code> the index WILL NOT use a bloom
   *     filter.
   ...
   * exists.
   *
   * @param bloomFilterFactory The new value (may be null).
   * @see BloomFilterFactory#DEFAULT
   */
   ...
   * whose size on the disk is bounded by the parameter specified using {@link
   * OverflowManager.Options#NOMINAL_SHARD_SIZE}. Indices which require certain guarantees for
   * atomicity, such as the {@link SparseRowStore}, must override this default.
   *
   * @return The {@link ISimpleSplitHandler} -or- <code>null</code> if the application has not
   *     imposed any additional constraints on the separator keys when splitting index partitions.
   */
   ...
   * @return An instance of that class -or- <code>null</code> iff the class name is <code>null
   *     </code>.
   * @throws RuntimeException if the class does not implement that interface or for any other
   *     reason.
   */
  @SuppressWarnings("unchecked")
  private static <T> T newInstance(final String className, final Class<T> iface) {
    ...
    try {
      ...
      return (T) cls.getConstructor(new Class[] {}).newInstance(new Object[] {});
  ...
  public IndexMetadata(
...

    {
       ...
       * Attempt to interpret the value as either a UUID or the name of
        ...
        try {
          ...
          if (log.isInfoEnabled()) log.info("Not a UUID: " + val);
    ...
    this.branchingFactor =
        getProperty(
            ...
            new IntegerRangeValidator(
                Options.MIN_BRANCHING_FACTOR, Options.MAX_BTREE_BRANCHING_FACTOR));

    {
      ...
      //            }
      this.writeRetentionQueueCapacity =
          getProperty(
              indexManager,
              properties,
              namespace,
    ...
    this.writeRetentionQueueScan =
        getProperty(
            indexManager,
            properties,
            namespace,
            Options.WRITE_RETENTION_QUEUE_SCAN,
    ...
    this.btreeRecordCompressorFactory =
        newInstance(
            getProperty(
                indexManager,
                properties,
    ...
    this.indexSegmentBranchingFactor =
        getProperty(
            indexManager,
            properties,
            namespace,
            Options.INDEX_SEGMENT_BRANCHING_FACTOR,
            Options.DEFAULT_INDEX_SEGMENT_BRANCHING_FACTOR,
            new IntegerRangeValidator(
                Options.MIN_BRANCHING_FACTOR, Options.MAX_INDEX_SEGMENT_BRANCHING_FACTOR));

    this.indexSegmentBufferNodes =
    ...
    this.indexSegmentRecordCompressorFactory =
        newInstance(
            getProperty(
                indexManager,
                properties,
    ...
    this.nodeKeysCoder =
        newInstance(
            getProperty(
                ...
                properties,
                namespace,
                Options.NODE_KEYS_CODER,
                keyRabaCoder.getName()),
            IRabaCoder.class);
    ...
    // this.tupleSer = DefaultTupleSerializer.newInstance();
    {
      ...
      final IKeyBuilderFactory keyBuilderFactory =
          DefaultTupleSerializer.getDefaultKeyBuilderFactory();
      ...
      final IRabaCoder valuesCoder =
          newInstance(
              getProperty(
                  ...
                  properties,
                  namespace,
                  Options.LEAF_VALUES_CODER,
                  CanonicalHuffmanRabaCoder.class.getName()),
              IRabaCoder.class);
     ...
     */
    {
      ...
      this.asynchronousIndexWriteConfiguration =
    ...
    // Scatter-split configuration
    {
      final boolean scatterSplitEnabled =
          Boolean.parseBoolean(
              getProperty(
                  indexManager,
                  properties,
      ...
      final int scatterSplitDataServicesCount =
          Integer.parseInt(
              getProperty(
                  indexManager,
                  properties,
      ...
      final int scatterSplitIndexPartitionsCount =
          Integer.parseInt(
              getProperty(
                  indexManager,
                  properties,
   ...
   * @return The address of the metadata record is set on this object as a side effect.
   * @throws IllegalStateException if the record has already been written on the store.
   * @throws IllegalStateException if the {@link #indexUUID} field is <code>null</code> - this
   *     generally indicates that you used the de-serialization constructor rather than one of the
   *     constructor variants that accept the required UUID parameter.
   */
   ...
   * @param addr the address of the metadata record.
   * @return the metadata record. The address from which it was loaded is set on the metadata record
   *     as a side-effect.
   */
  public static IndexMetadata read(final IRawStore store, final long addr) {

    final IndexMetadata metadata = (IndexMetadata) SerializerUtil.deserialize(store.read(addr));
   ...
   * the {@link #indexType} field. This field defaults to {@link IndexTypeEnum#BTree} for all prior
   * versions.
   */
  private static final transient int VERSION4 = 0x4;
  ...
  public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    btreeRecordCompressorFactory = (IRecordCompressorFactory<?>) in.readObject();
    ...
    indexSegmentRecordCompressorFactory = (IRecordCompressorFactory<?>) in.readObject();

    asynchronousIndexWriteConfiguration = (AsynchronousIndexWriteConfiguration) in.readObject();
   ...
   *
   * <p>The class identified by {@link #getCheckpointClassName()} MUST declare a public constructor
   ...
   * </pre>
   *
   * @param oldCheckpoint The last checkpoint for the index of the old backing store.
   * @return The first {@link Checkpoint} for the index on the new backing store.
   * @throws IllegalArgumentException if the oldCheckpoint is <code>null</code>.
   */
  @SuppressWarnings("unchecked")
  public final Checkpoint overflowCheckpoint(final Checkpoint oldCheckpoint) {
   ...
   *
   * <p>Note: If you change this value in a manner that is not backward compatable once entries have
   * been written on the index then you may be unable to any read data already written.
   *
   * <p>Note: This method delegates to {@link ITupleSerializer#getKeyBuilder()}. This {@link
   * IKeyBuilder} SHOULD be used to form all keys for <i>this</i> index. This is critical for
   * indices that have Unicode data in their application keys as the formation of Unicode sort keys
   * from Unicode data depends on the {@link IKeyBuilderFactory}. If you use a locally configured
   * {@link IKeyBuilder} then your Unicode keys will be encoded based on the {@link Locale}
   * configured for the JVM NOT the factory specified for <i>this</i> index.
   */
  @Override
  ...
  protected <E> E getProperty(
    ...
    return Configuration.getProperty(
        indexManager, properties, namespace, globalName, defaultValue, validator);
  }
}
