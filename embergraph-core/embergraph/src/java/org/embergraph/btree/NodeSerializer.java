 ...
 * that some fields can be tested without full de-serialization, especially whether the record
 * contains a leaf vs a node. This fixed record also makes it possible to update some fields in the
 * header once the entire record has been serialized, including the checksum, the #of bytes in the
 * serialized record, and the prior/next addresses for leaves.
 *
 * <p>The methods defined by this class all work with {@link ByteBuffer}s. On read, the buffer must
 * be positioned to the start of the data to be read. After a read, the buffer will be positioned to
 * the first byte after the data read. If there is insufficient data available in the buffer then an
 * {@link BufferUnderflowException} will be thrown. On write, the data will be written on an
 * internal buffer whose size is automatically extended. The write buffer is reused for each write
 * and quickly achieves a maximum size for any given {@link BTree}.
 *
 * <p>Note: while the {@link NodeSerializer} is NOT thread-safe for writers, it is thread-safe for
 * readers. This design mirrors the concurrency capabilities of the {@link AbstractBTree}.
 *
...
public class NodeSerializer {
  ...
  /*
   * An object that knows how to construct {@link Node}s and {@link Leaf}s from {@link INodeData}
   * and {@link ILeafData} objects.
   */
   ...
   * convert its nodes and leaves into coded data records as they are evicted from the write
   * retention queue.
   */
   ...
   *
   * <p>Note: this buffer is discarded by {@link #close()} when the btree is {@link
   * AbstractBTree#close() closed} and then reallocated on demand.
   *
   * <p>Note: It is important that this field NOT be used for a read-only {@link BTree} since only
   * mutable {@link BTree}s are single threaded - concurrent readers are allowed for read-only
   * btrees.
   *
  ...
  /** Instance used for writes - this keeps a hard reference since writes are single threaded. */
   ...
   * buffer capacity is this value times the {@link #branchingFactor}.
   */
   ...
   *     ILeafData leaves}.
   * @param branchingFactor The branching factor for target B+Tree. For the {@link
   *     IndexSegmentBuilder} this should be the branching factor of the {@link IndexSegment} which
   *     is being generated. Otherwise it is the branching factor of the {@link AbstractBTree}
   *     itself.
   * @param initialBufferCapacity The initial capacity for internal buffer used to serialize nodes
   *     and leaves. The buffer will be resized as necessary until it is sufficient for the records
   *     being serialized for the {@link BTree}. When zero (0), a default is used. A non-zero value
   *     is worth specifying only when the actual buffer size is consistently less than the default
   *     for some {@link BTree}. See {@link #DEFAULT_BUFFER_CAPACITY_PER_ENTRY}
   * @param indexMetadata The {@link IndexMetadata} record for the index.
   * @param readOnly <code>true</code> IFF the caller is asserting that they WILL NOT attempt to
   *     serialize any nodes or leaves using this {@link NodeSerializer} instance.
   *     <p>FIXME {@link IRecordCompressorFactory} should either be used here or moved into the
   *     {@link IRawStore} impl.
   * @todo the {@link IAddressManager} is not used any more. It was used by the {@link
   *     IAddressSerializer}.
   */
  public NodeSerializer(
    ...
    this.nodeCoder = new DefaultNodeCoder(indexMetadata.getNodeKeySerializer());

    this.leafCoder =
        new DefaultLeafCoder(
            indexMetadata.getTupleSerializer().getLeafKeysCoder(),
            indexMetadata.getTupleSerializer().getLeafValuesCoder());
    ...
    if (readOnly) {
      ...
      if (initialBufferCapacity == 0) {
        ...
        this.initialBufferCapacity = DEFAULT_BUFFER_CAPACITY_PER_ENTRY * branchingFactor;
  ...
  private void allocWriteBuffer() {
    ...
    _writeCompressor =
        recordCompressorFactory == null
            ? NOPRecordCompressor.INSTANCE
  ...
  int getWriteBufferCapacity() {
    ...
    if (tmp == null) return initialBufferCapacity;
   ...
   * leaf, or linked-leaf.
   *
   * @param buf The data record.
   * @return A {@link INodeData} or {@link ILeafData} instance for that data record.
   *     <p>FIXME modify to accept {@link IDataRecord} rather than {@link ByteBuffer}.
   */
  public IAbstractNodeData decode(final ByteBuffer buf) {
    ...
    final boolean isNode =
        AbstractReadOnlyNodeData.isNode(buf.get(AbstractReadOnlyNodeData.O_TYPE));
    ...
    if (!buf.hasArray()) {
      ...
      if (EmbergraphStatics.debug) System.err.print("[RO]");
    } else {
  ...
  /*
   * Wrap an {@link INodeData} or {@link ILeafData} instance as a {@link Node} or a {@link Leaf}.
   * This DOES NOT set the parent of the new {@link Node} or {@link Leaf}.
   *
   * @param btree The owning B+Tree.
   * @param addr The address of the data record in the backing store.
   * @param data The data record.
   * @return The node or leaf.
   */
  public AbstractNode<?> wrap(
      final AbstractBTree btree, final long addr, final IAbstractNodeData data) {
   ...
   * @return The buffer containing the coded data record for the node or leaf in an exact fit byte[]
   *     owned by the coded node or leaf.
   */
  ...
  public <T extends IAbstractNodeData> T encodeLive(final T node) {
    ...
    if (node.isLeaf()) {
      ...
      codedNode = (T) leafCoder.encodeLive((ILeafData) node, _writeBuffer);

    } else {

      codedNode = (T) nodeCoder.encodeLive((INodeData) node, _writeBuffer);
    }
   ...
   * writes on an internal buffer which is automatically extended as required.
   *
   * @param node The node or leaf.
   * @return The buffer containing the coded data record for the node or leaf in a <em>shared
   *     buffer</em>. The contents of this buffer may be overwritten by the next node or leaf
   *     serialized the same instance of this class. The position will be zero and the limit will be
   *     the #of bytes in the coded representation.
   * @deprecated This method is no longer used since I refactored the {@link IndexSegmentBuilder} to
   *     optionally stuff the generated nodes and leaves into the cache. It still works but it might
   ...
   *
   * <p>Note: In order to use this method to write linked leaves on the store you have to either
   * write behind at a pre-determined address on the store or settle for writing only the prior or
   * the next leaf address, but not both. It is up to the caller to perform these tricks. All this
   * method does is to touch up the serialized record.
   *
   * <p>Note: This method has NO side-effects on the <i>position</i> or <i>limit</i> of the caller's
   * {@link ByteBuffer}.
   *
   * @param b The serialization leaf.
   * @param priorAddr The address of the previous leaf in key order, <code>0L</code> if it is known
   *     that there is no previous leaf, and <code>-1L</code> if either: (a) it is not known whether
   *     there is a previous leaf; or (b) it is known but the address of that leaf is not known to
   *     the caller.
   * @param nextAddr The address of the next leaf in key order, <code>0L</code> if it is known that
   *     there is no next leaf, and <code>-1L</code> if either: (a) it is not known whether there is
   *     a next leaf; or (b) it is known but the address of that leaf is not known to the caller.
   * @see IndexSegmentBuilder
   * @see DefaultLeafCoder
   */
  public void updateLeaf(final ByteBuffer b, final long priorAddr, final long nextAddr) {

    if (AbstractReadOnlyNodeData.isNode(b.get(AbstractReadOnlyNodeData.O_TYPE))) {
