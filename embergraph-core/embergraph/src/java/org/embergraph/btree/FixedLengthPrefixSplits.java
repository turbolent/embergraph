...
public class FixedLengthPrefixSplits implements ISimpleSplitHandler, Serializable, Externalizable {
   ...
   * @todo We could also search backward or use a forward/backward search, or a progressive search,
   *     but there should seldom be any reason to do so.
   * @todo This could also be done without materializing the keys we are testing if it was written
  ...
  public byte[] getSeparatorKey(
    ...
    for (int i = splitAt + 1; i < toIndex; i++) {
       ...
       * The N byte prefix has changed. Clone the first N bytes of the
        ...
        System.arraycopy(
            b /* src */, 0 /* srcPos */, prefix /* dest */, 0 /* destPos */, N /* length */);
