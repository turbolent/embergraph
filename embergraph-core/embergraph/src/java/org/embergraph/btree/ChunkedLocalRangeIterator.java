 ...
 * buffer is full or as soon as the iterator is exhausted. This approach works with the somewhat
 * faster {@link ITupleIterator} construct. When the {@link IRangeQuery#CURSOR} flag is specified,
 * we directly use the {@link ITupleCursor} interface.
 *
...
public class ChunkedLocalRangeIterator<E> extends AbstractChunkedTupleIterator<E> {
   ...
   * @param filter
   */
  public ChunkedLocalRangeIterator(
      IIndex ndx, byte[] fromKey, byte[] toKey, int capacity, int flags, IFilter filter) {
  ...
  /** Note: <i>timestamp</i> is ignored since we are reading against a local index object. */
  @Override
  protected ResultSet getResultSet(
     ...
     * specified.
     */
    final int tmpFlags = (removeAll && !cursor) ? flags & ~IRangeQuery.REMOVEALL : flags;
    ...
    final int rangeCount = (int) Math.min((long) Integer.MAX_VALUE, ndx.rangeCount(fromKey, toKey));
    ...
    final ITupleIterator itr =
        ndx.rangeIterator(fromKey, toKey, limit, tmpFlags | IRangeQuery.KEYS, filter);
   ...
   *
   * <p>Note: Queuing for removal is done only for the local index so that data service range
   * iterators will do their deletes on the local index when this range iterator runs rather than
   * buffering the keys and then sending back a batch delete to the index later (this would also
   * make {@link IRangeQuery#REMOVEALL} non-atomic).
   */
   ...
   * ITupleFilter)} (since it is reading against a local index object).
   */
  @Override
