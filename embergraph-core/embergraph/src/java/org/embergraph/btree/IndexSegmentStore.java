...
public class IndexSegmentStore extends AbstractRawStore {
   ...
   * that we would always like to have on hand.
   */
   ...
   *
   * <p>Note: This buffer is acquired from the {@link DirectBufferPool} and MUST be released back to
   * that pool.
   *
   * <p>Note: While some nodes will be held in memory by the hard reference queue the use of this
   * buffer means that reading a node that has fallen off of the queue does not require any IO.
   */
   ...
   *
   * <p>A shared {@link FileLock} is requested. If the platform and the volume either DO NOT support
   * {@link FileLock} or support <em>shared</em> {@link FileLock}s then you will be able to open the
   * same {@link IndexSegmentStore} in multiple applications. However, if the platform does not
   * support shared locks then the lock request is converted (by Java) into an exclusive {@link
   * FileLock} and you will not be able to open the {@link IndexSegmentStore} in more than one
   * application at a time.
   *
   * <p>Note: A shared {@link FileLock} makes it impossible to delete an {@link IndexSegmentStore}
   * that is in use. {@link FileLock}s are automatically released when the {@link FileChannel} is
   * closed or the application dies. Using an advisory lock is NOT a good idea as it can leave lock
   * files in place which make it impossible to restart a data service after an abnormal
   * termination. For that reason it is better to NOT use advisory locks on platforms and volumes
   * which do not support {@link FileLock}.
   *
   * @see #reopenChannel()
   */
   ...
   * this during the ctor we do not have to force the entire index segment to be loaded just to
   * access the index metadata.
   */
   ...
   * LocalPartitionMetadata#getResources()} is <code>null</code>. This is because the {@link BTree}
   * on the {@link AbstractJournal} defines the index partition view and each {@link IndexSegment}
   * generally participates in MANY views - one per commit point on each {@link AbstractJournal}
   * where the {@link IndexSegment} is part of an index partition view.
   */
   ...
   *
   * <p>Note: If an exception is thrown then the backing file will be closed.
   *
   * <p>Note: Normally access to {@link IndexSegmentStore}s is mediated by the {@link StoreManager}
   * which imposes a canonicalizing weak value cache to ensure that we do not double-open an {@link
   * IndexSegmentStore}.
   *
   * @param file The file
   * @see #loadIndexSegment()
   * @throws RuntimeException if there is a problem.
   * @throws RootBlockException if the root block is invalid.
   */
  ...
  public IndexSegmentStore(final File file, final IEmbergraphFederation<?> fed) {
    ...
    try {
      ...
      this.segmentMetadata =
          new SegmentMetadata(file, checkpoint.segmentUUID, checkpoint.commitTime);
  ...
  protected void finalize() throws Exception {
    ...
    if (open) {
      ...
      if (log.isInfoEnabled()) log.info("Closing IndexSegmentStore: " + getFile());
   ...
   *
   * @throws RootBlockException if the root block is invalid.
   * @throws RuntimeException if there is a problem, including a {@link FileNotFoundException}.
   * @see #close()
   */
  public void reopen() {
        ...
        if (fed != null) {
          ...
          openCloseEvent =
              new Event(
                      ...
                      EventType.IndexSegmentStoreOpenClose)
                  .start();
        }
   ...
   * </code>
   *
   * <p>Note: Normally access to {@link IndexSegment}s is mediated by the {@link StoreManager} which
   * imposes a canonicalizing weak value cache to ensure that we do not double-open an {@link
   * IndexSegment}.
   *
   ...
   * IndexSegmentStore} will transparently {@link #reopen()} the store as long as the backing file
   * remains available. {@link #destroy()} provides an atomic "close and delete" operation.
   */
  public void close() {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info(file.toString());
  ...
  private void _close() {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("Closed: file=" + getFile());
   ...
   *
   * <p>Note: An LRU disk cache is a poor choice for the leaves. Since the btree already maintains a
   * cache of the recently touched leaf objects, a recent read against the disk is the best
   * indication that we have that we will NOT want to read that region again soon.
   */
  public ByteBuffer read(final long addr) {
    ...
    if (log.isDebugEnabled()) {
      ...
      log.debug(
          "addr=" + addr + "(" + addressManager.toString(addr) + "), isNodeAddr=" + isNodeAddr);
    }
   ...
   *
   * <p>Note: The caller MUST be synchronized such that {@link #buf_nodes} is in a known state
   * (either allocated or released).
   *
   * @param offset
   * @param length
   * @return
   ...
   *
   * @param offset The offset of the first byte to be read.
   * @param dst The buffer into which the data will be read. Bytes will be read into the buffer
   *     starting at the current position and up to the limit.
   * @throws IOException
   */
  final void readFromFile(final long offset, final ByteBuffer dst) throws IOException {
   ...
   * remains valid and we only need to re-open the {@link FileChannel} to the backing store and
   * retry. In particular, we do not need to re-read the root node, {@link IndexMetadata}, {@link
   * BloomFilter}, etc. All we have to do is re-open the {@link FileChannel}.
   *
   * <p>Note: This method is internally synchronized so that concurrent readers do not try to all
   * open the store at the same time. Further, this is the only method other than {@link #_close()}
   * that can set {@link #raf}. Since both this method and {@link #_close()} are synchronized the
   * state of that field is well known inside of this method.
   *
   ...
   * FileLock} and was in fact granted an exclusive lock when they requested a shared lock then this
   * code will be unwilling to send the resource. There are two ways to make that work out - either
   * we DO NOT use {@link FileLock} for read-only files (index segments) or we ALWAYS discard the
   * {@link FileLock} if it is not shared when we requested a shared lock and proceed without a
   * lock. For this reason, the behavior of this class and {@link ResourceService} MUST match.
   *
   * @see ResourceService
  ...
  private final FileChannel reopenChannel() throws IOException {
    ...
    try {
      ...
      try {
        ...
        if (log.isInfoEnabled()) log.info("FileLock not supported: file=" + getFile(), ex);
      }
  ...
  protected void bufferIndexNodes() throws IOException {
    ...
    if (checkpoint.extentNodes > DirectBufferPool.INSTANCE.getBufferCapacity()) {
      ...
      log.warn(
          "Node extent exceeds buffer capacity: index="
    ...
    try {
      ...
      buf_nodes = DirectBufferPool.INSTANCE.acquire(100 /* ms */, TimeUnit.MILLISECONDS);

      if (log.isInfoEnabled())
        log.info(
            "Buffering nodes: #nodes="
                + checkpoint.nnodes
                + ", #bytes="
                + checkpoint.extentNodes
                + ", file="
      ...
      FileChannelUtility.readAll(opener, tmp, checkpoint.offsetNodes);
  ...
  protected BloomFilter readBloomFilter() throws IOException {
    ...
    if (log.isInfoEnabled()) log.info("reading bloom filter: " + addressManager.toString(addr));
    ...
    if (log.isInfoEnabled()) log.info("Read bloom filter: bytesOnDisk=" + len);
  ...
  private final IndexMetadata readMetadata() throws IOException {
    ...
    if (log.isInfoEnabled()) log.info("reading metadata: " + addressManager.toString(addr));
    ...
    final IndexMetadata md = (IndexMetadata) SerializerUtil.deserialize(buf);

    if (log.isInfoEnabled()) log.info("Read metadata: " + md);
