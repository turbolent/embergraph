 ...
 * force).
 *
 * <p>Note that {@link MetadataService} extends {@link DataService} so this task can also be used to
 * enumerate the scale-out indices in an {@link IEmbergraphFederation}. However, when enumerating
 * metadata index names, note that they all use a prefix to place them into their own namespace.
 *
...
public class ListIndicesTask extends DataServiceCallable<String[]> {
   ...
   *     <p>Note: This SHOULD include a "." if you want to restrict the scan to only those indices
   *     in a given namespace. Otherwise you can find indices in <code>kb2</code> if you provide the
   *     prefix <code>kb</code> and both kb and kb2 are namespaces since the indices spanned by
   *     <code>kb</code> would include both <code>kb.xyz</code> and <code>kb2.xyx</code>.
   */
  ...
  public String[] call() {
    ...
    final AbstractJournal journal = getDataService().getResourceManager().getJournal(ts);
