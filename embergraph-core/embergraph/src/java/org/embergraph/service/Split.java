...
public class Split {
   ...
   *
   * @param pmd The metadata for the index partition within which the keys in this split lie
   *     (optional and used only in scale-out).
   * @param fromIndex The index of the first key that will enter that index partition (inclusive
   *     lower bound).
   * @param toIndex The index of the first key that will NOT enter that index partition (exclusive
   *     upper bound).
   */
  public Split(final IPartitionMetadata pmd, final int fromIndex, final int toIndex) {
    ...
    if (fromIndex < 0) throw new IllegalArgumentException("fromIndex=" + fromIndex);
   ...
   *
   * <p>Note: The historical hash code was just based on the {@link IPartitionMetadata} and would
   * thrown out an NPE if there was no {@link IPartitionMetadata}. This was changed as part of
   * BLZG-1537 to support cases where the {@link IPartitionMetadata} was not given (for index local
   * parallelism).
   *
  ...
  public int hashCode() {
    ...
    if (pmd != null) return pmd.hashCode();
  ...
  public boolean equals(final Object o) {
    ...
    if (o instanceof Split) return equals((Split) o);
