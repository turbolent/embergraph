...
public class CachingMetadataIndex extends CacheOnceMetadataIndex {
   ...
   * cache and since the B+Tree is NOT thread-safe if there is a writer, then we must synchronized
   * access to that B+Tree for the readers as well as the writer. We do this using a {@link
   * ReadWriteLock}, which permits higher concurrency for readers. This should work out well as
   * reads should far outway writes (stale locators are relatively rare).
   */
   ...
   * @return The cached partition metadata -or- <code>null</code> iff there is no such scale-out
   *     index.
   */
  ...
  public void staleLocator(final PartitionLocator locator) {
    ...
    if (log.isInfoEnabled()) log.info(locator.toString());
