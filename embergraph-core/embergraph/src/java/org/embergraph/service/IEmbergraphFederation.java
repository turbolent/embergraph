 ...
 */
public interface IEmbergraphFederation<T>
    extends IIndexManager, IFederationDelegate<T>, ICounterSetAccess {
   ...
   *
   * <p>Note: Applications MAY add their own counters (within a suitable namespace) to the returned
   * {@link CounterSet} in order to report their own performance data to the {@link
   * ILoadBalancerService}.
   *
   ...
   *
   * @param maxCount The maximum #of data services whose UUIDs will be returned. When zero (0) the
   ...
   * {@link IDataService} {@link UUID}s.
   *
   * <p>Note: This method will also resolve the {@link UUID} of an {@link IMetadataService}.
   *
   ...
   *
   * <p>Note: Whether the returned object is a proxy or the service implementation depends on
   * whether the federation is embedded (in process) or distributed (networked).
   *
   * @param serviceUUID The identifier for a {@link IDataService}.
   ...
   *
   * <p>Note: This method is here as a failsafe when the {@link ILoadBalancerService} is not
   * available.
   *
   ...
   * specified service name. Services that are not joined will not be discovered.
   *
   * <p>Note: At least some service fabrics (such as jini) do not enforce a uniqueness constraint on
   * the service name(s). In such cases an arbitrary {@link IDataService} method the other
   * requirements will be returned. It is the responsibility of the administrator to ensure that
   * each {@link IDataService} is assigned a distinct service name.
   *
   * @param name The service name.
   * @return A service assigned that name -or- <code>null</code> if none is joined with the {@link
   ...
   *
   * @param metadata The metadata template used to create component indices for {@link BTree}s this
   *     scale-out index (this also specifies the name of the scale-out index).
   ...
   * @param metadata The metadata template used to create component indices for {@link BTree}s this
   *     scale-out index (this also specifies the name of the scale-out index).
   * @param dataServiceUUID The data service identifier (optional). When <code>null</code>, a data
   *     service will be selected automatically. If {@link
   *     IndexMetadata.Options#INITIAL_DATA_SERVICE} was specified, then the identified service will
   *     be used. Otherwise an underutilized service will be selected using the {@link
   *     ILoadBalancerService}.
   ...
   * partitioned indices). Note that a <code>true</code> return does NOT imply that the federation
   * is running in a distributed environment, just that it uses the scale-out index architecture. A
   * <code>false</code> return indicates that {@link #getMetadataService()} WILL NOT return a {@link
   * IMetadataService} since key-range partitioned indices are NOT supported.
   *
   * @see IndexMetadata
   */
  boolean isScaleOut();
   ...
   * federation deployments are stable in this sense, but it is possible to create federation
   * instances backed solely by transient storage and those instances will report <code>false</code>
...
//     * Return the global {@link SparseRowStore} used to store named property
   ...
   * across all {@link IDataService}s).
   *
   * <p>This is useful for {@link ITx#READ_COMMITTED} operations that need to use a consistent
   ...
   * to factor our embergraph-jini dependency.
   *
   * <p>This should return true IFF the instance is backed by a
   * org.embergraph.service.jini.JiniFederation.
   *
   * @return
   */
