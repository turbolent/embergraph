 ...
 * @version $Id$
 * @todo register index and drop index could be rewritten as submitted procedures derived from this
 *     class. This would simplify the {@link IDataService} API and metrics collection further. The
 *     implementations would have to be distinct from {@link RegisterIndexTask} and {@link
 *     DropIndexTask} since those extend {@link AbstractTask} - that class does not implement {@link
 *     IIndexProcedure} and can not be sent across the wire.
...
public interface IDataServiceCallable extends IFederationCallable {
   ...
   * @throws IllegalStateException if {@link #setDataService(DataService)} has already been invoked
   *     and was set with a different value.
   */
   ...
   * @throws IllegalStateException if {@link #setDataService(DataService)} has not been invoked.
   */
