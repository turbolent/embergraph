...
public abstract class AbstractResourceScanner<V> implements Callable<Long> {
  ...
  /** Lock used to serialize the decision to transfer a chunk from the queue to the buffer. */
  ...
  /** @param buffer The buffer to which the resources should be added. */
  ...
  public void accept(final V resource) throws InterruptedException {
    ...
    if (log.isDebugEnabled()) log.debug("accept: " + resource);
  ...
  private void transferChunk() {
    ...
    final V[] a = (V[]) java.lang.reflect.Array.newInstance(c.getFirst().getClass(), chunkSize);
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info(
          "chunkSize=" + chunkSize + ", naccepted=" + acceptCount + ", chunkCount=" + chunkCount);
    }
  ...
  private void flushQueue() {
    ...
    if (log.isInfoEnabled()) log.info("Flushing queue to buffer.");
