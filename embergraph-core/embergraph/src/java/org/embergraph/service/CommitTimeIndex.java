 ...
 *     replicated right now in several places and there is no reason for that. Allow 0L for {@link
 *     #find(long)}, but check all callers first to see who might use that for error checking and
 *     then modify callers using 1L to use 0L. In fact, {@link #find(long)} should probably accept
...
public class CommitTimeIndex extends BTree {
   ...
   * @version $Id$
   */
  protected static class TupleSerializer extends DefaultTupleSerializer<Long, Void> {
