...
public abstract class AbstractIndexCache<T extends IRangeQuery> {
   ...
   *
   * <p>Note: The "dirty" flag associated with the object in this cache is ignored.
   */
   ...
   * @param name
   * @param timestamp
   * @return The index view -or- <code>null</code> if the described index does not exist.
   */
   ...
   * the indirection imposed by the {@link IClientIndex}, which converts a request by the client
   * into a request against the appropriate {@link IDataService}s via task(s) submitted against
   * those services.
   *
      ...
      if (ndx == null) {
        ...
        if ((ndx = newView(name, timestamp)) == null) {
          ...
          if (log.isInfoEnabled())
            log.info("name=" + name + " @ " + timestamp + " : no such index.");
        ...
        if (log.isInfoEnabled()) log.info("name=" + name + " @ " + timestamp + " : index exists.");

      } else {

        if (log.isInfoEnabled()) log.info("name=" + name + " @ " + timestamp + " : cache hit.");
      }
   ...
   * the cache.
   *
   * <p>Historical and transactional reads are still allowed, but we remove the read-committed or
   * unisolated views from the cache once the index has been dropped. If a client wants them, it
   * needs to re-request. If they have been re-registered on the metadata service then they will
   * become available again.
   *
   * <p>Note: Operations against unisolated or read-committed indices will throw exceptions if they
   * execute after the index was dropped.
   */
      ...
      while (itr.hasNext()) {
        ...
        if (name.equals(nt.getName())) {
          ...
          if (timestamp == ITx.UNISOLATED || timestamp == ITx.READ_COMMITTED) {
            ...
            if (log.isInfoEnabled()) log.info("dropped from cache: " + name + " @ " + timestamp);
