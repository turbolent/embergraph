 ...
 * @version $Id$
 * @todo failover. the service instances will need to track active/committed transactions, complain
 *     if their clocks get out of alignment, and refuse to generate a timestamp that would go
 *     backwards when compared to the timestamp generated by the last master service.
 */
public abstract class AbstractTransactionService extends AbstractService
  ...
  public interface Options {
     ...
     * transaction exists, the actual release time is:
     *
     * <pre>
     * releaseTime = min(lastCommitTime - 1, min(earliestRunningTx, now - minimumReleaseAge))
     * </pre>
     *
     * This ensures that history in use by running transactions is not released even when the
     * minimumReleaseAge is ZERO (0).
     *
     * <p>When no transactions exist the actual release time is:
     *
     * <pre>
     * releaseTime = min(commitTime - 1, now - minimumReleaseAge)
     * </pre>
     *
     * This ensures that the the release time advances when no transactions are in use, but that the
     * minimum release age is still respected.
     *
    ...
    String MIN_RELEASE_AGE_1W =
        "" + 7 /* d */ * 24 /* hr */ * 60 /* mn */ * 60 /* sec */ * 1000 /* ms */;
     ...
     *
     * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/638" >Change
     *     DEFAULT_MIN_RELEASE_AGE to 1ms</a>
     */
   ...
   * APIs for the transaction service.
   *
   * @param tx The transaction identifier (the signed value, NOT the absolute value).
   * @return The {@link TxState} -or- <code>null</code> if there is no such active transaction.
   */
...

    {
      this.minReleaseAge =
          LongValidator.GTE_ZERO.parse(
              ...
              properties.getProperty(Options.MIN_RELEASE_AGE, Options.DEFAULT_MIN_RELEASE_AGE));

      if (log.isInfoEnabled()) log.info(Options.MIN_RELEASE_AGE + "=" + minReleaseAge);
    }
   ...
   * @throws IllegalStateException if the requested state is not a legal state change.
   */
  protected synchronized void setRunState(final TxServiceRunState newval) {

    if (!lock.isHeldByCurrentThread()) throw new IllegalMonitorStateException();

    if (!runState.isTransitionLegal(newval)) {

      throw new IllegalStateException("runState=" + runState + ", but newval=" + newval);
    }
   ...
   */
  private void awaitRunningTx(long logTimeout, final TimeUnit unit) throws InterruptedException {
    ...
    if (log.isInfoEnabled()) log.info("activeCount=" + getActiveCount());

    while (getActiveCount() > 0) {

      // wait for a transaction to complete.
      if (txDeactivate.await(logTimeout, TimeUnit.NANOSECONDS) && getActiveCount() == 0) {
        ...
        if (log.isInfoEnabled()) log.info("No transactions remaining: elapsed=" + elapsed);
   ...
   * @param unit The unit in which that time is measured.
   */
   ...
   * are concurrently committing MAY fail (throwing exceptions from various methods, including
   * {@link #nextTimestamp()}) when the service halts.
   */
  @Override
  ...
  public void abortAllTx() {
    ...
    try {
      ...
      if (activeCount != 0) {
        ...
        log.warn("Service shutdown with active transactions: #nactive=" + activeTx.size());
      }
    } finally {
   ...
   * millisecond (the resolution of {@link #nextTimestamp()}) and requests for new read-write
   * transactions contend with other requests for {@link #nextTimestamp()}.
   *
   * <p>Note: The transaction service will refuse to start new transactions whose timestamps are LTE
   * to {@link #getReleaseTime()}.
   *
   * @throws RuntimeException Wrapping {@link TimeoutException} if a timeout occurs awaiting a start
   *     time which would satisfy the request for a read-only transaction (this can occur only for
   *     read-only transactions which must contend for start times which will read from the
   *     appropriate historical commit point).
   */
  @Override
   ...
   *
   * <p>Note: To avoid lock ordering problems DO NOT acquire this {@link #lock} if you are already
   * holding a {@link TxState#lock}. This causes a lock ordering problem and can result in deadlock.
   */
...
//     * The minimum over the absolute values of the active transactions.
   ...
   * active tx.
   *
   * <p>Note: The {@link #lock} is required in order to make atomic decisions about the earliest
   * active tx. Without the {@link #lock}, the tx could stop or a new tx could start, thereby
   * invalidating the "earliest active" guarantee.
   *
   ...
   * </code> to provide visibility to {@link #getCounters()} without taking the lock.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/467" > IllegalStateException
   *     trying to access lexicon index using RWStore with recycling </a>
   */
  ...
  public long getReleaseTime() {
    ...
    if (log.isTraceEnabled())
      log.trace("releaseTime=" + releaseTime + ", lastKnownCommitTime=" + getLastCommitTime());
   ...
   * consensus protocol. Otherwise it is automatically maintained by {@link #updateReleaseTime(long,
   * TxState)} and {@link #updateReleaseTimeForBareCommit(long)}.
   *
   * @param newValue The new value.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/671">Query on follower fails
   *     during UPDATE on leader </a>
   */
   ...
   * value would be negative, then ZERO (0L) is reported instead.
   *
   * <p>Note: This duplicates logic in {@link #updateReleaseTime(long)}, but handles the special
   * case in HA where the releaseTime is not being updated by {@link
   * #updateReleaseTimeForBareCommit(long)}.
   *
   * @return The effective release time.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/623">HA TXS </a>
  ...
  protected long getEffectiveReleaseTimeForHA() {
    ...
    try {
       ...
       * @see https://sourceforge.net/apps/trac/bigdata/ticket/467
       */
      final long effectiveReleaseTimeForHA =
          Math.min(
              lastCommitTime - 1, Math.min(earliestTxReadsOnCommitTime - 1, now - minReleaseAge));
  ...
  protected void activateTx(final TxState state) {
    ...
    try {
      ...
      if (!state.isActive()) throw new IllegalArgumentException();
   ...
   * @throws IllegalArgumentException if there is no such transaction.
   */
   ...
   *
   * <p>Note: Normally this method is invoked without the outer {@link #lock} which necessitates
   * lifting those method calls out of this method and into the caller.
   *
   * @param state The transaction.
   * @throws IllegalMonitorStateException unless the caller is holding the {@link TxState#lock}.
   */
  protected void deactivateTx(final TxState state) {
    ...
    if (!state.isComplete()) throw new IllegalArgumentException();
   ...
   *
   * <p>Note: When we are using a 2-phase commit, the leader can not update the release time from
   * commit() using this methods. It must rely on the consensus protocol to update the release time
   * instead.
   *
   ...
   *
   * <p>If the specified timestamp corresponds to the earliest running transaction, then the <code>
   * releaseTime</code> will be updated and the new releaseTime will be set using {@link
   * #setReleaseTime(long)}. For HA, the releaseTime is updated by a consensus protocol and the
   * individual services MUST NOT advance their releaseTime as transactions complete.
   *
   * <p>Note: When we are using a 2-phase commit, the leader can not update the release time from
   * commit() using this methods. It must rely on the consensus protocol to update the release time
   * instead.
   *
   * @param timestamp The absolute value of a transaction identifier that has just been deactivated.
   * @param deactivatedTx The transaction object that has been deactivated -or- <code>null</code> if
   *     there are known to be no active transactions remaining (e.g., startup and abortAll()).
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/467" > IllegalStateException
   *     trying to access lexicon index using RWStore with recycling </a>
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/671">Query on follower fails
   *     during UPDATE on leader </a>
   * @todo the {@link #startTimeIndex} could be used by {@link #findUnusedTimestamp(long, long)} so
   *     that it could further constrain its search within the half-open interval.
   */
  protected final void updateReleaseTime(final long timestamp, final TxState deactivatedTx) {
    ...
    synchronized (startTimeIndex) {
      ...
      if (indexOf != -1) startTimeIndex.remove(timestamp);
      ...
      if (log.isTraceEnabled()) log.trace("earliestActiveTx=" + earliestActiveTx);
    } // synchronized(startTimeIndex)
    ...
    if (isEarliestTx && !isReleaseTimeConsensusProtocol()) {
      ...
      final long earliestTxStartTime = earliestActiveTx == null ? now : earliestActiveTx.tx;

      final long earliestTxReadsOnCommitTime =
          earliestActiveTx == null ? now : earliestActiveTx.readsOnCommitTime;
   ...
   *
   * <p>Note: This needs to be a fairly low-latency operation since this method is invoked for all
   * commits on all data services and will otherwise be a global hotspot.
   */
  @Override
   ...
   *
   * <p>Note: This method was historically part of {@link #notifyCommit(long)}. It was moved into
   * its own method so it can be overridden for some unit tests.
   *
   * <p>Note: When we are using a 2-phase commit, the leader can not update the release time from
   * commit() using this methods. It must rely on the consensus protocol to update the release time
   * instead.
    ...
    try {
      ...
      synchronized (startTimeIndex) {
        if (!isReleaseTimeConsensusProtocol()
          ...
          final long releaseTime = Math.min(lastCommitTime - 1, now - minReleaseAge);

          if (this.releaseTime < releaseTime) {

            if (log.isInfoEnabled())
              log.info(
                  "Advancing releaseTime (no active tx)"
                      + ": lastCommitTime="
                      + lastCommitTime
                      + ", minReleaseAge="
                      + minReleaseAge
   ...
   *     <p>If minReleaseTime is increased, then the release time can be changed to match, but only
   *     by NOT advancing it until we are retaining enough history.
   *     <p>If minReleaseTime is decreased, then we can immediately release more history (or at
   *     least as soon as the task runs to notify the discovered data services of the new release
   *     time).
   */
   ...
   * reading.
   *
   * <p>Note: The absolute value constraint is imposed so that we can directly identify the earliest
   * active transaction in the index by its position (it will be at position zero). This would not
   * work if we let in negative start times.
   *
   * <p>Note: In order to support this, {@link #findUnusedTimestamp(long, long)} will not return a
   * timestamp whose absolute value corresponds to an active transaction.
   */
   ...
   * @throws InterruptedException if interrupted while awaiting a start time which would satisfy the
   *     request.
   * @throws InterruptedException if a timeout occurs while awaiting a start time which would
   *     satisfy the request.
   */
  protected final TxState assignTransactionIdentifier(final long timestamp)
   ...
   *
   * <p>Note: Under some circumstances the assignment of a read-only transaction identifier must be
   * delayed until a distinct timestamp becomes available between the designed start time and the
   * next commit point.
   *
   * @param timestamp The timestamp (identifies the desired commit point).
  ...
  private final TxState getStartTime(final long timestamp)
    ...
    final long txId =
        findUnusedTimestamp(commitTime, nextCommitTime, 1000 /* timeout */, TimeUnit.MILLISECONDS);
   ...
   *
   * @param commitTime The probe.
   * @return The successor or -1L iff the is no successor for that commit time.
   */
   ...
   * all such timestamps will read from the commit point associated with [commitTime].
   *
   * @param commitTime The commit time for the commit point on which the tx will read (this must be
   *     the exact timestamp associated with the desired commit point).
   * @param nextCommitTime The commit time for the successor of that commit point.
   * @param timeout The maximum length of time to await an available timestamp.
   * @param unit The unit in which <i>timeout</i> is expressed.
   */
  protected long findUnusedTimestamp(
      final long commitTime, final long nextCommitTime, final long timeout, final TimeUnit unit)
      throws InterruptedException, TimeoutException {
   ...
   *
   * <ol>
   *   <li>The transaction is {@link RunState#Active}; and
   *   <li>The caller holds the {@link TxState#lock}.
   * </ol>
   *
   * <p>Post-conditions:
   *
   * <ol>
   *   <li>The transaction is {@link RunState#Aborted}; and
   *   <li>The transaction write set has been discarded by each {@link Journal} or {@link
   *       IDataService} or which it has written (applicable for read-write transactions only).
   * </ol>
   *
   * <p>
   *
   * @param state The transaction state as maintained by the transaction server.
   */
   ...
   * </ol>
   *
   * <p>Post-conditions (success for read-only transaction or a read-write transaction with an empty
   * write set):
   *
   * <ol>
   *   <li>The transaction is {@link RunState#Committed}; and
   *   <li>The returned <i>commitTime</i> is ZERO (0L).
   * </ol>
   *
   * <p>Post-conditions (success for read-write transaction with a non-empty write set):
   *
   * <ol>
   *   <li>The transaction is {@link RunState#Committed};
   *   <li>The transaction write set has been made restart-safe by each {@link Journal} or {@link
   *       IDataService} or which it has written (applicable for read-write transactions only); and
   *   <li>The application can read exactly the data written by the transaction from the commit
   *       point identified by the returned <i>commitTime</i>.
   * </ol>
   *
   ...
   *     had an empty write set.
   * @throws Exception if something else goes wrong. This will be (or will wrap) a {@link
   *     ValidationError} if validation fails.
   */
  protected abstract long commitImpl(final TxState state) throws Exception;
  ...
  public void abort(final long tx) {
    ...
    try {
      ...
      if (state == null) throw new IllegalStateException(ERR_NO_SUCH);
   ...
   * single phase commits, which means that this class could only know their values for a
   * distributed transaction commit. Hence they are not represented here.
   */
  public class TxState implements ITxState {
     ...
     * visible using a volatile write for the methods on the {@link ITxState} interface (isActive(),
     * etc).
     */
     ...
     *
     * <p>Note: This field is guarded by the {@link #lock}.
     */
     ...
     * @throws IllegalStateException if the commit time has not been assigned.
     */
     ...
     *
     * <p>Note: We only track this information for a distributed database.
     */
    ...
    public String[] getResources() {
      ...
      if (resources == null) return EMPTY;
     ...
     *
     * @param dataServiceUUID The {@link UUID} identifying an {@link IDataService}.
     * @return <code>true</code> if this transaction has been started on that {@link IDataService}.
     *     <code>false</code> for read-only transactions.
     */
    public boolean isStartedOn(final UUID dataServiceUUID) {
      ...
      if (dataServices == null) return false;
    ...
    protected UUID[] getDataServiceUUIDs() {
      ...
      if (dataServices == null) throw new IllegalStateException();
     ...
     * RunState}.
     *
     * <p>Note: DO NOT attempt to acquire the outer {@link AbstractTransactionService#lock} if you
     * are already holding this {@link #lock}. This is a lock ordering problem and can result in a
     * deadlock.
     */
    protected final ReentrantLock lock = new ReentrantLock();
    ...
    protected TxState(final long tx, final long readCommitTime) {
      ...
      if (tx == ITx.UNISOLATED) throw new IllegalArgumentException();

      if (tx == ITx.READ_COMMITTED) throw new IllegalArgumentException();

      if (readCommitTime < 0) throw new IllegalArgumentException();
    ...
    public final boolean equals(final Object o) {
      ...
      if (!(o instanceof ITx)) return false;
     ...
     * @param resource An array of named resources on the data service on which the transaction will
     *     write (or at least for which it requires an exclusive write lock).
     * @throws IllegalStateException if the transaction is read-only.
     * @throws IllegalStateException if the transaction is not active.
     */
    public final void declareResources(final UUID dataService, final String[] resource) {
      ...
      if (readOnly) throw new IllegalStateException(ERR_READ_ONLY);

      if (!isActive()) throw new IllegalStateException(ERR_NOT_ACTIVE);
...
//         * Return <code>true</code> if the transaction is read-only or if a
     ...
     * @throws IllegalStateException if the transaction is read-only.
     * @throws IllegalMonitorStateException if the caller does not hold the lock.
     */
  ...
  public CounterSet getCounters() {
     ...
     */
    countersRoot.addCounter(
        "earliestReadsOnCommitTime",
        new Instrument<Long>() {
          @Override
          protected void sample() {
            final TxState tmp = earliestOpenTx;
            if (tmp != null) setValue(tmp.readsOnCommitTime);
          }
        });
