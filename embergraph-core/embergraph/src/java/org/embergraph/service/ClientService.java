...
public abstract class ClientService extends AbstractService implements IClientService, ISession {
   ...
   *     <p>Note that we have excellent locators for the best data service when the map/reduce input
   *     is the scale-out repository since the task should run on the data service that hosts the
   *     file block(s). When failover is supported, the task can run on the service instance with
   *     the least load. When the input is a networked file system, then additional network topology
   *     smarts would be required to make good choices.
