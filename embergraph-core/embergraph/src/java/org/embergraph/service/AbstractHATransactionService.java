 ...
 */
public abstract class AbstractHATransactionService extends AbstractTransactionService
    implements HATXSGlue {
   ...
   *     used to RMI back to the leader and therefore MUST be non- <code>null</code>.
   * @param serviceId The {@link UUID} of this service. This is required as part of the RMI back to
   *     the leader (so the leader knows which services responded) and therefore MUST be non-<code>
   *     null</code>.
   ...
   * quorum.
   *
   * @param newCommitCounter The commit counter that will be assigned to the new commit point.
   * @param newCommitTime The commit time that will be assigned to the new commit point.
   * @param joinedServiceIds The services that are joined with the met quorum as of an atomic
   *     decision point in {@link AbstractJournal#commitNow()}.
   * @param timeout The timeout for the release time consensus protocol.
   * @param units The units for that timeout.
   */
  public abstract IHANotifyReleaseTimeResponse updateReleaseTimeConsensus(
   ...
   * quorum to set the consensus release time of the quorum leader of the newly joined follower.
   * This prevents the follower from permitting new transaction starts against a commit point which
   * has been recycled by the quorum leader.
   */
  @Override
