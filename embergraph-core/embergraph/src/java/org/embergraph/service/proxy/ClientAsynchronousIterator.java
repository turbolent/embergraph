 ...
 * from the remote {@link IAsynchronousIterator} into a local {@link BlockingBuffer}. The client
 * implementation uses purely local operations to test the contents of the local {@link
 * BlockingBuffer}. This means that operations like {@link #hasNext(long, TimeUnit)} may be used
 * with short timeouts without possibility of blocking IO.
 *
 * <p>Note: This class is NOT thread-safe. There should be only a single process draining the
 * iterator.
 *
 * <p>Note: In order to conserve resources, the caller is advised to use {@link
 * #start(ExecutorService)} before making any other requests. This will allow the caller to specify
 * an {@link ExecutorService} that will be tasked with buffering elements from the {@link
 * RemoteAsynchronousIterator}. When {@link #start(ExecutorService)} is not invoked explicitly, a
 * new {@link Thread} will be allocated. Typically, an {@link ExecutorService} can be much more
 * efficient than creating a new {@link Thread}.
 *
 * @todo Note that we only use three methods to implement {@link ClientAsynchronousIterator} -
 *     hasNext(timeout,unit), isExhausted(), and next(). The rest of the {@link
 *     RemoteAsynchronousIterator} API does not need to be implemented.
...
public class ClientAsynchronousIterator<E> implements IAsynchronousIterator<E>, Serializable {
   ...
   * both declare and can throw {@link IOException}.
   */
  ...
  public ClientAsynchronousIterator(
      final RemoteAsynchronousIterator<E> remoteIterator, final int capacity) {

    if (remoteIterator == null) throw new IllegalArgumentException();
   ...
   * iterator.
   *
   * <p>When {@link #start(ExecutorService)} is not invoked explicitly, a new {@link Thread} will be
   * allocated and the {@link ReaderTask} will be run on that {@link Thread}. Typically, an {@link
   * ExecutorService} can be much more efficient than creating a new {@link Thread}. Therefore the
   * caller is encourged to break encapsulation and specify the {@link ExecutorService} on which the
   * {@link ReaderTask} will run directly.
   *
   * @throws IllegalStateException if the {@link ReaderTask} is already running.
   */
   ...
   * ReaderTask}.
   *
   * <p>Note: The whole reason for {@link #start(ExecutorService)} is that the {@link
   * ExecutorService} is NOT serializable. Typically this task will be run on the {@link
   * ExecutorService} associated with the local {@link IEmbergraphFederation} object for a {@link
   * DataService}.
   */
   ...
   *
   * <p>Note: A timeout is used instead of {@link RemoteAsynchronousIterator#hasNext()} in order to
   * avoid blocking during an RMI call on a remote thread.
   */
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        while (true) {
          ...
          if (trace) System.err.println("Polling remote iterator: ");
          ...
          if (!remoteIterator.hasNext(timeout, unit)) {
            ...
            if (remoteIterator.isExhausted()) {
              ...
              if (trace) System.err.println("Remote iterator is exhausted.");
          ...
          try {
            ...
            if (INFO) log.info(ex.getLocalizedMessage());
        ...
        if (INFO) log.info("Reader is done: interrupted" + interrupted);
