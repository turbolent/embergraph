...
public class ClientFuture<T> implements Future<T>, Serializable {
  ...
  /** @param proxy A proxy for the {@link RemoteFuture}. */
   ...
   * </pre>
   *
   * This appears to be an RMI/DGC bug where the remote future is somehow garbage collected even
   * though this class is holding a reference to its {@link RemoteFuture} proxy.
   *
   * <p>Since the remote future no longer exists we can assume that it is no longer running.
   * Therefore, I have modified this method to log a warning and return <code>false</code> when this
   * exception is thrown.
   *
   * <p>Note: I have also seen this problem where the stack trace involves <code>
   * java.rmi.ConnectException</code>.
   *
