 ...
 * @version $Id$
 * @todo compact event serialization when reporting to the {@link ILoadBalancerService}, including
 *     factoring out of the common metadata (some stuff will always be the same for a given reported
 *     and does not need to be reported with each event).
 * @todo Add a level property to events? (but events are not meant to replace logging)
 */
public class Event implements Serializable {
   ...
   * @param resource The resource for which the event was generated (store, index partition, etc).
   * @param majorEventType The major type of the event (use of enums is encouraged).
   * @param details Optional details for the event.
   */
  public Event(
   ...
   * @param resource The resource for which the event was generated (store, index partition, etc).
   * @param majorEventType The major type of the event (use of enums is encouraged).
   * @param minorEventType The minor type of the event (use of enums is encouraged).
   * @param details Optional details for the event.
   * @todo consider passing along the {@link UUID} of the parent event but then must correlate that
   *     {@link UUID} when the event is received.
   */
  protected Event(
      final IEmbergraphFederation fed,
      final EventResource resource,
    ...
    if (majorEventType == null) throw new IllegalArgumentException();

    if (minorEventType == null) throw new IllegalArgumentException();
   ...
   *
   * @param minorEventType
   * @param details Optional ordered (name,value) array.
   * @return The sub-event.
   */
  public Event newSubEvent(final Object minorEventType, final Map<String, Object> details) {
   ...
   * the event starts and {@link #end()} when the event ends. If you want to report an
   * "instantaneous" event then you can just use {@link #end()}.
   */
  ...
  /** Dispatch the {@link Event} via {@link AbstractFederation#sendEvent(Event)}. */
  ...
  /** A header that can be used to interpret the output of {@link #toString()} (with newline). */
  ...
  public String toString() {
    ...
    sb.append(eventUUID);
    sb.append('\t');
    sb.append(resource.indexName);
    sb.append('\t');
    sb.append(resource.partitionId);
    sb.append('\t');
    sb.append(resource.file);
    sb.append('\t');
    sb.append(majorEventType.getClass().getName());
    sb.append('\t');
    sb.append(majorEventType);
    sb.append('\t');
    sb.append(minorEventType.getClass().getName());
    sb.append('\t');
    sb.append(minorEventType);
    sb.append('\t');
    sb.append(startTime);
    sb.append('\t');
    sb.append(endTime);
    ...
    sb.append(hostname);
    sb.append('\t');
    sb.append(serviceIface.getName());
    sb.append('\t');
  ...
  protected Event(final String s) throws ClassNotFoundException {
    ...
    try {
      ...
      throw new RuntimeException(
          "At field: " + st.getCurrentFieldIndex() + " of " + st.getFieldCount() + " : " + t, t);
    }
  }
   ...
   * @throws ClassNotFoundException if any CSV fields specify an invalid classname
   */
