 ...
 *
 * <p>Note: This has internal state in order to provide a round-robin policy. Therefore the load
 * balancer MUST use a single instance of this class for stateful behavior to be observed.
 *
...
public abstract class AbstractRoundRobinServiceLoadHelper implements IServiceLoadHelper {
   ...
   *     Note that at least <i>minDataServices</i> elements will be present in this array but that
   *     ALL discovered data services may be reported.
   ...
   * we do not bother to adjust the counter. We always return the {@link UUID} of the service at
   * index MOD N, where N is the #of services that are joined at the time that this method looks at
   * the set. We then post-increment the counter.
   *
   * <p>The round-robin allocate strategy is a good choice where there is little real data on the
   * services, or when there is a set of services whose scores place them into an equivalence class
   * such that we have no principled reason for preferring one service over another among those in
   * the equivalence class.
   *
   * @throws TimeoutException
   * @throws InterruptedException
   * @todo develop the concept of equivalence classes further. divide the joined services into a set
   *     of equivalence classes. parameterize this method to accept an equivalence class. always
   *     apply this method when drawing from an equivalence class.
  ...
  public UUID[] getUnderUtilizedDataServices(
    ...
    if (INFO) log.info("minCount=" + minCount + ", maxCount=" + maxCount + ", exclude=" + exclude);
