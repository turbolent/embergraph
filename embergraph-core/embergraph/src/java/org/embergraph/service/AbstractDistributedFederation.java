 ...
 *     strategy is currently used. However, caching may be necessary for some kinds of application
 *     profiles, especially as the #of index partitions grows. If an application performs only
 *     unisolated and read-committed operations, then a single metadata index cache can be shared by
 *     the client for all operations against a given scale-out index. On the other hand, a client
 *     that uses transactions or performs historical reads will need to have a view of the metadata
 *     index as of the timestamp associated with the transaction or historical read.
...
public abstract class AbstractDistributedFederation<T> extends AbstractScaleOutFederation<T> {
   ...
   * environment.
   *
   * @param src The source iterator. Note that the iterator normally visits elements of some array
   *     type (chunks).
   * @param serializer The object responsible for (de-)serializing a chunk of elements visited by
   *     the iterator.
   * @param capacity The capacity for the internal buffer that is used to asynchronously transfer
   *     elements (chunks) from the remote iterator to the client iterator.
   * @return Either a thick iterator (when the results would fit within a single chunk) or a thin
   *     iterator that uses RMI to fetch chunks from the remote {@link IAsynchronousIterator}.
   * @throws IllegalArgumentException if the iterator is <code>null</code>.
   */
   ...
   * @return A proxy for that {@link IBuffer} that masquerades any RMI exceptions.
   */
   ...
   * @param enableDGC If distributed garbage collection should be used for the object.
   * @return The proxy.
   */
