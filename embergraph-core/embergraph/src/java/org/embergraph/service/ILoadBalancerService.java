...
public interface ILoadBalancerService extends IService, IEventReceivingService {
   ...
   *
   * @throws TimeoutException if there are no data services and a timeout occurs while awaiting a
   *     service join.
   ...
   * service {@link UUID}s, however the {@link UUID}s returned MAY contain duplicates if the {@link
   * LoadBalancerService} has a strong preference for allocating load to some services (or for NOT
   * allocating load to other services). Further, the {@link LoadBalancerService} MAY choose (or be
   * forced to choose) to return {@link UUID}s for services that are within a nominal utilization
   * range, or even {@link UUID}s for services that are highly-utilized if it could otherwise not
   * satisify the request.
   *
   * @param minCount The minimum #of services {@link UUID}s to return -or- zero (0) if there is no
   *     minimum limit.
   * @param maxCount The maximum #of services {@link UUID}s to return -or- zero (0) if there is no
   *     maximum limit.
   * @param exclude The optional {@link UUID} of a data service to be excluded from the returned
   *     set.
   * @return Up to <i>maxCount</i> under-utilized services -or- <code>null</code> IFF no services
   *     are recommended at this time as needing additional load.
   * @throws TimeoutException if there are no data services, or if there is only a single data
   *     service and it is excluded by the request, and a timeout occurs while awaiting a service
   *     join.
   * @throws InterruptedException if the request is interrupted.
   * @todo generalize to also accept the class or interface of the service so that it can be used
   *     with services other than data services, e.g., metadata services, map/reduce services,
   *     {@link IEmbergraphClient}s, etc.
   ...
   * This implementation SHOULD reflect the relative rank of the service among all services as well
   * as its absolute load.
   *
