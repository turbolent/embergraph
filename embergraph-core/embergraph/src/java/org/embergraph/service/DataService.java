 ...
 * @todo Startup should be broken into two aspects: local startup and service connect and disconnect
 *     events. For example, we on the tx service connect the store manager should notify the tx
 *     service of the last commit time on the live journal. On disconnect, the data service needs to
 *     go offline. The metadata service is required only for overflow processing, but if it remains
 *     down then we will eventually need to bring the data service offline when the buffered writes
 *     would cause the live journal to no longer be fully buffered as the overflow processing time
 *     will be increased if we need to read through to the disk during overflow.
 * @todo Write benchmark test to measure interhost transfer rates. Should be 100Mbits/sec (~12M/sec)
 *     on a 100BaseT switched network. With full duplex in the network and the protocol, that rate
 *     should be bidirectional. Can that rate be sustained with a fully connected bi-directional
 *     transfer?
 *     <p>FIXME Probably ALL of the methods {@link IDataService} should be subsumed under {@link
 *     #submit(Callable)} or {@link #submit(long, String, IIndexProcedure)} so they do not block on
 *     the {@link DataService} and thereby absorb a thread.
 * @todo Review JERI options to support secure RMI protocols. For example, using SSL or an SSH
 *     tunnel. For most purposes I expect embergraph to operate on a private network, but replicate
 *     across gateways is also a common use case. Do we have to handle it specially?
 */
public abstract class DataService extends AbstractService
    implements IDataService, IServiceShutdown, ISession // IWritePipeline
{
  ...
  /*
   * @todo improve reporting here and for block write as well (goes through unisolated tasks at the
   *     present).
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  ...
  /*
   * Object supports local transactions and does handshaking with the {@link
  ...
  /** The object used to support distributed query against an {@link IEmbergraphFederation}. */
  ...
  /*
   * Core constructor - you MUST {@link #start()} the {@link DataService} before it can be used.
   *
   ...
   * is not usable until {@link StoreManager#isStarting()} returns <code>false</code> (there is
   * asynchronous processing involved in reading the existing store files or creating the first
   * store file and you can not use the {@link DataService} until that processing has been
   * completed). The {@link ConcurrencyManager} will block for a while waiting for the {@link
   * StoreManager} startup to complete and will reject tasks if startup processing does not complete
   * within a timeout.
   */
  ...
  /*
   * Concrete implementation manages the local state of transactions executing on a {@link
   * DataService}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
   ...
   *
   * @todo it would be nice if {@link #start()} could restart after {@link #shutdown()} but that is
   *     hardly necessary.
   */
  @Override
  ...
  /*
   * Delegate handles custom counters for the {@link ResourceManager}, local {@link
   * AbstractTransactionService} and the {@link ConcurrencyManager}, dynamic re-attachment of
   * counters, etc. This delegate must be set on the {@link AbstractClient} for those additional
   * features to work.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  public static class DataServiceFederationDelegate
    ...
    public boolean isServiceReady() {
      ...
      if (!dataService.resourceManager.isOpen()) {
         ...
         */
        if (log.isInfoEnabled()) log.info("Store manager not open - will shutdown.");
    ...
    /*
     * Extended to setup {@link DataService} specific counters and to write the client URL onto a
     * file in the service's data directory.
     */
    @Override
    ...
    protected void setupCounters() {
      ...
      final CounterSet serviceRoot = dataService.getFederation().getServiceCounterSet();

      serviceRoot
          .makePath(IDataServiceCounters.resourceManager)
          .attach(dataService.resourceManager.getCounters());
      ...
      serviceRoot
          .makePath(IDataServiceCounters.transactionManager)
...

      {
        ...
        if (queryEngine != null) {
          ...
          serviceRoot.makePath(IDataServiceCounters.queryEngine).attach(queryEngine.getCounters());
        }
      }
              ...
              public void sample() {
                ...
                long secs =
                    TimeUnit.SECONDS.convert(
                        dataService.readBlockApiCounters.readBlockNanos, TimeUnit.NANOSECONDS);

                final double v;

                if (secs == 0L) v = 0d;
                else v = dataService.readBlockApiCounters.readBlockCount / secs;
  ...
  /*
   * Polite shutdown does not accept new requests and will shutdown once the existing requests have
   * been processed.
   */
  @Override
   ...
   * DataService} and the various services which it uses.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  ...
  public long singlePhaseCommit(final long tx)
    ...
    try {
...

      {
        ...
        try {
          ...
          if (localState.isEmptyWriteSet()) {
            ...
            ((DataServiceTransactionManager) journal.getLocalTransactionManager())
      ...
      final AbstractTask<Void> task =
          new SinglePhaseCommit(
              concurrencyManager, journal.getLocalTransactionManager(), localState);
  ...
  public void prepare(final long tx, final long revisionTime)
    ...
    try {
      ...
      concurrencyManager
          .submit(
              new DistributedCommitTask(
                  concurrencyManager, resourceManager, getServiceUUID(), state, revisionTime))
          .get();
  ...
  /*
   * Task handling the distributed commit protocol for the {@link IDataService}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private static class DistributedCommitTask extends AbstractTask<Void> {
    ...
    public DistributedCommitTask(
      ...
      super(concurrencyManager, ITx.UNISOLATED, localState.getDirtyResource());

      if (resourceManager == null) throw new IllegalArgumentException();

      if (localState == null) throw new IllegalArgumentException();

      if (revisionTime == 0L) throw new IllegalArgumentException();

      if (revisionTime <= localState.getStartTimestamp()) throw new IllegalArgumentException();
    ...
    protected Void doTask() throws Exception {
      ...
      final IDistributedTransactionService txService =
          (IDistributedTransactionService)
              resourceManager.getLiveJournal().getLocalTransactionManager().getTransactionService();
    ...
    /*
     * Prepare the transaction (validate and merge down onto the unisolated indices and then
     * checkpoints those indices).
     *
     * <p>Note: This presumes that we are already holding exclusive write locks on the named indices
     * such that the pre-conditions for validation and its post-conditions can not change until we
     * either commit or discard the transaction.
     *
     * <p>Note: The indices need to be isolated as by {@link AbstractTask} or they will be enrolled
     * onto {@link Name2Addr}'s commitList when they become dirty and then checkpointed and included
     * with the NEXT commit.
     *
     * <p>For this reason, the {@link DistributedCommitTask} is an UNISOLATED task so that we can
     * reuse the existing mechanisms as much as possible.
     *
     * <p>FIXME This will work if we can grab the write service lock from within the task (which
     * will mean changing that code to allow the lock with the caller only still running or simply
     * waiting until we are signaled by the txService that all participants are either go (continue
     * execution and will commit at the next group commit, but then we need a protocol to impose the
     * correct commit time, e.g., by passing it on the task and ensuring that there is no other tx
     * ready in the commit group) or abort (just throw an exception).
     */
    ...
    /*
     * Obtain the exclusive lock on the write service. This will prevent any other tasks using the
     * concurrency API from writing on the journal.
     */
     ...
     *
     * <p>Note: There are no persistent side-effects unless this method returns successfully.
     *
     * @param commitTime The commit time that must be used.
     */
  ...
  /*
   * Forms the name of the index corresponding to a partition of a named scale-out index as
   * <i>name</i>#<i>partitionId</i>.
   *
   * <p>Another advantage of this naming scheme is that index partitions are just named indices and
   * all of the mechanisms for operating on named indices and for concurrency control for named
   * indices apply automatically. Among other things, this means that different tasks can write
   * concurrently on different partitions of the same named index on a given {@link DataService}.
   *
  ...
  /** Returns either {@link IDataService} or {@link IMetadataService} as appropriate. */
  @Override
  ...
  public void registerIndex(final String name, final IndexMetadata metadata)
    ...
    try {
      ...
      final AbstractTask<UUID> task = new RegisterIndexTask(concurrencyManager, name, metadata);
  ...
  public void dropIndex(final String name)
    ...
    try {
      ...
      final AbstractTask<Boolean> task = new DropIndexTask(concurrencyManager, name);
  ...
  public IndexMetadata getIndexMetadata(final String name, final long timestamp)
    ...
    try {
      ...
      final long startTime = (timestamp == ITx.UNISOLATED ? ITx.READ_COMMITTED : timestamp);
  ...
  /*
   * Retrieves the {@link IndexMetadata} for the named index as of the specified timestamp.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  public static class GetIndexMetadataTask extends AbstractTask<IndexMetadata> {
    ...
    public GetIndexMetadataTask(
        ConcurrencyManager concurrencyManager, long startTime, String name) {
  ...
  /*
   * Note: This chooses {@link ITx#READ_COMMITTED} if the the index has {@link ITx#UNISOLATED}
   * isolation and the {@link IIndexProcedure} is an read-only operation. This provides better
   * concurrency on the {@link DataService} by moving read-only operations off of the {@link
   * WriteExecutorService}.
   *
   * <p>Note: When the {@link DataService} is accessed via RMI the {@link Future} MUST be a proxy.
   * This gets handled by the concrete server implementation.
   */
  @Override
  public <T> Future<T> submit(final long tx, final String name, final IIndexProcedure<T> proc) {
    ...
    try {
      ...
      final long timestamp = (tx == ITx.UNISOLATED && proc.isReadOnly() ? ITx.READ_COMMITTED : tx);
  ...
  /*
   * Note: When the {@link DataService} is accessed via RMI the {@link Future} MUST be a proxy. This
   * gets handled by the concrete server implementation.
   *
   * @see AbstractDistributedFederation#getProxy(Future)
   * @todo we should probably put the federation object in a sandbox in order to prevent various
   *     operations by tasks running in the {@link DataService} using the {@link
   *     IDataServiceCallable} interface to gain access to the {@link DataService}'s federation. for
  ...
  public ResultSet rangeIterator(
    ...
    try {
      ...
      final RangeIteratorTask task =
          new RangeIteratorTask(
              concurrencyManager, timestamp, name, fromKey, toKey, capacity, flags, filter);
  ...
  /*
   * @todo this operation should be able to abort an {@link IBlock#inputStream() read} that takes
   *     too long or if there is a need to delete the resource.
   * @todo this should be run on the read service.
   * @todo coordinate close out of stores.
   * @todo efficient (stream-based) read from the journal (IBlockStore API). This is a fully
   *     buffered read and will cause heap churn.
   */
  @Override
  public IBlock readBlock(IResourceMetadata resource, final long addr) {
    ...
    if (addr == 0L) throw new IllegalArgumentException();
  ...
  protected static class RangeIteratorTask extends AbstractTask<ResultSet> {
    ...
    public ResultSet doTask() throws Exception {
      ...
      final ITupleIterator<?> itr =
          ndx.rangeIterator(fromKey, toKey, limit, flags | IRangeQuery.KEYS, filter);
  ...
  public void forceOverflow(final boolean immediate, final boolean compactingMerge)
    ...
    try {
      ...
      log.warn(
          "Will force overflow: immediate=" + immediate + ", compactingMerge=" + compactingMerge);
        ...
        getConcurrencyManager()
            .submit(
                new AbstractTask<Void>(
                    getConcurrencyManager(), ITx.UNISOLATED, new String[] {"__forceOverflow"}) {
  ...
  /*
   * Task sets the flag that will cause overflow processing to be triggered on the next group
   * commit.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  ...
  public boolean isOverflowActive() throws IOException {
    ...
    try {
       ...
       */
      return resourceManager.isOverflowEnabled() && !resourceManager.isOverflowAllowed();
