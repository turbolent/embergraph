...
public class EventReceiver implements IEventReceivingService, IEventReportingService {
   ...
   * your tab-delimited events log and you may have to filter it out before you can process the
   * events log analytically.
   */
   ...
   *
   * <p>Since this is a "linked" collection, the order of the events in the collection will always
   * reflect their arrival time. This lets us scan the events in temporal order, filtering for
   * desired criteria and makes it possible to prune the events in the buffer as new events arrive.
   */
   ...
   * @param eventStore A {@link BTree} used to record the completed events and to implement the
   *     {@link IEventReportingService} interface.
   */
  @SuppressWarnings("unchecked")
  public EventReceiver(final long eventHistoryMillis, final EventBTree eventBTree) {

    if (eventHistoryMillis <= 0) throw new IllegalArgumentException();

    if (eventBTree == null) throw new IllegalArgumentException();
  ...
  public static class EventBTree extends BTree {
    ...
    public static EventBTree create(final IRawStore store) {
      ...
      metadata.setTupleSerializer(
          new EventBTreeTupleSerializer(new ASCIIKeyBuilderFactory(Bytes.SIZEOF_LONG)));
     ...
     * @version $Id$
     */
    protected static class EventBTreeTupleSerializer extends DefaultTupleSerializer<Long, Entry> {
   ...
   *
   * <p>An error is logged if an event is purged before its end() event arrives. This generally
   ...
   *
   * @todo if this event proves to be a bottleneck then a queue could be used a thread could migrate
   *     events from the queue to the log and the BTree.
   */
  protected void logEvent(final Event e) {

    if (e == null) throw new IllegalArgumentException();

    if (!e.isComplete()) throw new IllegalArgumentException();
  ...
  public long rangeCount(final long fromTime, final long toTime) {
    ...
    return ndx.rangeCount(tupleSer.serializeKey(fromTime), tupleSer.serializeKey(toTime));
  }

  @SuppressWarnings("unchecked")
  public Iterator<Event> rangeIterator(final long fromTime, final long toTime) {

    final ITupleIterator<Event> tupleItr =
        ndx.rangeIterator(tupleSer.serializeKey(fromTime), tupleSer.serializeKey(toTime));

    final Iterator<Event> itr = new Striterator(tupleItr).addFilter(new TupleObjectResolver());
