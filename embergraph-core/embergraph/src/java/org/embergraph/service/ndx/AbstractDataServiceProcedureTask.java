 ...
 * split of the target index partition.
 *
 * <p>Note: If an index partition is moved then the key range is unchanged.
 *
 * <p>Note: If an index partition is split, then the key range for each new index partition is a
 * sub-range of the original key range and the total key range for the new index partitions is
 * exactly the original key range.
 *
 * <p>Note: If an index partition is joined with another index partition then the key range of the
 * new index partition is increased. However, we will wind up submitting one request to the new
 * index partition for each index partition that we knew about at the time that the procedure was
 * first mapped across the index partitions.
 *
...
abstract class AbstractDataServiceProcedureTask implements Callable<Void> {
  ...
  protected static final transient Logger log =
      Logger.getLogger(AbstractDataServiceProcedureTask.class);
  ...
  /** Used to aggregate results when a procedure can span more than one index partition. */
   ...
   * @param resultHandler
   */
  public AbstractDataServiceProcedureTask(
    ...
    if (split.pmd == null) throw new IllegalArgumentException();
    ...
    this.taskCountersByIndex =
        ndx.getFederation().getIndexCounters(ndx.getName()).synchronousCounters;
   ...
   * @throws Exception
   */
  protected final void submit(final PartitionLocator locator) throws Exception {
    ...
    if (dataService == null) throw new RuntimeException("DataService not found: " + locator);
    ...
    if (log.isInfoEnabled()) log.info("Submitting task=" + this + " on " + dataService);
   ...
   *
   * @param dataService The data service on which the procedure will be executed.
   * @param name The name of the index partition on that data service.
   * @todo do not require aggregator for {@link ISimpleIndexProcedure} so make this abstract in the
   *     base class or just override for {@link ISimpleIndexProcedure}. This would also mean that
   ...
   * partition which we thought we were addressing and then re-map the operation against those
   * updated locator(s). Note that a split will go from one locator to N locators for a key range
   * while a join will go from N locators for a key range to 1. A move does not change the #of
   * locators for the key range, just where that index partition is living at this time.
   *
   * @throws Exception
   */
