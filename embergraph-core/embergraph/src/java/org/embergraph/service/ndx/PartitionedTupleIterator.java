 ...
 * specified to the ctor. {@link StaleLocatorException}s are handled transparently by restarting the
 * locator scan and continuing the range query request from the successor (or predecessor for a
 * reverse scan) of the last key visited. When the operation is unisolated, this class will
 * correctly complete the range iterator request even if the index partition is split, joined or
 * moved during traversal.
 *
...
public class PartitionedTupleIterator<E> implements ITupleIterator<E> {
  ...
  /** The first key to visit -or- <code>null</code> iff no lower bound (from the ctor). */
  private final byte[] fromKey;

  /** The first key to NOT visit -or- <code>null</code> iff no upper bound (from the ctor). */
   ...
   * handle a {@link StaleLocatorException} then these fields will be used to restart the locator
   * scan.
   *
   * <p>The {@link #currentFromKey} is initially set by the ctor to the {@link #fromKey}.
   *
   * <p>For a reverse scan, the {@link #currentFromKey} remains unchanged.
   *
   * <p>For a forward scan the {@link #currentFromKey} is set to the last key visited in each {@link
   * ResultSet}.
   *
   * @todo {@link ResultSet#getLastKey()} will return <code>null</code> if there were no tuples to
   *     visit. This class does not appear to handle that condition, but it is able to pass a unit
   *     test where there is an empty partition in the middle of the key range scan for both forward
   *     and reverse traversal and a test where there is a non-empty partition whose tuples are
   *     filtered out such that it is effectively empty. see TestRangeQuery#test_reverseScan()
   * @todo if we restart a locator scan and the index partition boundaries have changed, then what
   *     do we need to do in order to make sure that we are issuing the query?
   */
   ...
   *
   * <p>For a reverse scan, the {@link #currentToKey} is set to the last key visited in each {@link
   * ResultSet}.
   */
   ...
   *
   * @deprecated The #of partitions is a bit tricky since splits and joins can change the #of index
   *     partitions dynamically (for unisolated or read-committed reads where read-consistent is not
   *     true).
   */
   ...
   *     partitions unless you are using a read-consistent view.
   */
   ...
   * than {@link ITx#READ_COMMITTED}. The latter will use dirty reads (each time a {@link ResultSet}
   * is fetched it will be fetched from the most recently committed state of the database).
   *
   * @param ndx
   ...
   * @param flags
   * @param filter
   * @throws IllegalArgumentException if readConsistent is requested and the index view is {@link
   *     ITx#UNISOLATED}.
   */
  public PartitionedTupleIterator(
  ...
  public boolean hasNext() {
    ...
    try {
      ...
      final StaleLocatorException cause =
          (StaleLocatorException) InnerCause.getInnerCause(ex, StaleLocatorException.class);
       ...
       * This happens if we get a StaleLocatorException,
            ...
            throw new RuntimeException("Missing index partition on data service? " + locator, ex);
          }
        }
        ...
        locatorItr = ndx.locatorScan(ts, currentFromKey, currentToKey, reverseScan);
  ...
  private boolean nextPartition() {
    ...
    if (!locatorItr.hasNext()) {
      ...
      if (log.isInfoEnabled()) log.info("No more locators");
    ...
    if (log.isInfoEnabled()) log.info("locator=" + locator);
  ...
  private void rangeQuery() {
    ...
    try {
      ...
      final byte[] _fromKey =
          AbstractKeyRangeIndexProcedure.constrainFromKey(currentFromKey, locator);

      final byte[] _toKey = AbstractKeyRangeIndexProcedure.constrainToKey(currentToKey, locator);
      ...
      if (log.isInfoEnabled())
        log.info(
            "name="
                ...
                + partitionId
                + ", fromKey="
                + BytesUtil.toString(_fromKey)
                + ", toKey="
                + BytesUtil.toString(_toKey));
      ...
      src =
          new DataServiceTupleIterator<E>(
              ndx,
              dataService,
              DataService.getIndexPartitionName(ndx.getName(), partitionId),
              ts,
              _fromKey,
              _toKey,
              capacity,
       ...
       * Overridden so that we observe each distinct result set obtained from the DataService.
            ...
            protected ResultSet getResultSet(
              ...
              final ResultSet tmp =
                  super.getResultSet(timestamp, fromKey, toKey, capacity, flags, filter);
       ...
       * We are moving backwards through the key order so we
                ...
                if (INFO)
                  log.info("New exclusive upper bound: " + BytesUtil.toString(currentToKey));
       ...
       * We are moving forwards through the key order so we
                ...
                if (INFO)
                  log.info("New inclusive lower bound: " + BytesUtil.toString(currentFromKey));
