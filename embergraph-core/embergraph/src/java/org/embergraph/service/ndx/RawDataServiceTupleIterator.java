 ...
 * partitions, by the {@link AbstractDistributedFederation}.
 *
...
public class RawDataServiceTupleIterator<E> extends AbstractChunkedTupleIterator<E> {
   ...
   * partition splits, joins, and moves!
   *
   * @todo this should failover if a data service dies. we need the logical data service UUID for
   *     that and we need to have (or discover) the service that maps logical data service UUIDs to
   *     physical data service UUIDs
   */
   ...
   * @param readConsistent This option is only available for {@link ITx#READ_COMMITTED}. When <code>
   *     true</code>, the first read will be against the most recent commit point on the database
   *     and any continuation queries will be against the <em>same</em> commit point. When <code>
   *     false</code>, each read will be against the most recent commit point (so the data can be
   *     drawn from multiple commit points if there are concurrent commits).
   *     <p>The <i>readConsistent</i> option is a tweak available only at this low level for the
   *     {@link RawDataServiceTupleIterator}. It avoids a possible RMI to obtain the most recent
   *     global commit point using {@link IIndexStore#getLastCommitTime()} in favor of using the
   *     most recent commit point on the index partition at the time that the query is actually
   *     executed. If you are reading against a scale-out index, then similar effects are obtained
   *     by choosing either {@link ITx#READ_COMMITTED} or {@link IIndexStore#getLastCommitTime()}.
   *     See {@link ClientIndexView} and {@link PartitionedTupleIterator}.
   * @param fromKey
   * @param toKey
   * @param capacity
   * @param flags
   * @param filter
   */
  public RawDataServiceTupleIterator(
      final IDataService dataService,
      final String name,
      final long timestamp,
      final boolean readConsistent,
      final byte[] fromKey,
      final byte[] toKey,
      final int capacity,
      final int flags,
   ...
   * value stored by this class. This allows us to have read-consistent semantics if desired for
   * {@link ITx#UNISOLATED} or {@link ITx#READ_COMMITTED} operations.
   */
  @Override
  protected ResultSet getResultSet(
    ...
    try {
      ...
      return dataService.rangeIterator(timestamp, name, fromKey, toKey, capacity, flags, filter);
      ...
      dataService
          .submit(
              ...
              BatchRemoveConstructor.RETURN_MUTATION_COUNT.newInstance(
                  0 /* fromIndex */, n /* toIndex */, keys, null /*vals*/))
          .get();
    ...
    try {
      ...
      dataService
          .submit(
              ...
              name,
              BatchRemoveConstructor.RETURN_MUTATION_COUNT.newInstance(
                  0 /* fromIndex */, 1 /* toIndex */, new byte[][] {key}, null /*vals*/))
          .get();
