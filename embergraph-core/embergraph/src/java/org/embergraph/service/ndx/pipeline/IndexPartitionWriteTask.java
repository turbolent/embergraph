...
public class IndexPartitionWriteTask<
  ...
  public IndexPartitionWriteTask(
      final M master,
      final L locator,
   ...
   * must be written.
   *
   * @param sourceChunk A chunk (in sorted order).
   * @return <code>true</code> iff a {@link StaleLocatorException} was handled, in which case the
  ...
  protected boolean handleChunk(final E[] sourceChunk)
    ...
    if (master.duplicateRemover == null) {
      ...
      if (duplicateCount > 0 && log.isDebugEnabled())
        log.debug(
            "Filtered out " + duplicateCount + " duplicates from " + chunk.length + " elements");
    }
    ...
    for (int i = 0; i < chunkSize; i++) {
      ...
      if (sendValues) vals[i] = chunk[i].val;
    }
    ...
    try {
      ...
      final T proc =
          master.ctor.newInstance(
              master.ndx, 0 /* fromIndex */, chunkSize /* toIndex */, keys, vals);
      ...
      for (int ntries = 0; ntries < maxtries; ntries++) {
        ...
        try {
          ...
          result = ((Future<R>) dataService.submit(timestamp, indexPartitionName, proc)).get();
          done = true;
          break;
          ...
          if (cause != null) {
            ...
            if (ntries + 1 < maxtries) {
              ...
              log.error("Will retry (" + ntries + " of " + maxtries + "): " + this, ex);
      ...
      if (master.resultHandler != null) {
        ...
        // aggregate results.
        master.resultHandler.aggregate(
            result, new Split(locator, 0 /* fromIndex */, chunkSize /* toIndex */));
        ...
        if (master.resultHandler instanceof IAsyncResultHandler) {
          ...
          ((IAsyncResultHandler<R, A, O, KVO<O>>) master.resultHandler)
              .aggregateAsync(
                  chunk, result, new Split(locator, 0 /* fromIndex */, chunkSize /* toIndex */));
        }
      }
