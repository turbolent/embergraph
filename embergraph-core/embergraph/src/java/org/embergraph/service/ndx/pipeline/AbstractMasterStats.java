 ...
 * practice is to be <code>synchronized</code> on this reference before you update those counters.
 * Without this, the counters might not update correctly as thing like <code>a += 2</code> may not
 * produce the correct result.
 *
 * <p>Note: The use of {@link AtomicLong}s provides atomic visibility changes relied on by some unit
 * tests.
 *
 * @param <L> The generic type of the key used to lookup subtasks in the internal map.
 * @param <HS> The generic type of the subtask statistics.
...
public abstract class AbstractMasterStats<L, HS extends AbstractSubtaskStats> {
  ...
  /** The #of chunks drained from the {@link BlockingBuffer} by the {@link AbstractMasterTask}. */
   ...
   * approximated as {@link #elementsTransferred}/ {@link #chunksTransferred} X the
   * averageMasterQueueSize). Neither does it include the #of elements in a prepared or outstanding
   * write on an index partition.
   */
   ...
   * chunk drained from the master will be registered as some larger #of chunks transferred to the
   * sink(s) for that master.
   */
   ...
   * average chunk size entering the sink accordingly.
   */
   ...
   * AbstractSubtask sink task}s are strongly held until they are closed so we don't really need a
   ...
   *
   * @param locator The index partition.
  ...
  public HS getSubtaskStats(final L locator) {
    ...
    synchronized (sinkStats) {
      ...
      if (t == null) {
        ...
        maximumPartitionCount.set(Math.max(maximumPartitionCount.get(), sinkStats.size()));
      }
