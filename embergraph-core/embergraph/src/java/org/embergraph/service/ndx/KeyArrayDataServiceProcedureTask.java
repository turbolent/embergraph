...
class KeyArrayDataServiceProcedureTask extends AbstractDataServiceProcedureTask {
   ...
   *
   * <p>Note: On the client, {@link #keys} may be partitioned across many {@link Split}s but is
   * always dense on the server. This conversion to a dense array is achieved by the {@link
   * AbstractKeyArrayIndexProcedure} when it serializes its keys.
   */
   ...
   *
   * <p>Note: On the client, {@link #vals} may be partitioned across many {@link Split}s but is
   * always dense on the server. This conversion to a dense array is achieved by the {@link
   * AbstractKeyArrayIndexProcedure} when it serializes its values.
   */
   ...
   * @param vals The original vals[][].
   * @param split The split identifies the subset of keys and values to be applied by this
   *     procedure.
   * @param proc The procedure instance.
   * @param resultHandler The result aggregator.
   * @param ctor The object used to create instances of the <i>proc</i>. This is used to re-split
   *     the data if necessary in response to stale locator information.
   */
  public KeyArrayDataServiceProcedureTask(
  ...
  protected void retry() throws Exception {
    ...
    try {
       ...
       */
      ((ClientIndexView) ndx)
          .submit(ts, split.fromIndex, split.toIndex, keys, vals, ctor, resultHandler);
