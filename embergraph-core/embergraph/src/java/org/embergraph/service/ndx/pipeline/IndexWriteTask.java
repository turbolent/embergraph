 ...
 *
 * <p>If the task is interrupted, it will refuse additional writes by closing its {@link
 * BlockingBuffer} and will cancel any sub-tasks and discard any buffered writes.
 *
 ...
 * @param <L> The generic type of the key used to lookup a subtask in the internal map (must be
 *     unique and must implement hashCode() and equals() per their contracts).
 * @param <HS> The generic type of the value returned by {@link Callable#call() } for the subtask.
 * @param <T> The generic type of the CTOR for the procedure used to write on the index.
 * @param <R> The type of the result from applying the index procedure to a single {@link Split} of
 *     data.
...
public abstract class IndexWriteTask<
   ...
   *     wait inside of the {@link IAsynchronousIterator} when it polls the iterator for a chunk.
   *     This value should be relatively small so that the sink remains responsible rather than
   *     blocking inside of the {@link IAsynchronousIterator} for long periods of time.
   * @param sinkQueueCapacity The capacity of the internal queue for the per-sink output buffer.
   * @param sinkChunkSize The desired size of the chunks written that will be written by the {@link
   *     AbstractSubtask sink}.
   * @param sinkChunkTimeoutNanos The maximum amount of time in nanoseconds that a sink will combine
   *     smaller chunks so that it can satisfy the desired <i>sinkChunkSize</i>.
   * @param duplicateRemover Removes duplicate key-value pairs from the (optional).
   * @param ctor The ctor instantiates an {@link IIndexProcedure} for each chunk written on an index
   *     partition.
   * @param resultHandler Aggregates results across the individual index partition write operations
   *     (optional).
   * @param stats The index statistics object.
   * @param buffer The buffer on which the application will write.
   */
  public IndexWriteTask(
    ...
    if (sinkQueueCapacity <= 0) throw new IllegalArgumentException();

    if (sinkChunkSize <= 0) throw new IllegalArgumentException();

    if (sinkChunkTimeoutNanos <= 0) throw new IllegalArgumentException();
   ...
   * appropriate sink.
   */
  protected void handleChunk(final E[] a, final boolean reopen) throws InterruptedException {
    ...
    try {
      ...
      final LinkedList<Split> splits =
          ndx.splitKeys(ndx.getTimestamp(), 0 /* fromIndex */, a.length /* toIndex */, a);
      ...
      for (Split split : splits) {
        ...
        addToOutputBuffer((L) split.pmd, a, split.fromIndex, split.toIndex, reopen);
      }
  ...
  protected S newSubtask(final L locator, final BlockingBuffer<E[]> out) {
    ...
    if (dataService == null)
      throw new RuntimeException("DataService not found: " + locator.getDataServiceUUID());
  ...
  @Override
  protected void submitSubtask(final FutureTask<? extends AbstractSubtaskStats> subtask) {
