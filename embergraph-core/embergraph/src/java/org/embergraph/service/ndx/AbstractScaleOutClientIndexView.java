...
public abstract class AbstractScaleOutClientIndexView implements IScaleOutClientIndex {
  ...
  protected static final transient Logger log =
      Logger.getLogger(AbstractScaleOutClientIndexView.class);
   ...
   *
   * @param ts The timestamp for the {@link IMetadataIndex} view that will be applied to choose the
   *     {@link Split}s.
   ...
   * @param keys An array of keys. Each key is an interpreted as an unsigned byte[]. All keys must
   *     be non-null. The keys must be in sorted order.
   * @return The {@link Split}s that you can use to form requests based on the identified first/last
   *     key and partition identified by this process.
   ...
   *     IMetadataService}. Or perhaps the locators can be cached in a local BTree and a miss there
   *     would result in a read through to the remote {@link IMetadataService} but then we have the
   *     problem of figuring out when to release locators if the client is long-lived.
   */
  @Override
  public LinkedList<Split> splitKeys(
    ...
    while (currentIndex < toIndex) {
       ...
       */
      final PartitionLocator locator = fed.getMetadataIndex(name, ts).find(keys[currentIndex]);
   ...
   * introduce a truely enourmous burden on the client's task queue since the #of tasks arising is
