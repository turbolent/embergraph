 ...
 * @param <M> The generic type of the master task implementation class.
...
public abstract class AbstractSubtask<
        ...
        M extends
            AbstractMasterTask<
                    ? extends AbstractMasterStats<L, HS>, E, ? extends AbstractSubtask, L>,
        E,
        L>
   ...
   * idle. (A sink with an available chunk is never idle.)
   */
  protected volatile long lastChunkAvailableNanos = lastChunkNanos;

  public String toString() {

    return getClass().getName() + "{locator=" + locator + ", open=" + buffer.isOpen() + "}";
  }

  public AbstractSubtask(final M master, final L locator, final BlockingBuffer<E[]> buffer) {

    if (master == null) throw new IllegalArgumentException();
  ...
  public HS call() throws Exception {
    ...
    try {
      ...
      final NonBlockingChunkedIterator itr = new NonBlockingChunkedIterator(src);
      ...
      while (itr.hasNext()) {
        ...
        if (handleChunk(chunk)) {
          ...
          if (log.isInfoEnabled()) log.info("Eager termination.");
      ...
      if (buffer.isOpen()) throw new AssertionError(toString());

      if (log.isInfoEnabled()) log.info("Done: " + locator);
   ...
   * available, then it is added to an ordered list of chunks which is maintained internally by this
   * class. {@link #next()} combines those chunks, using a merge sort to maintain their order, and
   * returns their data in a single chunk.
   *
   * <p>Note: This does not implement {@link Iterator} since its methods throw {@link
  ...
  private class NonBlockingChunkedIterator {
    ...
    public boolean hasNext() throws InterruptedException {
      ...
      while (true) {
        ...
        final boolean idle =
            elapsedSinceLastChunk > master.sinkIdleTimeoutNanos
        ...
        if (src.hasNext(master.sinkPollTimeoutNanos, TimeUnit.NANOSECONDS)) {
          ...
          assert a.length != 0;
          assert a[0] != null
              : "chunk with nulls: chunkSize=" + a.length + ", chunk=" + Arrays.toString(a);
     ...
     * the data in the chunk fully ordered.
     */
    public E[] next() {
      ...
      final E[] a =
          (E[])
              java.lang.reflect.Array.newInstance(
                  ...
                  firstChunk.getClass().getComponentType(), chunkSize);
   ...
   * #handleChunk(Object[])}.
   *
   * <p>This method asynchronously closes the <i>sink</i>, so that no further data may be written on
   * it by setting the <i>cause</i> on {@link BlockingBuffer#abort(Throwable)}. Next, the current
   * chunk is placed onto the master's redirectQueue and the sink's {@link #buffer} is drained,
   * transferring all chunks which can be read from that buffer onto the master's redirectQueue.
   *
   * @param chunk The chunk which the sink was processing when it discovered that it need to
   *     redirect its outputs to a different sink (that is, a chunk which it had already read from
   *     its buffer and hence which needs to be redirected now).
   * @param cause The stale locator exception.
   * @throws InterruptedException
   */
   ...
   */
  protected abstract void notifyClientOfRedirect(L locator, Throwable cause);
}
