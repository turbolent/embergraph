 ...
 *
 * @todo If the index was dropped then that should cause the operation to abort (only possible for
 *     read committed or unisolated operations).
 *     <p>Likewise, if a transaction is aborted, then then index should refuse further operations.
 * @todo detect data service failure and coordinate cutover to the failover data services. ideally
 *     you can read on a failover data service at any time but it should not accept write operations
 *     unless it is the primary data service in the failover chain.
 *     <p>Offer policies for handling index partitions that are unavailable at the time of the
 *     request (continued operation during partial failure).
 * @todo We should be able to transparently use either a hash mod N approach to distributed index
 *     partitions or a dynamic approach based on overflow. This could even be decided on a per-index
 *     basis. The different approaches would be hidden by appropriate implementations of this class.
 *     <p>A hash partitioned index will need to enforce optional read-consistent semantics. This can
 *     be done by choosing a recent broadcast commitTime for the read or by re-issuing queries that
 *     come in with a different commitTime.
 * @todo This class could consolidate parallelized operations by data service, issuing a chunk of
 *     requests for each index partition on a given data service. This would reduce the #of RMI
 *     requests to one per data service against which the parallelized operation must be mapped.
...
public class ClientIndexView implements IScaleOutClientIndex {
   ...
   *     have a timestamp in effect which is different from the timestamp of the view (e.g., a
   *     read-consistent transaction).
   */
   ...
   *     have a timestamp in effect which is different from the timestamp of the view (e.g., a
   *     read-consistent transaction).
   * @todo should be protected, but some unit tests in a different package access this.
  ...
  private ThreadLocal<AtomicInteger> recursionDepth =
  ...
  /** Metadata for the {@link MetadataIndex} that manages the scale-out index (cached). */
  ...
  public boolean contains(final byte[] key) {
    ...
    if (batchOnly) log.error(NON_BATCH_API, new RuntimeException());
    else if (WARN) log.warn(NON_BATCH_API);
  ...
  public byte[] insert(final byte[] key, final byte[] value) {
    ...
    submit(
        ...
        keys,
        vals,
        BatchInsertConstructor.RETURN_OLD_VALUES,
        resultHandler);
  ...
  public byte[] putIfAbsent(final byte[] key, final byte[] value) {
    ...
    submit(
        ...
        keys,
        vals,
        BatchPutIfAbsentConstructor.RETURN_OLD_VALUES,
        resultHandler);
  ...
  public byte[] remove(final byte[] key) {
    ...
    submit(
        ...
        keys,
        null /* vals */,
        BatchRemoveConstructor.RETURN_OLD_VALUES,
        resultHandler);
  ...
  public long rangeCount(final byte[] fromKey, final byte[] toKey) {
    ...
    final RangeCountProcedure proc =
   ...
   */
  @Override
  public final long rangeCountExactWithDeleted(final byte[] fromKey, final byte[] toKey) {
   ...
   * range on a given index partition. Once the iterator is exhausted on a given index partition it
   * is then applied to the next index partition spanned by the key range.
   *
   * @todo If the return iterator implements {@link ITupleCursor} then this will need be modified to
   *     defer request of the initial result set until the caller uses first(), last(), seek(),
   *     hasNext(), or hasPrior().
   */
  @Override
  public ITupleIterator rangeIterator(
    ...
    try {
      ...
      if (parallel) {
        ...
        return parallelRangeIterator(
            ts, isReadConsistentTx, fromKey, toKey, capacity, flags, filter);
        ...
        return new PartitionedTupleIterator(
   ...
   *
   * @param ts The timestamp for the view (may be a transaction).
   * @param isReadConsistentTx <code>true</code> iff the caller specified timestamp is a
   *     read-historical transaction created specifically to give the iterator read-consistent
   *     semantics. when <code>true</code>, this class will ensure that the transaction is
   *     eventually aborted so that its read lock will be released. This is done eagerly when the
   *     iterator is exhausted and with a {@link #finalize()} method otherwise.
   * @param fromKey
   * @param toKey
   * @param capacity
   * @param flags
   ...
   * @return
   */
  private ITupleIterator parallelRangeIterator(
      final long ts,
      final boolean isReadConsistentTx,
      final byte[] fromKey,
      final byte[] toKey,
     ...
     * limited parallelism.
     */
    final ParallelRangeIteratorTask task =
  ...
  private static class UnchunkedTupleIterator<E> implements ITupleIterator<E> {
    ...
    public boolean hasNext() {
      ...
      while (!exhausted && (chunk == null || chunk.length == 0 || index >= chunk.length)) {
  ...
  private class ParallelRangeIteratorTask implements Callable<Void> {
    ...
    public ParallelRangeIteratorTask(
        final long ts,
        final boolean isReadConsistentTx,
        final byte[] fromKey,
        final byte[] toKey,
      ...
      maxTasks = poolSize == 0 ? maxTasksPerRequest : Math.min(poolSize, maxTasksPerRequest);
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        final Iterator<PartitionLocator> itr =
            locatorScan(ts, fromKey, toKey, false /* reverseScan */);
        ...
        while (itr.hasNext()) {
          ...
          for (int i = 0; i < maxTasks && itr.hasNext(); i++) {
            ...
            final byte[] _fromKey =
                AbstractKeyRangeIndexProcedure.constrainFromKey(fromKey, locator);

            final byte[] _toKey = AbstractKeyRangeIndexProcedure.constrainToKey(toKey, locator);
    ...
    private class RobustIteratorTask implements Callable<Void> {
      ...
      RobustIteratorTask(final byte[] fromKey, final byte[] toKey) {
        ...
        itr =
            new PartitionedTupleIterator(
                ClientIndexView.this,
                ts,
                isReadConsistentTx,
                fromKey,
                toKey,
      ...
      public Void call() throws Exception {
        ...
        try {
          ...
          final UnsynchronizedArrayBuffer unsyncBuffer =
   ...
   * single shard. Therefore, a read-consistent view of the index will NOT be obtained for
   * read-committed or unisolated requests as the operation should already be shard-wise ACID and it
   * addresses only a single shard. This effects all read-only point operations on the index,
   * including lookup() and contains() as well as custom procedures such as GRS reads.
   *
   * <p>Procedures which require read-consistent protection across more than one shard MUST be
   * designed with a <i>fromKey</i> and a <i>toKey</i> rather than just a <i>key</i>. The
   * <i>fromKey</i> and <i>toKey</i> are used to identify the relevant shard(s) spanned by the
   * operation. Read-consistent isolation is then optionally imposed depending on the client.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/454">Global Row Store Read on
   *     Cluster uses Tx </a>
   */
  @Override
   ...
   * @return
   */
  private <T> T submit(final long ts, final byte[] key, final ISimpleIndexProcedure<T> proc) {
   ...
   *
   * <p>Note: In order to avoid growing the task execution queue without bound, an upper bound of
   * {@link Options#CLIENT_MAX_PARALLEL_TASKS_PER_REQUEST} tasks will be placed onto the queue at a
   * time. More tasks will be submitted once those tasks finish until all tasks have been executed.
   * When the task is not parallelizable the tasks will be submitted to the corresponding index
   * partitions at a time and in key order.
   */
  @Override
  public void submit(
      final byte[] fromKey,
   ...
   *
   * <p>Note: Unlike mapping an index procedure across a key range, this method is unable to
   * introduce a truly enormous burden on the client's task queue since the #of tasks arising is
   * equal to the #of splits and bounded by <code>n := toIndex - fromIndex</code>.
   *
   * @return The aggregated result of applying the procedure to the relevant index partitions.
   */
  @Override
  public void submit(
      final int fromIndex,
    ...
    final long ts;
    {
       ...
       */
      final IKeyArrayIndexProcedure proc = ctor.newInstance(this, fromIndex, toIndex, keys, vals);
   ...
   * @param tasks The tasks.
   */
  protected void runParallel(final ArrayList<AbstractDataServiceProcedureTask> tasks) {
   ...
   * @param tasks The tasks.
   */
  protected void runInCallersThread(final ArrayList<AbstractDataServiceProcedureTask> tasks) {
  ...
  //                    }
  //
  //                    if(log.isDebugEnabled()) log.debug("Exact match on rightSeparator: pos=" +
  // pos
  ...
  public <T extends IKeyArrayIndexProcedure, O, R, A> IRunnableBuffer<KVO<O>[]> newWriteBuffer(
    ...
    final AsynchronousIndexWriteConfiguration conf =
