 ...
 *
 * The asynchronous write API exposes a blocking buffer to the application which accepts concurrent
 * writes of {@link KVO}[] chunks, in which each {@link KVO} represents a tuple to be written on a
 * scale-out index. The buffer is drained by a master task, which transfers chunks to sinks tasks,
 * each of which writes on a specific index partition.
 *
 * <p>The master is provisioned with a CTOR which is used to convert the {@link KVO}s into <code>
 * unsigned byte[]</code> <i>keys</i> and <code>byte[]</code> <i>values</i> for writes on the
 * scale-out index. The master may be provisioned with a mechanism to filter out duplicate tuples.
 *
 * <p>Writes on the index partitions are asynchronous with respect to the application. However, a
 * {@link KVOC} / {@link KVOLatch} combination can be used to coordinate notification when some set
 * of tuples of interest have been successfully written onto the scale-out index. This combination
 * can also be used to pass back values from the write operation if they are assigned by side-effect
 * onto the {@link KVO#obj} reference.
 *
 * <p>The asynchronous write implementation is divided into a master, with an input queue and a
 * redirect queue, and sinks, each of which has an input queue and writes chunks onto a specific
 * index partition for the scale-out index. The input queues for the master and the sinks are
 * bounded. The redirect input queue is unbounded. The master and each sink is assigned its own
 * worker thread.
 *
 * <p>The master transfers chunks from its input queue to the sinks. It polls the redirect queue for
 * a chunk. If that queue was empty, then it polls a chunk from the input queue. If no chunks are
 * available, it needs to check again. The master stops polling the input queue when the input queue
 * is closed, but it continues to drain the redirect queue until all sinks are done or the master is
 * canceled.
 *
 * <p>Note: The requirement for polling arises because: (a) we are not coordinating signals for the
 * arrival of a chunk on the input or redirect queues; and (b) a chunk can be redirected at any time
 * if there is an outstanding write by a sink on an index partition.
 *
 * <p>The atomic decision to terminate the master is made using a {@link #lock}. The lock is
 * specific to the life cycle of the sinks. The lock is held when a sink is created. When a sink
 * terminates, its last action is to grab the lock and signal the {@link #subtaskDone} {@link
 * Condition}. The master terminates when, while holding the lock, it observes that no sinks are
 * running AND the redirect queue is empty. Since chunks are placed onto the redirect queue by sinks
 * (and there are no sinks running) and by the master (which is not issuing a redirect since it is
 * running its termination logic) these criteria are sufficient for termination. However, the sink
 * must ensure that its buffer is closed before it terminates, even if it terminates by exception,
 * so that an attempt to transfer a chunk to the sink will not block forever.
 *
 * <p>Once the master is holding a chunk, it splits the chunk into a set of dense chunks correlated
 * with the locators of the index partitions on which those chunks will be written. The split
 * operation is NOT atomic, but it is consistent in the following sense. If a {@link Split} is
 * identified based on old information, then the chunk will be directed to an index partition which
 * no longer exists. An attempt to write on that index partition will result in a stale locator
 * exception, which is handled.
 *
 * <p>Once the chunk has been split, the split chunks are transferred to the appropriate sink(s).
 * Since the master is not holding any locks, a blocking put() may be used to transfer the chunk to
 * sink.
 *
 * <p>The sink drain chunks from its input queue. If the input queue is empty and the idle timeout
 * expires before a chunk is transferred to the input queue, then the sink will be asynchronously
 * closed and an {@link IdleTimeoutException} will be set on the input queue. If the master attempts
 * to transfer a chunk to the sink's input queue after the idle timeout, then an exception wrapping
 * the idle timeout exception will be thrown. The master handles the wrapped idle timeout exception
 * by re-opening the sink and will retry the transfer of the chunk to the (new) sink's input queue.
 * After the sink closes it's input queue by an idle timeout, it will continue to drain the input
 * queue until it is empty, at which point the sink will terminate (this handles the case where the
 * master concurrently transferred a chunk to the sink's input queue before it was closed by the
 * idle time out).
 *
 * <p>The sink combines chunks drained from its input queue until the target chunk size for a write
 * is achieved or until the chunk timeout for the sink is exceeded. The sink then writes on the
 * index partition corresponding to its locator. This write occurs in the thread assigned to the
 * sink and the sink will block during the write request.
 *
 * <p>If a stale locator exception is received by the sink in response to a write, it will: (a)
 * notify the client of the stale locator exception; (b) close the input queue, setting the stale
 * locator exception as the cause; (c) place the chunk for that write onto the master's (unbounded)
 * redirect queue; and (d) drain its input queue and transfer the chunks to the master's redirect
 * queue.
 *
 * <p>If the master attempts to transfer a chunk to the input queue for a sink which has closed its
 * input queue in response to a stale locator exception, then an exception will be thrown with the
 * stale locator exception as the inner cause. The master will trap that exception and place the
 * chunk on the redirect queue instead.
 *
 * <p>If the sink RMI is successful, the sink will invoke the optional result handler and touch each
 * tuple in the chunk using KVO#done(). These protocols can be used to pass results from
 * asynchronous writes back to the application.
 *
 * @param <H> The generic type of the value returned by {@link Callable#call()} for the master.
 * @param <E> The generic type of the elements in the chunks stored in the {@link BlockingBuffer}.
 * @param <S> The generic type of the subtask implementation class.
 * @param <L> The generic type of the locator object used to lookup a subtask in the internal map
 *     (must be unique and must implement hashCode() and equals() per their contracts).
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @see ISplitter
 * @todo Update javadoc to reflect that the master no longer waits for a closed sink in {@link
 *     #getSink(Object, boolean)} but instead places the sink onto the {@link
...
public abstract class AbstractMasterTask<
   ...
   *
   * <p>This design allows us to avoid deadlocks when a sink is full and the master is blocked
   * attempting to add another chunk to that sink. If a {@link StaleLocatorException} is thrown for
   ...
   */
  protected final void redirectChunk(final E[] chunk) throws InterruptedException {
   ...
   * #drainFutures()}. If a {@link Future} reports an error, then the master is halted. This is how
   * we ensure that all subtasks complete normally.
   *
   * @param subtask The subtask.
   * @throws InterruptedException
   */
  protected void notifySubtaskDone(final AbstractSubtask subtask) throws InterruptedException {

    if (subtask == null) throw new IllegalArgumentException();

    if (subtask.buffer.isOpen()) throw new IllegalStateException();
   ...
   * @param sinkIdleTimeoutNanos The time in nanoseconds after which an idle sink will be closed.
   *     Any buffered writes are flushed when the sink is closed. This must be GTE the
   *     <i>sinkChunkTimeout</i> otherwise the sink will decide that it is idle when it was just
   *     waiting for enough data to prepare a full chunk.
   * @param sinkPollTimeoutNanos The time in nanoseconds that the {@link AbstractSubtask sink} will
   *     wait inside of the {@link IAsynchronousIterator} when it polls the iterator for a chunk. If
   *     this value is too large then the sink will block for noticeable lengths of time and will be
   *     less responsive to interrupts. Something in the 10s of milliseconds is appropriate.
   * @todo sinkQueueCapacity, sinkChunkSize, and sinkChunkTimeoutNanos should be arguments for this
   *     class and a default {@link #newSubtaskBuffer()} implementation should be provided. The unit
   *     tests for the {@link AbstractMasterTask} need to be updated for that change, as do the
   *     other derived classes.
   */
  public AbstractMasterTask(
      final H stats,
    ...
    if (sinkIdleTimeoutNanos <= 0) throw new IllegalArgumentException();

    if (sinkPollTimeoutNanos <= 0) throw new IllegalArgumentException();
    ...
    try {
      ...
      while (true) {
        ...
        if (a == null) {
          ...
          if (src.hasNext(buffer.getChunkTimeout(), TimeUnit.NANOSECONDS)) {
       ...
       * If the master's input buffer is closed and has
          ...
          if (log.isInfoEnabled()) log.info("Read chunk from redirectQueue");
        }
   ...
   *
   * @param chunk A chunk.
   * @param reopen When <code>false</code> it is an error if the output buffer has been closed. When
   *     <code>true</code> the output buffer will be (re-)opened as necessary. This will be <code>
   *     false</code> when invoked by {@link #call()} (since the output buffers are not closed until
   *     the master's buffer is closed) and should be <code>true</code> if you are handling
   *     redirects.
   */
   ...
   * queried while holding the {@link #lock}. A true return indicates that there are no pending work
   * items. The default implementation returns <code>true</code>.
   *
   * <p>The work perform by the client must be idempotent (it must be safe to re-perform the
   * operation). Pending work items may be in an unknown state, the master may submit the same work
   * item to multiple clients (where that is permitted by the locator semantics), the client task
   * may fail before the work item is complete, and a failed client can cause work items associated
   * with that client to be posted to another client.
   *
   * <p>To handle master termination, the pending set must track outstanding work items. Those work
   * item should be removed from the pending set as soon as any client has successfully completed
   * that work item (since the work is idempotent).
   *
   * <p>To handle client failure, the subtask must track the pending set for its client. If the
   * client dies, then the subtask must handle the client by placing all pending work items for that
   * client (including any in the chunk for the current request) onto the {@link #redirectQueue}.
   */
   ...
   * atomically.
   *
   * @throws ExecutionException This will report the first cause.
   * @throws InterruptedException If interrupted while awaiting the {@link #lock} or the child
   *     tasks.
   * @todo The test suite should include a case where a set of redirects appear just as the sinks
   *     are processing their last chunk.
   */
  private void awaitAll() throws InterruptedException, ExecutionException {
    ...
    while (true) {
       ...
       * We are done since there are no running sinks, and no
            ...
            if (log.isInfoEnabled()) log.info("All subtasks are done: " + this);
       ...
       * Check for sinks which have finished.
          ...
          if (log.isDebugEnabled())
            log.debug("Waiting for " + sinks.size() + " subtasks : " + this);
   ...
   *
   * <p>Note: The caller should have already invoked {@link #halt(Throwable)}.
   */
  private void cancelAll(final boolean mayInterruptIfRunning) throws InterruptedException {
   ...
   *
   * <p>Note: The caller is single threaded since this is invoked from the master's thread. This
   * code depends on that assumption.
   *
   * @param locator The locator (unique subtask key).
   * @param reopen <code>true</code> IFF a closed buffer should be re-opened (in fact, this causes a
   *     new buffer to be created and the new buffer will be drained by a new {@link
   *     IndexPartitionWriteTask}).
   * @return The sink for that locator.
   * @throws IllegalArgumentException if the argument is <code>null</code>.
   * @throws InterruptedException if interrupted.
   * @throws RuntimeException if {@link #halted()}
   */
  protected S getSink(final L locator, final boolean reopen) throws InterruptedException {
    ...
    try {
       ...
       * Start subtask.
         ...
         */
        {
          ...
          final FutureTask<? extends AbstractSubtaskStats> ft = new FutureTask(sink);
   ...
   * @param out The {@link BlockingBuffer} on which the master will write for that subtask.
   * @return The subtask.
   */
...
//     * Submit the subtask to an {@link Executor}.
  ...
  private void drainFutures() throws InterruptedException, ExecutionException {
    ...
    while (true) {
      ...
      if (sink.buffer.isOpen()) throw new IllegalStateException(sink.toString());
       ...
       * Increment this counter immediately when the subtask is done
        ...
        if (log.isDebugEnabled()) log.debug("subtaskEndCount incremented: " + sink.locator);
      }
    }
  }
   ...
   *
   * <p>This is done atomically using the {@link #lock}. This is invoked both by {@link
   * AbstractSubtask#call()} (when it is preparing to exit call()) and by {@link #getSink(Object,
   * boolean)} (when it discovers that the sink for a locator is closed, but not yet finished with
   * its work).
   *
   * <p>Note: The sink MUST be atomically transferred from {@link #sinks} to {@link
   * #finishedSubtaskQueue} and {@link #awaitAll()} MUST verify that both are empty in order for the
   * termination condition to be atomic.
   *
   * @param locator The locator.
   * @param sink The sink.
   * @todo this method really should be private. It is exposed for one of the unit tests.
   */
  protected void moveSinkToFinishedQueueAtomically(final L locator, final AbstractSubtask sink)
      throws InterruptedException {
    ...
    try {
      ...
      if (sinks.remove(locator, sink)) {
        ...
        if (log.isDebugEnabled()) log.debug("Removed output buffer: " + locator);
      }
   ...
   * output buffer.
   *
   * @param split The {@link Split} identifies both the tuples to be dispatched and the {@link
   *     PartitionLocator} on which they must be written.
   * @param a The array of tuples. Only those tuples addressed by the <i>split</i> will be written
  ...
  protected void addToOutputBuffer(
      final L locator, final E[] a, final int fromIndex, final int toIndex, boolean reopen)
      throws InterruptedException {
    ...
    final E[] b = (E[]) java.lang.reflect.Array.newInstance(a.getClass().getComponentType(), n);
    ...
    while (!added) {
      ...
      try {
        ...
        added = sink.buffer.add(b, offerWarningTimeoutNanos, TimeUnit.NANOSECONDS);
  ...
  private static final long offerWarningTimeoutNanos = TimeUnit.MILLISECONDS.toNanos(5000);
}
