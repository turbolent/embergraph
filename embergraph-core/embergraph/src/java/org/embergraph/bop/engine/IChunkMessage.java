 ...
 * ResourceService}.
 *
 * @param <E> The generic type of the elements in the chunk (binding sets, elements from a relation,
 *     etc).
...
public interface IChunkMessage<E> extends IOpMessage {
   ...
   *     cluster. The message format is slimmer if we instead rely on resolution of {@link
   *     #getQueryControllerId()} against a service discovery cache.
   *     <p>We can not discover the query controller using river because it is not registered as a
   *     service. We can find the query peers on the data service nodes easily enough because they
   *     are all registered with river. However, the QueryEngine serving as the query controller is
   *     not currently registered with river and hence it can not be discovered using the UUID of
   *     the query controller alone. Probably the right thing to do is to register the query
   *     controller with river so it can be discovered. We could then modify getQueryPeer() (or add
   *     getQueryClient(UUID)) which would hit the discovery cache.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/475">Optimize serialization for
   *     query messages on cluster</a>
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/487">The query controller should
   *     be discoverable</a>
   */
   ...
   * run (depending on the {@link BOpEvaluationContext}). When so invoked, the {@link IChunkMessage}
   * will be associated with an empty source and this flag will be set.
   *
