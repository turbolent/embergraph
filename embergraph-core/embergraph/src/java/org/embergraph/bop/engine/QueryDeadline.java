 ...
 * queries that will expire soonest are first, those that can run longer come later. This ordering
 * is used for a priority queue.
 *
 * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/772">Query timeout only checked at
 *     operator start/stop. </a>
...
class QueryDeadline implements Comparable<QueryDeadline> {
   ...
   * ordered last.
   *
   * <p>Note: A natural order based on deadline was added to support timely termination of compute
   * bound queries that exceed their deadline.
   *
   ...
   *
   * @param nowNanosIsIgnored A current timestamp.
   * @return <code>null</code> if the query is terminated and <code>this</code> if the query is not
   *     terminated.
   */
