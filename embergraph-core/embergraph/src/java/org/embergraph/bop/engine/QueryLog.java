...
import org.embergraph.bop.joinGraph.rto.PathIds;
...
public class QueryLog {
   ...
   */
  public static void log(final IRunningQuery q) {
    ...
    try {
       ...
       * log(IRunningQuery) is ONLY invoke by AbstractRunningQuery.cancel() and then only runs IFF
       * QueryLog is @ INFO. Since this is a rare combination, allocating a new StringBuilder object
       * here will not have an adverse impact on the heap and avoids the possibility of a deadlock.
       *
       * @see <a href="http://trac.blazegraph.com/ticket/992" > Deadlock between
       *     AbstractRunningQuery.cancel(), QueryLog.log(), and ArbitraryLengthPathTask</a>
       */
      ...
      //                synchronized (sb)
      {
...

        {
          final Map<Integer /* bopId */, QueueStats> queueStats =
              ((ChunkedRunningQuery) q).getQueueStats();
  ...
  private static void logDetailRows(
    ...
    for (Integer bopId : order) {
      ...
      sb.append(getTableRow(q, orderIndex, bopId, false /* summary */, queueStats));
  ...
  private static void logSummaryRow(
      final IRunningQuery q,
      final Map<Integer /* bopId */, QueueStats> queueStats,
      final StringBuilder sb) {

    sb.append(
        getTableRow(q, -1 /* orderIndex */, q.getQuery().getId(), true /* summary */, queueStats));
  ...
  private static String getTableRow(
    ...
    sb.append('\t');
    if (q.getDeadline() != Long.MAX_VALUE) sb.append(dateFormat.format(new Date(q.getDeadline())));
    sb.append('\t');
    sb.append(elapsed);
    sb.append('\t');
    ...
    final IPredicate pred = (IPredicate<?>) bop.getProperty(PipelineJoin.Annotations.PREDICATE);
    ...
    if (pred != null) {
      ...
      while (itr.hasNext()) {
        ...
        final IVariableOrConstant<?> x = (IVariableOrConstant<?>) itr.next();
        if (x.isVar()) {
          sb.append("?");
          sb.append(x.getName());
        } else {
        ...
        for (int i = 0; i < refs.length; i++) {
          ...
          if (t != null) {
            ...
            final IHashJoinUtility state =
                (IHashJoinUtility) (attrs == null ? null : attrs.get(ref));
            if (state != null) {
              // Prefer the IHashUtilityState
              sb.append(state.toString());
    ...
    if (bop instanceof ChunkedMaterializationOp) {
      sb.append(
          cdata(
              "vars="
                  + Arrays.toString(((ChunkedMaterializationOp) bop).getVars())
     ...
     */
    {
      if (pred != null) {
        ...
        final IKeyOrder<?> keyOrder =
            (IKeyOrder<?>) pred.getProperty(AST2BOpJoins.Annotations.ORIGINAL_INDEX);

        // Explicit override of the key order (if given).
        final Object overrideKeyOrder = pred.getProperty(IPredicate.Annotations.KEY_ORDER);

        final Long rangeCount =
            (Long) pred.getProperty(AST2BOpJoins.Annotations.ESTIMATED_CARDINALITY);

        sb.append('\t'); // keyorder
        if (keyOrder != null) sb.append(keyOrder);

        sb.append('\t'); // keyorder override.
        if (overrideKeyOrder != null) sb.append(overrideKeyOrder.toString());

        sb.append('\t'); // nvars
        if (keyOrder != null) sb.append(pred.getVariableCount(keyOrder));

        sb.append('\t'); // rangeCount
        if (rangeCount != null) sb.append(rangeCount);
    ...
    final long numRunning = ((AbstractRunningQuery) q).getRunningCount(bopId);
    ...
    if (bop != null) {
      ...
      final RunStateEnum runState = ((AbstractRunningQuery) q).tryGetRunState(bopId);
   ...
   *
   * @param queryStr The original text of the query (e.g., a SPARQL query) (optional).
   * @param q The {@link IRunningQuery}.
   * @param children The child query(s) -or- <code>null</code> if they are not to be displayed.
   * @param w Where to write the table.
   * @param summaryOnly When <code>true</code> only the summary row will be written.
   * @param maxBopLength The maximum length to display from {@link BOp#toString()} and ZERO (0) to
   *     display everything. Data longer than this value will be accessible from a flyover, but not
   *     directly visible in the page.
   * @param clusterStats When <code>true</code>, provides additional statistics that are specific to
   *     the scale-out cluster architecture.
   * @param detailedStats When <code>true</code>, provides additional query statistics that are
   *     below the level of detail for most people.
   * @param mutationStats When <code>true</code>, provides additional statistics about mutations
   *     (only report for inference rules at this time).
   * @throws IOException
   */
  public static void getTableXHTML(
      ...
      final boolean clusterStats,
      final boolean detailedStats,
      final boolean mutationStats)
      throws IOException {
    ...
    // Main query.
    {
      ...
      // Summary first.
      getSummaryRowXHTML(
          queryStr, q, w, queueStats, maxBopLength, clusterStats, detailedStats, mutationStats);
  ...
  public static void getTableHeaderXHTML(
      final Writer w,
      final boolean clusterStats,
      final boolean detailedStats,
      final boolean mutationStats)
    ...
    if (detailedStats) {
      w.write("<th>");
      w.write(cdata("solutions/ms"));
      w.write("</th>");
    }
    if (mutationStats) {
      w.write("<th>");
      w.write(cdata("mutations/ms"));
      w.write("</th>");
    }
   ...
   * @param q The {@link IRunningQuery}.
   * @param w Where to write the rows.
   * @param maxBopLength The maximum length to display from {@link BOp#toString()} and ZERO (0) to
   *     display everything. Data longer than this value will be accessible from a flyover, but not
   *     directly visible in the page.
   * @param clusterStats When <code>true</code>, provides additional statistics that are specific to
   ...
   * @throws IOException
   */
  public static void getTableRowsXHTML(
      final String queryStr,
      final IRunningQuery q,
      final Writer w,
      final Map<Integer /* bopId */, QueueStats> queueStats,
   ...
   * @return The row of the table.
   */
  private static void getTableRowXHTML(
      final String queryStr,
      final IRunningQuery q,
      final Writer w,
      final int evalOrder,
    ...
    if (summary) {
      ...
      if (clusterStats) {
        w.write(TD);
        w.write(TDx); // evalContext
      }
      if (detailedStats) {
        w.write(TD);
        w.write(TDx); // controller
        w.write(TD);
        w.write("total"); // bopId
        w.write(TDx);
      }
      ...
      if (detailedStats) {
        ...
        w.write(
            cdata(
                bop.getProperty(BOp.Annotations.CONTROLLER, BOp.Annotations.DEFAULT_CONTROLLER)
                    .toString()));
        w.write(TDx);
        w.write(TD);
    ...
    final IPredicate pred =
        summary ? null : (IPredicate<?>) bop.getProperty(PipelineJoin.Annotations.PREDICATE);
    final Integer predId = pred == null ? null : (Integer) pred.getProperty(BOp.Annotations.BOP_ID);
    if (detailedStats) {
      w.write(TD);
      if (predId != null) {
    ...
    if (!summary) {
      ...
      if (bop instanceof GroupByOp) {
        ...
        if (detailedStats) {
          ...
          w.write(cdata(((GroupByOp) bop).getGroupByRewrite().toString()));
        }
      }
      if (bop instanceof DropOp) {
      ...
      if (bop instanceof ConditionalRoutingOp) {
        w.write(cdata(((ConditionalRoutingOp) bop).getCondition().toString()));
      }
      if (bop instanceof JoinGraph) {
        final JoinGraph t = ((JoinGraph) bop);
        final Path p = t.getPath(q);
        final Map<PathIds, EdgeSample> samples = t.getSamples(q);
      ...
      if (bop instanceof ProjectionOp) {
        w.write(cdata(Arrays.toString(((ProjectionOp) bop).getVariables())));
      }
     ...
     */
    {
      if (pred != null) {
        ...
        if (detailedStats) {
          ...
          w.write(TD);
          if (keyOrder != null) w.write(keyOrder.toString());
          w.write(TDx);

          // override keyorder
          w.write(TD);
          if (overrideKeyOrder != null) w.write(overrideKeyOrder.toString());
          w.write(TDx);
        }

        // nvars
        w.write(TD);
        if (keyOrder != null) w.write(Integer.toString(pred.getVariableCount(keyOrder)));
        w.write(TDx);

        // rangeCount
        w.write(TD);
        if (rangeCount != null) w.write(Long.toString(rangeCount));
        w.write(TDx);
    ...
    if (detailedStats) {
      ...
      w.write(cdata(elapsed == 0 ? "0" : Long.toString(stats.unitsOut.get() / elapsed)));
      w.write(TDx);
    }
    // mutations/ms
    if (mutationStats) {
      w.write(TD);
      w.write(cdata(elapsed == 0 ? "0" : Long.toString(stats.mutationCount.get() / elapsed)));
      w.write(TDx);
    }
    w.write("</tr\n>");
  }
   ...
   * @param anns The annotations (optional).
   * @throws IOException
   */
  private static void showAnnotations(final Writer w, final Map<String, Object> anns)
      throws IOException {
   ...
   * @throws IOException
   */
  private static void getSummaryRowXHTML(
      final String queryStr,
      final IRunningQuery q,
      final Writer w,
      final Map<Integer /* bopId */, QueueStats> queueStats,
   ...
   * @param y The denomerator.
   * @return The average.
   */
  private static double avg(final long x, final long y) {
   ...
   * @param queryId The {@link UUID} of the desired query.
   * @return The {@link IRunningQuery} iff it can be found and otherwise <code>null</code>.
   */
