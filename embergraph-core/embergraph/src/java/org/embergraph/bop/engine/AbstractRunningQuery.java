 ...
 * query.
 *
 * <p>There are common requirements for the {@link IRunningQuery}, but a variety of ways in which
 * those requirements can be met. Among the common requirements are a means to manage tradeoffs in
 * the allocation of various resources to the operators in each query. Some of the more important
 * tradeoffs are the #of threads to allocate to each operator (threads bounds IO for Java 6 since we
 * are using a synchronous IO model) and the amount of RAM allocated to each operator (including RAM
 * on the JVM heap and RAM on the native Java process heap). If the #of threads is too restrictive,
 * then queries will progress slowly due to insufficient IO level parallelism. If the query buffers
 * too much data on the JVM heap, then it can cause GC overhead problems that can drastically reduce
 * the responsiveness and throughput of the JVM. Data can be moved off of the JVM heap onto the Java
 * process heap by serializing it into <em>direct</em> {@link ByteBuffer}s. This can be very
 * efficient in combination with hash joins at the expense of increasing the latency to the first
 * result when compared with pipelined evaluation.
 *
...
public abstract class AbstractRunningQuery implements IRunningQuery {
  ...
  /*
   * Error message used when an operation which must be performed on the query controller is
   * attempted on some other {@link IQueryPeer}.
   */
  protected static final String ERR_NOT_CONTROLLER =
      "Operator only permitted on the query controller";
  ...
  /*
   * Error message used when a request is addressed to an operator other than the head of the
   * pipeline in a context where the request must be addressed to the operator at the head of the
   * pipeline (e.g., when presenting the initial binding sets to get the query moving.)
   */
  protected static final String ERR_NOT_PIPELINE_START = "Not pipeline start";

  /*
   * Error message used when no operator can be found for a given {@link BOp.Annotations#BOP_ID}.
   */
  protected static final String ERR_NO_SUCH_BOP = "No such bop: id=";

  /** Error message used when two operators have the same {@link BOp.Annotations#BOP_ID}. */
  ...
  /** The timestamp (ms) when the query is done executing and ZERO (0L) if the query is not done. */
  private final AtomicLong doneTime = new AtomicLong(0L);

  /** <code>true</code> iff the outer {@link QueryEngine} is the controller for this query. */
  private final boolean controller;

  /*
   * The client executing this query (aka the query controller).
   *
   * <p>Note: The proxy is primarily for light weight RMI messages used to coordinate the
   * distributed query evaluation. Ideally, all large objects will be transfered among the nodes of
   * the cluster using NIO buffers.
   */
  private final IQueryClient clientProxy;

  /*
   * The original message which kicked off this query on the query controller. This is NOT required
   * when the query is materialized on another node and MAY be <code>null</code>, but the original
   * message used to kick off the query on the query controller MUST be provided so we can ensure
   * that the source iteration is always closed when the query is cancelled.
   */
  ...
  /*
   * An index from the {@link BOp.Annotations#BOP_ID} to the {@link BOp}. This index is generated by
   * the constructor. It is immutable and thread-safe.
   */
   ...
   *
   * <p>Note: This is exposed to the {@link QueryEngine} to let it cache the {@link Future} for
   * recently finished queries.
   */
  ...
  /*
   * The runtime statistics for each {@link BOp} in the query and <code>null</code> unless this is
   * the query controller.
   */
  private final ConcurrentHashMap<Integer /* bopId */, BOpStats> statsMap;

  /*
   * The buffer used for the overall output of the query pipeline.
   *
   * <p>Note: This only exists on the query controller, and then only when the top-level operator is
   * not a mutation. In order to ensure that the results are transferred to the query controller in
   * scale-out, the top-level operator in the query plan must specify {@link
   * BOpEvaluationContext#CONTROLLER}. For example, {@link SliceOp} or {@link EndOp} both require
   * this {@link BOpEvaluationContext}.
   */
  private final IBlockingBuffer<IBindingSet[]> queryBuffer;

  /*
   * The iterator draining the {@link #queryBuffer} and <code>null</code> iff the {@link
   * #queryBuffer} is <code>null</code>.
   */
  ...
  /*
   * A lock guarding various state changes. This guards changes to the internal state of the {@link
   * #runState} object. It is also used to serialize requests to {@link #acceptChunk(IChunkMessage)}
   * and {@link #cancel(boolean)} and make atomic decision concerning whether to attach a new {@link
   * IChunkMessage} to an operator task which is already running or to start a new task for that
   * message.
   *
   * @see RunState
   */
  ...
  /** The run state of this query and <code>null</code> unless this is the query controller. */
  private final RunState runState;

  /*
   * Flag used to prevent retriggering of query tear down activities in {@link #cancel(boolean)}.
   */
  ...
  //     */
  //    private final Map<Integer/* bopId */, AtomicBoolean> tornDown = new LinkedHashMap<Integer,
  // AtomicBoolean>();

  /*
   * Set the query deadline. The query will be cancelled when the deadline is passed. If the
   * deadline is passed, the query is immediately cancelled.
   *
   * @param deadline The deadline.
   * @throws IllegalArgumentException if the deadline is non-positive.
   * @throws IllegalStateException if the deadline was already set.
   * @throws UnsupportedOperationException unless node is the query controller.
   */
  public final void setDeadline(final long deadline) {
  ...
  public final long getElapsed() {
    ...
    if (mark == 0L) mark = System.currentTimeMillis();

    return mark - startTime.get();
  }

  /*
   * Return the buffer used for the overall output of the query pipeline and <code>null</code> if
   * this is not the query controller.
   */
  ...
  @Override
  public final Map<Integer /* bopId */, BOpStats> getStats() {

    return Collections.unmodifiableMap(statsMap);
  }

  /*
   * Return the {@link BOpStats} instance associated with the given {@link BOp} identifier.
   *
   * @param bopId The {@link BOp} identifier.
   * @return The associated {@link BOpStats} object -or- <code>null</code> if there is no entry for
   *     that {@link BOp} identifier.
   * @throws IllegalArgumentException if the argument is <code>null</code>.
   */
  public final BOpStats getStats(final Integer bopId) {

    if (bopId == null) throw new IllegalArgumentException();

    if (statsMap == null)
      throw new IllegalStateException("bopId=" + bopId + ", query=" + BOpUtility.toString(query));
   ...
   * @param bopId The {@link BOp} identifier.
   * @return The {@link BOp}.
   * @throws IllegalArgumentException if there is no {@link BOp} with that identifier declared in
   *     this query.
   */
  public final BOp getBOp(final int bopId) {

    final BOp bop = getBOpIndex().get(bopId);

    if (bop == null) {

      throw new IllegalArgumentException("Not found: id=" + bopId + ", query=" + query);
    }
   ...
   * @param controller <code>true</code> iff the {@link QueryEngine} is the query controller for
   *     this query (the {@link QueryEngine} which will coordinate the query evaluation).
   * @param clientProxy The query controller. In standalone, this is the same as the
   *     <i>queryEngine</i>. In scale-out, this is an RMI proxy for the query controller whenever
   *     the query is instantiated on a node other than the query controller itself.
   * @param query The query.
   * @param realSource The original message which kicked off this query on the query controller.
   *     This is NOT required when the query is materialized on another node and MAY be <code>null
   *     </code>, but the original message used to kick off the query on the query controller MUST
   *     be provided so we can ensure that the source iteration is always closed when the query is
   *     cancelled.
   * @throws IllegalArgumentException if any argument is <code>null</code>.
   * @throws IllegalArgumentException if the <i>readTimestamp</i> is {@link ITx#UNISOLATED} (queries
   *     may not read on the unisolated indices).
   * @throws IllegalArgumentException if the <i>writeTimestamp</i> is neither {@link ITx#UNISOLATED}
   *     nor a read-write transaction identifier.
   */
  public AbstractRunningQuery(
  ...
  /*
   * Return the buffer that will be used to absorb solutions. The solutions will be drained from the
   * buffer using its iterator.
   *
   * @param query The root of the query plan.
   * @param queryStats Used to track statistics on the solutions to the query (#of chunks, #of
   *     units).
   * @return The buffer.
   */
  ...
  /*
   * Pre-populate a map with {@link BOpStats} objects for the query. Only the child operands are
  ...
  private void populateStatsMap(final BOp op) {
    ...
    if (!(op instanceof PipelineOp)) return;
  ...
  /*
   * Message provides notice that the query has started execution and will consume some specific
   * number of binding set chunks.
   *
   * @param msg The initial message presented to the query. The message is used to update the query
   *     {@link RunState}. However, the message will not be consumed until it is presented to {@link
   *     #acceptChunk(IChunkMessage)} by the {@link QueryEngine} .
   * @throws UnsupportedOperationException If this node is not the query coordinator.
   */
  protected final void startQuery(final IChunkMessage<IBindingSet> msg) {
  ...
  /*
   * Message provides notice that the operator has started execution and will consume some specific
   * number of binding set chunks.
   *
   * @param msg The {@link IStartOpMessage}.
   * @throws UnsupportedOperationException If this node is not the query coordinator.
   */
  protected final void startOp(final IStartOpMessage msg) {
  ...
  /*
   * Message provides notice that the operator has ended execution. The termination conditions for
   * the query are checked. (For scale-out, the node controlling the query needs to be involved for
   * each operator start/stop in order to make the termination decision atomic).
   *
   * @param msg The {@link IHaltOpMessage}
   * @throws UnsupportedOperationException If this node is not the query coordinator.
   */
  ...
  /*
   * Method handles the case where there are downstream operators awaiting last pass evaluation or
   * at-once evaluation is not re-triggered by the last {@link IChunkMessage} output from an
   * upstream operator. If this situation arises the query will just sit there waiting for a trigger
   * to kick of last pass evaluation. This method works around that by sending an empty {@link
   * IChunkMessage} if the operator would not otherwise have been triggered.
   *
   * @param msg
  ...
  private void triggerOperatorsAwaitingLastPass() {
    ...
    for (Integer bopId : runState.getLastPassRequested()) {
      ...
      if (runState.getOperatorRunState(bopId) == RunStateEnum.StartLastPass) {
        ...
        if (log.isInfoEnabled()) log.info("Triggering last pass: " + bopId);
  ...
  /*
   * Queue empty {@link IChunkMessage}s to trigger the last evaluation pass for an operator which
   * can not be re-triggered by any upstream operator or by {@link IChunkMessage}s which have
   * already been buffered.
   *
   * <p>Note: If the queue for accepting new chunks could block then this could deadlock. We work
   * around that by using the same lock for the AbstractRunningQuery and the queue of accepted
   * messages. If the queue blocks, this thread will be yield the lock and another thread may make
   * progress.
   *
   * @param msg
   * @param doneOn The collection of shards or services on which the operator need to receive a last
   *     evaluation pass message.
   */
  @SuppressWarnings("rawtypes")
  protected void doLastPass(final int bopId, final Set doneOn) {
    ...
    if (doneOn == null) {
       ...
       */
      throw new AssertionError("doneOn is null? : bopId=" + bopId + ", runState=" + runState);
    }
    ...
    if (doneOn.isEmpty()) {
       ...
       * AllDone).
       */
      throw new AssertionError("doneOn is empty? : bopId=" + bopId + ", runState=" + runState);
    }
    ...
    if (doneOn.size() != 1) {
       ...
       */
      throw new AssertionError(
          "doneOn set not single element? : bopId="
              + bopId
              + ", runState="
              + runState
              + ", doneOn="
  ...
  /*
   * Return <code>true</code> iff the preconditions have been satisfied for the "at-once" invocation
   * of the specified operator (no predecessors are running or could be triggered and the operator
   * has not been evaluated).
   *
   * @param bopId Some operator identifier.
  ...
  /*
   * Attempt to return the {@link RunStateEnum} for an operator (non-blocking).
   *
   * <p>Note: This method is intended for use in contexts where it is desirable, but not critical,
   * to have the {@link RunStateEnum} for the operator. For example, in log messages. The
   * implementation is non-blocking and will barge in if the lock is available and return the {@link
   * RunStateEnum} of the operator. If the lock is not available, it will return <code>null</code>.
   *
   * @param bopId The operator.
   * @return It's {@link RunStateEnum} and <code>null</code> if the lock could not be acquired.
   */
  ...
  /*
   * Release native memory associated with this operator, if any (NOP, but overridden in scale-out
   * to release NIO buffers used to move solutions around in the cluster).
   *
   * <p>Note: Operators are responsible for releasing their child {@link IMemoryManager} context, if
   * any, when they terminate and should specify the {@link PipelineOp.Annotations#LAST_PASS}
   * annotation to receive notice in the form of a final evaluation pass over an empty {@link
   * IChunkMessage}. If they do NOT release an {@link IMemoryManager} context which is a child of
   * the {{@link #getMemoryManager() query's context}, then their child {@link IMemoryManager}
   * context will be retained until the termination of the query, at which point the query's {@link
   * IMemoryManager} context will be release, and all child contexts will be released automatically
   * along with it.
   *
   * @param bopId
   ...
   *
   * <p>FIXME This could cause direct buffers to be released back to the pool before the operator
   * tasks have terminated. That is NOT safe as the buffers could then be reissued to other threads
   * while existing threads still have references to the buffers. Really, the same problem exists
   * with the allocation contexts used for NIO transfers of IBindingSet[]s.
   *
   * <p>We will have to be very careful to wait until each operator's Future isDone() before calling
   * clear() on the IMemoryManager to release the native buffers back to the pool. If we release a
   * buffer while an operator is still running, then we will get data corruption arising from the
   * recycling of the buffer to another native buffer user.
   *
   * <p>AbstractRunningQuery.cancel(...) is where we need to handle this, more specifically
   * cancelRunningOperators(). Right now it is not waiting for those operators to terminate.
   *
   * <p>Making this work is tricky. AbstractRunningQuery is holding a lock. The operator tasks do
   * not actually require that lock to terminate, but they are wrapped by a ChunkWrapperTask, which
   * handles reporting back to the AbstractRunningQuery and *does* need the lock, and also by a
   * ChunkFutureTask. Since we actually do do ChunkFutureTask.get(), we are going to deadlock if we
   * invoke that while holding the AbstractRunningQuery's lock.
   *
   * <p>The alternative is to handle the tear down of the native buffers for a query asynchronously
   * after the query has been cancelled, deferring the release of the native buffers back to the
   * direct buffer pool until all tasks for the query are known to be done.
   *
   * @see BLZG-1658 MemoryManager should know when it has been closed
   *     <p>FIXME We need to have distinct events for the query evaluation life cycle and the query
   *     results life cycle. Really, this means that temporary solution sets are scoped to the
   *     parent query. This is a matter of the scope of the allocation context for the {@link
   *     DirectBufferPoolAllocator} and releasing that scope when the parent query is done (in
   *     cancel()). [Also consider scoping the temporary solution sets to a transaction or an HTTP
   *     session, e.g., by an integration with the NSS using traditional session concepts.]
   */
   ...
   * @todo Reconcile {@link #acceptChunk(IChunkMessage)} and {@link #consumeChunk()}. Why {@link
   *     #consumeChunk()} is also used by the {@link QueryEngine}.
   */
  protected abstract boolean acceptChunk(final IChunkMessage<IBindingSet> msg);

  /*
   * Instruct the {@link IRunningQuery} to consume an {@link IChunkMessage} already on its input
   * queue.
   */
  ...
  @Override
  public final void halt(final Void v) {
  ...
  @Override
  public final <T extends Throwable> T halt(final T t) {
   ...
   *
   * <ul>
   *   <li>must reject new chunks
   *   <li>must cancel any running operators
   *   <li>must not begin to evaluate operators
   *   <li>must release all of their resources
   *   <li>must not cause the solutions to be discarded before the client can consume them.
   * </ul>
   */
  @Override
  public final boolean cancel(final boolean mayInterruptIfRunning) {
    ...
    try {
      ...
      if (didQueryTearDown.compareAndSet(false /* expect */, true /* update */)) {
        ...
        if (isController()) QueryLog.log(this);
   ...
   */
  protected abstract boolean cancelRunningOperators(final boolean mayInterruptIfRunning);

  /*
   * Close the {@link IAsynchronousIterator} for any {@link IChunkMessage}s which have been
   * <em>accepted</em> for this queue on this node (internal API).
   *
   * <p>Note: This must be invoked while holding a lock which is exclusive with the lock used to
   * hand off {@link IChunkMessage}s to operator tasks otherwise we could wind up invoking {@link
   * IAsynchronousIterator#close()} from on an {@link IAsynchronousIterator} running in a different
   * thread. That would cause visibility problems in the close() semantics unless the {@link
   * IAsynchronousIterator} is thread-safe for close (e.g., volatile write, synchronized, etc.). The
   * appropriate lock for this is {@link AbstractRunningQuery#lock}. This method is only invoked out
   * of {@link AbstractRunningQuery#cancel(boolean)} which owns that lock.
   */
   ...
   *
   * <p>Note: The default implementation verifies that the caller is holding the {@link #lock} but
   * is otherwise a NOP. This is overridden for scale-out.
   *
   * @param cause When non-<code>null</code>, the cause.
   * @return <code>true</code> iff something was cancelled.
   * @throws IllegalMonitorStateException unless the {@link #lock} is held by the current thread.
   * @throws UnsupportedOperationException unless this is the query controller.
   */
  protected boolean cancelQueryOnPeers(
      final Throwable cause, final Set<UUID /*ServiceId*/> startedOn) {
  ...
  @Override
  public final Void get(final long arg0, final TimeUnit arg1)
  ...
  /*
   * Return the #of shards or nodes on which the operator has started evaluation. This is basically
   * a measure of the fan out of the operator across the cluster. For example, it will report the
   * #of shards on which a sharded join has read based on the solutions being mapped across that
   * join. The units are shards if the operator is sharded and nodes if the operator is hash
   * partitioned.
   *
  ...
  private MemoryManager newMemoryManager() {
    ...
    if (maxMemoryBytesPerQuery == 0) {
      ...
      nsectors = (int) Math.ceil(maxMemoryBytesPerQuery / (double) bufferCapacity);
    }
  ...
  /*
   * Report a snapshot of the known (declared) child {@link IRunningQuery}s for this {@link
   * IRunningQuery} and (recursively) for any children of this {@link IRunningQuery}.
   *
   * @return An array providing a snapshot of the known child {@link IRunningQuery}s and never
   *     <code>null</code>.
   */
   ...
   *
   * <p>Queries as submitted do not know about parent/child relationships
   *
   * @param childQuery The child query.
   * @return <code>true</code> if the child query was not already declared.
   */
  public final boolean addChild(final IRunningQuery childQuery) {
   ...
   *
   * <p>Note: Exposed for log messages in derived classes since {@link #runState} is private.
   */
  ...
  /*
   * Return <code>true</code> iff the root cause of the {@link Throwable} was an interrupt. This
   * checks for any of the different kinds of exceptions which can be thrown when an interrupt is
   * encountered.
   *
   * @param t The throwable.
   * @return <code>true</code> iff the root cause was an interrupt.
   *     <p>TODO This could be optimized by checking once at each level for any of the indicated
   *     exceptions.
   */
  public static boolean isRootCauseInterrupt(final Throwable t) {
