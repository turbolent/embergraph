...
public class BOpStats implements Serializable {
   ...
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/793">Explain reports incorrect
   *     value for opCount</a>
   */
   ...
   * causing any runtime exception which can be observed at the query level. This counter provides a
   * means to observe those errors.
   */
   ...
   * on an index. The per-index tuple mutation count MAY also be reported, but it should be reported
   * by extending this class and declaring additional mutation counters.
   *
   * <p>Note: Operations which fix point some closure depend on the correctness of the {@link
   * #mutationCount} for their termination condition.
   *
   * <p>Note: Mutation operations MUST NOT overwrite existing tuples with identical state. This is
   * important for two reasons. First, fix point operations depend on the {@link #mutationCount}
   * being ZERO (0) in a given round if nothing was changed in order to terminate the closure.
   * Second, due to the general MVCC and copy-on-write architecture, index writes drive IO. If the
   * tuple state would not be changed, then the index write SHOULD NOT be performed.
   */
   ...
   * AbstractRunningQuery#haltOp(IHaltOpMessage)}.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/793">Explain reports incorrect
   *     value for opCount</a>
   */
