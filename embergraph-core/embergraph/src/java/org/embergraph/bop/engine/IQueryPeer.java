...
public interface IQueryPeer extends Remote {
   ...
   *     unique and distinct UUID if the {@link QueryEngine} is not running against an
   ...
   *
   * @param queryDecl The query declaration.
   * @throws UnsupportedOperationException unless running in scale-out.
   * @deprecated This method is unused and will probably disappear. The nodes in a cluster reach
   *     back to the query controller using {@link IQueryClient#getQuery(UUID)} to resolve the query
   *     on its first reference. This is handled by the {@link FederatedQueryEngine} within its
   *     run() loop where it accepts {@link IChunkMessage}s.
   */
   ...
   * @param cause The cause. When <code>null</code>, this is presumed to be normal query
   *     termination.
   */
