...
public class LocalNativeChunkMessage implements IChunkMessage<IBindingSet> {
  ...
  public LocalNativeChunkMessage(
  ...
  public LocalNativeChunkMessage(
    ...
    if (queryContext == null) throw new IllegalArgumentException();

    if (bindingSetChunks == null) throw new IllegalArgumentException();
    ...
    if (bindingSetChunks.length == 0
        || (bindingSetChunks.length == 1 && bindingSetChunks[0].length == 0)) {
      ...
      final MemStore store =
          new MemStore(queryContext.getMemoryManager().createAllocationContext());

      final SolutionSetStream ssstr =
          new SolutionSetStream(store, checkpoint, metadata, false /* readOnly */);
  ...
  public String toString() {
    ...
    return getClass().getName()
        ...
        + partitionId
        + ", ssstr="
        + (ssstr == null ? "null" : ssstr.toString() + ":" + ssstr.getStats().toString())
        + "}";
  }
