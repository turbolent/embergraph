 ...
 * @version $Id$
 *     <p>TODO Large samples should be buffered on the {@link IMemoryManager} so they do not pose a
 *     burden on the heap. This will require us to manage the allocation contexts so we can release
 *     samples in a timely manner once they are no longer used and always release samples by the
 *     time the RTO is finished. [There is an additional twist if we have fully materialized some
 *     part of the join since we no longer need to evaluate that path segment. If the RTO can
...
public abstract class SampleBase {
   ...
   *
   * <p>TODO When using a non-perfect index, the estimated cardinality is only part of the cost. The
   * #of tuples scanned is also important. Even when scanning and filtering in key order this could
   * trigger random IOs unless the file has index order (an IndexSegment file has index order but a
   ...
   * against the sample rather than the disk by wrapping the sample as an inline access path.
   *
   * <p>TODO This field should be used to avoid needless re-computation of a join whose exact
   * solution is already known. We already do this within the runtime optimizer. To go further than
   * that we need to do the partial evaluation of the join graph.
   */
   ...
   * indicates that an access path (if it has filters) or a join may not have any solutions in the
   * data.
   */
   ...
   *     <p>TODO Wrap up as an {@link IChunkMessage} so we can store this on the native heap?
   */
  ...
  void releaseSample() {
    ...
    if (sampleRef.getAndSet(null) != null) {
      ...
      if (log.isTraceEnabled()) log.trace("Released sample: " + this);
    }
  }
   ...
   *     when making a cardinality estimate.
  ...
  public SampleBase(
    ...
    if (estimatedCardinality < 0L) throw new IllegalArgumentException();

    if (limit <= 0) throw new IllegalArgumentException();

    if (estimateEnum == null) throw new IllegalArgumentException();
