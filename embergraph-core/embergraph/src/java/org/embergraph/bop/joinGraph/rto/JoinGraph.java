 ...
 * relations), shared variables (which identify joins), and {@link IConstraint}s (which may reject
 * some solutions for those joins). Operators other than standard joins (including optional joins,
 * sort, order by, etc.) must be handled downstream from the join graph in a "tail plan".
 *
 * <p>The {@link JoinGraph} operator works in two phases. On its first invocation, it constructs a
 * {@link JGraph join graph} and identifies a join path having a low cost join ordering. This join
 * path is converted into a query plan and set as the {@link Attributes#QUERY_PLAN} attribute on the
 * {@link IRunningQuery}. The upstream solutions are then flooded into sub-query that executes the
 * chosen query plan. The solutions from the sub-query are simply copied to the output sink of the
 * {@link JoinGraph} operator. Once the query plan has been identified by the first invocation,
 * subsequent invocations of this operator simply push more data into the sub-query using the
 * pre-identified query plan.
 *
 * <p>TODO This approach amounts to bottom-up evaluation of the {@link JGraph}. Thus, the RTO is not
 * using information from the upstream query when it decides on a query plan. Therefore, we could
 * lift-out the RTO sections of the query into named subqueries, run them first in parallel, and
 * then INCLUDE their results into the main query. This would require an AST optimizer to modify the
 * AST. (Currently the RTO is integrated when the query plan is generated in {@link AST2BOpUtility}
 * rather than as an AST optimizer.)
 *
 * @see http://arxiv.org/PS_cache/arxiv/pdf/0810/0810.4809v1.pdf, XQuery Join Graph Isolation.
...
public class JoinGraph extends PipelineOp {
  ...
  public interface Annotations extends PipelineOp.Annotations {
     ...
     * <i>nedges</i> of those edges having the lowest cardinality are used to form the initial set
     * of join paths. For each edge selected to form a join path, the starting vertex will be the
  ...
  public Map<PathIds, EdgeSample> getSamples(final IRunningQuery q) {
    ...
    return (Map<PathIds, EdgeSample>) q.getAttributes().get(getId() + "-" + Attributes.SAMPLES);
  }
  ...
  private void setSamples(final IRunningQuery q, final Map<PathIds, EdgeSample> samples) {
  ...
  public PipelineOp getQueryPlan(final IRunningQuery q) {
    ...
    return (PipelineOp) q.getAttributes().get(getId() + "-" + Attributes.QUERY_PLAN);
  }

  private void setQueryPlan(final IRunningQuery q, final PipelineOp queryPlan) {
  ...
  public JoinGraph(final BOp[] args, final Map<String, Object> anns) {
    ...
    if (vertices == null) throw new IllegalArgumentException(Annotations.VERTICES);

    if (vertices.length == 0) throw new IllegalArgumentException(Annotations.VERTICES);

    if (getLimit() <= 0) throw new IllegalArgumentException(Annotations.LIMIT);

    if (getNEdges() <= 0) throw new IllegalArgumentException(Annotations.NEDGES);
    ...
    if (!isController()) throw new IllegalArgumentException();
      ...
      if (getQueryPlan(context.getRunningQuery()) == null) {
        ...
        final PipelineOp queryOp =
            AST2BOpRTO.compileJoinGraph(
                context.getRunningQuery().getQueryEngine(), JoinGraph.this, path);
   ...
   * have to take appropriate care to ensure that the results are copied out of the subquery and
   * into the parent query. See {@link AbstractSubqueryOp} for how this is done.
   */
  private static void runSubquery(
      final BOpContext<IBindingSet> parentContext, final PipelineOp queryOp) throws Exception {

    if (parentContext == null) throw new IllegalArgumentException();

    if (queryOp == null) throw new IllegalArgumentException();
    ...
    final IBindingSet[] bindingSets =
        BOpUtility.toArray(parentContext.getSource(), parentContext.getStats());
     ...
     */
    final IRunningQuery runningSubquery = queryEngine.eval(queryOp, bindingSets);
