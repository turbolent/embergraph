 ...
 * they arrive. The state is shared across invocations of the operator for each source chunk. The
 * operator waits until the last chunk has been consumed before writing the output solutions. In
 * order to observe the lastInvocation signal, the operator MUST be single threaded ( {@link
 * PipelineOp.Annotations#MAX_PARALLEL}:=1) and running on the query controller.
 *
 * <p>Note: Since this operator evaluates {@link IAggregate}s incrementally (one input solution at a
 * time), it relies on {@link IAggregate}'s contract for "sticky" errors. See {@link
 * IAggregate#get(IBindingSet)} and {@link IAggregate#done()}.
 *
 * <p>Note: This this operator will be invoked multiple times, and potentially on multiple nodes in
 * a cluster, it is critical that the anonymous variables assigned by the {@link GroupByRewriter}
 * are stable across all invocations on any node of the cluster (this caution also applies for a
 * single node where the operator can still be invoked multiple times).
 *
...
public class PipelinedAggregationOp extends GroupByOp implements ISingleThreadedOp {
   ...
   * evaluate aggregation requests which use DISTINCT or which nest {@link IAggregate}s in other
   * {@link IAggregate}s.
   */
  @Override
  ...
  public PipelinedAggregationOp(final BOp[] args, final Map<String, Object> annotations) {
  ...
  private static class SolutionGroup {
     ...
     *     <p>// * @return The new {@link SolutionGroup} -or- <code>null</code> if any // * of the
     *     value expressions evaluates to a <code>null</code> // * -OR- throws a {@link
  ...
  private static class SolutionGroupState {
     ...
     * @param bset The first input solution encountered for the group (the one which led to the
     *     group becoming defined).
     */
    SolutionGroupState(
   ...
   *
   * <p>Note: mutable fields on instances of this class are guarded by the monitor for the instance.
   */
  ...
  private static class SharedState {
     ...
     * invocation!!!
     *
     * <p>Note: This is only iff an explicit GROUP_BY clause is used.
     */
     ...
     * invocation!!!
     *
     * <p>Note: This is bound iff all solutions will be collected within a single implicit group.
     */
    private final LinkedHashMap<IAggregate<?>, IVariable<?>> aggExpr;
  ...
  private static class ChunkTask implements Callable<Void> {
    ...
    ChunkTask(final PipelinedAggregationOp op, final BOpContext<IBindingSet> context) {
      ...
      synchronized (stats) {
        if (stats.first) {
          ...
          context.getRunningQuery().getAttributes().put(sharedStateKey, sharedState);
        } else {
          sharedState = (SharedState) context.getRunningQuery().getAttributes().get(sharedStateKey);
        }
      } // synchronized(stats)
      /*
    ...
    private void accept(final IBindingSet bset) {
      ...
      if (m == null) {
        ...
        map.put(s, m = new SolutionGroupState(context, groupBy, rewrite.getAggExpr(), bset));
      }
      ...
      try {
        ...
        if (context.isLastInvocation()) {
          ...
          if (groupBy == null) {
             ...
             * that clause.
             *
             * <p>This evaluates the (rewritten) SELECT expressions. The rewritten HAVING clause (if
             * any) is then evaluated. If the solution is not dropped, then only the SELECTed
             * variables are projected out.
             */
            ...
            if (!drop) {
              ...
              final IBindingSet out =
                  aggregates.copy(groupByState.getSelectVars().toArray(new IVariable[0]));
            ...
            for (SolutionGroupState groupState : map.values()) {
              ...
              // Finalize and bind on [aggregates].
              finalizeAggregates(groupState.aggExpr, aggregates, stats);
              ...
              if (!drop) {
                ...
                final IBindingSet out =
                    aggregates.copyMinusErrors(
                        groupByState.getSelectVars().toArray(new IVariable[0]));
   ...
   *
   * @param aggExpr The aggregate expressions to be evaluated.
   ...
   *
   * @param aggExpr The aggregate expressions to be evaluated.
   * @param aggregates The binding set where the aggregates will become bound.
   */
  private static void finalizeAggregates(
      final LinkedHashMap<IAggregate<?>, IVariable<?>> aggExpr,
