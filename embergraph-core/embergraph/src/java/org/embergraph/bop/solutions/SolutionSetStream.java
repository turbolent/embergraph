 ...
 *
 * <p>TODO Test performance with and without gzip. Extract into the CREATE schema / IndexMetadata so
 * we can do this declaratively.
 */
public final class SolutionSetStream extends Stream implements ISolutionSet {
   ...
   * @see #needsCheckpoint()
   * @see #flush()
   *     <p>FIXME GIST : This is hacked into the bloomFilterAddr. It should have its own address.
   *     The Checkpoint class needs a protocol for populating and reporting fields which are
   *     specific to derived classes, not just BTree, HTree, and Stream. Or we need to add a general
   *     concept of a "summary statistics object" for a persistent data structure.
   ...
   * <p>{@inheritDoc}
   *
   * <p>FIXME GIST : This is not setting the SolutionSetStream class when invoked by {@link
   * Checkpoint#create(IRawStore, IndexMetadata)} since Stream.create() is being invoked rather than
   ...
   * ISolutionSetStats} are not loaded, then the address from the last {@link Checkpoint} record is
   * returned.
   */
  ...
  public ICloseableIterator<IBindingSet[]> get() {
    ...
    if (rootAddr == IRawStore.NULL) throw new IllegalStateException();

    // Open input stream reading decompressed data from store.
    final DataInputStream in =
        new DataInputStream(wrapInputStream(getStore().getInputStream(rootAddr)));

    // Wrap with iterator pattern that will decode solutions.
    final SolutionSetStreamDecoder decoder =
        new SolutionSetStreamDecoder(metadata.getName(), in, rangeCount());
  ...
  public void put(final ICloseableIterator<IBindingSet[]> src2) {
    ...
    if (src2 == null) throw new IllegalArgumentException();
    ...
    final SolutionSetStreamEncoder encoder = new SolutionSetStreamEncoder(name);
    ...
    try {
      ...
      final DataOutputStream os = new DataOutputStream(wrapOutputStream(out));
  ...
  public ICloseableIterator<IBindingSet> scan() {
    ...
    return (ICloseableIterator<IBindingSet>)
        new Striterator(get())
            .addFilter(
                new Expander() {
  ...
  protected boolean needsCheckpoint() {
    ...
    if (super.needsCheckpoint()) return true;

    if (solutionSetStats != null && solutionSetStats.addr != getCheckpoint().getBloomFilterAddr()) {
  ...
  protected void flush() {
    ...
    if (solutionSetStats != null
      ...
      solutionSetStats.addr =
          getStore().write(ByteBuffer.wrap(SerializerUtil.serialize(solutionSetStats.delegate)));
    }
  }
  ...
  protected void setCheckpoint(final Checkpoint checkpoint) {
...

    {
      ...
      if (addr != IRawStore.NULL) {
        ...
        this.solutionSetStats =
            new MySolutionSetStats(
                (ISolutionSetStats) SerializerUtil.deserialize(getStore().read(addr)));
