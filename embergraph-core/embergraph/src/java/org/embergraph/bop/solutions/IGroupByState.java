...
public interface IGroupByState {
   ...
   * SELECT SUM(DISTINCT ?x), SUM(?y)</code>, this would report <code>{?x,?y}</code>. The variables
  ...
  /**
   * <code>true</code> iff any aggregate functions will be applied to the DISTINCT values arising
   ...
   * IAggregate} functions. When <code>true</code> certain optimizations are possible.
   *
   * <p>For example, the following query does not use an {@link IAggregate} function in the HAVING
   * clause. Instead, it references an aggregate defined in the SELECT clause.
   *
   * <pre>
   * SELECT SUM(?y) as ?x
   * GROUP BY ?z
