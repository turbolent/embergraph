...
public class BindingSetComparator<E> implements Comparator<IBindingSet> {
   ...
   *     computed and bound on the solutions to be compared by the caller.
   * @param valueComparator The comparator used on the values extracted from the {@link
   *     IBindingSet}s.
   */
  public BindingSetComparator(
      final ISortOrder<E>[] sortOrder, final Comparator<E> valueComparator) {

    if (sortOrder == null) throw new IllegalArgumentException();

    if (sortOrder.length == 0) throw new IllegalArgumentException();

    if (valueComparator == null) throw new IllegalArgumentException();
