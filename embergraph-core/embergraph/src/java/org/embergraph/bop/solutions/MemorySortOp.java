 ...
 *
 * <p>Computing the value expressions first is not only an efficiency, but is also required in order
 * to detect type errors. When a type error is detected for a value expression the corresponding
 * input solution is kept but with no new bindings, see trac-765. Since the computed value
 * expressions must become bound on the solutions to be sorted, the caller is responsible for
 * wrapping any value expression more complex than a variable or a constant with an {@link IBind}
 * onto an anonymous variable. All such variables will be dropped when the solutions are written
 * out. Since this operator must be able to compare all {@link IV}s in all {@link IBindingSet}s, it
 * depends on the materialization of non-inline {@link IV}s and the ability of the value comparator
 * to handle comparisons between materialized non-inline {@link IV}s and inline {@link IV}s.
 *
 * <p>TODO External memory ORDER BY operator.
 *
 * <p>SPARQL ORDER BY semantics are complex and evaluating a SPARQL ORDER BY is further complicated
 * by the schema flexibility of the value to be sorted. The simplest path to a true external memory
 * sort operator would be to buffer and manage paging for blocks of inline IVs without materialized
 * RDF values s and non-inline IVs with materialized RDF values.
 *
 * <p>An operator could also be written which buffers the solutions on the native heap but the
 * as-bound values which will be used to order those solutions are either buffered on the JVM heap
 * or materialized onto the JVM heap when the sort is executed. This could scale better than a pure
 * JVM heap version, but only to the extent that the keys are smaller than the total solutions. The
 * solutions would probably be written as serialized binding sets on the memory manager such that
 * each solution has its own int32 address. That address can then be paired with the as-bound key to
 * be sorted on the JVM heap.
 *
...
public class MemorySortOp extends SortOp {
  ...
  public MemorySortOp(final BOp[] args, final Map<String, Object> annotations) {
    ...
    for (ISortOrder<?> s : getSortOrder()) {
      ...
      if (expr instanceof IVariableOrConstant<?>) continue;
  ...
  private static class SortTask implements Callable<Void> {
    ...
    SortTask(final MemorySortOp op, final BOpContext<IBindingSet> context) {
    ...
    void release() {
      ...
      if (log.isInfoEnabled()) log.info("Releasing state");
     ...
     */
    private void acceptSolutions(final ICloseableIterator<IBindingSet[]> itr) {
      ...
      try {
        ...
        if (log.isInfoEnabled()) log.info("Buffered " + solutions.size() + " solutions so far");
    ...
    private void doOrderBy(final IBlockingBuffer<IBindingSet[]> sink) {
      ...
      if (log.isInfoEnabled()) log.info("Sorting.");

      final IBindingSet[] all = solutions.toArray(new IBindingSet[0]);

      @SuppressWarnings({"rawtypes", "unchecked"})
      final Comparator<IBindingSet> c =
          new BindingSetComparator(sortOrder, op.getValueComparator());
