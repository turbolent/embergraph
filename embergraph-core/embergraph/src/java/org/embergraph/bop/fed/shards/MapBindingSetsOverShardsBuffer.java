 ...
 * partition(s) associated with an {@link IPredicate} and {@link IKeyOrder}. For each source chunk,
 * "as bound" versions of the target {@link IPredicate} are constructed and the {@link IBindingSet}s
 * in the chunk are reordered based on {@link IKeyOrder#getFromKey(IKeyBuilder, IPredicate)} for
 * each asBound predicate. The {@link PartitionLocator}s are discovered for each fromKey using an
 * ordered locator scan and the binding sets are output onto a shard or node specific {@link
 * IBuffer} created by a concrete subclass. The subclass is responsible for getting the binding sets
 * from this node onto the node associated with each output buffer.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: UnsyncDistributedOutputBuffer.java 3448 2010-08-18 20:55:58Z thompsonbry $
 * @param <E> The generic type of the elements in the buffer.
 * @param <F> The generic type of the elements in the relation associated with the {@link
 *     IPredicate}.
 * @todo This could be refactored such that it no longer implemented {@link IBuffer} but instead was
 *     a {@link BOp} with binding sets streaming in from its source. However, unlike a normal {@link
 *     BOp} it would have a compound sink and it would have to be tightly integrated with the {@link
 *     QueryEngine} to be used.
 *     <p>In fact, this is pretty much just doing a join against the metadata index. However, it
 *     presumes that there are far fewer index partitions than tuples flowing through the system and
 *     that it is better to read remotely from the {@link IMetadataIndex} and cache than ro use the
 *     general purpose pipeline join, which would cause all binding sets to be routed through the
 *     centralized {@link IMetadataIndex}.
 */
public abstract class MapBindingSetsOverShardsBuffer<E extends IBindingSet, F>
    extends AbstractUnsynchronizedArrayBuffer<E> {

  //    static transient private final Logger log =
  // Logger.getLogger(MapBindingSetsOverShardsBuffer.class);
   ...
   *     which the target operator must read or write. For example, when the target operator is a
   *     JOIN, this is the {@link IPredicate} associated with the right hand operator of the join.
   * @param timestamp The timestamp associated with the operation on the target access path. If the
   *     binding sets will be used to read on the shards of the target access path, then this is the
   *     read timestamp. If they will be used to write on the target access path, then this is the
   *     write timestamp.
  ...
  public MapBindingSetsOverShardsBuffer(
    ...
    //         */
    {
      ...
      this.relation = (IRelation<F>) fed.getResourceLocator().locate(namespace, timestamp);

      if (relation == null)
        throw new RuntimeException(
            "Not found: relation=" + namespace + "@" + TimestampUtility.toString(timestamp));
  ...
  protected IMetadataIndex getMetadataIndex(final IKeyOrder<F> keyOrder) {
    ...
    if (mdi == null) {
      ...
      throw new NoSuchIndexException(
          "name=" + namespace + ", timestamp=" + TimestampUtility.toString(timestamp));
    }
   ...
   * dimension.
   *
   * @param a A chunk of {@link IBindingSet}s.
   */
  ...
  protected Iterator<PartitionLocator> locatorScan(
      final IKeyOrder<F> keyOrder, final byte[] fromKey, final byte[] toKey) {
   ...
   * the collection and when visiting the elements of the collection. However, the {@link
   ...
   *     be better to hook the production of the binding sets for a given {@link PartitionLocator}
   *     since many of the {@link IShardMapper}s can know when they will not see another binding set
   *     for a given {@link PartitionLocator} and hence the data can be immediately flushed to that
   *     target.
   *     <p>A similar scaling concern with very large numbers of source binding sets is that we may
   *     be better off applying a {@link SortOp} to the binding sets, which allows us to use
   *     external merge sorts or even hash partitioned distributed merge sorts. This suggests that
   *     we should really unpack this class as a general purpose operator with special integration
   ...
   * shard.
   *
   * <p>Note: The concrete implementation may chose whether to associate buffers with the target
   * shard or the target node.
   *
   * @param locator The locator for the target index partition.
   * @return The buffer.
   */
  protected abstract IBuffer<IBindingSet[]> newBuffer(PartitionLocator locator);
}
