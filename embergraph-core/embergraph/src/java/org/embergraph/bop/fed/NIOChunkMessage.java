 ...
 *     <p>TODO Implement {@link Externalizable} for this class based on the {@link
 *     ThickChunkMessage} and {@link IVSolutionSetEncoder}.
 */
public class NIOChunkMessage<E> implements IChunkMessage<E>, Serializable {
   ...
   * @param sinkId
   * @param partitionId
   * @param allocations The ordered list of {@link IAllocation}s comprising the chunk.
   * @param addr The Internet address and port where the receiver can fetch the payload using the
   *     sender's {@link ResourceService}.
   */
  public NIOChunkMessage(
    ...
    final List<IAllocation> allocations = moveToNIOBuffers(allocationContext, source, nsolutions);
   ...
   * @return
   */
  private static <E> List<IAllocation> moveToNIOBuffers(
      final IAllocationContext allocationContext,
      final E[] source,
  ...
  /** Metadata about an allocation to be retrieved from the sender's {@link ResourceService}. */
  private static final class A implements Serializable {

    /** */
    private static final long serialVersionUID = 1L;

    /** The identifier of the resource on the sender's {@link ResourceService}. */
    ...
    public A(final UUID bufferId, final int nbytes) {
      ...
      if (bufferId == null) throw new IllegalArgumentException();
  ...
  public void materialize(final FederatedRunningQuery runningQuery) {
    ...
    final AllocationContextKey key = new ShardContext(queryId, bopId, partitionId);

    final IAllocationContext allocationContext = runningQuery.getAllocationContext(key);

    final ManagedResourceService resourceService =
        runningQuery.getQueryEngine().getResourceService();
   ...
   * @param allocationContext
   * @todo The {@link ResourceService} does not currently use NIO and there is no benefit to passing
   *     in direct {@link ByteBuffer}s yet.
   */
  protected synchronized void materialize(
    ...
    try {
      ...
      for (A a : allocations) {
        ...
        new ResourceService.ReadBufferTask(addr, a.bufferId, buf).call();
   ...
   * sets which leverages the known set of variables in play as of the operator which generated
   * those intermediate results.
   *
   * <p>Note: Some similar work was done to improve the htree performance.
   *
   * <p>Note: Very small chunks (1-5 solutions) are common on a cluster and might be optimized
   * different than modest chunks (10-100+).
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/395">HTree performance
   *     tuning</a>
   */
  private class ChunkAccessor implements IChunkAccessor<E> {
    ...
    public ChunkAccessor() {
      ...
      if (tmp == null) throw new UnsupportedOperationException();
