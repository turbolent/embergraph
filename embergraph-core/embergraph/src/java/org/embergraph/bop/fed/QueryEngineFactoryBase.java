...
public class QueryEngineFactoryBase implements IQueryEngineFactory {
   ...
   * QueryEngine} objects wired into the cache unless the application is holding a hard reference to
   ...
   */
  @Override
  public QueryEngine getExistingQueryController(final IBTreeManager indexManager) {
  ...
  /* (non-Javadoc)
   * @see org.embergraph.bop.fed.IQueryEngineFactory#getQueryController(org.embergraph.journal.IIndexManager)
   */
  @Override
  ...
  /* (non-Javadoc)
   * @see org.embergraph.bop.fed.IQueryEngineFactory#getStandaloneQueryController(org.embergraph.journal.IBTreeManager)
   */
  @Override
  public QueryEngine getStandaloneQueryController(final IBTreeManager indexManager) {
   ...
   */
  protected QueryEngine newStandaloneQueryEngine(final IBTreeManager indexManager) {

    if (log.isInfoEnabled()) log.info("Initializing query engine: " + indexManager);
   ...
   */
  @Override
  public FederatedQueryEngine getFederatedQueryController(final IEmbergraphFederation<?> fed) {
   ...
   * @return The new query engine.
   *     <p>TODO Parameterize the local resource service and temporary storage. For example, maybe
   *     it should be using a journal backed by a MemStore?
   */
  private FederatedQueryEngine newFederatedQueryEngine(final IEmbergraphFederation<?> fed) {

    if (log.isInfoEnabled()) log.info("Initiallizing query engine: " + fed);
    ...
    try {
      ...
      queryEngine =
          new FederatedQueryController(
              fed.getServiceUUID(), fed, queryEngineStore, queryEngineResourceService);

    } catch (Throwable t) {

      if (queryEngineStore != null) queryEngineStore.destroy();

      if (queryEngineResourceService != null) queryEngineResourceService.shutdownNow();
  ...
  /* (non-Javadoc)
   * @see org.embergraph.bop.fed.IQueryEngineFactory#getQueryControllerCount()
   */
  @Override
