 ...
 * BOpEvaluationContext#ANY}, all of these cases wind up writing the intermediate results onto a
 * direct {@link ByteBuffer} and notifying the receiving service that there are intermediate results
 * which it can pull when it is ready to process them. This pattern allows the receiver to impose
 * flow control on the producer.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: FederationChunkHandler.java 6038 2012-02-17 17:43:26Z thompsonbry $
 * @param <E> The generic type of the objects in the relation.
 * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/488">Vector query engine messages
 *     per node</a>
 */
public class FederationChunkHandler<E> extends StandaloneChunkHandler {
   ...
   * @see NIOChunkMessage
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/462">Invalid byte: 3 in
   *     ResourceService (StatusEnum) on cluster </a>
   */
   ...
   *     messages. This allows the test cases to avoid RDF specific logic in the {@link
   *     IChunkMessage} serialization.
   */
   ...
   *     executing on a given node destined for the same shard/node. (There is code in the {@link
   *     DistributedJoinTask} which does this for the same shard, but it does it on the receiver
   *     side.) Pay attention to the #of threads running in the join, the potential concurrency of
   *     threads targeting the same (bopId,shardId) and how to best combine their data together.
   */
  @Override
  public int handleChunk(
    ...
    if (targetOp == null) throw new IllegalStateException("Not found: " + sinkId);
      ...
      case SHARDED:
        {
          ...
          final MapBindingSetsOverShardsBuffer<IBindingSet, E> mapper =
              new MapBindingSetsOverShardsBuffer<IBindingSet, E>(
          ...
          for (Map.Entry<PartitionLocator, IBuffer<IBindingSet[]>> e :
              mapper.getSinks().entrySet()) {
            ...
            final IBlockingBuffer<IBindingSet[]> shardSink =
                (IBlockingBuffer<IBindingSet[]>) e.getValue();
      ...
      case CONTROLLER:
        {
          ...
          sendChunkMessage(
              q, q.queryControllerUUID, sinkId, -1 /* partitionId */, allocationContext, chunk);
   ...
   * service.
   *
   * @param serviceUUID The {@link UUID} of the {@link IQueryPeer} who is the recipient.
   * @param sinkId The identifier of the target {@link BOp}.
   * @param allocationContext The allocation context within which the {@link ByteBuffer}s will be
   *     managed for this {@link NIOChunkMessage}.
   * @param source The binding sets to be formatted onto a buffer.
   * @return The {@link NIOChunkMessage}.
   * @todo This is basically a factory for creating {@link IChunkMessage}s. That factory pattern in
   *     combined with the logic to send the message so we can do within JVM handoffs. We could
   *     break these things apart using {@link IChunkMessage#isMaterialized()} to detect inline
   *     cases. That would let us send out the messages in parallel, which could help to cut latency
   *     when an operator has a large fan out (in scale-out when mapping over shards or nodes).
   * @todo Release the allocations associated with each output chunk once it is received by the
   *     remote service.
   *     <p>When the query terminates all output chunks targeting any node EXCEPT the query
   *     controller should be immediately dropped.
   *     <p>If there is an error during query evaluation, then the output chunks for the query
   *     controller should be immediately dropped.
   *     <p>If the iterator draining the results on the query controller is closed, then the output
   *     chunks for the query controller should be immediately dropped.
   * @todo There are a few things for which the resource must be made available to more than one
   *     operator evaluation phase. The best examples are temporary graphs for parallel closure and
   *     large collections of graphIds for SPARQL "NAMED FROM DATA SET" extensions.
   * @todo Rethink the multiplicity relationship between chunks output from an operator, chunks
   *     output from mapping the operator over shards or nodes, RMI messages concerning buffers
   *     available for the sink operator on the various nodes, and the #of allocations per RMI
   *     message on both the sender and the receiver.
   *     <p>I am pretty sure that none of these are strongly coupled, e.g., they are not 1:1. Some
   *     stages can combine chunks. Multiple allocations could be required on either the sender or
   *     the receiver purely due to where the slices fall on the backing direct {@link ByteBuffer}s
   *     in the {@link DirectBufferPool} and the sender and receiver do not need to use the same
   *     allocation context or have the same projection of slices onto the backing buffers.
   *     <p>The one thing which is critical is that the query controller is properly informed of the
   *     #of chunks made available to an operator and consumed by that operator, that those reports
   *     must be in the same units, and that the reports must be delivered back to the query
   *     controller in a manner which does not transiently violate the termination conditions of the
   *     query.
   */
  protected void sendChunkMessage(
    ...
    if (peerProxy == null) throw new RuntimeException("Not found: serviceId=" + serviceUUID);
    ...
    if (thisService) {
      ...
      if (log.isDebugEnabled()) log.debug("Sending local message: " + msg);
      ...
      if (source.length <= nioThreshold) {
        ...
        msg =
            new NIOChunkMessage<>(
                ...
                allocationContext,
                source,
                q.getQueryEngine().getResourceService().getAddr());
      }
    }

    if (log.isDebugEnabled()) log.debug("Sending remote message: " + msg);

    // Update counters since message will cross machine boundary.
    final FederatedQueryEngineCounters c = q.getQueryEngine().getQueryEngineCounters();
