...
public class FederatedRunningQuery extends ChunkedRunningQuery {
   ...
   * necessary so we can send chunks to multiple receivers or retry transmission if a receiver dies.
   *
   * <p>FIXME When a chunk is transferred to the receiver, its allocation(s) must be released on the
   * sender. That should be done based on the life cycle of the allocation. This implies that we
   * should maintain a list for each life cycle so they can be cleared without scanning the set of
   * all allocations.
   *
   * <p>FIXME When the query is done, those allocations MUST be released (excepting only allocations
   * associated with the solutions which need to be released as those data are consumed).
   *
   * <pre>
   ...
   * </pre>
   *
   * The allocation contexts which use a bopId MUST use the bopId of the operator which will consume
   * the chunk. That way the producer can release outputs buffered for that consumer as they are
   * transferred to the consumer. [If we leave the allocations in place until the bop evaluates the
   * chunk then we could retry if the node running the bop fails.]
   *
   * <p>Allocations which are tied to the life cycle of a {@link BOp}, rather the the chunks
   * processed for that {@link BOp}, should use (queryId,bopId). For example, a DHT imposing
   * distinct on a default graph access path has a life cycle linked to the join reading on that
   * access path (not the predicate since the predicate is not part of the pipeline and is evaluated
   * by the join rather than the query engine).
   *
   * <p>Allocations tied to the life cycle of a query will not be released until the query
   * terminates. For example, a temporary graph containing the ontology for a parallel distributed
   * closure operation.
   */
  ...
  protected void releaseNativeMemoryForOperator(final int bopId) {
    ...
    final Iterator<Map.Entry<AllocationContextKey, IAllocationContext>> itr =
        allocationContexts.entrySet().iterator();

    while (itr.hasNext()) {

      final Map.Entry<AllocationContextKey, IAllocationContext> e = itr.next();
  ...
  public FederatedRunningQuery(
    ...
    if (queryControllerId == null) throw new IllegalArgumentException();
   ...
   *
   * @param serviceUUID The service identifier for the peer.
   * @return The proxy for the service, the actual {@link QueryEngine} reference if the identified
   *     service is <i>this</i> service, or <code>null</code> if the service could not be
   *     discovered.
   */
   ...
   */
  public IAllocationContext getAllocationContext(final AllocationContextKey key) {

    final IAllocationContext ctx =
        getQueryEngine().getResourceService().getAllocator().getAllocationContext(key);

    // note the allocation contexts associated with this running query.
    final IAllocationContext ctx2 = allocationContexts.putIfAbsent(key, ctx);

    if (ctx2 != null) return ctx2;
      ...
      case HASHED:
        {
          ...
          for (UUID serviceId : ((Set<UUID>) doneOn)) {
            ...
            try {
              ...
              throw new RuntimeException(
                  "Could not send message: serviceId=" + serviceId + " : " + e, e);
            }
          }
          return;
        }
