...
public class FederatedQueryEngine extends QueryEngine {
   ...
   * #init()}.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/472">acceptTaskService pool size
   *     on cluster </a>
   */
  ...
  /** Return <code>true</code> iff the query engine instance is hosted by a {@link DataService}. */
  ...
  public FederatedQueryEngine(final DataService dataService) {
    ...
    this(
        ...
        dataService.getFederation(),
        new DelegateIndexManager(dataService),
        dataService.getResourceManager().getResourceService(),
        true /* isDataService */);
  }
  ...
  private FederatedQueryEngine(
    ...
    if (resourceService == null) throw new IllegalArgumentException();
   ...
   * consumption by this node.
   *
   * <p>TODO The {@link #acceptTaskService} is not used right now since we are always running the
   * {@link MaterializeMessageTask} in the caller's thread. If it becomes used, then we should
   * reconsider the pool size.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/472">acceptTaskService pool size
   *     on cluster</a>
   */
  ...
  private class MaterializeMessageTask implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        if (!accept(msg)) {
          ...
          if (log.isDebugEnabled()) log.debug("dropping: " + msg);
    ...
    private boolean accept(final IChunkMessage<?> msg) throws RemoteException {
      ...
      if (q == null) {
        ...
        final boolean isController = getServiceUUID().equals(msg.getQueryControllerId());
        ...
        if (isController) {
          ...
          throw new AssertionError(
              "Query not running on controller: thisService=" + getServiceUUID() + ", msg=" + msg);
        }
        ...
        try {
          ...
          if (log.isInfoEnabled())
            log.info("Query is gone: isDataService=" + isDataService() + ", message=" + msg, ex);
     ...
     * query controller.
     *
     * <p>FIXME This should use a memoizer pattern. We are probably resolving the query multiple
     * times when the first operation on a node is scattered over more than one shard on that node.
     *
     * <p>Note: The operations performed here MUST NOT encounter any bounded resource, such as the
     * acceptTaskService. If they did, then this request could deadlock.
     */
    private FederatedRunningQuery getDeclaredQuery(final UUID queryId) throws RemoteException {
   ...
   * when the data are being moved around using NIO rather than {@link ThickChunkMessage}. At stake
   * is when the intermediate solutions are materialized on the node where they will be consumed. We
   * can either do this synchronous or asynchronously when bufferReady() is caller or we can defer
   * the transfer until the target operator on this node is ready to run.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/472">acceptTaskService pool size
   *     on cluster </a>
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/131">unselective joins on
   *     cluster</a>
   */
  @Override
  ...
  protected FederatedRunningQuery newRunningQuery(
    ...
    return new FederatedRunningQuery(
        this /* queryEngine */,
        queryId,
        controller,
        clientProxy,
   ...
   * a query controller unless an {@link IDataService} is being used as the query controller.
   *
  ...
  protected IQueryPeer getQueryPeer(final UUID serviceUUID) {
    ...
    if (proxy == null) {
      ...
      if (dataService == null) throw new RuntimeException("No such service: " + serviceUUID);
