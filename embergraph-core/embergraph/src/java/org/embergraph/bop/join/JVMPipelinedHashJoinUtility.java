...
public class JVMPipelinedHashJoinUtility extends JVMHashJoinUtility
  ...
  void addBindingsSetSourceToHashIndexOnce(
      final JVMHashIndex rightSolutions, final IBindingSet[] bsFromBindingsSetSource) {
    ...
    for (final IBindingSet left : chunk) {
      ...
      if (bucket != null) {
        ...
        while (ritr.hasNext()) {
          ...
          final IBindingSet outSolution =
              BOpContext.bind(right.solution, left, joinConstraints, getSelectVars());
      ...
      switch (getJoinType()) {
        ...
        case Exists:
          {
            if (askVar != null) {
              left.set(askVar, new Constant<XSDBooleanIV<?>>(XSDBooleanIV.valueOf(matchExists)));
              outputSolution(outputBuffer, left);
            }
            break;
          }
