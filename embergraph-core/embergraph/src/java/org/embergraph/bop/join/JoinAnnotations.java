...
public interface JoinAnnotations extends org.embergraph.bop.PipelineOp.Annotations {
   ...
   *
   * <p>Note: Not all join operators support all join types. For example, {@link PipelineJoin}
   * supports {@link JoinTypeEnum#Normal} and {@link JoinTypeEnum#Optional}. The hash join operators
   * tend to support more join types, but some hash join operators only support {@link
   * JoinTypeEnum#Normal} joins because the solutions from the query engine are flowing through the
   * join rather than being at a fixed point in a hash index which is then fed into the join. When
   * the query engine solutions are folowing through the join it is not possible to do any of the
   * non-required joins (Optional, Exists, NotExists).
   *
   * @see JoinTypeEnum
   */
  ...
  /**
   * An optional {@link IVariable}[] identifying the variables to be retained in the {@link
   * IBindingSet}s written out by the operator. All variables are retained unless this annotation is
   * specified.
   */
  String SELECT = JoinAnnotations.class.getName() + ".select";

  /**
   * An {@link IConstraint}[] which places restrictions on the legal patterns in the variable
   * bindings (optional).
   *
   * <p>Note: Constraints are applied to input solutions which join. If an input solution does not
   * join and the {@link IPredicate} is {@link IPredicate.Annotations#OPTIONAL} then the {@link
   * #CONSTRAINTS} are NOT tested and the original solution is output.
   */
  String CONSTRAINTS = JoinAnnotations.class.getName() + ".constraints";

  /**
   * The maximum #of solutions which will be generated by the join (default {@value
   * #DEFAULT_LIMIT}).
   *
   * <p>Note: This annotation is used by the RTO to sample evaluation plans. Not all joins support
   * this annotation and the RTO is responsible for using only those joins which DO support this
   * annotation when sampling cutoff evaluation of operator path segments.
   *
   * @see JoinGraph
   */
