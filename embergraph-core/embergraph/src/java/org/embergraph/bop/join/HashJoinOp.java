 ...
 * scanned and the hash index (of intermediate solutions from the pipeline) is probed for each
 * solution read from the {@link IAccessPath}. Solutions which join are output.
 *
...
public abstract class HashJoinOp<E> extends PipelineOp implements IShardwisePipelineOp<E> {
  ...
  public interface Annotations
      extends AccessPathJoinAnnotations, NamedSetAnnotations, HashJoinAnnotations {}
   ...
   *
   * @param context The operator evaluation context.
   * @param state The {@link IHashJoinUtility} instance.
   */
  protected abstract boolean runHashJoin(final BOpContext<?> context, final IHashJoinUtility state);
  ...
  private static class ChunkTask<E> implements Callable<Void> {
    ...
    private IBindingSetAccessPath<?> getAccessPath() {
      ...
      return (IBindingSetAccessPath<?>) context.getAccessPath(relation, pred);
    }
    ...
    private void doHashJoin() {
      ...
      final ICloseableIterator<IBindingSet[]> itr =
          accessPath.solutions(context, cutoffLimit, stats);
