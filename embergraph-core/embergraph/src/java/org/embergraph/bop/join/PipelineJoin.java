...
import org.embergraph.relation.accesspath.ThreadLocalBufferFactory;
 ...
 *
 * <p>Note: In order to support pipelining, query plans need to be arranged in a "left-deep" manner.
 *
 * <p>Note: In scale-out, the {@link PipelineJoin} is generally annotated as a {@link
 * BOpEvaluationContext#SHARDED} or {@link BOpEvaluationContext#HASHED} operator and the {@link
 * IPredicate} is annotated for local access paths. If you need to use remote access paths, then the
 * {@link PipelineJoin} should be annotated as a {@link BOpEvaluationContext#ANY} operator since it
 * will be issuing a remote read from each node on which it has source solutions to be joined.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @todo Break the star join logic out into its own join operator and test suite (or just discard
 *     it).
 */
public class PipelineJoin<E> extends PipelineOp implements IShardwisePipelineOp<E> {
  ...
  public interface Annotations extends AccessPathJoinAnnotations {
     ...
     * chunk.
     *
     * <p>Note: When ZERO (0), everything will run in the caller's {@link Thread}, but there will
     * still be one thread per pipeline join task which is executing concurrently against different
     * source chunks. When GT ZERO (0), tasks will run on an {@link ExecutorService} with the
     * specified maximum parallelism.
     *
     * <p>Note: This is NOT the same as {@link PipelineOp.Annotations#MAX_PARALLEL}. This option (
     * {@link #MAX_PARALLEL_CHUNKS} limits the #of chunks for a single task which may be processed
     * concurrently. {@link PipelineOp.Annotations#MAX_PARALLEL} limits the #of task instances which
     * may run concurrently.
     *
     * @see #DEFAULT_MAX_PARALLEL_CHUNKS
     * @todo Chunk level parallelism does not appear to benefit us when using the
     *     ChunkedRunningQuery class and the new QueryEngine, so this option might well go away
     *     which would allow us to simplify the PipelineJoin implementation. (All the thread local
     *     buffer stuff would go away as would much of the complexity of the haltable since we could
     *     directly interrupt the task if it all runs in the caller's thread).
     */
     ...
     * efficiency of the join since it reads the access path once per set of input binding sets
     * which are coalesced. This option does NOT reduce the #of solutions generated.
     *
     * <p>This option can cause some error in the join hit ratio when it is estimated from a cutoff
     * join.
     *
     * @see PipelineJoinStats#getJoinHitRatio()
     * @todo unit tests when (en|dis)abled.
     */
    String COALESCE_DUPLICATE_ACCESS_PATHS =
        (PipelineJoin.class.getName() + ".coalesceDuplicateAccessPaths").intern();
     ...
     * and output solutions for complex joins (those which required materialization of RDF Values
     * for FILTER evaluation).
     *
     * @todo unit tests when (en|dis)abled.
     */
    String REORDER_ACCESS_PATHS = (PipelineJoin.class.getName() + ".reorderAccessPaths").intern();
     ...
     * situation where a certain thread gets assigned longer-running tasks s.t. other threads are
     * finished earlier (leading to, effectively, decreased parallelism). On the downside,
     * increasing this parameter leads to overhead in setting up additional subtasks, so values >5
     * are generally discouraged.
     *
     * <p>Note that this is an upper bound only: we always consider the MIN_DATAPOINTS_PER_TASK
     * threshold in order to avoid generating threads with a low workload.
     *
     * <p>Currently only implemented for the geospatial feature (see BLZG-1478), but this might be
     * implemented more generally for the pipelined join in the future.
     *
     * <p>Must be a value >= 1.
     */
    String NUM_TASKS_PER_THREAD = (PipelineJoin.class.getName() + ".numTasksPerThread").intern();
  ...
  public int getMaxParallelChunks() {
    ...
    // return 5;
    return getProperty(Annotations.MAX_PARALLEL_CHUNKS, Annotations.DEFAULT_MAX_PARALLEL_CHUNKS);
  }
  ...
  private static class JoinTask<E> extends Haltable<Void> implements Callable<Void> {
     ...
     * Long#MAX_VALUE}.
     *
     * <p>Note: Invoking {@link #halt(Object)} is necessary to enforce the limit.
     *
     ...
     * be set until we begin to execute the {@link #left} operand and that should not happen until
     * we are in {@link #call()} in order to ensure that the producer will be terminated if there is
     * a problem setting up this join. Given that, it might need to be an atomic reference or
     * volatile or the like.
     */
     ...
     * Those unsynchronized buffers overflow onto the per-JoinTask {@link #sink}, which is a
     * thread-safe {@link IBlockingBuffer}. The downstream pipeline operator drains that {@link
     * IBlockingBuffer} using its iterator(). When the {@link #sink} is closed and everything in it
     * has been drained, then the downstream operator will conclude that no more bindingSets are
     * available and it will terminate.
     */
     ...
     * Normally the {@link BOpContext#getSink()} can be used for both the joins which succeed and
     * those which fail. The alternative sink is only necessary when the failed join needs to jump
     * out of a join group rather than routing directly to the ancestor in the operator tree.
     */
     ...
     * @param context
     */
    public JoinTask(final PipelineJoin<E> joinOp, final BOpContext<IBindingSet> context) {

      if (joinOp == null) throw new IllegalArgumentException();
      ...
      if (maxParallelChunks < 0)
        throw new IllegalArgumentException(
            Annotations.MAX_PARALLEL_CHUNKS + "=" + maxParallelChunks);
      if (maxParallelChunks > 0) {
        // shared service.
        service =
            new LatchedExecutor(context.getIndexManager().getExecutorService(), maxParallelChunks);
      } else {
      ...
      this.coalesceAccessPaths =
          joinOp.getProperty(
              Annotations.COALESCE_DUPLICATE_ACCESS_PATHS,
              Annotations.DEFAULT_COALESCE_DUPLICATE_ACCESS_PATHS);
      this.reorderAccessPaths =
          joinOp.getProperty(
              Annotations.REORDER_ACCESS_PATHS, Annotations.DEFAULT_REORDER_ACCESS_PATHS);

      this.threadLocalBufferFactory = new TLBFactory(sink);

      this.threadLocalBufferFactory2 = sink2 == null ? null : new TLBFactory(sink2);
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        if (log.isDebugEnabled()) log.debug("JoinTask done: joinOp=" + joinOp);
        ...
        try {
          ...
          if (threadLocalBufferFactory2 != null) threadLocalBufferFactory2.reset();
        } catch (Throwable t2) {
     ...
     * IAsynchronousIterator}(s) on which the sink is reading will remain open and the sink will
     * never decide that it has exhausted its source(s).
     *
     ...
     * QueryTimeoutException} as a normal termination cause for a {@link JoinTask}. The {@link
     * Haltable} for the {@link AbstractRunningQuery} will have its root cause set to the {@link
     * QueryTimeoutException} and from there the exception will get eventually converted back into
     * the appropriate openrdf exception. We do things differently here because the {@link JoinTask}
     * termination is not the same as the {@link AbstractRunningQuery} termination. {@link JoinTask}
     * is the only place right now where we extend haltable and the only place where we have to make
     * this specific override.
     *
     * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/772">Query timeout only
     *     checked at operator start/stop. </a>
     */
    @Override
    ...
    protected IBindingSet[] nextChunk() throws InterruptedException {
      ...
      while (source.hasNext()) {
        ...
        if (log.isDebugEnabled())
          log.debug("Read chunk from source: chunkSize=" + chunk.length + ", joinOp=" + joinOp);
      ...
      if (log.isDebugEnabled()) log.debug("Source exhausted: joinOp=" + joinOp);
     ...
     * {@link AccessPathTask}s are sorted based on their <code>fromKey</code> so as to order the
     * execution of those tasks in a manner that will maximize the efficiency of index reads. The
     * ordered {@link AccessPathTask}s are then submitted to the caller's {@link Executor} or run in
     * the caller's thread if the executor is <code>null</code>.
     *
     * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
     */
    protected class BindingSetConsumerTask implements Callable<Void> {
       ...
       *     caller's thread.
       * @param chunk A chunk of binding sets from the upstream producer.
       */
      public BindingSetConsumerTask(final Executor executor, final IBindingSet[] chunk) {
       ...
       * the ctor.
       *
       * <p>Note: When running with an {@link ExecutorService}, the caller is responsible for
       * waiting on that {@link ExecutorService} until the {@link AccessPathTask}s to complete and
       * must verify all tasks completed successfully.
       *
       * @return <code>null</code>
      ...
      public Void call() throws Exception {
        ...
        try {
          ...
          // normal termination - ignore exception.
          if (log.isDebugEnabled()) log.debug("Caught and ignored exception: " + t);
          return null;
        }
      }
      ...
      private void runOneTask() throws Exception {
        ...
        if (chunk.length != 1) throw new AssertionError();
      ...
      protected AccessPathTask[] generateAccessPaths(final IBindingSet[] chunk) {
        ...
        if (coalesceAccessPaths) {
          ...
          // Exact fit array.
          tasks = tmp.toArray(new JoinTask.AccessPathTask[tmp.size()]);
        }
       ...
       * the {@link IAccessPath} obtained for the asBound {@link IPredicate}. This has the natural
      ...
      protected Map<IPredicate<E>, Collection<IBindingSet>> combineBindingSets(
        ...
        if (log.isDebugEnabled())
          log.debug("chunkSize=" + chunk.length + ", #distinct predicates=" + map.size());
      ...
      protected AccessPathTask[] getAccessPathTasks(
        ...
        if (log.isDebugEnabled()) log.debug("#distinct predicates=" + n);

        final AccessPathTask[] tasks = new JoinTask.AccessPathTask[n];

        final Iterator<Map.Entry<IPredicate<E>, Collection<IBindingSet>>> itr =
        ...
        while (itr.hasNext()) {
          ...
          final Map.Entry<IPredicate<E>, Collection<IBindingSet>> entry = itr.next();
       ...
       * chunk of {@link IBindingSet}s.
       *
       * @param tasks The tasks.
       * @see <a href="http://trac.bigdata.com/ticket/1192" > RDR query fails with
    ...
    protected class AccessPathTask implements Callable<Void>, Comparable<AccessPathTask> {
       ...
       * IPredicate} then this will be an array with a single element. However, if multiple source
       * {@link IBindingSet}s result in the same asBound {@link IPredicate} within the same chunk
       * then those are aggregated and appear together in this array.
       *
       ...
       * IBindingSet}. When the join is optional and no solutions were produced for a given {@link
       * IBindingSet}, the {@link IBindingSet} is output anyway.
       */
       ...
       *
       * @todo Layered access paths do not expose a fromKey, but the information we need is
       *     available {@link IKeyOrder#getFromKey(IKeyBuilder, IPredicate)}.
       * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/631">ClassCastException in
       *     SIDs mode query </a>
       */
      ...
      public boolean equals(final Object o) {
        ...
        if (!(o instanceof JoinTask.AccessPathTask)) return false;
       ...
       * {@link IPredicate}, then the task will emit one {@link IBindingSet} for each source {@link
       * IBindingSet}.
       *
      ...
      public AccessPathTask(
          final IPredicate<E> predicate, final Collection<IBindingSet> bindingSets) {
      ...
      /** A vectored pipeline join (chunk at a time processing) for {@link IElement}s. */
      ...
      protected void handleJoin() {
        ...
        final IChunkedOrderedIterator<?> itr =
            accessPath.iterator(0, cutoffLimit, 0 /* capacity */);
        ...
        try {
          ...
          while (itr.hasNext()) {
            ...
            //						System.err.println("#chunks="+stats.accessPathChunksIn+",
            // chunkSize="+chunk.length);

            // process the chunk in the caller's thread.
            new ChunkTask(bindingSets, naccepted, unsyncBuffer, chunk).call();
          } // next chunk.
          ...
          if (optional) {
            ...
            final AbstractUnsynchronizedArrayBuffer<IBindingSet> unsyncBuffer2 =
                threadLocalBufferFactory2 == null ? null : threadLocalBufferFactory2.get();

            for (int bindex = 0; bindex < bindingSets.length; bindex++) {

              if (naccepted[bindex] > 0) continue;
              ...
              if (log.isTraceEnabled())
                log.trace("Passing on solution which fails an optional join: " + bs);

              if (limit != Long.MAX_VALUE && exactOutputCount.incrementAndGet() > limit) {
                // break query @ limit.
                if (log.isInfoEnabled())
                  log.info(
                      "Breaking query @ limit: limit="
      ...
      protected void handleJoin2() {
        ...
        final ICloseableIterator<IBindingSet[]> itr =
            ((IBindingSetAccessPath<?>) accessPath).solutions(context, cutoffLimit, stats);
     ...
     * The {@link IBindingSet}s to be output are buffered into chunks and the chunks added to the
     * {@link JoinPipelineTask#bindingSetBuffers} for the corresponding predicate.
     *
     * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
     */
    protected class ChunkTask implements Callable<Void> {
       ...
       *     each of the {@link IBindingSet} s.
       * @param unsyncBuffer A per-{@link Thread} buffer used to accumulate chunks of generated
       *     {@link IBindingSet}s.
      ...
      public ChunkTask(
        ...
        if (naccepted == null) throw new IllegalArgumentException();

        if (unsyncBuffer == null) throw new IllegalArgumentException();
          ...
          for (Object e : chunk) {
            ...
            for (IBindingSet bset : bindingSets) {
              ...
              if (BOpContext.bind(predicate, constraints, e, bset)) {
                ...
                bset = variablesToKeep == null ? bset : bset.copy(variablesToKeep);
    ...
    private class TLBFactory
        extends ThreadLocalBufferFactory<
            AbstractUnsynchronizedArrayBuffer<IBindingSet>, IBindingSet> {

      private final IBlockingBuffer<IBindingSet[]> sink;

      /** @param sink The thread-safe buffer onto which the thread-local buffer overflow. */
