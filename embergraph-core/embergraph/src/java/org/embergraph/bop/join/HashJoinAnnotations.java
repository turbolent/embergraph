...
public interface HashJoinAnnotations {
   ...
   * are not permitted.
   *
   * <p>Note: The source solutions presented to a hash join MUST have bindings for the {@link
   * #JOIN_VARS} in order to join (source solutions can still be passed on as optionals, but they
   * will not join unless the join variables are not bound).
   *
   * <p>Note: If no join variables are specified, then the join will consider the N x M cross
   * product, filtering for solutions which join. This is very expensive when compared to a hash
   * join. Whenever possible you should identify one or more variables which must be bound for the
   * join and specify those as the join variables.
   */
   ...
   *
   * <p>Note: This supports the bridge between the evaluation of the (NOT) EXISTS graph pattern and
   * the processing whether or not the "EXISTS" graph pattern was successful, which is encoded on
   * the "ask variable."
   */
