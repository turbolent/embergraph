...
import org.embergraph.bop.IndexAnnotations;
...
import org.embergraph.rwstore.sector.MemoryManagerClosedException;
 ...
 *
 * In order to provide efficient encoding and persistence of solutions on the {@link HTree}, this
 * class is written directly to the RDF data model. Rather than POJO serialization, solutions are
 * encoded as logical {@link IV}[]s in a manner very similar to how we represent the keys of the
 * statement indices.
 *
 * <p>Since this encoding does not persist the {@link IV#getValue() cache}, a separate mapping must
 * be maintained from {@link IV} to {@link EmbergraphValue} for those {@link IV}s which have a
 * materialized {@link EmbergraphValue}.
 *
 * <p>TODO Do a 64-bit hash version which could be used for hash indices having more than 500M
 * distinct join variable combinations. Note that at 500M distinct join variable combinations we
 * have a 1 in 4 chance of a hash collision. Whether or not that turns into a cost really depends on
 * the cardinality of the solutions per distinct combination of the join variables. If there is only
 * one solution per join variable combination, then those collisions will cause basically no
 * increase in the work to be done. However, if there are 50,000 solutions per distinct combination
 * of the join variables then we would be better off using a 64-bit hash code.
 *
...
public class HTreeHashJoinUtility implements IHashJoinUtility {
      ...
      new IHashJoinUtilityFactory() {
        ...
        public IHashJoinUtility create(
          ...
          return new HTreeHashJoinUtility(
              context.getMemoryManager(namedSetRef.getQueryId()), op, joinType);
        }
      };
   ...
   * @param bset The bindings whose as-bound hash code for the join variables will be computed.
   * @param ignoreUnboundVariables If a variable without a binding should be silently ignored.
   * @return The hash code.
   * @throws JoinVariableNotBoundException if there is no binding for a join variable.
   *     <p>FIXME Does anything actually rely on the {@link JoinVariableNotBoundException}? It would
   *     seem that this exception could only be thrown if the joinvars[] was incorrectly formulated
   *     as it should only include "known bound" variables. (I think that this is related to
   *     incorrectly passing along empty solutions for named subquery hash joins.)
   */
  private static int hashCode(
      final IVariable<?>[] joinVars, final IBindingSet bset, final boolean ignoreUnboundVariables)
  ...
  /** Utility class for compactly and efficiently encoding and decoding {@link IBindingSet}s. */
   ...
   *
   * <p>TODO HINTS: Annotation and query hint for this. Probably on {@link HashJoinAnnotations}.
   */
  ...
  /** Setup the {@link IndexMetadata} for {@link #rightSolutions} or {@link #joinSet}. */
  protected static HTreeIndexMetadata getIndexMetadata(final PipelineOp op) {
    ...
    final int addressBits =
        op.getProperty(HTreeAnnotations.ADDRESS_BITS, HTreeAnnotations.DEFAULT_ADDRESS_BITS);
    ...
    metadata.setRawRecords(
        op.getProperty(HTreeAnnotations.RAW_RECORDS, HTreeAnnotations.DEFAULT_RAW_RECORDS));

    metadata.setMaxRecLen(
        op.getProperty(HTreeAnnotations.MAX_RECLEN, HTreeAnnotations.DEFAULT_MAX_RECLEN));

    metadata.setWriteRetentionQueueCapacity(
        op.getProperty(
   ...
   */
  public HTreeHashJoinUtility(
      final IMemoryManager mmgr, final PipelineOp op, final JoinTypeEnum joinType) {

    if (mmgr == null) throw new IllegalArgumentException();

    if (op == null) throw new IllegalArgumentException();

    if (joinType == null) throw new IllegalArgumentException();
    ...
    this.selectVars = filter ? joinVars : (IVariable<?>[]) op.getProperty(JoinAnnotations.SELECT);
    ...
    this.encoder =
        new IVBindingSetEncoder(
            EmbergraphValueFactoryImpl.getInstance(
                ((String[]) op.getRequiredProperty(Predicate.Annotations.RELATION_NAME))[0]),
            filter);
   ...
   * solutions ({@link #rightSolutions} and {@link #ivCache} ) and then re-load the them in a
   * read-only mode from their checkpoint(s). This exposes a view of the {@link HTree} which is safe
  ...
  private void checkpointHTree(final AtomicReference<HTree> ref) {
    ...
    if (tmp != null) {
      ...
      final HTree readOnly = HTree.load(store, checkpoint.getCheckpointAddr(), true /* readOnly */);
  ...
  @Override
  public long acceptSolutions(final ICloseableIterator<IBindingSet[]> itr, final BOpStats stats) {
  ...
  public long filterSolutions(
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        // Vector a chunk of solutions.
        final BS[] a =
            vector(it.next(), joinVars, selectVars, true /* ignoreUnboundVariables */, vectorSize);
   ...
   * relatively expensive since it must do lookups in persistence capable data structures. The
   * caller MUST use {@link IBindingSetDecoder#resolveCachedValues(IBindingSet)} to resolve the
   * {@link IVCache} associations once they decide that the decoded solution can join.
   *
   * <p>Note: This instance method is required by the MERGE JOIN logic which associates the schema
   * with the first {@link IHashJoinUtility} instance.
   *
   * @param t A tuple whose value is an encoded {@link IV}[].
  ...
  public static class BS implements Comparable<BS> {
    ...
    @Override
    public String toString() {
      return getClass().getName() + "{hashCode=" + hashCode + ",bset=" + bset + "}";
    }
  }
  ...
  public void hashJoin2(
    ...
    try {
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "rightSolutions: #nnodes="
                + rightSolutions.getNodeCount()
                + ",#leaves="
                + rightSolutions.getLeafCount()
                + ",#entries="
                + rightSolutions.getEntryCount());
      }

      final IKeyBuilder keyBuilder = rightSolutions.getIndexMetadata().getKeyBuilder();
      ...
      while (it.hasNext()) {
        ...
        while (fromIndex < n) {
          ...
          int nrejected = 0;
          {
             ...
             *     <p>FIXME This appears to be the crux of the problem for #764. If you replace
             *     lookupAll(key) with rangeIterator() then the hash join is correct. Of course, it
             *     is also scanning all tuples each time so it is very inefficient. The root cause
             *     is the FrontCodedRabaCoder. It is doing a binary search on the BucketPage.
             *     However, the FrontCodedRabaCoder was not developed to deal with duplicates on the
             *     page. Therefore it is returning an offset into the middle of a run of duplicate
             *     keys when it does its binary search. We will either need to modify this
             *     IRabaCoder to handle this case (where duplicate keys are allowed) or write a new
             *     IRabaCoder that is smart about duplicates.
             */
            ...
            while (titr.hasNext()) {
              ...
              for (int i = fromIndex; i < toIndex; i++) {
                ...
                final IBindingSet outSolution =
                    BOpContext.bind(leftSolution, rightSolution, constraints, selectVars);

                nJoinsConsidered.increment();

                if (noJoinVars && nJoinsConsidered.get() == noJoinVarsLimit) {

                  if (nleftConsidered.get() > 1 && nrightConsidered.get() > 1) {
      ...
      if (log.isInfoEnabled()) log.info("done: " + toString());
   ...
   * @param joinVars The variables on which the hash code will be computed.
   * @param selectVars When non-<code>null</code>, all other variables are dropped. (This is used
   *     when we are modeling a DISTINCT solutions filter since we need to drop anything which is
   *     not part of the DISTINCT variables list.)
   * @param ignoreUnboundVariables When <code>true</code>, an unbound variable will not cause a
   *     {@link JoinVariableNotBoundException} to be thrown.
   * @param vectorSize The vector size (set by side-effect). This will be LTE the number of
   *     solutions in <code>leftSolutions</code>. (If some solutions are eliminated because they
   *     lack a binding for a required join variable, then vectorSize is LT the number of <code>
   *     leftSolutions</code>).
   * @return The vectored chunk of solutions ordered by hash code.
   */
  protected BS[] vector(
      final IBindingSet[] leftSolutions,
    ...
    for (int i = 0; i < a.length; i++) {
      ...
      final IBindingSet bset =
          selectVars == null ? leftSolutions[i] : leftSolutions[i].copy(selectVars);
   ...
   * are the full encoded {@link IBindingSet}.
   */
  protected void saveInJoinSet(final int joinSetHashCode, final byte[] val) {
    ...
    if (true) {
      ...
      final IKeyBuilder keyBuilder = joinSet.getIndexMetadata().getKeyBuilder();

      final byte[] key = keyBuilder.reset().append(joinSetHashCode).getKey();
  ...
  public void outputOptionals(final IBuffer<IBindingSet> outputBuffer) {
    ...
    try {
      ...
      if (log.isInfoEnabled()) {
        ...
        log.info(
            "joinSet: #nnodes="
                + joinSet.getNodeCount()
                + ",#leaves="
                + joinSet.getLeafCount()
                + ",#entries="
      ...
      while (sitr.hasNext()) {
        ...
        if (!found) {
          ...
          if (f != null) {
            ...
            if (bs == rightSolution) bs = rightSolution.clone();
  ...
  public ICloseableIterator<IBindingSet> indexScan() {
    ...
    // source.
    final ITupleIterator<?> solutionsIterator = rightSolutions.rangeIterator();
    ...
    try {
      ...
      while (solutionsIterator.hasNext()) {
        ...
        if (outputDistinctJVs) {
          ...
          final int newHashCode = hashCode(joinVars, bset, true /* ignoreUnboundVariables */);

          final boolean newBucket = distinctSet == null || newHashCode != lastHashCode;
  ...
  public void outputJoinSet(final IBuffer<IBindingSet> out) {
    ...
    try {
      ...
      // source.
      final ITupleIterator<?> solutionsIterator = joinSet.rangeIterator();
      ...
      while (solutionsIterator.hasNext()) {
        ...
        if (t != null) {
          ...
          if (selectVars == null) bset = bset.clone();
   ...
   * computed from the solutions using the join variables. Since the keys are hash codes and not the
   * join variable bindings, each hash code identifies a collision bucket from the perspective of
   * the merge join algorithm. Of course, from the perspective of the {@link HTree} those solutions
   * are just consequective tuples readily identified using {@link HTree#lookupAll(int)}.
   *
   * <p>FIXME Either always project everything or raise [select] into a parameter for this method.
   * We DO NOT want to only project whatever was projected by the first source.
   */
  @Override
  public void mergeJoin(
    ...
    try {
      ...
      if (others == null) throw new IllegalArgumentException();

      if (others.length == 0) throw new IllegalArgumentException();

      if (outputBuffer == null) throw new IllegalArgumentException();
       ...
       */
      final IConstraint[] c = JVMHashJoinUtility.combineConstraints(constraints, all);
      ...
      long njoined = 0, nrejected = 0;
      {
        ...
        final Striterator sols0 = new Striterator(all[0].getRightSolutions().rangeIterator());
        {
          sols0.addFilter(
              new Visitor() {
                private static final long serialVersionUID = 1L;
        ...
        while (sols0.hasNext()) {
          ...
          if (in != null) {
            if (log.isDebugEnabled()) log.debug("Output solution: " + in);
   ...
   * @throws Exception
   * @see http://sourceforge.net/apps/trac/bigdata/ticket/508 (LIMIT causes hash join utility to log
   *     errors)
