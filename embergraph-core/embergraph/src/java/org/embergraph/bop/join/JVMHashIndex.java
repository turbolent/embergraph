 ...
 *
 * <p>Note: The {@link JVMDistinctBindingSetsOp} does not use this class right now because it enjoys
 * better concurrency than the {@link JVMHashIndex}. Also see {@link JVMDistinctFilter}, which is
 * the backing implementation for the {@link JVMDistinctBindingSetsOp}.
 *
...
public class JVMHashIndex {
   ...
   * @return The as-bound values for the {@link #keyVars} for that solution -or- <code>null</code>
   *     if one or more join variables is not bound by the solution and {@link
   *     #indexSolutionsHavingUnboundJoinVars} is <code>false</code>.
   * @see #keyVars
  ...
  private Key makeKey( // final IVariable<?>[] keyVars,
      ...
      if (c == null) {
        ...
        if (!indexSolutionsHavingUnboundJoinVars) {
          ...
          if (log.isDebugEnabled())
            log.debug("Join variable is not bound: var=" + v + ", solution=" + bset);
    ...
    if (log.isTraceEnabled())
      log.trace("hashCode=" + h + ", joinVars=" + Arrays.toString(keyVars) + " : " + bset);
  ...
  public static class SolutionHit {
    ...
    public String toString() {
      ...
      return getClass().getName() + "{nhits=" + nhits + ",solution=" + solution + "}";
    }
  } // class SolutionHit
  ...
  public static class Bucket implements Iterable<SolutionHit>, Comparable<Bucket> {
    ...
    @Override
    public String toString() {
      return super.toString() + "{hashCode=" + hashCode + ",#solutions=" + solutions.size() + "}";
    }
     ...
     * DISTINCT filter. Further, only the "join" variables are "selected" and participate in a
     * DISTINCT filter. Therefore, if we have a hash collision such that two solutions would be
     * directed into the same {@link Bucket} then we can not improve matters but must simply scan
     * the solutions in the bucket to decide whether the new solution duplicates a solution which is
     * already present.
     *
     * @param solution The solution.
     * @return <code>true</code> iff the bucket was modified by this operation.
     */
    ...
    @Override
    public final int compareTo(final Bucket o) {
   ...
   * filtering for solutions which join. This is very expensive when compared to a hash join.
   * Whenever possible you should identify one or more variables which must be bound for the join
   * and specify those as the join variables.
   */
   ...
   *
   * <p>Note: This must be <code>true</code> for DISTINCT, OPTIONAL, and NOT EXISTS / MINUS since in
   * each case we do not want to drop solutions lacking a binding for some {@link #keyVars}. For
   * DISTINCT, this is because we want to project all solutions, regardless of unbound variables.
   * For OPTIONAL and NOT EXISTS / MINUS, this is because we must index all solutions since we will
   * report only those solutions that do not join. Once all solutions that do join have been
   * identified, the solutions that do not join are identified by a scan of the hash index looking
   * for {@link SolutionHit#nhits} equals ZERO (0L).
   */
   ...
   *     specified, then the join will consider the N x M cross product, filtering for solutions
   *     which join. This is very expensive when compared to a hash join. Whenever possible you
   *     should identify one or more variables which must be bound for the join and specify those as
   *     the join variables.
   * @param indexSolutionsHavingUnboundJoinVars When <code>true</code>, we allow solutions to be
   *     stored in the hash index that have unbound variables for the {@link #keyVars}. When <code>
   *     false</code>, such solutions are dropped (they are not added to the index).
   * @param map The backing map. A {@link HashMap} should be faster for insert and search. A {@link
   *     LinkedHashMap} should be faster for scans. Some join patterns do not require us to use
   *     scans, in which case {@link HashMap} is the clear winner. (For example, a non-optional hash
   *     join against an access path never uses the iterator over the hash index.)
   */
  public JVMHashIndex(
   ...
   * @return The {@link Key} iff the solution was added to the index and <code>null</code> iff the
   *     solution was not added (because a {@link Key} could not be formed for the solution given
   *     the specified {@link #keyVars}).
   */
   ...
   * solutions that (a) violate the join contract; and (b) that are present in the hash bucket due
   * to a hash collection rather than because they have the same bindings for the join variables.
   *
   * @param left The probe.
   * @return The hash {@link Bucket} into which the given solution is mapped -or- <code>null</code>
   *     if there is no such hash bucket.
   */
