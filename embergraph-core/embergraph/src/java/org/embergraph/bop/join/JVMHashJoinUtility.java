...
public class JVMHashJoinUtility implements IHashJoinUtility {
  ...
  public JVMHashJoinUtility(final PipelineOp op, final JoinTypeEnum joinType) {
    ...
    final IVariable<?>[] keyVars =
        filter ? (IVariable<?>[]) op.getProperty(JoinAnnotations.SELECT) : joinVars;
    ...
    rightSolutionsRef.set(
        new JVMHashIndex(
            ...
            new LinkedHashMap<>(
                ...
                op.getProperty(
                    HashMapAnnotations.LOAD_FACTOR, HashMapAnnotations.DEFAULT_LOAD_FACTOR))));
  }
  ...
  public long acceptSolutions(final ICloseableIterator<IBindingSet[]> itr, final BOpStats stats) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) log.debug("Materialized: " + all.length + " source solutions.");
   ...
   * variables for that source solution. A join hit counter is carried for each solution in the hash
   * index and is used to support OPTIONAL joins.
   */
  @Override
    ...
    try {
      ...
      while (leftItr.hasNext()) {
        ...
        for (IBindingSet left : leftChunk) {
          ...
          if (log.isDebugEnabled()) log.debug("Considering " + left);
          ...
          while (ritr.hasNext()) {
            ...
            if (log.isDebugEnabled()) log.debug("Join with " + right);
            ...
            final IBindingSet outSolution =
                BOpContext.bind(right.solution, left, constraints, selectVars);
   ...
   * @param outSolution The solution.
   */
  protected void outputSolution(
      final IBuffer<IBindingSet> outputBuffer, final IBindingSet outSolution) {

    if (log.isDebugEnabled()) log.debug("Output solution: " + outSolution);
  ...
  public void outputOptionals(final IBuffer<IBindingSet> outputBuffer) {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("rightSolutions: #buckets=" + rightSolutions.bucketCount());
      ...
      while (bitr.hasNext()) {
        ...
        for (SolutionHit hit : b) {
          ...
          if (f != null) {
            ...
            if (bs == hit.solution) bs = bs.clone();
          ...
          if (log.isDebugEnabled()) log.debug("Optional solution: " + bs);
        }
      }
    ...
    try {
      ...
      while (bucketIterator.hasNext()) {
        ...
        for (SolutionHit solutionHit : bucket) {
          ...
          if (log.isDebugEnabled()) log.debug("Output solution: " + bs);
        }
      }
   ...
   * @param all The sources for the join.
   * @return The combined constraints and <code>null</code> iff there are no constraints.
   */
   ...
   * if this is a required join, this method will return <code>false</code> and the caller must
   * advance to the next bucket in the first source; or (b) if this is an optional join, there will
   * be a <code>null</code> in the <i>currentBucket[]</i> for that source.
   *
   * @param sortedSourceBuckets An array of {@link Bucket}[]s for each source. The vector of {@link
   *     Bucket}s for each source has been sorted. This means that we can scan down those vectors
   *     and observe {@link Bucket} s having strictly increasing hash codes for each source.
   * @param sourceIndex The next index into each source.
   * @param currentBucket The current bucket for each source.
   * @param optional <code>true</code> iff this is an optional join.
   * @return <code>true</code> if we are on a bucket which might join. if this method returns <code>
   *     false</code>, then the caller should immediately advance to the next bucket from the first
   *     source without attempting a join.
   */
  private static boolean advanceOtherSources(
  ...
  public void mergeJoin(
    ...
    while (sourceIndex[0] < sortedSourceBuckets[0].length) {
      ...
      if (advanceOtherSources(sortedSourceBuckets, sourceIndex, currentBucket, optional)) {
   ...
   * solutions in each source bucket. All buckets will have the same hash code. If this is an
   * optional join, then some entries in buckets[] MAY be <code>null</code>. However, the first
   * entry is never <code>null</code> since that is the primary source for the join.
   *
   * @param currentBucket The current {@link Bucket} from each source. The first entry in this array
   *     is the source from which optional solutions will be reported if the join is optional.
   * @param constraints
