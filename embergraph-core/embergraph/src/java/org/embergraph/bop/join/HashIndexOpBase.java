...
public abstract class HashIndexOpBase extends PipelineOp implements ISingleThreadedOp {
  ...
  protected HashIndexOpBase(final BOp[] args, final Map<String, Object> annotations) {
    ...
    final IHashJoinUtilityFactory factory =
        (IHashJoinUtilityFactory) getRequiredProperty(Annotations.HASH_JOIN_UTILITY_FACTORY);
   ...
   */
  protected abstract static class ChunkTaskBase implements Callable<Void> {
    ...
    public ChunkTaskBase(final HashIndexOpBase op, final BOpContext<IBindingSet> context) {
...

      {
        ...
        if (state == null) {
          ...
          final JoinTypeEnum joinType =
              (JoinTypeEnum) op.getRequiredProperty(Annotations.JOIN_TYPE);

          final IHashJoinUtilityFactory factory =
              (IHashJoinUtilityFactory)
                  op.getRequiredProperty(Annotations.HASH_JOIN_UTILITY_FACTORY);
     ...
     * output by this operator could appear at the {@link SolutionSetHashJoinOp} before this
     * operator is done and it would have the mutable view of the {@link HTree} rather than the
