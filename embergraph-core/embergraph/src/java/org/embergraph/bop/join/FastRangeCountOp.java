...
public class FastRangeCountOp<E> extends PipelineOp {
  ...
  public interface Annotations extends AccessPathJoinAnnotations {
    ...
    /*
     * The name of the variable that will be bound to the fast-range count of the access path
   ...
   * @param annotations
   */
  public FastRangeCountOp(final BOp[] args, final Map<String, Object> annotations) {
  ...
  protected static class ChunkTask<E> implements Callable<Void> {
    ...
    protected ChunkTask(final FastRangeCountOp<E> op, final BOpContext<IBindingSet> context) {
    ...
    protected long determineRangeCount(final IPredicate<E> pred) {
       ...
       *
       * <p>Note: The exact range count using will be two key probes unless the index supports
       * delete markers or there is a filter attached to the access path.
       *
       * <p>Note: This will throw an exception if either of those conditions is true (the index
       * supports delete markers or there is a filter attached to the access path). The exception is
       * thrown since those conditions change the cost of this operator from O(2) (the cost of TWO
       * key probes) to the O(fast-range-count) (the cost of a key-range scan). Thus, generating
       * this operator when those conditions are violated leads to incorrect reasoning about the
       * cost of the operator.
       */
      final IAccessPath<E> accessPath = context.getAccessPath(relation, pred);
