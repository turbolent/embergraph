 ...
 * @version $Id$
 */
public class HTreePipelinedHashJoinUtility extends HTreeHashJoinUtility
  ...
  public HTreePipelinedHashJoinUtility(
      PipelineOp op, JoinTypeEnum joinType, BOpContext<IBindingSet> context, int chunkCapacity) {
  ...
  public long acceptAndOutputSolutions(
    ...
    if (!dontRequireSubqueryEvaluation.isEmpty()) {
      ...
      hashJoinAndEmit(
          dontRequireSubqueryEvaluation.toArray(new IBindingSet[0]),
          stats,
          out,
          joinConstraints,
          askVar);
    ...
    try {
      ...
      runningSubquery =
          queryEngine.eval(subquery, distinctProjectionBuffer.toArray(new IBindingSet[0]));
      ...
      try {
        ...
        final BS[] vectoredDistinctProjectionBuffer =
            vector(
                distinctProjectionBuffer.toArray(new IBindingSet[0]),
                getJoinVars(),
                null,
                false,
                vectorSize);
    ...
    hashJoinAndEmit(
        incomingBindingsBuffer.toArray(new IBindingSet[0]), stats, out, joinConstraints, askVar);
   ...
   * @return
   */
  protected boolean joinsWith(final BS bs, final IKeyBuilder keyBuilder, final HTree htree) {
    ...
    while (titr.hasNext()) {
      ...
      if (BOpContext.bind(bs.bset, joinCandidate, null /* unconstrained */, getSelectVars())
          != null) {
        return true;
      }
    }
  ...
  void addBindingsSetSourceToHashIndexOnce(
      final HTree rightSolutions, final IBindingSet[] bsFromBindingsSetSource) {
          ...
          while (titr.hasNext()) {
            ...
            for (int i = fromIndex; i < toIndex; i++) {
              ...
              final IBindingSet outSolution =
                  BOpContext.bind(leftSolution, rightSolution, getConstraints(), getSelectVars());

              nJoinsConsidered.increment();

              if (noJoinVars && nJoinsConsidered.get() == getNoJoinVarsLimit()) {
            ...
            case Exists:
              {
                ...
                if (askVar != null) {
                  leftSolutionWithoutMatch.set(
                      askVar, new Constant<XSDBooleanIV<?>>(XSDBooleanIV.valueOf(false)));
                  outputBuffer.add(leftSolutionWithoutMatch);
                }
                break;
              }
