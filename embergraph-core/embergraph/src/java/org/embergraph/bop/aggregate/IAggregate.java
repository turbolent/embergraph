...
public interface IAggregate<E> extends IValueExpression<E> {
   ...
   *
   * <p>Note: If evaluation of the {@link IAggregate} throws an error, then that error must be
   * "sticky" and reported out by {@link #done()} as well. This contract is relied on to correctly
   * propagate errors within a group when using incremental (pipelined) evaluation of {@link
   * IAggregate}s. The error state is cleared by {@link #reset()}.
   */
   ...
   *     then that error is "sticky" and the first such error encountered will be thrown out of
   *     {@link #done()} as well.
   */
  E done();
}
