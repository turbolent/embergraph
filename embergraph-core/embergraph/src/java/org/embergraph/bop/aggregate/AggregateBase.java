...
public abstract class AggregateBase<E> extends ImmutableBOp implements IAggregate<E> {
   ...
   *     COUNT(DISTINCT x)</code> or <code>y+x</code> in <code>MIN(x+y)</code>. Note that only COUNT
   *     may be used with the special variable <code>*</code>.
   */
  //    * @param functionCode
   ...
   * AggregateBase} is to be directly evaluated. However, note that the computation of aggregate
   * functions is often based on hard coded recognition of the appropriate function code.
   *
   * <p>Note: DISTINCT is merely carried as a marker on COUNT (and the other aggregate functions).
   * The application of DISTINCT to the inner expression to form a column projection is handled by
   * the driving evaluation logic rather than by {@link #get(IBindingSet)}.
   */
