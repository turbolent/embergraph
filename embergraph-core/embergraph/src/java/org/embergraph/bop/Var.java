 ...
 * @version $Id$
 * @todo it is not entirely satisfactory to have variable names disappear from rules as they are
 *     specialized. It might be better to define a BoundVar (extends Var) whose id was the bound
 *     value and which reported true for both isVariable() and isConstant(). However this would mean
 *     that var(x) != var(x,id) where the latter is bound to a constant. Or if {@link Var} is
 *     rule-local (or rule execution thread local) then it could define set(Object) and get():Object
 *     to access its binding. A rule that was specialized would then flag bound variables as
 *     immutable.
 * @todo Variable canonicalization could be limited in scope to a rule or its derived rules (via
 *     specialization or for truth maintenance). This would make it easier to place constraints
 *     directly on the variable so that it can limit the types of binding that it will accept.
 *     Constraints on {@link IPredicate}s limit the matched tuples. Constraints on rules limit the
 *     binding patterns across the rule. (In fact, variables can be "named" by their index into the
 *     binding set for most purposes.)
 */
public final class Var<E> extends ImmutableBOp implements IVariable<E>, Comparable<IVariable<E>> {
  ...
  /** @todo Why two versions of equals? This one is coming from IConstantOrVariable. */
  @Override
   ...
   * variable name, the "scope" of the variable is always constrained by the rule within which it is
   * used. The purpose of the singleton factory is to let us test for the same variable using "=="
   * (reference testing) and also to have a shorthand for variable creation.
   *
   * @param name The variable name.
   * @return The singleton variable for that name.
   */
  public static Var var(final String name) {
