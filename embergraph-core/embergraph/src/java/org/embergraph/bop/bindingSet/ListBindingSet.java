...
public class ListBindingSet implements IBindingSet {
   ...
   * @param vals A copy is made of the data.
   */
   ...
   * @param variablesToKeep The variables to be retained for the symbol table on the top of the
   *     stack (optional).
   */
  @SuppressWarnings("rawtypes")
  protected ListBindingSet(final ListBindingSet src, final IVariable[] variablesToKeep) {
   ...
   * @param variablesToKeep When non-<code>null</code>, only the bindings for the variables listed
   *     in this array will copied.
   * @return The copy.
   */
  @SuppressWarnings("rawtypes")
  private List<E> copy(final List<E> src, final IVariable[] variablesToKeep) {
    ...
    while (itr.hasNext()) {
      ...
      if (keep) dst.add(new E(e.var, e.val));
    }
  ...
  private List<E> copyMinusErrors(final List<E> src, final IVariable[] variablesToKeep) {
    ...
    while (itr.hasNext()) {
      ...
      if (e.val == Constant.errorValue()) continue;
  ...
  public final IBindingSet copyMinusErrors(final IVariable[] variablesToKeep) {
    return new ListBindingSet(copyMinusErrors(this.current, variablesToKeep));
  }
  ...
  public final boolean containsErrorValues() {
    for (E e : this.current) {
      ...
      if (e.val == Constant.errorValue()) return true;
    }
    return false;
  }
  ...
  public Iterator<Map.Entry<IVariable, IConstant>> iterator() {
    ...
    return (Iterator<Map.Entry<IVariable, IConstant>>) ((List) current()).iterator();
  }
  ...
  public Iterator<IVariable> vars() {
    return (Iterator<IVariable>)
        new Striterator(Collections.unmodifiableList(current()).iterator())
            .addFilter(
                new Resolver() {
                  private static final long serialVersionUID = 1L;
