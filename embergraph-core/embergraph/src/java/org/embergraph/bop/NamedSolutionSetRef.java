 ...
 * <dl>
 *   <dt>queryId
 *   <dd>The {@link UUID} of the query which generated the named solution set. This provides the
 *       scope for the named solution set. It is used to (a) locate the data; and (b) release the
 *       data when the query goes out of scope.
 *   <dt>namedSet
 *   <dd>The "name" of the solution set as given in the query. The name is not a sufficient
 *       identifier since the same solution set name may be used in different queries and with
 *       different join variables.
 *   <dt>joinVars[]
 *   <dd>The ordered array of the join variable. This serves to differentiate among named solution
 *       sets having the same data but different join variables.
 * </dl>
 *
...
public class NamedSolutionSetRef implements INamedSolutionSetRef {
   ...
   *
   * <p>Note: The value <code>null</code> has two different interpretations depending on which
   * constructor form was used.
   *
   * <p>
   *
   * <dl>
   *   <dt>new NamedSolutionSetRef(queryId, namedSet, joinVars);
   *   <dd>This form is used to refer to an {@link org.embergraph.bop.IQueryAttributes}. When
   *       queryId is <code>null</code>, the code will look at the {@link
   *       org.embergraph.bop.IQueryAttributes} for the current running query (this form is the new
   *       usage and makes it possible to scope an attribute to an instance of a query when the same
   *       query plan is executed more than once, as it is for the sub-queries issued by the
   *       property path operator). Otherwise it will resolve the identified query and use its
   *       {@link org.embergraph.bop.IQueryAttributes} (this 2nd form is the historical usage and
   *       makes it possible to reference an attribute on a parent query).
   *   <dt>NamedSolutionSetRef(namespace, timestamp, localName, joinVars);
   *   <dd>This form is used to refer to a named solution set.
   * </dl>
   *
   ...
   */
  @SuppressWarnings("rawtypes")
  NamedSolutionSetRef(final UUID queryId, final String namedSet, final IVariable[] joinVars) {

    //    	See BLZG-1493
    //        if (queryId == null)
    //            throw new IllegalArgumentException();

    if (namedSet == null) throw new IllegalArgumentException();
    ...
    if (fqn == null) {
      ...
      synchronized (localName) {
        if (namespace == null) {
          ...
          fqn = NamedSolutionSetRefUtility.getFQN(namespace, localName, joinVars);
        }
      }
    }
  ...
  public int hashCode() {
    if (h == 0) {
      ...
      h =
          (queryId == null
                  ? namespace == null ? 0 : namespace.hashCode() + (int) timestamp
                  : queryId.hashCode())
              + localName.hashCode()
  ...
  public boolean equals(final Object o) {
    ...
    if (!(o instanceof NamedSolutionSetRef)) return false;

    final NamedSolutionSetRef t = (NamedSolutionSetRef) o;

    if (queryId != null) {

      if (!queryId.equals(t.queryId)) return false;

    } else {

      if (!namespace.equals(t.namespace)) return false;
