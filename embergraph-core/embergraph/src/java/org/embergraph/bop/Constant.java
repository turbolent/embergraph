...
public final class Constant<E> extends ImmutableBOp implements IConstant<E> {
  ...
  public interface Annotations extends ImmutableBOp.Annotations {
     ...
     * propagating the binding onto the variable for solutions which join.
     *
     * <p>Note: The {@link Var} class in embergraph provides a guarantee of reference testing for
     * equality, which is why we can not simply attach the constant to the variable and have the
     * variable report its bound value. *
     */
  ...
  public Constant(final Constant<E> op) {
    ...
    if (op == Constant.ERROR_VALUE) throw new IllegalArgumentException();
   ...
   * solutions are joined the resulting solutions will have the variable with its constant bound
   * value.
   *
   * <p>Note: The {@link Var} class in embergraph provides a guarantee of reference testing for
   * equality, which is why we can not simply attach the constant to the variable and have the
   * variable report its bound value. *
   *
   * <p>Note: A very similar effect may be achieved by simply binding the variable on the {@link
   * IBindingSet} to the constant. However, there are some (few) cases where we can not do that
   * because the binding must be applied for all solutions and we lack access to the input
   * solutions.
   *
   * <p>See {@link BOpContext#bind(IPredicate, IConstraint[], Object, IBindingSet)}, which takes
   * care of propagating the binding onto the variable for solutions which join.
   *
  ...
  public final boolean equals(final IVariableOrConstant<E> o) {
    ...
    return ((Constant) o).value != null && value.equals(((Constant) o).value);
  }
   ...
   */
  @Override
  public final E get(final IBindingSet bindingSet) {
