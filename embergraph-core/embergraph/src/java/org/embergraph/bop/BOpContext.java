...
public class BOpContext<E> extends BOpContextBase {
   ...
   *
   * <ol>
   *   <li>{@link PipelineOp.Annotations#LAST_PASS} is <code>true</code>
   *   <li>{@link PipelineOp.Annotations#PIPELINED} is <code>true</code>
   *   <li>{@link PipelineOp.Annotations#MAX_PARALLEL} is <code>1</code>
   * </ol>
   *
   * Under these circumstances, it is possible for the {@link IQueryClient} to atomically decide
   * that a specific invocation of the operator task for the query will be the last invocation for
   * that task. This is not possible if the operator allows concurrent evaluation tasks. Sharded
   * operators are intrinsically concurrent since they can evaluate at each shard in parallel. This
   * is why the evaluation context is locked to the query controller. In addition, the operator must
   * declare that it is NOT thread safe in order for the query engine to serialize its evaluation
   * tasks.
   */
   ...
   * controller will have access to additional state, such as the aggregation of the {@link
   * BOpStats} for the query on all nodes.
   */
  public IRunningQuery getRunningQuery() {
    return runningQuery;
  }

  /** The index partition identifier -or- <code>-1</code> if the index is not sharded. */
   ...
   *     sharded.
   * @param stats The object used to collect statistics about the evaluation of this operator.
   * @param source Where to read the data to be consumed by the operator.
   * @param op The operator that is being executed.
   * @param lastInvocation <code>true</code> iff this is the last invocation pass for that operator.
   * @param sink Where to write the output of the operator.
   * @param sink2 Alternative sink for the output of the operator (optional). This is used by things
   *     like SPARQL optional joins to route failed joins outside of the join group.
   * @throws IllegalArgumentException if the <i>stats</i> is <code>null</code>
   * @throws IllegalArgumentException if the <i>source</i> is <code>null</code> (use an empty source
   *     if the source will be ignored).
   * @throws IllegalArgumentException if the <i>sink</i> is <code>null</code>
   * @todo Modify to accept {@link IChunkMessage} or an interface available from getChunk() on
   *     {@link IChunkMessage} which provides us with flexible mechanisms for accessing the chunk
   *     data.
   *     <p>When doing that, modify to automatically track the {@link BOpStats} as the <i>source</i>
   *     is consumed.
   *     <p>Note: The only call to this method outside of the test suite is from
   *     ChunkedRunningQuery. It always has a fully materialized chunk on hand and ready to be
   *     processed.
   */
  public BOpContext(
    ...
    this(
        runningQuery,
        runningQuery.getFederation(),
        runningQuery.getLocalIndexManager(),
        partitionId,
        stats,
        op,
        lastInvocation,
   ...
   * @param localIndexManager The <strong>local</strong> index manager (required).
   * @param runningQuery The {@link IRunningQuery} (required).
   * @param partitionId The index partition identifier -or- <code>-1</code> if the index is not
   *     sharded.
   * @param stats The object used to collect statistics about the evaluation of this operator.
  ...
  private static class SetContextIterator implements ICloseableIterator<IBindingSet[]> {
    ...
    public SetContextIterator(
        final BOpContext<?> context, final ICloseableIterator<IBindingSet[]> src) {
    ...
    public boolean hasNext() {
       ...
       */
      this.cur =
          nxt.getClass().getComponentType() == IBindingSet.class
              ? nxt
   ...
   * @throws RuntimeException if the {@link IRunningQuery} has halted.
   * @throws RuntimeException if the {@link IRunningQuery} is not found.
   */
   ...
   *     IQueryAttributes} of this query.
  ...
  //            final SolutionSetStreamPredicate<IBindingSet> p =
  // (SolutionSetStreamPredicate<IBindingSet>) predicate;
  //
   ...
   *
   * @return An iterator visiting the solutions to be indexed and never <code>null</code>.
   * @throws RuntimeException if the source can not be resolved.
  ...
  public ICloseableIterator<IBindingSet[]> getAlternateSource(
    ...
    if (queryId != null) {
       ...
       * to be interpreted as indicating the current query. However this code path is pre-existing
       * and is used to locate an alternative source based on a NamedSolutionSetRef. When the
       * queryID is non-null (above) we look at the specified query. When it is null (here) we look
       * at the index manager for a durable NamedSolutionSet.
       *
       * <p>BLZG-1493 might need to be applied here at some point. Right now, this code path
       * provides access to a durable named solution set. If we need per-query scoped named solution
       * sets that for cases with multiple executions of the same sub-query (this is the case that
       * motivates BLZG-1493 - multiple sub-query evaluation for the same property path) then we
       * would need to refactor the NamedSolutionSetRef to de-conflict these various use cases.
       *
      ...
      final ISolutionSetManager sparqlCache =
          new SolutionSetManager(localIndexManager, namespace, timestamp);
   ...
   * have a side-effect on the bindings even if the constraints were not satisfied. Therefore you
   * should clone the bindings before calling this method.
   *
   * @param pred The {@link IPredicate} from which the element was read.
   * @param constraint A constraint which must be satisfied (optional).
   * @param e An element materialized by the {@link IAccessPath} for that {@link IPredicate}.
   * @param bindingSet the bindings to which new bindings from the element will be applied.
   * @return <code>true</code> unless the new bindings would violate any of the optional {@link
   *     IConstraint}.
   ...
   * {@link IBindingSet}.
   *
   * <p>Note: A variable which is bound outside of the query to a constant gets turned into a {@link
   * Constant} with that variable as its annotation. This method causes the binding to be created
   * for the variable and the constant when the constant is JOINed.
   *
   * @param e The element.
   * @param pred The predicate.
   * @param bindingSet The binding set, which is modified as a side-effect.
   *     <p>TODO Make this method package private once we convert to using an inline access path.
   */
  @SuppressWarnings("unchecked")
   ...
   *
   * @param left The left binding set (target).
   * @param right The right binding set (source).
   * @param constraints An array of constraints (optional). When given, destination {@link
   *     IBindingSet} will be validated <em>after</em> mutation.
   * @param varsToKeep An array of variables whose bindings will be retained. The bindings are not
   *     stripped out until after the constraint(s) (if any) have been tested.
   * @return The solution with the combined bindings and <code>null</code> if the bindings were not
   *     consistent, if a constraint was violated, etc. Note that either <code>left</code> or <code>
   *     right</code> MAY be returned if the other solution set is empty (optimization).
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  public static IBindingSet bind(
        ...
        if (sval != null) {
          ...
          if (dval != null) {
            ...
            if (!sval.equals(dval)) {
              ...
              //                            log.error("FAIL : " + var + " have " + sval + " and " +
              // dval);
              return null;
   ...
   * @param pred The predicate for that {@link IAccessPath}
   * @param stats Statistics to be updated as elements and chunks are consumed (optional).
   * @return An iterator visiting chunks of solutions. The order of the original {@link IElement}s
   *     is preserved.
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/209 (AccessPath should visit binding sets
   *     rather than elements when used for high level query.)
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/233 (Inline access path).
   *     <p>TODO Move to {@link IAccessPath}? {@link AccessPath}?
   */
  //    * @param vars
  ...
  public ICloseableIterator<IBindingSet[]> solutions(
    ...
    final IChunkedStriterator itr1 =
        new org.embergraph.striterator.ChunkedStriterator(src)
            .addFilter(
                //                        new ChunkedFilter() {
            ...
            .addFilter(
                new org.embergraph.striterator.Resolver() {
    ...
    final ICloseableIterator<IBindingSet[]> itr2 =
        new CloseableChunkedIteratorWrapperConverter<IBindingSet>(itr1);
