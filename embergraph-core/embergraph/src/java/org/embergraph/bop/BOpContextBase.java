...
public class BOpContextBase {
   ...
   * indices and to other embergraph services.
   */
  public final IEmbergraphFederation<?> getFederation() {
   ...
   * @param localIndexManager The <strong>local</strong> index manager.
   */
  public BOpContextBase(final IEmbergraphFederation<?> fed, final IIndexManager localIndexManager) {
   ...
   * @return The {@link IRelation} -or- <code>null</code> if the relation did not exist for that
   *     timestamp.
   * @todo Replaces {@link IJoinNexus#getTailRelationView(IPredicate)}. In order to support mutation
   *     operator we will also have to pass in the {@link #writeTimestamp} or differentiate this in
   *     the method name.
   */
   ...
   * @param timestamp The timestamp of the view of that relation.
   * @return The {@link ILocatableResource} -or- <code>null</code> if the relation did not exist for
   *     that timestamp.
   */
  public ILocatableResource<?> getResource(final String namespace, final long timestamp) {
     ...
     */
    final IIndexManager tmp = getFederation() == null ? getIndexManager() : getFederation();
   ...
   * IPredicate}.
   *
   * <p>Note: Passing in the {@link IRelation} is important since it otherwise must be discovered
   * using the {@link IResourceLocator}. By requiring the caller to resolve it before hand and pass
   * it into this method the contention and demand on the {@link IResourceLocator} cache is reduced.
   *
   * <p>
   *
   * <h2>Scale-Out</h2>
   *
   * <p>Note: You MUST be extremely careful when using expanders with a local access path for a
   * shared-partitioned or hash-partitioned index. Only expanders whose semantics remain valid with
   * a partial view of the index will behave as expected. Here are some examples that DO NOT work:
   *
   * <ul>
   *   <li>"DISTINCT" on a partitioned local access path is not coherent
   *   <li>Expanders which generate reads against keys not found on that shard are not coherent.
   * </ul>
   *
   * If you have requirements such as these, then either use a remote access path or change your
   * query plan design more radically to take advantage of efficient shard-wise scans in scale-out.
   *
   ...
   */
  public <E> IAccessPath<E> getAccessPath(
      final IRelation<E> relation, final IPredicate<E> predicate) {
