...
public abstract class PipelineOp extends BOpBase {
  ...
  public interface Annotations extends BOp.Annotations, BufferAnnotations {
     ...
     * instance.
     *
     * <p>Note: {@link BOp#getEvaluationContext()} MUST be overridden to return {@link
     * BOpEvaluationContext#CONTROLLER} if this annotation is overridden to <code>true</code>.
     *
     * <p>When <code>true</code>, the {@link QueryEngine} will impose the necessary constraints when
     ...
     *     always preserved. </a>
     */
    String REORDER_SOLUTIONS = PipelineOp.class.getName() + ".reorderSolutions";
     ...
     * this value can limit the concurrency with which certain operators are evaluated and that can
     * have a negative effect on the throughput, it controls both the demand on the JVM heap and the
     * #of threads consumed.
     *
     * <p>Note: {@link #MAX_PARALLEL} is the annotation for pipelined joins which has the strongest
     * effect on performance. Changes to both {@link #MAX_MESSAGES_PER_TASK} and {@link
     * #PIPELINE_QUEUE_CAPACITY} have less effect and performance tends to be best around a modest
     ...
     * combine multiple {@link IChunkMessage}s from the work queue of an operator for each
     * evaluation pass made for that operator. When ONE (1), each {@link IChunkMessage} will be
     * assigned to a new evaluation task for the operator. The value of this annotation must be a
     * positive integer. If the operator is not-pipelined, then the maximum amount of data to be
     * assigned to an evaluation task is governed by {@link #MAX_MEMORY} instead.
     */
    String MAX_MESSAGES_PER_TASK = PipelineOp.class.getName() + ".maxMessagesPerTask";
     ...
     * query evaluation.
     */
    String PIPELINE_QUEUE_CAPACITY = PipelineOp.class.getName() + ".pipelineQueueCapacity";
     ...
     *
     * <p>If an operator buffers its data on the Java heap then this MUST be ZERO (0L). At-once
     * evaluation is always used for non-pipelined operators which buffer their data on the Java
     * heap as there is no ready mechanism to bound their heap demand.
     *
     * <p>If an operator buffers its data on the native heap, then this MUST be some positive value
     * which specifies the maximum #of bytes which may be buffered before the operator is evaluated.
     * At-once evaluation for operators which buffer their data on the native heap is indicated with
     * the value {@link Long#MAX_VALUE}.
     *
     * <p>Note: For a sharded operation, the value is the maximum #of bytes which may be buffered
     * per shard.
     *
     * @see PipelineOp#isPipelinedEvaluation()
     */
     ...
     * BOpContext#isLastInvocation()} will report <code>true</code>.
     *
     * <p>Note: A final evaluation pass will be triggered even if the operator was never triggered
     * by a normal evaluation pass. This behavior is necessary for several language constructs.
     * Operators are free to do nothing if they can ignore the final evaluation pass in this case.
     */
   ...
   * @param annotations
   */
  protected PipelineOp(final BOp[] args, final Map<String, Object> annotations) {
  ...
  public final int getChunkCapacity() {
    ...
    return getProperty(Annotations.CHUNK_CAPACITY, Annotations.DEFAULT_CHUNK_CAPACITY);
  }
  ...
  public final int getChunkOfChunksCapacity() {
    ...
    return getProperty(
        Annotations.CHUNK_OF_CHUNKS_CAPACITY, Annotations.DEFAULT_CHUNK_OF_CHUNKS_CAPACITY);
  }

  /** @see BufferAnnotations#CHUNK_TIMEOUT */
  public final long getChunkTimeout() {

    return getProperty(Annotations.CHUNK_TIMEOUT, (long) Annotations.DEFAULT_CHUNK_TIMEOUT);
  }
   ...
   * Together with {@link Annotations#PIPELINED}, {@link Annotations#MAX_MEMORY} is used to
   * determine whether an operator uses <i>pipelined</i>, <i>at-once</i>, or <i>blocked</i>
   * evaluation.
   *
   ...
   * <dl>
   *   <dt>Pipelined
   *   <dd>Pipelined operators stream chunks of intermediate results from one operator to the next
   *       using a producer / consumer pattern. Each time a set of intermediate results is available
   *       for a pipelined operator, it is evaluated against those inputs producing another set of
   *       intermediate results for its target operator(s). Pipelined operators may be evaluated
   *       many times during a given query and often have excellent parallelism due to the
   *       concurrent evaluation of the different operators on different sets of intermediate
   *       results.
   *   <dt>At-Once
   *   <dd>An "at-once" operator will run exactly once and must wait for all of its inputs to be
   *       assembled before it runs. There are some operations for which "at-once" evaluation is
   *       always required, such as ORDER_BY. Other operations MAY use operator-at-once evaluation
   *       in order to benefit from a combination of more efficient IO patterns and simpler design.
   *       At-once operators may either buffer their data on the Java heap (which is not scalable
   *       due to the heap pressure exerted on the garbage collector) or buffer their data on the
   *       native heap (which does scale).
   *   <dt>Blocked
   *   <dd>Blocked operators buffer large amounts of data on the native heap and run each time they
   *       exceed some threshold #of bytes of buffered data. A blocked operator is basically an
   *       "at-once" operator which buffers its data on the native heap and which can be evaluated
   *       in multiple passes. For example, a hash join could use a blocked operator design while an
   *       ORDER_BY operator can not. By deferring their evaluation until some threshold amount of
   *       data has been materialized, they may be evaluated once or more than once, depending on
   *       the data scale, but still retain many of the benefits of "at-once" evaluation in terms of
   *       IO patterns. Whether or not an operator can be used as a "blocked" operator is a matter
   *       of the underlying operator implementation.
   * </dl>
   *
  ...
  public final boolean isAtOnceEvaluation() {
    ...
    if (isPipelinedEvaluation()) return false;
   ...
   * </pre>
   *
   * When the operator is not pipelined then it is either "blocked" or "at-once". When MAX_MEMORY is
   * ZERO, the operator will buffer its data on the Java heap. All operators which buffer data on
   * the java heap will buffer an unbounded amount of data and are therefore "at-once" rather than
   * "blocked". Operators which buffer their data on the native heap may support either "blocked"
   * and/or "at-once" evaluation, depending on the operators. E.g., a hash join can be either
   * "blocked" or "at-once" while an ORDER-BY is always "at-once".
   */
  ...
  /** If parallel evaluation is not allowed, then throws {@link IllegalArgumentException}. */
  protected final void assertMaxParallelOne() {
    ...
    if (getProperty(
      ...
      throw new IllegalArgumentException(
          PipelineOp.Annotations.MAX_PARALLEL + "=" + getMaxParallel());
    }
  }
   ...
   * operator evaluation task is not thread-safe.
   *
   ...
   * @todo Modify to return a {@link Callable}s for now since we must run each task in its own
   *     thread until Java7 gives us fork/join pools and asynchronous file I/O. For the fork/join
   *     model we will probably return the ForkJoinTask.
   */
