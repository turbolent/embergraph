 ...
 * @version $Id$
 * @todo Add a parameter for the write retention queue? The capacity of the queue could be turned
 *     into an estimate of the #of nodes and leaves buffered. Alternatively, we have an estimate of
 *     the #of distinct nodes and leaves on the queue in {@link
 *     AbstractBTree#ndistinctOnWriteRetentionQueue}. With that, we could decide how likely it is
 *     that the first N leaves of the {@link BTree} are in the cache. However, this is all fuzzy
 *     since a focus on one branch of the {@link BTree} could cause nothing but the root to be in
 *     the cache when probing a different branch.
 */
public class BTreeCostModel implements Serializable {
  ...
  public BTreeCostModel(final DiskCostModel diskCostModel) {
    ...
    if (diskCostModel == null) throw new IllegalArgumentException();
   ...
   */
  public static int estimateHeight(final int entryCount, final int branchingFactor) {
   ...
   * @param m The B+Tree branching factor.
   * @param h The B+Tree height.
   * @param leafUtilization The leaf utilization percentage [0:100].
   * @return The estimated cost (milliseconds).
   */
  public double rangeScan(
      final long rangeCount, final int m, final int h, final int leafUtilization) {

    if (rangeCount == 0) return 0d;

    // average seek time to a leaf.
    final double averageSeekTime = Math.max(0, (h - 1)) * diskCostModel.seekTime;
     ...
     * (fillRate is zero).
     */
    final double expectedLeafCount =
        Math.ceil((((double) rangeCount) / m) * Math.min(1, (1 / leafFillRate)));
   ...
   *
   * @param args ignored.
   * @see <a href="src/resources/architectures/query-cost-model.xls">query-cost-model.xls</a>
   */
  public static void main(String[] args) {
    ...
    for (DiskCostModel diskCostModel : diskCostModels) {
      ...
      final BTreeCostModel btreeCostModel = new BTreeCostModel(diskCostModel);
       ...
       * Skip range counts which are too large for the
            ...
            final double cost = btreeCostModel.rangeScan(rangeCount, m, h, leafUtilization);
