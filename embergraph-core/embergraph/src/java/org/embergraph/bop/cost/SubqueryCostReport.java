...
public class SubqueryCostReport implements Serializable {
   ...
   * @param rangeCount An estimated range count based on the samples and adjusted for the #of
   *     graphs.
   * @param cost An estimated cost (latency in milliseconds) based on the samples and adjusted for
   *     the #of graphs.
   */
  public SubqueryCostReport(
      final int ngraphs,
      final int limit,
      final int nsamples,
    ...
    if (ngraphs < 0) throw new IllegalArgumentException();

    if (limit < 1) throw new IllegalArgumentException();

    if (nsamples < 0) throw new IllegalArgumentException();
