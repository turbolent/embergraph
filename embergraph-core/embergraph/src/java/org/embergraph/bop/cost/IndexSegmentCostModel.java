 ...
 * probe them all for their exact costs.
 *
...
public class IndexSegmentCostModel implements Serializable {
   ...
   * @param xferBufferSize The size of the disk transfer buffer.
   * @return The estimated time for the range scan (milliseconds).
   */
  public double rangeScan(
      final int rangeCount,
      final int branchingFactor,
      final int averageBytesPerLeaf,
      final int xferBufferSize) {

    if (rangeCount == 0) return 0d;

    if (xferBufferSize == 0) throw new IllegalArgumentException();
    ...
    final int xfers = (int) Math.ceil(((double) leafBytesToXFer) / xferBufferSize);

    // Expected transfer time (ms).
    final double xferTime = leafBytesToXFer / (diskCostModel.transferRate / 1000);
