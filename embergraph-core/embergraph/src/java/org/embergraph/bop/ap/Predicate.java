...
public class Predicate<E> extends AbstractAccessPathOp<E> implements IPredicate<E> {
   ...
   * @param relationName Identifies the relation to be queried.
   */
  public Predicate(
      final IVariableOrConstant<?>[] values, final String relationName, final long timestamp) {
   ...
   * @param values The values (order is important!).
   * @param optional true iff the predicate is optional when evaluated in a JOIN.
   * @param constraint An optional constraint.
   * @param expander Allows selective override of the predicate evaluation.
   */
  public Predicate(
      final IVariableOrConstant<?>[] values,
      final String relationName,
      final int partitionId,
      final boolean optional,
      final IElementFilter<E> constraint,
      final IAccessPathExpander<E> expander,
  ...
  public String getOnlyRelationName() {
    ...
    if (relationName.length != 1) throw new IllegalStateException();
  ...
  public final int getVariableCount() {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (get(i) instanceof IVariable<?>) nvars++;
    }

    return nvars;
  }

  public final boolean isFullyBound(final IKeyOrder<E> keyOrder) {

    return getVariableCount(keyOrder) == 0;
  }

  public final int getVariableCount(final IKeyOrder<E> keyOrder) {
    int nunbound = 0;
  ...
  public final boolean isRemoteAccessPath() {
    return getProperty(Annotations.REMOTE_ACCESS_PATH, Annotations.DEFAULT_REMOTE_ACCESS_PATH);
  }
   ...
   * {@link #asBound(IBindingSet)}. That provides a significant reduction in heap churn.
   */
  public Predicate<E> asBound(final IBindingSet bindingSet) {
    ...
    return new Predicate<E>(argsCopy(), anns)._asBound(bindingSet);
  }
  ...
  public IKeyOrder<E> getKeyOrder() {
    ...
    if (o instanceof IKeyOrder<?>) return (IKeyOrder<E>) o;
   ...
   * related to a join intensive process in {@link CoreBaseBOp#clone()} using reflection to make a
   * copy of an {@link #asBound(IBindingSet)} predicate.
   */
  @Override
  //    @SuppressWarnings("unchecked")
  ...
  private void addFilter(final String name, final IFilter filter) {
    ...
    if (current == null) {
       ...
       * Wrap the filter.
       */
      _setProperty(name, new NOPFilter().addFilter(current).addFilter(filter));
    }
  }
    ...
    for (int i = 0; i < arity; i++) {
      ...
      sb.append(
          v == null
              ...
              : v.isConstant()
                  ? v.toString()
                  : (v + "=" + (bindingSet == null ? null : bindingSet.get((IVariable<?>) v))));
    }
