 ...
 * Annotations#OPTIONAL} is <code>true</code>, then the original binding set is copied to the
 * default sink (optional join semantics). Each subquery is run as a separate query but will be
 * cancelled if the parent query is cancelled.
 *
 ...
 * <h3>Usage Notes</h3>
 *
 * If there are no shared variables which must already be bound in the caller, then subquery join is
 * (or may be if there are some "might" be bound variables) the full cross product (constraints are
 * still applied and optional solutions must be reported if a constraint fails and the join is
 * optional). Such subqueries should be run as named subqueries instead so they run once, rather
 * than once per binding set.
 *
 * <p>If there are variables in scope in the parent query which are not projected by the subquery
 * but which appear in the subquery as well, then such variables in the subquery are effectively
 * distinct from those having the same name which appear in the parent query. In order to have
 * correct bottom-up evaluation semantics under these conditions. This is handled by "projecting"
 * only those variables into the subquery which it will project out.
 *
 * <h3>Efficiency</h3>
 *
 * This non-vectored operator issues one sub-query per source solution flowing into the operator. In
 * general, it is MUCH more efficient to vector the solutions into a sub-plan. The latter is
 * accomplished by building a hash index over the source solutions, flooding them into the sub-plan,
 * and then executing the appropriate hash join back against the source solutions after the
 * sub-plan.
 *
 * <p>There are a few cases where it may make sense to use the non-vectored operator. For example,
 * for EXISTS where LIMIT ONE can be imposed on the subquery. However, there can still be cases
 * where the vectored sub-plan is more efficient.
 *
...
public class SubqueryOp extends PipelineOp {
  ...
  public interface Annotations extends SubqueryJoinAnnotations {
    ...
    /**
     * When non-<code>null</code>, the {@link IVariable} which will be bound to <code>true</code>
     * iff there is at least one solution for the subquery. When specified, {@link #SELECT} SHOULD
     * be <code>null</code> in order to project all bindings from the parent's context into the
     * subquery. However, bindings in the subquery WILL NOT be projected back into the parent.
     *
     * <p>Note: This supports EXISTS and NOT EXISTS semantics.
     */
     ...
     * subquery unless the variable is projected from the subquery.
     *
     * <p>When this option is given, only the variables projected by the subquery will be visible
     * during the subquery (this models SPARQL 1.1 subquery variable scope semantics). A variable
     * having the same name in the parent context and the subquery which is not projected by the
     * subquery is a distinct variable. This constraint is enforced by passing only the projected
     * variables into the subquery and then merging the unprojected variables from the source
     * solution into each result produced by the subquery for a given source solution.
     *
     * <p>When this option is not given, all variables are passed into the subquery (basically, the
     * semantics are those of <code>SELECT *</code> ).
     *
     * <p>Note: This overrides the semantics of the same named annotation on the {@link
     * JoinAnnotations} interface.
     */
    String SELECT = SubqueryJoinAnnotations.SELECT;

    /**
     * Boolean annotation should be <code>true</code> if the subquery is an aggregate (default
     * {@value #DEFAULT_IS_AGGREGATE}).
     *
     * <p>Note: We need to have distinct {@link IAggregate} objects in each subquery issued since
     * they have internal state in order to prevent inappropriate sharing of state across
     * invocations of the subquery.
     */
   ...
   * @param annotations
   */
  public SubqueryOp(final BOp[] args, final Map<String, Object> annotations) {
    ...
    if (selectVars != null && selectVars.length == 0) {
      ...
      throw new IllegalArgumentException(
          Annotations.SELECT + " is optional, but may not be empty.");
    }
    ...
    switch (joinType) {
      ...
      case Optional:
        break;
      default:
        throw new UnsupportedOperationException(Annotations.JOIN_TYPE + "=" + joinType);
    }
  }
  ...
  private static class ControllerTask implements Callable<Void> {
    ...
    /**
     * Bound to true or false depending on whether or not there are solutions for the subquery
     * (optional).
     */
    ...
    public ControllerTask(final SubqueryOp controllerOp, final BOpContext<IBindingSet> context) {
      ...
      joinType = (JoinTypeEnum) controllerOp.getRequiredProperty(Annotations.JOIN_TYPE);

      this.aggregate =
          controllerOp.getProperty(Annotations.IS_AGGREGATE, Annotations.DEFAULT_IS_AGGREGATE);
      ...
      try {
        ...
        while (sitr.hasNext()) {
          ...
          for (IBindingSet bset : chunk) {
            ...
            final IRunningQuery runningSubquery = new SubqueryTask(bset, subquery, context).call();

            if (!runningSubquery.isDone()) {

              throw new AssertionError("Future not done: " + runningSubquery.toString());
            }
          }
        }
    ...
    private class SubqueryTask implements Callable<IRunningQuery> {
      ...
      public SubqueryTask(
        ...
        if (aggregate) {
          ...
          this.subQueryOp = BOpUtility.makeAggregateDistinct(subQuery);
      ...
      public IRunningQuery call() throws Exception {
         ...
         * visible.).
         */
        final IBindingSet childSolutionIn = parentSolutionIn.copy(selectVars);
        ...
        try {
          ...
          runningSubquery = queryEngine.eval(subQueryOp, childSolutionIn);
          ...
          try {
            ...
            if (askVar != null) {
              ...
              final IV<EmbergraphLiteral, Boolean> success =
                  XSDBooleanIV.valueOf(subquerySolutionItr.hasNext());

              //                            System.err
              //                                    .println("in="
              //                                            + childSolutionIn
              //                                            + ", out="
              //                                            + (subquerySolutionItr.hasNext() ?
              // subquerySolutionItr
              //                                                    .next() : "N/A")
              //                                            + ", askVar=" + success);
              //
          ...
          if (ncopied == 0L && joinType.isOptional()) {
            ...
            final IBlockingBuffer<IBindingSet[]> optionalSink =
                parentContext.getSink2() != null
                    ? parentContext.getSink2()
                    : parentContext.getSink();

            optionalSink.add(new IBindingSet[] {parentSolutionIn});
          }
