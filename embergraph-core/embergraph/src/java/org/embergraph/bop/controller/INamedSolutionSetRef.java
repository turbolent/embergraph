 ...
 * the <em>namespace</em> and <em>timestamp</em> of an MVCC view (this works for both cached and
 * durable named solution sets). Either {@link #getQueryId()} will be non- <code>null</code> or
 * {@link #getNamespace()} will be non-<code>null</code> , but not both.
 *
...
public interface INamedSolutionSetRef extends Serializable {
   ...
   *
   * <p>Note: When the named solution set is attached to an {@link IQueryAttributes}, the {@link
   * INamedSolutionSetRef} itself is used as the key into the {@link IQueryAttributes}. In this
   * case, the combination of the {@link #getQueryId()} (identifying the query) and the {@link
   * INamedSolutionSetRef} object together provide the equivalent of a fully qualified name.
   */
  String getFQN();
