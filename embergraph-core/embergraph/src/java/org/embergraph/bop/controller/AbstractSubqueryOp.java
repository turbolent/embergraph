 ...
 * parallelism. Since the #of subqueries is generally small (2), this means that the subqueries run
 * in parallel.
 *
 * <p>Note: This operator must execute on the query controller.
 *
 * <p>If you want to route intermediate results from other computations into subqueries, then
 * consider a {@link Tee} pattern instead.
 *
 * <p>For example:
 *
 * <pre>
 ...
 * </pre>
 *
 * Will run the subqueries <i>a</i>, <i>b</i>, and <i>c</i> in parallel. Each subquery will be run
 * once for each source {@link IBindingSet}.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @todo There is relatively little difference between this class and SubqueryOp and we should
 *     consider converging them into a single concrete subquery operator with specializations for
 *     UNION and STEPS. The main difference is that the SubqueryOp can not run multiple subqueries.
 */
public abstract class AbstractSubqueryOp extends PipelineOp {
  ...
  public interface Annotations extends PipelineOp.Annotations {
    ...
    /*
     * The ordered {@link BOp}[] of subqueries to be evaluated for each binding set presented
     * (required).
     */
  ...
  public int getMaxParallelSubqueries() {
    return getProperty(
        Annotations.MAX_PARALLEL_SUBQUERIES, Annotations.DEFAULT_MAX_PARALLEL_SUBQUERIES);
  }
   ...
   * @param annotations
   */
  public AbstractSubqueryOp(final BOp[] args, final Map<String, Object> annotations) {
    ...
    if (subqueries.length == 0) throw new IllegalArgumentException(Annotations.SUBQUERIES);
  ...
  /*
   * Evaluates the arguments of the operator as subqueries. The arguments are evaluated in order. An
   * {@link Executor} with limited parallelism to evaluate the arguments. If the controller operator
   * is interrupted, then the subqueries are cancelled. If a subquery fails, then all subqueries are
   * cancelled.
   */
  private static class ControllerTask implements Callable<Void> {
    ...
    public ControllerTask(
        final AbstractSubqueryOp controllerOp, final BOpContext<IBindingSet> context) {
      ...
      this.executor =
          new LatchedExecutor(context.getIndexManager().getExecutorService(), nparallel);
    }
    ...
    private void consumeBindingSet(final IBindingSet bset)
      ...
      try {
        ...
        for (FutureTask<IRunningQuery> ft : tasks) ft.cancel(true /* mayInterruptIfRunning */);
      }
    }
    ...
    private class SubqueryTask implements Callable<IRunningQuery> {
      ...
      public SubqueryTask(
          final BOp subQuery, final BOpContext<IBindingSet> parentContext, final IBindingSet bset) {
