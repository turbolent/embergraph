...
public class HTreeNamedSubqueryOp extends PipelineOp
    implements INamedSubqueryOp, ISingleThreadedOp {
   ...
   * @param annotations
   */
  public HTreeNamedSubqueryOp(final BOp[] args, final Map<String, Object> annotations) {
  ...
  private static class ControllerTask implements Callable<Void> {
    ...
    public ControllerTask(final HTreeNamedSubqueryOp op, final BOpContext<IBindingSet> context) {
        ...
        if (state == null) {
          ...
          state =
              new HTreeHashJoinUtility(
                  context.getMemoryManager(namedSetRef.getQueryId()), op, JoinTypeEnum.Normal);
