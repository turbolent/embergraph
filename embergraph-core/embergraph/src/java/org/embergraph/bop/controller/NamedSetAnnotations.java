...
public interface NamedSetAnnotations {
   ...
   * state. It may also be used to identify a named index that will be resolved against the SPARQL
   * CACHE, the local index manager, or the federation index manager (on a cluster).
   *
   * <p>The attribute name includes the query UUID. The query UUID must be extracted and used to
   * lookup the {@link IRunningQuery} to which the solution set was attached.
   *
   * <p>Note: For a MERGE JOIN, the value under this attribute is a {@link INamedSolutionSetRef}[].
   *
