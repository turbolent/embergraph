...
public class JournalIndex extends BTree {
  ...
  public static JournalIndex createTransient() {
    ...
    metadata.setTupleSerializer(new TupleSerializer(new ASCIIKeyBuilderFactory(Bytes.SIZEOF_LONG)));
   ...
   * start time. In this context the LTE search identifies the most recent commit state that not
   * later than the start time of the transaction.
   *
   ...
   */
  public synchronized JournalMetadata find(final long timestamp) {
  ...
  private JournalMetadata valueAtIndex(final long index) {
    ...
    final JournalMetadata entry = (JournalMetadata) SerializerUtil.deserialize(val);
   ...
   */
  public synchronized JournalMetadata findNext(final long timestamp) {
   ...
   *     timestamp.
   */
  public synchronized void add(final JournalMetadata resourceMetadata) {
   ...
   * @version $Id$
   */
  protected static class TupleSerializer extends DefaultTupleSerializer<Long, JournalMetadata> {
