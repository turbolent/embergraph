...
public class BuildResult extends AbstractResult implements Params {
   ...
   * full view of the index partition. They MAY be comprised of only a subset of the full view as
   * long as the subset is formed from the 1st N sources in the full view and is taken in the same
   * order as the full view. As a degenerate case, the subset may include only the data from the
   * mutable {@link BTree} on the old journal. However, the subset can also include additional
   * sources that were merged together in order to generate the new {@link IndexSegment}.
   */
   ...
   * view.
   *
   * <p>Note: A compacting merge is ONLY permitted when the entire view is processed. This is
   * necessary in order for the index partition to remain consistent. A compacting merge assumes a
   * closed world (that is, it assumes that there are no deleted tuples which are not present in the
   * set of sources which it processed). A non-merge build DOES NOT make this assumption and
   * therefore WILL NOT discard deleted tuples which unless they have been overwritten in more
   * recent sources in the view.
   */
   ...
   *     the view from which the {@link IndexSegment} was generated.
   * @param segmentMetadata The metadata describing the generated {@link IndexSegment}.
   * @param builder Contains more interesting information about the build.
   */
  public BuildResult(
      final String name,
