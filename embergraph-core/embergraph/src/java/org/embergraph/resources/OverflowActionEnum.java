...
public enum OverflowActionEnum {
   ...
   *
   * <p>The #of sources to use in a build is chosen in order to keep the build operation fast while
   * also minimizing the #of sources that are accumulated in the view over time and thereby reducing
   * the frequency with which a compacting merge must be performed.
   *
   * <p>If all sources in the view are used, then a build has the same semantics as a {@link
   * #Merge}, but it is still reported as a build since actions selected as builds tend to be
   * lighter weight even when all sources are still used (for example, consider the first overflow
   * event, where there is only one source in the view - while that could be called a merge, the
