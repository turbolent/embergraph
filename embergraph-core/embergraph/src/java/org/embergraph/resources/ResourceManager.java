 ...
 * indices.
 *
 * <p>This class is implemented in several layers:
 *
 * <dl>
 *   <dt>{@link ResourceManager}
 *   <dd>Concrete implementation.
 *   <dt>{@link OverflowManager}
 *   <dd>Overflow processing.
 *   <dt>{@link IndexManager}
 *   <dd>Manages indices
 *   <dt>{@link StoreManager}
 *   <dd>Manages the journal and index segment files, including the release of old resources.
 *   <dt>{@link ResourceEvents}
 *   <dd>Event reporting API
 * </dl>
 *
 * @todo Document backup procedures for the journal (the journal is a log-structured store; it can
 *     be deployed on RAID for media robustness; can be safely copied using normal file copy
 *     mechanisms and restored; can be compacted offline; a compact snapshot of a commit point (such
 *     as the last commit point) can be generated online and used for recovery; and can be exported
 *     onto index segments which can later be restored to any journal, but those index segments need
 *     additional metadata to recreate the appropriate relations which is found in the sparse row
 *     store) and the federation (relies on service failover (primary and secondaries) and a history
 *     retention policy; can be recovered from existing index partitions in a bottom up matter, but
 *     that recovery code has not been written).
...
public abstract class ResourceManager extends OverflowManager implements IPartitionIdFactory {
  ...
  public interface IResourceManagerCounters
    ...
    /**
     * The namespace for counters pertaining for overflow tasks (within the {@link #OverflowManager}
     * namespace).
     */
    ...
    /**
     * The namespace for counters pertaining to the live {@link ManagedJournal}.
     *
     * <p>Note: these counters are detached and reattached to the new live journal during overflow
     * processing.
     */
  ...
  /*synchronized*/ public CounterSet getCounters() {
    ...
    // Live Journal
    {
      ...
      root.makePath(IResourceManagerCounters.LiveJournal).attach(getLiveJournal().getCounters());
    }

    // OverflowManager
    {
      final CounterSet tmp = root.makePath(IResourceManagerCounters.OverflowManager);

      tmp.addCounter(
          IOverflowManagerCounters.OverflowEnabled,
      ...
      tmp.addCounter(
          IOverflowManagerCounters.OverflowAllowed,
          new Instrument<Boolean>() {
            public void sample() {
      ...
      tmp.addCounter(
          IOverflowManagerCounters.ShouldOverflow,
          new Instrument<Boolean>() {
            public void sample() {
      ...
      tmp.addCounter(
          IOverflowManagerCounters.SynchronousOverflowCount,
          new Instrument<Long>() {
            public void sample() {
              ...
              if (isOverflowEnabled() && !isOverflowAllowed()) {
                ...
                t +=
                    (System.currentTimeMillis()
                        - overflowCounters.asynchronousOverflowStartMillis.get());
              }
              setValue(t);
            }
          });
       ...
       */
      {
        final CounterSet tmp2 = tmp.makePath(IResourceManagerCounters.IndexPartitionTasks);

        tmp2.addCounter(
            IIndexPartitionTaskCounters.ConcurrentBuildCount,
        ...
        tmp2.addCounter(
            IIndexPartitionTaskCounters.ConcurrentMergeCount,
            new Instrument<Integer>() {
              public void sample() {
            ...
            new Instrument<String>() {
              public void sample() {
                ...
                for (Map.Entry<Long, IndexSegmentBuilder> e : map.entrySet()) {
                  ...
                  final IndexSegmentBuilder task = e.getValue();
                  //                                        if (n > 0)
                  //                                            sb.append(" ");
                  final String name = task.metadata.getName();
                  final LocalPartitionMetadata pmd = task.metadata.getPartitionMetadata();
                  final int partitionId = (pmd == null ? -1 : pmd.getPartitionId());
                  final String cause =
    ...
    // IndexManager
    {
      ...
      tmp.addCounter(
          IIndexManagerCounters.StaleLocatorCacheCapacity,
          new Instrument<Integer>() {
            public void sample() {
      ...
      tmp.addCounter(
          IIndexManagerCounters.StaleLocatorCacheSize,
          new Instrument<Integer>() {
            public void sample() {
      ...
      if (false)
        tmp.addCounter(
            IIndexManagerCounters.StaleLocators,
            new Instrument<String>() {
              public void sample() {
                final StringBuilder sb = new StringBuilder();
                final Iterator<ICacheEntry<String /* name */, StaleLocatorReason>> itr =
                    staleLocatorCache.entryIterator();
                while (itr.hasNext()) {
                  try {
      ...
      tmp.addCounter(
          IIndexManagerCounters.IndexCount,
          new Instrument<Long>() {
            public void sample() {
              ...
              if (lastCommitTime == 0L) {
                 ...
                 * data service.
                 */
                if (log.isInfoEnabled()) log.info("No commit points on the live journal?");
                return;
              }
              final long indexCount = liveJournal.getName2Addr(lastCommitTime).rangeCount();
              setValue(indexCount);
            }
          });

      tmp.addCounter(
          IIndexManagerCounters.IndexCacheCapacity,
          new Instrument<Integer>() {
            public void sample() {
      ...
      tmp.addCounter(
          IIndexManagerCounters.IndexCacheSize,
          new Instrument<Integer>() {
            public void sample() {
      ...
      tmp.addCounter(
          IIndexManagerCounters.IndexSegmentCacheCapacity,
          new Instrument<Integer>() {
            public void sample() {
      ...
      tmp.addCounter(
          IIndexManagerCounters.IndexSegmentCacheSize,
          new Instrument<Integer>() {
            public void sample() {
      ...
      tmp.makePath(IIndexManagerCounters.Indices).attach(getIndexCounters());
    }

    // StoreManager
    {
      final CounterSet tmp = root.makePath(IResourceManagerCounters.StoreManager);

      tmp.addCounter(
          IStoreManagerCounters.DataDir,
          new Instrument<String>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.TmpDir,
          new Instrument<String>() {
            public void sample() {
              setValue(tmpDir == null ? "N/A" : tmpDir.getAbsolutePath());
            }
          });

      tmp.addCounter(
          IStoreManagerCounters.IsOpen,
          new Instrument<Boolean>() {
            public void sample() {
              setValue(isOpen());
            }
      ...
      tmp.addCounter(
          IStoreManagerCounters.IsStarting,
          new Instrument<Boolean>() {
            public void sample() {
              setValue(isStarting());
            }
      ...
      tmp.addCounter(
          IStoreManagerCounters.IsRunning,
          new Instrument<Boolean>() {
            public void sample() {
              setValue(isRunning());
            }
      ...
      tmp.addCounter(
          IStoreManagerCounters.ManagedJournalCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.ManagedSegmentStoreCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.JournalReopenCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.SegmentStoreReopenCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.JournalDeleteCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.SegmentStoreDeleteCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.BytesUnderManagement,
          new Instrument<Long>() {
            public void sample() {
              if (isRunning()) {
      ...
      tmp.addCounter(
          IStoreManagerCounters.JournalBytesUnderManagement,
          new Instrument<Long>() {
            public void sample() {
              if (isRunning()) {
      ...
      tmp.addCounter(
          IStoreManagerCounters.SegmentBytesUnderManagement,
          new Instrument<Long>() {
            public void sample() {
              if (isRunning()) {
      ...
      tmp.addCounter(
          IStoreManagerCounters.BytesDeleted,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          ...
          new Instrument<Long>() {
            public void sample() {
              if (!isTransient()) setValue(getDataDirFreeSpace());
            }
          });
      ...
      tmp.addCounter(
          IStoreManagerCounters.PurgeResourcesMillis,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.ReleaseTime,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.LastOverflowTime,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.LastCommitTimePreserved,
          new Instrument<Long>() {
            public void sample() {
      ...
      tmp.addCounter(
          IStoreManagerCounters.LastCommitTime,
          new Instrument<Long>() {
            public void sample() {
  ...
  public void setConcurrencyManager(final IConcurrencyManager concurrencyManager) {

    if (concurrencyManager == null) throw new IllegalArgumentException();

    if (this.concurrencyManager != null) throw new IllegalStateException();
   ...
   *
   * <p>Note: You MUST use {@link #setConcurrencyManager(IConcurrencyManager)} after calling this
   * constructor (the parameter can not be passed in since there is a circular dependency between
   * the {@link IConcurrencyManager} and {@link ManagedJournal#getLocalTransactionManager()}.
   *
  ...
  /**
   * Requests a new index partition identifier from the {@link MetadataService} for the specified
   * scale-out index (RMI).
   *
