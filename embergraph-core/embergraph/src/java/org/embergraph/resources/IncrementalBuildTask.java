 ...
 * build).
 *
 * <p>Build uses mutable {@link BTree} of the lastCommitTime for the old journal PLUS ZERO OR MORE
 * additional source(s) taken in view order up to but not including the source in the view with
 * significant content. This let's us keep the #of {@link IndexSegment}s in the view down without
 * incurring the cost of a compacting merge. (The cost of the compacting merge itself comes from
 * having a large index segment in the view, generally in the last position of the view.) In turn,
 * this keeps the cost of overflow down and can be a significant win if there are a number of large
 * index partitions that receive a few writes in each overflow.
 *
 * <p>For example, assuming a large index segment exists from a previous compacting merge, then once
 * the #of writes exceeds the "copy" threshold there will be an index build. The view will then have
 * [live, smallSeg1, largeSeg1]. The next time the copy threshold is exceeded we would get [live,
 * smallSeg2, smallSeg1, largeSeg1]. However if we include smallSeg1 in the build, then we get
 * [live, smallSeg2, largeSeg1]. This can continue until we have enough data to warrant a split or
 * until we have another "large" segment but not yet enough data to split, at which point we get
 * [live, largeSeg2, largeSeg1] and then [live, smallSeg3, largeSeg2, largeSeg1].
 *
 * <p>Note: As its last action, this task submits a {@link AtomicUpdateIncrementalBuildTask} which
 * replaces the view with one defined by the current {@link BTree} on the journal and the newly
 * built {@link IndexSegment}.
 *
 * <p>Note: If the task fails, then the output {@link IndexSegment} will be deleted.
 *
...
public class IncrementalBuildTask extends AbstractPrepareTask<BuildResult> {
  ...
  /** @param vmd Metadata about the index partition view. */
  ...
  protected BuildResult doTask() throws Exception {
    ...
    try {
      ...
      try {
         ...
         */
        final BuildViewMetadata buildViewMetadata =
            new BuildViewMetadata(vmd.getView(), resourceManager.maximumBuildSegmentBytes, e);
        ...
        buildResult =
            resourceManager.buildIndexSegment(
                ...
                buildViewMetadata.compactingMerge,
                vmd.commitTime,
                null /* fromKey */,
                null /* toKey */,
                e);
        ...
        if (INFO) log.info("buildResult=" + buildResult);

        {
          ...
          if (INFO) log.info("indexSegmentStore=" + segStore.loadIndexSegment());
        }
      ...
      try {
        ...
        concurrencyManager
            .submit(
                new AtomicUpdateIncrementalBuildTask(
                    resourceManager,
                    concurrencyManager,
                    vmd.name,
                    vmd.indexMetadata.getIndexUUID(),
                    buildResult,
                    e))
            .get();
   ...
   * that the {@link IndexSegment} contains the same data as the {@link BTree} as of the
   * lastCommitTime. The new view (as defined by this task) will be selected when the desired view
   * is GTE the lastCommitTime. The old view will be used whenever the desired view is LT the
   * lastCommitTime.
   *
   * <pre>
   * journal A
  ...
  protected static class AtomicUpdateIncrementalBuildTask
     ...
     * @param buildResult
     */
    public AtomicUpdateIncrementalBuildTask(
    ...
    protected IResourceMetadata[] doTask() throws Exception {
      ...
      final Event updateEvent = parentEvent.newSubEvent(OverflowSubtaskEnum.AtomicUpdate).start();
        ...
        final IResourceMetadata[] currentResources = currentpmd.getResources();
        {
          ...
          if (currentResources.length < 2) {
            ...
            throw new IllegalStateException(
                "Expecting at least 2 resources in the view: " + Arrays.toString(currentResources));
          }
