 ...
 * both the potential concurrency of operations on that index partition and to permit more resources
 * to be brought to bear on the index partition. The "equal" splits are achieved by an "adjustment"
 * to the split handler.
 *
 * <p>The task reads from the lastCommitTime of the old journal after an overflow. It uses a key
 * range scan to sample the index partition, building an ordered set of {key,offset} tuples. Based
 * on the actual #of index entries and the target #of index entries per index partition, it chooses
 * the #of output index partitions, N, and selects N-1 {key,offset} tuples to split the index
 * partition. If the index defines a constraint on the split rule, then that constraint will be
 * applied to refine the actual split points, e.g., so as to avoid splitting a logical row of a
 * {@link SparseRowStore}.
 *
 * <p>Once the N-1 split points have been selected, N index segments are built - one from each of
 * the N key ranges which those N-1 split points define. Once the index segment for each split has
 * been built, an {@link AtomicUpdateSplitIndexPartitionTask} will atomically re-define the source
 * index partition as N new index partition. During the atomic update the original index partition
 * becomes un-defined and new index partitions are defined in its place which span the same total
 * key range and have the same data.
 *
 * @see AtomicUpdateSplitIndexPartitionTask, which MUST be invoked in order to update the index
 *     partition definitions on the live journal and the {@link MetadataIndex} as an atomic
 *     operation.
...
public class ScatterSplitTask extends AbstractPrepareTask<AbstractResult> {
  ...
  /*
   * An array of move targets for the new index partitions. The index partitions will be assigned to
   * the move targets using a round robin process. If one of the move targets is this data service,
   * then the corresponding index partition will not be moved.
   */
  protected final UUID[] moveTargets;

  /*
   * The target size of a shard for the scatter split. This is computed by dividing the size of the
   * compact segment on the disk by the #of desired splits.
   */
  protected final long adjustedNominalShardSize;

  /*
   * @param vmd The metadata for the index partition to be split.
   * @param nsplits The index will be split into this many index partitions without regard to the
   *     #of tuples in each split.
   * @param moveTargets An array of move targets for the new index partitions. The index partitions
   *     will be assigned to the move targets using a round robin process. If one of the move
   *     targets is this data service, then the corresponding index partition will not be moved.
   */
  protected ScatterSplitTask(final ViewMetadata vmd, final int nsplits, final UUID[] moveTargets) {
    ...
    if (nsplits <= 1) throw new IllegalArgumentException();

    if (moveTargets != null) {

      if (moveTargets.length == 0) throw new IllegalArgumentException();
  ...
  /*
   * Breaks the index partition into N splits, where N was specified to the ctor, and redistributes
   * those splits onto the move targets using a round robin.
   *
   * @return A {@link SplitResult} if the index partition was split into 2 or more index partitions
   *     -or- a {@link BuildResult} iff the index partition was not split.
   */
  @Override
  protected AbstractResult doTask() throws Exception {
        ...
        new Event(
            ...
            .addDetail(
                ...
                OverflowActionEnum.ScatterSplit
                    ...
                    + nsplits
                    + ")")
            .addDetail("moveTargets", Arrays.toString(moveTargets))
            .start();
    ...
    try {
      ...
      try {
        ...
        final ISimpleSplitHandler splitHandler = vmd.indexMetadata.getSplitHandler();

        final Split[] splits =
            SplitUtility.getSplits(
                resourceManager,
                vmd.pmd,
                (IndexSegment) src.getSources()[1],
        ...
        if (splits == null) {
          ...
          final double overextension =
              ((double) vmd.sumSegBytes) / resourceManager.nominalShardSize;
          ...
          if (overextension > resourceManager.shardOverextensionLimit
            ...
            log.error(
                "Shard will not split - writes are disabled"
                    + ": name="
                    + vmd.name
                    + ", size="
                    + vmd.sumSegBytes
                    ...
                    + "x"
                    + ", splitHandler="
                    + (splitHandler == null ? "N/A" : splitHandler.getClass().getName()));
          ...
          // Incremental build.
          return concurrencyManager.submit(new IncrementalBuildTask(vmd)).get();
        }
      ...
      // create the move tasks.
      {
        for (int i = 0; i < nsplits; i++) {
          ...
          if (resourceManager.getDataServiceUUID().equals(moveTarget)) {
            ...
            // ignore move to self.
            if (INFO) log.info("Ignoring move to self.");
            continue;
          }
           ...
           */
          moveTasks.add(
              new MoveTask.AtomicUpdate(
                  resourceManager,
                  nameOfPartitionToMove,
                  splitResult.buildResults[i],
                  moveTarget,
                  newPartitionId,
                  e));
        }
       ...
       * outcomes.
       */
      final List<Future<MoveResult>> futures =
          resourceManager.getConcurrencyManager().invokeAll(moveTasks);
