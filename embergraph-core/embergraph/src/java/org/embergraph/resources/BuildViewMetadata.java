 ...
 * Additional sources are incorporated into the identified view until the specified thresholds would
 * be exceeded. The sources are ordered and the order of the sources is maintained. If all sources
 * can be incorporated without exceeding the specified thresholds, then the view is flagged as a
 * compacting merge rather than an incremental build and deleted tuples will be purged from the
 * view.
 *
 * <p>For example, given a view with a {@link BTree} on a journal followed by 2 {@link
 * IndexSegment}s, the output view might include just the {@link BTree} or the {@link BTree}
 * followed by the first {@link IndexSegment}, or the {@link BTree} followed by both of the {@link
 * IndexSegment}s.
 *
...
class BuildViewMetadata {
  ...
  /** The maximum #of bytes of {@link IndexSegment}s to allow into the view (from the ctor). */
   ...
   * because the goal is to keep down the #of components in the view without doing the heavy work of
   * processing a large source (which winds up copying a lot of tuples). We only do that additional
   * work when a compacting merge was selected as the action rather than an incremental build.
   *
   * @param src The source view.
   * @param maxSumSegBytes The maximum #of index segment bytes that will be incorporated into the
   *     accepted view.
   * @param parentEvent The parent event.
   */
  public BuildViewMetadata(
      final ILocalBTreeView src, final long maxSumSegBytes, final Event parentEvent) {

    if (src == null) throw new IllegalArgumentException();

    if (maxSumSegBytes < 0) throw new IllegalArgumentException();

    final Event e = parentEvent.newSubEvent(OverflowSubtaskEnum.ChooseView).start();
    ...
    try {
      ...
      if (naccepted == 1) {
        ...
        acceptedView = new FusedView(accepted.toArray(new AbstractBTree[naccepted]));
      }
