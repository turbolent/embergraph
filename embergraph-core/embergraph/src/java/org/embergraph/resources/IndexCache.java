...
public class IndexCache<H extends ILocalBTreeView>
   ...
   *
   * @todo an alternative is to maintain a sorted set of the timestamps (breaking ties with the
   *     index name) and then always return the first entry (lowest value) from the set. in fact the
   *     tx service does something similar, but it has an easier time since the timestamps are
   *     unique where the same timestamp may be used for multiple indices here.
   */
   ...
   *
   * <p>Note: Due to the concurrent operations of the garbage collector, this method MAY return a
   * time that is earlier than necessary. This possibility arises because {@link WeakReference}
   * values may be cleared at any time. There is no negative consequence to this behavior - it
   * simply means that fewer resources will be released than might otherwise be possible. This
   * "hole" can not be closed by polling the {@link ReferenceQueue} since it is not when the entries
   * are removed from the map which matters, but when their {@link WeakReference} values are
   * cleared. However, periodically clearing stale references using {@link #clearStaleRefs()} will
   * keep down the size of that "hole".
   */
    ...
    synchronized (retentionTime) {
      if (retentionTime.get() == -1L) {
        ...
        while (itr.hasNext()) {
          ...
          if (debug) log.warn("considering: " + entry.getKey());
