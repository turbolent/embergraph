 ...
 *
 * @todo There is neither a "CREATE_TEMP_DIR" and "DELETE_ON_CLOSE" does not remove all directories
 *     created during setup. One of the consequences is that you have to explicitly clean up after a
 *     unit test using a {@link ResourceManager} or it will leave its files around.
 * @todo {@link BufferMode#Temporary} is not supported (verify whether the Transient mode is
 *     supported).
 * @todo If we approach the limit on free space for the {@link #dataDir} then we need to shed index
 *     partitions to other data services or potentially become more aggressive in releasing old
 *     resources. See {@link #getDataDirFreeSpace(File)}
...
public abstract class StoreManager extends ResourceEvents implements IResourceManager {
  ...
  public interface Options extends org.embergraph.journal.Options {
     ...
     * created (no default). This property is required unless the instance is transient. If you
     * specify {@link org.embergraph.journal.Options#BUFFER_MODE} as {@link BufferMode#Transient}
     * then journals will be NOT stored in the file system and {@link ResourceManager#overflow()}
     * will be disabled.
     *
     * <p>The files are created within subdirectories as follows: The "journals" subdirectory
     * contains the journal files. The "segments" directory contains subdirectories corresponding to
     * the index UUID for each scale-out index. Within those index-specific directories, the index
     * segment files are assigned to files using the temporary file mechanisms using the munged
     * index name as the file prefix and {@link Options#SEG} as the file suffix. If the index is
     * partitioned then the partition identifier appears as part of the file prefix.
     *
     * <p>Note: While files are stored per the scheme described above, the entire {@link #DATA_DIR}
     * will be scanned recursively to identify all journal files and index segments during startup.
     * Files will be used where ever they are found but the {@link IResourceMetadata#getFile()} read
     * from a given resource MUST correspond to its relative location within the {@link #DATA_DIR}.
     *
     * <p>Note: Each {@link DataService} or {@link MetadataService} MUST have its own {@link
     * #DATA_DIR}.
     */
    String DATA_DIR = StoreManager.class.getName() + ".dataDir";

    /*
     * The capacity of the LRU cache of open {@link IRawStore}s. The capacity of this cache
     * indirectly controls how many stores will be held open. The main reason for keeping an store
     * open is to reuse its buffers if another request arrives "soon" which would read on that
     * store. Note that "stores" includes both {@link ManagedJournal}s and {@link
     * IndexSegmentStore}s.
     *
     ...
     * the JVM will eventually GC the index object, thereby releasing its object graph. Since stores
     * which are strongly reachable never have their weak reference cleared this provides our
     * guarantee that stores are never closed if they are in use.
     *
     * <p>Stores have non-transient resources and MUST explicitly be closed. Since we are not
     * notified before the weak reference is closed, our only remaining option is {@link
     * AbstractJournal#finalize()} and {@link IndexSegmentStore#finalize()}, both of which close the
     * store if it is still open.
     *
    ...
    /*
     * The time in milliseconds before an entry in the store cache will be cleared from the backing
     * {@link HardReferenceQueue} (default {@value #DEFAULT_STORE_CACHE_TIMEOUT}). This property
     * controls how long the store cache will retain an {@link IRawStore} which has not been
     * recently used. This is in contrast to the cache capacity.
     */
    String STORE_CACHE_TIMEOUT = StoreManager.class.getName() + ".storeCacheTimeout";

    String DEFAULT_STORE_CACHE_TIMEOUT = "" + (60 * 1000); // One minute.

    /*
     * A boolean property whose value determines whether or not startup will complete successfully
     * if bad files are identified during the startup scan (default {@value
     * #DEFAULT_IGNORE_BAD_FILES}). When <code>false</code> the {@link StoreManager} will refuse to
     * start if if find bad files. When <code>true</code> the {@link StoreManager} will startup
     * anyway but some index views may not be available. Regardless, bad files will be logged as
     * they are identified and all files will be scanned before the {@link StoreManager} aborts.
     */
    ...
    String PURGE_OLD_RESOURCES_DURING_STARTUP =
        StoreManager.class.getName() + ".purgeOldResourcesDuringStartup";

    String DEFAULT_PURGE_OLD_RESOURCES_DURING_STARTUP = "true";

    /*
     * Option specifies the #of bytes under management below which we will accelerate the overflow
     * of the live journal by reducing its maximum extent below the nominal configured maximum
     * extent. The purpose of this option is to promote rapid overflow of a new data service (where
     * new is measured by the #of bytes under management). This helps to increase the rate at which
     * index partitions are split (and moved if the there is more than one new data service
     * starting). When ZERO (0) the feature is disabled.
     */
    String ACCELERATE_OVERFLOW_THRESHOLD =
        StoreManager.class.getName() + ".accelerateOverflowThreshold";
  ...
  public interface IStoreManagerCounters {
    ...
    /** The #of bytes in journals currently under management by the {@link StoreManager}. */
    String JournalBytesUnderManagement = "Journal Bytes Under Management";

    /** The #of bytes in index segments currently under management by the {@link StoreManager}. */
    String SegmentBytesUnderManagement = "Segment Bytes Under Management";

    /*
     * The #of bytes in resources that have been deleted by the {@link StoreManager} after they
     * became release free.
     */
    ...
    /*
     * The maximum extent of any journal managed by this service as of the time when it was closed
     * out by synchronous overflow processing.
     */
    ...
    /*
     * The most recent commit time preserved when resources were last purged from the {@link
     * StoreManger}.
     *
   ...
   *
   * <p>Note: It is a hard requirement that each resource is located by the {@link
   * IResourceMetadata#getFile() path} relative to the {@link #dataDir}.
   *
  ...
  /*
   * The performance counters for the {@link IBufferStrategy} backing the live journal and any
   * historical journals which are concurrently open with the live journal. A single instance of
   * this object is used, and a hard reference to that instance is held here, so that we can track
   * the cumulative performance counters across the live cycles of all journal instances used by the
   * data service over time. The performance counters are not themselves persistent and do not
   * survive a restart of the {@link StoreManager}.
   */
  ...
  /*
   * A map over the journal histories. The map is transient and is re-populated from a scan of the
   * file system during startup.
   *
   * <p>The keys are the timestamp at which the journal was put into service. The values are the
   * journal resource descriptions. Given the timestamp of some historical state of an index, this
   * map is used to locate the journal on which that historical state of the index would be found.
   */
  private final JournalIndex journalIndex;

  /*
   * A map over the index segments by ascending createTime and UUID. The map is transient and is
   * re-populated from a scan of the file system during startup.
   *
   * <p>The keys are the createTime of the index segment followed by the index segment UUID (to
   * break ties). The values are the {@link IResourceMetadata} object describing that index segment.
   * This map is used to provide some basic reporting but is primarily used to delete index segment
   * resources once they are no longer required.
   *
   * @todo Is this strictly necessary? Do we have all the necessary information in the journals?
   *     Review the logic and decide.
   */
  private final IndexSegmentIndex segmentIndex;

  /*
   * A non-thread-safe collection of {@link UUID}s for {@link IndexSegment}s which have been newly
   * built but not yet incorporated in a re-start safe manner into an index partition view. {@link
   * UUID}s in this collection are excluded from release by {@link #purgeOldResources()}.
   *
   ...
   * not yet incorporated into an index partition view and hence we must take special cautions to
  ...
  /*
   * Remove an {@link IndexSegment} from the {@link #retentionSet}. DO NOT invoke this until the
   * {@link IndexSegment} has been incorporated in a restart safe manner into an index partition
   * view (that is, post-commit rather than during the task that incorporates it into the view) or
   * is known to be no longer required (post MOVE, task failed, etc).
   *
   * @param uuid The {@link UUID} of the {@link IndexSegmentStore}.
  ...
  /*
   * A cache that is used by the to automatically close out unused {@link IndexSegmentStore}s. An
   * {@link IndexSegment} that is no longer used will have its reference cleared when it is swept by
   * the garbage collector and will automatically release all of its buffers (node and leaf cache,
   * etc). However, at that point the {@link IndexSegmentStore} is still open, and it can buffer a
   * significant amount of data in addition to the file handle.
   *
   * <p>When the weak reference is cleared we know that there are no longer any hard references to
   * the {@link IndexSegment} and hence the corresponding {@link IndexSegmentStore} should be
   * closed. In fact, we can immediately remove the {@link IndexSegmentStore} from the cache of open
   * stores and then close the store. At this point if the store is re-opened it will be a new
   * object. This is easy enough to do since the {@link UUID} of the {@link IndexSegmentStore} is
   * the key in our map!
   *
   ...
   * ManagedJournal}s and IndexSegment}s. There MAY be more {@link IRawStore}s open than are
   * reported by this method if there are hard references held by the application to those {@link
   * IRawStore}s. {@link IRawStore}s that are not fixed by a hard reference will be quickly
...
//     * A direct {@link ByteBuffer} that will be used as the write cache for the
  ...
  /*
   * <code>true</code> initially and until {@link #start()} completes successfully, this is used to
   * disambiguate the startup transient state from the shutdown state.
   *
   * @see #isStarting()
   */
  ...
  /*
   * The service used to send files to other data services and to exchange NIO {@link ByteBuffer} in
   * support of distributed query processing.
   */
...
//     * The port at which you can connect to the {@link ResourceService}. This
  ...
  /*
   * Used to run the {@link Startup}. @todo defer to init() outside of ctor. Also, defer {@link
   * Startup} until init() outside of ctor.
   */
  private final ExecutorService startupService =
      Executors.newSingleThreadExecutor(
          new DaemonThreadFactory(getClass().getName() + ".startupService"));

  /*
   * Succeeds if the {@link StoreManager} {@link #isOpen()} and is NOT {@link #isStarting()} (the
   * test itself is NOT atomic).
   *
   * @throws IllegalStateException unless open and not starting.
   */
  protected void assertRunning() {

    if (!isOpen()) throw new IllegalStateException("Not open");

    if (isStarting()) throw new IllegalStateException("Starting up");
  }

  /*
   * Return <code>true</code> iff the {@link StoreManager} is open and startup processing has been
   * completed.
   */
  ...
  /** @throws IllegalStateException unless open. */
  ...
  /*
   * Return <code>true</code> iff the {@link StoreManager} is running. If the {@link StoreManager}
   * is currently starting up, then this will await the completion of the {@link Startup} task.
   *
   * @return <code>true</code> if the {@link StoreManager} is running and <code>false</code> if it
   *     is shutdown.
   */
  public boolean awaitRunning() {
    ...
    while (isOpen() && isStarting()) {
      ...
      try {
        ...
        if (log.isInfoEnabled()) log.info("Waiting on startup : " + dataDir + " ...");
  ...
  /*
   * A map from the resource UUID to the absolute {@link File} for that resource.
   *
   * <p>Note: The {@link IResourceMetadata} reported by an {@link AbstractJournal} or {@link
  ...
  /*
   * The last value computed by {@link #getEffectiveReleaseTime()} and ZERO(0) until a value has
   * been calculated.
   */
  protected long lastCommitTimePreserved = 0L;

  /*
   * The last commit time corresponding to the last synchronous overflow event and ZERO (0L) until
   * there has been a synchronous overflow event.
   */
  protected long lastOverflowTime = 0L;

  /*
   * The observed maximum size of a journal (its length in bytes) as measured at each synchronous
   * overflow event.
   */
  ...
  /*
   * The #of bytes currently under management EXCEPT those on the live journal. This is incremented
   * each time a new resource is added using {@link #addResource(IResourceMetadata, File)} and
   * decremented each time a resource is deleted.
   */
   ...
   *
   * @throws IllegalStateException during startup or if the {@link StoreManager} is closed.
   */
  ...
  /*
   * The #of bytes in {@link ManagedJournal}s, including those written on the live journal.
   *
   * @throws IllegalStateException during startup or if the {@link StoreManager} is closed.
   */
  public long getJournalBytesUnderManagement() {
    ...
    return journalBytesUnderManagement.get() + getLiveJournal().getBufferStrategy().getExtent();
  }
  ...
  /*
   * The #of bytes of free space remaining on the volume hosting the {@link #dataDir}.
   *
   * @return The #of bytes of free space remaining -or- <code>-1L</code> if the free space could not
   *     be determined.
   */
  ...
  /*
   * The #of bytes of free space remaining on the volume hosting the {@link #tmpDir}.
   *
   * @return The #of bytes of free space remaining -or- <code>-1L</code> if the free space could not
   *     be determined.
   */
   ...
   *
   * @param dir A directory hosted on some volume.
   * @return The #of bytes of free space remaining for the volume hosting the directory -or- <code>
   *     -1L</code> if the free space could not be determined.
   */
  /*
  ...
  private long getFreeSpace(final File dir) {
    ...
    try {
      ...
      log.error("Could not get free space: dir=" + dir + " : " + t, t);
...
//     * Return the free space in bytes on the volume hosting some directory.
  ...
  /*
   * Note: This constructor starts an asynchronous thread that scans the data directory for journals
   * and index segments and creates the initial journal if no store files are found.
   *
   * <p>Note: The store files are NOT accessible until the asynchronous startup is finished.
   * Caller's MUST verify that the {@link StoreManager#isOpen()} AND NOT submit tasks until {@link
   * StoreManager#isStarting()} returns <code>false</code>.
   *
  ...
  protected StoreManager(final Properties properties) {
    ...
    // ignoreBadFiles
    {
      ignoreBadFiles =
          Boolean.parseBoolean(
              properties.getProperty(Options.IGNORE_BAD_FILES, Options.DEFAULT_IGNORE_BAD_FILES));

      if (log.isInfoEnabled()) log.info(Options.IGNORE_BAD_FILES + "=" + ignoreBadFiles);
    }
    ...
    // purgeOldResourcesDuringStartup
    {
      ...
      if (log.isInfoEnabled())
        log.info(Options.PURGE_OLD_RESOURCES_DURING_STARTUP + "=" + purgeOldResourcesDuringStartup);
    }
    ...
    // accelerateOverflowThreshold
    {
      ...
      if (log.isInfoEnabled())
        log.info(Options.ACCELERATE_OVERFLOW_THRESHOLD + "=" + accelerateOverflowThreshold);
     ...
     */
    {
      final int storeCacheCapacity =
          Integer.parseInt(
              properties.getProperty(
                  Options.STORE_CACHE_CAPACITY, Options.DEFAULT_STORE_CACHE_CAPACITY));

      if (log.isInfoEnabled()) log.info(Options.STORE_CACHE_CAPACITY + "=" + storeCacheCapacity);

      if (storeCacheCapacity <= 0)
        throw new RuntimeException(Options.STORE_CACHE_CAPACITY + " must be positive");

      final long storeCacheTimeout =
          Long.parseLong(
              properties.getProperty(
                  Options.STORE_CACHE_TIMEOUT, Options.DEFAULT_STORE_CACHE_TIMEOUT));

      if (log.isInfoEnabled()) log.info(Options.STORE_CACHE_TIMEOUT + "=" + storeCacheTimeout);
    ...
    if (isTransient) {
      ...
      if (!dataDir.exists()) {
        ...
        if (log.isInfoEnabled()) log.info("Creating: " + dataDir);
      ...
      if (!journalsDir.exists()) {
        ...
        if (log.isInfoEnabled()) log.info("Creating: " + journalsDir);

        if (!journalsDir.mkdirs()) {

          throw new RuntimeException(
              "Could not create directory: " + journalsDir.getAbsolutePath());
        }
      }

      if (!segmentsDir.exists()) {

        if (log.isInfoEnabled()) log.info("Creating: " + segmentsDir);

        if (!segmentsDir.mkdirs()) {

          throw new RuntimeException(
              "Could not create directory: " + segmentsDir.getAbsolutePath());
        }
      }
      ...
      if (!journalsDir.isDirectory()) {
        ...
        throw new RuntimeException("Not a directory: " + journalsDir.getAbsolutePath());
      }

      if (!segmentsDir.isDirectory()) {

        throw new RuntimeException("Not a directory: " + segmentsDir.getAbsolutePath());
      }
    ...
    // temp directory.
    {
      ...
      try {
        ...
        tmpDir =
            new File(properties.getProperty(Options.TMP_DIR, System.getProperty("java.io.tmpdir")))
                .getCanonicalFile();
      ...
      if (log.isInfoEnabled()) log.info(Options.TMP_DIR + "=" + tmpDir);

      if (!tmpDir.exists()) {

        if (log.isInfoEnabled()) log.info("Creating temp directory: " + tmpDir);
  ...
  /*
   * Runs a startup scan of the data directory and creates the initial journal if none was found. If
   * the {@link Startup} task fails or is interrupted then the {@link StoreManager} will be {@link
   * StoreManager#shutdownNow()}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private class Startup implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        try {
          ...
          if (purgeOldResourcesDuringStartup) purgeOldResources();
        ...
        if (log.isInfoEnabled())
          log.info(
              "Startup "
                  ...
                  + " : "
                  + (isTransient ? "transient" : Options.DATA_DIR + "=" + dataDir));
      }
    }
     ...
     *
     * <p>Note: Implementations of this method MUST be <code>synchronized</code>.
     *
     * @throws InterruptedException
     * @throws IllegalStateException if the {@link IConcurrencyManager} has not been set (after a
     *     timeout).
     * @throws IllegalStateException if the the {@link ResourceManager} is already running.
    ...
    private final void start() throws InterruptedException {
       ...
       */
      {
        ...
        while (true) {
          try {
            ...
            Thread.sleep(100 /* ms */);
            if (++nwaits % 50 == 0) log.warn("Waiting for concurrency manager");
          }
        }
      }
      ...
      if (!isTransient) {
        ...
        if (log.isInfoEnabled()) log.info("Starting scan of data directory: " + dataDir);
        ...
        if (log.isInfoEnabled()) log.info("Scan results: " + stats);
       ...
       * Note: This exception will be thrown if we could not
        ...
        assert resourceFiles.size() + nbad == stats.nfiles
            : "#resourceFiles="
                + resourceFiles.size()
                + ", #nbad="
                + nbad
      ...
      final long lastCommitTime = liveJournalRef.get().getLastCommitTime();
      ...
      try {
        ...
        throw new RuntimeException("Could not start: " + resourceService, ex);
      }
    }
    ...
    private void openLiveJournal() throws InterruptedException {
      ...
      if (log.isInfoEnabled()) log.info("Creating/opening the live journal: dataDir=" + dataDir);
       ...
       * There are no existing journal files. Create new journal using
        ...
        if (log.isInfoEnabled()) log.info("Creating initial journal: dataDir=" + dataDir);
       ...
       * There is at least one pre-existing journal file, so we open
        ...
        if (log.isInfoEnabled()) log.info("Will open as live journal: " + resource);

        assert resource != null : "No resource? : timestamp=" + Long.MAX_VALUE;
        ...
        if (log.isInfoEnabled())
          log.info("Opening most recent journal: " + file + ", resource=" + resource);
      ...
      if (log.isInfoEnabled())
        log.info("Open/create of live journal: newJournal=" + newJournal + ", file=" + file);
...
//         * Purge any index partition moves which did not complete successfully
    ...
    // btree.getIndexMetadata().getPartitionMetadata();
    //
    //                if (pmd != null) {
    //
    ////                    System.err.println("\nname=" + name + "\npmd=" + pmd);
  ...
  /** <code>true</code> initially and until {@link #start()} completes successfully. */
  ...
  /*
   * <code>false</code> initially and remains <code>false</code> until {@link #start()} completes
   * successfully. once <code>true</code> this remains <code>true</code> until either {@link
   * #shutdown()} or {@link #shutdownNow()} is invoked.
   */
  @Override
...
//     * Clears any stale entries in the LRU backing the {@link #storeCache}
   ...
   * populating the internal {@link #resourceFiles} map. In addition, all journal files are listed
   * in the {@link #journals} map so that we can find the relevant journal quickly for a given
   * timestamp.
   *
   * <p>Note: This requires that we open each resource in order to extract its {@link
   * IResourceMetadata} description. We only open the {@link IRawStore} for the resource, not its
   * indices. The stores are closed again immediately.
   *
  ...
  private void scanFile(final File file, final Stats stats) throws InterruptedException {

    if (Thread.interrupted()) throw new InterruptedException();

    if (log.isInfoEnabled()) log.info("Scanning file: " + file + ", stats=" + stats);
    ...
    if (len > 0 && name.endsWith(Options.JNL)) {
      ...
      try {
        ...
        log.error("Problem opening journal: file=" + file.getAbsolutePath(), ex);
      ...
      try {
        ...
        log.error("Problem opening segment: file=" + file.getAbsolutePath(), ex);
      ...
      if (len == 0L && (name.endsWith(Options.JNL) || name.endsWith(Options.SEG))) {
    ...
    if (log.isInfoEnabled()) log.info("Found " + resource + " in " + file);
   ...
   *
   * <p>Note: This is invoked by {@link #shutdown()} and {@link #shutdownNow()}.
   */
  ...
  /*
   * Notify the resource manager of a new resource. The resource is added to {@link #resourceFiles}
   * and to either {@link #journalIndex} or {@link #segmentIndex} as appropriate. As a
   * post-condition, you can use {@link #openStore(UUID)} to open the resource using the {@link
   * UUID} specified by {@link IResourceMetadata#getUUID()}.
   *
   * <p>Note: This also adds the size of the store in bytes as reported by the OS to {@link
   * #bytesUnderManagement}.
   *
   * <p>Note: Adding a resource to the store manager has no persistent effect other than the
   * presumed presence of the specified file in the file system. However, error handling routines
   * SHOULD invoke {@link #deleteResource(UUID, boolean)} in order to remove a resource that was not
   * built correctly or not incorporated into the view. Otherwise the mapping from the {@link UUID}
   * to the {@link File} will be maintained in memory and the {@link StoreManager} will overreport
   * the #of bytes under management.
   *
   * @param resourceMetadata The metadata describing that resource.
   * @param file The file in the local file system which is the resource.
   * @throws RuntimeException if the file does not exist.
   * @throws RuntimeException if there is already a resource registered with the same UUID as
   *     reported by {@link IResourceMetadata#getUUID()}
   * @throws RuntimeException if the {@link #journalIndex} or {@link #segmentIndex} already know
   *     about that resource.
   * @throws RuntimeException if {@link #openStore(UUID)} already knows about that resource.
   * @throws IllegalArgumentException if the <i>resourceMetadata</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>file</i> is <code>null</code> and {@link
   *     #isTransient} is <code>false</code>.
   * @see #deleteResource(UUID, boolean)
   * @see #retentionSetAdd(UUID)
   * @see #retentionSetRemove(UUID)
   */
  protected synchronized void addResource(final IResourceMetadata resourceMetadata, File file) {

    if (resourceMetadata == null) throw new IllegalArgumentException();

    if (file == null && !isTransient) throw new IllegalArgumentException();

    assertOpen();

    final UUID uuid = resourceMetadata.getUUID();

    if (log.isInfoEnabled()) log.info("file=" + file + ", uuid=" + uuid);
    ...
    if (!isTransient) {
      ...
      if (tmp != null) {
        ...
        throw new RuntimeException(
            "Resource already registered: uuid="
                + uuid
                + " as file="
                + tmp
                + " (given file="
  ...
  /*
   * Returns a filter that is used to recognize files that are managed by this class. The {@link
   * ResourceManager} will log warnings if it sees an unexpected file and will NOT {@link
   * #deleteResources()} files that it does not recognize.
   *
   * @see ResourceFileFilter
   * @todo perhaps define setFileFilter and getFileFilter instead since subclassing this method is a
   *     bit difficult. The {@link ResourceFileFilter} would have to be a static class and we would
   *     have to pass in the {@link IResourceManager} so that it could get the {@link #dataDir}.
   */
   ...
   *
   * @throws IllegalStateException if the object has not been set yet using {@link
  ...
  /*
   * The {@link ManagedJournal} provides the backing store used to absorb writes and retain history
   * for the scale-out architecture.
   *
   * <p>Note: This implementation is designed to use a shared {@link ConcurrencyManager} across all
   * open journal instances for a {@link DataService}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
...
//         * Note: Each instance of the {@link ManagedJournal} reuses the SAME
    ...
    protected ManagedJournal(final Properties properties) {
      ...
      if (getBufferStrategy() instanceof DiskOnlyStrategy) {
        ...
        ((DiskOnlyStrategy) getBufferStrategy()).setStoreCounters(getStoreCounters());

      } else if (getBufferStrategy() instanceof WORMStrategy) {

        ((WORMStrategy) getBufferStrategy()).setStoreCounters(getStoreCounters());
      }
    }
    ...
    public String toString() {
      ...
      return getClass().getName()
          ...
          + (rootBlock != null ? ", uuid=" + getRootBlockView().getUUID() : "")
          + "}";
    }

    /*
     * Note: Exposed for the {@link DataService} which needs this for its 2-phase commit protocol.
     */
    @Override
    ...
    /*
     * Exposed for {@link StoreManger#getResourcesForTimestamp(long)} which requires access to the
     * {@link CommitRecordIndex} for the lastCommitTime on the historical journals.
     *
     * <p>Note: This always returns a distinct index object. The code relies on this fact to avoid
     * contention with the live {@link CommitRecordIndex} for the live journal.
     */
    @Override
    public CommitRecordIndex getCommitRecordIndex(final long addr, final boolean readOnly) {
    ...
    public AbstractLocalTransactionManager getLocalTransactionManager() {
      ...
      return (AbstractLocalTransactionManager) getConcurrencyManager().getTransactionManager();
    }
    ...
    public DefaultResourceLocator getResourceLocator() {
      ...
      return (DefaultResourceLocator) getFederation().getResourceLocator();
    }
    ...
    /*
     * Extended to set the {@link IResourceMetadata} to this journal if it is <code>null</code>
     * since a remote caller can not have the correct metadata on hand when they formulate the
     * request.
     */
    @Override
    protected void validateIndexMetadata(final String name, final IndexMetadata metadata) {
       ...
       * A [null] for the resources field is a specific indication
        ...
        if (!pmd.getResources()[0].isJournal()) {
          ...
          throw new RuntimeException(
              "Expecting resources[0] to be journal: name=" + name + ", pmd=" + pmd);
        }

        if (!pmd.getResources()[0].getUUID().equals(getRootBlockView().getUUID())) {
   ...
   *
   * @throws IllegalStateException if the {@link StoreManager} is not open.
   * @throws IllegalStateException if the {@link StoreManager} is still starting up.
   */
  @Override
  public ManagedJournal getLiveJournal() {
    ...
    assert tmp != null : "open=" + isOpen() + ", starting=" + isStarting() + ", dataDir=" + dataDir;
    assert tmp.isOpen();
...
//     * This lock is used to prevent asynchronous processes such as
   ...
   * @throws IllegalStateException if the {@link StoreManager} is not open.
   * @throws IllegalStateException if the {@link StoreManager} is still starting up.
   * @todo write tests for unisolated and read-committed. make sure that there is no fencepost for
   *     read committed immediately after an overflow (there should not be since we do a commit when
   *     we register the indices on the new store).
   */
  @Override
   ...
   * @throws IllegalStateException if the {@link StoreManager} is not open.
   * @throws IllegalStateException if the {@link StoreManager} is still starting up.
   * @throws IllegalArgumentException if <i>uuid</i> is <code>null</code>.
   * @throws NoSuchStoreException if the {@link UUID} is not recognized.
   * @throws NoSuchStoreException if the resource for that {@link UUID} could not be found.
   * @throws RuntimeException if something else goes wrong.
   * @todo it seems that we always have the {@link IResourceMetadata} on hand when we need to
   *     (re-)open a store so it might be nice to pass that in as it would make for more informative
   *     error messages when something goes wrong (except that I was planning to drop the file name
   *     from that interface).
   */
  @Override
    ...
    try {
       ...
       * Verify the resource UUID.
        ...
        if (!actualUUID.equals(uuid)) {
          ...
          throw new RuntimeException(
              "Wrong UUID: file=" + file + ", expecting=" + uuid + ", actual=" + actualUUID);
        }
  ...
  protected long nextTimestamp() {
    ...
    final ILocalTransactionManager transactionManager =
        getConcurrencyManager().getTransactionManager();
  ...
  public void deleteResources() {
    ...
    if (log.isInfoEnabled()) log.info("Deleting all resources: " + dataDir);
   ...
   * itself.
   *
   * <p>Note: Files that are not recognized will be logged by the {@link ResourceFileFilter}.
   *
   ...
   *
   * <p>Data services MAY release data for views whose timestamp is less than or equal to the
   * specified release time IFF that action would be in keeping with their local history retention
   * policy (minReleaseAge) AND if the data is not required for the most current committed state
   * (data for the most current committed state is not releasable regardless of the release time or
   * the minReleaseAge).
   *
   * @see #purgeOldResources(), which is responsible for actually deleting the old resources.
   */
  ...
  /*
   * In order to have atomic semantics and prevent a read-historical operation from starting
   * concurrently that would have access to a view that is being purged, {@link
   * IndexManager#getIndex(String, long)} and {@link StoreManager#purgeOldResources()} MUST contend
   * for a shared lock. This is a {@link ReentrantReadWriteLock} since concurrent getIndex()
   * requests can proceed as long as {@link StoreManager#purgeOldResources()} is not running. Also
   * note that contention is not required for {@link ITx#UNISOLATED} index views.
   */
  protected final ReentrantReadWriteLock indexCacheLock = new ReentrantReadWriteLock();

  /*
   * Identify and delete resources no longer required by the index views from the current
   * releaseTime up to the lastCommitTime.
   *
   * <p>Note: The ability to read from a historical commit point requires the existence of the
   * journals back until the one covering that historical commit point. This is because the distinct
   * historical commit points for the indices are ONLY defined on the journals. The index segments
   * carry forward the committed state of a specific index as of the commitTime of the index from
   * which the segment was built. This means that you can substitute the index segment for the
   * historical index state on older journals, but the index segment carries forward only a single
   * commit point for the index so it can not be used to read from arbitrary historical commit
   * points.
   *
   * <p>The caller MUST hold the exclusive lock on the {@link WriteExecutorService}.
   *
   * @return A summary of the work done -or- <code>null</code> if the preconditions for the purge
   *     operation were not satisfied.
  ...
  protected final PurgeResult purgeOldResources() {
    ...
    if (lastCommitTime == 0L) {
      ...
      if (log.isInfoEnabled()) log.info("Nothing committed yet.");
     ...
     */
    {
      ...
      try {
        ...
        final ITransactionService txService = fed.getTransactionService();
    ...
    //            }
    //            {
    //                Iterator<WeakReference<ILocalBTreeView>> itr2 = ((IndexManager)
    // this).indexCache
    //                        .iterator();
    //                while (itr2.hasNext()) {
    ...
    final Event e =
        new Event(getFederation(), new EventResource(), EventType.PurgeResources).start();
    ...
    try {
      ...
      if (log.isInfoEnabled())
        log.info(
            "Choosen releaseTime="
                ...
                + " seconds before/after the lastCommitTime="
                + lastCommitTime
                + ")");
      ...
      final long firstCommitTime;
      {
        ...
        // open that journal.
        final AbstractJournal j0 = (AbstractJournal) openStore(resource.getUUID());
       ...
       * If the computed [releaseTime] is before the first commit
        ...
        if (log.isInfoEnabled()) log.info("Release time is earlier than any commit time.");
       ...
       * If the computed [releaseTime] GTE the last commit point then
        ...
        if (log.isInfoEnabled())
          log.info(
              "commitTimeToPreserve := " + commitTimeToPreserve + " (this is the lastCommitTime)");
      ...
      final long elapsedScanCommitIndicesTime;
      {
        ...
        elapsedScanCommitIndicesTime = System.currentTimeMillis() - begin;
      }
      if (log.isInfoEnabled()) {
        ...
        for (UUID uuid : resourcesInUse) {
          log.info("In use: file=" + resourceFiles.get(uuid) + ", uuid=" + uuid);
        }
      }
  ...
  /*
   * Delete unused resources given a set of resources that are still in use. The unused resources
   * are identified by scanning the {@link #journalIndex} and the {@link #segmentIndex}. For each
   * resource found in either of those indices which is NOT found in <i>resourcesInUse</i> and whose
   * createTime is GTE the specified timestamp, we take the following steps:
   *
   * <ol>
   *   <li>close iff open
   *   <li>remove from lists of known resources
   *   <li>clear the associated {@link ILRUCache}
   *   <li>delete in the file system
   * </ol>
   *
   * Note: {@link IndexSegment}s pose a special case. Their create time is the timestamp associated
   * with their source view. During asynchronous overflow processing we generate {@link
   * IndexSegment}s from the lastCommitTime of the old journal. Therefore their createTime timestamp
   * is often LT the <i>commitTimeToPreserve</i>. In order to prevent these {@link IndexSegment}s
   * from being released before they are put to use (by incorporating them into an index partition
   * view) we DO NOT add them to the {@link #segmentIndex} until they are part of an index partition
   * view.
   *
   * @param commitTimeToPreserve Resources created as of or later than this timestamp WILL NOT be
   *     deleted.
   * @param resourcesInUse The set of resources required by views as of the
   *     <i>commitTimeToPreserve</i>. These resources have create times LTE to
   *     <i>commitTimeToPreserve</i> but are in use but at least one view as of that commit time and
   *     therefore MUST NOT be deleted.
   * @see IndexManager#buildIndexSegment(String, org.embergraph.btree.ILocalBTreeView, boolean,
   *     long, byte[], byte[], Event)
   */
  private void deleteUnusedResources(
      final long commitTimeToPreserve, final Set<UUID> resourcesInUse) {
    ...
    int njournals = 0;
    {
      ...
      while (itr.hasNext()) {
        ...
        final IResourceMetadata resourceMetadata =
            (IResourceMetadata) SerializerUtil.deserialize(tuple.getValue());
       ...
       * Do NOT delete any resources whose createTime is GTE the
          ...
          if (log.isInfoEnabled())
            log.info(
                "Stopping at resource GTE commitTime to preserve: createTime="
                    + createTime
                    + ", file="
        ...
        try {
          ...
          log.error("Could not delete journal: " + resourceMetadata.getFile(), t);
        }
    ...
    int nsegments = 0;
    {
       ...
       * Do NOT delete any resources whose createTime is GTE the
        ...
        try {
          ...
          // log error and keep going.
          log.error("Could not delete segment - continuing: " + resourceMetadata.getFile(), t);
        }
  ...
  /*
   * Delete the resource in the file system and remove it from the {@link #storeCache} and {@link
   * #resourceFiles} and either {@link #journalIndex} or {@link #segmentIndex} as appropriate.
   *
   * <p><strong>DO NOT delete resources that are in use!</strong>
   *
   * <p>A resource that has not yet been incoporated into a view may be deleted without futher
   * concern. However, once a resource has been incorporated into a view then you MUST arange for
   * appropriate synchronization before the resource may be deleted. For example, {@link
   * #purgeOldResources()} imposes that constraint on the caller that they are responsible for
   * synchronization and is generally invoked during synchronous overflow since we know that there
   * are no active writers at that time.
   *
   * <p>Pre-conditions:
   *
   * <ul>
   *   <li>The resource identified by that {@link UUID} exists and is not the live journal.
   *   <li>The resource is not in use (not checked).
   *   <li>The resource is found in {@link #resourceFiles}.
   * </ul>
   *
   * Post-conditions:
   *
   * <ul>
   *   <li>The resource is closed if it was open and is no longer found in the {@link #storeCache}.
   *   <li>The resource is no longer found in {@link #resourceFiles}.
   *   <li>The backing file for the resource has been deleted (the backing file is obtain from
   *       {@link #resourceFiles}).
   *   <li>Various counters maintained by the {@link StoreManager} have been updated (bytes delete,
   *       bytes under management, etc).
   *   <li>The file has been removed from either the {@link #journalIndex} or the {@link
   *       #segmentIndex} as appropriate.
   * </ul>
   *
   * @param uuid The {@link UUID} which identifies the resource.
   * @param isJournal <code>true</code> if the resource is a journal.
   */
  protected void deleteResource(final UUID uuid, final boolean isJournal)
      throws NoSuchStoreException {

    if (log.isInfoEnabled()) log.info("deleteResource: uuid=" + uuid + ", isJournal=" + isJournal);
     ...
     */
    {
      ...
      if (log.isInfoEnabled())
        log.info("DELETE: file=" + file + ", uuid=" + uuid + ", isJournal=" + isJournal);
     ...
     */
    {
      ...
      if (isJournal) {
        ...
        synchronized (journalIndex) {
          ...
          final ITupleIterator<JournalMetadata> itr =
              journalIndex.rangeIterator(
                  null /* fromKey */,
                  null /* toKey */,
                  0 /* capacity */,
        ...
        synchronized (segmentIndex) {
          @SuppressWarnings("unchecked")
          final ITupleIterator<SegmentMetadata> itr =
              segmentIndex.rangeIterator(
                  null /* fromKey */,
                  null /* toKey */,
                  0 /* capacity */,
      ...
      if (!found) throw new NoSuchStoreException(uuid);
    }
  }

  /*
   * Variant used by {@link #deleteUnusedResources(long, Set)}, which is in turned invoked by {@link
   * #purgeOldResources()}. This implementation is different in that we have the {@link
   * IResourceManager} on hand when we need to delete the resource. I judge it worth the redundency
   * in the code to have a variant specific to this use case so that the DELETE log messages report
   * the {@link IResourceMetadata#getCreateTime() create time} which can be used as a cross-check on
   * {@link #purgeOldResources()}. Pre-conditions:
   *
   * <ul>
   *   <li>The resource described by the {@link IResourceMetadata} exists and is not the live
   *       journal.
   *   <li>The resource is not in use (not checked).
   *   <li>The resource is found in {@link #resourceFiles}.
   * </ul>
   ...
   *   <li>The resource is closed if it was open and is no longer found in the {@link #storeCache}.
   *   <li>The resource is no longer found in {@link #resourceFiles}.
   *   <li>The {@link ILRUCache} for that resource has been cleared.
   *   <li>The backing file for the resource has been deleted (the backing file is obtain from
   *       {@link #resourceFiles}).
   *   <li>Various counters maintained by the {@link StoreManager} have been updated (bytes delete,
   *       bytes under management, etc).
   * </ul>
   *
   * <p>Note: The caller MUST remove the entry for the resource from either {@link #journalIndex} or
   * the {@link #segmentIndex} as appropriate. For this use case, the caller can handle that
   * efficiently since they are already traversing an iterator on the appropriate {@link BTree} and
   * can use {@link Iterator#remove()} to delete the corresponding entry from the {@link BTree}.
   *
   * @param resourceMetadata The metadata describing the resource to be deleted.
   */
  private void deleteUnusedResource(final IResourceMetadata resourceMetadata) {

    if (log.isInfoEnabled()) log.info("deleteResource: " + resourceMetadata);
  ...
  /*
   * Finds the journal spanning the first {@link ICommitRecord} that is strictly greater than the
   * specified timestamp and returns the timestamp of that {@link ICommitRecord}.
   *
   * @param releaseTime A release time as set by {@link #setReleaseTime(long)}. Any resource as of
   *     this timestamp is available for release.
   * @return The timestamp of the first commit record whose timestamp is strictly greater than
   *     <i>releaseTime</i>.
   * @throws IllegalArgumentException If there is no commit point that is strictly greater than the
   *     releaseTime. This implies that the release time is either in the future or, if the
   *     releaseTime is equal to the last commitTime, that you are trying to release everything in
   *     the database.
   */
  protected long getCommitTimeStrictlyGreaterThan(final long releaseTime) {
    ...
    final ICommitRecord commitRecord = journal.getCommitRecordStrictlyGreaterThan(releaseTime);
    ...
    if (commitRecord == null) {
      ...
      log.warn(
          "Examining prior journal (fence post): closeTime="
              + closeTime
              + ", releaseTime="
              + releaseTime);
    ...
    log.warn("Chose commitTime=" + commitTime + " given releaseTime=" + releaseTime);
  ...
  /*
   * Finds all resources used by any registered index as of the <i>commitTimeToPreserve</i> up to
   * and including the lastCommitTime for the live journal.
   *
   * <p>Note: We include all dependencies for all commit points subsequent to the probe in order to
   * ensure that we do not accidently release dependencies required for more current views of the
   * index.
   *
   * <p>Note: This method solely considers the index views as defined at each commit point starting
   * with the given commit point. It DOES NOT pay attention to the release time or to any other
   * aspect of the state of the system.
   *
   * @param commitTimeToPreserve The commit time corresponding to the first commit point which must
   *     be preserved.
   * @return The set of resource {@link UUID}s required by at least one index for any commit time
   *     GTE the specified commit time.
   */
  protected Set<UUID> getResourcesForTimestamp(final long commitTimeToPreserve) {
    ...
    synchronized (journalIndex) {
      ...
      while (itr.hasNext()) {
        ...
        final long lastCommitTime = journal.getRootBlockView().getLastCommitTime();
       ...
       * In order to scan timestamps from [commitTime] through to
           ...
           */
          @SuppressWarnings("unchecked")
          final ITupleIterator<ICommitRecord> itr2 =
              commitRecordIndex.rangeIterator(commitTimeToPreserve /* fromKey */, null /* toKey */);
       ...
       * For each distinct checkpoint, load the BTree and
             ...
             */
            final ICommitRecord commitRecord = commitRecordIndex.fetchCommitRecord(entry2);

            final Name2Addr name2addr =
                (Name2Addr)
                    Name2Addr.load(
                        journal,
                        commitRecord.getRootAddr(AbstractJournal.ROOT_NAME2ADDR),
                        true /* readOnly */);
       ...
       * New checkpoint address.
                ...
                final IndexMetadata indexMetadata =
                    IndexMetadata.read(journal, checkpoint.getMetadataAddr());
   ...
   */
  public static String munge(final String s) {
  ...
  public File getIndexSegmentFile(final IndexMetadata indexMetadata) {
    ...
    return getIndexSegmentFile(
        indexMetadata.getName(),
        indexMetadata.getIndexUUID(),
        pmd == null ? -1 : pmd.getPartitionId());
  }

  /*
   * Return the file on which a new {@link IndexSegment} should be written. The file will exist but
   * will have zero length. The file is created using the {@link File#createTempFile(String, String,
   * File)} mechanism within the configured {@link #dataDir} in the subdirectory for the specified
   * scale-out index.
   *
   * <p>Note: The index name appears in the file path above the {@link UUID} of the scale-out index.
   * Therefore it is not possible to have collisions arise in the file system when given indices
   * whose scale-out names differ only in characters that are munged onto the same character since
   * the files will always be stored in a directory specific to the scale-out index.
   *
   ...
   *     partitioned (handles the MDS which does not use partitioned indices at this time).
   * @return The {@link File} on which a {@link IndexSegmentStore} for that index partition may be
   *     written. The file will be unique and empty.
   * @throws IllegalArgumentException if any argument is <code>null</code>
   * @throws IllegalArgumentException if the partitionId is negative and not <code>-1</code>
   * @todo should the filename be relative or absolute?
   */
  public File getIndexSegmentFile(
      final String scaleOutIndexName, final UUID indexUUID, final int partitionId) {

    assertOpen();

    if (scaleOutIndexName == null) throw new IllegalArgumentException();

    if (indexUUID == null) throw new IllegalArgumentException();

    if (partitionId < -1) throw new IllegalArgumentException();
    ...
    final File indexDir = new File(segmentsDir, mungedName + File.separator + indexUUID.toString());

    // make sure that directory exists.
    indexDir.mkdirs();

    final String partitionStr =
        (partitionId == -1 ? "" : "_shardId" + leadingZeros.format(partitionId));
    ...
    if (log.isInfoEnabled()) log.info("Created file: " + file);

    return file;
  }

  /*
   * This attempts to obtain the exclusive lock for the {@link WriteExecutorService}. If successful,
   * it purges any resources that are no longer required based on {@link
   * StoreManager.Options#MIN_RELEASE_AGE} and optionally truncates the live journal such that no
   * free space remains in the journal.
   *
   ...
   * the system is heavily loaded by write tasks.
   *
   * @param timeout The timeout (in milliseconds) that the method will await the pause of the write
   *     service.
   ...
   * @return <code>true</code> if successful and <code>false</code> if the write service could not
   *     be paused after the specified timeout.
   * @param truncateJournal When <code>true</code> the live journal will be truncated such that no
   *     free space remains in the journal. If writes are directed to the live journal after it has
   *     been truncated then it will transparently re-extended.
   * @throws IOException
   * @throws InterruptedException
   * @throws IllegalStateException if the {@link StoreManager} is not running.
   */
  public boolean purgeOldResources(final long timeout, final boolean truncateJournal)
      throws InterruptedException {

    final WriteExecutorService writeService = getConcurrencyManager().getWriteService();
  ...
  /*
   * When the {@link StoreManager} is relatively new (as measured by the #of bytes under management)
   * we discount the journal extent in order to trigger overflow earlier. Together with the discount
   * applied to the split handler by the {@link AsynchronousOverflowTask}, this helps to break down
   * new index partitions allocated on the new data service and re-distribute those index partitions
   * (if there are other data services which have even less utilization).
   *
   * @param p The properties (modified as side-effect).
   */
  protected void overrideJournalExtent(final Properties p) {

    final long bytesUnderManagement = this.bytesUnderManagement.get();

    if (accelerateOverflowThreshold == 0 || bytesUnderManagement >= accelerateOverflowThreshold) {
    ...
    final double d = (double) bytesUnderManagement / accelerateOverflowThreshold;

    final long initialExtent =
        Long.parseLong(p.getProperty(Options.INITIAL_EXTENT, Options.DEFAULT_INITIAL_EXTENT));

    final long maximumExtent =
        Long.parseLong(p.getProperty(Options.INITIAL_EXTENT, Options.DEFAULT_MAXIMUM_EXTENT));

    /*
     * Don't allow a journal w/ less than 10M or the minimum specified by
     * Options.
     */
    final long minimumExtent = Math.max(Options.minimumInitialExtent, Bytes.megabyte * 10);
     ...
     * maximum extent.
     */
    final long adjustedExtent = Math.max(minimumExtent, (long) (maximumExtent * d));
