 ...
 *
 * <ol>
 *   <li>mutable unisolated indices open on the journal
 *   <li>mutable isolated indices open in writable transactions
 *   <li>historical read-only indices open on old journals
 *   <li>historical read-only index segments
 * </ol>
 *
 * The latter two classes of event sources exist iff {@link Journal#overflow()} is handled by
 * creating a new {@link Journal} and evicting data from the old {@link Journal} asynchronously onto
 * read-optimized {@link IndexSegment}s.
 *
 * <p>Other resource consumption events deal directly with transactions
 *
 * <ol>
 *   <li>open a transaction
 *   <li>close a transaction
 *   <li>a heartbeat for each write operation on a transaction is used to update the resource
 *       consumption of the store
 * </ol>
 *
 * <p>Latency events include
 *
 * <ol>
 *   <li>request latency, that is, the time that a request waits on a queue before being serviced
 *   <li>transactions per second
 * </ol>
 *
 * FIXME Revisit and refactor - this is incomplete as it stands. This is event focused, which is
 * fine. We also have the {@link CounterSet}s which allow us to report statistics regarding classes
 * of events.
 *
 * @todo use {@link MDC} to put metadata into the logging context {thread, host, dataService, global
 *     index name, local index name (includes the index partition), etc}.
...
public class ResourceEvents {
   ...
   *     order to aggregate results from multiple hosts in a scale-out solution.
   */
   ...
   */
  public static void openUnisolatedIndex(final String name) {
   ...
   *
   * @param name The index name.
   * @todo never invoked since we do not explicitly close out indices and are not really able to
   *     differentiate the nature of the index when it is finalized (unisolated vs isolated vs index
   *     segment can be identified based on their interfaces).
   * @todo add reporting for {@link AbstractBTree#reopen()}.
   */
  public static void closeUnisolatedIndex(final String name) {
   ...
   * @param name The index name.
   */
  public static void dropUnisolatedIndex(final String name) {
   ...
   * @param revisionTime The timestamp assigned to the revisions written by the transactions when it
   *     commits (non-zero iff this was a writable transaction that committed successfully and zero
   *     otherwise).
   * @param aborted True iff the transaction aborted vs completing successfully.
   */
  ...
  public static void isolateIndex(long startTime, String name) {
    ...
    if (log.isInfoEnabled()) log.info("tx=" + startTime + ", name=" + name);
   ...
   * @param nbytes The total #of bytes available on the journal.
   * @param bufferMode The buffer mode in use by the journal.
   */
   ...
   *
   * @param filename The filename or null iff the journal was not backed by a file.
   * @param nbytes The total #of bytes available (vs written) on the journal.
   * @todo this does not differentiate between extension of a buffer backing a journal and extension
   *     of a {@link TemporaryRawStore}. This means that the resources allocated to a transaction vs
   *     the unisolated indices on a journal can not be differentiated.
   */
   ...
   *
   * @param filename The filename or null iff the journal was not backed by a file.
   * @todo also report deletion of resources for journals that were already closed but not yet
   *     deleted pending client leases or updates of the metadata index (in the {@link
   *     MasterJournal}).
   */
