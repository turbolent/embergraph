 ...
 * {@link SoftReference} was chosen because it is important to keep these {@link BTree}s open so
 * that we do not loose their buffers until we have finish asynchronous overflow for a given {@link
 * BTree}. Once asynchronous overflow processing is complete for the {@link BTree} you SHOULD use
 * {@link #clearRef()} to release your hold those buffers.
 *
...
class BTreeMetadata {
  ...
  /** The object which may be used to (re-)open the {@link BTree} and the index partition view. */
  protected final ResourceManager resourceManager;

  /** The commit time associated with the {@link BTree} and the index partition view. */
  ...
  public final BTree getBTree() {
    ...
    if (btree == null) {
      ...
      synchronized (this) {
        ...
        final AbstractJournal store = resourceManager.getJournal(commitTime);

        btree = (BTree) resourceManager.getIndexOnStore(name, commitTime, store);
   ...
   *     <p>Note: This field can be dropped once we are running split, tailSplit, and scatterSplit
   *     as merge after actions. At the same time, make sure that those tasks will not accept a view
  ...
  /** The percentage of leaf splits which occurred at or near the head of the {@link BTree}. */
   ...
   * the {@link #lock}. If you want to make an atomic decision based on this value, then make sure
   ...
   * @throws IllegalMonitorStateException unless the {@link #lock} is held by the caller.
   * @throws IllegalStateException if the action has already been set.
   */
  public void setAction(final OverflowActionEnum action) {
    ...
    if (actionRef.get() != null) {
      ...
      throw new IllegalStateException("Already set: " + actionRef.get() + ", given=" + action);
    }
   ...
   * @param btreeCounters The aggregated counters for the {@link BTree} or the {@link
   *     ILocalBTreeView index partition view} as reported by the {@link IndexManager}
   */
  public BTreeMetadata(
    ...
    if (pmd == null) log.warn("Not an index partition: " + name);
    ...
    if (sourceJournalCount + sourceSegmentCount < 2) {
       ...
       * IOWAIT.
       */
      this.mergePriority = (sourceJournalCount - 1) * A + (sourceSegmentCount * B)
    ...
    this.percentHeadSplits = btreeCounters.headSplit / (btreeCounters.leavesSplit + 1d);

    // Note: +1 in the denominator to avoid divide by zero.
    this.percentTailSplits = btreeCounters.tailSplit / (btreeCounters.leavesSplit + 1d);
  }
