...
public class SplitResult extends AbstractResult {
   ...
   * @param splits Note: At this point we have the history as of the lastCommitTime in N index
   *     segments. Also, since we constain the resource manager to refuse another overflow until we
   *     have handle the old journal, all new writes are on the live index.
   * @param buildResults A {@link BuildResult} for each output split.
   */
  public SplitResult(
      final String name,
