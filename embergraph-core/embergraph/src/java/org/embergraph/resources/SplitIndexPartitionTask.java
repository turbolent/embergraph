 ...
 * partition is passed into this task because it is not yet part of the view. Based on the nominal
 * size of the index partition and the size of the segment, N=segSize/nominalSize splits will be
 * generated, requiring N-1 separator keys.
 *
 * <p>The task uses the linear list API to identify N-1 separator key which would split the index
 * segment and assumes that the data is evenly distributed across the keys within the index segment.
 * The buffered writes are ignored when determining the separator keys (most data will be on the
 * index segment if the journal extent roughly the same as the nominal index segment extent and
 * multiple index partitions are registered on the journal). Application constraints on the choice
 * of the separator keys will be honored and can result in fewer splits being generated.
 *
 ...
 * index partition as N new index partition and copy the buffered writes into the appropriate index
 * partition. During the atomic update the original index partition becomes un-defined and new index
 * partitions are defined in its place which span the same total key range and have the same data.
 *
 ...
 * @version $Id: SplitIndexPartitionTask.java 2265 2009-10-26 12:51:06Z thompsonbry $
 */
public class SplitIndexPartitionTask extends AbstractPrepareTask<AbstractResult> {
  ...
  /**
   * The adjusted nominal bytes on disk for a full shard after a compacting merge. Note that this
   * MAY have been overridden in order to promote a split so you MUST use this instance and NOT
   ...
   */
  protected SplitIndexPartitionTask(final ViewMetadata vmd, final UUID moveTarget) {
  ...
  /**
   * @param vmd
   * @param moveTargets When non-<code>null</code> the index partitions generated by the split will
   *     be moved to the identified data services. If this data service is included in the array,
   *     then an index partition will be left on this data service. If the array contains a single
   *     element, then only the rightSibling of the split will be moved.
   */
  protected SplitIndexPartitionTask(final ViewMetadata vmd, final UUID[] moveTargets) {
    ...
    if (moveTargets != null) {
      ...
      if (moveTargets.length == 1 && resourceManager.getDataServiceUUID().equals(moveTargets[0])) {
  ...
  /**
   * Decides how many index partitions should be generated (N) and builds N {@link IndexSegment}s
   * from the source index partition. If N will be ONE (1) if a detailed inspection of the source
   * index partition reveals that it SHOULD NOT be split.
   *
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          log.error("Split handler failure - will do build instead: name=" + name + " : " + t, t);
        ...
        if (splits == null) {
          ...
          if (moveTargets != null && moveTargets.length >= 1) {
            ...
            return concurrencyManager.submit(new MoveTask(vmd, moveTargets[0])).get();
            ...
            return concurrencyManager.submit(new CompactingMergeTask(vmd)).get();
      ...
      if (moveTargets != null) {
        ...
        if (moveTargets.length == 1) {
          ...
          MoveTask.doAtomicUpdate(
              ...
              splitResult.buildResults[bestMoveIndex],
              moveTargets[0],
              newPartitionId,
              e);
   ...
   * @param parentEvent
   */
  protected static void doSplitAtomicUpdate(
    ...
    try {
      ...
      final String[] resources = new String[splits.length + 1];
      {
        ...
        for (final Split split : splits) {
          ...
          resources[i + 1] =
              DataService.getIndexPartitionName(vmd.indexMetadata.getName(), partitionId);
      ...
      final AbstractTask<Void> task =
          new AtomicUpdateSplitIndexPartitionTask(
              resourceManager,
              resources,
              action,
              ...
              result,
              parentEvent
                  .newSubEvent(OverflowSubtaskEnum.AtomicUpdate)
                  .addDetail(
                      "summary", action + "(" + vmd.name + "->" + Arrays.toString(resources)));
      ...
      for (BuildResult r : result.buildResults) {
        ...
        // delete it.
        resourceManager.deleteResource(r.segmentMetadata.getUUID(), false /* isJournal */);
      }
  ...
  /**
   * An {@link ITx#UNISOLATED} operation that splits the live index using the same {@link Split}
   * points, generating new index partitions with new partition identifiers. The old index partition
   * is deleted as a post-condition. The new index partitions are registered as a post-condition.
   * Any data that was accumulated in the live index on the live journal is copied into the
   * appropriate new {@link BTree} for the new index partition on the live journal.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  protected static class AtomicUpdateSplitIndexPartitionTask
    ...
    public AtomicUpdateSplitIndexPartitionTask(
      ...
      if (splitResult == null) throw new IllegalArgumentException();
    ...
    protected Void doTask() throws Exception {
      ...
      try {
        ...
        if (oldpmd.getSourcePartitionId() != -1) {
          ...
          throw new IllegalStateException(
              "Split not allowed during move: sourcePartitionId=" + oldpmd.getSourcePartitionId());
        }
        ...
        for (int i = 0; i < splits.length; i++) {
          ...
          assert newCounter > oldCounter
              : "newCounter=" + newCounter + " not GT oldCounter=" + oldCounter;
          ...
          if (INFO)
            log.info(
                "Copied "
                    + ncopied
                    + " index entries from the live index "
                    + name
                    + " onto "
                    + name2);
          ...
          if (INFO) log.info("Registering index: " + name2);
        ...
        if (INFO) log.info("Dropping source index: " + name);
        ...
        if (INFO)
          log.info(
              "Notified metadata service: name="
                  + name
                  + " was split into "
                  + Arrays.toString(locators));

        // will notify tasks that index partition was split.
        resourceManager.setIndexPartitionGone(name, StaleLocatorReason.Split);
