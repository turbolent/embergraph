 ...
 * writers are running, the {@link WriteExecutorService} triggers synchronous overflow. Synchronous
 * overflow is a low-latency process which creates a new journal to absorb future writes, re-defines
 * the views for all index partitions found on the old journal to include the new journal as their
 * first source, and initiates a background thread performing asynchronous overflow post-processing.
 *
 * <p>Asynchronous overflow post-processing is responsible for identifying index partitions overflow
 * (resulting in a split into two or more index partitions), index partition underflow (resulting in
 * the join of the under-capacity index partition with its rightSibling), index partition moves (the
 * index partition is moved to a different {@link DataService}), and index partition builds (an
 * {@link IndexSegment} is created from the current view in what is effectively a compacting merge).
 * Overflow processing is suspended during asynchronous post-processing, but is automatically
 * re-enabled once post-processing completes.
 *
...
public abstract class OverflowManager extends IndexManager {
   ...
   *     introduced in the future to restrict the rate at which a DS can shed shards by moving them
   *     to other nodes.
   *     <p>Note: This is also used to disable moves by some of the unit tests so we need a way to
   *     replace that functionality before this can be taken out.
   */
   ...
   * views must be made compact and SHOULD NOT be used for deployed federations</strong>). The state
   * of the flag is cleared each time asynchronous overflow processing begins.
   *
   * @see DataService#forceOverflow(boolean, boolean)
   */
   ...
   * @todo Encapsulate with split accelerator factor when this is the first index partition for some
   *     scale-out index.
   */
   ...
   * By disallowing further writes on the shard we prevent it from dragging down performance for the
   * entire data service and push the problem back on the application. In order to remedy this issue
   * on a pre-existing index you must fix the split handler, register the new split handler on the
   * MDS and on each shard on the index, and then re-enable writes for the index.
   *
  ...
  public boolean isOverflowEnabled() {
    ...
    return overflowEnabled
        && (overflowMaxCount == 0
            || overflowCounters.synchronousOverflowCounter.get() < overflowMaxCount);
  }

  /** <code>true</code> unless an overflow event is currently being processed. */
  ...
  public interface Options extends IndexManager.Options, IServiceShutdown.Options {
     ...
     * be split, joined nor moved away from this {@link ResourceManager}.
     */
     ...
     * but it can be enabled if you want higher throughput (for a while) and you know that the data
     * will be well distributed on the federation after N overflows. Once synchronous overflow is
     * disabled, all future writes will be buffered by the live journal and index partition builds,
     * merges, splits, joins, and moves will no longer be executed. Eventually the live journal
     * extent will grow large enough that throughput will drop (due to IOWAIT on random seeks
     * against the journal) and it is possible that the maximum possible journal extent can be
     * exceeded unless you also configure {@link org.embergraph.journal.Options#OFFSET_BITS} for
     * scale-up.
     *
     * @deprecated This is no longer used, even for testing.
     */
    String OVERFLOW_MAX_COUNT = OverflowManager.class.getName() + ".overflowMaxCount";
     ...
     * org.embergraph.journal.Options#MAXIMUM_EXTENT}. If the result is GTE the current extend of
     * the live journal, then synchronous overflow processing will be triggered. However, note that
     * synchronous overflow processing can not be triggered until asynchronous overflow processing
     * for the last journal is complete. Therefore if asynchronous overflow processing takes a long
     * time, the overflow threshold might not be checked until after it has already been exceeded.
     *
     * <p>The main purpose of this property is to trigger overflow processing before the maximum
     * extent is exceeded. The trigger needs to lead the maximum extent somewhat since overflow
     * processing can not proceed until there is an exclusive lock on the write service, and tasks
     * already running will continue to write on the live journal. Overflowing the maximum extent is
     * not a problem as long as the {@link BufferMode} supports transparent extension of the
     * journal. However, some {@link BufferMode}s do not and therefore they can not be used reliably
     * with the overflow manager.
     */
    String OVERFLOW_THRESHOLD = OverflowManager.class.getName() + ".overflowThreshold";
     ...
     * ZERO (0), index partitions will not be copied during overflow processing (unless they are
     * empty). While it is important to keep down the latency of synchronous overflow processing,
     * small indices can be copied so quickly that it is worth it to avoid the heavier index segment
     * build operation.
     *
     ...
     * IDataService}. Since each index (partition) is single threaded for writes, we can increase
     * the potential concurrency if we split the initial index partition. We accelerate decisions to
     * split index partitions by reducing the minimum and target #of tuples per index partition for
     * an index with fewer than the #of index partitions specified by this parameter. When ZERO (0)
     * this feature is disabled and we do not count the #of index partitions.
     */
    String ACCELERATE_SPLIT_THRESHOLD =
        OverflowManager.class.getName() + ".accelerateSplitThreshold";
     ...
     * than <code>1.0</code> are permissible and imply that the post-split leftSibling index
     * partition will be approximately a nominal index partition. However the maximum percentage may
     * not be greater than <code>2.0</code> (200 percent).
     */
    String PERCENT_OF_SPLIT_THRESHOLD =
        OverflowManager.class.getName() + ".percentOfSplitThreshold";
     ...
     */
    String TAIL_SPLIT_THRESHOLD = OverflowManager.class.getName() + ".tailSplitThreshold";
     ...
     * resources. This is basically an acceleration factor for index partition splits when a host
     * has a relatively higher workload than its peers. The purpose of a "hot split" is to increase
     * the potential concurrency by breaking an active index partition into two index partitions. If
     * the writes on the index partition are evenly distributed, then this can double the
     * concurrency if the host has spare cycles. Reasonable values are on the order of [.25:.75].
     * Hot splits may be effectively disabled by raising the percent of split to GTE {@value
     * #PERCENT_OF_SPLIT_THRESHOLD}.
     *
     * @deprecated Hot splits are not implemented and this option does not do anything. It will be
     *     going away soon.
     */
    String HOT_SPLIT_THRESHOLD = OverflowManager.class.getName() + ".hotSplitThreshold";
     ...
     */
    String SCATTER_SPLIT_ENABLED = OverflowManager.class.getName() + ".scatterSplitEnabled";
     ...
     * #ACCELERATE_SPLIT_THRESHOLD} behaviors since the target for the split size increases as a
     * function of the #of index partitions. For example, a scatter split can cause the adjust
     * nominal size of a shard to jump to its configured setting, which will cause the shards to be
     * "undercapacity" and hence drive JOINs. In order to fix this we have to somehow discount
     * joins, either by requiring deletes on the index partition or by waiting some #of overflows
     * since the split, etc. Alternatively, joins could be ignored unless there are more partitions
     * of a given index than were (or would be) produced by a scatter split. For the moment joins
     * are disabled by default.
     */
    String JOINS_ENABLED = OverflowManager.class.getName() + ".joinsEnabled";
     ...
     *
     * <p>Note: This makes sure that we don't do a move if there are only a few active index
     * partitions on this service. This value is also used to place an upper bound on the #of index
     * partitions that can be moved away from this service - if we move too many (or too many at
     * once) then this service stands a good chance of becoming under-utilized and index partitions
     * will just bounce around which is very inefficient.
     *
     * <p>Note: Even when only a single index partition for a new scale-out index is initially
     * allocated on this service, if it is active and growing it will eventually split into enough
     * index partitions that we will begin to re-distribute those index partitions across the
     * federation.
     *
     * <p>Note: Index partitions are considered to be "active" iff {@link ITx#UNISOLATED} or {@link
     * ITx#READ_COMMITTED} operations are run against the index partition during the life cycle of
     * the live journal. There may be many other index partitions on the same service that either
     * are never read or are subject only to historical reads. However, since only the current state
     * of the index partition is moved, not its history, moving index partitions which are only the
     * target for historical reads will not reduce the load on the service. Instead, read burdens
     * are reduced using replication.
     *
     * @see #DEFAULT_MINIMUM_ACTIVE_INDEX_PARTITIONS
     */
    String MINIMUM_ACTIVE_INDEX_PARTITIONS =
        OverflowManager.class.getName() + ".minimumActiveIndexPartitions";
     ...
     *
     * <p>Note: Index partition moves MAY be disabled by setting this property to ZERO (0).
     *
     * @see #DEFAULT_MAXIMUM_MOVES
     * @deprecated Moves are now decided on a case by case basis. An alternative parameter might be
     *     introduced in the future to restrict the rate at which a DS can shed shards by moving
     *     them to other nodes.
     */
    String MAXIMUM_MOVES = OverflowManager.class.getName() + ".maximumMoves";
     ...
     *
     * <p>Note: Index partitions are moved to the identified under-utilized services using a
     * round-robin approach which aids in distributing the load across the federation.
     *
     * <p>Note: Index partition moves MAY be disabled by setting this property to ZERO (0).
     *
     ...
     */
    String MAXIMUM_MOVE_PERCENT_OF_SPLIT =
        OverflowManager.class.getName() + ".maximumMovePercentOfSplit";
     ...
     * data evenly across the federation.
     */
    String MOVE_PERCENT_CPU_TIME_THRESHOLD =
        OverflowManager.class.getName() + ".movePercentCpuTimeThreshold";
     ...
     *
     * <p>Once this #of optional compacting merge tasks have been identified for a given overflow
     * event, the remainder of the index partitions that are neither split, joined, moved, nor
     * copied will use incremental builds. An incremental build is generally cheaper since it only
     * copies the data on the mutable {@link BTree} for the lastCommitTime rather than the fused
     * view. A compacting merge permits the older index segments to be released and results in a
     * simpler view with view {@link IndexSegment}s. Either a compacting merge or an incremental
     * build will permit old journals to be released once the commit points on those journals are no
     * longer required.
     *
     * <p>Note: Mandatory compacting merges are identified based on {@link
     * #MAXIMUM_JOURNALS_PER_VIEW} and {@link #MAXIMUM_SEGMENTS_PER_VIEW}. There is NO limit the #of
     * mandatory compacting merges that will be performed during an asynchronous overflow event.
     * However, each mandatory compacting merge does count towards the maximum #of optional merges.
     * Therefore if the #of mandatory compacting merges is greater than this parameter then NO
     * optional compacting merges will be selected in a given overflow cycle.
     *
     * @deprecated merges are now performed in priority order while time remains in a given
     *     asynchronous overflow cycle.
     */
    String MAXIMUM_OPTIONAL_MERGES_PER_OVERFLOW =
        OverflowManager.class.getName() + ".maximumOptionalMergesPerOverflow";
     ...
     * OverflowActionEnum#Copy} is not selected. As long as index partition splits, builds or merges
     * are performed the #of journals in the view WILL NOT exceed 2 and will always be ONE (1) after
     * an asynchronous overflow in which a split, build or merge was performed.
     *
     * <p>It is extremely important to perform compacting merges in order to release dependencies on
     * old resources (both journals and index segments) and keep down the #of sources in a view.
     * This is especially true when those sources are journals. Journals are organized by write
     * access, not read access. Once the backing buffer for a journal is released there will be
     * large spikes in IOWAIT when reading on an old journal as reads are more or less random.
     *
     * <p>Note: The {@link #MAXIMUM_OPTIONAL_MERGES_PER_OVERFLOW} will be ignored if a compacting
     * merge is recommended for an index partition based on this parameter.
     *
     * <p>Note: Synchronous overflow will refuse to copy tuples for an index partition whose mutable
     * {@link BTree} otherwise satisfies the {@link #COPY_INDEX_THRESHOLD} if the #of sources in the
     * view exceeds thresholds which demand a compacting merge.
     *
     * @deprecated merges are now performed in priority order while time remains in a given
     *     asynchronous overflow cycle.
     */
    String MAXIMUM_JOURNALS_PER_VIEW = OverflowManager.class.getName() + ".maximumJournalsPerView";
     ...
     * <p>It is extremely important to perform compacting merges in order to release dependencies on
     * old resources (both journals and index segments) and keep down the #of sources in a view.
     * However, this is less important when those resources are {@link IndexSegment}s since they are
     * very efficient for read operations. In this case the main driver is to reduce the complexity
     * of the view, to require fewer open index segments (and associated resources) in order to
     * materialize the view, and to make it possible to release index segments and thus have less of
     * a footprint on the disk.
     *
     * <p>Note: The {@link #MAXIMUM_OPTIONAL_MERGES_PER_OVERFLOW} will be ignored if a compacting
     * merge is recommended for an index partition based on this parameter.
     *
     ...
     *     asynchronous overflow cycle.
     */
    String MAXIMUM_SEGMENTS_PER_VIEW = OverflowManager.class.getName() + ".maximumSegmentsPerView";
     ...
     * 200M, a reasonable value for this might be 1/10th to 1/5th of that, so 20-40M. The key is to
     * keep the builds fast so they should not do too much work while reducing the frequency with
     * which we must do a compacting merge. This option only effects the #of {@link IndexSegment}s
     * that will be incorporated into an {@link OverflowActionEnum#Build} operation. When ZERO (0L),
     * {@link OverflowActionEnum#Build} operations will only include the data from the historical
     * journal.
     *
     * @todo Configure as a percentage of the nominal shard size (ignoring any acceleration factor).
     */
    String MAXIMUM_BUILD_SEGMENT_BYTES =
        OverflowManager.class.getName() + ".maximumBuildSegmentsBytes";
     ...
     * will be canceled.
     *
     * <p>Asynchronous overflow processing is responsible for splitting, moving, and joining index
     * partitions. The asynchronous overflow tasks are written to fail "safe". Also, each task may
     * succeed or fail on its own. Iff the task succeeds, then its effect is made restart safe.
     * Otherwise clients continue to use the old view of the index partition.
     *
     * <p>If asynchronous overflow processing DOES NOT complete each time then we run several very
     * serious and non-sustainable risks, including: (a) the #of sources in a view can increase
     * without limit; and (b) the #of journal that must be retained can increase without limit.
     *
     * @deprecated Asynchronous overflow processing should run to completion with a minimum goal of
     *     an incremental build for each index partition having data on the previous journal.
     */
     ...
     *
     * @deprecated by {@link #MERGE_SERVICE_CORE_POOL_SIZE} and {@link
     *     #BUILD_SERVICE_CORE_POOL_SIZE}.
     */
    String OVERFLOW_TASKS_CONCURRENT = OverflowManager.class.getName() + ".overflowTasksConcurrent";
     ...
     *     an incremental build for each index partition having data on the previous journal.
     */
    String OVERFLOW_CANCELLED_WHEN_JOURNAL_FULL =
        OverflowManager.class.getName() + ".overflowCancelledWhenJournalFull";
    ...
    //         */
    //        String PURGE_RESOURCES_TIMEOUT = OverflowManager.class.getName() +
    ...
    String BUILD_SERVICE_CORE_POOL_SIZE =
        OverflowManager.class.getName() + ".buildService.corePoolSize";

    // @todo or (ncores/2)-1?
    String DEFAULT_BUILD_SERVICE_CORE_POOL_SIZE = "3";

    /** The #of threads in the pool handling index partition merges. */
    String MERGE_SERVICE_CORE_POOL_SIZE =
        OverflowManager.class.getName() + ".mergeService.corePoolSize";
     ...
     *
     * <p>Note: If you modify this, you may also need to modify the size of the buffers in the
     * {@link DirectBufferPool} used to fully buffer the nodes region of the index segment file.
     */
    String NOMINAL_SHARD_SIZE = OverflowManager.class.getName() + ".nominalShardSize";
  ...
  public OverflowManager(final Properties properties) {
    ...
    // overflowEnabled
    {
      overflowEnabled =
          Boolean.parseBoolean(
              properties.getProperty(Options.OVERFLOW_ENABLED, Options.DEFAULT_OVERFLOW_ENABLED));

      if (log.isInfoEnabled()) log.info(Options.OVERFLOW_ENABLED + "=" + overflowEnabled);
    }
      ...
      overflowMaxCount =
          Integer.parseInt(
              properties.getProperty(
                  Options.OVERFLOW_MAX_COUNT, Options.DEFAULT_OVERFLOW_MAX_COUNT));

      if (log.isInfoEnabled()) log.info(Options.OVERFLOW_MAX_COUNT + "=" + overflowMaxCount);
    }
    ...
    // overflowThreshold
    {
      overflowThreshold =
          Double.parseDouble(
              properties.getProperty(
                  Options.OVERFLOW_THRESHOLD, Options.DEFAULT_OVERFLOW_THRESHOLD));

      if (log.isInfoEnabled()) log.info(Options.OVERFLOW_THRESHOLD + "=" + overflowThreshold);
    }
    ...
    // overflowTimeout
    {
      overflowTimeout =
          Long.parseLong(
              properties.getProperty(Options.OVERFLOW_TIMEOUT, Options.DEFAULT_OVERFLOW_TIMEOUT));

      if (log.isInfoEnabled()) log.info(Options.OVERFLOW_TIMEOUT + "=" + overflowTimeout);
    }
    ...
    // overflowTasksConcurrent
    {
      overflowTasksConcurrent =
          Integer.parseInt(
              properties.getProperty(
                  Options.OVERFLOW_TASKS_CONCURRENT, Options.DEFAULT_OVERFLOW_TASKS_CONCURRENT));

      if (log.isInfoEnabled())
        log.info(Options.OVERFLOW_TASKS_CONCURRENT + "=" + overflowTasksConcurrent);

      if (overflowTasksConcurrent < 0) {

        throw new IllegalArgumentException(
            Options.OVERFLOW_TASKS_CONCURRENT + " : must be non-negative.");
      }
    }
    ...
    // overflowCancelledWhenJournalFull
    {
      ...
      if (log.isInfoEnabled())
        log.info(
            Options.OVERFLOW_CANCELLED_WHEN_JOURNAL_FULL + "=" + overflowCancelledWhenJournalFull);
    }
    ...
    // copyIndexThreshold
    {
      copyIndexThreshold =
          Integer.parseInt(
              properties.getProperty(
                  Options.COPY_INDEX_THRESHOLD, Options.DEFAULT_COPY_INDEX_THRESHOLD));

      if (log.isInfoEnabled()) log.info(Options.COPY_INDEX_THRESHOLD + "=" + copyIndexThreshold);
    ...
    // accelerateSplitThreshold
    {
      accelerateSplitThreshold =
          Integer.parseInt(
              properties.getProperty(
                  Options.ACCELERATE_SPLIT_THRESHOLD, Options.DEFAULT_ACCELERATE_SPLIT_THRESHOLD));

      if (log.isInfoEnabled())
        log.info(Options.ACCELERATE_SPLIT_THRESHOLD + "=" + accelerateSplitThreshold);
    ...
    // percentOfSplitThreshold
    {
      percentOfSplitThreshold =
          Double.parseDouble(
              properties.getProperty(
                  Options.PERCENT_OF_SPLIT_THRESHOLD, Options.DEFAULT_PERCENT_OF_SPLIT_THRESHOLD));

      if (log.isInfoEnabled())
        log.info(Options.PERCENT_OF_SPLIT_THRESHOLD + "=" + percentOfSplitThreshold);
    ...
    // tailSplitThreshold
    {
      tailSplitThreshold =
          Double.parseDouble(
              properties.getProperty(
                  Options.TAIL_SPLIT_THRESHOLD, Options.DEFAULT_TAIL_SPLIT_THRESHOLD));

      if (log.isInfoEnabled()) log.info(Options.TAIL_SPLIT_THRESHOLD + "=" + tailSplitThreshold);
    ...
    // scatterSplitEnabled
    {
      scatterSplitEnabled =
          Boolean.parseBoolean(
              properties.getProperty(
                  Options.SCATTER_SPLIT_ENABLED, Options.DEFAULT_SCATTER_SPLIT_ENABLED));

      if (log.isInfoEnabled()) log.info(Options.SCATTER_SPLIT_ENABLED + "=" + scatterSplitEnabled);
    }
    ...
    // joinsEnabled
    {
      joinsEnabled =
          Boolean.parseBoolean(
              properties.getProperty(Options.JOINS_ENABLED, Options.DEFAULT_JOINS_ENABLED));

      if (log.isInfoEnabled()) log.info(Options.JOINS_ENABLED + "=" + joinsEnabled);
    }
    ...
    // minimumActiveIndexPartitions
    {
      ...
      if (log.isInfoEnabled())
        log.info(Options.MINIMUM_ACTIVE_INDEX_PARTITIONS + "=" + minimumActiveIndexPartitions);
    ...
    // maximum moves
    {
      maximumMoves =
          Integer.parseInt(
              properties.getProperty(Options.MAXIMUM_MOVES, Options.DEFAULT_MAXIMUM_MOVES));

      if (log.isInfoEnabled()) log.info(Options.MAXIMUM_MOVES + "=" + maximumMoves);
    ...
    // maximum moves per target
    {
      maximumMovesPerTarget =
          Integer.parseInt(
              properties.getProperty(
                  Options.MAXIMUM_MOVES_PER_TARGET, Options.DEFAULT_MAXIMUM_MOVES_PER_TARGET));

      if (log.isInfoEnabled())
        log.info(Options.MAXIMUM_MOVES_PER_TARGET + "=" + maximumMovesPerTarget);
      ...
      if (maximumMovesPerTarget > maximumMoves) {
        ...
        throw new RuntimeException(
            Options.MAXIMUM_MOVES_PER_TARGET + " must be less than " + Options.MAXIMUM_MOVES);
      }
    }
    ...
    // movePercentOfSplitThreshold
    {
      ...
      if (log.isInfoEnabled())
        log.info(Options.MAXIMUM_MOVE_PERCENT_OF_SPLIT + "=" + maximumMovePercentOfSplit);
    ...
    // movePercentCpuTimeThreshold
    {
      ...
      if (log.isInfoEnabled())
        log.info(Options.MOVE_PERCENT_CPU_TIME_THRESHOLD + "=" + movePercentCpuTimeThreshold);

      if (movePercentCpuTimeThreshold < .0 || movePercentCpuTimeThreshold > 1.) {

        throw new RuntimeException(
            Options.MOVE_PERCENT_CPU_TIME_THRESHOLD + " must be in [0.0:1.0] ");
      }
    }
...

    {
      ...
      if (log.isInfoEnabled())
        log.info(
            Options.MAXIMUM_OPTIONAL_MERGES_PER_OVERFLOW + "=" + maximumOptionalMergesPerOverflow);

      if (maximumOptionalMergesPerOverflow < 0) {

        throw new RuntimeException(
            Options.MAXIMUM_OPTIONAL_MERGES_PER_OVERFLOW + " must be non-negative");
      }
    }

    {
      maximumJournalsPerView =
          Integer.parseInt(
              properties.getProperty(
                  Options.MAXIMUM_JOURNALS_PER_VIEW, Options.DEFAULT_MAXIMUM_JOURNALS_PER_VIEW));

      if (log.isInfoEnabled())
        log.info(Options.MAXIMUM_JOURNALS_PER_VIEW + "=" + maximumJournalsPerView);
...

    {
      maximumSegmentsPerView =
          Integer.parseInt(
              properties.getProperty(
                  Options.MAXIMUM_SEGMENTS_PER_VIEW, Options.DEFAULT_MAXIMUM_SEGMENTS_PER_VIEW));

      if (log.isInfoEnabled())
        log.info(Options.MAXIMUM_SEGMENTS_PER_VIEW + "=" + maximumSegmentsPerView);
    ...
    // maximumBuildSegmentBytes
    {
      ...
      if (log.isInfoEnabled())
        log.info(Options.MAXIMUM_BUILD_SEGMENT_BYTES + "=" + maximumBuildSegmentBytes);
    }
    ...
    // shutdownTimeout
    {
      shutdownTimeout =
          Long.parseLong(
              properties.getProperty(Options.SHUTDOWN_TIMEOUT, Options.DEFAULT_SHUTDOWN_TIMEOUT));
      ...
      if (log.isInfoEnabled()) log.info(Options.SHUTDOWN_TIMEOUT + "=" + shutdownTimeout);
    }
    ...
    // nominalShardSize
    {
      nominalShardSize =
          Long.parseLong(
              properties.getProperty(
                  Options.NOMINAL_SHARD_SIZE, Options.DEFAULT_NOMINAL_SHARD_SIZE));
      ...
      if (nominalShardSize < minShardSize) {
        ...
        throw new RuntimeException(
            "The '" + Options.NOMINAL_SHARD_SIZE + "' must be GTE " + minShardSize);
      }

      if (log.isInfoEnabled()) log.info(Options.NOMINAL_SHARD_SIZE + "=" + nominalShardSize);
    }
    ...
    if (overflowEnabled) {
      ...
      overflowService =
          Executors.newFixedThreadPool(
              ...
              new DaemonThreadFactory(
                  (serviceName == null ? "" : serviceName + "-") + "overflowService"));
      ...
      // buildService
      {
        buildServiceCorePoolSize =
            Integer.parseInt(
                properties.getProperty(
                    Options.BUILD_SERVICE_CORE_POOL_SIZE,
                    Options.DEFAULT_BUILD_SERVICE_CORE_POOL_SIZE));

        if (log.isInfoEnabled())
          log.info(Options.BUILD_SERVICE_CORE_POOL_SIZE + "=" + buildServiceCorePoolSize);
      }
      ...
      // mergeService
      {
        mergeServiceCorePoolSize =
            Integer.parseInt(
                properties.getProperty(
                    Options.MERGE_SERVICE_CORE_POOL_SIZE,
                    Options.DEFAULT_MERGE_SERVICE_CORE_POOL_SIZE));

        if (log.isInfoEnabled())
          log.info(Options.MERGE_SERVICE_CORE_POOL_SIZE + "=" + mergeServiceCorePoolSize);
      }
  ...
  public synchronized void shutdown() {
     ...
     */
    if (overflowService != null) overflowService.shutdownNow();
    //            {
    //
    //            /*
  ...
  public boolean shouldOverflow() {
    ...
    if (forceOverflow.get()) {
      ...
      if (log.isInfoEnabled()) log.info("Forcing overflow.");
    ...
    if (isTransient()) {
      ...
      if (log.isDebugEnabled()) log.debug("Overflow processing not allowed for transient journals");
    ...
    if (!isOverflowEnabled()) {
      ...
      if (log.isDebugEnabled()) log.debug("Overflow processing is disabled");
    ...
    if (!overflowAllowed.get()) {
      ...
      if (log.isInfoEnabled()) log.info("Asynchronous overflow still active");
   ...
   *
   * <p>Note: The caller is responsible for ensuring that this method is invoked with an exclusive
   * lock on the write service.
   *
   * <p>Preconditions:
   *
   * <ol>
   *   <li>Exclusive lock on the {@link WriteExecutorService}
   *   <li>{@link #isOverflowAllowed()}
   * </ol>
   *
   * <p>Post-conditions:
   *
   * <ol>
   *   <li>Overflowed onto new journal
   *   <li>{@link PostProcessOldJournal} task was submitted.
   *   <li>{@link #isOverflowAllowed()} was set <code>false</code> and will remain <code>false
   *       </code> until {@link PostProcessOldJournal}
   * </ol>
   *
   * @todo write unit test for an overflow edge case in which we attempt to perform an
   *     read-committed task on a pre-existing index immediately after an {@link #overflow()} and
   *     verify that a commit record exists on the new journal and that the read-committed task can
   *     read from the fused view of the new (empty) index on the new journal and the old index on
   *     the old journal.
   */
  public Future<Object> overflow() {
    ...
    final Event e =
        new Event(getFederation(), new EventResource(), EventType.SynchronousOverflow)
            .addDetail(
                "synchronousOverflowCounter", overflowCounters.synchronousOverflowCounter.get())
            .start();
    ...
    try {
      ...
      if (asyncOverflowEnabled.get()) {
        ...
        if (forceOverflow || overflowMetadata.postProcess) {
          ...
          return overflowService.submit(
              new AsynchronousOverflowTask((ResourceManager) this, overflowMetadata));
        }
   ...
   *
   * <p>Index partitions that have fewer than some threshold #of index entries will be copied onto
   * the new journal. Otherwise the view of the index will be re-defined to place writes on the new
   * journal and read historical data from the old journal.
   *
   * <p>This uses {@link #purgeOldResources()} to delete old resources from the local file system
   * that are no longer required as determined by {@link #setReleaseTime(long)} and {@link
   * #getEffectiveReleaseTime()}.
   *
   * <p>Note: This method does NOT start a {@link AsynchronousOverflowTask}.
   *
   * <p>Note: You MUST have an exclusive lock on the {@link WriteExecutorService} before you invoke
   * this method!
   *
   * @return Metadata about the overflow operation including whether or not asynchronous should be
   *     performed.
   */
  protected OverflowMetadata doSynchronousOverflow() {
     ...
     */
    final OverflowMetadata overflowMetadata = new OverflowMetadata((ResourceManager) this);
     ...
     */
    {
      ...
      if (log.isInfoEnabled()) log.info("New live journal: " + newJournal.getFile());
    }
     ...
     */
    {
      ...
      if (log.isInfoEnabled()) log.info("Closed old journal against further writes.");

      if (maximumJournalSizeAtOverflow < oldJournal.size()) {

        maximumJournalSizeAtOverflow = oldJournal.getBufferStrategy().getExtent();
      }
    }
    ...
    try {
      ...
      final CounterSet serviceRoot = getFederation().getServiceCounterSet();
      ...
      if (serviceRoot != null) {
        ...
        final CounterSet tmp =
            (CounterSet) serviceRoot.getPath(IDataServiceCounters.resourceManager);
    ...
    try {
      ...
      log.error("Problem purging old resources? service=" + getFederation().getServiceName(), t);
    }
  ...
  private void propagateIndexDecls(
      final AbstractJournal oldJournal,
    ...
    final long lastCommitTime = oldJournal.getRootBlockView().getLastCommitTime();
    {
      ...
      while (itr.hasNext()) {
         ...
         */
        {
          ...
          if (log.isInfoEnabled())
            log.info(
                "Re-defining view on new journal"
                    + ": name="
                    + bm.name
                    + ", copyIndex="
                    + copyIndex
          ...
          if (copyIndex) {
            ...
            if (log.isDebugEnabled())
              log.debug(
                  "Copying data to new journal: name="
                      + bm.name
                      + ", entryCount="
                      + entryCount
                      + ", threshold="
  ...
  /** Return some interesting performance counters for this service (local operations). */
   ...
   * defaults are chosen, but they could still trigger inappropriate behavior depending on the
   * thresholds set for move/split and if the host is selected as "highly utilized" by the LBS.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  public static class ResourceScores implements Serializable {
    ...
    ResourceScores(final OverflowManager overflowManager) {
      ...
      percentCPUTime =
          overflowManager.getHostCounter(
      ...
      dataDirBytesFree =
          overflowManager.getServiceCounter(
   ...
   * @param defaultValue The default value to use if the counter was not found.
   ...
   * @return The value if found and otherwise the defaultValue.
   */
  protected double getServiceCounter(final String path, final double defaultValue) {
