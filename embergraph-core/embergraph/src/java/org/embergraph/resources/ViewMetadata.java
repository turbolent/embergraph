 ...
 * BTree}. That is why there is a separation between the {@link BTreeMetadata} class and the {@link
 * ViewMetadata}. The latter will force any {@link IndexSegment} in the view to be (re-)opened.
 *
...
class ViewMetadata extends BTreeMetadata implements Params {
   ...
   * compact view since the size on disk of the compact segment can be much less. This is especially
   * true when many deleted tuples are purged by the compacting merge, in which case the segment
   ...
   * is invoked including {@link #getRangeCount()}, etc.
   */
   ...
   *     usage to make sure that we are invoking the correct method.
   */
   ...
   * @param view The view.
   */
  private synchronized void initView(final ILocalBTreeView view) {
     ...
     */
    {
      ...
      try {
        ...
        final IMetadataIndex mdi =
            resourceManager.getFederation().getMetadataIndex(indexMetadata.getName(), commitTime);
   ...
   * on disk of the compact segment can be much less. This is especially true when many deleted
   * tuples are purged by the compacting merge, in which case the segment could shrink to zero
   * during the merge.
   */
   ...
   * RMI. Those operations are done lazily in order to not impose their latency during synchronous
   * overflow.
   */
  public ViewMetadata(
