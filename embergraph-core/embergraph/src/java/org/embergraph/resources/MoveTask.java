 ...
 * been copied, this task then submits an {@link AtomicUpdateMoveIndexPartitionTask}. The atomic
 * update is an {@link ITx#UNISOLATED} operation. It is responsible copying any writes buffered for
 * the index partition on the live journal to the target {@link IDataService} and then updating the
 * {@link MetadataIndex}. Once the atomic update task is finished, clients will discover that the
 * source index partition does not exist. When they query the {@link MetadataService} they will
 * discover that the key(-range) is now handled by the new index partition on the target {@link
 * IDataService}.
 *
 * <p>Note: This task is run on the target {@link IDataService} and it copies the data from the
 * source {@link IDataService}. This allows us to use standard {@link IRangeQuery} operations to
 * copy the historical view. However, the {@link AtomicUpdateMoveIndexPartitionTask} is run on the
 * source {@link IDataService} since it needs to obtain an exclusive lock on the index partition
 * that is being moved in order to prevent concurrent writes during the atomic cutover. For the same
 * reason, the {@link AtomicUpdateMoveIndexPartitionTask} can not use standard {@link IRangeQuery}
 * operations. Instead, it initiates a series of data transfers while holding onto the exclusive
 * lock until the target {@link IDataService} has the current state of the index partition. At that
 * point it notifies the {@link IMetadataService} to perform the atomic cutover to the new index
 * partition.
 *
 * <p>Note: This task does NOT cause any resources associated with the current view of the index
 * partition to be released on the source {@link IDataService}. The reason is two-fold. First, the
 * {@link IndexSegment}(s) associated with that view MAY be in used by historical views. Second,
 * there MAY be historical commit points for the index partition on the live journal before the
 * atomic cutover to the new {@link IDataService} - those historical commit points MUST be preserved
 * until the release policy for those views has been satisfied.
 *
 * <p>Note: The MOVE task MUST be explicitly coordinated with the target {@link IDataService}.
 * Failure to coordinate the move results in an error message reported by the {@link
 * MetadataService} indicating that the wrong partition locator was found under the key. The cause
 * is a MOVE operation during which the target data service undergoes concurrent synchronous (and
 * then asynchronous) overflow. What happens is the {@link MoveTask} registers the new index
 * partition on the target data service. One registered on the {@link IDataService}, the index
 * partition it is visible during synchronous overflow BEFORE the MOVE is complete and BEFORE the
 * index is registered with the {@link MetadataService} and hence discoverable to clients. If the
 * target {@link IDataService} then undergoes synchronous and asynchronous overflow and chooses an
 * action which would change the index partition definition (split, join, or move) WHILE the index
 * partition is still being moved onto the target {@link IDataService} THEN the MOVE is not atomic
 * and the definition of the index partition in the {@link MetadataService} will not coherently
 * reflect either the MOVE or the action chosen by the target {@link IDataService}, depending on
 * which one makes its atomic update first.
 *
 * <p>The target {@link IDataService} MAY undergo both synchronous and asynchronous overflow as
 * {@link IDataService}s are designed to allow continued writes during those operations. Further, it
 * MAY choose to copy, build, or compact the index partition while it is being moved. However, it
 * MUST NOT choose any action (split, join, or move) that would change the index partition
 * definition until the move is complete (whether it ends in success or failure).
 *
 * <p>This issue is addressed by the following protocol:
 *
 * <ol>
 *   <li>The {@link MoveTask} set the <code>sourcePartitionId</code> on the {@link
 *       LocalPartitionMetadata} when it registers the index partition on the target {@link
 *       IDataService}. When <code>sourcePartitionId != -1</code>. the target {@link IDataService}
 *       is restricted to for that index partition to overflows actions which do not change the
 *       index partition definition (copy, build, or merge). Further, any index partition found on
 *       restart whose by the target {@link IDataService} whose <code>sourcePartitionId != -1</code>
 *       is deleted as it was never successfully put into play (this prevents partial moves from
 *       accumulating state which could not otherwise be released.)
 *   <li>The atomic update task causes the <code>sourcePartitionId</code> to be set to <code>-1
 *       </code> as one of its last actions, thereby allowing the target {@link IDataService} to use
 *       operations that could re-define the index partition (split, join, move) and also preventing
 *       the target index partition from being deleted on restart.
 * </ol>
 *
 * FIXME javadoc
 *
 * <p>Note: There are only two entry points: a simple move and a move where the compacting merge has
 * already been performed, e.g., by a split, and we just need to do the atomic update phase.
 *
...
public class MoveTask extends AbstractPrepareTask<MoveResult> {
  ...
  /** The summary used for the event description and the partition history record. */
   ...
   */
  public MoveTask(final ViewMetadata vmd, final UUID targetDataServiceUUID) {
    ...
    this.newPartitionId = resourceManager.nextPartitionId(vmd.indexMetadata.getName());

    this.targetIndexName =
        DataService.getIndexPartitionName(vmd.indexMetadata.getName(), newPartitionId);

    this.summary = OverflowActionEnum.Move + "(" + vmd.name + "->" + targetIndexName + ")";
   ...
   * target data service.
   *
   * @return A {@link MoveResult} describing the move operation (this is returned mainly for
   *     historical reasons).
   */
  @Override
  protected MoveResult doTask() throws Exception {
    ...
    try {
      ...
      try {
        ...
        if (INFO)
          log.info(
              "Generated compact index segment from historical view: "
                  + historicalWritesBuildResult);
      ...
      if (historicalWritesBuildResult != null) {
        ...
        resourceManager.deleteResource(
            historicalWritesBuildResult.segmentMetadata.getUUID(), false /* isJournal */);
      }
   ...
   * @param historicalWritesBuildResult An index segment containing all data for the source view as
   *     of the last commit time on the old journal. This index segment should be generated by a
   *     compacting merge or by an index partition split with the same semantics so that we will
   *     move the minimum amount of data.
   * @param targetDataServiceUUID The {@link UUID} of the target data service.
   * @param targetIndexPartitionId The partition identifier assigned to the target index partition.
   * @param parentEvent
   * @throws ExecutionException
   * @throws InterruptedException
   */
  protected static MoveResult doAtomicUpdate(
   ...
   * containing the historical view of the source index partition and generates and moves an index
   * segment containing any buffered writes on the live journal for the source index partition to
   * the target data service. Once the target index partition is registered on the target data
   * service and the {@link IMetadataService} has been updated to reflect the move, this task
   * updates the stale locator cache. At that point clients addressing tasks to the source index
   * partition will discover that it has been moved.
   *
   * <p>Note: If the operation fails, then it has no side-effects but the caller is responsible for
   * deleting the <i>historicalWritesBuildResult</i> iff that is deemed necessary (that is, if it is
   * not in use then either put it to use or delete it -- an attractive alternative is to
   * incorporate it into the source index partition view instead.)
   *
   * <p>Tasks executing after this one will discover that the source index partition no longer
   * exists as of the timestamp when this task commits. Clients that submit tasks for the source
   * index partition will be notified that it no longer exists. When the client queries the {@link
   * MetadataService} it will discover that the key range has been assigned to a new index partition
   * - the one on the target data service.
   *
   * <p>Note: This task runs as an {@link ITx#UNISOLATED} operation since it MUST have an exclusive
   * lock in order to ensure that the buffered writes are transferred to the target index partition
   * without allowing concurrent writes on the source index partition.
   *
   * <p>Note: I have placed the "receive" of the historical index partition view within the atomic
   * update task deliberately. It should add at most a few seconds to the execution time of that
   * task and makes it easier to write corrective actions for the atomic update since we can offer a
   * guarantees such that the existence of the target index partition on the target data service is
   * sufficient to determine that the entire operation was successful.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @todo optimization to NOT send an empty index segment if there are no buffered writes on the
   *     live journal.
   */
  protected static class AtomicUpdate extends AbstractAtomicUpdateTask<MoveResult> {
     ...
     *     as of the last commit time on the old journal. This index segment should be generated by
     *     a compacting merge or by an index partition split with the same semantics so that we will
     *     move the minimum amount of data.
     * @param targetDataServiceUUID The {@link UUID} of the target data service.
     * @param targetIndexPartitionId The partition identifier assigned to the target index
     *     partition.
     * @param parentEvent
     */
    protected AtomicUpdate(
      ...
      if (historicalWritesBuildResult == null) throw new IllegalArgumentException();
      ...
      //            try {
      //                this.thisInetAddr =
      // InetAddress.getByName(NicUtil.getIpAddress("default.nic", "default", false));
      //            } catch(Throwable t) {
    ...
    public MoveResult doTask() throws Exception {
      ...
      final Event e = parentEvent.newSubEvent(OverflowSubtaskEnum.AtomicUpdate).start();
      ...
      try {
        ...
        final String targetIndexName =
            DataService.getIndexPartitionName(scaleOutIndexName, targetIndexPartitionId);
        ...
        bufferedWritesBuildResult =
            resourceManager.buildIndexSegment(
                sourceIndexName,
                src,
                false /* compactingMerge */,
                sourceCommitTime,
                null /* fromKey */,
                null /* toKey */,
                parentEvent);

        {
          final IDataService targetDataService =
              resourceManager.getFederation().getDataService(targetDataServiceUUID);
           ...
           */
          {
            final Event receiveIndexPartitionEvent =
                parentEvent.newSubEvent(OverflowSubtaskEnum.ReceiveIndexPartition).start();
              ...
              targetDataService
                  .submit(
                      new ReceiveIndexPartitionTask(
                          ...
                          resourceManager.getResourceService().getAddr()))
                  .get();
        ...
        if (bufferedWritesBuildResult != null) {
           ...
           */
          resourceManager.deleteResource(
              bufferedWritesBuildResult.segmentMetadata.getUUID(), false /* isJournal */);
        }
     ...
     * outcome failed (the exception is logged as a warning and this method returns normally since
     * the move was in fact successful) to cases where it must rollback the change to the MDS (the
     * commit of the target index partition failed after it had updated the MDS - in this cases some
     * clients may temporarily see the locator for the target index partition, but they will
     * discover that the index partition does not exist), to cases where the move failed before the
     * MDS was updated (no compensating action is required).
     *
     * @throws Exception the caller's {@link Throwable}, wrapped iff necessary.
     */
    private void rollbackMove(
      ...
      try {
        ...
        final IndexMetadata tmp =
            targetDataService
                .submit(ITx.UNISOLATED, targetIndexName, new IsIndexRegistered_UsingWriteService())
                .get();
      ...
      try {
        ...
        final PartitionLocator current =
            resourceManager
                .getFederation()
                .getMetadataService()
                .get(scaleOutIndexName, ITx.UNISOLATED, oldLocator.getLeftSeparatorKey());

        final boolean mdsWasUpdated = current.getPartitionId() != oldLocator.getPartitionId();
        ...
        if (mdsWasUpdated) {
          ...
          try {
            ...
            resourceManager
                .getFederation()
                .getMetadataService()
                .moveIndexPartition(scaleOutIndexName, newLocator, oldLocator);
   ...
   * submits the {@link InnerReceiveIndexPartitionTask} once it is running on the target data
   * service.
   *
  ...
  protected static class ReceiveIndexPartitionTask extends DataServiceCallable<Void> {
     ...
     *     partition.
     * @param historyIndexSegmentMetadata Describes the {@link IndexSegmentStore} containing the
     *     historical data for the source index partition.
     * @param bufferedWritesIndexSegmentMetadata Describes the {@link IndexSegmentStore} containing
     *     the buffered writes from the live journal for the source index partition.
     * @param addr The {@link InetSocketAddress} of the {@link ResourceService} running on the
     *     source data service (the one from which the resources will be copied during the move).
     */
    ...
    public Void call() throws Exception {
       ...
       */
      final String targetIndexName =
          DataService.getIndexPartitionName(sourceIndexMetadata.getName(), targetIndexPartitionId);

      /*
       * Run the inner task on the write service of the target data
       * service.
       */
      final ResourceManager resourceManager = getDataService().getResourceManager();
      try {
        ...
        resourceManager.retentionSetRemove(historyIndexSegmentMetadata.getUUID());

        resourceManager.retentionSetRemove(bufferedWritesIndexSegmentMetadata.getUUID());
      }
    }
  }
   ...
   *
   * <p>The new index partition initially will have three sources in the view: the live journal on
   * the target data service, the buffered writes index segment from the source data service, and
   * the history index segment from source data service.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private static class InnerReceiveIndexPartitionTask extends AbstractTask<Void> {
     ...
     * @param bufferedWritesIndexSegmentMetadata Describes the {@link IndexSegmentStore} containing
     *     the buffered writes from the live journal for the source index partition.
     * @param addr The {@link InetSocketAddress} of the {@link ResourceService} of the source data
     *     service (the one from which the resources will be copied).
     */
    InnerReceiveIndexPartitionTask(
      ...
      super(resourceManager.getConcurrencyManager(), ITx.UNISOLATED, targetIndexName);

      if (sourceIndexMetadata == null) throw new IllegalArgumentException();
      if (sourceDataServiceUUID == null) throw new IllegalArgumentException();
      if (historyIndexSegmentMetadata == null) throw new IllegalArgumentException();
      if (bufferedWritesIndexSegmentMetadata == null) throw new IllegalArgumentException();
      ...
      this.sourceIndexName =
          DataService.getIndexPartitionName(scaleOutIndexName, sourceIndexPartitionId);
      ...
      this.summary = OverflowActionEnum.Move + "(" + sourceIndexName + "->" + targetIndexName + ")";

      this.parentEvent =
          new Event(
              resourceManager.getFederation(),
              new EventResource(sourceIndexMetadata.getName(), sourceIndexPartitionId),
              OverflowActionEnum.Move);
     ...
     *
     * <p>The caller still needs to delete the moved resources on their end, delete the source index
     * partition, and update the stale locator cache so that tasks in the write queue will be
     * redirected to the new index partition.
     *
     * <p>If this task throws an exception the caller MUST query the {@link IMetadataService} and
     * determine whether the source index partition or the target index partition is now registered
     * therein. If the target index partition is registered with the {@link IMetadataService}, then
     * the caller needs to rollback that change using a compensating action.
     *
     * @throws Exception
     */
    public Void doTask() throws Exception {
      ...
      try {
        ...
        targetBufferedWritesSegmentMetadata =
            receiveIndexSegmentStore(sourceBufferedWritesSegmentMetadata);

        final MoveResult moveResult =
            registerIndexPartition(
                targetHistorySegmentMetadata, targetBufferedWritesSegmentMetadata);
        ...
        if (targetHistorySegmentMetadata != null) {
          try {
            resourceManager.retentionSetRemove(targetHistorySegmentMetadata.getUUID());
            resourceManager.deleteResource(
                targetHistorySegmentMetadata.getUUID(), false /* isJournal */);
          } catch (Throwable t2) {
            // ignore
          }
        }
        ...
        if (targetBufferedWritesSegmentMetadata != null) {
          try {
            resourceManager.retentionSetRemove(targetBufferedWritesSegmentMetadata.getUUID());
            resourceManager.deleteResource(
                targetBufferedWritesSegmentMetadata.getUUID(), false /* isJournal */);
          } catch (Throwable t2) {
            // ignore
          }
        }
     ...
     *
     * <p>Note: Since the file (in this case an index segment) is being transferred from one data
     * service to another the dataDir, the target index partitionId, and the basename of the file
     * will all be different. This code make sure that the file winds up in the correct directory
     * for the scale-out index _partition_ to which it belongs.
     *
     * @param sourceSegmentMetadata The {@link SegmentMetadata} for the resource on the source data
     *     service.
     * @return The {@link SegmentMetadata} for the resource where it resides on this data service.
     */
    protected SegmentMetadata receiveIndexSegmentStore(final SegmentMetadata sourceSegmentMetadata)
        throws Exception {

      final Event e =
          parentEvent
              .newSubEvent(
                  OverflowSubtaskEnum.ReceiveIndexSegment, sourceSegmentMetadata.getParams())
              .start();

      try {

        if (sourceSegmentMetadata == null) throw new IllegalArgumentException();

        final long begin = System.currentTimeMillis();

        // name for the segFile on this data service.
        final File file =
            resourceManager.getIndexSegmentFile(
                scaleOutIndexName, sourceIndexMetadata.getIndexUUID(), targetIndexPartitionId);
        ...
        final SegmentMetadata targetSegmentMetadata =
            new SegmentMetadata(
                file, sourceSegmentMetadata.getUUID(), sourceSegmentMetadata.getCreateTime());

        try {

          // read the resource, writing onto that file.
          new ResourceService.ReadResourceTask(addr, sourceSegmentMetadata.getUUID(), file).call();
     ...
     *     historical writes up to the last commit point of the old journal for the source index
     *     partition.
     * @param bufferedWritesSegmentMetadata The metadata for the received index segment containing
     *     the buffered writes from the live journal for the source index partition.
     * @return {@link MoveResult} which contains the information we need to update the {@link
     *     IMetadataService} when the move is complete.
     */
    protected MoveResult registerIndexPartition(
      ...
      final Event e =
          parentEvent
              .newSubEvent(OverflowSubtaskEnum.RegisterIndex)
              .addDetail("targetIndexName", targetIndexName)
              .start();
     ...
     * service will not be notified that the index partition has been moved until this task
     * completes and the caller updates the stale locator cache and deletes the source index
     * partition.
     *
     * @throws ExecutionException
     * @throws InterruptedException
    ...
    protected void updateMetadataIndex(final MoveResult moveResult)
      ...
      resourceManager
          .getFederation()
          .getMetadataService()
          .moveIndexPartition(scaleOutIndexName, moveResult.oldLocator, moveResult.newLocator);
