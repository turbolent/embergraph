...
public class SplitUtility {
  ...
  /**
   * Dynamic sharding operations are not currently supported for indices with more than MAX_INT
   * tuples. It is not really a use case for scale-out, and that is what the SplitUtility is
   * designed to support. Various methods in this class therefore will cast to an int32 value.
   *
   * @param entryCount The entry count of some index.
   * @return The int32 entry count.
   * @throws UnsupportedOperationException if entryCount is GT MAX_INT.
   */
  private static int assertEntryCount(final long entryCount) {
   ...
   */
  public static void validateSplits(final IIndex src, final Split[] splits) {

    if (src == null) throw new IllegalArgumentException();

    if (splits == null) throw new IllegalArgumentException();

    final LocalPartitionMetadata pmd = src.getIndexMetadata().getPartitionMetadata();

    if (pmd == null) throw new IllegalArgumentException();

    validateSplits(pmd, splits, true /* checkFromToIndex */);
  }

  /**
   * Validate splits, including: that the separator keys are strictly ascending, that the separator
   * keys perfectly cover the source key range without overlap, that the rightSeparator for each
   * split is the leftSeparator for the prior split, that the fromIndex offsets are strictly
   * ascending, etc.
   *
   * @param originalPartitionMetadata The description of the key range of the index partition.
   * @param splits The recommended split points.
   * @param checkFromToIndex If the {@link Split#fromIndex}, {@link Split#toIndex} and {@link
   *     Split#ntuples} fields should be validated.
   * @throws IllegalArgumentException if any argument is <code>null</code>.
   */
  public static void validateSplits(
      final LocalPartitionMetadata originalPartitionMetadata,
      final Split[] splits,
      final boolean checkFromToIndex) {

    if (originalPartitionMetadata == null) throw new IllegalArgumentException();

    if (splits == null) throw new IllegalArgumentException("splits[] is null.");

    final int nsplits = splits.length;

    if (nsplits <= 1)
      throw new AssertionError("Expecting at least two splits, but found " + nsplits);
    ...
    for (int i = 0; i < nsplits; i++) {
      ...
      if (split.pmd == null) throw new AssertionError();

      if (!(split.pmd instanceof LocalPartitionMetadata)) throw new AssertionError();

      final LocalPartitionMetadata pmd = (LocalPartitionMetadata) split.pmd;

      // check the leftSeparator key.
      if (pmd.getLeftSeparatorKey() == null) throw new AssertionError();
      if (!BytesUtil.bytesEqual(fromKey, pmd.getLeftSeparatorKey())) throw new AssertionError();

      // verify rightSeparator is ordered after the left
      // separator.
      if (pmd.getRightSeparatorKey() != null) {
        if (BytesUtil.compareBytes(fromKey, pmd.getRightSeparatorKey()) >= 0)
    ...
    if (!BytesUtil.bytesEqual(
        originalPartitionMetadata.getLeftSeparatorKey(), splits[0].pmd.getLeftSeparatorKey())) {

      throw new AssertionError(
          "leftSeparator[0]"
              + ": expected="
              + BytesUtil.toString(originalPartitionMetadata.getLeftSeparatorKey())
              + ", actual="
        ...
        if (originalPartitionMetadata.getRightSeparatorKey() != null)
          throw new AssertionError(
              "rightSeparator for lastSplit: expected="
                  + BytesUtil.toString(originalPartitionMetadata.getRightSeparatorKey())
                  + ", actual=null");

      } else {

        // otherwise must compare as equals byte-by-byte.
        if (!rightSeparator.equals(originalPartitionMetadata.getRightSeparatorKey()))
          throw new AssertionError(
              "rightSeparator for lastSplit: expected="
                  + BytesUtil.toString(originalPartitionMetadata.getRightSeparatorKey())
                  + ", actual="
   ...
   *
   * <p>The split point is chosen by locating the right-most non-leaf node. The key range which
   * would enter that node is placed within the new right-sibling index partition (the tail). The
   * rest of the key range is placed within the new left-sibling index partition (the head).
   *
   * <p>
   *
   * @param btree The {@link BTree}.
   * @return The splits (split[0] is the head split, split[1] is the tail split).
   */
  public static Split[] tailSplit(final ResourceManager resourceManager, final BTree btree) {
    ...
    final byte[] separatorKey = leaf == null ? null : leaf.getKeys().get(0 /* index */);
    ...
    if (leaf == null || separatorKey == null) {
      ...
      throw new RuntimeException(
          "Could not locate separator key? Node="
              + node
              + ", nchildren="
              + node.getChildCount()
              + ", childIndex="
              ...
              + ", leaf="
              + leaf
              + (leaf == null ? "" : ("nkeys=" + leaf.getKeyCount() + ", keys=" + leaf.getKeys())));
    }
   ...
   *
   * <p>Note: This is done in parallel to minimize latency.
   *
   * <p>Note: The generated {@link IndexSegment}s are on the retentionSet and MUST be removed from
   * that set once it has been incorporated in a restart safe manner into an index partition view or
   * once the task fails.
   *
   * @see StoreManager#retentionSetAdd(java.util.UUID)
   * @throws InterruptedException
   * @throws ExecutionExceptions
   * @todo The operation could be serialized (or run with limited parallelism) in order to minimize
   *     the RAM burden for buffers during index segment creation. You can also limit the
   *     parallelism to some upper bound. During normal operations, the #of splits generated should
   *     be fairly small, e.g., N >= 2 and N ~ 2. This requires a thread pool (or delegate for a
   *     thread pool) that can impose a limit on the actual parallelism.
   */
  public static SplitResult buildSplits(
      final ViewMetadata vmd, final Split[] splits, final Event parentEvent)
    ...
    for (int i = 0; i < splits.length; i++) {
       ...
       * for the split.
       */
      final BuildIndexSegmentSplitTask task =
          new BuildIndexSegmentSplitTask(vmd, split, parentEvent);
    ...
    final List<Future<BuildResult>> futures =
        vmd.resourceManager.getConcurrencyManager().invokeAll(tasks);
    ...
    if (!causes.isEmpty()) {
      ...
      for (BuildResult result : buildResults) {
        ...
        // delete it.
        vmd.resourceManager.deleteResource(result.segmentMetadata.getUUID(), false /* isJournal */);
      }
    ...
    if (log.isInfoEnabled())
      log.info("Generated " + splits.length + " index segments: name=" + vmd.name);

    // form the split result.
    final SplitResult result = new SplitResult(vmd.name, vmd.indexMetadata, splits, buildResults);

    return result;
  }

  /**
   * Task used to build an {@link IndexSegment} from a restricted key-range of an index during a
   * {@link SplitIndexPartitionTask}. This is a compacting merge since we want as much of the data
   * for the index as possible in a single {@link IndexSegment}.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  protected static class BuildIndexSegmentSplitTask
    ...
    /**
     * Builds an {@link IndexSegment} from the lastCommitTime of the old journal.
     *
     * @param vmd
     * @param split
     */
    public BuildIndexSegmentSplitTask(
        final ViewMetadata vmd, final Split split, final Event parentEvent) {
    ...
    /**
     * Note: The generated {@link IndexSegment} is on the retentionSet and MUST be removed from that
     * set once it has been incorporated in a restart safe manner into an index partition view or
     * once the task fails.
     *
     * @see StoreManager#retentionSetAdd(java.util.UUID)
     */
    ...
    protected BuildResult doTask() throws Exception {
      ...
      final BuildResult result =
          resourceManager.buildIndexSegment(
              name, src, true /* compactingMerge */, vmd.commitTime, fromKey, toKey, parentEvent);
  ...
  /**
   * Choose a set of splits which may be reasonably expected to divide the {@link IndexSegment} into
   * extents each of which is approximately 50% full. The first split MUST use the leftSeparator of
   * the index view as its leftSeparator. The last split MUST use the rightSeparator of the index
   * view as its rightSeparator. The #of splits SHOULD be chosen such that the resulting index
   * partitions are approximately 50% full.
   *
   * @param keyRange The left and right separator keys for the view.
   * @param seg The {@link IndexSegment} containing most of the data for the view.
   * @param nominalShardSize The nominal size of an index partition (typically 200MB).
   * @param splitHandler Applies an application constraint to the choice of the separator key
   *     (optional).
   * @return A {@link Split}[] array contains everything that we need to define the new index
   *     partitions -or- <code>null</code> if a more detailed examination reveals that the index
   *     SHOULD NOT be split at this time. The returned array MUST containing at least two elements.
   *     If the {@link IndexSegment} CAN NOT be split, this MUST return <code>null</code> rather
   *     than array with a single element.
  ...
  public static Split[] getSplits(
    ...
    if (partitionIdFactory == null) throw new IllegalArgumentException();

    if (oldpmd == null) throw new IllegalArgumentException();

    if (seg == null) throw new IllegalArgumentException();

    if (nominalShardSize <= 0) throw new IllegalArgumentException();
    ...
    while (low < high) {
      ...
      if (remainingSplits == 1) {
        ...
        if (splitHandler != null) {
          ...
          final byte[] chosenKey =
              splitHandler.getSeparatorKey(seg, fromIndex, (high + 1) /* toIndex */, splitAt);
          ...
          if (chosenKey == null) {
            ...
            final double overextension = ((double) seg.getStore().size()) / nominalShardSize;
