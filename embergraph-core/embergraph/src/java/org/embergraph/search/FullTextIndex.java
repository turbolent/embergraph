 ...
 *
 * <p>The frequency distributions may be normalized to account for a variety of effects producing
 * "term weights". For example, normalizing for document length or relative frequency of a term in
 * the overall collection. Therefore the logical model is:
 *
 * <pre>
 *
 ...
 *
 * <p>The freq and weight are optional values that are representative of the kinds of statistical
 * data that are kept on a per-token-document basis. The freq is the token frequency (the frequency
 * of occurrence of the token in the document). The weight is generally a normalized token frequency
 * weight for the token in that document in the context of the overall collection.
 *
 * <p>In fact, we actually represent the data as follows:
 *
 * <pre>
 *
 ...
 * </pre>
 *
 * That is, there is a distinct entry in the full text B+Tree for each field in each document in
 * which a given token was recognized. The text of the token is not stored in the key, just the
 * Unicode sort key generated from the token text. The value associated with the B+Tree entry is
 * optional - it is simply not used unless we are storing statistics for the token-document pair.
 * The advantages of this approach are: (a) it reuses the existing B+Tree data structures
 * efficiently; (b) we are never faced with the possibility overflow when a token is used in a large
 * number of documents. The entries for the token will simply be spread across several leaves in the
 * B+Tree; (c) leading key compression makes the resulting B+Tree very efficient; and (d) in a
 * scale-out range partitioned index we can load balance the resulting index partitions by choosing
 * the partition based on an even token boundary.
 *
 * <p>A field is any pre-identified text container within a document. Field identifiers are
 * integers, so there are <code>32^2</code> distinct possible field identifiers. It is possible to
 * manage the field identifiers through a secondary index, but that has no direct bearing on the
 * structure of the full text index itself. Field identifies appear after the token in the key so
 * that queries may be expressed that will be matched against any field in the document. Likewise,
 * field identifiers occur before the document identifier in the key since we always search across
 * documents (in a search key, the document identifier is always {@link Long#MIN_VALUE} and the
 * field identifier is always {@link Integer#MIN_VALUE}). There are many applications for fields:
 * for example, distinct fields may be used for the title, abstract, and full text of a document or
 * for the CDATA section of each distinct element in documents corresponding to some DTD. The
 * application is responsible for recognizing the fields in the document and producing the
 * appropriate token stream, each of which must be tagged by the field.
 *
 * <p>A query is tokenized, producing a (possibly normalized) token-frequency vector. The relevance
 * of documents to the query is generally taken as the cosine between the query's and each
 * document's (possibly normalized) token-frequency vectors. The main effort of search is assembling
 * a token frequency vector for just those documents with which there is an overlap with the query.
 * This is done using a key range scan for each token in the query against the full text index.
 *
 * <pre>
 ...
 * </pre>
 *
 * and extracting the appropriate token frequency, normalized token weight, or other statistic. When
 * no value is associated with the entry we follow the convention of assuming a token frequency of
 * ONE (1) for each document in which the token appears.
 *
 * <p>Tokenization is informed by the language code (when declared) and by the configured {@link
 * Locale} for the database otherwise. An appropriate {@link Analyzer} is chosen based on the
 * language code or {@link Locale} and the "document" is broken into a token-frequency distribution
 * (alternatively a set of tokens). The same process is used to tokenize queries, and the API allows
 * the caller to specify the language code used to select the {@link Analyzer} to tokenize the
 * query.
 *
 * <p>Once the tokens are formed the language code / {@link Locale} used to produce the token is
 * discarded (it is not represented in the index). The reason for this is that we never utilize the
 * total ordering of the full text index, merely the manner in which it groups tokens that map onto
 * the same Unicode sort key together. Further, we use only a single Unicode collator configuration
 * regardless of the language family in which the token was originally expressed. Unlike the
 * collator used by the terms index (which often is set at IDENTICAL strength), the collector used
 * by the full text index should be chosen such that it makes relatively few distinctions in order
 * to increase recall (e.g., set at PRIMARY strength). Since a total order over the full text index
 * is not critical from the perspective of its IR application, the {@link Locale} for the collator
 * is likewise not critical and PRIMARY strength will produce significantly shorter Unicode sort
 * keys.
 *
 * <p>The term frequency within that literal is an optional property associated with each term
 * identifier, as is the computed weight for the token in the term.
 *
 * <p>Note: Documents should be tokenized using an {@link Analyzer} appropriate for their declared
 * language code (if any). However, once tokenized, the language code is discarded and we perform
 * search purely on the Unicode sort keys resulting from the extracted tokens.
 *
 * <h2>Scale-out</h2>
 *
 * <p>Because the first component in the key is the token, both updates (when indexing document) and
 * queries (reading against different tokens) will be scattered across shards. Therefore it is not
 * necessary to register a split handler for the full text index.
 *
 * @todo The key for the terms index is {term,docId,fieldId}. Since the data are not pre-aggregated
 *     by {docId,fieldId} we can not easily remove only those tuples corresponding to some document
 *     (or some field of some document).
 *     <p>In order to removal of the fields for a document we need to know either which fields were
 *     indexed for the document and the tokens found in those fields and then scatter the removal
 *     request (additional space requirements) or we need to flood a delete procedure across the
 *     terms index (expensive).
 * @todo provide M/R alternatives for indexing or computing/updating global weights.
 * @todo Consider model in which fields are declared and then a "Document" is indexed. This lets us
 *     encapsulate the "driver" for indexing. The "field" can be a String or a Reader, etc.
 *     <p>Note that lucene handles declaration of the data that will be stored for a field on a per
 *     Document basis {none, character offsets, character offsets + token positions}. There is also
 *     an option to store the term vector itself. Finally, there are options to store,
 *     compress+store, or not store the field value. You can also choose {None, IndexTokenized,
 *     IndexUntokenized} and an option dealing with norms.
 * @todo lucene {@link Analyzer}s may be problematic. For example, it is difficult to tokenize
 *     numbers. consider replacing the lucene analyzer/tokenizer with our own stuff. this might help
 *     with tokenization of numbers, etc. and with tokenization of native html or xml with intact
 *     offsets.
 * @todo lucene analyzers will strip stopwords by default. There should be a configuration option to
 *     strip out stopwords and another to enable stemming. how we do that should depend on the
 *     language family. Likewise, there should be support for language family specific stopword
...
public class FullTextIndex<V extends Comparable<V>> extends AbstractRelation {
  ...
  public IIndex getIndex() {
    ...
    if (ndx == null) {
      ...
      synchronized (this) {
        ...
        if (ndx == null) throw new IllegalStateException();
      }
    }
  ...
  public interface Options {
     ...
     * written on the index. When <code>false</code>, the existing value will be retained. This
     * option is an optimization which makes sense when the corpus (a) only grows; and (b) the
     * content of the documents in the corpus never changes. For example, this is true for an RDF
     * database since the set of terms only grows and each term is immutable.
     */
     ...
     *
     * <p>Note: {@link StrengthEnum#Primary} is generally what you want for a full text index as
     * search will consider tokens which differ in case and other subtle features to be the same
     * token (a 'match').
     *
     ...
     * timeout expires before all tasks complete then the search results will only reflect partial
     * information.
     */
  ...
  public FullTextIndex(
...

    {
      overwrite =
          Boolean.parseBoolean(
              properties.getProperty(Options.OVERWRITE, Options.DEFAULT_OVERWRITE));

      if (log.isInfoEnabled()) log.info(Options.OVERWRITE + "=" + overwrite);
    }

    {
      timeout =
          Long.parseLong(
              properties.getProperty(Options.INDEXER_TIMEOUT, Options.DEFAULT_INDEXER_TIMEOUT));

      if (log.isInfoEnabled()) log.info(Options.INDEXER_TIMEOUT + "=" + timeout);
    }
...

    {
      hitCacheSize =
          Integer.parseInt(
              properties.getProperty(Options.HIT_CACHE_SIZE, Options.DEFAULT_HIT_CACHE_SIZE));

      if (log.isInfoEnabled()) log.info(Options.HIT_CACHE_SIZE + "=" + hitCacheSize);
    }
      ...
      hitCacheTimeoutMillis =
          Long.parseLong(
              properties.getProperty(
                  Options.HIT_CACHE_TIMEOUT_MILLIS, Options.DEFAULT_HIT_CACHE_TIMEOUT_MILLIS));
...

    {
      final String className =
          getProperty(Options.ANALYZER_FACTORY_CLASS, Options.DEFAULT_ANALYZER_FACTORY_CLASS);

      if (log.isInfoEnabled()) log.info(Options.ANALYZER_FACTORY_CLASS + "=" + className);
  ...
  public void create() {
    ...
    indexMetadata.setTupleSerializer(
        new FullTextIndexTupleSerializer<V>(
            keyBuilderFactory,
   ...
   * then there will be an index write. Once the caller is done indexing, they MUST invoke {@link
   * TokenBuffer#flush()} to force any data remaining in their <i>buffer</i> to the indices.
   *
   * <p>Note: If a document is pre-existing, then the existing data for that document MUST be
   * removed unless you know that the fields to be found in the will not have changed (they may have
   * different contents, but the same fields exist in the old and new versions of the document).
   *
   * @param buffer Used to buffer writes onto the text index.
   * @param docId The document identifier.
   * @param fieldId The field identifier.
   * @param languageCode The language code -or- <code>null</code> to use the default {@link Locale}.
   * @param r A reader on the text to be indexed.
   * @param filterStopwords if true, filter stopwords from the token stream
   ...
   * indexed "documents". The cosine may be directly interpreted as the "relevance" of a "document"
   * to the query. The query and document term-frequency vectors are normalized, so the cosine
   * values are bounded in [0.0:1.0]. The higher the cosine the more relevant the document is to the
   * query. A cosine of less than .4 is rarely of any interest.
   *
   * <p>The implementation creates and runs a set parallel tasks, one for each distinct token found
   * in the query, and waits for those tasks to complete or for a timeout to occur. Each task uses a
   * key-range scan on the terms index, collecting metadata for the matching "documents" and
   * aggregating it on a "hit" for that document. Since the tasks run concurrently, there are
   * concurrent writers on the "hits". On a timeout, the remaining tasks are interrupted.
   *
   * <p>The collection of hits is scored and hits that fail a threshold are discarded. The remaining
   * hits are placed into a total order and the caller is returned an iterator which can read from
   * that order. If the operation is interrupted, then only those {@link IHit}s that have already
   * been computed will be returned.
   *
   * @param query The query (it will be parsed into tokens).
   * @param languageCode The language code that should be used when tokenizing the query -or- <code>
   *     null</code> to use the default {@link Locale} ).
   * @param minCosine The minimum cosine that will be returned.
   * @param maxCosine The maximum cosine that will be returned.
   * @param minRank The min rank of the search result.
   * @param maxRank The max rank of the search result.
   * @param prefixMatch When <code>true</code>, the matches will be on tokens which include the
   *     query tokens as a prefix. This includes exact matches as a special case when the prefix is
   *     the entire token, but it also allows longer matches. For example, <code>free</code> will be
   *     an exact match on <code>free</code> but a partial match on <code>freedom</code>. When
   *     <code>false</code>, only exact matches will be made.
   * @param matchAllTerms if true, return only hits that match all search terms
   * @param timeout The timeout -or- ZERO (0) for NO timeout (this is equivalent to using {@link
   *     Long#MAX_VALUE}).
   * @param unit The unit in which the timeout is expressed.
   * @return The hit list.
   * @todo Allow search within field(s). This will be a filter on the range iterator that is sent to
   *     the data service such that the search terms are visited only when they occur in the
   *     matching field(s).
   */
  ...
  public int count(final FullTextQuery query) {
    ...
    if (cache.containsKey(query)) {
      ...
      if (log.isInfoEnabled()) log.info("found hits in cache");

      return cache.get(query).length;

    } else {

      if (log.isInfoEnabled()) log.info("did not find hits in cache");
    }
  ...
  public Hit<V>[] _search(final FullTextQuery query) {
    ...
    if (minCosine < 0d || minCosine > 1d) throw new IllegalArgumentException();

    if (minRank <= 0 || maxRank <= 0) throw new IllegalArgumentException();

    if (minRank > maxRank) throw new IllegalArgumentException();
    ...
    if (cache.containsKey(cacheKey)) {
      ...
      if (a.length == 0) {
        ...
        log.warn(
            "No hits after matchAllTerms pruning: languageCode=["
                + languageCode
                + "], query=["
                + queryStr
                + "]");
      ...
      if (a.length == 0) {
        ...
        log.warn(
            "No hits after regex pruning: languageCode=["
                + languageCode
                + "], query=["
                + queryStr
                + "], regex=["
      ...
      if (log.isInfoEnabled()) log.info("Rank ordering " + a.length + " hits by relevance");
    ...
    if (log.isInfoEnabled()) log.info("Done: " + a.length + " hits in " + elapsed + "ms");
  ...
  protected Hit<V>[] slice(final FullTextQuery query, Hit<V>[] a) {
    ...
    if (log.isDebugEnabled()) log.debug("new max rank: " + newMax);
   ...
   * materialize the hits and check them for exact match.
   */
   ...
   * lexicon to materialize the hits and check them against the regex.
   */
