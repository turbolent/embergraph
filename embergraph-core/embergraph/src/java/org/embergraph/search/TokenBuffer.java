...
public class TokenBuffer<V extends Comparable<V>> {
   ...
   *     it will overflow. The buffer will NOT overflow until you exceed this capacity.
   * @param textIndexer The object on which the buffer will write when it overflows or is {@link
   *     #flush()}ed.
   */
   ...
   *
   * <p>Note: There is an assumption that the caller will process all tokens for a given field in
   * the same document at once. Failure to do this will lead to only part of the term-frequency
   * distribution for the field being captured by the indices.
   *
   ...
   * docId, fieldId}. This method will batch up and then apply a set of updates, but the total
   * operation is not atomic. Therefore search results which are concurrent with indexing may not
   * have access to the full data for concurrently indexed documents. This issue may be resolved by
   * allowing the indexer to write ahead and using a historical commit time for the search.
   *
  ...
  public void flush() {
    ...
    if (log.isInfoEnabled())
      log.info(
          "count=" + count + ", ndocs=" + ndocs + ", nfields=" + nfields + ", nterms=" + nterms);
    ...
    @SuppressWarnings("unchecked")
    final ITupleSerializer<ITermDocKey<V>, ITermDocVal> tupleSer =
        textIndexer.getIndex().getIndexMetadata().getTupleSerializer();
   ...
   */
  protected long writeOnIndex(final int n, final byte[][] keys, final byte[][] vals) {
   ...
   * @return The #of pre-existing records that were updated.
   */
  protected long deleteFromIndex(final int n, final byte[][] keys, final byte[][] vals) {
