 ...
 *
 * <p>The embergraph properties are used to map language ranges, as specified by <a
 * href="http://www.rfc-editor.org/rfc/rfc4647.txt">RFC 4647</a> to classes which extend {@link
 * Analyzer}. Supported classes included all the natural language specific classes from Lucene, and
 * also:
 *
 * <ul>
 ...
 * </ul>
 *
 * is usable. If the class has a static method named <code>getDefaultStopSet()</code> then this is
 * assumed to do what it says on the can; some of the Lucene analyzers store their default stop
 * words elsewhere, and such stopwords are usable by this class. If no stop word set can be found,
 * and there is a constructor without stopwords and a constructor with stopwords, then the former is
 * assumed to use a default stop word set.
 *
 * <p>Configuration is by means of the embergraph properties file. All relevant properties start
 * <code>org.embergraph.search.ConfigurableAnalyzerFactory</code> which we abbreviate to <code>
 * c.b.s.C</code> in this documentation. Properties from {@link Options} apply to the factory.
 *
 * <p>Other properties, from {@link AnalyzerOptions} start with <code>
 * c.b.s.C.analyzer.<em>language-range</em></code> where <code><em>language-range</em></code>
 * conforms with the extended language range construct from RFC 4647, section 2.2. There is an issue
 * that embergraph does not allow '*' in property names, and we use the character '_' to substitute
 * for '*' in extended language ranges in property names. These are used to specify an analyzer for
 * the given language range.
 *
 * <p>If no analyzer is specified for the language range <code>*</code> then the {@link
 * StandardAnalyzer} is used.
 *
 * <p>Given any specific language, then the analyzer matching the longest configured language range,
 * measured in number of subtags is returned by {@link #getAnalyzer(String, boolean)} In the event
 * of a tie, the alphabetically first language range is used. The algorithm to find a match is
 * "Extended Filtering" as defined in section 3.3.2 of RFC 4647.
 *
 * <p>Some useful analyzers are as follows:
 *
 * <dl>
 *   <dt>{@link KeywordAnalyzer}
 *   <dd>This treats every lexical value as a single search token
 *   <dt>{@link WhitespaceAnalyzer}
 *   <dd>This uses whitespace to tokenize
 *   <dt>{@link PatternAnalyzer}
 *   <dd>This uses a regular expression to tokenize
 *   <dt>{@link TermCompletionAnalyzer}
 *   <dd>This uses up to three regular expressions to specify multiple tokens for each word, to
 *       address term completion use cases.
 *   <dt>{@link EmptyAnalyzer}
 *   <dd>This suppresses the functionality, by treating every expression as a stop word.
 * </dl>
 *
 * there are in addition the language specific analyzers that are included by using the option
 * {@link Options#NATURAL_LANGUAGE_SUPPORT}
 *
...
public class ConfigurableAnalyzerFactory implements IAnalyzerFactory {
  ...
  public interface Options {
     ...
     * specification (with <code>c.b.s.C</code> expanding to <code>
     * org.embergraph.search.ConfigurableAnalyzerFactory</code>)
     *
     * <pre>
     ...
     * </pre>
     */
    String NATURAL_LANGUAGE_SUPPORT =
        ConfigurableAnalyzerFactory.class.getName() + ".naturalLanguageSupport";
  ...
  public interface AnalyzerOptions {
     ...
     * explicitly or implicitly, then {@link #LIKE} must be specified.
     */
     ...
     *
     * <p>{@link #LIKE} loops are not permitted.
     *
     * <p>If this is option is specified for a language range, then no other option is permitted.
     */
    String LIKE = "like";
     ...
     *
     * <dl>
     *   <dt>{@link #STOPWORDS_VALUE_NONE}
     *   <dd>This analyzer is used without stop words.
     *   <dt>{@link #STOPWORDS_VALUE_DEFAULT}
     *   <dd>Use the default setting for stopwords for this analyzer. It is an error to set this
     *       value on some analyzers such as {@link SimpleAnalyzer} that do not supprt stop words.
     *   <dt>A fully qualified class name
     *   <dd>... of a subclass of {@link Analyzer} which has a static method <code>
     *       getDefaultStopSet()</code>, in which case, the returned set of stop words is used.
     * </dl>
     *
     * If the {@link #ANALYZER_CLASS} does not support stop words then any value other than {@link
     * #STOPWORDS_VALUE_NONE} is an error. If the {@link #ANALYZER_CLASS} does support stop words
     * then the default value is {@link #STOPWORDS_VALUE_DEFAULT}
     */
   ...
   * NeedsConfiguringAnalyzerFactory#getAnalyzer(String, boolean)} does the configuration and
   * replaces itself here with a {@link ConfiguredAnalyzerFactory}
   */
