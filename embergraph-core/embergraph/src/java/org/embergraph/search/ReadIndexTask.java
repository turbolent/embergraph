 ...
 * "chunk" of results at a time. Those results are aggregated on the {@link Hit} collection, which
 * is maintained in a thread-safe hash map.
 *
 * <p>Note: An {@link ISimpleSplitHandler} imposes the constraint that index partitions may only
 * fall on a term boundary, hence all tuples for any given term will always be found on the same
 * index partition.
 *
...
public class ReadIndexTask<V extends Comparable<V>> extends AbstractIndexTask<V>
   ...
   * @param termNdx The index of this term within the overall search.
   * @param numTerms The overall number of search terms.
   * @param prefixMatch When <code>true</code> any term having <i>termText</i> as a prefix will be
   *     matched. Otherwise the term must be an exact match for the <i>termText</i>.
   * @param queryTermWeight The weight for the search term.
   * @param searchEngine The search engine.
   * @param hits The map where the hits are being aggregated.
   */
  public ReadIndexTask(
    ...
    if (hits == null) throw new IllegalArgumentException();
  ...
  private long run() {
    ...
    while (itr.hasNext()) {
      ...
      if (nhits % 1000 == 0 && Thread.interrupted()) {
        ...
        log.warn("Interrupted: queryTerm=" + queryTerm + ", nhits=" + nhits);
   ...
   * @throws Exception
   */
  private RuntimeException launderThrowable(final Throwable t) throws Exception {
    try {
      // log an error
      //    		log.error(t, t);
    } finally {
