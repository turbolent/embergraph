 ...
 * signatures, and our code needs to navigate each sort.
 *
...
class NeedsConfiguringAnalyzerFactory implements IAnalyzerFactory {
  ...
  private static class AnalyzerPair extends ConfiguredAnalyzerFactory.AnalyzerPair {
     ...
     *
     * @param range
     * @param cons A Constructor including a {@link java.util.Set} argument for the stop words.
     * @param params The arguments to pass to the constructor including a populated stopword set.
     * @throws Exception
     */
    AnalyzerPair(String range, Constructor<? extends Analyzer> cons, Object... params)
        throws Exception {
  ...
  private static class VersionSetAnalyzerPair extends AnalyzerPair {
    public VersionSetAnalyzerPair(ConfigOptionsToAnalyzer lro, Class<? extends Analyzer> cls)
        throws Exception {
      super(
          lro.languageRange,
          getConstructor(cls, Version.class, Set.class),
  ...
  private static class PatternAnalyzer extends AnalyzerPair {
    public PatternAnalyzer(ConfigOptionsToAnalyzer lro, Pattern pattern, CharArraySet stopWords)
      ...
      super(
          lro.languageRange,
          new PatternAnalyzerImpl(pattern, stopWords),
   ...
   *
   * <p>Instances of this class are only alive during the execution of {@link
   * NeedsConfiguringAnalyzerFactory#ConfigurableAnalyzerFactory(FullTextIndex)}, the life-cycle is:
   *
   * <ol>
  ...
  private static class ConfigOptionsToAnalyzer {
    ...
    public Set<?> getStopWords() {
      ...
      if (doNotUseStopWords()) return CharArraySet.EMPTY_SET;
    ...
    boolean doNotUseStopWords() {
      return AnalyzerOptions.STOPWORDS_VALUE_NONE.equals(stopwords)
          || (stopwords == null && pattern != null);
    }
    ...
    protected Set<?> getStopWordsForClass(String clazzName) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Failed to find stop words from " + clazzName + " for language range " + languageRange);
      }
    }

    protected boolean useDefaultStopWords() {
      return (stopwords == null && pattern == null)
    ...
    public void validate() {
        ...
        if (className != null && className != PatternTokenizer.class.getName()) {
          throw new RuntimeException(
              "Bad Option: Language range "
                  + languageRange
                  + " with pattern propety for class "
                  + className);
        }
      ...
      if (this.wordBoundary != null) {
        ...
        if (alwaysRemoveSoftHyphens != null && softHyphens == null) {
          throw new RuntimeException(
              "Bad option: Language range "
                  + languageRange
                  + ": must specify softHypens when setting alwaysRemoveSoftHyphens");
        }
      ...
      } else if (subWordBoundary != null
          || softHyphens != null
          || alwaysRemoveSoftHyphens != null
          || TermCompletionAnalyzer.class.getName().equals(className)) {
        throw new RuntimeException(
            "Bad option: Language range "
                + languageRange
                + ": must specify wordBoundary for TermCompletionAnalyzer");
      }
      ...
      if (PatternTokenizer.class.getName().equals(className) && pattern == null) {
        throw new RuntimeException(
            "Bad Option: Language range "
                + languageRange
                + " must specify pattern for PatternTokenizer.");
      }
      if ((like != null) == (className != null)) {
        throw new RuntimeException(
            "Bad Option: Language range "
                + languageRange
                + " must specify exactly one of implementation class or like.");
      }
      if (stopwords != null && like != null) {
        throw new RuntimeException(
            "Bad Option: Language range "
                + languageRange
                + " must not specify stopwords with like.");
      }
    }

    /** The third and final phase of the life-cyle used for identifying the AnalyzerPair. */
    private AnalyzerPair construct() throws Exception {
      ...
      if (softHyphens != null) {
        return new AnalyzerPair(
            ...
            new TermCompletionAnalyzer(
                wordBoundary, subWordBoundary, softHyphens, alwaysRemoveSoftHyphens));
      }
      if (wordBoundary != null) {
        return new AnalyzerPair(
            languageRange, new TermCompletionAnalyzer(wordBoundary, subWordBoundary));
      }
      ...
      if (hasConstructor(cls, CharArraySet.class)) {
        return new AnalyzerPair(
            languageRange,
            cls.getConstructor(CharArraySet.class)
                .newInstance(new CharArraySet(getStopWords(), false)),
            cls.getConstructor(CharArraySet.class)
                .newInstance(new CharArraySet(Collections.emptySet(), false)));
      }
      ...
      if (hasConstructor(cls, Version.class, Set.class)) {
        ...
        if (RussianAnalyzer.class.equals(cls)) {
          if (useDefaultStopWords()) {
            return new AnalyzerPair(
                languageRange, new RussianAnalyzer(), new RussianAnalyzer(CharArraySet.EMPTY_SET));
          }
      ...
      if (stopwords != null && !stopwords.equals(AnalyzerOptions.STOPWORDS_VALUE_NONE)) {
        throw new RuntimeException(
            "Bad option: language range: "
                + languageRange
                + " stopwords are not supported by "
                + className);
      }
      ...
      throw new RuntimeException(
          "Bad option: cannot find constructor for class "
              + className
              + " for language range "
              + languageRange);
    }
     ...
     * @return
     */
    AnalyzerPair followLikesToAnalyzerPair(
        int depth, int max, Map<String, ConfigOptionsToAnalyzer> analyzers) {
      if (result == null) {
        if (depth == max) {
          throw new RuntimeException(
              "Bad configuration: - 'like' loop for language range " + languageRange);
        }
        ConfigOptionsToAnalyzer next = analyzers.get(like);
        if (next == null) {
          throw new RuntimeException(
              "Bad option: - 'like' not found for language range "
                  + languageRange
                  + " (not found: '"
                  + like
                  + "')");
        ...
        result =
            new AnalyzerPair(
                languageRange, next.followLikesToAnalyzerPair(depth + 1, max, analyzers));
      }
      return result;
    }
    ...
    protected Class<? extends Analyzer> getAnalyzerClass(String className2) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Bad option: cannot find class " + className2 + " for language range " + languageRange,
            e);
      }
      return cls;
    }
  ...
  NeedsConfiguringAnalyzerFactory(
      ConfigurableAnalyzerFactory parent, final FullTextIndex<?> fullTextIndex) {
  ...
  protected static Constructor<? extends Analyzer> getConstructor(
      Class<? extends Analyzer> cls, Class<?>... parameterTypes) {
    try {
  ...
  private void properties2analyzers(
      Properties props, Map<String, ConfigOptionsToAnalyzer> analyzers) {
      ...
      if (prop.startsWith(Options.ANALYZER)) {
        ...
        if (languageRangeAndProperty.length == 2) {
          ...
          String languageRange =
              languageRangeAndProperty[0].toLowerCase(
                  Locale.US); // Turkish "I" could create a problem
          String shortProperty = languageRangeAndProperty[1];
          String value = props.getProperty(prop);
          log.info(
              "Setting language range: " + languageRange + "/" + shortProperty + " = " + value);
  ...
  protected Properties initProperties() {
    ...
    Properties myProps;
    if (Boolean.valueOf(
        parentProperties.getProperty(
            Options.NATURAL_LANGUAGE_SUPPORT, Options.DEFAULT_NATURAL_LANGUAGE_SUPPORT))) {
  ...
  private boolean hasPropertiesForStarLanguageRange(Properties from) {
    ...
    while (en.hasMoreElements()) {
      String prop = (String) en.nextElement();
      if (prop.startsWith(Options.ANALYZER + "_.") || prop.startsWith(Options.ANALYZER + "*.")) {
        return true;
      }
    }
    return false;
