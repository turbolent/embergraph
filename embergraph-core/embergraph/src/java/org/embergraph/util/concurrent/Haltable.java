 ...
 *
 * <p>This class embeds certain knowledge about which exceptions may be observed during normal
 * termination of asynchronous processes using I/O, thread pools, and {@link IBlockingBuffer}s. See
 * {@link #isNormalTerminationCause(Throwable)} for a list of the {@link Throwable} causes which are
 * treated as normal termination.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: AbstractHaltableProcess.java 2265 2009-10-26 12:51:06Z thompsonbry $
 */
public class Haltable<V> implements IHaltable<V> {
  ...
  /** Set to <code>true</code> iff the process was {@link #cancel(boolean) cancelled}. */
  ...
  @Override
  public final void halt(final V v) {
    lock.lock();
    try {
      if (halt) {
        // Already halted.
   ...
   *
   * <p>Note: External processes waiting on the {@link Future} interface should use {@link
   * #isDone()} which does not have the semantics of asserting that the process should still be
   * running.
   *
  ...
  public final V get() throws InterruptedException, ExecutionException {
    ...
    try {
      ...
      if (error) throw new ExecutionException(firstCause);
      return result;
    } finally {
      lock.unlock();
    }
  ...
  public final V get(final long timeout, final TimeUnit unit)
    ...
    if (lock.tryLock(remaining, TimeUnit.NANOSECONDS)) {
      try {
        ...
        while (!halt) {
          if (remaining > 0) remaining = halted.awaitNanos(remaining);
   ...
   *   <dd>The process was terminated by an interrupt. Interrupts are typically used to terminate
   *       asynchronous processes when their production limit has been satisfied or the consumer
   *       otherwise chooses to {@link IAsynchronousIterator#close()} the iterator through which
   *       they are consuming results from the process.
   *   <dt>{@link CancellationException}
   *   <dd>A process has been canceled using its {@link Future}.
   *   <dt>{@link ClosedByInterruptException}
   *   <dd>A process was interrupted during an IO operation.
   *   <dt>{@link BufferClosedException}
   *   <dd>The {@link IBlockingBuffer} on which the process was writing was asynchronously closed.
   *   <dt>{@link RejectedExecutionException}
   *   <dd>A process was not executed because the pool against which it was submitted had been
   *       shutdown (this of course implies that the work queue was unbounded as a bounded pool will
   *       throw this exception if the work queue is full).
   * </dl>
   *
   * @param cause The {@link Throwable}.
   * @return <code>true</code> if the {@link Throwable} indicates normal termination.
   * @see #getCause()
   */
   ...
   * trace for this since it is, in fact, an exception termination mode for a query.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/772">Query timeout only checked
   *     at operator start/stop. </a>
   */
  ...
  private void logCause(final boolean isFirstCause, final Throwable cause) {
    if (isFirstCause) {
      if (deadline) {
        ...
        log.error(this + " : isFirstCause=" + isFirstCause + " : " + cause, cause);
      }
      ...
      log.trace(this + " : isFirstCause=" + isFirstCause + " : " + cause, cause);
    }
  }
}
