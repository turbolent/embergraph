 ...
 *     ThreadPoolExecutorBaseStatisticsTask}, then this class, then a derived class for the {@link
...
public class ThreadPoolExecutorStatisticsTask implements Runnable {
  ...
  static final double scalingFactor = 1d / TimeUnit.NANOSECONDS.convert(1, TimeUnit.MILLISECONDS);
   ...
   */
  public ThreadPoolExecutorStatisticsTask(
      final String serviceName, final ThreadPoolExecutor service) {
   ...
   * @param serviceName The label for the service.
   * @param service The service to be monitored.
   * @param taskCounters The per-task counters used to compute the latency data for tasks run on
   *     that service.
   */
  public ThreadPoolExecutorStatisticsTask(
      final String serviceName, final ThreadPoolExecutor service, final TaskCounters taskCounters) {
   ...
   * @param taskCounters The per-task counters used to compute the latency data for tasks run on
   *     that service.
   * @param w The weight to be used by {@link #getMovingAverage(double, double, double)}
   */
  public ThreadPoolExecutorStatisticsTask(
    ...
    if (w <= 0d || w >= 1d) throw new IllegalArgumentException();
   ...
   * @param q The current value (e.g., the instantaneous measurement of the #of active tasks plus
   ...
   *
   * <p>Note: don't throw anything from here or it will cause the scheduled task executing this to
   * no longer be run!
   */
  public void run() {
    ...
    try {

      {
         ...
         */
        averageQueueLength = getMovingAverage(averageQueueLength, (activeCount + queueSize), w);
      }
      ...
      if (service instanceof WriteExecutorService) {
        ...
        final int activeCountWithLocksHeld =
            ((WriteExecutorService) service).getActiveTaskCountWithLocksHeld();

        averageActiveCountWithLocksHeld =
            getMovingAverage(averageActiveCountWithLocksHeld, activeCountWithLocksHeld, w);
      }
      ...
      if (taskCounters != null) {
        ...
        if (taskCount > 0) {
          ...
          if (service instanceof WriteExecutorService) {
            ...
            final long newValue = ((WriteTaskCounters) taskCounters).lockWaitingNanoTime.get();
        ...
        if (service instanceof WriteExecutorService) {
          ...
          if (groupCommitCount > 0) {
            ...
            {
              final long newValue = writeTaskCounters.commitWaitingNanoTime.get();
              ...
              averageCommitWaitingTime =
                  getMovingAverage(
                      averageCommitWaitingTime, (delta * scalingFactor / groupCommitCount), w);
            }

            // Time servicing the commit.
            {
              final long newValue = writeTaskCounters.commitServiceNanoTime.get();
              ...
              averageCommitServiceTime =
                  getMovingAverage(
                      averageCommitServiceTime, (delta * scalingFactor / groupCommitCount), w);
            }
          }

          // moving average of the size nready.
          averageReadyCount = getMovingAverage(averageReadyCount, tmp.getReadyCount(), w);

          // moving average of the size of the commit groups.
          averageCommitGroupSize =
              getMovingAverage(averageCommitGroupSize, tmp.getCommitGroupSize(), w);

          // moving average of the #of bytes written since the
          // previous commit.
          averageByteCountPerCommit =
              getMovingAverage(averageByteCountPerCommit, tmp.getByteCountPerCommit(), w);
   ...
   *
   * @param counterSet The counters will be added to this {@link CounterSet}.
  ...
  public CounterSet getCounters() {
     ...
     */
    {
      if (service != null) {
        ...
        counterSet.addCounter(
            IThreadPoolExecutorCounters.PoolSize,
            new Instrument<Integer>() {
              public void sample() {
        ...
        counterSet.addCounter(
            IThreadPoolExecutorCounters.LargestPoolSize,
            new Instrument<Integer>() {
              public void sample() {
    ...
    if (taskCounters != null) {
      ...
      {
        counterSet.addCounter(
            ITaskCounters.InterArrivalTime,
            new Instrument<Long>() {
              public void sample() {
                setValue(TimeUnit.NANOSECONDS.toMillis(taskCounters.interArrivalNanoTime.get()));
              }
            });

        counterSet.addCounter(
            ITaskCounters.QueueWaitingTime,
            new Instrument<Long>() {
              public void sample() {
                setValue(TimeUnit.NANOSECONDS.toMillis(taskCounters.queueWaitingNanoTime.get()));
              }
            });

        counterSet.addCounter(
            ITaskCounters.ServiceTime,
            new Instrument<Long>() {
              public void sample() {
                setValue(TimeUnit.NANOSECONDS.toMillis(taskCounters.serviceNanoTime.get()));
              }
            });

        counterSet.addCounter(
            ITaskCounters.CheckpointTime,
            new Instrument<Long>() {
              public void sample() {
                setValue(TimeUnit.NANOSECONDS.toMillis(taskCounters.checkpointNanoTime.get()));
              }
            });

        counterSet.addCounter(
            ITaskCounters.QueuingTime,
            new Instrument<Long>() {
              public void sample() {
                setValue(TimeUnit.NANOSECONDS.toMillis(taskCounters.queuingNanoTime.get()));
              }
            });
      }
      ...
      counterSet.addCounter(
          ...
          new Instrument<Double>() {
            @Override
            protected void sample() {
              final double t = interArrivalNanoTimeTask.getMovingAverage() * scalingFactor;
              if (t != 0d) setValue(1d / t);
            }
          });
      ...
      counterSet.addCounter(
          ...
          new Instrument<Double>() {
            @Override
            protected void sample() {
              final double t = serviceNanoTimeTask.getMovingAverage() * scalingFactor;
              if (t != 0d) setValue(1d / t);
            }
          });
    ...
    if (service instanceof WriteExecutorService) {
      ...
      counterSet.addCounter(
          IWriteServiceExecutorCounters.CommitCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      counterSet.addCounter(
          IWriteServiceExecutorCounters.AbortCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      counterSet.addCounter(
          IWriteServiceExecutorCounters.OverflowCount,
          new Instrument<Long>() {
            public void sample() {
      ...
      counterSet.addCounter(
          IWriteServiceExecutorCounters.MaxCommitServiceTime,
          new Instrument<Long>() {
            public void sample() {
      ...
      counterSet.addCounter(
          IWriteServiceExecutorCounters.MaxCommitGroupSize,
          new Instrument<Long>() {
            public void sample() {
      ...
      counterSet.addCounter(
          IWriteServiceExecutorCounters.MaxRunning,
          new Instrument<Long>() {
            public void sample() {
