...
public class MovingAverageTask implements Runnable {
   ...
   */
  public MovingAverageTask(final String name, final Callable<? extends Number> sampleTask) {
   ...
   *
   * @param name The label for log messages.
   * @param queue Task that returns a sampled value.
   * @param w The weight to be used by {@link #getMovingAverage(double, double, double)}
   */
  public MovingAverageTask(
      final String name, final Callable<? extends Number> sampleTask, final double w) {

    if (name == null) throw new IllegalArgumentException();

    if (sampleTask == null) throw new IllegalArgumentException();
