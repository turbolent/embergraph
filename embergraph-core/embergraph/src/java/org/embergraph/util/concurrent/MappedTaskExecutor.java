 ...
 * @version $Id$
 * @deprecated by {@link LatchedExecutorService} which has higher throughput and plays more nicely
 *     with the {@link Executor} paradigm.
 */
public class MappedTaskExecutor {
   ...
   * purpose specific thread pool of a given worker thread capacity, but there is overhead
   * associated with creating the thread pool and its worker threads.
   *
   * @param maxParallel The maximum parallelism. This is ONE (1) to run the tasks sequentially. If
   *     the value is 2 or greater, then the tasks will be executed with limited parallelism. You
   *     may specify {@link Integer#MAX_VALUE} to indicate that this class should not impose any
   *     constraint on their parallel execution.
   * @param tasks The tasks to be executed.
   * @return The list of value returned by the {@link Future}(s) of the tasks IFF all tasks were
   *     executed successfully.
   * @throws TimeoutException
   * @throws ExecutionException
   * @throws InterruptedException
   */
  public <T> List<T> runTasks(
      final List<Callable<T>> tasks, final long timeout, final TimeUnit unit, final int maxParallel)
    ...
    if (maxParallel <= 0) throw new IllegalArgumentException();
  ...
  private <T> List<T> runOne(final Callable<T> task, final long timeout, final TimeUnit unit)
      throws InterruptedException, ExecutionException, TimeoutException {
  ...
  private <T> List<T> runSequence(final List<Callable<T>> tasks, long timeout, final TimeUnit unit)
      throws InterruptedException, ExecutionException, TimeoutException {
  ...
  private <T> List<T> runMaxParallel(
      final List<Callable<T>> tasks, final long timeout, final TimeUnit unit)
    ...
    final List<Future<T>> futures = getThreadPool().invokeAll(tasks, timeout, unit);
  ...
  private <T> List<T> runLimitedParallel(
      final List<Callable<T>> tasks, long timeout, final TimeUnit unit, final int maxParallel)
    ...
    while (titr.hasNext()) {
      ...
      final List<Future<T>> futures =
          getThreadPool().invokeAll(taskChunk, nanos, TimeUnit.NANOSECONDS);
