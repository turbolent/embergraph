 ...
 * counter after that operation. This pattern may be safely used in combination with nested
 * invocations and with concurrent threads.
 *
 * <p>Note: This class is very similar to a {@link CountDownLatch}, however the counter maximum is
...
public class Latch {
  ...
  public long inc() {
    ...
    try {
      ...
      if (c <= 0) throw new IllegalStateException(toString());
   ...
   * @throws IllegalStateException if the counter would become negative.
   */
  public long dec() {
  ...
  private final void _signal() { // throws InterruptedException {
    ...
    if (log.isInfoEnabled()) log.info("signalAll()");
   ...
   *
   * <p>CAUTION: DO NOT invoke any operation from within this method which could block as that would
   * cause the thread running the asynchronous write task in which this method is invoked to block.
   * If you are transferring objects to a queue, the queue MUST be unbounded.
   */
   ...
   * @return <code>true</code> if the counter reached zero and <code>false</code> if the timeout was
   *     exceeded before the counter reached zero.
   * @throws InterruptedException
   */
    ...
    if (lock.tryLock(remaining, TimeUnit.NANOSECONDS)) {
      ...
      try {
        ...
        while ((c = counter.get()) != 0) {
          if (c < 0) throw new IllegalStateException(toString());
          if (remaining > 0) remaining = cond.awaitNanos(remaining);
          else return false;
        }
