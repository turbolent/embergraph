...
public interface IHaltable<V> extends Future<V> {
   ...
   * termination.
   *
   * <p>{@link IHaltable} considers exceptions triggered by an interrupt to be normal termination of
   * the process and will return <code>null</code> for such exceptions.
   *
   * @return The first {@link Throwable} which caused this process to halt and <code>null</code> if
   *     the process has not halted or if it halted through normal termination.
   */
  Throwable getCause();
