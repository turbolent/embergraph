 ...
 * the {@link #cache} is unbounded, it will just take on more and more memory as new results are
 * computed. This could be handled by imposing a capacity constraint on the cache, by the use of
 * {@link WeakReference}s if they can be made to stand for the specific computation to be performed,
 * or by the use of a timeout on the cache entries.
 *
  ...
  public V compute(final A arg) throws InterruptedException {
    while (true) {
       ...
       * Wrap the exception to indicate whether or not the interrupt
         ...
         */
        final InterruptedException e2 = new InterruptedException("Interrupted: willRun=" + willRun);
        e2.initCause(e);
        throw e2;
