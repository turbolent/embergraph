 ...
 * are run.
 *
 * <p>Note: The field names here are consistent with those in the {@link
 * ThreadPoolExecutorStatisticsTask}.
 *
 * <p>Note: The various counters are {@link AtomicLong}s since we need them to be thread-safe. (
 * <code>counter++</code> and <code>counter+=foo</code> are NOT guaranteed to be thread-safe for
 * simple fields or even <code>volatile</code> fields).
 *
...
public class TaskCounters {
   ...
   * the actual elapsed time during which those tasks were executed.
   *
   * <p>Note: Service time on the client includes queuing time on the service and the checkpoint
   * time (for write tasks).
   */
   ...
   * the actual elapsed time during which those tasks were executed.
   *
   * <p>Note: This time is already reported by the {@link #serviceNanoTime} but is broken out here
   * for additional detail. Checkpoint time can be most of the service time for a task since indices
   * buffer writes and but are required to flush those writes to the backing during during a
   * checkpoint.
   */
   ...
   * they are complete.
   *
   * <p>Note: Queuing time on the client includes queuing time on the service.
   */
  ...
  public CounterSet getCounters() {
    ...
    counterSet.addCounter(
        ITaskCounters.TaskCompleteCount,
        new Instrument<Long>() {
          public void sample() {
    ...
    counterSet.addCounter(
        ITaskCounters.TaskSuccessCount,
        new Instrument<Long>() {
          public void sample() {
    ...
    counterSet.addCounter(
        ...
        new Instrument<Long>() {
          public void sample() {
            setValue(TimeUnit.NANOSECONDS.toMillis(interArrivalNanoTime.get()));
          }
        });
    ...
    counterSet.addCounter(
        ...
        new Instrument<Long>() {
          public void sample() {
            setValue(TimeUnit.NANOSECONDS.toMillis(queueWaitingNanoTime.get()));
          }
        });
    ...
    counterSet.addCounter(
        ...
        new Instrument<Long>() {
          public void sample() {
            setValue(TimeUnit.NANOSECONDS.toMillis(serviceNanoTime.get()));
          }
        });
    ...
    counterSet.addCounter(
        ...
        new Instrument<Long>() {
          public void sample() {
            setValue(TimeUnit.NANOSECONDS.toMillis(checkpointNanoTime.get()));
          }
        });
    ...
    counterSet.addCounter(
        ...
        new Instrument<Long>() {
          public void sample() {
            setValue(TimeUnit.NANOSECONDS.toMillis(queuingNanoTime.get()));
          }
        });
