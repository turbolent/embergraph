...
public abstract class CanonicalFactory<KEY, VAL, STATE> {
   ...
   *
   * @param key The key.
   * @param state Additional state from the caller which will be passed through to {@link
   *     #newInstance(Object, Object)} when creating a new instance (optional).
   ...
   * resource is destroyed it may be necessary to discard the cache entry to avoid inappropriate
   * carry over via the cache if the resource is then recreated.
   *
   ...
   *
   * @param key The key.
   * @param state Additional state used to initialize the new instance (optional).
