...
public interface IQueueCounters extends ICounterHierarchy {
  ...
  interface ITaskCounters {
     ...
     * its work).
     *
     * <p>Note: For tasks which acquire resource lock(s), this does NOT include the time waiting to
     * acquire the resource lock(s).
     */
     ...
     * indices.
     *
     * <p>Note: This time is already reported by the {@link #ServiceTime} but is broken out here for
     * additional detail. Checkpoint time can be most of the service time for a task since indices
  ...
  interface IThreadPoolExecutorCounters extends IQueueCounters {
     ...
     * run and those that are currently running in the definition of the "queue length".
     */
   ...
   * AbstractTask}s.
   *
   * <p>Note: The {@link ConcurrencyManager} and {@link AbstractTask} work together to maintain the
   * per-service {@link TaskCounters}s on which these additional counters are based.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  interface IThreadPoolExecutorTaskCounters extends IQueueCounters {
     ...
     * acquire the resource lock(s).
     *
     * <p>Note: The average service rate is the inverse of the {@link #AverageServiceTime}.
     *
     ...
     * pages to the backing store and then writing a new {@link Checkpoint} record on the backing
     * store. This operation is an after action of a successful write task.
     *
  ...
  interface IWriteServiceExecutorCounters extends IThreadPoolExecutorTaskCounters {
     ...
     * unisolated).
     *
     * <p>Note: This DOES NOT imply that this many bytes were written in the commit itself. Indices
     * are checkpointed after each task. All writes on the journal should have already occurred
     * before the group commit. The commit protocol itself involves writing a very few bytes and
     * then syncing the disk.
     */
     ...
     * handling index partition builds, splits, joins, moves, etc.
     */
