 ...
 * <p>
 *
 * @todo Consider requiring all default parameters to be made explicit during the constructor /
 *     parse of the MIMEType. It might greatly simplify the logic if we treated the "default" as a
 *     syntactic sugar that did NOT survive a round trip. Or survived by virtue of being marked with
 *     a 'defaultValue' flag on a per-parameter-type (or per-parameter-instance) basis by a
 *     subclass.
 *     <p>
 * @todo IF MIME is constrained to not have duplicate parameter names then (a) we should check for
 *     that and throw an error when duplicates exist; and (b) we can change the {@link #getParams()}
 *     return value to an (immutable) {@link Map}. This sort of logic already partly exists in
 *     {@link #getIntersection( MIMEType other, String[] dropParams )}.
 * @todo Consider migrating the NVPair[] to a Vector of NVPair in order to provide for a mutable
 *     implementation of {@link MIMEType}, in which case we will also need a wrapper that provides
 *     an immutable view. However, I don't necessarily want to expose a mutable iterface as fat as
 *     {@link Vector}, so this may not be a good idea. See {@link AcceptMIMEType}.
 * @todo Add support for mutable parameters, including addParam( String name, String value ),
 *     removeParam( String name ), and setParam( String name, String value ). If duplicate parameter
 *     names are not permitted by MIME, then perhaps we can simplify this interface. Derive an
 *     ImmutableMIMEType class for people who don't want their MIMEType expressions to be changable.
 */
public class MIMEType {
   ...
   *
   * @return The default implementation always returns an empty {@link String}[] since it does not
   *     recognize any default MIME type parameters. Implementors that subclass {@link MIMEType}
   *     MUST extend this method if the subclass recognizes any default MIME type parameters.
   */
   ...
   *
   * @return The default implementation always returns <code>false</code>. Implementors that
   *     subclass {@link MIMEType} MUST extend this method if the subclass contains any parameters
   *     that should be ignored by the <code>spans</code> logic.
   */
   ...
   * <p>
   *
   * @param type The media type or "*" (if the <i>subtype</i> if also "*").
   * @param subtype The media subtype or "*".
   * @param params An array of name-value pairs specifying the MIME attributes.
   * @exception IllegalArgumentException If the method is unable to validate the syntax of the type,
   *     subtype and params. Everything must be an HTTP <code>token</code> except the attribute
   *     value, which is interchanged as an HTTP <code>quoted-string</code> any may contain pretty
   *     much any character sequence. Also thrown if <i>params</i> is <code>null</code>.
   */
  public MIMEType(String type, String subtype, NVPair[] params) throws IllegalArgumentException {
    ...
    if (!HTTPHeaderUtility.isHttpToken(subtype)) {
      ...
      throw new IllegalArgumentException(
          "MIME subtype is not an HTTP token" + " : '" + subtype + "'");
    }
    ...
    for (NVPair param : params) {
      ...
      if (!HTTPHeaderUtility.isHttpToken(attribute)) {
        ...
        throw new IllegalArgumentException(
            "MIME attribute is not an HTTP token" + " : '" + attribute + "'");
      }
    }
  }
   ...
   *
   * <p>From the HTTP/1.1 specification: The type, subtype, and parameter attribute names are
   * case-insensitive. Parameter values might or might not be casesensitive, depending on the
   * semantics of the parameter name. Linear white space (LWS) MUST NOT be used between the type and
   * subtype, nor between an attribute and its value.
   *
   * <p>
   *
   * @exception IllegalArgumentException Indicates that the string could not be parsed as a valid
   *     MIME expression.
   */
  public MIMEType(String s) throws IllegalArgumentException {
    ...
    log.debug("subtype = '" + m_subtype + "'");

    if (m_type.equals("*") && !m_subtype.equals("*")) {

      throw new IllegalArgumentException(
          "The mime type wildcard is '*/*'" + ", not " + m_type + "/" + m_subtype);
    }
    ...
    log.debug("input length = " + parameters.length());
    ...
    while (m2.find()) {
      ...
      log.debug("parameter : " + attribute + "=" + value);
   ...
   * parameters.
   *
   * <p>Note: There is no LWS (linear whitespace) between the type and the subtype for a MIME
   * string.
   *
   * @see #toString()
   */
   ...
   * <strong>case-insensitive</strong> comparison.
   *
   * <p>Note: The MIME parameter values are compared using a case-sensitive comparison, which is not
   * correct for all MIME types since some explictly provide for case-insensitive semantics for
   * their parameter values. Implementors are free to further specialize this method in derived
   * subclasses to obtain MIME type specific comparision of MIME type parameter values or to
   * establish default parameter values.
   *
   * <p>Note: Defaults may be provided for MIME parameters through subclasses by extending {@link
   * #sameParamValue ( String name, String realValue, String otherValue )}, {@link
   * #getDefaultParamNames()}, and {@link #isIgnoredParamName( String name )}.
   *
   * <p>
   */
   ...
   *
   * <p>The default implementation uses a case-sensitive comparison of the MIME parameter value.
   * Implementors are free to created subclasses that extend this method to incorporate MIME type
   * specific logic in their comparison.
   *
   * <p>
   *
   * @param name The name of the parameter whose value is being compared. This parameter is ignored
   *     by this implementation but is provided for subclasses that may have type specific value
   *     comparison logic and/or parameter value default logic.
   * @param realValue The value of the parameter on this {@link MIMEType} object as discovered,
   *     e.g., by {@link #getParamValue( String name )}. This may explicitly be <code>null</code>.
   *     Subclasses that have a default value for this named parameter, e.g., {@link AcceptMIMEType}
   *     which has a default value for the "q" parameter, MUST substitute their default value when
   *     <i>realValue</i> is <code>null</code>.
   * @param otberValue The value that is being compared to the <i>foundValue</i>. This parameter MAY
   *     be <code>null</code>, in which case a subclass MAY choose to declare a default value to be
   *     used in this comparison.
   * @todo Consider defining this as spansParamValue so that it can be used to test constraints that
   *     define a range of values rather than just a point value.
   */
  public boolean sameParamValue(String name, String realValue, String otherValue)
   ...
   * NOT a fully determined media type, then the server MUST make it into a concrete media type.
   *
   * @param other The other media-range whose intersection with this media-range will be return.
   * @param dropParams An array of zero or more parameter names that will be dropped out of the
   *     intersected media-ranges. For example, this is used to drop the "q" parameter during
   *     content negotiation when computing the negotiated joint media-range.
   * @return The intersection of the two media-ranges or <code>null</code> if the intersection does
   *     not exist. For example, if the two media-ranges each specify different concrete subtypes,
   *     such as "text/xml" vs "text/plain".
   */
  public MIMEType getIntersection(MIMEType other, String[] dropParams) {
    ...
    if (true) { // local variables.
      ...
      Iterator itr = Arrays.asList(getParams()).iterator();
    ...
    if (true) { // local variables.
      ...
      Iterator itr = Arrays.asList(other.getParams()).iterator();
      ...
      while (itr.hasNext()) {
        ...
        if (!dropThisParam) {
          ...
          NVPair existingParam = (NVPair) params.get(name);
          ...
          if (existingParam != null
              && !sameParamValue(name, existingParam.getValue(), param.getValue())) {
   ...
   *
   * @param compareParams IFF true the MIME type parameters are also compared by this method. The
   *     specific parameter values are compared using {@link #sameParamValue( String name, String
  ...
  public boolean spans(MIMEType other, boolean compareParams) {
    ...
    if (getType().equalsIgnoreCase(other.getType())
        && (getSubtype().equalsIgnoreCase(other.getSubtype()) || isSubtypeWildcard())) {
   ...
   *     specific parameter values are compared using {@link #sameParamValue( String name, String
   *     realValue, String otherValue )}.
   *     <p>Note: This is a convenience for <code>spans( other, false
   * );</code>.
   ...
   *
   * @todo This returns the backing array. Should this interface be changed to increase the
   *     encapsulation? We MUST change this implementation if we want to enforce an immutable
   *     interface.
   */
   ...
   * <p>
   *
   * @param name The name of a MIME parameter, which is case-insensitive. Implementors can create
   *     subclasses if a specific MIME type declares some case-sensitive MIME type parameter names.
   *     <p>
   */
   ...
   *
   * <p>Note: There is no LWS (linear whitespace) between the type and the subtype for a MIME
   * string. Also, there is no LWS between the attribute name and the attribute value, e.g.,
   * "charset=xxx" but not "charset = xxx".
   */
