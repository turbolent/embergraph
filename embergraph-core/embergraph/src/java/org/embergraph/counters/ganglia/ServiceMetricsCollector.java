...
public class ServiceMetricsCollector implements IGangliaMetricsCollector {
  ...
  public ServiceMetricsCollector(
      final AbstractStatisticsCollector statisticsCollector, final Pattern filter) {
  ...
  public void collect(final IGangliaMetricsReporter reporter) {
     ...
     */
    final String pathPrefix = basePrefix + statisticsCollector.getProcessName();

    final CounterSet counters = (CounterSet) statisticsCollector.getCounters().getPath(pathPrefix);
