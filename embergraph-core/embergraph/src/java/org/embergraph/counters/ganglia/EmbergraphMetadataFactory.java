 ...
 * metrics. This is illustrated below.
 *
 * <pre>
 ...
 * </pre>
 *
 * In order to map this information onto ganglia, the host and service names must be identified in
 * the counter path. This allows us to recognize the local name of the metric, which may itself
 * include some path components. For example, <code>Memory/Resident Set Size</code> is the local
 * name of the metric whose value is the resident set size of the service process. Some metrics
 * appear in both the per-host and per-service sections of the counter set hierarchy. For example,
 * <code>CPU/% Processor Time</code> is reported both for the host and for the service (when using
 * <code>pidstat</code> to monitor the service).
 *
 * <p>For the per-host metrics, embergraph and ganglia sometimes collect what is essentially the
 * same metric. For example, <code>CPU/% User Time</code> is essentially the same as <code>cpu_user
 * </code>. However, sometimes the units in which the metric is collected differ. For example,
 * embergraph collects <code>Memory/Bytes Free</code> in <code>byte</code>s using a <code>double
 * </code> counter while ganglia collects <code>mem_free</code> in <code>KB</code>s using a <code>
 * float</code> metric. In these cases, we need to scale the metric and also change the name of the
 * metric in order to report it as if it were a core ganglia metric.
 *
 * <p>Another difference is that embergraph tends to put the units into the name of the metric while
 * ganglia has explicit metadata about the units. For metrics such as <code>CPU/% System Time</code>
 * we need to register the appropriate units (note that ganglia also can not handle the <code>%
 * </code> in the name of the metric).
 *
 * <p>TODO Service name, service iface, service UUID.
 *
 * <p>TODO Right now metrics which we are not aligning with Ganglia are being reported out in other
 * top-level groups (Bytes Free and Swap Bytes Used are two good examples). [Bytes Free has been
 * fixed.]
 *
 * <p>TODO Rather than munging a "%" to a "Percent", it would be better to rename the metric and
 * specify "%" as the units.
 */
public class EmbergraphMetadataFactory extends GangliaCoreMetricDecls
   ...
   * component or (for a service counter) the host plus the service component.
   */
   ...
   * @param decl The declaration for the metric.
   */
  protected void addHostCounter(final String metricName, final IGangliaMetadataMessage decl) {

    addHostCounter(metricName, decl, false /* prefer */);
  }

  protected void addHostCounter(
      final String metricName, final IGangliaMetadataMessage decl, boolean prefer) {

    if (prefer) resolveDecls.put(decl.getMetricName(), decl);
   ...
   * @param decl The declaration for the metric.
   */
  protected void addServiceCounter(final String metricName, final IGangliaMetadataMessage decl) {

    addServiceCounter(metricName, decl, false /* prefer */);
  }

  protected void addServiceCounter(
      final String metricName, final IGangliaMetadataMessage decl, final boolean prefer) {
   ...
   *
   * <p>TODO We could actually meld the data generating a new declaration having the desired
   * behavior but the same data as the caller's argument. That would require a new constructor (or
   * factory method) on a rich metadata class.
   */
  @Override
   ...
   * @param dmax The value of dmax to use in the declarations.
   * @param heartbeatInteval The heartbeat interval in seconds -or- ZERO (0) if we will not be
   *     sending out the ganglia host heartbeat.
   */
  public EmbergraphMetadataFactory(
      final String hostName,
      final String serviceName,
      final GangliaSlopeEnum slope,
      final int tmax,
      final int dmax,
  ...
  protected void addHostMemoryMetrics() {
    ...
    addHostCounter(
        ...
        new GangliaMetadataMessage(
            ...
            tmax,
            dmax,
            getMap(GROUP_MEMORY, "Free Memory", "Amount of available memory")) {
  ...
  protected void addServiceCPUMetrics() {
    ...
    addServiceCounter(
        ...
        new GangliaMetadataMessage(
            ...
            90, // tmax
            dmax, // dmax,
            getMap(GROUP_CPU, "CPU Total", "Percentage of the time the processor is not idle")));
