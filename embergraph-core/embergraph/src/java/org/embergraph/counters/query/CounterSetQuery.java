 ...
 *
 * <p>A single pass is made through the specified XML counter set files. Each file is read into
 * memory by itself, and each query implied by a listed URL is run against the in-memory {@link
 * CounterSet} hierarchy. The results are collected in independent {@link CounterSet} provisioned
 * for the specified reporting units, etc. Once the last XML counter set file has been processed,
 * the various reports requested by the listed URLs are generated.
 *
 * <p>For each generated report, the name of the file on which the report will be written is taken
 * from the name of the counter whose value was extracted for that report. This filename may be
 * overridden by including the URL query parameter {@value URLQueryModel#FILE}, which specifies the
 * file on which to write the report for that query.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @see IHostCounters
 * @see IRequiredHostCounters
 * @todo When rendering HTML output using flot, the flot resources need to be available in order to
 *     view the graphs. They should be written once into the output directory and the links in the
 *     (X)HTML output should resolve them there.
 * @todo Permit nsamples to be specified in units of minutes, hours, days. E.g., 3d would be 3 days.
 * @todo Performance for long runs could be improved if we use more efficient classes for mutable
 *     strings in {@link XMLUtility} and perhaps {@link CounterSet}. E.g., mg4j mutable string or
 *     http://javolution.org/.
 */
public class CounterSetQuery {
   ...
   * @throws IOException
   */
  private static Collection<URL> readURLs(final File file) throws IOException {
  ...
  private static void readFiles(
    ...
    if (false /* sequential */) {
      ...
      readFilesSequential(flatFileList, counterSet, nsamples, period, regex);

    } else {

      // process the files in parallel.
      readFilesParallel(flatFileList, counterSet, nsamples, period, regex);
    }
  }
   ...
   *   <dt>-mimeType
   *   <dd>The default MIME type for the rendered reports. The default is <code>text/plain</code>,
   *       but can be overridden on a query by query basis using {@link URLQueryModel#MIMETYPE}.
   *   <dt>-nsamples
   *   <dd>Override for the default #of history samples to be retained. It is an error if there are
   *       more distinct samples in the processed XML counter set files (that is, if the #of time
   *       periods sampled exceeds this many samples). If there are fewer, then some internal arrays
   *       will be dimensioned larger than is otherwise necessary.
   *   <dt>-events &lt;file&gt;
   *   <dd>A file containing tab-delimited {@link Event}s. The {@link Event}s are not required for
   *       simple performance counter views.
   *   <dt>-queries &lt;file&gt;
   *   <dd>A file, or directory of files, containing a list of URLs, each of which is interpreted as
   *       a {@link URLQueryModel}.
   *   <dt>&lt;file&gt;(s)
   *   <dd>One or more XML counter set files or directories containing such files. All such files
   *       will be processed before the reports are generated.
   * </dl>
   *
  ...
  public static void main(final String[] args)
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (arg.startsWith("-")) {
        ...
        if (arg.equals("-outputDir")) {
          ...
          if (nsamples <= 0) throw new IllegalArgumentException("nslots must be positive.");
    ...
    readFiles(counterSetFiles, counterSet, nsamples, PeriodEnum.Minutes, regex);

    /*
     * Run each query in turn against the filtered pre-loaded counter set.
     */
    if (log.isInfoEnabled()) log.info("Evaluating " + queries.size() + " queries.");
    ...
    for (URLQueryModel model : queries) {
      ...
      try {
        ...
        final Writer w = new BufferedWriter(new FileWriter(file, false /* append */));
