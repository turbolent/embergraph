 ...
 * @version $Id$
 *     <p>FIXME provide aggregation either here or in how the counters are selected. The capturing
 *     groups should specify the dimensions of aggregation.
 */
public class HistoryTable {
  ...
  public HistoryTable(final ICounter[] a, final PeriodEnum basePeriod) {
    ...
    if (log.isInfoEnabled())
      log.info("#counters=" + a.length + ", units=" + units + ", period=" + period);
    ...
    for (int col = 0; col < ncols; col++) {
      ...
      if (a[col] == null) throw new IllegalArgumentException();
      ...
      final SampleIterator itr =
          ((HistoryInstrument) a[col].getInstrument()).getHistory(/*basePeriod*/ ).iterator();
      ...
      if (maxSamplesIndex == -1 || sampleCount > hitrs[maxSamplesIndex].getSampleCount()) {
    ...
    if (maxSamplesIndex != -1) {
      ...
      for (int col = 0; col < ncols; col++) {
        ...
        final int x = getFirstRowIndex(col) + hitrs[col].getSampleCount();
      ...
      if (log.isInfoEnabled()) {
        ...
        log.info("maxSamples=" + maxSamples + " @ index=" + maxSamplesIndex);

        log.info("firstTimestamp=" + firstTimestamp + " @ index=" + firstSampleTimeIndex);

        log.info("lastTimestamp=" + lastTimestamp + " @ index=" + lastSampleTimeIndex);
    ...
    for (int col = 0; col < ncols; col++) {
      ...
      if (log.isDebugEnabled() && col == 0) log.debug(a[col].getPath());
      ...
      while (itr.hasNext()) {
        ...
        if (log.isDebugEnabled() && col == 0) log.debug("data[" + row + "," + col + "] = " + e);
   ...
   * </pre>
   *
   * @param counterIndex The index of the counter in the array specified to the ctor.
   */
   ...
   * timestamp to be reported ({@link #firstSampleTimeIndex}) and then examining the other counters
   * and deciding if they have a value for the same reporting period. If a counter has a value for
   * the same reporting period then the value is incorporated into that row and the row index for
   * that counter is advanced. Otherwise the row index for that counter IS NOT advanced. If there is
   * no data for a given counter in a given row then that cell will be <code>null</code>. It is
   * necessary to align the samples in this manner as counters are created and destroyed over the
   * life of the system and thus some counters may not have data for some reporting periods.
   */
   ...
   *
   * @param row The row.
   * @return The timestamp of the first sample which would be allowed into that row.
   */
