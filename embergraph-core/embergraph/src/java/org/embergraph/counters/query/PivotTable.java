...
public class PivotTable {
   ...
   *     ignored. This makes it easier switch back and forth between a {@link PivotTable} and a
   *     {@link HistoryTable}. If the counter name was in fact treated as a capturing group for the
   *     purposes of determining the category columns, then that would give you a distinct row for
   *     every sample and blanks in the other value columns.
   * @param category The ordered labels to be assigned to the category columns (optional). When
   *     given, the order of the category names parameters MUST correspond with the order of the
   *     capturing groups in the <i>pattern</i>.
   * @param t The source data.
   */
  public PivotTable(final Pattern pattern, final String[] category, final HistoryTable t) {
    ...
    // aggregate counters into sets sharing the same category values.
    {
      ...
      if (log.isInfoEnabled()) log.info("csets: " + csets);

      //            // #of capturing groups in the pattern.
      //            final int ngroups = pattern.matcher("").groupCount();

      if (log.isInfoEnabled()) log.info("ngroups=" + ngroups);
    }
    ...
    if (log.isInfoEnabled()) log.info("category names=" + Arrays.toString(cnames));
    ...
    for (int row = 0; row < t.nrows; row++) {
      ...
      for (CSet cset : csets) {
        ...
        assert cset.cats.length == cnames.length
            : "cset categories="
   ...
   * @version $Id$
   *     <p>FIXME Not yet in use. Either finish or remove this abstraction.
   */
  class PivotRow {
     ...
     * @param cset The category set for this row. The values for the category columns in the row are
     *     {@link CSet#cats}.
     * @param values The value columns for the row. There is one element in the array for each
     *     element in {@link PivotTable#vcols}. The element MAY be <code>null</code> in which case
     *     there was no data for that counter for this row.
     */
    PivotRow(final int row, final long timestamp, final CSet cset, final Double[] values) {

      if (cset == null) throw new IllegalArgumentException();

      if (cset.cats.length != cnames.length) throw new IllegalArgumentException();

      if (values == null) throw new IllegalArgumentException();

      if (values.length != vcols.size()) throw new IllegalArgumentException();
   ...
   *
   * <p>Note: This automatically detects if the last capturing group captures the counter name and
   * then drop that from the set of category columns. This makes it much easier to switch between a
   * correlated view and a pivot view since you often want the counter name to be a capturing group
   * for the correlated view.
   *
   * @param ngroups The #of capturing groups which were actually used (by side-effect). This is
   *     either the #of capturing groups which were specified in the <i>pattern</i> -or- one less
   *     than that value iff the last capturing group captures the counter name.
   */
  protected static List<CSet> getCategoryValueSets(
      final Pattern pattern, final ICounter[] a, final AtomicInteger ngroups) {
