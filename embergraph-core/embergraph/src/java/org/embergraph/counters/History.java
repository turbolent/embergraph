 ...
 * @version $Id$
 * @param <T> Which must be Double, Long, or String.
 */
public class History<T> {
  ...
  public class SampleIterator implements Iterator<IHistoryEntry<T>> {
    ...
    protected SampleIterator() {
      ...
      _data = (T[]) java.lang.reflect.Array.newInstance(data.getClass().getComponentType(), n);
   ...
   *     reported for dates, strings, etc).
   * @todo could report the most frequent value for non-numeric data or a list of the distinct
   *     values.
   */
  public T getAverage() {
   ...
   *     10 minutes. The reporting periods have to be read from the logicalSlot for 10 minutes ago
   *     up through the current logicalSlot.
   * @return The average over the last N reporting periods.
   * @throws IllegalArgumentException If you request data that is older (in reporting periods) that
   *     is stored within the history. E.g., you can not ask for more than a 60 minute average if
   *     the reporting period is minutes and the capacity is 60.
   */
  public synchronized T getAverage(final int nperiods) {
    ...
    if (nperiods < 1 || nperiods > capacity) {
      ...
      throw new IllegalArgumentException("Must be in [0:" + capacity + "], not " + nperiods);
    }
    ...
    assert tmpi >= 0 && tmpi < capacity
        : "index=" + tmpi + ", lastLogicalSlot=" + lastLogicalSlot + ", capacity=" + capacity;
    ...
    for (long ls = firstLogicalSlot; ls <= currentLogicalSlot; ls++) {
      ...
      if (data[physicalSlot] == null) continue;
  ...
  protected T valueOf(double d) {
    ...
    if (isLong()) return (T) Long.valueOf((long) d);

    if (isDouble()) return (T) Double.valueOf(d);
   ...
   * its <i>timestamp</i>.
   *
   * <p>If the history wraps around into the next period and there is another history that
   * aggregates this one, then the average for the last period will be added to the aggregating
   * history.
   *
   * <p>Multiple samples in the same period are recorded as (a) the total of those samples in the
   * period; and (b) the #of samples in the period.
   *
   * @param timestamp The timestamp associated with the sample.
   * @param value The sampled value.
   * @throws IllegalArgumentException if the timestamp is non-postitive.
   * @throws TimestampOrderException if the timestamp is way out of the current range for the
   *     history buffer.
   */
  public synchronized void add(final long timestamp, final T value) {

    if (log.isInfoEnabled()) log.info("timestamp=" + timestamp + ", value=" + value);
      ...
      if ((lastLogicalSlot - logicalSlot) >= capacity) {
        ...
        if (log.isInfoEnabled())
          log.info(
              ...
              new TimestampOrderException("timestamp=" + timestamp + ", value=" + value));
    ...
    if (lastLogicalSlot == -1) {
      ...
      for (long ls = lastLogicalSlot + 1; ls <= logicalSlot; ls++) {
        ...
        if (ps == 0 && sink != null) {
          ...
          if (log.isInfoEnabled()) log.info("overflow: t=" + t + ", avg=" + avg);
        ...
        if (data[ps] != null) {
          ...
          if (!overwrite) {
            ...
            throw new RuntimeException(
                "Would overwrite data: ps=" + ps + ", capacity=" + capacity + ", size=" + size);
          }
      ...
      data[physicalSlot] =
          (data[physicalSlot] == null || !isNumeric()
              ? value
              : valueOf(
                  ((Number) data[physicalSlot]).doubleValue() + ((Number) value).doubleValue()));
   ...
   *     array will be used to store the data. (This API requirement arises since generics are fixed
   *     at compile time rather than runtime.)
   * @param period The period covered by each slot in milliseconds.
   * @param overwrite <code>true</code> iff overwrite of slots in the buffer is allowed (when <code>
   *     false</code> the buffer will fill up and then refuse additional samples if they would
   *     overwrite slots which are in use).
   */
  @SuppressWarnings("unchecked")
   ...
   * @param source The source whose values are aggregated each time its history overflows.
   */
  @SuppressWarnings("unchecked")
    ...
    this.data =
        (T[])
            java.lang.reflect.Array.newInstance(
                source.data.getClass().getComponentType(), capacity);
   ...
   *
   * <p>The [logicalSlot] is a strictly increasing index corresponding to the #of elapsed periods
   * since the epoch (when timestamp was 0).
   *
   * <p>The [physicalSlot] is the index at which the new sample will be placed in the buffer. This
   * is always interpreted as logically greater than the last sample (we have already asserted that
   * the timestamp is greater than lastModified), even if the actual index is less than or equal to
   * the current index.
   */
