 ...
 * e.g., every 60 seconds or so. The purpose of these data is to support decision-making concerning
 * the over- and under-utilization of hosts in support of load balancing of services deployed over
 * those hosts.
 *
 * <p>An effort has been made to align the core set of counters for both Windows and Un*x platforms
 * so as to support the declared counters on all platforms.
 *
...
public abstract class AbstractStatisticsCollector implements IStatisticsCollector {
  ...
  static {
    ...
    hostPathPrefix = ICounterSet.pathSeparator + fullyQualifiedHostName + ICounterSet.pathSeparator;
  ...
  protected AbstractStatisticsCollector(final int interval, final String processName) {

    if (interval <= 0) throw new IllegalArgumentException();

    if (processName == null) throw new IllegalArgumentException();
...
//     * Return the load average for the last minute if available and -1
   ...
   *
   * <p>Note: Subclasses MUST extend this method to initialize their own counters.
   *
   * <p>TODO Why does this use the older <code>synchronized</code> pattern with a shared {@link
   * #countersRoot} object rather than returning a new object per request? Check assumptions in the
   * scale-out and local journal code bases for this.
   */
  @Override
   ...
   * @param serviceIface The class or interface that best represents the service or client.
   * @param properties The properties used to configure that service or client.
   */
  public static void addBasicServiceOrClientCounters(
  ...
  public static CounterSet getMemoryCounterSet() {
    ...
    // Service per-process memory data
    {
      ...
      AbstractStatisticsCollector.addGarbageCollectorMXBeanCounters(
          serviceRoot.makePath(ICounterHierarchy.Memory_GarbageCollectors));

      // add counters for memory pools.
      AbstractStatisticsCollector.addMemoryPoolMXBeanCounters(
          serviceRoot.makePath(ICounterHierarchy.Memory_Memory_Pools));

      /*
       * Add counters reporting on the various DirectBufferPools.
       */
      serviceRoot
          .makePath(IProcessCounters.Memory + ICounterSet.pathSeparator + "DirectBufferPool")
          .attach(DirectBufferPool.getCounters());
   ...
   * OneShotInstrument}.
   *
   * @param serviceInfoSet The {@link ICounterHierarchy#Info} {@link CounterSet} for the service.
   * @param properties The properties to be reported out.
   */
  public static void addServiceProperties(
      final CounterSet serviceInfoSet, final Properties properties) {
   ...
   *
   * @param counterSet The counters set that is the direct parent.
   */
  public static void addGarbageCollectorMXBeanCounters(final CounterSet counterSet) {
    ...
    synchronized (counterSet) {
      final List<GarbageCollectorMXBean> list = ManagementFactory.getGarbageCollectorMXBeans();
              ...
              tmp.addCounter(
                  ...
                  new Instrument<String>() {
                    ...
                    protected void sample() {
                      ...
                      setValue(Arrays.toString(bean.getMemoryPoolNames()));
                    }
                  });
            }
          }
   ...
   * @param counterSet The counters set that is the direct parent.
   */
  public static void addMemoryPoolMXBeanCounters(final CounterSet counterSet) {
    ...
    synchronized (counterSet) {
      final List<MemoryPoolMXBean> list = ManagementFactory.getMemoryPoolMXBeans();
  ...
  public void start() {
    ...
    if (log.isInfoEnabled()) log.info("Starting collection.");
   ...
   * service termination. However, under eclipse (at least when running under Windows) you may find
   * that the shutdown hook does not run when you terminate a Java application and that typedef
   * process build up in the Task Manager as a result. This should not be the case during normal
   * deployment.
   */
  ...
  public interface Options {
     ...
     *
     * <p><strong>/<i>fullyQualifiedHostname</i>/<i>processName</i>/...</strong>
     *
     * <p>Note: Services are generally associated with a {@link UUID} and that {@link UUID} is
   ...
   * @throws UnsupportedOperationException If there is no implementation available on the operating
   *     system on which you are running.
   * @see Options
   */
  public static AbstractStatisticsCollector newInstance(final Properties properties) {
    ...
    if (SystemUtil.isLinux()) {
      ...
      throw new UnsupportedOperationException(
          "No implementation available on " + System.getProperty("os.getname"));
    }
  }
   ...
   * Collection will occur every {@link Options#PERFORMANCE_COUNTERS_SAMPLE_INTERVAL} seconds. The
   * program will make 10 collections by default and will write the updated counters on stdout every
   * {@link Options#PERFORMANCE_COUNTERS_SAMPLE_INTERVAL} seconds.
   *
   * <p>Parameters also may be specified using <code>-D</code>. See {@link Options}.
   *
   * @param args <code>[<i>interval</i> [<i>count</i>]]</code>
   *     <p><i>interval</i> is the collection interval in seconds and defaults to {@link
   *     Options#DEFAULT_PERFORMANCE_COUNTERS_SAMPLE_INTERVAL}.
   *     <p><i>count</i> is the #of collections to be made and defaults to <code>10</code>. Specify
   *     zero (0) to run until halted.
   * @throws InterruptedException
   * @throws RuntimeException if the arguments are not valid.
   * @throws UnsupportedOperationException if no implementation is available for your operating
   *     system.
   */
  public static void main(final String[] args) throws InterruptedException {
    ...
    if (interval <= 0) throw new RuntimeException("interval must be positive");

    if (count < 0) throw new RuntimeException("count must be non-negative");
    ...
    final AbstractStatisticsCollector client = AbstractStatisticsCollector.newInstance(properties);
    ...
    System.err.println(
        "Starting performance counter collection: interval="
            + client.interval
            + ", count="
            + count);
   ...
   * @return The #of bytes.
   */
  public static Double kb2b(final String kb) {
