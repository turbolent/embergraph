...
public class PIDUtil {
  ...
  public static int getLinuxPIDWithBash() {
    ...
    if (pr.exitValue() == 0) {
      ...
      if (val == null) throw new RuntimeException("Nothing read.");

      if (log.isInfoEnabled()) log.info("read: [" + val + "]");
   ...
   * @see RuntimeMXBean#getName(), A web search will show that this is generally of the form
   *     "pid@host". However this is definitely NOT guaranteed by the javadoc.
   */
  public static int getPIDWithRuntimeMXBean() {
    ...
    if (log.isInfoEnabled()) log.info("pid=" + pid);
