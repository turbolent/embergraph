 ...
 * process name. Since we run multiple services per host, collection by process name is not
 * sufficient to facilitate decision making by the load balancer. Instead, the load balancer relies
 * on service specific performance counters, including the average queue length and response time.
 *
 * @see
...
public class StatisticsCollectorForWindows extends AbstractStatisticsCollector {
  ...
  public StatisticsCollectorForWindows(final int interval, final String processName) {
