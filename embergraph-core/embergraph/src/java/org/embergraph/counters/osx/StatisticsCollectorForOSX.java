 ...
 *
 * <ul>
 *   <li>{@link IRequiredHostCounters#PhysicalDisk_BytesReadPerSec}
 *   <li>{@link IRequiredHostCounters#PhysicalDisk_BytesWrittenPerSec}
 *   <li>{@link IHostCounters#CPU_PercentIOWait}
 * </ul>
 *
 * However, we are collecting the following which may be substituted to some extent:
 *
 * <ul>
 *   <li>{@link IHostCounters#PhysicalDisk_BytesPerSec}
 *   <li>{@link IHostCounters#PhysicalDisk_TransfersPerSec}
 * </ul>
 *
 ...
 * @version $Id: StatisticsCollectorForLinux.java 4290 2011-03-11 15:12:26Z thompsonbry $
 */
public class StatisticsCollectorForOSX extends AbstractStatisticsCollector {
   ...
   * its output.
   *
   * <p><code>-l 0</code> turns on "logging" mode. The ZERO (0) indicates infinite samples, rather
   * than some finite number of samples.
   *
   * <p><code>-s [delay]</code> sets the delay between reports.
   *
   * <p><code>-S</code> adds reporting on swap space into the headers.
   *
   * <pre>
   * Swap: 545M + 479M free.
   ...
   * </pre>
   *
   * Top can be used to go after per-process statistics by specifying <code>-pid [pid]</code>. It is
   * also possible to request all processes owned by a user with <code>-user [user]</code>. Here is
   * an example where the pid is specified.
   *
   * <pre>
  ...
  public void start() {
    ...
    if (log.isInfoEnabled()) log.info("starting collectors");
  ...
  public void stop() {
    ...
    if (log.isInfoEnabled()) log.info("stopping collectors");
   ...
   */
  public StatisticsCollectorForOSX(final int interval, final String processName) {
