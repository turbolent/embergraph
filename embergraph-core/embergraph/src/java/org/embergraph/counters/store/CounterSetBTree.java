 ...
 * value rounded down to an even number of minutes and represented a long. The second component is
 * the fully qualified path of the counter. The last component is the exact timestamp (in
 * milliseconds) of the sampled counter value, represented as a long. These are formatted into an
 * unsigned byte[] following the standard practice.
 *
 * <p>The value stored under the key is the counter value. Normally counter values are doubles or
 * longs, but you can store any of the counter value types which are supported by the {@link
 * SparseRowStore}.
 *
 * <p>Using this approach, writes of the same counter value with different timestamps will be
 * recorded as different tuples in the {@link IIndex} and you can store counter values sampled at
 * rates of once per second while retaining good compression for the keys in the index.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 *     <p>FIXME Reading through per-minute counters from a CounterSetBTree grows slow very quickly.
 *     <pre>
 ...
 * </pre>
 *     In order to improve performance, put the counter paths in a separate dictionary and apply the
 *     regex there. Once we have the set of matched paths we can scatter range queries against the
 *     BTree and drag back the data for those counters (this would also make Unicode counter names
 *     viable). If the key was then [pathId,timestamp] we could do ordered reads of just the
 *     necessary key range for each desired counter. Prefix compression would still be efficent for
 *     this representation. While the data arrive in history blocks, we would still need to buffer
 *     them for ordered writes since otherwise the writes would be scattered by the first key
 *     component (pathId).
 *     <p>I would have to encapsulate the counters as a counter for this to work, much like the RDF
 *     DB. There would be two relations: the dictionary and the timestamped values.
 *     <p>Space efficient encoding of the counter values would also help quite a bit - it is Java
 *     default serialization, but we only store Long, Double or String. All values for a given
 *     counter should have the same data type (it is required by how we allocate the History) so the
 *     data type can be part of the dictionary and that can be used to decode the value. (If values
 *     tend to be close then a delta encoding would help.)
 */
public class CounterSetBTree extends BTree {
   ...
   */
  public static CounterSetBTree create(final IRawStore store) {
    ...
    metadata.setTupleSerializer(
        new CounterSetBTreeTupleSerializer(new ASCIIKeyBuilderFactory(INITIAL_CAPACITY)));
  ...
  public static class Entry {
    ...
    public Entry(final long timestamp, final String path, final Object value) {
  ...
  protected static class CounterSetBTreeTupleSerializer
     ...
     */
    public CounterSetBTreeTupleSerializer(final IKeyBuilderFactory keyBuilderFactory) {
    ...
    public Entry deserialize(final ITuple tuple) {
      ...
      final String path =
          KeyBuilder.decodeASCII(
              key, Bytes.SIZEOF_LONG /* off */, key.length - (2 * Bytes.SIZEOF_LONG) /* len */);

      final long timestamp = KeyBuilder.decodeLong(key, key.length - Bytes.SIZEOF_LONG /* off */);
   ...
   * the store). For a counter without history, the current value of the counter will be written on
   * the BTree.
   */
  public void writeHistory(final Iterator<ICounter> src) {
    ...
    while (src.hasNext()) {
      ...
      if (c.getInstrument() instanceof HistoryInstrument) {
        ...
        while (sitr.hasNext()) {
          ...
          final Entry entry = new Entry(e.lastModified(), path, e.getValue());
        ...
        final Entry entry = new Entry(c.lastModified(), path, c.getValue());
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info("Wrote " + nwritten + " of " + a.length + " tuples in " + elapsed + "ms");
    }
  }
   ...
   * History}s).
   *
   * <p>Note that the counter set iterator will normally be in alpha order already and all samples
   * should be close to the same minute, so this is already efficient for local operations.
   *
   * @todo More efficient storage for Double, Long, Integer and String values (this is using Java
   ...
   *
   * @param fromTime The first time whose counters will be visited (in milliseconds).
   * @param toTime The first time whose counters WILL NOT be visited (in milliseconds).
   * @param unit The unit of aggregation for the reported counters.
   * @param filter Only paths matched by the filter will be accepted (optional).
   * @param depth When non-zero, only counters whose depth is LTE to the specified <i>depth</i> will
   *     be returned.
   * @return A collection of the selected performance counters together with their ordered
   *     timestamped values for the specified time period.
   * @todo In an act of cowardice, this assumes that the counter paths are ASCII and encodes them as
   *     such. This allows us to decode the counter path since it is not a compressed sort key. If
   *     we don't take this "hack" then we need a 2nd index to resolve the Unicode path from the
   *     sort key (once we hack off the leading minutes component).
   *     <p>The other problem is that tacking the milliseconds onto the end of the key might break
   *     the natural order of the counter paths in the index.
   *     <p>The two index approach is not so bad. The main drawback is that it can't be encapsulated
   *     as easily.
   */
  public CounterSet rangeIterator(
      long fromTime, long toTime, final TimeUnit unit, final Pattern filter, final int depth) {

    if (fromTime < 0) throw new IllegalArgumentException();

    if (toTime < 0) throw new IllegalArgumentException();
     ...
     */
    final long nslots =
        unit.convert(toTime, TimeUnit.MILLISECONDS)
            - unit.convert(fromTime, TimeUnit.MILLISECONDS)
            + 1;

    if (nslots > Integer.MAX_VALUE) throw new IllegalArgumentException("too many samples");

    final CounterSetBTreeTupleSerializer tupleSer =
        (CounterSetBTreeTupleSerializer) getIndexMetadata().getTupleSerializer();

    final IKeyBuilder keyBuilder = getIndexMetadata().getTupleSerializer().getKeyBuilder();
     ...
     */
    final long toMinutes = TimeUnit.MILLISECONDS.toMinutes(toTime + TimeUnit.MINUTES.toMillis(1));
    ...
    while (itr.hasNext()) {
      ...
      if (c == null) {
        ...
        if (log.isDebugEnabled()) {
          ...
          log.debug("Matched: ndistinct=" + nselected + ", " + entry.path);
        }
    ...
    if (log.isInfoEnabled())
      log.info("nselected=" + nselected + ", nvalues=" + nvalues + ", nvisited=" + nvisited);
  ...
  public long getFirstTimestamp() {
    ...
    if (getEntryCount() == 0) return 0L;

    return ((Entry)
            rangeIterator(null, null, 1 /* capacity */, IRangeQuery.DEFAULT, null /* filter */)
                .next()
                .getObject())
        .timestamp;
  }
