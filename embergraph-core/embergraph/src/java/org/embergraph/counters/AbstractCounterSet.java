...
public abstract class AbstractCounterSet implements ICounterSet {
  ...
  public ICounterNode getPath(String path) {
    ...
    if (path.startsWith(pathSeparator)) {
      ...
      // start at the root
      if (parent != null) return getRoot().getPath(path);
    }
   ...
   *
   * <p>A sample output is below.
   *
   * <p><code>cs</code> is a {@link CounterSet} element and has a <code>path</code> attribute which
   * expresses the location of the counter set within the hierarchy (counter set elements are not
   * nested inside of each other in the XML serialization). Only counter sets with counters are
   * emitted.
   *
   * <p><code>c</code> is a {@link Counter} element and is nested inside of the corresponding
   * counter set. Each counter carries a <code>name</code> attribute, a simple XML Schema Datatype,
   * a timestamp (milliseconds since the epoch per {@link System#currentTimeMillis()}, and has a
   * counter value which is the inner content of the <code>c</code> element.
   *
   * <pre>
   ...
   * </pre>
   */
  public void asXML(final OutputStream os, final String encoding, final Pattern filter)
      throws IOException {
   ...
   *
   * @param w The writer.
   * @param encoding The character set encoding used by that writer.
   * @param filter An optional filter.
   * @throws IOException
   */
  public void asXML(final Writer w, final String encoding, final Pattern filter)
      throws IOException {
