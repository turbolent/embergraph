 ...
 * @see http://en.wikipedia.org/wiki/Linux_kernel for a summary of linux kernel versioning.
 */
public class KernelVersion {
  ...
  public KernelVersion(final String val) {
    ...
    if (!m.matches()) throw new IllegalArgumentException("Not a kernel version? [" + val + "]");
  ...
  public static KernelVersion get() {
    ...
    if (pr.exitValue() == 0) {
      ...
      if (val == null) throw new RuntimeException("Nothing returned.");
