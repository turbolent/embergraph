 ...
 * protected by striped locks. You {@link #acquire()} an instance and then directly manipulate the
 * value of the counters on that instance. When you {@link #release()} the instance, the counters
 * MAY be published to the outer instance visible to your application using {@link
 * #add(StripedCounters)}. This is similar to an eventually consistent transactional model, except
 * that there are no constraints on the validity of the updates imposed by this scheme.
 *
 * <p>If reads will be made against the shared instance without holding a lock, then the individual
 * counters should be volatile fields, {@link AtomicLong}, or the like so that updates will be
 * visible each time {@link #add(StripedCounters)} is invoked by {@link #release()} will be
 * published in a timely manner.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @todo The problem with this approach is that we can not decide that some counters should be
 *     published immediately while others should be published only every M touches [this might be
 *     something which can be hacked for specific counters using an incFoo() or addFoo() method for
 *     that counter which always updates the value on the parent if invoked on a striped counter.]
 */
public class StripedCounters<T extends StripedCounters<T>> {
   ...
   *
   * <p>Note: When using this class you MUST be careful to avoid constructions which could lead to
   * lock ordering problems. If there are other locks help, then each {@link #acquire()} MAY result
   * in a deadlock with an {@link #acquire()} by another thread depending on which stripe is
   * selected for each thread. Such deadlocks are obviously path dependent and difficult to detect.
   */
  @SuppressWarnings("unchecked")
  public final T acquire() {
    if (a == null) return (T) this;
   ...
   * <i>batchSize</i>.
   *
   * @param batchSize The values on the instance stripes will be published to the outer instance
   *     every batchSize {@link #release()}s.
   */
   ...
   *     {@link #acquire()} based on {@link Thread#hashCode()}. The chances of a contention with a
   *     concurrent thread are <code>(1.0/nstripes)</code>.
   * @param batchSize The values on the instance stripes will be published to the outer instance
   *     every batchSize {@link #release()}s.
   */
  public StripedCounters(final int nstripes, final int batchSize) {
    if (nstripes < 1) throw new IllegalArgumentException();
