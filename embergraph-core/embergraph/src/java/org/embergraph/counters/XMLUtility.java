...
public class XMLUtility {
   ...
   * @param w Where to write the XML.
   * @param filter A filter to be applied to the counters (optional). Only the matched counters will
   *     be serialized.
   * @throws IOException
   */
    ...
    while (itr.hasNext()) {
      ...
      while (itr2.hasNext()) {
        ...
        if (time < 0L) {
          ...
          log.warn(
              "Ignoring counter with invalid timestamp: name="
                  + name
                  + ", timestamp="
                  + time
                  + ", value="
   ...
   * @throws IOException
   */
  protected void writeHistory(final Writer w, final History h, final String units)
      throws IOException {
  ...
  public void readXML(
    ...
    if (instrumentFactory == null) throw new IllegalArgumentException();
  ...
  private static class MyHandler extends DefaultHandler {
      ...
      if (qName.equals("counters")) {
        ...
        if (filter != null) {
          ...
          if (!filter.matcher(fqn).matches()) {
            ...
            if (log.isInfoEnabled()) log.info("Does not match filter: " + fqn);
     ...
     * @param name
     * @param typ
     * @return The counter -or- <code>null</code> iff the path and name identify a pre-existing
     *     {@link CounterSet}, which conflicts with the described {@link ICounter}.
     */
    protected ICounter getCounter(final String path, final String name, Class typ) {
      ...
      synchronized (root) {
        ...
        node =
            root.getPath(
                path.equals(ICounterSet.pathSeparator)
                    ? name
                    : path + ICounterSet.pathSeparator + name);

        if (node == null) {

          final IInstrument inst = instrumentFactory.newInstance(typ);
     ...
     * @param typ The value type of the counter.
     * @param text The text of the value to be interpreted.
     * @param time The timestamp for the value.
     */
    protected static void setValue(
        final ICounter counter, final Class typ, final String text, final long time) {
      ...
      if (inst instanceof OneShotInstrument) {
        ...
        log.warn(
            OneShotInstrument.class.getName()
                + " : ignoring update: path="
                + counter.getPath()
                + ", value="
                + text);
    ...
    protected static void addValue(final History history, final long time, final String text) {
   ...
   * @return The corresponding XML datatype -or- "xsd:anyType" if no more specific datatype could be
   *     determined.
   */
  private String getXSDType(Object value) {

    if (value == null) return xsd_anyType;

    Class c = value.getClass();

    if (c.equals(Long.class)) return xsd_long;
    else if (c.equals(Integer.class)) return xsd_int;
    else if (c.equals(Double.class)) return xsd_double;
    else if (c.equals(Float.class)) return xsd_float;
    else if (c.equals(String.class)) return xsd_string;
    else if (c.equals(Boolean.class)) return xsd_boolean;
    else return xsd_anyType;
  }
}
