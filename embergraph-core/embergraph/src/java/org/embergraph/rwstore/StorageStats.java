 ...
 *
 * <p>Stats are retained on external requests and also on internal allocator use.
 *
 * <p>totalSlots represents the total reserved slots in all the AllocationBlocks for the associated
 * FixedAllocators.
 *
 * <p>slotAllocations is the total of all allocations made
 *
 * <p>slotDeletes is the total of all slot deletions
 *
 * <p>Therefore the total of currently allocated slots is slotAllocations - slotDeletes
 *
 * <p>sizeAllocations is the total in bytes of the actual data stored in the slots
 *
 * <p>sizeDeletes is the total in bytes of the actual data that has been deleted
 *
 * <p>Therefore the size of the total in bytes stored in currently allocated slots is
...
public class StorageStats {
  ...
  public class Bucket {
    ...
    //         * See BLZG-1646 BytesAppData counter can not be tracked accurately and should be
    // removed
    //		 */
    //		long m_sizeDeletes;
    ...
    public void delete(final int sze) {
      if (sze < 0) throw new IllegalArgumentException("delete requires positive size, got: " + sze);
      ...
      if (sze > m_size) {
        ...
        throw new IllegalArgumentException(
            "Deletion of address with size greater than slot - " + sze + " > " + m_size);
      }
     ...
     *
     * <p>See BLZG-1551 : The data reported here used to be bad since {@link #m_sizeDeletes} could
     * not being tracked correctly in the {@link StorageStats} because we do not know the actual #of
     * bytes in the slot that contain application data when we finally recycle the slot.
     *
     * <p>See BLZG-1646 BytesAppData counter can not be tracked accurately and should be removed
     */
    ...
    public float slotChurn() {
      ...
      if (slotsReserved.signum() == 0) return 0f;
   ...
   *       outcome).
   *   <dt>%SlotsAllocated
   *   <dd>SlotsAllocated/(Sum of SlotsAllocated across all slot sizes).
   *   <dt>SlotsRecycled
   *   <dd>Cumulative recycled slots to date in this slot size (regardless of the transaction
   *       outcome).
   *   <dt>SlotsChurn
   *   <dd>How frequently slots of this size are re-allocated (SlotsAllocated/SlotsReserved).
   *   <dt>SlotsInUse
   *   <dd>SlotsAllocated - SlotsRecycled (net slots in use for this slot size).
   *   <dt>%SlotsInUse
   *   <dd>SlotsInUse / (total SlotsInUse across all slots sizes).
   *   <dt>MeanAllocation
   *   <dd>((Total application bytes used across all allocations for this slot size) /
   *       SlotsAllocated).
   *   <dt>SlotsReserved
   ...
   *   <dt>BytesReserved
   *   <dd>The space reserved on the backing file for those allocation slots (AllocatorSlots *
   *       SlotsReserved).
   *   <dt>UsedStore
   *   <dd>The #of bytes in the allocated slots.
   *   <dt>%SlotWaste
   ...
   *       (BytesReserved/Sum(BytesReserved)). TODO TotalWaste
   *   <dt>%StoreWaste
  ...
  public void showStats(final StringBuilder str) {
    ...
    str.append(
        String.format(
            "%-16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s %16s \n",
            "AllocatorSize",
            "AllocatorCount",
            "SlotsAllocated",
            "%SlotsAllocated",
    ...
    for (Bucket b : m_buckets) {
      str.append(
          String.format(
              "%-16d %16d %16d %16.2f %16d %16.2f %16d %16.2f %16d %16d %16.2f %16d %16d %16.2f %16.2f %16.2f %16.2f  %16.2f \n",
   ...
   * @return
   */
  private static float dataPercent(final long usedData, final long totalData) {

    if (totalData == 0) return 0.0f;
