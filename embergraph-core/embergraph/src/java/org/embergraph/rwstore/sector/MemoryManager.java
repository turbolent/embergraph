 ...
 *
 * <p>The most complex aspect of the implementation is the BLOB representation, requiring a mapping
 * across multiple allocation slots. This is managed using recursive calls in the main three
 * methods: allocate, free and get.
 *
...
public class MemoryManager implements IMemoryManager, ISectorManager {
   ...
   *
   * @see BLZG-1658 MemoryManager should know when it has been closed
   * @see BLZG-1660 MemoryManager should use read/write allocation lock to protect readers against
   *     concurrent close()
   */
  ...
  private final Condition m_sectorFree = m_allocationLock.writeLock().newCondition();
   ...
   * {@link MemoryManager} will be (effectively) unbounded. If either the pool is bounded, then
   * <em>blocking</em> allocation requests may block, otherwise allocation requests will be
   * non-blocking.
   *
   * <p>The garbage collection of direct {@link ByteBuffer}s depends on a full GC pass. In an
   * application which managers its heap pressure well, full GC passes are rare. Therefore, the best
   * practice is to share an unbounded pool across multiple purposes. Since there are typically
   * multiple users of the pool, the demand can not always be predicated and deadlocks can arise
   * with a bounded pool.
   *
   * <p>Individual buffers will be allocated as necessary and released if they become empty.
   * However, since allocation patterns may cause the in use data to be scattered across the
   * allocated buffers, the backing buffers may not be returned to the backing pool until the
   * top-level allocation context is cleared.
   *
   * <p>Any storage allocated by this instance will be released no later than when the instance is
   * {@link #finalize() finalized}. Storage may be returned to the pool within the life cycle of the
   * {@link MemoryManager} using {@link #clear()}. Nested allocation contexts may be created and
   * managed using {@link #createAllocationContext()}.
   *
   * @param pool The pool from which the {@link MemoryManager} will allocate its buffers (each
   *     "sector" is one buffer).
   * @throws IllegalArgumentException if <i>pool</i> is <code>null</code>.
   */
   ...
   *
   * <p>The backing {@link DirectBufferPool} may be either bounded or (effectively) unbounded. The
   * {@link MemoryManager} may also be bounded or (effectively) unbounded. If either the pool or the
   * memory manager is bounded, then <em>blocking</em> allocation requests may block. Neither
   * non-blocking allocation requests nor allocation requests made against an unbounded memory
   * manager backed by an unbounded pool will block. The preferred method for bounding the memory
   * manager is to specify a maximum #of buffers which it may consume from the pool.
   *
   * <p>The garbage collection of direct {@link ByteBuffer}s depends on a full GC pass. In an
   * application which managers its heap pressure well, full GC passes are rare. Therefore, the best
   * practice is to share an unbounded pool across multiple purposes. Since there are typically
   ...
   * @param pool The pool from which the {@link MemoryManager} will allocate its buffers (each
   *     "sector" is one buffer).
   * @param sectors The maximum #of buffers which the {@link MemoryManager} will allocate from that
   *     pool (each "sector" is one buffer). This may be {@link Integer#MAX_VALUE} for an
   *     effectively unbounded capacity.
   * @throws IllegalArgumentException if <i>pool</i> is <code>null</code>.
   * @throws IllegalArgumentException if <i>sectors</i> is non-positive.
   */
   ...
   *     pool (each "sector" is one buffer). This may be {@link Integer#MAX_VALUE} for an
   *     effectively unbounded capacity.
   * @param blocks When <code>true</code> an allocation request will block until it can be
   *     satisfied. When <code>false</code> and allocation request that can not be satisfied
   *     immediately will result in a {@link MemoryManagerOutOfMemory}.
   * @param properties Used to communicate various configuration properties, including {@link
   *     AbstractTransactionService.Options#MIN_RELEASE_AGE} (optional).
   * @throws IllegalArgumentException if <i>pool</i> is <code>null</code>.
   * @throws IllegalArgumentException if <i>sectors</i> is non-positive.
   * @see <a href="http://jira.blazegraph.com/browse/BLZG-42" > Per query memory limit for analytic
   *     query mode. </a>
  ...
  public MemoryManager(
    ...
    if (sectors <= 0) throw new IllegalArgumentException();
    ...
    m_deferredFreeOut =
        PSOutputStream.getNew(this, SectorAllocator.BLOB_SIZE + 4 /*allow for checksum*/, null);
   ...
   * will throw a {@link MemoryManagerOutOfMemory} exception if the backing memory pool is
   * exhausted.
   *
   * @see <a href="http://jira.blazegraph.com/browse/BLZG-42" > Per query memory limit for analytic
   *     query mode. </a>
   ...
   * side-effect on the position of each source buffer.
   */
  ...
  private SectorAllocator scanForSectorWithFreeSpace(final int nbytes) {
    ...
    for (SectorAllocator s : m_sectors) {
      ...
      if (s.m_free[tag] > 0) {
        ...
        if (log.isDebugEnabled()) log.debug("Can allocate from sector: " + s);
   ...
   */
  private SectorAllocator getSectorFromFreeList(final boolean blocks, final int nbytes) {
    ...
    while (m_free.isEmpty()) {
      ...
      if (m_sectors.size() < m_maxSectors) {
        ...
        if (m_activeTxCount > 0 || !m_contexts.isEmpty()) sector.preserveSessionData();
        ...
        if (blocks) {
          ...
          if (log.isDebugEnabled()) log.debug("Blocking...");
  ...
  public long allocate(final int nbytes, final boolean blocks) {
    ...
    try {
      ...
      if (nbytes <= SectorAllocator.BLOB_SIZE) {
        ...
        final SectorAllocator sector = getSectorFromFreeList(blocks, nbytes);

        final int rwaddr = sector.alloc(nbytes);

        if (SectorAllocator.getSectorIndex(rwaddr) >= m_sectors.size()) {
          throw new IllegalStateException(
              "Address: " + rwaddr + " yields index: " + SectorAllocator.getSectorIndex(rwaddr));
        }
        ...
        m_slotBytes.addAndGet(sector.getPhysicalSize(SectorAllocator.getSectorOffset(rwaddr)));
        ...
        try {
          ...
          if (log.isTraceEnabled()) log.trace("Allocation BLOB at: " + retaddr);
          ...
          try {
            ...
            log.warn(
                "Problem trying to release partial allocations after MemoryManagerOutOfMemory", t);
          }
  ...
  public ByteBuffer[] get(final long addr) {
    ...
    // BLZG-1658 We need at least a readLock() on the allocation lock to protect against a
    // concurrent close().
    ...
    try {
      ...
      if (size <= SectorAllocator.BLOB_SIZE) {
        ...
        for (int i = 0; i < nblocks; i++) {
          ...
          int blockSize =
              remaining <= SectorAllocator.BLOB_SIZE ? remaining : SectorAllocator.BLOB_SIZE;
   ...
   * @param addr The allocation address.
   ...
   *
   * <p>This is required to support get and free.
   *
   ...
   * @param size The byte length of the user application.
   * @return The mutable view.
   */
  ByteBuffer getBuffer(final int rwaddr, final int size) {

    // BLZG-1658 I think we need to assert that the allocation lock (or a readLock() on it) is held
    // by the caller.
    final Lock lock = m_allocationLock.readLock();
    lock.lock();
    try {
      ...
      if (nlimit > ret.capacity() || nlimit < 0) {
        throw new IllegalStateException(
            "Buffer Limit Error - Capacity: " + ret.capacity() + ", new limit: " + nlimit);
      }
      ret.limit(nlimit);
  ...
  public void free(final long addr) {
    ...
    if (log.isTraceEnabled()) log.trace("Releasing allocation at: " + rwaddr + "[" + size + "]");
  ...
  private void immediateFree(final long addr) {
    ...
    try {
      if (size <= SectorAllocator.BLOB_SIZE) {
        ...
        removeFromExternalCache(getPhysicalAddress(addr), sector.getPhysicalSize(offset));
  ...
  /** The total #of bytes in the backing buffers currently attached to the {@link MemoryManager}. */
  ...
  public void close() {
    ...
    try {
      if (open.compareAndSet(true /* expect */, false /* update */)) {
        if (log.isDebugEnabled()) log.debug("MemoryManager is closed.");
  ...
  public int getAssociatedSlotSize(final int addr) {
    // BLZG-1658 MemoryManager should know when it has been closed (operation is not protected
  ...
  public void postCommit() {
    if (!m_allocationLock.isWriteLockedByCurrentThread()) throw new IllegalMonitorStateException();
    assertOpen(); // BLZG-1658 MemoryManager should know when it has been closed
    //		try {
  ...
  private int freeDeferrals(final AbstractJournal journal, final long fromTime, final long toTime) {
    ...
    final ITupleIterator<CommitRecordIndex.Entry> commitRecords =
        commitRecordIndex.rangeIterator(fromKey, toKey);
    ...
    try {
      ...
      if (context.isIsolated()) {
        ...
        if (alloc != null) {
          ...
          if (m_activeTxCount == 0 && m_contexts.isEmpty()) releaseSessions();
        }
      }
  ...
  private AllocationContext getContextAllocation(final IAllocationContext context) {
  ...
  public IAllocationContext newAllocationContext(final boolean isolated) {
    // BLZG-1658 This needs a readLock() on the allocation lock to protect against a concurrent
    // close.
    final Lock lock = m_allocationLock.readLock();
    lock.lock();
    try {
      ...
      if (isolated) {
        ...
        if (m_activeTxCount == 0 && m_contexts.size() == 1) acquireSessions();
      }
