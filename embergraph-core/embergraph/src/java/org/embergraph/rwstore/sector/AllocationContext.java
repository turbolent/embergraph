 ...
 *
 * <p>There are two obvious implementation strategies:
 *
 * <ol>
 *   <li>Retaining set of addresses allocated
 *   <li>Retaining a copy of the allocated bits
 * </ol>
 *
 * If it was not for the BLOB implementations which require length data to manage the freeing of an
 * allocation, it would be efficient to maintain copies of the allocation bits. This remains an
 * option for the future but requires a relatively complex callback protocol.
 *
 * <p>For this reason, the initial implementation maintains a set of allocated addresses.
 *
...
public class AllocationContext implements IAllocationContext, IMemoryManager { // , IStore {
  ...
  public long alloc(byte[] buf, int size, IAllocationContext context) {
    ...
    return MemoryManager.getAllocationAddress(
        allocate(ByteBuffer.wrap(buf, 0, size))); // return the rwaddr!
  }
