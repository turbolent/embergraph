...
public class MemStore extends AbstractRawStore implements IAllocationManagerStore {
   ...
   *
   * @param pool The pool from which the backing direct {@link ByteBuffer}s will be allocated.
   * @param bufferCapacity The maximum #of buffers which may be allocated by this {@link MemStore}
   *     from that pool. This may be {@link Integer#MAX_VALUE} for an effectively unlimited
   *     capacity.
   */
  ...
  private void assertOpen() {
    ...
    if (!isOpen()) throw new IllegalStateException(AbstractBufferStrategy.ERR_NOT_OPEN);
  }
  ...
  public void deleteResources() {
    ...
    if (open) throw new IllegalStateException(AbstractBufferStrategy.ERR_OPEN);
   ...
   * is because the {@link MemoryManager}, like the {@link RWStore}, does not know the #of bytes of
   * user data in each allocation slot. Therefore it is not able to keep accurate track of the user
   * bytes as allocation slots are cycled.
   *
   ...
   * IPSOutputStream}.
   *
   * @param context The context within which any allocations are made by the returned {@link
   *     IPSOutputStream}.
