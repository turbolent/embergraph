 ...
 *     int, int)} to copy the data rather than cloning it.
 * @todo Review the locks held during reads against {@link AllocBlock}. Is it possible that we could
 *     have updates which are not being made visible to readers?
...
public class AllocBlock {
  ...
  AllocBlock(
      final int addrIsUnused,
      final int bitSize,
      final FixedAllocator allocator) { // , final RWWriteCacheService cache) {
    //		m_writeCache = cache;
   ...
   *
   * <p>If no existing shadow is registered, and a new allocation can be made then this AllocBlock
   * is registered with the shadow.
   *
   * <p>Note that when shadows are used, an allocator on a free list may not have allocations
   * available for all contexts, so the assumption that presence on the free list implies
   * availability is not assertable.
   */
   ...
   * commit, were set within this context.
   *
   * <p>The m_commit is the m_transients bits at the point of the link of the allocationContext with
   * this allocator, bits set in m_live that are not set in m_commit, were made by this allocator
   * for the aborted context.
   *
   * <p>L 1100 0110 AC 0111 AB 0110 T 1100 1110 1111 1110 C 1100 1100 1110 1100
   *
   * <p>BUT the shadow abort must not undo transients bits that were deleted from other contexts.
   * This issue is resolved with the deferred free mechanism linked to the ContextAllocation.
   */
  ...
  private int clearCacheBits(
      final RWWriteCacheService cache, final int startBit, final int chkbits) {
    int freebits = 0;
    ...
    if (chkbits != 0) {
      ...
      for (int b = 0; b < 32; b++) {
        if ((chkbits & (1 << b)) != 0) {
          final long clr =
              RWStore.convertAddr(m_addr) + ((long) m_allocator.m_size * (startBit + b));

          if (s_islogTrace) log.trace("releasing address: " + clr);

          // obtained the latched address for that bit.
          final int latchedAddr =
              -((m_allocator.getIndex() << RWStore.OFFSET_BITS) + (startBit + b));
   ...
   *
   * <p>With no Isolated writes the state simply reverts to the committed state as retained in the
   * m_commit array and any buffered allocations are cleared from the cache.
   *
   ...
   * the committed, thus releasing slots for re-allocation.
   *
   * <p>For each transient bit, check if cleared and ensure any write is removed from the write
   * cache. Where the bit is set in the session protected but not in the recalculated transient.
   * Tested with new &= ~old;
   *
   * @param cache
   ...
   *
   * @param cache The {@link WriteCacheService}.
