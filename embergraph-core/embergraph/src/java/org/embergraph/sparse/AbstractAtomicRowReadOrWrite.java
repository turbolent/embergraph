 ...
 */
public abstract class AbstractAtomicRowReadOrWrite extends AbstractIndexProcedure<TPS>
    implements ISimpleIndexProcedure<TPS>, IRowStoreConstants, Externalizable {
...
//     * True iff the {@link #log} level is DEBUG or less.
   ...
   * @param primaryKey The value of the primary key (identifies the logical row to be read).
   ...
   *     -or- {@link IRowStoreConstants#CURRENT_ROW} to accept only the most current binding whose
   *     timestamp is GTE <i>fromTime</i>.
   ...
   * @param primaryKey The primary key identifies the logical row of interest.
   * @param fromTime The first timestamp for which timestamped property values will be accepted.
   * @param toTime The first timestamp for which timestamped property values will NOT be accepted
   *     -or- {@link IRowStoreConstants#CURRENT_ROW} to accept only the most current binding whose
   *     timestamp is GTE <i>fromTime</i>.
   * @param writeTime The resolved timestamp for an atomic write operation -or- ZERO (0L) IFF the
   *     operation is NOT a write.
   * @param filter An optional filter used to select the values for property names accepted by that
   *     filter.
   * @return The logical row for that primary key -or- <code>null</code> iff there is no data for
   *     the <i>primaryKey</i>.
   */
  protected static TPS atomicRead(
      final IIndex ndx,
      final Schema schema,
      final Object primaryKey,
      final long fromTime,
   ...
   * @param toTime
   * @param filter
   * @param tps The object into which the timestamped property values will be read.
   * @return The {@link TPS} -or- <code>null</code> iff there is no data for the logical row which
   *     satisified the various criteria (the schema, fromTime, toTime, and filter).
   */
  protected static TPS atomicRead(
      final IIndex ndx,
      final byte[] fromKey,
    ...
    final ITupleIterator<?> itr =
        ndx.rangeIterator(fromKey, toKey, 0 /* capacity */, flags, null /* filter */);
    ...
    while (itr.hasNext()) {
      ...
      if (log.isInfoEnabled())
        log.info("Accept: name=" + col + ", timestamp=" + columnValueTimestamp + ", value=" + v);
    }
   ...
   * @return The current binding -or- <code>null</code> iff there is no current binding.
   * @todo this can be optimized by including the encoded column name in the generated [fromKey] and
   *     [toKey] so that we scan less data from the index and by using a reverse traversal iterator
   *     to read the most recent value in the key range first. This is especially important if
   *     timeseries data are being stored.
   */
  protected static ITPV getCurrentValue(
      final IIndex ndx, final Schema schema, final Object primaryKey, final String name) {
