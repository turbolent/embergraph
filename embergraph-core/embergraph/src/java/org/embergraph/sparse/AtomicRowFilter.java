 ...
 *     logical row, or at least all property values that were materialized from the logical row. in
 *     fact, since this is a many:one transform, we must override the remove() impl for the inner
 *     class on {@link TupleTransformer}.
 * @todo You could replace the {@link AtomicRowRead} with this iterator by setting the capacity to
 *     ONE (1). However, that will do more work when we are only trying to read a single row on a
 *     local index since we will have to serialize and then de-serialize the {@link TPS} for that
 *     logical row. (For a remote read the effort should be the same).
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 */
public class AtomicRowFilter extends TupleTransformer<TPV, TPS> implements IRowStoreConstants {
   ...
   * @param nameFilter An optional filter used to select the values for property names accepted by
   *     that filter.
   */
  protected AtomicRowFilter(
      final Schema schema, final long fromTime, final long toTime, final INameFilter nameFilter) {

    super(TPSTupleSerializer.newInstance());

    SparseRowStore.assertArgs(schema, Boolean.TRUE /* fake */, fromTime, toTime);
  ...
  private class Transformerator<E extends TPV /* src */, F extends TPS /* out */>
    ...
    public ITuple<F> next() {
      ...
      final AbstractTuple<F> tuple =
          new AbstractTuple<F>(IRangeQuery.DEFAULT) {
             ...
             *     <p>There are some notes on introducing blob support into the {@link
             *     SparseRowStore} on that class. Basically there would need to be a blob value type
             *     and it would have to carry the UUID for the store from which the blob could be
             *     read as part of the property value stored in a TPS.
             *     <p>Currently, blobs are stored in the same index as the blob reference. In order
             *     to allow blobs to be stored in a different index the name of the scale out index
             *     would have to be in the blob reference.
     ...
     * resulting logical row is set on {@link #current}.
     */
