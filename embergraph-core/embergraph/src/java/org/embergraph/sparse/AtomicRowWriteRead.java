...
public class AtomicRowWriteRead extends AbstractAtomicRowReadOrWrite {
   ...
   * @param propertySet The property set. An entry bound to a <code>null</code> value will cause the
   *     corresponding binding to be "deleted" in the index.
   * @param fromTime <em>During pre-condition and post-condition reads</em>, the first timestamp for
   *     which timestamped property values will be accepted.
   * @param toTime <em>During pre-condition and post-condition reads</em>, the first timestamp for
   *     which timestamped property values will NOT be accepted -or- {@link
   *     IRowStoreConstants#CURRENT_ROW} to accept only the most current binding whose timestamp is
   *     GTE <i>fromTime</i>.
   * @param writeTime The timestamp to be assigned to the property values by an atomic write -or-
   *     {@link IRowStoreConstants#AUTO_TIMESTAMP} if the timestamp will be assigned by the server
   ...
   *
   * @return The set of tuples for the primary key as a {@link TPS} instance -or- <code>null</code>
  ...
  public TPS apply(final IIndex ndx) {
     ...
     */
    final long writeTime = TimestampChooser.chooseTimestamp(ndx, this.writeTime);
    ...
    if (precondition != null) {
      ...
      final TPS tps = atomicRead(ndx, schema, primaryKey, fromTime, toTime, writeTime, filter);
    ...
    try {
      return atomicRead(ndx, schema, primaryKey, fromTime, toTime, writeTime, filter);
    } catch (Throwable t) {
  ...
  protected void atomicWrite(
    ...
    final Iterator<Map.Entry<String, Object>> itr = propertySet.entrySet().iterator();
   ...
   *
   * @throws UnsupportedOperationException if a property has an auto-increment type and the {@link
   *     ValueType} of the property does not support auto-increment.
   * @throws UnsupportedOperationException if there is no successor for the property value.
   */
  protected long inc(
      final IIndex ndx,
      final Schema schema,
    ...
    if (log.isInfoEnabled()) log.info("Auto-increment: name=" + col + ", counter=" + counter);
  ...
  public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    if (log.isInfoEnabled()) log.info("Reading " + n + " property values");
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (log.isInfoEnabled()) log.info("name=" + name + ", value=" + value);
    }
  }
