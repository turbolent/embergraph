 ...
 * @version $Id$
 * @param R The generic type of the object that identifies a resource for the purposes of the
 *     locking system. This is typically the <i>name</i> of an index.
 * @param T The generic type of the return value of the delegate {@link Callable}.
 */
public class LockManagerTask<R extends Comparable<R>, T> implements Callable<T> {
   ...
   *     acquired.
   */
  public LockManagerTask(
      final LockManager<R> lockManager, final R[] resource, final Callable<T> target) {

    if (lockManager == null) throw new NullPointerException();
   ...
   *
   * @exception DeadlockException if the locks could not be acquired (last exception encountered
   *     only).
   * @exception TimeoutException if the locks could not be acquired (last exception encountered
   *     only).
  ...
  public final T call() throws Exception {
    ...
    try {
      ...
      if (INFO) log.info("Acquired locks");
