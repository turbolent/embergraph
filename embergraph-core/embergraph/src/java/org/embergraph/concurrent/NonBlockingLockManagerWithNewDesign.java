 ...
 * a lock. When those locks become available, {@link #ready(Runnable)} will be invoked with the
 * task.
 *
 * <p>The class will use an optional {@link TxDag WAITS_FOR} graph to detect deadlocks if locks are
 * not being pre-declared (and hence deadlocks are possible).
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: NonBlockingLockManagerWithNewDesign.java 4280 2011-03-08 15:06:58Z thompsonbry $
 ...
 * @see #ready(Runnable)
 * @todo 2PL
 *     <p>Note: It is not really possible to have deadlocks without 2PL. Instead, what happens is
 *     that the maximum multi-programming capacity of the {@link TxDag} is temporarily exceeded.
 *     Review the unit tests again when supporting 2PL and verify that the deadlock handling logic
 *     works.
 *     <p>In order to support 2PL we need to decouple the {@link LockFutureTask} from the
 *     transaction with which it is associated and use the latter in the {@link TxDag}. Otherwise
 *     each {@link #submit(Comparable[], Callable)} will always look like a new transaction (2PL is
 *     impossible unless you can execute multiple tasks for the same transaction).
 *     <p>To introduce 2PL I need to create interfaces which extend {@link Callable} which are
 *     understood by this class. One such interface should provide a method by which a task can
 *     release its locks. For 2PL, either that interface or an extension of the interface would need
 *     to have a method by which a task could post new lock requests. Such a callable could be
 *     submitted directly. It would wait if it declared any precondition locks and otherwise pass
 *     through to {@link #ready(Runnable)} immediately. We can't really just pass the task through
 *     again when it requests additional locks, so maybe the Tx would submit a task to the lock
 *     service each time it wanted to gain more or more locks and {@link #ready(Runnable)} would
 ...
 *     guarantee since the VM is not (normally) providing real-time guarantees and since we are not
...
public abstract class NonBlockingLockManagerWithNewDesign</* T, */ R extends Comparable<R>>
   ...
   *
   * <p>Note: The timeout is set to one LBS reporting period so you can see which resource queues
  ...
  /*
   * The maximum #of times that a task whose lock requests would produce a deadlock will be retried.
   * Deadlock is typically transient but the potential for deadlock can vary depending on the
   * application. Note that deadlock CAN NOT arise if you are predeclaring and sorting the lock
   * requests.
   */
  ...
  /*
   * This {@link Runnable} should be submitted to a {@link ScheduledExecutorService} in order to
   * track the average queue size for each active {@link ResourceQueue} and various moving averages
  ...
  /*
   * Lock used to protect changes various state changes, including change to the {@link
   * #serviceRunState}.
   */
  ...
  /*
   * A queue of tasks (a) whose lock requests would have resulted in a deadlock when they were first
   * submitted; -or- (b) which would have exceeded the maximum multi-programming capacity of the
   * {@link TxDag}. These tasks are NOT waiting on their locks.
   *
   * <p>For deadlock, up {@link #maxLockTries} will be made to post the lock requests for a task,
   * including the initial try which is made when the task is accepted by this service. Tasks which
   * are enqueued because the {@link TxDag} was at its multi-programming capacity are retried until
   * they can post their lock requests.
   *
   * <p>Tasks are removed from this queue if their lock requests can be posted without creating a
  ...
  enum ServiceRunState {
     ...
     * queued [tasksCancelled] is never considered in this run state.)
     */
    ...
    /*
     * @param val The integer value for this run state.
     * @param tasksAccepted <code>true</code> iff new tasks are accepted and will be eventually
     *     executed while in this run state.
    ...
    /*
     * Return <code>true</code> iff new tasks are accepted and will be eventually executed while in
     * this run state.
     */
    ...
    /*
     * Return <code>true</code> iff tasks (whether accepted, waiting for their locks, or executed)
     * are cancelled while in this run state.
     */
    ...
    /*
     * Return <code>true</code> iff the transition the specified run state is legal.
     *
     * @param newval The new run state.
  ...
  /*
   * Note: You MUST submit {@link #statisticsTask} to a {@link ScheduledExecutorService} in order
   * counter values which report moving averages to be maintained.
   *
   * <p>Note: A new instance is returned every time. This makes the pattern where the counters are
   * "attached" to a hierarchy work since that has the side-effect of "detaching" them from the
   * returned object.
   */
  //    synchronized
  public CounterSet getCounters() {
     ...
     */
    root.addCounter(
        "averageRunningWithLocksHeldCount",
        new Instrument<Double>() {
          public void sample() {
            setValue(statisticsTask.nrunningWithLocksHeldAverageTask.getMovingAverage());
          }
        });
     ...
     */
    {
      ...
      while (itr.hasNext()) {
        ...
        tmp.addCounter(
            queue.resource.toString(), new WeakRefResourceQueueInstrument(queue.resource));
      }
    }
  ...
  /*
   * A class that dynamically resolves the {@link ResourceQueue} by its resource from {@link
   * NonBlockingLockManagerWithNewDesign#resourceQueues}. If there is a {@link ResourceQueue} for
   * that resource then the moving average of its queue size is reported. Otherwise, ZERO (0d) is
   * reported as the moving average of its queue size.
   */
  private class WeakRefResourceQueueInstrument extends Instrument<Double> {
    ...
    public void sample() {
      ...
      if (queue == null) {
        ...
        final double averageQueueSize = queue.statisticsTask.getAverageQueueSize();
  ...
  protected static class Counters {
    ...
    /*
     * The #of tasks that are currently on the retry queue awaiting an opportunity to post their
     * lock requests that does not result in a deadlock or exceed the multi-programming capacity of
     * the optional {@link TxDag}.
     */
    public int nretry;
    ...
    /*
     * #of tasks are currently executing (in {@link LockFutureTask#run()}) AND are holding their
     * locks.
     */
   ...
   *
   * @param maxConcurrency The maximum multi-programming level (ignored if <i>predeclareLocks</i> is
   *     <code>true</code>).
   * @param maxLockTries The maximum #of times that a task whose lock requests would produce a
   *     deadlock will be retried. Deadlock is typically transient but the potential for deadlock
   *     can vary depending on the application. Note that deadlock CAN NOT arise if you are
   *     predeclaring and sorting the lock requests.
   * @param predeclareLocks When <code>true</code>, operations MUST declare all locks before they
   *     begin to execute. This makes possible several efficiencies and by sorting the resources in
   *     each lock request into a common order we are able to avoid deadlocks entirely.
   */
  public NonBlockingLockManagerWithNewDesign(
      final int maxConcurrency, final int maxLockTries, final boolean predeclareLocks) {

    this(maxConcurrency, maxLockTries, predeclareLocks, true /* sortLockRequests */);
  }
  ...
  /*
   * Method invoked when a task is ready to execute holding any locks which it declared to {@link
   * #submit(Comparable[], Callable)} or {@link #submit(Comparable[], Runnable, Object)}. The
   * implementation will normally submit the {@link Runnable} to an {@link Executor}. The {@link
   * Runnable} wraps the original task and the task will automatically release its locks when it is
   * done executing.
   *
   * <p>Note: Implementations SHOULD NOT cause the {@link Runnable} to execute in the caller's
   * thread. That will cause this service to block while the task is executing. The implementation
   * can safely submit the task to a {@link ThreadPoolExecutor} whose work queue is a {@link
   * SynchronousQueue} as long as the the {@link ThreadPoolExecutor} has an unbounded pool size.
   * Another option is to submit the task to a {@link ThreadPoolExecutor} whose work queue is
   * unbounded queue, such as {@link LinkedBlockingQueue} when no queue capacity was specified. The
   * {@link SynchronousQueue} may be the better choice since the {@link ResourceQueue}s already
   * provide an unbounded queue and the actual concurrency of the delegate will be bounded by the
   * #of distinct resources for which tasks are actively contending for locks. See the discussion on
   * queues at {@link ThreadPoolExecutor}.
   *
   * @param task The {@link Callable} or {@link Runnable} wrapped up as a {@link LockFutureTask}.
   */
  ...
  public void shutdown() {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("Waiting for service shutdown.");
      service.awaitTermination(Long.MAX_VALUE, TimeUnit.MILLISECONDS);
      if (log.isInfoEnabled()) log.info("Service is shutdown.");
  ...
  private final void setServiceRunState(final ServiceRunState newval) {
    ...
    if (!serviceRunState.isTransitionLegal(newval)) {
      ...
      throw new IllegalStateException("runState=" + serviceRunState + ", but newValue=" + newval);
    }

    if (serviceRunState != newval) {

      if (INFO) log.info("Set runState=" + newval);
   ...
   * outcome.
   *
   * @param resource An array of resources whose locks are required to execute the <i>task</i>.
   * @param task The task to be executed.
   * @throws IllegalArgumentException if <i>resource</i> is <code>null</code> or if any element of
   *     that array is <code>null</code>.
   * @throws IllegalArgumentException if the <i>task</i> is <code>null</code>.
   * @throws RejectedExecutionException if the task can not be queued for execution (including if
   *     the service is not running or if a blocking queue was used and the queue is at capacity).
   */
  public <T> FutureTask<T> submit(final R[] resource, final Callable<T> task) {

    if (serviceRunState != ServiceRunState.Running) throw new RejectedExecutionException();
    ...
    if (maxLockTries <= 0) throw new IllegalArgumentException();
   ...
   *
   * @param resource The declared locks.
   * @param task The {@link Runnable} target.
   * @param val The value to be returned by the {@link Future}.
   * @return The {@link Future} for that task.
   * @param <T> The generic type of the value which will be returned by the {@link Future}.
   * @throws IllegalArgumentException if <i>resource</i> is <code>null</code> or if any element of
   *     that array is <code>null</code>.
   * @throws IllegalArgumentException if the <i>task</i> is <code>null</code>.
   * @throws RejectedExecutionException if the task can not be queued for execution (including if
   *     the service is not running or if a blocking queue was used and the queue is at capacity).
   */
  public <T> FutureTask<T> submit(final R[] resource, final Runnable task, final T val) {
  ...
  /*
   * If there is a task holding ALL of the specified locks then its locks are released. This is
   * intended to support workflows in which the task can release its locks before {@link
   * Runnable#run()} is finished.
   *
   * @param resource[] The declared locks for the task.
   * @throws IllegalStateException if there is no task which holds all the declared locks.
   */
  ...
  public Runnable getTaskWithLocks(final R[] resource) {
    ...
    try {
       ...
       * There is no ResourceQueue for this resource so there can
          ...
          if (DEBUG) log.debug("No task holds this lock: " + r);
       ...
       * verify that the task holds the rest of its declared
          ...
          if (task != resourceQueue.queue.peek()) {
            ...
            if (DEBUG) log.debug("Task holding the other locks does not hold this lock: " + r);
      ...
      if (task.taskRunState != TaskRunState.RunningWithLocks) {
        ...
        throw new IllegalStateException("taskRunState=" + task.taskRunState);
      }
  ...
  enum TaskRunState {
    ...
    /** Task has issued its lock requests and now holds all locks but is not yet running. */
    ...
    /*
     * The task is halted (absorbing state). A task in this run state: (1) IS NOT holding any locks;
     * (2) DOES NOT have any lock requests posted in the {@link ResourceQueue}s; and (3) DOES NOT
     * have any edges or vertices in the {@link TxDag}.
     *
     * <p>A transition to this state is triggered by {@link Future#cancel(boolean)}, by {@link
     * LockFutureTask#setException(Throwable)}, or when {@link LockTaskFuture#run()} completes.
     */
    ...
    public boolean isTransitionLegal(final TaskRunState newval) {
      ...
      if (this == New) {
        ...
        if (newval == Retry) return true;

        // allows task to directly request locks.
        if (newval == LocksRequested) return true;
    ...
    /*
     * @param val The integer value corresponding to the run state.
     * @param lockRequestsPosted <code>true</code> iff a task has posted lock requests in each
    ...
    /*
     * Return <code>true</code> iff a task has posted lock requests in each {@link ResourceQueue}
     * corresponding to a lock declared by that task.
     */
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @param <T> The generic type of the outcome for the {@link Future}.
   */
  public static class LockFutureTask<R extends Comparable<R>, T> extends FutureTaskMon<T> {
     ...
     *
     * <p>Note: I ran into compilation errors on 64-bit systems when making references to non-static
     * inner classes so this is now an explicit ctor parameter.
     */
    ...
    /*
     * Incremented each time a deadlock is detected. We will not retry if {@link #maxLockTries}
     * would be exceeded.
     */
    private int ntries = 0;

    /*
     * The timestamp in nanoseconds when this task was accepted for eventual execution. This is used
     * to determine the {@link #lockWaitingTime}.
     */
     ...
     *
     * <p>Note: This collection is required in order for the {@link ResourceQueue}s for which the
     * task has asserted a lock request to remain strongly reachable. Without such hard references
     ...
     *
     * @param newval The new run state.
     * @throws IllegalStateException if the state transition is not legal.
     */
    private void setTaskRunState(final TaskRunState newval) {

      if (!lockService.lock.isHeldByCurrentThread()) throw new IllegalMonitorStateException();

      if (newval == null) throw new IllegalArgumentException();

      if (!taskRunState.isTransitionLegal(newval)) {
        log.error("Illegal state change: current=" + taskRunState + ", newval=" + newval);
        throw new IllegalStateException("current=" + taskRunState + ", newval=" + newval);
      }
    ...
    /*
     * Request any locks declared by the task. If the locks are available then submit the task for
     * execution immediately.
     *
     * @return <code>true</code> if the task should be removed from {@link
     *     NonBlockingLockManagerWithNewDesign#retryQueue} by the caller.
     */
    private boolean requestLocks() {
      ...
      try {
        ...
        if (lockService.waitsFor != null) nvertices = lockService.waitsFor.size();
       ...
       * Note: When TxDag is used we MUST NOT add the lock
          ...
          if (INFO) log.info("Maximum multi-programming capacity.");
       ...
       * Post the lock requests.
        ...
        if (lockService.postLockRequests(this)) {
          // run now.
          if (INFO) log.info("Task is ready to run: " + this);
       ...
       * Note: DeadlockException is the ONLY expected exception
          ...
          if (INFO) log.info("Deadlock: " + this + ", task=" + this /* , ex */);

          if (++ntries < lockService.maxLockTries) {

            // leave on queue to permit retry.

            if (INFO) log.info("Will retry task: " + this);
       ...
       * Paranoia check to make sure that we did not leave
          ...
          if (nvertices != nafter) {
            throw new AssertionError("#vertices: before=" + nvertices + ", after=" + nafter);
          }
        }
    ...
    /*
     * Note: We do not need to remove the task from the readyQueue when it an exception is set on
     * it. When the task is pulled from the queue by an executor service, the task will be marked as
     * cancelled and its run() method will be a NOP.
     */
    @Override
    ...
    /*
     * Note: We do not need to remove the task from the readyQueue when it is cancelled. When the
     * task is pulled from the queue by an executor service, the task will be marked as cancelled
     * and its run() method will be a NOP.
     */
    @Override
    ...
    public void run() {
      ...
      if (callersTask instanceof AbstractTask
        ...
        ((WriteTaskCounters) ((AbstractTask) callersTask).getTaskCounters())
            .lockWaitingNanoTime.addAndGet(lockWaitingTime);
      }
      ...
      if (DEBUG) log.debug("Running: " + this);
       ...
       * Note: FutureTask asynchronously reports the result back to
        ...
        try {
          if (taskRunState.isRunning()) {
            if (DEBUG) log.debug("Did run: " + this);
            lockService.counters.nrunning--;
            setTaskRunState(TaskRunState.Halted);
          }
        } finally {
...
//         * Releases the locks and changes to {@link TaskRunState#Halted}.
  ...
  /*
   * {@link Runnable} drains the {@link #retryQueue} queue and manages service state changes.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  private static class AcceptTask<R extends Comparable<R>> implements Runnable {
    ...
    public void run() {
       ...
       * Cancel all tasks, clearing all queues.
              ...
              lockManager.lock.lock();
              try {
                if (INFO) log.info(lockManager.serviceRunState);
    ...
    /*
     * IFF there is no work that could be performed and we are in the expected {@link
     * ServiceRunState} then this blocks until someone signals {@link
     * NonBlockingLockManagerWithNewDesign#stateChanged}.
     *
     * @see LockFutureTask
     * @see NonBlockingLockManagerWithNewDesign#submit(Comparable[], Callable)
     */
    private void awaitStateChange(final ServiceRunState expected) {
       ...
       * While we hold the lock we verify that there really is no work
        ...
        if (INFO) log.info("Waiting...");
        lockManager.stateChanged.await();
        if (INFO) log.info("Woke up...");
     ...
     *
     * @return <code>true</code> iff any tasks were removed from this queue.
     */
    private boolean processRetryQueue() {
      ...
      if (INFO && nchanged > 0) log.info("#nchanged=" + nchanged);
   ...
   *
   * @param task The task.
   * @return <code>true</code> if the task is at the head of the queue for each lock declared for
   *     that task (e.g., if the task can execute immediately).
   * @throws DeadlockException If the lock request(s) would cause a deadlock.
   * @throws IllegalStateException If locks are being predeclared and there are already locks held
   *     by the operation.
   */
  private <T> boolean postLockRequests(final LockFutureTask<R, T> task) throws DeadlockException {
    ...
    if (DEBUG) log.debug("Acquiring lock(s): " + Arrays.toString(task.resource));
   ...
   */
  private <T> void releaseLocksForTask(final LockFutureTask<R, T> t, final boolean waiting) {

    if (!lock.isHeldByCurrentThread()) throw new IllegalMonitorStateException();

    if (DEBUG) log.debug("Releasing locks: " + t);
    ...
    try {
       ...
       * Remove lock request from resource queue
        ...
        if (!resourceQueue.queue.remove(t)) {
          ...
          log.error(
              "Lock request not found: resource=" + resourceQueue.getResource() + ", task=" + t);
        }
     ...
     */
    {
      ...
      while (itr.hasNext()) {
        ...
        if (task != null
          ...
          if (INFO) log.info("Task is ready to run: " + task);
  ...
  /*
   * Return <code>true</code> if the lock is held by the task at the moment when it is inspected.
   *
   * @param lock The lock.
   * @param task The task.
  ...
  public String toString() {
    ...
    return getClass().getName()
        ...
        + counters.ncancel
        + ", averageDeadlock="
        + ((int) (10 * statisticsTask.nretryAverageTask.getMovingAverage()) / 10d)
        + ", averageWaiting="
        + ((int) (10 * statisticsTask.nwaitingAverageTask.getMovingAverage()) / 10d)
        + ", averageReady="
        + ((int) (10 * statisticsTask.nreadyAverageTask.getMovingAverage()) / 10d)
        + ", averageRunning="
        + ((int) (10 * statisticsTask.nrunningAverageTask.getMovingAverage()) / 10d)
        + ", averageRunningWithLocksHeld="
        + ((int) (10 * statisticsTask.nrunningWithLocksHeldAverageTask.getMovingAverage()) / 10d)
        + ", #maxrunning="
        + counters.maxRunning
        +
  ...
  protected static class ResourceQueue<
     ...
     */
    private final BlockingQueue<T /* tx */> queue;
    ...
    public ResourceQueue(
        final NonBlockingLockManagerWithNewDesign<R> lockService, final R resource) {
  ...
  /*
   * Class for tracking the average queue size of each {@link ResourceQueue} and various other
   * moving averages for the service as a whole.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
  ...
  protected class StatisticsTask implements Runnable {
     ...
     * #nqueueBusyAverageTask}. This is set by {@link #run()}
     */
    ...
    /*
     * This method updates the {@link QueueSizeMovingAverageTask} for each {@link ResourceQueue}
     * each time it is run.
     *
     * <p>Note: This is written so as to not cause hard references to be retained to the {@link
     * ResourceQueue}s. The {@link QueueSizeMovingAverageTask} is a member field for the {@link
     * ResourceQueue} for the same reason. This way the statistics for active {@link ResourceQueue}s
     * are tracked and may be reported but {@link ResourceQueue}s will remain strongly reachable
     * only if there are tasks holding their locks.
     */
    public void run() {
