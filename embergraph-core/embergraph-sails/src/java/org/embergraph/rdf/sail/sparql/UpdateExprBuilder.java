...
import org.embergraph.rdf.sparql.ast.CreateEntailments;
import org.embergraph.rdf.sparql.ast.CreateGraph;
import org.embergraph.rdf.sparql.ast.DeleteData;
import org.embergraph.rdf.sparql.ast.DeleteInsertGraph;
import org.embergraph.rdf.sparql.ast.DisableEntailments;
import org.embergraph.rdf.sparql.ast.DropEntailments;
import org.embergraph.rdf.sparql.ast.DropGraph;
import org.embergraph.rdf.sparql.ast.EnableEntailments;
...
public class UpdateExprBuilder extends EmbergraphExprBuilder {
   ...
   */
  @Override
  public InsertData visit(final ASTInsertData node, final Object data) throws VisitorException {

    final InsertData op = new InsertData();

    // variables are disallowed within DELETE DATA (per the spec).
    op.setData(
        doUnparsedQuadsDataBlock(node, data, false /* allowVars */, true /* allowBlankNodes */));
   ...
   */
  @Override
  public DeleteData visit(final ASTDeleteData node, final Object data) throws VisitorException {
    ...
    op.setData(
        doUnparsedQuadsDataBlock(node, data, false /* allowVars */, false /* allowBlankNodes */));

    return op;
  }

  @Override
  public QuadData visit(final ASTQuadsNotTriples node, final Object data) throws VisitorException {

    final GroupGraphPattern parentGP = graphPattern;

    graphPattern = scopedGroupGraphPattern(node);

    final TermNode contextNode = (TermNode) node.jjtGetChild(0).jjtAccept(this, data);
   ...
   * </pre>
   *
   * This is similar to a CONSTRUCT without an explicit template. The WHERE clause provides both the
   * pattern to match and the template for the quads to be removed.
   *
   * <p>Note: The grammar production for the WHERE clause for the DELETE WHERE shortcut form is the
   * same production that is used for the DELETE and INSERT clauses. This results in a {@link
   * QuadData} object containing {@link StatementPatternNode}s. That object must be transformed into
   * a {@link JoinGroupNode} in order to be valid as a WHERE clause.
   *
  ...
  @Override
  public LoadGraph visit(final ASTLoad node, final Object data) throws VisitorException {

    final ConstantNode sourceGraph = (ConstantNode) node.jjtGetChild(0).jjtAccept(this, data);

    final LoadGraph op = new LoadGraph();

    op.setSourceGraph(sourceGraph);

    if (node.isSilent()) op.setSilent(true);

    if (node.jjtGetNumChildren() > 1) {

      final ConstantNode targetGraph = (ConstantNode) node.jjtGetChild(1).jjtAccept(this, data);
   ...
   */
  @Override
  public ClearGraph visit(final ASTClear node, final Object data) throws VisitorException {
    ...
    if (graphRef.jjtGetNumChildren() > 0) {
      ...
      final TermNode targetGraph = (TermNode) graphRef.jjtGetChild(0).jjtAccept(this, data);
   ...
   */
  @Override
  public DropGraph visit(final ASTDrop node, final Object data) throws VisitorException {
  ...
  @Override
  public CreateGraph visit(final ASTCreate node, final Object data) throws VisitorException {

    final TermNode target = (TermNode) node.jjtGetChild(0).jjtAccept(this, data);
    ...
    if (target instanceof ConstantNode) {
      ...
      final EmbergraphStatement[] params =
          doQuadsData(node, data, false /*allowVars*/, true /* allowBlankNodes */);
  ...
  @Override
  public CopyGraph visit(final ASTCopy node, final Object data) throws VisitorException {

    final CopyGraph op = new CopyGraph();

    if (node.isSilent()) op.setSilent(true);

    final ASTGraphOrDefault sourceNode = (ASTGraphOrDefault) node.jjtGetChild(0);

    if (sourceNode.jjtGetNumChildren() > 0) {

      final ConstantNode sourceGraph =
          (ConstantNode) sourceNode.jjtGetChild(0).jjtAccept(this, data);

      op.setSourceGraph(sourceGraph);
    }

    final ASTGraphOrDefault destinationNode = (ASTGraphOrDefault) node.jjtGetChild(1);

    if (destinationNode.jjtGetNumChildren() > 0) {

      final ConstantNode targetGraph =
          (ConstantNode) destinationNode.jjtGetChild(0).jjtAccept(this, data);
  ...
  @Override
  public MoveGraph visit(final ASTMove node, final Object data) throws VisitorException {
    ...
    final ASTGraphOrDefault targetNode = (ASTGraphOrDefault) node.jjtGetChild(1);

    if (targetNode.jjtGetNumChildren() > 0) {

      final ConstantNode targetGraph =
          (ConstantNode) targetNode.jjtGetChild(0).jjtAccept(this, data);
  ...
  @Override
  public AddGraph visit(final ASTAdd node, final Object data) throws VisitorException {
   ...
   */
  @Override
  public DeleteInsertGraph visit(final ASTModify node, final Object data) throws VisitorException {
    ...
    ConstantNode with = null;
    {
      ...
      if (withNode != null) {
        ...
        // moved to org.embergraph.rdf.sail.sparql.ASTDeferredIVResolution.fillInIV(AST2BOpContext,
        // BOp)
...

    {
      ...
      if (whereClause != null) {
        ...
        final GraphPatternGroup<IGroupMemberNode> ret =
            (GraphPatternGroup<IGroupMemberNode>) whereClause.jjtAccept(this, null /* data */);
...

    {
      ...
      if (deleteNode != null) {
        ...
        if (deleteNode.getName() == null) {
          ...
          final QuadData quadData = (QuadData) deleteNode.jjtAccept(this, data);
          ...
          final QuadsDataOrNamedSolutionSet deleteClause =
              new QuadsDataOrNamedSolutionSet(deleteNode.getName());
...

    {
      ...
      if (insertNode != null) {
        ...
        if (insertNode.getName() == null) {
          ...
          // This is a QuadPattern (versus ground data)
          final QuadData quadData = (QuadData) insertNode.jjtAccept(this, data);
          ...
          final QuadsDataOrNamedSolutionSet insertClause =
              new QuadsDataOrNamedSolutionSet(insertNode.getName());
  ...
  @Override
  public QuadData visit(final ASTDeleteClause node, final Object data) throws VisitorException {

    return doQuadsPatternClause(node, data, false /* allowBlankNodes */);
  }

  @Override
  public QuadData visit(final ASTInsertClause node, final Object data) throws VisitorException {

    return doQuadsPatternClause(node, data, true /* allowBlankNodes */);
  }

  private EmbergraphStatement[] doUnparsedQuadsDataBlock(
      final ASTUpdate node,
      final Object data,
      final boolean allowVars,
      final boolean allowBlankNodes)
      throws VisitorException {
    ...
    // @see https://jira.blazegraph.com/browse/BLZG-1176
    // moved to org.embergraph.rdf.sail.sparql.ASTDeferredIVResolution.fillInIV(AST2BOpContext, BOp)
    //        if (!context.tripleStore.isQuads()) {
   ...
   *
   * <p>Note: The QuadData is basically modeled by the embergraph AST as recursive {@link
   * StatementPatternNode} containers. This visits the parser AST nodes and then flattens them into
   * an {@link ISPO}[]. The {@link ISPO}s are {@link EmbergraphStatement}s at this point, but they
   * can be converted to {@link SPO}s when the INSERT DATA or DELETE DATA operation runs.
   *
   * @return A flat {@link ISPO}[] modeling the quad data.
   * @throws VisitorException
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/573">NullPointerException when
   *     attempting to INSERT DATA containing a blank node </a>
   */
  private EmbergraphStatement[] doQuadsData(
      final ASTUpdate node,
      final Object data,
      final boolean allowVars,
      final boolean allowBlankNodes)
   ...
   */
  private void assertNotAnonymousVariable(final TermNode t) throws VisitorException {
    ...
    if (t.isVariable()) {
      ...
      if (iv.isBNode()) throw new VisitorException("BlankNode not permitted in this context: " + t);
    }
  }
   ...
   * DATA (DELETE DATA does not allow blank nodes).
   *
   * @param t The {@link TermNode}.
  ...
  private EmbergraphValue toValue(final TermNode t) {
    if (t.isVariable() && ((VarNode) t).isAnonymous()) {
      final EmbergraphBNode s = context.valueFactory.createBNode(t.getValueExpression().getName());
      return s;
    }
    return t.getValue();
  }
   ...
   *
   * @param data
   * @return The {@link QuadData} (aka template). The {@link QuadData} as returned by this method is
   *     not flattened, but the context has been applied within any GRAPH section.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/571">DELETE/INSERT WHERE
   *     handling of blank nodes </a>
   */
  private QuadData doQuadsPatternClause(
      final Node node, final Object data, final boolean allowBlankNodes) throws VisitorException {
