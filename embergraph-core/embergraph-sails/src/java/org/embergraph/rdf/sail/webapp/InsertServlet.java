...
public class InsertServlet extends EmbergraphRDFServlet {
   ...
   * </pre>
   *
   * <p>Where <code>BODY</code> is the new RDF content using the representation indicated by the
   ...
   * </pre>
   *
   * <p>Where <code>URI</code> identifies a resource whose RDF content will be inserted into the
   * database. The <code>uri</code> query parameter may occur multiple times. All identified
   * resources will be loaded within a single native transaction. Embergraph provides snapshot
   * isolation so you can continue to execute queries against the last commit point while this
   ...
   * @throws Exception
   */
  private void doPostWithBody(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (log.isInfoEnabled()) log.info("RDFFormat=" + format);
  ...
  private static class InsertWithBodyTask extends AbstractRestApiTask<Void> {
    ...
    public Void call() throws Exception {
      ...
      try {
         ...
         *
         * @see <a href="http://trac.blazegraph.com/ticket/991" >REST API: INSERT does not handle
         *     .gz</a>
         */
   ...
   * @throws Exception
   */
  private void doPostWithURIs(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (uris == null || uris.length == 0) {
      ...
      buildAndCommitResponse(
          ...
          MIME_TEXT_PLAIN,
          "Parameter must be specified one or more times: '" + EmbergraphRDFContext.URI + "'");
    ...
    try {
      ...
      submitApiTask(
              new InsertWithURLsTask(req, resp, namespace, ITx.UNISOLATED, defaultContext, urls))
          .get();
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        for (URL url : urls) {
          ...
          //                        final Resource defactoContext = defaultContext == null ? new
          // URIImpl(
          ...
          try {
             ...
             *     <p>FIXME This does not handle .gz or .zip files. We handle this in the
             * @see <a href="http://trac.blazegraph.com/ticket/991" >REST API: INSERT does not
             *     handle .gz</a>
             */
            ...
            rdfParser.parse(is, url.toExternalForm() /* baseURL */);
  ...
  static class AddStatementHandler extends RDFHandlerBase {
    ...
    public void handleStatement(final Statement stmt) throws RDFHandlerException {
      ...
      try {
        ...
        conn.addStatement(stmt.getSubject(), stmt.getPredicate(), stmt.getObject(), c);
