...
public class BackupServlet extends EmbergraphRDFServlet {
   ...
   * to &quot;backup.jnl&quot;.
   *
   * <p>The file must be writable and must not exist. If the file is passed with a relatively
   * filename, it will be converted to an absolute path.
   */
   ...
   *
   * <p>If it is set to false, the backup will not be available until the {@link SnapshoTask} is
   * completed.
   */
  ...
  protected void processRequest(HttpServletRequest req, HttpServletResponse res)
    ...
    {
      final BasicSnapshotFactory snapfact = new BasicSnapshotFactory(file, compress);

      if (debug) {
        log.debug("Snapshot requested.  Writing backup to " + snapfact.getFile());
      }

      log.warn("Snapshot requested.  Writing backup to " + snapfact.getFile());

      final Future<ISnapshotResult> f = ((Journal) getIndexManager()).snapshot(snapfact);
      ...
      if (block) {
        ...
        try {
          ...
          if (debug) {
            log.debug("Snapshot completed at " + r.getFile().getAbsolutePath());
          }
    ...
    if (hasError) {
      buildAndCommitResponse(
          res, HTTP_INTERNALERROR, MIME_TEXT_PLAIN, errorMessage.toString() + "\n");
    } else {
