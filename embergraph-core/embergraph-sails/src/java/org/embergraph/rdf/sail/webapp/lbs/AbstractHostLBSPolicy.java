...
public abstract class AbstractHostLBSPolicy extends AbstractLBSPolicy {
  ...
  public interface InitParams extends AbstractLBSPolicy.InitParams {
     ...
     * derived class).
     *
     * <p>The purpose of the {@link IHostScoringRule} is to compute a single workload number based
     * on those host metrics. The resulting scores are then normalized. Load balancing decisions are
     * made based on those normalized scores.
     *
     * <p>Note: The default policy is specific to the concrete instance of the outer class.
     */
     ...
     * the hosts having services that are joined with the met quorum. This may be set to ONE (1.0)
     * to disable this bias. The default is {@value #DEFAULT_LOCAL_FORWARD_THRESHOLD}.
     *
     * <p>This bias is designed for use when an external round-robin policy is distributing the
     * requests evenly across the services. In this case, the round-robin smooths out most of the
     * workload and the {@link IHALoadBalancerPolicy} takes over only when there is a severe
     * workload imbalance (as defined by the value of this parameter).
     *
     * <p>For example, if you have 3 hosts and they are equally available, then their normalized
     * availability scores will be <code>.3333</code>. If the score for a given host is close to
     * this normalized availability, then a local forward is a reasonable choice.
     *
     * <p>TODO In fact, we could automatically compute and use a reasonable value based on the
     * quorum size as <code>ceil((1/replicationFactor)-.01)</code>. With this approach, the local
     * forward bias is automatic. However, we still only want to do this if there is a round-robin
     * over the services. Otherwise we will slam this host whenever its load gets below the
     * threshold while not assigning any work to the other hosts until the next update of the {@link
     * HostTable}.
     */
  ...
  private static final RuntimeException CAUSE_NO_HOST_SELECTED =
      new RuntimeException("No host selected for request.");

  /** The load balancing logic failed to select a service to handle the request. */
   ...
   *
   * <p>Note: The policy needs to be specific to the LBS implementation since the names of the host
   * metrics depend on the system that is being used to collect and report them.
   */
  ...
  @Override
  public void init(final ServletConfig servletConfig, final IIndexManager indexManager)
    ...
    {
      final String s =
          HALoadBalancerServlet.getConfigParam(
              servletConfig,
      ...
      if (log.isInfoEnabled()) log.info(InitParams.LOCAL_FORWARD_THRESHOLD + "=" + d);
    ...
    {
      scoringRuleRef.set(
          HALoadBalancerServlet.newInstance(
              servletConfig,
              AbstractHostLBSPolicy.class, // owningClass
      ...
      if (log.isInfoEnabled())
        log.info(InitParams.HOST_SCORING_RULE + "=" + scoringRuleRef.getClass().getName());
    }

    {
      ...
      if (log.isInfoEnabled()) log.info(InitParams.HOST_DISCOVERY_DELAY + "=" + hostDiscoveryDelay);
    }
  ...
  protected void conditionallyUpdateServiceTable() {
    ...
    final HostScore[] hostScores = hostTable == null ? null : hostTable.hostScores;
  ...
  protected void updateHostTable() {
    ...
    final Map<String /* hostname */, IHostMetrics> hostMetricsMap =
        getHostReportForKnownServices(scoringRule, serviceScores);
    ...
    if (log.isTraceEnabled()) log.trace("hostMetricsMap=" + hostMetricsMap);

    final HostTable newHostTable = normalizeHostScores(scoringRule, hostMetricsMap);

    if (log.isTraceEnabled()) log.trace("newHostTable=" + newHostTable);
   ...
   * members. If we do it for the other hosts then the normalization is not meaningful since we will
   * only load balance across the services that are joined with a met {@link Quorum}.
   *
   * @param scoringRule The {@link IHostScoringRule} used to integrate the per-host performance
   *     metrics.
   * @param hostMetricsMap The per-host performance metrics for the known hosts.
  ...
  private static HostTable normalizeHostScores(
...

    {
      ...
      for (Map.Entry<String, IHostMetrics> e : hostMetricsMap.entrySet()) {
        ...
        double hostScore = metrics == null ? NO_INFO : scoringRule.getScore(metrics);
    ...
    final HostScore[] scores = new HostScore[nhosts];
    {
      for (int i = 0; i < nhosts; i++) {
        ...
        if (totalAvailability == 0) {
          ...
          normalizedAvailability = availability[i] / totalAvailability;
        }

        // Normalize host scores.
        final HostScore hostScore = scores[i] = new HostScore(hostname, normalizedAvailability);
  ...
  public String getReaderURI(final HttpServletRequest req) {
    ...
    final ServiceScore serviceScore = getService(rand, hostScore, serviceScores);
   ...
   *
   * @param d A random number in the half-open [0:1).
   * @param hostScores The {@link HostScore}s.
   * @return The {@link HostScore} of the host to which a read request should be proxied -or- <code>
   *     null</code> if the request should not be proxied (because we lack enough information to
   *     identify a target host).
   * @see embergraph/src/resources/architecture/HA_LBS.xls
   */
  static HostScore getHost(final double d, final HostScore[] hostScores) {

    if (d < 0 || d >= 1d) throw new IllegalArgumentException();

    if (hostScores == null) throw new IllegalArgumentException();
   ...
   * Normal deployment allocates only one service per host.
   *
   * <p>Note: This is package private in order to expose it to the test suite.
   *
   * @param rand A random number generator.
   * @param hostScore The {@link HostScore} of the selected host.
   * @param serviceScores The {@link ServiceScore}s for the joined services.
   * @return The {@link ServiceScore} of a service on the host identified by the caller to which a
   *     read request should be proxied -or- <code>null</code> if the request should not be proxied
   *     (because we lack enough information to identify a target service on that host).
   *     <p>TODO Optimize the lookup of the services on a given host. The trick is making sure that
   *     this mapping remains consistent as services join/leave. However, the normal case is one
   *     service per host. For that case, this loop wastes the maximum effort since it scans all
   *     services.
   */
   ...
   * the request to the local service. This help to reduce the latency of the request since it is
   * not being proxied.
   *
   * @see InitParams#LOCAL_FORWARD_THRESHOLD
   */
  ...
  protected boolean conditionallyForwardReadRequest(
    ...
    final HostScore thisHostScore = hostTable == null ? null : hostTable.thisHost;

    if (thisHostScore != null
        && thisHostScore.getAvailability() >= localForwardThresholdRef.get()) {

      servlet.forwardToLocalService(false /* isLeaderRequest */, request, response);
   ...
   *
   * @param scoringRule The {@link IHostScoringRule} to be applied.
   * @param serviceScores The set of known services.
   * @return The map.
   *     <p>TODO If there is more than one service on the same host, then we will have one record
   *     per host, not per service. This means that we can not support JVM specific metrics, such as
   *     GC time. This could be fixed if the map was indexed by Service {@link UUID} and the host
   *     metrics were combined into the map once for each service.
   */
  protected abstract Map<String, IHostMetrics> getHostReportForKnownServices(
