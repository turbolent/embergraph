 ...
 */
public class EmbergraphRDFServletContextListener implements ServletContextListener {

  private static final transient Logger log =
      Logger.getLogger(EmbergraphRDFServletContextListener.class);
   ...
   * overrides as an attribute and then consult them from within {@link
   * EmbergraphRDFServletContextListener#contextInitialized(javax.servlet.ServletContextEvent)} .
   */
   ...
   *
   * @param key The name of the desired init parameter.
  ...
  public EmbergraphRDFServletContextListener() {
    super();
  }

  protected EmbergraphRDFContext getEmbergraphRDFContext() {
  ...
  public void contextInitialized(final ServletContextEvent e) {
     ...
     */
    {
       ...
       */
      {
        ...
        final Map<String, String> initParamOverrides =
            (Map<String, String>)
                context.getAttribute(EmbergraphRDFServletContextListener.INIT_PARAM_OVERRIDES);
    ...
    final String namespace;
    {
      ...
      if (s == null) s = ConfigParams.DEFAULT_NAMESPACE;

      namespace = s;

      if (log.isInfoEnabled()) log.info(ConfigParams.NAMESPACE + "=" + namespace);
    }
    ...
    final boolean create;
    {
      ...
      if (s != null) create = Boolean.valueOf(s);
      else create = ConfigParams.DEFAULT_CREATE;

      if (log.isInfoEnabled()) log.info(ConfigParams.CREATE + "=" + create);
    }
    ...
    if (context.getAttribute(IIndexManager.class.getName()) != null) {
      ...
      final String FQN_PROPERTY_FILE =
          ConfigParams.class.getName() + "." + ConfigParams.PROPERTY_FILE;

      // The default value is taken from the web.xml file.
      final String defaultValue = getInitParameter(ConfigParams.PROPERTY_FILE);

      // The effective location of the property file.
      final String propertyFile = System.getProperty(FQN_PROPERTY_FILE, defaultValue);

      if (propertyFile == null)
        throw new RuntimeException("Required config-param: " + ConfigParams.PROPERTY_FILE);

      if (log.isInfoEnabled()) log.info(ConfigParams.PROPERTY_FILE + "=" + propertyFile);
    ...
    if (create) {
      ...
      if (indexManager instanceof IEmbergraphFederation) {
        ...
        properties = ((IEmbergraphFederation<?>) indexManager).getClient().getProperties();
      } else {
    ...
    txs =
        (indexManager instanceof Journal
            ...
            : ((IEmbergraphFederation<?>) indexManager).getTransactionService());
    ...
    final long timestamp;
    {
       ...
       * Obtain a read-only transaction which will assert a read lock
        ...
        log.warn("Holding read lock: readLock=" + readLock + ", tx: " + timestamp);
    ...
    final int queryThreadPoolSize;
    {
      ...
      queryThreadPoolSize =
          s == null ? ConfigParams.DEFAULT_QUERY_THREAD_POOL_SIZE : Integer.valueOf(s);

      if (queryThreadPoolSize < 0) {

        throw new RuntimeException(
            ConfigParams.QUERY_THREAD_POOL_SIZE + " : Must be non-negative, not: " + s);
      }

      if (log.isInfoEnabled())
        log.info(ConfigParams.QUERY_THREAD_POOL_SIZE + "=" + queryThreadPoolSize);
    }
    ...
    final boolean describeEachNamedGraph;
    {
      ...
      describeEachNamedGraph =
          s == null ? ConfigParams.DEFAULT_DESCRIBE_EACH_NAMED_GRAPH : Boolean.valueOf(s);

      if (log.isInfoEnabled())
        log.info(ConfigParams.DESCRIBE_EACH_NAMED_GRAPH + "=" + describeEachNamedGraph);
    }
    ...
    final boolean readOnly;
    {
      ...
      readOnly = s == null ? ConfigParams.DEFAULT_READ_ONLY : Boolean.valueOf(s);

      if (log.isInfoEnabled()) log.info(ConfigParams.READ_ONLY + "=" + readOnly);
    }
    ...
    final long queryTimeout;
    {
      ...
      queryTimeout = s == null ? ConfigParams.DEFAULT_QUERY_TIMEOUT : Long.valueOf(s);

      if (queryTimeout < 0) {

        throw new RuntimeException(
            ConfigParams.QUERY_TIMEOUT + " : Must be non-negative, not: " + s);
      }

      if (log.isInfoEnabled()) log.info(ConfigParams.QUERY_TIMEOUT + "=" + queryTimeout);
    }
    ...
    final long warmupTimeoutMillis;
    {
      ...
      warmupTimeoutMillis = s == null ? ConfigParams.DEFAULT_WARMUP_TIMEOUT : Long.valueOf(s);

      if (warmupTimeoutMillis < 0) {

        throw new RuntimeException(
            ConfigParams.WARMUP_TIMEOUT + " : Must be non-negative, not: " + s);
      }

      if (log.isInfoEnabled()) log.info(ConfigParams.WARMUP_TIMEOUT + "=" + warmupTimeoutMillis);
    }

    final SparqlEndpointConfig config =
        new SparqlEndpointConfig(
            namespace,
            timestamp,
            queryThreadPoolSize,
            describeEachNamedGraph,
    ...
    context.setAttribute(EmbergraphServlet.ATTRIBUTE_INDEX_MANAGER, indexManager);

    // Used by EmbergraphRDFBaseServlet
    context.setAttribute(EmbergraphRDFServlet.ATTRIBUTE_RDF_CONTEXT, rdfContext);
    ...
    if (indexManager instanceof Journal && warmupTimeoutMillis > 0L) {
      ...
      final List<String> warmupNamespaceList = new LinkedList<String>();
      {
        ...
        if (log.isInfoEnabled())
          log.info(ConfigParams.WARMUP_NAMESPACE_LIST + "=" + warmupNamespaceList);
      }
      ...
      final int warmupThreadPoolSize;
      {
        ...
        warmupThreadPoolSize =
            s == null ? ConfigParams.DEFAULT_WARMUP_THREAD_POOL_SIZE : Integer.valueOf(s);

        if (warmupThreadPoolSize <= 0) {

          throw new RuntimeException(
              ConfigParams.WARMUP_THREAD_POOL_SIZE + " : Must be positive, not: " + s);
        }

        if (log.isInfoEnabled())
          log.info(ConfigParams.WARMUP_THREAD_POOL_SIZE + "=" + warmupThreadPoolSize);
      }
      ...
      log.warn(
          "Warming up the journal: namespaces="
              + (warmupNamespaceList == null || warmupNamespaceList.isEmpty()
                  ? "ALL"
                  : warmupNamespaceList)
              + ", warmupTheads="
              + warmupThreadPoolSize
              + ", timeout="
              + TimeUnit.MILLISECONDS.toSeconds(warmupTimeoutMillis)
              + "s");

      // Submit the warmup procedure.
      final Future<Map<String, BaseIndexStats>> ft =
          indexManager
              .getExecutorService()
              .submit(
                  new WarmUpTask(
    ...
    //        context.setAttribute(EmbergraphServlet.ATTRIBUTE_SPARQL_CACHE,
    //                new SparqlCache(new MemoryManager(DirectBufferPool.INSTANCE)));

    {
      final boolean forceOverflow = Boolean.valueOf(getInitParameter(ConfigParams.FORCE_OVERFLOW));

      if (forceOverflow && indexManager instanceof IEmbergraphFederation<?>) {
  ...
  public void contextDestroyed(final ServletContextEvent e) {
    ...
    if (txs != null && readLock != null && readLock != -1L) {
      ...
      try {
        ...
        log.error("Could not release transaction: tx=" + readLockTx, ex);
      }
    ...
    if (jnl != null) {
      ...
      if (closeIndexManager) jnl.close();
    ...
    if (jiniClient != null) {
      ...
      if (closeIndexManager) jiniClient.disconnect(true /* immediateShutdown */);
  ...
  private IIndexManager openIndexManager(final String propertyFile) {
    ...
    if (new File(propertyFile).exists()) {
      ...
      // Check the classpath.
      propertyFileUrl =
          EmbergraphRDFServletContextListener.class.getClassLoader().getResource(propertyFile);
    }

    if (log.isInfoEnabled())
      log.info(
          "embergraph configuration: propertyFile="
              + propertyFile
              + ", propertyFileUrl="
              + propertyFileUrl);
    ...
    if (propertyFile.endsWith(".config")) {
       ...
       */
      throw new RuntimeException(
          "File must have '.config' or '.properties' extension: " + propertyFile);
    }
    ...
    try {
       ...
       * A embergraph federation.
        ...
        jiniClient.setDelegate(new NanoSparqlServerFederationDelegate(jiniClient, this));
       ...
       * Note: we only need to specify the FILE when re-opening a
        ...
        final Properties properties = new Properties();
        {
          ...
          final InputStream is = new BufferedInputStream(propertyFileUrl.openStream());
          try {
            properties.load(is);
          } finally {
            is.close();
   ...
   * @param <T>
   */
  private static class NanoSparqlServerFederationDelegate<T> extends DefaultClientDelegate<T> {

    private final IEmbergraphClient<?> client;
    private final EmbergraphRDFServletContextListener servletContextListener;

    /** The path component under which the query engine performance counters will be reported. */
    private static final String QUERY_ENGINE = "Query Engine";

    public NanoSparqlServerFederationDelegate(
        final IEmbergraphClient<?> client,
        final EmbergraphRDFServletContextListener servletContextListener) {

      super(client, null /* clientOrService */);

      this.client = client;

      if (servletContextListener == null) throw new IllegalArgumentException();
    ...
    public void reattachDynamicCounters() {
      ...
      //        	final EmbergraphRDFContext rdfContext = servletContextListener.rdfContext;
       ...
       * TODO It would be better to have this on the EmbergraphRDFContext
         ...
         * been issued a query.
         */
        final QueryEngine queryEngine = QueryEngineFactory.getInstance().getQueryController(fed);
     ...
     * {@link NanoSparqlServer} already provides a {@link CountersServlet} through which this stuff
