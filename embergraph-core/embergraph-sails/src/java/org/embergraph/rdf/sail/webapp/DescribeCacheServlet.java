 ...
 * endpoint?query=DESCRIBE uri(,uri)*</code>).
 *
 * <p>The {@link DescribeCacheServlet} will recognize and perform the DESCRIBE of cached resources
 * where those resources are attached to a request attribute
 *
 * <p>TODO Http cache control. Different strategies can make sense depending on the scalability of
 * the application and the tolerance for stale data. We can use an expire for improved scalability
 * with caching into the network, but invalidation notices can not propagate beyond the DESCRIBE
 * cache to the network. We can use E-Tags with Must-Validate to provide timely invalidation. If we
 * support faceting (by schema or source) then we need to provide E-Tags for each schema/source so
 * the client can inspect/require certain facets.
 *
 * <p>TODO VoID for forward/reverse link set and attribute sketch.
 *
 * <p>TODO Conneg for the actual representation, but internally use an efficient representation that
 * can provide summaries (a SKETCH) through a jump table with statistics for each predicate type.
 *
 * <p>TODO Options for DESCRIBE filters by schema to get only properties and links for a given
 * schema namespace?
 *
 * <p>TODO Describe caching in an open web context requires maintaining metadata about the web
 * resources that have been queried for a given URI. This should include at least the URI itself and
 * could include well known aggregators that might have data for that URI.
 *
 * <p>TODO Take advantage of the known materialization performed by a DESCRIBE query when running
 * queries (materialized star-join). Also, store IVs in the Graph as well as Values. We need both to
 * do efficient star-joins (enter by IV and have IV on output).
 *
 * <p>TODO Hash partitioned DESCRIBE fabric. The partitioned map is easy enough and could be
 * installed at each DS, CS, etc. node. However, the distributed invalidating scheme is slightly
 * trickier. We would need to install this servlet at each endpoint exposed to mutation, which is
 * not actually all that difficult. There needs to be configuration information for either each
 * namespace or for the webapp that specifies how to locate and maintain the cache.
 *
 * <p>TODO Expose (and declare through VoID) a simple URI lookup service that is powered by this
 * cache and turns into a DESCRIBE query if there is a cache miss (or turn it into a DESCRIBE query
 * and let that route to the cache first). VoID has the concept of this kind of "lookup" service.
 *
...
public class DescribeCacheServlet extends EmbergraphRDFServlet {
  ...
  /*
   * The name of a request attribute whose bound value is a {@link Set} of {@link URI}s to be
   * described by the {@link DescribeCacheServlet}.
   */
   ...
   *
   * <p>FIXME DESCRIBE: TX ISOLATION for request but ensure that cache is not negatively effected by
   * that isolation (i.e., how does the cache index based on time tx view).
   */
  @Override
  protected void doGet(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    final QueryEngine queryEngine =
        QueryEngineFactory.getInstance().getQueryController(context.getIndexManager());
    ...
    final IDescribeCache describeCache =
        cacheConn == null ? null : cacheConn.getDescribeCache(namespace, timestamp);
     ...
     */
    {
      final EmbergraphValue[] values = internalURIs.toArray(new EmbergraphValue[nvalues]);

      final long numNotFound =
          tripleStore.getLexiconRelation().addTerms(values, nvalues, true /* readOnly */);

      if (log.isInfoEnabled()) log.info("Not found: " + numNotFound + " out of " + values.length);
    }
    ...
    try {
      ...
      for (Statement s : g) w.handleStatement(s);
      ...
      EmbergraphRDFServlet.launderThrowable(e, resp, "DESCRIBE: uris=" + internalURIs);
    }
  }
}
