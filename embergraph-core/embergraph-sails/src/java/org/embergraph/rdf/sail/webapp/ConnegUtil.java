...
public class ConnegUtil {
   ...
   * Trac Ticket #984.
   *
   * <p>Checks if a query parameter "out" has been passed to set the response type. If found, it
   * maps to the requested Accept Header. It currently supports json, sparql-results+json, xml, and
   * sparql-results+xml.
   *
   * @param req
    ...
    if (outputFormat != null) {
      switch (outputFormat.toLowerCase()) {
        ...
        default:
          acceptHeaderValue = EmbergraphRDFServlet.MIME_SPARQL_RESULTS_XML;
          log.warn(
              "Unknown value for QUERY PARAMETER: "
   ...
   *
   * <p>Checks if a query parameter "out" has been passed to set the response type. If found, it
   * maps to the requested Accept Header. It currently supports json and xml.
   *
   * <p>This method should be used for general HTTP requests rather than Sparql query results.
   *
   * <p>TODO: Support HTML, XML, and RDF
   *
   * @param req
   * @return The value of the format string, if present or the Accept Header.
   * @see http://trac.blazegraph.com/ticket/984
  ...
  /** @param acceptStr The <code>Accept</code> header. */
  public ConnegUtil(String acceptStr) {
...

    {
      for (String s : a) {
        ...
        // RDFFormat
        {
          final RDFFormat rdfFormat = RDFFormat.forMIMEType(t.getMimeType());
        ...
        {
          final TupleQueryResultFormat tupleFormat =
              TupleQueryResultFormat.forMIMEType(t.getMimeType());
        ...
        // BooleanQueryResultFormat
        {
          BooleanQueryResultFormat booleanFormat =
              BooleanQueryResultFormat.forMIMEType(t.getMimeType());
        ...
        // PropertiesFormat
        {
          final PropertiesFormat format = PropertiesFormat.forMIMEType(t.getMimeType());
        ...
        // CounterSetFormat
        {
          final CounterSetFormat format = CounterSetFormat.forMIMEType(t.getMimeType());
   ...
   *
   * @return The best {@link RDFFormat} -or- <code>null</code> if no {@link RDFFormat} was
   *     requested.
   */
   ...
   * by the <code>q</code> parameter.
   *
   * @param fallback The caller's default, which is returned if no match was specified.
   * @return The best {@link RDFFormat} -or- <i>fallback</i> if no {@link RDFFormat} was requested.
   */
   ...
   *
   * @return The best {@link TupleQueryResultFormat} -or- <code>null</code> if no {@link
   *     TupleQueryResultFormat} was requested.
   */
   ...
   *
   * @param fallback The caller's default, which is returned if no match was specified.
   * @return The best {@link TupleQueryResultFormat} -or- <i>fallback</i> if no {@link
   *     TupleQueryResultFormat} was requested.
   */
  public TupleQueryResultFormat getTupleQueryResultFormat(final TupleQueryResultFormat fallback) {
   ...
   * "best" is measured by the <code>q</code> parameter.
   *
   * @return The best {@link BooleanQueryResultFormat} -or- <code>null</code> if no {@link
   *     BooleanQueryResultFormat} was requested.
   */
  public BooleanQueryResultFormat getBooleanQueryResultFormat() {
   ...
   *
   * @param fallback The caller's default, which is returned if no match was specified.
   * @return The best {@link BooleanQueryResultFormat} -or- <i>fallback</i> if no {@link
   *     BooleanQueryResultFormat} was requested.
   */
  public BooleanQueryResultFormat getBooleanQueryResultFormat(
   ...
   *
   * @return The best {@link PropertiesFormat} -or- <code>null</code> if no {@link PropertiesFormat}
   *     was requested.
   */
   ...
   *
   * @param fallback The caller's default, which is returned if no match was specified.
   * @return The best {@link PropertiesFormat} -or- <i>fallback</i> if no {@link PropertiesFormat}
   *     was requested.
   */
   ...
   * measured by the <code>q</code> parameter.
   *
   * @return The best {@link CounterSetFormat} -or- <code>null</code> if no {@link CounterSetFormat}
   *     was requested.
   */
   ...
   *
   * @param fallback The caller's default, which is returned if no match was specified.
   * @return The best {@link CounterSetFormat} -or- <i>fallback</i> if no {@link CounterSetFormat}
   *     was requested.
   */
