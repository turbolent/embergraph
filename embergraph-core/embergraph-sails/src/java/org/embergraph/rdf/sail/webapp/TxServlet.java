 ...
 *
 * <p>Transaction isolation requires that the namespace is provisioned with support for isolatable
 * indices. See {@link EmbergraphSail.Options#ISOLATABLE_INDICES}. When isolation is enabled for a
 * namespace, the updates for the namespace will be buffered by isolated indices. When a transaction
 * prepares, the write set will be validated by comparing the modified tuples against the unisolated
 * indices touched by the write set and looking for conflicts (revision timestamps that have been
 * updated since the transaction start). If a conflict can not be reconciled, then the transaction
 * will be unable to commit.
 *
 * <p>Read only transactions may also be requested in order to have snapshot isolation across a
 * series of queries. embergraph provides snapshot isolation for queries regardless. The use of an
 * explicit read only transaction is only required to maintain the same snapshot across multiple
 * queries.
 *
...
public class TxServlet extends EmbergraphRDFServlet {
  ...
  /** The name of the URL query parameter which indicates the timestamp for a CREATE message. */
  ...
  protected void doPost(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    if (req.getRequestURI().endsWith("/tx")) {
      ...
      buildAndCommitResponse(
          ...
          MIME_TEXT_HTML,
          "Unknown transaction management request");
    }
  }
  ...
  private void doCreateTx(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (timestamp == ITx.UNISOLATED) {
      ...
      buildAndCommitResponse(
          ...
          HttpServletResponse.SC_BAD_REQUEST,
          MIME_TEXT_HTML,
          "Illegal value: timestamp=" + timestamp);
    ...
    try {
      ...
      // path.
      final String txURL = req.getRequestURL().append('/').append(Long.valueOf(txId)).toString();

      final long elapsedMillis = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - beginNanos);
  ...
  /** ABORT-TX(txId) */
  private void doAbortTx(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (!getTxId(req, resp, txId)) return;

    try {

      if (getIndexManager() instanceof IEmbergraphFederation) {

        ((IEmbergraphFederation<?>) getIndexManager()).getTransactionService().abort(txId.get());

      } else {

        // On the journal we can lookup the Tx.
        final ITx tx = ((Journal) getIndexManager()).getTransactionManager().getTx(txId.get());
        ...
        if (tx == null) {
          ...
          buildAndCommitResponse(
              ...
              MIME_TEXT_PLAIN,
              "ABORT-TX: Transaction not found: txId=" + txId);
          return;
        }
  ...
  /** COMMIT-TX(txId) */
  private void doCommitTx(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    try {
      ...
      if (getIndexManager() instanceof IEmbergraphFederation) {
        ...
        ((IEmbergraphFederation<?>) getIndexManager()).getTransactionService().commit(txId.get());
        ...
        if (tx == null) {
          ...
          buildAndCommitResponse(
              ...
              HttpServletResponse.SC_NOT_FOUND,
              MIME_TEXT_PLAIN,
              "COMMIT-TX: Transaction not found: txId=" + txId);
          return;
        }
      ...
      if (InnerCause.isInnerCause(e, ValidationError.class)) {
        ...
        buildAndCommitResponse(
            resp, HttpServletResponse.SC_CONFLICT, MIME_APPLICATION_XML, w.toString());
   ...
   * and commit? Should it be rolled back? Can we have additional writes on the tx and reconcile
   * additional conflicts in another PREPARE or a COMMIT?
   */
  private void doPrepareTx(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    try {
      ...
      final int statusCode = ok ? HttpServletResponse.SC_OK : HttpServletResponse.SC_CONFLICT;
  ...
  /** <code>STATUS-TX</code> */
  private void doStatusTx(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
  ...
  /** <code>LIST-TX</code> */
  private void doListTx(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (getIndexManager() instanceof IEmbergraphFederation) {
      ...
      a = ((Journal) getIndexManager()).getTransactionManager().getActiveTx();
    }
   ...
   *
   * <p>COMMIT-TX, ABORT-TX, PREPARE-TX, and STATUS-TX all need to extract the transaction
   * identifier from the last component of the path which should be <code>/tx/txId</code>.
   *
   * @param req The request.
   * @param resp The response.
   * @param ref The transaction identifier will be saved in this reference.
   * @return <code>true</code> if a transaction identifier was extracted. if <code>false</code> then
   *     no transaction identifier was found and a {@link HttpServletResponse#SC_BAD_REQUEST}
  ...
  private final boolean getTxId(
    ...
    if (pathInfo.length() < 2) {
      buildAndCommitResponse(
          ...
          HttpServletResponse.SC_BAD_REQUEST,
          MIME_TEXT_HTML,
          "No transaction identifier in path: pathInfo=" + pathInfo);
      return false;
    }
   ...
   * @return The readsOnCommitTime if it is available and otherwise <code>null</code>.
   *     <p>TODO This information is not available in scale-out. See <a
   *     href="http://trac.bigdata.com/ticket/#266" > Refactor native long tx id to thin object.
   *     </a>
   */
  private Long getReadsOnCommitTimeOrNull(final long txId) {
    ...
    final ITxState tx = ((Journal) getIndexManager()).getLocalTransactionManager().getTx(txId);
  ...
  private static void addTx(final Node parent, final ITxState tx) throws IOException {

    addTx(parent, tx.getStartTimestamp(), tx.getReadsOnCommitTime());
  }

  private static void addTx(final Node parent, final long txId, final Long readsOnCommitTime)
      throws IOException {
