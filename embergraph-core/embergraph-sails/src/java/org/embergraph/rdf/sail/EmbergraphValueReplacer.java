 ...
 * @version $Id: EmbergraphValueReplacer.java 5159 2011-09-08 15:35:15Z thompsonbry $
 */
public class EmbergraphValueReplacer {

  private static final Logger log = Logger.getLogger(EmbergraphValueReplacer.class);

  private final AbstractTripleStore database;

  public EmbergraphValueReplacer(final AbstractTripleStore database) {
   ...
   *
   * <p>Note: The native rule execution must examine the resulting {@link EmbergraphValue}s. If any
   * value does not exist in the lexicon then its term identifier will be ZERO (0L). Term
   * identifiers of ZERO (0L) WILL NOT match anything in the data and MUST NOT be executed since a
   * ZERO (0L) will be interpreted as a variable!
   *
   * @return yucky hack, need to return a new dataset and a new binding set. dataset is [0], binding
   *     set is [1]
   */
  public Object[] replaceValues(final Dataset dataset, final BindingSet[] bindingSets) {
    ...
    if (dataset != null) {
      ...
      for (URI uri : dataset.getDefaultGraphs())
        if (uri != null) values.put(uri, valueFactory.asValue(uri));
     ...
     */
    {
      ...
      database.getLexiconRelation().addTerms(terms, terms.length, true /* readOnly */);
    ...
    if (dataset == null) {
      ...
      for (URI uri : dataset.getDefaultGraphs()) dataset2.addDefaultGraph((URI) values.get(uri));

      for (URI uri : dataset.getNamedGraphs()) dataset2.addNamedGraph((URI) values.get(uri));
    }
