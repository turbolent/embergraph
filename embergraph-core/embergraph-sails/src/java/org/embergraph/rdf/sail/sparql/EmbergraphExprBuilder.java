...
import org.embergraph.rdf.sparql.ast.IProjectionDecl;
 ...
 * @openrdf
 */
public class EmbergraphExprBuilder extends GroupGraphPatternBuilder {

  private static final Logger log = Logger.getLogger(EmbergraphExprBuilder.class);

  public EmbergraphExprBuilder(final EmbergraphASTContext context) {
   ...
   * ASTQueryContainer} node. They are being ignored here as they should have been processed before
   * running the {@link EmbergraphExprBuilder}.
   *
   * <p>Note: {@link ASTQuery} is an abstract type. The concrete classes are: {@link ASTAskQuery},
   * {@link ASTConstructQuery}, {@link ASTDescribeQuery}, and {@link ASTSelectQuery}. This method
   * will wind up delegating to the visitor method for the appropriate concrete {@link ASTQuery}
   * instance.
   */
  @Override
  public QueryRoot visit(final ASTQueryContainer node, final Object data) throws VisitorException {
   ...
   *     whether or not the {@link ASTSelectQuery} appears as a top-level query or a subquery.
   */
  @Override
  public QueryBase visit(final ASTSelectQuery astQuery, Object data) throws VisitorException {

    final QueryBase queryRoot = getQueryBase(astQuery, data, QueryType.SELECT);
  ...
  /** ASK query. */
  @Override
  public QueryBase visit(final ASTAskQuery node, Object data) throws VisitorException {
   ...
   * (it is an equivalent grammar in that it accepts the same inputs). This means that the
   * "projection" part of the DESCRIBE query is visited on the {@link ASTDescribe} node.
   */
  @Override
  public QueryBase visit(final ASTDescribeQuery node, Object data) throws VisitorException {
   ...
   * projected normally. <code>*</code> is projected as the variable named "*".
   *
   * <p>Note: This {@link ProjectionNode} models the "DESCRIBE" but does not provide a query plan.
   * The {@link QueryBase} MUST be further rewritten in order to supply an appropriate query plan.
   * This should be done using the appropriate {@link IASTOptimizer}.
   *
   * @param data The {@link QueryBase}.
   */
  @Override
  public Void visit(final ASTDescribe node, Object data) throws VisitorException {
    ...
    if (node.isWildcard()) {
      ...
      for (int i = 0; i < nchildren; i++) {
        ...
        final TermNode resource = (TermNode) node.jjtGetChild(i).jjtAccept(this, null);
   ...
   * construct template and attaches the where clause and solution modifiers. However, this is NOT a
   * complete description of the semantics of the query. An {@link IASTOptimizer} will intercept the
   * CONSTRUCT query and rewrite it before it is executed in order to provide an appropriate {@link
   * ProjectionNode}.
   */
  @Override
  public QueryBase visit(final ASTConstructQuery node, final Object data) throws VisitorException {

    final QueryBase queryRoot = getQueryBase(node, data, QueryType.CONSTRUCT);
    ...
    final ConstructNode tmp = (ConstructNode) constructNode.jjtAccept(this, null /* data */);
    ...
    if (tmp.isEmpty()) {
      ...
      for (IGroupMemberNode child : whereClause) {
        ...
        final StatementPatternNode sp =
            (StatementPatternNode) ((StatementPatternNode) child).clone();
  ...
  @Override
  public final GroupByNode visit(final ASTGroupClause node, Object data) throws VisitorException {
  ...
  @Override
  public final List<OrderByExpr> visit(final ASTOrderClause node, Object data)
  ...
  @Override
  public final OrderByExpr visit(final ASTOrderCondition node, Object data)
      throws VisitorException {

    final ValueExpressionNode valueExpr =
        (ValueExpressionNode) node.jjtGetChild(0).jjtAccept(this, null);
   ...
   * back to an instance of this visitor.
   *
   * @param astQuery The AST query node. This is an abstract base class. There are concrete
   *     instances for SELECT, ASK, DESCRIBE, and CONSTRUCT.
   * @param queryRoot The embergraph query root.
   */
  private void handleNamedSubqueryClause(final ASTQuery astQuery, final QueryBase queryRoot)
      throws VisitorException {
    ...
    for (int i = 0; i < nchildren; i++) {
      ...
      final SubqueryRoot subquery =
          (SubqueryRoot) aNamedSubquery.jjtAccept(this, queryRoot /* data */);

      // Create instance of the right class.
      final NamedSubqueryRoot namedSubquery =
          new NamedSubqueryRoot(subquery.getQueryType(), aNamedSubquery.getName());
   ...
   * {@link SubqueryRoot}.
   *
   * @param node The {@link ASTQuery} node.
   * @param data The data. This can be bound to some things other than a {@link QueryBase}. Such
   *     bindings are ignored.
   * @param queryType The type of the {@link ASTQuery}.
   * @return Some kind of {@link QueryBase} object.
   */
  private QueryBase getQueryBase(
      final ASTQuery node, final Object data, final QueryType queryType) {

    final Node p = node.jjtGetParent();

    if (log.isInfoEnabled())
      log.info(
          "parent=" + p + ", data=" + (data == null ? "null" : data.getClass().getSimpleName()));
   ...
   * IProjectionDecl}.
   *
   * @param select The {@link ASTSelect} node.
   * @param queryRoot The {@link QueryBase}.
   * @throws VisitorException
   */
  protected void handleSelect(final ASTSelect select, final IProjectionDecl queryRoot)
      throws VisitorException {
    ...
    if (select.isDistinct()) projection.setDistinct(true);

    if (select.isReduced()) projection.setReduced(true);
      ...
      while (itr.hasNext()) {
        ...
        if (!e.hasAlias()) {
          ...
          final IValueExpressionNode ve =
              (IValueExpressionNode) expr.jjtAccept(this, null /* data */);
  ...
  private void handleWhereClause(final ASTQuery astQuery, final QueryBase queryRoot)
      throws VisitorException {
    ...
    if (whereClause != null) {
      ...
      final ASTGraphPatternGroup graphPatternGroup = whereClause.getGraphPatternGroup();

      graphPattern = scopedGroupGraphPattern(astQuery);

      final GraphPatternGroup<IGroupMemberNode> ret =
          (GraphPatternGroup<IGroupMemberNode>) graphPatternGroup.jjtAccept(this, null /* data */);
  ...
  private void handleGroupBy(final ASTQuery astQuery, final QueryBase queryRoot)
    ...
    final GroupByNode groupBy = (GroupByNode) groupNode.jjtAccept(this, null);
  ...
  private void handleHaving(final ASTQuery astQuery, final QueryBase queryRoot)
    ...
    if (havingClause != null) {
      ...
      for (int i = 0; i < nchildren; i++) {
        ...
        final IValueExpressionNode ve =
            (IValueExpressionNode) havingClause.jjtGetChild(i).jjtAccept(this, null /* data */);
   ...
   * @param queryRoot The embergraph query root.
   */
  private void handleOrderBy(final ASTQuery astQuery, final QueryBase queryRoot)
      throws VisitorException {
    ...
    if (orderNode != null) {
      ...
      @SuppressWarnings("unchecked")
      final List<OrderByExpr> orderElemements = (List<OrderByExpr>) orderNode.jjtAccept(this, null);

      for (OrderByExpr orderByExpr : orderElemements) orderBy.addExpr(orderByExpr);
   ...
   * @param queryRoot The embergraph query root.
   */
  private void handleSlice(final ASTQuery astQuery, final QueryBase queryRoot) {
    ...
    if (theLimit != null || theOffset != null) {
      ...
      if (theLimit != null) theSlice.setLimit(theLimit.getValue());

      if (theOffset != null) theSlice.setOffset(theOffset.getValue());
   ...
   * @throws VisitorException
   */
  private void handleBindings(final ASTQuery astQuery, final QueryBase queryRoot)
      throws VisitorException {
   ...
   */
  @Override
  public final BindingsClause visit(final ASTBindingsClause node, Object data)
      throws VisitorException {
     ...
     */
    {
      ...
      for (ASTBindingSet bindingNode : bindingNodes) {
        ...
        final IBindingSet bindingSet = (IBindingSet) bindingNode.jjtAccept(this, declaredVars);
   ...
   * @return The binding set. Variables will be bound unless UNDEF was specified at the index
   *     corresponding to a given variable.
   */
  @Override
  public IBindingSet visit(final ASTBindingSet node, final Object data) throws VisitorException {
    ...
    for (int i = 0; i < numberOfBindingValues; i++) {
      ...
      final ConstantNode ve = (ConstantNode) node.jjtGetChild(i).jjtAccept(this, null);
   ...
   * @throws VisitorException
   */
  public static void verifyAggregate(final QueryBase queryBase) throws VisitorException {
