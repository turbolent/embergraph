...
public class DeleteServlet extends EmbergraphRDFServlet {
  ...
  protected void doDelete(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    //        final boolean suppressTruthMaintenance = getBooleanValue(req,
    // QueryServlet.ATTR_TRUTH_MAINTENANCE, false);
  ...
  private void doDeleteWithQuery(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    final boolean suppressTruthMaintenance =
        getBooleanValue(req, QueryServlet.ATTR_TRUTH_MAINTENANCE, false);

    final Map<String, Value> bindings = parseBindings(req, resp);
    if (bindings
        == null) { // invalid bindings definition generated error response 400 while parsing
      return;
    }

    if (queryStr == null) throw new UnsupportedOperationException();

    if (log.isInfoEnabled()) log.info("delete with query: " + queryStr);
    ...
    try {
      ...
      if (getIndexManager().isGroupCommit()) {
        ...
        submitApiTask(
                new DeleteWithQueryMaterializedTask(
                    req,
                    resp,
                    namespace,
                    ITx.UNISOLATED,
        ...
        submitApiTask(
                new DeleteWithQuerySteamingTask(
                    req,
                    resp,
                    namespace,
                    ITx.UNISOLATED,
      ...
      launderThrowable(
          t, resp, "UPDATE-WITH-QUERY" + ": queryStr=" + queryStr + ", baseURI=" + baseURI);
    }
  }
   ...
   *
   * <p>Note: To avoid materializing the statements, this runs the query against the last commit
   * time and uses a pipe to connect the query directly to the process deleting the statements. This
   * is done while it is holding the unisolated connection which prevents concurrent modifications.
   * Therefore the entire SELECT + DELETE operation is ACID. However, when group commit is enabled
   * the last commit time might not be the last modification to the indices which would break the
   * serializability guarantee of the applied mutations.
   *
   * @author bryan
   */
  private static class DeleteWithQuerySteamingTask extends AbstractRestApiTask<Void> {
     ...
     * @param bindings
     */
    public DeleteWithQuerySteamingTask(
   ...
   * (or transparently overflow to the HTree).
   *
   * <p>TODO This operator materializes the RDF Values for the CONSTRUCTED statements, buffers then,
   * and then removes those statements. The materialization step is not necessary if we handle this
   * all at the IV layer.
   *
   * @author bryan
   */
  private static class DeleteWithQueryMaterializedTask extends AbstractRestApiTask<Void> {
     ...
     * @param baseURI The base URI for the operation.
     */
    public DeleteWithQueryMaterializedTask(
        final HttpServletRequest req,
        final HttpServletResponse resp,
        final String namespace,
        final long timestamp,
    ...
    public Void call() throws Exception {
      ...
      try {
...

        {
          ...
          final BufferStatementHandler buffer =
              new BufferStatementHandler(conn, nmodified /*, defaultDeleteContext*/);
  ...
  /** DELETE request with a request body containing the statements to be removed. */
  private void doDeleteWithBody(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    if (contentType == null) throw new UnsupportedOperationException();
    ...
    final Resource[] defaultContext;
    {
      ...
      if (s != null && s.length > 0) {
        try {
          ...
          buildAndCommitResponse(
              resp, HTTP_INTERNALERROR, MIME_TEXT_PLAIN, ex.getLocalizedMessage());
          return;
        }
      } else {
  ...
  private static class DeleteWithBodyTask extends AbstractRestApiTask<Void> {
     ...
     * @param timestamp The timestamp used to obtain a mutable connection.
     * @param baseURI The base URI for the operation.
     * @param defaultContext The context(s) for triples without an explicit named graph when the KB
     *     instance is operating in a quads mode.
     * @param rdfParserFactory The factory for the {@link RDFParser}. This should have been chosen
     *     based on the caller's knowledge of the appropriate content type.
     */
    public DeleteWithBodyTask(
        final HttpServletRequest req,
        final HttpServletResponse resp,
        final String namespace,
        final long timestamp,
  ...
  private void doDeleteWithAccessPath(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    try {
      ...
      submitApiTask(
              new DeleteWithAccessPathTask(
                  req, resp, namespace, ITx.UNISOLATED, suppressTruthMaintenance, s, p, o, c))
          .get();
  ...
  private static class DeleteWithAccessPathTask extends AbstractRestApiTask<Void> {
     ...
     *     based on the caller's knowledge of the appropriate content type.
     */
    public DeleteWithAccessPathTask(
        final HttpServletRequest req,
        final HttpServletResponse resp,
        final String namespace,
        final long timestamp,
