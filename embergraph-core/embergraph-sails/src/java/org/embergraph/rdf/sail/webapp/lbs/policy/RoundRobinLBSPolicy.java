...
public class RoundRobinLBSPolicy extends AbstractLBSPolicy {
   ...
   * and appears to be joined with the met quorum, then the request can be proxied to that service.
   */
  @Override
  public String getReaderURI(final HttpServletRequest request) {
    ...
    for (int i = 0; i < serviceScores.length; i++) {
       ...
       * MAX_VALUE.
       */
      final int hostIndex = (int) Math.abs(((i + startIndex) % serviceScores.length));

      serviceScore = serviceScores[hostIndex];

      if (serviceScore == null) continue;
