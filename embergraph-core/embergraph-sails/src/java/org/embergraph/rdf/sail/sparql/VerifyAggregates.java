 ...
 *
 * <p>Note: This is a port of {@link org.embergraph.bop.solutions.GroupByState} that does not depend
 * on the embergraph operator model. It was developed as part of BLZG-1176 to decouple the SPARQL
 * parser from the database.
 *
...
public class VerifyAggregates {
  ...
  public VerifyAggregates(
    ...
    if (projection == null) throw new IllegalArgumentException();

    if (projection.arity() == 0) throw new IllegalArgumentException();
    ...
    if (having != null) {
      ...
      for (final IValueExpressionNode node : having) {
        ...
        if (ve != null) {
          exprs2[i] = new SPARQLConstraint<XSDBooleanIV<EmbergraphLiteral>>(ve);
        } else if (node instanceof FunctionNode) {

          final FunctionNode exprNode = (FunctionNode) node;
          final BOp expr = convertAggregates(exprNode);
          exprs2[i] = new SPARQLConstraint<XSDBooleanIV<EmbergraphLiteral>>(new BOp[] {expr}, null);
  ...
  private IValueExpression convertAggregates(final BOp exprNode) {
    ...
    if (exprNode instanceof FunctionNode) {
      if ((exprNode instanceof FunctionNode)
          && FunctionRegistry.isAggregate(((FunctionNode) exprNode).getFunctionURI())) {
        expr =
            new AggregateBase(args, null) {
              @Override
              public void reset() {}
  ...
  private boolean isAggregate(
    ...
    if (op instanceof IAggregate<?>
        || ((op instanceof FunctionNode)
            && FunctionRegistry.isAggregate(((FunctionNode) op).getFunctionURI()))) {
