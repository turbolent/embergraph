...
public abstract class AbstractLBSPolicy
    implements IHALoadBalancerPolicy, QuorumListener, Serializable {
  ...
  public void init(final ServletConfig servletConfig, final IIndexManager indexManager)
    ...
    final IHAJournal journal = (IHAJournal) EmbergraphServlet.getIndexManager(servletContext);

    if (journal == null) throw new ServletException("No journal?");
   ...
   * service, which makes it more efficient under some circumstances to handle the read-request on
   * the service where it was originally received).
   *
   * @throws IOException
   */
   ...
   */
  @Override
  public final String getLeaderURI(final HttpServletRequest request) {
   ...
   *
   * <p>FIXME The {@link QuorumListener} is unregistered by {@link AbstractQuorum#terminate()}. This
   * happens any time the HAJournalServer goes into the error state. When this occurs, we stop
   * getting {@link QuorumEvent}s and the policy stops being responsive (it can not proxy the
   * request to a service that is still up because it does not know what services are up, or maybe
   * it just can not learn if services go down).
   *
   * <p>We probably need to either NOT clear the quorum listener and/or add an event type that is
   * sent when {@link Quorum#terminate()} is called and/or use our own listener (independent of the
   * HAJournalServer, which would require us to use an HAClient).
   *
   * <p>This should be robust even when the HAQuorumService is not running. We do not want to be
   * unable to proxy to another service just because this one is going through an error state. Would
  ...
  protected void updateServiceTable() {
    ...
    for (int i = 0; i < joined.length; i++) {
      ...
      try {
        ...
        serviceScores[i] = ServiceScore.newInstance(journal, contextPath.get(), serviceId);
    ...
    if (log.isInfoEnabled()) log.info("Updated servicesTable: #services=" + serviceScores.length);
