 ...
 * model of the load on each host. Ganglia MUST be reporting metrics for each host running an {@link
 * HAJournalServer} instance. This can be achieved either using the <code>gmond</code> utility from
 * the ganglia distribution or using the {@link GangliaPlugIn}.
 *
 * <p>Note: The actual performance metrics will be reported by a given ganglia daemon on some host
 * depend on the ganglia implementation, ganglia configuration, and the host OS. Some key
 * performance metrics may not be available on all hosts. For example, IO Wait is not available
 * under OSX. The available performance metrics constraints the {@link IHostScoringRule}s that you
 * can utilize on a given cluster. You can monitor the available performance metrics by running the
 * {@link GangliaListener} utility.
 *
 * <p>Note: The ganglia updates are not synchronized across a cluster. They pour in ever N seconds
 * from each host. However, the hosts do not begin to report on the same N second boundary. All you
 * know is that (on average) all hosts should have reported in within N seconds.
 *
 ...
 *     >Ganglia Wiki (has a section on the defined metics and the platform where they are
 *     available)</a>
...
public class GangliaLBSPolicy extends AbstractHostLBSPolicy {
  ...
  private final AtomicReference<GangliaService> gangliaServiceRef =
      new AtomicReference<GangliaService>();
  ...
  public void init(final ServletConfig servletConfig, final IIndexManager indexManager)
    ...
    gangliaServiceRef.set((GangliaService) ((Journal) indexManager).getGangliaService());
   ...
   * maintained by the integrated {@link GangliaService}.
   */
  protected Map<String, IHostMetrics> getHostReportForKnownServices(
    ...
    final Map<String /* hostname */, IHostMetrics> hostMetricsMap =
        new HashMap<String, IHostMetrics>();
    ...
    for (int i = 0; i < hostReport.length; i++) {
      ...
      for (int j = 0; j < serviceScores.length; j++) {
        ...
        final String localname =
            (index == -1) ? null : hostname.substring(0 /* beginIndex */, index /* endIndex */);
