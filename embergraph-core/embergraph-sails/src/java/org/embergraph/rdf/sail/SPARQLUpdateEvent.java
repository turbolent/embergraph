...
public class SPARQLUpdateEvent {
   ...
   *
   * @see BLZG-1446 (Provide detailed statistics on execution performance inside of SPARQL UPDATE
   *     requests).
   */
   ...
   * @param deleteInsertWhereStats Statistics associated with the processing of a DELETE/INSERT
   *     WHERE operation (and otherwise <code>null</code>).
   */
  public SPARQLUpdateEvent(
      final Update op,
      final long elapsed,
      final long connectionFlushNanos,
      final long batchResolveNanos,
   ...
   * case an RDF Value not known at the time that the SPARQL UPDATE parser was run has since become
   * defined through a side-effect of a previous SPARQL UPDATE request within the scope of the same
   * connection).
   *
   * @see BLZG-1306
   */
  ...
  public static class LoadProgress extends SPARQLUpdateEvent {
    ...
    public LoadProgress(
        final Update op, final long elapsed, final long nparsed, final boolean done) {

      super(
          op,
          elapsed,
          0L /* connectionFlushNanos */,
          0L /* batchResolveNanos */,
          null /* cause */,
     ...
     * often run ahead of the actual index writes. This can manifest as periods during which the
     * "parsed" statement count climbs quickly followed by periods in which it is stable. That
     * occurs when the parser is blocked because it can not add statements to the connection while
     * the connection is being flushed to the database. There is a ticket to fix this blocking
     * behavior so the parser can continue to run while we are flushing statements onto the database
     * - see below.
     *
     * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/529">Improve load performance
     *     </a>
     */
     ...
     * that the parsed is done running.
     */
