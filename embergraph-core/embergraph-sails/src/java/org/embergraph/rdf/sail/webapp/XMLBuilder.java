...
public class XMLBuilder {
  ...
  public XMLBuilder(final boolean xml, final String encoding, final Writer w) throws IOException {
  ...
  public class Node {
    ...
    public String toString() {
      ...
      return "<" + m_tag + "...>" + (m_parent == null ? "" : "(parent=" + m_parent.m_tag + ")");
    }
     ...
     * @param value The value of the attribute, which will be automatically encoded.
     * @return This {@link Node}
     * @throws IOException
     */
    public Node attr(final String attr, final Object value) throws IOException {
     ...
     * Write out the text.
     *
     * <p>The head for the current element is first closed if it was still open. Then the text is
     * then encoded and written out.
     *
     * <p>This does not generate the closing tag for the node. More text or embedded nodes can still
     * be output.
     *
     * @param text The text, which will be automatically encoded in a manner suitable for a CDATA
     *     section.
     * @return This {@link Node}
     * @throws IOException
     */
     ...
     *
     * <p>The head for the current element is first closed if it was still open. Then the text is
     * NOT encoded before written out.
     *
     * <p>This does not generate the closing tag for the node. More text or embedded nodes can still
     * be output.
     *
    ...
    /**
     * Create and return a new {@link Node}. The head of the {@link Node} will be open. Attributes
     * may be specified until the closing tag is generated by {@link Node#close()} or until the head
     * of the node is closed by {@link Node#text(String)}.
     *
     * @param tag The tag for that {@link Node}.
    ...
    /**
     * Generate an open element for the tag, output the indicated text, and then output the closing
     * element for that tag.
     *
     * @param tag The tag (element name).
     * @param text The text, which will be automatically encoded as appropriate for a CDATA section.
     * @return This {@link Node}.
     * @throws IOException
     */
    public Node node(final String tag, final String text) throws IOException {
    ...
    /**
     * Return <code>true</code> if this is an HTML element which can be closed without an XML style
     * end tag.
     *
     * <p>TODO This does not cover all cases by a long shot. It just covers the ones that we are
     * currently using.
     *
     * <p>TODO This should be done with a hash map for faster lookup once there are more tags that
     * we recognize in this method.
     */
     ...
     * Close the open element.
     *
     * @param simpleEnd When <code>true</code> an open tag without a body will be closed by a single
     *     &gt; symbol rather than the XML style &#47;&gt;.
     * @return The parent element.
     * @throws IOException
     */
