...
public class RESTServlet extends EmbergraphRDFServlet {
  ...
  public void init() throws ServletException {
    ...
    {
      final String provider = this.getInitParameter(ConfigParams.BLUEPRINTS_SERVLET_PROVIDER);

      final String blueprintsProvider =
          provider == null || provider.equals("")
              ? ConfigParams.DEFAULT_BLUEPRINTS_SERVLET_PROVIDER
              : provider;

      m_blueprintsServlet =
          BlueprintsServletProxy.BlueprintsServletFactory.getInstance(blueprintsProvider);
    }

    {
      final String provider = this.getInitParameter(ConfigParams.MAPGRAPH_SERVLET_PROVIDER);

      final String mapgraphProvider =
          provider == null || provider.equals("")
              ? ConfigParams.DEFAULT_MAPGRAPH_SERVLET_PROVIDER
              : provider;

      m_mapgraphServlet =
          new MapgraphServletProxy.MapgraphServletFactory().getInstance(mapgraphProvider);
    }
  ...
  protected void doGet(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    if (log.isInfoEnabled()) log.info(req.toString());
    ...
    if (pathInfo != null && !pathInfo.endsWith("/sparql")) {
      ...
      req.setAttribute(QueryServlet.ATTR_QUERY, "DESCRIBE <" + uri.stringValue() + ">");
  ...
  protected void doPost(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    if (req.getParameter(QueryServlet.ATTR_QUERY) != null
      ...
      StatusServlet.doCancelQuery(req, resp, getIndexManager(), getEmbergraphRDFContext());
  ...
  static boolean hasMimeType(final HttpServletRequest req, final String mimeType) {
   ...
   * "INSERT". Use POST instead for that purpose.
   */
  @Override
