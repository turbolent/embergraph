 ...
 *     for people who want to modify some properties after a namespace has been created. This is in
 *     fact possible for some kinds of properties but not for others, but there is very little
 *     documentation about when this is and is not possible. You need to actually understand and
 *     reason about whether the property controls the manner in which the data is stored on the disk
...
public class EmbergraphSailHelper {
   ...
   * @param namespace The namespace of the triple store.
   * @param timestamp The timestamp of the view.
   * @return The SAIL.
   */
  public EmbergraphSail getSail(
      final IEmbergraphFederation<?> fed, final String namespace, final long timestamp) {

    ScaleOutTripleStore tripleStore =
        (ScaleOutTripleStore) fed.getResourceLocator().locate(namespace, timestamp);
    ...
    if (tripleStore == null) {
      ...
      if (timestamp == ITx.UNISOLATED) {
        ...
        tripleStore = new ScaleOutTripleStore(fed, namespace, timestamp, getProperties());
        ...
        throw new RuntimeException(
            "No such triple store: namespace=" + namespace + ", timestamp=" + timestamp);
      }
    }
   ...
   */
  public EmbergraphSail getSail(
      final String filename, final String namespace, final long timestamp) {
    ...
    LocalTripleStore tripleStore =
        (LocalTripleStore) journal.getResourceLocator().locate(namespace, timestamp);
    ...
    if (tripleStore == null) {
      ...
      if (timestamp == ITx.UNISOLATED) {
        ...
        tripleStore = new LocalTripleStore(journal, namespace, timestamp, getProperties());
   ...
   *
   * <p>Note: You need to edit this code to correspond to your application requirements. Currently
   * the code reflects an application using a triple store without inference, without statement
   * level provenance, and with the full text index enabled. Another common configuration is a
   * triple store with RDFS++ inference, which can be realized by changing the {@link
   * AbstractTripleStore.Options#AXIOMS_CLASS} property value and possibly enabling {@link
   * Options#TRUTH_MAINTENANCE} depending on whether or not you will be incrementally or bulk
   * loading data.
   *
   * @todo Bundle some sample properties files which will make it easier for people to configure a
   *     {@link EmbergraphSail} and make that a command line option. The jini configuration approach
   *     is nice since it allows us to use symbolic constants in the configuration files.
   */
  public Properties getProperties() {
    ...
    //         * execution).
    //         */
    //        properties.setProperty(EmbergraphSail.Options.NATIVE_JOINS, "true");
     ...
     */
    properties.setProperty(
        org.embergraph.rdf.store.DataLoader.Options.COMMIT, DataLoader.CommitEnum.None.toString());
   ...
   */
  public Properties getProperties(final EmbergraphSail sail) {
   ...
   *     AbstractTripleStore#getIndexManager()}.
   * @param namespace The namespace of a locatable resource such as an {@link AbstractTripleStore},
   *     {@link SPORelation} or {@link LexiconRelation}.
   * @return The persistent properties.
   */
  protected Properties getProperties(final IIndexManager indexManager, final String namespace) {

    final Map<String, Object> map =
        indexManager.getGlobalRowStore().read(RelationSchema.INSTANCE, namespace);
   ...
   *
   * <p>Note: this changes the persistent property values associated with the SAIL. It DOES NOT
   * change the properties associated with the given instance. You MUST re-open the SAIL in order
   * for the new properties to be in effect.
   *
   * <p>Note: While many property values can be changed dynamically, some may not. In particular,
   * the properties that effect the way in which the keys for the indices are generated as stored
   * within the indices themselves. Among other things this ensures that Unicode configuration
   * options are applied uniformly when an is accessed by any host in a federation.
   *
   * @param sail The SAIL.
   * @param properties The properties.
   * @return The post-modification properties.
   */
  public Properties setProperties(final EmbergraphSail sail, final Properties properties) {
  ...
  protected Properties setProperties(
      final IIndexManager indexManager, final String namespace, final Properties properties) {
   ...
   */
  public static void showLexiconIndexDetails(final EmbergraphSailConnection cxn) {
   ...
   */
  public static void showSPOIndexDetails(final EmbergraphSailConnection cxn) {
   ...
   * </code> file. The JiniFederation (JDS) mode is inferred when the filename is a <code>.config
   * </code> file. If neither of those file extensions is used, then you must specify the either LTS
   * or JDS explicitly.
   *
   * <p>Note: The <i>namespace</i> identifies which triple store you are accessing and defaults to
   * <code>kb</code>.
   *
   * <p>Note: The <i>timestamp</i> identifies which commit point you are accessing and defaults to
   * the {@link ITx#UNISOLATED} view, which can also be specified as {@value#ITx#UNISOLATED}).
   *
   * <p>Note: The <i>properties</i> is a file containing property overrides to be applied to the kb.
   *
   * @param args <i>filename</i> ((LTS|JDS ((<i>namespace</i> (<i>timestamp</i>)))properties)
   * @throws SailException
  ...
  public static void main(final String[] args)
      throws SailException, IOException, InterruptedException {
    ...
    switch (fedType) {
      ...
      case JDS:
        if (file.isDirectory()) {
          ...
          System.err.println(fedType + " requires plain file, not a directory: dir=" + filename);
    ...
    final long timestamp = args.length > 3 ? Long.valueOf(args[3]) : ITx.UNISOLATED;
    ...
    final EmbergraphSailHelper helper = new EmbergraphSailHelper();
    ...
    switch (fedType) {
      ...
      case JDS:
        ...
        fed =
            (AbstractFederation<?>)
                ScaleOutClientFactory.getJiniClient(new String[] {args[0]}).connect();
    ...
    try {
      ...
      boolean ok = false;
      final EmbergraphSailConnection cxn = sail.getUnisolatedConnection();
      try {
