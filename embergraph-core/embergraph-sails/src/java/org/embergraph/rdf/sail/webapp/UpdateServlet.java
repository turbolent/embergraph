...
public class UpdateServlet extends EmbergraphRDFServlet {
   ...
   * Therefore the entire <code>SELECT + DELETE</code> operation is ACID.
   */
  private void doUpdateWithQuery(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    try {
      ...
      if (getIndexManager().isGroupCommit()) {
        ...
        submitApiTask(
                new UpdateWithQueryMaterializedTask(
                    req,
                    resp,
                    namespace,
                    ITx.UNISOLATED,
        ...
        submitApiTask(
                new UpdateWithQueryStreamingTask(
                    req,
                    resp,
                    namespace,
                    ITx.UNISOLATED,
   ...
   * doing this. However, if group commit is enabled, then it might not observe mutations which may
   * have been applied since the last commit which breaks the serializability guarantee.
   *
   * @author bryan
   */
  private static class UpdateWithQueryStreamingTask extends AbstractRestApiTask<Void> {
     ...
     * @param defaultContextDelete When removing statements, the context(s) for triples without an
     *     explicit named graph when the KB instance is operating in a quads mode.
     * @param defaultContextInsert When inserting statements, the context(s) for triples without an
     *     explicit named graph when the KB instance is operating in a quads mode.
     */
    public UpdateWithQueryStreamingTask(
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        {
          if (log.isInfoEnabled()) log.info("update with query: " + queryStr);
          ...
          try {
            ...
            // Run DELETE
            {
              final RDFParserFactory factory =
                  RDFParserRegistry.getInstance().get(deleteQueryFormat);
   ...
   * preserves the serializability guarantee.
   *
   * @author bryan
   */
  private static class UpdateWithQueryMaterializedTask extends AbstractRestApiTask<Void> {
     ...
     *     explicit named graph when the KB instance is operating in a quads mode.
     */
    public UpdateWithQueryMaterializedTask(
        final HttpServletRequest req,
        final HttpServletResponse resp,
        final String namespace,
        final long timestamp,
...

          {
            ...
            // Run DELETE
            {
              ...
              final BufferStatementHandler buffer =
                  new BufferStatementHandler(conn, nmodified, defaultContextDelete);
   ...
   */
  private void doUpdateWithBody(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        if (remove != null) {
          ...
          final RemoveStatementHandler handler =
              new RemoveStatementHandler(conn, nmodified, defaultContextDelete);
  ...
  private boolean validateItem(final HttpServletResponse resp, final FileItem item)
      throws IOException {
