...
public class RDRHistory implements IChangeLog {
  ...
  public void init() {
    ...
    try {
      ...
      final IV<?, ?>[] ivs = resolveTerms(new URI[] {Vocab.ADDED, Vocab.REMOVED});
      added = ivs[0];
      removed = ivs[1];
  ...
  protected IV<?, ?>[] resolveTerms(final URI[] terms) {
  ...
  public TempTripleStore newTempTripleStore() {
    ...
    properties.setProperty(
        org.embergraph.rdf.store.AbstractTripleStore.Options.ONE_ACCESS_PATH, "true");
  ...
  public void transactionCommited(final long commitTime) {
    ...
    try {
      ...
      final IChunkedOrderedIterator<ISPO> it = tempStore.getAccessPath(SPOKeyOrder.SPO).iterator();
       ...
       */
      final IV<?, ?> timestamp =
          database.addTerm(database.getValueFactory().createXSDDateTime(commitTime));
      ...
      database.addStatements(
          new ChunkedResolvingIterator<ISPO, ISPO>(
            ...
            protected ISPO resolve(final ISPO spo) {
              ...
              final ISPO timestamped = new SPO(spo.s(), spo.p(), timestamp, StatementEnum.Explicit);
