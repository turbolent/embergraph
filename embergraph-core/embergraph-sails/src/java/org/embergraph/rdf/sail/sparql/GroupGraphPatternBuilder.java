 ...
 * @version $Id: GroupGraphPatternBuilder.java 5064 2011-08-21 22:50:55Z thompsonbry $
 * @openrdf
 */
public class GroupGraphPatternBuilder extends TriplePatternExprBuilder {
  ...
  @Override
  public final ConstructNode visit(final ASTConstruct node, final Object data)
   ...
   * patterns.
   *
   * <p>Note: (NOT) EXISTS uses a temporary graph pattern in order to avoid the side-effect on the
   * parent's graph pattern. Other value functions with inner graph patterns should do this as well.
   *
   * @return The {@link GroupNodeBase}. This return value is used by the visitor method for the
   *     {@link ASTWhereClause} and also in SPARQL UPDATE.
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/562">Sub-select in INSERT cause
   *     NPE in UpdateExprBuilder </a>
   */
  @Override
  public final GroupNodeBase<?> visit(final ASTGraphPatternGroup node, Object data)
      throws VisitorException {
    ...
    if (ret instanceof SubqueryRoot) {
      ...
      if (node.jjtGetParent() instanceof ASTWhereClause) {
         ...
         * </pre>
         *
         * Note: This no longer returns [null] due to the problem cited in the ticket below.
         *
       ...
       */
      ASTGraphGraphPattern scopePattern = firstASTGraphGraphAncestor(node.jjtGetParent());
      if (scopePattern != null) {
        Node child = scopePattern.jjtGetChild(0);
        if (child != null) {
          final TermNode s = (TermNode) scopePattern.jjtGetChild(0).jjtAccept(this, data);

          if (s != null) joinGroup.setContext(s);
        }
      }
    ...
    if (log.isInfoEnabled())
      log.info("\ndepth=" + depth(node) + ", graphPattern(out)=" + graphPattern);
   ...
   */
  @Override
  public final Void visit(final ASTOptionalGraphPattern node, Object data) throws VisitorException {
    ...
    if (tmp instanceof SubqueryRoot) {
       ...
       *     shared variables when lifting out named subqueries </a>
       */
   ...
   *
   * <p>Note: The grammar file treats OPTIONAL somewhat differently than MINUS due to the scope of
   * the filters. For OPTIONAL, the child group can see the variable bindings in the parent group.
   * That is not true for MINUS. Therefore the code in this method only sets the MINUS attribute on
   * the group and does not need to create a new group.
   */
  @Override
  public Object visit(final ASTMinusGraphPattern node, Object data) throws VisitorException {
    ...
    final JoinGroupNode group = (JoinGroupNode) graphPattern.buildGroup(new JoinGroupNode()).get(0);
  ...
  @Override
  public final Void visit(final ASTGraphGraphPattern node, Object data) throws VisitorException {

    final TermNode oldContext = graphPattern.getContext();
    final Scope oldScope = graphPattern.getStatementPatternScope();

    final TermNode newContext = (TermNode) node.jjtGetChild(0).jjtAccept(this, null);
   ...
   * </pre>
   *
   * That is, UNION in a binary operator in the parse tree. If the right hand argument is also a
   * UNION, then you are looking at a sequence of UNIONs at the same level. Such sequences should be
   * turned into a single {@link UnionNode} in the embergraph AST.
   *
   * <p>In contrast, this is a UNION of two groups with another UNION embedded in the 2nd group.
   *
   * <pre>
   * select ?s where { { ?s ?p1 ?o } UNION  { { ?s ?p2 ?o } UNION  { ?s ?p3 ?o } }  }
   * </pre>
   ...
   */
  @Override
  public final Void visit(final ASTUnionGraphPattern node, Object data) throws VisitorException {
   ...
   */
  @Override
  public final Object visit(final ASTBind node, final Object data) throws VisitorException {

    if (node.jjtGetNumChildren() != 2)
      throw new AssertionError(
          "Expecting two children, not " + node.jjtGetNumChildren() + ", node=" + node.dump(">>>"));
   ...
   */
  @Override
  public final BindingsClause visit(final ASTInlineData node, Object data) throws VisitorException {
   ...
   */
  @Override
  public final AssignmentNode visit(final ASTLet node, final Object data) throws VisitorException {

    if (node.jjtGetNumChildren() != 2)
      throw new AssertionError(
          "Expecting two children, not " + node.jjtGetNumChildren() + ", node=" + node.dump(">>>"));

    final ValueExpressionNode ve = (ValueExpressionNode) node.jjtGetChild(1).jjtAccept(this, data);
   ...
   */
  @Override
  public final ValueExpressionNode visit(final ASTConstraint node, final Object data)
      throws VisitorException {
   ...
   * {@link PrefixDeclProcessor}.
   *
   * <p>TODO Do we need to pass through the baseURI? Can this be used to abbreviate serialized IRIs?
   * (I would think that we would use the prefix declarations for that.)
   */
  @Override
  public final Void visit(final ASTServiceGraphPattern node, final Object data)
      throws VisitorException {

    // left arg is the service reference (a value expression).
    final TermNode serviceRef = (TermNode) node.jjtGetChild(0).jjtAccept(this, null);
