...
class GroupGraphPattern {
  ...
  public void addSP(final TermNode s, final TermNode p, final TermNode o) {
    ...
    if (log.isInfoEnabled()) log.info("pattern= ( " + s + " " + p + " " + o + " )");
  ...
  public void addSP(final StatementPatternNode sp) {
    ...
    if (log.isInfoEnabled()) log.info("pattern=" + sp);
  ...
  public void addConstraint(final ValueExpressionNode constraint) {
    ...
    if (log.isInfoEnabled()) log.info("constraint=" + constraint);
  ...
  public JoinGroupNode getSingletonGroup() {
    ...
    if (children.size() != 1)
      throw new RuntimeException("Expecting one child, not " + children.size());
