...
public class ServiceScore {
   ...
   *
   * <p>Note: This needs to be a URL, not just a relative path. Otherwise you get an NPE.
   *
   * <p>This is formed as:
   *
   * <pre>
   ...
   *
   * <p>The <code>port</code> is obtained from {@link HAGlue#getNSSPort()}.
   *
   * <p>TODO How do we configure the protocol for the remote NSS instance? This code assumes that it
   * is <code>http</code>, but <code>https</code> is also possible. This could be handled by an
   * {@link IHARequestURIRewriter} but maybe the {@link HAGlue} interface should be declaring this
   * too?
   */
   ...
   */
  ServiceScore(final UUID serviceUUID, final String hostname, final String requestURI) {

    if (serviceUUID == null) throw new IllegalArgumentException();

    if (hostname == null) throw new IllegalArgumentException();

    if (requestURI == null) throw new IllegalArgumentException();
   ...
   * @param contextPath The Context-Path of the web application (/embergraph).
   * @param serviceUUID The {@link UUID} of the service.
   * @return The {@link ServiceScore}.
   * @throws IllegalArgumentException if any argument is <code>null</code>.
   * @throws ClassCastException if the {@link IIndexManager} is not an {@link HAJournal}.
   * @throws IOException If an RMI to the {@link HAGlue} proxy fails.
  ...
  public static ServiceScore newInstance(
    ...
    final String requestURI = "http://" + hostname + ":" + port + contextPath;
