...
import org.embergraph.bop.engine.StaticAnalysisStat;
...
public class QueryServlet extends EmbergraphRDFServlet {
   ...
   * used to support chaining of a linked data GET as a SPARQL DESCRIBE query.
   */
  ...
  /** The name of the URL query parameter that contains the SPARQL UPDATE request. */
   ...
   *
   * <dl>
   *   <dt>{@value ITx#UNISOLATED}
   *   <dd>A unisolated operation (iff the operation is a mutation - query is always against a
   *       read-only snapshot). See {@link ITx#UNISOLATED}.
   *   <dt>{@value ITx#READ_COMMITTED}
   *   <dd>A read-only operation with snapshot isolation semantics against the most recent commit
   *       point. See {@link ITx#READ_COMMITTED}
   *   <dt>timestamp
   *   <dd>The operation will be executed again the most recent committed state whose commit
   *       timestamp is less than or equal to <i>timestamp</i>.
   *   <dt>transaction
   *   <dd>The operation will be isolated by the transaction. The namespace MUST support isolatable
   *       indices. See {@link EmbergraphSail.Options#ISOLATABLE_INDICES}.
   * </dl>
   *
   * <p>When not specified, the default for read-only operations is determined by {@link
   * SparqlEndpointConfig#timestamp} and will generally be either {@link ITx#READ_COMMITTED} or a
   * specific commit time that has been pinned by the server using {@link ConfigParams#READ_LOCK}.
   * The default for mutation operations is {@link ITx#UNISOLATED} if the namespace does not support
   * isolatable indices and a read/write transaction scoped to the operation otherwise.
   *
   * @see TxServlet
   ...
   */
  private void doUUID(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
  ...
  private void doServiceDescription(final HttpServletRequest req, final HttpServletResponse resp) {
    ...
    try {
      ...
      final String[] serviceURI = EmbergraphServlet.getServiceURIs(getServletContext(), req);
      ...
      final Graph g = new LinkedHashModel();
      {
        ...
        final SparqlEndpointConfig config = getEmbergraphRDFContext().getConfig();

        sd.describeService(true /* describeStatistics */, config.describeEachNamedGraph);
      }
   ...
   * @throws IOException
   */
  private void doSparqlUpdate(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (updateStr == null) {
      ...
      buildAndCommitResponse(
          resp, HTTP_BADREQUEST, MIME_TEXT_PLAIN, "Required parameter not found: " + ATTR_UPDATE);
    ...
    try {
       ...
       */
      submitApiTask(
              new SparqlUpdateTask(
                  req, resp, namespace, timestamp, updateStr, bindings, getEmbergraphRDFContext()))
          .get();
  ...
  void doSparqlQuery(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    if (queryStr == null) {
      ...
      buildAndCommitResponse(resp, HTTP_BADREQUEST, MIME_TEXT_PLAIN, "Not found: query");
    ...
    // @see BLZG-1207
    final boolean includeInferred = getBooleanValue(req, INCLUDE_INFERRED, true /* default */);
...

        {
          ...
          if (queryTask.explain) {
            ...
            if (isAttachment(queryTask.mimeType)) {
              ...
              resp.setHeader(
                  "Content-disposition",
                  "attachment; filename=query" + queryTask.queryId + "." + queryTask.fileExt);
            }
   ...
   * turning it into a SPARQL DESCRIBE query.
   *
   * @throws IOException
   */
  private static String getQueryString(final HttpServletRequest req) throws IOException {
    if (RESTServlet.hasMimeType(req, MIME_SPARQL_QUERY)) {
      // return the body of the POST, see trac 711
      return readFully(req.getReader());
    }
    return req.getParameter(ATTR_QUERY) != null
        ? req.getParameter(ATTR_QUERY)
        : (String) req.getAttribute(ATTR_QUERY);
  }

  private static String getUpdateString(final HttpServletRequest req) throws IOException {
   ...
   * @param os
   * @throws Exception
   *     <p>TODO The complexity here is due to the lack of a tight coupling between the {@link
   *     RunningQuery}, the {@link EmbergraphSailQuery}, and the {@link IRunningQuery}. It was not
   *     possible to obtain that tight coupling with the 1.0.x releases of embergraph due to the
   *     integration with the Sail. This led to the practice of setting the query {@link UUID} so we
   *     could resolve it once the query was executing using {@link
   *     QueryEngine#getRunningQuery(UUID)}. This also required the spin loops in explainQuery()
   *     since we had to wait for the {@link IRunningQuery} to become available. This is also the
   *     reason why we can fail to report parts of the explanation for fast queries.
   *     <p>This issue could be revisited now. Probably the right way to do this is by defining our
   *     own evaluate() method on the {@link EmbergraphSailQuery} which would provide either an
   *     object to be monitored or an interface for a query listener. Either approach could be used
   *     to ensure that we always have the {@link IRunningQuery} for an {@link AbstractQueryTask}
   *     which was submitted for evaluation.
   *     <p>I have not moved on this issue because the RTO integration will change things again.
   *     Right now, we have one {@link IRunningQuery} per top-level submitted query (plus one for
   *     each named subquery). With the RTO integration, there could be more {@link IRunningQuery}s
   *     issued and we will also want to paint the statics which it uncovers in its rounds.
   */
  private static void explainQuery(
     ...
     */
    if (log.isDebugEnabled()) log.debug("Will build explanation");
    UUID queryId2 = null;
    ...
    final HTMLBuilder doc = new HTMLBuilder(queryTask.charset.name(), w);
    {
       ...
       */
      if (queryId2 != null) {
        if (log.isDebugEnabled()) log.debug("Resolving IRunningQuery: queryId2=" + queryId2);
        final IIndexManager indexManager =
            EmbergraphServlet.getIndexManager(queryTask.req.getServletContext());
        ...
        if (q != null) if (log.isDebugEnabled()) log.debug("Resolved IRunningQuery: query=" + q);
      }
      ...
      if (q != null) {
...

        {
          ...
          final Iterator<BOp> it = ExplainHints.explainHintAnnotatedBOpIterator(optimizedAST);
   ...
   *     </a>
   */
  private void doEstCard(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    try {
      ...
      buildAndCommitResponse(resp, HTTP_BADREQUEST, MIME_TEXT_PLAIN, ex.getLocalizedMessage());
      return;
    }

    if (log.isInfoEnabled())
      log.info(
          "ESTCARD: access path: (exact="
              + exact
              + ", s="
              + s
              + ", p="
    ...
    try {
      ...
      submitApiTask(
              new EstCardTask(req, resp, getNamespace(req), getTimestamp(req), exact, s, p, o, c))
          .get();
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        if (c != null && c.length > 0) {
          for (Resource r : c) {
            rangeCount += conn.getTripleStore().getAccessPath(s, p, o, r).rangeCount(exact);
          }
        } else {
          rangeCount +=
              conn.getTripleStore().getAccessPath(s, p, o, (Resource) null).rangeCount(exact);
        }
   ...
   */
  private void doHasStmt(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (log.isInfoEnabled())
      log.info(
          "HASSTMT: access path: (includeInferred="
              + includeInferred
              + ", s="
              + s
              + ", p="
    ...
    try {
      ...
      submitApiTask(
              new HasStmtTask(
                  req, resp, getNamespace(req), getTimestamp(req), includeInferred, s, p, o, c))
          .get();
  ...
  private static class HasStmtTask extends AbstractRestApiTask<Void> {
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        // REST API.
        final boolean found =
            conn.hasStatement(s, p, o, includeInferred, c == null ? new Resource[0] : c);
  ...
  /** Return statements. */
  private void doGetStmts(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (log.isInfoEnabled())
      log.info(
          "GETSTMTS: access path: (includeInferred="
              + includeInferred
              + ", s="
              + s
              + ", p="
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        if (mimeTypes != null) {
          ...
          while (mimeTypes.hasMoreElements()) {
            for (String mt : mimeTypes.nextElement().split(",")) {
              mt = mt.trim();
              RDFFormat fmt = RDFWriterRegistry.getInstance().getFileFormatForMIMEType(mt);
              if (conn.getTripleStore().isQuads()
                      && (mt.equals(RDFFormat.NQUADS.getDefaultMIMEType())
                          || mt.equals(RDFFormat.TURTLE.getDefaultMIMEType()))
                  || !conn.getTripleStore().isQuads() && fmt != null) {
                mimeType = mt;
                format = fmt;
                break mimeTypesLoop;
              }
        ...
        if (format == null) {
          ...
          format = RDFWriterRegistry.getInstance().getFileFormatForMIMEType(mimeType);
        }
  ...
  private void doContexts(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    try {
      ...
      submitApiTask(new RestApiGetContextsTask(req, resp, getNamespace(req), getTimestamp(req)))
          .get();
   ...
   * @param resp
   */
  private void doShardReport(final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
    ...
    if (!getEmbergraphRDFContext().isScaleOut()) {
      buildAndCommitResponse(resp, HTTP_BADREQUEST, MIME_TEXT_PLAIN, "Not scale-out");
      return;
    }
    ...
    if (log.isInfoEnabled())
      log.info("SHARDS: access path: (s=" + s + ", p=" + p + ", o=" + o + ", c=" + c + ")");

    try {

      submitApiTask(
              new ShardsTask(
                  req, resp, getNamespace(req), getTimestamp(req), s, p, o, c, doRangeCount))
          .get();

    } catch (Throwable t) {

      launderThrowable(
          t, resp, "SHARDS: access path: (s=" + s + ", p=" + p + ", o=" + o + ", c=" + c + ")");
    }
  }
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        final AccessPath<?> accessPath =
            (AccessPath<?>) conn.getTripleStore().getAccessPath(s, p, o, c);
        ...
        try {
          ...
          final IEmbergraphFederation<?> fed =
              (IEmbergraphFederation<?>) // getEmbergraphRDFContext()
                  getIndexManager();
          ...
          while (itr.hasNext()) {
            ...
            final IDataService ds = fed.getDataService(loc.getDataServiceUUID());

            final String hostname = ds == null ? "N/A" : ds.getHostname();

            AtomicInteger nshards = hosts.get(hostname);

            if (nshards == null) {

              hosts.put(hostname, nshards = new AtomicInteger());
            }
          ...
          final long rangeCount = doRangeCount ? accessPath.rangeCount(false /* exact */) : -1;
