...
public abstract class EmbergraphServlet extends HttpServlet implements IMimeTypes {
   ...
   */
  public static final transient String ATTRIBUTE_RDF_CONTEXT = EmbergraphRDFContext.class.getName();
   ...
   *
   * <p>Note: Do NOT reference the <code>HALoadBalancerServlet</code> or anything in the <code>
   * org.embergraph.rdf.sail.webapp.lbs</code> package here. It will drag in the jetty dependencies
   * and that breaks the tomcat WAR deployment.
   */
  ...
  public static final String HTTP_HEADER_EMBERGRAPH_READ_ONLY = "X-EMBERGRAPH-READ-ONLY";
  ...
  protected final EmbergraphRDFContext getEmbergraphRDFContext() {
  ...
  static final EmbergraphRDFContext getEmbergraphRDFContext(final ServletContext servletContext) {
    ...
    //
    //            m_context =
    return getRequiredServletContextAttribute(servletContext, ATTRIBUTE_RDF_CONTEXT);
  ...
  //    private volatile EmbergraphRDFContext m_context;
  ...
  /** The backing {@link IIndexManager}. */
  public static IIndexManager getIndexManager(final ServletContext servletContext) {
   ...
   *
   * <p><strong>The servlet API MUST NOT close the output stream from within the a submitted
   * mutation task. Closing the output stream within the mutation task permits the client to
   * conclude that the operation was finished before the group commit actually occurs which breaks
   * the visibility guarantee of an ACID commit (the change is not yet visible).</strong>
   *
   * <p>This arises because the {@link AbstractApiTask} implementation has invoked conn.commit() and
   * hence believes that it was successful, but the {@link AbstractTask} itself has not yet been
   * through a checkpoint and the write set for the commit group has not yet been melded into a
   * stable commit point. Instead, the caller MUST allow the servlet container to close the output
   * stream once the submitted task has completed successfully (at which point the group commit will
   * be stable). This provides the necessary and correct visibility barrier for the updates.
   *
   * <p><strong>CAUTION: Non-success outcomes MUST throw exceptions!</strong> Once the flow of
   * control enters an {@link AbstractRestApiTask} the task MUST throw out a typed exception that
   * conveys the necessary information to the launderThrowable() code which can then turn it into an
   * appropriate HTTP response. If the task does not throw an exception then it is presumed to be
   * successful and it will join the next group commit. Failure to follow this caution can cause
   * partial write sets to be made durable, thus breaking the ACID semantics of the API.
   *
   * @param task The task.
  ...
  static boolean isWritable(
    ...
    if (req.getHeader(HTTP_HEADER_EMBERGRAPH_READ_ONLY) != null
        || getConfig(servletContext).readOnly) {

      buildAndCommitResponse(resp, HTTP_METHOD_NOT_ALLOWED, MIME_TEXT_PLAIN, "Not writable.");
    ...
    switch (haStatus) {
      ...
      default:
        ...
        buildAndCommitResponse(resp, HTTP_METHOD_NOT_ALLOWED, MIME_TEXT_PLAIN, haStatus.name());
        // Not writable.  Response has been committed.
        return false;
    }
  }
  ...
  static boolean isReadable(
      final ServletContext ctx, final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
   ...
   */
  public static String[] getServiceURIs(
     ...
     *
     * <p>http://localhost:9999/embergraph/sparql
     *
     * <p>to
     *
     * <p>http://localhost:9999/embergraph/LBS/sparql
     *
     * <p>where LBS is the prefix of the load balancer servlet.
     */
    {
      final String prefix = (String) servletContext.getAttribute(ATTRIBUTE_LBS_PREFIX);
      ...
      if (prefix != null) {
        ...
        final int nextSlash = uri.indexOf('/', doubleSlash + 2 /* fromIndex */);
        ...
        final int endContextPath = nextSlash + contextPath.length();

        // everything up to the *start* of the ContextPath
        final String baseURL = uri.substring(0 /* beginIndex */, nextSlash /* endIndex */);
   ...
   * invoked before the group commit point! All code paths in the REST API that have actually
   * performed a mutation (vs simply reporting a client or server error before entering into their
   * mutation code path) MUST use {@link #submitApiTask(AbstractRestApiTask)}.
   *
   * <p>Note: It is NOT safe to invoke this method once you are inside an {@link
   * AbstractRestApiTask} EVEN if the purpose is to report a client or server error. The task MUST
   * throw out a typed exception that conveys the necessary information to the launderThrowable()
   * code which can then turn it into an appropriate HTTP response. If the task does not throw an
   * exception then it is presumed to be successful and it will join the next group commit!
   *
   * @param resp
   * @param status The http status code.
   * @param mimeType The MIME type of the response.
   * @param content The content (optional).
   * @param headers Zero or more headers.
   * @throws IOException
   */
  public static void buildAndCommitResponse(
      final HttpServletResponse resp,
      final int status,
  ...
  protected boolean getBooleanValue(
      final HttpServletRequest req, final String name, final boolean defaultValue) {
   ...
   *
   * @param req The request.
   * @param name The name of the request parameter to be decoded.
   * @return An array of decoded resources and never <code>null</code>. If the request parameter
   *     does not appear in the request then this method returns <code>Resource[0]</code>.
   * @see BD#NULL_GRAPH
   ...
   */
  protected static Resource[] decodeContexts(final HttpServletRequest req, final String name) {
  ...
  protected static String readFully(final Reader reader) throws IOException {
