 ...
 *
 * <p>We can confirm that the correct data is associated with the relevant commit time
 *
 * <p>On closing and re-opening the store, the historical data remains accessible, however, the
 * release time is recalculated when connections are closed. The code clarifies several of the
 * issues involved.
 *
...
public class ReleaseTimes {
  ...
  public static void main(String[] args)
      throws IOException, InterruptedException, SailException, RepositoryException {
    ...
    check(!checkState(repo, state3, devuri, RDFS.LABEL, mc));
    System.out.println(
        "History of last commit point of re-opened journal no longer accessible after update");
   ...
   *
   * <p>Commit, and return the store state.
   */
  private static long assertStatement(
      EmbergraphSailRepositoryConnection cxn, URI id, URI pred, Literal label)
   ...
   *
   * <p>An IllegalStateException is thrown if no valid connection can be established.
   *
   * <p>We then retrieve statements from the connection, checking that only the requested statement
   * is present.
   */
  ...
  public static Properties getProperties(String releaseAge) throws IOException {

    Properties properties = new Properties();

    // create temporary file for this application run
    if (filename == null) filename = File.createTempFile("BIGDATA", "jnl").getAbsolutePath();
