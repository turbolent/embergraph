 ...
 * results rise as more data is loaded.
 *
 * @author mikep
 */
public class ScaleOut {
   ...
   * @throws Exception
   */
  private static AbstractTripleStore createTripleStore(JiniFederation fed) throws Exception {

    AbstractTripleStore tripleStore = null;

    // locate the resource declaration (aka "open").
    tripleStore = (AbstractTripleStore) fed.getResourceLocator().locate(namespace, ITx.UNISOLATED);
    ...
    if (tripleStore == null) {
       ...
       */
      final Properties properties = fed.getClient().getProperties(ScaleOut.class.getName());
   ...
   * @throws Exception
   */
  private static AbstractTripleStore openTripleStore(JiniFederation fed, long timestamp)
      throws Exception {

    AbstractTripleStore tripleStore = null;

    // locate the resource declaration (aka "open").
    tripleStore = (AbstractTripleStore) fed.getResourceLocator().locate(namespace, timestamp);
  ...
  */
  ...
  private static class EmbergraphWriter implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        // get the unisolated triple store for writing
        final AbstractTripleStore tripleStore = openTripleStore(fed, ITx.UNISOLATED);
    ...
    private void loadU10(Repository repo) throws Exception {
      ...
      try {
        ...
        { // first add the LUBM ontology
          InputStream is = ScaleOut.class.getResourceAsStream("univ-bench.owl");
  ...
  private static class EmbergraphReader implements Runnable {
    ...
    private void doQuery() throws Exception {
      ...
      long transactionId = fed.getTransactionService().newTx(ITx.READ_COMMITTED);

      try {

        // open the read-only triple store
        final AbstractTripleStore tripleStore = openTripleStore(fed, transactionId);
