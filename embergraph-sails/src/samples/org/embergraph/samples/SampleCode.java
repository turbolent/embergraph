...
public class SampleCode {
   ...
   * @throws Exception
   */
  public void loadSomeDataFromADocument(Repository repo, String resource, String baseURL)
      throws Exception {
    ...
    try {
      InputStream is = getClass().getResourceAsStream(resource);
      if (is == null && new File(resource).exists()) is = new FileInputStream(resource);
      if (is == null) throw new Exception("Could not locate resource: " + resource);
   ...
   * @throws Exception
   */
  public void executeSelectQuery(Repository repo, String query, QueryLanguage ql) throws Exception {
   ...
   * @throws Exception
   */
  public void executeConstructQuery(Repository repo, String query, QueryLanguage ql)
      throws Exception {
  ...
  public boolean executeFreeTextQuery(Repository repo) throws Exception {
    if (((EmbergraphSailRepository) repo).getDatabase().getLexiconRelation().getSearchEngine()
        == null) {
      /*
    ...
    try {
      cxn.add(
          new URIImpl("http://www.embergraph.org/A"), RDFS.LABEL, new LiteralImpl("Yellow Rose"));
      cxn.add(new URIImpl("http://www.embergraph.org/B"), RDFS.LABEL, new LiteralImpl("Red Rose"));
      cxn.add(
          new URIImpl("http://www.embergraph.org/C"),
          RDFS.LABEL,
          new LiteralImpl("Old Yellow House"));
      cxn.add(new URIImpl("http://www.embergraph.org/D"), RDFS.LABEL, new LiteralImpl("Loud Yell"));
      cxn.commit();
    } catch (Exception ex) {
      cxn.rollback();
      throw ex;
  ...
  public boolean executeProvenanceQuery(Repository repo) throws Exception {
    ...
    try {
      ...
      cxn.add(getReader(getClass(), "provenance.rdf"), "", RDFFormat.RDFXML);
      cxn.commit();

      RepositoryResult<Statement> results = cxn.getStatements(null, null, null, false);
   ...
   * behavior is controlled by the deployment mode (RW, WORM, or cluster) and by {@link
   * AbstractTransactionService.Options#MIN_RELEASE_AGE}. Except for the WORM deployment mode, you
   * MUST guard a historical commit point on which you want to read using a read-lock. The read-lock
   * itself is just a read-only connection. It can be obtained for any historical commit point that
   * you want to "pin" and can be released once you are no longer need to "pin" that commit point.
   *
   * <p>The read-lock is not required for the WORM deployment because it never releases historical
   * commit points.
   *
   * @param repo
   * @throws Exception
  ...
  public void executeHistoricalQuery(final Repository repo) throws Exception {
    ...
    final IIndexManager indexManager =
        ((EmbergraphSailRepository) repo).getDatabase().getIndexManager();

    final boolean isJournal = indexManager instanceof Journal;

    final boolean isWorm =
        isJournal
            && ((Journal) indexManager).getBufferStrategy().getBufferMode().getStoreType()
                == StoreTypeEnum.WORM;
    ...
    try {
      ...
      final RepositoryConnection readLock =
          isWorm ? null : ((EmbergraphSailRepository) repo).getReadOnlyConnection();
      ...
      try {
        ...
        history = ((EmbergraphSailRepository) repo).getReadOnlyConnection(time);
      ...
      try {
        ...
        final TupleQuery tupleQuery = history.prepareTupleQuery(QueryLanguage.SPARQL, query);
        tupleQuery.setIncludeInferred(false /* includeInferred */);
        TupleQueryResult result = tupleQuery.evaluate();

        Collection<BindingSet> answer = new LinkedList<BindingSet>();
        answer.add(createBindingSet(new BindingImpl("s", MIKE)));
   ...
   * @throws Exception
   */
  public void doLUBMTest(final String lubmResource) throws Exception {
    ...
    */
    ...
    try {
      ...
      cxn.add(getReader(getClass(), "univ-bench.owl"), LUBM.NS, RDFFormat.RDFXML);
      ...
      EmbergraphSailConnection sailCxn =
          (EmbergraphSailConnection) ((EmbergraphSailRepositoryConnection) cxn).getSailConnection();
      ...
      */
      ...
      sb.addTriplePattern(
          "?x", LUBM.WORKS_FOR, new URIImpl("http://www.Department0.University0.edu"));
      ...
      final GraphQuery graphQuery = cxn.prepareGraphQuery(QueryLanguage.SPARQL, sb.toString());
  ...
  public static void main(final String[] args) {
    ...
    try {
      ...
      if (properties.getProperty(AbstractTransactionService.Options.MIN_RELEASE_AGE) == null) {
        // Retain old commit points for at least 60s.
        properties.setProperty(AbstractTransactionService.Options.MIN_RELEASE_AGE, "60000");
      }
      ...
      try {
        ...
        final URI MIKE = new URIImpl("http://www.embergraph.org/rdf#Mike");
        sampleCode.loadSomeData(repo);
        System.out.println("Loaded sample data.");
        sampleCode.readSomeData(repo, MIKE);
        sampleCode.executeSelectQuery(
            repo, "select ?p ?o where { <" + MIKE.toString() + "> ?p ?o . }", QueryLanguage.SPARQL);
        System.out.println("Did SELECT query.");
        sampleCode.executeConstructQuery(
            repo,
            "construct { <"
                + MIKE.toString()
                + "> ?p ?o . } where { <"
        ...
        // sampleCode.doU10(); // I see loaded: 1752215 in 116563 millis: 15032 stmts/sec, what do
        // you see?
        // sampleCode.doU1();
  ...
  protected void compare(final TupleQueryResult result, final Collection<BindingSet> answer)
