...
if [ -z "${ECLIPSE_WORKSPACE}" ] ; then
    ...
    #Deployment artifacts used in Eclipse
 	mvn -f "${BASE_DIR}"/../pom.xml eclipse:eclipse -P Deployment --projects embergraph-war,embergraph-jar $*
    ...
    #Deployment artifacts used in Eclipse
 	mvn -f "${BASE_DIR}"/../pom.xml eclipse:eclipse -Declipse.workspace="${ECLIPSE_WORKSPACE}" -P Deployment --projects embergraph-war,embergraph-jar $*
