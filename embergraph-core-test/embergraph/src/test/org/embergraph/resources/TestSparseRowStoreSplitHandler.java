...
public class TestSparseRowStoreSplitHandler extends AbstractTestSegSplitter {
   ...
   * @param pmd
   */
  private BTree generateSparseRowStoreData(
      final IJournal store, final String name, final int nrows, final LocalPartitionMetadata pmd) {
    ...
    final String[] names = new String[] {"Bryan", "Mike", "Martyn", "Paul", "Julie"};
    ...
    try {
      ...
      try {
        ...
        final Split[] splits =
            SplitUtility.getSplits(
                pidFactory, pmd, seg, nominalShardSize, new LogicalRowSplitHandler());
       ...
       * Look at the 1st logical row before and after the separator
         ...
         */
        {
          ...
          final Long primaryKeyBefore = (Long) keyDecoderBefore.getPrimaryKey();

          assertNotNull(primaryKeyBefore);

          final Long primaryKeyAfter = (Long) keyDecoderAfter.getPrimaryKey();
          ...
          assertTrue(primaryKeyBefore.longValue() < primaryKeyAfter.longValue());
          ...
          final Map<String, Object> rowBefore = rowStore.read(schema, primaryKeyBefore);

          assertNotNull(rowBefore);

          // Extract the logical row for those primary keys.
          final Map<String, Object> rowAfter = rowStore.read(schema, primaryKeyAfter);
