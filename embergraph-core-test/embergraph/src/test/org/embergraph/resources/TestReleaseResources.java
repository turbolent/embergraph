...
public class TestReleaseResources extends AbstractResourceManagerTestCase {
  ...
  public static class TestWithCopyNoRelease extends TestReleaseResources {
     ...
     * rather than causing an index segment build to be scheduled. The original journal should not
     * be released.
     *
     * @throws IOException
     * @throws ExecutionException
    ...
    public void test() throws InterruptedException, ExecutionException {
      ...
      // for j0
      assertEquals(
          j1.getRootBlockView().getFirstCommitTime(),
          resourceManager.getCommitTimeStrictlyGreaterThan(createTime1));
  ...
  public static class TestWithCopyImmediateRelease extends TestReleaseResources {
     ...
     * rather than causing an index segment build to be scheduled. The original journal should be
     * released (deleted) during synchronous overflow processing.
     *
     * @throws IOException
     * @throws ExecutionException
     * @throws InterruptedException
    ...
    public void test() throws InterruptedException, ExecutionException {
       ...
       * exactly [j1].
       */
      final Set<UUID> actualResourceUUIDs =
          resourceManager.getResourcesForTimestamp(j1.getRootBlockView().getFirstCommitTime());

      System.err.println("resources=" + actualResourceUUIDs);

      assertSameResources(new IRawStore[] {j1}, actualResourceUUIDs);
    }
  }
  ...
  public static class TestWithCopy_NonZeroMinReleaseAge extends TestReleaseResources {
     ...
     * longer if you run into problems).
     *
     * <p>Note: 200000 is 200 seconds. This can be used for debugging, but always restore the value
     * so that the test will run in a reasonable time frame.
     */
    ...
    public Properties getProperties() {
      ...
      properties.setProperty(
          AbstractTransactionService.Options.MIN_RELEASE_AGE, "" + MIN_RELEASE_AGE);
     ...
     * synchronous overflow processing (assuming that synchronous overflow processing is
     * substantially faster than the [minReleaseAge]). We then wait until the [minReleaseAge] has
     * passed and force overflow processing again and verify that the original journal was released
     * while the 2nd and 3rd journals are retained.
     *
     * @throws IOException
     * @throws ExecutionException
     * @throws InterruptedException
