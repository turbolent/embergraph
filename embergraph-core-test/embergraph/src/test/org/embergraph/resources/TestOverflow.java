...
//     * Forces the use of persistent journals so that we can do overflow
   ...
   * on that index. An overflow operation is executed, which causes a new {@link Journal} to be
   * created and brought into play. The test then verifies that the index is re-defined on the new
   * journal such that its view includes the data on the old journal as well.
   *
   * @throws IOException
   */
  public void test_overflow() throws IOException {
    ...
    final int nentries = 100;
    {
      ...
      IndexMetadata indexMetadata = new IndexMetadata(indexName, UUID.randomUUID());
      ...
      indexMetadata.setPartitionMetadata(
          new LocalPartitionMetadata(
              ...
              null, // rightSeparator.
              new IResourceMetadata[] {journal.getResourceMetadata()},
