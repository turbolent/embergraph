...
public class TestBuildTask2 extends AbstractResourceManagerTestCase {
  ...
  private static final long maxSumSegBytes = (Bytes.kilobyte * 6);
   ...
   * field.
   *
   * <p>Note: There needs to be a preponderance of inserts so the test will eventually complete. If
   * there are a lot of deletes and a compacting merge is always chosen then the resulting index
   * segment always will be small and will fail to trigger an incremental build so the test will not
   * terminate.
   */
   ...
   * actually a compacting merge as well as cases where only an ordered subset of the sources are
   * accepted, which is the true incremental build (the use case when there is only data on the
   * journal also qualifies as a compacting merge since we are able to discard deleted tuples).
   *
  ...
  public Properties getProperties() {
    ...
    properties.setProperty(
        OverflowManager.Options.MAXIMUM_BUILD_SEGMENT_BYTES, "" + maxSumSegBytes);
   ...
   * not result in a compact view.
   *
   * <p>Note: If an index partition view has only a little bit of data, then the "build" operation
   * will actually incorporate all sources in the view, in effect doing a compacting merge instead
   * of an incremental build in an effort to keep down the complexity of the view when that can be
   * done with little cost. Therefore, it actually takes several passes before we have enough data
   * on hand to take the code path which does an incremental build rather than a compacting merge.
   *
   * @todo Run this test through two or more cycles such that #of index segments in the view builds
   *     up to more than just one, perhaps until there is enough data on hand to require a split.
   */
  public void test_builds() throws InterruptedException, ExecutionException {
    ...
    while (npasses++ < 50) {
      ...
      final Event e =
          new Event(resourceManager.getFederation(), new EventResource(name), "test")
              .addDetail("pass", "" + npasses)
              .start();
      ...
      try {
         ...
         */
        {
          ...
          for (int i = 0; i < nentries; i++) {
            ...
            if (randomKeys) {
              ...
              keys[i] = TestKeyBuilder.asSortKey(base + r.nextInt(maxKeyInc) + 1);
            } else {
              // strictly increasing non-random ordered keys.
              keys[i] = TestKeyBuilder.asSortKey(i + npasses * nentries);
            }
        ...
        if (percentRemove > 0) {
          ...
          final int maxentries = (int) ((r.nextInt(maxtuples) + 1) * percentRemove + 1);

          final byte[][] keys = new byte[maxentries][];

          for (int i = 0; i < maxentries && groundTruth.getEntryCount() > 0; i++) {

            final long entryCount2 = groundTruth.getEntryCount();

            if (entryCount2 > Integer.MAX_VALUE) fail("Test case does not handle massive indices");
          ...
          final IIndexProcedure proc =
              BatchRemoveConstructor.RETURN_MUTATION_COUNT.newInstance(
                  indexMetadata, 0 /* fromIndex */, nentries /* toIndex */, keys, null /* vals */);
        ...
        {
          final long priorOverflowCount = resourceManager.getSynchronousOverflowCount();

          // createTime of the old journal.
          final long createTime0 =
              resourceManager.getLiveJournal().getRootBlockView().getCreateTime();
          ...
          oldJournal = (ManagedJournal) resourceManager.getJournal(createTime0);
        ...
        {
          final ILocalBTreeView actual =
              resourceManager.getIndex(name, oldJournal.getLastCommitTime());

          acceptedView = new BuildViewMetadata(actual, maxSumSegBytes, e);

          if (log.isInfoEnabled()) {

            log.info(
                AbstractResourceManagerTask.toString(
                    "actualViewResources", actual.getResourceMetadata()));

            log.info(
                AbstractResourceManagerTask.toString("actualViewSources  ", actual.getSources()));

            log.info("\npass=" + npasses + " : acceptedView=" + acceptedView);
          }
        ...
        final BuildResult buildResult;
        {
          ...
          if (log.isInfoEnabled()) log.info("pre-condition view: " + vmd);
           ...
           */
          {
            final IResourceMetadata[] expected =
                resourceManager
        ...
        if (acceptedView.compactingMerge) {
          ...
          final IndexSegmentStore segStore =
              (IndexSegmentStore) resourceManager.openStore(buildResult.segmentMetadata.getUUID());
    ...
    fail("Terminated after npasses=" + npasses + " without doing incremental build.");
  }
}
