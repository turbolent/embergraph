 ...
 * "-U" option may be used to explicitly request the load average stats.) The "-n 999" tells it to
 * display up to 999 devices. Otherwise it will truncate the output at 80 columns.
 *
 * <pre>
 ...
 * </pre>
 *
 * The output of this command depends on the #of devices which are attached, and that can actually
 * change over time (device connect/disconnect). Unless we are going to aggregate across the devices
 * reported in each time period it might make more sense to only use this for the CPU stats, which
 * can be done by specifying "-n 0" for NO devices.
 *
 * <pre>
...
public class TestParse_iostat extends AbstractParserTestCase {
  ...
  /** Test parsing of a CPU only report from <code>iostat -d -C -n 0 -w 1 | cat</code> */
   ...
   * </pre>
   *
   * Note that the #of device columns will vary for this format. The headers will also periodically
   * repeat. The #of columns for the report can presumably vary as devices are connected and
   * disconnected!
   */
  public void test_parsing_iostat_devices_plus_cpu() {
    ...
    // test 2nd header parse.
    {
      //		final String h0 = "        disk0           disk1           disk2           disk3
      // disk4       cpu";
      //		final String h1 = "     KB/t tps  MB/s     KB/t tps  MB/s     KB/t tps  MB/s     KB/t tps
      // MB/s     KB/t tps  MB/s  us sy id";
      //		final String d0 = "   197.14  26  5.10    79.10   1  0.07     3.74   0  0.00    41.31   0
      // 0.00    13.03   0  0.00  31  6 63";
  ...
  protected void test_iostat_collector(String output) throws IOException, InterruptedException {
    ...
    final IOStatCollector.IOStatReader ioStatReader =
        (IOStatCollector.IOStatReader) ioStatCollector.getProcessReader();
    ...
    double cpu_usr =
        (Double)
            ((ICounter) counterSet.getChild(IProcessCounters.CPU).getChild("% User Time"))
                .getInstrument()
                .getValue();
    double tps =
        (Double)
            ((ICounter)
                    counterSet
                        .getChild(IProcessCounters.PhysicalDisk)
                        .getChild("Transfers Per Second"))
                .getInstrument()
                .getValue();
