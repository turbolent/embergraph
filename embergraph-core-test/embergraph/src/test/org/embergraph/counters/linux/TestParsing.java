...
public class TestParsing extends TestCase2 {
  ...
  public void test_splitDataLine01() {
    ...
    if (log.isInfoEnabled()) log.info(Arrays.toString(fields));
  ...
  public void test_splitDataLine02() {
    ...
    assertEquals(
        new String[] {"06:35:15", "501", "0.00", "0.01", "0.00", "1", "kjournald"}, fields);
  }
   ...
   * @param fields The fields parsed from that header.
   * @param field The field number in [0:#fields-1].
   * @param expected The expected value of the header for that field.
   */
  protected static void assertField(
      final String header, final String[] fields, final int field, final String expected) {
    ...
    if (field >= fields.length)
      throw new AssertionFailedError(
          "There are only " + fields.length + " fields, but field=" + field + "\n" + header);
  ...
  protected void test_pid_stat_collector(String output) throws IOException, InterruptedException {
    ...
    final PIDStatCollector.PIDStatReader pidStatReader =
        (PIDStatCollector.PIDStatReader) pidStatCollector.getProcessReader();
    ...
    long rss =
        (Long)
            ((ICounter) counterSet.getChild(IProcessCounters.Memory).getChild("Resident Set Size"))
                .getInstrument()
                .getValue();
