...
public class TestStripedCounters extends TestCase2 {
  ...
  public void test_stripedCounters() {
    ...
    for (int i = 1; i < batchSize; i++) {
      ...
      t.nreads++;
      t.release();
      if (log.isInfoEnabled()) log.info("pass=" + i + ", value=" + c.nreads);
      if (c.nreads != 0) fail("updated on pass: i=" + i + ", value=" + c.nreads);
    }
    ...
    // do one more update and verify outer instance has changed.
    {
      ...
      if (c.nreads == 0) fail("Counter value was not updated.");
    ...
    for (int i = 1; i < batchSize; i++) {
      ...
      t.release();
      if (log.isInfoEnabled()) log.info("pass=" + i + ", value=" + c.nreads);
      if (c.nreads != lastValue) fail("updated on pass: i=" + i + ", value=" + c.nreads);
    }
    ...
    // do one more update and verify outer instance has changed.
    {
      ...
      t.release();
      if (log.isInfoEnabled()) log.info("pass=" + batchSize + ", value=" + c.nreads);
      if (c.nreads == lastValue) fail("Counter value was not updated.");
