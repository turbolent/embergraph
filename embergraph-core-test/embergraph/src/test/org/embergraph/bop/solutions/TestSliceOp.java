...
public class TestSliceOp extends TestCase2 {
  ...
  /*
   * Unit test where the offset is never satisfied. For this test, all binding sets will be consumed
   * but none will be emitted.
   *
   * @throws InterruptedException
   * @throws ExecutionException
   */
  ...
  /*
   * Unit test where the offset plus the limit is never satisfied. For this test, all binding sets
   * will be consumed and some will be emitted, but the slice is never satisfied.
   *
   * @throws InterruptedException
   * @throws ExecutionException
   */
  public void test_slice_offsetPlusLimitNeverSatisfied()
  ...
  public void test_slice_threadSafe() throws Exception {
    ...
    doStressTest(500L /* offset */, 1500L /* limit */, timeout, ntrials, poolSize);
  }
  ...
  protected int doStressTest(
    ...
    final ThreadPoolExecutor service = (ThreadPoolExecutor) Executors.newFixedThreadPool(poolSize);
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info(getClass().getName() + "." + getName() + " : " + msg);

      if (nerror > 0) fail(msg);

      if (nexpected != naccepted) fail(msg);
  ...
  private static class MockRunningQuery extends org.embergraph.bop.engine.MockRunningQuery {
