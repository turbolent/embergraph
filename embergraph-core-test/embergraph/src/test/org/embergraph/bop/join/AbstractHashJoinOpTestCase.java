...
public abstract class AbstractHashJoinOpTestCase extends TestCase2 {
   ...
   * @return
   */
  protected abstract PipelineOp newJoin(
      final BOp[] args,
      final int joinId,
  ...
  public void test_join_simple_noJoinVars() {
    ...
    final IVariable<IV> y = Var.var("y");
    @SuppressWarnings("unchecked")
    final IVariable<IV>[] joinVars = new IVariable[] {
          /* x */
        };
    final UUID queryId = UUID.randomUUID();
    ...
    final PipelineOp query =
        newJoin(
            ...
            new NV(
                ...
                new IConstraint[] {
                  Constraint.wrap(new EQConstant(x, new Constant<IV>(setup.brad))),
                }));
  ...
  public void test_join_simple_selectOnly_x() {
    ...
    final PipelineOp query =
        newJoin(
            ...
            predOp,
            queryId,
            new NV(JoinAnnotations.SELECT, new IVariable[] {x}));
  ...
  public void test_optionalJoin_and_constraint() {
     ...
     * However, it is passed along any as an "optional" solution.
     *
     * <p>bset2: This has x:=Luke, which does not join. However, this is an optional join so x:=Luke
     * should be output anyway. There is a constraint that x!= Luke, but that constraint does not
     * fail the solution because it is an optional solution.
     *
     * <p>bset3: This has x:=Mary, which joins and is output as a solution.
     */
    final IAsynchronousIterator<IBindingSet[]> source;
    {
    ...
    final IBindingSet[] expected =
        new IBindingSet[] {
          ...
          new ListBindingSet(new IVariable[] {x}, new IConstant[] {new Constant<IV>(setup.luke)}),
          // bset3: joins.
          new ListBindingSet(new IVariable[] {x}, new IConstant[] {new Constant<IV>(setup.mary)}),
        };
  ...
  public void test_optionalJoin_withAltSink() {
    ...
    try {
      ...
      AbstractQueryEngineTestCase.assertSameSolutionsAnyOrder(expected2, sink2.iterator(), ft);
