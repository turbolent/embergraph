 ...
 *
 * <p>TODO Test variant with SELECT projects only the selected variables.
 *
...
public abstract class HashIndexOpTestCase extends TestCase2 {
   ...
   *     HTree} variants of the {@link HashIndexOp}).
   */
  protected abstract HashIndexOp newHashIndexOp(
      final String namespace, final BOp[] args, final NV... anns);
   ...
   * mechanisms for those two operators.
   */
  ...
  public void test_hashIndexOp_01() throws Exception {
    ...
    final INamedSolutionSetRef namedSolutionSet =
        NamedSolutionSetRefUtility.newInstance(queryId, solutionSetName, joinVars);

    final HashIndexOp op =
        newHashIndexOp(
            setup.namespace,
            BOp.NOARGS,
    ...
    final SolutionSetHashJoinOp op2 =
        newSolutionSetHashJoinOp(
            new BOp[] {op},
    ...
    final IRunningQuery runningQuery =
        queryEngine.eval(
            queryId, query, null /* queryAttributes */, concat(bindingSets1, bindingSets2));
   ...
   * unit test above. However, in this case the joinSet will have been created by the {@link
   * HashIndexOp} and utilized by the {@link SolutionSetHashJoinOp}.
   */
  @SuppressWarnings("rawtypes")
   ...
   *
   * <p>FIXME Also test with {@link IHashJoinUtility} query attribute as the <em>source</em> (rather
   * than a {@link SolutionSetStream}) and an {@link IPredicate}.
   */
  ...
  public void test_hashIndexOp_buildFromSolutionSet() throws Exception {
    ...
    final INamedSolutionSetRef namedSolutionSetSource =
    ...
    final SolutionSetStream stream;
    {
       ...
       */
      {
        IStriterator itr = new Striterator(Arrays.asList(solutionsToIndex).iterator());
    ...
    final IRunningQuery runningQuery =
        queryEngine.eval(queryId, query, queryAttributes, bindingSets);
  ...
  private static class ValidateIndexOp extends PipelineOp {
    ...
    interface Annotations extends PipelineOp.Annotations {
      ...
      /** The expected solutions that should be reported by a scan of the generated index. */
      String EXPECTED_SOLUTIONS = ValidateIndexOp.class.getName() + ".expectedSolutions";
    }

    public ValidateIndexOp(final BOp[] args, final Map<String, Object> annotations) {
    ...
    private static class ChunkTask implements Callable<Void> {
      ...
      public ChunkTask(final ValidateIndexOp op, final BOpContext<IBindingSet> context) {
      ...
      public Void call() {
        ...
        final IBindingSet[] expectedSolutions =
            (IBindingSet[]) op.getRequiredProperty(Annotations.EXPECTED_SOLUTIONS);
        ...
        final IQueryAttributes queryAttributes2 = context.getQueryAttributes();

        // verify that the generated solution set exists.
        assertNotNull(
            "Generated index not found: " + namedSetRef, queryAttributes2.get(namedSetRef));
