...
public class TestNamedSolutionSetRef extends TestCase2 {
  ...
  public void test_ctor() {
    ...
    {
      final INamedSolutionSetRef ref = new NamedSolutionSetRef(queryId, namedSet, joinVars);
  ...
  public void test_valueOf() {
    ...
    final INamedSolutionSetRef ref =
        NamedSolutionSetRefUtility.newInstance(queryId, namedSet, joinVars);
    ...
    if (log.isInfoEnabled()) log.info(ref2.toString());
    ...
    final INamedSolutionSetRef refX =
        NamedSolutionSetRefUtility.newInstance(namespace, timestamp, namedSet, joinVars);

    final String sX = refX.toString();

    if (log.isInfoEnabled()) log.info(sX);

    final INamedSolutionSetRef refX2 = NamedSolutionSetRefUtility.valueOf(sX);

    if (log.isInfoEnabled()) log.info(refX2.toString());
