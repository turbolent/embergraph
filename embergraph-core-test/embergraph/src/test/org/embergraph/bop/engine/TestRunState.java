 ...
 * @todo Unit tests of a query with multiple operators where the of {@link IChunkMessage}s generated
 *     from a non-terminal operator is zero. The query should terminate even though there are
 *     additional operators since there are no message available to trigger those operators.
 * @todo Unit tests where some messages target the alternative sink.
 * @todo Write query cancelled and operator error tests, including verifying that the {@link
 *     RunState} correctly reflects that nothing is left running.
 * @todo concurrent stress tests.
 * @todo test subquery evaluation, including things like cancelling a query terminates the subquery
 *     (slice in the subquery).
 */
public class TestRunState extends TestCase2 {
  ...
  private void assertSameState(final InnerState expected, final InnerState actual) {

    if (log.isInfoEnabled()) log.info("actual=" + actual);
    ...
    assertEquals("stepCount", expected.stepCount.get(), actual.stepCount.get());

    assertEquals(
        "totalAvailableCount",
        expected.totalAvailableCount.get(),
        actual.totalAvailableCount.get());

    assertEquals(
        "totalRunningCount", expected.totalRunningCount.get(), actual.totalRunningCount.get());

    assertEquals(
        "totalLastPassRemainingCount",
        expected.totalLastPassRemainingCount.get(),
        actual.totalLastPassRemainingCount.get());

    assertEquals("availableMap", expected.availableMap, actual.availableMap);

    assertEquals("runningMap", expected.runningMap, actual.runningMap);

    assertEquals("serviceIds", expected.serviceIds, actual.serviceIds);

    assertEquals("startedOn", expected.startedOn, actual.startedOn);

    assertEquals("doneOn", expected.doneOn, actual.doneOn);

    assertEquals("lastPassRequested", expected.lastPassRequested, actual.lastPassRequested);
  }
  ...
  public void test_ctor() {
    ...
    final InnerState expected = new InnerState(query, queryId, deadline, begin, bopIndex);
  ...
  public void test_ctor_correctRejection() {
    ...
    try {
      new RunState(new InnerState(null /* query */, queryId, deadline, begin, bopIndex));
    ...
    try {
      new RunState(new InnerState(query, null /* queryId */, deadline, begin, bopIndex));
      fail("Expecting: " + IllegalArgumentException.class);
    } catch (IllegalArgumentException ex) {
      if (log.isInfoEnabled()) log.info("Ignoring expected exception: " + ex);
    }
    ...
    try {
      new RunState(new InnerState(query, queryId, 0L /* deadline */, begin, bopIndex));
      fail("Expecting: " + IllegalArgumentException.class);
    } catch (IllegalArgumentException ex) {
      if (log.isInfoEnabled()) log.info("Ignoring expected exception: " + ex);
    }
    ...
    try {
      new RunState(new InnerState(query, queryId, -1L /* deadline */, begin, bopIndex));
      fail("Expecting: " + IllegalArgumentException.class);
    } catch (IllegalArgumentException ex) {
      if (log.isInfoEnabled()) log.info("Ignoring expected exception: " + ex);
    }
    ...
    try {
      new RunState(new InnerState(query, queryId, deadline, begin, null /* bopIndex */));
      fail("Expecting: " + IllegalArgumentException.class);
    } catch (IllegalArgumentException ex) {
      if (log.isInfoEnabled()) log.info("Ignoring expected exception: " + ex);
    }
  ...
  public void test_getOperatorRunState() {
    ...
    final PipelineOp join2Op =
        new MockPipelineOp(new BOp[] {join1Op}, new NV(PipelineOp.Annotations.BOP_ID, joinId2));
     ...
     */
    {
      assertEquals(RunStateEnum.AllDone, runState.getOperatorRunState(startId));

      assertEquals(RunStateEnum.AllDone, runState.getOperatorRunState(joinId1));

      assertEquals(RunStateEnum.AllDone, runState.getOperatorRunState(joinId2));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.Running, runState.getOperatorRunState(startId));

      assertEquals(RunStateEnum.Running, runState.getOperatorRunState(joinId1));

      assertEquals(RunStateEnum.Running, runState.getOperatorRunState(joinId2));
    }
   ...
   *
   * <p>TODO Do another variant where there are two such operators. For this variant, we want to
   * make sure that the last pass evaluation for each operator is executed properly.
   */
  public void test_getOperatorRunState_lastPassRequested() {
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.AllDone, runState.getOperatorRunState(orderId));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.Running, runState.getOperatorRunState(orderId));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.StartLastPass, runState.getOperatorRunState(orderId));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.RunningLastPass, runState.getOperatorRunState(orderId));
    }
  ...
  public void test_getOperatorRunState_atOnceRequested() {
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.AllDone, runState.getOperatorRunState(countId));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.Running, runState.getOperatorRunState(countId));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.StartLastPass, runState.getOperatorRunState(countId));
    }
     ...
     */
    {
      ...
      assertEquals(RunStateEnum.RunningLastPass, runState.getOperatorRunState(countId));
    }
   ...
   * allDone). The post-condition of the {@link RunState} object is verified after each action.
   *
  ...
  public void test_runSingleOperatorQuery() throws TimeoutException, ExecutionException {
    ...
    runState.startQuery(
        new LocalChunkMessage(
            queryController, queryId, startId, -1 /* partitionId */, new HashBindingSet()));
    ...
    runState.startOp(
        new StartOpMessage(
            queryId, startId, -1 /* partitionId */, serviceId, 1 /* nmessages */
  ...
  public void test_runTwoOperatorQuery() throws TimeoutException, ExecutionException {
    ...
    runState.startOp(
        new StartOpMessage(
            queryId, otherId, -1 /* partitionId */, serviceId, 1 /* nmessages */
   ...
   * @throws InterruptedException
   */
  public void test_deadline_haltOp()
      throws TimeoutException, ExecutionException, InterruptedException {
  ...
  public void test_lastPassRequested()
    ...
    runState.startOp(
        new StartOpMessage(
            queryId, orderId, -1 /* partitionId */, serviceId, 1 /* nmessages */
    ...
    //        expected.startedOn.put(orderId, newSet(new UUID[] { serviceId })); // already
    // accounted for.
  ...
  private void assertEquals(
      final Map<Integer, AtomicLong> expected, final Map<Integer, AtomicLong> actual) {
   ...
   */
  private void assertEquals(
      String msg, final Map<Integer, AtomicLong> expected, final Map<Integer, AtomicLong> actual) {
    ...
    while (eitr.hasNext()) {
      ...
      if (a == null) fail(msg + "Not expecting null: key=" + k);
      ...
      if (e.get() != a.get()) {
        ...
        fail(msg + "Wrong value: key=" + k + ", expected=" + e + ", actual=" + a);
      }
    }
  }
   ...
   * @param actual
   */
  private <T> void assertSameMap(final Map<Integer, T> expected, final Map<Integer, T> actual) {
  ...
  private <T> void assertSameMap(
      String msg, final Map<Integer, T> expected, final Map<Integer, T> actual) {
