 ...
 * @todo Test each ORDER BY implementation here (buffered on the Java heap, buffered on the native
 *     heap using the Memory Manager, external memory sort, radix sort variation, wordsort
 *     variation, etc).
 */
...
//     * Return an {@link IAsynchronousIterator} that will read a single, chunk
   ...
   * returns LT ZERO (0) when comparing with the previous solution visited by the query iterator.
   *
   ...
   * @throws Exception
   */
  protected void doOrderByTest(
      final int maxInt, final long timeout, final int ntrials, final int poolSize)
      throws Exception {
    ...
    final IRunningQuery q =
        queryEngine.eval(
            ...
            query,
            null /* attributes */,
            new LocalChunkMessage(queryEngine, queryId, startId, -1 /* partitionId */, chunks));
    ...
    try {
      final BindingSetComparator<?> c = new BindingSetComparator(sortOrder, valueComparator);
      while (itr.hasNext()) {
        ...
        for (IBindingSet thisSolution : chunk) {
          if (lastSolution != null) {
            if (c.compare(lastSolution, thisSolution) > 0) {
              fail(
                  "Solutions out of order: nvisited="
                      + (nsolutions + 1)
                      + ", lastSolution:"
                      + lastSolution
                      + ", thisSolution="
