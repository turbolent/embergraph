...
public class TestAll extends TestCase {
   ...
   *
   * <p>Most of the aggregation operators either can not "run" or can not emit their outputs until
   * all of the solutions have been materialized. For example:
   *
   * <ul>
   *   <li>ORDER BY : This operator must buffer the solutions until there no more operators can feed
   *       it solutions, at which point it must apply the sort order to all of the buffered
   *       solutions.
   *   <li>GROUP BY : When targeting a hash table, GROUP BY can pipeline aggregations, but it must
   *       do so into a hash table instance which is shared by each GROUP BY invocation for that
   *       query. <br>
   *       GROUP BY can also run on the result of an ORDER BY, in which case it is again pipelined,
   *       but it can not run until the ORDER BY begins to emit its solutions.
   *   <li>DISTINCT : When implemented using a hash table, this operator can be pipelined, but all
   *       invocations of the operator must share the same hash table state.<br>
   *       When implemented using an ORDER BY, this operator will run once and must wait for the
   *       ORDER BY to begin emitting its solutions before it can execute.
   * </ul>
   */
