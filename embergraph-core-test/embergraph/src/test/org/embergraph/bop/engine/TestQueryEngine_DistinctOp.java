...
/**
 * Test suite for DISTINCT solution operators when integrated with the query engine. This test suite
 * is designed to examine cases where the DISTINCT operator will have to buffer multiple chunks of
 ...
 *     <p>FIXME Finish this stress test. We can allocate an array of known distinct solutions and
 *     then randomly sample from the array to create redundancy. Correct operation can be easily
 *     verified by checking that the reported solutions are those in the array.
 */
public class TestQueryEngine_DistinctOp extends TestCase2 {
  ...
  //        {
  //            super ( new BOp [] {}, NV.asMap ( new NV [] { new NV (
  // ComparatorOp.Annotations.ORDER, sors ) } ) ) ;
  //            _sors = sors ;
  //        }
  //
