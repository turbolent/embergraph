...
public class TestBOpStats extends TestCase2 {
  ...
  private static void doSerializationTest(final BOpStats expected) {
    ...
    final BOpStats actual =
        (BOpStats) SerializerUtil.deserialize(SerializerUtil.serialize(expected));

    assertSameStats(expected, actual);
  }

  private static void assertSameStats(final BOpStats expected, final BOpStats actual) {
    ...
    assertEquals("chunksOut", expected.chunksOut.get(), actual.chunksOut.get());
    ...
    assertEquals("typeErrors", expected.typeErrors.get(), actual.typeErrors.get());

    assertEquals("mutationCount", expected.mutationCount.get(), actual.mutationCount.get());
  }
}
