...
public class TestQueryEngine extends AbstractQueryEngineTestCase {
  ...
  /*
   * Starts and stops the {@link QueryEngine}, but does not validate the semantics of shutdown()
   * versus shutdownNow() since we need to be evaluating query mixes in order to verify the
  ...
  public void test_query_startRun() throws Exception {
    ...
    //                        AbstractQueryEngineTestCase.newBindingSetIterator(new
    // HashBindingSet())));
    ...
    // validate the query solution stats.
    {
      ...
      assertNotNull(stats);
      if (log.isInfoEnabled()) log.info(stats.toString());
    ...
    // Verify results.
    {
      ...
      final IBindingSet[] expected = new IBindingSet[] {new ListBindingSet()};
  ...
  /*
   * Test the ability run a simple join without a {@link StartOp}. An empty binding set[] is fed
   * into the join. The join probes the index once for "Mary" and binds "Paul" when it does so.
   * There there is one solution, which is "value=Paul".
   *
   * @throws Exception
   */
  public void test_query_join1_without_StartOp() throws Exception {
    ...
    // validate the stats for the join operator.
    {
      ...
      assertNotNull(stats);
      if (log.isInfoEnabled()) log.info("join : " + stats.toString());
  ...
  /*
   * Test the ability run a simple join. There are three operators. One feeds an empty binding set[]
   * into the join, another is the predicate for the access path on which the join will read (it
   * probes the index once for "Mary" and binds "Paul" when it does so), and the third is the join
  ...
  public void test_query_join1() throws Exception {
    ...
    // validate the stats for the start operator.
    {
      final BOpStats stats = statsMap.get(startId);
      assertNotNull(stats);
      if (log.isInfoEnabled()) log.info("start: " + stats.toString());
  ...
  /*
   * Test the ability run a simple join when multiple binding sets are submitted as the initial
   * input. The access path associated with the join does not have any constants but the join picks
   * up bindings from the input binding sets and uses them to constrain the access path.
   *
   * @todo Support for this test is no longer present. It was lost when the {@link
   *     StandaloneChunkHandler} was written (ONE_MESSAGE_PER_CHUNK).
   */
  public void test_query_join1_multipleChunksIn() throws Exception {
    ...
    final IRunningQuery runningQuery =
        queryEngine.eval(queryId, query, null /* queryAttributes */, sources);
    ...
    // validate the stats for the slice operator.
    {
      ...
      assertNotNull(stats);
      if (log.isInfoEnabled()) log.info("slice: " + stats.toString());
  ...
  /*
   * Test verifies the ability close the iterator draining a result set before the query has
   * finished executing and also verify that the query is correctly terminated.
   *
   * <p>Note: This is difficult to test without having significant data scale since there is an
   * implicit race between the consumer and the producer to close out the query evaluation, but the
   * {@link PipelineDelayOp} can be used to impose sufficient latency on the pipeline that the test
   * can close the query buffer iterator first.
   *
   * <p>Note: This must also be tested in scale-out to make sure that the data backing the solutions
   * is not discarded before the caller can use those data.
   *
   * <p>This could be handled by: (a) materializing binding set objects out of a {@link ByteBuffer}
   * rather than using a live decode of the data in that {@link ByteBuffer}; or by (b) using an
   * special {@link IAllocationContext} which is scoped to the query results such that they are not
   * released until the iterator draining the buffer is closed.
   *
   * @throws Exception
   */
...
//     * Test the ability of the query engine to defer the evaluation of a one
  ...
  /*
   * Unit test runs chunks into a slice without a limit. This verifies that the query terminates
   * properly even though the slice is willing to accept more data.
   *
   * <p>When the {@link IRunningQuery} implementation supports it, the source data are presented in
   * multiple chunks in order to verify the behavior of the {@link SliceOp} across multiple (and
   * potentially concurrent) invocations of that operator.
   *
   * @throws Exception
   */
  public void test_query_slice_noLimit() throws Exception {
    ...
    // validate the stats for the slice operator.
    {
       ...
       * The unit test setup will result in four distinct chunks
        ...
        fail("Unknown implementation class: " + runningQuery.getClass().getName());
      }
    }
  }

  /*
   * Run a join with a slice. The slice is always evaluated on the query controller so adding it to
   * the query plan touches a slightly different code path from adding another join (joins are
   * evaluated shardwise, at least in scale-out).
   *
   * <p>Note: While the logic for visiting only the solutions selected by the slice can be tested
   * against a mock object, the integration by which a slice halts a query when it is satisfied has
   * to be tested against a {@link QueryEngine}.
   *
   * <p>This must also be tested in scale-out to make sure that the data backing the solutions is
   * not discarded before the caller can use those data. [This could be handled by materializing
   * binding set objects out of a {@link ByteBuffer} rather than using a live decode of the data in
   * that {@link ByteBuffer}.]
   */
  ...
  public void test_query_join2() throws Exception {
    ...
    // validate the stats for the 1st join operator.
    {
      ...
      assertNotNull(stats);
      if (log.isInfoEnabled()) log.info("join1: " + stats.toString());
    ...
    // validate the stats for the 2nd join operator.
    {
      ...
      assertNotNull(stats);
      if (log.isInfoEnabled()) log.info("join2: " + stats.toString());
  ...
  /*
   * A stress test of {@link #test_query_join2()} which runs a fixed number of presentations in a
   * single thread.
   *
   * @throws Exception
   */
  public void test_queryJoin2_stressTest() throws Exception {
    ...
    if (nsuccess < ntrials) {
      ...
      fail("Only completed " + nsuccess + " out of " + ntrials + " trials");
    }
  }

  /*
   * Concurrent stress test of {@link #test_queryJoin2()} which runs a fixed number of trials on a
   * pool of N=10 threads.
   *
   * @throws Exception
   */
   ...
   * @throws Exception
   */
  protected int doStressTest(final long timeout, final int ntrials, final int poolSize)
      throws Exception {
    ...
    for (FutureTask<Void> ft : futures) {
      ...
      if (remaining <= 0) ft.cancel(true /* mayInterruptIfRunning */);
      try {
    ...
    if (nerror > 0) {
      ...
      System.err.println("\n" + getClass().getName() + "." + getName() + " : " + msg);
      // fail the test.
      fail(msg);
    }

    // write out the summary.
    System.err.println(getClass().getName() + "." + getName() + " : " + msg);

    return nsuccess;
  }

  /*
   * Unit test for optional join. Two joins are used and target a {@link SliceOp}. The 2nd join is
   * marked as optional. Intermediate results which do not succeed on the optional join are
   * forwarded to the {@link SliceOp} which is the target specified by the {@link
    ...
    final PipelineOp condOp =
        new ConditionalRoutingOp(
            ...
            NV.asMap(
                ...
                new NV(ConditionalRoutingOp.Annotations.CONDITION, Constraint.wrap(new EQ(x, z)))));
  ...
  /*
   * Unit test for {@link ConditionalRoutingOp}. This test case tests when the condition is true
   * (the joins are not skipped) in which case the test (and results) are essentially identical to
   * test_query_join2().
   */
  ...
  /*
   * Unit test for {@link ConditionalRoutingOp}. This test case tests when the condition is false
   * (the joins are skipped).
   */
   ...
   * @throws Exception
   */
  private IRunningQuery initQueryWithConditionalRoutingOp(
      IConstraint condition, int startId, int joinId1, int joinId2) throws Exception {
  ...
  /*
   * Test the LatchExecutor to verify reliable progression with multiple threads contending to add
   * tasks.
   *
  ...
  public void testLatchExecutorProgression() throws InterruptedException {
    ...
    final LatchedExecutor latched = new LatchedExecutor(Executors.newCachedThreadPool(), 1);
