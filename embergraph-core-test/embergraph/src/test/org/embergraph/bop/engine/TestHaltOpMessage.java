...
public class TestHaltOpMessage extends TestCase2 {
  ...
  private static void doSerializationTest(final IHaltOpMessage expected) {
    ...
    final IHaltOpMessage actual =
        (IHaltOpMessage) SerializerUtil.deserialize(SerializerUtil.serialize(expected));

    assertSameOp(expected, actual);
  }

  private static void assertSameOp(final IHaltOpMessage expected, final IHaltOpMessage actual) {
    ...
    assertEquals("sinkMessagesOut", expected.getSinkMessagesOut(), actual.getSinkMessagesOut());

    assertEquals(
        "altSinkMessagesOut", expected.getAltSinkMessagesOut(), actual.getAltSinkMessagesOut());
