 ...
 *     <p>FIXME Just like {@link TestPredicateAccessPath}, this test suite needs to cover all of the
 *     combinations of global views of partitioned and unpartitioned indices.
 */
public class TestSampleIndex extends TestCase2 {
  ...
  private int loadData(final int scale) {
    ...
    final String[] names =
        new String[] {
          "John", "Mary", "Saul", "Paul", "Leon", "Jane", "Mike", "Mark", "Jill", "Jake", "Alex",
          "Lucy"
        };
    ...
    for (int i = 0; i < scale; i++) {
      ...
      final String n1 = names[rnd.nextInt(names.length)] + fmt.format(rnd.nextInt(populationSize));

      final String n2 = names[rnd.nextInt(names.length)] + fmt.format(rnd.nextInt(populationSize));
  ...
  public void test_offsetSamplers() {
    ...
    for (int trial = 0; trial < ntrials; trial++) {
      ...
      final int fromIndex = r.nextDouble() < .1 ? 0 : r.nextInt(entryCount);

      final int remaining = entryCount - fromIndex;

      final int toIndex = r.nextDouble() < .1 ? entryCount : (fromIndex + r.nextInt(remaining) + 1);
      ...
      final int limit =
          r.nextDouble() < .1
              ? r.nextInt(100) + 1
              : r.nextDouble() < .5 ? r.nextInt(entryCount) + 1 : r.nextInt(10000) + 1;
      ...
      for (IOffsetSampler sampler : samplers) {
        ...
        try {
          ...
          final long[] offsets = sampler.getOffsets(seed, limit, fromIndex, toIndex);
          ...
          if (limit > rangeCount) assertTrue(noffsets <= rangeCount);
          else assertTrue(noffsets == limit);
          ...
          for (int j = 0; j < offsets.length; j++) {
            ...
            if (offset >= toIndex)
              fail(
                  "index=" + j + ", offset GTE toIndex: offset=" + offset + ", toIndex=" + toIndex);

            if (offset <= lastOffset) {
              fail("index=" + j + ", lastOffset=" + lastOffset + ", but offset=" + offset);
            }
   ...
   * @todo test when the inclusive lower bound of a key range is an insertion point (no tuple for
   *     that key).
   * @todo test when the exclusive upper bound of a key range is an insertion point (no tuple for
   *     that key).
   */
  public void test_something() {
    ...
    for (SampleType sampleType : SampleType.values()) {
      ...
      if (log.isInfoEnabled()) log.info("Testing: SampleType=" + sampleType);
  ...
  private void doTest(
    ...
    assertEquals(
        "#samples (limit=" + limit + ", nrecords=" + nrecords + ", nexpected=" + nexpected + ")",
        nexpected,
        a.length);
  }
}
