 ...
 *
 * <p>Note: You can not use multiple {@link DataService}s in this test suite with the {@link
 * EmbeddedFederation} because the services are not sufficiently distinct based on how they are
 * created. Therefore unit tests against more than one {@link DataService} are located in the <code>
 * embergraph-jini</code> module.
 *
 * <p>Note: Distributed query processing generally means that the order in which the chunks arrive
 * is non-deterministic. Therefore the order of the solutions can not be checked unless a total
 * order is placed on the solutions using a {@link SortOp}.
 *
 * <pre>
 ...
 * @version $Id: TestFederatedQueryEngine.java 3508 2010-09-05 17:02:34Z thompsonbry $
...
public class TestFederatedQueryEngine extends AbstractEmbeddedFederationTestCase {
    ...
    final PipelineOp query =
        new StartOp(
            ...
            NV.asMap(
                ...
                new NV(
                    QueryEngine.Annotations.CHUNK_HANDLER, FederationChunkHandler.TEST_INSTANCE)));
  ...
  /*
   * Unit test uses a {@link StartOp} to copy some binding sets through a {@link SliceOp} without
   * involving any joins or access path reads. For this test, the binding sets never leave the query
   * controller.
   *
   * @throws Exception
   */
  public void test_query_startThenSlice_noJoins() throws Exception {
    ...
    final IRunningQuery runningQuery =
        queryEngine.eval(queryId, query, null /* queryAttributes */, expected);
  ...
  /*
   * Test the ability run a simple join which is mapped across two index partitions. There are three
   * operators. One feeds an empty binding set[] into the join, another is the predicate for the
   * access path on which the join will read (no variables are bound so it will read everything),
   * and the third is the join itself.
   *
   * @throws Exception
   */
  public void test_query_join_2shards_nothingBoundOnAccessPath() throws Exception {
    ...
    final PipelineJoin<E> joinOp =
        new PipelineJoin<E>(
            ...
            new NV(Predicate.Annotations.EVALUATION_CONTEXT, BOpEvaluationContext.SHARDED));
  ...
  /*
   * Test the ability run a simple join which is mapped across two index partitions. The join is
   * constrained to filter for only solutions in which [y==Paul].
   */
  public void test_query_join_2shards_nothingBoundOnAccessPath_withConstraint() throws Exception {
    ...
    // validate the stats for the join operator.
    {
      ...
      assertEquals(
          2L, stats.unitsOut.get()); // one tuple on each shard will satisfy the constraint.
      assertEquals(
          2L,
          stats.chunksOut
              .get()); // since we read on both shards and both shards have one tuple which joins.
    }
  ...
  /*
   * Test the ability to run a simple join reading on a single shard. There are three operators. One
   * feeds an empty binding set[] into the join, another is the predicate for the access path on
   * which the join will read (it probes the index once for "Mary" and binds "Paul" and "John" when
   * it does so), and the third is the join itself (there are two solutions, which are value="Paul"
   * and value="John").
   */
  public void test_query_join_1shard() throws Exception {
    ...
    // validate the stats for the join operator.
    {
      ...
      assertEquals(2L, stats.unitsOut.get());
      assertEquals(
          1L, stats.chunksOut.get()); // @todo this depends on which index partitions we read on.
    }
  ...
  public void test_query_join2() throws Exception {
    ...
    // validate the stats for the 2nd join operator.
    {
      ...
      assertEquals(
          2L, stats.chunksOut.get()); // since join ran on two shards and each had one solution.
    }
   ...
   *
   * @todo Write unit test for optional join groups. Here the goal is to verify that intermediate
   *     results may skip more than one join. This was a problem for the old query evaluation
   *     approach since binding sets had to cascade through the query one join at a time. However,
