...
public class TestMapBindingSetsOverShards extends AbstractEmbeddedFederationTestCase {
  ...
  public void test_locatorScan() throws IOException {
    ...
    final PartitionLocator[] expected =
  ...
  public void test_data() throws InterruptedException, ExecutionException, IOException {
    ...
    final R rel = (R) fed.getResourceLocator().locate(namespace, ITx.UNISOLATED);
...

    {
      ...
      assertTrue(ndx.contains(keyBuilder.reset().append("Mary").append("John").getKey()));

      // but this is not found in the index.
      assertFalse(ndx.contains(keyBuilder.reset().append("Mary").append("Fred").getKey()));
    }
    ...
    try {
       ...
       */
      {
        ...
        // partition0
        {
          ...
          assertEquals("partitionId", 0 /* partitionId */, bundle0.locator.getPartitionId());

          assertEquals("#of binding sets", partition0.size(), bundle0.bindingSets.length);

          AbstractQueryEngineTestCase.assertSameSolutionsAnyOrder(
              partition0.toArray(new IBindingSet[0]),
              Arrays.asList(bundle0.bindingSets).iterator());
        }
        ...
        // partition1
        {
          ...
          assertEquals("partitionId", 1 /* partitionId */, bundle1.locator.getPartitionId());

          assertEquals("#of binding sets", partition1.size(), bundle1.bindingSets.length);

          AbstractQueryEngineTestCase.assertSameSolutionsAnyOrder(
              partition1.toArray(new IBindingSet[0]),
              Arrays.asList(bundle1.bindingSets).iterator());
        }
      }
  ...
  private static class Bundle {
    ...
    public Bundle(final PartitionLocator locator, final IBindingSet[] bindingSets) {
   ...
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @param <F> The generic type of the elements in the relation for the target predicate.
   */
  private static class MockMapBindingSetsOverShardsBuffer<F>
      extends MapBindingSetsOverShardsBuffer<IBindingSet, F> {
