 ...
 * @version $Id$
 * @todo There are some operations which depend on equality or hash code behavior for vertices and
 *     perhaps edges so those things should also be tested.
 * @todo Test (re-)sampling for a vertex.
 * @todo Test sampling of the initial edge(s), including when there are constraints which can be
 *     applied to those edges and when one of the edges does not have any solutions (true zero).
 * @todo Test sampling of an edge when extending a path and resampling of the path.
 */
public class TestJGraph extends TestCase2 {
  ...
  //        final long ninserts = rel.insert(new ChunkedArrayIterator<E>(a.length, a, null/*
  // keyOrder */));
  //
