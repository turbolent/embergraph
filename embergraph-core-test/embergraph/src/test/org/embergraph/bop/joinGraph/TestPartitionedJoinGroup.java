 ...
 * @version $Id$
 *     <p>FIXME Add test to ensure that constraints are run regardless as of the last join even if
 *     their variables are not known to be bound. Also, modify the constructor to accept a set of
 *     variables which are known to be bound on entry into the join group.
 */
public class TestPartitionedJoinGroup extends TestCase2 {
  ...
  /** A test based loosely on LUBM Q2. There are no RDF specific constructions used here. */
  public void test_requiredJoins() {
    ...
    // Test w/o any constraints.
    {
      ...
      final PartitionedJoinGroup fixture = new PartitionedJoinGroup(preds, constraints);

      // all variables are bound within the join graph.
      assertSameIteratorAnyOrder(
          "joinGraphVars", new IVariable[] {x, y, z}, fixture.getJoinGraphVars().iterator());

      // verify all predicates were placed into the join graph.
      assertSameIteratorAnyOrder(
          "joinGraph", preds, Arrays.asList(fixture.getJoinGraph()).iterator());

      // there are no constraints.
      assertEquals(
          "joinGraphConstraints.size", new IConstraint[] {}, fixture.getJoinGraphConstraints());
    ...
    // Test w/ constraint(s) on the join graph.
    {
       ...
       */
      {
        ...
        assertEquals(
            new IConstraint[] {c2},
            fixture.getJoinGraphConstraints(
                new int[] {p0.getId(), p1.getId()}, false // pathIsComplete
                ));
      }
  ...
  public void test_withOptionalJoins() {
    ...
    // Test w/o any constraints.
    {
      ...
      assertSameIteratorAnyOrder(
          "joinGraphVars", new IVariable[] {x, y}, fixture.getJoinGraphVars().iterator());

      // verify predicates placed into the join graph.
      assertSameIteratorAnyOrder(
          "joinGraph",
          new IPredicate[] {p0, p1, p5},
          Arrays.asList(fixture.getJoinGraph()).iterator());

      // there are no constraints on the join graph predicates.
      assertEquals("joinGraphConstraints.size", 0, fixture.getJoinGraphConstraints().length);

      // {p2, p3,p4} are in the tail plan.
      assertEquals("tailPlan", new IPredicate[] {p2, p3, p4}, fixture.getTailPlan());

      // no constraints were assigned to optional predicate [p2].
      assertEquals("", 0, fixture.getTailPlanConstraints(p2.getId()).length);

      // no constraints were assigned to optional predicate [p3].
      assertEquals("", 0, fixture.getTailPlanConstraints(p3.getId()).length);

      // no constraints were assigned to optional predicate [p4].
      assertEquals("", 0, fixture.getTailPlanConstraints(p4.getId()).length);
    }
    ...
    // Test w/ constraint(s) on the join graph.
    {
      ...
      // no constraints were assigned to optional predicate [p2].
      assertEquals("", new IConstraint[] {}, fixture.getTailPlanConstraints(p2.getId()));

      // no constraints were assigned to optional predicate [p3].
      assertEquals("", new IConstraint[] {}, fixture.getTailPlanConstraints(p3.getId()));

      // the constraint on [z] was assigned to optional predicate [p4].
      assertEquals("", new IConstraint[] {c3}, fixture.getTailPlanConstraints(p4.getId()));
    }
  }
