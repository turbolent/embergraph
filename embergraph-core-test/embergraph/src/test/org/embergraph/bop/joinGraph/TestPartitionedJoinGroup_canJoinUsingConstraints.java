 ...
 * which are allowed based on the variables directly given with two {@link IPredicate}s. The purpose
 * of this test suite is to explore when joins (based on shared variables) become permissible
 * through {@link IConstraint}s as the variable(s) used within those constraints become bound.
 *
 * <p>Note: To avoid a dependency on the RDF model layer, this just uses String constants for URIs
 * and Literals.
 *
 ...
 * </ul>
 *
 * The predicates ({@link #p3} and {@link #p5}) do not directly join with any of the other
 * predicates (they do not directly share any variables). In general, a join without shared
 * variables means the cross product of the sources will be materialized and such joins should be
 * run last.
 *
 * <p>However, in this case there are two SPARQL FILTERs ({@link #c1} and {@link #c2}) which (a) use
 * those variables ({@link #origProperty1} and {@link #origProperty2}); and (b) can constrain the
 * query. This means that running the predicates without shared variables and applying the
 * constraints before the tail of the plan can in fact lead to a more efficient join path.
 *
...
public class TestPartitionedJoinGroup_canJoinUsingConstraints extends TestCase2 {
  ...
  private static final int GT = 0, LT = 1; // , EQ = 2, GTE = 3, LTE = 4;
  ...
  private static final class MathBOp extends ImmutableBOp implements IValueExpression {
    ...
    public MathBOp(final BOp[] args, Map<String, Object> anns) {
      ...
      if (args.length != 2 || args[0] == null || args[1] == null || getProperty(OP) == null) {
     ...
     */
    public MathBOp(final IValueExpression left, final IValueExpression right, final int op) {
  ...
  private static final String rdfsLabel = rdfs + "label";

  private static final String productFeature = bsbm + "productFeature";

  private static final String productPropertyNumeric1 = "productPropertyNumeric1";

  private static final String productPropertyNumeric2 = bsbm + "productPropertyNumeric2";

  private static final String productInstance =
  ...
  private final IPredicate<?> p0 =
      new Predicate(
          new BOp[] {product, new Constant(rdfsLabel), productLabel},
          new NV(BOp.Annotations.BOP_ID, nextId++));

  /** productInstance bsbm:productFeature ?prodFeature . */
  private final IPredicate<?> p1 =
      new Predicate(
          new BOp[] {new Constant(productInstance), new Constant(productFeature), prodFeature},
          new NV(BOp.Annotations.BOP_ID, nextId++));

  /** ?product bsbm:productFeature ?prodFeature . */
  private final IPredicate<?> p2 =
      new Predicate(
          new BOp[] {product, new Constant(productFeature), prodFeature},
          new NV(BOp.Annotations.BOP_ID, nextId++));
  ...
  private final IPredicate<?> p4 =
      new Predicate(
          new BOp[] {product, new Constant(productPropertyNumeric1), simProperty1},
          new NV(BOp.Annotations.BOP_ID, nextId++));
  ...
  private final IPredicate<?> p5 =
      new Predicate(
          new BOp[] {
            new Constant(productInstance), new Constant(productPropertyNumeric2), origProperty2
          },
          new NV(BOp.Annotations.BOP_ID, nextId++));
  ...
  private final IPredicate<?> p6 =
      new Predicate(
          new BOp[] {product, new Constant(productPropertyNumeric2), simProperty2},
          new NV(BOp.Annotations.BOP_ID, nextId++));
   ...
   * represented as its own IConstraint, but I have combined them for the purposes of these unit
   * tests.
   */
  ...
  public void test_attachConstraints_BSBM_Q5_path01() {
    ...
    final IConstraint[][] actual =
        PartitionedJoinGroup.getJoinGraphConstraints(
            path, constraints, null /* knownBoundVars */, true /* pathIsComplete */);

    final Set<IConstraint>[] expected =
        new Set[] {
          NA, // p1
          C0, // p2
          NA, // p4
   ...
   * @param actual
   */
  static void assertSameConstraints(
      final Set<IConstraint>[] expected, final IConstraint[][] actual) {
