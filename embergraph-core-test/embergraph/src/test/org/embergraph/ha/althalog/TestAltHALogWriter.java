...
public class TestAltHALogWriter extends TestCase {
  ...
  static class SimpleWriter implements Runnable {
    ...
    SimpleWriter(IRootBlockView rbv, HALogManager manager, ChecksumUtility checker, int count)
        throws IOException {
      ...
      try {
        for (int i = 0; i < count; i++) {
          ...
          if (((i + 1) % (1 + r.nextInt(count / 5))) == 0) {
            log.info("Cycling HALog after " + sequence + " records");
            rbv = closeRBV(rbv);
            writer.close(rbv);
            sequence = 0;
  ...
  static class ReaderRun implements Runnable {
    ...
    ReaderRun(
        final HALogManager manager, final AtomicInteger reads, final AtomicInteger openReaders) {
      this.manager = manager;
      this.reads = reads;
  ...
  public void testStressConcurrentRWWriterReader() throws FileNotFoundException, IOException {
    ...
    Thread wthread = new Thread(new SimpleWriter(rbv, manager, checker, recordWrites));
