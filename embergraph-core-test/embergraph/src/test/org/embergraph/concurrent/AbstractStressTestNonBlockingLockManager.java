 ...
 * <p>Goals:
 *
 * <p>1. higher concurrency of unisolated operations on the ds/journal with group commit. this only
 * requires a "lock" per writable named index, e.g., an operation will lock exactly one resource.
 * show consistency of the data in a suite of stress tests with varying #s of threads, tasks, and
 * resources. Each task will lock exactly one resource - the unisolated named index on which it
 * would write. DO THIS W/O the TxDAG first and get group commit debugged before trying to work
 * through the tx commit stuff, which requires the WAITS_FOR graph support.
 *
 * <p>2. transaction processing integrated with unisolated operations. since a transaction MAY write
 * on more than one index this requires a TxDAG and a queue of resources waiting to get into the
 * granted group (which will always be a singleton since writes on unisolated named indices require
 * exclusive locks). show that concurrency control never deadlocks in a suite of stress tests with
 * varying #s of threads, tasks, resources, and resource locked per task. Again, a resource is a
 * unisolated named index.
 *
...
public abstract class AbstractStressTestNonBlockingLockManager extends TestCase {
  ...
  protected static final Logger log =
      Logger.getLogger(StressTestNonBlockingLockManagerWithPredeclaredLocks.class);
    ...
    public T call() throws Exception {
      ...
      synchronized (this) {
        try {
          ...
          if (DEBUG) log.debug("Waiting: " + this);
   ...
   * expectations for the #of index partitions on a journal or federation.
   *
   * <p>Note: The likelyhood of deadlock increases as (a) more locks are requested per task; and (b)
   * fewer resources are available to be locked. When minLocks==maxLocks==nresources then tasks will
   * be serialized since each task requires all resources in order to proceed.
   *
   * <p>Note: At minLocks==maxLocks==1 this test can be used to explore the behavior of tasks that
   * lock only a single resource, eg., unisolated operations on the {@link DataService}.
   */
  public Result doComparisonTest(final Properties properties) throws Exception {
    ...
    final long testTimeout =
        (TimeUnit.SECONDS.toNanos(
                    Long.parseLong(
                        properties.getProperty(TestOptions.TIMEOUT, TestOptions.DEFAULT_TIMEOUT)))
                == 0L)
            ? Long.MAX_VALUE
            : TimeUnit.SECONDS.toNanos(
                Long.parseLong(
    ...
    final int corePoolSize = Integer.parseInt(properties.getProperty(TestOptions.CORE_POOL_SIZE));

    final int maxPoolSize =
        (properties.getProperty(TestOptions.MAX_POOL_SIZE) == null
            ? corePoolSize
            : Integer.parseInt(properties.getProperty(TestOptions.MAX_POOL_SIZE)));

    if (maxPoolSize < corePoolSize) {
      throw new IllegalArgumentException(
          TestOptions.MAX_POOL_SIZE
              + "="
              + maxPoolSize
              + ", but "
              + TestOptions.CORE_POOL_SIZE
              + "="
              + corePoolSize);
    }
    ...
    final boolean prestartCoreThreads =
        Boolean.parseBoolean(
            properties.getProperty(
                TestOptions.PRESTART_CORE_THREADS, TestOptions.DEFAULT_PRESTART_CORE_THREADS));

    final boolean synchronousQueue =
        Boolean.parseBoolean(
            properties.getProperty(
                TestOptions.SYNCHRONOUS_QUEUE, TestOptions.DEFAULT_SYNCHRONOUS_QUEUE));

    final boolean synchronousQueueFair =
        Boolean.parseBoolean(
            properties.getProperty(
                TestOptions.SYNCHRONOUS_QUEUE_FAIR, TestOptions.DEFAULT_SYNCHRONOUS_QUEUE_FAIR));

    final int ntasks = Integer.parseInt(properties.getProperty(TestOptions.NTASKS));

    final double percentTaskDeath =
        Double.parseDouble(
            properties.getProperty(
                TestOptions.PERCENT_TASK_DEATH, TestOptions.DEFAULT_PERCENT_TASK_DEATHS));
    ...
    final long taskTimeout =
        (Long.parseLong(
                    properties.getProperty(
                        TestOptions.TASK_TIMEOUT, TestOptions.DEFAULT_TASK_TIMEOUT))
                == 0L)
            ? Long.MAX_VALUE
    ...
    final int maxLockTries =
        Integer.parseInt(
            properties.getProperty(TestOptions.MAX_LOCK_TRIES, TestOptions.DEFAULT_MAX_LOCK_TRIES));

    final boolean predeclareLocks =
        Boolean.parseBoolean(
            properties.getProperty(
                TestOptions.PREDECLARE_LOCKS, TestOptions.DEFAULT_PREDECLARE_LOCKS));

    final boolean sortLockRequests =
        Boolean.parseBoolean(
            properties.getProperty(
                TestOptions.SORT_LOCK_REQUESTS, TestOptions.DEFAULT_SORT_LOCK_REQUESTS));
    ...
    final NonBlockingLockManagerWithNewDesign<String> lockManager =
        new NonBlockingLockManagerWithNewDesign<String>(
            maxPoolSize /* multi-programming level */,
            maxLockTries,
            predeclareLocks,
            sortLockRequests) {
    ...
    final ScheduledExecutorService sampleService;
    {
      ...
      final ThreadPoolExecutorStatisticsTask delegateQueueStatisticsTask =
          new ThreadPoolExecutorStatisticsTask(
              "delegateService", delegateService, null /* delegateServiceCounters */, w);

      delegateCounterSet.makePath("delegate").attach(delegateQueueStatisticsTask.getCounters());

      sampleService.scheduleWithFixedDelay(delegateQueueStatisticsTask, initialDelay, delay, unit);

      // sample the lock service.
      sampleService.scheduleWithFixedDelay(lockManager.statisticsTask, initialDelay, delay, unit);
    }
    ...
    try {
      ...
      for (int i = 0; i < ntasks; i++) {
        ...
        final int nlocks =
            (minLocks == maxLocks ? minLocks : r.nextInt(maxLocks - minLocks) + minLocks);
      ...
      int nsubmitted = 0;
      {
        ...
        long elapsed;
        {
          final Iterator<LockCallableImpl<String, Object>> itr = tasks.iterator();

          while ((elapsed = (System.nanoTime() - begin)) < testTimeout && itr.hasNext()) {

            final LockCallableImpl<String, Object> task = itr.next();

            futures.add(lockManager.submit(task.resource, task.task));
  ...
  public static class TestOptions {
     ...
     *
     * <p>Note: The {@link ThreadPoolExecutor} must not block when we submit a {@link Runnable} to
     * it from {@link NonBlockingLockManagerWithNewDesign#ready(Runnable)}. This goal can be
     * achieved either by having a {@link SynchronousQueue} and an unbounded {@link #MAX_POOL_SIZE}
     * -or- by using an unbounded work queue. In the former case new {@link Thread}s will be created
     * on demand. In the latter can tasks will be queued until the {@link #CORE_POOL_SIZE} threads
     * can process them.
     *
     ...
     * obtain the necessary locks for the task.
     */
     ...
     * simple expediency of sorting the resources in each lock request into a common order. With
     * this option deadlocks are NOT possible but all locks MUST be pre-declared by the operation
     ...
     * the deadlock detection logic in {@link TxDag} and the handling of deadlocks when they are
     * detected.
     */
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @todo refactor the tests generated below to use apply() and run each basic condition with and
   *     without lock timeout and with and without predeclaration of locks (and without sorting when
   *     locks are NOT predeclared so that we can exercise the deadlock detection stuff). We could
   *     also run each condition at (2), (20), and (100) threads.
   */
  public static class Generate extends ExperimentDriver {
    ...
    public static void main(String[] args) throws Exception {
      ...
      // low concurrency.
      conditions.add(
          getCondition(defaultProperties, new NV[] {new NV(TestOptions.CORE_POOL_SIZE, "2")}));
      // and with a non-zero lock timeout.
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.CORE_POOL_SIZE, "2"), new NV(TestOptions.TASK_TIMEOUT, "1000")
              }));
      ...
      // and with a non-zero lock timeout.
      conditions.add(
          getCondition(defaultProperties, new NV[] {new NV(TestOptions.TASK_TIMEOUT, "1000")}));

      // default concurrency with 10% task death.
      conditions.add(
          getCondition(
              defaultProperties, new NV[] {new NV(TestOptions.PERCENT_TASK_DEATH, ".10")}));
      // and with a non-zero lock timeout.
      conditions.add(
          getCondition(
              defaultProperties,
      ...
      // high concurrency.
      conditions.add(
          getCondition(defaultProperties, new NV[] {new NV(TestOptions.CORE_POOL_SIZE, "100")}));
      // and with a non-zero lock timeout
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.CORE_POOL_SIZE, "100"), new NV(TestOptions.TASK_TIMEOUT, "1000")
              }));
      ...
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.NRESOURCES, "1"),
                new NV(TestOptions.MIN_LOCKS, "1"),
                new NV(TestOptions.MAX_LOCKS, "1")
              }));
      // and with a non-zero lock timeout
      conditions.add(
          getCondition(
      ...
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.NRESOURCES, "3"),
                new NV(TestOptions.MIN_LOCKS, "3"),
                new NV(TestOptions.MAX_LOCKS, "3")
              }));
      // and with a non-zero lock timeout
      conditions.add(
          getCondition(
