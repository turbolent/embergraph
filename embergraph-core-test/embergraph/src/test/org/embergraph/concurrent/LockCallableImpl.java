 ...
 * @version $Id$
 * @param <R> The generic type of the declared lock objects.
 * @param <T> The generic type of the {@link Callable}'s outcome.
 */
public class LockCallableImpl<R extends Comparable, T> implements LockCallable<R, T> {
  ...
  public LockCallableImpl(final R[] resource, final Callable<T> task) {
    ...
    for (R r : resource) if (r == null) throw new IllegalArgumentException();
