...
public class TestNonBlockingLockManagerWithNewDesign extends TestCase {
  ...
  protected static final Logger log =
      Logger.getLogger(TestNonBlockingLockManagerWithNewDesign.class);
  ...
  public void test_submitOneThenCancel() throws InterruptedException, ExecutionException {
    ...
    try {
      ...
      final LockFutureTask<String, String> f =
      ...
      assertEquals(
          NonBlockingLockManagerWithNewDesign.TaskRunState.LocksReady, f.getTaskRunState());
      ...
      assertEquals(NonBlockingLockManagerWithNewDesign.TaskRunState.Halted, f.getTaskRunState());

      try {
        f.get();
        fail("Expecting: " + CancellationException.class);
      } catch (CancellationException ex) {
        if (INFO) log.info("Expected exception: " + ex);
      }
    ...
    try {
      ...
      try {
        ...
        if (ex.getCause() instanceof HorridTaskDeath) {
          ...
          final AssertionFailedError err =
              new AssertionFailedError("Expecting: " + HorridTaskDeath.class + " as the cause");
          err.initCause(ex);
          throw err;
        }
      }
  ...
  protected <R extends Comparable<R>, T> void assertLocksHeld(
      final NonBlockingLockManagerWithNewDesign<R> service, final LockFutureTask<R, T> task) {
   ...
   * @throws TimeoutException
   */
  public void test_runOneWithLock()
      throws InterruptedException, ExecutionException, TimeoutException {
   ...
   * @throws TimeoutException
   */
  public void test_runOneWithLockAndReleaseLockFromTask()
      throws InterruptedException, ExecutionException, TimeoutException {
