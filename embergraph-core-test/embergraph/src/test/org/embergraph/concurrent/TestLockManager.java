...
public class TestLockManager extends TestCase implements IComparisonTest {
   ...
   *
   * <p>Note: The likelihood of deadlock increases as (a) more locks are requested per task; and (b)
  ...
  public Result doComparisonTest(final Properties properties) throws Exception {
    ...
    final long testTimeout =
        Integer.parseInt(properties.getProperty(TestOptions.TIMEOUT, TestOptions.DEFAULT_TIMEOUT));

    final int nthreads = Integer.parseInt(properties.getProperty(TestOptions.NTHREADS));
    ...
    final long lockTimeout =
        Integer.parseInt(
            properties.getProperty(TestOptions.LOCK_TIMEOUT, TestOptions.DEFAULT_LOCK_TIMEOUT));
  ...
  public static class TestOptions {
     ...
     * expediency of sorting the resources in each lock request into a common order. With this
     * option deadlocks are NOT possible but all locks MUST be pre-declared by the operation before
  ...
  public void test_singleResourceLocking_serialized_lowConcurrency5() throws Exception {
    ...
    properties.setProperty(TestOptions.MIN_LOCKS, "1");
    properties.setProperty(TestOptions.MAX_LOCKS, "1");
    //        properties.setProperty(TestOptions.LOCK_TIMEOUT,"0"); // Note: timeout==0 when
    // debugging.
    properties.setProperty(TestOptions.PREDECLARE_LOCKS, "false");
    properties.setProperty(TestOptions.SORT_LOCK_REQUESTS, "false");
   ...
   */
  public void test_singleResourceLocking_serialized_lowConcurrency5_withTaskDeath()
      throws Exception {
   ...
   * locks for each operation and only 100 operations the chances of a deadlock on any given
   * operation are extremely high. However, since we are predeclaring our locks and the lock
   * requests are being sorted NO deadlocks should result.
   *
   * @throws Exception
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/237" > CI deadlock in
   *     org.embergraph.concurrent.TestLockManager.test_multipleResourceLocking_resources10_locktries10</a>
   */
  public void test_multipleResourceLocking_resources10_locktries10_predeclareLocks()
      throws Exception {
    ...
    public static void main(String[] args) throws Exception {
      ...
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.NTHREADS, "2"), new NV(TestOptions.LOCK_TIMEOUT, "1000")
              }));
      ...
      // and with a non-zero lock timeout.
      conditions.add(
          getCondition(defaultProperties, new NV[] {new NV(TestOptions.LOCK_TIMEOUT, "1000")}));
      ...
      // high concurrency.
      conditions.add(
          getCondition(defaultProperties, new NV[] {new NV(TestOptions.NTHREADS, "100")}));
      // and with a non-zero lock timeout
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.NTHREADS, "100"), new NV(TestOptions.LOCK_TIMEOUT, "1000")
              }));
      ...
      conditions.add(
          getCondition(
              defaultProperties,
              new NV[] {
                new NV(TestOptions.NTHREADS, "100"), new NV(TestOptions.PERCENT_TASK_DEATH, ".10")
              }));
      // and with a non-zero lock timeout.
      conditions.add(
          getCondition(
