 ...
 * @version $Id$
 */
public class StressTestNonBlockingLockManagerWithTxDag
    extends AbstractStressTestNonBlockingLockManager {
   ...
   * observed concurrency should be the #of tasks in the thread pool.
   */
  ...
  public void test_singleResourceLocking_waitsFor_highConcurrency100() throws Exception {
    ...
    assertTrue(
        "nthreads=" + nthreads + ", but maxrunning is only" + maxrunning,
  ...
  public void test_singleResourceLocking_serialized_waitsFor_lowConcurrency5_withTaskDeath()
    ...
    if ((actualErrorRate < expectedErrorRate - .1 /*.05*/)
      ...
      fail("error rate: expected=" + expectedErrorRate + ", actual=" + actualErrorRate);
    }
  }
   ...
   * out a lock requests when a task is cancelled either while awaiting its locks or while running.
   */
   ...
   *
   * <p>FIXME We don't have real-deadlocks w/o 2PL since we are processing the lock requests
   * atomically (all requests for a given task are posted at once). So we don't really need {@link
   * TxDag} for that. Modify to support 2PL and to use {@link TxDag} when 2PL is possible and locks
   * are not predeclared (you can not use 2PL if you predeclare locks).
   */
   ...
   * locks for each operation and only 100 resources the chances of a deadlock on any given
   * operation are extremely high.
   *
   * <p>FIXME We don't have real-deadlocks w/o 2PL since we are processing the lock requests
   * atomically (all requests for a given task are posted at once). So we don't really need {@link
   * TxDag} for that. Modify to support 2PL and to use {@link TxDag} when 2PL is possible and locks
