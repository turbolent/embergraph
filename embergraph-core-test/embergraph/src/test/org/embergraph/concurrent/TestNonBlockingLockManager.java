 ...
 *
 * @todo test to verify that we can cancel a running task using its {@link Future}.
 * @todo test cancellation of the task using get() w/ timeout (or get rid of the lock timeout).
...
public class TestNonBlockingLockManager extends TestCase {
  ...
  private class MockExecutor implements Executor {
    ...
    public void execute(final Runnable arg0) {
      ...
      if (INFO) log.info("Executing: " + arg0);

      if (delegate == null) {

        throw new AssertionFailedError("Not expecting task (no delegate): " + arg0);
      }

      delegate.execute(arg0);

      if (INFO) log.info("Executed: " + arg0);
    }
  }
  ...
  //
  //        final MockNonBlockingLockManager<String> service = new
  // MockNonBlockingLockManager<String>(
  ...
  //
  //            final NonBlockingLockManager<String>.LockFutureTask<Void> t1 =
  // service.newFutureTask(
  ...
  //                    });
  //
  //            final NonBlockingLockManager<String>.LockFutureTask<Void> t2 =
  // service.newFutureTask(
  //                    new String[] { "a" }, new Callable<Void>() {
  //                        public Void call() throws Exception {
  //                            return null;
