 ...
 * controller, or disk drive can defeat synchronous IO with the results that: (a) the sequence in
 * which writes are issued by the application may not be the sequence in which the data are laid
 * down on the disk; and (b) the write operation may return before the data are stable on disk.
 *
 * <p>Both of these results can break the semantics of the atomic commit protocol in at least the
 * following ways:
 *
 * <ul>
 *   <li>If the root blocks are updated before the application data are on stable media then a power
 *       failure will cause the application to read from the new root block but the application data
 *       will not have been written.
 *   <li>If the write operation returns before the data are on stable media then the application may
 *       conclude that the commit was successful when in fact the data are not yet on disk. In fact,
 *       since the error is not reported synchronously the application may never learn that the
 *       write has failed unless it continues to write on the disk and a subsequent write turns up
 *       the error.
 * </ol>
 *
...
public class TestRandomAccessFileSynchronousWrites extends TestCase {
   ...
   *
   * @param baseline The <code>rw</code> performance {@link Stats}.
   * @param syncio The performance for either the <code>rws</code> or <code>rwd</code> modes, which
   *     require synchronization to the disk after each write.
   * @throws AssertionFailedError unless the write IOPs are significantly lower for the
   *     <i>syncio</i> condition.
   */
  protected void assertWriteCacheDisabled(final Stats baseline, final Stats syncio) {
   ...
   * platform.
   *
   * <p>Note: The file is created using the temp file mechanisms so this is only really testing the
   * behavior of the disk on which the temp files are stored.
   *
   * <p>Note: The more write operations that you request and the larger the file on which those
   * writes are randomly distributed the more you are likely to defeat any cache mechanisms.
   *
   * <pre>
   ...
   * </pre>
   *
   * Based on the data above, you can see that merely requesting synchronous IO in Java clearly does
   * not disable all layers of the write cache.
   *
   * @param mode The file mode to be used.
  ...
  private static class Stats {
    ...
    public Stats(
        final String mode, final long elapsed, final long writesPerSec, final long bytesPerSec) {
