...
public class TestRollbackCommit extends ProxyTestCase<Journal> {
  ...
  public void test_rollback() {
    ...
    try {
      ...
      assertNotSame(isRootBlock0, journal.getRootBlockView().isRootBlock0());

      journal.rollback();

      if (log.isInfoEnabled()) log.info("After rollback =" + journal.getRootBlockView());
  ...
  private void assertRootBlockOk(
      final Journal journal, final ByteBuffer expected, final boolean isRootBlock0) {

    final ByteBuffer actual = journal.getBufferStrategy().readRootBlock(isRootBlock0);
