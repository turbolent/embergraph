...
public class TestAbort extends ProxyTestCase<Journal> {
  ...
  public void test_rollback() {
    ...
    try {
      ...
      final IRootBlockView rootBlockView0 =
          new RootBlockView(true /* rootBlock0 */, rootBlock0, checker);

      if (log.isInfoEnabled()) log.info("rootBlock0=" + rootBlockView0);

      final ByteBuffer rootBlock1 =
          journal.getBufferStrategy().readRootBlock(false /* rootBlock0 */);

      final IRootBlockView rootBlockView1 =
          new RootBlockView(false /* rootBlock0 */, rootBlock1, checker);

      if (log.isInfoEnabled()) log.info("rootBlock1=" + rootBlockView1);

      // true iff rootBlock0 is the current root block.
      final boolean isRootBlock0 = journal.getRootBlockView().isRootBlock0();

      if (log.isInfoEnabled()) log.info("Using rootBlock=" + (isRootBlock0 ? 0 : 1));
      ...
      if (log.isInfoEnabled()) log.info("After commit   =" + journal.getRootBlockView());

      // verify the root blocks have not been changed.
      assertEquals(rootBlock0, journal.getBufferStrategy().readRootBlock(true /* rootBlock0 */));

      assertEquals(rootBlock1, journal.getBufferStrategy().readRootBlock(false /* rootBlock0 */));
  ...
  public void test_abort() {
    ...
    try {
      ...
      assertEquals(firstOffset, store.getRootBlockView().getNextOffset());
      ...
      final byte[] a;
      {
        a = new byte[DirectBufferPool.INSTANCE.getBufferCapacity() + 1];
      ...
      // verify the write.
      assertEquals(new byte[] {2, 4}, ndx.lookup(new byte[] {1, 3}));
    } finally {
  ...
  /** Run {@link #test_abort()} a bunch of times to look for path dependent problems. */
