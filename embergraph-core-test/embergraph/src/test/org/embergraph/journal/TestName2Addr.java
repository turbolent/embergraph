...
public class TestName2Addr extends ProxyTestCase<Journal> {
  ...
  private void assertKeyEncoding(final String name, final IIndex n2a) {
    ...
    final ITupleSerializer tupleSer = n2a.getIndexMetadata().getTupleSerializer();
  ...
  public void test_namedIndexScan() {
    ...
    try {
      ...
      assertEquals(
          0, getIndexNames(journal, null /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(0, getIndexNames(journal, "" /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(
          0, getIndexNames(journal, sharedPrefix /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(
          0, getIndexNames(journal, name1 /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(
          0, getIndexNames(journal, name2 /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(0, getIndexNames(journal, name1 /* prefix */, ITx.READ_COMMITTED).size());

      assertEquals(0, getIndexNames(journal, name2 /* prefix */, ITx.READ_COMMITTED).size());
      ...
      final HTreeIndexMetadata md1 = new HTreeIndexMetadata(name1, indexUUID);

      final HTreeIndexMetadata md2 = new HTreeIndexMetadata(name2, indexUUID);
      ...
      assertEquals(
          2, getIndexNames(journal, null /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(2, getIndexNames(journal, "" /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(
          2, getIndexNames(journal, sharedPrefix /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(
          1, getIndexNames(journal, name1 /* prefix */, journal.getLastCommitTime()).size());

      assertEquals(
          1, getIndexNames(journal, name2 /* prefix */, journal.getLastCommitTime()).size());
      ...
      assertEquals(2, getIndexNames(journal, null /* prefix */, commitTime1).size());

      assertEquals(2, getIndexNames(journal, "" /* prefix */, commitTime1).size());

      assertEquals(2, getIndexNames(journal, sharedPrefix /* prefix */, commitTime1).size());

      // Visible in the read-committed view.
      assertEquals(1, getIndexNames(journal, name1 /* prefix */, ITx.READ_COMMITTED).size());
      assertEquals(1, getIndexNames(journal, name2 /* prefix */, ITx.READ_COMMITTED).size());

      // Visible in read-historical committed view.
      assertEquals(1, getIndexNames(journal, name1 /* prefix */, commitTime1).size());
      assertEquals(1, getIndexNames(journal, name2 /* prefix */, commitTime1).size());
   ...
   * @param prefix The prefix for those named indices (may be <code>null</code> or empty).
   ...
   * @return The matching index names. The names will be reported in the returned set in the same
   *     order in which they are reported by the Journal.
   */
  private static Set<String> getIndexNames(
      final Journal jnl, final String prefix, final long timestamp) {
