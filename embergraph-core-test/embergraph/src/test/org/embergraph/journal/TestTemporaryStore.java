...
public class TestTemporaryStore extends AbstractRawStoreTestCase {
  ...
  public static Test suite() {
    ...
    ProxyTestSuite suite = new ProxyTestSuite(delegate, "Temporary Raw Store Test Suite");
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   *     <p>Note: You can not re-open a transient store, hence it is not possible to extend {@link
   ...
   */
  protected long writeRandomData(final TemporaryRawStore store, final long nbytesToWrite) {
  ...
  public void test_writeWithExtend() {
    ...
    try {
      ...
      // verify extension of buffer.
      assertTrue("userExtent", userExtent + b2.length <= bufferStrategy.getUserExtent());
...
//     * Test that the store transparently overflows onto disk when the maximum
  ...
  public void test_registerAndUseHTree() {
    ...
    try {
      ...
      final HTreeIndexMetadata metadata = new HTreeIndexMetadata(name, indexUUID);
