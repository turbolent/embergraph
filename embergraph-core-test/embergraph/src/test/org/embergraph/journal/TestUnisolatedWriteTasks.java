 ...
 * verifies that dirty indices are retained by {@link Name2Addr} on its commit list and that writes
 * are NOT lost in large commit groups.
 *
...
public class TestUnisolatedWriteTasks extends ProxyTestCase<Journal> {
  ...
  public void test_writeOneIndex() throws InterruptedException, ExecutionException {
    ...
    try {
          ...
          .submit(
              ...
              new AbstractTask<Object>(journal, ITx.UNISOLATED, resource) {
                ...
                protected Object doTask() {
                  ...
                  getJournal()
                      .registerIndex(getOnlyResource(), new IndexMetadata(resource[0], indexUUID));

                  if (log.isInfoEnabled()) log.info("Will write on " + getOnlyResource());
                  ...
                  //                        // Note: clone values since replaced with old values by
                  // the batch op.
                  //                        ndx.insert(new BatchInsert(ninserts, keys,
                  // vals.clone()));
      ...
      assertEquals(
          "commitCounter", commitCounterBefore + 1, journal.getRootBlockView().getCommitCounter());
          ...
          .submit(
              new AbstractTask<Object>(journal, ITx.READ_COMMITTED, resource) {
                ...
                protected Object doTask() {
                  ...
                  //                        BatchLookup op = new
                  // BatchLookup(ninserts,keys,actualValues);
                  //
                  //                        ndx.lookup(op);
          ...
          .submit(
              new AbstractTask<Object>(journal, ITx.UNISOLATED, resource) {
                ...
                protected Object doTask() {
                  ...
                  getJournal()
                      .registerIndex(resource[1], new IndexMetadata(resource[1], indexUUID2));

                  {
                    IIndex ndx = getIndex("foo");

                    assertEquals("indexUUID", indexUUID1, ndx.getIndexMetadata().getIndexUUID());
...

                  {
                    ...
                    assertEquals("indexUUID", indexUUID2, ndx.getIndexMetadata().getIndexUUID());
   ...
   *
   * <p>Note: This does all writes in a single task in order to simulate a large commit group and
   * stress the commit list mechanism used by {@link Name2Addr}. However, it does not matter whether
   * the data are validated by a single read task or by multiple read tasks.
   *
   * @throws ExecutionException
   * @throws InterruptedException
   */
              ...
              new AbstractTask<Object>(journal, ITx.UNISOLATED, resource) {
                ...
                protected Object doTask() {
                  ...
                  for (int i = 0; i < nindices; i++) {
                    ...
                    assertEquals(
                        "indexUUID", indices[i].indexUUID, ndx.getIndexMetadata().getIndexUUID());
              ...
              new AbstractTask<Object>(journal, ITx.READ_COMMITTED, resource) {
                ...
                protected Object doTask() {
                  ...
                  for (int i = 0; i < nindices; i++) {
                    ...
                    // byte[stuff.ninserts][];
                    //
                    //                            BatchLookup op = new BatchLookup(stuff.ninserts,
                    // stuff.keys,
                    //                                    actualValues);
                    //
                    //                            ndx.lookup(op);
