 ...
 * comparison to the journal write rate, and the overehead of (large) transactions can be estimated
 * in comparison to the unisolated index write rate (this test does not estimate the overhead for
 * small transactions for a variety of reasons).
 *
 * <p>The results from this benchmark can be used to compare the performance of the different {@link
 * IBufferMode} implementations. The theoretical maximum for a platform is the sustained write rate
 * for the disk subsystem on which the journal file is located - this can be obtained with a bit of
 * research on your disk drives, e.g., using <a href="http://www.storagereview.com/">
 * storagereview.com </a>. It is generally achieved by {@link BenchmarkBlockBasedOptimium}
 *
 * <p>Note: you should run these tests multiple times to make sure that you are getting valid
 * numbers for your platform. You should also compare the data with the expected disk maximum write
 * rate for your platform. You can monitor your platform using "perfmon" on Windows or "vmstat" on
 * Un*x. It is important that your system has not swapped out parts of the JVM or the benchmark will
 * run poorly (this can be a problem with a memory-limited Windows platform).
 *
 * <p>Analysis: The Transient mode achieves 30x the raw write rate when compared to any of the
 * disk-backed modes (1,282 MB/sec vs 40 MB/sec). However, the index write rates are essentially
 * constant across the buffer modes (roughly 20MB/sec for unisolated writes, which is ~50% of the
 * journal write rate when backed by disk, and 9MB/sec for isolated writes, or ~25% of the journal
 * write rate when backed by disk). The limiting factor for index writes is the btree code itself
 * (it tends to be key search). The limiting factor for the isolated index writes is that the
 * transaction write set overflows onto disk, so we wind up doing much more IO for a large
 * transaction (however small transactions have very little overhead when compared to unisolated
 * index writes). The disk-only buffer mode does a little better than the fully-buffered modes for
 * the isolated writes - presumably since (a) the monotonically increasing keys defeat the index
 * node and leaf cache; and (b) the disk-only mode is able to make more RAM available to the JVM
 * since it does not maintain the large in memory buffer.
 *
 * @see src/architecture/performance.xls.
 * @todo Test the impact of an AIO strategy.
 *     <p>FIXME Use the {@link ExperimentDriver} and compare the various buffer modes and other
 *     variables and the write rates for the {@link IRawStore} vs unisolated indices. Checkout the
 *     disk queue under the performance monitor and make sure that we are driving the disk as hard
 *     as possible.
 * @todo Quantify impact of the disk-only mode write cache.
 * @todo Note that the target performance environment requires multiple journals and multiple
 *     read-optimized databases. Do not over-optimize for a single writer. Write benchmarks for
 *     write absorption rates for concurrent writers with and without concurrent readers.
...
public abstract class BenchmarkJournalWriteRate extends TestCase2 {
   ...
   *
   * <p>Note: A higher branching factor can be choosen for this test since the btree writes use
   * monotoically increasing keys.
   */
   ...
   *
   * @param writeSize The size of the object to be written.
   ...
   * the journal would overflow in order to measure only the cost of writes without buffer extension
   * handling.
   *
   * <p>Note that for transactional writes, the writes are buffered in memory and then on disk,
   * validated against the buffered writes, and finally transferred to the unisolated index on the
   * journal. Short transactions are therefore very fast, but large transactions will be
   * significantly slower than the corresponding unisolated writes since there is several times more
   * IO for large transactions (write on tx buffer, read tx buffer and validate against the
   * unisolated index, read tx buffer and write on the unisolated index). However, there is also
   * logic to defeat validation when no concurrent writes have occurred, so the worst case will not
   * be demonstrated by a single write process.
   *
   * @param name The name of the index on which the writes will be performed. The named index MUST
   *     have been registered by the caller and that registration MUST have been committed.
   * @param tx The transaction identifier -or- 0L if the writes will not be isolated by a
   *     transaction.
   * @param valueSize The size in bytes of the value to be written under each key.
   * @return The elapsed time for the test.
   */
  public long doIndexWriteRateTest(String name, long tx, int valueSize) {
    ...
    IIndex ndx =
        (tx == 0
            ? journal.getIndex(name)
            : journal.getLocalTransactionManager().getTx(tx).getIndex(name));

    System.err.println(
        "Begin: index write rate, isolated="
            + (tx == 0 ? "no" : "yes")
            + ", isolatable="
            + ndx.getIndexMetadata().isIsolatable()
            + ", bufferMode="
            + journal.getBufferStrategy().getBufferMode());
    ...
    final int nwrites = (int) (journal.getBufferStrategy().getExtent() * percentFull / valueSize);
   ...
   * system).
   *
   * <p>Note: This test is conducted without the use of a journal. It is bundled in the same source
   * code file so that we can compare the journal performance with the raw IO performance of the
   * platform.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
  ...
  public abstract static class AbstractBenchmarkOptimium extends TestCase2 {
    ...
    public void doOptimiumWriteRateTest(int recordSize) throws IOException {
      ...
      System.err.println(
          "Begin: optimum write rate test: #writes=" + nwrites + ", dataSize=" + dataSize);
   ...
   * large blocks. Note that the btree does exactly this, but the limiting factor for throughput is
   * the write on the btree data structures (mostly key search) rather than the writes on the
   * journal and their consequent IO.
   *
   * <p>Note: This overrides several methods in the base class in order to conduct a test without
   ...
   * sustained IO</i>. The journal SHOULD NOT be able approach this best case scenario. Comparison
   * to this case should reveal the overhead of the journal, Java, and block-oriented IO when
   * compare to sustained sequential data transfer from RAM to disk. Since block-based IO is, in
   * fact, better, one can only presume that the nio library has some problem with very large
   * writes.
   *
   * <p>Note: This overrides several methods in the base class in order to conduct a test without
   * the use of a journal.
   *
   * <p>Note: I have seen block-based IO perform better in cases where system resources were low
   ...
   * memory given over to direct buffers - and things can actually slow down with more memory.
   */
