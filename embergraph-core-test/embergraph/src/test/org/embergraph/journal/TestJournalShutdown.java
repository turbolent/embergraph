...
public class TestJournalShutdown extends TestCase2 {
   ...
   *
   * <p>Note: You SHOULD NOT need to close the Journal. Once it is no longer strongly referenced it
   * SHOULD get finalized(). This MAY be set to [true] to verify that the journal is properly
   * shutting down all of its thread pools, but it MUST be [false] for CI since the whole purpose of
   * this test is to verify that Journals are eventually finalized() if the application no longer
   * holds a strong reference to the journal.
   *
   * @throws InterruptedException
   */
   ...
   *     creates. Otherwise, it relies on the finalized() method to close() the {@link Journal}.
   * @throws InterruptedException
   */
  private void doMemoryLeakTest(final boolean closeJournal) throws InterruptedException {
    ...
    try {
      try {
        ...
        for (int i = 0; i < limit; i++) {
              ...
              new Journal(properties) {
                protected void finalize() throws Throwable {
                  ...
                  if (log.isDebugEnabled())
                    log.debug(
                        "Journal was finalized: ncreated=" + ncreated + ", nalive=" + nunfinalized);
          ...
          try {
             ...
             */
            final AbstractTask task2 =
                new RegisterIndexTask(
                    jnl.getConcurrencyManager(),
                    "name",
        ...
        log.error("Out of memory after creating " + ncreated + " journals.");
      }
      ...
      if (log.isInfoEnabled()) {
        ...
        log.info("There are " + nunfinalized + " journals which are still alive.");
      }
  ...
  private static class NOpTask extends AbstractTask<Void> {
     ...
     * @param resource
     */
    protected NOpTask(IConcurrencyManager concurrencyManager, long timestamp, String resource) {
