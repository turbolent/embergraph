 ...
 * indices spanned by that namespace.
 *
 ...
 *     <p>FIXME GROUP COMMIT (hierarchical locking): We need to test each different kind of index
 *     access here (unisolated, isolated by a read/write tx, and isolated by a read-only tx, and
 *     read-historical (no isolation)).
 */
public class TestHierarchicalLockingTasks extends ProxyTestCase<Journal> {
  ...
  /*
   * Test creates several named indices some of which have a shared namespace prefix and verifies
   * that a lock declared for the namespace prefix permits that task to access any index in that
   * namespace. The test also verifies that an index that does not share the namespace prefix may
   * not be accessed by the task.
   *
  ...
  /*
   * Unit test for hierarchical locking verifies that we can declared a namespace for a task which
   * then creates multiple indices spanned by that namespace.
   *
   * @throws InterruptedException
   * @throws ExecutionException
   */
                  ...
                  new AbstractTask<UUID[]>(
                    ...
                    protected UUID[] doTask() throws Exception {
                      ...
                      for (int i = 0; i < allowed_indices.length; i++) {
                        ...
                        ndx =
                            getJournal()
                                .registerIndex(name, new IndexMetadata(name, UUID.randomUUID()));
              ...
              new AbstractTask<Void>(journal.getConcurrencyManager(), ITx.UNISOLATED, namespace) {
                ...
                protected Void doTask() throws Exception {
                  ...
                  for (int i = 0; i < allowed_indices.length; i++) {
                    ...
                    // add a key.
                    ndx.insert(new byte[] {}, new byte[] {(byte) i});
                  }
