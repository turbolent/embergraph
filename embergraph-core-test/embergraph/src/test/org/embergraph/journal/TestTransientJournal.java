...
public class TestTransientJournal extends AbstractJournalTestCase {
  ...
  public static Test suite() {
    ...
    ProxyTestSuite suite = new ProxyTestSuite(delegate, "Transient Journal Test Suite");
  ...
  public void test_create_transient01() throws IOException {
    ...
    try {
      ...
      final TransientBufferStrategy bufferStrategy =
          (TransientBufferStrategy) journal.getBufferStrategy();
      ...
      assertEquals(
          ...
          0L /*soft limit for transient mode*/,
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   * @todo While the transient store uses root blocks there is no means currently defined to re-open
   *     a journal based on a transient store and hence it is not possible to extend {@link
