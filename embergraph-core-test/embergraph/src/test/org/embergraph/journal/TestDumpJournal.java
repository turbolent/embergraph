 ...
 * @version $Id$
 *     <p>TODO Test historical journal artifacts for compatibility.
 *     <p>TODO Test command line utility.
 *     <p>FIXME GIST : Test other types of indices (HTree (one test exists now), Stream (no tests
 *     yet)).
...
public class TestDumpJournal extends ProxyTestCase<Journal> {
  ...
  public void test_journal_oneIndexRandomData()
    ...
    try {
...

      {
        ...
        final KV[] a = AbstractBTreeTestCase.getRandomKeyValues(1000 /* ntuples */);
   ...
   *
   * @throws Exception
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/762">DumpJournal does not
   *     protect against concurrent updates (NSS) </a>
   */
  public void test_dumpJournal_concurrent_updates() throws Exception {
    ...
    try {
      ...
      final LatchedExecutor executor1 =
          new LatchedExecutor(src.getExecutorService(), 1 /* nparallel */);

      // Setup executor that limits parallelism.
      final LatchedExecutor executor2 =
          new LatchedExecutor(src.getExecutorService(), 1 /* nparallel */);
      ...
      try {
        ...
        for (FutureTask<Void> ft : tasks2) executor2.execute(ft);
