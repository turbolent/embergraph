...
public class TestCommitCounterUtility extends TestCase2 {
  ...
  public void test01() {
    ...
    final File f = new File("/tmp/000/000/000/000/000/000/000000000000000000001.tmp");

    assertEquals(f, CommitCounterUtility.getCommitCounterFile(dir, 1L, ext));

    assertEquals(1L, CommitCounterUtility.parseCommitCounterFile(f.getName(), ext));

    assertEquals("000000000000000000001", CommitCounterUtility.getBaseName(f.getName(), ext));
  }
  ...
  public void test_findGreatestCommitCounter() throws IOException {
    ...
    try {
      ...
      if (!dir.mkdirs()) fail("Could not create: " + dir);

      final File f1 = CommitCounterUtility.getCommitCounterFile(dir, 1L, ext);
      final File f10 = CommitCounterUtility.getCommitCounterFile(dir, 10L, ext);
      final File f100 = CommitCounterUtility.getCommitCounterFile(dir, 100L, ext);
      final File f1000 = CommitCounterUtility.getCommitCounterFile(dir, 1000L, ext);
      final File f10000 = CommitCounterUtility.getCommitCounterFile(dir, 10000L, ext);

      // No files. Returns null.
      assertEquals(null, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

      // Create directory structure.
      if (!f10.getParentFile().mkdirs()) fail("Could not create directory structure: " + f1000);

      // No files. Returns null.
      assertEquals(null, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

      if (!f10.createNewFile()) fail("Could not create: " + f10);

      // This is the only file. It should be returned.
      assertEquals(f10, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

      // Create a file with a commit counter LT that file.
      if (!f1.createNewFile()) fail("Could not create: " + f1);

      // The return value should not change.
      assertEquals(f10, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

      // Create a file with a larger commit counter.
      if (!f100.createNewFile()) fail("Could not create: " + f100);

      // That file should now be returned.
      assertEquals(f100, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

      // Create a file with a larger commit counter. The commit counter
      // will cause another directory to be created.
      if (!f1000.getParentFile().mkdirs()) fail("Could not create directory structure: " + f1000);
      if (!f1000.createNewFile()) fail("Could not create: " + f1000);

      // That file should now be returned.
      assertEquals(f1000, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));
      ...
      if (!f10000.getParentFile().mkdirs()) fail("Could not create directory structure: " + f10000);

      // The same file should be returned since the new dir is empty.
      assertEquals(f1000, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

      // Add a file to that directory.
      if (!f10000.createNewFile()) fail("Could not create: " + f10000);

      // That file should be returned.
      assertEquals(f10000, CommitCounterUtility.findGreatestCommitCounter(dir, fileFilter));

    } finally {

      CommitCounterUtility.recursiveDelete(false /* errorIfDeleteFails */, dir, fileFilter);
    }
  }
}
