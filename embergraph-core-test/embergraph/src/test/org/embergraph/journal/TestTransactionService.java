...
public class TestTransactionService extends TestCase2 {
  ...
  protected static class MockTransactionService extends AbstractTransactionService {
    ...
    public void awaitRunState(final TxServiceRunState expectedRunState)
      ...
      if (expectedRunState == null) throw new IllegalArgumentException();
     ...
     *
     * <p>TODO This override of {@link #nextTimestamp()} should be removed once that issue is fixed.
     */
    @Override
     ...
     *
     * <p>This version takes the lock since we are controlling concurrency explicitly in the test
     * suite. This makes it easier to write the tests.
     */
    @Override
   ...
   * ITimestampService#nextTimestamp()}. Therefore they are monotonically increasing. New read-write
   * transactions may be created at any time - there are no preconditions other than that the
   * transaction service is running. Likewise there is no contention other than for the next
   * distinct timestamp.
   */
   ...
   * start time is the last commit time on the database. As such the abort and commit procedure are
   * the same as for a read-only transaction. The only difference is in how the start time of the
   ...
   * edge case is allowed where the callers timestamp exactly corresponds to the lastCommitTime, but
   * it is not permitted to be GT the lastCommitTime since that could allow data not yet committed
   * to become visible during the transaction (breaking isolation).
   *
   * <p>A commitTime is identified by looking up the callers timestamp in a log of the historical
   * commit times and returning the first historical commit time LTE the callers timestamp.
   *
   * <p>The transaction start time is then chosen from the half-open interval <i>commitTime</i>
   * (inclusive lower bound) : <i>nextCommitTime</i> (exclusive upper bound).
   *
   * <p>Note: This test (used to) fail occasionally. This occured if the timestamps assigned by the
   * {@link MockTransactionService} are only 1 unit apart. When that happens, there are not enough
   * distinct values available to allow 2 concurrent read-only transactions. See <a href=
   * "https://sourceforge.net/apps/trac/bigdata/ticket/145">ISSUE#145 </a>. Also see {@link
  ...
  public void test_newTx_readOnly() throws IOException {
    ...
    properties.setProperty(AbstractTransactionService.Options.MIN_RELEASE_AGE, "" + Long.MAX_VALUE);
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("tx1=" + tx1);
      ...
      if (log.isInfoEnabled()) log.info("tx2=" + tx2);
      ...
      if (log.isInfoEnabled()) log.info("tx3=" + tx3);
   ...
   * any commit point (all commit points are in the future).
   */
   ...
   * next commit point on the database (which in this case would be the first commit point as well).
   */
  ...
  public void test_newTx_readOnly_releaseTimeRespectsReadsOnCommitTime() throws IOException {
    ...
    try {
      ...
      assertEquals(txState0.getReadsOnCommitTime(), txState1.getReadsOnCommitTime());
   ...
   * where (...) indicates the commit time on which the transaction is reading.<br>
   * where rt# is a releaseTime. The releaseTime and lastCommitTime are initially ZERO (0).<br>
   * The long "====" lines above the timeline represent the period during which a given commit point
   * is available to a new transaction.<br>
   * The long "---" lines above the timeline represent the life cycle of a specific transaction.<br>
   *
   * <p>Any transaction which starts before ct1 will see the history going back to commitTime=0.
   * This commit time is initially available because there is no committed data. It is pinned by tx0
   * and then by tx1. Once both of those transactions complete, that commit time is released
   * (minReleaseAge is zero).
   *
   * <p>When tx1 completes, the release time is advanced (rt=ct0-1).
   *
   ...
   * (minReleaseAge is zero).
   *
   * <p>When tx1 completes, the release time is advanced (rt=now-1).
   *
