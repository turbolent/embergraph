...
public class TestHelper {
   ...
   * @param testClass The instance of the delegate test class for a proxy test suite. For example,
   *     TestWORMStrategy.
   */
  public static void checkJournalsClosed(final TestCase test, final TestCase testClass) {
    ...
    if (nopen != nclose) {
      ...
      log.error(
          "Test did not close journal(s)"
              ...
              + "."
              + test.getName()
              + (testClass == null ? "" : ", testClass=" + testClass.getClass().getName()));
    }
    ...
    if (nopen > 0 && ndestroy == 0) {
      ...
      log.error(
          "Test did not destroy journal(s)"
              + ": nopen="
              + nopen
              + ", nclose="
              + nclose
   ...
   *     TestWORMStrategy.
   */
  private static void checkTempStoresClosed(final TestCase test, final TestCase testClass) {
    ...
    if (nopen != nclose) {
      ...
      log.error(
          "Test did not close temp store(s)"
              + ": nopen="
              + nopen
              + ", nclose="
              + nclose
