...
public abstract class AbstractIndexManagerTestCase<S extends IIndexManager> extends TestCase3 {
   ...
   * from {@link GenericProxyTestCase} to the delegate. Invoking this method from the
   * implementations of those methods in this class provides a means of catching omissions where the
   * corresponding method is NOT being delegated. Failure to delegate these methods means that you
   * are not able to share properties or object manager instances across tests, which means that you
   * can not do configuration-based testing of integrations and can also wind up with mutually
   * inconsistent test fixtures between the delegate and each proxy test.
   */
   ...
   * using the temporary file creation mechanism, but it is then // * deleted. Ideally the returned
   * filename is unique for the scope of the // * test and will not be reported by the journal as a
   * "pre-existing" file. // * // *
   *
   * <p>// * Note: This method is not advised for performance tests in which the disk // *
   * allocation matters since the file is allocated in a directory choosen by // * the OS. // * // *
   * // * @param properties // * The configured properties. This is used to extract metadata // *
   * about the journal test configuration that is included in the // * generated filename. Therefore
   * this method should be invoked // * after you have set the properties, or at least the // *
   * {@link Options#BUFFER_MODE}. // * // * @return The unique filename. // * // * @see {@link
   * #getProperties()}, which sets the "deleteOnClose" flag for // * unit tests. //
   */
   ...
   */
  protected void fail(final Callable<?> c, final Class<? extends Throwable> expected) {
