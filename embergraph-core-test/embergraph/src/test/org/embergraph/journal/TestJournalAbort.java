 ...
 *     <p>TODO Thia should be a proxied test suite. It is RWStore specific.
 */
public class TestJournalAbort extends TestCase2 {
   ...
   * method that AbstractJournal calls after calling bufferStragey.reset().
   *
  ...
  public void test_simpleAbortFailure() throws InterruptedException {
    ...
    if (log.isInfoEnabled())
      log.info("Pre Abort Commit Counter: " + jnl.getCommitRecord().getCommitCounter());
    btree.close(); // force re-open
    addSomeData(btree);
    btree.writeCheckpoint();
    jnl.abort();
    if (log.isInfoEnabled())
      log.info("Post Abort Commit Counter: " + jnl.getCommitRecord().getCommitCounter());
    ...
    final BTree btree2 =
        jnl.getIndex(btreeName); // Note: BTree was marked as invalid. Must be reloaded.
