...
public class TestAddDropIndexTask extends ProxyTestCase {
  ...
  public void test_addDropIndex_twice() throws InterruptedException, ExecutionException {
    ...
    try {
       ...
       */
      {
        ...
        assertEquals(indexUUID, journal.getIndex(name).getIndexMetadata().getIndexUUID());
      }
   ...
   * means that the {@link AbstractJournal#_name2Addr} can not be loaded from the commit record!
   *
   * @throws InterruptedException
   */
