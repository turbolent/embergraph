 ...
 * dependency on this test suite to get it right for the base class.
 *
 * @todo Test {@link IBlock} read through semantics.
 * @todo Test when version timestamps are supported.
 * @todo write a test suite for concurrent modification under traversal and implement support for
 *     that feature in the various iterators.
 * @todo test with to/from keys.
 * @todo test with filter.
 * @todo Test delete behind semantics more throughly.
 * @todo write tests when some entries are deleted (when deletion markers are and are not supported
 *     and when DELETED are and are not requested). make sure that the concept of delete marker vs
 *     version timestamp is differentiated in the {@link ITuple} interface and the {@link
 *     ITupleIterator} implementations.
 * @todo check the last visited key when changing chunks and its interaction with the delete behind
 *     semantics.
 * @todo test delete behind of the last key of the last chunk (non batch, but everything up to that
 *     should be batched on the chunked boundary).
...
public class TestChunkedIterators extends AbstractBTreeTestCase {
  ...
  /** Unit test for (de-)serialization of {@link ResultSet}s used by the chunked iterators. */
  public void test_chunkedIteratorResultSets() {

    doChunkedIteratorResultSetTest(
        1000 /* N */, true /* deleteMarkers */, true /* versionTimestamps */);

    doChunkedIteratorResultSetTest(
        1000 /* N */, true /* deleteMarkers */, false /* versionTimestamps */);

    doChunkedIteratorResultSetTest(
        1000 /* N */, false /* deleteMarkers */, true /* versionTimestamps */);
   ...
   */
  protected void doChunkedIteratorResultSetTest(
      final int N, final boolean deleteMarkers, final boolean versionTimestamps) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      // insert first
      ndx.insert(
          key, val, false /* delete */, false /*putIfAbsent*/, timestamp, ndx.getWriteTuple());
    ...
    doDeserializationTest(
        ndx,
        tupleSer.serializeKey(2L) /* fromKey */,
        null /* toKey */,
        0 /* capacity */,
        IRangeQuery.DEFAULT /* flags */,
        null /* filter */);
    ...
    doDeserializationTest(
        ndx,
        null /* fromKey */,
        tupleSer.serializeKey(20L) /* toKey */,
        0 /* capacity */,
        IRangeQuery.DEFAULT /* flags */,
        null /* filter */);
    ...
    doDeserializationTest(
        ndx,
        tupleSer.serializeKey(2L) /* fromKey */,
        tupleSer.serializeKey(10L) /* toKey */,
        0 /* capacity */,
        IRangeQuery.DEFAULT /* flags */,
        null /* filter */);
    ...
    doDeserializationTest(
        ...
        null /* toKey */,
        0 /* capacity */,
        IRangeQuery.DEFAULT | IRangeQuery.REVERSE /* flags */,
        null /* filter */);
    ...
    doDeserializationTest(
        ...
        null /* toKey */,
        0 /* capacity */,
        IRangeQuery.DEFAULT | IRangeQuery.CURSOR /* flags */,
        null /* filter */);
    ...
    final int n1; // the #of tuples that were deleted.
    {
      final ITupleIterator<?> itr =
          ndx.rangeIterator(
              ...
              null /* toKey */,
              0 /* capacity */,
              IRangeQuery.DEFAULT | IRangeQuery.REMOVEALL /* flags */,
              null /* filter */);
    ...
    final int n2 =
        doDeserializationTest(
            ...
            null /* toKey */,
            N /* capacity */,
            IRangeQuery.DEFAULT | IRangeQuery.DELETED /* flags */,
            null /* filter */);
   ...
   * force (de-)serialization of each {@link ResultSet}. The iterators are then compared and should
   * visit the same {@link ITuple}s in the same order.
   *
   * @param ndx
   * @param fromKey
   * @param toKey
   ...
   * @throws AssertionError if the tuples visited differ in any manner.
   */
  protected int doDeserializationTest(
      final BTree ndx,
      final byte[] fromKey,
      final byte[] toKey,
      final int capacity,
    ...
    final ITupleIterator<String> itre =
        ndx.rangeIterator(fromKey, toKey, capacity, flags, filterCtor);
    ...
    final ITupleIterator<String> itra =
        new ChunkedLocalRangeIterator<String>(ndx, fromKey, toKey, capacity, flags, filterCtor) {
   ...
   * @param itra The 'actual' iterator.
   * @return The #of tuples visited.
   * @throws AssertionError if the tuples visited differ in any manner.
   * @todo refactor into base class for B+Tree unit tests.
   */
  private int assertSameIterator(final ITupleIterator<?> itre, final ITupleIterator<?> itra) {
    ...
    while (itre.hasNext()) {
      ...
      assertEquals("versionTimestamp", te.getVersionTimestamp(), ta.getVersionTimestamp());
       ...
       * from a fused view.
       */
      assertEquals("sourceIndex", te.getSourceIndex(), ta.getSourceIndex());

      {
        ...
        try {
          oa = ta.getObject();
          if (unsupported) fail("Should not be able to de-serialize an object here");
        } catch (UnsupportedOperationException ex) {
