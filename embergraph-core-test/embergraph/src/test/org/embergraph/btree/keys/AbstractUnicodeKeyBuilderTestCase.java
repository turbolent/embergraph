...
public abstract class AbstractUnicodeKeyBuilderTestCase extends TestCase2 {
  ...
  public void test_keyBuilder_unicode_string_key_us_primary() {
    ...
    properties.setProperty(Options.STRENGTH, StrengthEnum.Primary.toString());
   ...
   *
   * <p>Note: The trailing <code>nul</code> byte is appended by the ICU library in order to have
   * compatibility with their C library, but it is not of interest for Java processing. However,
   * note that a <code>nul</code> byte MAY be used to separate components of a complex key.
   */
  public void test_keyBuilder_unicode_String_noTrailingNul() {
    ...
    assertNotSame("Not expecting a trailing nul byte.", (byte) 0, key[key.length - 1]);
  }
  ...
  public void test_keyBuilder_normalizeTrailingPadCharacters() {
    ...
    assertEquals(keyBuilder.normalizeText(""), keyBuilder.normalizeText(" "));
    assertEquals(keyBuilder.normalizeText(""), keyBuilder.normalizeText("  "));
    assertEquals(keyBuilder.normalizeText(""), keyBuilder.normalizeText("      "));
    assertEquals(keyBuilder.normalizeText(" "), keyBuilder.normalizeText("      "));
    assertEquals(keyBuilder.normalizeText("abc"), keyBuilder.normalizeText("abc      "));
    assertEquals(keyBuilder.normalizeText("   abc"), keyBuilder.normalizeText("   abc      "));
