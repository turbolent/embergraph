...
public class TestKeyBuilder extends TestCase2 {
  ...
  public void test_keyBuilder_ensureCapacity03() {
    ...
    assertEquals(
        ...
        BytesUtil.compareBytesWithLenAndOffset(
            0, expected.length, expected, 0, expected.length, keyBuilder.array()));
  ...
  public void test_keyBuilder_append_bytes() {
    ...
    assertEquals(
        0,
        BytesUtil.compareBytesWithLenAndOffset(
            0, expected.length, expected, 0, keyBuilder.len(), keyBuilder.array()));
   ...
   * (signed) becomes 0x80 (unsigned) while -1 (signed) is becomes to 0x79 (0x79 LT 0x80).
   */
  ...
  public void test_keyBuilder_short_key() {
    ...
    assertEquals(
        (short) 0,
        KeyBuilder.decodeShort(TestKeyBuilder.asSortKey(Short.valueOf((short) 0)), 0 /* off */));

    assertEquals(
        (short) -1,
        KeyBuilder.decodeShort(TestKeyBuilder.asSortKey(Short.valueOf((short) -1)), 0 /* off */));
    ...
    assertEquals(
        Short.MIN_VALUE,
        KeyBuilder.decodeShort(
            TestKeyBuilder.asSortKey(Short.valueOf(Short.MIN_VALUE)), 0 /* off */));

    assertEquals(
        Short.MAX_VALUE,
        KeyBuilder.decodeShort(
            TestKeyBuilder.asSortKey(Short.valueOf(Short.MAX_VALUE)), 0 /* off */));
  }
  ...
  public void test_keyBuilder_float_key() throws NoSuccessorException {
    ...
    final byte[] kmin =
        keyBuilder.reset().append(SuccessorUtil.FNEG_MAX).getKey(); // largest negative float.
    final byte[] kn1 = keyBuilder.reset().append(SuccessorUtil.FNEG_ONE).getKey(); // -1f
    final byte[] kneg =
        keyBuilder.reset().append(SuccessorUtil.FNEG_MIN).getKey(); // smallest negative float.
    ...
    final byte[] kpos =
        keyBuilder.reset().append(SuccessorUtil.FPOS_MIN).getKey(); // smallest positive float.
    final byte[] kp1 = keyBuilder.reset().append(SuccessorUtil.FPOS_ONE).getKey(); // +1f;
    final byte[] kmax =
        keyBuilder.reset().append(SuccessorUtil.FPOS_MAX).getKey(); // max pos float.
  ...
  public void test_keyBuilder_double_key() throws NoSuccessorException {
    ...
    final byte[] kmin =
        keyBuilder.reset().append(SuccessorUtil.DNEG_MAX).getKey(); // largest negative double.
    final byte[] kn1 = keyBuilder.reset().append(SuccessorUtil.DNEG_ONE).getKey(); // -1f
    final byte[] kneg =
        keyBuilder.reset().append(SuccessorUtil.DNEG_MIN).getKey(); // smallest negative double.
    ...
    final byte[] kpos =
        keyBuilder.reset().append(SuccessorUtil.DPOS_MIN).getKey(); // smallest positive double.
    final byte[] kp1 = keyBuilder.reset().append(SuccessorUtil.DPOS_ONE).getKey(); // +1f;
    final byte[] kmax =
        keyBuilder.reset().append(SuccessorUtil.DPOS_MAX).getKey(); // max pos double.
   ...
   */
  static void doMultiFieldTests(final boolean unicode, final KeyBuilder keyBuilder) {
   ...
   * </pre>
   *
   * @param unicode When <code>true</code> the text will be encoded as Unicode using the default
   *     collator. Otherwise the text will be encoded as ASCII.
   * @param text The text to be encoded into the 1st field of the key.
   * @param nextValue The value to be encoded into the next field of the key.
   */
  static void doMultiFieldTest(
      KeyBuilder keyBuilder, final boolean unicode, final String text, final Object nextValue) {

    // form a key from [text][nextValue].
    keyBuilder.reset();
    final byte[] k1 =
        keyBuilder.appendText(text, unicode, false /* successor */).append(nextValue).getKey();

    // form a key from [successor(text)][nextValue].
    keyBuilder.reset();
    final byte[] k2 =
        keyBuilder.appendText(text, unicode, true /* successor */).append(nextValue).getKey();
  ...
  public void test_float_order() {
    ...
    for (int i = 0; i < limit; i++) {
      ...
      if (oldval != null) {
        ...
        fail("Key already exists: " + BytesUtil.toString(key) + " with value=" + oldval);
      }
    }
   ...
   *
   * @todo It fact, it appears that the operation is symmetric. So perhaps get rid of one? Or just
   *     make a note of this on the KeyBuilder methods?
   * @todo KeyBuilder#encodeByte(byte) is only used by the RDF package to generate the prefix for
   *     the terms index. if the order is wrong then that prefix could be unsigned.
   */
    ...
    if (cause != null || !expected.equals(actual)) {
      ...
      final String msg =
          "BigInteger"
              ...
              + "\nunsigned="
              + BytesUtil.toString(expected.toByteArray())
              + "\nencoded ="
              + BytesUtil.toString(encoded)
              + "\nactual  ="
              + actual
              + (actual != null
                  ? "\nactualS ="
    ...
    if (ret >= 0) {
      ...
      fail(
          "BigInteger"
              ...
              + i2
              + "\ns1="
              + Arrays.toString(i1.toByteArray())
              + "\ns2="
              + Arrays.toString(i2.toByteArray())
              + "\nu1="
              + BytesUtil.toString(i1.toByteArray())
              + "\nu2="
              + BytesUtil.toString(i2.toByteArray())
              + "\nk1="
              + BytesUtil.toString(k1)
              + "\nk2="
              + BytesUtil.toString(k2)
              + "\nret="
              + (ret == 0 ? "EQ" : (ret < 0 ? "LT" : "GT")));
    }
  }
  ...
  /** Test with positive and negative {@link BigDecimal}s with large exponents */
  ...
  /** Stress test with random byte[]s from which we then construct {@link BigInteger}s. */
  ...
  /** Stress test with random byte[]s from which we then construct {@link BigDecimal}s. */
   ...
   * decimal point.
   *
   * <p>FIXME This fails for "0" and "0.0". The trailing .0 is considered a significant digit and is
   * not being stripped. We need to also strip trailing zeros which are significant.
   *
   * <pre>
  ...
  //                      "\nunsigned=" +
  // BytesUtil.toString(expected.unscaledValue().toByteArray())+
  ...
  private enum CompareEnum {
    ...
    public static CompareEnum valueOf(final int ret) {
      if (ret < 0) return LT;
      if (ret > 0) return GT;
      return EQ;
    }
   ...
   *
   * <p>Note: Strings are Unicode safe for the default locale. See {@link Locale#getDefault()}. If
   * you require a specific local or different locals at different times or for different indices
   * then you MUST provision and apply your own {@link KeyBuilder}.
   *
   * <p>Note: This method circumvents explicit configuration of the {@link KeyBuilder} and is used
   * nearly exclusively by unit tests. While explicit configuration is not required for keys which
   * do not include Unicode sort key components, this method also relies on a single global {@link
   * KeyBuilder} instance protected by a lock. That lock is therefore a bottleneck. The correct
   * practice is to use thread-local or per task {@link IKeyBuilder}s to avoid lock contention.
   *
