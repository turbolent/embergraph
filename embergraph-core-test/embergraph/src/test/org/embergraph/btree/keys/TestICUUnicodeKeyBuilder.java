...
public class TestICUUnicodeKeyBuilder extends AbstractUnicodeKeyBuilderTestCase {
   ...
   * the successor was formed.
   *
   * <p>Note: Since {@link Collator#IDENTICAL} appears to be required to differentiate a trailing
   * nul character (i.e., the successor of some Unicode string), then I would strongly recommend
   * that you form the sort key first and then its successor (by appending a trailing nul).
   */
  public void test_keyBuilder_unicode_trailingNuls() {
    ...
    System.err.println(
        "Minimum strength ("
            + minStrength
            + ") to differentiate trailing nul character is: "
            + (minStrength == Collator.PRIMARY
                ? "PRIMARY"
                : (minStrength == Collator.SECONDARY
                    ? "SECONDARY"
                    : (minStrength == Collator.TERTIARY
                        ? "TERTIARY"
                        : (minStrength == Collator.QUATERNARY
                            ? "QUARERNARY"
                            : (minStrength == Collator.IDENTICAL
                                ? "IDENTICAL"
                                : "" + minStrength))))));
  }
   ...
   * @return True iff the collector differentiates between the string and its successor (formed by
   *     appending a nul character) in its generated sort keys.
   */
  protected boolean doSuccessorTest(String s, Properties properties) {
    ...
    if (ret >= 0) {
      log.warn("Key1 does NOT order less than successor(key1) : comparator returns " + ret);
      System.err.println("text=" + s);
      System.err.println("strength=" + properties.getProperty(Options.STRENGTH));
