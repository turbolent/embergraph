...
public class TestIncrementalWrite extends AbstractBTreeTestCase {
  ...
  protected BTree getBTree(int branchingFactor, final int queueCapacity, final int queueScan) {
    ...
    BTree btree =
        new TestBTree(store, checkpoint, md, false /*readOnly*/) {
  ...
  public void test_incrementalWrite02() {
    ...
    assertEquals(
        new PO[] {a, b, c},
        ((MyHardReferenceQueue<PO>) btree.writeRetentionQueue).toArray(new PO[0]));
   ...
   * the persistent node knows the persistent identity of each child).
   */
