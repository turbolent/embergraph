 ...
 * write-write conflicts can be detected and provide versions of those tests where the conflict can
 * and can not be validated and verify the end state in each case.
 *
 * <p>State-based validation requires transparency at the object level, including the ability to
 * deserialize versions into objects, to compare objects for consistency, to merge data into the
 * most recent version where possible and according to data type specific rules, and to
 * destructively merge objects when the conflict arises on <em>identity</em> rather than state.
 *
 * <p>An example of an identity based conflict is when two objects are created that represent URIs
 * in an RDF graph. Since the lexicon for an RDF graph generally requires uniqueness those objects
 * must be merged into a single object since they have the same identity. For an RDFS store
 * validation on the lexicon or statements ALWAYS succeeds since they are always consistent.
 *
 * @todo Verify that we can handle the bank account example (this is state-based conflict resolution
 *     altogether requiring that we carry a richer representation of state in the objects and then
 *     use that additional state to validate and resolve some kinds of data type specific
 *     conflicts).
 * @todo Do tests that verify that multiple conflicts are correctly detected and resolved.
 * @todo Verify that we can handle examples in which we have to traverse an object graph during
 *     conflict resolution. (Really, two object graphs: a readOnly view of the ground state for the
 *     transaction and the readWriteTx that we are trying to validate.) This last issue is by far
 *     the trickyest and may require support for concurrent modification of the transaction indices
 *     during traveral (or more simply of reading from a fused view of the resolved and
 *     unconflicting entries in the read-write tx index views).
 * @todo Destructive merging of objects in a graph can propagate changes other objects. Unless the
 *     data structures provide for encapsulation, e.g., by defining objects that serve as collectors
 *     for the link set members in a given segment, that change could propagate beyond the segment
 *     in which it is detected. If changes can propagate in that manner then care MUST be taken to
 *     ensure that validation terminates.
