 ...
 *     First, there is a distinct between stable state (the account balance) and transaction local
 *     state (the low, high, and change for that account within the transaction).<br>
 *     This notion of validation is in terms of the objects API (credit and debit in this case)
 *     rather than in terms of writes of opaque state that are then unpacked when a write-write
 *     conflict is detected.<br>
 *     In the atomic commit protocol, validation examines transaction local state as well as the
 *     global state and updates the global state atomically iff validation succeeds.<br>
 *     This does not appear to depend on the notion of version counters to trigger state-based This
 *     raises lots of questions. I need to think through how a transaction containing multiple
 *     objects could be modeled, how this relates to what is already implemented, and the
 *     relationship between this approach and the notions that I have for handling link set
 *     membership and clustered index membership changes with high concurrency.
...
public class TestAccount extends TestCase {
   ...
   * against the basic data type and on commit it updates the basic data type.
   *
   * @see http://www.cs.brown.edu/~mph/Herlihy90a/p96-herlihy.pdf, section 5.2 page 112.
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
   ...
   * due to the operations on the transaction is applied to the {@link Account} balance.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
   */
  public static class TxAccount {
     ...
     *
     * @param cents The amount.
     * @throws OverdraftException iff an overdraft must result.
     */
     ...
     * atomicity with respect to the account balance.
     */
     ...
     * synchronizing on the {@link #account}. However that technique will not work in a distributed
     * system.
     */
    public void commit() {
   ...
   * This schedule interleaves two transactions, P and Q. There is an initial balance of $0. There
   * is a $5 credit on P followed by a $6 credit on Q. P then validates and commits (validation
   * occurs during the commit protocol), with a resulting stable balance of $5. A $10 debit is then
   * made on Q and Q validates and commits (again, validation is part of the commit). The final
   * stable balance is $1.
   *
   * <pre>
   *         a Credit($5)/Ok( ) P
