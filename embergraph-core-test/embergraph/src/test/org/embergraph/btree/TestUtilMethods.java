...
public class TestUtilMethods extends AbstractBTreeTestCase {
  ...
  //
  //        assertEquals("[{1, 2, 3}, {4, 5, 6}]", AbstractNode.keysAsString(3, new
  // int[]{1,2,3,4,5,6}));
  //
  //        assertEquals("[{a, b, c}, {d, e, f}]", AbstractNode.keysAsString(3, new
  // char[]{'a','b','c','d','e','f'}));
  //
  //    }
  //
  ...
  /*
   * Note: the arrays are dimensions to nkeys + 1 for a leaf to provide a slot for overflow during a
   * split.
   */
  public void test_assertKeysMonotonic() {
    ...
    final Leaf leaf =
        new Leaf(
            ...
            1L,
            new MutableLeafData(
                new MutableKeyBuffer(
                    nkeys,
                    // keys
                    new byte[][] {new byte[] {1}, new byte[] {2}, new byte[] {3}, null}),
                // vals
                new MutableValueBuffer(
                    nkeys,
                    // vals
