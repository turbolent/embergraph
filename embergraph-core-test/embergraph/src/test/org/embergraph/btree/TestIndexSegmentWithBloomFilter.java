 ...
 * @todo report on the cost to construct the filter and its serialized size and runtime space.
 * @todo verify the target error rate.
 * @todo explore different error rates, including Fast.mostSignificantBit( n ) + 1 which would
 *     provide an expectation of no false positives.
 * @todo Compare for each build algorithm, just like {@link TestIndexSegmentBuilderWithLargeTrees}.
...
public class TestIndexSegmentWithBloomFilter extends AbstractBTreeTestCase {
   ...
   * should be build for a given set of entries (key-value pairs) and a given output branching
    ...
    for (int i = 0; i < branchingFactors.length; i++) {
      ...
      doBuildIndexSegmentAndCompare(
          doSplitWithRandomDenseKeySequence(
              getBTree(m, new BloomFilterFactory(m /* n */, p, maxP)), m, m));

      doBuildIndexSegmentAndCompare(
          doSplitWithRandomDenseKeySequence(
              getBTree(m, new BloomFilterFactory(m * m /* n */, p, maxP)), m, m * m));

      doBuildIndexSegmentAndCompare(
          doSplitWithRandomDenseKeySequence(
              getBTree(m, new BloomFilterFactory(m * m * m /* n */, p, maxP)), m, m * m * m));

      // @todo overflows the initial journal extent.
      //            doBuildIndexSegmentAndCompare( doSplitWithRandomDenseKeySequence(
      // getBTree(m,errorRate), m, m*m*m*m ) );
  ...
  /*
   * A stress test for building {@link IndexSegment}s. A variety of {@link BTree}s are built from
   * spase random keys using a variety of branching factors. For each {@link BTree}, a variety of
  ...
  /*
   * Test when the input tree is a root leaf with three values. The output tree will also be a root
   * leaf.
   *
   * @throws IOException
   */
  public void test_rootLeaf() throws Exception {
    ...
    final BTree btree = getBTree(m, new BloomFilterFactory(100, 1 / 64d, 1 / 32d));
  ...
  public void doBuildIndexSegmentAndCompare(final BTree btree) throws Exception {
    ...
    try {
       ...
       * Note: Since we know the exact #of index entries in an index
           ...
           */
          metadata.setBloomFilterFactory(
              new BloomFilterFactory(1 /* n */, 1 / 64d /* p */, 1 / 32d /* maxP */));
       ...
       * Closing the journal.
       */
      if (log.isInfoEnabled()) log.info("Closing journal.");
      btree.getStore().destroy();
    }
  }

  /*
   * Test the bloom filter for false negatives given the ground truth set of keys. if it reports
   * that a key was not in the bloom filter then that is a false negative. bloom filters are not
   * supposed to have false negative.
   *
   * @param keys The ground truth keys that were inserted into the bloom filter.
   */
  protected void doBloomFilterTest(String label, IBloomFilter bloomFilter, byte[][] keys) {
    ...
    for (int i = 0; i < order.length; i++) {
      ...
      assertTrue("false negative: i=" + i + ", key=" + BytesUtil.toString(key), found);
    }
  }
}
