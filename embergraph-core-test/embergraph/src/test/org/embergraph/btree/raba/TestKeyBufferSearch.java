 ...
 *     perhaps it could be used by elevating the method to test the prefix into the performance
 *     test. another twist for the performance test would be testing for loop unrolling conditions
 *     (N<~4) and testing JNI vs pure Java for the comparison functions.
 * @todo do version of test with negative and positive integer keys so that the search on the
 *     encoded keys detects whether signed bytes or unsigned bytes are being compared.
 * @todo do tests with JNI code linked in. note that we only use {@link
 *     BytesUtil#compareBytesWithLenAndOffset(int, int, byte[], int, int, byte[])} for searching on
 *     the key buffers since that allows non-zero offsets into the search key.
...
public class TestKeyBufferSearch extends TestCase2 {
  ...
  private void doPrefixMatchLengthTest(AbstractKeyBuffer kbuf) {
    ...
    assertEquals(0, kbuf._prefixMatchLength(prefixLength, new byte[] {1, 3, 4, 0, 1, 3}));
    /*
...
//         * Generate a set of N random distinct byte[] keys in sorted order using
  ...
  //            Set<byte[]> set = new
  // TreeSet<byte[]>(BytesUtil.UnsignedByteArrayComparator.INSTANCE);
  //
  //            byte[][] keys = new byte[nkeys][];
  //
  //            int n = 0;
