...
public abstract class AbstractRabaCoderTestCase extends TestCase2 {
    ...
    if (rabaCoder.isKeyCoder()) {
...

      {
        ...
        final AbstractFixedByteArrayBuffer data =
            rabaCoder.encode(expected, new DataOutputBuffer());
   ...
   * handling a logical byte[][] consisting solely of <code>null</code>s. We handle this for the
   * case of an empty logical byte[][] using an {@link EmptyRabaValueDecoder}. I suppose that could
   * be parameterized to identify the <code>null</code>s or else just fix the ctor.
   */
  ...
  /** Test with {@link IRaba} having a size of ONE (1) and a variety of capacities. */
  ...
  /** Test with {@link IRaba} having a size of TWO (2) and a variety of capacities. */
  ...
  public void test_randomURIs() throws Exception {
    ...
    if (rabaCoder.isValueCoder()) {
      ...
      // layer on interface.
      final IRaba raba =
          new ReadOnlyValuesRaba(
              0 /* fromIndex */, data.length /* toIndex */, data.length /* capacity */, data);
   ...
   * MORE space than the original byte[][] since the canonical huffman code can not be shorter than
   * the original byte values on average when all values are equally likely. When you want to
   * measure the expected compression ratio you need to test with a gaussian distribution (bell
   * curve) over the values, a USASCII distribution, etc.
   *
   * @param size The #of entries in the byte[][].
   * @param capacity The capacity of the byte[][].
   * @throws IOException
   */
  protected void doRandomRoundTripTest(
    ...
    if (dataCoder.isValueCoder()) {
      ...
      // layer on interface.
      final IRaba raba =
          new ReadOnlyValuesRaba(0 /* fromIndex */, size /* toIndex */, capacity, data);
    ...
    if (dataCoder.isKeyCoder()) {
      ...
      for (int i = 0; i < size; i++, nactual++) {
        ...
        final int incRange = (int) Math.max(nominalIncRange, Math.min(nominalIncRange, remainder));
      ...
      // layer on interface.
      final IRaba raba =
          new ReadOnlyKeysRaba(0 /* fromIndex */, nactual /* toIndex */, capacity, data);

      doRoundTripTest(dataCoder, raba);
    }
  }

  public static void doRoundTripTest(final IRabaCoder rabaCoder, final IRaba expected) {
    ...
    try {
      ...
      final ICodedRaba liveCodedRaba = rabaCoder.encodeLive(expected, new DataOutputBuffer());
      ...
      // verify we can decode the encoded data.
      {
        ...
        // format.
        AbstractBTreeTestCase.assertSameRaba(expected, rabaCoder.decode(actual0.data()));
      }
      ...
      // returns a slice which has the same data.
      {
        ...
        final int off = 10;
        final DataOutputBuffer out = new DataOutputBuffer(off, new byte[100 + off]);

        // encode onto that buffer.
        final AbstractFixedByteArrayBuffer slice = rabaCoder.encode(expected, out);
      ...
      // non-zero offset
      {
        ...
        System.arraycopy(originalData.array(), originalData.off(), tmp, off, originalData.len());

        // create slice
        final FixedByteArrayBuffer slice = new FixedByteArrayBuffer(tmp, off, originalData.len());
   ...
   * are parameterized here by a normalized vector of the rates of the different operations
   * (search(), get(), length(), etc).
   *
   * <dl>
   *   <dt>nops
   *   <dd>The #of random operations to be performed. Large values for <i>nops</i> need to be used
   *       to get beyond the initial JVM performance tuning so you can more accurately compare the
   *       performance of the different coders. For example, a value of 1M (1000000) will run for ~
   *       30-40s for the front-coded coders. For shorter run times, the order in which we test the
   *       coders will dominate their performance.
   *   <dt>size
   *   <dd>The #of entries in the raba to be tested (must be LTE the capacity)
   * </dl>
   *
   * @param args [nops [generator [size]]]
   *     <p>FIXME parameterize the generator choice.
   */
  public static void main(final String[] args) {
    ...
    if (args.length > 0) nops = Integer.valueOf(args[0]);
    ...
    int size = 256;
    if (args.length > 2) nops = Integer.valueOf(args[2]);
    if (size <= 0) throw new IllegalArgumentException();

    // The coders to be tested.
    final IRabaCoder[] coders =
        new IRabaCoder[] {
    ...
    System.out.println("nops=" + nops + ", size=" + size + ", ncoders=" + coders.length);
    ...
    for (IRabaCoder rabaCoder : coders) {
      ...
      try {
        ...
        recordLength = doRabaCoderPerformanceTest(expected, rabaCoder, size, nops, r, op);
  ...
  public void test_keyCoderPerformance() {
    ...
    final int[] branchingFactors =
        new int[] {3, 4, 8, 16, 27, 32, 48, 64, 96, 99, 112, 128, 256, 512, 1024, 4096};

    final int capacity = branchingFactors[r.nextInt(branchingFactors.length)];

    final int size = r.nextInt(capacity) + 1;

    // Generate a read-only raba.
    final ReadOnlyKeysRaba expected =
        new ReadOnlyKeysRaba(size, AbstractBTreeTestCase.getRandomKeys(capacity, size));
   ...
   *
   * @param rabaCoder The coder to be tested.
   * @param size The #of keys.
   * @param nops The #of operations to perform.
   * @param r The random number generator.
   * @param op The distribution of the operations to be performed.
   * @return The size of the coded record.
   */
  public static int doRabaCoderPerformanceTest(
      final IRaba expected,
      final IRabaCoder rabaCoder,
      final int size,
      final int nops,
      final Random r,
      final Op op) {
    ...
    final byte[] originalData;
    {
      ...
      // encode the record.
      final AbstractFixedByteArrayBuffer data = rabaCoder.encode(expected, new DataOutputBuffer());
    ...
    for (int i = 0; i < nops; i++) {
      ...
      switch (code = op.nextOp(r)) {
        case Op.ISNULL:
          {
            ...
            if (log.isDebugEnabled())
              log.debug(op.getName(code) + "(" + index + ") : expected=" + expected.isNull(index));
            assertEquals(op.getName(code), expected.isNull(index), actual.isNull(index));
            break;
          }
        case Op.LENGTH:
          {
            final int index = r.nextInt(size);
            if (log.isDebugEnabled())
              log.debug(op.getName(code) + "(" + index + ") : expected=" + expected.length(index));
            assertEquals(op.getName(code), expected.length(index), actual.length(index));
            break;
          }
        case Op.GET:
          {
            ...
            assertEquals(op.getName(code), expected.get(index), actual.get(index));
            break;
          }
        case Op.COPY:
          {
            ...
            assertTrue(
                0
                    == BytesUtil.compareBytesWithLenAndOffset(
                        0, len, os.array(), 0, len, expected.get(index)));
            break;
          }
        case Op.SEARCH:
          {
            ...
            if (log.isDebugEnabled())
              log.debug(op.getName(code) + "(" + index + ") : key=" + BytesUtil.toString(key));
            { // search at the key.
        ...
        case Op.ITERATOR:
          {
            if (log.isDebugEnabled()) log.debug(op.getName(code) + "()");
        ...
        case Op.RECODE:
          {
            ...
            final byte[] tmp = (r.nextFloat() < .1 ? new byte[0] : new byte[r.nextInt(100) * size]);
            ...
            // random start offset.
            final int start = Math.min(tmp.length, (r.nextFloat() < .2 ? 0 : r.nextInt(20)));
            //                final int start = 0;

            if (log.isDebugEnabled())
              log.debug(op.getName(code) + "(): start=" + start + ", buf.len=" + tmp.length);
            ...
            try {
              ...
              final AbstractFixedByteArrayBuffer data = rabaCoder.encode(actual, buf);
              ...
              fail(op.getName(code) + "(): start=" + start + ", buf.len=" + tmp.length, ex);
            }
    ...
    double totalNS = 0;
    for (long ns : elapsed) totalNS += ns;
    ...
    for (int i = 0; i < count.length; i++) {
      ...
      System.out.println(
          op.getName(i)
              ...
              + "\t"
              + (elapsed[i] == 0 ? "N/A" : rateF.format(count[i] / (elapsed[i] * scalingFactor))));
    }
   ...
   * @param aitr The actual iterator.
   */
  protected static void assertSameIterator(
      final Iterator<byte[]> eitr, final Iterator<byte[]> aitr) {
  ...
  static class Op {
    ...
    public Op(
      ...
      float total =
          isNullRate + lengthRate + getRate + copyRate + searchRate + iteratorRate + recodeRate;
      if (total == 0.0) {
