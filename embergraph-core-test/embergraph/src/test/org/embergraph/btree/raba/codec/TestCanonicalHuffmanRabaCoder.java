...
public class TestCanonicalHuffmanRabaCoder extends AbstractRabaCoderTestCase {
   ...
   */
  protected static String printCodeBook(final BitVector[] codeWords) {
  ...
  public void doRoundTripTest(final int[] frequency) {
    ...
    final CanonicalFast64CodeWordDecoder actualDecoder =
        new CanonicalFast64CodeWordDecoder(decoderInputs.getLengths(), decoderInputs.getSymbols());
   ...
   *
   * @param frequency The frequency[] should include a reasonable proportion of symbols with a zero
   *     frequency in order to replicate the expected conditions when coding non-random data such as
  ...
  public void doRecoderRoundTripTest(final int[] frequency) throws IOException {
    ...
    if (log.isDebugEnabled()) log.debug(printCodeBook(codec.codeWords()));
    ...
    final byte[] codedValue;
    {
      ...
      assertEquals(
          ...
          BytesUtil.compareBytesWithLenAndOffset(
              0 /* aoff */, ebaos.length, ebaos.array, 0 /* boff */, abaos.length, abaos.array));
   ...
   * the {@link DecoderInputs} due to a bug in the {@link HuffmanCodec} when nsymbols == 0.
   * Therefore, this verifies that the {@link Coder} and {@link DecoderInputs} are <code>null</code>
   * and that the symbol count is zero.
   *
   * @throws IOException
   */
  ...
  public void test_keyRabaSetup() throws IOException {
    ...
    doDecoderInputRoundTripTest(setup.getSymbolCount(), setup.decoderInputs());

    // verify that we can re-create the coder.
    doCoderRoundTripTest(
        setup.codec().codeWords(),
        setup.decoderInputs().getShortestCodeWord(),
        setup.decoderInputs().getLengths(),
        setup.decoderInputs().getSymbols());
  }
   ...
   * @throws IOException
   */
  private void doDecoderInputRoundTripTest(final int nsymbols, final DecoderInputs decoderInputs)
      throws IOException {
    ...
    final byte[] in;
    {
      ...
      final StringBuilder sb =
          CanonicalHuffmanRabaCoder.log.isDebugEnabled() ? new StringBuilder() : null;
...

    {
      ...
      final DecoderInputs actualInputs =
          CanonicalHuffmanRabaCoder.readDecoderInputs(nsymbols, ibs, sb);
      ...
      assertEquals("length[]", decoderInputs.getLengths(), actualInputs.getLengths());

      assertEquals("symbol[]", decoderInputs.getSymbols(), actualInputs.getSymbols());
    }
  }
   ...
   * @param
   */
  private void doCoderRoundTripTest(
      final BitVector[] expected,
      final BitVector shortestCodeWord,
      final int[] length,
      final int[] symbol) {

    final PrefixCoder newCoder = HuffmanCodec.newCoder(shortestCodeWord, length, symbol);
   ...
   *
   * <p>TODO Could be a performance comparison.
   *
   * @throws IOException
   */
