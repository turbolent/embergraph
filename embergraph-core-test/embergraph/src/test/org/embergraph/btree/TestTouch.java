...
public class TestTouch extends AbstractBTreeTestCase {
   ...
   * means that it is not made persistent when it is evicted.
   */
  public void test_touch01() {
    ...
    final BTree btree;
    {
      ...
      btree =
          new BTree(store, checkpoint, metadata, false /*readOnly*/) {
   ...
   * the btree is created. The test verifies that merely touching the root leaf causes a reference
   * to the leaf to be evicted from the queue, but does NOT cause the leaf to be made persistent.
   * {@link BTree#touch(AbstractNode)} handles this condition by incrementing the reference counter
   * before appending the node to the queue and therefore ensuring that the reference counter for
   * the node that touched is not zero if the node is also selected for eviction. The test also
   * verifies that the reference counter is correctly maintained across the touch. Since the counter
   * was one before the touch and since the root was itself evicted, the counter after the touch is
   * <code>1+1-1 = 1</code>.
   *
   * <p>FIXME This test needs to use a tree with nodes and leaves or fake another root leaf since
   * the minimum cache size is (2)
   */
