 ...
 * @see TestDirtyIterators, which handles tests when some nodes or leaves are NOT dirty and verifies
 *     that the iterators do NOT visit such nodes or leaves. This tests {@link
 *     AbstractNode#postOrderNodeIterator()} as well since that is just {@link
 *     AbstractNode#postOrderNodeIterator(boolean, boolean)} with <code>false</code> passed in.
...
public class TestIterators extends AbstractBTreeTestCase {
  ...
  public void test_childIterator01() {
    ...
    assertSameIterator(
        new Object[] {a, b}, c.childIterator(k6, k7)); // fence post - visits (a) and (b).
    ...
    assertSameIterator(
        new Object[] {a, b, c},
        c.postOrderIterator(
            k6, k7)); // fence post - visits both leaves even though no keys lie in the range.
  ...
  public void test_postOrderIterator01() {
    ...
    // verify iterator.
    assertSameIterator(new IAbstractNode[] {a, b, c}, btree.root.postOrderNodeIterator());
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {a, d, b, c}, btree.root.postOrderNodeIterator());
    ...
    // verify iterator
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, b, f, g}, btree.root.postOrderNodeIterator());
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {d, e, b, c}, btree.root.postOrderNodeIterator());
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {d, b, c}, btree.root.postOrderNodeIterator());
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {b}, btree.root.postOrderNodeIterator());
  }
