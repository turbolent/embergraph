...
public class TestMutableBTreeCursors extends AbstractBTreeCursorTestCase {
  ...
  public void test_cursor_remove_during_forward_traversal() {
    ...
    final BTree btree;
    {
      btree = BTree.create(new SimpleMemoryRawStore(), new IndexMetadata(UUID.randomUUID()));
  ...
  /** Test ability to remove tuples using {@link ITupleCursor#remove()} during reverse traversal. */
   ...
   * becomes the new root leaf.
   *
   * <p>Note: This test does not covert overflow where a tuple is rotated to the sibling or
   * underflow where a tuple is rotated from the sibling. Those cases (and all cases involving
   * deeper trees) are covered by the various stress tests where a BTree is perturbed randomly and
   ...
   * tuple. Since deleted tuples are being visited by the cursor, the caller will see the deleted
   * tuples.
   */
  public void test_delete_markers_visitDeleted() {
     ...
     */
    {
      ...
      assertEquals(
          new TestTuple<String>(flags, 10, null, true /* deleted */, 0L /* timestamp */),
          cursor.tuple());
   ...
   * willing to visited deleted tuples. Since delete markers are enabled remove() will actually be
   * an update that sets the delete marker on the tuple and clears the value associated with that
   * tuple. Since deleted tuples are NOT being visited by the cursor, the caller deleted tuples will
   * appear to "disappear" just like they do when a tuple is remove()'d from an index that does not
