...
public class TestIndexSegmentAddressManager extends AbstractBTreeTestCase {
  ...
  /** Test of correct decoding of addresses by the {@link IndexSegmentAddressManager}. */
  public void test_addressManager_decode() {
    ...
    final long length;
    {
      ...
      final long addrRoot =
          am.toAddr((int) extentNodes, IndexSegmentRegion.BASE.encodeOffset(offsetNodes));
      ...
      final long addrMetadata =
          am.toAddr(sizeMetadata, IndexSegmentRegion.BASE.encodeOffset(offsetMetadata));
      ...
      if (log.isInfoEnabled()) log.info("Checkpoint: " + checkpoint);
    }
...

    {
      ...
      doRoundTripTest(IndexSegmentRegion.BASE, nbytes, offset, offsetBase + offset, am);

      doRoundTripTest(IndexSegmentRegion.NODE, nbytes, offset, offsetNodes + offset, am);

      doRoundTripTest(IndexSegmentRegion.BLOB, nbytes, offset, offsetBlobs + offset, am);
    }
    ...
    assertEquals(
        0L, am.getOffset(am.toAddr((int) (length - 1), IndexSegmentRegion.BASE.encodeOffset(0L))));
    assertEquals(
        0L, am.getOffset(am.toAddr((int) length, IndexSegmentRegion.BASE.encodeOffset(0L))));
    // illegal.
    try {
      assertEquals(
          0L,
          am.getOffset(am.toAddr((int) (length + 1), IndexSegmentRegion.BASE.encodeOffset(0L))));
    ...
    assertEquals(
        offsetNodes,
        am.getOffset(am.toAddr((int) (extentNodes - 1), IndexSegmentRegion.NODE.encodeOffset(0L))));
    assertEquals(
        offsetNodes,
        am.getOffset(am.toAddr((int) extentNodes, IndexSegmentRegion.NODE.encodeOffset(0L))));
    // illegal.
    try {
      assertEquals(
          offsetNodes,
          am.getOffset(
              am.toAddr((int) (extentNodes + 1), IndexSegmentRegion.NODE.encodeOffset(0L))));
      fail("Expecting exception");
    } catch (AssertionError ex) {
      log.info("Ignoring expected exception: " + ex);
    }
    ...
    assertEquals(
        offsetBlobs,
        am.getOffset(am.toAddr((int) (extentBlobs - 1), IndexSegmentRegion.BLOB.encodeOffset(0L))));
    assertEquals(
        offsetBlobs,
        am.getOffset(am.toAddr((int) extentBlobs, IndexSegmentRegion.BLOB.encodeOffset(0L))));
    // illegal.
    try {
      assertEquals(
          offsetBlobs,
          am.getOffset(
              am.toAddr((int) (extentBlobs + 1), IndexSegmentRegion.BLOB.encodeOffset(0L))));
      fail("Expecting exception");
    } catch (AssertionError ex) {
      log.info("Ignoring expected exception: " + ex);
    }
   ...
   * @param nbytes The #of bytes in the addressed record.
   * @param offset The offset of the addressed record (relative to the start of the region).
   * @param am The object used to decode the address.
   */
  protected static void doRoundTripTest(
      IndexSegmentRegion region,
      int nbytes,
      long offset,
      long expectedOffset,
    ...
    assertEquals("nbytes in " + region + " region", nbytes, am.getByteCount(addr));

    assertEquals("offset in " + region + " region", expectedOffset, am.getOffset(addr));
  }
}
