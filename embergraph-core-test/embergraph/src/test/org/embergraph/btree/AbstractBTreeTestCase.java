...
public abstract class AbstractBTreeTestCase extends TestCase2 {
  ...
  protected static final Logger log = Logger.getLogger(AbstractBTreeTestCase.class);
   ...
   */
  public static void assertKeys(final IRaba expected, final IRaba actual) {
      ...
      for (int i = 1; i < nkeys; i++) {
        ...
        if (BytesUtil.compareBytes(expected.get(i), expected.get(i - 1)) <= 0) {
          ...
          throw new AssertionError(
              "Keys out of order at index=" + i + ", keys=" + expected.toString());
        }

        if (BytesUtil.compareBytes(actual.get(i), actual.get(i - 1)) <= 0) {

          throw new AssertionError(
              "Keys out of order at index=" + i + ", keys=" + actual.toString());
        }
      }
    }
  }
   ...
   *
   * @param keys An array of the defined <code>int</code> keys.
   * @param node The node whose keys will be tested.
   */
   ...
   *
   * @param msg A label, typically the node name.
   * @param values An array containing the expected defined values. The #of values in this array
   *     should be exactly the #of defined values (that is, do not include trailing nulls or attempt
   *     to size the array to the branching factor of the tree).
   */
  public void assertValues(String msg, final Object[] values, final Leaf leaf) {
    ...
    if (!leaf.isReadOnly()) {
       ...
       */
      assertEquals(msg + "values[] capacity", leaf.maxKeys() + 1, leaf.getValues().capacity());
    }
    ...
    for (int i = 0; i < nvalues; i++) {
      ...
      final byte[] expected =
          (values[i] instanceof byte[]) ? (byte[]) values[i] : SerializerUtil.serialize(values[i]);
    ...
    for (int i = nvalues; i < valueCapacity; i++) {
      ...
      if (actual != null) {
        ...
        fail(msg + "values[" + i + "]: expected=null, actual=" + Arrays.toString(actual));
      }
    }
  }
  ...
  public static void assertSameNodeOrLeaf(final AbstractNode<?> n1, final AbstractNode<?> n2) {
   ...
   * @param n2 The actual node state.
   */
  public static void assertSameNode(final Node n1, final Node n2) {
    ...
    assertEquals("branchingFactor", n1.getBranchingFactor(), n2.getBranchingFactor());
   ...
   * @param n2 The actual leaf state.
   */
  public static void assertSameLeaf(final Leaf n1, final Leaf n2) {
  ...
  protected static void assertSameAbstractNodeData(
    ...
    if (n1 instanceof ISpannedTupleCountData) {
      ...
      assertEquals(
          "entryCount",
          ((ISpannedTupleCountData) n1).getSpannedTupleCount(),
          ((ISpannedTupleCountData) n2).getSpannedTupleCount());
    }

    if (n1 instanceof IKeysData) {

      assertEquals("keyCount", ((IKeysData) n1).getKeyCount(), ((IKeysData) n2).getKeyCount());

      assertKeys(((IKeysData) n1).getKeys(), ((IKeysData) n2).getKeys());
    }

    assertEquals("hasVersionTimestamps", n1.hasVersionTimestamps(), n2.hasVersionTimestamps());

    if (n1.hasVersionTimestamps()) {

      assertEquals(
          "minimumVersionTimestamp",
          n1.getMinimumVersionTimestamp(),
          n2.getMinimumVersionTimestamp());

      assertEquals(
          "maximumVersionTimestamp",
          n1.getMaximumVersionTimestamp(),
          n2.getMaximumVersionTimestamp());
    }
  }

  /** Verify all data accessible from {@link INodeData}. */
  public static void assertSameNodeData(final INodeData n1, final INodeData n2) {
    ...
    for (int i = 0; i < n1.getChildCount(); i++) {
      ...
      if (expectedChildEntryCount != actualChildEntryCount) {
        ...
        assertEquals("childEntryCount(" + i + ")", expectedChildEntryCount, actualChildEntryCount);
      }
    }
  }
  ...
  public static void assertSameLeafData(final ILeafData n1, final ILeafData n2) {
    ...
    assertEquals("hasDeleteMarkers", n1.hasDeleteMarkers(), n2.hasDeleteMarkers());

    if (n1.hasDeleteMarkers()) {

      for (int i = 0; i < n1.getKeyCount(); i++) {

        assertEquals("deleteMarkers[" + i + "]", n1.getDeleteMarker(i), n2.getDeleteMarker(i));
      }
    }
    ...
    if (n1.hasVersionTimestamps()) {
      ...
      for (int i = 0; i < n1.getKeyCount(); i++) {
        ...
        assertEquals(
            "versionTimestamps[" + i + "]", n1.getVersionTimestamp(i), n2.getVersionTimestamp(i));
      }
    }
    ...
    assertEquals("hasRawRecords", n1.hasRawRecords(), n2.hasRawRecords());

    if (n1.hasRawRecords()) {

      for (int i = 0; i < n1.getKeyCount(); i++) {

        assertEquals("rawRecords[" + i + "]", n1.getRawRecord(i), n2.getRawRecord(i));
      }
    }
   ...
   * the search API will also be tested.
   *
   * @param expected
   * @param actual
   */
  public static void assertSameRaba(final IRaba expected, final IRaba actual) {
    ...
    for (int i = 0; i < expected.size(); i++) {
      ...
      assertEquals("isNull(" + j + ")", expected.isNull(j), actual.isNull(j));
      ...
      if (!expected.isNull(j)) {
        ...
        // same length
        if (ea.length != aa.length) assertEquals("get(" + j + ").length", ea.length, aa.length);

        // same data.
        if (!BytesUtil.bytesEqual(ea, aa)) assertEquals("get(" + j + ")", ea, aa);

        // verify same byte[] length reported.
        assertEquals("length(" + j + ")", expected.length(j), actual.length(j));
        ...
        try {
          ...
          actual.copy(j, dos);
          dos.flush();
          assertEquals("copy(" + j + ",dos)", expected.get(j), baos.toByteArray());
        ...
        try {
          ...
          if (log.isDebugEnabled()) log.debug("Ignoring expected exception: " + ex);
        }
    ...
    if (expected.isKeys()) {
      ...
      for (int i = 0; i < expected.size(); i++) {
        ...
        { // search at random length prefix of the key.
          ...
          final int prefixLength = Math.max(0, r.nextInt(Math.max(1, key.length)) - 1);
...
//	 * Verifies details for the {@link IBucketData} interface.
   ...
   *     identities in the node.
   * @param node The node.
   */
  public static void assertChildKeys(final long[] childAddr, final Node node) {
    ...
    for (int i = 0; i < nChildAddr; i++) {
      ...
      assertEquals("childAddr[" + i + "]", childAddr[i], node.getChildAddr(i));
    }
   ...
   * @param node The node.
   */
  public static void assertKeys(final byte[][] keys, final AbstractNode<?> node) {
   ...
   *     in the node as returned by {@link INodeData#getChildEntryCounts()}. The sum of the expected
   *     array is also tested against the value returned by {@link
   *     IAbstractNodeData#getSpannedTupleCount()}.
   * @param node The node.
   */
  public static void assertEntryCounts(final int[] expected, final INodeData node) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      assertEquals("childEntryCounts[" + i + "]", expected[i], node.getChildEntryCount(i));
   ...
   * tree operations in this test suite are known to be conducted in an environment without
  ...
  public BTree getBTree(final int branchingFactor, final ITupleSerializer tupleSer) {
...
//     * <p>
   ...
   * They depend on the manner in which the leaves get filled, whether or not holes are created in
   * the leaves, etc.
   *
   * <p>Once all keys have been inserted into the tree the keys are removed in forward order (from
   * the smallest to the largest). This stresses a specific pattern of joining leaves and nodes
   * together with their right sibling.
   *
   * @param m The branching factor.
   * @param ninserts The #of keys to insert.
   */
  public void doSplitWithIncreasingKeySequence(final BTree btree, final int m, final int ninserts) {
    ...
    for (int i = 0; i < keys.length; i++) {
      ...
      if (i > 0 && i % 10000 == 0) {
        ...
        if (log.isInfoEnabled()) log.info("i=" + i + ", key=" + ikey);
      }
   ...
   * or not holes are created in the leaves, etc.
   *
   * <p>Once all keys have been inserted into the tree the keys are removed in reverse order (from
   * the largest to the smallest). This stresses a specific pattern of joining leaves and nodes
   * together with their left sibling.
   *
   * @param m The branching factor.
   * @param ninserts The #of keys to insert.
   */
  public void doSplitWithDecreasingKeySequence(final BTree btree, final int m, final int ninserts) {

    if (log.isInfoEnabled()) log.info("m=" + m + ", ninserts=" + ninserts);
   ...
   *
   * @param m The branching factor. The tree.
   * @param ninserts The #of distinct key-value pairs to insert.
   * @param trace The trace level (zero disables most tracing).
   */
  public BTree doInsertRandomKeySequenceTest(
      final BTree btree, final int ninserts, final int trace) {
   ...
   *
   * @param m The branching factor for the source B+Tree.
   * @param ninserts The #of distinct key-value pairs to insert.
   * @param trace The trace level (zero disables most tracing).
   * @return The populated {@link BTree}.
   */
  public static BTree doInsertRandomSparseKeySequenceTest(
      final BTree btree, final int ninserts, final int trace) {
   ...
   * @return The populated {@link BTree}.
   */
  public static BTree doInsertRandomKeySequenceTest(
      final BTree btree, final int[] keys, final SimpleEntry[] entries, final int trace) {
   ...
   *
   * @param m The branching factor.
   * @param order The key presentation sequence.
   * @param trace The trace level.
   */
  public static void doKnownKeySequenceTest(final BTree btree, final int[] order, final int trace) {
   ...
   * @param keys The keys.
   * @param entries The entries.
   * @param order The order in which the key-entry pairs will be inserted.
   * @param trace The trace level (zero disables most tracing).
   * @return The populated {@link BTree}.
   */
  protected static BTree doInsertKeySequenceTest(
      final BTree btree,
      final int[] keys,
      final SimpleEntry[] entries,
      final int[] order,
      final int trace) {
    ...
    try {
      ...
      for (int i = 0; i < keys.length; i++) {
        ...
        if (i > 0 && i % 10000 == 0) {
          ...
          if (log.isInfoEnabled()) log.info("index=" + i + ", key=" + ikey + ", entry=" + entry);
        }
        ...
        if (trace >= 2) {
          ...
          System.err.println("Before insert: index=" + i + ", key=" + BytesUtil.toString(key));
          assertTrue(btree.dump(System.err));
        }
        ...
        if (trace >= 2) {
          ...
          System.err.println("After insert: index=" + i + ", key=" + BytesUtil.toString(key));
   ...
   * @param ninserts The #of keys to insert.
   */
  public BTree doSplitWithRandomDenseKeySequence(
      final BTree btree, final int m, final int ninserts) {
  ...
  protected void doRandomKeyInsertTest(
      final BTree btree, final int[] keys, final SimpleEntry[] entries, final int[] order) {

    if (log.isInfoEnabled()) log.info("m=" + btree.getBranchingFactor() + ", nkeys=" + keys.length);
   ...
   */
  public void doInsertLookupRemoveStressTest(final int m, final int nkeys, final int ntrials) {

    if (log.isInfoEnabled()) log.info("m=" + m + ", nkeys=" + nkeys + ", ntrials=" + ntrials);
   ...
   * with a root leaf. All inserted keys are eventually deleted by this test and the end state is an
   * empty btree of height zero(0) having a single root leaf.
   */
  public void doRemoveStructureStressTest(final int m, final int nkeys) {

    if (log.isInfoEnabled()) log.info("m=" + m + ", nkeys=" + nkeys);
    ...
    for (int i = 0; i < nkeys; i++) {
      ...
      keys[i] =
          TestKeyBuilder.asSortKey(i + 1); // Note: this produces dense keys with origin ONE(1).
   ...
   * height, branching factor, #of nodes and #of leaves may differ (the test does not presume that
   * the btrees were built with the same branching factor, but merely with the same data and key
   * type).
   *
   * @param expected The ground truth btree.
   * @param actual The btree that is being validated.
   */
  public static void assertSameBTree(final AbstractBTree expected, final IIndex actual) {
    ...
    assertEquals(
        "indexUUID",
        expected.getIndexMetadata().getIndexUUID(),
     ...
     */
    {
      final long actualTupleCount =
          doEntryIteratorTest(expected.rangeIterator(), actual.rangeIterator());
      ...
      final long actualTupleCount =
          doEntryIteratorTest(
              expected.rangeIterator(
                  ...
                  null /* toKey */,
                  0 /* capacity */,
                  IRangeQuery.KEYS | IRangeQuery.VALS | IRangeQuery.REVERSE,
                  null /* filter */),
    ...
    if (log.isInfoEnabled()) log.info("Examining expected tree for inconsistencies");
    ...
    if (actual instanceof /*Abstract*/ BTree) {
      if (log.isInfoEnabled()) log.info("Examining actual tree for inconsistencies");
   ...
   * @param actual The iterator to be tested.
   ...
   */
  protected static long doEntryIteratorTest(
    ...
    while (expectedItr.hasNext()) {
       ...
       * Lazily generate message.
          ...
          fail(
              "Values differ: index="
                  ...
                  + BytesUtil.toString(expectedKey)
                  + ", expected="
                  + Arrays.toString(expectedVal)
                  + ", actual="
                  + Arrays.toString(actualVal),
              ex);
        }
      }
       ...
       * Lazily generate message.
        ...
        assertEquals(
            ...
            expectedTuple.getVersionTimestamp(),
            actualTuple.getVersionTimestamp());
      }
   ...
   * @param keys The keys in key order (out).
   * @param vals The values in key order (out).
   */
  public static void getKeysAndValues(
      final AbstractBTree btree, final byte[][] keys, final byte[][] vals) {
   ...
   * @param vals the values in key order.
   */
  public static void doRandomLookupTest(
      final String label, final IIndex btree, final byte[][] keys, final byte[][] vals) {
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info(label + " : tested " + nentries + " keys order in " + elapsed + "ms");
   ...
   * @param vals the values in key order.
   */
  public static void doRandomIndexOfTest(
      final String label, final AbstractBTree btree, final byte[][] keys, final byte[][] vals) {
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info(label + " : tested " + nentries + " keys in " + elapsed + "ms");
   ...
   */
  public static void assertSameIterator(byte[][] expected, ITupleIterator actual) {
   ...
   * the expected order. Errors are reported if too few or too many values are produced, etc.
   */
  public static void assertSameIterator(
      String msg, final byte[][] expected, final ITupleIterator actual) {
    ...
    while (actual.hasNext()) {
      ...
      if (i >= expected.length) {
        ...
        fail(msg + ": The iterator is willing to visit more than " + expected.length + " values.");
      }
    ...
    if (i < expected.length) {
      ...
      fail(
          msg
              + ": The iterator SHOULD have visited "
              + expected.length
              + " values, but only visited "
              + i
              + " values.");
    }
  }
   ...
   * index entries in the <i>actual</i> index are ignored.
   *
   * @param expected
   * @param actual
   */
   ...
   * comparison function.
   *
   * @param maxKeys The capacity of the array.
   * @param nkeys The #of keys to generate.
   * @return A byte[][] with nkeys non-null byte[] entries and a capacity of maxKeys.
   */
  public static byte[][] getRandomKeys(final int maxKeys, final int nkeys) {
    ...
    return new RandomKeysGenerator(r, maxKeys, maxKeyLength).generateKeys(nkeys);
   ...
   * data are presented in key order.
   *
   * <p>Note: This method MAY return entries having duplicate keys.
   *
   * @param N The #of key-value pairs to generate.
   ...
   * @todo parameter for random deletes, in which case we need to reframe the batch operation since
   *     a batch insert won't work. Perhaps a BatchWrite would be the thing.
