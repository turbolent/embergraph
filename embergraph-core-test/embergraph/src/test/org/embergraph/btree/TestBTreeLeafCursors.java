 ...
 * BTree} depth of 2 (a root node, two nodes beneath that, and then a layer of leaves under those
 * nodes) in order to test the recursive handling of the node stack by prior() and next(). {@link
 * #getProblem1()} is used for this purpose.
 *
 * <p>Note: The leaves are verified by comparing the first key in the leaf against the expected key
 * for that position of that leaf.
 *
...
public class TestBTreeLeafCursors extends TestCase2 {
  ...
  public void test_seek() {
    ...
    assertEquals(
        TestKeyBuilder.asSortKey(1), cursor.seek(TestKeyBuilder.asSortKey(1)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(1), cursor.seek(TestKeyBuilder.asSortKey(2)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(3), cursor.seek(TestKeyBuilder.asSortKey(3)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(3), cursor.seek(TestKeyBuilder.asSortKey(4)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(5), cursor.seek(TestKeyBuilder.asSortKey(5)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(5), cursor.seek(TestKeyBuilder.asSortKey(6)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(7), cursor.seek(TestKeyBuilder.asSortKey(7)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(7), cursor.seek(TestKeyBuilder.asSortKey(8)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(9), cursor.seek(TestKeyBuilder.asSortKey(9)).getKeys().get(0));

    assertEquals(
        TestKeyBuilder.asSortKey(9), cursor.seek(TestKeyBuilder.asSortKey(10)).getKeys().get(0));

    // verify seek to key that would be in the last leaf but is not actually in the B+Tree.
    assertEquals(
        TestKeyBuilder.asSortKey(9), cursor.seek(TestKeyBuilder.asSortKey(12)).getKeys().get(0));
  }
   ...
   * [1:10]. The values are {@link String}s objects formed using "v+"+i, where i is the integer from
   * which the key was formed.
   *
   * @return The btree.
