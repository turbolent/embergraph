...
public class TestBloomFilter extends TestCase2 {
  ...
  public void test_errorRate() {
    ...
    // inputs.
    System.err.println("targets: nkeys=" + nkeys + ", errorRate=" + errorRate);
...

    {
      ...
      // outputs.
      System.err.println(
          "error rate on keys added to the filter"
              + ": #errors="
              + falsePositiveCount
              + ", actual error rate="
     ...
     */
    {
      ...
      // outputs.
      System.err.println(
          "error rate on random distinct keys"
              + ": #errors="
              + falsePositiveCount2
              + ", actual error rate="
   ...
   *
   * <p>Note: The table has spans within each column where the same bit length is computed. This is
   * because the #of hash functions is discrete. Each time we increase the #of hash functions the
   * table moves to a new bit length value in a given column.
   */
  public void test_spaceCurve() {
    ...
    final int[] capacity =
        new int[] {1, 100, 1000, 10000, 100000, 1000000, 2000000, 10000000, 20000000, 100000000};
   ...
   * and <code>.20</code> by increments of <code>.01</code> showing the #of index entries for which
   * that would be the expected error rate.
   *
   * <p>You can verify the computation by examining certain entries in the table generated by {@link
   * #test_spaceCurve()}. The filter as configured for this test has a bit length of <code>8,656,171
   * </code>. If you look at the expected error rate for a bloom filter of 2 million index entries
   * of the same bit length you will see that it lies somewhere in <code>0.13 - 0.24</code> (this
   * table has discrete regions, see the javadoc for {@link #test_spaceCurve()}). If you examine the
   * output of this test, you will see that an error rate of <code>0.18</code> is predicated when
   * the #of index entries is on the order of 2 million (2,008,182). The relevant cells are
