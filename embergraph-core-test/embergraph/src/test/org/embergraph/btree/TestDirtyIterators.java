 ...
 * @see TestIterators, which handles iterators that do not differentiate between clear and dirty
 *     nodes.
...
public class TestDirtyIterators extends AbstractBTreeTestCase {
   ...
   * which throws an exception if the tree attempts a node eviction.
   */
  public void test_dirtyChildIterator01() {
    ...
    assertSameIterator(new IAbstractNode[] {a, b}, ((Node) btree.root).childIterator(false));
    assertSameIterator(new IAbstractNode[] {a, b}, ((Node) btree.root).childIterator(true));
    ...
    // verify visiting all children.
    assertSameIterator(new IAbstractNode[] {a, d, b}, ((Node) btree.root).childIterator(false));
    assertSameIterator(new IAbstractNode[] {a, d, b}, ((Node) btree.root).childIterator(true));
   ...
   * written onto the store. Note that this does not force the eviction of nodes or leaves but
   * rather requests that the are written out directly. Whenever we make an immutable node or leaf
   * mutable using copy-on-write, we wind up with a new reference for that node or leaf and update
   * the variables in the test appropriately.
   */
  public void test_dirtyChildIterator02() {
    ...
    assertSameIterator(new IAbstractNode[] {d, b}, ((Node) btree.root).childIterator(true));
    ...
    assertSameIterator(new IAbstractNode[] {d}, ((Node) btree.root).childIterator(true));

    // write (d) onto the store and verify that it is no longer visited.
    btree.writeNodeOrLeaf(d);
    assertFalse(d.isDirty());
    assertTrue(d.isPersistent());
    assertSameIterator(new IAbstractNode[] {}, ((Node) btree.root).childIterator(true));
    ...
    assertSameIterator(new IAbstractNode[] {a1}, ((Node) btree.root).childIterator(true));
    ...
    // verify visiting dirty children.
    assertSameIterator(new IAbstractNode[] {a1, b1}, ((Node) btree.root).childIterator(true));
    ...
    // verify visiting dirty children.
    assertSameIterator(new IAbstractNode[] {a2}, ((Node) btree.root).childIterator(true));
  ...
  public void test_dirtyPostOrderIterator01() {
    ...
    // verify iterator.
    assertSameIterator(new IAbstractNode[] {a, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {a, b, c}, btree.root.postOrderNodeIterator(true));
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {a, d, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {a, d, b, c}, btree.root.postOrderNodeIterator(true));
    ...
    // verify iterator
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, b, f, g}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, b, f, g}, btree.root.postOrderNodeIterator(true));
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {d, e, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {d, e, b, c}, btree.root.postOrderNodeIterator(true));
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {d, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {d, b, c}, btree.root.postOrderNodeIterator(true));
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {b}, btree.root.postOrderNodeIterator(false));
   ...
   * or leaf makes it immutable. In order to make the node or leaf dirty again we have to modify it,
   * which triggers copy-on-write. Copy on write propagates up from the leaf where we make the
   * mutation and causes any immutable parents to be cloned as well. Nodes and leaves that have been
   * cloned by copy-on-write are distinct objects from their immutable predecessors.
   */
  public void test_dirtyPostOrderIterator02() {
    ...
    assertSameIterator(new IAbstractNode[] {btree.root}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {}, btree.root.postOrderNodeIterator(true));
    ...
    assertSameIterator(new IAbstractNode[] {a, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {b, c}, btree.root.postOrderNodeIterator(true));
    ...
    assertFalse(c.isPersistent());
    assertSameIterator(new IAbstractNode[] {a, d, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {a, d, c}, btree.root.postOrderNodeIterator(true));
    ...
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, b, f, g}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, f, g}, btree.root.postOrderNodeIterator(true));
    ...
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, b, f, g}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {e, f, g}, btree.root.postOrderNodeIterator(true));
    ...
    assertSameIterator(
        new IAbstractNode[] {a, d, c, e, b, f, g}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {f, g}, btree.root.postOrderNodeIterator(true));
    ...
    // verify iterator
    assertSameIterator(new IAbstractNode[] {d, e, b, c}, btree.root.postOrderNodeIterator(false));
    assertSameIterator(new IAbstractNode[] {d, c}, btree.root.postOrderNodeIterator(true));
