...
public abstract class AbstractNodeOrLeafDataRecordTestCase extends AbstractBTreeTestCase {
   ...
   */
  protected final ILeafData mockLeafFactory(final IRaba keys, final IRaba vals) {
   ...
   * @param keys The keys.
   * @param vals The values.
   * @param deleteMarkers The delete markers (optional).
   * @param versionTimestamps The version timestamps (optional).
   * @param rawRecords The bit flags indicating which tuples are have their value stored as a raw
   *     record on the backing persistence store (optional).
   * @return The mock {@link ILeafData} object.
   */
  protected ILeafData mockLeafFactory(
    ...
    return new MockLeafData(keys, vals, deleteMarkers, versionTimestamps, rawRecords);
  }
   ...
   * combined with a large #of nodes.
   *
   * @param ntrials The #of trials. Each trial has a random slotSize and branchingFactor. 50% of the
   *     trials (on average) will use record compression.
   * @param nnodes The #of random nodes per trial.
   */
  public void doStressTest(final int ntrials, final int nnodes) {
    ...
    for (int trial = 0; trial < ntrials; trial++) {
      ...
      for (int i = 0; i < nnodes; i++) {
        ...
        final IAbstractNodeData expected =
            getRandomNodeOrLeaf(branchingFactor, deleteMarkers, versionTimestamps, rawRecords);
   ...
   * @param buf
   */
  protected void doRoundTripTest(
      final IAbstractNodeData expected,
      final IAbstractNodeDataCoder<?> coder,
    ...
    if (expected.isLeaf()) {
      ...
      final ILeafData liveCodedLeaf =
          ((IAbstractNodeDataCoder<ILeafData>) coder)
              .encodeLive((ILeafData) expected, new DataOutputBuffer());

      final AbstractFixedByteArrayBuffer liveCodedData = liveCodedLeaf.data();

      assertSameLeafData((ILeafData) expected, liveCodedLeaf);

      // encode
      final AbstractFixedByteArrayBuffer originalData =
          ((IAbstractNodeDataCoder<ILeafData>) coder).encode((ILeafData) expected, buf);
      ...
      // Verify we can decode the record.
      {
        ...
        // decode.
        final ILeafData actual = ((IAbstractNodeDataCoder<ILeafData>) coder).decode(originalData);
      ...
      // returns a slice which has the same data.
      {
        ...
        final int off = 10;
        final DataOutputBuffer out = new DataOutputBuffer(off, new byte[1000 + off]);

        // encode onto that buffer.
        final AbstractFixedByteArrayBuffer slice =
            ((IAbstractNodeDataCoder<ILeafData>) coder).encode((ILeafData) expected, out);
      ...
      // non-zero offset
      {
        ...
        // decode the slice.
        final ILeafData actual = ((IAbstractNodeDataCoder<ILeafData>) coder).decode(slice);
      ...
      // Test the live coded path (returns coded instance for immediate use).
      final INodeData liveCodedNode =
          ((IAbstractNodeDataCoder<INodeData>) coder)
              .encodeLive((INodeData) expected, new DataOutputBuffer());

      final AbstractFixedByteArrayBuffer liveCodedData = liveCodedNode.data();

      assertSameNodeData((INodeData) expected, liveCodedNode);

      // encode
      final AbstractFixedByteArrayBuffer originalData =
          ((IAbstractNodeDataCoder<INodeData>) coder).encode((INodeData) expected, buf);

      // Verify we can decode the record.
      {

        // decode
        final INodeData actual = ((IAbstractNodeDataCoder<INodeData>) coder).decode(originalData);
      ...
      // returns a slice which has the same data.
      {
        ...
        // encode onto that buffer.
        final AbstractFixedByteArrayBuffer slice =
            ((IAbstractNodeDataCoder<INodeData>) coder).encode((INodeData) expected, out);
      ...
      // non-zero offset
      {
        ...
        // decode the slice.
        final INodeData actual = ((IAbstractNodeDataCoder<INodeData>) coder).decode(slice);
  ...
  public INodeData getRandomNode(final int m) {
    ...
    final int nchildren = r.nextInt((branchingFactor + 1) / 2) + (branchingFactor + 1) / 2;
    ...
    final long minimumVersionTimestamp = hasVersionTimestamp ? System.currentTimeMillis() : 0L;

    final long maximumVersionTimestamp =
        hasVersionTimestamp ? System.currentTimeMillis() + r.nextInt() : 0L;
  ...
  public ILeafData getRandomLeaf(
    ...
    final int nkeys = r.nextInt((branchingFactor + 1) / 2) + (branchingFactor + 1) / 2;
    ...
    final boolean[] deleteMarkers = isDeleteMarkers ? new boolean[branchingFactor + 1] : null;

    final long[] versionTimestamps = isVersionTimestamps ? new long[branchingFactor + 1] : null;

    final boolean[] rawRecords = isRawRecords ? new boolean[branchingFactor + 1] : null;
    ...
    for (int i = 0; i < nkeys; i++) {
      ...
      if (deleted) {
        ...
        final boolean isRawRecord = rawRecords != null && r.nextDouble() < .3;

        // Not a deleted tuple.
        if (isRawRecord) {

          // Raw record on the backing store.
          values[i] = AbstractBTree.encodeRecordAddr(recordAddrBuf, nextAddr());
          ...
          if (rawRecords != null) rawRecords[i] = false;
        }
      }

      if (versionTimestamps != null) {

        versionTimestamps[i] = System.currentTimeMillis() + r.nextInt(10000);
      }
    }
  ...
  public IAbstractNodeData getRandomNodeOrLeaf(
      final int m,
      final boolean deleteMarkers,
    ...
    if (!mayGenerateLeaves()) {
      ...
      return getRandomLeaf(m, deleteMarkers, versionTimestamps, rawRecords);
  ...
  public void test_nodeOrLeafFlag() {
    ...
    // isLeaf()
    assertTrue(AbstractReadOnlyNodeData.isLeaf(AbstractReadOnlyNodeData.LEAF));

    assertTrue(AbstractReadOnlyNodeData.isLeaf(AbstractReadOnlyNodeData.LINKED_LEAF));

    assertFalse(AbstractReadOnlyNodeData.isLeaf(AbstractReadOnlyNodeData.NODE));

    // isNode()
    assertFalse(AbstractReadOnlyNodeData.isNode(AbstractReadOnlyNodeData.LEAF));

    assertFalse(AbstractReadOnlyNodeData.isNode(AbstractReadOnlyNodeData.LINKED_LEAF));

    assertTrue(AbstractReadOnlyNodeData.isNode(AbstractReadOnlyNodeData.NODE));
  }
