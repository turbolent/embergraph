...
public class TestPrefixFilter extends TestCase2 {
   ...
   * included in the prefix key even if they are present in the keys in the index.
   */
  ...
  /** Used to form the keys for the index and to (de-)serialize the values stored in the index. */
  final ITupleSerializer tupleSer = DefaultTupleSerializer.newInstance();

  protected ITuple<String> newTestTuple(String s) {

    return new TestTuple<String>(
  ...
  protected byte[] asPrefixKey(Object key) {
    ...
    if (log.isInfoEnabled()) log.info("key=" + key + ", byte[]=" + BytesUtil.toString(b));
...

    {
      final ITupleIterator<String> itr =
          btree.rangeIterator(
              ...
              0 /* capacity */,
              IRangeQuery.DEFAULT | IRangeQuery.CURSOR,
              new PrefixFilter<String>(asPrefixKey("Mike")));
      ...
      AbstractTupleCursorTestCase.assertEquals(newTestTuple("Mike"), itr.next());

      assertTrue(itr.hasNext());

      AbstractTupleCursorTestCase.assertEquals(newTestTuple("Mike Personick"), itr.next());
...

    {
      final ITupleIterator<String> itr =
          btree.rangeIterator(
              ...
              0 /* capacity */,
              IRangeQuery.DEFAULT | IRangeQuery.CURSOR,
              new PrefixFilter<String>(asPrefixKey("Bryan")));
      ...
      AbstractTupleCursorTestCase.assertEquals(newTestTuple("Bryan"), itr.next());

      assertTrue(itr.hasNext());

      AbstractTupleCursorTestCase.assertEquals(newTestTuple("Bryan Thompson"), itr.next());
...

    {
      final ITupleIterator<String> itr =
          btree.rangeIterator(
              ...
              0 /* capacity */,
              IRangeQuery.DEFAULT | IRangeQuery.CURSOR,
              new PrefixFilter<String>(asPrefixKey("Mi")));
      ...
      AbstractTupleCursorTestCase.assertEquals(newTestTuple("Michael"), itr.next());

      assertTrue(itr.hasNext());

      AbstractTupleCursorTestCase.assertEquals(newTestTuple("Michael Personick"), itr.next());
    ...
    // nothing matches.
    {
      final ITupleIterator<String> itr =
          btree.rangeIterator(
              ...
              0 /* capacity */,
              IRangeQuery.DEFAULT | IRangeQuery.CURSOR,
              new PrefixFilter<String>(asPrefixKey("Ma")));
...

    {
      final ITupleIterator<String> itr =
          btree.rangeIterator(
              ...
              0 /* capacity */,
              IRangeQuery.DEFAULT | IRangeQuery.CURSOR,
              new PrefixFilter<String>(new byte[][] {asPrefixKey("Bryan"), asPrefixKey("Mike")}));
