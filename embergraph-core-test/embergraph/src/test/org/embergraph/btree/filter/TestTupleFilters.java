 ...
 *     testing with the federation generally requires additional effort in order to look for fence
 *     posts around chunk and partition boundaries.
...
public class TestTupleFilters extends AbstractBTreeTestCase {
  ...
  /** Test of {@link IRangeQuery#REMOVEALL}. The state of the index is verified afterwards. */
  ...
  /**
   * Test of {@link IRangeQuery#REMOVEALL} using a {@link TupleFilter}. Only the even keys are
   * deleted. The state of the index is verified afterwards.
   */
  public void test_removeAll_with_TupleFilter() {
     ...
     */
    {
      ...
      while (itr.hasNext()) {
        ...
        if (log.isInfoEnabled()) log.info("visiting: " + tuple);
