...
public class TestTransientBTree extends AbstractBTreeTestCase {
  ...
  /** Verifies that closing a transient {@link BTree} is allowed and that all data is discarded. */
  ...
  public void test_eviction() {
    ...
    final int writeRetentionQueueCapacity = btree.writeRetentionQueue.capacity();

    if (log.isInfoEnabled()) log.info(btree.toString());
   ...
   * factor to force a large #of nodes and leaves to be created. A traversal is then performed of
   * the nodes and leaves and all of their references are placed into a weak value collection. It
   * then removes all entries in a key range, which should cause some leaves (and perhaps some
   * nodes) to become weakly reachable. Finally, it forces a large number of object allocations in
   * order to prompt a GC that will clear those weak references. The weak reference collection is
   * then scanned to verify that its size has been decreased.
   *
   * <p>Note: This test is of necessity subject to the whims of the garbage collector. If it fails,
   * try increasing some of the constants in the test and see if that will provoke a GC that will
  ...
  public void test_delete() {
        ...
        new LinkedList<WeakReference<AbstractNode>>();
    {
      ...
      if (log.isInfoEnabled()) log.info("There are " + refs.size() + " nodes in the btree");

      if (log.isInfoEnabled()) log.info("after inserting keys: " + btree.toString());
     ...
     */
    {
      final ITupleIterator itr =
          btree.rangeIterator(
              TestKeyBuilder.asSortKey(10000L),
              TestKeyBuilder.asSortKey(20000L),
              0 /* capacity */,
      ...
      if (log.isInfoEnabled()) log.info("after deleting key range: " + btree.toString());

      assertTrue(btree.getNodeCount() + btree.getLeafCount() < refs.size());
    }
    ...
    for (int x = 0; x < limit; x++) {
      ...
      if (log.isInfoEnabled())
        log.info("pass " + x + "of " + limit + ": #of cleared references=" + n);
   ...
   * IRawStore}.
   *
   * @todo since the code is identical other than allocating the {@link BTree} , factor out a
   *     doDeleteTest(BTree) method.
   */
