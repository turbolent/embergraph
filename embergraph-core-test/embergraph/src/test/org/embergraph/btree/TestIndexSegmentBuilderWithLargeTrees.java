...
public class TestIndexSegmentBuilderWithLargeTrees extends AbstractIndexSegmentTestCase {
   ...
   * on the returned {@link BTree}. If you do not close and destroy the backing journal then the
   * thread pool will grow during the test runs.
   *
...
// * Test exercises a known problem case.
   ...
   *
   * <p>Note: Regardless of the branching factor in the source btree, the same {@link IndexSegment}
   * should be built for a given set of entries (key-value pairs) and a given output branching
   * factor for the {@link IndexSegment}. However, input trees of different heights also stress
   ...
   * each {@link IndexSegment}, we then compare it against its source {@link BTree} for the same
   * total ordering.
   */
  public void test_randomDenseKeys() throws Exception {
    ...
    for (int i = 0; i < branchingFactors.length; i++) {
      ...
      //            doBuildIndexSegmentAndCompare( doSplitWithRandomDenseKeySequence( getBTree(m),
      // m, m*m*m*m ) );
   ...
   * IndexSegment}, we then compare it against its source {@link BTree} for the same total ordering.
   */
  public void test_randomSparseKeys() throws Exception {
    ...
    for (int i = 0; i < branchingFactors.length; i++) {
      ...
      doBuildIndexSegmentAndCompare(doInsertRandomSparseKeySequenceTest(getBTree(m), m * m, trace));
...
//     * This stress test is designed to expose problems when most or all of the
    ...
    try {
      ...
      for (int i = 0; i < branchingFactors.length; i++) {
        ...
        for (BuildEnum buildEnum : BuildEnum.values()) {
          ...
          doBuildIndexSegmentAndCompare(getName(), btree, m, buildEnum, true /* bufferNodes */);

          doBuildIndexSegmentAndCompare(getName(), btree, m, buildEnum, false /* bufferNodes */);
        }
      ...
      if (log.isInfoEnabled()) log.info("Closed journal.");
    }
  }
   ...
   *
   * @param prefix A file name prefix. The actual file name is formed using the branching factor and
   *     {@link BuildEnum} as well.
   * @param btree The source {@link BTree}.
   * @param m The output branching factor for the build.
   * @param buildEnum The algorithm to be used.
   * @return The builder, which has metadata about the operation.
   * @throws Exception
   */
  private static IndexSegmentBuilder doBuildIndexSegmentAndCompare(
      final String prefix,
      final BTree btree,
      final int m,
    ...
    final IndexSegmentBuilder builder =
        doBuildIndexSegment(prefix, btree, m, buildEnum, bufferNodes);

    if (log.isInfoEnabled()) log.info("Opening index segment.");
   ...
   * @throws Exception
   */
  static IndexSegmentBuilder doBuildIndexSegment(
      final String prefix,
      final BTree btree,
      final int m,
      final BuildEnum buildEnum,
    ...
    final File outFile = new File(prefix + "_m" + m + "_" + buildEnum + ".seg");
   ...
   */
  private static void doIndexSegmentCompare(final BTree btree, final IndexSegment seg) {

    if (log.isInfoEnabled()) log.info("Verifying index segment: " + seg.getStore().getFile());
    ...
    try {
       ...
       * Verify the total index order.
       */
      if (log.isInfoEnabled()) log.info("Verifying index segment.");
      assertSameBTree(btree, seg);

      if (log.isInfoEnabled()) log.info("Closing index segment.");
      seg.close();
       ...
       */
      if (log.isInfoEnabled()) log.info("Re-verifying index segment.");
      assertSameBTree(btree, seg);

    } finally {
      // Close again so that we can delete the backing file.
      if (log.isInfoEnabled()) log.info("Re-closing index segment.");
      seg.close();
    }
  }
...
//     * Test helper builds an index segment from the btree using several
  ...
  //         */
  //        assert !btree.getIndexMetadata().getDeleteMarkers() : "The source BTree must not
  // maintain delete markers for this unit test.";
  //
...
//     * Driver for comparison of index segment build performance using different
  ...
  //            }
  //
  //            final String header = "algorithm, elapsed(ms), build(ms), write(ms), mbPerSec,
  // size(mb)";
  //
  ...
  //                        btree, m, buildEnum);
  //
  //                // The total elapsed build time, including range count or pre-materialization of
  // tuples.
