...
/*
 * Test the quorum semantics for a highly available quorum of 3 services. The main points to test
 * here are the particulars of events not observable with a singleton quorum, including that all
 * events are perceived by all clients via the watcher for their quorum, that a service join which
 * does not trigger a quorum meet, that a service leave which does not trigger a quorum break, a
 * leader leave, etc.
 *
 * <p>These conditions arise because the quorum can meet with 2 out of 3 services forming a
 * consensus. Even if the third service has the same lastCommitTime, if it votes after the other
 * services, then it can be in the position of joining a quorum which has already met. This leads
 * naturally into an exploration of the synchronization protocol for the persistent state of the
 * services when joining a met quorum.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 *     <p>FIXME We must also test with k:=5 in order to see some situations which do not appear with
 *     k:=3 (not sure which ones off hand, but I remember noting that there are some).
 *     <p>FIXME There will need to be unit tests for synchronization when a service wants to join a
 *     met quorum. Persistent services need to verify their root blocks in detail against the
 *     leader. If they are not identical, then the services need to synchronize rather than join.
 *     Also, services which attempt to join an already met quorum must synchronize before they can
 *     join. All of this needs to be tested, but we need to do those tests with live journals.
 *     <p>FIXME There will need to be unit tests for hot spares.
 */
public class TestHA3QuorumSemantics extends AbstractQuorumTestCase {
  ...
  /*
   * Unit test for write pipeline add/remove, including the {@link PipelineState} of the downstream
   * service as maintained by the {@link MockQuorumMember}.
   *
   * @throws InterruptedException
   */
  public void test_pipelineAddRemove3() throws InterruptedException {
    ...
    assertCondition(
        () -> assertEquals(new UUID[] {serviceId0, serviceId1, serviceId2}, quorum0.getMembers()));
    ...
    assertCondition(
        () -> {
          // test prior/next.
          assertEquals(
              new UUID[] {null, serviceId1}, quorum0.getPipelinePriorAndNext(serviceId0));
          assertEquals(
              new UUID[] {serviceId0, serviceId2}, quorum0.getPipelinePriorAndNext(serviceId1));
          assertEquals(
              new UUID[] {serviceId1, null}, quorum0.getPipelinePriorAndNext(serviceId2));
        });
  ...
  public void test_voting3() throws InterruptedException {
    ...
    assertEquals(new UUID[] {serviceId0, serviceId2}, quorum0.getVotes().get(lastCommitTime1));
    assertEquals(new UUID[] {serviceId1}, quorum0.getVotes().get(lastCommitTime2));
    ...
    assertEquals(new UUID[] {serviceId0, serviceId2}, quorum1.getVotes().get(lastCommitTime1));
    ...
    assertCondition(
        () -> {
          ...
          assertEquals(toString(votes0), 0, votes0.size());
          assertEquals(toString(votes1), 0, votes1.size());
          assertEquals(toString(votes2), 0, votes2.size());
  ...
  /*
   * Unit test for service join/leave where services vote in the pipeline order so the leader does
   * not need to reorganize the pipeline when the number of joined services reaches (k+1)/2. The
   * quorum meets after two services cast their vote. When the third service casts its vote, it
   * joins the met quorum.
   *
   * @throws InterruptedException
   */
    ...
    final long token1;
    {
      ...
      assertCondition(
          () -> {
            ...
            assertEquals(
                new UUID[] {serviceId0, serviceId1}, quorum0.getVotes().get(lastCommitTime));
             ...
             * votes.
             */
            assertEquals(new UUID[] {serviceId0, serviceId1}, quorum0.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId1}, quorum1.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId1}, quorum2.getJoined());
     ...
     */
    {
      ...
      assertCondition(
          () -> {
            ...
            // verify the vote order.
            assertEquals(
                new UUID[] {serviceId0, serviceId1, serviceId2},
            ...
            assertEquals(new UUID[] {serviceId0, serviceId1, serviceId2}, quorum0.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId1, serviceId2}, quorum1.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId1, serviceId2}, quorum2.getJoined());
          });
     ...
     */
    {
      ...
      assertCondition(
          () -> {
            ...
            // verify the vote order.
            assertEquals(
                new UUID[] {serviceId0, serviceId2}, quorum0.getVotes().get(lastCommitTime));
             ...
             * votes.
             */
            assertEquals(new UUID[] {serviceId0, serviceId2}, quorum0.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId2}, quorum1.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId2}, quorum2.getJoined());
          });
      ...
      assertCondition(
          () -> {
            ...
            // verify the vote order.
            assertEquals(
                new UUID[] {serviceId0, serviceId2, serviceId1},
                quorum0.getVotes().get(lastCommitTime));
            assertEquals(
                new UUID[] {serviceId0, serviceId2, serviceId1},
                quorum1.getVotes().get(lastCommitTime));
            assertEquals(
                new UUID[] {serviceId0, serviceId2, serviceId1},
                quorum2.getVotes().get(lastCommitTime));
            ...
            // Service join in the same order in which they cast their
            // votes.
            assertEquals(new UUID[] {serviceId0, serviceId2, serviceId1}, quorum0.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId2, serviceId1}, quorum1.getJoined());
            assertEquals(new UUID[] {serviceId0, serviceId2, serviceId1}, quorum2.getJoined());
          });
      ...
      assertCondition(
          () -> {
            assertEquals(new UUID[] {serviceId0, serviceId2, serviceId1}, quorum0.getPipeline());
            assertEquals(new UUID[] {serviceId0, serviceId2, serviceId1}, quorum1.getPipeline());
            assertEquals(new UUID[] {serviceId0, serviceId2, serviceId1}, quorum2.getPipeline());
          });
    }
      ...
      assertCondition(
          () -> {
            ...
            assertEquals(
                new UUID[] {serviceId0, serviceId1}, quorum0.getVotes().get(lastCommitTime));
            assertEquals(
                new UUID[] {serviceId0, serviceId1}, quorum1.getVotes().get(lastCommitTime));
            assertEquals(
                new UUID[] {serviceId0, serviceId1}, quorum2.getVotes().get(lastCommitTime));
            // verify the consensus was NOT updated.
            assertEquals(lastCommitTime, client0.lastConsensusValue);
            assertEquals(lastCommitTime, client1.lastConsensusValue);
            assertEquals(lastCommitTime, client2.lastConsensusValue);
  ...
  /*
   * Unit tests for pipeline reorganization when the leader is elected. This tests the automatic
   * reorganization of the pipeline order where the service which will become the leader is not at
   * the head of the pipeline. When the leader is elected, the leader must cause the pipeline to be
   * reorganized such that the leader is at the head of the pipeline. In general, we can not control
   * the pipeline order imposed by the leader when it reorganizes the pipeline and the leader that
   * was aware of the network topology could chose to reorganize the pipeline in order to optimize
   * it even though the leader was already at the head of the pipeline.
   *
   * @throws InterruptedException
   */
  public void test_pipelineReorganization() throws InterruptedException {
    ...
    assertCondition(
        () -> {
          assertEquals(new UUID[] {serviceId1, serviceId0}, quorum0.getPipeline());
          assertEquals(new UUID[] {serviceId1, serviceId0}, quorum1.getPipeline());
          assertEquals(new UUID[] {serviceId1, serviceId0}, quorum2.getPipeline());
        });
  ...
  /*
   * Variations on standard pipelineReorganization to test conditions that occasionally fail in
   * TestHA3
   *
   * @throws InterruptedException
   */
    ...
    final long token1;
    {
      ...
      assertCondition(
          () -> {
            ...
            // verify the vote order.
            log.warn("expected: " + Arrays.toString(new UUID[] {serviceId1, serviceId0}));
            log.warn("actual  : " + Arrays.toString(quorum0.getVotes().get(lastCommitTime)));

            assertEquals(
                new UUID[] {serviceId1, serviceId0}, quorum0.getVotes().get(lastCommitTime));
            assertEquals(
                new UUID[] {serviceId1, serviceId0}, quorum1.getVotes().get(lastCommitTime));
            assertEquals(
                new UUID[] {serviceId1, serviceId0}, quorum2.getVotes().get(lastCommitTime));
             ...
             * votes.
             */
            assertEquals(new UUID[] {serviceId1, serviceId0}, quorum0.getJoined());
            assertEquals(new UUID[] {serviceId1, serviceId0}, quorum1.getJoined());
            assertEquals(new UUID[] {serviceId1, serviceId0}, quorum2.getJoined());

            // The leader is now at the front of the pipeline, with
            // service2 moved to end.
            assertEquals(new UUID[] {serviceId1, serviceId0, serviceId2}, quorum0.getPipeline());
            assertEquals(new UUID[] {serviceId1, serviceId0, serviceId2}, quorum1.getPipeline());
            assertEquals(new UUID[] {serviceId1, serviceId0, serviceId2}, quorum2.getPipeline());
          });
      ...
      actor2.castVote(lastCommitTime);
      assertEquals(
          quorum1.getCastVoteIfConsensus(serviceId1), quorum1.getCastVoteIfConsensus(serviceId2));
      ...
      assertEquals(new UUID[] {serviceId1, serviceId0, serviceId2}, quorum0.getJoined());
      assertEquals(new UUID[] {serviceId1, serviceId0, serviceId2}, quorum1.getJoined());
      assertEquals(new UUID[] {serviceId1, serviceId0, serviceId2}, quorum2.getJoined());
    }
  }
}
