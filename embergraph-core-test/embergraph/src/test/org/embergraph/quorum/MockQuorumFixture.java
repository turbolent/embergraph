 ...
 * the zookeeper behavior and ensures that each watcher sees the events in a total order. Zookeeper
 * promises that all watchers proceed at the same rate. We enforce that with a {@link
 * #globalSynchronousLock}. Once all watchers have drained the event, the next event is made
 * available to the watchers.
 *
 * <p>The fixture only generates events for actual state changes. This also mimics the behavior of
 * zookeeper.
 *
...
public class MockQuorumFixture {
   ...
   * @return The {@link QuorumMember} -or- <code>null</code> if there is none known for that
   *     serviceId.
   */
   ...
   * @return The service.
   * @throws QuorumException if there is no known {@link QuorumMember} for that serviceId.
   */
  ...
  public synchronized void start() {
    ...
    executorService = Executors.newCachedThreadPool(new DaemonThreadFactory("executorService"));

    dispatchService =
        Executors.newSingleThreadScheduledExecutor(new DaemonThreadFactory("dispatchService"));
  ...
  private class DispatcherTask implements Runnable {
    ...
    private void runOnce() throws Throwable {
      ...
      for (MockQuorumWatcher watcher : listeners) {
        ...
        try {
          if (log.isInfoEnabled()) log.info("Queuing event: " + e + " on listener#" + i);
   ...
   * </pre>
   *
   * can throw an {@link QuorumException} since the actor's local quorum state in all likelihood
   * will not have been updated before we attempt to add the actor to the pipeline in the
   * distributed quorum state.
   *
   * <p>However, the following sequence will succeed.
   *
   * <pre>
   * actor.memberAdd();
   * fixture.awaitDeque();
   ...
   * @deprecated The semantics of the {@link QuorumActor} have been modified to require it to block
   *     until the requested change has been observed by the {@link QuorumWatcher} and made visible
   *     in the local model of the quorum state maintained by the {@link AbstractQuorum}.
   ...
   * duplicate events arising from duplicate requests.
   *
   * @param e The event.
   */
  ...
  private void memberAdd(final UUID serviceId) {
    ...
    try {
      if (members.add(serviceId)) {
        ...
        accept(new AbstractQuorum.E(QuorumEventEnum.MEMBER_ADD, lastValidToken, token, serviceId));
      }
    } finally {
      lock.unlock();
    }
  ...
  private void memberRemove(final UUID serviceId) {
    ...
    try {
      if (members.remove(serviceId)) {
        if (log.isDebugEnabled()) log.debug("serviceId=" + serviceId);
        accept(
            new AbstractQuorum.E(QuorumEventEnum.MEMBER_REMOVE, lastValidToken, token, serviceId));
      }
    } finally {
      lock.unlock();
    }
      ...
      if (tmp.add(serviceId)) {
        ...
        accept(
            new AbstractQuorum.E(
                QuorumEventEnum.CAST_VOTE, lastValidToken, token, serviceId, lastCommitTime));
      }
    } finally {
      lock.unlock();
    }
    ...
    try {
      ...
      while (itr.hasNext()) {
        ...
        if (votes.remove(serviceId)) {
          ...
          accept(
              new AbstractQuorum.E(
                  QuorumEventEnum.WITHDRAW_VOTE, lastValidToken, token, serviceId));
          break;
        }
      }
    } finally {
  ...
  private void pipelineAdd(final UUID serviceId) {
    ...
    try {
      if (pipeline.add(serviceId)) {
        if (log.isDebugEnabled()) log.debug("serviceId=" + serviceId);
        accept(
            new AbstractQuorum.E(QuorumEventEnum.PIPELINE_ADD, lastValidToken, token, serviceId));
      }
    } finally {
      lock.unlock();
    }
  ...
  private void pipelineRemove(final UUID serviceId) {
    ...
    try {
      if (pipeline.remove(serviceId)) {
        ...
        accept(
            new AbstractQuorum.E(
                QuorumEventEnum.PIPELINE_REMOVE, lastValidToken, token, serviceId));
      }
    } finally {
      lock.unlock();
    }
  ...
  private void serviceJoin(final UUID serviceId) {
    ...
    try {
      if (joined.add(serviceId)) {
        if (log.isDebugEnabled()) log.debug("serviceId=" + serviceId);
        accept(
            new AbstractQuorum.E(QuorumEventEnum.SERVICE_JOIN, lastValidToken, token, serviceId));
      }
    } finally {
      lock.unlock();
    }
  ...
  private void serviceLeave(final UUID serviceId) {
    ...
    try {
      if (joined.remove(serviceId)) {
        if (log.isDebugEnabled()) log.debug("serviceId=" + serviceId);
        accept(
            new AbstractQuorum.E(QuorumEventEnum.SERVICE_LEAVE, lastValidToken, token, serviceId));
      }
    } finally {
      lock.unlock();
    }
  ...
  private void setToken(final long newToken) {
    ...
    try {
      if (lastValidToken != newToken) {
        ...
        if (log.isDebugEnabled()) log.debug("newToken=" + newToken);
        accept(
            new AbstractQuorum.E(
                QuorumEventEnum.QUORUM_MEET, lastValidToken, token, null /* serviceId */));
      }
    } finally {
      lock.unlock();
    }
  ...
  private void clearToken() {
    ...
    try {
      if (token != Quorum.NO_QUORUM) {
        ...
        accept(
            new AbstractQuorum.E(
                QuorumEventEnum.QUORUM_BROKE, lastValidToken, token, null /* serviceId */));
      }
    } finally {
      lock.unlock();
    }
   ...
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id: MockQuorumFixture.java 2984 2010-06-06 22:10:32Z thompsonbry $
   */
  public static class MockQuorum<S extends Remote, C extends QuorumMember<S>>
     ...
     *
     * <p>The task actually <em>peeks</em> at the {@link #queue} to get the event and leaves the
     * event on the {@link #queue} until it has been executed, finally removing the event from the
     * {@link #queue}. This makes each of the watchers run in a strictly sequence, which mirrors the
    ...
    @Override
    protected QuorumActorBase newActor(final String logicalServiceId, final UUID serviceId) {
    ...
    private class WatcherTask implements Runnable {
      ...
      private final MockQuorumWatcher watcher;
      ...
      public void run() {
        while (true) {
          try {
            ...
            if (InnerCause.isInnerCause(t, InterruptedException.class)) log.info("Shutdown : " + t);
            else log.error(t, t);
            break;
          }
        }
      ...
      private void runOnce() throws InterruptedException {
        ...
        try {
          ...
          if (log.isInfoEnabled()) log.info("Accepted event : " + e);
          // delegate the event
          //                        new Thread() {
          //                            public void run() {
    ...
    protected class MockQuorumActor extends QuorumActorBase {
      ...
      public MockQuorumActor(final String logicalServiceId, final UUID serviceId) {
     ...
     * MockQuorum}.
     *
     * <p>Note: This relies on the {@link QuorumEvent} mechanism. If there are errors, they will be
     * logged rather than propagated. This actually mimics what happens if a client spams zookeeper
     * with some bad data. The errors will be observed in the QuorumWatcher of the clients
     * monitoring that quorum.
     */
    protected class MockQuorumWatcher extends QuorumWatcherBase implements QuorumListener {
       ...
       *     the watchers need to be setup atomically as we read the distributed quorum state.
       */
      @Override
   ...
   * implementation you use DOES NOT have to be derived from this class. .
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id: MockQuorumFixture.java 2970 2010-06-03 22:21:22Z thompsonbry $
   */
  static class MockQuorumMember<S extends Remote> extends AbstractQuorumMember<S> {
    ...
    protected MockQuorumMember(final String logicalServiceId, MockQuorumFixture fixture) {
