 ...
 *
 * <p>There is a worksheet <code>src/architecture/search.xls</code> that gives the background for
 * this test suite.
 *
...
public class TestSearch extends AbstractSearchTest {
   ...
   *
   * @param hits The expected hits.
   * @param itr The iterator visiting the actual hits.
   */
  protected void assertSameHits(final IHit[] hits, final Iterator<? extends IHit> itr) {
    ...
    for (int i = 0; i < nhits; i++) {
      ...
      if (actualCosine < expectedCosine - .01d || actualCosine > expectedCosine + .01d) {
    ...
    assertFalse(
        "Iterator will visit too many hits - only " + nhits + " are expected", itr.hasNext());
  }
