...
public class TestKeyBuilder extends AbstractSearchTest {
   ...
   * use the configured value or the default for the text indexer rather than the standard default.
   * This is done because you typically want to recognize only Primary differences for text search
   * while you often want to recognize more differences when generating keys for a B+Tree.
   */
  ...
  public void test_keyOrder() {
    ...
    assertEquals(
        ...
        StrengthEnum.valueOf(FullTextIndex.Options.DEFAULT_INDEXER_COLLATOR_STRENGTH));
    ...
    init(
        ...
        // Use English.
        KeyBuilder.Options.USER_LANGUAGE,
        "en");

    final FullTextIndexTupleSerializer<Long> tupleSer =
        (FullTextIndexTupleSerializer<Long>) getIndexMetadata().getTupleSerializer();

    if (log.isInfoEnabled()) log.info(tupleSer.toString());
  ...
  protected void doKeyOrderTest(
    ...
    final ITupleSerializer<ITermDocKey<Long>, ITermDocVal> tupleSer =
  ...
  private static class ReadOnlyTermDocKey<V extends Comparable<V>> implements ITermDocKey<V> {
    ...
    private ReadOnlyTermDocKey(final String token, final V docId, final int fieldId) {
      this.token = token;
      this.docId = docId;
      this.fieldId = fieldId;
    }

    private ReadOnlyTermDocKey(
        final String token, final V docId, final int fieldId, final double termWeight) {
      this.token = token;
      this.docId = docId;
      this.fieldId = fieldId;
