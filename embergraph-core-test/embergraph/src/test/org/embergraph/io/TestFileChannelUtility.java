 ...
 *     have been observed to break into multiple IOs under load. transferAll() is neither atomic nor
 *     isolated since it has side-effects on the position of the source and target channels.
...
public class TestFileChannelUtility extends TestCase {
  ...
  private static final int bufferSize = DirectBufferPool.INSTANCE.getBufferCapacity();

  /** Start at any position in the source file (up to int32 offset). */
  protected long getRandomPosition(final RandomAccessFile raf) throws IOException {
   ...
   *
   * @param pos A position within that file.
   * @throws IOException
   */
  protected int getRandomLength(final RandomAccessFile raf, final long pos) throws IOException {

    final int count =
        (int)
            Math.min(
                Integer.MAX_VALUE,
                Math.min(raf.length() - pos, bufferSize * r.nextInt(3) + r.nextInt(bufferSize)));
  ...
  public void test_oneTrial_readAll_writeAll() throws IOException {
    ...
    try {
      ...
      final int ioCount1 =
          FileChannelUtility.writeAll(raf.getChannel(), ByteBuffer.wrap(expected), 0L /* pos */);

      final byte[] actual = new byte[expected.length];

      final int ioCount2 =
          FileChannelUtility.readAll(raf.getChannel(), ByteBuffer.wrap(actual), 0L);
    ...
    try {
       ...
       * If we will do another trial we first purturb the ground
          ...
          if (log.isInfoEnabled())
            log.info(
                "purturbing region after trial: trial=" + trial + ", off=" + off + ", len=" + len);
          ...
          final int ioCount =
              FileChannelUtility.writeAll(raf.getChannel(), ByteBuffer.wrap(a), (long) off);
   ...
   * @param raf A file pre-written with that ground truth data.
   * @throws IOException
   */
  protected void doReadTest(final int ntrials, final byte[] expected, RandomAccessFile raf)
      throws IOException {
    ...
    for (int trial = 0; trial < ntrials; trial++) {
      ...
      if (log.isInfoEnabled()) log.info("verifying data: pos=" + pos + ", count=" + count);
      ...
      if (ioCount > 1) {
        ...
        if (log.isInfoEnabled()) log.info("Note: read required: " + ioCount + " IOs");
      }

      if (0
          != BytesUtil.compareBytesWithLenAndOffset(
              (int) pos, count, expected, 0, count, actual.array())) {
  ...
  public void test_transferAllFrom() throws IOException {
    ...
    try {
      ...
      // write ground truth onto the file.
      FileChannelUtility.writeAll(source.getChannel(), ByteBuffer.wrap(expected), 0L /* pos */);
       ...
       * Transfer some number of bytes from the source channel to the
        ...
        final int ioCount1 =
            FileChannelUtility.transferAll(
                source.getChannel(), fromPosition, count, target, toPosition);

        // changed : new position is [fromPosition + count]
        assertEquals("sourcePosition", fromPosition + count, source.getChannel().position());
       ...
       * Read the data back from the target channel.
        ...
        final int ioCount2 = FileChannelUtility.readAll(target.getChannel(), actual, fromPosition);
       ...
       * Verify that the transferred data agrees with the ground truth.
        ...
        if (0
            != BytesUtil.compareBytesWithLenAndOffset(
                (int) fromPosition, count, expected, 0, count, actual.array())) {
    ...
    try {
      ...
      try {
        ...
        final FileChannelUtility.ReopenerInputStream instr =
            new FileChannelUtility.ReopenerInputStream(reopener);
        try {
          int totalReads = 0;
          ...
          while (totalReads < filelen) {
            ...
            if (rdlen == -1) {
              throw new EOFException("Unexpected, total reads: " + totalReads);
            }
            totalReads += rdlen;
          }
        } finally {
  ...
  public void no_testAsyncReadersCancelled() throws IOException, InterruptedException {
    ...
    final File sourceFile =
        new File("/Volumes/NonSSD/embergraph/interrupted.jnl"); // External non-SSD drive
