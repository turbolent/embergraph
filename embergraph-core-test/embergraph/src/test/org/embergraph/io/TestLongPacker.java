...
public class TestLongPacker extends TestCase {
  ...
  public void testUnpack() throws IOException {
    ...
    doUnpackTest(
        0xaeede00a539271fL,
        new byte[] {
          (byte) 0x8a, (byte) 0xee, (byte) 0xde, (byte) 0x00,
          (byte) 0xa5, (byte) 0x39, (byte) 0x27, (byte) 0x1f
        });
  }

  public void doPackTest(final long v, final byte[] expected) throws IOException {
  ...
  private static class RandomNonNegativeLeadingZerosLong implements LongGenerator {
    ...
    public RandomNonNegativeLeadingZerosLong(Random rnd) {
      ...
      for (int i = 0; i < masks.length; i++) {
        ...
        //                mask &= 1L<<63; // always set the high bit so that the values are
        // non-negative.
  ...
  public void doStressTest(int ntrials, LongGenerator g) throws IOException {
    ...
    for (int i = 0; i < ntrials; i++) {
      ...
      if (expected < 0L) {
        ...
        assertEquals(
            "trial=" + i + ", v=" + expected + ", nbytes", actualByteLength1, actualByteLength2);
