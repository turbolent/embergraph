 ...
 * @version $Id$
 *     <p>FIXME add unit tests using read-only {@link ByteBuffer}s and one using a {@link
 *     DirectBufferPool} to give us a direct buffer for testing. This will test the logic that
 *     handles cases when we can not access the backing byte[].
 */
public abstract class AbstractRecordCompressorTestCase extends TestCase2
   ...
   * to the caller.
   *
   * @param n The #of terms to include in the record.
   */
   ...
   *
   * @param c The (de-)compressor.
   * @param expected The data to be compressed.
   * @return The #of bytes in the compressed record.
   */
  protected int doCompressionTest(
      final IRecordCompressor c, final byte[] expected, final int off, final int len) {
  ...
  public void test_recordCompressor02() {
    ...
    final IRecordCompressor c =
        (IRecordCompressor) SerializerUtil.deserialize(SerializerUtil.serialize(getInstance()));
  ...
  public void test_recordCompressor_stressTest() {
    ...
    for (int i = 0; i < limit; i++) {
      ...
      if (i % 100 == 0) {
        ...
        c = (IRecordCompressor) SerializerUtil.deserialize(SerializerUtil.serialize(c));
      }
    }
