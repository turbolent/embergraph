...
public class TestManagedReads extends TestCase {
   ...
   * random data to file.
   *
   * <p>Assume average size of (minrec + maxec)/2, this should give rough number of records.
   *
   * <p>We want to have approximately 20,000 records to read from so adjust proportion created to
   * that stored by using a mod. For example, if we want a 10Gb file of average 512 byte records
   * that is 20Million records which would result in a modulus of 1000 to be used to save sample
   * record entries. A 1Gb file (the smallest worth testing) would use result in a modulus of 100.
   *
   * @param filesize
   * @param minrec
   * @param maxrec
  ...
  public void unmanagedReads() {
    ...
    System.out.println(
        "testUnmanagedReads 5000 completed in " + (System.currentTimeMillis() - start) + "ms");
  }
  ...
  public void managedReads() {
    ...
    System.out.println(
        "testManagedReads 5000 completed in " + (System.currentTimeMillis() - start) + "ms");
  }
}
