...
import org.embergraph.btree.BloomFilter;
...
import org.embergraph.journal.AbstractInterruptsTestCase;
...
import org.embergraph.journal.AbstractRestartSafeTestCase;
...
import org.embergraph.journal.TestJournalAbort;
import org.embergraph.journal.TestJournalBasics;
import org.embergraph.journal.VerifyCommitRecordIndex;
...
public class TestRWJournal extends AbstractJournalTestCase {
  ...
  public Properties getProperties() {
    ...
    properties.setProperty(
        org.embergraph.rwstore.RWStore.Options.DEFAULT_FREE_BITS_THRESHOLD, "1000");
   ...
   * allocations.
   *
   * <p>This test stresses these methods.
   */
  ...
  public void test_removeAllRWStore() {
    ...
    try {
      ...
      for (int i = 0; i < NTRIALS; i++) {
        ...
        for (int j = 0; j < NINSERTS; j++) {
          ...
          if (r.nextDouble() < checkpointRate) {
            ...
            if (log.isInfoEnabled()) log.info("checkpoint: " + btree.getStatistics());
          ...
          if (r.nextDouble() < removeAllRate) {
            ...
            if (log.isInfoEnabled()) log.info("removeAll: " + btree.getStatistics());
      ...
      if (log.isInfoEnabled()) log.info("will removeAll: " + btree.getStatistics());

      btree.removeAll();

      if (log.isInfoEnabled()) log.info("will checkpoint: " + btree.getStatistics());

      btree.writeCheckpoint();

      if (log.isInfoEnabled()) log.info(" did checkpoint: " + btree.getStatistics());
   ...
   * {@link RWStore#reset()}.
   *
   * <p>This test writes some records and commits. It then deletes those records and invokes {@link
   * RWStore#reset()}. It then re-deletes those records and commits. If we have failed to discard
   * the logged deletes, then we will now have double-deletes in the delete blocks for that commit
   * point. The commit time is noted.
   *
   * <p>Next, the test creates several more commit points in order to trigger recycling. We verify
   * that the commit record for the commit time noted above is no longer accessible. If the test
   * reaches this point, then we know that double-deletes were not logged.
   *
   * <p>Note: This test was verified as correctly detecting the bug described in the ticket.
   *
   * <p>Note: The recycler MUST be used for this test.
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/602">RWStore does not discard
   *     logged deletes on reset()</a>
   */
  ...
  public static class TestRawStore extends AbstractRestartSafeTestCase {
    ...
    protected IRawStore getSmallSlotStore(
        final int slotSize, final int nWasteAllocators, final float maxWaste) {

      final Properties properties = new Properties(getProperties());

      properties.setProperty(AbstractTransactionService.Options.MIN_RELEASE_AGE, "0");

      properties.setProperty(RWStore.Options.SMALL_SLOT_TYPE, "" + slotSize);

      properties.setProperty(
          RWStore.Options.SMALL_SLOT_WASTE_CHECK_ALLOCATORS, "" + nWasteAllocators);

      properties.setProperty(RWStore.Options.SMALL_SLOT_HIGH_WASTE, "" + maxWaste);
    ...
    protected Journal getStore(final long retentionMillis) {
      ...
      properties.setProperty(
          AbstractTransactionService.Options.MIN_RELEASE_AGE, "" + retentionMillis);
     ...
     *
     * <p>To mitigate this problem, when more than some minimum number of allocators are in use the
     * storage stats are used to check on overall usage. If the waste is above some specified
     * amount, then an attempt is made to locate a "reasonable" candidate allocator to be used.
     *
     * <p>To test below sets thresholds to quickly trigger this behaviour, with a low minimum number
     * of allocators, and low "candidate" re-use threshold.
     *
    ...
    public void test_smallSlotWasteRecylcing() {
      ...
      final Journal store =
          (Journal)
              getSmallSlotStore(
                  1024,
                  10 /*min allocators to check*/,
                  20.0f /* % waste of store total slot waste before checking for candidates*/);
      try {
        ...
        for (int i = 0; i < 500000; i++) {
          ...
          if (r.nextInt(7)
              < 3) { // less than 50% recycle, so will not be added automatically to free list
            recycle.add(a);
          }
     ...
     * block size, then we would expect the physical address to increase by 128 bytes for each
     * allocation.
     */
        ...
        if (log.isInfoEnabled())
          log.info(
              "Final allocation: "
                  + faddr
                  + ", allocations: "
                  + (rw.getTotalAllocations() - numAllocs)
                  + ", allocated bytes: "
     ...
     * density. The DirectFixedAllocators will allocate from DirectBuffers, where locality of
     * reference is less important than efficient management of the memory, which is optimized by
     * allocations in smaller amounts that match the demands at a finer granularity.
     */
    ...
    private void showAllocReserve(
        final boolean optDensity, final int slotSize, final int reserve, final int mod) {
    ...
    private long reallocBatch(RWStore rw, int tsts, int sze, int grp) {
      ...
      for (int t = 0; t < tsts; t++) {
        for (int i = 0; i < grp; i++) {
          ...
          if (i % 2 == 0) rw.free(old, 1); // dunno what the real size is
        }
      }
     ...
     * freed addresses.
     *
     * <p>The process is to: Allocate a range of data (to establish a number of Allocators) Create
     * an Isolated context and allocate a handful more Create an Unisolated context Free some
     * committed addreses Abort the Unisolated context - calling abort on the Journal Commit the
     * Isolated context Observe if any of the previously committed data is freed!
     */
    public void testUnisolatedAllocationContextRecycling() {
      ...
      try {
        ...
        final IAllocationContext cntxt =
            rw.newAllocationContext(false /*Isolated*/); // Unisolated context
        ...
        IAllocationContext iso_cntxt =
            rw.newAllocationContext(true /*Isolated*/); // Isolated context
        for (int a = 0; a < 3; a++) {
        ...
        for (int a = 0; a < 36; a++) {
          ...
          if (log.isInfoEnabled())
            log.info(
                "Address: "
                    + rw.physicalAddress(rwaddr)
                    + ", Committed: "
                    + rw.isCommitted(rwaddr));
        }
        ...
        // This is the point that the AllocationContext ends up setting the free bits, iff we have
    ...
    public void testSimpleUnisolatedAllocationContextRecycling() {
      ...
      try {
        ...
        // Now create some allocation contexts
        final IAllocationContext iso_cntxt =
            rw.newAllocationContext(true /*Isolated*/); // Isolated context
    ...
    private long reallocBatchWithRead(
        Journal store, int tsts, int min, int sze, int grp, boolean commit, boolean reopen) {
        ...
        for (int i = 0; i < (grp / 5); i++) {
          ...
          try {
            ...
            throw new RuntimeException(
                "problem handling read: " + i + " in test: " + t + " from address: " + old, e);
          }
          final ByteBuffer bb = ByteBuffer.wrap(buf, 0, szes[i]);
          addr[i] = bs.write(bb);
          bb.flip();
     ...
     *
     * <p>Then to add an additional 128K allocations and re-measure recycling to observe if
     * performance degrades.
     *
     * <p>The test is "observational" and console output should be monitored.
     *
     * <p>Should be disabled for CI.
     */
    ...
    public void test_blob_realloc() throws InterruptedException {
      ...
      try {
        ...
        try {
          ...
          if (InnerCause.isInnerCause(t, PhysicalAddressResolutionException.class)) {
            if (log.isInfoEnabled()) {
            ...
            fail(
                "Expected: "
                    + PhysicalAddressResolutionException.class.getName()
                    + " reading from "
                    + (faddr >> 32)
                    + ", instead got: "
                    + t,
     ...
     *
     * <p>If an allocation is made for an AllocationContext then this will result in a
     * ContextAllocation object being created in the RWStore within which "shadow" allocations can
     * be made. If such a shadow allocation is deleted, within the AllocationContext, then this can
    ...
    public void test_allocationContexts() throws IOException {
      ...
      try {
        ...
        assertTrue(
            "allocator re-use", bs.getPhysicalAddress(addr1c) > bs.getPhysicalAddress(addr2b));
    ...
    public void test_snapshotData() throws IOException {
      ...
      try {
        ...
        while (data.hasNext()) {
          ...
          log.info("Position: " + e.getAddress() + ", data size: " + e.getData().length);
        }
      } finally {
        journal.destroy();
      }
    ...
    void stressSessionProtection(final int smallSlotSize) {
      ...
      try {
...

        {
          ...
          assertTrue(
              reserved - used > (addrs.size() / 2)); // must not have reclaimed recycled slots
        }
    ...
    public void test_pureAlloc() {
      ...
      try {
        ...
        if (log.isInfoEnabled())
          log.info(
              "Final allocations: "
                  + rw.getTotalAllocations()
                  + ", allocated bytes: "
                  + rw.getTotalAllocationsSize()
                  + ", file length: "
                  + rw.getStoreFile().length());
        store.close();
        if (log.isInfoEnabled()) log.info("Re-open Journal");
     ...
     *
     * <p>assert that allocation remains committed
     */
     ...
     *
     * @see <a href="http://trac.blazegraph.com/ticket/973" >RWStore commit is not robust to
     *     internal failure.</a>
     */
     ...
     * header.
     *
     * <p>To test this we use a max fixed alloc of 192 bytes, so 24 8 bytes addresses can be stored
     * in each "leaf" and 48 leaves can be addressed in a standard blob header.
     *
     * <p>This should set the allocation threshold at 48 * 24 = 1152 deferred addresses
     *
     * <p>Anything greater than that requires that the blob header itself is a blob and this is what
     * we need to test.
     */
    public void test_stressBlobBlobHeaderDeferredFrees() {
      ...
      properties.setProperty(RWStore.Options.ALLOCATION_SIZES, "1,2," + maxFixed);
     ...
     * <p>replaced with
     *
     * <p>final int commitPointsRemoved = commitPointsRecycled;
     */
     ...
     *
     * <p>assert that allocation is no longer committed
     */
     ...
     *
     * <p>To be able to check for the address release, we must ensure that the same address cannot
    ...
    public void test_allocBlobBoundariesCommitFreeCommitWithHistory() {
      ...
      try {
        ...
        if (false) {
          ...
          if (log.isInfoEnabled()) log.info("Bloomfilter: " + ((int) addrb));
        }
     ...
     * <p>Allocate - Commit - Free - Commit
     *
     * <p>Tracks writeCache state through allocation
     */
     ...
     * <p>Allocate - Commit - Free - Commit
     *
     * <p>..but with session protection using a RawTx
     */
     ...
     * committed and allocate new data Reset Test that deletion and new allocation are void for
     ...
     *
     * <p>Commit some data UnIsolated: Delete committed and allocate new data Isolated: Delete
     * committed and allocate new data UnIsolated: Delete committed and allocate new data Reset Test
     * that deletion and new allocation are void for unisolated actions but not isolated
     */
     ...
     * the root block.
     *
     * <p>This test should confirm that.
     *
     * <p>Establishes some committed data and runs two readers concurrent with a single writer
     * intermittently aborting
     *
     * @throws InterruptedException
     */
