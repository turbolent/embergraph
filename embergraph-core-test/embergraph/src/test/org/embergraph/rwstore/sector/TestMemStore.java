...
public class TestMemStore extends AbstractJournalTestCase {
  ...
  public static Properties setProperties(final Properties properties) {
    ...
    properties.setProperty(Options.BUFFER_MODE, BufferMode.MemStore.toString());
     ...
     */
    {
      ...
      if (Boolean.valueOf(
          properties.getProperty(Options.CREATE_TEMP_FILE, Options.DEFAULT_CREATE_TEMP_FILE))) {
  ...
  public static class TestRawStore extends AbstractRawStoreTestCase {
    ...
    private void verifyCommitIndex(final Journal jrnl, int expectedRecords) {
      ...
      if (jrnl.getBufferStrategy().getBufferMode() != BufferMode.MemStore) {
        System.err.println(
            "Buffer mode should be MemStore not " + jrnl.getBufferStrategy().getBufferMode());
      ...
      final byte[] releaseKey =
          metadata.getTupleSerializer().serializeKey(System.currentTimeMillis());
