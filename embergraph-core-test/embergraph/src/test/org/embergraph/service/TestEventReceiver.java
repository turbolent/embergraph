...
public class TestEventReceiver extends TestCase2 {
  ...
  public void test_start_end() throws InterruptedException {
    ...
    final EventReceiver eventReceiver =
        new EventReceiver(1000 /* eventHistoryMillis */, eventBTree);

    final IEmbergraphFederation fed = new MockFederation(eventReceiver);

    final Event e = new MyEvent(fed, new EventResource("testIndex"), "testEventType");
    ...
    assertTrue(eventReceiver.rangeIterator(e.startTime, e.startTime + 1).hasNext());

    final Event t = eventReceiver.rangeIterator(e.startTime, e.startTime + 1).next();
  ...
  public void test_purgesHistory() throws InterruptedException {
    ...
    final EventReceiver eventReceiver = new EventReceiver(eventHistoryMillis, eventBTree);
    ...
    while ((elapsed = System.currentTimeMillis() - begin) < eventHistoryMillis / 2) {
      ...
      if (r.nextDouble() < .2) {
        ...
        try {
          ...
          Thread.sleep(r.nextInt((int) eventHistoryMillis / 10) /* ms */);
  ...
  private static class EventFactory implements Callable<Void> {
    ...
    public EventFactory(final IEmbergraphFederation fed, final int nevents) {
  ...
  private static class EventConsumer implements Callable<Void> {
    ...
    public Void call() throws Exception {
      ...
      for (int i = 0; i < 50; i++) {
        ...
        final Iterator<Event> itr = eventReportingService.rangeIterator(0L, Long.MAX_VALUE);
