...
public class TestReceiveBuffer extends TestCase3 {
   ...
   *
   * @throws Exception
   * @todo do test where the receive buffer is too small or is setup with a position and limit which
   *     are not sufficient to receive the data from the source buffer.
   */
  public void test_receiveBuffer() throws Exception {

    final UUID allowedUUID = UUID.randomUUID();

    final IBufferAccess allowedBufferdb = DirectBufferPool.INSTANCE.acquire();
    ...
    try {
      ...
      if (log.isInfoEnabled())
        log.info("allowedUUID=" + allowedUUID + ", allowedBuffer: " + allowedBuffer);
      ...
      final IBufferAccess receiveBufferdb = DirectBufferPool.INSTANCE.acquire(1, TimeUnit.SECONDS);
   ...
   */
  public void test_receiveBuffer_stress()
    ...
    final ExecutorService exService =
        Executors.newFixedThreadPool(nbuffers, DaemonThreadFactory.defaultThreadFactory());
    ...
    try {
      ...
      for (int i = 0; i < nbuffers; i++) {
        ...
        buffers.put(uuids[i] = UUID.randomUUID(), b = DirectBufferPool.INSTANCE.acquire());
