 ...
 *     using just 2 data services to make setup easier.
 *     <p>An alternative setup would pre-partition a single scale-out index so that it was on both
 *     data services.
 * @todo There should be explicit tests of distributed transactions that involve different indices,
 *     including the case where the MDS is participating in the tx. This should really be no
 *     different, but the tests should cover the case anyway. In point, perhaps the easiest thing to
 *     do is to register different indices on each data service since the commit protocol is
 *     entirely in terms of the local index resources (it pays no attention to scale-out indices, so
 *     this is best really).
...
//     * Writes a key/val pair on a named index on the specified
  ...
  public void test_localTxAbort() throws IOException, InterruptedException, ExecutionException {
    ...
    {
      final IndexMetadata md = new IndexMetadata(name1, UUID.randomUUID());
