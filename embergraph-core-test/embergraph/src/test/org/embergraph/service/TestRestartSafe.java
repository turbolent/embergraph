...
public class TestRestartSafe extends AbstractEmbeddedFederationTestCase {
   ...
   * the data services. The federation is then closed and a new instance of the federation is opened
   * and we verify that the metadata and data services were discovered, that the index is still
   * registered, and that the data is still in the index. We also verify that the next partition
   * number assigned to the metadata index is strictly ascending after restart.
   *
   * @throws IOException
  ...
  public void test_restartSafe() throws IOException, InterruptedException, ExecutionException {
     ...
     * metadata services.
     */
    assertEquals("#dataServices", 2, ((EmbeddedFederation) fed).getDataServiceCount());

    final UUID metadataServiceUUID = fed.getMetadataService().getServiceUUID();

    final UUID dataService0UUID = ((EmbeddedFederation) fed).getDataService(0).getServiceUUID();

    final UUID dataService1UUID = ((EmbeddedFederation) fed).getDataService(1).getServiceUUID();
    ...
    assertEquals("#dataServices", 2, ((EmbeddedFederation) fed).ndataServices);
    ...
    assertEquals(
        "metadataService UUID", metadataServiceUUID, fed.getMetadataService().getServiceUUID());

    // verify services have distinct UUIDs.
    assertNotSame(dataService0.getServiceUUID(), dataService1.getServiceUUID());
    ...
    if (!dataService0.getServiceUUID().equals(dataService0UUID)
      ...
      fail("Not expecting data service with UUID: " + dataService0.getServiceUUID());
    }
    ...
    if (!dataService1.getServiceUUID().equals(dataService0UUID)
      ...
      fail("Not expecting data service with UUID: " + dataService1.getServiceUUID());
    }
    ...
    assertEquals(indexUUID, fed.getIndex(name, ITx.UNISOLATED).getIndexMetadata().getIndexUUID());

    /*
     * Verify the next partition identifier assigned.
     */
    assertEquals("nextPartitionId", nextPartitionId1 + 1, metadataService.nextPartitionId(name));
