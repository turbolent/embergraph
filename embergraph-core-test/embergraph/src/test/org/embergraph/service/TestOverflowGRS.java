 ...
 * Journal} and an {@link IndexSegment}) after the overflow event.
 *
...
public class TestOverflowGRS extends AbstractEmbeddedFederationTestCase {
  ...
  public Properties getProperties() {
    ...
    properties.setProperty(
        EmbeddedClient.Options.COPY_INDEX_THRESHOLD, Integer.toString(copyIndexThreshold));

    // pre-size the journal to the mimumum extent.
    properties.setProperty(
        EmbeddedClient.Options.INITIAL_EXTENT, Long.toString(Options.minimumInitialExtent));

    // overflow as soon as we exceed that minimum extent.
    properties.setProperty(
        EmbeddedClient.Options.MAXIMUM_EXTENT, Long.toString(Options.minimumInitialExtent));
   ...
   */
  public void test_GRS_Overflow_Scan()
     ...
     */
    {
      ...
      final Iterator<? extends ITPS> itr = rowStore.rangeIterator(GlobalRowStoreSchema.INSTANCE);

      while (itr.hasNext()) {

        final ITPS tps = itr.next();

        if (log.isInfoEnabled()) log.info(tps);
    ...
    final Map<String, Object> propertySet = new LinkedHashMap<String, Object>();
    {
      for (int i = 0; i < N; i++) {
        ...
        if (c > 0) fail("Asynchronous overflow: i=" + i + ", asynchronousOverflowCounter=" + c);
      }
    }
    ...
    final long overflowCounter1;
    {
      ...
      // trigger overflow
      dataService0.forceOverflow(true /* immediate */, false /* compactingMerge */);
     ...
     */
    {
      ...
      final IndexMetadata md = dataService0.getIndexMetadata(shardName, ITx.UNISOLATED);

      final LocalPartitionMetadata lpmd = md.getPartitionMetadata();

      if (log.isInfoEnabled()) log.info("GRS PMD on DS: " + lpmd);

      if (lpmd.getResources().length < 2) {

        fail("Expecting at least two resources in the GRS shard view: " + lpmd);
      }
    }
