 ...
 * @version $Id$
 *     <p>FIXME modify this and {@link TestLDS} to be proxy test suites so that I can run tests
 *     against both with ease and make use of them for testing the {@link FullTextIndex} and {@link
 *     EmbergraphFileSystem}.
 */
public abstract class AbstractEmbeddedFederationTestCase extends AbstractBTreeTestCase {
  ...
  public Properties getProperties() {
    ...
    properties.setProperty(
        DistributedTransactionService.Options.DATA_DIR,
     ...
     */
    properties.setProperty(AbstractClient.Options.COLLECT_PLATFORM_STATISTICS, "false");
  ...
  public void setUp() throws Exception {
    ...
    if (log.isInfoEnabled()) log.info("metadataService: " + metadataService.getServiceUUID());

    dataService0 = ((EmbeddedFederation<?>) fed).getDataService(0);
    if (log.isInfoEnabled()) log.info("dataService0   : " + dataService0.getServiceUUID());

    if (((EmbeddedFederation<?>) fed).getDataServiceCount() > 1) {

      dataService1 = ((EmbeddedFederation<?>) fed).getDataService(1);
      if (log.isInfoEnabled()) log.info("dataService1   : " + dataService1.getServiceUUID());
    }
  }
  ...
  private void recursiveDelete(final File f) {
    ...
    if (log.isInfoEnabled()) log.info("Deleting: length=" + f.length() + ", file=" + f);

    if (!f.delete()) throw new RuntimeException("Could not remove: " + f);
   ...
   * @param indexUUID The unique identifier assigned to all instances of that index.
   */
  protected final void assertIndexRegistered(IDataService dataService, String name, UUID indexUUID)
      throws Exception {
    ...
    for (int i = 0; i < expected.length; i++) {
      ...
      if (expected[i] == null) {
        ...
        if (BytesUtil.compareBytes(expected[i], actual[i]) != 0) {
          ...
          fail(
              "expected="
                  + BytesUtil.toString(expected[i])
                  + ", actual="
                  + BytesUtil.toString(actual[i]));
        }
      }
    }
  }
   ...
   *
   * <p>Note: Normally you use bring the data service to the brink of the desired overflow event,
   * note the current overflow counter using {@link IDataService#getAsynchronousOverflowCounter()},
   * use {@link IDataService#forceOverflow()} to set the forceOverflow flag, do one more write to
   * trigger group commit and overflow processing, and then invoke this method to await the end of
   ...
   * @throws IOException
   */
  protected long awaitAsynchronousOverflow(
      final IDataService dataService, final long priorOverflowCounter) throws IOException {

    final long begin = System.currentTimeMillis();

    long newOverflowCounter;

    while ((newOverflowCounter = dataService.getAsynchronousOverflowCounter())
        == priorOverflowCounter) {
    ...
    if (log.isInfoEnabled())
      log.info(
          "\nOverflow complete: elapsed="
              + elapsed
              + " ms : priorOverflowCounter="
              + priorOverflowCounter
              + ", newOverflowCounter="
              + newOverflowCounter);
   ...
   * {@link MetadataIndex} does not use delete markers so a normal range count is exact).
   *
  ...
  protected int getPartitionCount(final String name) {
    ...
    while (itr.hasNext()) {
      ...
      if (log.isInfoEnabled()) log.info(SerializerUtil.deserialize(tuple.getValue()));
    }
