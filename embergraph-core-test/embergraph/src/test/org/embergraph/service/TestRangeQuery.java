...
public class TestRangeQuery extends AbstractEmbeddedFederationTestCase {
  ...
  public void test_rangeCount_staticPartitions_01() throws IOException {
    ...
    fed.registerIndex(
        ...
        new UUID[] {dataService0.getServiceUUID(), dataService1.getServiceUUID()});
  ...
  public void test_rangeQuery_staticPartitions_unbounded_emptyIndex_2partitions()
     ...
     */
    {
      final PartitionedTupleIterator itr = (PartitionedTupleIterator) ndx.rangeIterator(null, null);
  ...
  public void test_rangeQuery_staticPartitions_unbounded_2entries_2partitions_02()
     ...
     */
    {
      ...
      final ITupleIterator itr = ndx.rangeIterator(null, null, capacity, flags, null /* filter */);
  ...
  public void test_removeAll_limit1_twoPartitions() throws IOException {
    ...
    fed.registerIndex(
        metadata,
        new byte[][] {
          new byte[] {}, TestKeyBuilder.asSortKey(5) // the half-way point.
        },
        new UUID[] {dataService0.getServiceUUID(), dataService1.getServiceUUID()});
     ...
     */
    {
      ...
      final ITupleIterator itr =
          ndx.rangeIterator(null, null, 0 /* capacity */, IRangeQuery.ALL, null /* filter */);
   ...
   * in the correct order. One of the index partitions is deliberately left empty in order to verify
   * that the iterator will correctly cross over an index partition in which the chunked iterator
   * does not visit anything.
   *
   * @throws IOException
   */
  public void test_reverseScan() throws IOException {
    ...
    fed.registerIndex(
        metadata,
        new byte[][] {
          new byte[] {}, new byte[] {4}, new byte[] {7}, new byte[] {10},
        },
    ...
    {
      final ITupleIterator itr = ndx.rangeIterator(null, null, capacity, IRangeQuery.DEFAULT, null);
     ...
     */
    {
      final ITupleIterator itr =
          ndx.rangeIterator(
              null, null, capacity, IRangeQuery.DEFAULT | IRangeQuery.REVERSE, null /* filter */);
    ...
    {
      final ITupleIterator itr =
          ndx.rangeIterator(null, null, capacity, IRangeQuery.DEFAULT, filter);
    ...
    // reverse scan w/ filter.
    {
      final ITupleIterator itr =
          ndx.rangeIterator(
              null, null, capacity, IRangeQuery.DEFAULT | IRangeQuery.REVERSE, filter);
   ...
   * index partitions, with all of its various flags (including REMOVEALL), when a stale locator
   * exception must be handled, when the iterator is closed early, and when it is closed early with
   * a read consistent tx created on the caller's behalf.
   */
  ...
  public void test_parallelRangeIterator() {
...

    {
      ...
      assertSameIteratorAnyOrder(
          new ITuple[] {
            new TestTuple(
                flags, tupleSer, new byte[] {10}, new byte[] {10}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {11}, new byte[] {11}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {12}, new byte[] {12}, false /* deleted */, timestamp),
          },
          itr);
    }
...

    {
      ...
      assertSameIteratorAnyOrder(
          new ITuple[] {
            new TestTuple(
                flags, tupleSer, new byte[] {1}, new byte[] {1}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {2}, new byte[] {2}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {3}, new byte[] {3}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {4}, new byte[] {4}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {5}, new byte[] {5}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {6}, new byte[] {6}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {10}, new byte[] {10}, false /* deleted */, timestamp),
            new TestTuple(
                flags, tupleSer, new byte[] {11}, new byte[] {11}, false /* deleted */, timestamp),
   ...
   */
  public static <E> void assertSameIteratorAnyOrder(
   ...
   */
  public static <E> void assertSameIteratorAnyOrder(
      final String msg, final ITuple<E>[] expected, final ITupleIterator<E> actual) {
    ...
    while (nfound < expected.length) {
      ...
      if (log.isInfoEnabled())
        log.info("nvisited=" + (nfound + 1) + ", actualTuple=" + actualTuple);
      ...
      for (int k = 0; k < expected.length && !found; k++) {
        ...
        if (expectedTuple == null) continue;
      ...
      if (!found)
        fail("Tuple not expected" + ": nvisited=" + (nfound + 1) + ", tuple=" + actualTuple);
    }
   ...
   * @param actual
   */
  public static boolean sameTuple(final ITuple<?> expected, final ITuple<?> actual) {

    if (expected == null) throw new IllegalArgumentException();

    if (actual == null) throw new IllegalArgumentException();

    if (!BytesUtil.bytesEqual(expected.getKey(), actual.getKey())) return false;

    if (expected.isNull() != actual.isNull()) return false;

    if (!expected.isNull()) {

      if (!BytesUtil.bytesEqual(expected.getValue(), actual.getValue())) return false;
    }
