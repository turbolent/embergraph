...
public class TestEmbeddedClient extends AbstractEmbeddedFederationTestCase {
  ...
  public void test_staticPartitioning() throws Exception {
     ...
     * scale-out index.
     */
    assertIndexRegistered(
        dataService0, DataService.getIndexPartitionName(name, partitionId0), indexUUID);

    assertIndexRegistered(
        dataService1, DataService.getIndexPartitionName(name, partitionId1), indexUUID);
     ...
     */
    {
      IndexMetadata actual =
          dataService0.getIndexMetadata(
              DataService.getIndexPartitionName(name, partitionId0), ITx.UNISOLATED);

      // verify index partition exists on that data service.
      assertNotNull(actual);

      // partition metadata.
      assertEquals("partitionId", partitionId0, actual.getPartitionMetadata().getPartitionId());

      assertEquals(
          "leftSeparator", new byte[] {}, actual.getPartitionMetadata().getLeftSeparatorKey());

      assertEquals(
          "rightSeparator", new byte[] {5}, actual.getPartitionMetadata().getRightSeparatorKey());
    ...
    {
      IndexMetadata actual =
          dataService1.getIndexMetadata(
              DataService.getIndexPartitionName(name, partitionId1), ITx.UNISOLATED);

      // verify index partition exists on that data service.
      assertNotNull(actual);

      // partition metadata
      assertEquals("partitionId", partitionId1, actual.getPartitionMetadata().getPartitionId());

      assertEquals(
          "leftSeparator", new byte[] {5}, actual.getPartitionMetadata().getLeftSeparatorKey());

      assertEquals("rightSeparator", null, actual.getPartitionMetadata().getRightSeparatorKey());
  ...
  public void test_splitKeys_staticPartitions01() throws IOException {
     ...
     */
    {
      ...
      final List<Split> splits = ndx.splitKeys(ITx.UNISOLATED, 0, keys.length, keys);
