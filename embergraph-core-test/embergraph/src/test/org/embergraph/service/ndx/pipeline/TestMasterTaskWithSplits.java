 ...
 * @version $Id$
 *     <p>FIXME Finish this stress test and enable in {@link TestAll}.
 *     <p>Note: {@link KeyBuilder#decodeBigInteger(int, byte[])} has since been written. However,
 *     the encoding of a {@link BigInteger} as an unsigned byte[] requires a 2 byte signum/runLength
 *     prefix. Thus, while {@link KeyBuilder#append(BigInteger)} and the decode method might be used
 *     to complete this test case, the separator keys will need to be properly formed initially in
 *     order to have the leading signum/runlength. The <code>null</code> for the last rightSeparator
...
public class TestMasterTaskWithSplits extends AbstractKeyRangeMasterTestCase {
   ...
   *
   * @param leftSeparator The left separator key. The left-most separator key is always an empty
   *     byte[] (<code>byte[0]</code>).
   * @param rightSeparator The right separator key. The right-most separator key is always <code>
   *     null</code>.
   * @return A separator key which lies 1/2-way between the given keys.
   */
  protected byte[] getSeparatorKey(final byte[] leftSeparator, final byte[] rightSeparator) {
  ...
  private BigInteger decodeKey(final byte[] key) {
    ...
    if (key.length == 0) return BigInteger.ZERO;
   ...
   * points and does not go further to verify that a desired translation between signed longs,
   * unsigned byte[] keys, and {@link BigInteger} values is being carried out. [See notes at the top
   * of this file.]
   */
  public void test_decodeKey() {
    ...
    assertEquals(BigInteger.valueOf(Long.MIN_VALUE + 1), decodeKey(TestKeyBuilder.asSortKey(1L)));
  ...
  public void test_getSeparatorKey() {
    ...
    assertEquals(
        Long.MAX_VALUE,
        decodeKey(getSeparatorKey(new byte[0] /* leftSeparator */, null /* rightSeparator */))
            .longValue());
  }
   ...
   * MOVEs, or JOINs. The test writes {@link KVO} tuples on a {@link M master}. The master allocates
   * the tuples to output buffers based on the {@link IMetadataIndex} mapping. A single thread
   * executes an {@link Op}[] schedule while N concurrent producer threads write on the master. The
   * test ends when the schedule is done.
   *
  ...
  public void test_redirectStressTestWithSplits() throws InterruptedException, ExecutionException {
    ...
    final long subtaskChunkTimeoutNanos = TimeUnit.MILLISECONDS.toNanos(Long.MAX_VALUE);
            ...
            new DS(uuid) {
              ...
              protected void acceptWrite(final L locator, final KVO<O>[] chunk) {
                ...
                final long delayMillis =
                    (long) (r.nextDouble() * (maxWriteDelay - minWriteDelay)) + minWriteDelay;
                ...
                if (log.isInfoEnabled()) log.info("Wrote on " + this + ".");
              }
            });
      }
    }
    ...
    final Op[] schedule =
        new Op[] {
          ...
          //                new Op(OpCode.ScatterSplit, 5, scheduleUnit), // FIXME test scatter
          // split.
    ...
    //        final long timeoutMillis;
    {
      ...
      //            timeoutMillis = t;
      if (log.isInfoEnabled()) log.info("Test will run for " + t + "ms");
    }
      ...
      public Void call() throws Exception {
        ...
        try {
          ...
          while (true) {
            ...
            // chunks.
            Thread.sleep(r.nextInt((int) (maxProducerDelay - minProducerDelay)) + minProducerDelay);
            ...
            for (int i = 0; i < a.length; i++) {
              ...
              final byte[] key = keyBuilder.reset().append(r.nextLong()).getKey();
    ...
    class RedirectTask implements Callable<Void> {
      ...
      public Void call() throws Exception {
        ...
        for (Op op : schedule) {
          ...
          if (log.isInfoEnabled())
            log.info("Will wait " + delayMillis + "ms before executing: " + op.code);
          ...
          switch (op.code) {
            ...
            case Done:
              ...
              break;
            default:
              throw new UnsupportedOperationException(op.code.toString());
          }
        }
      ...
      protected void split(final Op op) {
        ...
        try {
          ...
          final byte[] separatorKey =
              getSeparatorKey(oldLocator.getLeftSeparatorKey(), oldLocator.getRightSeparatorKey());

          final L newLeftSibling =
              new L(
                  master.mdi.incrementAndGetNextPartitionId(),
                  getRandomDataService().uuid,
                  oldLocator.getLeftSeparatorKey(),
                  separatorKey);

          final L newRightSibling =
              new L(
                  master.mdi.incrementAndGetNextPartitionId(),
                  getRandomDataService().uuid,
                  separatorKey,
          ...
          // remove old locator.
          assertNotNull(master.mdi.remove(oldLocator.getLeftSeparatorKey()));

          // add new locators covering the same key-range.
          master.mdi.insert(newLeftSibling.getLeftSeparatorKey(), newLeftSibling);
          master.mdi.insert(newRightSibling.getLeftSeparatorKey(), newRightSibling);

          // Notify DS so it will issue stale locator response.
          final DS oldDS = dataServices.get(oldLocator.getDataServiceUUID());
    ...
    final M master =
        new M(masterStats, masterBuffer, executorService, sinkIdleTimeout, sinkPollTimeout) {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("Closing master buffer: " + masterBuffer);
      ...
      while (true) {
        try {
          ...
          if (log.isInfoEnabled())
            log.info(
                "Waiting on master: ~subtaskCount="
                    + masterStats.getActiveSinkCount()
                    + ", ~elementsRemaining="
                    + (masterStats.elementsIn.get() - masterStats.elementsOut.get()));
        }
      }
