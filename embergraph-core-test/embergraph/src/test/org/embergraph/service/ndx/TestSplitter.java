 ...
 * @version $Id$
 * @todo review the costs of the validation logic used by {@link AbstractSplitter}.
 */
public class TestSplitter extends TestCase2 {
  ...
  public void test_splitter_correctRejection() {
    ...
    try {
      ...
      splitter.splitKeys(
          timestampOk, -1 /* fromIndex */, 1 /* toIndex */, new byte[][] {} /* keys */);
    ...
    try {
      ...
      final byte[][] keys = new byte[][] {new byte[] {}, new byte[] {}};

      splitter.splitKeys(timestampOk, 2 /* fromIndex */, 3 /* toIndex */, keys);
    ...
    try {
      ...
      splitter.splitKeys(timestampOk, 1 /* fromIndex */, 1 /* toIndex */, keys);
    ...
    try {
      ...
      splitter.splitKeys(timestampOk, 0 /* fromIndex */, 1 /* toIndex */, keys);
  ...
  public void test_splitter_correctSplits_01() {
    ...
    final byte[][] keys = new byte[][] {new byte[] {1}};

    assertSplits(
        new Split[] {new Split(loc1, 0, 1)},
  ...
  public void test_splitter_correctSplits_02() {
    ...
    final byte[][] keys = new byte[][] {new byte[] {1}, new byte[] {3}};
  ...
  public void test_splitter_correctSplits_03() {
    ...
    final byte[][] keys = new byte[][] {new byte[] {1}, new byte[] {2}};
    ...
    assertSplits(
        ...
        splitter.splitKeys(ts, 1 /* fromIndex */, 2 /* toIndex */, keys).iterator());
   ...
   *     null (done). This means that we have to test with more than one index partition.
   */
  public void test_splitter_keysOutOfOrder() {
    ...
    final byte[][] keys =
        new byte[][] {
          new byte[] {1}, new byte[] {3}, new byte[] {2},
        };

    try {
      splitter.splitKeys(ts, 0 /* fromIndex */, keys.length /* toIndex */, keys);
      fail("Expecting: " + IllegalArgumentException.class);
    } catch (IllegalArgumentException ex) {
      if (log.isInfoEnabled()) log.info("Ignoring expected exception: " + ex);
    }
   ...
   * @param actual
   */
  protected void assertSplits(final Split[] expected, final Iterator<Split> actual) {

    for (Split eSplit : expected) {

      assertTrue("Actual did not produce enough splits.", actual.hasNext());

      final Split aSplit = actual.next();

      assertEquals(eSplit.pmd.getPartitionId(), aSplit.pmd.getPartitionId());

      assertEquals(eSplit.pmd.getLeftSeparatorKey(), aSplit.pmd.getLeftSeparatorKey());

      assertEquals(eSplit.pmd.getRightSeparatorKey(), aSplit.pmd.getRightSeparatorKey());
