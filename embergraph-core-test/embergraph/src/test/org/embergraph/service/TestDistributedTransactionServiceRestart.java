...
public class TestDistributedTransactionServiceRestart extends TestCase2 {
  ...
  public void test_setReleaseTime() {
    ...
    final MockDistributedTransactionService service =
        new MockDistributedTransactionService(properties).start();
   ...
   * has the expected values (read from the snapshot). Another entry is then added to the commit
   * index and the release time is advanced, which should cause the head of the commit index to be
   * truncated. The test then verifies the expected state of the commit index, shuts down the
   * service, and verifies that another snapshot file was written. Finally, we restart the service
   * one more time and verify that the commit index has the data that we would expect if it had read
   * the 2nd snapshot.
   */
  public void test_snapshotCommitIndex() {
    ...
    MockDistributedTransactionService service =
        new MockDistributedTransactionService(properties).start();

    final File file0 =
        new File(
            service.dataDir,
            DistributedTransactionService.BASENAME + "0" + DistributedTransactionService.EXT);

    final File file1 =
        new File(
            service.dataDir,
            DistributedTransactionService.BASENAME + "1" + DistributedTransactionService.EXT);
