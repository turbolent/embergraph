 ...
 * EmbeddedDataServiceImpl} has its own {@link WriteExecutorService}. Unlike a distributed
 * federation, an embedded federation starts and stops with the client. An embedded federation may
 * be used to assess or remove the overhead of network operations, to simplify testing of client
 * code, or to deploy a scale-up (vs scale-out) solution.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id$
 * @todo Put the services into directories named by the service class, e.g., MetadataService, just
 *     like scale-out.
 * @todo The EDS/LDS should use their own options in their own namespace to specify the data
 *     directory for the federation (they could just use a jini configuration). Ditto for the
 *     "transient" or "createTempFile" properties. Everything is namespaced now and the overridden
 *     semantics of org.embergraph.journal.Options.CREATE_TEMP_FILE and StoreManager#DATA_DIR are
 *     just getting us into trouble. Look at all uses of these options in the unit tests and
 *     decouple them from the journal's options.
 */
public class EmbeddedFederation<T> extends AbstractScaleOutFederation<T> {
   ...
   *
   * @param serviceUUID
   * @return The {@link DataService} for that UUID or <code>null</code> if there is no data service
   *     instance with that service UUID.
   */
  public final IDataService getDataService(final UUID serviceUUID) {
   ...
   */
  public final DataService getDataService(final int index) {
  ...
  public final UUID[] getDataServiceUUIDs(final int maxCount) {

    assertOpen();

    if (maxCount < 0) throw new IllegalArgumentException();

    final int n = maxCount == 0 ? ndataServices : Math.min(maxCount, ndataServices);
  ...
  protected EmbeddedFederation(final EmbeddedClient<T> client) {
    ...
    isTransient =
        BufferMode.Transient.toString().equals(properties.getProperty(Options.BUFFER_MODE));
    ...
    if (isTransient) {
      ...
      if (!dataDir.isDirectory()) {
        ...
        throw new RuntimeException(
            "Regular file exists with that name: " + dataDir.getAbsolutePath());
      }
    }
     ...
     */
    {
      ...
      if (isTransient) {
        ...
        // disable snapshots
        p.setProperty(EmbeddedTransactionServiceImpl.Options.SHAPSHOT_INTERVAL, "0");

      } else {

        // specify the data directory for the txService.
        p.setProperty(
            EmbeddedTransactionServiceImpl.Options.DATA_DIR,
            new File(dataDir, "txService").toString());
      }

      abstractTransactionService = new EmbeddedTransactionServiceImpl(UUID.randomUUID(), p).start();
    }
...

    {
      ...
      if (isTransient) {
        ...
        p.setProperty(
            EmbeddedLoadBalancerServiceImpl.Options.LOG_DIR, new File(dataDir, "lbs").toString());
      }
      ...
      try {
        ...
        loadBalancerService = new EmbeddedLoadBalancerServiceImpl(UUID.randomUUID(), p).start();
    ...
    if (isTransient) {
      ...
      if (serviceDirs.length == 0) {
        ...
        for (File serviceDir : serviceDirs) {
          ...
          //                    p.setProperty(Options.FILE, new
          // File(serviceDir,"journal"+Options.JNL).toString());
...

    {
      ...
      for (IDataService ds : this.dataService) {
        ...
        try {
          ...
          loadBalancerService.join(ds.getServiceUUID(), ds.getServiceIface(), hostname);
      ...
      loadBalancerService.join(
          abstractTransactionService.getServiceUUID(),
          abstractTransactionService.getServiceIface(),
          hostname);

      loadBalancerService.join(
          loadBalancerService.getServiceUUID(), loadBalancerService.getServiceIface(), hostname);
   ...
   *     <p>FIXME The embedded federation setup is not correct when transient buffers are requested.
   *     We wind up creating the resource manager files in the current working directory when we
   *     really want the resource manager to startup with transient journals (and disallow overflow
   *     since you can not re-open a store or even write an index segment).
   */
  private int createFederation(final Properties properties, final boolean isTransient) {
    ...
    {
      final String val =
          properties.getProperty(Options.NDATA_SERVICES, Options.DEFAULT_NDATA_SERVICES);

      ndataServices = Integer.parseInt(val);

      if (ndataServices <= 0) {

        throw new IllegalArgumentException(Options.NDATA_SERVICES + "=" + val);
      }
    }
  ...
  public void destroy() {
    ...
    if (metadataService != null) {
      ...
      if (!isTransient()) {
        ...
        final File tmp =
            new File(metadataService.getResourceManager().getDataDir(), EmbeddedFederation.MDS);
