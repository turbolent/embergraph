...
public class TestScatterSplit extends AbstractEmbeddedFederationTestCase {
  ...
  public Properties getProperties() {
    ...
    //         */
    //        properties.setProperty(LoadBalancerService.Options.INITIAL_ROUND_ROBIN_UPDATE_COUNT,
    // "0");
  ...
  /**
   * Test writes on a scale-out index until it has enough data to undergo a scatter split, validates
   * that the index was distributed into N shards per DS and validates the scale-out index after the
   * scatter split against ground truth.
   *
   * @throws IOException
  ...
  public void test_scatterSplit() throws IOException, InterruptedException, ExecutionException {
    ...
    final UUID indexUUID = UUID.randomUUID();
    {
       ...
       */
      indexMetadata.setScatterSplitConfiguration(
     ...
     */
    {
      ...
      assertEquals("partitionCount=" + npartitions, expectedIndexPartitionCount, npartitions);

      assertEquals(
          "#dataService0=" + ndataService0, expectedIndexPartitionCount / 2, ndataService0);

      assertEquals(
          "#dataService1=" + ndataService0, expectedIndexPartitionCount / 2, ndataService1);
    }
  }
}
