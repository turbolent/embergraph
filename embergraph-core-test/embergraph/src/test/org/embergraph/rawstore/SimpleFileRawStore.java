...
//     * This provides a purely transient means to identify deleted records.  This
  ...
  /*
   * Open a store. The file will be created if it does not exist and it is opened for writing. If
   * the file is opened for writing, then an exception will be thrown unless an exclusive lock can
   * be obtained on the file.
   *
   * @param file The name of the file to use as the backing store.
   * @param mode The file open mode for {@link RandomAccessFile#RandomAccessFile(File, String)()}.
   */
  public SimpleFileRawStore(final File file, final String mode) throws IOException {
  ...
  public boolean isReadOnly() {
    ...
    if (!open) throw new IllegalArgumentException();
  ...
  public long write(ByteBuffer data) {
    ...
    if (nbytes == 0) throw new IllegalArgumentException("No bytes remaining in buffer");
