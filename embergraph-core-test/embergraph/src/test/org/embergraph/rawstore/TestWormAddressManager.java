...
//     * Test of packing and unpacking addresses with a set of addresses selected
  ...
  //
  //        for (int i = WormAddressManager.MIN_OFFSET_BITS; i <=
  // WormAddressManager.MAX_OFFSET_BITS; i++) {
  //
  ...
  public void doTestGet(int offsetBits, int limit) {
    ...
    for (int i = 0; i < limit; i++) {
       ...
       */
      final int nbytes =
          r.nextInt((am.byteCountBits >= 32 ? Integer.MAX_VALUE : (int) am.maxByteCount));
...
//     * Helper method verifies (de-)serialization of addresses.
  ...
  public static long nextAddr(Random r, WormAddressManager am) {
  ...
  public static long nextNonZeroAddr(Random r, WormAddressManager am) {
   ...
   * @todo this does not excercise the entire possible range addresses that would satisify the
   *     contract of this method.
   */
  public static long nextNonZeroAddr(Random r, WormAddressManager am, long limit) {

    final long offset = r.nextInt((int) Math.min(Integer.MAX_VALUE, limit));

    final int nbytes =
        r.nextInt(
            (int) Math.min(am.getMaxByteCount(), Math.min(Integer.MAX_VALUE, limit - offset)));
   ...
   * @param nbytes The ground truth byte count.
   * @param offset The ground truth byte offset.
   */
  static void assertEncodeDecode(WormAddressManager am, int nbytes, long offset) {
   ...
   */
  public static int nextByteCount(Random r, WormAddressManager am) {
   ...
   * Integer#MAX_VALUE} bytes.
   */
  public static int nextNonZeroByteCount(Random r, WormAddressManager am) {

    int nbytes = 0;

    while (nbytes == 0) {

      nbytes = r.nextInt((am.byteCountBits >= 32 ? Integer.MAX_VALUE : (int) am.maxByteCount));
    }
  ...
  public static long nextOffset(Random r, WormAddressManager am) {
  ...
  public static long nextNonZeroOffset(Random r, WormAddressManager am) {
