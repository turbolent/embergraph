...
public class TestDuplicates extends AbstractHTreeTestCase {
  ...
  public void test_duplicateKeys() {
    ...
    try {
      ...
      HTree htree =
          getHTree(store, 3 /* addressBits */, false /* rawRecords */, true /* persistent */);
   ...
   * @param ndups The #of duplicate entries for a given key.
   */
  private void doTest(final int addressBits, final int ndups) {
    ...
    try {
      ...
      HTree htree = getHTree(store, addressBits, false /* rawRecords */, true /* persistent */);
      ...
      try {
        for (int i = 0; i < ndups; i++) {
          ...
          assertTrue(
              failure,
              htree.contains(k1)); // 2000 dups fails here with 10 bit depth, 70 dups with 6 bit
      ...
      assertTrue(
          failString + getCount(htree, k2),
          getCount(htree, k2) == ndups); // 50 dups fails with 8bit depth, 5 dups with 6 bit depth
      assertTrue(
          failString + getCount(htree, k3),
          getCount(htree, k3) == ndups); // 500 dups fails here with 10bit depth
  ...
  public void test_duplicateKeyValues() {
    ...
    try {
      ...
      HTree htree =
          getHTree(store, 3 /* addressBits */, false /* rawRecord */, true /* persistent */);
