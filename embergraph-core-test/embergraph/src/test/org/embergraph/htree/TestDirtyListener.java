...
public class TestDirtyListener extends AbstractHTreeTestCase {
  ...
  /*
   * Test exercises the various ways in which {@link HTree#fireDirtyEvent()} should and should not
   * be triggered.
   *
   * <p>Note: One of the things that this is testing is that we do not fire dirty events when the
   * tree is already dirty. This is unlikely to be harmful since the {@link IDirtyListener} is
   * primarily used to build up a commit list but it does not add any information since we just want
   * to know when a htree becomes dirty so that we can make sure that it will participate in a
   * commit.
   */
  public void test_dirtyListener1() {
    ...
    try {
       ...
       * Write again -- there should be no notice at the listener
        ...
        btree.insert(k4, val);
        btree.insert(k5, val);
        if (log.isInfoEnabled()) log.info("after insert of 3 more keys:\n" + btree.PP());
  ...
  /*
   * Used to detect correct and incorrect {@link IDirtyListener#dirtyEvent(ICheckpointProtocol)}
   * events.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
  ...
  private static class MyDirtyListener implements IDirtyListener {
     ...
     * this event, the listener is not registered until after the index has been created so we never
     * see that event.
     */
    ...
    public void dirtyEvent(final ICheckpointProtocol htree) {
      ...
      if (log.isInfoEnabled()) log.info("event: htree=" + htree);
