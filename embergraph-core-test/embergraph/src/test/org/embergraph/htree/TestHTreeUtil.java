...
public class TestHTreeUtil extends TestCase2 {
  ...
  public void test_isPowerOf2() {
    ...
    for (int i = 0; i < 32; i++) {
      ...
      if (v > 1) assertFalse(HTreeUtil.isPowerOf2(v + 1));
    }
  }
   ...
   * <dl>
   *   <dt>addressBits
   *   <dd>Size of the address space (fixed for a given hash tree).
   *   <dt>globalDepth
   *   <dd>The size of the address space for a buddy hash table on a directory page.
   *   <dt>nbuddies
   *   <dd>The #of buddy tables (buckets) on a directory (bucket) page. This depends solely on
   *       <i>addressBits</i> (a constant) and <i>globalDepth</i>.
   *   <dt>slots/buddy
   *   <dd>The #of directory entries in a buddy hash table. This depends solely on
   *       <i>globalDepth</i>.
   *   <dt>npointers
   *   <dd>The #of pointers in the parent which point to a given child (given based on a scan of the
   *       parent).
   *   <dt>localDepth
   *   <dd>The #of bits in the key examined at a given level in the tree. The range is
   *       [0:addressBits]. This depends solely on the <i>globalDepth</i> of a directory page and
   *       the #of pointers to child (<i>npointers</i>) in that directory page.
   *   <dt>nbits
   *   <dd>This is <code>(globalDepth-localDepth)</code>. It is the #of bits of the hash code which
   *       will be used to compute the <i>buddyOffset</i>.
   *   <dt>hashBits
   *   <dd>The LSB <code>globalDepth-localDepth</code> bits of the hash code. This is used to
   *       compute the <i>buddyOffset</i>.
   *   <dt>buddyOffset
   *   <dd>The offset of the buddy hash table or buddy bucket within the child.
   * </dl>
   */
  private void doHTreeMath(final int addressBits, final StringBuilder sb) {
    ...
    for (int globalDepth = 0; globalDepth <= addressBits; globalDepth++) {
      ...
      for (int npointers = (1 << globalDepth); npointers >= 1; npointers--) {
        ...
        final int localDepth = HTreeUtil.getLocalDepth(addressBits, globalDepth, npointers);
        ...
        for (int nbits = 0; nbits <= NBITS; nbits++) {
          ...
          final int buddyOffset = HTreeUtil.getBuddyOffset(hashBits, globalDepth, localDepth);
  ...
  public void test_getBuddyOffset_globalDepth2_localDepth0() {
    ...
    assertEquals(0, HTreeUtil.getBuddyOffset(0x00 /* 00 */, globalDepth, localDepth));

    assertEquals(1, HTreeUtil.getBuddyOffset(0x01 /* 01 */, globalDepth, localDepth));

    assertEquals(2, HTreeUtil.getBuddyOffset(0x02 /* 10 */, globalDepth, localDepth));

    assertEquals(3, HTreeUtil.getBuddyOffset(0x03 /* 11 */, globalDepth, localDepth));
  }
  ...
  public void test_getBuddyOffset_globalDepth2_localDepth1() {
    ...
    assertEquals(2, HTreeUtil.getBuddyOffset(0x01 /* 01 */, globalDepth, localDepth));
  }
}
