...
public class TestHTreeWithMemStore extends TestCase {
   ...
   *     tree.
   */
  private void doOverflowStressTest(
      final int addressBits, final int writeRetentionQueueCapacity, final int numOverflowPages) {
    ...
    try {
      ...
      for (int i = 0; i < inserts; i++) {
        ...
        try {
          ...
          System.err.println(
              "Problem on " + i + "th insert, with key: " + BytesUtil.toString(keys[0]));
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "Htree Leaves: "
                ...
                + ", allocation count="
                + ((MemStore) store).getMemoryManager().getAllocationCount());
   ...
   * @param writeRetentionQueueCapacity
   */
  private void doStressTest(final int addressBits, final int writeRetentionQueueCapacity) {
    ...
    try {
      ...
      try {
        ...
        final byte[] badkey =
            keyBuilder.reset().append(new Integer(s_limit * 32).hashCode()).getKey();
        ...
        for (int i = 0; i < s_limit; i++) {
          ...
          if (log.isTraceEnabled()) log.trace("after key=" + i + "\n" + htree.PP());
        }
        ...
        final long elapsedLookupFirstTime = System.currentTimeMillis() - beginLookupFirst;

        final long beginValueIterator = System.currentTimeMillis();

        // Verify the iterator visits all of the tuples.
        AbstractHTreeTestCase.assertSameIteratorAnyOrder(keys, htree.values());

        final long elapsedValueIteratorTime = System.currentTimeMillis() - beginValueIterator;
        ...
        assertFalse(
            htree
                .lookupAll(badkey)
                .hasNext()); // should not lazily create bucketPage if value not present!
       ...
       * Note: This code verifies that the dumpPages() code is
          ...
          final BaseIndexStats stats =
              htree.dumpPages(true /* recursive */, true /* visitLeaves */);
  ...
  private void doOrderedTest(final int addressBits, final int writeRetentionQueueCapacity) {
    ...
    try {
      ...
      try {
        ...
        // Verify the iterator visits all of the tuples.
        AbstractHTreeTestCase.assertSameOrderIterator(keys, htree.values());
