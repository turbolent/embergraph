 ...
 *     page because nothing has been inserted into that part of the address space.
 */
public class TestHTree extends AbstractHTreeTestCase {
   ...
   * {1,2,3,4}.
   *
   * <p>Note: This test stops after the 4th tuple insert. At that point the {@link HTree} is in a
   * state where inserting the next tuple in the sequence (5) will force a new directory page to be
   * introduced. The next two unit tests examine in depth the logic which introduces that directory
   * page. They are followed by another unit test which picks up on this same insert sequences and
   * goes beyond the introduction of the new directory page.
   *
  ...
  public void test_example1_addressBits2_insert_1_2_3_4() {
    ...
    try {
      ...
      assertEquals(null, htree.lookupFirst(unused));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v1}, htree.lookupAll(k1));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {}, htree.lookupAll(unused));
      // assertSameIterator(new AbstractPage[]{a}); // TODO Verify the direct and recursive
      // iterators.
       ...
       * entry and this will force a split of the buddy bucket. That means that a new bucket page
       * will be allocated, the pointers in the parent will be updated to link in the new bucket
       * page, and the buddy buckets will be redistributed among the old and new bucket page.
       *
       * <pre>
       * root := [2] (a,a,b,b)
       * a    := [1]   (1,2;-,-)
       ...
       */
      htree.insert(k2, v2);
      if (log.isInfoEnabled()) log.info(htree.PP());
      assertEquals("nnodes", 1, htree.getNodeCount());
      assertEquals("nleaves", 1, htree.getLeafCount());
      ...
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v1}, htree.lookupAll(k1));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v2}, htree.lookupAll(k2));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {}, htree.lookupAll(unused));
      ...
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v1}, htree.lookupAll(k1));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v2}, htree.lookupAll(k2));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v3}, htree.lookupAll(k3));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {}, htree.lookupAll(unused));
       ...
       *
       * <p>Note: Inserting another tuple into this buddy bucket will not only cause it to split but
       * it will also introduce a new directory page into the hash tree.
       *
       * <pre>
       * root := [2] (a,c,b,b)
       * a    := [2]   (1,2,3,4)
      ...
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v2}, htree.lookupAll(k2));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v3}, htree.lookupAll(k3));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {v4}, htree.lookupAll(k4));
      AbstractBTreeTestCase.assertSameIterator(new byte[][] {}, htree.lookupAll(unused));
   ...
   * level operations when structural changes are required. This allows us to view as many of the
   * intermediate states of the {@link HTree} as possible by taking the control logic "out of the
   * loop" as it were.
   *
   * <p>In order to insert the key (5) we will need to split (a). However, the prefix length for (a)
   * is only THREE (3) bits and we need a prefix length of 5 bits before we can split (a). This
   * prefix length requirement arises because we can not split (a) until the prefix bits considered
   * are sufficient to redistribute the tuples between (a) and its siblings (children of the same
   * parent directory page). The first such split will occur when we have a prefix length of 5 bits
   * and therefore can make a distinction among the keys in (a), which are:
   *
   * <pre>
   * key|bits
   * ---+--------
   ...
   * </pre>
   *
   * Consulting the table immediately above, the first non-zero bit is bit 5 of the key whose value
   * is FOUR (4).
   */
  public void test_example1_splitDir_addressBits2_1_2_3_4_5() {
    ...
    try {
      ...
      if (log.isInfoEnabled()) log.info("after inserting 1,2,3,4: \n" + htree.PP());
       ...
       * level which decreases depth(d) and makes it possible to split (a). There will be two
       * references to (a) in the post-condition, at which point it will be possible to split (a).
       *
       * <pre>
       * root := [2]  (d,c,b,b)
       ...
       * updated to verify this structure:
       *
       * <pre>
       * D#88 [2] (D#48,-,-,-)
       * D#48 [2]     (B#68,B#68,-,-)
   ...
   * of page overflow. The basic approach to page overflow is to double the address space on the
   * page. This is conceptually simple. However, this means that the address space of the page will
   * not be the same as the address space of the other pages and that could violate various
   * assumptions in the existing control logic.
   */
  ...
  public void test_distinctBits() {
    ...
    try {
      ...
      try {
        ...
        if (log.isInfoEnabled()) log.info("Pretty Print Error: " + htree.PP());
