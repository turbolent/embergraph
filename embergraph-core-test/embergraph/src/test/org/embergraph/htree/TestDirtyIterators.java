 ...
 * @version $Id$
 *     <p>TODO Extend this test suite to verify that remove() on the htree uses the copy-on-write
 *     pattern correctly.
 */
public class TestDirtyIterators extends AbstractHTreeTestCase {
   ...
   * true since we never evict a node onto the store during this test (the index is setup to throw
   * an exception if the tree attempts a node eviction).
   */
  public void test_dirtyChildIterator01() {
    ...
    try {
...

      {
        ...
        assertSameIterator(new AbstractPage[] {a}, root.childIterator(false /* dirtyOnly */));
        assertSameIterator(new AbstractPage[] {a}, root.childIterator(true /* dirtyOnly */));

        // empty tree visits the sole bucket page followed by the root.
        assertSameIterator(new AbstractPage[] {a, root}, htree.root.postOrderNodeIterator(false));
        assertSameIterator(new AbstractPage[] {a, root}, htree.root.postOrderNodeIterator(true));
      }
      {
        htree.insert(k1, v1);
        htree.insert(k2, v2);
        ...
        if (log.isInfoEnabled()) log.info("After insert: " + htree.PP());
        ...
        assertSameIterator(new AbstractPage[] {a}, htree.root.childIterator(false /* onlyDirty */));

        assertSameIterator(new AbstractPage[] {a}, htree.root.childIterator(true /* onlyDirty */));
      }

      /*
       * Insert another tuple to force some splits.
       *
       * <p>The expected post-condition is:
       *
       * <pre>
       ...
       */
      {
        ...
        if (log.isInfoEnabled()) log.info("After another insert: " + htree.PP());
        ...
        assertSameIterator(
            new AbstractPage[] {a, b, c}, htree.root.childIterator(true /* onlyDirty */));
      }
  ...
  public void test_dirtyChildIterator02() {
    ...
    try {
      ...
      /*
       * Insert another tuple to force some splits. The expected post-condition is:
       *
       * <pre>
       * #nodes=3, #leaves=3, #entries=5
      ...
      {
        if (log.isInfoEnabled()) log.info("After initial inserts: " + htree.PP());
       ...
       * Verify that iterator visits all direct children (all direct
        ...
        assertSameIterator(
            new AbstractPage[] {a, b, c}, htree.root.childIterator(true /* onlyDirty */));
        // a's children
        assertSameIterator(new AbstractPage[] {d, e, f}, a.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {d, e, f}, a.childIterator(true /* onlyDirty */));
        // d's children
        assertSameIterator(new AbstractPage[] {g, h, i}, d.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {g, h, i}, d.childIterator(true /* onlyDirty */));
       ...
       * write (b) onto the store and verify that it is no longer
        ...
        assertSameIterator(
            new AbstractPage[] {a, b, c}, htree.root.childIterator(false /* onlyDirty */));
        assertSameIterator(
            new AbstractPage[] {a, c}, htree.root.childIterator(true /* onlyDirty */));
       ...
       * write (d) onto the store and verify that it is no longer
        ...
        // verify direct children.
        assertSameIterator(new AbstractPage[] {d, e, f}, a.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {e, f}, a.childIterator(true /* onlyDirty */));
        // also verify that none of the children of (d) are visited by
        // the dirty iterator for (d).
        assertSameIterator(new AbstractPage[] {g, h, i}, d.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {}, d.childIterator(true /* onlyDirty */));
        // verify the post-order iterator.
        assertSameIterator(
            new AbstractPage[] {g, h, i, d, e, f, a, b, c, root},
            htree.root.postOrderNodeIterator(false /* dirtyOnly */));
       ...
       * write (f) onto the store and verify that it is no longer
        ...
        assertSameIterator(new AbstractPage[] {d, e, f}, a.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {e}, a.childIterator(true /* onlyDirty */));
        // verify the post-order iterator.
        assertSameIterator(
            new AbstractPage[] {g, h, i, d, e, f, a, b, c, root},
            htree.root.postOrderNodeIterator(false /* dirtyOnly */));
       ...
       * write (a) onto the store and verify that it is no longer
        ...
        assertSameIterator(
            new AbstractPage[] {a, b, c}, htree.root.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {}, htree.root.childIterator(true /* onlyDirty */));
        // verify the post-order iterator.
        assertSameIterator(
            new AbstractPage[] {g, h, i, d, e, f, a, b, c, root},
            htree.root.postOrderNodeIterator(false /* dirtyOnly */));
        assertSameIterator(
            new AbstractPage[] {root}, htree.root.postOrderNodeIterator(true /* dirtyOnly */));
      ...
      /*
       * insert a duplicate key (3). this goes into (g) which becomes full but is not split. verify
       * that the children of the parent (d) are unchanged but that (g) is no longer persistent and
       * is now dirty. Since the parent (d) of (g) was persistent, copy-on-write was also invoked
       * for (d) and it is now mutable and dirty.
       *
       * <p>Note: Some of the page references will have been changed by copy-on-write, so we
       * re-validate the index structure at this time. The expected post-condition is:
       *
       * <pre>
       ...
       */
      {
        ...
        assertSameIterator(new AbstractPage[] {g, h, i}, d.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {g}, d.childIterator(true /* onlyDirty */));

        // check dirty iterator for (d)'s parent (a).
        assertSameIterator(new AbstractPage[] {d, e, f}, a.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {d}, a.childIterator(true /* onlyDirty */));
       ...
       * Write the root node of the tree onto the store.
        ...
        assertSameIterator(new AbstractPage[] {d, e, f}, a.childIterator(false /* onlyDirty */));
        assertSameIterator(new AbstractPage[] {}, a.childIterator(true /* onlyDirty */));
        ...
        assertSameIterator(
            ...
            htree.root.postOrderNodeIterator(false /* dirtyOnly */));
        assertSameIterator(
            new AbstractPage[] {}, htree.root.postOrderNodeIterator(true /* dirtyOnly */));
      }
...
//     * Test ability to visit the dirty nodes of the tree in a post-order
  ...
  //        assertTrue(b.isPersistent());
  //        assertTrue(c.isPersistent());
  //        btree.insert(TestKeyBuilder.asSortKey(1), v1); // triggers copy on write for (a) and
  // (c).
  //        assertNotSame(c,btree.root);
  //        c = (Node)btree.root;
