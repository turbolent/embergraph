 ...
 * @version $Id$
 *     <p>TODO The 2nd two tests in this file both rely on structural assumptions about the B+Tree.
 *     They will have to be carefully recrafted to work for the HTree.
 *     <p>TODO There is an apparent problem with the persistence backed stress tests where
 *     incremental eviction can cause a parent directory page to become immutable during handling of
 *     addLevel(). The tests in this suite do not have the data scale necessary to trigger this
 *     issue.
 *     <p>TODO This focuses on the reference counts and reference changes while {@link
 *     TestDirtyIterators} focuses on structural changes and the visitation patterns for children
 *     and post-order traversal both for all pages and for only dirty pages.
...
public class TestIncrementalWrite extends AbstractHTreeTestCase {
  ...
  protected HTree getHTree(
      final IRawStore store, final int addressBits, final int queueCapacity, final int queueScan) {
    ...
    HTree btree =
        new TestHTree(store, checkpoint, md, false /*readOnly*/) {
  ...
  public void test_incrementalWrite() {
    ...
    try {
       ...
       */
      final HTree btree =
          getHTree(store, 2 /* addressBits */, 2 /* queueCapacity */, 1 /* queueScan */);
