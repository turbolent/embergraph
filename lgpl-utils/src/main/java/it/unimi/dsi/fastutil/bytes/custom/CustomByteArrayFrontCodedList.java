 ...
 * below). It implements an immutable type-specific list that returns the <var>i</var>-th array when
 * calling {@link #get(int) get(<var>i</var>)}. The returned array may be freely modified.
 *
 * <p>Front coding is based on the idea that if the <var>i</var>-th and the (<var>i</var>+1)-th
 * array have a common prefix, we might store the length of the common prefix, and then the rest of
 * the second array.
 *
 * <p>This approach, of course, requires that once in a while an array is stored entirely. The
 * <def>ratio</def> of a front-coded list defines how often this happens (once every {@link
 * #ratio()} arrays). A higher ratio means more compression, but means also a longer access time, as
 * more arrays have to be probed to build the result. Note that we must build an array every time
 ...
 *
 * <p>By setting the ratio to 1 we actually disable front coding: however, we still have a data
 ...
 *
 * <p>Note that the typical usage of front-coded lists is under the form of serialized objects;
 ...
 *
 * <p>All arrays are stored in a large array. A separate array of pointers indexes arrays whose
 * position is a multiple of the ratio: thus, a higher ratio means also less pointers.
 *
 * <p>More in detail, an array whose position is a multiple of the ratio is stored as the array
 * length, followed by the elements of the array. The array length is coded by a simple
 * variable-length list of <var>k</var>-1 bit blocks, where <var>k</var> is the number of bits of
 * the underlying primitive type. All other arrays are stored as follows: let <code>common</code>
 * the length of the maximum common prefix between the array and its predecessor. Then we store the
 * array length decremented by <code>common</code>, followed by <code>common</code>, followed by the
 * array elements whose index is greater than or equal to <code>common</code>. For instance, if we
 * store <samp>foo</samp>, <samp>foobar</samp>, <samp>football</samp> and <samp>fool</samp> in a
 * front-coded character-array list with ratio 3, the character array will contain
 *
 * <pre>
 ...
 * <H2>Limitations</H2>
 *
 * <p>All arrays are stored in a large array: thus, the compressed list must not exceed {@link
 * java.lang.Integer#MAX_VALUE} elements. Moreover, iterators are less efficient when they move
 * back, as {@link java.util.ListIterator#previous() previous()} cannot build incrementally the
 * previous array (whereas ( {@link java.util.ListIterator#next() next()} can).
 *
 * <h3>Modifications</h3>
 *
 * This class was derived from <code>it.unimi.dsi.fastutil.bytes.ArrayFrontCodedList</code>, which
 * is part of fastutils. The folowing changes were made:
 *
 * <ul>
 *   <li>The name of the class has been changed to prevent classpath problems.
 *   <li>The class has a new {@link #serialVersionUID} and the serialization logic has been modified
 *       to allow serialization against {@link DataOutput} by defining {@link #getBackingBuffer()}
 *       and a new constructors that operate on a byte[] slice.
 *   <li>The test code from main() has been isolated in a junit test suite.
 *   <li>The backing <code>byte[] array</code> has been replaced by an interface suitable for
 *       wrapping either a <code>byte[]</code> or a {@link ByteBuffer}. This was done in order to
 *       permit access to the front-coded representation without "de-serializing" the data and a
 *       suitable constructor was added for the {@link ByteBuffer} case.
 *   <li>Make the {@link Collection} and {@link Iterator} ctors strongly typed.
 * </ul>
 */
public class CustomByteArrayFrontCodedList extends AbstractObjectList<byte[]>
  ...
  private class BackingByteArray implements BackingBuffer {
    ...
    public void arraycopy(final int pos, final byte[] dest, final int destPos, final int len) {
    ...
    public int writeOn(final OutputStream dos, final int aoff, final int alen) throws IOException {
   ...
   * they do then ALL methods which touch the buffer need to be synchronized so NONE of them can
   * have a concurrent read during which the position/limit has been transiently modified. The
   * culprits here are the bulk byte transfer methods ByteBuffer#get() and ByteBuffer#put(). This is
   * really a huge limitation on the use of a ByteBuffer for concurrent access to a read-only data
   * structure.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   * @version $Id$
  ...
  private static class BackingByteBuffer implements BackingBuffer {
    ...
    public int readInt(final int pos) {
      synchronized (b) {
        if (get(pos) >= 0) return get(pos);
        if (get(pos + 1) >= 0) return (-get(pos) - 1) << 7 | get(pos + 1);
        if (get(pos + 2) >= 0)
    ...
    public int writeOn(final OutputStream dos, final int off, final int len) throws IOException {
  ...
  public CustomByteArrayFrontCodedList(
      final Iterator<byte[]> arrays, final int ratio, final boolean hasDups) {
   ...
   * @param c a collection containing arrays.
   * @param ratio the desired ratio.
   * @param hasDups <code>true</code> iff the list allows duplicate keys.
   */
  public CustomByteArrayFrontCodedList(
      final Collection<byte[]> c, final int ratio, final boolean hasDups) {
  ...
  private int extract(final int index, final byte a[], final int offset, final int length) {
    ...
    // the index-th.
    //        int pos, arrayLength = readInt(array, pos = startPos), prevArrayPos, currLen = 0,
    // actualCommon;
   ...
   * @todo An alternative optimization would be to specify a variant of {@link #get(int)} which
   *     accepts a org.embergraph.io.ByteArrayBuffer that is automatically extended to have
   *     sufficient capacity.
   */
  public int writeOn(final OutputStream os, final int index) throws IOException {
      ...
      public byte[] next() {
        ...
        if (i % ratio == 0) {
          ...
          if (inSync) {
            ...
            bb.arraycopy(pos + count(length) + count(common), a, common, length);
            pos += count(length) + count(common) + length;
            length += common;
          } else {
   ...
   * @param ratio The ratio of this front-coded list.
   * @param array The array containing the compressed arrays.
   */
  public CustomByteArrayFrontCodedList(final int n, final int ratio, final byte[] array) {
   ...
   * @param n The #of elements in the array.
   * @param ratio The ratio of this front-coded list.
   * @param b The view onto the compressed arrays.
   */
  public CustomByteArrayFrontCodedList(final int n, final int ratio, final ByteBuffer b) {
   ...
   * a linear scan is performed starting with the insertion point as identified by the binary
   * search. The combination of a binary search followed by a linear scan implies that search is
   * fastest when ratio is small. However, the compression is highest when the ratio is equal to the
   * #of entries in the list. Therefore, search performance is traded off against compression.
   *
   * <p>Note: The full length entry is coded every [i/ratio] entries. However, the subsequent
   * entries code their common length with respect to the previous front-coded entry NOT to the full
   * length entry. This means that the length of the common prefix can increase or decrease as we
   * scan a bucket and the #of already matched bytes can increase or decrease as well. Consider the
   * following example, when coded with a ratio of 8.
   *
   * <pre>
   ...
   * </pre>
   *
   * The common length grows for entry #4 because the [54] in the next to last byte in the array
   * already appears in the same position in the previous front-coded entry. However, the common
   * length decreases again for the next entry (#5).
   *
   * <p>The following rules guide the linear search of the bucket identified by the binary search.
   *
   * <ol>
   *   <li>If <code>clen GT mlen</code>, then skip to the next entry in the bucket as no match is
   *       possible (the common prefix was demonstrated to be longer than the matched prefix on a
   *       previous entry).
   *   <li>Compare the remaining bytes in the search probe with the remainder for the current entry.
   *   <li>If the search probe is EQ to the bucket entry, then halt. The probe key was found.
   *   <li>If the search probe is LT the bucket entry, then halt. The probe key was not found.
   *   <li>Otherwise, <code>mlen += prefixLength</code>, where prefixLength is the length of the
   *       matched prefix from step 2.
   * </ol>
   *
   * @param a The search probe, which is interpreted as an <em>unsigned byte[]</code>
   * @return index of the search key, if it is found; otherwise, <code>(-(insertion point) - 1)
   *     </code>. The insertion point is defined as the point at which the key would be inserted.
  ...
  public int search(final byte[] a) {
    ...
    for (delta = 0; delta < limit; delta++) {
      ...
      final int ret = compareBytes(a, mlen, a.length - mlen, bb, pos, rlen);
   ...
   * @return index of the search key into the pointer array, if it is an exact match with any of the
   *     full length values whose offsets are stored in the pointer array; otherwise, <code>
   *     (-(insertion point) - 1)</code>. The insertion point is defined as the point at which the
   *     key would be inserted into the list. Note that this guarantees that the return value will
   *     be >= 0 if and only if the key is matched by any of the full length coded entries.
   */
   ...
   * BackingBuffer}.
   *
   * @param index The index into the full length keys.
   * @param key The probe key.
   * @return A value which indicates whether the key at that offset into the backing buffer is LT,
   *     GT, or EQ to the caller's key.
   */
   ...
   *
   * @param a The caller's probe.
   * @param bb The {@link BackingBuffer}.
   * @param boff The offset of the first byte to be compared in the {@link BackingBuffer}.
   * @param blen The length of the byte[] at that offset in the {@link BackingBuffer}.
   * @return The return is a negative integer, zero, or a positive integer if the first argument is
   *     less than, equal to, or greater than the byte[] in the {@link BackingBuffer} at the
   *     specified offset. The return value also codes the length of the shared prefix, which may be
   *     computed as <code>Math.abs(ret)-1</code>.
   */
  private int compareBytes(
      final byte[] a,
      final int aoff,
      final int alen,
      final BackingBuffer bb,
      final int boff,
      final int blen) {
