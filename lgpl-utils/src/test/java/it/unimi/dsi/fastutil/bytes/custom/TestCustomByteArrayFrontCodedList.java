...
public class TestCustomByteArrayFrontCodedList extends TestCase {
  ...
  private void _test(final int n) {
    ...
    CustomByteArrayFrontCodedList m =
        new CustomByteArrayFrontCodedList(
            it.unimi.dsi.fastutil.objects.ObjectIterators.wrap(a), r.nextInt(4) + 1);
    it.unimi.dsi.fastutil.objects.ObjectArrayList t =
        new it.unimi.dsi.fastutil.objects.ObjectArrayList(a);
    ...
    /* Now we check cloning. */
        ...
        contentEquals(m, (java.util.List) m.clone()),
        "Error (" + seed + "): m does not equal m.clone()");
...

    {
      ...
      for (int k = 0; k < 2 * n; k++) {
            ...
            i.hasPrevious() == j.hasPrevious(),
            "Error (" + seed + "): divergence in hasPrevious()");

        if (r.nextFloat() < .8 && i.hasNext()) {
          ensure(
              java.util.Arrays.equals((byte[]) i.next(), (byte[]) j.next()),
              "Error (" + seed + "): divergence in next()");

        } else if (r.nextFloat() < .2 && i.hasPrevious()) {
          ensure(
              java.util.Arrays.equals((byte[]) i.previous(), (byte[]) j.previous()),
              "Error (" + seed + "): divergence in previous()");
        }

        ensure(i.nextIndex() == j.nextIndex(), "Error (" + seed + "): divergence in nextIndex()");
        ensure(
            i.previousIndex() == j.previousIndex(),
            "Error (" + seed + "): divergence in previousIndex()");
      }
    }

    {
      ...
      for (int k = 0; k < 2 * n; k++) {
            ...
            "Error ("
                + seed
                + "): divergence in hasNext() (iterator with starting point "
                + from
                + ")");
        ensure(
            i.hasPrevious() == j.hasPrevious(),
            "Error ("
                + seed
                + "): divergence in hasPrevious() (iterator with starting point "
                + from
                + ")");
        ...
        if (r.nextFloat() < .8 && i.hasNext()) {
              ...
              "Error ("
                  + seed
                  + "): divergence in next() (iterator with starting point "
                  + from
                  + ")");
          // System.err.println("Done next " + I + " " + J + "  " + badPrevious);
              ...
              "Error ("
                  + seed
                  + "): divergence in previous() (iterator with starting point "
                  + from
                  + ")");
        }
      }
     ...
     */
    try {
      CustomByteArrayFrontCodedList m2 =
          new CustomByteArrayFrontCodedList(m.size(), m.ratio(), m.getBackingBuffer().toArray());
      contentEquals(m, m2);
    } catch (Exception e) {
      throw new AssertionError(e);
    }
  ...
  //
  //        try {
  //            if ("speedTest".equals(args[0]) || "speedComp".equals(args[0])) speedTest( n,
  // "speedComp".equals(args[0]) );
