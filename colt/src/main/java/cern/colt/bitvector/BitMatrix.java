 ...
 *
 * <p>Individual bits can be examined, set, or cleared. Rectangular parts (boxes) can quickly be
 * extracted, copied and replaced. Quick iteration over boxes is provided by optimized internal
 * iterators (<tt>forEach()</tt> methods). One <code>BitMatrix</code> may be used to modify the
 * contents of another <code>BitMatrix</code> through logical AND, OR, XOR and other similar
 * operations.
 *
 * <p>Legal coordinates range from <tt>[0,0]</tt> to <tt>[columns()-1,rows()-1]</tt>. Any attempt to
 * access a bit at a coordinate <tt>column&lt;0 || column&gt;=columns() || row&lt;0 ||
 * row&gt;=rows()</tt> will throw an <tt>IndexOutOfBoundsException</tt>. Operations involving two
 * bit matrices (like AND, OR, XOR, etc.) will throw an <tt>IllegalArgumentException</tt> if both
 * bit matrices do not have the same number of columns and rows.
 *
 * <p>If you need extremely quick access to individual bits: Although getting and setting individual
 * bits with methods <tt>get(...)</tt> and <tt>put(...)</tt> is quick, it is even quicker (<b>but
 * not safe</b>) to use <tt>getQuick(...)</tt> and <tt>putQuick(...)</tt>.
 *
...
public class BitMatrix extends cern.colt.PersistentObject {
  ...
  protected void checkDimensionCompatibility(BitMatrix other) {
    if (columns != other.columns() || rows != other.rows())
      throw new IllegalArgumentException(
          "Incompatible dimensions: (columns,rows)=("
              + columns
              + ","
              + rows
   ...
   *
   * @throws IllegalArgumentException if <tt>columns &lt; 0 || rows &lt; 0 || columns*rows &gt;
   *     bits.length*64</tt>
   */
   ...
   * <code>true</code> as the receiver.
   *
   ...
   * @param procedure a procedure object taking as first argument the current column and as second
   *     argument the current row. Stops iteration if the procedure returns <tt>false</tt>,
   *     otherwise continues.
   * @return <tt>false</tt> if the procedure stopped before all elements where iterated over,
   *     <tt>true</tt> otherwise.
   */
  public boolean forEachCoordinateInState(
      boolean state, final cern.colt.function.IntIntProcedure procedure) {
    /*
   ...
   * is within bounds.</b> Precondition (unchecked): <tt>column&gt;=0 && column&lt;columns() &&
   * row&gt;=0 && row&lt;rows()</tt>.
   *
   * @param column the index of the column-coordinate.
   * @param row the index of the row-coordinate.
   ...
   * <tt>[column+width-1,row+height-1]</tt>, all inclusive. Does nothing if <tt>width &lt;= 0 ||
   * height &lt;= 0</tt>. If <tt>source==this</tt> and the source and destination box intersect in
   * an ambiguous way, then replaces as if using an intermediate auxiliary copy of the receiver.
   *
   * @param column the index of the column-coordinate.
   * @param row the index of the row-coordinate.
   * @param width the width of the box.
   ...
   * @throws IndexOutOfBoundsException if <tt>column&lt;0 || column+width&gt;columns() || row&lt;0
   *     || row+height&gt;rows()</tt>
   * @throws IndexOutOfBoundsException if <tt>sourceColumn&lt;0 ||
   *     sourceColumn+width&gt;source.columns() || sourceRow&lt;0 ||
   *     sourceRow+height&gt;source.rows()</tt>
   */
  public void replaceBoxWith(
      int column,
      int row,
      int width,
      int height,
   ...
   * not what you want, you should first use <tt>copy()</tt> to make sure both objects use separate
   * internal storage.
   */
