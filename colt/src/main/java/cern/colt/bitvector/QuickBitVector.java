 ...
 * parameters these method may return (or set) invalid values without throwing any exception. <b>You
 * should only use this class when performance is critical and you are absolutely sure that indexes
 * are within bounds.</b>
 *
 * <p>A bitvector is modelled as a long array, i.e. <tt>long[] bits</tt> holds bits of a bitvector.
 * Each long value holds 64 bits. The i-th bit is stored in bits[i/64] at bit position i % 64 (where
...
public class QuickBitVector extends Object {
   ...
   * return value are set to 0. If <tt>from &gt; to</tt> then returns zero (<tt>0L</tt>).
   ...
   * unaffected. If <tt>from &gt; to</tt> then does nothing. Precondition (not checked):
   * <tt>to-from+1 &lt;= 64</tt>.
   *
   * @param bits the bitvector.
  ...
  public static void putLongFromTo(long[] bits, long value, int from, int to) {
    ...
    */
    ...
    if (fromIndex
        == toIndex) { // range does not cross unit boundaries; should go into one single long value.
