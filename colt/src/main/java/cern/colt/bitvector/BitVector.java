 ...
 *
 * <p>Individual indexed bits can be examined, set, or cleared. Subranges can quickly be extracted,
 * copied and replaced. Quick iteration over subranges is provided by optimized internal iterators
 * (<tt>forEach()</tt> methods). One <code>BitVector</code> may be used to modify the contents of
 * another <code>BitVector</code> through logical AND, OR, XOR and other similar operations.
 *
 * <p>All operations consider the bits <tt>0..size()-1</tt> and nothing else. Operations involving
 * two bitvectors (like AND, OR, XOR, etc.) will throw an <tt>IllegalArgumentException</tt> if the
 * secondary bit vector has a size smaller than the receiver.
 *
 * <p>A <tt>BitVector</tt> is never automatically resized, but it can manually be grown or shrinked
 * via <tt>setSize(...)</tt>.
 *
 * <p>For use cases that need to store several bits per information entity, quick accessors are
 * provided that interpret subranges as 64 bit <tt>long</tt> integers.
 *
 * <p>Why this class? Fist, <tt>boolean[]</tt> take one byte per stored bit. This class takes one
 * bit per stored bit. Second, many applications find the semantics of <tt>java.util.BitSet</tt> not
 * particularly helpful for their needs. Third, operations working on all bits of a bitvector are
 * extremely quick. For example, on NT, Pentium Pro 200 Mhz, SunJDK1.2.2, java -classic, for two
 * bitvectors A,B (both much larger than processor cache), the following results are obtained.
 *
 * <ul>
 *   <li><tt>A.and(B)</tt> i.e. A = A & B --> runs at about 35 MB/sec
 *   <li><tt>A.cardinality()</tt>, i.e. determining the selectivity, the number of bits in state
 *       "true" --> runs at about 80 MB/sec
 *   <li>Similar performance for <tt>or, xor, andNot, not, copy, replace, partFromTo, indexOf,
 *       clear</tt> etc.
 * </ul>
 *
 * If you need extremely quick access to individual bits: Although getting and setting individual
 * bits with methods <tt>get(...)</tt>, <tt>set(...)</tt> and <tt>put(...)</tt>is quick, it is even
 * quicker (<b>but not safe</b>) to use <tt>getQuick(...)</tt> and <tt>putQuick(...)</tt> or even
 * <tt>QuickBitVector</tt>.
 *
 * <p><b>Note</b> that this implementation is not synchronized.
 *
...
public class BitVector extends cern.colt.PersistentObject {
  ...
  /*
   * The bits of this object. The ith bit is stored in bits[i/64] at bit position i % 64 (where bit
   ...
   * Constructs a bit vector with the given backing bits and size. <b>WARNING:</b> For efficiency
   * reasons and to keep memory usage low, <b>the array is not copied</b>. So if subsequently you
   * modify the specified array directly via the [] operator, be sure you know what you're doing.
   *
   * <p>A bitvector is modelled as a long array, i.e. <tt>long[] bits</tt> holds bits of a
   * bitvector. Each long value holds 64 bits. The i-th bit is stored in bits[i/64] at bit position
   * i % 64 (where bit position 0 refers to the least significant bit and 63 refers to the most
   * significant bit).
   *
  ...
  /*
   * Performs a logical <b>AND</b> of the receiver with another bit vector (A = A & B). The receiver
   * is modified so that a bit in it has the value <code>true</code> if and only if it already had
   * the value <code>true</code> and the corresponding bit in the other bit vector argument has the
  ...
  /*
   * Clears all of the bits in receiver whose corresponding bit is set in the other bitvector (A = A
   * \ B). In other words, determines the difference (A=A\B) between two bitvectors.
   *
  ...
  protected void checkSize(BitVector other) {
    if (nbits > other.size())
      throw new IllegalArgumentException(
          "Incompatible sizes: size=" + nbits + ", other.size()=" + other.size());
  }
  ...
  /*
   * Cloning this <code>BitVector</code> produces a new <code>BitVector</code> that is equal to it.
   * The clone of the bit vector is another bit vector that has exactly the same bits set to <code>
   * true</code> as this bit vector and the same current size, but independent state.
   *
  ...
  /*
   * You normally need not use this method. Use this method only if performance is critical. Returns
   * the bit vector's backing bits. <b>WARNING:</b> For efficiency reasons and to keep memory usage
   * low, <b>the array is not copied</b>. So if subsequently you modify the returned array directly
   ...
   * the bit vector's backing bits and size. <b>WARNING:</b> For efficiency reasons and to keep
   * memory usage low, <b>the array is not copied</b>. So if subsequently you modify the specified
   ...
   * if the argument is not <code>null</code> and is a <code>BitVector</code> object that has the
   * same size as the receiver and the same bits set to <code>true</code> as the receiver. That is,
   * for every nonnegative <code>int</code> index <code>k</code>,
   *
  ...
  public boolean equals(Object obj) {
    ...
    for (int i = fullUnits; --i >= 0; ) if (bits[i] != other.bits[i]) return false;
  ...
  /*
   * Applies a procedure to each bit index within the specified range that holds a bit in the given
   * state. Starts at index <tt>from</tt>, moves rightwards to <tt>to</tt>. Useful, for example, if
   * you want to copy bits into an image or somewhere else.
   *
   * <p>Optimized for speed. Particularly quick if one of the following conditions holds
   *
   * <ul>
   *   <li><tt>state==true</tt> and the receiver is sparse (<tt>cardinality()</tt> is small compared
   *       to <tt>size()</tt>).
   *   <li><tt>state==false</tt> and the receiver is dense (<tt>cardinality()</tt> is large compared
   *       to <tt>size()</tt>).
   * </ul>
   *
   ...
   * @param procedure a procedure object taking as argument the current bit index. Stops iteration
   *     if the procedure returns <tt>false</tt>, otherwise continues.
   * @return <tt>false</tt> if the procedure stopped before all elements where iterated over,
   *     <tt>true</tt> otherwise.
   * @throws IndexOutOfBoundsException if (<tt>size()&gt;0 && (from&lt;0 || from&gt;to ||
   *     to&gt;=size())</tt>).
   */
  public boolean forEachIndexFromToInState(
      int from, int to, boolean state, cern.colt.function.IntProcedure procedure) {
    /*
  ...
  /*
   * Returns a long value representing bits of the receiver from index <tt>from</tt> to index
   ...
   * the return value are set to 0. If <tt>to-from+1==0</tt> then returns zero (<tt>0L</tt>).
   *
   * @param from index of start bit (inclusive).
   * @param to index of end bit (inclusive).
   * @return the specified bits as long value.
   * @throws IndexOutOfBoundsException if <tt>from&lt;0 || from&gt;=size() || to&lt;0 ||
   *     to&gt;=size() || to-from+1<0 || to-from+1>64</tt>
   */
  public long getLongFromTo(int from, int to) {
    ...
    if (from < 0
        ...
        || to >= nbits
        || width < 0
        || width > QuickBitVector.BITS_PER_UNIT)
      throw new IndexOutOfBoundsException("from:" + from + ", to:" + to);
    return QuickBitVector.getLongFromTo(bits, from, to);
  }
  /*
   * Returns from the bitvector the value of the bit with the specified index; <b>WARNING:</b> Does
   * not check preconditions. The value is <tt>true</tt> if the bit with the index <tt>bitIndex</tt>
   * is currently set; otherwise, returns <tt>false</tt>.
   *
   * <p>Provided with invalid parameters this method may return invalid values without throwing any
   * exception. <b>You should only use this method when you are absolutely sure that the index is
   * within bounds.</b> Precondition (unchecked): <tt>bitIndex &gt;= 0 && bitIndex &lt; size()</tt>.
   *
   * @param bitIndex the bit index.
   * @return the value of the bit with the specified index.
   */
  ...
  /*
   * Returns a hash code value for the receiver. The hash code depends only on which bits have been
   * set within the receiver. The algorithm used to compute it may be described as follows.
   *
   * <p>Suppose the bits in the receiver were to be stored in an array of <code>long</code> integers
   * called, say, <code>bits</code>, in such a manner that bit <code>k</code> is set in the receiver
   * (for nonnegative values of <code>k</code>) if and only if the expression
   *
   * <pre>((k&gt;&gt;6) &lt; bits.length) && ((bits[k&gt;&gt;6] & (1L &lt;&lt; (bit & 0x3F))) != 0)
   * </pre>
   *
  ...
  /*
   * Returns the index of the first occurrence of the specified state. Returns <code>-1</code> if
   * the receiver does not contain this state. Searches between <code>from</code>, inclusive and
   * <code>to</code>, inclusive.
   *
   * <p>Optimized for speed. Preliminary performance (200Mhz Pentium Pro, JDK 1.2, NT): size=10^6,
   * from=0, to=size-1, receiver contains matching state in the very end --> 0.002 seconds elapsed
   * time.
   *
   * @param state state to search for.
   * @param from the leftmost search position, inclusive.
   * @param to the rightmost search position, inclusive.
   * @return the index of the first occurrence of the element in the receiver; returns <code>-1
   *     </code> if the element is not found.
   * @exception IndexOutOfBoundsException if (<tt>size()&gt;0 && (from&lt;0 || from&gt;to ||
   *     to&gt;=size())</tt>).
   */
  ...
  /*
   * Returns the number of bits used in the trailing PARTIAL unit. Returns zero if there is no such
  ...
  /*
   * Performs a logical <b>OR</b> of the receiver with another bit vector (A = A | B). The receiver
   * is modified so that a bit in it has the value <code>true</code> if and only if it either
   * already had the value <code>true</code> or the corresponding bit in the other bit vector
  ...
  /*
   * Constructs and returns a new bit vector which is a copy of the given range. The new bitvector
   * has <tt>size()==to-from+1</tt>.
   *
  ...
  public void put(int bitIndex, boolean value) {
    if (bitIndex < 0 || bitIndex >= nbits)
      throw new IndexOutOfBoundsException(String.valueOf(bitIndex));
    if (value) QuickBitVector.set(bits, bitIndex);
    else QuickBitVector.clear(bits, bitIndex);
  }
  /*
   * Sets bits of the receiver from index <code>from</code> to index <code>to</code> to the bits of
   ...
   *
   * <p>Provided with invalid parameters this method may set invalid values without throwing any
   * exception. <b>You should only use this method when you are absolutely sure that the index is
   * within bounds.</b> Precondition (unchecked): <tt>bitIndex &gt;= 0 && bitIndex &lt; size()</tt>.
   *
   * @param bitIndex the index of the bit to be changed.
   ...
   * Replaces the range <tt>[from,to]</tt> with the contents of the range
   * <tt>[sourceFrom,sourceFrom+to-from]</tt>, all inclusive. If <tt>source==this</tt> and the
   * source and destination range intersect in an ambiguous way, then replaces as if using an
   * intermediate auxiliary copy of the receiver.
   *
   * <p>Optimized for speed. Preliminary performance (200Mhz Pentium Pro, JDK 1.2, NT): replace 10^6
   * ill aligned bits --> 0.02 seconds elapsed time.
   *
   * @param from the start index within the receiver, inclusive.
   * @param to the end index within the receiver, inclusive.
   ...
   * @throws IndexOutOfBoundsException if <tt>size()&gt;0 && (from&lt;0 || from&gt;to ||
   *     to&gt;=size() || sourceFrom<0 || sourceFrom+to-from+1>source.size()))</tt>.
   */
  public void replaceFromToWith(int from, int to, BitVector source, int sourceFrom) {
    ...
    if (source.bits == this.bits
        && from <= sourceFrom
        && sourceFrom <= to) { // dangerous intersection
      source = source.copy();
    }
    ...
    */
    ...
    // This could be done even faster by implementing on even lower levels. But then the code would
    // probably become a "don't touch" piece.
    ...
    for (int i = blocks; --i >= 0; ) {
      long val =
          QuickBitVector.getLongFromTo(sourceBits, sourceFrom, sourceFrom + bitsPerUnitMinusOne);
   ...
   *
   * <p>Optimized for speed. Preliminary performance (200Mhz Pentium Pro, JDK 1.2, NT): replace 10^6
   * ill aligned bits --> 0.002 seconds elapsed time.
   *
  ...
  /*
   * Shrinks or expands the receiver so that it holds <tt>newSize</tt> bits. If the receiver is
   * expanded, additional <tt>false</tt> bits are added to the end. If the receiver is shrinked, all
   * bits between the old size and the new size are lost; their memory is subject to garbage
   * collection. (This method introduces a new backing array of elements. WARNING: if you have more
   * than one BitVector or BitMatrix sharing identical backing elements, be sure you know what you
   * are doing.)
   *
  ...
  /*
   * Returns a string representation of the receiver. For every index for which the receiver
   * contains a bit in the "set" (<tt>true</tt>) state, the decimal representation of that index is
   * included in the result. Such indeces are listed in order from lowest to highest, separated by
  ...
  /*
   * Performs a logical <b>XOR</b> of the receiver with another bit vector (A = A ^ B). The receiver
   * is modified so that a bit in it has the value <code>true</code> if and only if one of the
   * following statements holds:
   *
   * <ul>
   *   <li>The bit initially has the value <code>true</code>, and the corresponding bit in the
   *       argument has the value <code>false</code>.
   *   <li>The bit initially has the value <code>false</code>, and the corresponding bit in the
   *       argument has the value <code>true</code>.
   * </ul>
   *
   * @param other a bit vector.
