 ...
 *
 * <p>Numbers can be compressed when minimum and maximum of all values ever to be stored in the list
 * are known. For example, if min=16, max=27, only 4 bits are needed to store a value. No
 * compression is achieved for <tt>float</tt> and <tt>double</tt> values.
 *
 * <p>You can add, get and set elements quite similar to <tt>java.util.ArrayList</tt>.
 *
 * <p><b>Applicability:</b> Applicable if the data is non floating point, highly skewed without
 * "outliers" and minimum and maximum known in advance.
 *
 * <p><b>Performance:</b> Basic operations like <tt>add()</tt>, <tt>get()</tt>, <tt>set()</tt>,
 * <tt>size()</tt> and <tt>clear()</tt> are <tt>O(1)</tt>, i.e. run in constant time.
 * <dt>200Mhz Pentium Pro, JDK 1.2, NT:
 * <dt><tt>10^6</tt> calls to <tt>getQuick()</tt> --> <tt>0.5</tt> seconds. (50 times slower than
 *     reading from a primitive array of the appropriate type.)
 * <dt><tt>10^6</tt> calls to <tt>setQuick()</tt> --> <tt>0.8</tt> seconds. (15 times slower than
 *     writing to a primitive array of the appropriate type.)
 *
 *     <p>This class can, for example, be useful when making large lists of numbers persistent. Also
 *     useful when very large lists would otherwise consume too much main memory.
 *
 *     <p>Upon instantiation a contract is signed that defines the interval values may fall into. It
 *     is not legal to store values not contained in that interval. WARNING: The contract is not
 *     checked. Be sure you do not store illegal values. If you need to store <tt>float</tt> or
 *     <tt>double</tt> values, you must set the minimum and maximum to
 *     <tt>[Integer.MIN_VALUE,Integer.MAX_VALUE]</tt> or <tt>[Long.MIN_VALUE,Long.MAX_VALUE]</tt>,
 *     respectively.
 *
 *     <p>Although access methods are only defined on <tt>long</tt> values you can also store all
 *     other primitive data types: <tt>boolean</tt>, <tt>byte</tt>, <tt>short</tt>, <tt>int</tt>,
 *     <tt>long</tt>, <tt>float</tt>, <tt>double</tt> and <tt>char</tt>. You can do this by
 *     explicitly representing them as <tt>long</tt> values. Use casts for discrete data types. Use
 *     the methods of <tt>java.lang.Float</tt> and <tt>java.lang.Double</tt> for floating point data
 *     types: Recall that with those methods you can convert any floating point value to a
 *     <tt>long</tt> value and back <b>without losing any precision</b>:
 *
 *     <p><b>Example usage:</b>
 *
 *     <pre>
...
public class MinMaxNumberList extends cern.colt.list.AbstractLongList {
  ...
  public void addAllOfFromTo(long[] elements, int from, int to) {
    ...
    for (int times = to - from + 1; --times >= 0; ) {
      QuickBitVector.putLongFromTo(
          theBits, elements[i++] - min, firstBit, firstBit + bitsPerElemMinusOne);
   ...
   * the index is within bounds.</b> Precondition (unchecked): <tt>index &gt;= 0 && index &lt;
   * size()</tt>.
   *
   ...
   * </pre>
   */
  public void partFromTo(
      final int from,
      final int to,
      final BitVector qualificants,
      final int qualificantsFrom,
      long[] part,
      final int partFrom) {
    int width = to - from + 1;
    if (from < 0
   ...
   * you are absolutely sure that the index is within bounds.</b> Precondition (unchecked):
