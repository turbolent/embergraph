...
/**
 * Abstract base class for resizable lists holding <code>float</code> elements; abstract. First see
...
public abstract class AbstractFloatList extends AbstractList {
   ...
   * If the receiver contains multiple elements equal to the specified object, there is no guarantee
   ...
   * capacity.
   *
   * <p><b>WARNING:</b> For efficiency reasons and to keep memory usage low, this method may decide
   * <b>not to copy the array</b>. So if subsequently you modify the returned array directly via the
   ...
   * size()</tt>.
   *
   * <p>This method is normally only used internally in large loops where bounds are explicitly
   * checked before the loop and need no be rechecked within the loop. However, when desperately,
   * you can give this method <tt>public</tt> visibility in subclasses.
   *
   * @param index index of element to return.
   */
   ...
   * algorithm offers n*log(n) performance on many data sets that cause other quicksorts to degrade
  ...
  public void replaceFromToWithFromTo(
      int from, int to, AbstractFloatList other, int otherFrom, int otherTo) {
  ...
  public void replaceFromWith(int from, java.util.Collection other) {
    ...
    for (int i = 0; i < limit; i++) set(index++, ((Number) e.next()).floatValue()); // delta
  }
  /**
   ...
   *
   * <p>If your subclass overrides and delegates size changing methods to some other object, you
   * must make sure that those overriding methods not only update the size of the delegate but also
   * of this class. For example: public DatabaseList extends AbstractFloatList { ... public void
