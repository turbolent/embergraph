...
public class ObjectArrayList extends AbstractList {
   ...
   *
   * <p><b>WARNING:</b> For efficiency reasons and to keep memory usage low, <b>the array is not
   * copied</b>. So if subsequently you modify the specified array directly via the [] operator, be
   ...
   * @exception IndexOutOfBoundsException index is out of range (<tt>index &lt; 0 || index &gt;
   *     size()</tt>).
   */
   ...
   * multiple elements equal to the specified object, there is no guarantee which instance will be
   * found.
   *
   ...
   * vice versa; However, structurally modifying the copy (for example changing its size, setting
   * other objects at indexes, etc.) does not affect the receiver and vice versa.
   *
   * @return a copy of the receiver.
   */
   ...
   *
   * <p>Tests elements for equality or identity as specified by <tt>testForEquality</tt>. When
   * testing for equality, two elements <tt>e1</tt> and <tt>e2</tt> are <i>equal</i> if
   ...
   *
   * <p><b>WARNING:</b> For efficiency reasons and to keep memory usage low, <b>the array is not
   * copied</b>. So if subsequently you modify the returned array directly via the [] operator, be
   * sure you know what you're doing.
   *
   ...
   * defined to be equal if they contain the same elements in the same order. Two elements
   ...
   *
   * <p>Tests for equality or identity as specified by testForEquality.
   *
   ...
   *
   * <p>Tests for equality or identity as specified by <code>testForEquality</code>.
   *
   * @param element element to search for.
   * @param from the leftmost search position, inclusive.
   * @param to the rightmost search position, inclusive.
   ...
   * @param testForEquality if <code>true</code> -> test for equality, otherwise for identity.
   * @return the index of the last occurrence of the element in the receiver; returns <code>-1
   *     </code> if the element is not found.
   */
   ...
   * than part B). Parts may overlap. Receiver and other list may (but most not) be identical. If
   * <code>from &gt; to</code>, then inserts other part before <code>from</code>.
   *
  ...
  public void replaceFromToWithFromTo(
      int from, int to, ObjectArrayList other, int otherFrom, int otherTo) {
   ...
   *
   * <p>If the receiver fits in the specified array with room to spare (i.e., the array has more
   * elements than the receiver), the element in the array immediately following the end of the
   * receiver is set to null. This is useful in determining the length of the receiver <em>only</em>
   * if the caller knows that the receiver does not contain any null elements.
   *
   * @param array the array into which the elements of the receiver are to be stored, if it is big
