 ...
 *
 * <p><b>Applicability:</b> Applicable if data is highly skewed and legal values are known in
 * advance. Robust in the presence of "outliers".
 *
 * <p><b>Performance:</b> Operations <tt>get()</tt>, <tt>size()</tt> and <tt>clear()</tt> are
 * <tt>O(1)</tt>, i.e. run in constant time. Operations like <tt>add()</tt> and <tt>set()</tt> are
 * <tt>O(log(distinctValues.length))</tt>.
 *
 * <p>Upon instantiation a contract is signed that defines the distinct values allowed to be hold in
 * this list. It is not legal to store elements other than specified by the contract. Any attempt to
 * violate the contract will throw an <tt>IllegalArgumentException</tt>.
 *
 * <p>Although access methods are only defined on <tt>long</tt> values you can also store all other
 * primitive data types: <tt>boolean</tt>, <tt>byte</tt>, <tt>short</tt>, <tt>int</tt>,
 * <tt>long</tt>, <tt>float</tt>, <tt>double</tt> and <tt>char</tt>. You can do this by explicitly
 * representing them as <tt>long</tt> values. Use casts for discrete data types. Use the methods of
 * <tt>java.lang.Float</tt> and <tt>java.lang.Double</tt> for floating point data types: Recall that
 * with those methods you can convert any floating point value to a <tt>long</tt> value and back
 * <b>without losing any precision</b>:
 *
 * <p><b>Example usage:</b>
 *
 * <pre>
...
public class DistinctNumberList extends cern.colt.list.AbstractLongList {
   ...
   *
   * @param distinctValues an array sorted ascending containing the distinct values allowed to be
   *     hold in this list.
