...
public class Statistic extends Object {
  ...
  // public static DoubleMatrix2D aggregate(DoubleMatrix2D matrix, hep.aida.bin.BinFunction1D[]
  // aggr, DoubleMatrix2D result) {
  ...
  */
   ...
   * diagonal cells (i.e. the correlation between a variable and itself) will equal 1, for the
   * simple reason that the correlation coefficient of a variable with itself equals 1. The
   * correlation of two column vectors x and y is given by <tt>corr(x,y) = cov(x,y) /
   ...
   * another def</A>. Compares two column vectors at a time. Use dice views to compare two row
   * vectors at a time.
   *
   * @param covariance a covariance matrix, as, for example, returned by method {@link
   *     #covariance(DoubleMatrix2D)}.
   ...
   * cells (i.e. the covariance between a variable and itself) will equal the variances. The
   * covariance of two column vectors x and y is given by <tt>cov(x,y) = (1/n) * Sum((x[i]-mean(x))
   ...
   * product by region.
   *
   * <p>Computes the distinct values of x and y, yielding histogram axes that capture one distinct
   * value per bin. Then fills the histogram.
   *
   * <p>Example output:
   *
   * <table>
   ...
   * @throws IllegalArgumentException if <tt>x.size() != y.size() || y.size() !=
   *     weights.size()</tt>.
   */
  // FIXME:   Should this exist without hep.aida
  // public static hep.aida.IHistogram2D cube(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D
  // weights) {
  //	if (x.size() != y.size() || y.size() != weights.size()) throw new
  // IllegalArgumentException("vectors must have same size");
  //
  ...
  //	// since bins are right-open [from,to) we need an additional dummy bin so that the last
  // distinct value does not fall into the overflow bin
   ...
   *
   * <p>Computes the distinct values of x and y and z, yielding histogram axes that capture one
   * distinct value per bin. Then fills the histogram.
   *
   * @return the histogram containing the cube.
   * @throws IllegalArgumentException if <tt>x.size() != y.size() || x.size() != z.size() ||
   *     x.size() != weights.size()</tt>.
   */
  // FIXME:  Should this test exist without hep.aida
  // public static hep.aida.IHistogram3D cube(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D z,
  // DoubleMatrix1D weights) {
  //	if (x.size() != y.size() || x.size() != z.size() || x.size() != weights.size()) throw new
  // IllegalArgumentException("vectors must have same size");
  ...
  //	sorted.sort();
  //	cern.jet.stat.Descriptive.frequencies(sorted, distinct, null);
  // since bins are right-open [from,to) we need an additional dummy bin so that the last distinct
  // value does not fall into the overflow bin
  //	if (distinct.size()>0) distinct.add(distinct.get(distinct.size()-1) + epsilon);
  //	distinct.trimToSize();
  //	hep.aida.IAxis xaxis = new hep.aida.ref.VariableAxis(distinct.elements());
   ...
   * (i.e. the distance between a variable and itself) will be zero. Compares two column vectors at
   * a time. Use dice views to compare two row vectors at a time.
   *
   * @param matrix any matrix; a column holds the values of a given variable (vector).
   * @param distanceFunction (EUCLID, CANBERRA, ..., or any user defined distance function operating
   *     on two vectors).
   * @return the distance matrix (<tt>n x n, n=matrix.columns</tt>).
   */
  public static DoubleMatrix2D distance(
      DoubleMatrix2D matrix, VectorVectorFunction distanceFunction) {
    int columns = matrix.columns();
   ...
   */
  // FIXME:  Should this test exist without hep.aida
  // public static hep.aida.IHistogram1D histogram(hep.aida.IHistogram1D histo, DoubleMatrix1D
  // vector) {
   ...
   */
  // FIXME:  Should this test exist without hep.aida
  // public static hep.aida.IHistogram2D histogram(hep.aida.IHistogram2D histo, DoubleMatrix1D x,
  // DoubleMatrix1D y) {
  ...
  // FIXME:  Should this test exist without hep.aida
  // public static hep.aida.IHistogram2D histogram(hep.aida.IHistogram2D histo, DoubleMatrix1D x,
  // DoubleMatrix1D y, DoubleMatrix1D weights) {
  //	if (x.size() != y.size() || y.size() != weights.size()) throw new
  // IllegalArgumentException("vectors must have same size");
  //	for (int i=x.size(); --i >= 0; ) {
   ...
   */
  // FIXME:  Should this test exist without hep.aida
  // public static hep.aida.IHistogram3D histogram(hep.aida.IHistogram3D histo, DoubleMatrix1D x,
  // DoubleMatrix1D y, DoubleMatrix1D z, DoubleMatrix1D weights) {
  //	if (x.size() != y.size() || x.size() != z.size() || x.size() != weights.size()) throw new
  // IllegalArgumentException("vectors must have same size");
  //	for (int i=x.size(); --i >= 0; ) {
   ...
   * @param randomGenerator a uniform random number generator; set this parameter to <tt>null</tt>
   *     to use a default generator seeded with the current time.
   * @return the sampling view.
   * @throws IllegalArgumentException if <tt>! (0 <= rowFraction <= 1 && 0 <= columnFraction <=
   *     1)</tt>.
   * @see cern.jet.random.sampling.RandomSampler
   */
  public static DoubleMatrix1D viewSample(
      DoubleMatrix1D matrix, double fraction, RandomEngine randomGenerator) {
    ...
    if (randomGenerator == null)
      randomGenerator =
          new cern.jet.random.engine.MersenneTwister((int) System.currentTimeMillis());
   ...
   * @see cern.jet.random.sampling.RandomSampler
   */
  public static DoubleMatrix2D viewSample(
   ...
   *     to use a default generator seeded with the current time.
   * @return the sampling view.
   * @throws IllegalArgumentException if <tt>! (0 <= sliceFraction <= 1 && 0 <= rowFraction <= 1 &&
   *     0 <= columnFraction <= 1)</tt>.
   * @see cern.jet.random.sampling.RandomSampler
   */
  public static DoubleMatrix3D viewSample(
