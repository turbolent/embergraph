 ...
 * yet very efficient APIs. The sorts return convenient <i>sort views</i>. This enables the usage of
 * algorithms which scale well with the problem size: For example, sorting a 1000000 x 10000 or a
 * 1000000 x 100 x 100 matrix performs just as fast as sorting a 1000000 x 1 matrix. This is so,
 * because internally the algorithms only move around integer indexes, they do not physically move
 * around entire rows or slices. The original matrix is left unaffected.
 *
 * <p>The quicksort is a derivative of the JDK 1.2 V1.26 algorithms (which are, in turn, based on
 * Bentley's and McIlroy's fine work). The mergesort is a derivative of the JAL algorithms, with
 * optimisations taken from the JDK algorithms. Mergesort is <i>stable</i> (by definition), while
 * quicksort is not. A stable sort is, for example, helpful, if matrices are sorted successively by
 * multiple columns. It preserves the relative position of equal elements.
 *
...
public class Sorting extends cern.colt.PersistentObject {
  ...
  public static final Sorting mergeSort = new Sorting() { // override quicksort with mergesort
        ...
        protected void runSort(
            int fromIndex, int toIndex, IntComparator c, cern.colt.Swapper swapper) {
   ...
   * @return a new sorted vector (matrix) view. <b>Note that the original matrix is left
   *     unaffected.</b>
   */
   ...
   * determinining whether one is smaller, equal or larger than the other. To sort ranges use
   * sub-ranging views. To sort descending, use flip views ...
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   * @return a new matrix view sorted as specified. <b>Note that the original vector (matrix) is
   *     left unaffected.</b>
   */
  public DoubleMatrix1D sort(
      final DoubleMatrix1D vector, final cern.colt.function.DoubleComparator c) {
    int[] indexes = new int[vector.size()]; // row indexes to reorder instead of matrix itself
    for (int i = indexes.length; --i >= 0; ) indexes[i] = i;
   ...
   * matrix values in the virtual column <tt>aggregates</tt>; Particularly efficient when comparing
   * expensive aggregates, because aggregates need not be recomputed time and again, as is the case
   * for comparator based sorts. Essentially, this algorithm makes expensive comparisons cheap.
   * Normally each element of <tt>aggregates</tt> is a summary measure of a row. Speedup over
   * comparator based sorting = <tt>2*log(rows)</tt>, on average. For this operation, quicksort is
   * usually faster.
   *
   * <p>The returned view is backed by this matrix, so changes in the returned view are reflected in
   * this matrix, and vice-versa. To sort ranges use sub-ranging views. To sort columns by rows, use
   * dice views. To sort descending, use flip views ...
   *
   * <p><b>Example:</b> Each aggregate is the sum of a row
   *
   * <table border="1" cellspacing="0">
   * <tr nowrap>
   ...
   * @return a new matrix view having rows sorted. <b>Note that the original matrix is left
   *     unaffected.</b>
   ...
   * @return a new matrix view having rows sorted by the given column. <b>Note that the original
  ...
  public DoubleMatrix2D sort(DoubleMatrix2D matrix, int column) {
    if (column < 0 || column >= matrix.columns())
      throw new IndexOutOfBoundsException(
          "column=" + column + ", matrix=" + Formatter.shape(matrix));
   ...
   * one is smaller, equal or larger than the other. To sort ranges use sub-ranging views. To sort
   * columns by rows, use dice views. To sort descending, use flip views ...
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   * @param matrix the matrix to be sorted.
   * @param c the comparator to determine the order.
   * @return a new matrix view having rows sorted as specified. <b>Note that the original matrix is
   *     left unaffected.</b>
   */
  ...
  /*
   * Sorts the matrix rows into ascending order, according to the <i>natural ordering</i> of the
   * values computed by applying the given aggregation function to each row; Particularly efficient
   * when comparing expensive aggregates, because aggregates need not be recomputed time and again,
   * as is the case for comparator based sorts. Essentially, this algorithm makes expensive
   * comparisons cheap. Normally <tt>aggregates</tt> defines a summary measure of a row. Speedup
   * over comparator based sorting = <tt>2*log(rows)</tt>, on average.
   *
   * <p>The returned view is backed by this matrix, so changes in the returned view are reflected in
   * this matrix, and vice-versa. To sort ranges use sub-ranging views. To sort columns by rows, use
   * dice views. To sort descending, use flip views ...
  ...
  // a bit clumsy, because Statistic.aggregate(...) is defined on columns, so we need to transpose
  // views
   ...
   * ranges use sub-ranging views. To sort by other dimensions, use dice views. To sort descending,
   * use flip views ...
   *
   * <p>The algorithm compares two 2-d slices at a time, determinining whether one is smaller, equal
   * or larger than the other. Comparison is based on the cell <tt>[row,column]</tt> within a slice.
   ...
   * @param column the index of the column inducing the order.
   * @return a new matrix view having slices sorted by the values of the slice view
   *     <tt>matrix.viewRow(row).viewColumn(column)</tt>. <b>Note that the original matrix is left
   *     unaffected.</b>
   * @throws IndexOutOfBoundsException if <tt>row < 0 || row >= matrix.rows() || column < 0 ||
   *     column >= matrix.columns()</tt>.
   */
   ...
   * determinining whether one is smaller, equal or larger than the other. To sort ranges use
   * sub-ranging views. To sort by other dimensions, use dice views. To sort descending, use flip
   * views ...
   *
   * <p><b>Example:</b>
   *
   ...
   * @param matrix the matrix to be sorted.
   * @param c the comparator to determine the order.
   * @return a new matrix view having slices sorted as specified. <b>Note that the original matrix
   *     is left unaffected.</b>
   */
  public DoubleMatrix3D sort(final DoubleMatrix3D matrix, final DoubleMatrix2DComparator c) {
    int[] sliceIndexes = new int[matrix.slices()]; // indexes to reorder instead of matrix itself
    for (int i = sliceIndexes.length; --i >= 0; ) sliceIndexes[i] = i;

    final DoubleMatrix2D[] views =
        new DoubleMatrix2D[matrix.slices()]; // precompute views for speed
  ...
  public static void zdemo5(int rows, int columns, boolean print) {
    Sorting sort = quickSort;
    // for reliable benchmarks, call this method twice: once with small dummy parameters to "warm
    // up" the jitter, then with your real work-load
    ...
    if (print) {
      ...
      System.out.println(
          "first part of sorted result = \n"
              + new cern.colt.matrix.doublealgo.Formatter("%G")
                  .toTitleString(
