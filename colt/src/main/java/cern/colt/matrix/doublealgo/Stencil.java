 ...
 * pass between convergence checks. Always does two iterations at a time for efficiency. These class
 * is for convencience and efficiency.
 *
...
public class Stencil extends Object {
   ...
   *     <tt>hasConverged.apply(A)==true</tt>. Set this parameter to <tt>null</tt> to indicate that
   *     no convergence checks shall be made.
   * @param convergenceIterations the number of iterations to pass between each convergence check.
   *     (Since a convergence may be expensive, you may want to do it only every 2,4 or 8
   *     iterations.)
   * @return the number of iterations actually executed.
   */
  public static int stencil27(
      DoubleMatrix3D A,
      cern.colt.function.Double27Function function,
      int maxIterations,
