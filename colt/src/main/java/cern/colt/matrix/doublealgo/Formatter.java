 ...
 * happy with the default formatting. This class is for advanced requirements.
 *
 * <p>Can't exactly remember the syntax of printf format strings? See {@link corejava.Format} or <a
 * href="http://www.braju.com/docs/index.html">Henrik Nordberg's documentation</a>, or the <a
 * href="http://www.dinkumware.com/htm_cl/lib_prin.html#Print%20Functions">Dinkumware's C Library
 * Reference</a>.
 *
 * <p><b>Examples:</b>
 *
 * <p>Examples demonstrate usage on 2-d matrices. 1-d and 3-d matrices formatting works very
 * similar.
 *
 ...
 * </table>
 *
 * <p>Here are some more elaborate examples, adding labels for axes, rows, columns, title and some
 ...
 * </table>
 *
 * <p>A column can be broader than specified by the parameter <tt>minColumnWidth</tt> (because a
 * cell may not fit into that width) but a column is never smaller than <tt>minColumnWidth</tt>.
 * Normally one does not need to specify <tt>minColumnWidth</tt> (default is <tt>1</tt>). This
 * parameter is only interesting when wanting to print two distinct matrices such that both matrices
 * have the same column width, for example, to make it easier to see which column of matrix A
 * corresponds to which column of matrix B.
 *
 * <p><b>Implementation:</b>
 *
 * <p>Note that this class is by no means ment to be used for high performance I/O (serialization is
 * much quicker). It is ment to produce well human readable output.
 *
 * <p>Analyzes the entire matrix before producing output. Each cell is converted to a String as
 * indicated by the given C-like format string. If <tt>null</tt> is passed as format string, {@link
 * java.lang.Double#toString(double)} is used instead, yielding full precision.
 *
 * <p>Next, leading and trailing whitespaces are removed. For each column the maximum number of
 * characters before and after the decimal point is determined. (No problem if decimal points are
 * missing). Each cell is then padded with leading and trailing blanks, as necessary to achieve
 * decimal point aligned, left justified formatting.
 *
...
public class Formatter extends AbstractFormatter {
  ...
  public static void demo2() {
    // parameters
    double[] values = {
      // 5, 0.0, -0.0, -Double.NaN, Double.NaN, 0.0/0.0, Double.NEGATIVE_INFINITY,
  ...
  public static void demo4() {
    ...
    */
    ...
    String[] rowNames = {
      "SunJDK1.2.2 classic", "IBMJDK1.1.8", "SunJDK1.3 Hotspot", "other1", "other2"
    };
  ...
  public static void demo7() {
    ...
    */
    ...
    // hep.aida.bin.BinFunction1D[] aggr = {F.mean, F.rms, F.quantile(0.25),
    // F.median,F.quantile(0.75), F.stdDev, F.min, F.max};
    String format = "%1.2G";

    // String[] columnNames = { "W", "X", "Y", "Z", "mean", "median", "sum"};
    // String[] rowNames = { "SunJDK1.2.2 classic", "IBMJDK1.1.8", "SunJDK1.3 Hotspot", "other1",
    // "other2", "mean", "median", "sum" };
    ...
    // Formatter().toSourceCode(cern.colt.matrix.DoubleFactory2D.dense.make(values)));
    // System.out.println(new
    // Formatter().toString(cern.colt.matrix.DoubleFactory2D.dense.make(values)));
    // System.out.println(new
    // Formatter().toTitleString(cern.colt.matrix.DoubleFactory2D.dense.make(values),rowNames,columnNames,rowAxisName,columnAxisName,title));
    // FIXME:  Should this test exist without hep.aida
    System.out.println("FIXME:  Should this test exist without hep.aida");
    // System.out.println(new
    // Formatter(format).toTitleString(cern.colt.matrix.DoubleFactory2D.dense.make(values),rowNames,columnNames,rowAxisName,columnAxisName,title, aggr));
   ...
   */
  protected String toTitleString(
    ...
    return new cern.colt.matrix.objectalgo.Formatter()
        .toTitleString(
            cern.colt.matrix.ObjectFactory2D.dense.make(s),
            rowNames,
            columnNames,
            rowAxisName,
            columnAxisName,
  ...
  /**
   * Same as <tt>toTitleString</tt> except that additionally statistical aggregates (mean, median,
   * sum, etc.) of rows and columns are printed. Pass <tt>null</tt> to one or more parameters to
   * indicate that the corresponding decoration element shall not appear in the string converted
   * matrix.
   *
   * @param matrix The matrix to format.
   * @param rowNames The headers of all rows (to be put to the left of the matrix).
  ...
  // public String toTitleString(DoubleMatrix2D matrix, String[] rowNames, String[] columnNames,
  // String rowAxisName, String columnAxisName, String title, hep.aida.bin.BinFunction1D[] aggr) {
  //	if (matrix.size()==0) return "Empty matrix";
  //	if (aggr==null || aggr.length==0) return
  // toTitleString(matrix,rowNames,columnNames,rowAxisName,columnAxisName,title);
  //
  //	DoubleMatrix2D rowStats = matrix.like(matrix.rows(), aggr.length); // hold row aggregations
  //	DoubleMatrix2D colStats = matrix.like(aggr.length, matrix.columns()); // hold column
  // aggregations
  //
  //	cern.colt.matrix.doublealgo.Statistic.aggregate(matrix, aggr, colStats); // aggregate an entire
  // column at a time
  //	cern.colt.matrix.doublealgo.Statistic.aggregate(matrix.viewDice(), aggr, rowStats.viewDice());
  // // aggregate an entire row at a time
  //
  //	// turn into strings
  //	// tmp holds "matrix" plus "colStats" below (needed so that numbers in a columns can be decimal
  // point aligned)
  ...
  //	cern.colt.matrix.ObjectMatrix2D allStats =
  // cern.colt.matrix.ObjectFactory2D.dense.make(matrix.rows()+aggr.length,
  ...
  //	s1 = null; s2 = null;
  //
  //	// append a vertical "|" separator plus names of aggregation functions to line holding
  // columnNames
  //	if (columnNames!=null) {
  ...
  //	String s = new cern.colt.matrix.objectalgo.Formatter().toTitleString(allStats,
   ...
   */
  // FIXME:  Should this exist without hep.aida
  // public String toTitleString(DoubleMatrix3D matrix, String[] sliceNames, String[] rowNames,
  // String[] columnNames, String sliceAxisName, String rowAxisName, String columnAxisName, String
  // title, hep.aida.bin.BinFunction1D[] aggr) {
  //	if (matrix.size()==0) return "Empty matrix";
  //	StringBuffer buf = new StringBuffer();
