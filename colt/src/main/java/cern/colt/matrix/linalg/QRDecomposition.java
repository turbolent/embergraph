 ...
 * solution of nonsquare systems of simultaneous linear equations. This will fail if
 * <tt>isFullRank()</tt> returns <tt>false</tt>.
 */
public class QRDecomposition implements java.io.Serializable {
   ...
   * @return a decomposition object to access <tt>R</tt> and the Householder vectors <tt>H</tt>, and
   *     to compute <tt>Q</tt>.
  ...
  public DoubleMatrix2D getR() {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i < j) R.setQuick(i, j, QR.getQuick(i, j));
        else if (i == j) R.setQuick(i, j, Rdiag.getQuick(i));
        else R.setQuick(i, j, 0);
      }
    }
    return R;
  }
   ...
   * @exception IllegalArgumentException if <tt>!this.hasFullRank()</tt> (<tt>A</tt> is rank
   *     deficient).
   */
