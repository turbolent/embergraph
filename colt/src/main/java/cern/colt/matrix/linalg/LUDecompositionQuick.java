 ...
 * hence also diagonal) matrices, as well as a solver tuned for vectors. Its disadvantage is that it
 * is a bit more difficult to use than <tt>LUDecomposition</tt>. Thus, you may want to disregard
 * this class and come back later, if a need for speed arises.
 *
 * <p>An instance of this class remembers the result of its last decomposition. Usage pattern is as
 * follows: Create an instance of this class, call a decompose method, then retrieve the
 * decompositions, determinant, and/or solve as many equation problems as needed. Once another
 * matrix needs to be LU-decomposed, you need not create a new instance of this class. Start again
 * by calling a decompose method, then retrieve the decomposition and/or solve your equations, and
 * so on. In case a <tt>LU</tt> matrix is already available, call method <tt>setLU</tt> instead of
 * <tt>decompose</tt> and proceed with solving et al.
 *
 * <p>If a matrix shall not be overriden, use <tt>matrix.copy()</tt> and hand the the copy to
 * methods.
 *
 * <p>For an <tt>m x n</tt> matrix <tt>A</tt> with <tt>m >= n</tt>, the LU decomposition is an <tt>m
 * x n</tt> unit lower triangular matrix <tt>L</tt>, an <tt>n x n</tt> upper triangular matrix
 ...
 *
 * <p>The LU decomposition with pivoting always exists, even if the matrix is singular, so the
 * decompose methods will never fail. The primary use of the LU decomposition is in the solution of
 * square systems of simultaneous linear equations. Attempting to solve such a system will throw an
 * exception if <tt>isNonsingular()</tt> returns false.
 *
 * <p>
 */
public class LUDecompositionQuick implements java.io.Serializable {
   ...
   * @exception IllegalArgumentException if </tt>B.rows() != A.rows()</tt>.
   * @exception IllegalArgumentException if A is singular, that is, if
