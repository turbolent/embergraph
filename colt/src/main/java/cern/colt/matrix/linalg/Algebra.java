...
public class Algebra extends cern.colt.PersistentObject {
   ...
   *
   * <p>Note that this Algebra object is immutable. Any attempt to assign a new Property object to
   * it (via method <tt>setProperty</tt>), or to alter the tolerance of its property object (via
  ...
  static {
    // don't use new Algebra(Property.DEFAULT.tolerance()), because then property object would be
    // mutable.
    DEFAULT = new Algebra();
   ...
   * @return <tt>C</tt>; a new matrix holding the results, with <tt>C.rows()=A.rows(),
   *     C.columns()==B.columns()</tt>.
   ...
   * @param A the matrix to hold the results. Set this parameter to <tt>null</tt> to indicate that a
   *     new result matrix shall be constructed.
  ...
  public DoubleMatrix2D multOuter(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix2D A) {
    ...
    for (int column = columns; --column >= 0; )
      A.viewColumn(column).assign(x, cern.jet.math.Functions.mult);
    return A;
  }
   ...
   * @param indexes the permutation indexes, must satisfy <tt>indexes.length==A.size() && indexes[i]
   *     >= 0 && indexes[i] < A.size()</tt>;
   * @param work the working storage, must satisfy <tt>work.length >= A.size()</tt>; set
   ...
   * matrix, and vice-versa. Use idioms like <tt>result = permute(...).copy()</tt> to generate an
   ...
   * @param indexes the permutation indexes, must satisfy <tt>indexes.length==A.columns() &&
   *     indexes[i] >= 0 && indexes[i] < A.columns()</tt>;
   * @param work the working storage, must satisfy <tt>work.length >= A.columns()</tt>; set
   *     <tt>work==null</tt> if you don't care about performance.
   * @return the modified <tt>A</tt> (for convenience only).
   ...
   *
   * @param A the matrix to permute.
   * @param indexes the permutation indexes, must satisfy <tt>indexes.length==A.rows() && indexes[i]
   *     >= 0 && indexes[i] < A.rows()</tt>;
   * @param work the working storage, must satisfy <tt>work.length >= A.rows()</tt>; set
   *     <tt>work==null</tt> if you don't care about performance.
   * @return the modified <tt>A</tt> (for convenience only).
   ...
   *
   * <ul>
   *   <li><tt>p &gt;= 1: B = A*A*...*A</tt>.
   *   <li><tt>p == 0: B = identity matrix</tt>.
   *   <li><tt>p &lt; 0: B = pow(inverse(A),-p)</tt>.
   * </ul>
   *
  ...
  public DoubleMatrix2D pow(DoubleMatrix2D A, int p) {
    ...
    Blas blas =
        SmpBlas.smpBlas; // for parallel matrix mult; if not initialized defaults to sequential blas
    ...
    if (p == 2) {
      blas.dgemm(
          false, false, 1, A, A, 0, T); // mult(A,A); // safes one auxiliary matrix allocation
      return T;
    }

    int k =
        cern.colt.bitvector.QuickBitVector.mostSignificantBit(
            p); // index of highest bit in state "true"
   ...
   * @throws UnsupportedOperationException if <tt>this==DEFAULT && property!=this.property()</tt> -
   ...
   * matrix, so changes in the returned matrix are <i>not reflected</i> in this matrix, and
   * vice-versa.
   *
   ...
   * @return a new sub matrix; with <tt>sub.rows()==rowIndexes.length;
   *     sub.columns()==columnTo-columnFrom+1</tt>.
   * @throws IndexOutOfBoundsException if <tt>columnFrom<0 || columnTo-columnFrom+1<0 ||
   *     columnTo+1>matrix.columns() || for any row=rowIndexes[i]: row < 0 || row >=
   *     matrix.rows()</tt>.
   */
  private DoubleMatrix2D subMatrix(
      DoubleMatrix2D A, int[] rowIndexes, int columnFrom, int columnTo) {
   ...
   * <i>not reflected</i> in this matrix, and vice-versa.
   *
   * @param A the source matrix to copy from.
   ...
   * @return a new sub matrix; with <tt>sub.rows()==rowTo-rowFrom+1;
   *     sub.columns()==columnIndexes.length</tt>.
   * @throws IndexOutOfBoundsException if <tt>rowFrom<0 || rowTo-rowFrom+1<0 ||
   *     rowTo+1>matrix.rows() || for any col=columnIndexes[i]: col < 0 || col >=
   *     matrix.columns()</tt>.
   */
   ...
   * <tt>result = subMatrix(...).copy()</tt> to generate an independent sub matrix.
   *
   * @param A the source matrix.
   ...
   * @throws IndexOutOfBoundsException if <tt>fromColumn<0 || toColumn-fromColumn+1<0 ||
   *     toColumn>=A.columns() || fromRow<0 || toRow-fromRow+1<0 || toRow>=A.rows()</tt>
   */
  public DoubleMatrix2D subMatrix(
      DoubleMatrix2D A, int fromRow, int toRow, int fromColumn, int toColumn) {
   ...
   * transpose(A).copy()</tt> to generate an independent matrix.
   *
   * <p><b>Example:</b>
   *
   * <table border="0">
