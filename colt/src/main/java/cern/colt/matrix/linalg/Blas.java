 ...
 *
 * <p>Mostly for compatibility with legacy notations. Most operations actually just delegate to the
 * appropriate methods directly defined on matrices and vectors.
 *
 * <p>This class implements the BLAS functions for operations on matrices from the matrix package.
 * It follows the spirit of the <A HREF="http://math.nist.gov/javanumerics/blas.html">Draft Proposal
 * for Java BLAS Interface</A>, by Roldan Pozo of the National Institute of Standards and
 * Technology. Interface definitions are also identical to the Ninja interface. Because the matrix
 * package supports sections, the interface is actually simpler.
 *
 * <p>Currently, the following operations are supported:
 *
 * <ol>
 *   <li>BLAS Level 1: Vector-Vector operations
 *       <ul>
 *         <li>ddot : dot product of two vectors
 *         <li>daxpy : scalar times a vector plus a vector
 *         <li>drotg : construct a Givens plane rotation
 *         <li>drot : apply a plane rotation
 *         <li>dcopy : copy vector X into vector Y
 *         <li>dswap : interchange vectors X and Y
 *         <li>dnrm2 : Euclidean norm of a vector
 *         <li>dasum : sum of absolute values of vector components
 *         <li>dscal : scale a vector by a scalar
 *         <li>idamax: index of element with maximum absolute value
 *       </ul>
 *   <li>2.BLAS Level 2: Matrix-Vector operations
 *       <ul>
 *         <li>dgemv : matrix-vector multiply with general matrix
 *         <li>dger : rank-1 update on general matrix
 *         <li>dsymv : matrix-vector multiply with symmetric matrix
 *         <li>dtrmv : matrix-vector multiply with triangular matrix
 *       </ul>
 *   <li>3.BLAS Level 3: Matrix-Matrix operations
 *       <ul>
 *         <li>dgemm : matrix-matrix multiply with general matrices
 *       </ul>
 * </ol>
 *
...
public interface Blas {
   ...
   * @throws IllegalArgumentException if <tt>A.columns() != B.columns() || A.rows() !=
   *     B.rows()</tt>.
   */
  ...
  /**
   * Returns the dot product of two vectors x and y, which is <tt>Sum(x[i]*y[i])</tt>. In fact
  ...
  /**
   * Generalized linear algebraic matrix-matrix multiply; <tt>C = alpha*A*B + beta*C</tt>. In fact
   * equivalent to <tt>A.zMult(B,C,alpha,beta,transposeA,transposeB)</tt>. Note: Matrix shape
  ...
  public void dgemm(
      ...
      DoubleMatrix2D C);
  /**
   * Generalized linear algebraic matrix-vector multiply; <tt>y = alpha*A*x + beta*y</tt>. In fact
   * equivalent to <tt>A.zMult(x,y,alpha,beta,transposeA)</tt>. Note: Matrix shape conformance is
   * checked <i>after</i> potential transpositions.
   *
   * @param transposeA set this flag to indicate that the multiplication shall be performed on A'.
   * @param alpha a scale factor.
   * @param A the source matrix.
  ...
  /**
   * Constructs a Givens plane rotation for <tt>(a,b)</tt>. Taken from the LINPACK translation from
   * FORTRAN to Java, interface slightly modified. In the LINPACK listing DROTG is attributed to
   * Jack Dongarra
   *
  ...
  /**
   * Symmetric matrix-vector multiplication; <tt>y = alpha*A*x + beta*y</tt>. Where alpha and beta
   * are scalars, x and y are n element vectors and A is an n by n symmetric matrix. A can be in
   * upper or lower triangular format.
   *
  ...
  public void dsymv(
      ...
      DoubleMatrix1D y);
  /**
   * Triangular matrix-vector multiplication; <tt>x = A*x</tt> or <tt>x = A'*x</tt>. Where x is an n
   ...
   * @param transposeA set this flag to indicate that the multiplication shall be performed on A'.
   * @param isUnitTriangular true --> A is assumed to be unit triangular; false --> A is not assumed
  ...
  public void dtrmv(
      ...
      DoubleMatrix1D x);
  /**
   * Returns the index of largest absolute value; <tt>i such that |x[i]| ==
   * max(|x[0]|,|x[1]|,...).</tt>.
   *
