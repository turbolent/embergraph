 ...
 * V.mult(D.mult(transpose(V)))</tt> and <tt>V.mult(transpose(V))</tt> equals the identity matrix.
 *
 * <p>If <tt>A</tt> is not symmetric, then the eigenvalue matrix <tt>D</tt> is block diagonal with
 * the real eigenvalues in 1-by-1 blocks and any complex eigenvalues, <tt>lambda + i*mu</tt>, in
 * 2-by-2 blocks, <tt>[lambda, mu; -mu, lambda]</tt>. The columns of <tt>V</tt> represent the
 * eigenvectors in the sense that <tt>A*V = V*D</tt>, i.e. <tt>A.mult(V) equals V.mult(D)</tt>. The
 * matrix <tt>V</tt> may be badly conditioned, or even singular, so the validity of the equation
