 ...
 *
 * <ul>
 *   <li>{@link #dgemm dgemm} (matrix-matrix multiplication)
 *   <li>{@link #dgemv dgemv} (matrix-vector multiplication)
 *   <li>{@link #assign(DoubleMatrix2D,cern.colt.function.DoubleFunction) assign(A,function)}
 *       (generalized matrix scaling/transform): Strong speedup only for expensive functions like
 *       logarithm, sin, etc.
 *   <li>{@link #assign(DoubleMatrix2D,DoubleMatrix2D,cern.colt.function.DoubleDoubleFunction)
 *       assign(A,B,function)} (generalized matrix scaling/transform): Strong speedup only for
 *       expensive functions like pow etc.
 * </ul>
 *
 * In all cases, no or only marginal speedup is seen for small problem sizes; they are detected and
 * the sequential algorithm is used.
 *
 * <h4>Usage</h4>
 *
 * Call the static method {@link #allocateBlas} at the very beginning of your program, supplying the
 * main parameter for SmpBlas, the number of available CPUs. The method sets the public global
 * variable <tt>SmpBlas.smpBlas</tt> to a blas using a maximum of <tt>CPUs</tt> threads, each
 * concurrently processing matrix blocks with the given sequential blas algorithms. Normally there
 * is no need to call <tt>allocateBlas</tt> more than once. Then use
 * <tt>SmpBlas.smpBlas.someRoutine(...)</tt> to run <tt>someRoutine</tt> in parallel. E.g.
 *
 * <table>
 * <td class="PRE">
 * <pre>
 ...
 * </table>
 *
 * Even if you don't call a blas routine yourself, it often makes sense to allocate a SmpBlas,
 * because other matrix library routines sometimes call the blas. So if you're lucky, you get
 * parallel performance for free.
 *
 * <h4>Notes</h4>
 *
 * <ul>
 *   <li>Unfortunately, there is no portable means of automatically detecting the number of CPUs on
 *       a JVM, so there is no good way to automate defaults.
 *   <li>Only improves performance on boxes with > 1 CPUs and VMs with <b>native threads</b>.
 *   <li>Currently only improves performance when working on dense matrix types. On sparse types,
 *       performance is likely to degrade (because of the implementation of sub-range views)!
 *   <li>Implemented using Doug Lea's fast lightweight task framework ({@link
 *       EDU.oswego.cs.dl.util.concurrent}) built upon Java threads, and geared for parallel
 *       computation.
 * </ul>
 *
...
public class SmpBlas implements Blas {
  ...
  protected Blas
      seqBlas; // blocks are operated on in parallel; for each block this seq algo is used.
   ...
   * @param seqBlas the sequential blas algorithms to be used on concurrently processed matrix
   *     blocks.
   */
  public static void allocateBlas(int maxThreads, Blas seqBlas) {
    ...
    if (maxThreads <= 1) smpBlas = seqBlas;
    else {
  ...
  public void assign(
      DoubleMatrix2D A, DoubleMatrix2D B, final cern.colt.function.DoubleDoubleFunction function) {
    run(
        A,
        B,
        false,
    ...
    if (C.rows() != m || C.columns() != p)
      throw new IllegalArgumentException(
          "Incompatibel result matrix: "
              ...
              + ", "
              + C.toStringShort());
    if (A == C || B == C) throw new IllegalArgumentException("Matrices must not be identical");

    long flops = 2L * m * n * p;
    int noOfTasks =
        (int)
            Math.min(
                flops / 30000, this.maxThreads); // each thread should process at least 30000 flops
  ...
  protected double[] run(
      DoubleMatrix2D A, DoubleMatrix2D B, boolean collectResults, Matrix2DMatrix2DFunction fun) {
