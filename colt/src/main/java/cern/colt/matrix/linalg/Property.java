 ...
 * for numerical instability, to a degree specified upon instance construction and returned by
 * method {@link #tolerance()}. The public static final variable <tt>DEFAULT</tt> represents a
 * default Property object with a tolerance of <tt>1.0E-9</tt>. The public static final variable
 * <tt>ZERO</tt> represents a Property object with a tolerance of <tt>0.0</tt>. The public static
 * final variable <tt>TWELVE</tt> represents a Property object with a tolerance of <tt>1.0E-12</tt>.
 * As long as you are happy with these tolerances, there is no need to construct Property objects.
 * Simply use idioms like <tt>Property.DEFAULT.equals(A,B)</tt>, <tt>Property.ZERO.equals(A,B)</tt>,
 * <tt>Property.TWELVE.equals(A,B)</tt>.
 *
 * <p>To work with a different tolerance (e.g. <tt>1.0E-15</tt> or <tt>1.0E-5</tt>) use the
 * constructor and/or method {@link #setTolerance(double)}. Note that the public static final
 * Property objects are immutable: Is is not possible to alter their tolerance. Any attempt to do so
 * will throw an Exception.
 *
 * <p>Note that this implementation is not synchronized.
 *
 * <p>Example: <tt>equals(DoubleMatrix2D A, DoubleMatrix2D B)</tt> is defined as follows
 *
 * <table>
 * <td class="PRE">
 * <pre>
...
public class Property extends cern.colt.PersistentObject {
  ...
  public void checkSquare(DoubleMatrix2D A) {
    if (A.rows() != A.columns())
      throw new IllegalArgumentException(
          "Matrix must be square: " + cern.colt.matrix.doublealgo.Formatter.shape(A));
  }
   ...
   * tolerance())</tt> holds for all indexes.
   *
   * @param A the first matrix to compare.
   ...
   * (Math.abs(A[row,col] - B[row,col]) > tolerance())</tt> holds for all coordinates.
   *
   * @param A the first matrix to compare.
   * @param B the second matrix to compare.
   * @return <tt>true</tt> if both matrices are equal; <tt>false</tt> otherwise.
   ...
   * (Math.abs(A[slice,row,col] - B[slice,row,col]) > tolerance())</tt> holds for all coordinates.
   *
   * @param A the first matrix to compare.
   * @param B the second matrix to compare.
   * @return <tt>true</tt> if both matrices are equal; <tt>false</tt> otherwise.
    ...
    for (int slice = slices; --slice >= 0; ) {
      for (int row = rows; --row >= 0; ) {
        for (int column = columns; --column >= 0; ) {
          ...
          // if (Math.abs(A.getQuick(slice,row,column) - B.getQuick(slice,row,column)) > epsilon)
          // return false;
          double x = A.getQuick(slice, row, column);
   ...
   * != i.</tt> Matrix may but need not be square.
   *
   ...
   * diagonal, and the <tt>k</tt> diagonal lines below the diagonal. The <i>semi-bandwith l</i> is
   * the number <tt>k+1</tt>. The <i>bandwidth p</i> is the number <tt>2*k + 1</tt>. For example, a
   * tridiagonal matrix corresponds to <tt>k=1, l=2, p=3</tt>, a diagonal or zero matrix corresponds
   * to <tt>k=0, l=1, p=1</tt>,
   *
   * <p>The <i>upper bandwidth</i> is the maximum <tt>j-i</tt> for which <tt>A[i,j]</tt> is nonzero
   * and <tt>j &gt; i</tt>. The <i>lower bandwidth</i> is the maximum <tt>i-j</tt> for which
   * <tt>A[i,j]</tt> is nonzero and <tt>i &gt; j</tt>. Diagonal, tridiagonal and triangular matrices
   * are special cases.
   *
   * <p>Examples:
   *
