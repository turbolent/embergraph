...
public abstract class DoubleMatrix3D extends AbstractMatrix3D {
   ...
   * f(get(0,0,0),other.get(0,0,0)), a(0)==Double.NaN</tt>.
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   * <tt>SparseDoubleMatrix3D</tt>, etc. In general, the new matrix should have internal
   ...
   * instance of type <tt>SparseDoubleMatrix3D</tt> the new matrix must also be of type
   ...
   * cern.colt.matrix.doublealgo.Sorting#sort(DoubleMatrix3D,int,int)}. For more advanced sorting
   * functionality, see {@link cern.colt.matrix.doublealgo.Sorting}.
   *
   ...
   * @throws IllegalArgumentException if <tt>rows() != B.rows() || columns() != B.columns() ||
   *     slices() != B.slices() </tt>.
   */
  public void zAssign27Neighbors(DoubleMatrix3D B, cern.colt.function.Double27Function function) {
    ...
    for (int k = 1; k < slices - 1; k++) {
      for (int i = 1; i < r; i++) {
        ...
        for (int j = 1; j < c; j++) {
          // in each step 18 cells can be remembered in registers - they don't need to be reread
          // from slow memory
          ...
          B.setQuick(
              ...
              j,
              function.apply(
                  a000, a001, a002, a010, a011, a012, a020, a021, a022, a100, a101, a102, a110,
