 ...
 *
 * <p>A matrix has a number of cells (its <i>size</i>), which are assigned upon instance
 * construction. Elements are accessed via zero based indexes. Legal indexes are of the form
 * <tt>[0..size()-1]</tt>. Any attempt to access an element at a coordinate <tt>index&lt;0 ||
 * index&gt;=size()</tt> will throw an <tt>IndexOutOfBoundsException</tt>.
 *
...
public abstract class ObjectMatrix1D extends AbstractMatrix1D {
   ...
   * specified Object is also at least an ObjectMatrix1D, both matrices have the same size, and all
   ...
   *
   * <p>In general, fill order is <i>unspecified</i>. This implementation fills like: <tt>for (index
   * = 0..size()-1) do ... </tt>. However, subclasses are free to us any other order, even an order
   * that may change over time as cell values are changed. (Of course, result lists indexes are
   ...
   * if the receiver is an instance of type <tt>SparseObjectMatrix1D</tt> the new matrix must also
   * be of type <tt>SparseObjectMatrix1D</tt>, etc. In general, the new matrix should have internal
   ...
   * the receiver is an instance of type <tt>SparseObjectMatrix1D</tt> the new matrix must be of
   ...
   *
   * <p>The view contains the cells from <tt>index..index+width-1</tt>. and has <tt>view.size() ==
   * width</tt>. A view's legal coordinates are again zero based, as usual. In other words, legal
   * coordinates of the view are <tt>0 .. view.size()-1==width-1</tt>. As usual, any attempt to
   * access a cell at other coordinates will throw an <tt>IndexOutOfBoundsException</tt>.
   *
   ...
   *     <i>all</i> cells shall be visible, simply set this parameter to <tt>null</tt>.
   * @return the new view.
   ...
   * cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix1D)}. For more advanced sorting
