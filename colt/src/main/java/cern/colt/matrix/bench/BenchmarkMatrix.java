 ...
 * href="doc-files/usage_dgemm.txt">help ouput for the dgemm</a> command. <a
 * href="../doc-files/dgemmColt1.0.1ibm1.3LxPIII_2.txt">Here</a> is a sample result. For more
 * results see the <a href="../doc-files/performanceLog.html">performance log</a>.
 *
...
public class BenchmarkMatrix {
  ...
  protected static Double2DProcedure fun_dgemv(final boolean transposeA) {
    return new Double2DProcedure() {
      ...
      public void apply(cern.colt.Timer timer) {
        cern.colt.matrix.linalg.SmpBlas.smpBlas.dgemv(
            transposeA, 1, A, B.viewRow(0), 0, C.viewRow(0));
      }
  ...
  protected static Double2DProcedure funCorrelation() {
    return new Double2DProcedure() {
      ...
      public void setParameters(DoubleMatrix2D A, DoubleMatrix2D B) {
        super.setParameters(
            A.viewDice(), B); // transposed --> faster (memory aware) iteration in correlation algo
      }
  ...
  protected static Double2DProcedure funLUDecompose() {
    return new Double2DProcedure() {
      cern.colt.matrix.linalg.LUDecompositionQuick lu =
          new cern.colt.matrix.linalg.LUDecompositionQuick(0);
  ...
  protected static Double2DProcedure funSetQuick() {
    return new Double2DProcedure() {
      ...
      public void apply(cern.colt.Timer timer) {
        ...
        for (int row = 0; row < rows; row++) {
          for (int column = 0; column < columns; column++) {
            // a very fast random number generator (this is an inline version of class
            // cern.jet.random.engine.DRand)
            current *= 0x278DDE6D;
            double random = (double) (current & 0xFFFFFFFFL) * 2.3283064365386963E-10;
            // random uniform in (0.0,1.0)
            if (random < density) A.setQuick(row, column, random);
            else A.setQuick(row, column, 0);
          }
        }
      }
    };
    ...
    if (cmd.equals("dgemm")) bench_dgemm(params);
      ...
      if (fun != null) {
        ...
        String s =
            "Command="
                + params[0]
                + " is illegal or unknown. Should be one of "
                + commands()
  ...
  protected static void run(
    ...
    DoubleMatrix3D timings =
        DoubleFactory3D.dense.make(types.length, sizes.length, densities.length);
    cern.colt.Timer runTime = new cern.colt.Timer().start();
    for (int k = 0; k < types.length; k++) {
      // DoubleFactory2D factory = (k==0 ? DoubleFactory2D.dense : k==1 ? DoubleFactory2D.sparse :
      // DoubleFactory2D.rowCompressed);
      // DoubleFactory2D factory = (k==0 ? DoubleFactory2D.dense : k==1 ? DoubleFactory2D.sparse :
      // k==2 ? DoubleFactory2D.rowCompressed : DoubleFactory2D.rowCompressedModified);
    ...
    // cern.colt.matrix.doublealgo.Formatter("%1.3G").toTitleString(timings,sliceNames,rowNames,colNames,sliceAxisName,rowAxisName,colAxisName,"Performance of "+title,aggr));
    /*
  ...
  protected static void runSpecial(double minSeconds, String title, Double2DProcedure function) {
    ...
    System.out.println("*");
    //	System.out.println(new
    // cern.colt.matrix.doublealgo.Formatter("%1.3G").toTitleString(timings,rowNames,colNames,rowAxisName,colAxisName,title,aggr));
  ...
  protected static String usage(String cmd) {
    String usage =
        cmd
            + " description: "
            + getGenericFunction(cmd).toString()
