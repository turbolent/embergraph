 ...
 *
 * <p>A matrix has a number of slices, rows and columns, which are assigned upon instance
 * construction - The matrix's size is then <tt>slices()*rows()*columns()</tt>. Elements are
 * accessed via <tt>[slice,row,column]</tt> coordinates. Legal coordinates range from
 * <tt>[0,0,0]</tt> to <tt>[slices()-1,rows()-1,columns()-1]</tt>. Any attempt to access an element
 * at a coordinate <tt>slice&lt;0 || slice&gt;=slices() || row&lt;0 || row&gt;=rows() || column&lt;0
 * || column&gt;=column()</tt> will throw an <tt>IndexOutOfBoundsException</tt>.
 *
...
public abstract class ObjectMatrix3D extends AbstractMatrix3D {
   ...
   *     second argument the transformed current cell value.
   * @param f a function transforming the current cell value.
   * @return the aggregated measure.
   * @see cern.jet.math.Functions
   */
  public Object aggregate(
      cern.colt.function.ObjectObjectFunction aggr, cern.colt.function.ObjectFunction f) {
   ...
   * f(get(0,0,0),other.get(0,0,0)), a(0)==null</tt>.
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   *
   * @param aggr an aggregation function taking as first argument the current aggregation and as
   *     second argument the transformed current cell values.
   * @param f a function transforming the current cell values.
   * @return the aggregated measure.
   * @throws IllegalArgumentException if <tt>slices() != other.slices() || rows() != other.rows() ||
   *     columns() != other.columns()</tt>
   * @see cern.jet.math.Functions
   */
  public Object aggregate(
      ObjectMatrix3D other,
      cern.colt.function.ObjectObjectFunction aggr,
      cern.colt.function.ObjectObjectFunction f) {
    checkShape(other);
    if (size() == 0) return null;
    Object a =
        f.apply(
            getQuick(slices - 1, rows - 1, columns - 1),
            other.getQuick(slices - 1, rows - 1, columns - 1));
    int d = 1; // last cell already done
    for (int slice = slices; --slice >= 0; ) {
      for (int row = rows; --row >= 0; ) {
        for (int column = columns - d; --column >= 0; ) {
          a =
              aggr.apply(
                  a, f.apply(getQuick(slice, row, column), other.getQuick(slice, row, column)));
        }
        d = 0;
      }
    }
   ...
   *
   * <p>The values are copied. So subsequent changes in <tt>values</tt> are not reflected in the
   * matrix, and vice-versa.
   *
   * @param values the values to be filled into the cells.
   * @return <tt>this</tt> (for convenience only).
   * @throws IllegalArgumentException if <tt>values.length != slices() || for any 0 &lt;= slice &lt;
   *     slices(): values[slice].length != rows()</tt>.
   * @throws IllegalArgumentException if <tt>for any 0 &lt;= column &lt; columns():
   *     values[slice][row].length != columns()</tt>.
   */
  public ObjectMatrix3D assign(Object[][][] values) {
    ...
    for (int slice = slices; --slice >= 0; ) {
      ...
      if (currentSlice.length != rows)
        throw new IllegalArgumentException(
            "Must have same number of rows in every slice: rows="
                + currentSlice.length
                + "rows()="
                + rows());
      for (int row = rows; --row >= 0; ) {
   ...
   * @param function a function object taking as first argument the current cell's value of
   *     <tt>this</tt>, and as second argument the current cell's value of <tt>y</tt>,
   * @return <tt>this</tt> (for convenience only).
   * @throws IllegalArgumentException if <tt>slices() != other.slices() || rows() != other.rows() ||
   *     columns() != other.columns()</tt>
   * @see cern.jet.math.Functions
   ...
   * backed by this matrix, so changes in the returned matrix are not reflected in this matrix, and
   * vice-versa.
   *
  ...
  /*
   * Compares the specified Object with the receiver for equality. Equivalent to
   * <tt>equals(otherObj,true)</tt>.
   *
   ...
   * specified Object is also at least an ObjectMatrix3D, both matrices have the same size, and all
   * corresponding pairs of cells in the two matrices are the same. In other words, two matrices are
   * defined to be equal if they contain the same cell values in the same order. Tests elements for
   ...
   * @throws IndexOutOfBoundsException if <tt>slice&lt;0 || slice&gt;=slices() || row&lt;0 ||
   *     row&gt;=rows() || column&lt;0 || column&gt;=column()</tt>.
   */
   ...
   *
   * <p>In general, fill order is <i>unspecified</i>. This implementation fill like: <tt>for (slice
   * = 0..slices-1) for (row = 0..rows-1) for (column = 0..colums-1) do ... </tt>. However,
   * subclasses are free to us any other order, even an order that may change over time as cell
   * values are changed. (Of course, result lists indexes are guaranteed to correspond to the same
   * cell). For an example, see {@link
   * ObjectMatrix2D#getNonZeros(IntArrayList,IntArrayList,ObjectArrayList)}.
   *
   ...
   * exception. <b>You should only use this method when you are absolutely sure that the coordinate
   * is within bounds.</b> Precondition (unchecked): <tt>slice&lt;0 || slice&gt;=slices() ||
   * row&lt;0 || row&gt;=rows() || column&lt;0 || column&gt;=column()</tt>.
   *
   * @param slice the index of the slice-coordinate.
   * @param row the index of the row-coordinate.
   * @param column the index of the column-coordinate.
   ...
   * of type <tt>DenseObjectMatrix3D</tt> the new matrix must also be of type
   ...
   * <tt>SparseObjectMatrix3D</tt>, etc. In general, the new matrix should have internal
   ...
   * instance of type <tt>SparseObjectMatrix3D</tt> the new matrix must also be of type
   ...
   */
  protected abstract ObjectMatrix2D like2D(
   ...
   * @throws IndexOutOfBoundsException if <tt>row&lt;0 || row&gt;=rows() || slice&lt;0 ||
   *     slice&gt;=slices() || column&lt;0 || column&gt;=column()</tt>.
   */
   ...
   *
   * <p><b>Note that the view is not a deep copy.</b> The returned matrix is backed by this matrix,
   * so changes in the returned matrix are reflected in this matrix, and vice-versa.
   *
   * <p>Use {@link #copy()} if you want to construct an independent deep copy rather than a new
   * view.
   *
   ...
   *
   * <p>To obtain a slice view on subranges, construct a sub-ranging view
   * (<tt>view().part(...)</tt>), then apply this method to the sub-range view. To obtain
   * 1-dimensional views, apply this method, then apply another slice view (methods
   * <tt>viewColumn</tt>, <tt>viewRow</tt>) on the intermediate 2-dimensional view. To obtain
   ...
   *     height<0 || row+height>rows() || column<0 || width<0 || column+width>columns()</tt>
   * @return the new view.
   */
   ...
   * this.get(sliceIndexes[k],rowIndexes[i],columnIndexes[j])</tt>. Indexes can occur multiple times
   * and can be in arbitrary order. For an example see {@link
   * ObjectMatrix2D#viewSelection(int[],int[])}.
   *
   * <p>Note that modifying the index arguments after this call has returned has no effect on the
   * view. The returned view is backed by this matrix, so changes in the returned view are reflected
   * in this matrix, and vice-versa.
   *
   * @param sliceIndexes The slices of the cells that shall be visible in the new view. To indicate
   *     that <i>all</i> slices shall be visible, simply set this parameter to <tt>null</tt>.
   * @param rowIndexes The rows of the cells that shall be visible in the new view. To indicate that
   *     <i>all</i> rows shall be visible, simply set this parameter to <tt>null</tt>.
   * @param columnIndexes The columns of the cells that shall be visible in the new view. To
   *     indicate that <i>all</i> columns shall be visible, simply set this parameter to
   *     <tt>null</tt>.
   * @return the new view.
   ...
   * @return a new view.
   */
  protected abstract ObjectMatrix3D viewSelectionLike(
   ...
   * cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix3D,int,int)}. For more advanced sorting
   * functionality, see {@link cern.colt.matrix.objectalgo.Sorting}.
   *
   * @return a new sorted vector (matrix) view.
   * @throws IndexOutOfBoundsException if <tt>row < 0 || row >= rows() || column < 0 || column >=
   *     columns()</tt>.
   */
   ...
   * cells <tt>this.get(k*sliceStride,i*rowStride,j*columnStride)</tt> for all <tt>k =
   * 0..slices()/sliceStride - 1, i = 0..rows()/rowStride - 1, j = 0..columns()/columnStride -
   * 1</tt>. The returned view is backed by this matrix, so changes in the returned view are
   * reflected in this matrix, and vice-versa.
   *
   ...
   * </pre>
   *
   * Note that an implementation may use more efficient techniques, but must not use any other
   * order.
   *
   * @param procedure a procedure object taking as argument the current cell's value. Stops
   *     iteration if the procedure returns <tt>false</tt>, otherwise continues.
   ...
   * order.
   *
   * @param procedure a procedure object taking as first argument the current slice, as second
   *     argument the current row, and as third argument the current column. Stops iteration if the
