...
public abstract class DoubleMatrix2D extends AbstractMatrix2D {
   ...
   * f(get(0,0),other.get(0,0)), a(0)==Double.NaN</tt>.
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   *
   * <p>Parameters to function are as follows: <tt>first==row</tt>, <tt>second==column</tt>,
   * <tt>third==nonZeroValue</tt>.
   *
   * @param function a function object taking as argument the current non-zero cell's row, column
   *     and value.
   * @return <tt>this</tt> (for convenience only).
   */
   ...
   * if the receiver is an instance of type <tt>SparseDoubleMatrix2D</tt> the new matrix must also
   * be of type <tt>SparseDoubleMatrix2D</tt>, etc. In general, the new matrix should have internal
   ...
   * the receiver is an instance of type <tt>SparseDoubleMatrix2D</tt> the new matrix must be of
   * type <tt>SparseDoubleMatrix1D</tt>, etc.
   *
   ...
   * advanced sorting functionality, see {@link cern.colt.matrix.doublealgo.Sorting}.
   *
   ...
   * </pre>
   *
   * Make sure that cells of <tt>this</tt> and <tt>B</tt> do not overlap. In case of overlapping
   * views, behaviour is unspecified. </pre>
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   * @param z the vector where results are to be stored. Set this parameter to <tt>null</tt> to
   *     indicate that a new result vector shall be constructed.
   ...
   * @param C the matrix where results are to be stored. Set this parameter to <tt>null</tt> to
   *     indicate that a new result matrix shall be constructed.
   * @return C (for convenience only).
   * @throws IllegalArgumentException if <tt>B.rows() != A.columns()</tt>.
   * @throws IllegalArgumentException if <tt>C.rows() != A.rows() || C.columns() !=
   *     B.columns()</tt>.
