 ...
 *
 * <p>Objects of this class are typically constructed via <tt>viewIndexes</tt> methods on some
 * source matrix. The interface introduced in abstract super classes defines everything a user can
 * do. From a user point of view there is nothing special about this class; it presents the same
 * functionality with the same signatures and semantics as its abstract superclass(es) while
 * introducing no additional functionality. Thus, this class need not be visible to users. By the
 * way, the same principle applies to concrete DenseXXX and SparseXXX classes: they presents the
 * same functionality with the same signatures and semantics as abstract superclass(es) while
 * introducing no additional functionality. Thus, they need not be visible to users, either. Factory
 * methods could hide all these concrete types.
 *
 * <p>This class uses no delegation. Its instances point directly to the data. Cell addressing
 * overhead is is 1 additional int addition and 3 additional array index accesses per get/set.
 *
 * <p>Note that this implementation is not synchronized.
 *
 * <p><b>Memory requirements:</b>
 *
 * <p><tt>memory [bytes] = 4*(sliceIndexes.length+rowIndexes.length+columnIndexes.length)</tt>.
 * Thus, an index view with 100 x 100 x 100 indexes additionally uses 8 KB.
 *
 * <p><b>Time complexity:</b>
 *
 * <p>Depends on the parent view holding cells.
 *
 * <p>
 *
...
class SelectedDenseDoubleMatrix3D extends DoubleMatrix3D {
   ...
   */
  protected SelectedDenseDoubleMatrix3D(
      double[] elements, int[] sliceOffsets, int[] rowOffsets, int[] columnOffsets, int offset) {
  ...
  public double getQuick(int slice, int row, int column) {
    ...
    return elements[
        offset
            + sliceOffsets[sliceZero + slice * sliceStride]
  ...
  public DoubleMatrix2D viewColumn(int column) {
    ...
    return new SelectedDenseDoubleMatrix2D(
        viewRows,
        viewColumns,
        this.elements,
        viewRowZero,
