 ...
 * overhead is 1 additional int addition and 2 additional array index accesses per get/set.
 *
 * <p>Note that this implementation is not synchronized.
 *
 * <p><b>Memory requirements:</b>
 *
 * <p><tt>memory [bytes] = 4*(rowIndexes.length+columnIndexes.length)</tt>. Thus, an index view with
 * 1000 x 1000 indexes additionally uses 8 KB.
 *
...
class SelectedSparseObjectMatrix2D extends ObjectMatrix2D {
   ...
   * @param offset
   */
  protected SelectedSparseObjectMatrix2D(
      int rows,
      int columns,
   ...
   */
  protected SelectedSparseObjectMatrix2D(
      AbstractIntObjectMap elements, int[] rowOffsets, int[] columnOffsets, int offset) {
    this(
        rowOffsets.length,
  ...
  public ObjectMatrix1D viewColumn(int column) {
    ...
    return new SelectedSparseObjectMatrix1D(
