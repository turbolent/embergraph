 ...
 *
 * <p>Internally holds one single contigous one-dimensional array, addressed in (in decreasing order
 * of significance): slice major, row major, column major. Note that this implementation is not
 * synchronized.
 *
 * <p><b>Memory requirements:</b>
 *
 * <p><tt>memory [bytes] = 8*slices()*rows()*columns()</tt>. Thus, a 100*100*100 matrix uses 8 MB.
 *
 * <p><b>Time complexity:</b>
 *
 * <p><tt>O(1)</tt> (i.e. constant time) for the basic operations <tt>get</tt>, <tt>getQuick</tt>,
 * <tt>set</tt>, <tt>setQuick</tt> and <tt>size</tt>,
 *
 * <p>Applications demanding utmost speed can exploit knowledge about the internal addressing.
 * Setting/getting values in a loop slice-by-slice, row-by-row, column-by-column is quicker than,
 * for example, column-by-column, row-by-row, slice-by-slice. Thus
 *
 * <pre>
...
public class DenseDoubleMatrix3D extends DoubleMatrix3D {
   ...
   * @return <tt>this</tt> (for convenience only).
   * @throws IllegalArgumentException if <tt>slices() != source.slices() || rows() != source.rows()
   *     || columns() != source.columns()</tt>
   */
