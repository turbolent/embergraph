 ...
 * Normally one does not need to specify <tt>minColumnWidth</tt>. Cells in a row are separated by a
 * separator string, similar separators can be set for rows and slices. For more info, see the
 * concrete subclasses.
 *
...
public abstract class AbstractFormatter extends cern.colt.PersistentObject {
  ...
  protected static void setupBlanksCache() {
    // Pre-fabricate 40 static strings with 0,1,2,..,39 blanks, for usage within method
    // blanks(length).
    // Now, we don't need to construct and fill them on demand, and garbage collect them again.
    // All 40 strings share the identical char[] array, only with different offset and length -->
    // somewhat smaller static memory footprint
    int size = 40;
