...
/*
* Abstract base class for 2-d matrices holding objects or primitive data types such as <code>int
...
public abstract class AbstractMatrix2D extends AbstractMatrix {
  ...
  /** Indicates non-flipped state or flipped state. see _setFlip() for further info. */
   ...
   *
   * @throws IllegalArgumentException if <tt>columns() != B.columns() || rows() != B.rows() ||
   *     columns() != C.columns() || rows() != C.rows()</tt>.
   */
