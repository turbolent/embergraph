 ...
 * way, the same principle applies to concrete DenseXXX, SparseXXX classes: they presents the same
 * functionality with the same signatures and semantics as abstract superclass(es) while introducing
 * no additional functionality. Thus, they need not be visible to users, either. Factory methods
 * could hide all these concrete types.
 *
 * <p>This class uses no delegation. Its instances point directly to the data. Cell addressing
 * overhead is 1 additional array index access per get/set.
 *
 * <p>Note that this implementation is not synchronized.
 *
 * <p><b>Memory requirements:</b>
 *
 * <p><tt>memory [bytes] = 4*indexes.length</tt>. Thus, an index view with 1000 indexes additionally
 * uses 4 KB.
 *
...
class SelectedSparseObjectMatrix1D extends ObjectMatrix1D {
   ...
   * @param offset
   */
  protected SelectedSparseObjectMatrix1D(
      int size, AbstractIntObjectMap elements, int zero, int stride, int[] offsets, int offset) {
