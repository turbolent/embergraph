...
public abstract class AbstractMatrix3D extends AbstractMatrix {
   ...
   *     depth<0 || slice+depth>slices || column<0 || width<0 || column+width>columns</tt>
   */
   ...
   * columns.
   *
   * @throws IllegalArgumentException if <tt>slices() != B.slices() || rows() != B.rows() ||
   *     columns() != B.columns()</tt>.
   */
   ...
   *
   * @throws IllegalArgumentException if <tt>slices() != B.slices() || rows() != B.rows() ||
   *     columns() != B.columns() || slices() != C.slices() || rows() != C.rows() || columns() !=
   *     C.columns()</tt>.
   */
  ...
  protected int index(int slice, int row, int column) {
    return _sliceOffset(_sliceRank(slice))
        + _rowOffset(_rowRank(row))
   ...
   */
  protected AbstractMatrix3D vPart(
      int slice, int row, int column, int depth, int height, int width) {
  ...
  protected AbstractMatrix3D vStrides(int sliceStride, int rowStride, int columnStride) {
    if (sliceStride <= 0 || rowStride <= 0 || columnStride <= 0)
      throw new IndexOutOfBoundsException(
          "illegal strides: " + sliceStride + ", " + rowStride + ", " + columnStride);
