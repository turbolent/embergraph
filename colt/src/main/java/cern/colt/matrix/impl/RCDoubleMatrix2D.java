 ...
 *
 * <p>Internally uses the standard sparse row-compressed format, with two important differences that
 * broaden the applicability of this storage format:
 *
 * <ul>
 *   <li>We use a {@link cern.colt.list.IntArrayList} and {@link cern.colt.list.DoubleArrayList} to
 *       hold the column indexes and nonzero values, respectively. This improves set(...)
 *       performance, because the standard way of using non-resizable primitive arrays causes
 *       excessive memory allocation, garbage collection and array copying. The small downside of
 *       this is that set(...,0) does not free memory (The capacity of an arraylist does not shrink
 *       upon element removal).
 *   <li>Column indexes are kept sorted within a row. This both improves get and set performance on
 *       rows with many non-zeros, because we can use a binary search. (Experiments show that this
 *       hurts < 10% on rows with < 4 nonZeros.)
 * </ul>
 *
 * <br>
 * Note that this implementation is not synchronized.
 *
 * <p><b>Memory requirements:</b>
 *
 * <p>Cells that
 *
 * <ul>
 ...
 *   <li>switch back from non-zero to zero state also do use memory. Their memory is <i>not</i>
 *       automatically reclaimed (because of the lists vs. arrays). Reclamation can be triggered via
 *       {@link #trimToSize()}.
 * </ul>
 *
 * <p><tt>memory [bytes] = 4*rows + 12 * nonZeros</tt>. <br>
 * Where <tt>nonZeros = cardinality()</tt> is the number of non-zero cells. Thus, a 1000 x 1000
 * matrix with 1000000 non-zero cells consumes 11.5 MB. The same 1000 x 1000 matrix with 1000
 * non-zero cells consumes 15 KB.
 *
 * <p><b>Time complexity:</b>
 *
 * <p>Getting a cell value takes time<tt> O(log nzr)</tt> where <tt>nzr</tt> is the number of
 * non-zeros of the touched row. This is usually quick, because typically there are only few
 * nonzeros per row. So, in practice, get has <i>expected</i> constant time. Setting a cell value
 * takes <i> </i>worst-case time <tt>O(nz)</tt> where <tt>nzr</tt> is the total number of non-zeros
 * in the matrix. This can be extremely slow, but if you traverse coordinates properly (i.e.
 * upwards), each write is done much quicker:
 *
 * <table>
 * <td class="PRE">
 * <pre>
 ...
 * </table>
 *
 * If for whatever reasons you can't iterate properly, consider to create an empty dense matrix,
 * store your non-zeros in it, then call <tt>sparse.assign(dense)</tt>. Under the circumstances,
 * this is still rather quick.
 *
 * <p>Fast iteration over non-zeros can be done via {@link #forEachNonZero}, which supplies your
 * function with row, column and value of each nonzero. Although the internally implemented version
 * is a bit more sophisticated, here is how a quite efficient user-level matrix-vector
 * multiplication could look like:
 *
 * <table>
 * <td class="PRE">
 * <pre>
 ...
 * </table>
 *
 * <p>Here is how a a quite efficient user-level combined scaling operation could look like:
 *
 * <table>
 * <td class="PRE">
 * <pre>
 ...
 * </table>
 *
 * Method {@link #assign(DoubleMatrix2D,cern.colt.function.DoubleDoubleFunction)} does just that if
 * you supply {@link cern.jet.math.Functions#plusMult} as argument.
 *
...
public class RCDoubleMatrix2D extends WrapperDoubleMatrix2D {
   ...
   * @throws IllegalArgumentException if <tt>for any 1 &lt;= row &lt; values.length:
   *     values[row].length != values[row-1].length</tt>.
   */
   ...
   * @throws IllegalArgumentException if <tt>rows<0 || columns<0 || (double)columns*rows >
   ...
   * @return <tt>this</tt> (for convenience only).
   * @throws IllegalArgumentException if <tt>columns() != source.columns() || rows() !=
   *     source.rows()</tt>
   */
  ...
  public void setQuick(int row, int column, double value) {
    int k = indexes.binarySearchFromTo(column, starts[row], starts[row + 1] - 1);
    if (k >= 0) { // found
      if (value == 0) remove(row, k);
      else values.setQuick(k, value);
      return;
    }
