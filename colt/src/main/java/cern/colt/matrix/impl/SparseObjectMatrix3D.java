 ...
 * cern.colt.map.OpenIntObjectHashMap}, which is a compact and performant hashing technique.
 *
 ...
 * complexity than its dense counterpart {@link DenseObjectMatrix2D}. However, constant factors are
 * considerably larger.
 *
...
public class SparseObjectMatrix3D extends ObjectMatrix3D {
   ...
   * @throws IllegalArgumentException if <tt>slices<0 || rows<0 || columns<0</tt>.
   */
  protected SparseObjectMatrix3D(
      int slices,
      int rows,
      int columns,
   ...
   */
  protected ObjectMatrix2D like2D(
   ...
   * @return a new view.
   */
  protected ObjectMatrix3D viewSelectionLike(
      int[] sliceOffsets, int[] rowOffsets, int[] columnOffsets) {
    return new SelectedSparseObjectMatrix3D(
