 ...
 * 25 MB and 50 MB. The same 1000 x 1000 matrix with 1000 non-zero cells consumes between 25 and 50
 * KB.
 *
 ...
 * considerably larger.
 *
 * <p>Cells are internally addressed in row-major. Performance sensitive applications can exploit
 * this fact. Setting values in a loop row-by-row is quicker than column-by-column, because fewer
 * hash collisions occur. Thus
 *
 * <pre>
...
public class SparseDoubleMatrix2D extends DoubleMatrix2D {
   ...
   * entries are initially <tt>0</tt>. For details related to memory usage see {@link
   ...
   * @throws IllegalArgumentException if <tt>rows<0 || columns<0 || (double)columns*rows >
   *     Integer.MAX_VALUE</tt> or flip's are illegal.
   */
  protected SparseDoubleMatrix2D(
      int rows,
      int columns,
  ...
  public DoubleMatrix2D assign(
      final DoubleMatrix2D y, cern.colt.function.DoubleDoubleFunction function) {
  ...
  public double getQuick(int row, int column) {
    // if (debug) if (column<0 || column>=columns || row<0 || row>=rows) throw new
    // IndexOutOfBoundsException("row:"+row+", column:"+column);
   ...
   * </ul>
   *
   * A sequence like <tt>set(r,c,5); set(r,c,0);</tt> sets a cell to non-zero state and later back
   * to zero state. Such as sequence generates obsolete memory that is automatically reclaimed from
   * time to time or can manually be reclaimed by calling <tt>trimToSize()</tt>. Putting zeros into
   * cells already containing zeros does not generate obsolete memory since no memory was allocated
