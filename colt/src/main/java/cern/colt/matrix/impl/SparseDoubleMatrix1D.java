...
/*
 * Sparse hashed 1-d matrix (aka <i>vector</i>) holding <tt>double</tt> elements. First see the <a
 ...
 * Where <tt>nonZeros = cardinality()</tt> is the number of non-zero cells. Thus, a 1000000 matrix
 * with minLoadFactor=0.25 and maxLoadFactor=0.5 and 1000000 non-zero cells consumes between 25 MB
 * and 50 MB. The same 1000000 matrix with 1000 non-zero cells consumes between 25 and 50 KB.
 *
 ...
 * complexity than its dense counterpart {@link DenseDoubleMatrix1D}. However, constant factors are
 * considerably larger.
 *
...
public class SparseDoubleMatrix1D extends DoubleMatrix1D {
  ...
  /*
   * Constructs a matrix with a given number of parameters. All entries are initially <tt>0</tt>.
  ...
  public void setQuick(int index, double value) {
    ...
    if (value == 0) this.elements.removeKey(i);
   ...
   * </ul>
   *
   * A sequence like <tt>set(i,5); set(i,0);</tt> sets a cell to non-zero state and later back to
   * zero state. Such as sequence generates obsolete memory that is automatically reclaimed from
