 ...
 * </center>
 *
 * <p align="left">As can be seen, sparse matrices are certainly not quite as quick as dense ones,
 * but not really slow either. Considering their minimal footprint they can be a real alternative.
 *
 * <p>Comparing the OO abstractions to zero-abstraction primitive Java arrays may or may not be
 * useful. Still, the table below provides some interesting information. For example, access to
 * <tt>Type_T_Matrix2D</tt> is quicker than naive usage of <tt>Type_T_[]</tt>. Primitive arrays
 * should only be considered if the optimized form can be applied. Note again that all benchmarks
 * only measure the time spent in accessing a matrix element; they exclude the loop itself.
 *
 * <p><center>
 *
...
class BenchmarkMatrix2D {
  ...
  public static void doubleBenchmark(
    ...
    // certain loops need to be constructed so that the jitter can't optimize them away and we get
    // fantastic numbers.
    ...
    if (kind.equals("sparse"))
      matrix =
          new SparseDoubleMatrix2D(rows, columns, initialCapacity, minLoadFactor, maxLoadFactor);
    ...
    System.out.println(
        "bytes needed per non-zero=" + (before - after) / (double) matrix.cardinality());
    if (print) {
  ...
  /** Runs a bench on matrices holding double elements. */
  public static void doubleBenchmarkMult(
      int runs,
      int rows,
      int columns,
      String kind,
  ...
  /** Runs a bench on matrices holding double elements. */
  public static void doubleBenchmarkPrimitiveOptimized(
      int runs, int rows, int columns, boolean print) {
    // certain loops need to be constructed so that the jitter can't optimize them away and we get
    // fantastic numbers.
    // this involves primarly read-loops
    cern.colt.Timer timer1 = new cern.colt.Timer();
  ...
  /** Runs a bench on matrices holding int elements. */
  public static void intBenchmarkPrimitiveOptimized(
      int runs, int rows, int columns, boolean print) {
    throw new InternalError();
    /*
    // certain loops need to be constructed so that the jitter can't optimize them away and we get fantastic numbers.
  ...
  public static void main(String[] args) {
    ...
    if (type.equals("int")) {
      if (kind.equals("primitive")) intBenchmarkPrimitive(runs, rows, columns, print);
        ...
        intBenchmark(
            runs, rows, columns, kind, print, initialCapacity, minLoadFactor, maxLoadFactor);
