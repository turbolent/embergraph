 ...
 *
 * <p>Note that this implementation is not synchronized. Uses a {@link
 * cern.colt.map.OpenIntDoubleHashMap}, which is a compact and performant hashing technique.
 *
 ...
 *   <li>switch back from non-zero to zero state also do use memory. However, their memory is
 *       automatically reclaimed from time to time. It can also manually be reclaimed by calling
 *       {@link #trimToSize()}.
 * </ul>
 *
 * <p>worst case: <tt>memory [bytes] = (1/minLoadFactor) * nonZeros * 13</tt>. <br>
 * best case: <tt>memory [bytes] = (1/maxLoadFactor) * nonZeros * 13</tt>. <br>
 * Where <tt>nonZeros = cardinality()</tt> is the number of non-zero cells. Thus, a 100 x 100 x 100
 * matrix with minLoadFactor=0.25 and maxLoadFactor=0.5 and 1000000 non-zero cells consumes between
 * 25 MB and 50 MB. The same 100 x 100 x 100 matrix with 1000 non-zero cells consumes between 25 and
 * 50 KB.
 *
 * <p><b>Time complexity:</b>
 *
 * <p>This class offers <i>expected</i> time complexity <tt>O(1)</tt> (i.e. constant time) for the
 * basic operations <tt>get</tt>, <tt>getQuick</tt>, <tt>set</tt>, <tt>setQuick</tt> and
 * <tt>size</tt> assuming the hash function disperses the elements properly among the buckets.
 * Otherwise, pathological cases, although highly improbable, can occur, degrading performance to
 * <tt>O(N)</tt> in the worst case. As such this sparse class is expected to have no worse time
 * complexity than its dense counterpart {@link DenseDoubleMatrix2D}. However, constant factors are
 * considerably larger.
 *
 * <p>Cells are internally addressed in (in decreasing order of significance): slice major, row
 * major, column major. Applications demanding utmost speed can exploit this fact. Setting/getting
 * values in a loop slice-by-slice, row-by-row, column-by-column is quicker than, for example,
 * column-by-column, row-by-row, slice-by-slice. Thus
 *
 * <pre>
...
public class SparseDoubleMatrix3D extends DoubleMatrix3D {
   ...
   * @throws IllegalArgumentException if <tt>for any 1 &lt;= slice &lt; values.length:
   *     values[slice].length != values[slice-1].length</tt>.
   * @throws IllegalArgumentException if <tt>for any 1 &lt;= row &lt; values[0].length:
   *     values[slice][row].length != values[slice][row-1].length</tt>.
   */
   ...
   * All entries are initially <tt>0</tt>. For details related to memory usage see {@link
   ...
   * @throws IllegalArgumentException if <tt>slices<0 || rows<0 || columns<0</tt>.
   */
  protected SparseDoubleMatrix3D(
      int slices,
      int rows,
      int columns,
   ...
   * before tt>set()</tt>ing a large number of non-zero values boosts performance, because the
  ...
  public double getQuick(int slice, int row, int column) {
    // if (debug) if (slice<0 || slice>=slices || row<0 || row>=rows || column<0 || column>=columns)
   ...
   */
  protected DoubleMatrix2D like2D(
   ...
   *   <li>are never set to non-zero values do not use any memory.
   *   <li>switch from zero to non-zero state do use memory.
   *   <li>switch back from non-zero to zero state also do use memory. However, their memory can be
   *       reclaimed by calling <tt>trimToSize()</tt>.
   * </ul>
   *
   * A sequence like <tt>set(s,r,c,5); set(s,r,c,0);</tt> sets a cell to non-zero state and later
   * back to zero state. Such as sequence generates obsolete memory that is automatically reclaimed
   * from time to time or can manually be reclaimed by calling <tt>trimToSize()</tt>. Putting zeros
   * into cells already containing zeros does not generate obsolete memory since no memory was
   ...
   * @return a new view.
   */
  protected DoubleMatrix3D viewSelectionLike(
