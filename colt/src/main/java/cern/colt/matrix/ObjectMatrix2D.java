 ...
 *
 * <p>A matrix has a number of rows and columns, which are assigned upon instance construction - The
 * matrix's size is then <tt>rows()*columns()</tt>. Elements are accessed via <tt>[row,column]</tt>
 * coordinates. Legal coordinates range from <tt>[0,0]</tt> to <tt>[rows()-1,columns()-1]</tt>. Any
 * attempt to access an element at a coordinate <tt>column&lt;0 || column&gt;=columns() || row&lt;0
 * || row&gt;=rows()</tt> will throw an <tt>IndexOutOfBoundsException</tt>.
 *
...
public abstract class ObjectMatrix2D extends AbstractMatrix2D {
   ...
   * terminators are <tt>a(1) == f(get(0,0)), a(0)==null</tt>.
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   * f(get(0,0),other.get(0,0)), a(0)==null</tt>.
   *
   * <p><b>Example:</b>
   *
   * <pre>
   ...
   * @return the aggregated measure.
   * @throws IllegalArgumentException if <tt>columns() != other.columns() || rows() !=
   *     other.rows()</tt>
   * @see cern.jet.math.Functions
   */
  public Object aggregate(
   ...
   * @throws IllegalArgumentException if <tt>values.length != rows() || for any 0 &lt;= row &lt;
   *     rows(): values[row].length != columns()</tt>.
   */
   ...
   * replaces <i>as if</i> using an intermediate auxiliary deep copy of <tt>other</tt>.
   *
   ...
   * specified Object is also at least an ObjectMatrix2D, both matrices have the same size, and all
   ...
   * @throws IndexOutOfBoundsException if <tt>column&lt;0 || column&gt;=columns() || row&lt;0 ||
   *     row&gt;=rows()</tt>
   */
   ...
   *
   * <p>In general, fill order is <i>unspecified</i>. This implementation fills like <tt>for (row =
   * 0..rows-1) for (column = 0..columns-1) do ... </tt>. However, subclasses are free to us any
   * other order, even an order that may change over time as cell values are changed. (Of course,
   * result lists indexes are guaranteed to correspond to the same cell).
   *
   * <p><b>Example:</b> <br>
   *
   * <pre>
  ...
  public void getNonZeros(
      IntArrayList rowList, IntArrayList columnList, ObjectArrayList valueList) {
    rowList.clear();
    columnList.clear();
    valueList.clear();
    int r = rows;
   ...
   * is within bounds.</b> Precondition (unchecked): <tt>0 &lt;= column &lt; columns() && 0 &lt;=
   * row &lt; rows()</tt>.
   *
   * @param row the index of the row-coordinate.
   * @param column the index of the column-coordinate.
   ...
   * <tt>DenseObjectMatrix2D</tt> the new matrix must also be of type <tt>DenseObjectMatrix2D</tt>,
   * if the receiver is an instance of type <tt>SparseObjectMatrix2D</tt> the new matrix must also
   * be of type <tt>SparseObjectMatrix2D</tt>, etc. In general, the new matrix should have internal
   ...
   * <tt>SparseObjectMatrix2D</tt>, etc. In general, the new matrix should have internal
   * parametrization as similar as possible.
   *
   ...
   * the receiver is an instance of type <tt>SparseObjectMatrix2D</tt> the new matrix must be of
   ...
   *
   * <p>Use {@link #copy()} to construct an independent deep copy rather than a new view.
   *
   ...
   * <tt>view.get(row,column)==this.get(column,row)</tt>. This is a zero-copy transposition, taking
   * O(1), i.e. constant time. The returned view is backed by this matrix, so changes in the
   * returned view are reflected in this matrix, and vice-versa. Use idioms like <tt>result =
   * viewDice(A).copy()</tt> to generate an independent transposed matrix.
   *
   * <p><b>Example:</b>
   *
   * <table border="0">
   ...
   *
   * <p>Operations on the returned view can only be applied to the restricted range. Any attempt to
   * access coordinates not contained in the view will throw an <tt>IndexOutOfBoundsException</tt>.
   *
   * <p><b>Note that the view is really just a range restriction:</b> The returned matrix is backed
   * by this matrix, so changes in the returned matrix are reflected in this matrix, and vice-versa.
   *
   * <p>The view contains the cells from <tt>[row,column]</tt> to
   * <tt>[row+height-1,column+width-1]</tt>, all inclusive. and has <tt>view.rows() == height;
   * view.columns() == width;</tt>. A view's legal coordinates are again zero based, as usual. In
   * other words, legal coordinates of the view range from <tt>[0,0]</tt> to
   * <tt>[view.rows()-1==height-1,view.columns()-1==width-1]</tt>. As usual, any attempt to access a
   * cell at a coordinate <tt>column&lt;0 || column&gt;=view.columns() || row&lt;0 ||
   ...
   * @throws IndexOutOfBoundsException if <tt>column<0 || width<0 || column+width>columns() || row<0
   *     || height<0 || row+height>rows()</tt>
   * @return the new view.
   */
   ...
   * this.get(rowIndexes[i],columnIndexes[j])</tt>. Indexes can occur multiple times and can be in
   * arbitrary order.
   *
   * <p><b>Example:</b>
   *
   ...
   *
   * <p>To indicate "all" rows or "all columns", simply set the respective parameter
   *
   ...
   * information, see {@link cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix2D,int)}. For more
   * advanced sorting functionality, see {@link cern.colt.matrix.objectalgo.Sorting}.
   *
   ...
   * <tt>this.get(i*rowStride,j*columnStride)</tt> for all <tt>i = 0..rows()/rowStride - 1, j =
   * 0..columns()/columnStride - 1</tt>. The returned view is backed by this matrix, so changes in
