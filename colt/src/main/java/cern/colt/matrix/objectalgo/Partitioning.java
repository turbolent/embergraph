 ...
 * * log(k))</tt>. Constants factors are minimized.
 *
...
public class Partitioning {
   ...
   *     checked; be sure that they are met.
   * @param splitIndexes a list into which this method fills the indexes of rows delimiting
   *     intervals. Therefore, must satisfy <tt>splitIndexes.length >= splitters.length</tt>.
   * @return a new matrix view having rows partitioned by the given column and splitters.
   */
  public static ObjectMatrix2D partition(
      ObjectMatrix2D matrix, int column, final Object[] splitters, int[] splitIndexes) {
    int rowFrom = 0;
    ...
    partition(
        matrix, rowIndexes, rowFrom, rowTo, column, splitters, splitFrom, splitTo, splitIndexes);
   ...
   * indicate so.
   *
   * <p>Of course, the column must not be a column of a different matrix. More formally, there must
   * hold: <br>
   ...
   * </table>
   */
  private static void xPartitionOld(
      ObjectMatrix2D matrix,
      ObjectMatrix1D column,
      int from,
      int to,
      Object[] splitters,
      int splitFrom,
   ...
   * <p>Note that arguments are not checked for validity.
   */
  private static int xPartitionOld(
      ObjectMatrix2D matrix, ObjectMatrix1D column, int from, int to, Object splitter) {
    /*
