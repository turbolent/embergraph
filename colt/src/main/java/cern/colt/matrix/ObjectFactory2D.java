 ...
 * <p>&nbsp;
 *
 * <p>If the factory is used frequently it might be useful to streamline the notation. For example
 * by aliasing:
 *
 * <table>
 * <td class="PRE">
...
public class ObjectFactory2D extends cern.colt.PersistentObject {
   ...
   *
   * <p>All matrices of a given column within <tt>parts</tt> must have the same number of columns.
   * All matrices of a given row within <tt>parts</tt> must have the same number of rows. Otherwise
   * an <tt>IllegalArgumentException</tt> is thrown. Note that <tt>null</tt>s within
   * <tt>parts[row,col]</tt> are an exception to this rule: they are ignored. Cells are copied.
   * Example:
   *
   ...
   * </pre>
   *
   * (The direct sum has <tt>A.rows()+B.rows()</tt> rows and <tt>A.columns()+B.columns()</tt>
   * columns). Cells are copied.
   *
