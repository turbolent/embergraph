 ...
 *
 * <p>Implements open addressing with double hashing, using "Brent's variation". Brent's variation
 * slows insertions a bit down (not much) but reduces probes (collisions) for successful searches,
 * in particular for large load factors. (It does not improve unsuccessful searches.) See D. Knuth,
 * Searching and Sorting, 3rd ed., p.533-545
 *
...
class QuickOpenIntIntHashMap extends OpenIntIntHashMap {
  ...
  public boolean put(int key, int value) {
    ...
    */
    ...
    // System.out.println("insert search for (key,value)=("+key+","+value+") at i="+i+",
    // dec="+decrement);
    ...
    if (this.distinct > this.highWaterMark) {
      ...
      // System.out.println("at distinct="+distinct+", capacity="+table.length+" to
    ...
    */
    while (t > 1) {
      ...
      if (stat[pc]
          != FREE) { // not a free slot, continue searching for free slot to move to, or break.
        p0 = pc;
        t--;
      } else { // free or removed slot found, now move...
        // System.out.println("copying p0="+p0+" to pc="+pc+",
        // (key,val)=("+tab[p0]+","+values[p0]+"), saving "+(t-1)+" probes.");
