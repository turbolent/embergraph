 ...
 *
 * <p>Choosing a prime is <tt>O(log 300)</tt> (binary search in a list of 300 int's). Memory
...
public class PrimeFinder extends Object {
   ...
   * number <= desired capacity * 1.11 (11%). For any desired capacity >= 200 the list includes a
   * prime number <= desired capacity * 1.16 (16%). For any desired capacity >= 16 the list includes
   * a prime number <= desired capacity * 1.21 (21%).
   *
   * <p>Therefore, primes can be retrieved which are quite close to any desired capacity, which in
   * turn avoids wasting memory. For example, the list includes
   * 1039,1117,1201,1277,1361,1439,1523,1597,1759,1907,2081. So if you need a prime >= 1040, you
   * will find a prime <= 1040*1.11=1154.
   *
   * <p>Chunks are chosen such that they are optimized for a hashtable growthfactor of 2.0; If your
   * hashtable has such a growthfactor then, after initially "rounding to a prime" upon hashtable
   * construction, it will later expand to prime capacities such that there exist no better primes.
   *
   * <p>In total these are about 32*10=320 numbers -> 1 KB of static memory needed. If you are
   * stingy, then delete every second or fourth chunk.
   */
  ...
  public static int nextPrime(int desiredCapacity) {
    ...
    // int i = new cern.colt.list.IntArrayList(primeCapacities).binarySearch(desiredCapacity); //
    // for debug only TODO
    if (i < 0) {
