 ...
 *
 * <p>Assume we have three arrays X, Y and Z. We want to sort all three arrays by X (or some
 * arbitrary comparison function). For example, we have<br>
 * <tt>X=[3, 2, 1], Y=[3.0, 2.0, 1.0], Z=[6.0, 7.0, 8.0]</tt>. The output should be <tt><br>
 * X=[1, 2, 3], Y=[1.0, 2.0, 3.0], Z=[8.0, 7.0, 6.0]</tt>.
 *
 * <p>How can we achive this? Here are several alternatives. We could ...
 *
 * <ol>
 *   <li>make a list of Point3D objects, sort the list as desired using a comparison function, then
 *       copy the results back into X, Y and Z. The classic object-oriented way.
 *   <li>make an index list [0,1,2,...,N-1], sort the index list using a comparison function, then
 *       reorder the elements of X,Y,Z as defined by the index list. Reordering cannot be done
 *       in-place, so we need to copy X to some temporary array, then copy in the right order back
 *       from the temporary into X. Same for Y and Z.
 *   <li>use a generic quicksort or mergesort which, whenever two elements in X are swapped, also
 *       swaps the corresponding elements in Y and Z.
 * </ol>
 *
 * Alternatives 1 and 2 involve quite a lot of copying and allocate significant amounts of temporary
 * memory. Alternative 3 involves more swapping, more polymorphic message dispatches, no copying and
 * does not need any temporary memory.
 *
 * <p>This class implements alternative 3. It operates on arbitrary shaped data. In fact, it has no
 * idea what kind of data it is sorting. Comparisons and swapping are delegated to user provided
 * objects which know their data and can do the job.
 *
 * <p>Lets call the generic data <tt>g</tt> (it may be one array, three linked lists or whatever).
 * This class takes a user comparison function operating on two indexes <tt>(a,b)</tt>, namely an
 * {@link IntComparator}. The comparison function determines whether <tt>g[a]</tt> is equal, less or
 * greater than <tt>g[b]</tt>. The sort, depending on its implementation, can decide to swap the
 * data at index <tt>a</tt> with the data at index <tt>b</tt>. It calls a user provided {@link
 * cern.colt.Swapper} object that knows how to swap the data of these indexes.
 *
 ...
 *
 * <p>Assume again we have three arrays X, Y and Z. Now we want to sort all three arrays, primarily
 * by Y, secondarily by Z (if Y elements are equal). For example, we have<br>
 * <tt>X=[6, 7, 8, 9], Y=[3.0, 2.0, 1.0, 3.0], Z=[5.0, 4.0, 4.0, 1.0]</tt>. The output should be
 * <tt><br>
 * X=[8, 7, 9, 6], Y=[1.0, 2.0, 3.0, 3.0], Z=[4.0, 4.0, 1.0, 5.0]</tt>.
 *
 * <p>Here is how to solve the problem. All code in the above example stays the same, except that we
 * modify the comparison function as follows
 *
 * <table>
 * <td class="PRE">
 * <pre>
 ...
 * <p>
 *
 * <p>Sorts involving floating point data and not involving comparators, like, for example provided
 * in the JDK {@link java.util.Arrays} and in the Colt {@link cern.colt.Sorting} handle floating
 * point numbers in special ways to guarantee that NaN's are swapped to the end and -0.0 comes
 * before 0.0. Methods delegating to comparators cannot do this. They rely on the comparator. Thus,
 * if such boundary cases are an issue for the application at hand, comparators explicitly need to
 * implement -0.0 and NaN aware comparisons. Remember: <tt>-0.0 < 0.0 == false</tt>, <tt>(-0.0 ==
 * 0.0) == true</tt>, as well as <tt>5.0 &lt; Double.NaN == false</tt>, <tt>5.0 &gt; Double.NaN ==
 * false</tt>. Same for <tt>float</tt>.
 *
 ...
 * optimisations taken from the JDK algorithms. Both quick and merge sort are "in-place", i.e. do
 * not allocate temporary memory (helper arrays). Mergesort is <i>stable</i> (by definition), while
...
public class GenericSorting extends Object {
   ...
   *     comp.apply(array[j], x)</code> is <code>true</code>.
   ...
   * @param first Beginning of the range
   * @param last One past the end of the range
   * @exception ArrayIndexOutOfBoundsException If the range is invalid.
   */
