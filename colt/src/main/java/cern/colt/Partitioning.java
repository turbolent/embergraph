 ...
 * <i>synchronously</i> partitioning either one, two or three arrays. They further come in templated
 * versions, either partitioning <tt>int[]</tt> arrays or <tt>double[]</tt> arrays.
 *
 * <p>You may want to start out reading about the simplest case: Partitioning one <tt>int[]</tt>
 * array into two intervals. To do so, read {@link #partition(int[],int,int,int)}.
 *
 * <p>Next, building upon that foundation comes a method partitioning <tt>int[]</tt> arrays into
 * multiple intervals. See {@link #partition(int[],int,int,int[],int,int,int[])} for related
 * documentation.
 *
 * <p>All other methods are no different than the one's you now already understand, except that they
 * operate on slightly different data types.
 *
 * <p><b>Performance</b>
 *
 * <p>Partitioning into two intervals is <tt>O( N )</tt>. Partitioning into k intervals is <tt>O( N
 * * log(k))</tt>. Constants factors are minimized. No temporary memory is allocated; Partitioning
 * is in-place.
 *
...
public class Partitioning extends Object {
   ...
   * @param comp the comparator determining the order of the generic data. Takes as first argument
   *     the index <tt>a</tt> within the generic splitters <tt>s</tt>. Takes as second argument the
   ...
   *     point</i>) - 1)</tt>. The <i>insertion point</i> is defined as the the point at which the
   *     value would be inserted into the list: the index of the first element greater than the key,
   *     or <tt>list.length</tt>, if all elements in the list are less than the specified key. Note
  ...
  public static void dualPartition(
    ...
    // Ideally, the pivot should be the median, because a median splits a list into two equal sized
    // sublists.
    ...
    if (splitFrom == splitTo) { // we don't really have a choice
      ...
      // Find the splitter closest to the pivot, i.e. the splitter that best splits the list into
      // two equal sized sublists.
    ...
    // Establish invariant: list[i] < splitter <= list[j] for i=from..medianIndex and
    // j=medianIndex+1 .. to
    ...
    if (splitFrom <= medianIndex - 1) {
      dualPartition(
          list, secondary, from, splitIndex, splitters, splitFrom, medianIndex - 1, splitIndexes);
    }
   ...
   */
  public static int dualPartition(
      double[] list, double[] secondary, int from, int to, double splitter) {
   ...
   * primary array is moved from index A to B, the correspoding element within the secondary array
   * is also moved from index A to B.
   *
   * <p><b>Use cases:</b>
   *
   * <p>Image having a large list of 2-dimensional points. If memory consumption and performance
   * matter, it is a good idea to physically lay them out as two 1-dimensional arrays (using
   * something like <tt>Point2D</tt> objects would be prohibitively expensive, both in terms of time
   * and space). Now imagine wanting to histogram the points. We may want to partially sort the
   * points by x-coordinate into intervals. This method efficiently does the job.
   *
   * <p><b>Performance:</b>
   *
   * <p>Same as for single-partition methods.
   */
  public static void dualPartition(
      int[] list,
      int[] secondary,
   ...
   * moved from index A to B, the correspoding element within the secondary array is also moved from
   * index A to B.
   *
   * <p><b>Performance:</b>
   *
   ...
   * <tt>int[]</tt> arrays.
   *
   * <p>This method operates on arbitrary shaped data and arbitrary shaped splitters. In fact, it
   * has no idea what kind of data by what kind of splitters it is partitioning. Comparisons and
   * swapping are delegated to user provided objects which know their data and can do the job.
   *
   * <p>Lets call the generic data <tt>g</tt> (it may be a matrix, one array, three linked lists or
   * whatever). Lets call the generic splitters <tt>s</tt>. This class takes a user comparison
   * function operating on two indexes <tt>(a,b)</tt>, namely an {@link IntComparator}. The
   * comparison function determines whether <tt>s[a]</tt> is equal, less or greater than
   * <tt>g[b]</tt>. This method can then decide to swap the data <tt>g[b]</tt> with the data
   * <tt>g[c]</tt> (yes, <tt>c</tt>, not <tt>a</tt>). It calls a user provided {@link
   * cern.colt.Swapper} object that knows how to swap the data of these two indexes.
   *
   * <p>Again, note the details: Comparisons compare <tt>s[a]</tt> with <tt>g[b]</tt>. Swaps swap
   * <tt>g[b]</tt> with <tt>g[c]</tt>. Prior to calling this method, the generic splitters
   * <tt>s</tt> must be sorted ascending and must not contain multiple equal values. These
   * preconditions are not checked; be sure that they are met.
   *
   * @param from the index of the first element within <tt>g</tt> to be considered.
   * @param to the index of the last element within <tt>g</tt> to be considered. The method
   *     considers the elements <tt>g[from] .. g[to]</tt>.
   ...
   *     the splitter elements <tt>s[splitFrom] .. s[splitTo]</tt>.
   * @param splitIndexes a list into which this method fills the indexes of elements delimiting
   ...
   * @param comp the comparator comparing a splitter with an element of the generic data. Takes as
   *     first argument the index <tt>a</tt> within the generic splitters <tt>s</tt>. Takes as
   *     second argument the index <tt>b</tt> within the generic data <tt>g</tt>.
   * @param comp2 the comparator to determine the order of the generic data. Takes as first argument
   *     the index <tt>a</tt> within the generic data <tt>g</tt>. Takes as second argument the index
   *     <tt>b</tt> within the generic data <tt>g</tt>.
   * @param comp3 the comparator comparing a splitter with another splitter. Takes as first argument
   *     the index <tt>a</tt> within the generic splitters <tt>s</tt>. Takes as second argument the
   *     index <tt>b</tt> within the generic splitters <tt>g</tt>.
   * @param swapper an object that knows how to swap the elements at any two indexes (a,b). Takes as
   *     first argument the index <tt>b</tt> within the generic data <tt>g</tt>. Takes as second
   *     argument the index <tt>c</tt> within the generic data <tt>g</tt>.
   ...
   */
  public static void genericPartition(
      int from,
      int to,
      int splitFrom,
      int splitTo,
    ...
    if (splitFrom <= medianIndex - 1) {
      genericPartition(
          from, splitIndex, splitFrom, medianIndex - 1, splitIndexes, comp, comp2, comp3, swapper);
    }
   ...
   * arrays.
   */
  private static int genericPartition(
      int from, int to, int splitter, IntComparator comp, Swapper swapper) {
    for (int i = from - 1; ++i <= to; ) {
   ...
   * <p><b>Example:</b>
   *
   * <p><tt>list = (7, 4, 5, 50, 6, 4, 3, 6), splitters = (5, 10, 30)</tt> defines the three
   * intervals <tt>[-infinity,5), [5,10), [10,30)</tt>. Lets define to sort the entire list
   * (<tt>from=0, to=7</tt>) using all splitters (<tt>splitFrom==0, splitTo=2</tt>).
   *
   * <p>The method modifies the list to be <tt>list = (4, 4, 3, 6, 7, 5, 6, 50)</tt> and returns the
   ...
   *   <li>All values <tt>list[7..6]</tt> fall into <tt>[10,30)</tt>, i.e. no elements, since
   *       <tt>7>6</tt>.
   *   <li>All values <tt>list[7 .. 7=list.length-1]</tt> fall into <tt>[30,infinity]</tt>.
   *   <li>In general, all values <tt>list[splitIndexes[j-1]+1 .. splitIndexes[j]]</tt> fall into
   *       interval <tt>j</tt>.
   * </ul>
   *
   * As can be seen, the list is partially sorted such that values falling into a certain interval
   * are placed next to each other. Note that <i>within</i> an interval, elements are entirelly
   * unsorted. They are only sorted across interval boundaries. In particular, this partitioning
   * algorithm is not <i>stable</i>: the relative order of elements is not preserved (Producing a
   * stable algorithm would require no more than minor modifications to method
   * partition(int[],int,int,int)).
   *
   * <p>More formally, this method guarantees that upon return <tt>for all j = splitFrom ..
   * splitTo</tt> there holds: <br>
   * <tt>for all i = splitIndexes[j-1]+1 .. splitIndexes[j]: splitters[j-1] <= list[i] <
   * splitters[j]</tt>.
   *
   * <p><b>Performance:</b>
   *
   * <p>Let <tt>N=to-from+1</tt> be the number of elements to be partitioned. Let
   * <tt>k=splitTo-splitFrom+1</tt> be the number of splitter elements. Then we have the following
   * time complexities
   *
   * <ul>
   *   <li>Worst case: <tt>O( N * log(k) )</tt>.
   *   <li>Average case: <tt>O( N * log(k) )</tt>.
   *   <li>Best case: <tt>O( N )</tt>. In general, the more uniform (skewed) the data is spread
   *       across intervals, the more performance approaches the worst (best) case. If no elements
   *       fall into the given intervals, running time is linear.
   * </ul>
   *
   ...
   *
   * <p>The algorithm can be seen as a Bentley/McIlroy quicksort where swapping and insertion sort
   * are omitted. It is designed to detect and take advantage of skew while maintaining good
   * performance in the uniform case.
   *
   * @param list the list to be partially sorted.
   * @param from the index of the first element within <tt>list</tt> to be considered.
   * @param to the index of the last element within <tt>list</tt> to be considered. The method
   *     considers the elements <tt>list[from] .. list[to]</tt>.
   * @param splitters the values at which the list shall be split into intervals. Must be sorted
   ...
   *     intervals. Upon return <tt>splitIndexes[splitFrom..splitTo]</tt> will be set accordingly.
   *     Therefore, must satisfy <tt>splitIndexes.length > splitTo</tt>.
   *     <p>Tip: Normally you will have <tt>splitIndexes.length == splitters.length</tt> as well as
   *     <tt>from==0, to==list.length-1</tt> and <tt>splitFrom==0, splitTo==splitters.length-1</tt>.
  ...
  public static void partition(
    ...
    */
    ...
    // recursively partition left half
    if (splitFrom <= medianIndex - 1) {
      // System.out.println("1.recursive: from="+from+", to="+splitIndex+", splitFrom="+splitFrom+",
      // splitTo="+(medianIndex-1));
      partition(list, from, splitIndex, splitters, splitFrom, medianIndex - 1, splitIndexes);
    }
    ...
    // recursively partition right half
    if (medianIndex + 1 <= splitTo) {
      // System.out.println("2.recursive: from="+(splitIndex+1)+", to="+to+",
      // splitFrom="+(medianIndex+1)+", splitTo="+splitTo);
      partition(list, splitIndex + 1, to, splitters, medianIndex + 1, splitTo, splitIndexes);
    }
   ...
   * <p><b>Example:</b>
   *
   * <p><tt>list = (7, 4, 5, 50, 6, 4, 3, 6), splitter = 5</tt> defines the two intervals
   * <tt>[-infinity,5), [5,+infinity]</tt>.
   *
   * <p>The method modifies the list to be <tt>list = (4, 4, 3, 50, 6, 7, 5, 6)</tt> and returns the
   * split index <tt>2</tt>. In other words,
   *
   * <ul>
   *   <li>All values <tt>list[0..2]</tt> fall into <tt>[-infinity,5)</tt>.
   ...
   *
   * <p>More formally, this method guarantees that upon return there holds:
   *
   * <ul>
   ...
   * <p><b>Performance:</b>
   *
   * <p>Let <tt>N=to-from+1</tt> be the number of elements to be partially sorted. Then the time
   * complexity is <tt>O( N )</tt>. No temporary memory is allocated; the sort is in-place.
   *
   * <p>
   *
   * @param list the list to be partially sorted.
  ...
  public static int partition(
      Object[] list, int from, int to, Object splitter, java.util.Comparator comp) {
   ...
   */
  public static void partition(
      IntArrayList list, int from, int to, IntArrayList splitters, IntArrayList splitIndexes) {
    partition(
        list.elements(),
        from,
        to,
   ...
   * <i>synchronously</i> partitions <tt>double[]</tt> rather than <tt>int[]</tt> arrays.
   */
  public static void triplePartition(
      double[] list,
      double[] secondary,
      double[] tertiary,
      int from,
   ...
   * <i>synchronously</i> partitions <tt>double[]</tt> rather than <tt>int[]</tt> arrays.
   */
  public static int triplePartition(
      double[] list, double[] secondary, double[] tertiary, int from, int to, double splitter) {
    double element; // int, double --> template type dependent
    for (int i = from - 1; ++i <= to; ) {
      element = list[i];
      if (element < splitter) {
   ...
   * the primary array is moved from index A to B, the correspoding element within the secondary
   * array as well as the corresponding element within the tertiary array are also moved from index
   * A to B.
   *
   * <p><b>Use cases:</b>
   *
   * <p>Image having a large list of 3-dimensional points. If memory consumption and performance
   * matter, it is a good idea to physically lay them out as three 1-dimensional arrays (using
   * something like <tt>Point3D</tt> objects would be prohibitively expensive, both in terms of time
   * and space). Now imagine wanting to histogram the points. We may want to partially sort the
   * points by x-coordinate into intervals. This method efficiently does the job.
   *
   * <p><b>Performance:</b>
   ...
   * moved from index A to B, the correspoding element within the secondary array as well as the
   * corresponding element within the tertiary array are also moved from index A to B.
   *
   * <p><b>Performance:</b>
   *
   * <p>Same as for single-partition methods.
   */
  public static int triplePartition(
      int[] list, int[] secondary, int[] tertiary, int from, int to, int splitter) {
    int element; // int, double --> template type dependent
    for (int i = from - 1; ++i <= to; ) {
      element = list[i];
      if (element < splitter) {
