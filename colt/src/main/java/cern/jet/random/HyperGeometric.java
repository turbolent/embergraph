 ...
 * probability distribution of the random variable X, whose value is the number of successes in a
 * sample of <tt>n</tt> items from a population of size <tt>N</tt> that has <tt>s</tt> 'success'
 * items and <tt>N - s</tt> 'failure' items.
 *
 * <p><tt>p(k) = C(s,k) * C(N-s,n-k) / C(N,n)</tt> where <tt>C(a,b) = a! / (b! * (a-b)!)</tt>.
 *
 * <p>valid for N >= 2, s,n <= N.
 *
 * <p>Instance methods operate on a user supplied uniform random number generator; they are
 * unsynchronized.
 * <dt>Static methods operate on a default uniform random number generator; they are synchronized.
 *
 *     <p><b>Implementation:</b> High performance implementation. Patchwork Rejection/Inversion
 *     method.
 ...
 *
 *     <p>H. Zechner (1994): Efficient sampling from continuous and discrete unimodal distributions,
...
public class HyperGeometric extends AbstractDiscreteDistribution {
  ...
  protected int nextInt(int N, int M, int n, RandomEngine randomGenerator) {
     ...
     * for parameters 1 <= n <= M <= N/2. * Otherwise parameters are re-defined in the set-up step
     * and the * random number K is adapted before delivering. * For l = m-max(0,n-N+M) < 10
     * Inversion method hmdu is applied: * The random numbers are generated via modal down-up
     * search, * starting at the mode m. The cumulative probabilities * are avoided by using the
     * technique of chop-down. * For l >= 10 the Patchwork Rejection method hprs is employed: * The
     * area below the histogram function f(x) in its * body is rearranged by certain point
     * reflections. Within a * large center interval variates are sampled efficiently by * rejection
     * from uniform hats. Rectangular immediate acceptance * regions speed up the generation. The
     * remaining tails are * covered by exponential functions. * *
     * ***************************************************************** * FUNCTION : - hprsc
     * samples a random number from the * Hypergeometric distribution with parameters * N (number of
     * red and black balls), M (number * of red balls) and n (number of trials) * valid for N >= 2,
     * M,n <= N. * REFERENCE : - H. Zechner (1994): Efficient sampling from * continuous and
     * discrete unimodal distributions, * Doctoral Dissertation, 156 pp., Technical * University
     * Graz, Austria. * SUBPROGRAMS: - flogfak(k) ... log(k!) with long integer k * - drand(seed)
     * ... (0,1)-Uniform generator with * unsigned long integer *seed. * - hmdu(seed,N,M,n) ...
     * Hypergeometric generator * for l<10 * - hprs(seed,N,M,n) ... Hypergeometric generator * for
     * l>=10 with unsigned long integer *seed, * long integer N , M , n. * *
     * ****************************************************************
     */
