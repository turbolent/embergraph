 ...
 *
 * <p><b>Implementation:</b> High performance implementation.
...
class Fun {
  ...
  public static double bessel2_fkt(double lambda, double beta) {
    ...
    final double[] b0 = {
      -1.5787132, -0.6130827, 0.1735823, 1.4793411, 2.6667307, 4.9086836, 8.1355339,
    };
    final double[] b05 = {
      -1.9694802, -0.7642538, 0.0826017, 1.4276355, 2.6303682, 4.8857787, 8.1207968,
    };
    final double[] b1 = {
      -2.9807345, -1.1969943, -0.1843161, 1.2739241, 2.5218256, 4.8172216, 8.0765633,
    };
    final double[] b2 = {
      -5.9889676, -2.7145389, -1.1781269, 0.6782201, 2.0954009, 4.5452152, 7.9003173,
    };
    final double[] b3 = {
      -9.6803440, -4.8211925, -2.6533185, -0.2583337, 1.4091915, 4.0993448, 7.6088310,
    };
    final double[] b5 = {
      -18.1567152, -10.0939408, -6.5819139, -2.9371545, -0.6289005, 2.7270412, 6.6936799,
    };
    final double[] b8 = {
      -32.4910195, -19.6065943, -14.0347298, -8.3839439, -4.9679730, -0.3567823, 4.5589697,
    };
    ...
    if ((lambda > 0.0) && ((beta - 0.04 * lambda) <= 0.0)) {
      if (lambda < 11.5) {
        ...
        erg =
            -(lambda + 1.0) * Math.log(2.0)
                - (lambda - 0.5) * Math.log(lambda)
                + lambda
    ...
    if (beta < 1.57) {
      ...
      for (i = 1; i <= nr_per; i++) {
        for (j = 1; j <= 10; j++) {
          new_value =
              (0.5 * (_fkt_value(lambda, z1, z2, x) + _fkt_value(lambda, z1, z2, x1))) * step;
          sum += new_value;
          x = x1;
          x1 += step;
        }
      ...
      erg =
          gamma(lambda + 0.5)
              * Math.pow((2.0 * z2), lambda)
              / (Math.sqrt(pi) * Math.pow(z1, lambda))
              * sum;
  ...
  public static double logGamma(double x) {
    final double c0 = 9.1893853320467274e-01,
