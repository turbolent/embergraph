 ...
 *
 * <p>If <tt>interpolationType == LINEAR_INTERPOLATION</tt> a linear interpolation within the bin is
 * computed, resulting in a constant density within each bin.
 * <dt>If <tt>interpolationType == NO_INTERPOLATION</tt> no interpolation is performed and the
 *     result is a discrete distribution.
 *
 ...
 *
 *     <p><b>Implementation:</b> A uniform random number is generated using a user supplied
 *     generator. The uniform number is then transformed to the user's distribution using the
 *     cumulative probability distribution constructed from the pdf. The cumulative distribution is
 *     inverted using a binary search for the nearest bin boundary.
 *
