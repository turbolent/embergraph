...
public class Distributions {
   ...
   *
   * <p>L. Devroye (1986): Non-Uniform Random Variate Generation, Springer Verlag, New York.
   *
   * <p>
   *
   * @param r must be &gt; 0.
  ...
  public static double nextBurr1(double r, int nr, RandomEngine randomGenerator) {
     ...
     * ***************************************************************** * FUNCTION : - burr1
     * samples a random number from one of the * Burr II, VII, VIII, X distributions with *
     * parameter r > 0 , where the no. of the * distribution is indicated by a pointer * variable. *
     * REFERENCE : - L. Devroye (1986): Non-Uniform Random Variate * Generation, Springer Verlag,
     * New York. * SUBPROGRAM : - drand(seed) ... (0,1)-uniform generator with * unsigned long
     * integer *seed. * * ****************************************************************
     */
    double y;
  ...
  public static double nextBurr2(double r, double k, int nr, RandomEngine randomGenerator) {
     ...
     * ***************************************************************** * FUNCTION : - burr2
     * samples a random number from one of the * Burr III, IV, V, VI, IX, XII distributions with *
     * parameters r > 0 and k > 0, where the no. of * the distribution is indicated by a pointer *
     * variable. * REFERENCE : - L. Devroye (1986): Non-Uniform Random Variate * Generation,
     * Springer Verlag, New York. * SUBPROGRAM : - drand(seed) ... (0,1)-Uniform generator with *
   ...
   *
   * <p><tt>p(x) = 1/ (mean*pi * (1+(x/mean)^2))</tt>.
   *
   * <p><b>Implementation:</b> This is a port of <tt>cin.c</tt> from the <A
   * HREF="http://www.cis.tu-graz.ac.at/stat/stadl/random.html">C-RAND / WIN-RAND</A> library.
   *
   * <p>
   *
   ...
   * <p><tt>p(k) = p * (1-p)^k</tt> for <tt> k &gt;= 0</tt>.
   *
   * <p><b>Implementation:</b> Inversion method. This is a port of <tt>geo.c</tt> from the <A
   * HREF="http://www.cis.tu-graz.ac.at/stat/stadl/random.html">C-RAND / WIN-RAND</A> library.
   *
  ...
  public static int nextGeometric(double p, RandomEngine randomGenerator) {
     ...
     * search is necessary, but in the * case of the Geometric distribution a direct transformation
     * is * possible because of the special parallel to the continuous * Exponential distribution
     * Exp(t): * X - Exp(t): G(x)=1-exp(-tx) * Geo(p): pk=G(k+1)-G(k)=exp(-tk)*(1-exp(-t)) *
     * p=1-exp(-t) * A random number of the Geometric distribution Geo(p) is * obtained by k=(long
     * int)x, where x is from Exp(t) with * parameter t=-log(1-p). * *
     * ***************************************************************** * FUNCTION: - geo samples a
     * random number from the Geometric * distribution with parameter 0<p<1. * SUBPROGRAMS: -
     * drand(seed) ... (0,1)-Uniform generator with * unsigned long integer *seed. * *
     * ****************************************************************
     */
   ...
   * C-RAND's implementation, in turn, is based upon
   *
   * <p>J.S. Ramberg, B:W. Schmeiser (1974): An approximate method for generating asymmetric
   * variables, Communications ACM 17, 78-82.
   *
   * <p>
   */
   ...
   * distribution L(0,1).
   *
   * <p><b>Implementation:</b> Inversion method. This is a port of <tt>lapin.c</tt> from the <A
   * HREF="http://www.cis.tu-graz.ac.at/stat/stadl/random.html">C-RAND / WIN-RAND</A> library.
   *
   * <p>
   *
  ...
  public static double nextTriangular(RandomEngine randomGenerator) {
     ...
     * a random number from the * standard Triangular distribution in (-1,1) * SUBPROGRAM : -
     * drand(seed) ... (0,1)-Uniform generator with * unsigned long integer *seed. * *
     * ****************************************************************
     */
    double u;
