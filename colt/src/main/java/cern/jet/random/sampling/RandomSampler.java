 ...
 * without replacement), no element will occur more than once. Each number from the <tt>N</tt>
 * numbers has the same probability to be included in the <tt>n</tt> chosen numbers.
 *
 * <p><b>Problem:</b> This class solves problems including the following: <i> Suppose we have a file
 * containing 10^12 objects. We would like to take a truly random subset of 10^6 objects and do
 * something with it, for example, compute the sum over some instance field, or whatever. How do we
 * choose the subset? In particular, how do we avoid multiple equal elements? How do we do this
 * quick and without consuming excessive memory? How do we avoid slowly jumping back and forth
 * within the file? </i>
 *
 * <p><b>Sorted Simple Random Sample Without Replacement (SRSWOR):</b> What are the exact semantics
 * of this class? What is a SRSWOR? In which sense exactly is a returned set "random"? It is random
 * in the sense, that each number from the <tt>N</tt> numbers has the same probability to be
 * included in the <tt>n</tt> chosen numbers. For those who think in implementations rather than
 * abstract interfaces: <i>Suppose, we have an empty list. We pick a random number between 1 and
 * 10^12 and add it to the list only if it was not already picked before, i.e. if it is not already
 * contained in the list. We then do the same thing again and again until we have eventually
 * collected 10^6 distinct numbers. Now we sort the set ascending and return it.</i>
 * <dt>It is exactly in this sense that this class returns "random" sets. <b>Note, however, that the
 *     implementation of this class uses a technique orders of magnitudes better (both in time and
 *     space) than the one outlined above.</b>
 *
 ...
 *     </center>
 *
 *     <p><b>Scalability:</b> This random sampler is designed to be scalable. In iterator style, it
 *     is able to compute and deliver sorted random sets stepwise in units called <i>blocks</i>.
 *     Example: Computing <tt>n=9</tt> random numbers from the interval <tt>[1,50]</tt> in 3 blocks
 *     may yield the blocks <tt>(7,13,14), (27,37,42), (45,46,49)</tt>. (The maximum of a block is
 *     guaranteed to be less than the minimum of its successor block. Every block is sorted
 *     ascending. No element will ever occur twice, both within a block and among blocks.) A block
 *     can be computed and retrieved with method <tt>nextBlock</tt>. Successive calls to method
 *     <tt>nextBlock</tt> will deliver as many random numbers as required.
 *
 *     <p>Computing and retrieving samples in blocks is useful if you need very many random numbers
 *     that cannot be stored in main memory at the same time. For example, if you want to compute
 *     10^10 such numbers you can do this by computing them in blocks of, say, 500 elements each.
 *     You then need only space to keep one block of 500 elements (i.e. 4 KB). When you are finished
 *     processing the first 500 elements you call <tt>nextBlock</tt> to fill the next 500 elements
 *     into the block, process them, and so on. If you have the time and need, by using such blocks
 *     you can compute random sets up to <tt>n=10^19</tt> random numbers.
 *
 *     <p>If you do not need the block feature, you can also directly call the static methods of
 *     this class without needing to construct a <tt>RandomSampler</tt> instance first.
 *
 *     <p><b>Random number generation:</b> By default uses <tt>MersenneTwister</tt>, a very strong
 *     random number generator, much better than <tt>java.util.Random</tt>. You can also use other
 *     strong random number generators of Paul Houle's RngPack package. For example,
 *     <tt>Ranecu</tt>, <tt>Ranmar</tt> and <tt>Ranlux</tt> are strong well analyzed research grade
 *     pseudo-random number generators with known periods.
 *
 *     <p><b>Implementation:</b> after J.S. Vitter, An Efficient Algorithm for Sequential Random
 *     Sampling, ACM Transactions on Mathematical Software, Vol 13, 1987. Paper available <A
 *     HREF="http://www.cs.duke.edu/~jsv"> here</A>.
 *
...
public class RandomSampler extends cern.colt.PersistentObject {
   ...
   * <tt>nextBlock</tt> will deliver as many random numbers as required.
   *
   * @param n the total number of elements to choose (must be <tt>n &gt;= 0</tt> and <tt>n &lt;=
   *     N</tt>).
   ...
   * @param randomGenerator a random number generator. Set this parameter to <tt>null</tt> to use
   ...
   *
   * <p>Numbers are filled into the specified array starting at index <tt>fromIndex</tt> to the
   * right. The array is returned sorted ascending in the range filled with numbers.
   *
   * @param count the number of elements to be filled into <tt>values</tt> by this call (must be
   *     &gt;= 0).
   * @param values the array into which the random numbers are to be filled; must have a length
   *     <tt>&gt;= count+fromIndex</tt>.
   ...
   * @param N the interval to choose random numbers from is <tt>[low,low+N-1]</tt>.
   * @param count the number of elements to be filled into <tt>values</tt> by this call (must be
   *     &gt;= 0 and &lt;=<tt>n</tt>). Normally, you will set <tt>count=n</tt>.
   * @param low the interval to choose random numbers from is <tt>[low,low+N-1]</tt>. Hint: If
   ...
   */
  protected static void rejectMethodD(
      long n,
      long N,
      int count,
      long low,
   ...
   * <p><b>Random number generation:</b> By default uses <tt>MersenneTwister</tt>, a very strong
   * random number generator, much better than <tt>java.util.Random</tt>. You can also use other
   * strong random number generators of Paul Houle's RngPack package. For example, <tt>Ranecu</tt>,
   * <tt>Ranmar</tt> and <tt>Ranlux</tt> are strong well analyzed research grade pseudo-random
   * number generators with known periods.
   *
   * @param n the total number of elements to choose (must be <tt>n &gt;= 0</tt> and <tt>n &lt;=
   *     N</tt>).
   * @param N the interval to choose random numbers from is <tt>[low,low+N-1]</tt>.
   ...
   * @param randomGenerator a random number generator.
   */
  protected static void sampleMethodA(
      long n,
      long N,
      int count,
      long low,
   ...
   * @param randomGenerator a random number generator.
   */
  protected static void sampleMethodD(
      long n,
      long N,
      int count,
      long low,
  ...
  public static void test(long n, long N, long low, int chunkSize, int times) {
    ...
    for (long t = times; --t >= 0; ) {
      RandomSampler sampler =
          new RandomSampler(n, N, low, cern.jet.random.AbstractDistribution.makeDefaultGenerator());
