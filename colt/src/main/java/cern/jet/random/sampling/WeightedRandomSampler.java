 ...
 * the first block, one element from the second block, ..., one element from the last block. weight
 * == 1.0 --> all elements are picked (sampled). weight == 10.0 --> Picks one random element from
 * successive blocks of 10 elements each. Etc. The subsequence is guaranteed to be <i>stable</i>,
 * i.e. elements never change position relative to each other.
 *
...
public class WeightedRandomSampler extends cern.colt.PersistentObject {
   ...
   * second, ..., one from the last block. weight == 1.0 --> all elements are consumed (sampled).
   * 10.0 --> Consumes one random element from successive blocks of 10 elements each. Etc.
   *
