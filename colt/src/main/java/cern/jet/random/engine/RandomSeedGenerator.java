 ...
 *
 * <p>This class has entirelly deterministic behaviour: Constructing two instances with the same
 * parameters at any two distinct points in time will produce identical seed sequences. However, it
 * does not (at all) generate uniformly distributed numbers. Do not use it as a uniformly
 * distributed random number generator!
 *
 * <p>Each generated sequence of seeds has a period of 10<sup>9</sup> numbers. Internally uses
 * {@link RandomSeedTable}.
 *
...
public class RandomSeedGenerator extends cern.colt.PersistentObject {
   ...
   * seed matrix. The seed matrix is a n*m matrix with <tt>1 + Integer.MAX_VALUE</tt> (virtual) rows
   * and <tt>RandomSeedTable.COLUMNS</tt> columns. Successive calls to method <tt>nextSeed()</tt>
   * will cycle over the given column, in ascending order: <tt>nextSeed()</tt> returns the seed
   * <tt>s[row,column], s[row+1,column], ... s[Integer.MAX_VALUE,column], s[0,column], s[1,column],
   * ...</tt>
   *
