 ...
 * this class are at the core of computational statistics, simulations, Monte Carlo methods, etc.
 *
 * <p>Subclasses produce uniformly distributed <tt>int</tt>'s and <tt>long</tt>'s in the closed
 ...
 * respectively.
 *
 * <p>Subclasses need to override one single method only: <tt>nextInt()</tt>. All other methods
 * generating different data types or ranges are usually layered upon <tt>nextInt()</tt>.
 * <tt>long</tt>'s are formed by concatenating two 32 bit <tt>int</tt>'s. <tt>float</tt>'s are
 * formed by dividing the interval <tt>[0.0f,1.0f]</tt> into 2<sup>32</sup> sub intervals, then
 * randomly choosing one subinterval. <tt>double</tt>'s are formed by dividing the interval
 * <tt>[0.0,1.0]</tt> into 2<sup>64</sup> sub intervals, then randomly choosing one subinterval.
 *
 * <p>Note that this implementation is <b>not synchronized</b>.
 *
 ...
 */
// public abstract class RandomEngine extends edu.cornell.lassp.houle.RngPack.RandomSeedable
// implements cern.colt.function.DoubleFunction, cern.colt.function.IntFunction {
public abstract class RandomEngine extends cern.colt.PersistentObject
    implements cern.colt.function.DoubleFunction, cern.colt.function.IntFunction {
  ...
  public double nextDouble() {
    ...
    do {
      ...
      // 5.421010862427522E-20 == 1 / Math.pow(2,64) == 1 / ((double) Long.MAX_VALUE - (double)
      // Long.MIN_VALUE);
