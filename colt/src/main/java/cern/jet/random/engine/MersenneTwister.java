 ...
 * <tt>double</tt>'s in the open unit intervals <tt>(0.0f,1.0f)</tt> and <tt>(0.0,1.0)</tt>,
 * respectively. The seed can be any 32-bit integer except <tt>0</tt>. Shawn J. Cokus commented that
 * perhaps the seed should preferably be odd.
 *
 * <p><b>Quality:</b> MersenneTwister is designed to pass the k-distribution test. It has an
 * astronomically large period of 2<sup>19937</sup>-1 (=10<sup>6001</sup>) and 623-dimensional
 * equidistribution up to 32-bit accuracy. It passes many stringent statistical tests, including the
 * <A HREF="http://stat.fsu.edu/~geo/diehard.html">diehard</A> test of G. Marsaglia and the load
 * test of P. Hellekalek and S. Wegenkittl.
 *
 * <p><b>Performance:</b> Its speed is comparable to other modern generators (in particular, as fast
 * as <tt>java.util.Random.nextFloat()</tt>). 2.5 million calls to <tt>raw()</tt> per second
 * (Pentium Pro 200 Mhz, JDK 1.2, NT). Be aware, however, that there is a non-negligible amount of
 * overhead required to initialize the data structures used by a MersenneTwister. Code like
 *
 * <pre>
 * double sum = 0.0;
 ...
 * </pre>
 *
 * instead. This allows the cost of constructing the MersenneTwister object to be borne only once,
 * rather than once for each iteration in the loop.
 *
 * <p><b>Implementation:</b> After M. Matsumoto and T. Nishimura, "Mersenne Twister: A
 * 623-Dimensionally Equidistributed Uniform Pseudo-Random Number Generator", ACM Transactions on
 * Modeling and Computer Simulation, Vol. 8, No. 1, January 1998, pp 3--30.
 * <dt>More info on <A HREF="http://www.math.keio.ac.jp/~matumoto/eindex.html"> Masumoto's
 *     homepage</A>.
 * <dt>More info on <A HREF="http://www.ncsa.uiuc.edu/Apps/CMP/RNG/www-rng.html"> Pseudo-random
 *     number generators is on the Web</A>.
 * <dt>Yet <A HREF="http://nhse.npac.syr.edu/random"> some more info</A>.
 *
 *     <p>The correctness of this implementation has been verified against the published output
 *     sequence <a
 *     href="http://www.math.keio.ac.jp/~nisimura/random/real2/mt19937-2.out">mt19937-2.out</a> of
 *     the C-implementation <a
 *     href="http://www.math.keio.ac.jp/~nisimura/random/real2/mt19937-2.c">mt19937-2.c</a>. (Call
 *     <tt>test(1000)</tt> to print the sequence).
 * <dt>Note that this implementation is <b>not synchronized</b>.
 *
 *     <p><b>Details:</b> MersenneTwister is designed with consideration of the flaws of various
 *     existing generators in mind. It is an improved version of TT800, a very successful generator.
 *     MersenneTwister is based on linear recurrences modulo 2. Such generators are very fast, have
 *     extremely long periods, and appear quite robust. MersenneTwister produces 32-bit numbers, and
 *     every <tt>k</tt>-dimensional vector of such numbers appears the same number of times as
 *     <tt>k</tt> successive values over the period length, for each <tt>k &lt;= 623</tt> (except
 *     for the zero vector, which appears one time less). If one looks at only the first <tt>n &lt;=
 *     16</tt> bits of each number, then the property holds for even larger <tt>k</tt>, as shown in
 *     the following table (taken from the publication cited above): <div align="center">
 *
 ...
 *     </div>
 *
 *     <p>MersenneTwister generates random numbers in batches of 624 numbers at a time, so the
 *     caching and pipelining of modern systems is exploited. The generator is implemented to
 *     generate the output by using the fastest arithmetic operations only: 32-bit additions and bit
 *     operations (no division, no multiplication, no mod). These operations generate sequences of
 *     32 random bits (<tt>int</tt>'s). <tt>long</tt>'s are formed by concatenating two 32 bit
 *     <tt>int</tt>'s. <tt>float</tt>'s are formed by dividing the interval <tt>[0.0,1.0]</tt> into
 *     2<sup>32</sup> sub intervals, then randomly choosing one subinterval. <tt>double</tt>'s are
 *     formed by dividing the interval <tt>[0.0,1.0]</tt> into 2<sup>64</sup> sub intervals, then
