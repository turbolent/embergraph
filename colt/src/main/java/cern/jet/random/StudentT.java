 ...
 *
 * <p><tt>p(x) = k * (1+x^2/f) ^ -(f+1)/2</tt> where <tt>k = g((f+1)/2) / (sqrt(pi*f) * g(f/2))</tt>
 * and <tt>g(a)</tt> being the gamma function and <tt>f</tt> being the degrees of freedom.
 *
 ...
 *
 *     <p>R.W. Bailey (1994): Polar generation of random variates with the t-distribution,
...
public class StudentT extends AbstractContinousDistribution {
  ...
  public double nextDouble(double degreesOfFreedom) {
    ...
    return (u
        * Math.sqrt(
            degreesOfFreedom * (Math.exp(-2.0 / degreesOfFreedom * Math.log(w)) - 1.0) / w));
  }
