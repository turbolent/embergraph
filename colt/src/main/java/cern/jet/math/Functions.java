 ...
 * Essentially, a function object is an object that can perform a function on some arguments. It has
 * a minimal interface: a method <tt>apply</tt> that takes the arguments, computes something and
 * returns some result value. Function objects are comparable to function pointers in C used for
 * call-backs.
 *
 * <p>Unary functions are of type {@link cern.colt.function.DoubleFunction}, binary functions of
 * type {@link cern.colt.function.DoubleDoubleFunction}. All can be retrieved via <tt>public static
 * final</tt> variables named after the function. Unary predicates are of type {@link
 * cern.colt.function.DoubleProcedure}, binary predicates of type {@link
 * cern.colt.function.DoubleDoubleProcedure}. All can be retrieved via <tt>public static final</tt>
 * variables named <tt>isXXX</tt>.
 *
 * <p>Binary functions and predicates also exist as unary functions with the second argument being
 * fixed to a constant. These are generated and retrieved via factory methods (again with the same
 * name as the function). Example:
 *
 * <ul>
 ...
 * </ul>
 *
 * More general, any binary function can be made an unary functions by fixing either the first or
 * the second argument. See methods {@link #bindArg1(DoubleDoubleFunction,double)} and {@link
 * #bindArg2(DoubleDoubleFunction,double)}. The order of arguments can be swapped so that the first
 * argument becomes the second and vice-versa. See method {@link #swapArgs(DoubleDoubleFunction)}.
 * Example:
 *
 * <ul>
 *   <li><tt>Functions.pow</tt> gives the function <tt>a<sup>b</sup></tt>.
 ...
 * </ul>
 *
 * <p>Even more general, functions can be chained (composed, assembled). Assume we have two unary
 * functions <tt>g</tt> and <tt>h</tt>. The unary function <tt>g(h(a))</tt> applying both in
 * sequence can be generated via {@link #chain(DoubleFunction,DoubleFunction)}:
 *
 * <ul>
 *   <li><tt>Functions.chain(g,h);</tt>
 * </ul>
 *
 * Assume further we have a binary function <tt>f</tt>. The binary function <tt>g(f(a,b))</tt> can
 * be generated via {@link #chain(DoubleFunction,DoubleDoubleFunction)}:
 *
 * <ul>
 *   <li><tt>Functions.chain(g,f);</tt>
 * </ul>
 *
 * The binary function <tt>f(g(a),h(b))</tt> can be generated via {@link
 ...
 * </ul>
 *
 * Arbitrarily complex functions can be composed from these building blocks. For example <tt>sin(a)
 * + cos<sup>2</sup>(b)</tt> can be specified as follows:
 *
 * <ul>
 ...
 * </pre>
 *
 * <p>For aliasing see {@link #functions}. Try this
 *
 * <table>
 * <td class="PRE">
 * <pre>
 ...
 * <H3>Performance</H3>
 *
 * Surprise. Using modern non-adaptive JITs such as SunJDK 1.2.2 (java -classic) there seems to be
 * no or only moderate performance penalty in using function objects in a loop over traditional code
 * in a loop. For complex nested function objects (e.g.
 * <tt>F.chain(F.abs,F.chain(F.plus,F.sin,F.chain(F.square,F.cos)))</tt>) the penalty is zero, for
 * trivial functions (e.g. <tt>F.plus</tt>) the penalty is often acceptable. <center>
 *
...
public class Functions {
   ...
   *
   * <p>is a bit awkward, to say the least. Using the aliasing you can instead write
   *
   * <p><tt>Functions F = Functions.functions; <br>
  ...
  */
  ...
  public static final DoubleFunction log2 =
      a -> Math.log(a) * 1.4426950408889634;
  ...
  */
  ...
  public static DoubleDoubleFunction chain(
      final DoubleDoubleFunction f, final DoubleFunction g, final DoubleFunction h) {
  ...
  public static void demo1() {
    ...
    DoubleDoubleFunction g =
        (x, y) -> Math.sin(x) + Math.pow(Math.cos(y), 2);
    System.out.println(g.apply(a, b));
  ...
  public static void demo2(int size) {
    ...
    DoubleDoubleFunction g =
        (x, y) -> Math.abs(Math.sin(x) + Math.pow(Math.cos(y), 2));
    System.out.println(g.apply(a, b));
   ...
   *
   * <p>Note that any random engine derived from {@link cern.jet.random.engine.RandomEngine} and any
   * random distribution derived from {@link cern.jet.random.AbstractDistribution} are function
   * objects, because they implement the proper interfaces. Thus, if you are not happy with the
   * default, just pass your favourite random generator to function evaluating methods.
   */
