...
public class Arithmetic extends Constants {
   ...
   * </pre>
   *
   * Coefficients are stored in reverse order, i.e. the zero order term is last in the array. Note N
   * is the number of coefficients, not the order.
   *
   * <p>If coefficients are for the interval a to b, x must have been transformed to x -> 2(2x - b -
   * a)/(b-a) before entering the routine. This maps x from (a, b) to (-1, 1), over which the
   * Chebyshev polynomials are defined.
   *
   * <p>If the coefficients are for the inverted interval, in which (a, b) is mapped to (1/b, 1/a),
   * the transformation required is x -> 2(2ab/x - b - a)/(b-a). If b is infinity, this becomes x ->
   * 4a/x - 1.
   *
   * <p>SPEED:
   *
   * <p>Taking advantage of the recurrence properties of the Chebyshev polynomials, the routine
   * requires one more addition per loop than evaluating a nested polynomial of the same degree.
   *
   ...
   * <dt>This method is safer than using (long) Math.floor(value), because of possible rounding
   *     error.
   */
   ...
   * values for small k) with int parameter k.
   *
   * <p><tt> log k! = (k + 1/2)log(k + 1) - (k + 1) + (1/2)log(2Pi) + stirlingCorrection(k + 1)
   *
   * <p>log k! = (k + 1/2)log(k) - k + (1/2)log(2Pi) + stirlingCorrection(k) </tt>
   */
