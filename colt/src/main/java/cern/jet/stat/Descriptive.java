...
public class Descriptive extends Object {
  ...
  public static double autoCorrelation(
      DoubleArrayList data, int lag, double mean, double variance) {
    int N = data.size();
    if (lag >= N) throw new IllegalArgumentException("Lag is too large");

    double[] elements = data.elements();
    double run = 0;
    for (int i = lag; i < N; ++i) run += (elements[i] - mean) * (elements[i - lag] - mean);
   ...
   *
   * <p>Distinct values are filled into <tt>distinctValues</tt>, starting at index 0. The frequency
   * of each distinct value is filled into <tt>frequencies</tt>, starting at index 0. As a result,
   * the smallest distinct value (and its frequency) can be found at index 0, the second smallest
   * distinct value (and its frequency) at index 1, ..., the largest distinct value (and its
   * frequency) at index <tt>distinctValues.size()-1</tt>.
   *
   * <p><b>Example:</b> <br>
  ...
  public static void frequencies(
      DoubleArrayList sortedData, DoubleArrayList distinctValues, IntArrayList frequencies) {
   ...
   * <tt>Math.exp( Sum( Log(data[i]) ) / size)</tt>.
   */
   ...
   * tries to avoid overflows at the expense of an equivalent but somewhat slow definition: <tt>geo
   * = Math.exp( Sum( Log(data[i]) ) / data.size())</tt>.
   */
   ...
   * sequence.
   *
   * <p>Assume we have already recorded some data sequence elements and know their minimum, maximum,
   * sum and sum of squares. Assume further, we are to record some more elements and to derive
   * updated values of minimum, maximum, sum and sum of squares.
   *
   * <p>This method computes those updated values without needing to know the already recorded
   * elements. This is interesting for interactive online monitoring and/or applications that cannot
   * keep the entire huge data sequence in memory.
   *
   * <p><br>
   * Definition of sumOfSquares: <tt>sumOfSquares(n) = Sum ( data[i] * data[i] )</tt>.
   *
   ...
   * @param to the index of the last element within <tt>data</tt> to consider. The method
   ...
   *
   * <p>Assume we have already recorded some data sequence elements <tt>data[i]</tt> and know the
   * values of <tt>Sum(data[i]<sup>from</sup>), Sum(data[i]<sup>from+1</sup>), ...,
   * Sum(data[i]<sup>to</sup>)</tt>. Assume further, we are to record some more elements and to
   * derive updated values of these sums.
   *
   * <p>This method computes those updated values without needing to know the already recorded
   * elements. This is interesting for interactive online monitoring and/or applications that cannot
   * keep the entire huge data sequence in memory. For example, the incremental computation of
   * moments is based upon such sums of powers:
   *
   * <p>The moment of <tt>k</tt>-th order with constant <tt>c</tt> of a data sequence, is given by
   * <tt>Sum( (data[i]-c)<sup>k</sup> ) / data.size()</tt>. It can incrementally be computed by
   * using the equivalent formula
   *
   * <p><tt>moment(k,c) = m(k,c) / data.size()</tt> where <br>
   * <tt>m(k,c) = Sum( -1<sup>i</sup> * b(k,i) * c<sup>i</sup> * sumOfPowers(k-i))</tt> for <tt>i =
   * 0 .. k</tt> and <br>
   * <tt>b(k,i) = </tt>{@link cern.jet.math.Arithmetic#binomial(long,long) binomial(k,i)} and <br>
   ...
   *       <li>...
   *       <li><tt>sumOfPowers[toSumIndex-fromSumIndex]</tt> is the old
   *           <tt>Sum(data[i]<sup>toSumIndex</sup>)</tt>.
   *     </ul>
   *     If no data sequence elements have so far been recorded set all old values of the sums to
   *     <tt>0.0</tt>.
   ...
   * squares. Assume further, we are to record some more elements and to derive updated values of
   ...
   * Definition of sumOfSquares: <tt>sumOfSquares = Sum ( data[i] * data[i] * weights[i])</tt>.
   *
   * @param data the additional elements to be incorporated into min, max, etc.
   * @param weights the weight of each element within <tt>data</tt>.
   * @param from the index of the first element within <tt>data</tt> (and <tt>weights</tt>) to
   *     consider.
   * @param to the index of the last element within <tt>data</tt> (and <tt>weights</tt>) to
   *     consider. The method incorporates elements <tt>data[from], ..., data[to]</tt>.
   * @param inOut the old values in the following format:
   *     <ul>
  ...
  public static void incrementalWeightedUpdate(
      DoubleArrayList data, DoubleArrayList weights, int from, int to, double[] inOut) {
   ...
   *
   * @param sumOfPowers <tt>sumOfPowers[m] == Sum( data[i]<sup>m</sup>) )</tt> for <tt>m =
   *     0,1,..,k</tt> as returned by method {@link
   *     #incrementalUpdateSumsOfPowers(DoubleArrayList,int,int,int,int,double[])}. In particular
   *     there must hold <tt>sumOfPowers.length == k+1</tt>.
   ...
   * @return the percentage <tt>phi</tt> of elements <tt>&lt;= element</tt> (<tt>0.0 &lt;= phi &lt;=
   *     1.0)</tt>.
   */
   ...
   *     be in the interval <tt>[0.0,1.0]</tt>.
   ...
   * lies in between two contained elements, then linear interpolation is used and a non integer
   * value is returned.
   *
   * @param sortedList the list to be searched (must be sorted ascending).
   * @param element the element to search for.
   ...
   * biological research (W.H. Freeman and Company, New York, 1998, 3rd edition) p. 114-115.
   *
   * @param size the number of elements of the data sequence.
   * @param moment4 the fourth central moment, which is <tt>moment(data,4,mean)</tt>.
   ...
   *
   * <p>Ref: R.R. Sokal, F.J. Rohlf, Biometry: the principles and practice of statistics in
   * biological research (W.H. Freeman and Company, New York, 1998, 3rd edition) p. 138.
   *
   * @param size the number of elements of the data sequence.
   */
   ...
   */
  public static double sampleWeightedVariance(
      double sumOfWeights, double sumOfProducts, double sumOfSquaredProducts) {
   ...
   *   <tt>splitters=(2,8)</tt> yields 3 bins: <tt>(1), (2,3,4,5) (8,8,8,10,11)</tt>. <br>
   *   <tt>splitters=()</tt> yields 1 bin: <tt>(1,2,3,4,5,8,8,8,10,11)</tt>. <br>
   *   <tt>splitters=(-5)</tt> yields 2 bins: <tt>(), (1,2,3,4,5,8,8,8,10,11)</tt>. <br>
   *   <tt>splitters=(100)</tt> yields 2 bins: <tt>(1,2,3,4,5,8,8,8,10,11), ()</tt>.
   * </ul>
   *
   ...
   * @return the sublists (an array with <tt>length == splitters.size() + 1</tt>. Each sublist is
  ...
  public static DoubleArrayList[] split(DoubleArrayList sortedList, DoubleArrayList splitters) {
    ...
    while (nextStart < listSize && i < noOfBins - 1) {
      ...
      if (index < 0) { // splitValue not found
        ...
        // For multiple identical elements ("runs"), binarySearch does not define which of all valid
        // indexes is returned.
  ...
  public static double sumOfPowerDeviations(
      final DoubleArrayList data, final int k, final double c, final int from, final int to) {
  ...
  public static double trimmedMean(DoubleArrayList sortedData, double mean, int left, int right) {
    ...
    double[] sortedElements = sortedData.elements();
    int N0 = N;
    for (int i = 0; i < left; ++i) mean += (mean - sortedElements[i]) / (--N);
    for (int i = 0; i < right; ++i) mean += (mean - sortedElements[N0 - 1 - i]) / (--N);
    return mean;
  }
  ...
  public static double winsorizedMean(
      DoubleArrayList sortedData, double mean, int left, int right) {
    int N = sortedData.size();
    if (N == 0) throw new IllegalArgumentException("Empty data.");
    if (left + right >= N) throw new IllegalArgumentException("Not enough data.");
    ...
    for (int i = 0; i < left; ++i) mean += (leftElement - sortedElements[i]) / N;

    double rightElement = sortedElements[N - 1 - right];
    for (int i = 0; i < right; ++i) mean += (rightElement - sortedElements[N - 1 - i]) / N;
