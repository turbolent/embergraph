 ...
 *     HREF="http://www.sci.usq.edu.au/staff/leighb/graph/Top.html">Java 2D Graph Package 2.4</A>,
 *     which in turn is a port from the <A
 *     HREF="http://people.ne.mediaone.net/moshier/index.html#Cephes">Cephes 2.2</A> Math Library
 *     (C). Most Cephes code (missing from the 2D Graph Package) directly ported.
 *
...
public class Probability extends cern.jet.math.Constants {
   ...
   * </pre>
   *
   * This function is identical to the incomplete beta integral function <tt>Gamma.incompleteBeta(a,
   * b, x)</tt>.
   *
   * <p>The complemented function is
   *
   * <p><tt>1 - P(1-x) = Gamma.incompleteBeta( b, a, x )</tt>;
   */
   ...
   * </pre>
   *
   * The terms are not summed directly; instead the incomplete beta integral is employed, according
   * to the formula
   *
   * <p><tt>y = binomial( k, n, p ) = Gamma.incompleteBeta( n-k, k+1, 1-p )</tt>.
   *
   * <p>All arguments must be positive,
   *
   * @param k end term.
   ...
   * to the formula
   *
   * <p><tt>y = binomialComplemented( k, n, p ) = Gamma.incompleteBeta( k+1, n-k, p )</tt>.
   *
   * <p>All arguments must be positive,
   *
   * @param k end term.
   ...
   *
   * <p>The incomplete gamma integral is used, according to the formula
   *
   * <p><tt>y = chiSquare( v, x ) = incompleteGamma( v/2.0, x/2.0 )</tt>.
   *
   * <p>The arguments must both be positive.
   *
   ...
   * <p>The incomplete gamma integral is used, according to the formula
   *
   * <p><tt>y = chiSquareComplemented( v, x ) = incompleteGammaComplement( v/2.0, x/2.0 )</tt>.
   *
   * <p>The arguments must both be positive.
   *
   * @param v degrees of freedom.
   ...
   *
   * <p>Code adapted from the <A HREF="http://www.sci.usq.edu.au/staff/leighb/graph/Top.html">Java
   * 2D Graph Package 2.4</A>, which in turn is a port from the <A
   * HREF="http://people.ne.mediaone.net/moshier/index.html#Cephes">Cephes 2.2</A> Math Library (C).
   *
   ...
   * </pre>
   *
   * <b>Implementation:</b> For small x, <tt>erfc(x) = 1 - erf(x)</tt>; otherwise rational
   ...
   *
   * <p><tt>y = Gamma.incompleteGamma( b, a*x )</tt>.
   *
   ...
   * The incomplete gamma integral is used, according to the relation
   *
   * <p>y = Gamma.incompleteGammaComplement( b, a*x ).
   *
   * @param a the paramater a (alpha) of the gamma distribution.
   * @param b the paramater b (beta, lambda) of the gamma distribution.
   * @param x integration end point.
   ...
   * </pre>
   *
   * In a sequence of Bernoulli trials, this is the probability that <tt>k</tt> or fewer failures
   * precede the <tt>n</tt>-th success.
   *
   * <p>The terms are not computed individually; instead the incomplete beta integral is employed,
   * according to the formula
   *
   * <p><tt>y = negativeBinomial( k, n, p ) = Gamma.incompleteBeta( n, k+1, p )</tt>.
   *
   * <p>All arguments must be positive,
   *
   * @param k end term.
   ...
   * </pre>
   *
   * The terms are not computed individually; instead the incomplete beta integral is employed,
   * according to the formula
   *
   ...
   *
   * <p>For small arguments <tt>0 < y < exp(-2)</tt>, the program computes <tt>z = sqrt( -2.0 *
   * log(y) )</tt>; then the approximation is <tt>x = z - log(z)/z - (1/z) P(1/z) / Q(1/z)</tt>.
   * There are two rational functions P/Q, one for <tt>0 < y < exp(-32)</tt> and the other for
   * <tt>y</tt> up to <tt>exp(-2)</tt>. For larger arguments, <tt>w = y - 0.5</tt>, and
  ...
  public static double normalInverse(double y0) throws ArithmeticException {
    ...
    else x1 = z * Polynomial.polevl(z, P2, 8) / Polynomial.p1evl(z, Q2, 8);
    x = x0 - x1;
    if (code != 0) x = -x;
    return (x);
  }
   ...
   * </pre>
   *
   * The terms are not summed directly; instead the incomplete gamma integral is employed, according
   * to the relation
   *
   * <p><tt>y = poisson( k, m ) = Gamma.incompleteGammaComplement( k+1, m )</tt>.
   *
   * <p>The arguments must both be positive.
   *
   ...
   * to the formula
   *
   * <p><tt>y = poissonComplemented( k, m ) = Gamma.incompleteGamma( k+1, m )</tt>.
   *
   * <p>The arguments must both be positive.
   *
   * @param k start term.
   ...
   *
   * <p><tt>1 - studentT(k,t) = 0.5 * Gamma.incompleteBeta( k/2, 1/2, z )</tt> where <tt>z = k/(k +
   * t**2)</tt>.
   *
   * <p>Since the function is symmetric about <tt>t=0</tt>, the area under the right tail of the
   * density is found by calling the function with <tt>-t</tt> instead of <tt>t</tt>.
   *
   ...
   *
   * <p>The function uses the studentT function to determine the return value iteratively.
   *
