 ...
 *     Accepted for Proc. of the 1999 ACM SIGMOD Int. Conf. on Management of Data, Paper (soon)
...
class UnknownDoubleQuantileEstimator extends DoubleQuantileEstimator {
   ...
   * @param precomputeEpsilon the epsilon for which quantiles shall be precomputed; set this value
   *     <=0.0 if nothing shall be precomputed.
   * @param generator a uniform random number generator.
   */
  public UnknownDoubleQuantileEstimator(
      int b, int k, int h, double precomputeEpsilon, RandomEngine generator) {
  ...
  protected DoubleBuffer[] buffersToCollapse() {
    ...
    // if there is only one buffer at the lowest level, then increase its level so that there are at
    // least two at the lowest level.
  ...
  protected static void sortAscendingByLevel(DoubleBuffer[] fullBuffers) {
    new ObjectArrayList(fullBuffers)
        .quickSortFromTo(
            0,
