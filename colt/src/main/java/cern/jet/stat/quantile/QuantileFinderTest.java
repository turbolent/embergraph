...
class QuantileFinderTest {
  ...
  public static double epsilon(
      DoubleArrayList sortedList, DoubleQuantileFinder finder, double phi) {
   ...
   * @param phis double[]
   */
  public static double observedEpsilonAtPhi(
      double phi, ExactDoubleQuantileFinder exactFinder, DoubleQuantileFinder approxFinder) {
    int N = (int) exactFinder.size();

    int exactRank = (int) Utils.epsilonCeiling(phi * N) - 1;
    // System.out.println("exactRank="+exactRank);
    exactFinder
        .quantileElements(new DoubleArrayList(new double[] {phi}))
        .get(0); // just to ensure exactFinder is sorted
    double approxElement =
        approxFinder.quantileElements(new DoubleArrayList(new double[] {phi})).get(0);
            ...
            for (int knownCounter = 0; knownCounter < 2; knownCounter++) {
              ...
              DoubleQuantileFinder finder =
                  QuantileFinderFactory.newDoubleQuantileFinder(
                      known_N, N, epsilon, delta, p, null);
              ...
              */
              ...
              // System.out.print(")=("+b*k/1000.0+","+b+","+k+", "+b*k*8/1024.0/1024.0+",
              // "+Math.round(b*k*8/1024.0/1024.0));
              System.out.print(
                  ")=("
                      + mem * 8.0 / 1024.0 / 1024.0
                      + ",  "
                      + mem / 1000.0
                      + ",  "
  ...
  public static void testQuantileCalculation(String[] args) {
    ...
    approxFinder =
        QuantileFinderFactory.newDoubleQuantileFinder(
            false, max_N, epsilon, delta, quantiles, null);
    ...
    */
    ...
    DoubleQuantileFinder exactFinder =
        QuantileFinderFactory.newDoubleQuantileFinder(false, -1, 0.0, delta, quantiles, null);
    ...
    if (computeExactQuantilesAlso) {
      ...
      */
      ...
      DoubleArrayList observedEpsilons =
          observedEpsilonsAtPhis(
              new DoubleArrayList(phis),
              (ExactDoubleQuantileFinder) exactFinder,
              approxFinder,
              epsilon);
      ...
      System.out.println(
          "exact elem(phi("
              + element
              + "))="
              + exactFinder.quantileElements(
                  new DoubleArrayList(new double[] {exactFinder.phi(element)})));
      System.out.println(
          "apprx elem(phi("
              + element
              + "))="
              + approxFinder.quantileElements(
                  new DoubleArrayList(new double[] {approxFinder.phi(element)})));
    }
  }
  /** Not yet commented. */
