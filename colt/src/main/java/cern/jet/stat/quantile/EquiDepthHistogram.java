 ...
 * class does not collect equi-depth histograms but only space efficiently stores already produced
 * histograms and provides operations for selectivity estimation. Uses linear interpolation.
 *
 * <p>This class stores a list <tt>l</tt> of <tt>float</tt> values for which holds:
 * <li>Let <tt>v</tt> be a list of values (sorted ascending) an equi-depth histogram has been
 *     computed over.
 * <li>Let <tt>s=l.length</tt>.
 * <li>Let <tt>p=(0, 1/s-1), 2/s-1,..., s-1/s-1=1.0)</tt> be a list of the <tt>s</tt> percentages.
 * <li>Then for each <tt>i=0..s-1: l[i] = e : v.contains(e) && v[0],..., v[p[i]*v.length] &lt;=
 *     e</tt>.
 * <li>(In particular: <tt>l[0]=min(v)=v[0]</tt> and <tt>l[s-1]=max(v)=v[s-1]</tt>.)
 *
...
public class EquiDepthHistogram extends cern.colt.PersistentObject {
  ...
  /*
   * Constructs an equi-depth histogram with the given quantile elements. Quantile elements must be
   * sorted ascending and have the form specified in the class documentation.
   */
  public EquiDepthHistogram(float[] quantileElements) {
    this.binBoundaries = quantileElements;
  }
  /*
   * Returns the bin index of the given element. In other words, returns a handle to the range the
   * element falls into.
   *
  ...
  /*
   * Returns the number of bins. In other words, returns the number of subdomains partitioning the
   * entire value domain.
   */
  public int bins() {
