 ...
 *
 *     <p>You usually don't instantiate quantile finders by using the constructor. Instead use the
 *     factory <tt>QuantileFinderFactor</tt> to do so. It will set up the right parametrization for
 *     you.
 *
...
class KnownDoubleQuantileEstimator extends DoubleQuantileEstimator {
   ...
   * @param samplingRate 1.0 --> all elements are consumed. 10.0 --> Consumes one random element
   *     from successive blocks of 10 elements each. Etc.
   * @param generator a uniform random number generator.
   */
  public KnownDoubleQuantileEstimator(
      int b, int k, long N, double samplingRate, RandomEngine generator) {
   ...
   * buffer become full.
   *
   * <p>This method removes the infinities which were previously temporarily added to a partial
   * buffer. Removing them is necessary if we want to continue filling more elements. Precondition:
   * the buffer is sorted ascending.
   *
  ...
  protected boolean sampleNextElement() {
     ...
     */
    // if ((totalElementsFilled-1) % N == 0) setSamplingRate(samplingRate, N); // delete if
    // appropriate
