 ...
 * used.
 *
 * <p>The approx. algorithms compute approximate quantiles of large data sequences in a single pass.
 * The approximation guarantees are explicit, and apply for arbitrary value distributions and
 * arrival distributions of the data sequence. The main memory requirements are smaller than for any
 * other known technique by an order of magnitude.
 *
 * <p>The approx. algorithms are primarily intended to help applications scale. When faced with a
 * large data sequences, traditional methods either need very large memories or time consuming disk
 * based sorting. In constrast, the approx. algorithms can deal with > 10^10 values without disk
 * based sorting.
 *
 ...
 *
 *     <p>Use methods <tt>newXXX(...)</tt> to get new instances of one of the following quantile
 *     finders.
 *
 *     <p><b>1. Exact quantile finding algorithm for known and unknown <tt>N</tt> requiring large
 *     main memory.</b> The folkore algorithm: Keeps all elements in main memory, sorts the list,
 *     then picks the quantiles.
 *
 *     <p>
 *
 *     <p><b>2. Approximate quantile finding algorithm for known <tt>N</tt> requiring only one pass
 *     and little main memory.</b>
 *
 *     <p>Needs as input the following parameters:
 *
 *     <p>
 * <dt>1. <tt>N</tt> - the number of values of the data sequence over which quantiles are to be
 *     determined.
 ...
 *     reduce memory requirements. With sampling, the approximation guarantees are explicit but
 *     probabilistic, i.e. they apply with respect to a (user controlled) confidence parameter
 *     "delta".
 ...
 *
 *     <p>After Gurmeet Singh Manku, Sridhar Rajagopalan and Bruce G. Lindsay, Approximate Medians
 *     and other Quantiles in One Pass and with Limited Memory, Proc. of the 1998 ACM SIGMOD Int.
 *     Conf. on Management of Data, Paper available <A
 *     HREF="http://www-cad.eecs.berkeley.edu/~manku/papers/quantiles.ps.gz"> here</A>.
 *
 *     <p>
 *
 *     <p><b>3. Approximate quantile finding algorithm for unknown <tt>N</tt> requiring only one
 *     pass and little main memory.</b> This algorithm requires at most two times the memory of a
 *     corresponding approx. quantile finder knowing <tt>N</tt>.
 *
 *     <p>Needs as input the following parameters:
 *
 *     <p>
 * <dt>2. <tt>quantiles</tt> - the number of quantiles to be computed. If unknown in advance, set
 *     this number large, e.g. <tt>quantiles &gt;= 1000</tt>.
 ...
 *     probabilistic, i.e. they apply with respect to a (user controlled) confidence parameter
 *     "delta".
 * <dt>3. <tt>delta</tt> - the probability allowed that the approximation error fails to be smaller
 *     than epsilon. Set <tt>delta</tt> to zero for explicit non probabilistic guarantees.
 *
 *     <p>After Gurmeet Singh Manku, Sridhar Rajagopalan and Bruce G. Lindsay, Random Sampling
 *     Techniques for Space Efficient Online Computation of Order Statistics of Large Datasets.
 *     Proc. of the 1999 ACM SIGMOD Int. Conf. on Management of Data, Paper available <A
...
public class QuantileFinderFactory extends Object {
   ...
   *
   * <p>Assumes that quantiles are to be computed over N values. The required sampling rate is
   * computed and stored in the first element of the provided <tt>returnSamplingRate</tt> array,
   ...
   * @param epsilon the approximation error which is guaranteed not to be exceeded (e.g.
   *     <tt>0.001</tt>) (<tt>0 &lt;= epsilon &lt;= 1</tt>). To get exact result, set
   *     <tt>epsilon=0.0</tt>;
   ...
   *     <tt>delta=0.0</tt>.
   * @param quantiles the number of quantiles to be computed (e.g. <tt>100</tt>) (<tt>quantiles
   *     &gt;= 1</tt>). If unknown in advance, set this number large, e.g. <tt>quantiles &gt;=
   *     10000</tt>.
   * @param samplingRate output parameter, a <tt>double[1]</tt> where the sampling rate is to be
   *     filled in.
   * @return <tt>long[2]</tt> - <tt>long[0]</tt>=the number of buffers, <tt>long[1]</tt>=the number
   *     of elements per buffer, <tt>returnSamplingRate[0]</tt>=the required sampling rate.
   */
  public static long[] known_N_compute_B_and_K(
      long N, double epsilon, double delta, int quantiles, double[] returnSamplingRate) {
  ...
  protected static long[] known_N_compute_B_and_K_quick(long N, double epsilon) {
    ...
    // with x = binomial(b+h-2, h-1) - binomial(b+h-3, h-3) + binomial(b+h-3, h-2) - N * epsilon *
    // 2.0
   ...
   * stored in the first element of the provided <tt>returnSamplingRate</tt> array, which, therefore
   ...
   * constraints.
   *
   * <p>Many applications don't know in advance over how many elements quantiles are to be computed.
   * However, some of them can give an upper limit, which will assist the factory in choosing
   * quantile finders with minimal memory requirements. For example if you select values from a
   * database and fill them into histograms, then you probably don't know how many values you will
   * fill, but you probably do know that you will fill at most <tt>S</tt> elements, the size of your
   * database.
   *
   * @param known_N specifies whether the number of elements over which quantiles are to be computed
   *     is known or not.
   * @param N if <tt>known_N==true</tt>, the number of elements over which quantiles are to be
   *     computed. if <tt>known_N==false</tt>, the upper limit on the number of elements over which
   *     quantiles are to be computed. If such an upper limit is a-priori unknown, then set <tt>N =
   *     Long.MAX_VALUE</tt>.
   * @param epsilon the approximation error which is guaranteed not to be exceeded (e.g.
   *     <tt>0.001</tt>) (<tt>0 &lt;= epsilon &lt;= 1</tt>). To get exact result, set
   *     <tt>epsilon=0.0</tt>;
   * @param delta the probability that the approximation error is more than than epsilon (e.g.
   *     0.0001) (0 &lt;= delta &lt;= 1). To avoid probabilistic answers, set <tt>delta=0.0</tt>.
   * @param quantiles the number of quantiles to be computed (e.g. <tt>100</tt>) (<tt>quantiles
   *     &gt;= 1</tt>). If unknown in advance, set this number large, e.g. <tt>quantiles &gt;=
   *     10000</tt>.
   * @param generator a uniform random number generator. Set this parameter to <tt>null</tt> to use
   *     a default generator.
   * @return the quantile finder minimizing memory requirements under the given constraints.
   */
  public static DoubleQuantileFinder newDoubleQuantileFinder(
      boolean known_N,
      long N,
      double epsilon,
      double delta,
    ...
    if (known_N) {
      ...
      if (true) { // no maximum N provided by user.
        ...
        return new UnknownDoubleQuantileEstimator(
      ...
      // with knownN-sampling we can only guarantee the errors if the input sequence has EXACTLY N
      // elements.
      // with knownN-no sampling we can also guarantee the errors for sequences SMALLER than N
      // elements.
   ...
   *
   * @param epsilon the approximation error which is guaranteed not to be exceeded (e.g.
   *     <tt>0.001</tt>) (<tt>0 &lt;= epsilon &lt;= 1</tt>). To get exact results, set
   *     <tt>epsilon=0.0</tt>;
   * @param delta the probability that the approximation error is more than than epsilon (e.g.
   *     <tt>0.0001</tt>) (<tt>0 &lt;= delta &lt;= 1</tt>). To get exact results, set
   *     <tt>delta=0.0</tt>.
   * @param quantiles the number of quantiles to be computed (e.g. <tt>100</tt>) (<tt>quantiles
   *     &gt;= 1</tt>). If unknown in advance, set this number large, e.g. <tt>quantiles &gt;=
   *     10000</tt>.
   * @return <tt>long[4]</tt> - <tt>long[0]</tt>=the number of buffers, <tt>long[1]</tt>=the number
   *     of elements per buffer, <tt>long[2]</tt>=the tree height where sampling shall start,
   ...
   *     <tt>long[3]==1</tt> if precomputing is better, otherwise 0;
   */
  protected static long[] unknown_N_compute_B_and_K_raw(
      double epsilon, double delta, int quantiles) {
    ...
    while (best_b == Long.MAX_VALUE && max_Iterations-- > 0) { // until we find a solution
      ...
      for (int b = 2; b <= max_b; b++) {
        for (int h = 2; h <= max_h; h++) {
          ...
          // let's compute c.
          // double c = Math.log(2.0/(delta/quantiles)) / (2.0*epsilon*epsilon*Math.min(Ld,
          // 8.0*Ls/3.0));
