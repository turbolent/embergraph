 ...
 *
 * <p>TODO Add unit tests which are specific to sids and quads modes. These tests should look at the
 * semantics of interchange of sids or quads specific data; queries which exercise the context
 * position; and the default-graph and named-graph URL query parameters for quads.
 *
...
public class TestNanoSparqlServerWithProxyIndexManager<S extends IIndexManager>
  ...
  static Journal getTemporaryJournal(final BufferMode bufferMode) {
    ...
    properties.setProperty(org.embergraph.journal.Options.BUFFER_MODE, bufferMode.toString());

    // Enable GROUP_COMMIT. See BLZG-192
    properties.setProperty(org.embergraph.journal.Journal.Options.GROUP_COMMIT, "false");
    ...
    if (bufferMode.isStable()) {
      ...
      properties.setProperty(org.embergraph.journal.Options.DELETE_ON_CLOSE, "true");
   ...
   *
   * @param indexManager The {@link Journal} or JiniFederation.
   * @param testMode Identifies what mode the kb instance will be using.
   */
  private TestNanoSparqlServerWithProxyIndexManager(
  ...
  public static class Test_NSS_quads extends TestCase {
    public static Test suite() {
      return TestNanoSparqlServerWithProxyIndexManager.suite(getTemporaryJournal(), TestMode.quads);
    }
  }
  ...
  public static class Test_NSS_sids extends TestCase {
    public static Test suite() {
      return TestNanoSparqlServerWithProxyIndexManager.suite(getTemporaryJournal(), TestMode.sids);
    }
  }

  /** Return suite running in the given mode against the given {@link IIndexManager}. */
  public static TestSuite suite(final IIndexManager indexManager, final TestMode testMode) {

    final boolean RWStoreMode =
        indexManager instanceof AbstractJournal
    ...
    if (RWStoreMode) {
      ...
      suite.addTestSuite(
          TestEmbergraphSailRemoteRepository.ReadWriteTx.class); // with isolatable indices.
  ...
  static ProxyTestSuite createProxyTestSuite(
      final IIndexManager indexManager, final TestMode testMode) {
    final TestNanoSparqlServerWithProxyIndexManager<?> delegate =
        new TestNanoSparqlServerWithProxyIndexManager(
  ...
  public Properties getProperties() {
    ...
    if (testMode == null) throw new IllegalStateException();
  ...
  public static void main(final String[] args) throws Exception {
    ...
    try {
      ...
      // Setup test suite
      final Test test = TestNanoSparqlServerWithProxyIndexManager.suite(indexManager, testMode);
