 ...
 * the subsequent attempts to resolve the KB fail - probably due to an issue with the default
 * resource locator cache. This is observed in the HA test suite where we wait for a quorum meet and
 * then spin in a loop looking for the KB to be concurrently created by the NSS. This test suite was
 * written to replicate and diagnose this problem. A representative stack trace is below. Once this
 * trace is generated, it is produced repeatedly. Presumably, a restart of the service would cure
 * the stack trace since it is a cache side effect.
 *
 * <pre>
 ...
 * </pre>
 *
 * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/617">Concurrent KB create fails
 *     with "No axioms defined?" </a>
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 */
public abstract class TestConcurrentKBCreate extends ProxyEmbergraphSailTestCase {
  ...
  public void test_2KBCreateAndDiscovery() throws Exception {
    ...
    try {
      ...
      // Create the KB instance.
      AbstractApiTask.submitApiTask(jnl, new CreateKBTask(namespace2, properties)).get();
   ...
   *
   * @see <a href="http://trac.blazegraph.com/ticket/948">Create/Destroy of KB followed by Create
   *     with different Vocabulary causes runtime exception </a>
   */
  public void test_CreateDestroy_ticket_948_00() throws Exception {
    ...
    properties1.setProperty(
        AbstractTripleStore.Options.VOCABULARY_CLASS, BSBMVocabulary.class.getName());
  ...
  public void test_CreateDestroy_ticket_948_01() throws Exception {
    ...
    properties2.setProperty(
        AbstractTripleStore.Options.VOCABULARY_CLASS, RDFSVocabulary.class.getName());
   ...
   * properties have been changed (especially the Vocabulary class).
   *
   * <p>In this variant, we shutdown the Journal and then re-open it.
   *
   * @see <a href="http://trac.blazegraph.com/ticket/948">Create/Destroy of KB followed by Create
   *     with different Vocabulary causes runtime exception </a>
   */
   ...
   *
   * <p>Note: This does not attempt to create the distinct KBs concurrently.
   */
  ...
  /** Run several concurrent {@link DiscoveryTask}s and one {@link CreateKBTask}. */
  private void doConcurrentCreateAndDiscoveryTest(
      final IIndexManager indexManager, final String namespace) throws Exception {
    ...
    final List<Future<Void>> futures = indexManager.getExecutorService().invokeAll(tasks);
  ...
  private class DiscoveryTask implements Callable<Void> {
    ...
    public DiscoveryTask(
        ...
        final AtomicBoolean discovered,
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        conn = getQueryConnection(indexManager, namespace, ITx.READ_COMMITTED);
   ...
   * @return The connection -or- <code>null</code> if the triple store instance was not found.
   * @throws SailException
   */
  private EmbergraphSailRepositoryConnection getQueryConnection(
     ...
     */
    final AbstractTripleStore tripleStore = getTripleStore(indexManager, namespace, timestamp);
  ...
  //        repo.initialize();
  //
  //        final EmbergraphSailRepositoryConnection conn = (EmbergraphSailRepositoryConnection)
  // repo
