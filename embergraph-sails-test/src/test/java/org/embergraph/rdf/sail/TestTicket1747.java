...
public class TestTicket1747 extends QuadsTestCase {
  ...
  public void testBug() throws Exception {
    ...
    //		Setup a triple store using a vocabulary that declares xsd:dateTime (this should be in the
    // default vocabulary).
    ...
    try {
      ...
      try {
        ...
        IV xsdDateTimeIV =
            conn.getTripleStore()
                .getLexiconRelation()
                .resolve(vf.createURI(XMLSchema.DATETIME.stringValue()))
                .getIV();
        ...
        xsdDateTimeIV =
            conn.getTripleStore().getLexiconRelation().resolve(XMLSchema.DATETIME).getIV();
        assertFalse(xsdDateTimeIV.isNullIV());
        assertTrue(xsdDateTimeIV.isVocabulary());
