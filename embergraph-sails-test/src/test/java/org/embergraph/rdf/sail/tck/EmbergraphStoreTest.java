...
public class EmbergraphStoreTest extends RDFStoreTest {
  ...
  public void tearDown() throws Exception {
    ...
    final IIndexManager backend = sail == null ? null : ((EmbergraphSail) sail).getIndexManager();
  ...
  protected Sail createSail() throws SailException {
    ...
    final Sail sail = new EmbergraphSail(getProperties());
   ...
   * semantics for transactional isolation.
   *
   * <p>Also modified to use SPARQL instead of SeRQL.
   */
  @Override
  ...
  private CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluate(
      final String query, final SailConnection con) throws Exception {

    return evaluate(query, con, EmptyBindingSet.getInstance());
  }

  private CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluate(
      final String query, final SailConnection con, final BindingSet bs) throws Exception {

    // new pattern
    ((EmbergraphSailConnection) con).flush();
    final AbstractTripleStore db = ((EmbergraphSailConnection) con).getTripleStore();
    ...
    final TupleQueryResult queryResult =
        ASTEvalHelper.evaluateTupleQuery(
            db, astContainer, new QueryBindingSet(bs), null /* dataset */);
  ...
  protected void testValueRoundTrip(Resource subj, URI pred, Value obj) throws Exception {
    ...
    final String query =
        "SELECT ?S ?P ?O WHERE { ?S ?P ?O filter(?P = <" + pred.stringValue() + ">) }";
  ...
  public void testAddWhileQuerying() throws Exception {
    ...
    //		tupleQuery = QueryParserUtil.parseTupleQuery(QueryLanguage.SERQL, "SELECT P FROM {} P {}",
    // null);
    //		iter = con.evaluate(tupleQuery.getTupleExpr(), null, EmptyBindingSet.getInstance(), false);
  ...
  protected int countQueryResults(String query) throws Exception {
    query =
        "PREFIX ex: <http://example.org/> PREFIX rdf: <"
            + RDF.NAMESPACE
            + "> PREFIX rdfs: <"
            + RDFS.NAMESPACE
            + "> "
