 ...
 */
public class TestEmbergraphExprBuilder extends AbstractEmbergraphExprBuilderTestCase {

  //    private static final Logger log = Logger
  //            .getLogger(TestEmbergraphExprBuilder.class);

  public TestEmbergraphExprBuilder() {}

  public TestEmbergraphExprBuilder(String name) {
    super(name);
  }
   ...
   * </pre>
   *
   * Note: The DESCRIBE projection and where class of the DESCRIBE query are modeled directly, but
   * this does not capture the semantics of the DESCRIBE query. You MUST run an {@link
   ...
   * of describe are imposed by an AST rewrite.
   *
   * <pre>
  ...
  public void test_describe_iri() throws MalformedQueryException, TokenMgrError {
    ...
    final QueryRoot expected = new QueryRoot(QueryType.DESCRIBE);
    {
      ...
      projection.addProjectionExpression(
          new AssignmentNode(
              anonvar,
              new ConstantNode(makeIV(valueFactory.createURI("http://www.embergraph.org")))));
    }
  ...
  public void test_construct_with_ground_terms()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.CONSTRUCT);
    {
      ...
      construct.addChild(
          new StatementPatternNode(
              new VarNode("s"),
              new ConstantNode(makeIV(valueFactory.createURI(RDF.TYPE.toString()))),
              new VarNode("o"),
              null /* c */,
              Scope.DEFAULT_CONTEXTS));
  ...
  public void test_from_and_from_named()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      {
        final EmbergraphURI uri1 = valueFactory.createURI("http://example.org/dft.ttl");

        final EmbergraphURI uri2 = valueFactory.createURI("http://example.org/alice");

        final EmbergraphURI uri3 = valueFactory.createURI("http://example.org/bob");

        final EmbergraphValue[] values = new EmbergraphValue[] {uri1, uri2, uri3};
...

      {
        ...
        whereClause.addChild(
            new StatementPatternNode(
                new VarNode("g"),
                new ConstantNode(makeIV(valueFactory.createURI(DC.PUBLISHER.toString()))),
                new VarNode("who"),
                null /* c */,
                Scope.DEFAULT_CONTEXTS));
        ...
        group.addChild(
            new StatementPatternNode(
                new VarNode("x"),
                new ConstantNode(
                    makeIV(valueFactory.createURI(FOAFVocabularyDecl.mbox.toString()))),
                new VarNode("mbox"),
                new VarNode("g"),
