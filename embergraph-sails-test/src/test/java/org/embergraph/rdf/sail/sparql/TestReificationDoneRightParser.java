 ...
 *     <p>TODO When triple pattern reference is nested within a GRAPH ?g {} clause (verify that the
 *     graph context is inherited correctly).
 *     <p>TODO Test recursive embedded at depth GT 1.
 */
public class TestReificationDoneRightParser extends AbstractEmbergraphExprBuilderTestCase {
  ...
  public void test_triple_ref_pattern_var_const_const()
    ...
    {
      final IV<?, ?> dcSource =
          makeIV(valueFactory.createURI("http://purl.org/dc/elements/1.1/source"));

      final IV<?, ?> bought = makeIV(valueFactory.createURI("http://example.com/bought"));

      final IV<?, ?> sybase = makeIV(valueFactory.createURI("http://example.com/sybase"));
   ...
   * </pre>
   *
   * The triple reference pattern must be translated as if it had been expressed with an explicit
   * variable using BIND(). The equivalent expression for that triple reference pattern is:
   *
   * <pre>
   ...
   * </pre>
   *
   * The BIND() of the triple reference pattern is itself translated into a {@link
   * StatementPatternNode} where the <code>?sid</code> variable appears in the SID role of that
  ...
  public void test_triple_ref_pattern_no_vars()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      final IV<?, ?> SAP = makeIV(valueFactory.createURI("http://example.com/SAP"));
  ...
  public void test_triple_ref_pattern_nested_in_subject_position()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      final IV<?, ?> b = makeIV(valueFactory.createURI("http://example.com/b"));

      final IV<?, ?> c = makeIV(valueFactory.createURI("http://example.com/c"));

      final IV<?, ?> d = makeIV(valueFactory.createURI("http://example.com/d"));
      ...
      final StatementPatternNode sp2 =
          new StatementPatternNode(
              sid1, new ConstantNode(d), new VarNode("e"), null /* c */, Scope.DEFAULT_CONTEXTS);
      sp2.setSid(sid2);
  ...
  public void test_triple_ref_pattern_nested_in_object_position()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      final StatementPatternNode sp2 =
          new StatementPatternNode(
              new VarNode("a"), new ConstantNode(d), sid1, null /* c */, Scope.DEFAULT_CONTEXTS);
      sp2.setSid(sid2);
   ...
   * </pre>
   *
   * Note that the SP for the fully bound triple pattern will enforce the semantics that the triple
  ...
  public void test_triple_ref_pattern_is_constant()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      final IV<?, ?> e = makeIV(valueFactory.createURI("http://example.com/e"));
      ...
      final StatementPatternNode sp2 =
          new StatementPatternNode(
              new VarNode("a"), new ConstantNode(d), sid, null /* c */, Scope.DEFAULT_CONTEXTS);
   ...
   * </pre>
   *
   * Note that the SP for the first bound triple pattern will enforce the semantics that the triple
   * must exist in the data in order for the query to succeed.
   */
  public void test_triple_ref_pattern_all_vars()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      // SP(?a, ?b, ?c) as ?sid .
      final StatementPatternNode sp1 =
          new StatementPatternNode(a, b, c, null /* c */, Scope.DEFAULT_CONTEXTS);
      sp1.setSid(sid);

      // SP(?d, ?e, ?sid).
      final StatementPatternNode sp2 =
          new StatementPatternNode(d, e, sid, null /* c */, Scope.DEFAULT_CONTEXTS);
  ...
  public void test_triple_ref_pattern_all_vars2()
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      // SP(?sid, ?d, ?e).
      final StatementPatternNode sp2 =
          new StatementPatternNode(sid, d, e, null /* c */, Scope.DEFAULT_CONTEXTS);
   ...
   * </pre>
   *
   * @see <a href="http://trac.bigdata.com/ticket/1201" > RDFnot parsed in SPARQL UPDATE </a>
