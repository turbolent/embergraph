 ...
 * @version $Id$
 */
public class EmbergraphSPARQLUpdateTest extends SPARQLUpdateTest {

  private static final Logger logger = Logger.getLogger(EmbergraphSPARQLUpdateTest.class);
  ...
  public void test_ticket538()
    ...
    //		((EmbergraphSailRepository) con.getRepository()).getDatabase().addTerms(
    //				new EmbergraphValue[] { (EmbergraphValue) gin, (EmbergraphValue) gout });
    ...
    final EmbergraphSailUpdate update =
        (EmbergraphSailUpdate) con.prepareUpdate(QueryLanguage.SPARQL, s);
    update.execute();
    ...
    if (false) con.prepareUpdate(QueryLanguage.SPARQL, s).execute();
   ...
   * </pre>
   *
   * The result should be that the combination of tempGraph and graphA should have the exact same
   * number of triples that we started with. But now notice that graphA has 0 triples:
   *
   * <pre>
   * PREFIX graphA:  <http://example/graphA>
   * PREFIX tempGraph:  <http://example/temp>
  ...
  public void testTicket571()
    ...
    con.prepareUpdate(
            ...
            "PREFIX graphA:  <http://example/graphA> \n"
                ...
                + "}}\n"
            //                "LOAD
            // <file:embergraph-sails/src/test/org/openrdf/query/parser/sparql/ticket571.ttl> INTO
            // GRAPH graphA: ;\n"
            )
        .execute();
  ...
  public void testTicket571b()
    ...
    con.prepareUpdate(
            ...
            "PREFIX graphA:  <http://example/graphA> \n"
                ...
                + "   <http://example/s> <http://example/p> 2 . \n"
                + "}}\n"
            // "LOAD <file:embergraph-sails/src/test/org/openrdf/query/parser/sparql/ticket571.ttl>
            // INTO GRAPH graphA: ;\n"
            )
        .execute();
  ...
  protected long countSolutions(final String query)
    ...
    try {
      ...
      while (result.hasNext()) {
        ...
        n++;
        if (logger.isInfoEnabled()) logger.info(bset.toString());
      }
      return n;
    } finally {
      result.close();
