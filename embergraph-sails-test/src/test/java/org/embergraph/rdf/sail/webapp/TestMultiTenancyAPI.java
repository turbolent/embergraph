 ...
 * configured things in order to get this right and the code must be correct as well (in terms of
 * using the configured serviceURL correctly).
 */
public class TestMultiTenancyAPI<S extends IIndexManager> extends AbstractTestNanoSparqlClient<S> {
   ...
   * describes the known data sets, rather than the more detailed report which is provided with the
  ...
  static class VoidSummary {
    ...
    public VoidSummary(final Resource dataset, final Graph g) {
      ...
      for (Statement x :
          getMatches(g, dataset, DCTermsVocabularyDecl.title, null /* namespace */)) {
       ...
       */
      for (Statement x :
          getMatches(g, dataset, VoidVocabularyDecl.sparqlEndpoint, null /* endpointURL */)) {
  ...
  protected Map<Resource, VoidSummary> getRepositoryDescriptions() throws Exception {
    ...
    final Statement[] a = getMatches(g, null /* dataset */, RDF.TYPE, VoidVocabularyDecl.Dataset);
  ...
  public void test_PreparedPropertiesContainNewValues() throws Exception {
    ...
    properties.put(RemoteRepository.OPTION_CREATE_KB_NAMESPACE, namespace);
    properties.put(
        "org.embergraph.namespace."
            + RemoteRepository.DEFAULT_NAMESPACE
            + ".spo.org.embergraph.btree.BTree.branchingFactor",
        "1024");
    properties.put(
        "org.embergraph.namespace."
            + RemoteRepository.DEFAULT_NAMESPACE
            + ".lex.org.embergraph.btree.BTree.branchingFactor",
        "400");
  ...
  private void doTestCreate(final String namespace2) throws Exception {
    ...
    final Map<String /* namespace */, VoidSummary> summaries =
        indexOnNamespace(getRepositoryDescriptions().values());

    // Should be (at least) two such summaries (more if end point is durable
    // across test suite runs, e.g., a federation).
    if (summaries.size() < 2)
      fail("Expecting at least 2 summaries, but only have " + summaries.size());
     ...
     */
    {
      ...
      while (itr.hasNext()) {
        ...
        if (e.getKey().equals(namespace) || e.getKey().equals(namespace2)) continue;
    ...
    for (VoidSummary summary : summaries.values()) {
...

      {
        ...
        // Test a SPARQL 1.1. Query against the data set.
        {
          final TupleQueryResult result =
              tmp.prepareTupleQuery("SELECT (COUNT(*) as ?count) {?s ?p ?o}").evaluate();

          final long nresults = countResults(result);

          if (log.isInfoEnabled()) log.info("namespace=" + ns + ", triples=" + nresults);
        }

        // Test a SPARQL 1.1 Update against the data set.
        {
          tmp.prepareUpdate("PREFIX : <http://www.embergraph.org> \n" + "INSERT DATA {:a :b :c}")
              .evaluate();
        }
      ...
      {
        // Get the summaries, indexed by the data set namespace.
        final Map<String /* namespace */, VoidSummary> summaries2 =
            indexOnNamespace(getRepositoryDescriptions().values());
      ...
      // Properties now fails.
      {
        try {
          ...
          assertEquals(EmbergraphServlet.HTTP_NOTFOUND, ex.getStatusCode());
        }
      }
