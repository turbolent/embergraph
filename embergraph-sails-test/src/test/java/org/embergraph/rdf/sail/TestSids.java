...

          {
            ...
            if (!result.hasNext()) {
              ...
              while (result.hasNext()) {
                ...
                //	                    log.info(bs.getBinding("s").getValue() + " " +
                // bs.getBinding("p").getValue() + " " + bs.getBinding("o").getValue() + " .");
                log.info(
                    (s == null ? bs.getBinding("s").getValue() : s)
                        + " "
                        + bs.getBinding("p").getValue()
                        + " "
    ...
    try {
      ...
      try {
        ...
        final EmbergraphStatement s2 =
            vf.createStatement(host1, connectedTo, swtch1, vf.createBNode());
        final EmbergraphStatement s3 =
            vf.createStatement(host1, connectedTo, swtch2, vf.createBNode());
        final EmbergraphStatement s4 =
            vf.createStatement(swtch1, RDF.TYPE, swtch, vf.createBNode());
        final EmbergraphStatement s5 =
            vf.createStatement(swtch2, RDF.TYPE, swtch, vf.createBNode());
