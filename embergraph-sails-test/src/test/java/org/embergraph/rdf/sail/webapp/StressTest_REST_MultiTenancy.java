 ...
 *     within a parameterized workload.
 */
public class StressTest_REST_MultiTenancy<S extends IIndexManager>
  ...
  public static Test suite() {
    ...
    return ProxySuiteHelper.suiteWhenStandalone(
        StressTest_REST_MultiTenancy.class,
        "test.*",
   ...
   *   <li>load data into a namespace
   *   <li>list the namespaces
   *   <li>run SPARQL against a namespace
   * </ul>
   *
   * Note: This test is intended to run for an hour or more to identify a problem. It is run for a
   * shorter period in CI.
   *
   * @see <a href="http://trac.bigdata.com/ticket/967">Replace Apache Http Components with jetty
  ...
  private void createNamespace(final String namespace) throws Exception {
    ...
    //        final Properties properties = getTestMode().getProperties(); // FIXME BLZG-2023: Use
    // the indicated test mode, but also test for triplesPlusTM.
   ...
   *
   * @param timeoutMillis The test duration. This needs to be at least 15 seconds to allow multiple
   *     namespaces to be created and some queries submitted.
   * @throws Exception
   */
  private void doMultiTenancyStressTest(final long timeoutMillis) throws Exception {
      ...
      executor.submit(
          new Runnable() {
            ...
            public void run() {
              ...
              if (testSucceeding.get()) executor.schedule(this, 5, TimeUnit.SECONDS);
            }
          });
      ...
      executor.submit(
          new Runnable() {
            ...
            public void run() {
              ...
              if (testSucceeding.get()) executor.schedule(this, 1, TimeUnit.SECONDS);
            }
          });
  ...
  private static Statement generateTriple() {
    ...
    return new StatementImpl(
        new URIImpl(URI_PREFIX + UUID.randomUUID()),
        new URIImpl(URI_PREFIX + UUID.randomUUID()),
        new URIImpl(URI_PREFIX + UUID.randomUUID()));
  }
}
