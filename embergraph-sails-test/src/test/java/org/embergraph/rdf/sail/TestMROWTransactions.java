 ...
 */
public abstract class TestMROWTransactions extends ProxyEmbergraphSailTestCase {
   ...
   *     read-only transaction (no sessions). Value is in [1:...].
   * @param nwriters The #of writer tasks (there is only one writer thread).
   * @param nreaders The #of reader tasks.
   * @param isolatableIndice When <code>true</code> the writers will use read/write transactions.
   *     Otherwise they will use the unisolated connection.
   * @throws Exception
   */
  void domultiple_csem_transaction2(
      final int retentionMillis,
      final int nreaderThreads,
      final int nwriters,
      final int nreaders,
    ...
    final EmbergraphSail sail = getSail(getProperties(retentionMillis, isolatableIndices));

    domultiple_csem_transaction2(sail, nreaderThreads, nwriters, nreaders, true);
  }

  static void domultiple_csem_transaction2(
     ...
     *
     * <p>The message of "invalid address" would be generated if an allocation has been freed and is
     * no longer protected from recycling when an attempt is made to read from it.
     *
     * <p>TODO Experiment with different values of [nthreads] for the with and w/o history
     * variations of this test. Consider lifting that parameter into the signature of this method.
     */
    ...
    // file="+((Journal)sail.getDatabase().getIndexManager()).getFile());
    try {
      ...
      try {
        ...
        writers = Executors.newSingleThreadExecutor(new DaemonThreadFactory("test-writer-pool"));

        readers =
            Executors.newFixedThreadPool(
                nreaderThreads, new DaemonThreadFactory("test-reader-pool"));
        ...
        for (int rdrs = 0; rdrs < nreaders; rdrs++) {
          ...
          final int nreads =
              rdrs == 0 ? Integer.MAX_VALUE : 60; // reasonably long running hopefully

          lastReaderFuture =
              readers.submit(
                  new Reader(r, nreads, nwriters, sail, failex, commits, nreadersDone, subs));
        }
       ...
       * Note whether or not there are failures before we
        ...
        if (writers != null) writers.shutdownNow();
  ...
  private static class Writer implements Callable<Long> {
    ...
    public Long call() throws Exception {
      ...
      try {
        ...
        ok = true;
        commits.increment();
        if (log.isInfoEnabled()) log.info("Commit #" + commits);
        ...
        // && aborts.get() < maxAborts
          ...
          if (failex.compareAndSet(null /* expected */, ise /* newValue */)) {
        ...
        if (con != null) {
          if (!ok) {
            con.rollback();
            aborts.increment();
            log.error("Abort #" + aborts + " (with " + commits.get() + " commits)");
          }
          con.close();
        }
      }
  ...
  private static class Reader implements Callable<Long> {
    ...
    public Long call() throws Exception {
       ...
       * Note: This sleep makes it much easier to hit the bug
         ...
         */
        if (commits.get() < Math.max(nwriters, 5)) Thread.sleep(2000 /* millis */);
        ...
        if (InnerCause.isInnerCause(ise, InterruptedException.class)) {
          ...
          if (failex.compareAndSet(null /* expected */, ise /* newValue */)) {
            ...
            if (log.isInfoEnabled()) log.info("Other error: " + ise, ise);
          }
        }
      } finally {
        if (con != null) {
  ...
  //    void domultiple_csem_transaction_onethread(final int retention, final int nuris, final int
  // npreds) throws Exception {
  //
  ...
  static void setProperties(
      final Properties props, final int retention, final boolean isolatableIndices) {
    props.setProperty(
        EmbergraphSail.Options.ISOLATABLE_INDICES, Boolean.toString(isolatableIndices));
    ...
    if (!isolatableIndices) {
      ...
      props.setProperty(
          "org.embergraph.namespace.kb.spo." + name + ".org.embergraph.btree.BTree.className",
  ...
  static String getArg(final String[] args, final String arg, final String def) {
    for (int p = 0; p < args.length; p += 2) {
      if (arg.equals(args[p])) return args[p + 1];
    }
   ...
   *
   * <p>A property file is required. Note that if a file is specified then it will be re-opened and
   * not removed for each run as specified by nruns.
   *
   * <p>Optional arguments -nruns - number of runs through the test -nreaderthreads - reader threads
   * -nwriters - writer tasks -nreaders - reader tasks
   */
