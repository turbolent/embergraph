...
public class TestNanoSparqlClient<S extends IIndexManager> extends AbstractTestNanoSparqlClient<S> {
  ...
  public static Test suite() {
    ...
    return ProxySuiteHelper.suiteWhenStandalone(
        TestNanoSparqlClient.class, "test.*", TestMode.quads
        //                , TestMode.sids
        //                , TestMode.triples
        );
  }
  ...
  public void testServiceNodeBindings() throws Exception {
    ...
    try {
      String queryStr =
          "select * where {SERVICE <http://DBpedia.org/sparql> { <http://dbpedia.org/resource/Tonga_(Nyasa)_language> rdfs:label ?langLabel. }}";
      //                String queryStr = "SELECT * WHERE { BIND
      // (<http://dbpedia.org/resource/Tonga_(Nyasa)_language> AS ?ref) . SERVICE
      // <http://DBpedia.org/sparql> { ?ref rdfs:label ?langLabel. } }";
