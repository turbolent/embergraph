 ...
 * without. A namespace DOES NOT need to be configured for isolatable indices in order to create and
 * manipulate transactions, but it DOES need to be configured with isolatable indices in order for
 * you to WRITE on the namespace using a transaction.
 *
 * @param <S> FIXME *** Explicit tests of web.xml tx URL rewrites. Can we hit the transaction
 *     manager if all we have is the sparqlEndpointURL?
 *     <p>FIXME *** Can we run the same repository test suite that we use for embedded tests?
 *     (EmbergraphSparqlTest). This would mean having a test suite class so we can use it from
 *     another maven project.
 *     <p>FIXME *** org.embergraph.rdf.sail.webapp.TestSparqlUpdate runs against the
 *     org.embergraph.rdf.sail.webapp.client.RemoteRepository. It could run against the {@link
 *     EmbergraphSailRemoteRepository}.
 *     <p>FIXME *** Verify that we are running the full embedded repository test suite, including
 *     the tests for the extended transaction API.
 */
public class TestEmbergraphSailRemoteRepository<S extends IIndexManager>
    extends AbstractTestNanoSparqlClient<S> {

  public TestEmbergraphSailRemoteRepository() {}

  public TestEmbergraphSailRemoteRepository(final String name) {
  ...
  public static Test suite() {
    ...
    //      return ProxySuiteHelper.suiteWhenStandalone(TestEmbergraphSailRemoteRepository.class,
    //            "test.*", TestMode.quads
    ////            , TestMode.sids
    ////            , TestMode.triples
    //            );
    ...
    return ProxySuiteHelper.suiteWhenStandalone(
        TestEmbergraphSailRemoteRepository.ReadWriteTx.class,
        //            "test.*", TestMode.quads
   ...
   */
  protected EmbergraphSailRemoteRepositoryConnection cxn = null;
  ...
  public void setUp() throws Exception {
    ...
    repo = m_repo.getEmbergraphSailRemoteRepository();
  ...
  @Override
  protected void doInsertWithBodyTest(
      final String method, final int ntriples, /*final String servlet,*/ final RDFFormat format)
      throws Exception {
  ...
  @Override
  protected long doInsertByBody(
      final String method,
      /*final String servlet,*/ final RDFFormat rdfFormat,
      final Graph g,
  ...
  protected RepositoryResult<Statement> doGetWithAccessPath(
      final URI s, final URI p, final Value o, final URI... c) throws Exception {
  ...
  public void test_INSERT_triples_with_BODY_and_defaultContext() throws Exception {
    ...
    final String resource = packagePath + "insert_triples_with_defaultContext.ttl";

    final Graph g = loadGraphFromResource(resource);

    // Load the resource into the KB.
    doInsertByBody("POST", RDFFormat.TURTLE, g, new URIImpl("http://example.org"));
  ...
  public void test_INSERT_triples_with_URI_and_defaultContext() throws Exception {
    ...
    // Load the resource into the KB.
    {
      final File file = new File(packagePath + "insert_triples_with_defaultContext.ttl");
  ...
  public void test_INSERT_veryLargeLiteral() throws Exception {
    ...
    // Load the resource into the KB.
    assertEquals(1L, doInsertByBody("POST", RDFFormat.RDFXML, g, null /* defaultContext */));
  ...
  public void test_INSERT_LOAD_FROM_URI() throws Exception {
    ...
    {
      cxn.add(
          this.getClass().getClassLoader().getResource("org/embergraph/rdf/rio/small.rdf"),
          "",
          RDFFormat.RDFXML);
      // cxn.add(new URL("file:embergraph-rdf/src/test/org/embergraph/rdf/rio/small.rdf"), "",
      // RDFFormat.RDFXML);
  ...
  public static class ReadWriteTx<S extends IIndexManager>
      extends TestEmbergraphSailRemoteRepository<S> {
     ...
     * connection.
     *
     * <p>TODO Test where we abort the connection. Verify write set is discarded.
     *
    ...
    public void test_tx_begin_addStatement_commit()
      ...
      {
        final EmbergraphSailRemoteRepositoryConnection cxn2 = repo.getConnection();
        try {
          assertTrue(cxn2 != cxn);
          // cxn2.begin();
     ...
     * @throws UpdateExecutionException
     */
    public void test_tx_begin_UPDATE_commit()
        throws RepositoryException, MalformedQueryException, UpdateExecutionException {
      ...
      cxn.prepareUpdate(
              QueryLanguage.SPARQL, getNamespaceDeclarations() + "INSERT DATA { :a :b :c }")
          .execute();
