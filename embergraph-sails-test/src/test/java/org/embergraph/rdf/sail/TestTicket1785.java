 ...
 *
 * @version $Id$
 * @see https://jira.blazegraph.com/browse/BLZG-1785 Wrong result from FILTER expression with || and
 *     NOT IN
 */
public class TestTicket1785 extends QuadsTestCase {
  ...
  private void executeQuery(final EmbergraphSailRepository repo)
    ...
    try {
      ...
      try {
        conn.add(getClass().getResourceAsStream("TestTicket1785.n3"), "", RDFFormat.TURTLE);
        conn.commit();

        final String query =
            "PREFIX wd:  <http://my.test.namespace/A#> \r\n"
