 ...
 *     <p>FIXME *** Cover the quads mode APs.
 *     <p>TODO Should test GET as well as POST (this requires that we configured the client
 *     differently).
 */
public class Test_REST_HASSTMT<S extends IIndexManager> extends AbstractTestNanoSparqlClient<S> {
  ...
  public static Test suite() {
    ...
    return ProxySuiteHelper.suiteWhenStandalone(
        Test_REST_HASSTMT.class, "test.*", TestMode.quads
        //                , TestMode.sids
        //                , TestMode.triples
        );
  }
  ...
  public void test_HASSTMT_s() throws Exception {
    ...
    assertFalse(
        m_repo.hasStatement(
            new URIImpl("http://www.embergraph.org/Fred"), // s
            null, // p
            null, // o
            false // includeInferred
            ));
  ...
  public static class ReadWriteTx<S extends IIndexManager> extends Test_REST_HASSTMT<S> {
    ...
    public static Test suite() {
      ...
      return ProxySuiteHelper.suiteWhenStandalone(
          Test_REST_HASSTMT.ReadWriteTx.class, "test.*", TestMode.triples
          //                 , TestMode.sids
          //                 , TestMode.triples
          );
    }
  ...
  public static class TruthMaintenance<S extends IIndexManager> extends Test_REST_HASSTMT<S> {
     ...
     * will fail if you attempt to run with quads since inference is not supported for quads.
     */
    @Override
    public Properties getProperties() {

      final Properties p =
          TestMode.triplesPlusTruthMaintenance.getProperties(super.getProperties());
    ...
    public void test_HASSTMT_includeInferred() throws Exception {
      ...
      assertFalse(
          m_repo.hasStatement(
              OWL.EQUIVALENTCLASS, RDFS.SUBPROPERTYOF, RDFS.SUBCLASSOF, false // includeInferred
              ));
      ...
      assertTrue(
          m_repo.hasStatement(
              OWL.EQUIVALENTCLASS, RDFS.SUBPROPERTYOF, RDFS.SUBCLASSOF, true // includeInferred
              ));
      ...
      mutationCount += m_repo.remove(new RemoveOp(null /* s */, RDF.TYPE /* p */, null /* o */));
      mutationCount += m_repo.remove(new RemoveOp(null /* s */, RDFS.LABEL /* p */, null /* o */));
      mutationCount += m_repo.remove(new RemoveOp(null /* s */, FOAF.KNOWS /* p */, null /* o */));
