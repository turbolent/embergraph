...
public class Test_REST_Structure<S extends IIndexManager> extends AbstractTestNanoSparqlClient<S> {
  ...
  public static Test suite() {
    ...
    return ProxySuiteHelper.suiteWhenStandalone(
        Test_REST_Structure.class, "test.*", TestMode.quads
        //                , TestMode.sids
        //                , TestMode.triples
        );
  }
  ...
  public void testMultipleFixtures() throws Exception {
    ...
    try {
      ...
      final String altrootURL =
          new URL("http", hostAddr, altport, "" /* contextPath */).toExternalForm();

      final String altserviceURL =
          new URL("http", hostAddr, altport, EmbergraphStatics.getContextPath()).toExternalForm();
   ...
   * </pre>
   *
   * The response is should be <code>index.html</code> since we want the embergraph webapp to
   * respond for the top-level context.
   *
   * <p>Note: You must ensure that the client follows redirects using a standard policy. This is
   * necessary for tests of the webapp structure since the container may respond with a redirect
   * (302) to the location of the webapp when the client requests the root URL.
   */
   ...
   * </pre>
   *
   * The response is should be <code>index.html</code>, which is specified through the welcome files
   * list.
   */
   ...
   * @throws IOException
   */
  private static JettyResponseListener checkResponseCode(
      final String url, final JettyResponseListener response) throws IOException {
  ...
  private JettyResponseListener doConnect(final ConnectOptions opts) throws Exception {
    ...
    try {
      ...
      request = m_repo.getRemoteRepositoryManager().newRequest(urlString.toString(), opts.method);
