...
public class TestVirtualGraphs extends AbstractEmbergraphExprBuilderTestCase {
   ...
   * </pre>
   *
   * Note: This should be turned into exactly the same AST as the {@link
   * #test_from_and_from_named()} test. The virtual graph definition is expanded with the same
   * effect as the original FROM NAMED clauses.
   *
  ...
  public void test_virtualGraphs_01()
    ...
    final EmbergraphURI vg = valueFactory.createURI("http://example.org/vg");

    final EmbergraphURI context = valueFactory.createURI("http://www.embergraph.org/context");

    final EmbergraphURI uri1 = valueFactory.createURI("http://example.org/dft.ttl");

    final EmbergraphURI alice = valueFactory.createURI("http://example.org/alice");
     ...
     */
    {
      final EmbergraphValue[] values =
          new EmbergraphValue[] {virtualGraph, vg, context, uri1, alice, bob};
    ...
    // Insert the statements.
    {
      ...
      sb.add(valueFactory.createStatement(vg, virtualGraph, bob, context, StatementEnum.Explicit));
   ...
   * </pre>
   *
   * Note: This is similar to the above test, but verifies that the virtual graph declaration is
   * expanded into the default graph membership for the dataset. This test also verifies that the
   * FROM and FROM VIRTUAL GRAPH clauses may be combined freely.
   *
   * @throws ParseException
   * @throws TokenMgrError
   * @throws MalformedQueryException
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
...

      {
        ...
        final DatasetNode datasetNode =
            new DatasetNode(defaultGraphs, null /* namedGraphs */, false /* update */);
        expected.setDataset(datasetNode);
      }
