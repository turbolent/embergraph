 ...
 */
public class TestBootstrapEmbergraphSail extends TestCase2 {

  /** */
  public TestBootstrapEmbergraphSail() {}

  /** @param arg0 */
  public TestBootstrapEmbergraphSail(final String arg0) {
    super(arg0);
  }
  ...
  public void test_ctor_1() throws SailException, IOException {
    ...
    final File file = new File(EmbergraphSail.Options.DEFAULT_FILE);
    ...
    try {
      ...
      if (!file.exists()) fail("Expected file does not exist: " + file);
   ...
   */
  public void test_getConnectionAllowedExactlyOnce1_oneThread()
      throws SailException, InterruptedException, ExecutionException {
   ...
   * should obtain the connection.
   *
   * @throws SailException
   * @throws InterruptedException
   * @throws ExecutionException
   */
  public void test_getConnectionAllowedExactlyOnce1_twoThreads()
      throws SailException, InterruptedException, ExecutionException, TimeoutException {
   ...
   * @throws InterruptedException
   */
  public void test_getConnectionAllowedExactlyOnce2()
      throws SailException, InterruptedException, ExecutionException {
    ...
    try {
      ...
      final EmbergraphSail sail2 = new EmbergraphSail(sail.getNamespace(), sail.getIndexManager());
      sail2.initialize();
   ...
   * AbstractTripleStore} on the same {@link Journal}. However, scale-out should not enforce this
   * constraint since it is shard-wise ACID for unisolated operations.)
   *
   * @throws SailException
   * @throws InterruptedException
   */
  public void test_getConnectionAllowedExactlyOnce3()
      throws SailException, InterruptedException, ExecutionException {
    ...
    try {
      ...
      final EmbergraphSail sail2;
      {
        ...
        final AbstractTripleStore tripleStore =
            new LocalTripleStore(jnl, "foo", ITx.UNISOLATED, properties);

        // create that triple store.
        tripleStore.create();

        // wrap a 2nd sail around the 2nd tripleStore.
        sail2 = new EmbergraphSail(tripleStore);
        sail2.initialize();
      }
   ...
   * read-committed view.
   *
   * <p>TODO variant that writes, aborts the write, and verifies that the data was not made restart
   * safe.
   *
   * @throws SailException
   * @throws InterruptedException
  ...
  public void test_isolationOfUnisolatedConnection() throws SailException, InterruptedException {
    ...
    EmbergraphSailConnection readConn = null;
    ...
    try {
      ...
      // verify read back within the connection.
      {
        ...
        final CloseableIteration<? extends Statement, SailException> itr =
            conn.getStatements(s, p, o, false /* includeInferred */);

        try {

          while (itr.hasNext()) {

            EmbergraphStatement stmt = (EmbergraphStatement) itr.next();
      ...
      // verify NO read back in the read-committed view.
      {
        ...
        final CloseableIteration<? extends Statement, SailException> itr =
            readConn.getStatements(s, p, o, false /* includeInferred */);
