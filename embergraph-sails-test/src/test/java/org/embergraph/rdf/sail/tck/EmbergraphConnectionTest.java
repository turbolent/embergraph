...
public class EmbergraphConnectionTest extends RepositoryConnectionTest {
  ...
  //	}
  //
  private static final Logger log = Logger.getLogger(EmbergraphConnectionTest.class);

  public EmbergraphConnectionTest(String name) {
    super(name);
  }
  ...
  protected Properties getProperties() {
     ...
     */
    props.setProperty(EmbergraphSail.Options.BUFFER_MODE, BufferMode.DiskWORM.toString());
    ...
    */
    props.setProperty(EmbergraphSail.Options.STORE_BLANK_NODES, "true");
  ...
  public void tearDown() throws Exception {
    ...
    final IIndexManager backend =
        testRepository == null
            ? null
            : ((EmbergraphSailRepository) testRepository).getSail().getIndexManager();
      ...
      if (log.isInfoEnabled() && backend instanceof Journal)
        log.info(
            QueryEngineFactory.getInstance()
                .getExistingQueryController((Journal) backend)
                .getCounters());
      backend.destroy();
    }
  }
  ...
  //
  //		assertTrue(testCon.hasStatement(bob, name, nameBob, false));
  ////		assertTrue(testCon2.hasStatement(bob, name, nameBob, false)); // No. This is read-committed
  // semantics.
  //		assertFalse(testCon2.hasStatement(bob, name, nameBob, false)); // Yes. This is snapshot
  // isolation semantics.
  //
  //    }
  //
  ...
  ////		queryBuilder.append(" FROM {person} foaf:name {").append(<UNICODE VAR FROM
  // SUPER>.getLabel()).append("}");
  ////		queryBuilder.append(" USING NAMESPACE foaf = <" + FOAF_NS + ">");
  //		queryBuilder.append(" PREFIX foaf: <" + FOAF_NS + ">");
  //		queryBuilder.append(" SELECT ?person");
  //		queryBuilder.append(" where { ?person foaf:name \"").append(<UNICODE VAR FROM
  // SUPER>.getLabel()).append("\" . }");
  //
  //
  //
   ...
   * full GC pause that would suspend the JVM for longer than the query timeout. This would fail the
   * test. Second, the query engine code used to only check for a deadline when a query operator
   * would start or stop. This meant that a compute bound operator would not be interrupted if there
   * was no other concurrent operators for that query that were starting and stoping. This was fixed
   * in #722.
   *
  ...
  public void testOrderByQueriesAreInterruptable() throws Exception {
     ...
     */
    final Class cls1 = Class.forName("com.sun.management.GarbageCollectorMXBean");
    ...
    for (int i = 0; i < NTRIALS; i++) {
      ...
      if (log.isInfoEnabled()) log.info("RUN-TEST-PASS #" + i);
      ...
      if (log.isInfoEnabled()) log.info("Query evaluation has begin");
      ...
      try {
        ...
        if (log.isInfoEnabled()) log.info("Actual query duration: " + duration + "ms on pass#" + i);
        ...
        if (ok) {
          ...
          final List<GarbageCollectorMXBean> mbeans =
          ...
          for (GarbageCollectorMXBean m : mbeans) {
            ...
            if (cls1.isAssignableFrom(m.getClass())) {
              ...
              if (lastDuration >= MAX_QUERY_TIME) {
                log.warn(
                    "Large GC pause caused artifical liveness problem: duration="
                        + duration
                        + "ms");
     ...
     */
    final String msg = "NTRIALS=" + NTRIALS + ", nok=" + nok + ", ngcfail=" + ngcfail;
