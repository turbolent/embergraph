...
public class TestNanoSparqlServer extends TestCase2 {
   ...
   * ConfigParams#NAMESPACE} and {@link ConfigParams#CREATE} properties. If those overrides are not
   * applied then the default KB will be created and this test will fail. If the test fails, the
   * place to look is {@link NanoSparqlServer} where it is overriding the init parameters for the
   * {@link WebAppContext}.
   *
  ...
  public void test_start_stop() {
    ...
    final AbstractTripleStore tripleStore =
        (AbstractTripleStore) m_indexManager.getResourceLocator().locate(namespace, ITx.UNISOLATED);

    assertNotNull("Explicitly create KB not found: namespace=" + namespace, tripleStore);

    final AbstractTripleStore tripleStore2 =
        (AbstractTripleStore)
            m_indexManager
                .getResourceLocator()
                .locate(EmbergraphSail.Options.DEFAULT_NAMESPACE, ITx.UNISOLATED);
