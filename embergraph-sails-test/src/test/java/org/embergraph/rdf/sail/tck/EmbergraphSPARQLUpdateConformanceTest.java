...
public class EmbergraphSPARQLUpdateConformanceTest extends SPARQLUpdateConformanceTest {
   ...
   *     testURI.
   */
  static EmbergraphSPARQLUpdateConformanceTest getSingleTest(
      final TestSuite suite, final String testURI) throws RuntimeException {
    ...
    while (e1.hasMoreElements()) {
      ...
      if (aTest instanceof TestSuite) {
        final EmbergraphSPARQLUpdateConformanceTest test =
            getSingleTest((TestSuite) aTest, testURI);
        if (test != null) return test;
      }
      if (aTest instanceof EmbergraphSPARQLUpdateConformanceTest) {
        final EmbergraphSPARQLUpdateConformanceTest test =
            (EmbergraphSPARQLUpdateConformanceTest) aTest;
  ...
  static TestSuite filterOutTests(final TestSuite suite1, final Collection<String> testURIs) {
    ...
    while (e.hasMoreElements()) {
      ...
      } else if (aTest instanceof EmbergraphSPARQLUpdateConformanceTest) {
        final EmbergraphSPARQLUpdateConformanceTest test =
            (EmbergraphSPARQLUpdateConformanceTest) aTest;
  ...
  public EmbergraphSPARQLUpdateConformanceTest(
      String testURI,
      String name,
      String requestFile,
      URI defaultGraphURI,
  ...
  public static Test suite() throws Exception {
    ...
    if (!testURIs.isEmpty()) {
      ...
      for (String s : testURIs) {
        final EmbergraphSPARQLUpdateConformanceTest test = getSingleTest(suite1, s);
  ...
  protected EmbergraphSailRepository newRepository() throws RepositoryException {
    ...
    //            return new ContextAwareRepository(new EmbergraphSailRepository(sail));
    ...
    try {
      ...
      for (String ng : inputNamedGraphs.keySet()) {
        ...
        conn.add(
            graphURL,
            null,
            RDFFormat.forFileName(graphURL.toString()),
  ...
  protected void runTest() throws Exception {
    EmbergraphSailRepositoryConnection con =
        (EmbergraphSailRepositoryConnection) dataRep.getConnection();
    EmbergraphSailRepositoryConnection erCon =
        (EmbergraphSailRepositoryConnection) expectedResultRepo.getConnection();
    try {
  ...
  protected void compareGraphs(
      Iterable<? extends Statement> actual,
      Iterable<? extends Statement> expected,
    ...
    if (!ModelUtil.equals(expected, actual)) {
      ...
      final EmbergraphSailUpdate query =
