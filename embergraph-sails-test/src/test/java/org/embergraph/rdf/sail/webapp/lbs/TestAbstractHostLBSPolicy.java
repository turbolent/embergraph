 ...
 *     <p>TODO Test load balancing for no hosts, one host (with 1 service, 2 services, 3 services)
 *     and 3 hosts (each with one service). Also test cases where some hosts have no active
 *     services, e.g., 3 hosts but only one is active or 3 hosts but only 2 are active.
 */
public class TestAbstractHostLBSPolicy extends TestCase2 {
  ...
  public void test_lbs_host_policy() {
    ...
    final HostScore[] hostScores = new HostScore[] {hostScore1};

    {
      final HostScore actualHost = AbstractHostLBSPolicy.getHost(.0, hostScores);
    ...
    {
      final HostScore actualHost = AbstractHostLBSPolicy.getHost(.1, hostScores);
    ...
    {
      final HostScore actualHost = AbstractHostLBSPolicy.getHost(.5, hostScores);
    ...
    {
      final HostScore actualHost = AbstractHostLBSPolicy.getHost(.9, hostScores);
    ...
    {
      {
        ...
        final ServiceScore[] serviceScores = new ServiceScore[] {serviceA};

        final ServiceScore actualService =
            AbstractHostLBSPolicy.getService(rand, hostScore1, serviceScores);
...

      {
        ...
        final ServiceScore[] serviceScores = new ServiceScore[] {serviceA, serviceB, serviceC};
  ...
  public void test_HA3() {
    ...
    //        assertEquals(1d, hostScore0.getScore() + hostScore1.getScore() +
    // hostScore2.getScore());
    //
    ...
    //        final UUID C = UUID.randomUUID();
    //
    //        final ServiceScore serviceA = new ServiceScore(A, hostScore0.getHostname(),
    // toRequestURI(hostScore0.getHostname()));
    //        final ServiceScore serviceB = new ServiceScore(B, hostScore1.getHostname(),
    // toRequestURI(hostScore1.getHostname()));
    //        final ServiceScore serviceC = new ServiceScore(C, hostScore2.getHostname(),
    // toRequestURI(hostScore2.getHostname()));
    //
    //        /*
    //         * Now check the method to identify a specific joined service known to
    //         * be running on that host.
