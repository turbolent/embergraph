...
public class TestQuadsAPI extends QuadsTestCase {
  ...
  public void test_2graphs() throws Exception {
    ...
    try {
      ...
      /**/
      cxn.add(a, new URIImpl(FOAF + "name"), new LiteralImpl("Alice"), graphA);
      cxn.add(a, new URIImpl(FOAF + "mbox"), new URIImpl("mailto:alice@work.example"), graphA);
      /*
       * Graph B.
       */
      cxn.add(b, new URIImpl(FOAF + "name"), new LiteralImpl("Bob"), graphB);
      cxn.add(b, new URIImpl(FOAF + "mbox"), new URIImpl("mailto:bob@work.example"), graphB);
      /**/
      ...
      assertSameIterationAnyOrder(new Resource[] {graphA, graphB}, cxn.getContextIDs());
   ...
   *
   * <p>mooseroy: If i remember correctly, early on there was a problem when you had something like
   * graph ?s{?s ?p ?o} where it wasn't ensuring the ?s in the context and subject position where
   * equal.
   */
