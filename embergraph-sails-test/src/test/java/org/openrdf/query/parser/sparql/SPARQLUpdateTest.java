...
public abstract class SPARQLUpdateTest extends TestCase {
   ...
   * prepared SPARQL UPDATE. However, embergraph does a commit() as the last action for the SPARQL
   * UPDATE.
   *
   * <p>(2) It relies on different transaction semantics. The snapshot isolation semantics of
   * embergraph read-only connections mean that con2 will never see the mutation from con.
   *
   * <p>Since we can not "fix" (2) (it is not an error - we have better transaction semantics), I am
   * going to comment out this test.
   *
   * <p>It is an open question whether we want to fix (1). I prefer our interpretation that SPARQL
   * UPDATE execute() implies a commit().
   *
   * <p>Bryan 11/11/2014
   */
  //	@Test
  ...
  public void testInsertWhereGraph() throws Exception {
    ...
    assertTrue(
        message, con.hasStatement(alice, RDFS.LABEL, f.createLiteral("Alice"), true, graph2));
    assertFalse(
        message, con.hasStatement(alice, RDFS.LABEL, f.createLiteral("Alice"), true, graph1));
  }
  ...
  public void testInsertData2() throws Exception {
    ...
    assertFalse(
        con.hasStatement(
            book1, DC.TITLE, f.createLiteral("the number four", XMLSchema.INTEGER), true));
  ...
  public void testInsertDataBlankNode() throws Exception {
    ...
    RepositoryResult<Statement> titleStatements =
        con.getStatements(null, DC.TITLE, f.createLiteral("book 1"), true);
    assertNotNull(titleStatements);

    RepositoryResult<Statement> creatorStatements =
        con.getStatements(null, DC.CREATOR, f.createLiteral("Ringo"), true);
  ...
  public void testDeleteDataMultiplePatterns() throws Exception {
    ...
    assertFalse(msg, con.hasStatement(alice, FOAF.KNOWS, bob, true));
    assertFalse(
        msg, con.hasStatement(alice, FOAF.MBOX, f.createLiteral("alice@example.org"), true));
  }
