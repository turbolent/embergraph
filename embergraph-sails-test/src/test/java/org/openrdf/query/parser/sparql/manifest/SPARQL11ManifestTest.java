...
public class SPARQL11ManifestTest {
   ...
   * @param officialWorkingGroupTests indicates whether to use the official W3C working group tests,
   *     or Sesame's own set of tests.
   * @param approvedTestsOnly if <code>true</code>, use working group-approved tests only. Has no
   *     influence when officialWorkingGroup tests is set to <code>false</code>.
   * @param useRemoteTests if set to <code>true</code>, use manifests and tests located at <code>
   *     http://www.w3.org/2009/sparql/docs/tests/data-sparql11/</code> , instead of local copies.
   * @param excludedSubdirs an (optionally empty) list of subdirectories to exclude from testing. If
   *     specified, test cases in one of the supplied subdirs will not be executed. If left empty,
  ...
  public static TestSuite suite(
    ...
    TestSuite suite =
        new TestSuite(factory.getClass().getName()) {
    ...
    TupleQueryResult manifestResults =
        con.prepareTupleQuery(QueryLanguage.SPARQL, query, manifestFile).evaluate();
  ...
  public static TestSuite suite(
    ...
    while (manifestResults.hasNext()) {
      ...
      if (includeSubManifest(subManifestFile, excludedSubdirs)) {
        suite.addTest(
            SPARQLUpdateConformanceTest.suite(subManifestFile, factory, approvedTestsOnly));
      }
    }
  ...
  private static String getManifestFile(boolean officialWorkingGroupTests, boolean useRemote) {
    ...
    if (useRemote) {
      ...
      if (officialWorkingGroupTests) {
        ...
        url =
            SPARQL11ManifestTest.class.getResource("/testcases-sparql-1.1/manifest-evaluation.ttl");
      }
   ...
   * @return <code>false</code> if the supplied list of excluded subdirs is not empty and contains a
   *     match for the supplied sub-manifest, <code>true</code> otherwise.
   */
