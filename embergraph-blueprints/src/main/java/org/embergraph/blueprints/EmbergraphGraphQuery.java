 ...
 */
public class EmbergraphGraphQuery implements GraphQuery {

  protected static final transient Logger log = Logger.getLogger(EmbergraphGraphQuery.class);
  ...
  /** The graph. */
  private final EmbergraphGraph graph;
  ...
  public EmbergraphGraphQuery(final EmbergraphGraph graph) {
    this.graph = graph;
  ...
  public GraphQuery has(final String key, final Predicate predicate, final Object value) {
    ...
    criteria.add(new Has(key, value, EmbergraphPredicate.toEmbergraphPredicate(predicate)));
    return this;
  }
  ...
  @Override
  @Deprecated
  public <T extends Comparable<T>> GraphQuery has(
   ...
   */
  @Override
  public <T extends Comparable<?>> GraphQuery interval(
      final String key, final T startValue, final T endValue) {
  ...
  protected String toQueryStr(final URI type) {
    ...
    for (Has has : criteria) {
      ...
      if (has instanceof HasNot) {
        ...
        if (has.val != null && (has.pred == null || has.pred == EmbergraphPredicate.EQ)) {
  ...
  private String toFilterStr(final EmbergraphPredicate pred, final String var, final Object val) {

    final BlueprintsValueFactory factory = graph.factory;

    final StringBuilder sb = new StringBuilder();

    if (pred == EmbergraphPredicate.EQ) {

      throw new IllegalArgumentException();

    } else if (pred == EmbergraphPredicate.GT
        || pred == EmbergraphPredicate.GTE
        || pred == EmbergraphPredicate.LT
        || pred == EmbergraphPredicate.LTE
        || pred == EmbergraphPredicate.NE) {
    ...
    } else if (pred == EmbergraphPredicate.IN || pred == EmbergraphPredicate.NIN) {

      sb.append("    filter(");

      if (pred == EmbergraphPredicate.NIN) {
        sb.append("!(");
      }
  ...
  private class Has {
    ...
    private EmbergraphPredicate pred;
    ...
    public Has(final String key, final Object val, final EmbergraphPredicate pred) {

      if (pred == EmbergraphPredicate.IN || pred == EmbergraphPredicate.NIN) {
