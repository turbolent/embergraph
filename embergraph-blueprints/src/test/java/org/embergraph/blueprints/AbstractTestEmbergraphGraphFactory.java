 ...
 * @author beebs
 */
public abstract class AbstractTestEmbergraphGraphFactory extends TestCase {

  private static final transient Logger log =
      Logger.getLogger(AbstractTestEmbergraphGraphFactory.class);
  ...
  protected final String testJnl = "testJournal-" + System.currentTimeMillis() + ".jnl";

  protected final String testData = "graph-example-1.xml";

  public AbstractTestEmbergraphGraphFactory() {
    super();
  }

  public AbstractTestEmbergraphGraphFactory(String name) {
    super(name);
  }
   ...
   * @return
   */
  protected abstract EmbergraphGraph loadGraph(String file) throws Exception;

  protected void loadTestGraph(EmbergraphGraph graph, String resource) throws IOException {

    GraphMLReader.inputGraph(graph, this.getClass().getClassLoader().getResourceAsStream(resource));
  }
  ...
  protected int vertexEdgeCount(EmbergraphGraph graph) {
    int v_e_cnt = 0;
    ...
    for (@SuppressWarnings("unused") Edge e : graph.getEdges()) {
  ...
  protected boolean evaluatePreconditions(String file) {
    ...
    if (f.exists()) {
      throw new RuntimeException("Running unit test with existing file." + file);
    } else {
      retVal = true;
    }
  ...
  public void testGraphFactoryCreateNew() throws Exception {
...

    {
      ...
      EmbergraphGraph graph = getNewGraph(testJnl);
   ...
   * @throws Exception
   */
  protected void openAndCheckGraphCount(String file, int loadCount) throws Exception {

    EmbergraphGraph graph = loadGraph(file);
