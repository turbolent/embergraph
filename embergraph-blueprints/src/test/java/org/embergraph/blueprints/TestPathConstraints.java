...
public class TestPathConstraints extends TestCase {
  ...
  private static final transient Logger log = Logger.getLogger(TestPathConstraints.class);
  ...
  public void testPathLength() throws Exception {
    ...
    final EmbergraphGraphEmbedded graph =
        (EmbergraphGraphEmbedded)
            EmbergraphGraphFactory.create(SimpleBlueprintsValueFactory.INSTANCE, props);
      ...
      { // join ordering
        ...
        try {
          final EmbergraphSailTupleQuery query =
              (EmbergraphSailTupleQuery) cxn.prepareQuery(QueryLanguage.SPARQL, queryStr);
          final QueryRoot optimized = query.optimize();
          if (log.isDebugEnabled()) {
            log.debug("optimized:\n" + optimized);
          }
  ...
  private void showOptimizedAST(final EmbergraphGraph graph, final String queryStr)
      throws Exception {
