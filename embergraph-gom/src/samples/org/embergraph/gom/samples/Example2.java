 ...
 *
 * <p>A real application could use the skin pattern to provide application specific interfaces that
 * simplify access to the vertex and link attributes of interest.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 */
public class Example2 implements Callable<Void> {

  private static final transient Logger log = Logger.getLogger(Example2.class);
   ...
   * @return The string value -or- <code>null</code> if the property is not bound to a {@link
   *     Literal}.
   */
  private static String getStr(final IGPO gpo, final URI property) {
   ...
   * @param connectTo The link property.
   * @param friendOfAFriend The link target.
   * @param weightProperty The link weight property.
   * @return The link weight -or- <code>null</code> if the link either does not exist or does not
   *     have the link weight property.
   */
  private static Integer getLinkWeight(
      final IGPO gpo, final URI connectTo, final IGPO friendOfAFriend, final URI weightProperty) {
  ...
  public Void call() throws Exception {
     ...
     */
    final EmbergraphURI connectTo = om.getValueFactory().createURI("http://example.org/connectTo");

    /*
     * URI used for the connection count (link weight).
     */
    final EmbergraphURI weightProperty =
        om.getValueFactory().createURI("http://example.org/connectionCount");

    final ICloseableIterator<BindingSet> itr =
        om.evaluate(
    ...
    try {
       ...
       * Bad FOAF data. The target should be a Resource, not a
        ...
        final Literal connectionCount = (Literal) bset.getValue("connectionCount");
    ...
    for (IGPO gpo : roots) {
      ...
      if (!friends.isEmpty()) {
        ...
        for (IGPO friendOfAFriend : friends) {
          ...
          final Integer connectionCount =
              getLinkWeight(gpo, connectTo, friendOfAFriend, weightProperty);

          System.out.println(
              "   friendOfAFriend: "
                  + friendOfAFriend.getId()
                  + " (name="
