...
public class CircularCharArrayBufferTest extends TestCase {
  ...
  private static void copyInto(CircularFifoBuffer cfb, char[] c, int offset, int length) {
    ...
    for (int i = 0; i < howMany; i++) c[offset + i] = ((Character) it.next()).charValue();
  }
  ...
  public void testAdd() {
    for (int size : sizes) {
      ...
      for (int j = 0; j < times; j++) {
        ...
        System.arraycopy(
            RandomStringUtils.randomAlphanumeric(c.length).toCharArray(), 0, c, 0, c.length);
        for (int i = offset; i < offset + len; i++) cfb.add(new Character(c[i]));
