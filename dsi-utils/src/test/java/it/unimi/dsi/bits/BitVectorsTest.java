...
public class BitVectorsTest extends TestCase {
  ...
  public void testReadWriteFast() throws IOException {
    ...
    for (BitVector bv : a) {
      ...
      dos.close();
      assertEquals(
          bv, BitVectors.readFast(new DataInputStream(new FastByteArrayInputStream(fbaos.array))));
      fbaos.reset();
    }
    ...
    for (BitVector bv : a) {
      ...
      assertEquals(
          bv,
          BitVectors.readFast(
              new DataInputStream(new FastByteArrayInputStream(fbaos.array)), labv));
      fbaos.reset();
    }
  }
