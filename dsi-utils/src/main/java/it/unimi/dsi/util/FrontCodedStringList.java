 ...
 * just as a handy way to manage a large amount of strings). It implements an immutable {@link
 * it.unimi.dsi.fastutil.objects.ObjectList} that returns the <var>i</var>-th string (as a {@link
 * MutableString}) when the {@link #get(int)} method is called with argument <var>i</var>. The
 * returned mutable string may be freely modified.
 *
 * <p>As a commodity, this class provides a main method that reads from standard input a sequence of
 * newline-separated words, and writes a corresponding serialized front-coded string list.
 *
 * <H2>Implementation Details</H2>
 *
 * <p>To store the list of strings, we use either a UTF-8 coded {@link ByteArrayFrontCodedList}, or
 * a {@link CharArrayFrontCodedList}, depending on the value of the {@link
 * #FrontCodedStringList(Iterator,int,boolean) utf8} parameter at creation time. In the first case,
 * if the strings are ASCII-oriented the resulting array will be much smaller, but access times will
 * increase manifold, as each string must be UTF-8 encoded before being returned.
 */
public class FrontCodedStringList extends AbstractObjectList<MutableString>
   ...
   */
  public FrontCodedStringList(
      final Iterator<? extends CharSequence> words, final int ratio, final boolean utf8) {

    this.utf8 = utf8;
    if (utf8) {
      byteFrontCodedList =
          new ByteArrayFrontCodedList(
   ...
   */
  public FrontCodedStringList(
      final Collection<? extends CharSequence> c, final int ratio, final boolean utf8) {
  ...
  public ObjectListIterator<MutableString> listIterator(final int k) {
    return new AbstractObjectListIterator<MutableString>() {
      ObjectListIterator<?> i =
          utf8 ? byteFrontCodedList.listIterator(k) : charFrontCodedList.listIterator(k);
      ...
      public MutableString previous() {
        return MutableString.wrap(
            utf8 ? byte2Char((byte[]) i.next(), null) : (char[]) i.previous());
      }
  ...
  public int size() {
    return utf8 ? byteFrontCodedList.size() : charFrontCodedList.size();
  }
  ...
  public static void main(final String[] arg)
    ...
    final SimpleJSAP jsap =
        new SimpleJSAP(
            FrontCodedStringList.class.getName(),
            "Builds a front-coded string list reading from standard input a newline-separated ordered list of terms.",
            new Parameter[] {
              new FlaggedOption(
                  "bufferSize",
    ...
    final FrontCodedStringList frontCodedStringList =
        new FrontCodedStringList(
            new LineIterator(
                new FastBufferedReader(
                    new InputStreamReader(
                        zipped ? new GZIPInputStream(System.in) : System.in, encoding),
                    bufferSize),
                pl),
            ratio,
            utf8);
