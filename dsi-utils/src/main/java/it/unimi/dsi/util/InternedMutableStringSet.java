...
public class InternedMutableStringSet extends ObjectOpenHashSet<MutableString> {
   ...
   * java.util.Collection#add(Object)}, but this method will return a mutable string equal to <code>
   * s</code> currently in this set. The string will <em>never</em> be <code>s</code>, as in the
   * case <code>s</code> is not in this set a {@linkplain MutableString#compact() compact copy} of
   * <code>s</code> will be stored instead.
   *
   * <p>The purpose of this method is similar to that of {@link String#intern()}, but obviously here
   * the user has much greater control.
   *
  ...
  private int findInsertionPoint(MutableString k) {
    int pos =
        ((k) == null
                ? 0x87fcd5c
                : it.unimi.dsi.fastutil.HashCommon.murmurHash3((k).hashCode() ^ mask))
            & mask;
