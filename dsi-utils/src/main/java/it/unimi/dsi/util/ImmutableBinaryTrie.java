 ...
 * the list. The words are then organised in a binary trie with path compression.
 *
 * <p>Once the trie has been built, it is possible to ask whether a word <var>w</var> is {@linkplain
 * #get(BooleanIterator) contained in the trie} (getting back its position in the list), the
 * {@linkplain #getInterval(BooleanIterator) interval given by the words extending <var>w</var>} and
 * the {@linkplain #getApproximatedInterval(BooleanIterator) approximated interval defined by
 * <var>w</var>}.
 *
...
public class ImmutableBinaryTrie<T> extends AbstractObject2LongFunction<T> implements Serializable {
   ...
   * @param transformationStrategy a transformation strategy that must turn <code>elements</code>
   *     into a list of distinct, lexicographically increasing (in iteration order) binary words.
   */
  public ImmutableBinaryTrie(
    ...
    if (iterator.hasNext()) {
      final LongArrayBitVector prev =
          LongArrayBitVector.copy(transformationStrategy.toBitVector(iterator.next()));
      words.add(prev.copy());
      BitVector curr;
   ...
   * @return a trie containing the suffixes of words in <code>words</code> starting at <code>pos
   *     </code>.
   */
  protected Node buildTrie(final ObjectList<LongArrayBitVector> elements, final int pos) {
    ...
    if (elements.size() == 1)
      return new Node(
          pos < prefix ? LongArrayBitVector.copy(first.subVector(pos, prefix)) : null, size++);
    ...
    if (prefix == first.size()) {
      ...
      n =
          new Node(
              prefix > pos ? LongArrayBitVector.copy(first.subVector(pos, prefix)) : null, size++);
      ...
      n =
          new Node(
              prefix > pos
                  ? LongArrayBitVector.copy(first.subVector(pos, prefix))
                  : null); // There's some common prefix
    ...
    while (n != null) {
      ...
      if (path != null) {
        ...
        for (i = 0; i < minLength; i++)
          if (word.getBoolean(pos + i) != QuickBitVector.get(path, i)) break;
   ...
   * @return the index of the specified word, or -1 if the word returned by the iterator is not this
   *     trie.
   * @see #getLong(Object)
   */
  public int get(final BooleanIterator iterator) {
    ...
    while (n != null) {
      ...
      if (pathLength != 0) {
        int i;
        path = n.path;
        for (i = 0; i < pathLength && iterator.hasNext(); i++)
          if (iterator.nextBoolean() != QuickBitVector.get(path, i)) break;
        // Incompatible with current path.
   ...
   *
   * @param word a word.
   * @return an interval given by the smallest and the largest word in the trie that start with
   *     <code>word</code> (thus, the {@linkplain Intervals#EMPTY_INTERVAL empty inteval} if no such
   *     words exist).
   ...
   * @return an interval given by the smallest and the largest word in the trie that start with the
   *     word returned by <code>iterator</code> (thus, the {@linkplain Intervals#EMPTY_INTERVAL
   *     empty inteval} if no such words exist).
  ...
  public Interval getInterval(final BooleanIterator iterator) {
    ...
    while (n != null) {
      ...
      if (pathLength != 0) {
        ...
        path = n.path;
        for (i = 0; i < pathLength && iterator.hasNext(); i++)
          if ((mismatch = (iterator.nextBoolean() != QuickBitVector.get(path, i)))) break;
        // Incompatible with current path--we return the empty interval.
   ...
   * the words in the approximator are thought of as left interval extremes in a larger
   * lexicographically ordered set of words, and we number these word intervals using the indices of
   * their left extremes, then the first word extending <var>w</var> would be in the word interval
   * given by the left extreme of the interval returned by this method, whereas the last word
   * extending <var>w</var> would be in the word interval given by the right extreme of the interval
   * returned by this method. If no word in the larger set could possibly extend <var>w</var>
   * (because <var>w</var> is smaller than the lexicographically smallest word in the approximator)
  ...
  public Interval getApproximatedInterval(final T element) {
    ...
    while (n != null) {
      ...
      if (path != null) {
        ...
        int i;
        for (i = 0; i < maxLength; i++)
          if (mismatch = (word.getBoolean(pos + i) != QuickBitVector.get(path, i))) break;
    ...
    if (pos == length && !exactMatch) {
      if (l.word == 0)
        return mismatch ? Intervals.EMPTY_INTERVAL : Interval.valueOf(l.word, n.word);
   ...
   *
   * @param iterator an iterator.
   * @return an approximated interval around the specified word: if the words in this trie are
   *     thought of as left interval extremes in a larger lexicographically ordered set of words,
   *     and we number these word intervals using the indices of their left extremes, then the first
   *     word extending <code>word</code> would be in the word interval given by the left extreme of
   *     the {@link Interval} returned by this method, whereas the last word extending <code>word
   *     </code> would be in the word interval given by the right extreme of the {@link Interval}
  ...
  public Interval getApproximatedInterval(final BooleanIterator iterator) {
    ...
    for (; ; ) {
      ...
      if (path != null) {
        ...
        for (i = 0; i < pathSize && iterator.hasNext(); i++)
          if ((mismatch = (iterator.nextBoolean() != QuickBitVector.get(path, i)))) break;
  ...
  private void recToString(
      final Node n,
      final MutableString printPrefix,
    ...
    recToString(
        n.right,
        printPrefix.replace(printPrefix.length() - 5, printPrefix.length(), "1 => "),
        result,
        path,
        level + 1);
    ...
    // System.err.println( "Path now: " + path + " Going to delete from " + ( path.length() -
    // n.pathLength));
