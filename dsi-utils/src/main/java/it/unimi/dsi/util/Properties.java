 ...
 *
 * <p>All accessors defined in {@link org.apache.commons.configuration.PropertiesConfiguration} have
 * a polymorphic counterpart taking an {@link java.lang.Enum} instead of a string: {@link
 * java.lang.Enum#name()} <strong>and {@link java.lang.String#toLowerCase()}</strong> are applied
 * before delegating to the corresponding string-based method. (This apparently wierd choice is due
 * to the need to accommodate the upper-case standard for {@link java.lang.Enum} elements and the
 * lower-case standard for property keys.)
 *
...
public class Properties extends PropertiesConfiguration implements Serializable {
   ...
   * strings associated to each key is the same. Note that the order in which different keys appear
   * in a property file is irrelevant, but <em>the order between properties with the same key is
   * significant</em>.
   *
   * <p>Due to the strictness of the check (e.g., no number conversion is performed) this method is
   * mainly useful when writing tests.
   *
