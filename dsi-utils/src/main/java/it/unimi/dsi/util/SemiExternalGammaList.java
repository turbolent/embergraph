 ...
 * access files containing frequencies.
 *
 * <p>Instead, this class accesses frequencies in their compressed forms, and provides entry points
 * for random access to each long. At construction time, entry points are computed with a certain
 * <em>step</em>, which is the number of longs accessible from each entry point, or, equivalently,
 * the maximum number of longs that will be necessary to read to access a given long.
 *
 * <p><strong>Warning:</strong> This class is not thread safe, and needs to be synchronised to be
...
public class SemiExternalGammaList extends AbstractLongList {
   ...
   * @param step the step used to build random-access entry points, or -1 to get {@link
   *     #DEFAULT_STEP}.
   * @param numLongs the overall number of offsets (i.e., the number of terms).
   */
  public SemiExternalGammaList(final InputBitStream longs, final int step, final int numLongs)
      throws IOException {
