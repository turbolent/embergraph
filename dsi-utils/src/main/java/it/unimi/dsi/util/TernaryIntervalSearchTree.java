 ...
 *
 * <p>Ternary <em>interval</em> search trees have the additional properties of being able to locate
 * quickly intervals of words extending a given prefix (where &ldquo;quickly&rdquo; means that no
 * more successful character comparisons than the prefix length are performed). They do so by
 * storing at each node the number of words covered by that node.
 *
 * <p>This implementation exposes a number of interfaces: in particular, the set of words is seen as
 * a lexicographically ordered {@link it.unimi.dsi.fastutil.objects.ObjectList}.
 *
 * <p>This class is mutable, but for the time it implements only {@link #add(CharSequence)}. Words
 * cannot be removed.
 */
public class TernaryIntervalSearchTree extends AbstractPrefixMap implements Serializable {
      ...
      if (c < path[i]) e = e.left;
        ...
        if (offset == l)
          return Interval.valueOf(
              wordsAtLeft,
              wordsAtLeft + (e.isWord ? 1 : 0) + (e.middle == null ? 0 : e.middle.numNodes) - 1);
        if (e.isWord) wordsAtLeft++;
        e = e.middle;
      }
    }
  ...
  public Interval getApproximatedInterval(final CharSequence s) {
    ...
    while (e != null) {
      ...
      if (offset + i == l) {
        ...
        return wordsAtLeft > 0
            ? Interval.valueOf(wordsAtLeft - 1, wordsAtLeft + e.numNodes - 1)
            : Interval.valueOf(wordsAtLeft, wordsAtLeft + e.numNodes - 1);
      }
      if (i < path.length - 1) {
   ...
   * @param e the subtree in which the characters should be inserted, or <code>null</code> if a new
  ...
  public static void main(final String[] arg)
    ...
    final SimpleJSAP jsap =
        new SimpleJSAP(
            TernaryIntervalSearchTree.class.getName(),
            "Builds a ternary interval search tree reading from standard input a newline-separated list of terms.",
            new Parameter[] {
              new FlaggedOption(
                  "bufferSize",
                  JSAP.INTSIZE_PARSER,
              ...
              new UnflaggedOption(
                  ...
                  JSAP.REQUIRED,
                  JSAP.NOT_GREEDY,
                  "The filename for the serialised tree.")
            });
    ...
    final FastBufferedReader terms =
        new FastBufferedReader(
            new InputStreamReader(System.in, (Charset) jsapResult.getObject("encoding")),
