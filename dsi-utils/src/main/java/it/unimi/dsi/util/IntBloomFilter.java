 ...
 * Bloom filter for <var>n</var> integers with <var>d</var> hash functions will use ln 2
 * <var>d</var><var>n</var> &#8776; 1.44 <var>d</var><var>n</var> bits; false positives will happen
 * with probability 2<sup>-<var>d</var></sup>.
 *
 * <p>Hash functions are generated at creation time using universal hashing. Each hash function uses
 * two integers <var>A</var> and <var>B</var>, and the integer <var>x</var> is mapped to
 * (<var>Ax</var>)&#8853;<var>B</var> before taking the remainder modulo the number of bits in the
 * filter.
 *
 * <p>This class exports access methods that are very similar to those of {@link java.util.Set}, but
 * it does not implement that interface, as too many non-optional methods would be unimplementable
 * (e.g., iterators).
 *
...
public class IntBloomFilter implements Serializable {
   ...
   * @return the position in the filter corresponding to <code>x</code> for the hash function <code>
   *     k</code>.
   */
   ...
   * This will happen with probability 2<sup>-<var>d</var></sup>, where <var>d</var> is the number
   * of hash functions specified at creation time, if the number of the elements in the filter is
   * less than <var>n</var>, the number of expected elements specified at creation time.
   *
   * @param x an integer.
   * @return true if the integer is in the filter (or if an integer with the same hash sequence is
   *     in the filter).
   */
