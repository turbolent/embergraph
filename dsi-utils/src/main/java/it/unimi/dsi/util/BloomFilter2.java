 ...
 *
 * <ul>
 *   <li>Removed use of the MersenneTwister random number generator to remove a dependency on the
 *       Colt project.
 *   <li>Removed the main() routine, including all of the various dependencies that it was dragging
 *       in.
 *   <li>Removed the use of LongArrays from the <code>fastutil</code> project. The code uses {@link
 *       Arrays#fill(long[], long)} instead to clear the state on reset.
 *   <li>Custom serialization (not compatible, new serialization UUID).
 *   <li>Made various public fields private since they can not be final with an {@link
 *       Externalizable} implementation and added access methods for their data.
 *   <li>Changed the class name to avoid possible class path confusions.
 * </ul>
 *
 ...
 * elements.
 *
 * <p>The intended usage is that character sequences and arrays should <em>not</em> be mixed (albeit
 * in principle it could work). A Bloom filter is rather agnostic with respect to the data it
 * contains&mdash;all it needs is a sequence of hash functions that can be applied to the data.
 *
 * <p>Bloom filters have an expected error rate, depending on the number of hash functions used, on
 * the filter size and on the number of elements in the filter. This implementation uses a variable
 * optimal number of hash functions, depending on the expected number of elements. More precisely, a
 * Bloom filter for <var>n</var> character sequences with <var>d</var> hash functions will use ln 2
 * <var>d</var><var>n</var> &#8776; 1.44 <var>d</var><var>n</var> bits; false positives will happen
 * with probability 2<sup>-<var>d</var></sup>. The maximum number of bits supported is {@link
 * #MAX_BITS}.
 *
 * <p>Hash functions are generated at creation time using a mix of universal hashing and
 * shift-add-xor hashing (for the latter, see <i>Performance in practice of string hashing
 * functions</i>, by M.V. Ramakrishna and Justin Zobel, <i>Proc. of the Fifth International
 * Conference on Database Systems for Advanced Applications</i>, 1997, pages 215&minus;223).
 *
 * <p>Each hash function uses {@link #NUMBER_OF_WEIGHTS} random integers, which are cyclically
 ...
 * (e.g., iterators).
 *
 * <p>A main method makes it easy to create serialised Bloom filters starting from a list of terms.
 *
 * @author Sebastiano Vigna, who wrote the original implementation.
 * @author thompsonbry@users.sourceforge.net, who hacked the implementation down to have fewer
 *     dependencies.
 */
public class BloomFilter2 implements Externalizable {
  ...
  public static final transient long MAX_BITS = (long) Long.SIZE * Integer.MAX_VALUE;

  private static final transient long LOG2_LONG_SIZE = 6;
  private static final transient long BIT_INDEX_MASK = (1 << LOG2_LONG_SIZE) - 1;
  ...
  private static final transient double NATURAL_LOG_OF_2 = Math.log(2);
  ...
  public BloomFilter2(final int n, final int d) {
    ...
    if (wantedNumberOfBits > MAX_BITS)
      throw new IllegalArgumentException(
          "The wanted number of bits (" + wantedNumberOfBits + ") is larger than " + MAX_BITS);
    ...
    for (int i = 0; i < d; i++) {
      weight[i] = new int[NUMBER_OF_WEIGHTS];
      init[i] = rand.nextInt();
      for (int j = 0; j < NUMBER_OF_WEIGHTS; j++) weight[i][j] = rand.nextInt();
    }
  }
   ...
   * @return the position in the filter corresponding to <code>s</code> for the hash function <code>
   *     k</code>.
   */
   ...
   * @param l the length of <code>s</code>.
   * @param k a hash function index (smaller than {@link #d}).
   * @return the position in the filter corresponding to <code>a</code> for the hash function <code>
   *     k</code>.
   */
   ...
   * @param l the length of <code>s</code>.
   * @param k a hash function index (smaller than {@link #d}).
   * @return the position in the filter corresponding to the slice of <code>a</code> for the hash
   *     function <code>k</code>.
   */
  ...
  private long hash(final float[] a, final int l, final int k) {
    ...
    int i = l;
    while (i-- != 0)
      h ^= (h << 5) + Float.floatToRawIntBits(a[i]) * w[i % NUMBER_OF_WEIGHTS] + (h >>> 2);
    return (h & 0x7FFFFFFFFFFFFFFFL) % m;
  }
  ...
  private long hash(final double[] a, final int l, final int k) {
    ...
    int i = l;
    while (i-- != 0)
      h ^= (h << 5) + Double.doubleToRawLongBits(a[i]) * w[i % NUMBER_OF_WEIGHTS] + (h >>> 2);
    return (h & 0x7FFFFFFFFFFFFFFFL) % m;
  }
   ...
   * filter. This will happen with probability 2<sup>-<var>d</var></sup>, where <var>d</var> is the
   * number of hash functions specified at creation time, if the number of the elements in the
   * filter is less than <var>n</var>, the number of expected elements specified at creation time.
   *
   * @param s a character sequence.
   * @return true if <code>s</code> (or some element with the same hash sequence as <code>s</code>)
   *     is in the filter.
   */
   ...
   *
   * @param a a byte array.
   * @return true if <code>a</code> (or some element with the same hash sequence as <code>a</code>)
   *     is in the filter.
   ...
   * distinct elements that have been added to the filter. False positives might make the number
   * returned by this method smaller than it should be.
   *
  ...
  //
  //        final SimpleJSAP jsap = new SimpleJSAP( BloomFilter.class.getName(), "Creates a Bloom
  // filter reading from standard input a newline-separated list of terms.",
  //                new Parameter[] {
  //                    new FlaggedOption( "bufferSize", IntSizeStringParser.getParser(), "64Ki",
  // JSAP.NOT_REQUIRED, 'b',  "buffer-size", "The size of the I/O buffer used to read terms." ),
  //                    new FlaggedOption( "encoding", ForNameStringParser.getParser( Charset.class
  // ), "UTF-8", JSAP.NOT_REQUIRED, 'e', "encoding", "The term file encoding." ),
  //                    new UnflaggedOption( "bloomFilter", JSAP.STRING_PARSER, JSAP.NO_DEFAULT,
  // JSAP.REQUIRED, JSAP.NOT_GREEDY, "The filename for the serialised front-coded list." ),
  //                    new UnflaggedOption( "size", JSAP.INTSIZE_PARSER, JSAP.NO_DEFAULT,
  // JSAP.REQUIRED, JSAP.NOT_GREEDY, "The size of the filter (i.e., the expected number of elements
  // in the filter; usually, the number of terms)." ),
  //                    new UnflaggedOption( "precision", JSAP.INTEGER_PARSER, JSAP.NO_DEFAULT,
  // JSAP.REQUIRED, JSAP.NOT_GREEDY, "The precision of the filter." )
  //        });
  //
  ...
  //
  //        BloomFilter filter = new BloomFilter( jsapResult.getInt( "size" ), jsapResult.getInt(
  // "precision" ) );
  //        final ProgressLogger pl = new ProgressLogger();
  //        pl.itemsName = "terms";
  //        pl.start( "Reading terms..." );
  //        MutableString s = new MutableString();
  //        FastBufferedReader reader = new FastBufferedReader( new InputStreamReader( System.in,
  // encoding ), bufferSize );
