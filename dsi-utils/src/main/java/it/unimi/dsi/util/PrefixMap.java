 ...
 * {@linkplain #rangeMap() function from string prefixes to intervals} and a {@linkplain
 * #prefixMap() function from intervals to string prefixes}.
 *
 * <p>In the first case, given a prefix, we can ask for the range of strings starting with that
 * prefix, expressed as an {@link Interval}. This information is very useful to satisfy prefix
 * queries (e.g., <samp>monitor*</samp>) with a brute-force approach.
 *
 * <p>Optionally, a prefix map may provide the opposite service: given an interval of terms, it may
 * provide the maximum common prefix. This feature can be checked for by calling {@link
 * #prefixMap()}.
 *
