 ...
 * for instances. This class provides a method {@link #fromSpec(String, Class, String[], String[])}
 * that will generate object instances starting from a specification of the form
 *
 * <pre style="text-align: center; padding: .5em">
 * <var>class</var>(<var>arg</var>,&hellip;)
 * </pre>
 *
 * <p>The format of the specification is rather loose, to ease use on the command line: each
 * argument may or may not be quote-delimited, with the the proviso that inside quotes you have the
 * usual escape rules, whereas without quotes the end of the parameter is marked by the next comma
 * or closed parenthesis, and surrounding space is trimmed. For empty constructors, parentheses can
 * be omitted. Valid examples are, for instance,
 *
 * <pre style="text-align: center; padding: .5em">
 * java.lang.Object
 * java.lang.Object()
 ...
 * </pre>
 *
 * <p>After parsing, we search for a constructor accepting as many strings as specified arguments,
 * or possibly a string varargs constructor. The second optional argument will be used to check that
 * the generated object is of the correct type, and the last argument is a list of packages that
 * will be prepended in turn to the specified class name. Finally, the last argument is an optional
 * list of static factory method names that will be tried before resorting to constructors. Several
 * polymorphic versions make it possible to specify just a subset of the arguments.
 *
 * <p>Additionally, it is possible to specify a {@linkplain #fromSpec(Object, String, Class,
 * String[], String[]) <em>context object</em>} that will be passed to the construction or factory
 * method used to generate the new instance. The context is class dependent, and must be correctly
 * understood by the target class. In this case, the resolution process described above proceed
 * similarly, but the signatures searched for contain an additional {@link Object} argument before
 * the string arguments.
 *
 * <p>Note that this arrangement requires some collaboration from the specified class, which must
 * provide string-based constructors. If additionally you plan on saving parseable representations
 * which require more than just the class name, you are invited to follow the {@link
 * #toSpec(Object)} conventions.
 *
 * <p>This class is a <a href="http://www.martiansoftware.com/jsap/"><acronym title="Java-based
 * Simple Argument Parser">JSAP</acronym></a> {@link StringParser}, and can be used in a JSAP
 * parameter specifications to build easily objects on the command line. Several constructors make
 * it possible to generate parsers that will check for type compliance, and possibly attempt to
 * prepend package names.
 */
public class ObjectParser extends StringParser {
   ...
   * @param packages a list of package names that will be prepended to the specification, or <code>
   *     null</code>.
   * @param factoryMethod a list of factory methods that will be used before trying constructors, or
   *     <code>null</code>.
   */
   ...
   *     objects)&mdash;possibly <code>null</code>.
   * @param type a type that will be used to check instantiated objects.
   * @param packages a list of package names that will be prepended to the specification, or <code>
   *     null</code>.
   * @param factoryMethod a list of factory methods that will be used before trying constructors, or
   ...
   */
  public static Object fromSpec(String spec)
   ...
   * @param type a type that will be used to check instantiated objects.
   */
  public static <S> S fromSpec(String spec, final Class<S> type)
      throws IllegalArgumentException, ClassNotFoundException, IllegalAccessException,
          InvocationTargetException, InstantiationException, NoSuchMethodException {
   ...
   *     null</code>.
   */
  public static <S> S fromSpec(String spec, final Class<S> type, final String[] packages)
      throws IllegalArgumentException, ClassNotFoundException, IllegalAccessException,
          InvocationTargetException, InstantiationException, NoSuchMethodException {
   ...
   *     <code>null</code>.
   */
  public static <S> S fromSpec(
      String spec, final Class<S> type, final String[] packages, final String[] factoryMethod)
      throws IllegalArgumentException, ClassNotFoundException, IllegalAccessException,
          InvocationTargetException, InstantiationException, NoSuchMethodException {
   ...
   * @param spec the object specification (see the {@linkplain ObjectParser class documentation}).
   */
  public static Object fromSpec(Object context, String spec)
      throws IllegalArgumentException, ClassNotFoundException, IllegalAccessException,
          InvocationTargetException, InstantiationException, NoSuchMethodException {
   ...
   * @param type a type that will be used to check instantiated objects.
   */
  public static <S> S fromSpec(Object context, String spec, final Class<S> type)
      throws IllegalArgumentException, ClassNotFoundException, IllegalAccessException,
          InvocationTargetException, InstantiationException, NoSuchMethodException {
   ...
   */
  public static <S> S fromSpec(
      Object context, String spec, final Class<S> type, final String[] packages)
      throws IllegalArgumentException, ClassNotFoundException, IllegalAccessException,
          InvocationTargetException, InstantiationException, NoSuchMethodException {
  ...
  public static <S> S fromSpec(
    ...
    if (endOfName < length) {
      ...
      while (pos < length) {
        ...
        if (pos == length || args.size() == 0 && pos == length - 1 && spec.charAt(pos) == ')')
          break;
        ...
        for (; ; ) {
          ...
          if (!inQuotes) {
            ...
            if (c == '"' && !escaped) {
              ...
              if (pos == length || (spec.charAt(pos) != ')' && spec.charAt(pos) != ','))
      ...
      for (String f : factoryMethod) {
        ...
        try {
          ...
          if (Modifier.isStatic(method.getModifiers()))
            instance = (S) method.invoke(null, argArray);
        ...
        try {
          if (contextualised) {
            ...
            if (Modifier.isStatic(method.getModifiers()))
              instance = (S) method.invoke(null, context, stringArgArray);
          } else {
            method = klass.getMethod(f, String[].class);
            if (Modifier.isStatic(method.getModifiers()))
              instance = (S) method.invoke(null, (Object) stringArgArray);
          }
        } catch (NoSuchMethodException niceTry) {
        }
    ...
    try {
      ...
      throw new NoSuchMethodException(
          contextualised
              ? "No contextual constructor with "
                  + stringArgArray.length
                  + " strings as argument for class "
                  + klass.getName()
              : "No constructor with "
                  + stringArgArray.length
                  + " strings as argument for class "
   ...
   *
   * <p>The standard approach to generate a parseable representation would be to have some interface
   * specifying a no-arg <code>toSpec()</code> method returning a {@link String}. Since most of the
   * typically parsed objects are singletons, and often one does not need to save a parseable
   * representation, we rather fetch such a method if available, but we will otherwise return just
   * the class name.
   *
   * @param o an object.
