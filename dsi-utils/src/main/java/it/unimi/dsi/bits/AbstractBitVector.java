 ...
 *
 * <p>Note that the integer-set view provided by {@link #asLongSet()} is not cached: if you want to
 * cache the result of the first call, you must do your own caching.
 *
 * <p><strong>Warning</strong>: this class has several optimised methods that assume that {@link
 * #getLong(long, long)} is implemented efficiently when its arguments are multiples of {@link
 * Long#SIZE} (see, e.g., the implementation of {@link #compareTo(BitVector)} and {@link
 * #longestCommonPrefixLength(BitVector)}). If you want speed up the processing of your own {@link
 * BitVector} implementations, just implement {@link #getLong(long, long)} so that it is fast under
...
public abstract class AbstractBitVector extends AbstractBooleanList implements BitVector {
  ...
  protected void ensureRestrictedIndex(final long index) {
    ...
    if (index >= length())
      throw new IndexOutOfBoundsException(
          "Index (" + index + ") is greater than or equal to length (" + (length()) + ")");
  }
  ...
  public void fill(final boolean value) {
    for (long i = length(); i-- != 0; ) set(i, value);
  }
  ...
  public boolean removeBoolean(final int index) {
    return removeBoolean((long) index);
  }
  ...
  public int size() {
    ...
    if (length > Integer.MAX_VALUE)
      throw new IllegalStateException(
          "The number of bits of this bit vector (" + length + ") exceeds Integer.MAX_INT");
    return (int) length;
  }
  ...
  public BitVector replace(final BitVector bv) {
    ...
    if (bv.length() % Long.SIZE != 0)
      append(bv.getLong(fullBits, bv.length()), (int) (bv.length() - fullBits));
    return this;
  }
  ...
  public int hashCode() {
    ...
    for (long i = 0; i < fullLength; i += Long.SIZE)
      h ^= (h << 5) + getLong(i, i + Long.SIZE) + (h >>> 2);
  ...
  public long[] bits() {
    final long[] bits =
        new long
            [(int)
                ((length() + LongArrayBitVector.BITS_PER_WORD - 1)
                    >> LongArrayBitVector.LOG2_BITS_PER_WORD)];
    final long length = length();
    for (int i = 0; i < length; i++)
      if (getBoolean(i))
        bits[i >> LongArrayBitVector.LOG2_BITS_PER_WORD] |=
            1L << (i & LongArrayBitVector.WORD_MASK);
    return bits;
  }
   ...
   * enlarged as needed (i.e., when a one beyond the current size is set), but it is never shrunk.
   */
   ...
   * shrunk.
   */
  public static class LongBigListView extends AbstractLongList
    ...
    public int size() {
      ...
      if (length > Integer.MAX_VALUE)
        throw new IllegalStateException(
            "The number of elements of this bit list (" + length + ") exceeds Integer.MAX_INT");
      return (int) length;
    }
    ...
    private final class LongBigListIterator extends AbstractLongListIterator {
      ...
      public int nextIndex() {
        if (pos >= Integer.MAX_VALUE)
          throw new IllegalStateException(
              "The current list position is larger than Integer.MAX_VALUE");
        return (int) pos;
      }

      public int previousIndex() {
        if (pos > Integer.MAX_VALUE + 1L)
          throw new IllegalStateException(
              "The current list position is larger than Integer.MAX_VALUE");
        return (int) (pos - 1);
      }
  ...
  public static class SubBitVector extends AbstractBitVector implements BitVector {
    ...
    public void add(final long index, final boolean value) {
      bitVector.add(from + index, value);
      to++;
    }
