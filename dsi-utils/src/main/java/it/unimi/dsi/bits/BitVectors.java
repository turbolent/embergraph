...
public class BitVectors {
  ...
  public static void ensureFromTo(final long bitVectorLength, final long from, final long to) {
    ...
    if (to > bitVectorLength)
      throw new ArrayIndexOutOfBoundsException(
          "End index (" + to + ") is greater than bit vector length (" + bitVectorLength + ")");
  }

  /** An immutable, singleton empty bit vector. */
  public static final BitVector EMPTY_VECTOR =
      new AbstractBitVector() {
   ...
   * Then, as many longs as necessary to write the bits in the bit vectors (i.e., {@link
   * LongArrayBitVector#numWords(long)} of the bit vector length), obtained via {@link
   * BitVector#getLong(long, long)}.
   *
   * <p>The main purpose of this function is to support {@link OfflineIterable} (see {@link
   * #OFFLINE_SERIALIZER}).
   *
   ...
   * avoid creating a bit vector at each call, please have a look at {@link
   * #readFast(DataInputStream, LongArrayBitVector)}.
   *
   * @param dis a data input stream.
   * @return the next bit vector in the stream, as saved by {@link #writeFast(BitVector,
   *     DataOutputStream)}.
   ...
   * @return <code>bv</code>, filled with the next bit vector in the stream, as saved by {@link
   *     #writeFast(BitVector, DataOutputStream)}.
   * @see #writeFast(BitVector, DataOutputStream)
   * @see #readFast(DataInputStream)
   */
  public static LongArrayBitVector readFast(DataInputStream dis, final LongArrayBitVector bv)
      throws IOException {
    final long length = dis.readLong();
