 ...
 * bit in the vector, but this is not enforced.
 *
 * <p>Operation of a bit vector are partially of boolean nature (e.g., logical operations between
 ...
 * of view, this interface extends {@link it.unimi.dsi.fastutil.booleans.BooleanList}, but also
 ...
 * width.
 *
 * <p>Most, if not all, classical operations on bit vectors can be seen as standard operations on
 * these two views: for instance, the number of bits set to one is just the number of elements of
 * the set returned by {@link #asLongSet()} (albeit a direct {@link #count()} method is provided,
 * too). The standard {@link java.util.Collection#addAll(java.util.Collection)} method can be used
 * to concatenate bit vectors, and {@linkplain java.util.List#subList(int, int) sublist views} make
 * it easy performing any kind of logical operation on subvectors.
 *
 * <p>The only <i>caveat</i> is that sometimes the standard interface naming clashes slightly with
 * standard usage: for instance, {@link #clear(long)} will <em>not</em> set to zero all bits (use
 ...
 *
 * <p>The {@link AbstractBitVector} class provides a fairly complete abstract implementation that
 * provides all methods except for the most basic operations. Of course, the methods of {@link
 * AbstractBitVector} are very inefficient, but implementations such as {@link LongArrayBitVector}
...
public interface BitVector extends RandomAccess, BooleanList {
   ...
   * beyond {@link #length(long)} are considered zeroes). The resulting infinite string is
   * interpreted as the characteristic function of a set of integers.
   *
   ...
   *
   * <p>Implementations are invited to provide high-speed implementations for the case in which
   ...
   * this method is semantically equivalent to {@link List#size()}.
   *
   ...
   *
   * <p>If the number of bits in this vector is smaller than or equal to {@link Integer#MAX_VALUE},
   * this method is semantically equivalent to {@link BooleanList#size(int)}.
   *
   * @return this bit vector.
   */
   ...
   * </pre>
   *
   * <p>The last value is the hash code of the bit vector. This hashing is based on shift-add-xor
   * hashing (M.V. Ramakrishna and Justin Zobel, &ldquo;Performance in practice of string hashing
   * functions&rdquo;, <i>Proc. of the Fifth International Conference on Database Systems for
   * Advanced Applications</i>, 1997, pages 215&minus;223).
   *
   * <p>The returned value is not a high-quality hash such as <a
   * href="http://sux4j.dsi.unimi.it/docs/it/unimi/dsi/sux4j/mph/Hashes.html#jenkins(it.unimi.dsi.bits.BitVector)">Jenkins's</a>,
   * but it can be computed very quickly; in any case, 32 bits are too few for a high-quality hash
   * to be used in large-scale applications.
   *
   * <p><strong>Important</strong>: all bit vector implementations are required to return the value
   ...
   * instance, <em>views</em> on other data structures (e.g., strings) might implement {@link
   * #getLong(long, long)} efficiently on multiples of {@link Long#SIZE}, but might fail to provide
   * a generic, truly efficient random access.
   *
   * <p>This method returns a (possibly immutable) bit vector with the same content as that of this
   * bit vector. However, the returned bit vector is guaranteed to provide fast random access.
   *
