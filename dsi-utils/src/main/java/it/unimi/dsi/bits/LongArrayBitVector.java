 ...
 * vector an array of longs that is enlarged as needed when new entries are created (by dividing the
 * current length by the golden ratio), but is <em>never</em> made smaller (even on a {@link
 * #clear()}). Use {@link #trim()} for that purpose.
 *
 * <p>Besides usual methods for setting and getting bits, this class provides <em>views</em> that
 * make it possible to access comfortably the bit vector in different ways: for instance, {@link
 * #asLongBigList(int)} provide access as a list of longs, whereas {@link #asLongSet()} provides
 * access in setwise form.
 *
 * <p>When enlarging the underlying array (e.g., for {@link #append(long, int)} operations or add
 * operations on the {@linkplain #asLongBigList(int) big list view}), or when invoking {@link
 * #ensureCapacity(long)}, this class calls {@link LongArrays#grow(long[], int, int)}, which could
 * enlarge the array more than expected. On the contrary, {@link #length(long)} (and the
 * corresponding method in the {@linkplain #asLongBigList(int) big list view}) size the underlying
 * array in an exact manner.
 *
 * <p>Bit numbering follows the right-to-left convention: bit <var>k</var> (counted from the right)
 * of word <var>w</var> is bit 64<var>w</var> + <var>k</var> of the overall bit vector.
 *
 * <p>If {@link #CHECKS} is true at compile time, boundary checks for all bit operations will be
 ...
 *
 * <p><strong>Warning</strong>: A few optional methods have still to be implemented (e.g., adding an
 * element at an arbitrary position using the list view).
 *
 * <p><strong>Warning</strong>: In some cases, you might want to cache locally the result of {@link
 * #bits()} to speed up computations on immutable bit vectors (this is what happens, for instance,
 * in <a href="http://sux.dsi.unimi.it/docs/it/unimi/dsi/sux4j/bits/Rank.html">static ranking
 * structures</a>). This class, however, does its own serialisation of the bit vector: as a result,
 * all cached references to the result of {@link #bits()} must be marked as transient and rebuilt at
 * deserialisation time, or you will end up saving the bits twice.
 */
...
public class LongArrayBitVector extends AbstractBitVector implements Cloneable, Serializable {
   ...
   */
  protected static int numWords(final long size) {
   ...
   * @return the index of the word that holds the bit of given index, or -1 if <code>index</code> is
   *     -1.
   */
  protected static int word(final long index) {
   ...
   * {@link #BITS_PER_WORD} in word 0, index 0, bit {@link #BITS_PER_WORD} + 1 in word 1, index 1,
   * and so on.
   *
   * @param index the index of a bit.
   * @return the inside-word index of the bit that would hold the bit of specified index.
   */
  protected static int bit(final long index) {
   ...
   */
  protected static long mask(final long index) {
   ...
   *
   * <p>Note that this constructor creates an <em>empty</em> bit vector. If you want a cleared bit
   * vector of a specified size, please use the {@link #ofLength(long)} factory method.
   *
  ...
  public void fill(final long from, final long to, final boolean value) {
    ...
    LongArrays.fill(
        bits, (int) ((from + Long.SIZE - 1) / Long.SIZE), (int) (to / Long.SIZE), value ? -1L : 0L);
    if (from % Long.SIZE != 0) {
   ...
   * that behaviour, call {@link #trim()} afterwards.
   */
  public void clear() {
  ...
  public LongArrayBitVector copy(final long from, final long to) {
    ...
    if (startBit == 0) {
      ...
      if (endBit == 0)
        copyBits[numWords - 1] |= bits[numWords + startWord] << bitsPerWordMinusStartBit;
      else {
  ...
  public static LongArrayBitVector copy(final BitVector bv) {
    ...
    for (long i = 0; i < fullBits; i += Long.SIZE)
      copy.bits[(int) (i / Long.SIZE)] = bv.getLong(i, i + Long.SIZE);
    if (length % Long.SIZE != 0)
      copy.bits[(int) (fullBits / Long.SIZE)] = bv.getLong(fullBits, length);
    copy.length = length;
    return copy;
  }
  ...
  public void add(final long index, final boolean value) {
    ...
    if (length == bits.length << LOG2_BITS_PER_WORD)
      bits = LongArrays.grow(bits, numWords(length + 1));
  ...
  public LongArrayBitVector append(long value, int width) {
    ...
    if (CHECKS && width < Long.SIZE && (value & -1L << width) != 0)
      throw new IllegalArgumentException(
          "The specified value ("
              + value
              + ") is larger than the maximum value for the given width ("
              + width
              + ")");
  ...
  public long nextOne(final long index) {
    ...
    if (maskedFirstWord != 0)
      return from * BITS_PER_WORD + Fast.leastSignificantBit(maskedFirstWord);
  ...
  public long longestCommonPrefixLength(final LongArrayBitVector v) {
    ...
    for (int i = 0; i < words; i++)
      if (bits[i] != vBits[i])
        return Math.min(
            minLength, i * BITS_PER_WORD + Fast.leastSignificantBit(bits[i] ^ vBits[i]));
    return minLength;
  }
   ...
   * unset, or an exception will be thrown.
   *
  ...
  public static LongArrayBitVector wrap(final long[] array, final long size) {
    if (size > array.length << LOG2_BITS_PER_WORD)
      throw new IllegalArgumentException(
          "The provided array is too short ("
              + array.length
              + " elements) for the given size ("
              + size
              + ")");
    ...
    if (lastWord < arrayLength && (array[lastWord] & ~((1L << size % Long.SIZE) - 1)) != 0)
      throw new IllegalArgumentException("Garbage beyond size in bit array");
    for (int i = lastWord + 1; i < arrayLength; i++)
      if (array[i] != 0) throw new IllegalArgumentException("Garbage beyond size in bit array");
    return result;
  }
   ...
   *
   * @param array an array of longs.
   * @return a bit vector of size <code>array.length * Long.SIZE</code> using <code>array</code> as
   *     backing array.
   */
  ...
  public LongArrayBitVector replace(final LongArrayBitVector bv) {
    ...
    if (bvFirstFreeWord < thisFirstFreeWord)
      LongArrays.fill(this.bits, bvFirstFreeWord, thisFirstFreeWord, 0);
  ...
  public LongArrayBitVector replace(final BitVector bv) {
    ...
    for (long i = 0; i < fullBits; i += Long.SIZE)
      bits[(int) (i / Long.SIZE)] = bv.getLong(i, i + Long.SIZE);
    final int bvFirstFreeWord = word(bvLength - 1) + 1;
    final int thisFirstFreeWord = word(length - 1) + 1;
    if (bvLength % Long.SIZE != 0)
      bits[(int) (fullBits / Long.SIZE)] = bv.getLong(fullBits, bvLength);
    if (bvFirstFreeWord < thisFirstFreeWord)
      LongArrays.fill(this.bits, bvFirstFreeWord, thisFirstFreeWord, 0);
   ...
   * given width. The vector is enlarged as needed (i.e., when adding new elements), but it is never
   * shrunk.
   */
  protected static class LongBigListView extends AbstractBitVector.LongBigListView {
    ...
    public long set(long index, long value) {
      ...
      if (startWord == endWord) {
        ...
        // Here startBit > 0.
        oldValue =
            bits[startWord] >>> startBit | bits[endWord] << (BITS_PER_WORD - startBit) & fullMask;
