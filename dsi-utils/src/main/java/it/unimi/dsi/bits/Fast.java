 ...
 *
 * <ul>
 *   <li>methods to map bijectively {@linkplain #int2nat(int) integer to natural numbers} and
 *       {@linkplain #nat2int(int) back};
 *   <li>an implementation of Gerth Brodal's fast algorithm for the {@linkplain
 *       #mostSignificantBit(long) most significant bit}, and derived methods such as {@link
 *       #log2(double)}, {@link #ceilLog2(int)}, etc.;
 *   <li>an implementation of the classical <em>{@linkplain #count(long) sideways addition}</em>
 *       algorithm (a case of <em>broadword programming</em>), which can be used to do
 *       <em>ranking</em>, and of a new, dual algorithm for <em>{@linkplain #select(long, int)
 *       selection}</em>; the latter has been presented by Sebastiano Vigna in &ldquo;<a
 *       href="http://vigna.dsi.unimi.it/papers.php#VigBIRSQ">Broadword Implementation of
 *       Rank/Select Queries</a>&rdquo;, <i>Proc. of the 7th International Workshop on Experimental
 *       Algorithms, WEA 2008</i>, number 5038 in Lecture Notes in Computer Science, pages
 *       154&minus;168. Springer&ndash;Verlag, 2008.
 * </ul>
 *
...
public final class Fast {
  ...
  private static final long INCR_STEP_8 =
      0x80L << 56
          | 0x40L << 48
          | 0x20L << 40
          | 0x10L << 32
   ...
   * integer <var>x</var> to 2<var>x</var>. It can be used to save integers in the range [{@link
   * Integer#MIN_VALUE}/2..{@link Integer#MAX_VALUE}/2] (i.e., [-2<sup>30</sup>..2<sup>30</sup>-1])
   * using the standard coding methods (which all work on natural numbers). Note that no range
   ...
   * <var>x</var> to 2<var>x</var>. It can be used to save longs in the range [{@link
   * Long#MIN_VALUE}/2..{@link Long#MAX_VALUE}/2] (i.e., [-2<sup>62</sup>..2<sup>62</sup>-1]) using
   * the standard coding methods (which all work on natural numbers). Note that no range checks are
   * performed.
   *
   ...
   * @return the position in <code>x</code> of the bit of rank <code>k</code> ones; if no such bit
   *     exists, returns 72.
   */
  public static int select(final long x, final int rank) {
    ...
    final long byteOffset =
        (((((rankStep8 | MSBS_STEP_8) - byteSums) & MSBS_STEP_8) >>> 7) * ONES_STEP_8 >>> 53)
            & ~0x7;
    ...
    final long bitSums =
        (((spreadBits | ((spreadBits | MSBS_STEP_8) - ONES_STEP_8)) & MSBS_STEP_8) >>> 7)
            * ONES_STEP_8;
    ...
    return (int)
        (byteOffset
            + (((((byteRankStep8 | MSBS_STEP_8) - bitSums) & MSBS_STEP_8) >>> 7) * ONES_STEP_8
                >>> 56));
  }
   ...
   * order of magnitude faster than standard bit-fiddling techniques.
   *
   * @param x a long.
   * @return the most significant bit of <code>x</code>, of <code>x</code> is nonzero; &minus;1,
   *     otherwise.
   */
  ...
  private static final byte[] LSB_TABLE = {
   ...
   *
   * @param x a long.
   * @return the least significant bit of <code>x</code>, of <code>x</code> is nonzero; &minus;1,
   *     otherwise.
   */
