 ...
 *       parse the attributes of a <samp>P</samp> element while searching for links;
 *   <li>it should parse HTML as a <em>regular language</em>, and leave context-free properties
 *       (e.g., stack maintenance and repair) to suitably designed callbacks.
 * </ul>
 *
 * <p>Thus, in fact the bullet parser is not a parser. It is a bunch of spaghetti code that analyses
 * a stream of characters pretending that it is an (X)HTML document. It has a very defensive
 * attitude against the stream character it is parsing, but at the same time it is forgiving with
 * all typical (X)HTML mistakes.
 *
 * <p>The bullet parser is officially StringFree&trade;. <a
 * href="http://dsiutils.dsi.unimi.it/docs/it/unimi/dsi/lang/MutableString.html"><code>MutableString
 * </code>s</a> are used for internal processing, and Java strings are used only to return attribute
 * values. All internal maps are {@linkplain it.unimi.dsi.fastutil.objects.Reference2ObjectMap
 * reference-based maps} from <a href="http://fastutil.dsi.unimi.it/"><samp>fastutil</samp></a>,
 * which helps to accelerate further the parsing process.
 *
 * <h2>HTML data</h2>
 *
 * <p>The bullet parser uses attributes and methods of {@link it.unimi.dsi.parser.HTMLFactory},
 * {@link it.unimi.dsi.parser.Element}, {@link it.unimi.dsi.parser.Attribute} and {@link
 * it.unimi.dsi.parser.Entity}. Thus, for instance, whenever an element is to be passed around it is
 * one of the shared objects contained in {@link it.unimi.dsi.parser.Element} (e.g., {@link
 * it.unimi.dsi.parser.Element#BODY}).
 *
 * <h2>Callbacks</h2>
 *
 * <p>The result of the parsing process is the invocation of a callback. The {@linkplain
 * it.unimi.dsi.parser.callback.Callback callback interface} of the bullet parser remembers closely
 * SAX2, but it has some additional methods targeted at (X)HTML, such as {@link
 * it.unimi.dsi.parser.callback.Callback#cdata(it.unimi.dsi.parser.Element,char[],int,int)}, which
 * returns characters found in a CDATA section (e.g., a stylesheet).
 *
 * <p>Each callback must configure the parser, by requesting to perform the analysis and the
 * callbacks it requires. A callback that wants to extract and tokenise text, for instance, will
 * certainly require {@link #parseText(boolean) parseText(true)}, but not {@link #parseTags(boolean)
 ...
 *
 * <p>The first important issue is what has to be required to the parser. A newly created parser
 * does not invoke any callback. It is up to every callback to add features so that it can do its
 * job. Remember that since many callbacks can be {@linkplain
 * it.unimi.dsi.parser.callback.ComposedCallbackBuilder composed}, you must always <em>add</em>
 * features, never <em>remove</em> them, and moreover your callbacks must be ready to be invoked
 * with features they did not request (e.g., attribute types added by another callback).
 *
 * <p>The following parse features may be configured; most of them are just boolean features, a.k.a.
 ...
 *   <li><em>attributes</em> ({@link #parseAttributes(boolean)} and {@link
 *       #parseAttribute(Attribute) methods)}: whether attributes should be parsed (of course,
 *       setting this flag is useless if you are not parsing tags); note that setting this flag will
 *       just activate the attribute parsing feature, but you must also {@linkplain
 *       #parseAttribute(Attribute) register} every attribute whose value you want to obtain.
 *   <li><em>text</em> ({@link #parseText(boolean)}method): whether text should be parsed; if this
 *       flag is set, the parser will call the {@link
 *       it.unimi.dsi.parser.callback.Callback#characters(char[], int, int, boolean)} method for
 ...
 *       {@link it.unimi.dsi.parser.callback.Callback#cdata(Element,char[],int,int)} method for
 *       every CDATA section found.
 * </ul>
 *
 * <h2>Invoking the parser</h2>
 *
 * <p>After {@linkplain #setCallback(Callback) setting the parser callback}, you just call {@link
 * #parse(char[], int, int)}.
 */
public class BulletParser {
  ...
  protected static final TextPattern SCRIPT_CLOSE_TAG_PATTERN =
      new TextPattern("</script>", TextPattern.CASE_INSENSITIVE);
  /** Closing tag for a style element. */
  protected static final TextPattern STYLE_CLOSE_TAG_PATTERN =
      new TextPattern("</style>", TextPattern.CASE_INSENSITIVE);
   ...
   * @throws IllegalStateException if {@link #parseAttributes(boolean) parseAttributes(true)} has
   *     not been invoked on this parser.
   * @return this parser.
   */
   ...
   * @return the character corresponding to the entity, or an ASCII NUL if no entity with that name
   *     was found.
   */
   ...
   *
   * <p>Real-world HTML pages often contain hundreds of misplaced ampersands, due to the unfortunate
   * idea of using the ampersand as query separator (<em>please</em> use the comma in new code!).
   * All such ampersand should be specified as <samp>&amp;amp;</samp>. If named entities are
   * delimited using a transition from alphabetical to non-alphabetical characters, we can easily
   * get false positives. If the parameter <code>loose</code> is false, named entities can be
   * delimited only by whitespace or by a comma.
   *
   ...
   * @param entity a support mutable string used to query {@link
   *     ParsingFactory#getEntity(MutableString)}.
   * @return the position of the last character of the entity, or -1 if no entity was found.
   */
  protected int scanEntity(
      final char[] a,
      final int offset,
      final int length,
      final boolean loose,
      final MutableString entity) {
    ...
    if (a[offset + 1] == '#') {
      if (length > 2 && a[offset + 2] == 'x') {
        for (i = 3;
            i < length
                && i < MAX_HEX_ENTITY_LENGTH
                && Character.digit(a[i + offset], HEXADECIMAL) != -1;
            i++) ;
        tmpEntity = new String(a, offset + 3, i - 3);
        if (i != 3) c = Integer.parseInt(tmpEntity, HEXADECIMAL);
      } else {
        for (i = 2;
            i < length && i < MAX_DEC_ENTITY_LENGTH && Character.isDigit(a[i + offset]);
            i++) ;
      ...
      if (Character.isLetter(a[offset + 1])) {
        ...
        if (i != 1
            && (loose
                || (i < length && (Character.isWhitespace(a[offset + i]) || a[offset + i] == ';')))
            && (lastEntity = entity2Char(entity.length(0).append(a, offset + 1, i - 1))) != 0) {
          if (i < length && a[i + offset] == ';') i++;
          return i + offset;
        }
      }
   ...
   * entities are replaced by the corresponding character.
   *
   * @param s a mutable string whose entities will be replaced by the corresponding characters.
   * @param entity a support mutable string used by {@link #scanEntity(char[], int, int, boolean,
   *     MutableString)}.
   * @param loose a parameter that will be passed to {@link #scanEntity(char[], int, int, boolean,
   *     MutableString)}.
   */
  protected void replaceEntities(
      final MutableString s, final MutableString entity, final boolean loose) {
  ...
  public void parse(final char[] text, final int offset, final int length) {
    ...
    while (pos < end) {
      ...
      switch (state) {
          ...
          switch (text[++pos]) {
            ...
            default:
              ...
              if (Character.isLetter(text[pos])) state = STATE_BEFORE_START_TAG_NAME;
                  ...
                  && (Character.isLetter(text[pos + 1]) || text[pos + 1] == '>')) {
        ...
        case STATE_BEFORE_END_TAG_NAME:
          ...
          for (start = pos;
              pos < end
                  && (Character.isLetterOrDigit(text[pos])
                      || text[pos] == ':'
                      || text[pos] == '_'
                      || text[pos] == '-'
                      || text[pos] == '.');
          ...
          if (DEBUG)
            System.err.println(
                (state == STATE_BEFORE_START_TAG_NAME ? "Opening" : "Closing")
                    + " tag for "
                    + tagElemTypeName
                    + " (element: "
                    + currentElement
        ...
        case STATE_IN_START_TAG:
          ...
          if (currChar != '>' && (currChar != '/' || pos == end - 1 || text[pos + 1] != '>')) {
            ...
            if (Character.isLetter(currChar)) {
              ...
              attrName.length(0);
              for (start = pos;
                  pos < end && (Character.isLetter(text[pos]) || text[pos] == '-');
                  pos++) ;
              ...
              if (pos < end && ((delim = text[pos]) == '"' || (delim = text[pos]) == '\'')) {
                ...
                for (start = pos;
                    pos < end && !Character.isWhitespace(text[pos]) && text[pos] != '>';
                    pos++) ;
    ...
    if (state == STATE_IN_START_TAG && parseTags && currentElement != null)
      callback.startElement(currentElement, attrMap);
    if (state == STATE_IN_END_TAG && parseTags && currentElement != null)
      callback.endElement(currentElement);
