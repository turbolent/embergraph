 ...
 * callback}, which will invoke (in order) the callbacks.
 */
public class ComposedCallbackBuilder {
  ...
  private static final class ComposedCallback implements Callback {
    ...
    public boolean characters(
        final char[] text, final int offset, final int length, final boolean flowBroken) {
      boolean retValue = false;
      for (int i = 0; i < size; i++) {
    ...
    public boolean cdata(
        final Element element, final char[] text, final int offset, final int length) {
      boolean retValue = false;
      for (int i = 0; i < size; i++) {
  ...
  private final ObjectArrayList<Callback> callbacks =
      ObjectArrayList.wrap(Callback.EMPTY_CALLBACK_ARRAY);
