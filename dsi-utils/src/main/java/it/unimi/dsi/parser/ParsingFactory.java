 ...
 *
 * <p>For instance, the {@linkplain it.unimi.dsi.parser.HTMLFactory standard factory for HTML}
 * parsing has ready-made interned versions of all names in the (X)HTML specification, and returns
 * them upon request, but other policies are possible. For instance, instances of {@link
 * WellFormedXmlFactory} intern every seen name, without reference to a data type (except for
 * entities, in which case the HTML set is used).
 *
 * <p>The idea of factoring out the creation of interned counterparts of SGML/XML syntactical
 * objects is due to Fabien Campagne.
 *
