 ...
 *
 * <p>By contract, all implementations of this interface are bound to be <em>reusable</em>: by
 * calling {@link #startDocument()}, a callback can be used again. It <strong>must</strong> be safe
 * to call {@link #startDocument()} any number of times.
 */
public interface Callback {
   ...
   * required by the callback is actually parsed. The configuration <strong>must</strong> be a
   * monotone process&mdash;you can only <em>set</em> properties and <em>add</em> attribute types to
   * be parsed.
   */
   ...
   * <strong>must</strong> be safe to invoke this method several times.
   */
  void startDocument();
   ...
   * just receive notifications for elements whose closing tag appears <em>explicitly</em> in the
   * document.
   *
   * <p>This method will never be called for element without closing tags, even if such a tag is
   * found.
   *
   ...
   *
   * <p><code>flowBroken</code> will be true iff the flow was broken before <code>text</code>. This
   * feature makes it possible to extract quickly the text in a document without looking at the
   * elements.
   *
   ...
   * <samp>SCRIPT</samp> element. In that case, the element will be passed as first argument.
   *
   * <p>You must not write into <code>text</code>, as it could be passed around to many callbacks.
   *
   * @param element the element enclosing the CDATA section, or <code>null</code> if the CDATA
   *     section was created with explicit markup.
   * @param text an array containing the character data.
   * @param offset the start position in the array.
   * @param length the number of characters to read from the array.
   * @return true to keep the parser parsing, false to stop it.
