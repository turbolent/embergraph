...
public final class Util {
   ...
   * <samp>M</samp>, <samp>G</samp>, and <samp>T</samp>) and printing just two fractional digits.
   *
   ...
   * <samp>Mi</samp>, <samp>Gi</samp>, and <samp>Ti</samp>) and printing <em>no</em> fractional
   * digits. The argument must be a power of 2.
   *
  ...
  public static void ensureLog4JIsConfigured(final Class<?> klass, final Level level) {
    if (!log4JIsConfigured()) {
      System.err.println(
          "WARNING: "
              + (klass != null ? klass.getSimpleName() + " is" : "We are")
              + " autoconfiguring Log4J (level: "
              + level
              + "). You should configure Log4J properly instead.");
