 ...
 *
 * <p>To use this class, you first create a new instance by passing a {@linkplain
 * org.apache.log4j.Logger Log4J logger}, a {@link org.apache.log4j.Priority} and a time interval in
 * millisecond (you can use constants such as {@link #ONE_MINUTE}). Information will be logged about
 * the current state of affairs no more often than the given time interval. The output of the logger
 * depends on the {@linkplain #itemsName items name} (the name that will be used to denote counted
 * items), which can be changed at any time.
 *
 * <p>To log the progress of an activity, you call {@link #start(CharSequence)} at the beginning,
 * which will display the given string. Then, each time you want to mark progress, you call {@link
 * #update()} or {@link #lightUpdate()}. The latter methods increase the item counter, and will log
 * progress information if enough time has passed since the last log (and if the counter is a
 * multiple of 2<sup>{@link #log2Modulus}</sup>, in the case of {@link #lightUpdate()}).
 *
 * <p>When the activity is over, you call {@link #stop()}. At that point, the method {@link
 * #toString()} returns information about the internal state of the logger (elapsed time, number of
 * items per second) that can be printed or otherwise processed. If {@link #update()} has never been
 * called, you will just get the elapsed time. By calling {@link #done()} instead of stop, this
 * information will be logged for you.
 *
 ...
 *
 * <p>After you finished a run of the progress logger, you can change its attributes and call {@link
 * #start()} again to measure another activity.
 *
 ...
 * </PRE>
 *
 * <p>A more flexible behaviour can be obtained at the end of the process by calling {@link
 * #stop()}:
 *
 * <PRE>
 ...
 *
 * <p>Note that the {@linkplain org.apache.log4j.Logger Log4J logger} and priority are available via
 * the public fields {@link #logger} and {@link #priority}: this makes it possible to pass around a
 * progress logger and log additional information on the same logging stream. The priority is
 * initialised to {@link Level#INFO}, but it can be set at any time.
 *
...
public final class ProgressLogger {
   ...
   * private method) so to suggest inlining. However, it performs a call to {@link
   * System#currentTimeMillis()} that takes microseconds (not nanoseconds). If you plan on calling
   * this method more than a few thousands times per second, you should use {@link #lightUpdate()}.
   */
  public void update() {
    count++;
  ...
  private String freeMemory() {
    return (displayFreeMemory
        ? "; used/avail/free/total/max mem: "
  ...
  private void updateInternal() {
    ...
    final long millisToEnd =
        Math.round((expectedUpdates - count) * ((currentTime - start) / (count + 1.0)));
    ...
    if (logger.isEnabledFor(priority))
      logger.log(
          ...
          Util.format(count)
              ...
              + millis2hms(millis())
              + ", "
              + Util.format((count * 1000.0) / (currentTime - start))
              + " "
              + itemsName
              + "/s"
              + (expectedUpdates > 0
                  ? "; "
                      + Util.format((100 * count) / expectedUpdates)
                      + "% done, "
                      + millis2hms(millisToEnd)
                      + " to end"
                  : "")
              + freeMemory()
              + (info != null ? "; " + info : ""));
   ...
   * will actually call {@link System#currentTimeMillis()} only if the new {@link #count} is a
   * multiple of 2<sup>{@link #log2Modulus}</sup>. This mechanism makes it possible to reduce the
   * number of calls to {@link System#currentTimeMillis()} arbitrarily.
   *
   ...
   *
   * @return the number of milliseconds between present time and the last call to {@link #start()},
   *     if this progress logger is running, or between the last call to {@link #stop()} and the
   *     last call to {@link #start()}, if this progress logger is stopped.
   */
  public long millis() {
