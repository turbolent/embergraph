 ...
 * that are radically different from the ones adopted in {@link java.io.BufferedReader}.
 *
 * <p>There is no support for marking. All methods are unsychronised. All methods returning strings
 * do so by writing in a given {@link it.unimi.dsi.lang.MutableString}.
 *
 * <p>Note that instances of this class can wrap {@linkplain #FastBufferedReader(char[]) an array}
 * or a {@linkplain #FastBufferedReader(MutableString) mutable string}. In this case, instances of
 * this class may be used as a lightweight, unsynchronised alternative to {@link
 * java.io.CharArrayReader} providing additional services such as word and line breaking.
 *
 * <p>As any {@link it.unimi.dsi.io.WordReader}, this class is serialisable. The only field kept is
 * the current buffer size, which will be used to rebuild a fast buffered reader with the same
 * buffer size. All other fields will be reset.
 *
 * <h2>Reading words</h2>
 *
 * <p>This class implements {@link WordReader} in the simplest way: words are defined as maximal
 * subsequences of characters satisfying {@link Character#isLetterOrDigit(char)}. To alter this
 * behaviour, you have two choices:
 *
 * <ul>
 *   <li>you can provide at construction time a {@link CharSet} of characters that will be
 *       considered <em>word constituents</em> besides those accepted by {@link
...
public class FastBufferedReader extends Reader implements WordReader {
   ...
   */
  public FastBufferedReader(
      final char[] array, final int offset, final int length, final CharSet wordConstituents) {
   ...
   * stored into <code>s</code>. Note that if <code>s</code> is not {@linkplain
   * it.unimi.dsi.lang.MutableString loose} this method will be quite inefficient.
   *
   * @param s a mutable string that will be used to store the next line (which could be empty).
   * @return <code>s</code>, or <code>null</code> if the end of file was found, in which case <code>
   *     s</code> is unchanged.
   */
   ...
   * changed by overwriting this method.
   *
   * @param c a character.
