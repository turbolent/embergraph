 ...
 * such a stream in several ways: writing an integer or long in fixed-width, unary, &gamma;,
 * &delta;, &zeta; and Golomb coding, or providing a vector of bytes.
 *
 * <p>This class can also {@linkplain #OutputBitStream(byte[]) wrap a byte array}; this is much more
 * lightweight than wrapping a {@link it.unimi.dsi.fastutil.io.FastByteArrayOutputStream} wrapping
 * the array, but overflowing the array will cause an {@link java.io.IOException}.
 *
 * <p>Note that when writing using a vector of bytes bits are written in the natural way: the first
 * bit is bit 7 of the first byte, the eightth bit is bit 0 of the first byte, the ninth bit is bit
 * 7 of the second byte and so on. When writing integers using some coding, instead, the
 * <em>lower</em> bits are considered for coding (in the fixed-width case, the given number of bits,
 * otherwise the lower bits starting from the most significant one).
 *
 * <h3>The bit stream format</h3>
 *
 * <p>The bit streams written by this class are <em>big endian</em>. That is, the first bit of the
 * stream is bit 7 of the first byte, the eightth bit is bit 0 of the first byte, the ninth bit is
 * bit 7 of the second byte and so on.
 *
 * <p>Blocks of bits (such as coded integers) are written <em>starting from the most significant
 * bit</em>. In other words, if you take the first bytes of a stream and print them in binary you
 * will see exactly the sequence of bits you have written. In particular, if you write 32-bit
 * integers you will get a stream which is identical to the one produced by a {@link
 * java.io.DataOutput}.
 *
 * <p>Additional features:
 *
 * <ul>
 *   <LI>This class provides an internal buffer. By setting a buffer of length 0 at creation time,
 *       you can actually bypass the buffering system: Note, however, that several classes providing
 *       buffering have synchronised methods, so using a wrapper instead of the internal buffer is
 *       likely to lead to a performance drop.
 *   <LI>To work around the schizophrenic relationship between streams and random access files in
 *       {@link java.io}, this class provides a {@link #flush()} method that byte-aligns the
 *       streams, flushes to the underlying byte stream all data and resets the internal state. At
 *       this point, you can safely reposition the underlying stream and write again afterwards. For
 *       instance, this is safe and will perform as expected:
 *       <PRE>
 ...
 * </PRE>
 *       <p>As a commodity, an instance of this class will try to cast the underlying byte stream to
 *       a {@link RepositionableStream} and to fetch by reflection the {@link
 *       java.nio.channels.FileChannel} underlying the given output stream, in this order. If either
 *       reference can be successfully fetched, you can use directly the {@link #position(long)
 *       position()} method with argument <code>pos</code> with the same semantics of a {@link
 *       #flush()}, followed by a call to <code>position(pos / 8)</code> (where the latter method
 *       belongs either to the underlying stream or to its underlying file channel). The specified
 *       position must be byte aligned, as there is no clean way of reading a fraction of a byte
 *       with the current APIs. However, since the reflective checks are quite heavy they can be
 *       disabled using a {@linkplain OutputBitStream#OutputBitStream(OutputStream, boolean)
 *       suitable constructor}.
 * </ul>
 *
 * <p><STRONG>This class is not synchronised</STRONG>. If multiple threads access an instance of
 * this class concurrently, they must be synchronised externally.
 *
...
public class OutputBitStream implements Flushable, Closeable {
  ...
  public static final int[] GAMMA = new int[MAX_PRECOMPUTED],
      DELTA = new int[MAX_PRECOMPUTED],
      ZETA_3 = new int[MAX_PRECOMPUTED],
   ...
   *
   * @param os the output stream to wrap.
   * @param testForPosition if false, the reflective tests that are necessary to support {@link
   *     #position(long)} will not be performed.
   */
  ...
  public OutputBitStream(final OutputStream os, final int bufSize, final boolean testForPosition) {
    ...
    if (os instanceof RepositionableStream) {
      ...
      try {
        fc =
            (FileChannel)
                (os.getClass().getMethod("getChannel", new Class[] {})).invoke(os, new Object[] {});
   ...
   *
   * <p>This constructor invokes directly {@link FileOutputStream#getChannel()} to support {@link
   * #position(long)}.
   *
   * @param os the output stream to wrap.
   */
   ...
   * to the {@link OutputStream#flush()} method of the underlying output stream.
   *
   * <p>This method is provided so that users of this class can easily wrap repositionable streams
   * (for instance, file-based streams, which can be repositioned using the underlying {@link
   * java.nio.channels.FileChannel}).
   *
   * <p>It is guaranteed that after calling this method the underlying stream can be repositioned,
   * and that the next write to the underlying output stream will start with the content of the
   ...
   * </code> the written-bits count after a {@link #flush()}.
   *
   ...
   *
   * <p>However, this method does <em>not</em> update {@link #writtenBits}. The caller should
   * increment {@link #writtenBits} by 8 at each call.
   */
   ...
   * written {@link #free} bits the bit buffer will be empty. In particular, there should never be 0
   * free bits in the buffer.
   *
   * @param b the bits to write in the <strong>lower</strong> positions; the remaining positions
   *     must be zero.
   * @param len the number of bits to write (0 is safe and causes no action).
   * @return the number of bits written.
   * @throws IllegalArgumentException if one tries to write more bits than available in the buffer
   ...
   * java.nio.channels.FileChannel}.
   *
   * <p>Given an underlying stream that implements {@link RepositionableStream} or that can provide
   * a {@link java.nio.channels.FileChannel} via the <code>getChannel()</code> method, a call to
   * this method has the same semantics of a {@link #flush()}, followed by a call to {@link
   * java.nio.channels.FileChannel#position(long) position(position / 8)} on the byte stream.
   * Currently there is no clean, working way of supporting out-of-byte-boundary positioning.
   *
   ...
   * @throws UnsupportedOperationException if the underlying byte stream does not implement {@link
   *     RepositionableStream} and if the channel it returns is not a {@link
   *     java.nio.channels.FileChannel}.
   ...
   * eightth bit is bit 0 of the first byte, the ninth bit is bit 7 of the second byte and so on.
   *
  ...
  public long write(final byte[] bits, final long offset, final long len) throws IOException {
    ...
    if (len <= initial)
      return writeInt((0xFF & bits[(int) (offset / 8)]) >>> (initial - len), (int) len);
    return writeInt(bits[(int) (offset / 8)], initial)
   ...
   * number of bits written increased by one and with the sign changed.
   *
   * @param i a boolean iterator.
   * @return if <code>i</code> did not throw a runtime exception, the number of bits written;
   *     otherwise, the number of bits written, plus one, with the sign changed.
   */
   ...
   *
   * @param x an integer.
   * @param len a bit length; this many lower bits of the first argument will be written (the most
   ...
   * <var>k</var>-1 in unary, followed by the lower <var>k</var>-1 bits of the number. The coding of
   * a natural number is obtained by adding one and coding.
   *
   * @param x a natural number.
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number.
   ...
   * is obtained writing <var>k</var> in unary, followed by the lower <var>k</var>-1 bits of the
   * number (equivalently, by writing <var>k</var> zeroes followed by the number).
   *
   * @param x a natural number.
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number.
   ...
   * <var>k</var>-1 in &gamma; coding, followed by the lower <var>k</var>-1 bits of the number. The
   * coding of a natural number is obtained by adding one and coding.
   *
   * @param x a natural number.
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number.
   ...
   * optimal code in which shorter words are assigned to smaller integers.
   *
   * @param x a natural number.
   * @param b a strict upper bound for <code>x</code>.
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number or use a nonpositive
   *     base.
   */
   ...
   * @see #writeMinimalBinary(int, int)
   */
  public int writeLongMinimalBinary(final long x, final long b, final int log2b)
      throws IOException {
    if (x < 0) throw new IllegalArgumentException("The argument " + x + " is negative");
    if (b < 1) throw new IllegalArgumentException("The bound " + b + " is not positive");
    if (x >= b) throw new IllegalArgumentException("The argument " + x + " exceeds the bound " + b);
   ...
   * of the division of <var>x</var> and <var>b</var> in {@linkplain #writeUnary(int) unary},
   ...
   * argument <code>x</code> may only be zero, and nothing will be written.
   *
   * @param x a natural number.
   * @param b the modulus for the coding.
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number or use a negative
   *     modulus.
   */
   ...
   * @param x a natural number.
   * @param b the modulus for the coding.
   * @param log2b the floor of the base-2 logarithm of the coding modulus (it is irrelevant when
   *     <code>b</code> is zero).
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number or use a negative
   *     modulus.
   ...
   * by coding <var>h</var> in unary, followed by a minimal binary coding of the offset in the
   * interval. The coding of a natural number is obtained by adding one and coding.
   *
   * <p>&zeta; codes were defined by Paolo Boldi and Sebastiano Vigna in &ldquo;<a
   * href="http://vigna.dsi.unimi.it/papers.php#BoVCWWW">Codes for the World&minus;Wide
   * Web</a>&rdquo;, <i>Internet Math.</i>, 2(4):405-427, 2005. The paper contains also a detailed
   * analysis.
   *
   * @param x a natural number.
   ...
   * to the left using zeroes, until its length is a multiple of three. Then, the resulting string
   * is broken in blocks of 3 bits, and each block is prefixed with a bit, which is zero for all
   * blocks except for the last one.
   *
   * @param x a natural number.
   * @return the number of bits written.
   * @throws IllegalArgumentException if you try to write a negative number.
