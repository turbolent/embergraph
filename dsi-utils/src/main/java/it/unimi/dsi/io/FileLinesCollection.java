 ...
 *
 * <p><strong>Warning</strong>: the lines returned by iterators generated by instances of this class
 * <em>are not cacheable</em>. The returned value is a {@link it.unimi.dsi.lang.MutableString}
 * instance that is reused at each call, and that is <em>modified by a call to {@link
 * java.util.Iterator#hasNext() hasNext()}</em>. Thus, for instance,
 *
 * <pre>
 ...
 * under the form of compact {@link it.unimi.dsi.lang.MutableString}s). Note also that {@link
 * #toString()} will return a single string containing all file lines separated by the string
 * associated to the system property <samp>line.separator</samp>.
 *
 * <p>An instance of this class allows to access the lines of a file as a {@link
 * java.util.Collection}. Using {@linkplain java.util.Collection#contains(java.lang.Object) direct
 * access} is strongly discouraged (it will require a full scan of the file), but the {@link
 * #iterator()} can be fruitfully used to scan the file, and can be called any number of times, as
 * it opens an independent input stream at each call. For the same reason, the returned iterator
 * type ({@link it.unimi.dsi.io.FileLinesCollection.FileLinesIterator}) is {@link
 * java.io.Closeable}, and should be closed after usage.
 *
 * <p>Using a suitable {@linkplain #FileLinesCollection(CharSequence, String, boolean) constructor},
 * it is possible to specify that the file is compresse in <samp>gzip</samp> format (in this case,
 * it will be opened using a {@link GZIPInputStream}).
 *
...
public class FileLinesCollection extends AbstractCollection<MutableString> {
  ...
  /*
   * Creates a file-lines collection for the specified filename with the specified encoding,
   * optionally assuming that the file is compressed using <samp>gzip</samp> format.
   *
   * @param filename a filename.
   * @param encoding an encoding.
   * @param zipped whether <samp>filename</samp> is zipped.
   */
  public FileLinesCollection(
      final CharSequence filename, final String encoding, final boolean zipped) {
    this.zipped = zipped;
   ...
   *
   * <p>Instances of this class open an {@link java.io.InputStream}, and thus should be {@linkplain
   * Closeable#close() closed} after usage. A &ldquo;safety-net&rdquo; finaliser tries to take care
   * of the cases in which closing an instance is impossible. An exhausted iterator, however, will
