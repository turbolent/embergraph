 ...
 *
 * <p>In particular, the factory method {@link #map(FileChannel, FileChannel.MapMode)} will
 * memory-map an entire file into an array of {@link ByteBuffer} and expose the array as a {@link
 * ByteBufferInputStream}. This makes it possible to access easily mapped files larger than 2GiB.
 *
...
public class ByteBufferInputStream extends MeasurableInputStream implements RepositionableStream {
  ...
  public static ByteBufferInputStream map(final FileChannel fileChannel, MapMode mapMode)
      throws IOException {
    ...
    for (int i = 0; i < chunks; i++)
      byteBuffer[i] =
          fileChannel.map(mapMode, CHUNK_SIZE * i, Math.min(CHUNK_SIZE, size - CHUNK_SIZE * i));
  ...
  private long remaining() {
    return curr == n - 1
        ? byteBuffer[curr].remaining()
        : byteBuffer[curr].remaining()
            + ((long) (n - 2 - curr) << FIRST_SHIFT)
            + ((long) (n - 2 - curr) << SECOND_SHIFT)
            + byteBuffer[n - 1].capacity();
  }
  ...
  public long position() {
    return ((long) curr << FIRST_SHIFT)
        + ((long) curr << SECOND_SHIFT)
        + byteBuffer[curr].position();
  }
  ...
  public void position(long newPosition) {
    ...
    byteBuffer[curr].position(
        (int) (newPosition - ((long) curr << FIRST_SHIFT) - ((long) curr << SECOND_SHIFT)));
  }
