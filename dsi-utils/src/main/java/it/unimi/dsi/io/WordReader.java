 ...
 * using {@link #setReader(Reader)}.
 *
 * <p>This interface is heavily oriented towards reusability and streaming. It is conceived so that
 * at most one method call has to be performed per <em>word</em>, rather than per
 * <em>character</em>, and that implementations may completely avoid object creation by {@linkplain
 * #setReader(Reader) setting explicitly the underlying reader}.
 *
 * <p>The standard implementation ({@link it.unimi.dsi.io.FastBufferedReader}) breaks words in the
 * trivial way. More complex implementations (e.g., for languages requiring segmentation) can
...
public interface WordReader extends Serializable {
   ...
   * extracted. It is acceptable that the <em>first</em> call to this method after creation or after
   * a call to {@link #setReader(Reader)} returns an empty word. In other words <em>both <code>word
   * </code> and <code>nonWord</code> are maximal</em>.
   *
