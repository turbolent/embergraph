 ...
 * {@link java.io.FileInputStream}) and exposes its contents as a number of separated input streams.
 * Each such stream, called a <em>block</em>, is defined by a start and a stop position (gaps
 ...
 * from one segment to the following one, whereas calling {@link SegmentedInputStream#close()} moves
 * from a block to the following one.
 *
...
public class SegmentedInputStream extends MeasurableInputStream {
  ...
  private static class SegmentBlock {
     ...
     * @throws IllegalArgumentException if the elements of <code>delimiter</code> are negative or
     *     not increasing.
     */
      ...
      for (int i = 0; i < delimiter.length - 1; i++)
        if (delimiter[i] > delimiter[i + 1])
          throw new IllegalArgumentException(
              "Segment "
                  + (i + 1)
                  + " is inconsistent as it starts after the next one: "
  ...
  private void ensureBlocksNotEmpty() {
    if (blocks.isEmpty())
      throw new IllegalStateException(
          "You must add at least one block before reading or closing a segmented stream");
  }
  ...
  private void nextSegment() throws IOException {
    ...
    if (currentStartMarker - absPos > 0) {
      ...
      if (diff != currentStartMarker - absPos)
        throw new IllegalStateException(
            "Should have skipped " + (currentStartMarker - absPos) + " bytes, got " + diff);
    }
   ...
   *
   * <p>This method performs the initial call to {@link #nextBlock()} when the first marker is put
   * in.
   *
   * @param delimiter a list of segment delimiters.
   * @throws IllegalArgumentException if the elements of <code>delimiter</code> are negative or not
   *     increasing.
   */
