...
public final class CanonicalFast64CodeWordDecoder implements Decoder, Serializable {
  ...
  public CanonicalFast64CodeWordDecoder(final int[] codeWordLength, final int[] symbol) {
    ...
    // We compute how many different codeword lengths are present. We check also for excessive or
    // nondecreasing length.
    int howManyLengths = 1;
    if (size > 0)
      for (int i = size - 1; i-- != 0; ) {
        if (codeWordLength[i] > Long.SIZE)
          throw new IllegalArgumentException("Codeword length must not exceed 64");
        if (codeWordLength[i] > codeWordLength[i + 1])
   ...
   * @return the bits read, stored into a long: the first read bit will be bit <code>length</code>
   *     &minus; 1.
   */
  ...
  public int decode(final BooleanIterator iterator) {
    ...
    for (; ; ) {
      if (x < lastCodeWordPlusOne[curr])
        return symbol[(int) (howManyUpToBlock[curr] - lastCodeWordPlusOne[curr] + x)];
