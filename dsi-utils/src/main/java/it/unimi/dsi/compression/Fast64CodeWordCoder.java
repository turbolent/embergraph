 ...
 *
 * <ol>
 *   <li>Modified to expose a ctor which accepts just the codeWord[].
 * </ol>
 */
public final class Fast64CodeWordCoder extends CodeWordCoder {
   ...
   * @param longCodeWord the same codewords as those specified in <code>codeWord</code>, but as
   *     right-aligned longs written in left-to-right fashion.
   */
  ...
  public Fast64CodeWordCoder(final BitVector[] codeWord) {
    ...
    for (int i = 0; i < codeWord.length; i++) {
      ...
      final long longValue = Long.reverse(v.getLong(0, Math.min(64, nbits)) << (64 - nbits));
