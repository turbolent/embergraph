...
public abstract class AbstractCachePolicyTest extends TestCase2 {
  ...
  public static void assertSameIterator(String msg, Object[] expected, Iterator actual) {
    ...
    while (actual.hasNext()) {
      ...
      assertTrue(
          msg + ": Different objects at index=" + i + ": expected=" + expected[i] + ", actual=" + g,
          expected[i].equals(g));
   ...
   * reference); and same dirty flag state.
   *
   * @param msg Message.
  ...
  public void assertSameEntryOrdering(String msg, ICacheEntry[] expected, Iterator actual) {
    ...
    while (actual.hasNext()) {
      ...
      assertEquals(
          msg + ": key differs at index=" + i, expectedEntry.getKey(), actualEntry.getKey());
  ...
  public static class MyCacheListener<K, T> implements ICacheListener<K, T> {
     ...
     * @exception AssertionFailedException If we are not expecting an event.
     * @exception AssertionFailedException If the object identifier or object in the event are
     *     incorrect. The objects are compared by reference, not by equals().
     */
     ...
     * @param obj The expected object (comparison by reference).
     * @param dirty Iff the expected object will be marked as dirty.
     * @see #clearLastEvent()
     * @see #denyEvents()
     */
    ...
    /** Allows more events. If an event had already been received, then it is cleared now. */
     ...
     * event.
     *
     * @exception IllegalStateException if no event has been received.
     */
   ...
   * cache.
   *
   * <p>Note: LRU cache policy tests that will be reused for the weak reference cache MUST hold hard
   * references to the objects that are inserted into the cache to ensure that the cache entries for
   * those objects are not cleared from the weak reference cache. The weak reference cache facility
   * for clearing cache entries once the objects in those entries become weakly reachable must be
   * tested by specialized test methods.
   *
   * @param capacity the capacity of the hard reference cache policy.
   ...
   *
   * <p>The test method retains a hard reference to the objects inserted into the cache so that the
   * cache entries for such objects can not be cleared when {@link #getCachePolicy(int)} returns a
   * cache with weak reference semantics. This makes the behavior of the test deterministic. Since
   * we are holding hard references, the size of the weak reference cache is not capped and should
   * refelect all objects inserted into the cache and not yet removed from the cache by the test
   * method.
   *
   * @see ICachePolicy
