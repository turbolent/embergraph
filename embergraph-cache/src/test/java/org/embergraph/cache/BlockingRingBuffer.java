 ...
 * @version $Id$
 * @deprecated This implementation is incomplete. Realizing this implementation would require adding
 *     support for signals when the buffer becomes "nonEmpty" and when it becomes "notFull".
 *     However, the {@link ArrayBlockingQueue} already provides a well-testing ring buffer which
 *     implements the {@link BlockingQueue} API. The only reason to pursue this implementation
 *     further would be to gain more control over the backing array. For example, so as to be able
 *     to combine chunks together when the generic type of the array is itself an array type.
 */
