...
public class TestRingBuffer extends TestCase2 {
   ...
   *
   * <p>The unit tests beings by adding in elements until the buffer is in this state:
   *
   * <pre>
   ...
   *
   * <p>The following condition is also tested. First, we fill the buffer. Then we remove an
   * element. Finally we add another element. This yields the following state, which is the same
   * state evaluated in {@link #test_ringBuffer()}.
   *
   * <pre>
   ...
   * cover here is the ability of the iterator to remove the correct element and to refuse to (a)
   * double-remove an element; and (b) to remove an element when non-has been visited.
   */
  ...
  public void test_toArray1_nonempty_oversized() {
    ...
    Object[] oversizedArr = buffer.toArray(scratch);
