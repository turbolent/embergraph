 ...
 * @todo make sure that the dirty flag is correctly maintained as entries are cleared and removed
 *     from the weak value cache (i.e., that we do not falsely apply the old state of the flag when
 *     recycling cache entries).
 * @todo performance test suite for tuning access paths in the cache implementation, including the
 *     load factor on the weak reference and hard reference caches. Is it possible to write this
 *     test so that it can be reused when a persistence layer is integrated?
 * @todo integration test with abstraction for the specific persistence layer, but never the less
 *     testing the semantics of the weak reference cache with the integrated persistence layer. The
 *     abstract could consist of the means for configuring the persistence layer with a known cache
 *     configuration.
 * @version $Id$
...
public class TestWeakValueCache extends AbstractCachePolicyTest {
  ...
  /** Test verifies that cache evictions are fired once the inner LRU cache is full. */
