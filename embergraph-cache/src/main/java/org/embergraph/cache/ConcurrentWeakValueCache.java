...
public class ConcurrentWeakValueCache<K, V> implements IConcurrentWeakValueCache<K, V> {
   ...
   *
   * @param queueCapacity The {@link HardReferenceQueue} capacity. When ZERO (0), there will not be
   *     a backing hard reference queue.
   * @param loadFactor The load factor.
   * @param concurrencyLevel The concurrency level.
  ...
  /*
   * Defaults the initial capacity of the map based on the capacity of the optional {@link
   * IHardReferenceQueue} and uses the Java default of <code>16</code> if there is no queue.
   *
   * @param queue The {@link IHardReferenceQueue} (optional).
   * @param loadFactor The load factor.
   * @param concurrencyLevel The concurrency level.
   * @param removeClearedReferences When <code>true</code> the cache will remove entries for cleared
   *     references. When <code>false</code> those entries will remain in the cache.
  ...
  public ConcurrentWeakValueCache(
    ...
    this(
        queue,
        (queue == null ? 16 : Math.max(16, queue.capacity() / 2)),
        loadFactor,
        concurrencyLevel,
        removeClearedReferences);
  }
   ...
   * @param k The key.
   * @return The value.
   * @todo If we can locate a ConcurrentRingBuffer then we could further improve performance by
   *     removing synchronization on the hard reference queue in this method.
   ...
   * does not block and only polls the {@link ReferenceQueue}.
   *
