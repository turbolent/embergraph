 ...
 * to be effective, <em>this class is not suitable when the total size of the cache is limited to a
 * relatively small number of entries</em>. Further, since there is no provision for flushing the
 * thread-local queues in response to inactivity or the end of some work cycle for a thread, the
 * thread-local queues can cause hard references to be retained for an arbitrary period of time.
 * Therefore <em>DO NOT use this class when objects entered into the cache MUST have their
 ...
 * objects in the cache. If the application holds hard references to more than this many objects,
 * e.g., in various threads, collections, etc., then the cache size can be larger than the queue
 * capacity. Likewise, if the JVM takes too long to finalize weakly reachable references, then the
 * cache size can exceed this limit even if the application does not hold ANY hard references to the
 * objects in the cache. For these reasons, this class uses an initialCapacity for the inner {@link
 * ConcurrentHashMap} that is larger than the <i>queueCapacity</i>. This helps to prevent resizing
 * the {@link ConcurrentHashMap} which is a relatively expensive operation.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: ConcurrentWeakValueCache.java 2265 2009-10-26 12:51:06Z thompsonbry $
...
public class ConcurrentWeakValueCacheWithBatchedUpdates<K, V>
  ...
  private static final transient Logger log =
      Logger.getLogger(ConcurrentWeakValueCacheWithBatchedUpdates.class);
   ...
   * is thread-safe due to the use of thread-local queues and batched updates to the backing {@link
   * IHardReferenceQueue}. This can substantially reduce thread contention.
   */
   ...
   *     a backing hard reference queue.
   */
   ...
   */
  public ConcurrentWeakValueCacheWithBatchedUpdates(
      final int queueCapacity, final float loadFactor, final int concurrencyLevel) {
   ...
   * @param loadFactor The load factor.
   * @param concurrencyLevel The concurrency level.
   * @param removeClearedReferences When <code>true</code> the cache will remove entries for cleared
   *     references. When <code>false</code> those entries will remain in the cache.
   */
  public ConcurrentWeakValueCacheWithBatchedUpdates(
   ...
   * IHardReferenceQueue}.
   *
   * @param queue The {@link IHardReferenceQueue}.
   * @param loadFactor The load factor.
   * @param concurrencyLevel The concurrency level.
   * @param removeClearedReferences When <code>true</code> the cache will remove entries for cleared
   *     references. When <code>false</code> those entries will remain in the cache.
   ...
   *     HardReferenceQueueWithBatchingUpdates} instance.
   ...
   *
   * @param k The key.
   * @return <code>true</code> iff the map contains an entry for that key whose weak reference has
   *     not been cleared.
   */
   ...
   * @param k The key.
   * @param v The value.
   * @return The old value under the key -or- <code>null</code> if there is no entry under the key
   *     or if the entry under the key has has its reference cleared.
   */
   ...
   *     mapping for the key or if the entry under the key has has its reference cleared.
   */
   ...
   *
   * @param k The key.
   * @param newRef The {@link WeakReference} for the new value. This was generated using {@link
   *     #newWeakRef(Object, Object, ReferenceQueue)}. {@link WeakReference#get()} will always
   *     return the value inserted into the map since it is on the stack and hence can not have been
   *     cleared during this callback.
   * @param oldRef The {@link WeakReference} for the old value. This will be <code>null</code> if
   *     there was no entry under the key for the map. If the entry for a cleared reference is
   *     updated then {@link WeakReference#get()} will return null for the <i>oldRef</i>.
   */
  protected void didUpdate(
      final K k, final WeakReference<V> newRef, final WeakReference<V> oldRef) {
   ...
   * {@link IBatchedUpdateListener} which is called when the access order updates are batched for a
   * given thread. This does not eliminate the possibility of synchronization costs inside of {@link
   * ReferenceQueue}, but it should dramatically reduce those costs.
   *
   * <p>Note: This method does not clear entries from the hard reference queue because it is not
   * possible to have a weak or soft reference cleared by the JVM while a hard reference exists, so
   * it is not possible to have an entry in the hard reference queue for a reference that is being
   * cleared here.
   */
  protected void removeClearedEntries() {

    if (referenceQueue == null) return;

    int counter = 0;

    for (Reference<? extends V> ref = referenceQueue.poll();
        ref != null;
   ...
   * not cause them to be retained any longer than they otherwise would have been retained.
   */
   ...
   * @param k The key.
   * @param v The value.
   * @param referenceQueue The {@link ReferenceQueue} used to remove map entries whose {@link
   *     WeakReference}s have been cleared (optional).
   * @return An instance of a class extending {@link WeakReference} -or- an instance of a class
   *     extending {@link WeakRef} if <code>referenceQueue!=null</code>.
   */
  protected WeakReference<V> newWeakRef(
      final K k, final V v, final ReferenceQueue<V> referenceQueue) {
