...
public class TestProxySetup extends TestCase {
  ...
  protected static final Logger log = Logger.getLogger(TestProxySetup.class);
   ...
   * method is run and throws a {@link NullPointerException}if the delegate was not set on the test.
   * This is used to verify that the expected number of tests are run and that the delegate was set
   * on each nested test.
   */
  ...
  public static class MyProxyTest extends TestCase implements IProxyTest {
    ...
    /** Throws {@link NullPointerException} if the delegate is not set when the test is run. */
