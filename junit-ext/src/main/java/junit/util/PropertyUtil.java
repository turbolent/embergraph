 ...
 * a {@link Properties} object such that all inherited properties are placed within a simple {@link
 * Map}. This reduces the problem to fusing two maps, which may be done using {@link Map#putAll( Map
 * other )}.
 *
 * <p>Under some circumstances, it may be desirable to report a conflict which would otherwise be
 * silently ignored by {@link Map#putAll( Map other )}. A helper method has been provided to report
 * such conflicts rather than letting one {@link Map} override another.
 *
 * <p>
 */
public class PropertyUtil {
   ...
   * the top-level Properties object placing the result into the output Map. The order of visitation
   * of the Properties levels does not matter since the value of the key is always defined against
   * the top-level Properties object which handles any defaults correctly.
   *
   * <p>
   *
   * <pre>
   ...
   * @param override Another configuration whose values will be fused with the <i>defaults</i>. If
   *     this is a {@link Properties} object then it is first flattened so that any inherited
   *     property values will be fused.
   * @return A {@link Properties} object. If <i>defaults</i> was a {@link Properties} object, then
   *     this is <i>defaults</i> and any values from <i>override</i> have been added to
   *     <i>defaults</i>. Otherwise a new {@link Properties} object is created, the entries from
   *     <i>defaults</i> are copied into that {@link Properties} object, and any values from
   *     <i>override</i> are copied onto that {@link Properties} object.
   */
   ...
   *     this is a {@link Properties} object then it is first flattened so that any inherited
   *     property values will be fused.
   * @param ignoreConflicts When true an exception is not reported if an entry from <i>override</i>
   *     would override an entry in <i>defaults</i>.
   * @return A {@link Properties} object. If <i>defaults</i> was a {@link Properties} object, then
   *     this is <i>defaults</i> and any values from <i>override</i> have been added to
   *     <i>defaults</i>. Otherwise a new {@link Properties} object is created, the entries from
   *     <i>defaults</i> are copied into that {@link Properties} object, and any values from
  ...
  public static Properties fuse(Map defaults, Map override, boolean ignoreConflicts) {
    ...
    if (override instanceof Properties) {
      ...
      override = flatten((Properties) override);
    }
