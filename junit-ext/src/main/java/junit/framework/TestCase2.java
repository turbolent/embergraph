 ...
 * has an impact on which files are recognized as tests to be submitted to a test runner. For
 * example, it is common practice that all files beginning or ending with "Test" will be submitted
 * to JUnit, though I suggest that the practice of naming things "FooTestCase" will serve you
 * better. Also note that the maven test plugin explictly skips files whose name matches the pattern
 * "*AbstractTestSuite".
 *
 * <p>
 */
public abstract class TestCase2 extends TestCase {
  ...
  public static void assertEquals(String msg, boolean[] expected, boolean[] actual) {
    ...
    if (expected == actual) {
       ...
       * fail.
       */
      assertEquals(msg + "length differs.", expected.length, actual.length);
    }
   ...
   * compiler. If either argument is not strongly typed, you MUST case it to a byte[] or {@link
   * #assertEquals(Object, Object)} will be invoked instead.
   *
   * @param expected
   * @param actual
   */
   ...
   * primitives and objects.
   *
   * <p>TODO This will throw a {@link ClassCastException} if <i>actual</i> is of a different
   * primitive array type. Change the code to throw {@link AssertionFailedError} instead.
   */
    ...
    if (expected != null && expected.getClass().isArray()) {
      ...
      if (expected.getClass().getComponentType().isPrimitive()) {
        ...
        if (componentType.equals(Boolean.TYPE)) {
          ...
          assertEquals(msg, (boolean[]) expected, (boolean[]) actual);

        } else if (componentType.equals(Byte.TYPE)) {

          assertEquals(msg, (byte[]) expected, (byte[]) actual);

        } else if (componentType.equals(Character.TYPE)) {

          assertEquals(msg, (char[]) expected, (char[]) actual);

        } else if (componentType.equals(Short.TYPE)) {

          assertEquals(msg, (short[]) expected, (short[]) actual);

        } else if (componentType.equals(Integer.TYPE)) {

          assertEquals(msg, (int[]) expected, (int[]) actual);

        } else if (componentType.equals(Long.TYPE)) {

          assertEquals(msg, (long[]) expected, (long[]) actual);

        } else if (componentType.equals(Float.TYPE)) {

          assertEquals(msg, (float[]) expected, (float[]) actual);

        } else if (componentType.equals(Double.TYPE)) {

          assertEquals(msg, (double[]) expected, (double[]) actual);
        ...
        assertTrue(msg, java.util.Arrays.equals((Object[]) expected, (Object[]) actual));
      }
  ...
  public static void assertSameIterator(String msg, Object[] expected, Iterator actual) {
    ...
    while (actual.hasNext()) {
      ...
      if (i >= expected.length) {
        ...
        fail(msg + ": The iterator is willing to visit more than " + expected.length + " objects.");
      }
  ...
  public static void assertSameIteratorAnyOrder(final Object[] expected, final Iterator actual) {
  ...
  public static void assertSameIteratorAnyOrder(
      final String msg, final Object[] expected, final Iterator actual) {
   ...
   * values can be generated and their presentation order randomized by indexing with the returned
   * array.
   */
   ...
   *
   * @param len The maximum length of the string. Each generated literal will have a mean length of
   *     <code>len/2</code> and the lengths will be distributed using a normal distribution (bell
   *     curve).
   * @param id A unique index used to obtain a unique string. Typically this is a one up identifier.
   */
  ...
  protected class RandomType {
    ...
    /** Java Object (must implement {@link Serializable} or {@link Externalizable}). */
     ...
     * @throws UnsupportedOperationException if the class is not one of those that is supported by
     *     {@link RandomType}.
     */
     ...
     * @param cls The class of the random instance to be returned -or- <code>null</code> to return
     *     an instance of any of the classes that can be generated by this method.
     * @param allowNull When true, a null object reference may be returned.
     * @return The random object.
     */
     ...
     *
     * @param rnd The random number generator.
     * @param type One of the values declared by this class.
     * @param allowNull When true, a null object reference may be returned.
     * @return The random object.
     */
   ...
   * <p>
   *
   * @param resourceName A resource identified by a series of '/' delimited path components
   *     <strong>without</strong> a leading '/'. Such resources are normally part of the JAR in
   *     which the test is bundled and correspond to the package naming convention with the
   *     exception that the '/' delimiter is used in place of the '.' delimiter used in package
   *     names.
   * @exception AssertionError If the resource could not be read for any reason.
   * @see ClassLoader#getResourceAsStream( String resourceName )
   */
  public InputStream getTestInputStream( // Object context,
      String resourceName) {
    ...
    log.info("Getting input stream for: " + resourceName);
    ...
    try {
      ...
      if (cl == null) {
        ...
        throw new AssertionError("Could not get ClassLoader" + ": " + context.getClass());
      }
      ...
      if (is == null) {
        ...
        log.error("Could not locate resource: '" + resourceName + "'");

        throw new AssertionError("Could not locate resource: '" + resourceName + "'");
      }
      ...
      log.error("Could not read from " + resourceName, ex);
   ...
   * <p>
   *
   * @param resourceName A resource identified by a series of path components beginning
   *     <strong>without</strong> a leading '/'. Such resources are normally part of the JAR in
   *     which the test is bundled.
   * @param encoding The character set encoding that will be used to read the text resource. If
   *     <code>null</code> then the platform default Java encoding will be used (which is not
   *     recommended as it is non-portable).
   *     <p>
   * @exception AssertionError If the resource could not be read for any reason.
   * @see ClassLoader#getResourceAsStream( String resourceName )
   */
  public String getTestResource( // Object context,
      String resourceName, String encoding) {

    log.info("Reading local test resource: " + resourceName);
    ...
    try {
      ...
      try {
        ...
        if (log.isInfoEnabled())
          log.info("Read " + sb.length() + " characters from " + resourceName);
  ...
  public String getTestResource(String resourceName) {
    ...
    return getTestResource(resourceName, null);
  }
   ...
   *
   * <p>The hierarchy is constructed from the following properties files in the following order. The
   * first property file in this list corresponds to the top of the property hierarchy. The last
   * property file in this list corresponds to the bottom of the property hierarchy. The property
   * resources are:
   *
   * <ol>
   *   <li>The System properties specified using the -D argument on the command line.
   *   <li>${user.home}/build.properties. ${user.home} is the value of the Java system environment
   *       variable named "user.home" and corresponds to the user home directory. A warning is
   *       logged if this properties file does not exist. <em>Note: The JUnit UI can wind up with a
   *       different value for this property than when you execute tests without the UI.</em> It is
   *       an error if the properties file exists but can not be read.
   *   <li>{class}.properties, where {class} is the class name reported by Java reflection, is
   *       loaded iff it is found.
   *   <li>{class} is repeatedly truncated by dropping the last package name component to generate a
   *       package name. If a properties file named "test.properties" exists in that package then
   *       those properties are loaded.
   * </ol>
   *
   * @return A {@link Properties} object that supplies bindings for property names according to the
   *     described hierarchy among property resources. The returned {@link Properties} is NOT
   *     cached.
   *     <p>TODO This does not handle the recursive truncation of the class name to search for
   *     "test.properties" yet.
   */
    ...
    if (m_properties == null) {
      ...
      try {
        ...
        classNameProperties:
        {
          ...
          log.info("Will try to read properties from resource: " + resourceName);
          ...
          if (is != null) {
            ...
            log.debug("No properties resource named: " + resourceName);
          }
        } // classNameProperties
   ...
   */
  protected static class MyProperties extends Properties {
    ...
    /** Exposes the protected member in the {@link Properties} class. */
   ...
   *
   * <p>This method is invoked by {@link #getProperties()}.
   *
   * <p>
   */
  protected void logProperties(MyProperties properties, int level) {
    ...
    if (defaults != null) {
      ...
      logProperties(defaults, level + 1);
    }
  }
   ...
   *
   * <p>Note that the current working directory in a multiproject build is normally NOT the same as
   * the individual project directory when running the unit tests. This method is designed to give
   * you access to project local resources (whether consumed or created) during unit test, even when
   * running a multiproject goal.
   *
   * <p>In order for this method to succeed, you MUST declare
   * <i>maven.junit.sysproperties="maven.build.dir ..."</i> when you invoke the test suite. See the
   * properties page for the <a
   * href="http://maven.apache.org/reference/plugins/test/properties.html">test-plugin </a> for more
   * information on how to specify this property.
   *
   * <p>
   *
   * @throws UnsupportedOperationException if the project build directory could NOT be determined
   *     since the <i>maven.junit.sysproperties</i> property was NOT defined.
  ...
  public static String getProjectBuildPath() throws UnsupportedOperationException {
    ...
    if (x == null) {
      ...
      final String msg =
          "The 'maven.build.dir' property is NOT defined.  Did you forget to defined 'maven.junit.sysproperties=maven.build.dir ...'";
   ...
   * @param expected
   * @param actual
   * @param maxUlps The maximum #of ulps difference which will be considered as equivalent.
   * @see #getUlps(double, double)
   */
  public static void assertEqualsWithinUlps(
      String message, double expected, double actual, final long maxUlps) {
  ...
  public static void assertSameArray(String message, Object[] expected, Object[] actual) {
    ...
    assertEquals(message + " : length is wrong.", expected.length, actual.length);
   ...
   * @param cls The class of exception that you are looking for in the stack trace.
   * @return An exception that is an instance of that class iff one exists in the stack trace and
   ...
   * @param t The stack trace.
   * @param cls The class of exception that you are looking for in the stack trace.
   * @return <code>true</code> iff an exception that is an instance of that class iff one exists in
   *     the stack trace.
   * @throws IllegalArgumentException if any parameter is null.
   */
