...
public class TestGangliaListenerShutdown extends TestCase {
  ...
  private static final Logger log = Logger.getLogger(TestGangliaListenerShutdown.class);
   ...
   * verifies that an interrupt is noticed and that the listener terminates in a timely manner.
   */
  public void test_gangliaListener_shutdown() throws UnknownHostException, InterruptedException {
    ...
    final GangliaListener gangliaListener =
        new GangliaListener(
    ...
    try {
       ...
       * the assert and linked the test to the ticket. We should fix this by a refactor of the
       * GangliaListener to use the Java 7 support for non-blocking IO and multicast.
       *
       * @see <a href="http://trac.bigdata.com/ticket/1188">
       *     org.embergraph.ganglia.TestGangliaListenerShutdown fails due to blocking NIO. </a>
       */
      //            assertFalse(gangliaListener.isListening());
      log.error(
          "Test is internally disabled due to lack of non-blocking IO and multicast in Java 6. See #1188.");
