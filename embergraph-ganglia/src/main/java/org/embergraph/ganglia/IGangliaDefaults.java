...
public interface IGangliaDefaults {
   ...
   * the {@link GangliaService} from transmitting a different heartbeat, which would confuse <code>
   * gmond</code> and <code>gmetad</code>.
   *
   * <p>The {@link GangliaService} MUST NOT send out a heartbeat if <code>gmond</code> is also
   * running on the host since the two heartbeats will be different (they are the start time of
   * <code>gmond</code>) and <code>gmond</code> and <code>gmetad</code> will both get confused.
   */
   ...
   * maintained by this service. If the metric value has never been published (timestamp is ZERO
   * (0)), if TMax might expire before we sample the metric again, or if the value of the metric has
   * changed "sufficiently" then the metric will be sent to any configured listener(s).
   *
