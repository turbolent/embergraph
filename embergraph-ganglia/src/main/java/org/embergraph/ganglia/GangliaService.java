 ...
 * from the underlying platform (OS/host) or from the application into which this service is
 * embedded.
 *
 * <p>Applications can embed this service by adding their own {@link IGangliaMetadataFactory}
 * instances (to get nice metric declarations) and can register an {@link IGangliaMetricsCollector}
 * to periodically report performance metrics out to the ganglia network. You can also use {@link
 * #setMetric(String, Object)} to directly update a metric.
 *
 * <p>The ganglia protocol replicates soft state into all <code>gmond</code> and <code>gmetad</code>
 * instances. The {@link GangliaService} is a full participant in the protocol and will also develop
 * a snapshot of the soft state of the cluster. This protocol has a lot of benefits, but the
 * metadata declarations can stick around and be discovered long after you have shutdown the {@link
 * GangliaService}.
 *
 * <p>When developing with the embedded {@link GangliaService} it is a Good Idea to use a private
 * <code>gmond</code> / <code>gmetad</code> configuration (non-default address and/or port). Then,
 * if you wind up making some mistakes when declaring your application counters and those mistakes
 * replicated into the soft state of the ganglia services you can just shutdown your private service
 * instances.
 *
 * <p>Another trick is to use the [mock] mode (it is a constructor argument) while you are
 * developing so the {@link GangliaService} does not actually send out any packets.
 *
 * <p>Finally, there is also a utility class (in the test suite) which can be used to monitor the
 * ganglia network for packets which can not be round-tripped. This can be used to identify bugs if
 * you are doing development on embedded {@link GangliaService} code base.
 *
 * @see <a href="http://ganglia.sourceforge.net/">The ganglia monitoring system</a>
 * @see <a
 ...
 *     <p>TODO Only allows one join group and port for listening and only listens by joining a
 *     multicast group.
 *     <p>Unicast support should not be that different. The list of metricsServers is fine for
 *     sending data whether using unicast or multicast. I need to experiment with a (list of)
 *     addresses for receiving data. Note that ganglia supports more than one address and port for
 *     unicast or multicast. It should not be difficult to do the same thing under java.
 *     <p>FIXME Setup a JXM bridge for counters. Anything which make it into ganglia or perhaps
 *     anything which is declared in our counters hierarchy.
 *     <p>TODO Turn off heartbeat automatically if we detect a heartbeat with a different timestamp
 *     while this GangliaService is running. That will help us avoid stomping on gmond.
 *     <p>TODO Setup a listener for TCP connections on the same port that we listen on for UDP. If
 *     there is a connection, then respond by streaming the XML data for the current counters. The
 *     XML DTD (and sample data) can be obtained using <code>telenet localhost 8649</code>. This is
 *     the hook used for the ganglia web UI, some nagios integrations, etc.
 *     <p>FIXME Provide a sample collector for the JVM and use it in main().
 *     <p>TODO Port the more sophisticated collectors into this module?
 */
public class GangliaService implements Runnable, IGangliaMetricsReporter {
  ...
  /** When <code>true</code> the service will listen for, and respond to, ganglia messages. */
   ...
   * network.
   *
   * <p>Note: Another technique for testing is to setup a {@link GangliaListener} and a {@link
   * GangliaService} talking to one another on a non-default unicast address and port or a
   * non-default multicast join group and port.
   */
   ...
   * have not already been observed by listening to the ganglia network.
   *
   ...
   * before we sample the metric again.
   *
  ...
  /** Object which knows how to decode the ganglia wire format into an {@link IGangliaMessage}. */
   ...
   * running on the same host. The {@link GangliaService} MUST NOT send out a heartbeat if <code>
   * gmond</code> is also running on the host since the two heartbeats will be different (they are
   * the start time of <code>gmond</code>) and <code>gmond</code> and <code>gmetad</code> will both
   * get confused.
   *
   * @param serviceName The name of the service in which you are embedding ganglia support.
  ...
  public GangliaService(
    ...
    if (metricsServers == null || metricsServers.length == 0) throw new IllegalArgumentException();

    if (listenGroup == null) throw new IllegalArgumentException();

    if (listenPort <= 0) throw new IllegalArgumentException();

    if (quietPeriod < 0) throw new IllegalArgumentException();

    if (initialDelay <= 0) throw new IllegalArgumentException();

    if (heartbeatInterval < 0) throw new IllegalArgumentException();

    if (monitoringInterval <= 0) throw new IllegalArgumentException();

    if (globalDMax <= 0) throw new IllegalArgumentException();

    if (metadataFactory == null) throw new IllegalArgumentException();
   ...
   * metadata request will not cause the timestamp of the metrics to be reset.
   */
  ...
  /** Return <code>true</code> if the {@link GangliaService} is currently listening. */
  ...
  protected void sendMessage(final IGangliaMessage msg) {
    ...
    if (msg == null) log.error("No message");
    ...
    public Void call() throws Exception {
      ...
      if (gangliaSender != null) {
        ...
        if (msg.isMetricMetadata()) {
          ...
          messageEncoder.writeMetadata(tmp.xdr, (IGangliaMetadataMessage) msg);

        } else if (msg.isMetricRequest()) {

          messageEncoder.writeRequest(tmp.xdr, (IGangliaRequestMessage) msg);

        } else if (msg.isMetricValue()) {

          messageEncoder.writeMetric(
              tmp.xdr, ((RichMetricMessage) msg).getMetadata(), (IGangliaMetricMessage) msg);
   ...
   * same host is that they need to synchronize the counter values that they are issuing or it will
   * confuse other gmond and gmetad instances.
   */
  private class HeartBeatTask implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        final long value = TimeUnit.MILLISECONDS.toSeconds(serviceStartTime.get());
   ...
   *
   * <p>Note: There should be an initialDelay of maybe a minute before running this task. It does
   * not need to run all that frequently. Once per minute should be fine. It will scan all the
   * counters when it does run, so once a minute seems like a good balance.
   *
   * <p>FIXME Purge of old hosts/metrics is broken. We are not updating the timestamp when we
   * receive a metric value (because we rely on the timestamp not changing when the metric is for
   * this host to let the service send out the metrics for this host). We will need to track the
   * timestamp of received updates separately from the timestamp of sent values. The "sent"
   * timestamp will be zero unless it is a record that we are sending out for this host, but that
   * does not mean we should delete the metric since we are still receiving the metric from other
   * hosts (and potentially from gmond on this host).
   */
  ...
  /** Task is run periodically and is responsible for harvesting metrics to be reported. */
  ...
  protected void gatherMetrics() {
    ...
    final Iterator<IGangliaMetricsCollector> itr = metricCollectors.iterator();
