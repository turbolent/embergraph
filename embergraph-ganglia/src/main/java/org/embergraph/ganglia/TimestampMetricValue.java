 ...
 * last sampled value remains fluid within instances of this class. The data type will be converted
 * to the {@link IGangliaMetadataMessage#getMetricType()} when an {@link IGangliaMetricMessage} is
 * generated from the current value.
 */
public class TimestampMetricValue implements ITimestampMetricValue {
  ...
  public int getAge() {
    ...
    return (int) TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - timestamp);
  }
   ...
   * with the {@link IGangliaMetadataMessage} for the metric.
   *
   * <p>The as stored data type is not important as long as it remains either {@link String} or a
   * {@link Number}. The {@link IGangliaMessageEncoder} is responsible for encoding the value
   * according to the ganglia data type declaration in the {@link IGangliaMetadataMessage}.
   *
   * @param newValue The new value.
   * @return <code>true</code> if the value changed enough to be worth reporting or if tmax has
   *     expired so we need to report the value anyway.
   * @throws IllegalArgumentException if <i>newValue</i> is <code>null</code>.
   * @throws IllegalArgumentException if the metricType is numeric and <i>newValue</i> is not a
   *     {@link Number}.
   */
  public synchronized boolean setValue(final Object newValue) {
    ...
    if (isNumeric && !(newValue instanceof Number)) {
       ...
       */
      throw new IllegalArgumentException(
          "Not a number: " + newValue + " (" + newValue.getClass() + ")");
    }
    ...
    if (isNumeric && !(translatedValue instanceof Number)) {
       ...
       */
      throw new IllegalArgumentException(
          "Not a number: " + translatedValue + " (" + translatedValue.getClass() + ")");
    }
   ...
   *
   * <p>The highest priority is associated with the most negative value. The lowest priority is
   * associated with the most positive value. These ar the semantics used by a {@link
