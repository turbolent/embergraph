 ...
 * reconcile host names, {@link IGangliaMetadataMessage}s with those already on hand, and/or
 * reconcile {@link IGangliaMetricMessage} with the {@link IGangliaMetadataMessage}s already on
 * hand. The {@link GangliaListener} does NOT perform such operations by design so it maybe used to
 * obtain a faithful transcription of the chatter on the ganglia network.
 */
public class GangliaListener implements Callable<Void>, IGangliaDefaults {
  ...
  /** Return <code>true</code> iff the listener is running when this method is invoked. */
   ...
   * @see #DEFAULT_PORT
   */
  public GangliaListener(final IGangliaMessageHandler handler) throws UnknownHostException {
   ...
   * @param port The port on which the ganglia services are talking.
   * @param decoder Object responsible for decoding packets.
   * @param handler The interface to receive the ganglia messages.
   */
  public GangliaListener(
   ...
   *
   * <p>Note: This method BLOCKS and does NOT notice an interrupt. This is because JDK 6 does not
   * support multicast NIO.
   *
   * <p>TODO Java 6 does not support non-blocking multicast receive. Write a JDK 7 specific version
   * of class class which uses multicast with non-blocking IO and hence can be interrupted.
   *
   * @see <a href="http://blogs.oracle.com/alanb/entry/multicasting_with_nio">Multicasting with
   *     NIO.</a>
   * @see <a
  ...
  public Void call() throws Exception {
    ...
    try {
      ...
      // Start the executor.
      service =
          Executors.newSingleThreadScheduledExecutor(new DaemonThreadFactory("GangliaListener"));
      ...
      while (true) {
        ...
        try {
          ...
          final IGangliaMessage msg =
              decodeRecord(packet.getData(), packet.getOffset(), packet.getLength());
   ...
   * @param off The offset of the first byte in the packet.
   * @param len The #of bytes in the packet.
   * @return The decoded record.
   */
  protected IGangliaMessage decodeRecord(final byte[] data, final int off, final int len) {

    if (log.isTraceEnabled()) log.trace(BytesUtil.toString(data, off, len));

    return decoder.decode(data, off, len);
  }

  /** Class dispatches a message to a handler and logs anything which is thrown. */
  private static class DispatchTask implements Callable<Void> {

    private final IGangliaMessageHandler handler;
    private final IGangliaMessage msg;

    public DispatchTask(final IGangliaMessageHandler handler, final IGangliaMessage msg) {
