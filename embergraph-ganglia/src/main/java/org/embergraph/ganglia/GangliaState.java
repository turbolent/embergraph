...
public class GangliaState implements IGangliaState {
   ...
   * which were no longer in use. However, we have to be a bit careful since we need to receive and
   * store the metadata declarations before we can enter a {@link TimestampMetricValue} record into
   * the map and without the {@link TimestampMetricValue} the {@link IGangliaMetadataMessage}
   * reference could easily be only weakly reachable and get finalized. The typical way to handle
   * this is with a backing hard reference queue using either a fixed capacity or a timeout.
   */
   ...
   * metric. This is the basis for reporting metrics whose values have changed, for reporting
   * metrics whose values have not changed but for which TMax would otherwise be exceeded, and for
   * sending out metadata declarations.
   */
   ...
   */
  public GangliaState(final String hostName, final IGangliaMetadataFactory metadataFactory) {
    ...
    this.knownHosts =
        new ConcurrentHashMap<
            String /* hostName */, ConcurrentHashMap<String, TimestampMetricValue>>();
  }
  ...
  public Iterator<TimestampMetricValue> iterator(final String hostName) {
    ...
    final ConcurrentHashMap<String, TimestampMetricValue> hostMetrics = knownHosts.get(hostName);
   ...
   * @return Either <i>decl</i>, a version of the declaration which was resolved by the {@link
   *     IGangliaMetadataFactory#resolve(IGangliaMetadataMessage)}, or the pre-existing declaration
   *     for the same metric.
   */
  public IGangliaMetadataMessage putIfAbsent(final IGangliaMetadataMessage decl) {
    ...
    if (resolved == null) throw new RuntimeException("Resolution error for " + decl);

    if (!decl.getMetricName().equals(resolved.getMetricName())) {
      // Santity check. The metric name should not be changed.
      throw new RuntimeException(
          "Resolution error: decl="
              + decl
              + ", but resolved metricName="
              + resolved.getMetricName());
    }
    ...
    if ((tmp = metadata.putIfAbsent(decl.getMetricName(), resolved)) == null) {
      ...
      // Newly declared.
      if (log.isInfoEnabled()) log.info("declared: " + resolved);
   ...
   * metadata declarations are canonical.
   *
   * @param hostName The host name.
   * @return The counters for that host and never <code>null</code> (counters are registered if they
   *     do not exist).
   */
  private ConcurrentHashMap<String /* mungedMetricName */, TimestampMetricValue> getHostCounters(
    ...
    ConcurrentHashMap<String /* mungedMetricName */, TimestampMetricValue> hostCounters =
        knownHosts.get(hostName);

    if (hostCounters == null) {

      hostCounters = new ConcurrentHashMap<String /* mungedMetricName */, TimestampMetricValue>();

      final ConcurrentHashMap<String /* mungedMetricName */, TimestampMetricValue> tmp =
          knownHosts.putIfAbsent(hostName, hostCounters);
  ...
  @Override
  public TimestampMetricValue getMetric(final String hostName, final String metricName) {
    ...
    // Note: never [null].
    final ConcurrentHashMap<String /* mungedMetricName */, TimestampMetricValue> hostCounters =
        getHostCounters(hostName);

    // Thin wrapper over the decl.
    final TimestampMetricValue tmp = new TimestampMetricValue(decl);

    // Add to map unless already present.
    final TimestampMetricValue old = hostCounters.putIfAbsent(metricName, tmp);
    ...
    if (log.isDebugEnabled()) log.debug("declared: host=" + hostName + ", decl" + decl);
   ...
   *
   * <p>Note: This method should be invoked periodically to age out hosts and metrics which have not
   * reported in for a long time.
   *
   * @param dmax The global DMax value.
   */
   ...
   *
   * @param hostName The host to be checked.
   * @param dmax The global DMax.
   ...
   * counter for the host. What can happen is that the other thread gets the counters for the host
   * and then returns the TMV for the metric while this method concurrently deletes the entry for
   * the host from knownHosts. This would mean that the other thread has returned a TMV entry which
   * is not going to remain visible. If there is a problem, it will be related to what is then done
   * with that TMV entry.
   *
   * @param hostName The host name.
   */
