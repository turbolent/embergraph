...
public enum GangliaMessageTypeEnum {
   ...
   * there is an explicit {@link IGangliaMetadataMessage} declaring the metric as having this
   * datatype. {@link IGangliaMetricMessage} associated with this ganglia data type by an {@link
   * IGangliaMetadataMessage} are internally modeled using a non-negative Java {@link Integer}.
   * However, when they are sent out on the wire they are written out using the printf format string
   * for an unsigned int16 value.
   */
   ...
   * there is an explicit {@link IGangliaMetadataMessage} declaring the metric as having this
   * datatype. {@link IGangliaMetricMessage} associated with this ganglia data type by an {@link
   * IGangliaMetadataMessage} are internally modeled using a non-negative Java {@link Long}.
   * However, when they are sent out on the wire they are written out using the printf format string
   * for an unsigned int32 value.
   */
  ...
  DOUBLE(128 + 7, true, "double", "%lf", new Class[] {Double.class, Long.class}),
   ...
   * @param gtype The string name of the ganglia data type.
   * @param format The <code>printf</code> string used for that ganglia data type.
  ...
  /** Return the <code>printf</code> format string suitable for use with the ganglia data type. */
   ...
   * @return The enum.
   */
  public static GangliaMessageTypeEnum valueOf(final int v) {
    switch (v) {
      case 128:
        return METADATA;
      case 128 + 1:
   ...
   */
  public static GangliaMessageTypeEnum fromGType(final String metricType) {

    final GangliaMessageTypeEnum e = gtype2Enum.get(metricType);

    if (e == null) throw new IllegalArgumentException("metricType=" + metricType);
   ...
   * collected from the Java application and an appropriate {@link IGangliaMetadataMessage} needs to
   * be constructed based on that value object to declare the metric to the ganglia network.
   *
   * <p>This method SHOULD NOT be used to discover the enum when you already have that information.
   * In particular, do NOT use this method when you are looking at an {@link IGangliaMetricMessage}
   * associated with one of the <strong>unsigned</strong> ganglia data types. Unsigned ganglia data
   * types are represented by a wider primitive data type in java (uint16 as int and uint32 as
   * long). Various methods correctly translate between the wire format for those messages when
   * encoding and decoding. The Java data type used for such messages will NOT be correctly
   * translated back into the actual {@link GangliaMessageTypeEnum} by this method. For example, a
   * {@link Integer} is translated by this method into {@link #UINT32} rather than {@link #UINT16}
   * since this method assumes that the Java object is expressing a signed numeric value rather than
   * an unsigned numeric value). Also, this method will translate a Java {@link Long} into {@link
   * #DOUBLE} in order to convey as much information as possible to ganglia (which lacks an int64
   * data type).
   *
   * @param value The data value.
   * @return The best match {@link GangliaMessageTypeEnum}.
   * @throws IllegalArgumentException if the <i>value</i> is <code>null</code>.
   * @throws IllegalArgumentException if the <i>value</i> is not one of those registered for use
   *     with this enum.
   */
  public static GangliaMessageTypeEnum forJavaValue(final Object value) {
