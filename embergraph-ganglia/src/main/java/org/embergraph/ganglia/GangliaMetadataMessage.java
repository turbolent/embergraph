 ...
 * send_metadata_interval = 0</code> in the default <code>gmond.conf</code>).
 */
public class GangliaMetadataMessage extends AbstractGangliaMessage
  ...
  public GangliaMetadataMessage(
      ...
      final boolean spoof,
      final GangliaMessageTypeEnum metricType,
      final String metricName2,
      final String units,
    ...
    if (metricType == null) throw new IllegalArgumentException();

    if (metricName2 == null) throw new IllegalArgumentException();

    if (!metricName.equals(metricName2)) throw new IllegalArgumentException();

    if (units == null) throw new IllegalArgumentException();

    if (slope == null) throw new IllegalArgumentException();

    if (extraValues == null) throw new IllegalArgumentException();
  ...
  public boolean equals(final Object o) {
    ...
    if (!getMetricType().equals(t.getMetricType())) return false;

    if (!getMetricName2().equals(t.getMetricName2())) return false;

    if (!getUnits().equals(t.getUnits())) return false;

    if (!getSlope().equals(t.getSlope())) return false;

    if (getTMax() != t.getTMax()) return false;

    if (getDMax() != t.getDMax()) return false;
    ...
    for (Map.Entry<String, String[]> e : e1.entrySet()) {
      ...
      if (!Arrays.equals(val1, val2)) return false;
    }
   ...
   * the application MUST resolve metadata declarations received over the wire to local declarations
   * in which this method has the desired behavior. This resolution step is necessary because the
   * ganglia wire protocol is data-only and the translation steps can not be readily expressed as
   * data (unless they are turned into a full formula evaluation model, adding substantial
   * complexity to the ganglia integration).
   *
   * <p>TODO We could also add a method which return true iff two values differ sufficient for an
   * updated metric record to be sent out.
   */
  @Override
