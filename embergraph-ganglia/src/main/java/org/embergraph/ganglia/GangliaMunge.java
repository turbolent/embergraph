...
public class GangliaMunge {
   ...
   * converted to an underscore character ("_"). This gets rid of all punctuation characters and
   * whitespace in the metric name itself, but will not translate unicode characters.
   *
   ...
   *
   * <p>Note: Munging is required because we receive counters from other ganglia services and those
   * counters must obey the rules for file systems, so we have to munge everything.
   *
   * @param groupName The group name (optional).
   * @param localName The local name (required).
   * @return The fully qualified, munged name.
   */
  public static String getFQName(final String groupName, final String localName) {

    final String metricName =
        groupName == null ? munge(localName) : munge(groupName) + "." + munge(localName);
