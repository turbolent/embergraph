...
public interface IGangliaMetadataMessage extends IGangliaMessage {
   ...
   * metric_id header and once in the metadata record; both values SHOULD be the same).
   *
   * @see #getMetricName()
   */
   ...
   * metric declaration. Use cases for translation include changing the units in which a metric is
   * collected (e.g., bytes versus kilobytes) to conform with ganglia or translating the metric from
   * an offset (e.g., 1-foo).
   *
   ...
   *
   * @param oldValue The old value and never <code>null</code>.
