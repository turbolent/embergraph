...
public class AbstractSailGraphTestCase extends AbstractGraphTestCase {
  ...
  protected static class SmallGraphProblem {
    ...
    /** The data file. */
    private static final String smallGraph1 =
        "embergraph-gas/src/test/resources/org/embergraph/rdf/graph/data/smallGraph.ttl";

    private static final String smallGraph2 =
        "src/test/resources/org/embergraph/rdf/graph/data/smallGraph.ttl";
    ...
    public SmallGraphProblem(final SailGraphFixture graphFixture) throws Exception {
      ...
      foafKnows = sail.getValueFactory().createURI("http://xmlns.com/foaf/0.1/knows");

      foafPerson = sail.getValueFactory().createURI("http://xmlns.com/foaf/0.1/Person");

      mike = sail.getValueFactory().createURI("http://www.embergraph.org/Mike");

      bryan = sail.getValueFactory().createURI("http://www.embergraph.org/Bryan");

      martyn = sail.getValueFactory().createURI("http://www.embergraph.org/Martyn");

      dc = sail.getValueFactory().createURI("http://www.embergraph.org/DC");

      vertices = new LinkedHashSet<>(Arrays.asList(foafPerson, mike, bryan, martyn, dc));

      linkTypes = new LinkedHashSet<>(Arrays.asList(rdfType, foafKnows));
    }
  ...
  protected static class SSSPGraphProblem {
    ...
    /** The data file. */
    private static final String ssspGraph1 =
        "src/test/resources/org/embergraph/rdf/graph/data/ssspGraph.ttlx";

    private static final String ssspGraph2 =
        "src/test/resources/org/embergraph/rdf/graph/data/ssspGraph.ttlx";
    ...
    public SSSPGraphProblem(final SailGraphFixture graphFixture) throws Exception {
      ...
      link = sail.getValueFactory().createURI("http://www.embergraph.org/ssspGraph/link");
