...
import org.embergraph.rdf.graph.analytics.CC.VS;
  ...
  public void testCC() throws Exception {
    ...
    try {
      ...
      try {
        ...
        final IGASContext<CC.VS, CC.ES, Value> gasContext =
            gasEngine.newGASContext(graphAccessor, gasProgram);

        final IGASState<CC.VS, CC.ES, Value> gasState = gasContext.getGASState();
         ...
         */
        final Map<Value, AtomicInteger> labels = gasProgram.getConnectedComponents(gasState);
        ...
        if (false) {
           ...
           */
          final Value label = gasState.getState(p1.getDC()).getLabel();
