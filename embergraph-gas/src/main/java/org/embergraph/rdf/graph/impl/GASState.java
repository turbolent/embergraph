...
public class GASState<VS, ES, ST> implements IGASState<VS, ES, ST> {
  ...
  public GASState(
    ...
    if (frontier == null) throw new IllegalArgumentException();

    if (gasScheduler == null) throw new IllegalArgumentException();
  ...
  public void reset() {
    ...
    if (edgeState != null) edgeState.clear();

    frontier.resetFrontier(
        0 /* minCapacity */, false /* sortFrontier */, GASUtil.EMPTY_VERTICES_ITERATOR);
  }
  ...
  public void setFrontier(final IGASContext<VS, ES, ST> ctx, final Value... vertices) {
    ...
    // Reset the frontier.
    frontier.resetFrontier(
        tmp.size() /* minCapacity */, tmp.size() > 1 && sortFrontier, tmp.iterator());
  }
   ...
   * maybe a fork/join pool would work better?
   *
   * <p>Note: We can not do a parallel reduction right now because the backing class does not expose
   * a parallel iterator, e.g., a segment-wise iterator. The reduction over the {@link #vertexState}
   * is quite slow as a result.
   *
   * <p>It looks like bulk parallel operators will be eventually introduced into the Java
   * concurrency collections. For now, it seems like the short term solution would be to drop them
   * onto stripped lists at the same time that they are first inserted into the CHM. I could then
   * read over those striped lists in parallel during the reduction.
   *
   * <p>The IReducer should run with parallel threads. This is a huge serial bottleneck right now.
   * Fixing this will require a data structure with a parallel iterator, not the CHM. See
   * http://stackoverflow.com/questions/20164690
   * /using-scalas-parhashmap-in-javas-project-instead-of-concurrenthashmap
   *
   * <p>It seems like the short term solution would be to drop them onto striped lists at the same
   * time that they are first inserted into the CHM. I could then read over those striped lists in
   * parallel during the reduction.
   */
  @Override
   ...
   * SCATTER implementations to impose a canonical mapping over the vertex objects for the edges
   * that are exposed to the {@link IGASProgram} during a single round.
   */
  @Override
