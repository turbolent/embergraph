 ...
 *     <p>FIXME Add a reducer to report the actual minimum length paths. This is similar to a BFS
 *     tree, but the path lengths are not integer values so we need a different data structure to
 *     collect them (we need to store the predecesor when we run SSSP to do this).
 */
public class SSSP extends BaseGASProgram<SSSP.VS, SSSP.ES, Integer /* dist */>
   ...
   * the link attributes materialized with the {@link Statement} for the link. That could be done
   * using a read-ahead filter on the striterator if the link weights are always clustered with the
   * ground triple. See {@link #decodeStatement(Value)}.
   *
   * <p>When we make this change, the distance should be of the same type as the link weight or
   * generalized as <code>double</code>.
   *
   * <p>Maybe add a factory method or alternative constructor for the version of SSSP that uses link
   * weights? All we need to do is filter out anything that is not a link weight. In addition, it
   * will often be true that there is a single link attribute type that is of interest, so the
   * caller should also be able to specify that.
   */
  private static final double EDGE_LENGTH = 1.0f;
  ...
  public static class VS {
     ...
     * side-effect.
     *
     * <p>FIXME This really needs to be a floating point value, probably double. We also need tests
     * with non-integer weights and non- positive weights.
     */
     ...
     */
    private synchronized boolean scatter(final Value predecessor, final double newDist) {
      /*
  ...
  private static final Factory<Value, SSSP.VS> vertexStateFactory =
      new Factory<Value, SSSP.VS>() {
  ...
  public Integer gather(
      final IGASState<SSSP.VS, SSSP.ES, Integer> state, final Value u, final Statement e) {
  ...
  @Override
  public Integer sum(
      final IGASState<SSSP.VS, SSSP.ES, Integer> state, final Integer left, final Integer right) {
    throw new UnsupportedOperationException();
  ...
  //    * {@inheritDoc}
  @Override
  public SSSP.VS apply(
      final IGASState<SSSP.VS, SSSP.ES, Integer> state, final Value u, final Integer sum) {
   ...
   * <dl>
   *   <dt>1
   *   <dd>The shortest distance from the initial frontier to the vertex.
   * </dl>
   */
  @Override
  public List<IBinder<SSSP.VS, SSSP.ES, Integer>> getBinderList() {

    final List<IBinder<SSSP.VS, SSSP.ES, Integer>> tmp = super.getBinderList();

    tmp.add(
        new BinderBase<SSSP.VS, SSSP.ES, Integer>() {
  ...
  public void prunePaths(IGASContext<VS, ES, Integer> ctx, Value[] targetVertices) {
    ...
    if (targetVertices == null) throw new IllegalArgumentException();
