...
*/
package org.embergraph.rdf.graph.impl.frontier;
...
public class StaticFrontier2 implements IStaticFrontier {
   ...
   * overlap.
   *
   * @param off The offset at which to copy the slice.
  ...
  @Override
  public void resetFrontier(final int minCapacity, final boolean sort, final Iterator<Value> itr) {
   ...
   *
   * @param minCapacity The minimum required capacity for the backing array.
   * @param itr The source from which we will repopulate the backing array.
   */
  private void copyScheduleIntoFrontier(final int minCapacity, final Iterator<Value> itr) {
