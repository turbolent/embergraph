...
public class SAILGASEngine extends GASEngine {
   ...
   * uses disk-based indices since the on-disk indices typically use a different order, e.g., over
   * <code>int</code> identifiers.
   */
  @Override
  ...
  public static class SAILGraphAccessor implements IGraphAccessor {
     ...
     *     explicit triples.
     * @param defaultContext The default context(s) for the operation (optional).
     */
    public SAILGraphAccessor(
        final SailConnection cxn, final boolean includeInferred, final Resource... defaultContext) {
    ...
    public Iterator<Statement> getEdges(
        final IGASContext<?, ?, ?> p, final Value u, final EdgesEnum edges) {
    ...
    private IStriterator getEdges(
       ...
       */
      final boolean posOptimization = linkTypeIV != null && inEdges;
      ...
      if (posOptimization) {
        ...
        citr = cxn.getStatements(s, null /* p */, o, includeInferred, defaultContext);
      }

      final IStriterator sitr =
          new Striterator(new Sesame2EmbergraphIterator<Statement, SailException>(citr));
    ...
    public VertexDistribution getDistribution(final Random r) {
      ...
      try {
        ...
        final CloseableIteration<? extends Statement, SailException> citr =
            cxn.getStatements(
                null /* s */, null /* p */, null /* o */, includeInferred, defaultContext);
