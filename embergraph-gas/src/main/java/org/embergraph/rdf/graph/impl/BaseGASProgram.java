...
public abstract class BaseGASProgram<VS, ES, ST> implements IGASProgram<VS, ES, ST> {
   ...
   * #getScatterEdges()} if {@link #getGatherEdges()} returns {@value EdgesEnum#NoEdges}.
   *
   * <p>TODO This ignores {@link IGASContext#isDirectedTraversal()}
   */
  @Override
   ...
   * FrontierEnum#AllVertices} {@link IGASProgram}.
   */
  @Override
   ...
   *     <p>TODO This has a random number generator whose initial seed is not controlled by the
   *     caller. However, the desired use case here is to produce a distribution over ALL vertices
   *     so the random number should be ignored - perhaps we should pass it in as <code>null</code>?
   */
  private void addAllVerticesToFrontier(final IGASContext<VS, ES, ST> ctx) {
    ...
    final VertexDistribution dist = ctx.getGraphAccessor().getDistribution(new Random());

    final Resource[] initialFrontier = dist.getAll();

    if (ctx.getLinkType() != null) {
      if (log.isDebugEnabled()) log.debug("linkType=" + ctx.getLinkType());
      ...
      if (log.isDebugEnabled()) log.debug("initialFrontier=" + Arrays.toString(initialFrontier));
  ...
  private void addSampledVerticesToFrontier(final IGASContext<VS, ES, ST> ctx) {
    ...
    final Resource[] initialFrontier = dist.getUnweightedSample(dist.size(), sampleEdges);
  ...
  public void initVertex(
      final IGASContext<VS, ES, ST> ctx, final IGASState<VS, ES, ST> state, final Value u) {
   ...
   * not the computation state of this vertex has changed.
   */
  @Override
   ...
   *
   * <dl>
   *   <dt>{@value Bindings#VISITED}
   *   <dd>The visited vertex itself.
   * </dl>
   */
  @Override
  public List<IBinder<VS, ES, ST>> getBinderList() {
    ...
    tmp.add(
        new BinderBase<VS, ES, ST>() {
          ...
          @Override
          public Value bind(
              final ValueFactory vf, final IGASState<VS, ES, ST> state, final Value u) {
