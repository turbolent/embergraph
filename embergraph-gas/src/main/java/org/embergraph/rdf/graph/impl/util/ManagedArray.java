 ...
 *
 * <p>This class is NOT thread-safe for mutation. The operation which replaces the {@link #array()}
 * when the capacity of the backing buffer must be extended is not atomic.
 *
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: ByteArrayBuffer.java 6279 2012-04-12 15:27:30Z thompsonbry $
 *     <p>TODO Refactor to create a test suite for this. Especially the slice methods since the
 *     slice relies on a nested inner class for its semantics.
 */
public class ManagedArray<T> implements IManagedArray<T> {
   ...
   */
  public ManagedArray(final Class<? extends T> elementClass, final int initialCapacity) {
   ...
   * point the caller's reference will be replaced by a larger array having the same data.
   *
  ...
  private int extend(final int required) {
    ...
    if (log.isDebugEnabled()) log.debug("Extending buffer to capacity=" + capacity + " bytes.");
  ...
  @Override
  public final void put(final int pos, final T[] b) {

    put(pos, b, 0, b.length);
  }

  @Override
  public final void put(final int pos, final T[] b, final int off, final int len) {
  ...
  @Override
  public final void get(final int srcoff, final T[] dst) {

    get(srcoff, dst, 0 /* dstoff */, dst.length);
  }

  @Override
  public final void get(final int srcoff, final T[] dst, final int dstoff, final int dstlen) {

    System.arraycopy(buf, srcoff, dst, dstoff, dstlen);
  }

  @Override
  public final void put(final int pos, final T v) {
  ...
  @Override
  public final T get(final int pos) {
  ...
  private class SliceImpl implements IArraySlice<T> {
    ...
    @Override
    public final void put(final int dstoff, final T[] src, final int srcoff, final int srclen) {
