 ...
 *     <p>TODO Do we need a different driver if the algorithm always visits all vertices? For such
 *     algorithms, we just run them once per graph (unless the graph is dynamic).
 */
public abstract class GASRunnerBase<VS, ES, ST> implements Callable<IGASStats> {
  ...
  protected class OptionData {
    ...
    public static final int DEFAULT_NTHREADS = 4; // TODO #of hardware threads?
   ...
   *     <dl>
   *       <dt>propertyFile
   *       <dd>The implementation specific property file or other type of configuration file.
   *     </dl>
   *     and <i>options</i> are any of:
   *     <dl>
   *       <dt>-nthreads
   *       <dd>The #of threads which will be used for GATHER and SCATTER operations (default {@value
   *           OptionData#DEFAULT_NTHREADS}).
   *       <dt>-nruns
   *       <dd>The #of times that the algorithm will be run. For algorithms that are initialized
   *           with a single starting vertex drawn from a random sample, total number of runs is
   *           <code>nruns * nsamples</code>. For algorithms that are initialize with either all
   *           vertices, there will be a total of nruns runs. (default {@value
   *           OptionData#DEFAULT_NRUNS})
   *       <dt>-nsamples
   *       <dd>For algorithms that use a single starting vertex (such as BFS, SSSP, etc.), this is
   *           the #of starting vertices that will be randomly selected. The sampled vertices will
   *           have at least one out-edge or in-edge as appropriate based on the {@link
   *           IGASProgram}. For algorithm will be run ONCE for EACH sampled vertex. This parameter
   *           is ignored for algorithms that initialize the frontier with all vertices (PR, CC,
   *           etc). (default {@value OptionData#DEFAULT_NSAMPLES})
   *       <dt>-seed
   *       <dd>The seed for the random number generator (default {@value OptionData#DEFAULT_SEED}).
   *       <dt>-schedulerClass
   *       <dd>Override the default {@link IGASScheduler}. Class must implement {@link
   *           IGASSchedulerImpl}.
   *       <dt>-load
   *       <dd>Loads the named resource IFF the KB is empty (or does not exist) at the time this
   *           utility is executed. This option may appear multiple times. The resources will be
   *           searched for as URLs, on the CLASSPATH, and in the file system.
  ...
  public GASRunnerBase(final String[] args) throws ClassNotFoundException {
    ...
    while (i.get() < args.length) {
      ...
      if (arg.startsWith("-")) {
        if (arg.equals("-seed")) {
          ...
          if (opt.nruns <= 0) {
            opt.usage(1 /* status */, "-nruns must be positive, not: " + s);
          }
        } else if (arg.equals("-nsamples")) {
          final String s = args[i.incrementAndGet()];
          opt.nsamples = Integer.valueOf(s);
          if (opt.nsamples <= 0) {
            opt.usage(1 /* status */, "-nsamples must be positive, not: " + s);
          }
        } else if (arg.equals("-nthreads")) {
          final String s = args[i.incrementAndGet()];
          opt.nthreads = Integer.valueOf(s);
          if (opt.nthreads < 0) {
            opt.usage(1 /* status */, "-nthreads must be non-negative, not: " + s);
          }
    ...
    if (nremaining != 2) {
       ...
       */
      opt.usage(1 /* status */, nremaining < 1 ? "Too few arguments." : "Too many arguments");
    }
     ...
     */
    {
      ...
      opt.analyticClass = (Class<IGASProgram<VS, ES, ST>>) Class.forName(s);
    }
   ...
   * destroyed, or using an exiting journal and optionally loading in some data set. When we load
   * the data the journal is destroyed afterwards and when the journal is pre-existing and we
   * neither load the data nor destroy the journal. This has to do with the effective BufferMode (if
   * transient) and whether the file is specified and whether a temporary file is created
   * (CREATE_TEMP_FILE). If we do our own file create if the effective buffer mode is non-transient,
   * then we can get all this information.
   */
  @Override
  ...
  protected final IGASStats runAnalytic() throws Exception {
    ...
    try {
      ...
      if (opt.schedulerClassOverride != null) {
        ...
        ((GASEngine) gasEngine).setSchedulerClass(opt.schedulerClassOverride);
      }
      ...
      final IGASContext<VS, ES, ST> gasContext = gasEngine.newGASContext(graphAccessor, gasProgram);

      final IGASState<VS, ES, ST> gasState = gasContext.getGASState();

      final FrontierEnum frontierEnum = gasProgram.getInitialFrontierEnum();
       ...
       */
      final VertexDistribution dist =
          frontierEnum.equals(FrontierEnum.SingleVertex)
              ? graphAccessor.getDistribution(opt.r)
              : null;
      ...
      final Value[] sampled;
      {
        switch (frontierEnum) {
          case SingleVertex:
            sampled = dist.getWeightedSample(opt.nsamples, gasProgram.getSampleEdgesFilter());
        ...
        if (frontierEnum == FrontierEnum.AllVertices) {
          ...
          for (int i = 0; i < sampled.length; i++) {
            ...
            if (stats.getFrontierSize() == 1) {
              ...
              if (log.isInfoEnabled())
                log.info(
                    "Ignoring unconnected startingVertex: " + startingVertex + ", stats=" + stats);
              nunconnected++;
              continue;
            }
