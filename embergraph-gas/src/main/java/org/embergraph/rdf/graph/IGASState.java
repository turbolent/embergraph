 ...
 *
 * <p>A concrete implementation of this interface for a cluster WILL ONLY provide O(1) access to
 * vertices whose state has not been materialized on a given node by a GATHER or SCATTER. State for
 * vertices that were not materialized will not be accessible.
 *
...
public interface IGASState<VS, ES, ST> {
   ...
   * @param v One or more vertices that will be included in the initial frontier.
   * @throws IllegalArgumentException if no vertices are specified.
   */
   ...
   * counter.
   *
   * <p>Note: The graph is NOT part of the computation and is not discared by this method.
   */
  void reset();
   ...
   *
   * @param v The vertex.
   * @return <code>true</code> iff there is vertex state associated with that vertex.
   */
   ...
   * @return <code>true</code> iff there is vertex state associated with all specified vertices.
   */
   ...
   * the edge. The materialization step is a random access, which is why this method is for debug
   * only. Efficient, vectored mechanisms exist to materialize RDF {@link Value}s for other
   * purposes, e.g., when exporting a set of edges as as graph in a standard interchange syntax.
   *
   * @param e The edge.
   ...
   * @return <code>true</code> iff that {@link Statement} is an edge of the graph.
   */
   ...
   * #decodeStatement(Value)}.
   *
   * @param e The statement.
   * @param linkAttribType The type for the link attribute.
   * @return <code>true</code> iff the statement is an instance of a link attribute for the
   *     specified link type.
   */
   ...
   * and then asserting a property value about that "link vertex". For embergraph, this is handled
   * efficiently as inline statements about statements. This approach subsumes the property graph
   * model (property graphs do not permit recursive nesting of these relationships) and is 100%
   * consistent with RDF reification, except that the link attributes are modeled efficiently inline
   * with the links. This is what we call <a
   * href="http://www.embergraph.org/whitepapers/reifSPARQL.pdf" > Reification Done Right </a>.
   *
   * @param v The vertex.
   * @return The edge decoded from that vertex and <code>null</code> iff the vertex is not an edge.
   * @see <a href="http://www.embergraph.org/whitepapers/reifSPARQL.pdf" > Reification Done Right
   *     </a>
   ...
   * the life-cycle of the GAS program evaluation.
   *
   * @param u A vertex.
   ...
   * @return The internal form and <code>null</code> if the argument is <code>null</code>.
   */
