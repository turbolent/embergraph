 ...
 *     <p>TODO Add option to order the vertices to provide a serializable execution plan (like
 *     GraphChi). I believe that this reduces to computing a DAG over the frontier before executing
 *     the GATHER and then executing the frontier such that the parallel execution is constrained by
 *     arcs in the DAG that do not have mutual dependencies. This would have to place a partial
 *     ordering over the vertices in the frontier and then process the frontier with limited
...
public interface IGASContext<VS, ES, ST> extends Callable<IGASStats> {
   ...
   * for the GATHER and SCATTER phases. The default is {@link TraversalDirectionEnum#Forward}.
   *
   ...
   *
   * @param newValue The maximum number of iterations.
   * @throws IllegalArgumentException if the new value is non-positive.
   */
   ...
   * value is exceeded.
   *
   * @param newValue The maximum number of vertices in the frontier.
   * @throws IllegalArgumentException if the new value is non-positive.
   */
   ...
   *
   * @return The {@link Value} for the predicate that identifies the desired link type (there can be
   *     many types of links - the return value specifies which attribute is of interest).
   */
  URI getLinkType();
   ...
   *
   * @param linkType The link type to visit (optional). When <code>null</code>, all link types are
   *     visited.
   */
   ...
   *
   * @return The {@link Value} for the predicate that identifies the desired link attribute type.
   * @see #setLinkType(URI)
   */
   ...
   *
   * @param linkAttributeType The link attribute type to visit (optional). When <code>null</code>,
   *     the link attributes for the visited links are NOT visited (the topology of the graph is
   *     visited, but not the attributes for the edges).
   */
   ...
   *
   * @param afterOp The {@link IReducer}.
   */
   ...
   *
   * @param newValue The maximum number of iterations past the target vertices.
   */
