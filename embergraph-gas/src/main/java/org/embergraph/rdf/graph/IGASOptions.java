...
public interface IGASOptions<VS, ES, ST> {
   ...
   * out-edge. To sample all vertices regardless of their edges, specify {@value EdgesEnum#NoEdges}.
   * To require that each vertex has at least one in-edge and one out-edge, specify {@link
   * EdgesEnum#AllEdges}.
   *
   * <p>FIXME This should be moved into {@link GASRunnerBase}. The only class that customizes this
   * is {@link CC}. (For {@link CC} we need to put all vertices into the frontier, even those
   * without edges.)
   */
   ...
   *
   * <p>TODO We may need to set dynamically when visting the vertex in the frontier rather than
   * having it be a one-time property of the vertex program.
   */
   ...
   * algorithm does not use vertex state, then the factory should return a singleton instance each
   * time it is invoked.
   */
