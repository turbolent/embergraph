...
public class RAMGASEngine extends GASEngine {
   ...
   * without any object encode/decode overhead that could be used to explore the possible
   * performance of GAS algorithms under Java.
   *
  ...
  public static class RAMGraph {
     ...
     * concurrently modify the graph during graph traversal operations.
     */
    private static class Vertex {
      ...
      public Iterator<Statement> inEdges() {
        if (inEdges == null) return EmptyIterator.DEFAULT;
        return inEdges.iterator();
      }

      public Iterator<Statement> outEdges() {
        if (outEdges == null) return EmptyIterator.DEFAULT;
        return outEdges.iterator();
      }

      public Iterator<Statement> attribs() {
        if (attribs == null) return EmptyIterator.DEFAULT;
     ...
     * @param create when <code>true</code> the {@link Vertex} will be created if it does not exist.
    ...
    private Vertex get(final Value x, final boolean create) {
      ...
      if (v == null && create) {
        ...
        final Vertex oldVal = vertices.putIfAbsent(x, v = new Vertex(x));
    ...
    public Iterator<Statement> inEdges(final Value v) {
      ...
      if (x == null) return EmptyIterator.DEFAULT;
      return x.inEdges();
    }
  ...
  public static class RAMGraphAccessor implements IGraphAccessor {
    ...
    public VertexDistribution getDistribution(final Random r) {
       ...
       * FIXME This is not ignoring self-loops. Realistically, we
          ...
          if (vertex.getInEdgeCount() > 0) sample.addInEdgeSample((Resource) v);

          if (vertex.getOutEdgeCount() > 0) sample.addOutEdgeSample((Resource) v);
        }
      }
