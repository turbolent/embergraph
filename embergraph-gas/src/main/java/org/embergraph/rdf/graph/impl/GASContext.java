...
public class GASContext<VS, ES, ST> implements IGASContext<VS, ES, ST> {
   ...
   * @param gasProgram The program to execute against that graph.
   */
  public GASContext(
      final GASEngine gasEngine,
    ...
    if (gasState == null) throw new IllegalArgumentException();

    if (gasProgram == null) throw new IllegalArgumentException();
  ...
  public IGASStats call() throws Exception {
    ...
    while (!gasState.frontier().isEmpty()) {
      ...
      if (targetVertices.size() > 0 && getMaxIterationsAfterTargets() < Integer.MAX_VALUE) {
       ...
       * If we've reached all target vertices then halt the
          ...
          synchronized (this.maxIterations) {
            this.maxIterations.set(
                Math.min(
                    getMaxIterations(), (int) total.getNRounds() + getMaxIterationsAfterTargets()));
          }

          if (log.isTraceEnabled()) {
            log.trace(
                "All targets reached at round "
                    + total.getNRounds()
                    + ", halting at round "
      ...
      if (total.getNRounds() + 1 > getMaxIterations()) {
        ...
        log.warn(
            "Halting: maxIterations=" + getMaxIterations() + ", #rounds=" + total.getNRounds());
    ...
    if (log.isInfoEnabled()) log.info("Done: " + total);
   ...
   * accelerate convergence (unlike an asynchronous neural network).
   *
   * <p>TODO There should be an option for property value access during the APPLY (either no
   * property values are required, or some (or all) are required and must optionally be
   * materialized. Likewise, there could be an option to force the materialization of the URIs for
   * the (s,p,o).
   *
   * <p>Property value access is on the SPO index. If we are doing a reverse gather (out-edges) then
   * it will be right there and the Apply should be pushed into the Gather. If we are doing a
   * forward gather (in-edges), then we are reading on OSP and will need to do a separate read on
   * SPO.
   */
  @Override
  public boolean doRound(final IGASStats stats)
     ...
     */
    final EdgesEnum gatherEdges = getTraversalDirection().asTraversed(program.getGatherEdges());
    final EdgesEnum scatterEdges = getTraversalDirection().asTraversed(program.getScatterEdges());
    ...
    if (gatherEdges == EdgesEnum.NoEdges) {
      ...
      gatherEdgeCount = gatherEdges(graphAccessor, f, gatherEdges, pushDownApplyInGather);
    }
    ...
    if (scatterEdges == EdgesEnum.NoEdges) {
      ...
      scatterEdgeCount =
          scatterEdges(
              graphAccessor, f, gasState.getScheduler(), scatterEdges, pushDownApplyInScatter);
    }
    ...
    if (log.isInfoEnabled()) {
      ...
      log.info(
          "\ntotal"
              ...
              + (runApplyStage ? ", apply=" + TimeUnit.NANOSECONDS.toMillis(elapsedApply) : "")
              + "\nscatter"
              + ": ms="
   ...
   * @param op The {@link IReducer}.
   * @return The {@link IReducer#get() result}.
   * @throws Exception
   */
  public <T> T reduceOverFrontier(final IStaticFrontier f, final IReducer<VS, ES, ST, T> op)
      throws Exception {
   ...
   * the scope of the scatter for each vertex in the frontier.
   *
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private abstract class VertexEdgesTask implements Callable<Long> {
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private abstract class ScatterTask extends VertexEdgesTask {
    ...
    public Long call() throws Exception {
      ...
      final Iterator<Statement> eitr = graphAccessor.getEdges(GASContext.this, u, getEdgesEnum());
   ...
   * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
   */
  private abstract class GatherTask extends VertexEdgesTask {
    ...
    public Long call() throws Exception {
       ...
       * Note: since (left,right) may be null, we need to known if
        ...
        while (eitr.hasNext()) {
          ...
          log.trace("u=" + u + ", e=" + gasState.toString(e) + ", sum=" + left);
...
//     * Filter visits only edges (filters out attribute values).
   ...
   * attribute type.
   *
   * <p>Note: For embergraph, the visited edges can be decoded to recover the original link as well.
   *
   * @see IGASState#isLinkAttrib(Statement, URI)
   * @see IGASState#decodeStatement(Value)
   */
  protected IFilter getLinkAttribFilter(
      final IGASContext<VS, ES, ST> ctx, final URI linkAttribType) {
  ...
  private class LinkAttribFilter extends Filter {
    ...
    public LinkAttribFilter(final IGASContext<VS, ES, ST> ctx, final URI linkAttribType) {
      if (linkAttribType == null) throw new IllegalArgumentException();
      this.gasState = ctx.getGASState();
