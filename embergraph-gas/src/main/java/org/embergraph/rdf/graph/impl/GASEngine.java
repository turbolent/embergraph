...
public abstract class GASEngine implements IGASEngine {
  ...
  /** The {@link ExecutorService} used to parallelize tasks (iff {@link #nthreads} GT ONE). */
   ...
   * Those that execute in parallel do so using the thread pool associated with the {@link
   * IGASEngine}.
   *
   * @param taskFactory The task to be mapped across the frontier.
  ...
  protected Callable<Long> newFrontierStrategy(
    ...
    if (nthreads == 1) return new RunInCallersThreadFrontierStrategy(taskFactory, f);
  ...
  private class ParallelFrontierStrategy extends AbstractFrontierStrategy {
     ...
     * do not need to use a thread-safe collection. We also do not care about order for this
     * collection. It is only there to filter out duplicate work.
     */
    private final HashSet<Value> scheduled;

    ParallelFrontierStrategy(final VertexTaskFactory<Long> taskFactory, final IStaticFrontier f) {
   ...
   * 1).
   *
   * @throws UnsupportedOperationException if nthreads==1.
   */
