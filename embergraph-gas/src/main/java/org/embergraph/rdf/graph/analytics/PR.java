 ...
 * problematic due to the manner in which the results of the floating point operations depend on the
 * sequence of those operations (which is why this implementation uses <code>double</code>
 * precision). Thus, page rank is typically executed a specific number of iterations, e.g., 50 or
 * 100. If convergence is based on epsilon, then it is possible that the computation will never
 * converge, especially for smaller values of epsilon.
 *
 * <dl>
 *   <dt>init
 *   <dd>All vertices are inserted into the initial frontier.
 *   <dt>Gather
 *   <dd>sum( neighbor_value / neighbor_num_out_edges ) over the in-edges of the graph.
 *   <dt>Apply
 *   <dd>value = <i>resetProb</i> + (1.0 - <i>resetProb</i>) * gatherSum
 *   <dt>Scatter
 *   <dd>if (a) value has significantly changed <code>(fabs(old-new) GT
 * <i>epsilon</i>)</code>; or (b) iterations LT limit
 * </dl>
 *
 * <ul>
 *   <li>where <i>resetProb</i> is a value that determines a random reset probability and defaults
 *       to {@value PR#DEFAULT_RESET_PROB}.
 *   <li>where <i>epsilon</i> controls the degree of convergence before the algorithm terminates and
 *       defaults to {@value PR#DEFAULT_EPSILON}.
 * </ul>
 *
 * FIXME PR UNIT TEST. Verify computed values (within variance) and max iterations. Ground truth
 * from GL?
 *
 * <p>FIXME PR: The out-edges can be taken directly from the vertex distribution. That will reduce
 * the initialization overhead for PR.
 *
...
public class PR extends BaseGASProgram<PR.VS, PR.ES, Double> {
  ...
  public static class VS {
     ...
     * iteration to the new estimated value by apply().
     */
    private double value;

    /** The number of out-edges. This is computed once, when the vertex state is initialized. */
  ...
  private static final Factory<Value, PR.VS> vertexStateFactory =
      new Factory<Value, PR.VS>() {
   ...
   *
   * <p>FIXME We need to do this efficiently. E.g., using a scan to find all of the vertices
   * together with their in-degree or out-degree. That should be done to populate the frontier,
   * initializing the #of out-edges at the same time.
   */
  @Override
  public void initVertex(
      final IGASContext<PR.VS, PR.ES, Double> ctx,
      final IGASState<PR.VS, PR.ES, Double> state,
      final Value u) {
  ...
  public Double gather(
      final IGASState<PR.VS, PR.ES, Double> state, final Value u, final Statement e) {
  ...
  @Override
  public Double sum(
      final IGASState<PR.VS, PR.ES, Double> state, final Double left, final Double right) {
   ...
   */
  @Override
  public PR.VS apply(final IGASState<PR.VS, PR.ES, Double> state, final Value u, final Double sum) {
   ...
   */
  @Override
  public boolean isChanged(final IGASState<VS, ES, Double> state, final Value u) {
   ...
   *
   * <dl>
   *   <dt>{@value Bindings#RANK}
   *   <dd>The page rank associated with the vertex..
   * </dl>
   */
  @Override
  public List<IBinder<PR.VS, PR.ES, Double>> getBinderList() {
    ...
    tmp.add(
        new BinderBase<PR.VS, PR.ES, Double>() {
          ...
          @Override
          public Value bind(
              final ValueFactory vf, final IGASState<PR.VS, PR.ES, Double> state, final Value u) {
  ...
  public class PageRankReducer implements IReducer<PR.VS, PR.ES, Double, Map<Value, Double>> {
    ...
    public void visit(final IGASState<VS, ES, Double> state, final Value u) {
      ...
      if (us != null) {
        ...
        if (Double.isNaN(pageRank)) return;

        // FIXME Do infinite values show up?
        if (Double.isInfinite(pageRank)) return;
