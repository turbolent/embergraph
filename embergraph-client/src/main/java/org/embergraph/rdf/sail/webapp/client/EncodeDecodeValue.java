...
public class EncodeDecodeValue {
   ...
   *
   * @param s The value to be decoded.
   * @return The URI or literal -or- <code>null</code> if the argument was <code>null</code>.
   * @throws IllegalArgumentException if the request parameter could not be decoded as an RDF {@link
   *     Value}.
   */
  public static Value decodeValue(final String s) {
    ...
    if (slen == 0) throw new IllegalArgumentException("<Empty String>");
    ...
    if (ch == '\"' || ch == '\'') {
      ...
      if (closeQuotePos == 0) throw new IllegalArgumentException(s);
      ...
      if (ch2 == '@') {
        ...
        if (s.charAt(closeQuotePos + 2) != '^') throw new IllegalArgumentException(s);
      ...
      if (s.charAt(slen - 1) != '>') throw new IllegalArgumentException(s);
   ...
   *
   * @param param The encoded value.
   * @return The Resource -or- <code>null</code> if the argument was <code>null</code>.
   */
  public static Resource decodeResource(final String param) {

    final Value v = decodeValue(param);

    if (v == null || v instanceof Resource) return (Resource) v;
   ...
   *
   * @param param The encoded value.
   * @return The URI -or- <code>null</code> if the argument was <code>null</code>.
   */
  public static URI decodeURI(final String param) {

    final Value v = decodeValue(param);

    if (v == null || v instanceof URI) return (URI) v;
  ...
  public static String encodeValue(final Value v) {
    ...
    if (v instanceof BNode) throw new IllegalArgumentException();
    if (v instanceof URI) {
   ...
   *
   * @param strings An array of encoded named graph contexts (required).
   ...
   *
   * <p>Note: When generating URL parameters, we will have zero or more <code>&amp;c=...</code>
   * instances. These are turned into a Resource[]. If there are no such arguments, then the array
   * needs to be turned into Resource[0] on receipt by the webapp in order to be correctly decoded
   * by {@link #decodeContexts(String[])}.
   *
   * @param values An array of RDF {@link Value}s.
   * @return An array of encoded RDF values that is 1:1 with the original array.
   * @throws IllegalArgumentException if the argument is <code>null</code>.
   * @see org.embergraph.rdf.store.BD;#NULL_GRAPH
   * @see <a href="http://trac.bigdata.com/ticket/1177">Resource... contexts not encoded/decoded
   *     according to openrdf semantics (REST API) </a>
