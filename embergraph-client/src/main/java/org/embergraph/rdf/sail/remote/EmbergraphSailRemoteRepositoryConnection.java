 ...
 *     API</a>
 * @see <a href="http://trac.bigdata.com/ticket/698">EmbergraphSailRemoteRepositoryConnection should
 *     implement interface methods </a>
 *     <p>FIXME (***) #698 Fix all the Query objects (TupleQuery, GraphQuery, BooleanQuery) to
 *     support the various possible operations on them, such as setting a binding.
 */
public class EmbergraphSailRemoteRepositoryConnection implements RepositoryConnection {

  private static final transient Logger log =
      Logger.getLogger(EmbergraphSailRemoteRepositoryConnection.class);

  private final EmbergraphSailRemoteRepository repo;

  public EmbergraphSailRemoteRepositoryConnection(final EmbergraphSailRemoteRepository repo) {
  ...
  public long count(final Resource s, final URI p, final Value o, final Resource... c)
  ...
  public BooleanQuery prepareBooleanQuery(
    ...
    try {
      ...
      return new EmbergraphRemoteBooleanQuery(repo.getRemoteRepository(), query, baseURI);
  ...
  public GraphQuery prepareGraphQuery(
    ...
    try {
      ...
      return new EmbergraphRemoteGraphQuery(repo.getRemoteRepository(), query, baseURI);
  ...
  public TupleQuery prepareTupleQuery(
    ...
    try {
      ...
      return new EmbergraphRemoteTupleQuery(remote, query, baseURI);
  ...
  @Override
  public boolean hasStatement(final Statement s, final boolean includeInferred, final Resource... c)
      throws RepositoryException {
   ...
   */
  @Override
  public void add(final Statement stmt, final Resource... c) throws RepositoryException {
  ...
  @Override
  public void add(final Iterable<? extends Statement> stmts, final Resource... c)
      throws RepositoryException {
  ...
  private void add(final AddOp op, final Resource... c) throws RepositoryException {
   ...
   */
  @Override
  public void remove(final Statement stmt, final Resource... c) throws RepositoryException {
  ...
  @Override
  public void remove(final Iterable<? extends Statement> stmts, final Resource... c)
      throws RepositoryException {
  ...
  private void remove(final RemoveOp op, final Resource... c) throws RepositoryException {
   ...
   *     inferred statements </a>
   *     <p>TODO size() and isEmpty() are defined by openrdf in terms of explicit statements.
   *     However, we have historically always realized them in terms of statements without respect
   *     to filtering out inferences (when present).
   */
  @Override
  ...
  @Override
  public Update prepareUpdate(final QueryLanguage ql, final String query, final String baseURI)
      throws RepositoryException, MalformedQueryException {

    if (baseURI != null) throw new UnsupportedOperationException("baseURI not supported");
   ...
   * be aborted (per the openrdf API all non-committed state is lost on {@link #close()}).
   *
   ...
   * an active transaction is in "auto-commit" mode.
   */
  @Deprecated
  @Override
   ...
   *
   * <p>Note: This is deprecated in openrdf since 2.7.x. The semantics are that a connection without
   * an active transaction is in "auto-commit" mode. If there is an open transaction and auto-commit
   * is disabled, the open transaction is committed. This is per the openrdf API.
   */
  @Deprecated
  @Override
  ...
  public void begin() throws RepositoryException {
    ...
    synchronized (remoteTx) {
      assertOpen();
      if (remoteTx.get() != null) throw new RepositoryException("Active transaction exists");
      try {
        remoteTx.set(
            repo.getRemoteRepository()
                .getRemoteRepositoryManager()
   ...
   *
   * <p>Note: Since all read operations have snapshot isolation, this is only necessary when
   * multiple read operations need to read on the same commit point.
   *
   * <p>TODO While the ability to do read-only operations against a specified timestamp without a
   * transaction exists, it is not exposed by this interface nor can be accomplished using {@link
   * RemoteRepository} since that interface also lacks mechanisms (e.g.,
