...
public class RemoteTransactionManager {
  ...
  public RemoteTransactionManager(final RemoteRepositoryManager remoteRepositoryManager) {
    ...
    if (remoteRepositoryManager == null) throw new IllegalArgumentException();
   ...
   * Therefore there is nothing that will allow us to ensure that the returned {@link RemoteTx}
   * objects are truely 1:1 with the transaction on the database. This means that {@link
   * RemoteTx#lock} that serializes operations on the {@link RemoteTx} may be defeated by this
   * factory if it is used for transactions discovered by <code>LIST-TX</code> rather than just
   * those known to be created for the client by <code>CREATE-TX</code> FIXME The
   * [readsOnCommitTime] is not available for scale-out. Either allow [null] or use -1L if it is not
   * available. Make this consistent in the server, client, and documentation.
   */
  private class RemoteTx implements IRemoteTx {
    ...
    @Override
    public void commit()
        throws RemoteTransactionValidationException, RemoteTransactionNotFoundException {
      synchronized (lock) {
        assertClientThinksTxActive();
        commitTx(txId);
        active.set(false);
   ...
   * <dl>
   *   <dt>{@link #UNISOLATED}
   *   <dd>his requests a new read/write transaction. The transaction will read on the last commit
   *       point on the database at the time that the transaction was created. This is the default
   *       behavior if the timestamp parameter is not specified. Note: The federation architecture
   *       (aka scale-out) does NOT support distributed read/write transactions - all mutations in
   *       scale-out are shard-wise ACID.
   *   <dt>{@link #READ_COMMITTED}
   *   <dd>This requests a new read-only transaction. The transaction will read on the last commit
   *       point on the database at the time that the transaction was created.
   *   <dt>A <i>timestamp</i>
   *   <dd>This requests a new read-only transaction. The operation will be executed again the most
   *       recent committed state whose commit timestamp is less than or equal to timestamp.
   * </dl>
   *
   * @param timestamp The timestamp used to indicate the type of transaction requested.
  ...
  public IRemoteTx createTx(final long timestamp) {
    ...
    try {
      ...
      final JettyResponseListener listener =
      ...
      switch (listener.getStatus()) {
        ...
        default:
          throw new HttpException(
              listener.getStatus(),
              "status=" + listener.getStatus() + ", reason" + listener.getReason());
      }
   ...
   * @throws RemoteTransactionNotFoundException if the transaction was not found on the server.
   ...
   *
   * @param txId The transaction identifier.
   * @return <code>true</code> if the transaction is read-only or if write set of the transaction
   *     was validated and <code>false</code> iff the server was unable to validate a read-write
   *     transaction known to the server.
   * @throws RemoteTransactionNotFoundException if there is no such transaction on the server.
   */
  private boolean prepareTx(final long txId) throws RemoteTransactionNotFoundException {
   ...
   * @param txId The transaction identifier.
   * @throws RemoteTransactionNotFoundException if there is no such transaction on the server.
   * @throws RemoteTransactionValidationException if the transaction exists but could not be
   *     validated.
   */
  private void commitTx(final long txId)
    ...
    try {
      ...
      switch (ex.getStatusCode()) {
        ...
        case 409: // CONFLICT
          // Validation failed.
          throw new RemoteTransactionValidationException(txId, mgr.getBaseServiceURL());
        default: // Unexpected status code.
          throw new RuntimeException(ex);
      }
  ...
  private IRemoteTxState0 singleTxResponse(final JettyResponseListener response) throws Exception {
  ...
  private List<IRemoteTxState0> multiTxResponse(final JettyResponseListener response)
      throws Exception {
      ...
      parser.parse(
          ...
          new DefaultHandler2() {
            ...
            public void startElement(
              ...
              final long readsOnCommitTime = Long.valueOf(attributes.getValue("readsOnCommitTime"));
