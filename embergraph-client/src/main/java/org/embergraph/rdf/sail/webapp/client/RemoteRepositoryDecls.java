...
public class RemoteRepositoryDecls {
   ...
   * @see <a href="http://trac.blazegraph.com/ticket/854">Allow overrride of maximum length before
   *     converting an HTTP GET to an HTTP POST </a>
   */
   ...
   *
   * <p>Note: I suspect that 2000 might be a better default limit. If the limit is 4096 bytes on the
   * target, then, even with UTF encoding, most queries having a request URL that is 2000 characters
   * long should go through with a GET. 1000 is a safe value but it could reduce http caching.
   */
   ...
   * API must not include a dependency on the Sail so it is given by value again here in a package
   * local scope.
   *
   ...
   * HTTP_HEADER_EMBERGRAPH_MAX_QUERY_MILLIS header} will be used as an effective timeout.
   *
