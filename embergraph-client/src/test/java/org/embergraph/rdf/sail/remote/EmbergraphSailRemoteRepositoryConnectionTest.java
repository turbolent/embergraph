 ...
 * @author igorkim
 */
public class EmbergraphSailRemoteRepositoryConnectionTest extends TestCase {

  private EmbergraphSailRemoteRepositoryConnection con;
  private MockRemoteRepository remote;
  private EmbergraphSailRemoteRepository repo;
  ...
  protected void setUp() {
    ...
    repo = remote.getEmbergraphSailRemoteRepository();
    con = new EmbergraphSailRemoteRepositoryConnection(repo);
  ...
  public void testTupleQueryBindings() throws Exception {
    ...
    tq.setBinding("o", o);
    {
      ...
      try {
        ...
        assertEquals(
            EncodeDecodeValue.encodeValue(s), remote.data.request.getParams().get("$s").getValue());
        assertEquals(
            EncodeDecodeValue.encodeValue(p), remote.data.request.getParams().get("$p").getValue());
  ...
  public void testTupleQueryIncludeInferred() throws Exception {
    ...
    assertEquals(
        "false",
        remote.data.request.getParams().get(RemoteRepositoryDecls.INCLUDE_INFERRED).getValue());

    tq.setIncludeInferred(true);
    final TupleQueryResult tqr = tq.evaluate();
    try {
      assertEquals(
          "true", remote.data.opts.getRequestParam(RemoteRepositoryDecls.INCLUDE_INFERRED));
      assertEquals(
          "true",
          remote.data.request.getParams().get(RemoteRepositoryDecls.INCLUDE_INFERRED).getValue());
    } finally {
  ...
  public void testTupleQueryDataset() throws Exception {
    ...
    try {
      ...
      Collection<String> optsDefaultGraphs =
          Arrays.asList(
              remote.data.opts.requestParams.get(RemoteRepositoryDecls.DEFAULT_GRAPH_URI));
      ...
      List<String> requestDefaultGraphs =
          remote.data.request.getParams().get(RemoteRepositoryDecls.DEFAULT_GRAPH_URI).getValues();
      ...
      Collection<String> optsNamedGraphs =
          Arrays.asList(remote.data.opts.requestParams.get(RemoteRepositoryDecls.NAMED_GRAPH_URI));
      assertTrue(optsNamedGraphs.contains(namedGraph1.stringValue()));
      assertTrue(optsNamedGraphs.contains(namedGraph2.stringValue()));
      List<String> requestNamedGraphs =
          remote.data.request.getParams().get(RemoteRepositoryDecls.NAMED_GRAPH_URI).getValues();
  ...
  public void testTupleQueryBaseURI() throws Exception {
    ...
    final TupleQuery tq =
        con.prepareTupleQuery(QueryLanguage.SPARQL, "select * where {?s ?p ?o}", baseURI);
    final TupleQueryResult tqr = tq.evaluate();
    try {
  ...
  public void testGetStatements() throws RepositoryException {
    ...
    try {
      ...
      assertEquals(
          ...
          remote.data.opts.getRequestParam(RemoteRepositoryDecls.INCLUDE_INFERRED));
    } finally {
      stmts.close();
    }
  }
