...
public class PubChemInlineURIFactory extends InlineURIFactory {
  ...
  public PubChemInlineURIFactory() {
    ...
    // http://www.bioassayontology.org/bao#BAO_0002877 fixed width 7
    addHandler(
        new InlinePrefixedFixedWidthIntegerURIHandler(
...

    {
      ...
      final InlineIntegerURIHandlerMap oboMap = new InlineIntegerURIHandlerMap(oboNS);

      int i = 0;
      // http://purl.obolibrary.org/obo/CHEBI_13453
      InlineURIHandler handler = new InlinePrefixedIntegerURIHandler(oboNS, "CHEBI_", i);
...

    {
      ...
      final InlineIntegerURIHandlerMap hMap = new InlineIntegerURIHandlerMap(descriptorNS);

      for (int i = 0; i < descriptorSuffix.length; i++) {
        final InlineURIHandler h =
            new InlinePrefixedSuffixedIntegerURIHandler(
