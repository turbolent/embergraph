 ...
 *
 * <p>The {@link ServiceRegistry} pattern provides a declarative association between a service and a
 * service provider. The service declarations are located in <code>META-INF/services/</code>
 * packages. Each file in such a package name the service interface and the contents of the file
 * name the service provider(s). The set of all such service provides located in all such service
 * packages is registered for a given service interface. For openrdf, the service provides are
 * initially associated with a <i>key</i>, such as an {@link RDFFormat}, {@link QueryLanguage}, etc.
 *
 * <p>However, this service registration pattern does not support the specification of a
 * <em>preferred</em> service provider. In the context of multiple service providers for the same
 * <i>key</i> and service interface, there is no way to control which service provider will remain
 * in the {@link ServiceRegistry}.
 *
 * <p>This effects things such as the embergraph extension for the RDF/XML parser which adds support
 * for SIDs mode interchange and the interchange of {@link org.embergraph.rdf.model.StatementEnum}
 * metadata.
 *
 * <p>This class is used to "hook" the various service registeries and force the use of the
 * embergraph extension when it adds semantics not present in the base service provider
 * implementation. For such "hooked" services providers, the service registry pattern using <code>
 * META-INF/services</code> is not manditory, but following the pattern is never the less
 * recommended.
 *
...
public class ServiceProviderHook {
  ...
  public static final String NTRIPLES_PARSER_FACTORY =
      "org.embergraph.rdf.rio.ntriples.EmbergraphNTriplesParserFactory";

  public static final String TURTLE_PARSER_FACTORY =
      "org.embergraph.rdf.rio.turtle.EmbergraphTurtleParserFactory";

  public static final String TURTLE_WRITER_FACTORY =
      "org.embergraph.rdf.rio.turtle.EmbergraphTurtleWriterFactory";

  public static final String JSON_WRITER_FACTORY =
      "org.embergraph.rdf.rio.json.EmbergraphSPARQLResultsJSONWriterFactory";

  public static final String JSON_CONSTRUCT_WRITER_FACTORY =
      "org.embergraph.rdf.rio.json.EmbergraphSPARQLResultsJSONWriterForConstructFactory";

  public static final String JSON_RESULT_PARSER_FACTORY =
      "org.embergraph.rdf.rio.json.EmbergraphSPARQLResultsJSONParserFactory";
  public static final String JSON_CONSTRUCT_PARSER_FACTORY =
      "org.embergraph.rdf.rio.json.EmbergraphSPARQLResultsJSONParserForConstructFactory";
   ...
   *
   * <p>Note: We need to use a synchronized pattern in order to ensure that any threads contending
   * for this method awaits its completion. It would not be enough for a thread to know that the
   * method was running. The thread needs to wait until the method is done.
   */
  public static synchronized void forceLoad() {
    ...
    if (log.isInfoEnabled()) {
      ...
      for (TupleQueryResultWriterFactory f :
          TupleQueryResultWriterRegistry.getInstance().getAll()) {
     ...
     */
    {
      ...
      assert r.has(
          ((RDFParserFactory) getInstanceForClass(NTRIPLES_PARSER_FACTORY)).getRDFFormat());
