 ...
 * @version $Id$
 *     <p>TODO Correct rejection test when a magic predicate for search appears outside of a named
 *     query clause.
 *     <p>TODO Correct rejection test when the subject of a magic predicate for search is a
 *     constant.
 *     <p>TODO Correct rejection test when an unknown magic predicate in the {@link
 *     BD#SEARCH_NAMESPACE} is observed.
 *     <p>TODO Add unit tests for slicing by rank and relevance.
 */
public class TestSearch extends AbstractDataDrivenSPARQLTestCase {
   ...
   *
   * <p>This version has an unbound graph variable.
   */
   ...
   * a subject join is automatically added into the named query using an anonymous variable for the
   * subject and predicate positions to impose that constraint.
   */
   ...
   * </pre>
   *
   * <p>Note: The first and second condition in the old test were identical. The first condition was
   * modified to run with no graphs specified when the tests were migrated to this format.
   *
   * <p>Note: This is the first of a series of tests which are a port of
   * org.embergraph.rdf.sail.TestSearchQuery#testWithNamedGraphs(). Some errors in that test were
   * corrected when the tests were migrated to this format.
   */
   ...
   * </pre>
   *
   * <p>Note: Note: this was running against graphB in the old test suite, but we do that in the
   * case above. It has been modified to run against graphA in this test suite.
   */
  ...
  /** Unit test with explicit {@link BD#MIN_RELEVANCE} of zero and a prefix match search. */
