...
public class TestDefaultGraphs extends AbstractDataDrivenSPARQLTestCase {
   ...
   *
   * <p>Given that there are no duplicate triples (i.e., no triple resides in more than one named
   * graph), this query still gives us the correct result. We also check that there is no distinct
   * filter in the query plan.
   */
  public void test_defaultGraphs_01i() throws Exception {
    ...
    final SPOPredicate spoPred =
        (SPOPredicate) pipelineJoin.annotations().get(AccessPathJoinAnnotations.PREDICATE);
   ...
   * result contains the triple only once. We also check that there is a distinct filter in the
   * query plan.
   */
   ...
   * triple above being contained twice in the result.
   */
