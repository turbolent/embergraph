 ...
 *
 * <p>Note: The static analysis depends on the AST as generated from the parse tree or as rewritten
 * by one or more {@link IASTOptimizer}s. Most tests do not run the {@link IASTOptimizer}s because
 * we want to analyze the AST as generated from the parse tree rather than the AST as optimized by
 * the suite of optimizers. In many cases, actually running the suite of optimizers will cause some
 * structures to be pruned from the AST. This is especially true of the unit tests for the static
 * analysis of the TCK queries, many of which use variables in filters when the variables are not
 * bound in that scope or use a "badly designed left join" pattern.
 *
 * <p>TODO If we modify the methods which report on the must/may bindings to look for null IVs, then
 * we will need to add the URIs appearing in the query to the database before we run the parser.
 *
...
public class TestStaticAnalysis extends AbstractASTEvaluationTestCase {
  ...
  public void test_static_analysis01() throws MalformedQueryException {
    ...
    final QueryRoot queryRoot =
        new Embergraph2ASTSPARQLParser().parseQuery2(queryStr, baseURI).getOriginalAST();
   ...
   *
   * <p>Note: The join should be on <code>?x</code> in this example.
   *
   * <p>FIXME Write more unit tests for {@link StaticAnalysis#getJoinVars(SubqueryRoot, Set)} and
   * friends.
   */
  public void test_static_analysis_join_vars() throws MalformedQueryException {
    ...
    // variables which must be bound in the named subquery's projection.
    {
      ...
      final NamedSubqueryRoot namedSubquery =
          (NamedSubqueryRoot) queryRoot.getNamedSubqueries().get(0);

      assertEquals(expectedProjected, sa.getDefinitelyProducedBindings(namedSubquery));
    }
   ...
   * </pre>
   *
   * This query does not present a problem since <code>?v</code> is in scope when the filter is
   * evaluated.
   */
  public void test_static_analysis_filter_nested_1() throws MalformedQueryException {
    ...
    final GraphPatternGroup<IGroupMemberNode> whereClause = queryRoot.getWhereClause();
   ...
   * </pre>
   *
   * <code>?v</code> is not bound in the FILTER because it is evaluated with bottom up semantics and
   * therefore the bindings from the parent group are not visible.
   */
  public void test_static_analysis_filter_nested_2() throws MalformedQueryException {
    ...
    // Test "must" bound bindings for the query.
    assertEquals(asSet(new String[] {"v"}), sa.getDefinitelyProducedBindings(queryRoot));
    ...
    {
      final JoinGroupNode filterClause = (JoinGroupNode) queryRoot.getWhereClause().get(1);
   ...
   *
   * <p><code>?w</code> is bound in the child join group regardless of whether the embedded optional
   * succeeds.
   *
   * <p><code>?v</code> is not bound in the FILTER because it is evaluated with bottom up semantics
   * and therefore the bindings from the outer parent group are not visible.
   *
   * <p>For reference, the AST for that SPARQL query is:
   *
   * <pre>
   * PREFIX : <http://example/>
   * QueryType: SELECT
   ...
   * </pre>
   *
   * This can be reduced to a filter which does not bind anything. Since the FILTER can not succeed,
   * it should logically be replaced by failing the group(s) within which it appears. This needs to
   * be done recursively up to the parent, stopping at the first parent group which is optional. If
   * all parents up to the WHERE clause are eliminated, then the WHERE clause itself can not succeed
   * and the query should be replaced by a "DropSolutionsBOp". The DropSolutionsOp can be
   * substituted in directly for a group which can not succeed and then we can work the pruning of
   * the parents as an efficiency.
   *
   * <p>Note: An AST optimizer needs to recognize and transform this query by lifting out
   *
   * <pre>
   *  :x :q ?w OPTIONAL {  :x :p ?v2 FILTER(?v = 1) }
   * </pre>
   *
   * into a named subquery. Since the named subquery runs without any incoming bindings, the result
   * will be as if we had used bottom up evaluation.
   *
  ...
  public void test_static_analysis_filter_scope_1() throws MalformedQueryException {
    ...
    queryRoot =
        (QueryRoot)
            new ASTWildcardProjectionOptimizer()
                .optimize(context, new QueryNodeWithBindingSet(queryRoot, null))
                .getQueryNode();
    ...
    // Test "must" bound bindings for the query.
    assertEquals(asSet(new String[] {"v", "w"}), sa.getDefinitelyProducedBindings(queryRoot));

    // Outer group clause { :x :p ?v . }
    {
      final GraphPatternGroup<IGroupMemberNode> group = (JoinGroupNode) queryRoot.getWhereClause();
    ...
    // Nested group clause {  :x :q ?w  }
    {
      final GraphPatternGroup<IGroupMemberNode> group =
          (JoinGroupNode) queryRoot.getWhereClause().get(1);
   ...
   * </pre>
   *
   * <p>Note: For this query, the bindings of <code>?X</code> in the outer group are not visible
   * when the inner groups are evaluated. Because of this, <code>?X</code> in the optional clause
   * binds for values which differ from the values bound on <code>?X</code> in the outer group. This
   * means that the solutions from the middle group with bindings for <code>?Y</code> and <code>?X
   * </code> fail to join with the solutions in the outer group. Note that there is no email address
   * for "paul" for the data set used to run this query. If there were, then the query would have a
   * result.
   *
   * <p>These group expressions need to be evaluated independently because they are not sharing a
   * binding for <code>?X</code> until we join them together on <code>?X</code>.
   *
   * <p>In order for us to run this query correctly we need to run
   *
   * <pre>
   * {?Y :name "george" . OPTIONAL { ?X :email ?Z } }
   * </pre>
   ...
   * </pre>
   *
   * This can be most easily achieved by lifting the former out into a named subquery.
   *
   * @see ASTBottomUpOptimizer
   */
  public void test_static_analysis_join_scope_1() throws MalformedQueryException {
    ...
    // Test "must" bound bindings for the query.
    assertEquals(asSet(new String[] {"X", "Y"}), sa.getDefinitelyProducedBindings(queryRoot));
   ...
   * </pre>
   *
   * Note: The "be:search", "bd:relevance" and <code>?subj ?p ?lit</code> statement patterns below
   * will be transformed into a {@link ServiceNode}. Since there is nothing else in the main query's
   * WHERE clause, the {@link ServiceNode} is not lifted out into a named subquery.
   */
  ...
  public void test_static_analysis_serviceCall2() throws MalformedQueryException {
    ...
    final ServiceNode endpoint2;
    {
      ...
      assertEquals(
          endpoint1.getServiceRef().getValue(),
          new URIImpl("http://localhost:18080/openrdf/repositories/endpoint1"));
      ...
      assertEquals(
          endpoint2.getServiceRef().getValue(),
          new URIImpl("http://localhost:18080/openrdf/repositories/endpoint2"));
  ...
  public void test_static_analysis_filters() throws MalformedQueryException {
    ...
    {
      final Iterator<JoinGroupNode> itr =
          BOpUtility.visitAll(queryRoot.getWhereClause(), JoinGroupNode.class);
    ...
    //        "     FILTER ( ?x < 100 ) .\n"+// prune-filter
    {
      ...
      for (IGroupMemberNode child : innerGroup) {
        if (child instanceof FilterNode) {
          ...
          final FunctionNode expr = (FunctionNode) tmp.getValueExpressionNode();
          if (expr.getFunctionURI().equals(FunctionRegistry.GT)) {
            GT = tmp;
          } else if (expr.getFunctionURI().equals(FunctionRegistry.NE)) {
            NE = tmp;
          } else if (expr.getFunctionURI().equals(FunctionRegistry.LT)) {
            LT = tmp;
    ...
    assertEquals("pre-filters", Collections.singletonList(a_NE_b), sa.getPreFilters(innerGroup));

    assertEquals(
        "join-filters", Collections.singletonList(ageA_GT_ageB), sa.getJoinFilters(innerGroup));

    assertEquals("post-filters", Collections.singletonList(x_LT_10), sa.getPostFilters(innerGroup));

    assertEquals(
        "prune-filters", Collections.singletonList(x_LT_10), sa.getPruneFilters(innerGroup));
    ...
    assertEquals("pre-filters", Collections.emptyList(), sa.getPreFilters(outerGroup));

    assertEquals("join-filters", Collections.emptyList(), sa.getJoinFilters(outerGroup));

    assertEquals(
        "post-filters", Collections.singletonList(ageA_GT_20), sa.getPostFilters(outerGroup));

    assertEquals("prune-filters", Collections.emptyList(), sa.getPruneFilters(outerGroup));
  ...
  public void test_static_analysis_getMaybeProducedBindings() {
    ...
    final StatementPatternNode sp1, sp2;
    {
      ...
      sp1 = new StatementPatternNode(new VarNode("a"), new ConstantNode(p), new VarNode("n"));
      whereClause.addChild(sp1);

      sp2 = new StatementPatternNode(new VarNode("a"), new ConstantNode(q), new VarNode("m"));
   ...
   * variables (such cases should be pruned); and (b) FILTERS in the right hand side will fail in
   * they assume visibility of variables in the left hand side.
   *
   * <pre>
   * PREFIX : <http://example/>
   * SELECT ?a ?n
  ...
  public void test_static_analysis_minus_sharedVariables() {
    ...
    final JoinGroupNode whereClause, minusGroup;
    {
      ...
      whereClause.addChild(
          new StatementPatternNode(new VarNode("a"), new ConstantNode(p), new VarNode("n")));
      ...
      minusGroup.addChild(
          new StatementPatternNode(new VarNode("a"), new ConstantNode(q), new VarNode("n")));
    }
  ...
  public void test_static_analysis_minus_nothingShared() {
    ...
    final JoinGroupNode whereClause, minusGroup;
    {
      ...
      minusGroup.addChild(
          new StatementPatternNode(new VarNode("x"), new VarNode("y"), new VarNode("z")));
    }
  ...
  public void test_static_analysis_minus_and_optional() {
    ...
    final JoinGroupNode whereClause, optGroup1, minusGroup, optGroup2;
    {
      ...
      optGroup1.addChild(
          new StatementPatternNode(new VarNode("b"), new ConstantNode(p), new ConstantNode(three)));
      ...
      optGroup2.addChild(
          new StatementPatternNode(new VarNode("c"), new ConstantNode(q), new ConstantNode(three)));
    }
  ...
  public void test_static_analysis_subGroups_and_optional() {
    ...
    final JoinGroupNode whereClause, optGroup1, childGroup, optGroup2;
    {
...

      {
        ...
        optGroup2.addChild(
            new StatementPatternNode(
                new VarNode("c"), new ConstantNode(q), new ConstantNode(three)));
      }

      {
        ...
        optGroup1.addChild(
            new StatementPatternNode(
                new VarNode("b"), new ConstantNode(p), new ConstantNode(three)));
      }
    }
   ...
   * </pre>
   *
   * This test verifies that the correct projection is computed for each of the sub-groups in the
   * query. Those projections are as follows:
   *
   * <pre>
   *         SELECT ?var1 ?var6
   *         WHERE {
  ...
  public void test_static_analysis_getProjectedVars() {
    ...
    @SuppressWarnings("rawtypes")
    final IV polititian =
        makeIV(new URIImpl("http://www.rdfabout.com/rdf/schema/politico/Politician"));
    @SuppressWarnings("rawtypes")
    ...
    final JoinGroupNode whereClause, optionalGroup1, optionalGroup2, optionalGroup3;
    {
...

      {
        ...
        optionalGroup2.addChild(
            new StatementPatternNode(
                new VarNode("var12"),
                new ConstantNode(sponsor),
                new VarNode("var1"),
                null /* c */,
                Scope.DEFAULT_CONTEXTS));
   ...
   *     <p>TODO Extend this test to cover the case where <code>b</code> is an exogenously bound
   *     variable. This does not change the MUST/MIGHT analysis of the query.
   */
  public void test_static_analysis_projection_01() {
    ...
    final JoinGroupNode whereClause;
    {
      ...
      // Top-level projection
      {
        ...
        projection.addProjectionExpression(new AssignmentNode(new VarNode("b"), new VarNode("a")));
      }
      ...
      whereClause.addChild(
          new StatementPatternNode(
              new VarNode("a"), new ConstantNode(rdfType), new ConstantNode(foafPerson)));
    }
   ...
   *
   * <p>TODO Extend this test to also cover the case where <code>b</code> is an exogenously bound
   * variable. In that case, it MUST be bound.
   *
   * <p>TODO Extend this test to also cover the case where <code>a</code> is an exogenously bound
   * variable. Since <code>a</code> is not projected, it's bound value SHOULD NOT be visible inside
   * of the query (variables with exogenous variables must obey the same scoping rules as variables
   * which only become bound during query evaluation).
   *
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/430
   */
    ...
    final JoinGroupNode whereClause;
    {
      ...
      // Top-level projection
      {
        ...
        projection.addProjectionExpression(
            new AssignmentNode(new VarNode("b"), new ConstantNode(foafPerson)));
      }
   ...
   * </pre>
   *
   * In this query, it is presumed that <code>?b</code> is the number of years of school and will be
   * ZERO in the data for people who have not yet begun formal schooling. Hence, the select
   * expression can evaluate to an error (divide by zero). However, since this is NOT an aggregation
   * query, the error will cause the solution to be dropped. Hence, <code>?c</code> is definitely
   * bound for this query.
   *
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/430
   *     <p>TODO Extend this test to cover the case where <code>b</code> is an exogenously bound
   *     variable. This does not change the MUST/MIGHT analysis of the query.
    ...
    final JoinGroupNode whereClause;
    {
      ...
      // Top-level projection
      {
        ...
        projection.addProjectionExpression(
            new AssignmentNode(
                new VarNode("c"),
                FunctionNode.binary(FunctionRegistry.DIVIDE, new VarNode("a"), new VarNode("b"))));
      }
      ...
      // WHERE clause.
      {
        ...
        whereClause.addChild(
            new StatementPatternNode(
                new VarNode("x"), new ConstantNode(rdfType), new ConstantNode(foafPerson)));

        whereClause.addChild(
            new StatementPatternNode(new VarNode("x"), new ConstantNode(age), new VarNode("a")));

        whereClause.addChild(
            new StatementPatternNode(new VarNode("x"), new ConstantNode(grade), new VarNode("b")));
      }
    }
   ...
   * In this query, it is presumed that <code>?b</code> is the number of years of school and will be
   * ZERO in the data for people who have not yet begun formal schooling. Hence, the select
   * expression can evaluate to an error (divide by zero).
   *
   * <p>Since this is an aggregation query, the solution will still be reported with an unbound
   * value for <code>?c</code>. This illustrates the general principle that we can not assume that a
   * select expression based on definitely bound variables will itself be definitely bound.
   *
   * @see https://sourceforge.net/apps/trac/bigdata/ticket/430
   *     <p>TODO Extend this test to cover the case where <code>b</code> is an exogenously bound
   *     variable. Does this change the MUST/MIGHT analysis of the query?
   */
  ...
  public void test_findParent_03() {
    ...
    final String namedSet = "set1";
    {
      ...
      // WHERE clause.
      {
        ...
        whereClause.addChild(include = new NamedSubqueryInclude(namedSet));
      }
    }
  ...
  public void test_findParent_04() {
    ...
    final FilterNode filterNode;
    {
      ...
      // WHERE clause.
      {
        ...
        serviceGroup.addChild(
            new StatementPatternNode(new VarNode("x"), new ConstantNode(age), new VarNode("a")));
        serviceNode = new ServiceNode(new ConstantNode(serviceUri), serviceGroup);
        whereClause.addChild(serviceNode);

        existsGroup = new JoinGroupNode();
        existsGroup.addChild(
            new StatementPatternNode(new VarNode("x"), new ConstantNode(grade), new VarNode("b")));
        filterNode = new FilterNode(new ExistsNode(new VarNode("anon-var-1"), existsGroup));
