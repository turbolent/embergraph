...
public class TestASTSparql11SubqueryOptimizer extends AbstractASTEvaluationTestCase {
  ...
  public void test_subSelectWithLimitAndOrderBy() {
    ...
    final QueryRoot given = new QueryRoot(QueryType.SELECT);
    {
      ...
      }
      {
        ...
        subqueryRoot.setSlice(new SliceNode(0L /* offset */, 3L /* limit */));

        final OrderByNode orderByNode = new OrderByNode();
        subqueryRoot.setOrderBy(orderByNode);

        orderByNode.addExpr(new OrderByExpr(new VarNode("s"), true /* asc */));
      }
    }
   ...
   * </pre>
   *
   * Note: <code>?s</code> is both bound by the statement pattern and is projected by the sub-select
   * so it will be a join variable.
   *
   * <p>Note: the sub-select does not meet any of the other criteria for being lifted (it does not
   * use LIMIT + ORDER BY and it is not an aggregate).
   */
