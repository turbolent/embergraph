 ...
 *
 * <p>TODO Test when knownBound is not empty/null. (Note that there is an inherent recursion since
 * we need to figure out the join order within the parent so we can figure out what is known bound
 * on entry to a child.)
 *
 * <p>TODO Test with filters which can not run until after the optionals.
 *
...
public class TestJoinSetUtil extends AbstractASTEvaluationTestCase {
  ...
  public void test_joinSetUtil_BSBMQ5() {
    ...
    final JoinSetUtil fixture =
        new JoinSetUtil(sa, null /* knownBound */, s.queryRoot.getWhereClause());
    ...
    assertEquals(new IJoinNode[] {s.p0, s.p1, s.p2, s.p3, s.p4, s.p5, s.p6}, fixture.requiredJoins);

    assertEquals("requiredJoinCount", 7, fixture.requiredJoinCount);

    // Verify the partitioning on the filters by when they will run.
    {
      assertEquals("preFilters", asSet(new FilterNode[] {}), fixture.preFilters);

      assertEquals("joinFilters", asSet(new FilterNode[] {s.c0, s.c1, s.c2}), fixture.joinFilters);

      assertEquals("postFilters", asSet(new FilterNode[] {}), fixture.postFilters);
    }

    // Verify the variables for each vertex.
    {
      assertEquals(
          asSet(new IVariable[] {Var.var("product"), Var.var("productLabel")}), fixture.vars[0]);

      assertEquals(asSet(new IVariable[] {Var.var("prodFeature")}), fixture.vars[1]);

      assertEquals(
          asSet(new IVariable[] {Var.var("product"), Var.var("prodFeature")}), fixture.vars[2]);

      assertEquals(asSet(new IVariable[] {Var.var("origProperty1")}), fixture.vars[3]);

      assertEquals(
          asSet(new IVariable[] {Var.var("product"), Var.var("simProperty1")}), fixture.vars[4]);

      assertEquals(asSet(new IVariable[] {Var.var("origProperty2")}), fixture.vars[5]);

      assertEquals(
          asSet(new IVariable[] {Var.var("product"), Var.var("simProperty2")}), fixture.vars[6]);
    }
    ...
    {
      assertTrue(
          fixture.directJoinSets.contains(
              new VertexJoinSet(
      ...
      assertTrue(
          fixture.directJoinSets.contains(
              new VertexJoinSet(
                  asSet(new Integer[] {3}), asSet(new IVariable[] {Var.var("origProperty1")}))));

      assertTrue(
          fixture.directJoinSets.contains(
              new VertexJoinSet(
                  asSet(new Integer[] {5}), asSet(new IVariable[] {Var.var("origProperty2")}))));
