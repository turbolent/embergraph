...
public class TestGearingQueryHint extends AbstractDataDrivenSPARQLTestCase {
  ...
  /** Simple s p* o pattern without user-defined gearing hint. Case where the pattern does match. */
   ...
   *
   * <p>The test makes sure that the query hint is properly attached.
   *
   * @throws Exception
   */
  ...
  private void assertQueryHintSet(final PipelineOp queryPlan, final String expectedGearing) {
    ...
    final List<ArbitraryLengthPathOp> apOps =
        BOpUtility.toList(queryPlan, ArbitraryLengthPathOp.class);
