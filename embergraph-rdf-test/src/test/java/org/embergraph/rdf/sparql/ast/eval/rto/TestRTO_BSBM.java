 ...
 * enough. In priciple, those queries might exhibit different correlations. However, the pc100 data
 * set may be too small for any interesting correlations. In fact, it may be too small since the
 * vertex estimates and cutoff joins may be exact before the RTO is run running. If so, then we need
 * to go back and use a larger data set. However, the specific parameterized queries will remain
 * valid against larger data sets since BSBM only adds more data when generating a larger data set.
 * Of course, the number of solutions for the queries may change.
 *
 * <p>Note: BSBM uses a lot of filters, subgroups, and sub-selects. As we build up coverage for
 * those constructions in the RTO, it will handle more of the query. As a result, the observed join
 * orders (and even the #of joins that are considered) are likely to change.
 *
 * <p>Note: Q6 is no longer run in BSBM (the query was dropped).
 *
 * <p>Note: Q9 is a simple DESCRIBE (too simple for the RTO). Sample query is:
 *
 * <pre>
...
public class TestRTO_BSBM extends AbstractRTOTestCase {
  ...
  public Properties getProperties() {
    ...
    properties.setProperty(EmbergraphSail.Options.AXIOMS_CLASS, NoAxioms.class.getName());
   ...
   * only one of the join groups is marked for RTO optimization in order to keep the test harness
   * simple. The test harness assumes that there is a single JOIN group that is optimized by the RTO
   * and then verifies the join ordering within that join group. The test harness breaks if there is
   * more than one join group optimized by the RTO.
   */
  ...
  public void test_BSBM_Q7b_pc100() throws Exception {
     ...
     */
    final int[] expected =
        QueryHints.DEFAULT_PIPELINED_HASH_JOIN ? new int[] {4, 5, 6, 7} : new int[] {5, 6, 7, 8};
