...
public class TestASTDescribeOptimizer extends AbstractASTEvaluationTestCase {
   ...
   * added which will capture the necessary bindings to support that CONSTUCT.
   */
  public void test_describeOptimizer_iri_only() {
    ...
    final EmbergraphValue[] values =
        new EmbergraphValue[] {
          rdfType, rdfsLabel, foafPerson, mikeURI, bryanURI, mikeLabel, bryanLabel
        };
    ...
    final QueryRoot queryRoot = new QueryRoot(QueryType.DESCRIBE);
    {
      ...
      projection.addProjectionExpression(
          new AssignmentNode(anonvar, new ConstantNode(mikeURI.getIV())));
    }
   ...
   * </pre>
   *
   * where the {@link ProjectionNode} has been explicitly marked to specify {@link
   * DescribeModeEnum#ForwardOneStep}.
   *
   * <p>This test verifies that the correct CONSTRUCT clause is generated and that a WHERE clause is
   * added which will capture the necessary bindings to support that CONSTUCT.
   */
  ...
  public void test_describeOptimizer_iri_only_describeMode_CBD() {
    ...
    final EmbergraphValue[] values =
        new EmbergraphValue[] {
          rdfType, rdfsLabel, foafPerson, mikeURI, bryanURI, mikeLabel, bryanLabel, rdfSubject
        };
    ...
    final QueryRoot expected = new QueryRoot(QueryType.CONSTRUCT);
    {
      ...
      // ?s1 ?p1b term1 .
      {
        ...
        //                constructNode
        //                        .addChild(new StatementPatternNode(s0, p0b, term0));
        constructNode.addChild(new StatementPatternNode(stmtVar, termRdfSubject, term0));
      }
  ...
  public void test_describeOptimizer_iri_and_var() {
    ...
    final QueryRoot queryRoot = new QueryRoot(QueryType.DESCRIBE);
    {
      ...
      anonvar.setAnonymous(true);
      projection.addProjectionExpression(
          new AssignmentNode(anonvar, new ConstantNode(dcURI.getIV())));
