...
public class TestASTSimpleOptionalOptimizer extends AbstractASTEvaluationTestCase {
   ...
   * mean that we can not lift out the statement pattern.
   */
  public void test_simpleOptional() throws MalformedQueryException {
    ...
    queryRoot =
        (QueryRoot)
            new ASTSimpleOptionalOptimizer()
                .optimize(context, new QueryNodeWithBindingSet(queryRoot, null))
                .getQueryNode();
     ...
     */
    {
      ...
      while (itr.hasNext()) {
        ...
        ngroups++;
        if (tmp.isOptional()) noptionalGroups++;
      }
   ...
   * </pre>
   *
   * Note: This TCK query will fail if we do not lift the simple optional correctly.
   *
  ...
  public void test_sparql_bev_5() throws Exception {
    ...
    final JoinGroupNode expectedClause = new JoinGroupNode();
    {
      ...
      // :x3 :q ?w
      final StatementPatternNode liftedSp =
          new StatementPatternNode(
              new VarNode("a"), // s
