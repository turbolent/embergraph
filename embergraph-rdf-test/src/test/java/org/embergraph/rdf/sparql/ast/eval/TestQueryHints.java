...
import org.embergraph.bop.join.HashJoinOp;
...
import org.embergraph.rdf.sparql.ast.hints.QueryHintException;
import org.embergraph.rdf.sparql.ast.optimizers.ASTQueryHintOptimizer;
import org.embergraph.rdf.sparql.ast.optimizers.TestASTQueryHintOptimizer;
 ...
 *     <p>TODO Unit test to verify that hint:optimistic actually changes the value so the query runs
 *     differently (that is, test the query hint not the interpretation of the hint by the static
 *     join order optimizer). (I believe that this was hitting one of the BSBM, or maybe the BSBM BI
 *     queries.)
 *     <p>TODO Unit test query hints for APs both at the BGP scope and in the Query scope (where
 *     they bind on AST2BOpContext). (E.g., the use of remote access paths, hash joins, sample
 *     limits, etc.).
 *     <p>TODO Unit test to verify that query hints are being attached to the {@link
...
public class TestQueryHints extends AbstractDataDrivenSPARQLTestCase {
   ...
   * @return An iterator visiting the operators having a binding for that property.
   */
  @SuppressWarnings("unchecked")
  private static Iterator<BOp> visitQueryHints(final BOp op, final String name) {
            ...
            new Filter() {
              ...
              public boolean isValid(final Object arg0) {
                ...
                final Properties queryHints =
                    (Properties) tmp.getProperty(ASTBase.Annotations.QUERY_HINTS);
                if (queryHints != null) {
                  if (queryHints.getProperty(name) != null) {
                    // exists as query hint property.
                    if (log.isDebugEnabled())
                      log.debug(
                          "Found as query hint: name="
                              + name
                              + ", op="
                              + tmp.toShortString()
                              + "#"
   ...
   */
  private static int countQueryHints(final BOp op, final String name) {
    int n = 0;
  ...
  public void test_query_hints_01() throws Exception {
    ...
    final ASTContainer astContainer = new TestHelper("query-hints-01").runTest();

    /*
     * Verify that the PREFIX "hint:" was implicitly declared.
     */
    assertEquals(QueryHints.NAMESPACE, astContainer.getOriginalAST().getPrefixDecls().get("hint"));
     ...
     */
    {
      if (log.isInfoEnabled()) log.info(BOpUtility.toString(astContainer.getOptimizedAST()));

      final JoinGroupNode whereClause =
          (JoinGroupNode) astContainer.getOptimizedAST().getWhereClause();
      ...
      assertEquals(
          "10",
          whereClause.getQueryHint(PipelineOp.Annotations.MAX_PARALLEL, "-1" /* defaultValue */));
      assertEquals(
          "20",
          whereClause.getQueryHint(
              PipelineOp.Annotations.PIPELINE_QUEUE_CAPACITY, "-1" /* defaultValue */));
      assertEquals(
          "20",
          whereClause.getQueryHint(
              PipelineOp.Annotations.CHUNK_OF_CHUNKS_CAPACITY, "-1" /* defaultValue */));
      ...
      assertEquals(
          1,
          countQueryHints(astContainer.getOptimizedAST(), PipelineOp.Annotations.CHUNK_CAPACITY));
      ...
      assertSameIteratorAnyOrder(
          ...
          visitQueryHints(astContainer.getOptimizedAST(), QueryHints.OPTIMIZER));
      ...
      // The 1st SP.
      {
        ...
        assertEquals(
            10,
            Integer.parseInt(
                sp.getQueryHint(PipelineOp.Annotations.MAX_PARALLEL, "-1" /* defaultValue */)));
      ...
      // The 2nd SP.
      {
        final StatementPatternNode sp = (StatementPatternNode) whereClause.get(1);
        ...
        assertEquals(
            1000,
            Integer.parseInt(
                sp.getQueryHint(BufferAnnotations.CHUNK_CAPACITY, "-1" /* defaultValue */)));
     ...
     */
    {
      if (log.isInfoEnabled()) log.info(BOpUtility.toString(astContainer.getQueryPlan()));

      @SuppressWarnings("rawtypes")
      final Iterator<PipelineJoin> jitr =
          BOpUtility.visitAll(astContainer.getQueryPlan(), PipelineJoin.class);
      ...
      assertFalse(jitr.hasNext());
      {
        ...
        if (log.isInfoEnabled()) log.info("pred=" + BOpUtility.toString(pred));

        assertEquals(RDFS.LABEL, ((IV<?, ?>) pred.get(1 /* p */).get()).getValue());
        ...
        assertNull(pred.getProperty(PipelineOp.Annotations.CHUNK_CAPACITY));
        assertEquals("20", pred.getProperty(PipelineOp.Annotations.CHUNK_OF_CHUNKS_CAPACITY));
        // Note: This hint was given, but it is not copied to the Predicate.
        assertNull(pred.getProperty(PipelineOp.Annotations.PIPELINE_QUEUE_CAPACITY));
        // Note: This annotation was not given.
        assertNull(pred.getProperty(IPredicate.Annotations.FULLY_BUFFERED_READ_THRESHOLD));
      }

      {
        ...
        assertEquals(RDF.TYPE, ((IV<?, ?>) pred.get(1 /* p */).get()).getValue());
        ...
        assertEquals("1000", pred.getProperty(PipelineOp.Annotations.CHUNK_CAPACITY));
        assertEquals("20", pred.getProperty(PipelineOp.Annotations.CHUNK_OF_CHUNKS_CAPACITY));
        // Note: This hint was given, but it is not copied to the Predicate.
        assertNull(pred.getProperty(PipelineOp.Annotations.PIPELINE_QUEUE_CAPACITY));
       ...
       */
      {
        final Iterator<ProjectionOp> projItr =
            BOpUtility.visitAll(astContainer.getQueryPlan(), ProjectionOp.class);
        ...
        // Not transferred since Scope:=Group.
        assertNull(projection.getProperty(PipelineOp.Annotations.CHUNK_CAPACITY));

        // Scope:=SubQuery, so transferred to the SELECT clause.
        assertNotNull(projection.getProperty(PipelineOp.Annotations.CHUNK_OF_CHUNKS_CAPACITY));
  ...
  public void test_query_hints_02() throws Exception {
    ...
    final ASTContainer astContainer = new TestHelper("query-hints-02").runTest();
  ...
  public void test_query_hints_04a() throws Exception {
    ...
    final ASTContainer astContainer = new TestHelper("query-hints-04a").runTest();

    final NamedSubqueriesNode namedSubqueries = astContainer.getOptimizedAST().getNamedSubqueries();
  ...
  public void test_query_hints_04b() throws Exception {
    ...
    final ASTContainer astContainer = new TestHelper("query-hints-04b").runTest();
  ...
  public void test_query_hints_05() throws Exception {
    ...
    final ASTContainer astContainer = new TestHelper("query-hints-05").runTest();
     ...
     */
    {
...

      {
        ...
        assertEquals(
            SPOKeyOrder.PCSO.toString(), sp.getQueryHint(IPredicate.Annotations.KEY_ORDER));

        assertEquals("true", sp.getQueryHint(QueryHints.HASH_JOIN));
      }
    } // end check of the AST.
     ...
     */
    {
      ...
      // Should be one pipeline join.
      {
        ...
        assertEquals(RDFS.LABEL, ((IV<?, ?>) join.getPredicate().get(1 /* p */).get()).getValue());
      ...
      {
        @SuppressWarnings("rawtypes")
        final Iterator<HashJoinOp> jitr =
            BOpUtility.visitAll(astContainer.getQueryPlan(), HashJoinOp.class);
        ...
        assertEquals(RDF.TYPE, ((IV<?, ?>) join.getPredicate().get(1 /* p */).get()).getValue());
   ...
   * (versus a hash join).
   *
   * <p>Note: This test does NOT verify that the pipeline join actually obeys that query hint. You
   * need to verify that with a unit test of the pipeline join evaluation and check to see that the
   * right index is used for each join.
   */
  public void test_query_hints_05a() throws Exception {
     ...
     */
    {
       ...
       */
      @SuppressWarnings("rawtypes")
      final PipelineJoin[] joins =
          BOpUtility.toList(BOpUtility.visitAll(astContainer.getQueryPlan(), PipelineJoin.class))
              .toArray(new PipelineJoin[0]);
  ...
  public void test_query_hints_06() throws Exception {
    ...
    final ASTContainer astContainer = new TestHelper("query-hints-06").runTest();
    ...
    // Check the optimized AST.
    {
      ...
      final JoinGroupNode whereClause = (JoinGroupNode) queryRoot.getWhereClause();
      ...
      {
        final Iterator<SubqueryBase> it =
            BOpUtility.visitAll(astContainer.getOptimizedAST(), SubqueryBase.class);
      ...
      final NamedSubqueryInclude nsi = BOpUtility.getOnly(whereClause, NamedSubqueryInclude.class);

      // Extract the SPs. There should be TWO (2).
      final List<StatementPatternNode> sps =
          BOpUtility.toList(queryRoot, StatementPatternNode.class);

      // Only 2 left after we remove the query hints.
      assertEquals(2, sps.size());

      // There is only one SP in the top-level WHERE clause.
      final StatementPatternNode queryRootSP =
          BOpUtility.getOnly(whereClause, StatementPatternNode.class);

      // There is only one SP in the named subquery WHERE clause.
      final StatementPatternNode subqueryRootSP =
          BOpUtility.getOnly(subqueryRoot.getWhereClause(), StatementPatternNode.class);
       ...
       */
      assertEquals(0, countQueryHints(astContainer.getOptimizedAST(), QueryHints.CHUNK_SIZE));
      ...
      assertNotNull(
          subqueryRoot.getProjection().getQueryHint(PipelineOp.Annotations.CHUNK_CAPACITY));
      assertNotNull(
          subqueryRoot.getWhereClause().getQueryHint(PipelineOp.Annotations.CHUNK_CAPACITY));
       ...
       */
      assertEquals(
          "5", queryRoot.getProjection().getQueryHint(PipelineOp.Annotations.CHUNK_CAPACITY));
      assertEquals(
          "5", subqueryRoot.getProjection().getQueryHint(PipelineOp.Annotations.CHUNK_CAPACITY));
     ...
     */
    {
      ...
      {
        @SuppressWarnings("rawtypes")
        final Iterator<PipelineJoin> itr = BOpUtility.visitAll(queryPlan, PipelineJoin.class);
      ...
      // Check the SliceOps.
      {
        final List<SliceOp> sliceOps = BOpUtility.toList(queryPlan, SliceOp.class);

        assertEquals(2, sliceOps.size());

        final SliceOp queryRootSliceOp =
            sliceOps.get(0).getLimit() == 20L ? sliceOps.get(0) : sliceOps.get(1);

        final SliceOp subqueryRootSliceOp =
            sliceOps.get(0).getLimit() == 10L ? sliceOps.get(0) : sliceOps.get(1);
      ...
      {
        final List<ProjectionOp> projectionOps = BOpUtility.toList(queryPlan, ProjectionOp.class);

        assertEquals(2, projectionOps.size());

        final ProjectionOp queryRootProjectionOp =
            projectionOps.get(0).getVariables().length == 2
                ? projectionOps.get(0)
                : projectionOps.get(1);

        final ProjectionOp subqueryRootProjectionOp =
            projectionOps.get(0).getVariables().length == 1
                ? projectionOps.get(0)
                : projectionOps.get(1);
      ...
      final INamedSubqueryOp namedSubqueryOp =
          BOpUtility.getOnly(queryPlan, INamedSubqueryOp.class);

      assertEquals(5, ((PipelineOp) namedSubqueryOp).getChunkCapacity());

      final SolutionSetHashJoinOp solutionSetHashJoin =
          BOpUtility.getOnly(queryPlan, SolutionSetHashJoinOp.class);
   ...
   * </pre>
   *
   * TODO We have no tests for {@link QueryHints#AT_ONCE} for anything other than {@link
   * PipelineJoin} (this test) and {@link ServiceNode} (below). Work through the code and tests
   * required to get the query hint into place for other kinds of joins as well.
   */
  public void test_query_hints_07() throws Exception {

    final ASTContainer astContainer = new TestHelper("query-hints-07").runTest();
    ...
    // Check the PipelineJoins.
    {
...

      {
        ...
        assertEquals(join.toString(), true, join.isPipelinedEvaluation());
      }

      {
        ...
        assertEquals(join.toString(), false, join.isPipelinedEvaluation());
      }
    }
  }
  ...
  public void test_query_hints_08() throws Exception {
    ...
    {
      final Iterator<ServiceCallJoin> itr = BOpUtility.visitAll(queryPlan, ServiceCallJoin.class);
