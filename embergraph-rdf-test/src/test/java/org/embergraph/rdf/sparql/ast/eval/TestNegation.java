...
public class TestNegation extends AbstractDataDrivenSPARQLTestCase {
   ...
   * </pre>
   *
   * There is only one solution to the first statement pattern. Since the MINUS group binds
   * different variables, no solutions are removed and the sole solution to the <code>?s ?p ?o
   * </code> statement pattern should be reported.
   *
   * <p>Note: Since there are no shared variables, we have to lift out the MINUS group into a named
   * subquery in order to have bottom up evaluation semantics.
   */
   ...
   * </pre>
   *
   * The variable <code>?n</code> is not bound inside of the FILTER (unless it is an exogenous
   * variable) because the right hand side of MINUS does not have visibility into the variables on
   * the left hand side of MINUS.
   */
   ...
   * </pre>
   *
   * This query demonstrated a problem where the anonymous variables created for the evaluation of
   * "exists" were not being taken into account when computing the variables to be projected out of
   * a hash join.
   *
   ...
   * out on there.
   *
   * <p>We wind up with a bad evaluation plan which does not produce any results. The basic problem
   * is that the first ASK subquery (which is in the top-level of the WHERE clause) is being run
   * BEFORE the nested {@link JoinGroupNode}. The fix was to {@link ASTJoinOrderByTypeOptimizer}
   * which now runs the ASK Subqueries after the required join groups.
   *
   * <p>Note: While that change fixes this query, it is possible that we still could get bad join
   * orderings when the variables used by the filter are only bound by OPTIONAL joins. It is also
   * possible that we could run the ASK subquery for FILTER (NOT) EXISTS earlier if the filter
   * variables are bound by required joins. This is really identical to the join filter attachment
   * problem. The problem in the AST is that both the ASK subquery and the FILTER are present. It
   * seems that the best solution would be to attach the ASK subquery to the FILTER and then to run
   * it immediately before the FILTER, letting the existing filter attachment logic decide where to
   * place the filter. We would also have to make sure that the FILTER was never attached to a JOIN
   * since the ASK subquery would have to be run before the FILTER was evaluated.
   *
   * <pre>
   * SELECT DISTINCT ?ar
   * WHERE {
  ...
  public void test_filter_not_exists2() throws Exception {
    ...
    final QueryRoot expected = new QueryRoot(QueryType.SELECT);
    {
      ...
      final EmbergraphURI refEntity = f.createURI("os:prop/analysis/refEntity");

      final EmbergraphURI Kriterion = f.createURI("os:elem/loc/Kriterion");

      final EmbergraphURI AnalysisResults = f.createURI("os:class/AnalysisResults");
      ...
      final EmbergraphValue[] values =
          new EmbergraphValue[] {refEntity, Kriterion, AnalysisResults, Artis, rdfType};
      ...
      final GlobalAnnotations globals =
          new GlobalAnnotations(store.getLexiconRelation().getNamespace(), store.getTimestamp());
      ...
      // Top-level projection.
      {
        ...
        projection.addProjectionExpression(
            new AssignmentNode(new VarNode("ar"), new VarNode("ar")));
      }
      ...
      expected.setWhereClause(whereClause);
      {
        ...
        sp1.setProperty("org.embergraph.rdf.sparql.ast.eval.AST2BOpBase.estimatedCardinality", 3L);
        sp1.setProperty(
      ...
      final SubqueryRoot notExistsSubquery2;
      {
...

        {
          ...
          projection.addProjectionExpression(
              new AssignmentNode(new VarNode("ar"), new VarNode("ar")));
          projection.addProjectionExpression(new AssignmentNode(askVar2, askVar2));
        }

        {
          ...
          final StatementPatternNode sp2 =
              new StatementPatternNode(
                  new VarNode("ar"),
                  new ConstantNode(refEntity.getIV()),
                  new ConstantNode(Kriterion.getIV()));

          whereClause2.addChild(sp2);

          sp2.setProperty(
              "org.embergraph.rdf.sparql.ast.eval.AST2BOpBase.estimatedCardinality", 1L);
          sp2.setProperty(
              "org.embergraph.rdf.sparql.ast.eval.AST2BOpBase.originalIndex", SPOKeyOrder.POCS);
      ...
      final SubqueryRoot notExistsSubquery1;
      {
         ...
         */
        {
...

          {
            ...
            projection.addProjectionExpression(
                new AssignmentNode(new VarNode("ar"), new VarNode("ar")));
            projection.addProjectionExpression(new AssignmentNode(askVar1, askVar1));
          }

          {
            ...
            final StatementPatternNode sp3 =
                new StatementPatternNode(
                    new VarNode("ar"),
                    new ConstantNode(refEntity.getIV()),
            ...
            sp3.setProperty(
                "org.embergraph.rdf.sparql.ast.eval.AST2BOpBase.estimatedCardinality", 2L);
            sp3.setProperty(
                "org.embergraph.rdf.sparql.ast.eval.AST2BOpBase.originalIndex", SPOKeyOrder.POCS);
       ...
       */
      {
        ...
        final NotExistsNode notExistsNode1 =
            new NotExistsNode(askVar1, notExistsSubquery1.getWhereClause());
      ...
      {
        @SuppressWarnings("unchecked")
        final NotExistsNode notExistsNode2 =
            new NotExistsNode(askVar2, notExistsSubquery2.getWhereClause());
