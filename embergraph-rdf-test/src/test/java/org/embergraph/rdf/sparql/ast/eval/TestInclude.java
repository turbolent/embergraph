...
/*
 * Data driven test suite for INCLUDE of named solution sets NOT generated by a {@link
 * NamedSubqueryRoot}. This test suite is examines several details, including the ability to locate
 * and join with a pre-existing named solution set, the ability to deliver the named solution set in
 * order
 *
...
public class TestInclude extends AbstractDataDrivenSPARQLTestCase {
  ...
  /*
   * Overridden to force the use of the {@link MemStore} since the solution set cache is only
   * enabled for {@link IRWStrategy} instances.
   */
  @Override
  ...
  /*
   * This test populates a named solution set and then examines the ability to deliver a SLICE of
   * that named solution set in the same order in which the data were stored. Normally, the named
   * solution set would be created using <code>INSERT INTO SOLUTIONS</code>, but in this case we
   * drop down a level and handle the creation of the named solution set in the test setup.
   *
   * <pre>
   * SELECT ?x ?y WHERE {
   *
  ...
  public void test_include_02() throws Exception {
    ...
    final QueryEngine queryEngine =
        QueryEngineFactory.getInstance().getQueryController(tripleStore.getIndexManager());
    ...
    sparqlCache.putSolutions(solutionSet, BOpUtility.asIterator(bindingSets));
  ...
  /*
   * A unit test for an INCLUDE with another JOIN. For this test, the INCLUDE will run first:
   *
   * <pre>
   * %solutionSet1::
   * {x=:Mike,  y=2}
   ...
   * </pre>
   *
   * Note: This excercises the code path in {@link AST2BOpUtility} where we do a SCAN on the named
   * solution set for the INCLUDE and then join with the access path.
   *
   * @see #test_include_03()
  ...
  /*
   * A unit test for an INCLUDE which is NOT the first JOIN in the WHERE clause. This condition is
   * enforced by turning off the join order optimizer for this query.
   *
   * <p>Note: Since there is another JOIN in this query, there is no longer any order guarantee for
  ...
  public void test_include_03() throws Exception {
     ...
     * The plan should be:
     *
     * <p>1. A PipelineJoin for the initial triple pattern
     *
     * <p>2. A HashIndexOp to generate an appropriate index for the join with the solution set. (The
     * main point of this test is to verify that we build an appropriate hash index to do the join
     * rather than using a NestedLoopJoinOp.)
     *
     * <p>3. A SolutionSetHashJoin
     *
     * <p>4. A ProjectionOp.
     */
    //        org.embergraph.bop.solutions.ProjectionOp[6](JVMSolutionSetHashJoinOp[5])[
    // org.embergraph.bop.BOp.bopId=6, org.embergraph.bop.BOp.evaluationContext=CONTROLLER,
    // org.embergraph.bop.PipelineOp.sharedState=true,
    // org.embergraph.bop.join.JoinAnnotations.select=[x, y],
    // org.embergraph.bop.engine.QueryEngine.queryId=562dbadb-afcc-4a2c-bf70-2486f1061dc3]
    //                org.embergraph.bop.join.JVMSolutionSetHashJoinOp[5](JVMHashIndexOp[4])[
    // org.embergraph.bop.BOp.bopId=5, org.embergraph.bop.BOp.evaluationContext=CONTROLLER,
    // org.embergraph.bop.PipelineOp.sharedState=true,
    // namedSetRef=NamedSolutionSetRef{queryId=562dbadb-afcc-4a2c-bf70-2486f1061dc3,namedSet=%solutionSet1,joinVars=[x]}, org.embergraph.bop.join.JoinAnnotations.constraints=null, class org.embergraph.bop.join.SolutionSetHashJoinOp.release=false]
    //                  org.embergraph.bop.join.JVMHashIndexOp[4](PipelineJoin[3])[
    // org.embergraph.bop.BOp.bopId=4, org.embergraph.bop.BOp.evaluationContext=CONTROLLER,
    // org.embergraph.bop.PipelineOp.maxParallel=1, org.embergraph.bop.PipelineOp.lastPass=true,
    // org.embergraph.bop.PipelineOp.sharedState=true,
    // org.embergraph.bop.join.JoinAnnotations.joinType=Normal,
    // org.embergraph.bop.join.HashJoinAnnotations.joinVars=[x],
    // org.embergraph.bop.join.JoinAnnotations.select=null,
    // namedSetSourceRef=NamedSolutionSetRef{queryId=562dbadb-afcc-4a2c-bf70-2486f1061dc3,namedSet=%solutionSet1,joinVars=[]}, namedSetRef=NamedSolutionSetRef{queryId=562dbadb-afcc-4a2c-bf70-2486f1061dc3,namedSet=%solutionSet1,joinVars=[x]}]
    //                    org.embergraph.bop.join.PipelineJoin[3]()[ org.embergraph.bop.BOp.bopId=3,
    // org.embergraph.bop.join.JoinAnnotations.constraints=null,
    // org.embergraph.bop.BOp.evaluationContext=ANY,
    // org.embergraph.bop.join.AccessPathJoinAnnotations.predicate=SPOPredicate[1]]
  ...
  public static class IncludeStressTests extends AbstractDataDrivenSPARQLTestCase {
    ...
    /*
     * This test populates a named solution set and then examines the ability to deliver that named
     * solution set in the same order in which the data were stored. Normally, the named solution
     * set would be created using <code>INSERT INTO SOLUTIONS</code>, but in this case we drop down
     * a level and handle the creation of the named solution set in the test setup.
     *
     * <pre>
