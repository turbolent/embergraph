 ...
 *     <p>FIXME Add unit test where there are no join variables because there is nothing which must
 *     be bound in one context or the other.
 *     <p>FIXME Add unit test where there are no join variables because the named solution set is
 *     included from multiple locations without any overlap in the incoming bindings (consider doing
 *     a hash index build for each include location other than the most specific location (the one
 *     with the most possible join variables)).
 *     <p>FIXME Add unit test where there are join variables, but they are only those which overlap
 *     for the different locations at which the named solution set is joined into the query.
 */
public class TestASTNamedSubqueryOptimizer extends AbstractASTEvaluationTestCase {
   ...
   *
   * <p>Note: Whether we use a join on <code>?x</code> or whether there are NO join variables
   * depends on the order imposed on the named subquery include versus the statement pattern, which
   * depends on the expected cardinality of the named solution set and <code>?x rdfs:label ?o</code>
   * . We should only use <code>?x</code> as a join variable if the expected cardinality of <code>
   * ?x rdfs:label ?o</code> is LT the expected cardinality of the named solution set. In fact, for
   * this case it probably won't be.
   */
  ...
  public void test_govtrack_query10() {
    ...
    final QueryRoot given = new QueryRoot(QueryType.SELECT);
    {
      ...
      {
        final NamedSubqueryRoot nsr = new NamedSubqueryRoot(QueryType.SELECT, set1);
      ...
      {
        final NamedSubqueryRoot nsr = new NamedSubqueryRoot(QueryType.SELECT, set2);
        namedSubqueries.add(nsr);
      ...
      {
        final NamedSubqueryRoot nsr = new NamedSubqueryRoot(QueryType.SELECT, set3);
        namedSubqueries.add(nsr);
