...
public class TestPipelinedHashJoin extends AbstractDataDrivenSPARQLTestCase {
  ...
  public void testPipelinedHashJoinDefaultUsedOptional01Analytic() throws Exception {
    ...
    assertPipelinedPlanOrNot(queryPlan, astContainer, QueryHints.DEFAULT_PIPELINED_HASH_JOIN, true);
  }
   ...
   */
  protected void assertPipelinedPlanOrNot(
      final PipelineOp queryPlan,
      final ASTContainer container,
      final boolean assertPipelined,
    ...
    if (assertPipelined) {
      ...
      if (!BOpUtility.visitAll(queryPlan, operatorClass).hasNext()) {
        ...
        fail("Expecting " + operatorClass + " in the plan: " + container.toString());
      }

      // verify that we're properly using the analytical vs. JVM version of the class
      final Class<?> expectedUtilClass =
          analyticMode ? HTreePipelinedHashJoinUtility.class : JVMPipelinedHashJoinUtility.class;

      final PipelinedHashIndexAndSolutionSetJoinOp firstMatch =
          (PipelinedHashIndexAndSolutionSetJoinOp)
              BOpUtility.visitAll(queryPlan, operatorClass).next();

      final Object utilFactory =
       ...
       *
       * <p>observedUtilClass.toString() := class
       * org.embergraph.bop.join.JVMPipelinedHashJoinUtility$1 expectedUtilClass.toString() := class
       * org.embergraph.bop.join.JVMPipelinedHashJoinUtility
       */
      if (!observedUtilClass.toString().contains(expectedUtilClass.toString())) {
        fail(
            "Expecting util factory of type "
                + expectedUtilClass
                + ", but used one is "
                + utilFactory.getClass());
      }
      ...
      if (BOpUtility.visitAll(queryPlan, operatorClass).hasNext()) {
        ...
        fail("Expecting *no* " + operatorClass + " in the plan: " + container.toString());
      }
    }
  }
}
