 ...
 * @deprecated None of these test queries are complicated enough to trigger the RTO. The class and
 *     its queries should just be dropped.
 */
public class TestRTO_FOAF extends AbstractRTOTestCase {
  ...
  /** Data files for 3-degrees of separation starting with a crawl of TBLs foaf card. */
   ...
   * </pre>
   *
   * FIXME This example is not complex enough to run through the RTO. This may change when we begin
   * to handle OPTIONALs. However, the FILTER NOT EXISTS would also need to be handled to make this
   * work since otherwise the query remain 2 required SPs with a simple FILTER, a sub-SELECTs (for
   * the FILTER NOT EXISTS) and then two simple OPTIONALs.
   */
   ...
   * </pre>
   *
   * FIXME This example is not complex enough to run through the RTO. This might change when we
   * handle the OPTIONAL join inside of the RTO, however it would remain 2 required JOINS and an
   * OPTIONAL join and there is no reason to run that query through the RTO. The query plan will
   * always be the most selective vertex, then the other vertex, then the OPTIONAL JOIN. This is
   * fully deterministic based on inspection on the query and the range counts. The RTO is not
   * required.
   */
