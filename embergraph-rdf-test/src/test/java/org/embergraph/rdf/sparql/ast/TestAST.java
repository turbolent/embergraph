...
public class TestAST extends TestCase {
  ...
  /** select * where { predicate1 . optional { predicate2 . } } */
  ...
  /** select * where { predicate1 . optional { predicate2 . predicate3 . } } */
  ...
  /** select * where { { predicate1 . predicate2 . } union { predicate3 . predicate4 . } } */
  ...
  public void testEverything() {
    ...
    havingBy.addExpr(
        new LegacyTestValueExpressionNode(
            new CompareBOp(Var.var("x"), Var.var("y"), CompareOp.GT)));
    ...
    {
      final NamedSubqueryRoot subquery = new NamedSubqueryRoot(QueryType.SELECT, "foo");
      ...
      final TermNode nameConstant = new ConstantNode(TermId.mockIV(VTE.URI));
      final JoinGroupNode whereClause = new JoinGroupNode();
      whereClause.addChild(
          new StatementPatternNode(new VarNode("y"), nameConstant, new VarNode("name")));
  ...
  public Predicate pred(final int id) {
    return new Predicate(BOp.NOARGS, NV.asMap(new NV(BOp.Annotations.BOP_ID, id)));
  }
  ...
  private static final class Filter extends XSDBooleanIVValueExpression {
    ...
    public Filter(final int id) {
      super(BOp.NOARGS, NV.asMap(new NV(BOp.Annotations.BOP_ID, id)));
    }
