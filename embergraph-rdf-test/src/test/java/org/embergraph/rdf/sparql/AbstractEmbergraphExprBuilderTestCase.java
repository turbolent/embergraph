 ...
 */
public class AbstractEmbergraphExprBuilderTestCase extends TestCase {

  private static final Logger log = Logger.getLogger(AbstractEmbergraphExprBuilderTestCase.class);

  /** */
  public AbstractEmbergraphExprBuilderTestCase() {}

  /** @param name */
  public AbstractEmbergraphExprBuilderTestCase(String name) {
    super(name);
  }
   ...
   * EmbergraphASTContext} against which the parse tree is being interpreted.
   *
   * @param varname The exact variable name (NOT a prefix).
  ...
  protected IV<EmbergraphValue, ?> makeIV(final EmbergraphValue value) {
  ...
  /** Applies the {@link Embergraph2ASTSPARQLParser}. */
  public QueryRoot parse(final String queryStr, final String baseURI)
      throws MalformedQueryException {

    final Embergraph2ASTSPARQLParser parser = new Embergraph2ASTSPARQLParser();
  ...
  protected static void assertSameAST(
      final String queryStr, final IQueryNode expected, final IQueryNode actual) {
    ...
    if (actual instanceof UpdateRoot) {
      // ignore ASTDatasetClause annotation in actual result, as tests are not configuring expected
      // values
