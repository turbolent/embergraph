...
public class TestExplainHints extends AbstractDataDrivenSPARQLTestCase {
   ...
   *
   * <p>SELECT ?s ?type WHERE { BIND(<http://example.com/Person> AS ?type) { ?s a ?o
   * FILTER(?o=?type) } UNION { ?s a ?o FILTER(?o!=?type) } }
   *
   * @throws Exception
   */
   ...
   * BIND(URI(CONCAT(?typeBase,"Person")) AS ?type) ?s a ?o FILTER(?o=?type) } UNION {
   * BIND(URI(CONCAT(?typeBase,"Animal")) AS ?type) ?s a ?o FILTER(?o=?type) } }
   *
   * @throws Exception
   */
   ...
   * FILTER(?o=?type) } UNION { BIND(URI(CONCAT(?typeBase,"Animal")) AS ?type) ?s a ?o
   * FILTER(?o=?type) } } VALUES (?typeBase) { ("http://example.com/") }
   *
   * @throws Exception
   */
   ...
   *
   * <p>SELECT ?s ?type WHERE { BIND("http://example.com/" AS ?typeBase)
   * BIND(URI(CONCAT(?typeBase,"Person")) AS ?type) ?s a ?o FILTER(?o=?type) }
   *
   * @throws Exception
   */
   ...
   * <http://example.com/image> ?image } . ?person <http://example.com/name> ?name }
   *
   * @throws Exception
   */
   ...
   */
  private static void assertCarriesExactlyOneExplainHintOfType(
      final QueryRoot ast, final Class<?> clazz) {
