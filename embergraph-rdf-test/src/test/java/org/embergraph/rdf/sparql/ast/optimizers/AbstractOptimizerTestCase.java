...
public abstract class AbstractOptimizerTestCase extends AbstractASTEvaluationTestCase {
  ...
  public interface Annotations
      extends org.embergraph.rdf.sparql.ast.GraphPatternGroup.Annotations,
  ...
  enum HelperFlag {
     ...
     */
    public abstract void apply(ASTBase target);
   ...
   * date.
   *
   * <p>NB: Given this goal, several Java naming conventions are ignored. e.g. methods whose names
   * are ALLCAPS or the same as ClassNames
   *
   * <p>Also, note that the intent is that this class be used in anonymous subclasses with a single
   * invocation of the {@link #test()} method, and the two fields {@link #given} and {@link
   * #expected} initialized in the subclasses constructor (i.e. inside a second pair of braces).
   *
   * <p>All of the protected members are wrappers around constructors, to allow the initialization
   * of these two fields, to have a style much more like Prolog than Java.
   *
   * @author jeremycarroll
   */
  ...
  public abstract class Helper {
    ...
    /** Variables */
    protected final String w = "w", x = "x", y = "y", z = "z", s = "s", p = "p", o = "o";
     ...
     * @param namedSubQuery A named subquery that is declared to that top-level query.
     * @param where The WHERE clause of the top-level query.
     * @param flags The flags to be applied to the resulting AST.
     */
    protected QueryRoot select(
        final VarNode[] varNodes,
        final NamedSubqueryRoot namedSubQuery,
        final JoinGroupNode where,
        final HelperFlag... flags) {
     ...
     */
    protected QueryRoot select(
        final VarNode[] varNodes, final JoinGroupNode where, final HelperFlag... flags) {
    ...
    protected ProjectionNode projection(final VarNode... varNodes) {
      ...
      // Setup projection.
      final ProjectionNode projection = new ProjectionNode();
      for (VarNode varNode : varNodes) projection.addProjectionVar(varNode);
    ...
    protected ProjectionNode projection(final AssignmentNode... assignmentNodes) {
      ...
      // Setup projection.
      final ProjectionNode projection = new ProjectionNode();
      for (AssignmentNode varNode : assignmentNodes) projection.addProjectionExpression(varNode);
     ...
     */
    protected QueryRoot select(
        final ProjectionNode projection, final JoinGroupNode where, final HelperFlag... flags) {
      final QueryRoot select = new QueryRoot(QueryType.SELECT);
    ...
    protected QueryRoot select(
        final VarNode varNode, final JoinGroupNode where, final HelperFlag... flags) {

      return select(new VarNode[] {varNode}, where, flags);
    }

    protected SubqueryRoot selectSubQuery(
        final ProjectionNode projection, final JoinGroupNode where, final HelperFlag... flags) {
      final SubqueryRoot rslt = new SubqueryRoot(QueryType.SELECT);
      return select(rslt, projection, where, flags);
    }

    private <T extends QueryBase> T select(
        T select,
      ...
      // Apply helper flags.
      for (HelperFlag flag : flags) flag.apply(select);
     ...
     */
    protected SubqueryRoot ask(final VarNode varNode, final JoinGroupNode where) {
     ...
     * @param varNode The projected variable.
     * @param where The where clause.
    ...
    protected ArbitraryLengthPathNode arbitartyLengthPropertyPath(
        TermNode left, TermNode right, HelperFlag card, JoinGroupNode joinGroupNode) {
      assert leftVar != null;
      assert rightVar != null;
      ArbitraryLengthPathNode rslt =
          new ArbitraryLengthPathNode(left, right, leftVar, rightVar, PathMod.ONE_OR_MORE);
      card.apply(rslt);
    ...
    protected PropertyPathNode propertyPathNode(
        final TermNode s, String pattern, final TermNode o) {
     ...
     * reduced to a single letter a-z. e.g. "(a/b?/c)+|a|c*"
     *
     * <p>The current support is that parenthesis, alternatives and negated properties are not
     * supported.
     *
     * @param pattern
     * @return
     ...
     *
     * <ol>
     *   <li>A context node: must be first (i.e. fourth)
     *   <li>integer cardinality
     *   <li>HelperFlag's
     * </ol>
     *
     * @param s
    ...
    protected StatementPatternNode statementPatternNode(
        final TermNode s, final TermNode p, final TermNode o, final Object... more) {
     ...
     * @param statements The children (including any optional {@link HelperFlag}s).
     * @return The group join group node.
     */
    protected JoinGroupNode joinGroupNode(final TermNode context, final Object... statements) {
    ...
    protected IValueExpressionNode functionNode(
        final String uri, final ValueExpressionNode... args) {
    ...
    // * @param scalarValues
    // * One or more scalar values that are passed to the function
    protected IValueExpressionNode functionNode(final URI uri, final ValueExpressionNode... args) {
     ...
     */
    protected AssignmentNode bind(final IValueExpressionNode valueNode, final VarNode varNode) {

      return new AssignmentNode(varNode, valueNode);
    }

    /** Logical OR of two value expressions. */
    protected FunctionNode or(final ValueExpressionNode v1, final ValueExpressionNode v2) {
  ...
  protected StatementPatternNode newStatementPatternNode(
      final TermNode s, final TermNode p, final TermNode o, final long cardinality) {
   ...
   * @param o The object.
   * @param cardinality The estimated cardinality -or- <code>-1</code> to NOT associate cardinality
   *     with the triple pattern.
   * @param optional <code>true</code> iff the triple pattern should be marked as a simple OPTIONAL.
   */
  protected StatementPatternNode newStatementPatternNode(
      final TermNode s,
      final TermNode p,
