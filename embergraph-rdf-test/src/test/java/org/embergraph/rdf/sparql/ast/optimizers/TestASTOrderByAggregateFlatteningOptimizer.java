...
public class TestASTOrderByAggregateFlatteningOptimizer extends AbstractASTEvaluationTestCase {
  ...
  public void test_orderByAggregateFlatteningOptimizer_simple_case_1()
    ...
    // QueryType: SELECT
    // SELECT VarNode(o) ( org.embergraph.rdf.sparql.ast.FunctionNode(VarNode(s))[
    // FunctionNode.scalarVals=null,
    // FunctionNode.functionURI=http://www.w3.org/2006/sparql-functions#count,
    // valueExpr=org.embergraph.bop.rdf.aggregate.COUNT(s)] AS
    // VarNode(d999c13a-6aea-4c0d-ae37-cf99a1a04d46) )[excludeFromProjection]
    //  JoinGroupNode {
    //    StatementPatternNode(VarNode(s), ConstantNode(TermId(0U)[http://example/p]), VarNode(o))
    // [scope=DEFAULT_CONTEXTS]
    //  }
    // group by VarNode(o)
    // ORDER BY
    // org.embergraph.rdf.sparql.ast.OrderByExpr(VarNode(d999c13a-6aea-4c0d-ae37-cf99a1a04d46))[
    // ascending=true]
    ...
    //       Check ( org.embergraph.rdf.sparql.ast.FunctionNode(VarNode(s))[
    // FunctionNode.scalarVals=null,
    // FunctionNode.functionURI=http://www.w3.org/2006/sparql-functions#count,
    // valueExpr=org.embergraph.bop.rdf.aggregate.COUNT(s)] AS
    // VarNode(d999c13a-6aea-4c0d-ae37-cf99a1a04d46) )[excludeFromProjection]
    ...
    assertEquals(
        var2.getName(), ((Var) orderByArg.getValueExpression()).getName()); // same as in SELECT
    ...
    assertTrue(
        ((Constant) ((ConstantNode) pattern.p()).getValueExpression()).get() instanceof TermId);
    assertEquals(
        "http://example/p",
        ((TermId) ((Constant) ((ConstantNode) pattern.p()).getValueExpression()).get())
            .getValue()
            .stringValue());
  ...
  public void test_orderByAggregateFlatteningOptimizer_high_coverage_case_1()
    ...
    // Check that the rewriter has produced something like this:
    // QueryType: SELECT
    // SELECT VarNode(x) VarNode(y) ( org.embergraph.rdf.sparql.ast.FunctionNode(VarNode(z))[
    // FunctionNode.scalarVals=null,
    // FunctionNode.functionURI=http://www.w3.org/2006/sparql-functions#max,
    // valueExpr=org.embergraph.bop.rdf.aggregate.MAX(z)] AS
    // VarNode(e6501b59-cd5b-4fbf-9587-1797b79464b2) )[excludeFromProjection] (
    // org.embergraph.rdf.sparql.ast.FunctionNode(VarNode(z))[ FunctionNode.scalarVals=null,
    // FunctionNode.functionURI=http://www.w3.org/2006/sparql-functions#count,
    // valueExpr=org.embergraph.bop.rdf.aggregate.COUNT(z)] AS
    // VarNode(086758ec-366a-4b33-86da-e958744d6650) )[excludeFromProjection]
    //  JoinGroupNode {
    //    StatementPatternNode(VarNode(x), ConstantNode(TermId(0U)[http://example.org/r]),
    // VarNode(y)) [scope=DEFAULT_CONTEXTS]
    //    StatementPatternNode(VarNode(y), ConstantNode(TermId(0U)[http://example.org/q]),
    // VarNode(z)) [scope=DEFAULT_CONTEXTS]
    //  }
    // group by VarNode(x) VarNode(y)
    // ORDER BY
    // org.embergraph.rdf.sparql.ast.OrderByExpr(VarNode(e6501b59-cd5b-4fbf-9587-1797b79464b2))[
    // ascending=false] org.embergraph.rdf.sparql.ast.OrderByExpr(VarNode(x))[ ascending=true]
    // org.embergraph.rdf.sparql.ast.OrderByExpr(VarNode(086758ec-366a-4b33-86da-e958744d6650))[
    ...
    assertEquals(var3.getName(), ((Var) orderByArg1.getValueExpression()).getName());
    ...
    assertEquals("x", ((Var) orderByArg2.getValueExpression()).getName());
    ...
    assertEquals(var4.getName(), ((Var) orderByArg3.getValueExpression()).getName());
    ...
    assertEquals("y", ((Var) orderByArg4.getValueExpression()).getName());
    ...
    assertTrue(
        ((Constant) ((ConstantNode) pattern1.p()).getValueExpression()).get() instanceof TermId);
    assertEquals(
        "http://example.org/r",
        ((TermId) ((Constant) ((ConstantNode) pattern1.p()).getValueExpression()).get())
            .getValue()
            .stringValue());
    ...
    assertTrue(
        ((Constant) ((ConstantNode) pattern2.p()).getValueExpression()).get() instanceof TermId);
    assertEquals(
        "http://example.org/q",
        ((TermId) ((Constant) ((ConstantNode) pattern2.p()).getValueExpression()).get())
            .getValue()
            .stringValue());
