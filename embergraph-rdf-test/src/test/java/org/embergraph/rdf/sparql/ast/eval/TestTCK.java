...
public class TestTCK extends AbstractDataDrivenSPARQLTestCase {
   ...
   * pass, but it will still fail in the Sesame test suite until they fix the issue in their test
   * manifest.
   *
   * <p>See http://www.openrdf.org/issues/browse/SES-884 (Aggregation with an solution set as input
   * should produce an empty solution as output)
   */
   ...
   * </pre>
   *
   * Note: Because this is a badly designed left join, it is translated into a named subquery.
   * However, the translated query will fail if the "OPTIONAL" semantics are not being attached to
   * the INCLUDE.
   *
  ...
  /** This case is not a problem. It provides a contrast to {@link #test_filter_nested_2()} */
  ...
  public void test_opt_complex_2() throws Exception {
    ...
    new TestHelper(
            ...
            new String[] {"opt-complex-2-data.ttl", "opt-complex-2-graphData.ttl"}, // dataFileURL
   ...
   * </pre>
   *
   * Note: This will drag in the SPORelation on the cluster if the backchain access path for <code>
   * (?foo rdf:type rdfs:Resource)</code> is enabled. I have explicitly disabled the backchainers in
   * scale-out so that people will not observe this problem. This was done in the Sail, which is
   ...
   * </pre>
   *
   * Both this test (<code>sort-3</code>) and <code>sparql11-aggregate-group-01</code> (an
   * aggregation query, which I believe is a Sesame specific test rather than a DAWG compliance
   * test) are hanging in scale-out CI runs. I suspect that this has to do with failing to correctly
   * trigger the last pass evaluation required by both sort and aggregation.
   */
   ...
   * SERVICE. This caused the 2nd SERVICE to deliver twice as many solutions as is expected. Because
   * the hash join was not correlated through a rowId, we wind up wind a full cross product of both
   * (empty) source solutions and all solutions coming back from the 2nd service.
   *
   * <p>It is interesting that this test passes without modification when run on purely local data.
   * This suggests that the problem is not one of bottom up evaluation, but the lack of a variable
   * to correlate the result sets from the remote services.
   *
   * <p>Note: This test is a rewrite of the same query to run against purely local data. All I did
   * was comment out the SERVICE keyword and service URI.
   *
   * <pre>
   * # Test for SES 899
   *
  ...
  public void test_stressTests() throws Exception {
    ...
    for (int i = 0; i < 100; i++) {
      final TestSuite suite = new TestSuite(TCKStressTests.class.getSimpleName());
  ...
  public static class TCKStressTests extends AbstractDataDrivenSPARQLTestCase {
     ...
     * </pre>
     *
     * A FILTER inside an OPTIONAL can reference a variable bound in the required part of the
     * OPTIONAL
     *
     * @see ASTBottomUpOptimizer
     * @see ASTSimpleOptionalOptimizer
