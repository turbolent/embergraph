 ...
 *
 * <p>The following predicates all join on {@link #product}:
 *
 * <ul>
 *   <li>{@link BSBMQ5Setup#p0}
 *   <li>{@link BSBMQ5Setup#p2}
 *   <li>{@link BSBMQ5Setup#p4}
 *   <li>{@link BSBMQ5Setup#p5}
 * </ul>
 *
 * The predicates ({@link BSBMQ5Setup#p3} and {@link BSBMQ5Setup#p5}) do not directly join with any
 * of the other predicates (they do not directly share any variables). In general, a join without
 * shared variables means the cross product of the sources will be materialized and such joins
 * should be run last.
 *
 * <p>However, in this case there are two SPARQL FILTERs ( {@link BSBMQ5Setup#c1} and {@link
 * BSBMQ5Setup#c2}) which (a) use those variables ({@link BSBMQ5Setup#origProperty1} and {@link
 * BSBMQ5Setup#origProperty2}); and (b) can constrain the query. This means that running the
 * predicates without shared variables and applying the constraints before the tail of the plan can
 * in fact lead to a more efficient join path.
 */
public class BSBMQ5Setup {

  /** Note: First id assigned is zero. This matches the naming of the statement pattern nodes. */
   ...
   * represented as its own {@link FilterNode}, but I have combined them for the purposes of these
   * unit tests.
   */
  ...
  public BSBMQ5Setup(final AbstractTripleStore store) {
    ...
    productPropertyNumeric1 = valueFactory.createURI(bsbm + "productPropertyNumeric1");

    productPropertyNumeric2 = valueFactory.createURI(bsbm + "productPropertyNumeric2");
    ...
    // Cache the value expressions for both ASTs.
    {
      ...
      final AST2BOpContext context = new AST2BOpContext(new ASTContainer(queryRoot), store);

      new ASTSetValueExpressionsOptimizer()
          .optimize(context, new QueryNodeWithBindingSet(queryRoot, bsets));
    }
  } // ctor
} // Setup
