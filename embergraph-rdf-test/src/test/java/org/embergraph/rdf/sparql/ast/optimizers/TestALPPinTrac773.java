...
import static org.embergraph.rdf.sparql.ast.optimizers.AbstractOptimizerTestCase.HelperFlag.ONE_OR_MORE;
 ...
 * had surprising effects, and that the default choice of order was often poor.
 *
 * <p>With this first commit we capture the incorrect behavior.
 */
public class TestALPPinTrac773 extends AbstractOptimizerTestCase {
  ...
  private class NotNestedHelper extends Helper {
    ...
    public NotNestedHelper(
        HelperFlag zero_or_one_to_one_or_more, String sym, boolean switchOrdering) {
      String pattern = "c" + sym;

      StatementPatternNode spn1 =
          statementPatternNode(varNode(y), constantNode(c), varNode(x), 15431);
      StatementPatternNode spn2 =
    ...
    ArbitraryLengthPathNode alpp1(HelperFlag zero_or_one_to_one_or_more) {
      return arbitartyLengthPropertyPath(
          varNode(x),
          constantNode(b),
          zero_or_one_to_one_or_more,
          joinGroupNode(
              statementPatternNode(leftVar(), constantNode(c), rightVar(), 26), SUBGROUP_OF_ALP));
    }
    ...
    ArbitraryLengthPathNode alpp2(HelperFlag zero_or_one_to_one_or_more) {
      return arbitartyLengthPropertyPath(
          ...
          zero_or_one_to_one_or_more,
          joinGroupNode(
              statementPatternNode(leftVar(), constantNode(c), rightVar(), 3135), SUBGROUP_OF_ALP));
    }
  }
    ...
    return new ASTOptimizerList(
        ...
        new ASTRangeCountOptimizer() {
          ...
          protected void estimateCardinalities(
            ...
            if (o != null) sp.setProperty(Annotations.ESTIMATED_CARDINALITY, 26l);
            else sp.setProperty(Annotations.ESTIMATED_CARDINALITY, 3135l);
          }
        },
                        ...
                        arbitartyLengthPropertyPath(
                            ...
                            joinGroupNode(
                                statementPatternNode(
                                    leftVar(), constantNode(d), rightVar(), 3135)))),
                    statementPatternNode(varNode(z), constantNode(a), varNode(w), 2054)));
      }
    }.test();
  }
