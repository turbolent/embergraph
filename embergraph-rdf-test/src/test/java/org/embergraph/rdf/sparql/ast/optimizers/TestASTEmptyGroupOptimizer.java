...
public class TestASTEmptyGroupOptimizer extends AbstractASTEvaluationTestCase {
   ...
   * </pre>
   *
   * Replace the outer {@link JoinGroupNode} with the inner {@link JoinGroupNode}.
   */
  ...
  public void test_eliminateJoinGroup01() {
    ...
    bdSearchIV.setValue(store.getValueFactory().createURI(BDS.SEARCH.toString()));
  ...
  public void test_eliminateJoinGroup03() {
    ...
    final QueryRoot given = new QueryRoot(QueryType.SELECT);
    {
      ...
      // { { {} } }
      final JoinGroupNode whereClause = new JoinGroupNode(new JoinGroupNode(new JoinGroupNode()));
      given.setWhereClause(whereClause);
    }
   ...
   *
   * <p>Note: This is <code>Filter-nested - 2</code> (Filter on variable ?v which is not in scope)
   * from the DAWG test suite. In this case, the filter can not be lifted because that change would
   * violate the bottom up semantics for this query. In general, the lifting of filters which MAY be
   * lifted should be handled by the {@link ASTLiftPreFiltersOptimizer} class. That class is
   * designed to recognize pre-filters, which are filters whose variables are all known bound on
   * entry to the group and, hence, could be run against the parent group.
   */
   ...
   * do not permit that structure) make the join group evaluation a nop in which it just enters the
   * child (the union) and evaluates that.
   *
   * <pre>
   ...
   * </pre>
   *
   * There is no change. The top-level join group is unchanged since the MINUS is not semantically
   * empty and hence can not be lifted into it.
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
   ...
   *
   * There is no change. The top-level join group is unchanged since the MINUS is not semantically
   * empty and hence can not be lifted into it. The NAMED GRAPH join group is NOT lifted since MINUS
   * is not semantically empty and is does not share the same context as the inner join group.
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
   ...
   * </pre>
   *
   * The inner MINUS group can not replace the outer MINUS. This case actually corresponds to
   * subtracting the solutions of the inner MINUS group from the solutions of the outer MINUS group
   * (an empty set).
   *
   * <p>Note: For this case, we could just wipe out both of the MINUS groups since subtracting
   * anything from an empty set yields an empty set for the MINUS groups and subtracting an empty
   * set from the parent yeilds whatever solutions are in the parent. I.e., the MINUS groups reduce
   * to a NOP. In fact, this probably is done by the {@link ASTBottomUpOptimizer} since there will
   * not be any shared variables between the first level MINUS and the outer parent group.
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
   ...
   * </pre>
   *
   * This must be evaluated as <code>G0 - (G1 - (G2))</code>. The two MINUS groups may not be
   * combined.
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
   ...
   * </pre>
   *
   * @see <a href="http://sourceforge.net/apps/trac/bigdata/ticket/504">UNION with Empty Group
   *     Pattern </a>
   */
  ...
  public void test_union_01() {
    ...
    dcTitle.setValue(store.getValueFactory().createURI(DC.TITLE.stringValue()));
    ...
    final QueryRoot given = new QueryRoot(QueryType.SELECT);
    {
      ...
      otherGroup.addChild(
          new StatementPatternNode(
              new VarNode("book"), new ConstantNode(dcTitle), new VarNode("title")));
