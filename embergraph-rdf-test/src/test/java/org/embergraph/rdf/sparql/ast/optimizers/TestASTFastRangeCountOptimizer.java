...
import static org.embergraph.rdf.sparql.ast.optimizers.AbstractOptimizerTestCase.HelperFlag.REDUCED;
  ...
  public static class TestQuadsModeAPs extends TestASTFastRangeCountOptimizer {
    ...
    public void test_fastRangeCountOptimizer_quads_mode_01() {
      ...
      new Helper() {
        {
          given =
              select(
                  projection(
                      bind(
                          functionNode(FunctionRegistry.COUNT.stringValue(), new VarNode("*")),
                          varNode(w))),
       ...
       * We need to convert:
           ...
           * </pre>
           *
           * where the triple pattern has been marked with the FAST-RANGE-COUNT:=?w annotation; and
           *
           * <p>where the ESTIMATED_CARDINALITY of the triple pattern has been set to ONE (since we
           * will not use a scan).
           *
           * <p>This means that the triple pattern will be directly interpreted as binding ?w to the
           * ESTCARD of the triple pattern.
           */
          // the triple pattern.
     ...
     *
     * OPTIONAL does not effect anything and the fast range count will still be correct so the
      ...
      new Helper() {
        {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, wildcard()), varNode(w))),
                  where(statementPatternNode(varNode(s), varNode(p), varNode(o), OPTIONAL)));
      ...
      new Helper() {
        {
          given =
              select(
                  ...
                  where(
                      statementPatternNode(
                          varNode(s), varNode(p), varNode(o), (Object[]) varNodes(z))));
      ...
      new Helper() {
        {
          given =
              select(
                  ...
                  where(
                      statementPatternNode(
                          constantNode(b), varNode(p), varNode(o), constantNode(a))));
      ...
      class WildCardHelper extends Helper {
        ...
        public WildCardHelper(HelperFlag... flags) {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, wildcard()), varNode(w))),
                  where(statementPatternNode(varNode(s), varNode(p), varNode(o))),
                  flags);
      ...
      new Helper() {
        {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, wildcard()), varNode(w))),
                  where(statementPatternNode(constantNode(a), varNode(p), varNode(o))));
      ...
      new Helper() {
        {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, wildcard()), varNode(w))),
                  where(statementPatternNode(varNode(s), constantNode(a), varNode(o))));
      ...
      new Helper() {
        {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, wildcard()), varNode(w))),
                  where(statementPatternNode(varNode(s), varNode(p), constantNode(a))));
      ...
      new Helper() {
        {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, wildcard()), varNode(w))),
                  where(statementPatternNode(constantNode(b), varNode(p), constantNode(a))));
      ...
      new Helper() {
        {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, varNode(s)), varNode(w))),
                  where(statementPatternNode(varNode(s), constantNode(a), constantNode(b))));
          ...
          given =
              select(
                  ...
                  where(
                      selectSubQuery(
                          projection(
                              bind(functionNode(FunctionRegistry.COUNT, varNode(s)), varNode(w))),
                          where(statementPatternNode(varNode(s), varNode(p), varNode(o))))));
    ...
    public void test_fastRangeCountOptimizer_triplesMode_wildcard_rejection() {
      class WildCardHelper extends Helper {
        ...
        public WildCardHelper(HelperFlag... flags) {
          given =
              select(
                  projection(bind(functionNode(FunctionRegistry.COUNT, varNode(p)), varNode(w))),
                  where(statementPatternNode(constantNode(a), varNode(p), varNode(o))),
                  flags);
