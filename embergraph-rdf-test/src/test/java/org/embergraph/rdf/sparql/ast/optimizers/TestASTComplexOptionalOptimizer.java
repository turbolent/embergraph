...
public class TestASTComplexOptionalOptimizer extends AbstractASTEvaluationTestCase {
   ...
   *     <p>TODO We need a unit test where there are some filters which should move and some which
   *     should not.
   *     <p>FIXME We should not be lifting the simple optional into the first named subquery. In is
   *     independent of the other optionals (they do not use the variable which is bound by the
   *     simple optional). Therefore it should be left in place in step (1) and then lifted out in
   *     step (2) into its own named subquery in support of a more efficient MERGE JOIN pattern.
   *     <p>FIXME The test is currently predicting var6 as projected from the lifted complex
   *     optional groups. This is because the only way that var6 makes it into the top-level
   *     projection is through inclusion of %set1 in the two lifted complex optional groups. I need
   *     to come back to the test and modify it represent the simple optional as an optional group
   *     instead and modify the code to translate both simple and complex optional groups into named
   *     subqueries. At that point the test should conform to the best known query plan.
   *     <p>FIXME Unit test with exogenousVars to make sure that they are being respected.
   */
