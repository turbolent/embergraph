...
public class TestSubQuery extends AbstractDataDrivenSPARQLTestCase {
   ...
   * </pre>
   *
   * mroycsi wrote: Based on sparql bottom up evaluation, the subquery will return s1,s2,s3 as the
   * solutions for ?s. Joined with the ?s :p ?o, you should only get the statements where ?s is
   * s1,s2,s3.
   *
   * <p>I haven't debugged embergraph so I don't know exactly what it is doing, but it seems that
   * currently with the embergraph evaluation, for each solution produced from ?s :p ?o, the
   * subquery is run, and it seems that the ?s binding in the subquery is getting constrained by the
   * ?s from the inbound solution, so results of the subquery are not always s1,s2,s3, depending on
   * the inbound solution.
   *
   * <p>thompsonbry wrote: Normally bottom up evaluation only differs when you are missing a shared
   * variable such that the bindings for variables having the same name are actually not correlated.
   *
   * <p>This is a bit of an odd case with an interaction between the order/limit and the as-bound
   * evaluation which leads to the "wrong" result. We probably do not want to always do bottom up
   * evaluation for a subquery (e.g., by lifting it into a named subquery). Are you suggesting that
   * there is this special case which needs to be recognized where the subquery MUST be evaluated
   * first because the order by/limit combination means that the results of the outer query joined
   * with the inner query could be different in this case?
   *
   * <p>mroycsi wrote: This is [a] pattern that is well known and commonly used with sparql 1.1
   * subqueries. It is definitely a case where the subquery needs to be evaluated first due to the
   * limit clause. The order clause probably doesn't matter if there isn't a limit since all the
   * results are just joined, so order doesn't matter till the solution gets to the order by
   * operations.
   *
   * <p>thompsonbry wrote: Ok. ORDER BY by itself does not matter and neither does LIMIT by itself.
   * But if you have both it matters and we need to run the subquery first.
   *
   * <p>Note: This is handled by {@link ASTSparql11SubqueryOptimizer}.
   *
   ...
   * materialization.
   *
   * <p>Note: The sub-select explicitly annotated using {@link QueryHints#RUN_ONCE} to ensure that
   * it gets lifted out as a {@link NamedSubqueryRoot}, but this query does not have any shared
  ...
  public void test_sparql11_subquery_filter_01() throws Exception {
    ...
    assertTrue(
        BOpUtility.visitAll(h.getASTContainer().getOptimizedAST(), NamedSubqueryRoot.class)
            .hasNext());
  }
   ...
   * since the {@link ASTComplexOptionalOptimizer} does not run. In the other case, the join group
   * is OPTIONAL rather than required and the problem is demonstrated.
   *
   * <pre>
   * select ?name
   * {
   ...
   * </pre>
   *
   * @see <a href="https://sourceforge.net/apps/trac/bigdata/ticket/801" > Adding OPTIONAL removes
   *     solutions</a>
   */
