...
public class TestNamedGraphs extends AbstractDataDrivenSPARQLTestCase {
   ...
   *
   * <p>Note: The correct result for this query should be NO solutions. This is one of the cases
   * covered by the DAWG/TCK graph-02 and graph-04 tests. One of those tests verifies that when the
   * data is from the default graph and the query is against the named graphs then there are no
   * solutions. The other test verifies that the opposite is also true - that when the data are from
   * the named graphs and the query is against the default graph that there are no solutions.
   */
   ...
   * access path. The filter will strip off the context information and filter for distinct (s,p,o).
   *
   * <p>Note: In this case there is a duplicate solution based on the duplicate (john,loves,mary)
   * triple which gets filtered out.
   */
   ...
   * default graph.
   *
   * <p>If you are getting an extra (john,mark,paul) solution then the default graph access path is
   * not enforcing "DISTINCT SPO" semantics.
   */
   ...
   * asserted.
   *
   * <p>Note: There is an additional occurrence of the relationship in [c4], but the query does not
   * consider that graph and that occurrence should not be matched.
   *
   * <p>Note: there is a statement duplicated in two of the named graphs (john,loves,mary) so we can
   * verify that both occurrences are reported.
   *
   * <p>Note: This query DOES NOT USE JOINS. See the next example below for one that does.
   */
   ...
   * verify that both occurrences are reported.
   *
   * <p>Note: This test case can fail if the logic is leaving [c] unbound when it should be
   * restricting [c] to the specific graphs in the FROM NAMED clauses.
   *
   * @todo query pattern involving a defaultGraph and one or more named graphs.
   */
   ...
   * an expander pattern). However, the graph variable also needs to become bound in the query
   * solutions. If we simply replace the graph variable with a constant, then the as-bound value of
   * the graph variable will not be reported in the result set.
   *
