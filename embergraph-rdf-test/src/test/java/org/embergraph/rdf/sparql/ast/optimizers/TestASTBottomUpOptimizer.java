...
public class TestASTBottomUpOptimizer extends AbstractASTEvaluationTestCase {
   ...
   * computed first and then joined against the (:x1 :p ?v) solutions.)
   *
   * <pre>
   * PREFIX : <http://example/>
   *
  ...
  public void test_bottomUpOptimizer_nested_optionals_1() throws MalformedQueryException {
    ...
    final EmbergraphURI x1 = f.createURI("http://example/x1");
    final EmbergraphURI x2 = f.createURI("http://example/x2");
    final EmbergraphURI x3 = f.createURI("http://example/x3");
    final EmbergraphURI p = f.createURI("http://example/p");
    final EmbergraphURI q = f.createURI("http://example/q");
    final EmbergraphValue[] values = new EmbergraphValue[] {x1, x2, x3, p, q};
    ...
    final NamedSubqueryRoot expectedNSR = new NamedSubqueryRoot(QueryType.SELECT, namedSet);
...

    {
      ...
      // The group from which the named subquery was lifted.
      {
        ...
        modifiedClause.addChild(
            new JoinGroupNode(true /* optional */, new NamedSubqueryInclude(namedSet)));
      }
    }

    final NamedSubqueryRoot nsr = (NamedSubqueryRoot) namedSubqueries.get(0);
  ...
  public void test_bottomUpOptimizer_nested_optionals_1_correctRejection()
    ...
    final EmbergraphURI p = f.createURI("http://example/p");
    final EmbergraphURI q = f.createURI("http://example/q");
    final EmbergraphURI c1 = f.createURI("http://example/c1");
    final EmbergraphURI c2 = f.createURI("http://example/c2");
    final EmbergraphValue[] values = new EmbergraphValue[] {x1, x2, x3, p, q, c1, c2};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
    ...
    final IBindingSet[] bindingSets =
        new IBindingSet[] {
          new ListBindingSet(new IVariable[] {v}, new IConstant[] {new Constant(c1.getIV())}),
          new ListBindingSet(new IVariable[] {v}, new IConstant[] {new Constant(c1.getIV())})
        };
  ...
  public void test_bottomUpOptimizer_filter_scope_1() throws MalformedQueryException {
     ...
     */
    final EmbergraphValueFactory f = store.getValueFactory();
    final EmbergraphURI x = f.createURI("http://example/x");
    final EmbergraphURI p = f.createURI("http://example/p");
    final EmbergraphURI q = f.createURI("http://example/q");
    final EmbergraphLiteral ONE = f.createLiteral("1", XSD.INTEGER);
    final EmbergraphValue[] values = new EmbergraphValue[] {x, p, q, ONE};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
  ...
  public void test_bottomUpOptimizer_join_scope_1() throws MalformedQueryException {
     ...
     */
    final EmbergraphValueFactory f = store.getValueFactory();
    final EmbergraphURI name = f.createURI("http://example/name");
    final EmbergraphURI email = f.createURI("http://example/email");
    final EmbergraphLiteral paul = f.createLiteral("paul");
    final EmbergraphLiteral george = f.createLiteral("george");
    final EmbergraphValue[] values = new EmbergraphValue[] {name, email, paul, george};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
   ...
   *
   * <p>Note that in the query below, variable ?o1 is bound in the outer join and in the innermost
   * OPTIONAL, but not inside the UNION. This query is not well designed. Hence, the {@link
   * ASTBottomUpOptimizer} replaces the inner OPTIONAL pattern through a named subquery (which did
   * not properly work for OPTIONAL clauses nested inside UNIONs, cf. ticket #1078). The test case
   * checks for the presence of such a {@link NamedSubqueryInclude} in the rewritten query.
   *
   * @throws MalformedQueryException
   */
  public void test_illDesignedOptUnionCombo() throws MalformedQueryException {
     ...
     */
    final EmbergraphValueFactory f = store.getValueFactory();
    final EmbergraphURI p1 = f.createURI("http://example/p1");
    final EmbergraphURI p2 = f.createURI("http://example/p2");
    final EmbergraphURI p3 = f.createURI("http://example/p3");
    final EmbergraphValue[] values = new EmbergraphValue[] {p1, p2, p3};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
    ...
    final Iterator<NamedSubqueryInclude> itr =
        BOpUtility.visitAll(queryRoot.getWhereClause(), NamedSubqueryInclude.class);
   ...
   * </pre>
   *
   * This is one of the DAWG "bottom-up" evaluation semantics tests. <code>?v</code> is not bound in
   * the FILTER because it is evaluated with bottom up semantics and therefore the bindings from the
   * parent group are not visible.
   */
  ...
  public void test_bottomUpOptimizer_filter_nested_2() throws MalformedQueryException {
    ...
    final EmbergraphURI p = f.createURI("http://example/p");
    final EmbergraphLiteral ONE = f.createLiteral("1", XSD.INTEGER);
    final EmbergraphValue[] values = new EmbergraphValue[] {x, p, ONE};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
    //        x.getIV().setValue(x);
    //        p.getIV().setValue(p);
  ...
  public void test_bottomUpOptimizer_filter_nested_2_withBindings() throws MalformedQueryException {
    ...
    final EmbergraphURI x = f.createURI("http://example/x");
    final EmbergraphURI p = f.createURI("http://example/p");
    final EmbergraphValue[] values = new EmbergraphValue[] {x, p};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
   ...
   * </pre>
   *
   * Thus, we need to handle v as if it were an exogenous variable for this query, which is to say
   * that we would just execute it normally.
   *
   * <p>This case is a bit puzzling. As other TCK examples show, you can have a badly designed left
   * join pattern with <code>SELECT *</code> so this only appear to affect the visibility of <code>
   * ?v</code> in the FILTER?
   *
   * <p>[1] http://www.w3.org/TR/sparql11-query/#variableScope
   *
   * @see ASTBottomUpOptimizer
   */
  public void test_opt_filter_1() throws Exception {
    ...
    final EmbergraphValueFactory f = store.getValueFactory();
    final EmbergraphURI x = f.createURI("http://example/x");
    final EmbergraphURI y = f.createURI("http://example/y");
    final EmbergraphURI p = f.createURI("http://example/p");
    final EmbergraphURI q = f.createURI("http://example/q");
    final EmbergraphValue[] values = new EmbergraphValue[] {x, y, p, q};
    store.getLexiconRelation().addTerms(values, values.length, false /* readOnly */);
   ...
   * </pre>
   *
   * This query does not need to be rewritten for bottom up semantics, so the expected AST is the
   * same as the given AST.
   */
  public void test_minus_sharedVariables() {
    ...
    final IQueryNode actual =
        rewriter
            .optimize(null /* AST2BOpContext */, new QueryNodeWithBindingSet(given, bsets))
            .getQueryNode();
   ...
   *
   * <p>This example is from the SPARQL 1.1 LCWD.
   *
   * <pre>
   * PREFIX : <http://example/>
   * SELECT ?s ?p ?o
  ...
  public void test_minus_noSharedVariables() {
    ...
    final QueryRoot given = new QueryRoot(QueryType.SELECT);
    {
      ...
      minusGroup.addChild(
          new StatementPatternNode(new VarNode("z"), new VarNode("x"), new VarNode("y")));
    }
  ...
  //        final EmbergraphURI owlClass = f.asValue(OWL.CLASS);
  //        final EmbergraphURI endpoint1 =
  // f.createURI("http://localhost:18080/openrdf/repositories/endpoint1");
  //        final EmbergraphValue[] values = new EmbergraphValue[] { rdfType, owlClass, endpoint1 };
  ...
  //         *       JoinGroupNode {
  //         *         StatementPatternNode(VarNode(a), ConstantNode(Vocab(14)), VarNode(t1),
  // DEFAULT_CONTEXTS)
  //         *         FILTER(
  // org.embergraph.rdf.sparql.ast.FunctionNode(VarNode(t1),ConstantNode(Vocab(39)))[
  // org.embergraph.rdf.sparql.ast.FunctionNode.functionURI=http://www.w3.org/2005/xpath-functions#equal-to] )
  //         *       }
  //         *     }
  //         *     SERVICE <ConstantNode(TermId(1U))> {
  //         *       JoinGroupNode {
  //         *         StatementPatternNode(VarNode(b), ConstantNode(Vocab(14)), VarNode(t2),
  // DEFAULT_CONTEXTS)
  //         *         FILTER(
  // org.embergraph.rdf.sparql.ast.FunctionNode(VarNode(t2),ConstantNode(Vocab(39)))[
  // org.embergraph.rdf.sparql.ast.FunctionNode.functionURI=http://www.w3.org/2005/xpath-functions#equal-to] )
  //         *       }
  //         *     }
  //         *   }
