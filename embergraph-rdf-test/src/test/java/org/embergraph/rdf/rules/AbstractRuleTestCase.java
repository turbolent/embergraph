...
public abstract class AbstractRuleTestCase extends AbstractInferenceEngineTestCase {
  ...
  protected void applyRule(
      AbstractTripleStore db, IRule rule, long expectedSolutionCount, long expectedMutationCount)
      throws Exception {
   ...
   *
   * <p>Invoke as <code>applyRule( store.{rule}, ..., ... )</code>
   *
   * @param rule The rule, which must be one of those found on the triple store or otherwise
   *     configured so as to run with the triple store instance.
   * @param expectedSolutionCount The #of entailments that should be computed by the rule (tested
   *     during Query) or <code>-1</code> if the #of solutions that will be computed is not known.
   * @param expectedMutationCount The #of new elements that should have been inserted into the db by
   *     the rule.
   */
  protected void applyRule(
      ...
      try {
        ...
        while (itr.hasNext()) {
          ...
          for (ISolution solution : chunk) {
            ...
            if (log.isInfoEnabled()) log.info("#" + n + " : " + solution.toString());
          }
        }
    ...
    // run as insert.
    {
      ...
      if (log.isInfoEnabled())
        log.info(
            "Inserted " + actualMutationCount + " elements : " + rule.getName() + " :: " + rule);

      assertEquals("mutationCount(Insert)", expectedMutationCount, actualMutationCount);
    }
   ...
   */
  protected void assertSameSolutionsAnyOrder(
   ...
   *
   * @param actual The binding set.
   * @param expected An array of expected binding sets.
   * @return <code>true</code> if <i>actual</i> was matched in the expected binding set array.
   */
  private boolean findAndClear(IBindingSet actual, IBindingSet[] expected) {
    ...
    for (int i = 0; i < expected.length; i++) {
      ...
      if (actual.equals(tmp)) {
        ...
        if (log.isInfoEnabled()) log.info("Matched: index=" + i + ", bindingSet=" + actual);
