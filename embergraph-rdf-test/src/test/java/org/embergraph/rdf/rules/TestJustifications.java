 ...
 *     <p>Check for overflow of long.
 *     <p>Check when this is a longer or shorter array than the other where the two justifications
 *     have the same data in common up to the end of whichever is the shorter array.
...
public class TestJustifications extends AbstractRuleTestCase {
  ...
  public void test_writeReadRetract() {
    ...
    try {
      ...
      store.addStatements(new SPO[] {new SPO(U, A, Y, StatementEnum.Explicit)}, 1);
      ...
      final SPO expectedEntailment =
          new SPO(A, vocab.get(RDF.TYPE), vocab.get(RDF.PROPERTY), StatementEnum.Inferred);

      {
         ...
         */
        {
           ...
           */
          final SPO[] expectedTail = new SPO[] {new SPO(NULL, A, NULL, StatementEnum.Inferred)};
