 ...
 * justifications).
 *
 * @todo also compare pipeline vs nested subquery closure (but only for LTS and LDS as nested
 *     subquery is too slow for EDS and JDS - or compare to LTS nested subquery for the EDS and JDS
 *     cases).
...
public class TestCompareFullAndFastClosure extends AbstractRuleTestCase {
  ...
  public void test_compareEntailments() throws Exception {
    ...
    // String[] resource = new String[]{"/org/embergraph/rdf/rules/testOwlSameAs.rdf"};
    // String[] resource = new String[]{"/org/embergraph/rdf/rules/testOwlSameAs.rdf"};
    String[] resource = new String[] {"org/embergraph/rdf/rules/small.rdf"};
  ...
  protected void doCompareEntailments(
    ...
    try {
      ...
      {
        final LoadStats loadStats = store1.getDataLoader().loadData(resource, baseURL, format);

        // store1.getInferenceEngine().computeClosure(null);

        if (log.isInfoEnabled()) log.info("Full forward closure: " + loadStats);

        if (log.isInfoEnabled()) log.info(store1.dumpStore(store1, true, true, false, true));
      }

      {
        final LoadStats loadStats = store2.getDataLoader().loadData(resource, baseURL, format);

        // store2.getInferenceEngine().computeClosure(null);

        if (log.isInfoEnabled()) log.info("Fast forward closure: " + loadStats);

        if (log.isInfoEnabled()) log.info(store2.dumpStore(store2, true, true, false, true));
      }
      ...
      if (store1 != null) store1.__tearDownUnitTest();
      if (store2 != null) store2.__tearDownUnitTest();
