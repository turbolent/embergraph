...
public class TestTMUtility extends AbstractRuleTestCase {
  ...
  public void test_rewrite1() {
    ...
    final Program program = TMUtility.INSTANCE.mapRuleForTruthMaintenance(r, focusStore);
...

    {
      ...
      if (log.isInfoEnabled()) log.info(r0.toString());
   ...
   *
   * <p>In the first rule, the first tail will read from the focusStore while the second tail reads
   * from the fused view of the focusStore and the database.
   *
   * <p>In the second rule, the first tail will read from the fused view while the second tail will
   * read from the focusStore.
   */
