 ...
 *     data set computed without those entailments and using backward chaining to supply the
 *     entailments. The result graphs should be equals.
...
public class TestBackchainTypeResourceIterator extends AbstractRuleTestCase {
  ...
  public void test_subjectBound() {
    ...
    try {
      ...
      {
        final IAccessPath<ISPO> accessPath = store.getAccessPath(store.getIV(A), NULL, NULL);
      ...
      assertSameSPOsAnyOrder(
          store,
          new SPO[] {
            new SPO(store.getIV(A), store.getIV(RDF.TYPE), store.getIV(B), StatementEnum.Explicit),
            new SPO(
                store.getIV(A),
  ...
  public void test_noneBound() {
    ...
    try {
      ...
      final IChunkedOrderedIterator<ISPO> itr;
      {
        final IAccessPath<ISPO> accessPath = store.getAccessPath(NULL, NULL, NULL);
      ...
      assertSameSPOsAnyOrder(
          ...
          new SPO[] {
            new SPO(store.getIV(A), store.getIV(RDF.TYPE), store.getIV(B), StatementEnum.Explicit),
            new SPO(store.getIV(B), store.getIV(RDF.TYPE), store.getIV(C), StatementEnum.Explicit),
            new SPO(
                store.getIV(A),
                store.getIV(RDF.TYPE),
                store.getIV(RDFS.RESOURCE),
  ...
  public void test_otherBound_01() {
    ...
    try {
      ...
      final IChunkedOrderedIterator<ISPO> itr;
      {
        final IAccessPath<ISPO> accessPath = store.getAccessPath(NULL, NULL, store.getIV(B));
   ...
   *
   * <p>FIXME test all access paths, including where the predicate is NULL and where it is rdf:type
   * and where the object is NULL and where it is rdfs:Resource.
   */
  public void test_backchain_foo_type_resource() {
    ...
    try {
      ...
      store.addStatements(new SPO[] {new SPO(s, p, o, StatementEnum.Explicit)}, 1);

      if (log.isInfoEnabled()) {
        log.info("\n:" + store.dumpStore());
      }

      AbstractTestCase.assertSameSPOs(
          new SPO[] {
      ...
      {
        // where s is bound.
        final IAccessPath<ISPO> accessPath = store.getAccessPath(s, rdfType, rdfsResource);

        final IChunkedOrderedIterator<ISPO> itr =
            BackchainTypeResourceIterator.newInstance(
                accessPath.iterator(), accessPath, store, rdfType, rdfsResource);
      ...
      {
        // where s is unbound.
        final IAccessPath<ISPO> accessPath = store.getAccessPath(NULL, rdfType, rdfsResource);
