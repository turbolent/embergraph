...
public class TestEquals extends TestCase {
  ...
  private Literal createLiteral(
      ValueFactory f, final String label, final URI datatype, final String languageCode) {

    if (datatype == null && languageCode == null) return f.createLiteral(label);

    if (datatype == null) return f.createLiteral(label, languageCode);

    return f.createLiteral(label, datatype);
  }

  private void doLiteralTest(final String label, final URI datatype, final String languageCode) {

    final EmbergraphValueFactory vf = EmbergraphValueFactoryImpl.getInstance(getName());

    final EmbergraphValueFactory vf2 = EmbergraphValueFactoryImpl.getInstance(getName() + "2");

    final EmbergraphLiteral v1 =
        (EmbergraphLiteral) createLiteral(vf, label, datatype, languageCode);

    final EmbergraphLiteral v2 =
        (EmbergraphLiteral) createLiteral(vf, label, datatype, languageCode);

    final Literal v3 = createLiteral(new ValueFactoryImpl(), label, datatype, languageCode);

    final EmbergraphLiteral v4 =
        (EmbergraphLiteral) createLiteral(vf2, label, datatype, languageCode);
