...
public class TestEmbergraphValueSerialization extends TestCase2 {
  ...
  private EmbergraphValue doDefaultJavaSerializationTest(final EmbergraphValue expected) {

    // serialize
    final byte[] data = SerializerUtil.serialize(expected);

    // deserialize
    final EmbergraphValue actual = (EmbergraphValue) SerializerUtil.deserialize(data);
  ...
  private EmbergraphValue doEmbergraphValueSerializationTest(final EmbergraphValue expected) {

    final EmbergraphValueSerializer<EmbergraphValue> ser =
        expected.getValueFactory().getValueSerializer();
