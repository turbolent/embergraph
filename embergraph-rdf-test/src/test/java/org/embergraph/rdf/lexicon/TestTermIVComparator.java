...
public class TestTermIVComparator extends TestCase2 {
  ...
  public void test_termIdComparator() {
    ...
    final EmbergraphValueFactory f =
        EmbergraphValueFactoryImpl.getInstance(getName() /*namespace*/);

    final EmbergraphValue vmin = f.createLiteral("a");
    vmin.setIV(lmin);
    final EmbergraphValue vm1 = f.createLiteral("b");
    vm1.setIV(lm1);
    final EmbergraphValue v0 = f.createLiteral("c");
    v0.setIV(l0);
    final EmbergraphValue vp1 = f.createLiteral("d");
    vp1.setIV(lp1);
    final EmbergraphValue vmax = f.createLiteral("e");
    vmax.setIV(lmax);
    ...
    if (log.isInfoEnabled()) log.info("unsorted ids  : " + Arrays.toString(actualIds));
    ...
    if (log.isInfoEnabled()) log.info("unsorted terms: " + Arrays.toString(terms));
    Arrays.sort(terms, c);
    if (log.isInfoEnabled()) log.info("sorted terms  : " + Arrays.toString(terms));
