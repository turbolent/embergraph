 ...
 * just as we represent the signed byte.
 *
 * <p>These IVs differ in that they must use a widening promotion when they expose the unsigned
 * value to callers. This is done in a manner similar to {@link KeyBuilder}. First we use a widening
 * promotion, e.g., from byte to short. Then we apply the rules to convert from an signed short into
 * an unsigned short. The unsigned short is reported to the caller. The edge case is when we must
 * promote a long. In this case, we promote it to a {@link BigInteger} and then operate on that.
 *
 * <p>These {@link IV}s also use a widening promotion when autoboxing the Java primitive value in
 * order to report it's unsigned value correctly. This the Java numeric class used as the generic
 * type parameter for {@link XSDUnsignedByte} is {@link Short} and not {@link Byte}.
 *
...
public class TestUnsignedIVs extends TestCase2 {
   ...
   * </pre>
   *
   * FIXME The guts of all this should be concentrated somewhere. The data from KeyBuilder is useful
   * but not complete (it does not show us the unsigned version of the value, which is after all
   * what we are after). There are lots of little edges for when something is interpreted as signed
   * and unsigned and especially the ordering implied by those things using different comparators.
   */
  public void test_xsd_unsignedByte() {
    ...
    final IV<?, ?>[] e = {
      MIN_VALUE, MINUS_ONE, ZERO, ONE, MAX_VALUE,
    };
