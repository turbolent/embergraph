...
public class TestTermIV extends TestCase2 {
  ...
  private void doTermIVTest(final VTE vte, final long termId) {
    ...
    assertEquals(
        AbstractIV.toFlags(vte, false /* inline */, false /* extension */, DTE.XSDBoolean),
        iv.flags());
    ...
    final EmbergraphValue value =
        EmbergraphValueFactoryImpl.getInstance(getName()).createLiteral("foo");
  ...
  public void test_TermId_URI_Counter_MINUS_ONE() {
    ...
    if (!IVUtility.PACK_TIDS) doTermIVTest(VTE.URI, -1L);
  }

  public void test_TermId_URI_Counter_MIN_VALUE() {

    if (!IVUtility.PACK_TIDS) doTermIVTest(VTE.URI, Long.MIN_VALUE);
  }
