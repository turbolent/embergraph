 ...
 */
public abstract class EmbergraphNTriplesParserTestCase extends TestCase2 {

  /*-----------*
   * Constants *
   *-----------*/

  private static String NTRIPLES_TEST_URL =
  ...
  private static final transient Logger log =
      Logger.getLogger(EmbergraphNTriplesParserTestCase.class);
  ...
  public void testNTriplesFile() throws Exception {
    ...
    InputStream in = EmbergraphNTriplesParser.class.getResourceAsStream(NTRIPLES_TEST_FILE);
    try {
  ...
  public void testNTriplesFileWithSIDS() throws Exception {
    ...
    final String fileUrl =
        EmbergraphNTriplesParser.class.getResource(NTRIPLES_TEST_FILE_WITH_SIDS).toExternalForm();

    // Note: This is a local copy.
    InputStream in =
        EmbergraphNTriplesParser.class.getResourceAsStream(NTRIPLES_TEST_FILE_WITH_SIDS);
    try {
  ...
  public void testNTriplesFileWithReification() throws Exception {
    ...
    final String fileUrl =
        EmbergraphNTriplesParser.class
            .getResource(NTRIPLES_TEST_FILE_WITH_REIFICATION)
            .toExternalForm();

    // Note: This is a local copy.
    InputStream in =
        EmbergraphNTriplesParser.class.getResourceAsStream(NTRIPLES_TEST_FILE_WITH_REIFICATION);
    try {
      turtleParser.parse(in, fileUrl);
    } catch (RDFParseException e) {
      fail("Failed to parse N-Triples test document: " + e.getMessage(), e);
