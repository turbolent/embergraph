 ...
 *     implementations as well? If not, then port tests so that they are also run against other
 *     implementations (especially the tests for reification done right).
 */
public class TestStatementBuffer extends AbstractTripleStoreTestCase {
  ...
  public void test_reificationDoneRight_disabled() {
    ...
    if (QueryHints.DEFAULT_REIFICATION_DONE_RIGHT) return;
    ...
    try {
      ...
      // ground statement.
      buffer.add(
          vf.createStatement(SAP, bought, sybase, null /* context */, StatementEnum.Explicit));

      // model of that statement (RDF reification).
      buffer.add(
          vf.createStatement(s1, RDF.SUBJECT, SAP, null /* context */, StatementEnum.Explicit));

      buffer.add(
          vf.createStatement(
              s1, RDF.PREDICATE, bought, null /* context */, StatementEnum.Explicit));

      buffer.add(
          vf.createStatement(s1, RDF.OBJECT, sybase, null /* context */, StatementEnum.Explicit));

      buffer.add(
          vf.createStatement(
              s1, RDF.TYPE, RDF.STATEMENT, null /* context */, StatementEnum.Explicit));

      // metadata statements.

      buffer.add(
          vf.createStatement(s1, dcSource, newsSybase, null /* context */, StatementEnum.Explicit));

      buffer.add(
          vf.createStatement(
              s1, dcCreated, createdDate, null /* context */, StatementEnum.Explicit));
  ...
  public void test_reificationDoneRight_enabled() {
    ...
    try {
      ...
      if (!store.isStatementIdentifiers()) {
         ...
         *
         * <pre>
         * Caused by: java.lang.IllegalArgumentException: context bound, but not quads or sids: < TermId(7B), TermId(5U), org.embergraph.rdf.internal.impl.literal.LiteralExtensionIV@25889b2f, TermId(8B) : Explicit >
         *     at org.embergraph.rdf.spo.SPOIndexWriter.call(SPOIndexWriter.java:275)
         * </pre>
         */
        return;
      }
      ...
      final EmbergraphStatement mds1 =
          vf.createStatement(s1, dcSource, newsSybase, null, StatementEnum.Explicit);

      final EmbergraphStatement mds2 =
          vf.createStatement(s1, dcCreated, createdDate, null, StatementEnum.Explicit);
