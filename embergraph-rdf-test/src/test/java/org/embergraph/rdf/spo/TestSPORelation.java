...
public class TestSPORelation extends AbstractTripleStoreTestCase {
   ...
   * choice of the access path in the absence of any data in the {@link SPORelation}.
   *
  ...
  public void test_ruleState() {
    ...
    try {
      ...
      final String relationIdentifier = store.getSPORelation().getNamespace();
       ...
       */
      {
...

        {
          ...
          // (u rdfs:subClassOf x)
          assertEquals(
              SPOKeyOrder.POS, joinNexus.getTailAccessPath(relation, asBound).getKeyOrder());
        }

        {
          final IPredicate asBound = rule.getTail(1).asBound(bindingSet);
       ...
       */
      {
        final IRule rule =
            new Rule(
                "testRule",
                // head
                new SPOPredicate(relationIdentifier, Var.var("x"), Var.var("y"), Var.var("z")),
                // tail
                new SPOPredicate[] {
                  new SPOPredicate(relationIdentifier, Var.var("x"), Var.var("y"), Var.var("z"))
                },
        ...
        // (x y z)
        assertEquals(SPOKeyOrder.SPO, joinNexus.getTailAccessPath(relation, asBound).getKeyOrder());
      }
       ...
       */
      {
...

        {
          ...
          // (1L y z)
          assertEquals(
              SPOKeyOrder.SPO, joinNexus.getTailAccessPath(relation, asBound).getKeyOrder());
        }
        {
          final IPredicate asBound = rule.getTail(1).asBound(bindingSet);
          final IRelation relation = joinNexus.getTailRelationView(asBound);
          // (x y 1L)
          assertEquals(
              SPOKeyOrder.OSP, joinNexus.getTailAccessPath(relation, asBound).getKeyOrder());
        }
      }
   ...
   * relation. Finally, the test simulates how an {@link ISolution} would be computed based on
  ...
  public void test_insertQuery() {
    ...
    try {
       ...
       */
      {
        final IEvaluationPlan plan = new DefaultEvaluationPlan2(joinNexus, rule);
       ...
       */
      {
        ...
        for (int i = 0; i < rule.getTailCount(); i++) {
          ...
          final IRelation relation = joinNexus.getTailRelationView(pred);

          final IAccessPath accessPath = joinNexus.getTailAccessPath(relation, pred);
       ...
       */
      {
        ...
        assertEquals(3, spoRelation.getAccessPath(NULL, NULL, NULL).rangeCount(true /*exact*/));
      }
       ...
       */
      {
        ...
        // (u rdf:subClassOf x)
        assertEquals(1, spoRelation.getAccessPath(rule.getTail(0)).rangeCount(false /* exact */));

        // (v rdf:type u)
        assertEquals(2, spoRelation.getAccessPath(rule.getTail(1)).rangeCount(false /* exact */));
      }
   ...
   *
   * @throws Exception
   * @todo the test is only verifying insert by range counts on access paths corresponding to the
   *     predicates in the tail of the rule. it should go further and verify the specific elements.
  ...
  public void test_runRule() throws Exception {
    ...
    try {
       ...
       */
      {
        ...
        final IBindingSet expectedBindingSet = new ListBindingSet()
       ...
       */
      {
        ...
        // (v rdf:type u)
        assertEquals(3, spoRelation.getAccessPath(rule.getTail(1)).rangeCount(false /* exact */));
      }
