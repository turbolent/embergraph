...
public class TestSPOKeyOrder extends TestCase2 {
  ...
  public void test_keyOrder_triples() {
    ...
    assertSPOCEquals(
        new SPO(S, P, O),
        SPOKeyOrder.SPO.decodeKey(SPOKeyOrder.SPO.encodeKey(keyBuilder, expected)));

    assertSPOCEquals(
        new SPO(P, O, S),
        SPOKeyOrder.SPO.decodeKey(SPOKeyOrder.POS.encodeKey(keyBuilder, expected)));
  ...
  public void test_keyOrder_quads() {
    ...
    assertSPOCEquals(
        new SPO(S, P, O, C),
        SPOKeyOrder.SPOC.decodeKey(SPOKeyOrder.SPOC.encodeKey(keyBuilder, expected)));

    assertSPOCEquals(
        new SPO(P, O, C, S),
        SPOKeyOrder.SPOC.decodeKey(SPOKeyOrder.POCS.encodeKey(keyBuilder, expected)));

    assertSPOCEquals(
        new SPO(O, C, S, P),
        SPOKeyOrder.SPOC.decodeKey(SPOKeyOrder.OCSP.encodeKey(keyBuilder, expected)));

    assertSPOCEquals(
        new SPO(C, S, P, O),
        SPOKeyOrder.SPOC.decodeKey(SPOKeyOrder.CSPO.encodeKey(keyBuilder, expected)));

    assertSPOCEquals(
        new SPO(P, C, S, O),
        SPOKeyOrder.SPOC.decodeKey(SPOKeyOrder.PCSO.encodeKey(keyBuilder, expected)));

    assertSPOCEquals(
        new SPO(S, O, P, C),
        SPOKeyOrder.SPOC.decodeKey(SPOKeyOrder.SOPC.encodeKey(keyBuilder, expected)));
  }
  ...
  public void test_getFromKey_getToKey_quads() {
    ...
    final SPOPredicate pred =
        new SPOPredicate(
            new BOp[] {Var.var("s"), new Constant<IV>(P), Var.var("o"), Var.var("c")},
    ...
    if (!BytesUtil.bytesEqual(fromKey, keyOrder.getFromKey(keyBuilder, pred))) {
      fail(
          "fromKey :: expected="
  ...
  public void test_getFromKey_getToKey_triples() {
    ...
    final SPOPredicate pred =
        new SPOPredicate(
            new BOp[] {Var.var("s"), new Constant<IV>(P), Var.var("o")},
            new NV(
  ...
  public void test_spoOnlyKeyOrder_iterator() {
    ...
    assertSameIteratorAnyOrder(
        new SPOKeyOrder[] {SPOKeyOrder.SPO}, SPOKeyOrder.spoOnlyKeyOrderIterator());
  }

  public void test_spocOnlyKeyOrder_iterator() {

    assertSameIteratorAnyOrder(
        new SPOKeyOrder[] {SPOKeyOrder.SPOC}, SPOKeyOrder.spocOnlyKeyOrderIterator());
  }
