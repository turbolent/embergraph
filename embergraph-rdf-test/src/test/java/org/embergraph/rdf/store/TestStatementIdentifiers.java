...
public class TestStatementIdentifiers extends AbstractTripleStoreTestCase {
    ...
    try {
...

      {
        final EmbergraphValue[] terms =
            new EmbergraphValue[] {
              x, y, z, A, B, C, rdfType, rdfsLabel, rdfsSubClassOf, lit1, lit2, lit3, bn1, bn2
            };
       ...
       */
      {
        ...
        try {
          ...
          for (int i = 0; i < stmts.length; i++) {
            ...
            assertEquals(
                "SID @ i=" + i, expected.getStatementIdentifier(), actual.getStatementIdentifier());
          }
  ...
  public void test_retractionOfStatementsAboutStatements2() {
    ...
    try {
...

      {
        final EmbergraphValue[] terms =
            new EmbergraphValue[] {
              x, y, z, A, B, C, rdfType, dcCreator, lit1, lit2,
            };
      ...
      SPO[] stmts3 =
          new SPO[] {
            new SPO(sid2, dcCreator.getIV(), lit2.getIV(), StatementEnum.Explicit),
          };
   ...
   * closed world assumption is Ok).
   *
   * @param store
   * @param all The expected statements (sorted into SPO order as a side-effect).
   * @param numStmts The #of statements <i>all</i>.
   */
  private void assertSameSPOArray(
      final AbstractTripleStore store, final SPO[] all, final int numStmts) {
    ...
    try {
      ...
      assertFalse(
          "iterator is willing to visit more than " + numStmts + " statements", itr.hasNext());
