...
public class TestLocalTripleStoreDestroy extends ProxyTestCase {
  ...
  public void test_destroyNoCommit() {
    ...
    try {
      ...
      assertEquals(
          new String[] {},
          getNamespaces(indexManager, lastCommitTime - 1).toArray(new String[] {}));

      assertTrue(
          store == indexManager.getResourceLocator().locate(store.getNamespace(), ITx.UNISOLATED));
      assertTrue(
          store.getLexiconRelation()
              == indexManager
                  .getResourceLocator()
                  .locate(namespaceLexiconRelation, ITx.UNISOLATED));
      assertTrue(
          store.getSPORelation()
              == indexManager.getResourceLocator().locate(namespaceSPORelation, ITx.UNISOLATED));
      assertNotNull(indexManager.getIndex(lexiconRelationIndexName, ITx.UNISOLATED));
      assertNotNull(indexManager.getIndex(primaryStatementIndexName, ITx.UNISOLATED));
      ...
      assertTrue(null == indexManager.getResourceLocator().locate(namespace, ITx.UNISOLATED));
      assertTrue(
          null
              == indexManager
                  .getResourceLocator()
                  .locate(namespaceLexiconRelation, ITx.UNISOLATED));
      assertTrue(
          null == indexManager.getResourceLocator().locate(namespaceSPORelation, ITx.UNISOLATED));

      // indicies are gone.
      assertNull(indexManager.getIndex(lexiconRelationIndexName, ITx.UNISOLATED));
      assertNull(indexManager.getIndex(primaryStatementIndexName, ITx.UNISOLATED));
      // but not at the last commit time.
      assertNotNull(indexManager.getIndex(primaryStatementIndexName, lastCommitTime));
      ...
      assertTrue(
          getNamespaces(indexManager, store.getIndexManager().getLastCommitTime()).isEmpty());
  ...
  public void test_destroyOne() {
    ...
    try {
      ...
      assertNotNull(indexManager.getResourceLocator().locate(namespace, commitTime - 1));
  ...
  private static List<String> getNamespaces(
    ...
    final Iterator<ITPS> itr = (Iterator<ITPS>) grs.rangeIterator(RelationSchema.INSTANCE);
   ...
   * </pre>
   *
   * Verify that destroying <code>kb</code> does not cause the indices for <code>kb1</code> to be
   * destroyed.
   *
  ...
  public void test_destroyTwo() {
    ...
    final String lexiconRelationIndexName1 =
        namespaceLexiconRelation1 + "." + LexiconKeyOrder.TERM2ID.getIndexName();
    final String primaryStatementIndexName1 =
        namespaceSPORelation1 + "." + SPOKeyOrder.SPO.getIndexName();
    ...
    try {
      ...
      final AbstractTripleStore kb1 =
          new LocalTripleStore(indexManager, namespace1, ITx.UNISOLATED, properties);
      kb1.create();
      ...
      // Verify post-conditions of the created KBs.
      {
         ...
         */
        assertTrue(
            kb == indexManager.getResourceLocator().locate(kb.getNamespace(), ITx.UNISOLATED));
        assertTrue(
