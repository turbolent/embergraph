 ...
 * property space, and a literal space. Those spaces can be pre-populated such that we can insert up
 * to ( #class * #prop * #value ) distinct statements if we select subjects from the class space,
 * predicates from the property space, and objects from the literal space and the class space.
 * However, we should also select objects from the class space, which produces additional distinct
 * statements. The literal space should be divided into plain literals, literals with language tags,
 * and typed literals. The space of types are the predefined XSD types plus those defined by RDFS
 * (rdf:xml).
 *
 * <p>In order to test plain RDF insert, we do not need to do anything beyond this.
 *
 * <p>In order to test RDFS insert, there needs to be some ontology. This can be introduced by
 * creating a class hierarchy from the class space and a property heirarchy from the property space.
 * Such a hierarchy could be formed by either by inserting or removing rdfs:subClassOf (or
 * rdfs:subPropertyOf) assertions from a fully connected matrix in the appropriate space.
 *
 * <p>FIXME modify to use variable length unsigned byte[] keys and the {@link
 * UnsignedByteArrayComparator} and see how that effects performance - the performance will be the
 * base line on which I can then improve. Once I have that baseline I can then go into the btree
 * code and strip out the polymorphic keys (except maybe int and long) and add in support for prefix
...
public class TestInsertRate extends AbstractTripleStoreTestCase {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // moderate
    //        int nclass = 300; // @todo at 300 this will force the journal to be extended on
    // commit.
    //        int nproperty = 20;
    //        int nliteral = 20;
    //        int litsize = 100;
  ...
  public static class XMLSchema {
     ...
     *
     * @todo [http://www.w3.org/2001/XMLSchema-datatypes] is a synonym for the same namespace....
     */
    ...
    /** The XSDatatype URI for "decimal" - used for {@link java.math.BigDecimal} */
   ...
   * @param nproperty The #of distinct properties.
   * @param nliteral The #of plain literals, the #of literals for each language type, and the #of
   *     typed literals for each datatype URI.
   * @param litsize The average size of a literal. The generated literals use a normal distribution
   *     with this as their mean length (in characters).
   */
  public void doTest(final int nclass, final int nproperty, final int nliteral, final int litsize)
      throws IOException {
  ...
  private void doTest(
    ...
    final int nliteral2 = nliteral + nliteral * tspace.length + nliteral * langSpace.length;
   ...
   * @param id A unique index used to obtain a unique value in the identified type space. Typically
   *     this is a one up identifier.
   */
  ...
  //
  //            int nclass = 200; // @todo at 300 this will force the journal to be extended on
  // commit.
  //            int nproperty = 20;
  //            int nliteral = 20;
  //            int litsize = 100;
