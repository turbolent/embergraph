...
public abstract class AbstractTestCase extends TestCase2 {
  ...
  public Properties getProperties() {
    ...
    if (m_properties == null) {
      ...
      m_properties.setProperty(AbstractClient.Options.COLLECT_PLATFORM_STATISTICS, "false");
      m_properties.setProperty(AbstractClient.Options.COLLECT_QUEUE_STATISTICS, "false");
  ...
  protected abstract AbstractTripleStore getStore(Properties properties);
  ...
  public static void assertSameSPOs(
      String msg, ISPO[] expected, IChunkedOrderedIterator<ISPO> actual) {
    ...
    while (actual.hasNext()) {
      ...
      if (i >= expected.length) {
        ...
        fail(
            msg
                + ": The iterator is willing to visit more than "
                + expected.length
                + " objects.  The next "
                + v.size()
                + " objects would be: "
   ...
   * @param actual
   */
  public static void assertSameSPOsAnyOrder(
      AbstractTripleStore store, ISPO[] expected, IChunkedOrderedIterator<ISPO> actual) {
    ...
    try {
      ...
      while (actual.hasNext()) {
        ...
        StatementEnum expectedType =
            expectedSPO.hasStatementType() ? expectedSPO.getStatementType() : null;
        StatementEnum actualType =
            actualSPO.hasStatementType() ? actualSPO.getStatementType() : null;
        if (expectedType != actualType) {
          // defer message generation until assert fails.
          assertEquals(
              "expected=" + expectedSPO + ",actual=" + actualSPO, expectedType, actualType);
        }
      ...
      if (!map.isEmpty()) {
        ...
        if (log.isInfoEnabled()) log.info("Iterator empty but still expecting: " + map.values());
  ...
  public static void assertSameStatements(
      Statement[] expected, EmbergraphStatementIterator actual) {
   ...
   */
  public static void assertSameStatements(
      final String msg, final Statement[] expected, final EmbergraphStatementIterator actual) {
   ...
   * @param store2
   */
  public static void assertLexiconIndicesConsistent(final AbstractTripleStore store) {
    ...
    final EmbergraphValueSerializer<EmbergraphValue> valSer =
        store.getValueFactory().getValueSerializer();
     ...
     */
    {
      final ITupleSerializer t2idTupleSer = t2id.getIndexMetadata().getTupleSerializer();

      final IKeyBuilder keyBuilder = id2t.getIndexMetadata().getTupleSerializer().getKeyBuilder();
      ...
      while (itr.hasNext()) {
        ...
        // Decode the Value.
        final EmbergraphValue decodedValue = valSer.deserialize(encodedValue);

        // Generate key for T2ID index.
        final byte[] term2IdKey = t2idTupleSer.serializeKey(decodedValue);
        ...
        if (encodedIV == null) {
          ...
          fail("No entry in TERMS index: v=" + decodedValue + ", iv=" + iv);
        }
     ...
     */
    {
      ...
      while (itr.hasNext()) {
        ...
        if (!iv.equals(decodedIV)) {
           ...
           */
          fail("IV: original=" + iv + ", afterRoundTrip=" + decodedIV + " for value=" + v);
        }
      }
    }
     ...
     */
    {
      ...
      assertEquals("lexicon range counts: t2id=" + rc1 + ", id2t=" + rc2, rc1, rc2);
    }
  }

  /** Validates that the same statements are found in each of the statement indices. */
  public static void assertStatementIndicesConsistent(
   ...
   * an error. Like a JOIN, we need to process a chunk at a time for efficiency and unroll the inner
   * loop (this is necessary for efficiency not only when using scale-out, but also to avoid doing a
   * whole bunch of point tests against an {@link UnisolatedReadWriteIndex}). Unlike a normal JOIN,
   * the join order is fixed - we visit the expected access path and then join against the actual
   * access path.
   *
   * @param db The database.
   * @param keyOrderExpected Scan this statement index.
   * @param keyOrderActual Verifying that each statement is also present in this index.
   * @param nerrs Used to report the #of errors.
   */
  private static void assertSameStatements(
    ...
    final SPOTupleSerializer tupleSer =
        (SPOTupleSerializer) actualIndex.getIndexMetadata().getTupleSerializer();

    /*
     * An iterator over the access path whose statements are being treated
     * as ground truth for this pass over the data.
     */
    final IChunkedOrderedIterator<ISPO> itre = db.getAccessPath(keyOrderExpected).iterator();
    ...
    try {
      ...
      while (itre.hasNext()) {
        ...
        final IResultHandler<?, ?> resultHandler =
            new IResultHandler<ResultBitBuffer, Long>() {
        ...
        actualIndex.submit(
            fromIndex, toIndex, keys, vals, BatchContainsConstructor.INSTANCE, resultHandler);
      }
   ...
   */
  protected static class StatementVerifier extends BasicRioLoader {
     ...
     * @param maxerrors The maximum #of errors before the test will abort.
     */
    public StatementVerifier(
      ...
      this.buffer =
          new AbstractArrayBuffer<Statement>(capacity, Statement.class, null /* filter */) {
    ...
    private void verifyStatements(final int n, final Statement[] a) {
      ...
      final Map<Value, EmbergraphValue> termSet = new LinkedHashMap<Value, EmbergraphValue>(n);
      {
        for (int i = 0; i < n; i++) {
          ...
          termSet.put(stmt.getSubject(), db.getValueFactory().asValue(stmt.getSubject()));

          termSet.put(stmt.getPredicate(), db.getValueFactory().asValue(stmt.getPredicate()));

          termSet.put(stmt.getObject(), db.getValueFactory().asValue(stmt.getObject()));
        }
        ...
        db.getLexiconRelation().addTerms(terms, nterms, true /* readOnly */);
        ...
        if (nunknown > 0) {
          ...
          log.warn("" + nunknown + " out of " + nterms + " terms were not found.");
        }
      }

      if (log.isInfoEnabled())
        log.info("There are " + termSet.size() + " distinct terms in the parsed statements.");
       ...
       */
      {
        ...
        final Map<IV<?, ?>, EmbergraphValue> reverseMap = db.getLexiconRelation().getTerms(ivs);
       ...
       */
      {
        ...
        for (int i = 0; i < n; i++) {
          ...
          final EmbergraphResource s =
              (EmbergraphResource) db.asValue(termSet.get(stmt.getSubject()));
          ...
          if (!ok) {
            ...
            b[n2++] = new SPO(s.getIV(), p.getIV(), o.getIV() /*, StatementType */);
          }
        }

        final IChunkedOrderedIterator<ISPO> itr = db.bulkCompleteStatements(b, n2);
        ...
        try {
          ...
          if (log.isInfoEnabled()) log.info("Verified " + i + " statements parsed from file.");
