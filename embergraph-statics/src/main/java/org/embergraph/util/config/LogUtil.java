 ...
 * themselves.
 *
 * <p>This class relies on the presence of either the <code>log4j.configuration</code> or the <code>
 * log4j.primary.configuration</code> property.
 *
 * <p>If neither of those properties is found, then this class searches the CLASSPATH for a log4j
 * configuration. While this is a change from the historical, searching the CLASSPATH is necessary
 * for webapp deployments.
 *
 * <p>This class understands files with any of the following extensions { <code>.properties</code>,
 * <code>.logging</code>, <code>.xml</code> . If neither configuration property is defined, if the
 * resource identified by the property can not be located, or if a log4j configuration resource can
 * not be located in the default location along the class path then this class will will a message
 * on <em>stderr</em>.
 *
 * <p>A watcher is setup on the log4j configuration if one is found.
 *
...
public class LogUtil {
   ...
   *
   * @return The log4j configuration resource -or- <code>null</code> if the resource was not
   *     configured properly.
   */
  static String getConfigPropertyValue() {

    final String log4jConfig = System.getProperty("log4j.primary.configuration");

    if (log4jConfig != null) return log4jConfig;

    final String log4jDefaultConfig = System.getProperty("log4j.configuration");

    if (log4jDefaultConfig != null) return log4jDefaultConfig;
  ...
  static URL getConfigPropertyValueUrl() {
    ...
    if (url == null) url = LogUtil.class.getResource("/log4j.logging");

    if (url == null) url = LogUtil.class.getResource("/log4j.xml");
  ...
  static {
    ...
    if (log4jConfig != null
        && (log4jConfig.endsWith(".properties") || log4jConfig.endsWith(".logging"))) {
