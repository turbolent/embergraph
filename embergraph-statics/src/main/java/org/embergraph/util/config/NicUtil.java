...
package org.embergraph.util.config;
...
public class NicUtil {
  ...
  private static final java.util.logging.Level WARNING = java.util.logging.Level.WARNING;
  private static final java.util.logging.Level INFO = java.util.logging.Level.INFO;
  private static final java.util.logging.Level CONFIG = java.util.logging.Level.CONFIG;
   ...
   * </code>.
   *
   * @param name <code>String</code> referencing the name of the network interface to return (for
   *     example, typical values for this parameter might be, "eth0", "eth1", "hme01", "lo", etc.,
   *     depending on how the underlying platform is configured).
   * @return an instance of <code>NetworkInterface</code> that represents the network interface
   *     corresponding to the given <code>name</code>, or <code>null</code> if there is no network
   *     interface with that name value.
   * @throws SocketException if there is an error in the underlying I/O subsystem and/or protocol.
   * @throws NullPointerException if <code>null</code> is input for <code>name</code>.
   */
   ...
   * assigned.
   *
   * @return a <code>Map</code> of key-value pairs in which the key is an instance of <code>
   *     InetAddress</code> referencing the address of one of the network interface cards installed
   *     on the current node, and the corresponding value is a <code>String</code> referencing the
   *     name of the associated network interface to which the address is assigned.
   * @throws SocketException if there is an error in the underlying I/O subsystem and/or protocol.
   */
   ...
   *
   * @param name <code>String</code> referencing the name to which the desired network interface(s)
   *     correspond.
   * @return an array whose elements are each instances of <code>NetworkInterface[]</code>, in which
   *     each such instance corresponds to the given <code>name</code>, or <code>null</code> if
   *     there is no network interface corresponding to that name value.
   *     <p>Note that if the value given for the <code>name</code> parameter is the <code>String
   *     </code> "all", then this method will return an array containing all of the network
   *     interfaces installed on the current node, regardless of each interface's name.
   * @throws SocketException if there is an error in the underlying I/O subsystem and/or protocol.
   * @throws NullPointerException if <code>null</code> is input for <code>name</code>.
   */
   ...
   *
   * <p>If this method fails to retrieve the desired value for the given network interface <code>
   * name</code> -- either because of a system error, or because a network interface with that name
   * doesn't exist -- then this method provides the following <i>optional</i> fallback strategies,
   * which will be executed in the order documented below:
   *
   * <p>
   *
   * <ul>
   *   <li>if the <code>host</code> parameter is non-<code>null</code>
   *   <li>return the <code>InetAddress</code> for the given <code>host</code> name (or IP address
   *       string value)
   * </ul>
   *
   ...
   * </ul>
   *
   * Thus, although this method gives priority to the network interface <code>name</code>, if one
   * wishes to force this method to return the <code>InetAddress</code> for a given host name rather
   * than for a network interface, then <code>null</code> (or a name value known to not exist on the
   * system) should be input for the <code>name</code> parameter. Similarly, if one wishes to force
   * this method to return the <code>InetAddress</code> of the local host, then <code>null</code>
   * should be input for both the <code>name</code> parameter and the <code>host</code> parameter.
   *
   * <p>If each of the strategies described above fail, then this method returns <code>null</code>.
   *
   * @param name <code>String</code> referencing the name of the network interface to query for the
   *     desired address.
   * @param index non-negative <code>int</code> value that indicates which IP address, from the list
   *     of IP address(es) assigned to the network interface, should be used when retrieving the
   *     <code>InetAddress</code> to return. Note that 0 is typically input for this value.
   * @param host <code>String</code> referencing the name of the host whose <code>InetAddress</code>
   *     should be returned if failure occurs for the <code>name</code> parameter.
   * @param localHost if <code>true</code>, then upon failure to retrieve a valid value for the
   *     given <code>name</code> and the given <code>host</code> (in that order), attempt to return
   *     the <code>InetAddress</code> of the local host.
   * @return the instance of <code>InetAddress</code> that represents the <code>index</code>-th IP
   *     address assigned to the network interface having the given <code>name</code>, or the given
   *     <code>host</code> name, or the local host.
   * @throws NullPointerException if <code>null</code> is input for both <code>name</code> and
   *     <code>host</code>, and <code>localHost</code> is <code>false</code>.
   * @throws IllegalArgumentException if the value input for <code>index</code> is negtive.
   * @throws IndexOutOfBoundsException if the value input for <code>index</code> is out of range;
   *     that is if the value input is greater than or equal to the number of IP address(es)
   *     assigned to the corresponding network interface.
   */
      ...
      for (int i = 0; i < interfaceAddrs.size(); i++) {
        ...
        if (inetAddr instanceof Inet4Address) {
          if (index == inet4AddrIndex) {
            ...
            utilLogger.log(Level.TRACE, "Inet4: address = " + hostAddr + ", name = " + hostName);
            return inetAddr;
          } else {
   ...
   * colons (:); e.g., <code>01:23:45:67:89:ab</code>.
   *
   * <p>If this method fails to retrieve the desired MAC address for the given network interface
   * <code>name</code> -- either because of a system error, or because a network interface with the
   * given <code>name</code> does not exist -- then <code>null</code> is returned.
   *
   * @param name <code>String</code> referencing the name of the network interface whose MAC address
   *     should be returned.
   * @return a <code>String</code>, in human-readable format, whose value is constructed from the
   *     MAC address assigned to the network interface having the given <code>name</code>; or <code>
   *     null</code> if the MAC address of the desired network interface cannot be retrieved.
   * @throws SocketException if there is an error in the underlying I/O subsystem and/or protocol.
   * @throws NullPointerException if <code>null</code> is input for <code>name</code>.
   */
...
//     * Three-argument version of <code>getInetAddress</code> that retrieves
   ...
   *
   * <p>If a non-<code>null</code> value is input for the <code>systemPropertyName</code> parameter,
   * then this method first determines if a system property with name equivalent to the given value
   * has been set and, if it has, returns the ip address of the nic whose name is equivalent to that
   * system property value; or <code>null</code> if there is no nic with the desired name installed
   * on the node.
   *
   * <p>If there is no system property whose name is the value of the <code>systemPropertyName
   * </code> parameter, and if the value "default" is input for the <code>defaultNic</code>
   * parameter, then this method will return the IPV4 based address of the first reachable nic that
   * can be found on the node; otherwise, if a non-<code>null</code> value not equal to "default" is
   * input for the the <code>defaultNic</code> parameter, then this method returns the ip address of
   * the nic corresponding to that given name; or <code>null</code> if there is no such nic name
   * installed on the node.
   *
   * <p>If, on the other hand, <code>null</code> is input for the <code>systemPropertyName</code>
   * parameter, then this method will attempt to find the desired ip address using only the value of
   * the <code>defaultNic</code>, and applying the same search criteria as described above.
   *
   * <p>Note that in all cases, if <code>true</code> is input for the <code>loopOk</code> parameter,
   * then upon failing to find a valid ip address using the specified search mechanism, this method
   * will return the <i>loop back</i> address; otherwise, <code>null</code> is returned.
   *
   * <p>This method can be called from within a configuration as well as from within program
   * control.
   *
   * @param systemPropertyName <code>String</code> value containing the name of a system property
   *     whose value is the network interface name whose ip address should be returned. May be
   *     <code>null</code>.
   * @param defaultNic <code>String</code> value containing the name of the network interface whose
   *     ip address should be returned if <code>null</code> is input for the <code>
   *     systemPropertyName</code> parameter, or if there is no system property with name equivalent
   *     the value of the <code>systemPropertyName</code> parameter.
   * @param loopbackOk if <code>true</code>, then return the <i>loop back</i> address upon failure
   *     to find a valid ip address using the search criteria specified through the <code>
   *     systemPropertyName</code> and <code>defaultNic</code> parameters.
   * @return a <code>String</code> representing an ip address associated with the current node;
   ...
   * address.
   *
   * <p>This method will always first examine addresses that are <i>not</i> the <i>loopback</i>
   * address (<i>local host</i>); returning a loopback adddress only if <code>true</code> is input
   * for the <code>loopbackOk</code> parameter, and none of the non-loopback addresses satisfy this
   * method's search criteria.
   *
   * <p>If this method fails to find any address that satisfies the above criteria, then this method
   * returns <code>null</code>.
   *
   * @param loopbackOk if <code>true</code>, then upon failure find an non-<i>loopback</i> address
   *     that satisfies this method's search criteria (an IPv4 type address and reachable), the
   *     first loopback address that is found to be reachable is returned.
   *     <p>If <code>false</code> is input for this parameter, then this method will examine only
   *     those addresses that do <i>not</i> correspond to the corresponding nic's loopback address.
   * @return a <code>String</code> value representing the first network interface address installed
   *     on the current node that is determined to be both <i>reachable</i> and an IPv4 type
   *     address; where the return value corresponds to a <i>loopback</i> address only if <code>true
   *     </code> is input for the <code>loopbackOk</code> parameter, and no non-loopback address
   *     satisfying the desired criteria can be found. If this method fails to find any address that
   *     satisfies the desired criteria, then <code>null</code> is returned.
   * @throws SocketException if there is an error in the underlying I/O subsystem and/or protocol
   *     while retrieving the the network interfaces currently installed on the node.
   * @throws IOException if a network error occurs while determining if a candidate return address
   *     is <i>reachable</i>.
   */
  public static String getDefaultIpv4Address(final boolean loopbackOk)
    ...
    // first consider the non-loopback addresses.
    {
      ...
      while (nics.hasMoreElements()) {
        ...
        final List<InterfaceAddress> interfaceAddrs = curNic.getInterfaceAddresses();
        for (InterfaceAddress interfaceAddr : interfaceAddrs) {
          ...
          if (isIpv4) {
            ...
            utilLogger.log(Level.TRACE, "default IPv4 address: " + retVal);
  ...
  private static class AddrCache<T> {
     ...
     * then the cache age is touched to defer expiration.
     *
      ...
      if (args.length == 0) {
        ...
        if (args[0].equals("getIpAddressByLocalHost")) {
          ...
          if (args.length == 2) {
            ...
            String tmpIpAddress = NicUtil.getIpAddress(args[1], 0, args[2], false);
            if (tmpIpAddress != null) ipAddress = tmpIpAddress;
          }
        } else {
          ipAddress = "NIC_UTIL_FAILURE";
          System.out.println(
              "NicUtil Failure: unexpected number of " + "arguments (" + args.length + ")");
        }
      }
    } catch (Throwable t) {
      ipAddress = "NIC_UTIL_FAILURE";
