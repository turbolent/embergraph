...
public class TestMillisecondTimestampFactory extends TestCase {
  ...
  /*
   * This test simply displays some timestamps where I observed time going backward on a Fedora core
   * 6 host using Sun JDK <code>1.6.0_03</code>. The first timestamp was the previous value assigned
   * by the factory as reported by {@link System#currentTimeMillis()}. The second timestamp was the
   * next distinct value reported by {@link System#currentTimeMillis()}. As you can see, the 2nd
   * timestamp was actually earlier than the first timestamp. This event lead to a refactor of the
   * {@link MillisecondTimestampFactory} to introduce a failback "auto-increment" mode for handling
   * precisely such bizarre events.
   *
   * <pre>
  ...
  /*
   * Test determines whether or not millisecond timestamps are always distinct from the last
   * generated timestamp (as assigned by {@link System#currentTimeMillis()}.
   *
  ...
  public void test_nextTimestamp1() {
    ...
    for (int i = 0; i < limit; i++) {
      ...
      if (timestamp == lastTimestamp) {
        ...
        System.err.println(
            "This platform can generate identical timestamps with millisecond resolution");
    ...
    System.err.println(
        "The minimum difference in milliseconds is " + minDiff + " over " + limit + " trials");
  }

  /*
   * Test verifies that timestamps are always distinct from the last generated timestamp as
   * generated by {@link MillisecondTimestampFactory#nextMillis()}.
   */
