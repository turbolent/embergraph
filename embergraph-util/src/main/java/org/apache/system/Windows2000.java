...
public final class Windows2000 implements CPUParser {
  ...
  public Windows2000() {
    ...
    try {
      ...
      {
        final Process proc = rt.exec("cmd.exe /C echo %NUMBER_OF_PROCESSORS%");
        final InputStream is = proc.getInputStream();
        BufferedReader reader = null;
        try {
          reader = new BufferedReader(new InputStreamReader(is));
          final String numProcs = reader.readLine();
          if (numProcs != null) procs = Integer.parseInt(numProcs);
        } finally {
      ...
      {
        final Process proc = rt.exec("cmd.exe /C echo %PROCESSOR_IDENTIFIER%");
        final InputStream is = proc.getInputStream();
        BufferedReader reader = null;
        try {
          reader = new BufferedReader(new InputStreamReader(is));
  ...
  /**
   * Return the cpu info for the processors (assuming symmetric multiprocessing which means that all
