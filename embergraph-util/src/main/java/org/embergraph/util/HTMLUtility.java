...
public class HTMLUtility {
   ...
   * automatically escape attribute values and PCDATA as they are inserted into the DOM instance.
   *
   * <p>The following notes are excerpted from the HTML and XML specifications
   *
   * <ul>
   *   <li>HTML: By default, SGML requires that all attribute values be delimited using either
   *       double quotation marks (ASCII decimal 34) or single quotation marks (ASCII decimal 39).
   *       Single quote marks can be included within the attribute value when the value is delimited
   *       by double quote marks, and vice versa. Authors may also use numeric character references
   *       to represent double quotes (&#34;) and single quotes (&#39;). For double quotes authors
   *       can also use the character entity reference &quot;.<br>
   ...
   *       colons (ASCII decimal 58). We recommend using quotation marks even when it is possible to
   *       eliminate them.
   *   <li>XML: The ampersand character (&) and the left angle bracket (<) may appear in their
   *       literal form only when used as markup delimiters, or within a comment, a processing
   *       instruction, or a CDATA section. If they are needed elsewhere, they must be escaped using
   *       either numeric character references or the strings "&amp;" and "&lt;" respectively. The
   *       right angle bracket (>) may be represented using the string "&gt;", and must, for
   *       compatibility, be escaped using "&gt;" or a character reference when it appears in the
   *       string "]]>" in content, when that string is not marking the end of a CDATA section.
   * </ul>
   */
