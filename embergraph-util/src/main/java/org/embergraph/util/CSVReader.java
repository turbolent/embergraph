 ...
 *
 * <p>Note: The default parsing of column values will provide {@link Long} integers and {@link
 * Double} precision floating point values rather than {@link Integer} or {@link Float}. If you want
 * to change this you need to customize the {@link Header} class since that is responsible for
 * interpreting column values.
 *
 * <p>Note: If no headers are defined (by the caller) or read from the file (by the caller), then
 * default headers named by the origin ONE column indices will be used.
 *
 * @todo replace with <a href="http://flatpack.sourceforge.net/">flatpack</a>? It uses an Apache 2
 *     license.
...
public class CSVReader implements Iterator<Map<String, Object>> {
  ...
  public static class Header {
     ...
     *
     * <p>Note that formats DO NOT require a complete match on the source test. For this reason,
     * they are arranged based on the expected ability for the format to be diagnositic. For
     * example, a date such as 4/6/2002 would be interpreted as an integer if you tested the integer
     * format first.
     *
     * @todo consider using regular expressions to select which formats to apply (or which formats
     *     to ignore).
     * @todo verify that the entire text was matched by the format in order to allow the format to
     *     be the approved interpretation of the text.
     */
  ...
  public CSVReader(final InputStream is, final String charSet) throws IOException {
  ...
  public boolean hasNext() {
    ...
    try {
      ...
      while (true) {
        ...
        if (skipBlankLines && line.trim().length() == 0) continue;

        if (skipCommentLines && line.length() > 0 && line.charAt(0) == '#') continue;
   ...
   *
   * @param line The line.
   * @return The columns. There will be one value for each column identified in the line.
  ...
  public void readHeaders() throws IOException {
    ...
    if (!hasNext()) throw new IOException("No more rows");
   ...
   * @param header The new {@link Header} definition.
   */
  public void setHeader(final int index, final Header header) {

    if (index < 0 || index > headers.length) throw new IndexOutOfBoundsException();
