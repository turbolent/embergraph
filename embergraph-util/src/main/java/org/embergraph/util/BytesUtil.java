 ...
 *
 * <p>Comparison operations that accept a starting offset and length are used when immutable keys
 * are stored in a single byte[] and an index into starting positions in that byte[] is maintained.
 *
 * <p>JNI methods are provided for unsigned byte[] comparison. However, note that the JNI methods do
 * not appear to be as fast as the pure Java methods - presumably because of the overhead of going
 * from Java to C. In order to execute using the JNI methods you MUST define the optional boolean
 * system property, e.g.,
 *
 * <pre>
 ...
 * </pre>
 *
 * See BytesUtil.c in this package for instructions on compiling the JNI methods. See {@link
 * #main(String[])} which provides a test for the JNI integration and some pointers on how to get
 * this running on your platform.
 *
...
public class BytesUtil {
  ...
  private static native int _compareBytes(int alen, byte[] a, int blen, byte[] b);

  private static native int _compareBytesWithOffsetAndLen(
      int aoff, int alen, byte[] a, int boff, int blen, byte[] b);
   ...
   * org.embergraph.btree.BytesUtil.jni=true</code> is specified, e.g., using
   *
   * <pre>
   *    java -Dorg.embergraph.btree.BytesUtil.jni=true ...
   * </pre>
   ...
   * @todo Return the index of the byte at which the difference with the sign adjusted to indicate
   *     the relative order of the data rather than the difference of the bytes at that index. The
   *     index would be negative or positive depending on which way the comparison went. See {@link
   *     CustomByteArrayFrontCodedList} for an implementation guideline.
   *     <p>Change all implementations in this class and also BytesUtil.c, which needs to be
   ...
   * @param alen The #of bytes in <i>a</i> to consider starting at <i>aoff</i>.
   * @param a A byte[].
   * @param boff The offset into <i>b</i> at which the comparison will begin.
   * @param blen The #of bytes in <i>b</i> to consider starting at <i>boff</i>.
   * @param b A byte[].
   ...
   * leaf.
   *
   * @param a A variable length unsigned byte array.
   ...
   * key.
   *
   * <p>Separator keys separate leaves and must be choosen with that purpose in mind. The simplest
   * way to choose the separator key is to just take the first key of the leaf - this is always
   * correct. However, shorter separator keys may be choosen by defining the separator key as the
   * shortest key that is less than or equal to the first key of a leaf and greater than the last
   * key of the left sibling of that leaf (that is, the key for the entry that immediately proceeds
   * the first entry on the leaf).
   *
   * <p>There are several advantages to always choosing the shortest separator key. The original
   * rationale (in "Prefix <i>B</i>-Trees" by Bayer and Unterauer) was to increase the branching
   * factors for fixed size pages. Since we use variable size serialized record, that is not an
   * issue. However, using the shortest separator keys in this implementation provides both smaller
   * serialized records for nodes and faster search since fewer bytes must be tested.
   *
   * <p>Note that this trick can not be used at higher levels in the btree - separator keys are
   * always formed based on the keys in the leaves and then propagated through the tree.
   *
   * <p>The rules are simple enough:
   *
   * <ol>
   *   <li>The separator contains all bytes in the shared prefix (if any) plus the 1st byte at which
   *       the given key differs from the prior key.
   *   <li>If the separator key would equal the given key by value then return the reference to the
   *       given key.
   * </ol>
   *
   * @param givenKey A key.
   * @param priorKey Another key that <em>proceeds</em> the <i>givenKey</i>.
   * @return The shortest key that is less than or equal to <i>givenKey</i> and greater than
   *     <i>priorKey</i>. This will be a reference to the <i>givenKey</i> iff that is also the
   *     shortest separator.
   * @see http://portal.acm.org/citation.cfm?doid=320521.320530
   * @throws IllegalArgumentException if either key is <code>null</code>.
   * @throws IllegalArgumentException if both keys are the same reference.
   */
  ...
  public static final byte[] getSeparatorKey(final byte[] givenKey, final byte[] priorKey) {
    ...
    if (givenKey == priorKey) throw new IllegalArgumentException();
   ...
   * @param base The offset of the base of the array within the buffer.
   * @param nmem The #of members in the array. When [nmem == 0], the array is empty.
   * @param key The key for the search.
   * @return index of the search key, if it is contained in <i>keys</i>; otherwise, <code>
   ...
   * using the PATH environment variable, e.g.,
   *
   * <pre>
   *   cd embergraph
   *   set PATH=%PATH%;lib
   ...
   * </pre>
   *
   * <p>In order to use the JNI library under un*x, you must specify the JNI library location
   *
   * <pre>
   ...
   *
   * @param args
   * @exception UnsatisfiedLinkError if the JNI methods can not be resolved.
   * @exception AssertionError if the JNI methods do not produce the expected answers.
   */
  public static void main(final String[] args) {
    ...
    if (0
        != BytesUtil._compareBytesWithOffsetAndLen(
            0, 3, new byte[] {1, 2, 3}, 0, 3, new byte[] {1, 2, 3})) {
   ...
   * OutputBitStream} and {@link InputBitStream}.
   *
   ...
   * @return The mask.
   * @throws IllegalArgumentException if <i>nbits</i> is LT ZERO (0).
   * @throws IllegalArgumentException if <i>nbits</i> is GT 32.
   */
  public static /* private */ int getMSBMask(final int nbits) {

    if (nbits < 0 || nbits > 32) throw new IllegalArgumentException();
   ...
   * @param nbits The #of bits already accounted for by the path from the root.
   * @return The hash value considering only the MSB <i>nbits</i> and shifted down into an
   *     <i>nbits</i> integer.
   */
   ...
   *
   * @param h The hash value.
   * @param nbits The #of LSB bits to be retained.
   ...
   * than 32 bits.
   *
   * <p>For example, given the following data and the bit range (0,2)
   *
   * <pre>
   * bit index: 01234567
   * ---------+----------
   ...
   * </pre>
   *
   * TWO (2) bits starting at bit offset ZERO (0) would be extracted and returned as a 2-bit
   * integer. For those data, the return value would be an int32 value whose binary representation
   * was <code>10</code> (with leading zeros suppressed).
   *
   * <p>Note: This method is design for use with the unsigned byte[] keys in a embergraph hash
   * index. All keys in embergraph are internally represented as unsigned byte[]s, which is why this
   * method accepts a byte[] rather than an long[] for the bits. Also, while the length of an
   * unsigned byte[] key can vary, they are never huge and an int32 value is sufficient to index
   * into the bits in the byte[]. Finally, the return value is an int because it will be used in
   * hash table designs to index into a hash table based on those bits in a hash code key which are
   * masked as relevant to that hash table. 32bits is far more than we will need to index into a
   * hash table. For an 8k page, we might expect a fan out of at most 1024 which is only 10 bits.
   *
   * @param a A byte[].
   * @param off The index of the first bit to be included.
   * @param len The number of bits to be returned in [0:32]. However, a bit length of zero will
   *     always return zero.
  ...
  public static int getBits(final byte[] a, final int off, final int len) {
    if (a == null || off < 0 || len < 0 || len > 32) throw new IllegalArgumentException();

    final int maxbits = a.length * 8;

    if (off + len > maxbits)
      throw new IllegalArgumentException(
          "off: " + off + ", len: " + len + ", a.length: " + a.length);
  ...
  public static long altGetBits64(final byte[] a, final int off, final int len) {
    ...
    if (a == null || off < 0 || len < 0 || len > 64) throw new IllegalArgumentException();
    if (len == 0) // zero length is always a zero.
    return 0;
    if (off + len > a.length * 8) throw new IllegalArgumentException();
   ...
   * was <code>10</code> (with leading zeros suppressed).
   *
   * <p>Note: This method is design for use in a embergraph hash index having native int32 keys
   * rather than unsigned byte[] keys.
   *
   * @param a An integer.
   * @param off The index of the first bit to be included.
   * @param len The number of bits to be returned in [0:32]. However, a bit length of zero will
  ...
  public static int getBits(final int a, final int off, final int len) {
    ...
    if (len < 0 || len > 32) throw new IllegalArgumentException();
    if (len == 0) // zero length is always a zero.
    return 0;
    if (off + len > 32) throw new IllegalArgumentException();
   ...
   * @throws IllegalArgumentException if there is a problem with the argument (<code>null</code>,
   *     ill-formed, etc).
   */
  public static long getByteCount(final String s) {
   ...
   * is ZERO (0), and the {@link ByteBuffer#limit()} is equal to the {@link ByteBuffer#capacity()}
   * then the backing array is returned. Otherwise, a new byte[] is allocated and the data are
   * copied into that byte[], which is then returned.
   *
   * @param b The {@link ByteBuffer}.
   * @return The byte[].
   */
  public static byte[] toArray(final ByteBuffer b) {
   ...
   *
   * <p>Under certain circumstances it is possible and may be desirable to return the backing {@link
   * ByteBuffer#array}. This behavior is enabled by <code>forceCopy := false</code>.
   *
   * <p>It is possible to return the backing byte[] when the {@link ByteBuffer} has a backing array,
   * the array offset is ZERO (0), and the {@link ByteBuffer#limit()} is equal to the {@link
   * ByteBuffer#capacity()} then the backing array is returned. Otherwise, a new byte[] must be
   * allocated, and the data are copied into that byte[], which may then be returned.
   *
   * @param b The {@link ByteBuffer}.
   * @param forceCopy When <code>false</code>, the backing array will be returned if possible.
   * @param dst A byte[] provided by the caller (optional). When non- <code>null</code> and having a
   *     length GTE {@link ByteBuffer#remaining()}, this array will be preferred to a newly
   *     allocated array.
   * @return The byte[] having the data. When <i>dst</i> is non- <code>null</code> this MAY be the
   *     caller's array. When it is the caller's array, it MAY be larger than the #of bytes actually
   *     read.
   */
  public static byte[] toArray(final ByteBuffer b, final boolean forceCopy, final byte[] dst) {
   ...
   */
  public static String toHexString(final int[] ibuf) {
   ...
   *
   * @param buf The data.
   * @param n The #of bytes to convert.
   * @return The hex string.
   */
  public static String toHexString(final byte[] buf, int n) {
   ...
   */
  public static void printHexString(final StringBuilder sb, final String hexData) {
