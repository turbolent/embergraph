 ...
 * pseudorandom order.
 *
 * <p>It is therefore particularly useful for test cases.
 *
 * <p>See: http://www.mactech.com/articles/mactech/Vol.06/06.12/SafeDissolve/index.html
 */
public class PseudoRandom {
  private final int m_mask; // = 0;
   ...
   * @param next The next number to visit in (0:range].
   */
  public PseudoRandom(final int range, final int next) {

    this(range);

    if (next < 0 || next >= range) throw new IllegalArgumentException();
  ...
  public int next() {
    if ((m_next & 1) == 1) m_next = (m_next >> 1) ^ m_mask;
    else m_next = (m_next >> 1);

    if (m_next > m_max) return next();
   ...
   * @throws IllegalStateException if the range is greater than the range specified to the
   *     constructor (ie., the generated could not produce all numbers in the given range).
   */
  public int nextInt(final int range) {
    if (range == 0) return 0;

    if (range < 0) throw new IllegalArgumentException();
    if (range > m_max) throw new IllegalStateException("Range exceeds max range of generator");
  ...
  public int next(final int prev) {
    ...
    if (prev < 0 || prev >= m_max) throw new IllegalArgumentException();
