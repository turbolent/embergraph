...
public class HybridTimestampFactory {
   ...
   * @todo Compute the maximum period after which the timestamps will overflow.
   * @todo Set the epoch when the timestamp factory is created and persist that epoch so that
   *     overflow is set the maximum #of milliseconds into the future. Note that overflow will be
   *     the point after which we can no longer generate timestamps (aka transaction identifiers)
  ...
  public long nextTimestamp() {
    ...
    if (counterBits == 0) return timestamp;
